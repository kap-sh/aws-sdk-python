"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#InvalidTypeException``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

from aws_sdk_elasticsearch_service.errors import ServiceError

if TYPE_CHECKING:
    import aws_sdk_elasticsearch_service.types.error_message


class InvalidTypeException_(TypedDict):
    message: NotRequired[
        "aws_sdk_elasticsearch_service.types.error_message.ErrorMessage"
    ]
    """<p>A description of the error.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: InvalidTypeException_) -> dict:
    out: dict = {}
    if "message" in value:
        out["message"] = value["message"]
    return out


def deserialize_json(data: dict) -> InvalidTypeException_:
    out: InvalidTypeException_ = {}  # type: ignore[typeddict-item]
    if "message" in data:
        out["message"] = data["message"]
    return out


class InvalidTypeException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticsearchservice#InvalidTypeException``."""

    code: str | None = "InvalidTypeException"

    def __init__(self, data: InvalidTypeException_):
        super().__init__(
            "client",
            is_throttling_error=False,
            is_retryable=False,
            code="InvalidTypeException",
        )
        self.data = data

    @classmethod
    def from_json(cls, data: dict) -> "InvalidTypeException":
        return cls(deserialize_json(data))
