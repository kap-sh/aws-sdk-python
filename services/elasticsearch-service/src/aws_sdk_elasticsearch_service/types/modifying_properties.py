...
if TYPE_CHECKING:
    import aws_sdk_elasticsearch_service.types.property_value_type
    import aws_sdk_elasticsearch_service.types.string


class ModifyingProperties(TypedDict):
    name: NotRequired["aws_sdk_elasticsearch_service.types.string.String"]
    ...
    pending_value: NotRequired["aws_sdk_elasticsearch_service.types.string.String"]
    """<p>The value that the property that is currently being modified will eventually have.</p>"""
    value_type: NotRequired[
        "aws_sdk_elasticsearch_service.types.property_value_type.PropertyValueType"
    ]
...
def serialize_json(value: ModifyingProperties) -> dict:
    ...
    if "value_type" in value:
        ...
        out["ValueType"] = (
            aws_sdk_elasticsearch_service.types.property_value_type.serialize_json(
                value["value_type"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ModifyingProperties:
    ...
    if "ValueType" in data:
        ...
        out["value_type"] = (
            aws_sdk_elasticsearch_service.types.property_value_type.deserialize_json(
                data["ValueType"]
            )
        )
    return out
