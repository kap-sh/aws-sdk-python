...
if TYPE_CHECKING:
    import aws_sdk_elasticsearch_service.types.backend_role
    import aws_sdk_elasticsearch_service.types.boolean
    import aws_sdk_elasticsearch_service.types.integer_class
    import aws_sdk_elasticsearch_service.types.saml_idp
...
class SAMLOptionsInput(TypedDict):
    ...
    master_user_name: NotRequired[
        "aws_sdk_elasticsearch_service.types.username.Username"
    ]
    ...
    master_backend_role: NotRequired[
        "aws_sdk_elasticsearch_service.types.backend_role.BackendRole"
    ]
    """<p>The backend role to which the SAML master user is mapped to.</p>"""
    subject_key: NotRequired["aws_sdk_elasticsearch_service.types.string.String"]
    """<p>The key to use for matching the SAML Subject attribute.</p>"""
    roles_key: NotRequired["aws_sdk_elasticsearch_service.types.string.String"]
...
def serialize_json(value: SAMLOptionsInput) -> dict:
    ...
    if "idp" in value:
        ...
        out["Idp"] = aws_sdk_elasticsearch_service.types.saml_idp.serialize_json(
            value["idp"]
        )
...
def deserialize_json(data: dict) -> SAMLOptionsInput:
    ...
    if "Idp" in data:
        ...
        out["idp"] = aws_sdk_elasticsearch_service.types.saml_idp.deserialize_json(
            data["Idp"]
        )
