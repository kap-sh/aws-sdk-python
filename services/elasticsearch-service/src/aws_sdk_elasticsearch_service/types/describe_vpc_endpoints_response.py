...
if TYPE_CHECKING:
    import aws_sdk_elasticsearch_service.types.vpc_endpoint_error_list
    import aws_sdk_elasticsearch_service.types.vpc_endpoints


class DescribeVpcEndpointsResponse(TypedDict, closed=True):
    vpc_endpoints: "aws_sdk_elasticsearch_service.types.vpc_endpoints.VpcEndpoints"
    """<p>Information about each requested VPC endpoint.</p>"""
    vpc_endpoint_errors: "aws_sdk_elasticsearch_service.types.vpc_endpoint_error_list.VpcEndpointErrorList"
...
def serialize_json(value: DescribeVpcEndpointsResponse) -> dict:
    ...
    out["VpcEndpoints"] = (
        aws_sdk_elasticsearch_service.types.vpc_endpoints.serialize_json(
            value["vpc_endpoints"]
        )
    )
    import aws_sdk_elasticsearch_service.types.vpc_endpoint_error_list
    ...
    out["VpcEndpointErrors"] = (
        aws_sdk_elasticsearch_service.types.vpc_endpoint_error_list.serialize_json(
...
def deserialize_json(data: dict) -> DescribeVpcEndpointsResponse:
    ...
    if "VpcEndpoints" in data:
        ...
        out["vpc_endpoints"] = (
            aws_sdk_elasticsearch_service.types.vpc_endpoints.deserialize_json(
                data["VpcEndpoints"]
            )
        )
    else:
    ...
    if "VpcEndpointErrors" in data:
        ...
        out["vpc_endpoint_errors"] = (
            aws_sdk_elasticsearch_service.types.vpc_endpoint_error_list.deserialize_json(
