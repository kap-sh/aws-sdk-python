"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#DescribeDomainChangeProgress``."""
...
def handle_response(
    ...
    out: aws_sdk_elasticsearch_service.types.describe_domain_change_progress_response.DescribeDomainChangeProgressResponse = aws_sdk_elasticsearch_service.types.describe_domain_change_progress_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_elasticsearch_service.types.describe_domain_change_progress_request.DescribeDomainChangeProgressRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/es/domain/{DomainName}/progress"
    url = url.replace("{DomainName}", quote(str(input_["domain_name"]), safe=""))
    params: dict[str, str] = {}
    if "change_id" in input_:
        params["changeid"] = str(input_["change_id"])
...
async def async_describe_domain_change_progress(
