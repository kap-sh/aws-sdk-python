...
class VpcEndpoint(TypedDict, closed=True):
    ...
    vpc_endpoint_owner: NotRequired[
        "capo_elasticsearch_service.types.aws_account.AWSAccount"
    ]
    """<p>The creator of the endpoint.</p>"""
    domain_arn: NotRequired["capo_elasticsearch_service.types.domain_arn.DomainArn"]
    ...
    status: NotRequired[
        "capo_elasticsearch_service.types.vpc_endpoint_status.VpcEndpointStatus"
    ]
    """<p>The current status of the endpoint.</p>"""
    endpoint: NotRequired["capo_elasticsearch_service.types.endpoint.Endpoint"]
...
def serialize_json(value: VpcEndpoint) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            capo_elasticsearch_service.types.vpc_endpoint_status.serialize_json(
                value["status"]
            )
        )
...
def deserialize_json(data: dict) -> VpcEndpoint:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            capo_elasticsearch_service.types.vpc_endpoint_status.deserialize_json(
                data["Status"]
            )
        )
    if "Endpoint" in data:
