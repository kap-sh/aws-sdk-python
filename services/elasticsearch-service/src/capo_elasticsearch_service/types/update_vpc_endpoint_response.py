...
class UpdateVpcEndpointResponse(TypedDict, closed=True):
    vpc_endpoint: "capo_elasticsearch_service.types.vpc_endpoint.VpcEndpoint"
...
def serialize_json(value: UpdateVpcEndpointResponse) -> dict:
    ...
    out["VpcEndpoint"] = capo_elasticsearch_service.types.vpc_endpoint.serialize_json(
        value["vpc_endpoint"]
    )
    return out
...
def deserialize_json(data: dict) -> UpdateVpcEndpointResponse:
    ...
    if "VpcEndpoint" in data:
        ...
        out["vpc_endpoint"] = (
            capo_elasticsearch_service.types.vpc_endpoint.deserialize_json(
                data["VpcEndpoint"]
            )
        )
    else:
