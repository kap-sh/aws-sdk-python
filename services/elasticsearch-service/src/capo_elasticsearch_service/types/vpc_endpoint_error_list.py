...
if TYPE_CHECKING:
    import capo_elasticsearch_service.types.vpc_endpoint_error

VpcEndpointErrorList: TypeAlias = list[
    "capo_elasticsearch_service.types.vpc_endpoint_error.VpcEndpointError"
]
...
def serialize_json(value: VpcEndpointErrorList) -> list:
    ...
    for item in value:
        out.append(
            capo_elasticsearch_service.types.vpc_endpoint_error.serialize_json(item)
        )
    return out
