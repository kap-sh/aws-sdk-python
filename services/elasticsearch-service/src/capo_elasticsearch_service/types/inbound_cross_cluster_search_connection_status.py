...
if TYPE_CHECKING:
    import capo_elasticsearch_service.types.cross_cluster_search_connection_status_message
    import capo_elasticsearch_service.types.inbound_cross_cluster_search_connection_status_code


class InboundCrossClusterSearchConnectionStatus(TypedDict, closed=True):
    status_code: NotRequired[
        "capo_elasticsearch_service.types.inbound_cross_cluster_search_connection_status_code.InboundCrossClusterSearchConnectionStatusCode"
    ]
    ...
    message: NotRequired[
        "capo_elasticsearch_service.types.cross_cluster_search_connection_status_message.CrossClusterSearchConnectionStatusMessage"
    ]
...
def serialize_json(value: InboundCrossClusterSearchConnectionStatus) -> dict:
    ...
    if "status_code" in value:
        ...
        out["StatusCode"] = (
            capo_elasticsearch_service.types.inbound_cross_cluster_search_connection_status_code.serialize_json(
                value["status_code"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> InboundCrossClusterSearchConnectionStatus:
    ...
    if "StatusCode" in data:
        ...
        out["status_code"] = (
            capo_elasticsearch_service.types.inbound_cross_cluster_search_connection_status_code.deserialize_json(
                data["StatusCode"]
            )
        )
    if "Message" in data:
