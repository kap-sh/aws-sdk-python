...
class EBSOptions(TypedDict, closed=True):
    ebs_enabled: NotRequired["capo_elasticsearch_service.types.boolean.Boolean"]
    """<p>Specifies whether EBS-based storage is enabled.</p>"""
    volume_type: NotRequired["capo_elasticsearch_service.types.volume_type.VolumeType"]
    ...
    iops: NotRequired["capo_elasticsearch_service.types.integer_class.IntegerClass"]
...
def serialize_json(value: EBSOptions) -> dict:
    ...
    if "volume_type" in value:
        ...
        out["VolumeType"] = capo_elasticsearch_service.types.volume_type.serialize_json(
            value["volume_type"]
        )
...
def deserialize_json(data: dict) -> EBSOptions:
    ...
    if "VolumeType" in data:
        ...
        out["volume_type"] = (
            capo_elasticsearch_service.types.volume_type.deserialize_json(
                data["VolumeType"]
            )
        )
