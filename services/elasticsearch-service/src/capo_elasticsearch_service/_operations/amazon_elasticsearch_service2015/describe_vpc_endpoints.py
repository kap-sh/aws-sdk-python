"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#DescribeVpcEndpoints``."""
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.describe_vpc_endpoints_response.DescribeVpcEndpointsResponse = capo_elasticsearch_service.types.describe_vpc_endpoints_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/es/vpcEndpoints/describe"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.describe_vpc_endpoints_request.serialize_json(
            input_
        )
    ).encode()
