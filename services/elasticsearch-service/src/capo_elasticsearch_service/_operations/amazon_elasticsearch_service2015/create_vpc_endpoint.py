"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#CreateVpcEndpoint``."""
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.create_vpc_endpoint_response.CreateVpcEndpointResponse = capo_elasticsearch_service.types.create_vpc_endpoint_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/es/vpcEndpoints"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.create_vpc_endpoint_request.serialize_json(
            input_
        )
    ).encode()
