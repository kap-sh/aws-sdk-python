"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#DescribeElasticsearchDomains``."""
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.describe_elasticsearch_domains_response.DescribeElasticsearchDomainsResponse = capo_elasticsearch_service.types.describe_elasticsearch_domains_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/es/domain-info"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.describe_elasticsearch_domains_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_describe_elasticsearch_domains(
