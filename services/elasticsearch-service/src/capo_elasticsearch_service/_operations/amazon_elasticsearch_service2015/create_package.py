"""Generated from Smithy shape ``com.amazonaws.elasticsearchservice#CreatePackage``."""
...
def handle_response(
    ...
    out: capo_elasticsearch_service.types.create_package_response.CreatePackageResponse = capo_elasticsearch_service.types.create_package_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-01-01/packages"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elasticsearch_service.types.create_package_request.serialize_json(input_)
    ).encode()
