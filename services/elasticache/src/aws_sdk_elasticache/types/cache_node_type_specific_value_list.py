...
if TYPE_CHECKING:
    import aws_sdk_elasticache.types.cache_node_type_specific_value

CacheNodeTypeSpecificValueList: TypeAlias = list[
    "aws_sdk_elasticache.types.cache_node_type_specific_value.CacheNodeTypeSpecificValue"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elasticache.types.cache_node_type_specific_value.serialize_query(
...
def deserialize_query(el: Element) -> CacheNodeTypeSpecificValueList:
    ...
    for child in el.findall("CacheNodeTypeSpecificValue"):
        out.append(
            aws_sdk_elasticache.types.cache_node_type_specific_value.deserialize_query(
                child
            )
        )
    return out
