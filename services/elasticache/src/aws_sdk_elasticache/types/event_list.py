...
EventList: TypeAlias = list["aws_sdk_elasticache.types.event.Event"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elasticache.types.event.serialize_query(
            item, pairs, f"{prefix}.Event.{n}"
        )
...
def deserialize_query(el: Element) -> EventList:
    ...
    for child in el.findall("Event"):
        out.append(aws_sdk_elasticache.types.event.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elasticache.types.event.serialize_query(item, pairs, f"{prefix}.{n}")
