...
if TYPE_CHECKING:
    import aws_sdk_elasticache.types.global_node_group

GlobalNodeGroupList: TypeAlias = list[
    "aws_sdk_elasticache.types.global_node_group.GlobalNodeGroup"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elasticache.types.global_node_group.serialize_query(
...
def deserialize_query(el: Element) -> GlobalNodeGroupList:
    ...
    for child in el.findall("GlobalNodeGroup"):
        out.append(aws_sdk_elasticache.types.global_node_group.deserialize_query(child))
    return out
