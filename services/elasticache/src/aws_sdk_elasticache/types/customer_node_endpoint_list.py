...
if TYPE_CHECKING:
    import aws_sdk_elasticache.types.customer_node_endpoint

CustomerNodeEndpointList: TypeAlias = list[
    "aws_sdk_elasticache.types.customer_node_endpoint.CustomerNodeEndpoint"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elasticache.types.customer_node_endpoint.serialize_query(
...
def deserialize_query(el: Element) -> CustomerNodeEndpointList:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elasticache.types.customer_node_endpoint.deserialize_query(child)
        )
    return out
