"""Generated from Smithy shape ``com.amazonaws.elasticache#AddTagsToResource``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("AddTagsToResourceResult")
...
def build_request(
    ...
    input_: aws_sdk_elasticache.types.add_tags_to_resource_message.AddTagsToResourceMessage,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    aws_sdk_elasticache.types.add_tags_to_resource_message.serialize_query(
        input_, pairs, ""
    )
