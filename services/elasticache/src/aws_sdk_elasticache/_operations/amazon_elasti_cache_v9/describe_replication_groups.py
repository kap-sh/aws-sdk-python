"""Generated from Smithy shape ``com.amazonaws.elasticache#DescribeReplicationGroups``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeReplicationGroupsResult")
    out: aws_sdk_elasticache.types.replication_group_message.ReplicationGroupMessage = (
        aws_sdk_elasticache.types.replication_group_message.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_elasticache.types.describe_replication_groups_message.DescribeReplicationGroupsMessage,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
...
async def async_describe_replication_groups(
