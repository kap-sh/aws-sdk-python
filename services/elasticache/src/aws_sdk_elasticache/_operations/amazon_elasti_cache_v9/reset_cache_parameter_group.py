"""Generated from Smithy shape ``com.amazonaws.elasticache#ResetCacheParameterGroup``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ResetCacheParameterGroupResult")
...
def build_request(
    ...
    input_: aws_sdk_elasticache.types.reset_cache_parameter_group_message.ResetCacheParameterGroupMessage,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "ResetCacheParameterGroup"))
    pairs.append(("Version", "2015-02-02"))
    aws_sdk_elasticache.types.reset_cache_parameter_group_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_reset_cache_parameter_group(
