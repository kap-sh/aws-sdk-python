"""Generated from Smithy shape ``com.amazonaws.elasticache#UserAlreadyExistsFault``."""
...
class UserAlreadyExistsFault_(TypedDict):
    message: NotRequired["aws_sdk_elasticache.types.exception_message.ExceptionMessage"]


# --- awsQuery ser/de ---
def serialize_query(
    value: UserAlreadyExistsFault_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.message", str(value["message"])))


def deserialize_query(el: Element) -> UserAlreadyExistsFault_:
...
class UserAlreadyExistsFault(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticache#UserAlreadyExistsFault``."""
    ...
    @classmethod
    def from_query(cls, el: Element) -> "UserAlreadyExistsFault":
