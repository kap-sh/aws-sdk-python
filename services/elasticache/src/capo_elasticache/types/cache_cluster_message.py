...
class CacheClusterMessage(TypedDict, closed=True):
    ...
    cache_clusters: NotRequired[
        "capo_elasticache.types.cache_cluster_list.CacheClusterList"
    ]
...
def serialize_query(
    ...
    if "cache_clusters" in value:
        ...
        capo_elasticache.types.cache_cluster_list.serialize_query(
            value["cache_clusters"], pairs, f"{key_prefix}CacheClusters"
        )
...
def deserialize_query(el: Element) -> CacheClusterMessage:
    ...
    if child_cache_clusters is not None:
        ...
        out["cache_clusters"] = (
            capo_elasticache.types.cache_cluster_list.deserialize_query(
