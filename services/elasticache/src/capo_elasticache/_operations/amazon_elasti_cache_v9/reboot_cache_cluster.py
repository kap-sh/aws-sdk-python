"""Generated from Smithy shape ``com.amazonaws.elasticache#RebootCacheCluster``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("RebootCacheClusterResult")
    out: capo_elasticache.types.reboot_cache_cluster_result.RebootCacheClusterResult = (
        capo_elasticache.types.reboot_cache_cluster_result.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "RebootCacheCluster"))
    pairs.append(("Version", "2015-02-02"))
    capo_elasticache.types.reboot_cache_cluster_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_reboot_cache_cluster(
