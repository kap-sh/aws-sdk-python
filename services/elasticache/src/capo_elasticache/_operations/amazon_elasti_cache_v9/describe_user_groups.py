"""Generated from Smithy shape ``com.amazonaws.elasticache#DescribeUserGroups``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeUserGroupsResult")
    out: capo_elasticache.types.describe_user_groups_result.DescribeUserGroupsResult = (
        capo_elasticache.types.describe_user_groups_result.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeUserGroups"))
    pairs.append(("Version", "2015-02-02"))
    capo_elasticache.types.describe_user_groups_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_user_groups(
