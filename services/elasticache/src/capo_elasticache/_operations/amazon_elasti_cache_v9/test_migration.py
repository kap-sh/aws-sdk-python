"""Generated from Smithy shape ``com.amazonaws.elasticache#TestMigration``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("TestMigrationResult")
    out: capo_elasticache.types.test_migration_response.TestMigrationResponse = (
        capo_elasticache.types.test_migration_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "TestMigration"))
    pairs.append(("Version", "2015-02-02"))
    capo_elasticache.types.test_migration_message.serialize_query(input_, pairs, "")
...
async def async_test_migration(
