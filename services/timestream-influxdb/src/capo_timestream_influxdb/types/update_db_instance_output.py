"""Generated from Smithy shape ``com.amazonaws.timestreaminfluxdb#UpdateDbInstanceOutput``."""
...
class UpdateDbInstanceOutput(TypedDict, closed=True):
    ...
    db_cluster_id: NotRequired[
        "capo_timestream_influxdb.types.db_cluster_id.DbClusterId"
    ]
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: UpdateDbInstanceOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["name"] = value["name"]
    out["arn"] = value["arn"]
...
def deserialize_aws_json_1_0(data: dict) -> UpdateDbInstanceOutput:
    out: UpdateDbInstanceOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("UpdateDbInstanceOutput.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("UpdateDbInstanceOutput.name required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("UpdateDbInstanceOutput.arn required")
    if "status" in data:
        import capo_timestream_influxdb.types.status
    ...
    if "vpcSubnetIds" in data:
        ...
        raise DeserializationError("UpdateDbInstanceOutput.vpc_subnet_ids required")
