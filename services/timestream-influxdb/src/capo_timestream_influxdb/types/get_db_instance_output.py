"""Generated from Smithy shape ``com.amazonaws.timestreaminfluxdb#GetDbInstanceOutput``."""
...
class GetDbInstanceOutput(TypedDict, closed=True):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: GetDbInstanceOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["name"] = value["name"]
    out["arn"] = value["arn"]
...
def deserialize_aws_json_1_0(data: dict) -> GetDbInstanceOutput:
    out: GetDbInstanceOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("GetDbInstanceOutput.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("GetDbInstanceOutput.name required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("GetDbInstanceOutput.arn required")
    if "status" in data:
        import capo_timestream_influxdb.types.status
    ...
    if "vpcSubnetIds" in data:
        ...
        raise DeserializationError("GetDbInstanceOutput.vpc_subnet_ids required")
