"""Generated from Smithy shape ``com.amazonaws.connectcases#BatchPutFieldOptions``."""
...
def handle_response(
    ...
    out: aws_sdk_connectcases.types.batch_put_field_options_response.BatchPutFieldOptionsResponse = aws_sdk_connectcases.types.batch_put_field_options_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_connectcases.types.batch_put_field_options_request.BatchPutFieldOptionsRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/domains/{domainId}/fields/{fieldId}/options"
    url = url.replace("{domainId}", quote(str(input["domain_id"]), safe=""))
    url = url.replace("{fieldId}", quote(str(input["field_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connectcases.types.batch_put_field_options_request

    body: bytes | None = json.dumps(
        aws_sdk_connectcases.types.batch_put_field_options_request.serialize_json(input)
    ).encode()
...
async def async_batch_put_field_options(
