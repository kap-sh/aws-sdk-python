"""Generated from Smithy shape ``com.amazonaws.connectcases#DeleteLayout``."""
...
def handle_response(
    ...
    out: aws_sdk_connectcases.types.delete_layout_response.DeleteLayoutResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_connectcases.types.delete_layout_request.DeleteLayoutRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{domainId}/layouts/{layoutId}"
    url = url.replace("{domainId}", quote(str(input_["domain_id"]), safe=""))
    url = url.replace("{layoutId}", quote(str(input_["layout_id"]), safe=""))
    params: dict[str, str] = {}
