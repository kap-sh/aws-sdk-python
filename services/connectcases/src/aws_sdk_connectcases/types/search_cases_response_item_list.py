...
SearchCasesResponseItemList: TypeAlias = list[
    "aws_sdk_connectcases.types.search_cases_response_item.SearchCasesResponseItem | None"
]
...
def serialize_json(value: SearchCasesResponseItemList) -> list:
    ...
    for item in value:
        if item is None:
            ...
            continue
        out.append(
            aws_sdk_connectcases.types.search_cases_response_item.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> SearchCasesResponseItemList:
    ...
    for item in data:
        if item is None:
            ...
            continue
        out.append(
            aws_sdk_connectcases.types.search_cases_response_item.deserialize_json(item)
        )
    return out
