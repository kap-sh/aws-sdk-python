...
class SearchCasesRequest(TypedDict):
    ...
    filter: NotRequired["aws_sdk_connectcases.types.case_filter.CaseFilter"]
    """<p>A list of filter objects.</p>"""
    sorts: NotRequired["aws_sdk_connectcases.types.sort_list.SortList"]
...
def serialize_json(value: SearchCasesRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = aws_sdk_connectcases.types.case_filter.serialize_json(
            value["filter"]
        )
    if "sorts" in value:
        import aws_sdk_connectcases.types.sort_list

        out["sorts"] = aws_sdk_connectcases.types.sort_list.serialize_json(
            value["sorts"]
        )
    if "fields" in value:
        import aws_sdk_connectcases.types.field_identifier_list
...
def deserialize_json(data: dict) -> SearchCasesRequest:
    ...
    if "filter" in data:
        ...
        out["filter"] = aws_sdk_connectcases.types.case_filter.deserialize_json(
            data["filter"]
        )
    if "sorts" in data:
        import aws_sdk_connectcases.types.sort_list

        out["sorts"] = aws_sdk_connectcases.types.sort_list.deserialize_json(
            data["sorts"]
        )
    if "fields" in data:
        import aws_sdk_connectcases.types.field_identifier_list
