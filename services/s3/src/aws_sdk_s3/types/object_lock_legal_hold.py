...
class ObjectLockLegalHold(TypedDict):
    status: NotRequired[
...
def serialize_xml(value: ObjectLockLegalHold, parent: Element, tag: str) -> None:
    ...
    if "status" in value:
        ...
        aws_sdk_s3.types.object_lock_legal_hold_status.serialize_xml(
...
def deserialize_xml(el: Element) -> ObjectLockLegalHold:
    ...
    if child_status is not None:
        ...
        out["status"] = aws_sdk_s3.types.object_lock_legal_hold_status.deserialize_xml(
            child_status
        )
    return out
