"""Generated from Smithy shape ``com.amazonaws.s3#TargetGrants``."""

from typing import TYPE_CHECKING, TypeAlias
from aws_sdk_s3._protocol.xml import Element, SubElement

if TYPE_CHECKING:
    import aws_sdk_s3.types.target_grant

TargetGrants: TypeAlias = list["aws_sdk_s3.types.target_grant.TargetGrant"]


# --- restXml ser/de ---
def serialize_xml(value: TargetGrants, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    for item in value:
        import aws_sdk_s3.types.target_grant

        aws_sdk_s3.types.target_grant.serialize_xml(item, el, "Grant")


def deserialize_xml(el: Element) -> TargetGrants:
    import aws_sdk_s3.types.target_grant

    out: TargetGrants = []
    for child in el.findall("Grant"):
        out.append(aws_sdk_s3.types.target_grant.deserialize_xml(child))
    return out


def serialize_xml_flat(value: TargetGrants, parent: Element, tag: str) -> None:
    """Variant used by parent structures with ``@xmlFlattened`` on the referencing member. Items emitted directly under ``parent``."""
    for item in value:
        import aws_sdk_s3.types.target_grant

        aws_sdk_s3.types.target_grant.serialize_xml(item, parent, tag)


def deserialize_xml_flat(parent: Element, tag: str) -> TargetGrants:
    import aws_sdk_s3.types.target_grant

    out: TargetGrants = []
    for child in parent.findall(tag):
        out.append(aws_sdk_s3.types.target_grant.deserialize_xml(child))
    return out
