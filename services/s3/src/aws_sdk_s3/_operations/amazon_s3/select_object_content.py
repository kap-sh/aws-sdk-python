...
from aws_sdk_s3._protocol.eventstream import (
    MessageDecoder,
...
def handle_response(
    ...
    _union_deser = (
        aws_sdk_s3.types.select_object_content_event_stream.deserialize_event_xml
    )
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_s3.types.select_object_content_output.SelectObjectContentOutput = {
        "payload": cast(
...
def build_request(
    ...
    input_: aws_sdk_s3.types.select_object_content_request.SelectObjectContentRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
