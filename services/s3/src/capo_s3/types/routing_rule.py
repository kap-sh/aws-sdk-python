...
if TYPE_CHECKING:
    import capo_s3.types.condition
    import capo_s3.types.redirect


class RoutingRule(TypedDict, closed=True):
    condition: NotRequired["capo_s3.types.condition.Condition"]
    ...
    redirect: "capo_s3.types.redirect.Redirect"
...
def serialize_xml(value: RoutingRule, parent: Element, tag: str) -> None:
    ...
    if "condition" in value:
        ...
        capo_s3.types.condition.serialize_xml(value["condition"], el, "Condition")
    import capo_s3.types.redirect

    capo_s3.types.redirect.serialize_xml(value["redirect"], el, "Redirect")
...
def deserialize_xml(el: Element) -> RoutingRule:
    ...
    if child_condition is not None:
        ...
        out["condition"] = capo_s3.types.condition.deserialize_xml(child_condition)
    child_redirect = el.find("Redirect")
    if child_redirect is not None:
        import capo_s3.types.redirect

        out["redirect"] = capo_s3.types.redirect.deserialize_xml(child_redirect)
    else:
