...
class DeleteObjectsOutput(TypedDict, closed=True):
    deleted: NotRequired["capo_s3.types.deleted_objects.DeletedObjects"]
    ...
    errors: NotRequired["capo_s3.types.errors.Errors"]
...
def serialize_xml(value: DeleteObjectsOutput, parent: Element, tag: str) -> None:
    ...
    if "deleted" in value:
        ...
        capo_s3.types.deleted_objects.serialize_xml_flat(
            value["deleted"], el, "Deleted"
        )
    if "errors" in value:
        import capo_s3.types.errors

        capo_s3.types.errors.serialize_xml_flat(value["errors"], el, "Error")
...
def deserialize_xml(el: Element) -> DeleteObjectsOutput:
    ...
    if el.find("Deleted") is not None:
        ...
        out["deleted"] = capo_s3.types.deleted_objects.deserialize_xml_flat(
            el, "Deleted"
        )
    if el.find("Error") is not None:
        import capo_s3.types.errors

        out["errors"] = capo_s3.types.errors.deserialize_xml_flat(el, "Error")
    return out
