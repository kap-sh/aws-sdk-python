...
if TYPE_CHECKING:
    import capo_s3.types.partitioned_prefix
    import capo_s3.types.simple_prefix


class TargetObjectKeyFormat(TypedDict, closed=True):
    simple_prefix: NotRequired["capo_s3.types.simple_prefix.SimplePrefix"]
    ...
    partitioned_prefix: NotRequired[
        "capo_s3.types.partitioned_prefix.PartitionedPrefix"
    ]
...
def serialize_xml(value: TargetObjectKeyFormat, parent: Element, tag: str) -> None:
    ...
    if "simple_prefix" in value:
        ...
        capo_s3.types.simple_prefix.serialize_xml(
            value["simple_prefix"], el, "SimplePrefix"
        )
    if "partitioned_prefix" in value:
        import capo_s3.types.partitioned_prefix

        capo_s3.types.partitioned_prefix.serialize_xml(
...
def deserialize_xml(el: Element) -> TargetObjectKeyFormat:
    ...
    if child_simple_prefix is not None:
        ...
        out["simple_prefix"] = capo_s3.types.simple_prefix.deserialize_xml(
            child_simple_prefix
        )
    ...
    if child_partitioned_prefix is not None:
        ...
        out["partitioned_prefix"] = capo_s3.types.partitioned_prefix.deserialize_xml(
