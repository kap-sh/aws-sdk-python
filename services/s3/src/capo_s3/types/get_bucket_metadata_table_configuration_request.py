"""Generated from Smithy shape ``com.amazonaws.s3#GetBucketMetadataTableConfigurationRequest``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_s3._protocol.xml import Element, SubElement

if TYPE_CHECKING:
    import capo_s3.types.account_id
    import capo_s3.types.bucket_name


class GetBucketMetadataTableConfigurationRequest(TypedDict, closed=True):
    bucket: "capo_s3.types.bucket_name.BucketName"
    """<p> The general purpose bucket that corresponds to the metadata table configuration that you want to retrieve. </p>"""
    expected_bucket_owner: NotRequired["capo_s3.types.account_id.AccountId"]
    """<p> The expected owner of the general purpose bucket that you want to retrieve the metadata table configuration for. </p>"""


# --- restXml ser/de ---
def serialize_xml(
    value: GetBucketMetadataTableConfigurationRequest, parent: Element, tag: str
) -> None:
    SubElement(parent, tag)


def deserialize_xml(el: Element) -> GetBucketMetadataTableConfigurationRequest:
    out: GetBucketMetadataTableConfigurationRequest = {}  # type: ignore[typeddict-item]
    return out
