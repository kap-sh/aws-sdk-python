...
if TYPE_CHECKING:
    import capo_s3.types.location_name_as_string
    import capo_s3.types.location_type


class LocationInfo(TypedDict, closed=True):
    type: NotRequired["capo_s3.types.location_type.LocationType"]
    """<p>The type of location where the bucket will be created.</p>"""
    name: NotRequired["capo_s3.types.location_name_as_string.LocationNameAsString"]
...
def serialize_xml(value: LocationInfo, parent: Element, tag: str) -> None:
    ...
    if "type" in value:
        ...
        capo_s3.types.location_type.serialize_xml(value["type"], el, "Type")
    if "name" in value:
...
def deserialize_xml(el: Element) -> LocationInfo:
    ...
    if child_type is not None:
        ...
        out["type"] = capo_s3.types.location_type.deserialize_xml(child_type)
