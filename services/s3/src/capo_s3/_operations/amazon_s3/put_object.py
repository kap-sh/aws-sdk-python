...
import capo_s3.errors.too_many_parts
...
import capo_s3.types.put_object_request
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "EncryptionTypeMismatch":
            raise capo_s3.errors.encryption_type_mismatch.EncryptionTypeMismatch.from_xml(
                error_el, message
            )
        case "InvalidRequest":
        ...
        case "InvalidWriteOffset":
            raise capo_s3.errors.invalid_write_offset.InvalidWriteOffset.from_xml(
                error_el, message
            )
        case "TooManyParts":
            raise capo_s3.errors.too_many_parts.TooManyParts.from_xml(error_el, message)
        case _:
...
def handle_response(
    ...
    out: capo_s3.types.put_object_output.PutObjectOutput = {}  # type: ignore[typeddict-item]
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/{Key+}?x-id=PutObject"
    ...
    if "write_offset_bytes" in input_:
        headers["x-amz-write-offset-bytes"] = str(input_["write_offset_bytes"])
...
def put_object(
    options: OperationOptions, input_: capo_s3.types.put_object_request.PutObjectRequest
) -> tuple[capo_s3.types.put_object_output.PutObjectOutput, zapros.Response]:
