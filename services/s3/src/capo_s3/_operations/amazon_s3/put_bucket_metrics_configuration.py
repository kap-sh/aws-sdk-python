"""Generated from Smithy shape ``com.amazonaws.s3#PutBucketMetricsConfiguration``."""
...
import capo_s3.types.put_bucket_metrics_configuration_request
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/{Bucket}?metrics"
    ...
    if "metrics_configuration" in input_:
        payload_root = Element("_")
        capo_s3.types.metrics_configuration.serialize_xml(
            input_["metrics_configuration"], payload_root, "MetricsConfiguration"
        )
...
async def async_put_bucket_metrics_configuration(
