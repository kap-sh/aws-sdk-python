"""Generated from Smithy shape ``com.amazonaws.s3#ListObjectAnnotations``."""
...
import capo_s3.errors.invalid_prefix
...
STATUS_CODE_TO_CODE = {400: "InvalidPrefix"}
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "InvalidPrefix":
            raise capo_s3.errors.invalid_prefix.InvalidPrefix.from_xml(
                error_el, message
            )
        case "NoSuchBucket":
...
def handle_response(
    ...
    out: capo_s3.types.list_object_annotations_output.ListObjectAnnotationsOutput = (
        capo_s3.types.list_object_annotations_output.deserialize_xml(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/{Key+}?annotation&x-id=ListObjectAnnotations"
    url = url.replace("{Key+}", quote(input_["key"], safe="/"))
    params: list[tuple[str, str]] = []
    if "version_id" in input_:
        params.append(("versionId", input_["version_id"]))
    if "max_annotation_results" in input_:
        params.append(("max-annotation-results", str(input_["max_annotation_results"])))
    if "annotation_prefix" in input_:
        params.append(("annotation-prefix", input_["annotation_prefix"]))
...
async def async_list_object_annotations(
