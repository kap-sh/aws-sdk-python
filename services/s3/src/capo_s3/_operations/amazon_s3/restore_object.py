"""Generated from Smithy shape ``com.amazonaws.s3#RestoreObject``."""
...
import capo_s3.types.restore_object_output
import capo_s3.types.restore_object_request
...
def handle_response(
...
) -> capo_s3.types.restore_object_output.RestoreObjectOutput:
    out: capo_s3.types.restore_object_output.RestoreObjectOutput = {}  # type: ignore[typeddict-item]
    ...
    if "x-amz-restore-output-path" in response.headers:
        out["restore_output_path"] = response.headers["x-amz-restore-output-path"]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/{Key+}?restore"
...
def restore_object(
    ...
    input_: capo_s3.types.restore_object_request.RestoreObjectRequest,
) -> tuple[capo_s3.types.restore_object_output.RestoreObjectOutput, zapros.Response]:
