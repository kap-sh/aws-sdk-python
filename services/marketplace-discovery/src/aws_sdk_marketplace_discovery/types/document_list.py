...
DocumentList: TypeAlias = list[
    "aws_sdk_marketplace_discovery.types.document_item.DocumentItem"
]
...
def serialize_json(value: DocumentList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_marketplace_discovery.types.document_item.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> DocumentList:
    ...
    for item in data:
        out.append(
            aws_sdk_marketplace_discovery.types.document_item.deserialize_json(item)
        )
    return out
