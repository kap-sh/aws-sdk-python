...
FulfillmentOptionsList: TypeAlias = list[
    "aws_sdk_marketplace_discovery.types.fulfillment_option.FulfillmentOption"
]
...
def serialize_json(value: FulfillmentOptionsList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_marketplace_discovery.types.fulfillment_option.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> FulfillmentOptionsList:
    ...
    for item in data:
        out.append(
            aws_sdk_marketplace_discovery.types.fulfillment_option.deserialize_json(
                item
            )
        )
    return out
