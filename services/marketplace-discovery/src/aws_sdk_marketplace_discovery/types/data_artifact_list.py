...
DataArtifactList: TypeAlias = list[
    "aws_sdk_marketplace_discovery.types.data_artifact.DataArtifact"
]
...
def serialize_json(value: DataArtifactList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_marketplace_discovery.types.data_artifact.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> DataArtifactList:
    ...
    for item in data:
        out.append(
            aws_sdk_marketplace_discovery.types.data_artifact.deserialize_json(item)
        )
    return out
