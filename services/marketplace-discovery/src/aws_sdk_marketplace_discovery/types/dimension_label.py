...
class DimensionLabel(TypedDict, closed=True):
    label_type: (
        "aws_sdk_marketplace_discovery.types.dimension_label_type.DimensionLabelType"
    )
...
def serialize_json(value: DimensionLabel) -> dict:
    ...
    out["labelType"] = (
        aws_sdk_marketplace_discovery.types.dimension_label_type.serialize_json(
            value["label_type"]
        )
    )
...
def deserialize_json(data: dict) -> DimensionLabel:
    ...
    if "labelType" in data:
        ...
        out["label_type"] = (
            aws_sdk_marketplace_discovery.types.dimension_label_type.deserialize_json(
                data["labelType"]
            )
        )
    else:
