...
if TYPE_CHECKING:
    ...
    import capo_evs.types.vm_id
    import capo_evs.types.vm_name


class VmEntitlement(TypedDict, closed=True):
    vm_id: NotRequired["capo_evs.types.vm_id.VmId"]
    ...
    connector_id: NotRequired["capo_evs.types.connector_id.ConnectorId"]
    """<p>The unique ID of the connector associated with the entitlement.</p>"""
    vm_name: NotRequired["capo_evs.types.vm_name.VmName"]
    """<p>The name of the virtual machine.</p>"""
    type: NotRequired["capo_evs.types.entitlement_type.EntitlementType"]
    """<p>The type of entitlement.</p>"""
    status: NotRequired["capo_evs.types.entitlement_status.EntitlementStatus"]
    ...
    error_detail: NotRequired["capo_evs.types.error_detail.ErrorDetail"]
...
def serialize_aws_json_1_0(value: VmEntitlement) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = capo_evs.types.entitlement_type.serialize_aws_json_1_0(
            value["type"]
        )
    if "status" in value:
        import capo_evs.types.entitlement_status

        out["status"] = capo_evs.types.entitlement_status.serialize_aws_json_1_0(
            value["status"]
        )
    if "last_synced_at" in value:
        import capo_evs.types._prelude.timestamp

        out["lastSyncedAt"] = capo_evs.types._prelude.timestamp.serialize_aws_json_1_0(
            value["last_synced_at"]
        )
    if "started_at" in value:
        import capo_evs.types._prelude.timestamp

        out["startedAt"] = capo_evs.types._prelude.timestamp.serialize_aws_json_1_0(
            value["started_at"]
        )
    if "stopped_at" in value:
        import capo_evs.types._prelude.timestamp

        out["stoppedAt"] = capo_evs.types._prelude.timestamp.serialize_aws_json_1_0(
            value["stopped_at"]
        )
    if "error_detail" in value:
        import capo_evs.types.error_detail

        out["errorDetail"] = capo_evs.types.error_detail.serialize_aws_json_1_0(
            value["error_detail"]
        )
    return out
