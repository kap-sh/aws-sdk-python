"""Generated from Smithy shape ``com.amazonaws.partnercentralbenefits#SubmitBenefitApplication``."""
...
def handle_response(
    ...
    out: capo_partnercentral_benefits.types.submit_benefit_application_output.SubmitBenefitApplicationOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/SubmitBenefitApplication"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "PartnerCentralBenefitsService.SubmitBenefitApplication"
    body: bytes | None = json.dumps(
        capo_partnercentral_benefits.types.submit_benefit_application_input.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_submit_benefit_application(
