"""Generated from Smithy shape ``com.amazonaws.lightsail#AccountSetupInProgressException``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_lightsail.errors import ServiceError

if TYPE_CHECKING:
    import capo_lightsail.types.string


class AccountSetupInProgressException_(TypedDict, closed=True):
    code: NotRequired["capo_lightsail.types.string.string"]
    docs: NotRequired["capo_lightsail.types.string.string"]
    message: NotRequired["capo_lightsail.types.string.string"]
    tip: NotRequired["capo_lightsail.types.string.string"]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: AccountSetupInProgressException_) -> dict:
    out: dict = {}
    if "code" in value:
        out["code"] = value["code"]
    if "docs" in value:
        out["docs"] = value["docs"]
    if "message" in value:
        out["message"] = value["message"]
    if "tip" in value:
        out["tip"] = value["tip"]
    return out


def deserialize_aws_json_1_1(data: dict) -> AccountSetupInProgressException_:
    out: AccountSetupInProgressException_ = {}  # type: ignore[typeddict-item]
    if "code" in data:
        out["code"] = data["code"]
    if "docs" in data:
        out["docs"] = data["docs"]
    if "message" in data:
        out["message"] = data["message"]
    if "tip" in data:
        out["tip"] = data["tip"]
    return out


class AccountSetupInProgressException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.lightsail#AccountSetupInProgressException``."""

    code: str | None = "AccountSetupInProgressException"

    def __init__(self, data: AccountSetupInProgressException_):
        super().__init__(
            "client",
            is_throttling_error=False,
            is_retryable=False,
            code="AccountSetupInProgressException",
        )
        self.data = data

    @classmethod
    def from_aws_json_1_1(cls, data: dict) -> "AccountSetupInProgressException":
        return cls(deserialize_aws_json_1_1(data))
