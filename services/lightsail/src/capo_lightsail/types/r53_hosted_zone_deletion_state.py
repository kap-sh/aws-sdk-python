...
class R53HostedZoneDeletionState(TypedDict, closed=True):
    code: NotRequired[
        "capo_lightsail.types.r53_hosted_zone_deletion_state_code.R53HostedZoneDeletionStateCode"
    ]
...
def serialize_aws_json_1_1(value: R53HostedZoneDeletionState) -> dict:
    ...
    if "code" in value:
        ...
        out["code"] = (
            capo_lightsail.types.r53_hosted_zone_deletion_state_code.serialize_aws_json_1_1(
                value["code"]
            )
        )
    if "message" in value:
...
def deserialize_aws_json_1_1(data: dict) -> R53HostedZoneDeletionState:
    ...
    if "code" in data:
        ...
        out["code"] = (
            capo_lightsail.types.r53_hosted_zone_deletion_state_code.deserialize_aws_json_1_1(
                data["code"]
            )
        )
    if "message" in data:
