"""Generated from Smithy shape ``com.amazonaws.lightsail#GetCostEstimate``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.get_cost_estimate_result.GetCostEstimateResult = (
        capo_lightsail.types.get_cost_estimate_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/budgettracker/getCostEstimate"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.GetCostEstimate"
    body: bytes | None = json.dumps(
        capo_lightsail.types.get_cost_estimate_request.serialize_aws_json_1_1(input_)
    ).encode()
...
def get_cost_estimate(
    ...
    capo_lightsail.types.get_cost_estimate_result.GetCostEstimateResult, zapros.Response
]:
...
async def async_get_cost_estimate(
