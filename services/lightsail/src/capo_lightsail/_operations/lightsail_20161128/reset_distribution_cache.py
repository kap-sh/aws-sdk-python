"""Generated from Smithy shape ``com.amazonaws.lightsail#ResetDistributionCache``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.reset_distribution_cache_result.ResetDistributionCacheResult = capo_lightsail.types.reset_distribution_cache_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/ResetDistributionCache"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.ResetDistributionCache"
    body: bytes | None = json.dumps(
        capo_lightsail.types.reset_distribution_cache_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_reset_distribution_cache(
