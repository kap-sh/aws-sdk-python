"""Generated from Smithy shape ``com.amazonaws.lightsail#UpdateContainerService``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.update_container_service_result.UpdateContainerServiceResult = capo_lightsail.types.update_container_service_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/ls/api/2016-11-28/container-services/{serviceName}"
    )
    url = url.replace("{serviceName}", quote(str(input_["service_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.UpdateContainerService"
    body: bytes | None = json.dumps(
        capo_lightsail.types.update_container_service_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_update_container_service(
