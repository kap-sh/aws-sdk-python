"""Generated from Smithy shape ``com.amazonaws.lightsail#DeleteAlarm``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.delete_alarm_result.DeleteAlarmResult = (
        capo_lightsail.types.delete_alarm_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/DeleteAlarm/{alarmName}"
    url = url.replace("{alarmName}", quote(str(input_["alarm_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.DeleteAlarm"
    body: bytes | None = b""
...
def delete_alarm(
    ...
    input_: capo_lightsail.types.delete_alarm_request.DeleteAlarmRequest,
) -> tuple[capo_lightsail.types.delete_alarm_result.DeleteAlarmResult, zapros.Response]:
