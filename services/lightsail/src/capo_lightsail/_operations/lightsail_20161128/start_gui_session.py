"""Generated from Smithy shape ``com.amazonaws.lightsail#StartGUISession``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.start_gui_session_result.StartGUISessionResult = (
        capo_lightsail.types.start_gui_session_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/start-gui-session"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.StartGUISession"
    body: bytes | None = json.dumps(
        capo_lightsail.types.start_gui_session_request.serialize_aws_json_1_1(input_)
    ).encode()
...
def start_gui_session(
    ...
    capo_lightsail.types.start_gui_session_result.StartGUISessionResult, zapros.Response
]:
...
async def async_start_gui_session(
