"""Generated from Smithy shape ``com.amazonaws.lightsail#GetAlarms``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.get_alarms_result.GetAlarmsResult = (
        capo_lightsail.types.get_alarms_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/GetAlarms"
    params: dict[str, str] = {}
    if "alarm_name" in input_:
        params["alarmName"] = str(input_["alarm_name"])
    if "page_token" in input_:
        params["pageToken"] = str(input_["page_token"])
    if "monitored_resource_name" in input_:
        params["monitoredResourceName"] = str(input_["monitored_resource_name"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.GetAlarms"
    body: bytes | None = b""
...
def get_alarms(
    ...
    input_: capo_lightsail.types.get_alarms_request.GetAlarmsRequest,
) -> tuple[capo_lightsail.types.get_alarms_result.GetAlarmsResult, zapros.Response]:
