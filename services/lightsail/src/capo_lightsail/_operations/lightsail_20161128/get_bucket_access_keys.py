"""Generated from Smithy shape ``com.amazonaws.lightsail#GetBucketAccessKeys``."""
...
def handle_response(
    ...
    out: capo_lightsail.types.get_bucket_access_keys_result.GetBucketAccessKeysResult = capo_lightsail.types.get_bucket_access_keys_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/GetBucketAccessKeys"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.GetBucketAccessKeys"
    body: bytes | None = json.dumps(
        capo_lightsail.types.get_bucket_access_keys_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_get_bucket_access_keys(
