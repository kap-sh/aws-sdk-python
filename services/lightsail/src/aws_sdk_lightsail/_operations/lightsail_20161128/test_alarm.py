"""Generated from Smithy shape ``com.amazonaws.lightsail#TestAlarm``."""
...
def handle_response(
    ...
    out: aws_sdk_lightsail.types.test_alarm_result.TestAlarmResult = (
        aws_sdk_lightsail.types.test_alarm_result.deserialize_aws_json_1_1(
...
def build_request(
    ...
    input_: aws_sdk_lightsail.types.test_alarm_request.TestAlarmRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/ls/api/2016-11-28/TestAlarm/{alarmName}"
    url = url.replace("{alarmName}", quote(str(input_["alarm_name"]), safe=""))
    params: dict[str, str] = {}
    if "state" in input_:
        params["state"] = str(input_["state"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Lightsail_20161128.TestAlarm"
    body: bytes | None = b""
...
def test_alarm(
    ...
    input_: aws_sdk_lightsail.types.test_alarm_request.TestAlarmRequest,
) -> tuple[aws_sdk_lightsail.types.test_alarm_result.TestAlarmResult, zapros.Response]:
