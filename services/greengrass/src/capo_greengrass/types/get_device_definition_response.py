"""Generated from Smithy shape ``com.amazonaws.greengrass#GetDeviceDefinitionResponse``."""
...
class GetDeviceDefinitionResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetDeviceDefinitionResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["Arn"] = value["arn"]
...
def deserialize_json(data: dict) -> GetDeviceDefinitionResponse:
    out: GetDeviceDefinitionResponse = {}  # type: ignore[typeddict-item]
    if "Arn" in data:
        out["arn"] = data["Arn"]
