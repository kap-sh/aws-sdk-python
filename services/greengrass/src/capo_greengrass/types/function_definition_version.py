...
class FunctionDefinitionVersion(TypedDict, closed=True):
    ...
    functions: NotRequired["capo_greengrass.types.__list_of_function.__listOfFunction"]
...
def serialize_json(value: FunctionDefinitionVersion) -> dict:
    ...
    if "default_config" in value:
        ...
        out["DefaultConfig"] = (
            capo_greengrass.types.function_default_config.serialize_json(
    ...
    if "functions" in value:
        ...
        out["Functions"] = capo_greengrass.types.__list_of_function.serialize_json(
            value["functions"]
        )
    return out
...
def deserialize_json(data: dict) -> FunctionDefinitionVersion:
    ...
    if "DefaultConfig" in data:
        ...
        out["default_config"] = (
            capo_greengrass.types.function_default_config.deserialize_json(
                data["DefaultConfig"]
            )
        )
    if "Functions" in data:
        ...
        out["functions"] = capo_greengrass.types.__list_of_function.deserialize_json(
            data["Functions"]
        )
    return out
