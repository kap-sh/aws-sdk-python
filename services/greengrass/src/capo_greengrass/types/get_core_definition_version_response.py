"""Generated from Smithy shape ``com.amazonaws.greengrass#GetCoreDefinitionVersionResponse``."""
...
class GetCoreDefinitionVersionResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetCoreDefinitionVersionResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["Arn"] = value["arn"]
    ...
    if "definition" in value:
        ...
        out["Definition"] = (
            capo_greengrass.types.core_definition_version.serialize_json(
                value["definition"]
            )
        )
    if "id" in value:
...
def deserialize_json(data: dict) -> GetCoreDefinitionVersionResponse:
    out: GetCoreDefinitionVersionResponse = {}  # type: ignore[typeddict-item]
    if "Arn" in data:
        out["arn"] = data["Arn"]
    ...
    if "Definition" in data:
        ...
        out["definition"] = (
            capo_greengrass.types.core_definition_version.deserialize_json(
                data["Definition"]
            )
        )
    if "Id" in data:
