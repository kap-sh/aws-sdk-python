...
class FunctionConfiguration(TypedDict, closed=True):
    encoding_type: NotRequired["capo_greengrass.types.encoding_type.EncodingType"]
    """The expected encoding type of the input payload for the function. The default is ''json''."""
    environment: NotRequired[
        "capo_greengrass.types.function_configuration_environment.FunctionConfigurationEnvironment"
    ]
    """The environment configuration of the function."""
    exec_args: NotRequired["capo_greengrass.types.__string.__string"]
    """The execution arguments."""
    executable: NotRequired["capo_greengrass.types.__string.__string"]
    """The name of the function executable."""
    memory_size: NotRequired["capo_greengrass.types.__integer.__integer"]
    """The memory size, in KB, which the function requires. This setting is not applicable and should be cleared when you run the Lambda function without containerization."""
    pinned: NotRequired["capo_greengrass.types.__boolean.__boolean"]
    """True if the function is pinned. Pinned means the function is long-lived and starts when the core starts."""
    timeout: NotRequired["capo_greengrass.types.__integer.__integer"]
    """The allowed function execution time, after which Lambda should terminate the function. This timeout still applies to pinned Lambda functions for each request."""
    function_runtime_override: NotRequired["capo_greengrass.types.__string.__string"]
...
def serialize_json(value: FunctionConfiguration) -> dict:
    ...
    if "encoding_type" in value:
        ...
        out["EncodingType"] = capo_greengrass.types.encoding_type.serialize_json(
            value["encoding_type"]
        )
    if "environment" in value:
        ...
        out["Environment"] = (
            capo_greengrass.types.function_configuration_environment.serialize_json(
                value["environment"]
            )
        )
...
def deserialize_json(data: dict) -> FunctionConfiguration:
    ...
    if "Environment" in data:
        ...
        out["environment"] = (
            capo_greengrass.types.function_configuration_environment.deserialize_json(
                data["Environment"]
            )
        )
    if "ExecArgs" in data:
