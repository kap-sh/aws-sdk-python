"""Generated from Smithy shape ``com.amazonaws.greengrass#AssociateRoleToGroup``."""
...
def handle_response(
    ...
    out: aws_sdk_greengrass.types.associate_role_to_group_response.AssociateRoleToGroupResponse = aws_sdk_greengrass.types.associate_role_to_group_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_greengrass.types.associate_role_to_group_request.AssociateRoleToGroupRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/greengrass/groups/{GroupId}/role"
    url = url.replace("{GroupId}", quote(str(input_["group_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_greengrass.types.associate_role_to_group_request.serialize_json(input_)
    ).encode()
...
async def async_associate_role_to_group(
