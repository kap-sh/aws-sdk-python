"""Generated from Smithy shape ``com.amazonaws.greengrass#UpdateCoreDefinition``."""
...
def handle_response(
    ...
    out: aws_sdk_greengrass.types.update_core_definition_response.UpdateCoreDefinitionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_greengrass.types.update_core_definition_request.UpdateCoreDefinitionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/greengrass/definition/cores/{CoreDefinitionId}"
    url = url.replace(
        "{CoreDefinitionId}", quote(str(input_["core_definition_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_greengrass.types.update_core_definition_request.serialize_json(input_)
    ).encode()
...
async def async_update_core_definition(
