"""Generated from Smithy shape ``com.amazonaws.greengrass#GetGroupVersion``."""
...
def handle_response(
    ...
    out: aws_sdk_greengrass.types.get_group_version_response.GetGroupVersionResponse = (
        aws_sdk_greengrass.types.get_group_version_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_greengrass.types.get_group_version_request.GetGroupVersionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{GroupVersionId}", quote(str(input_["group_version_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_group_version(
