...
if TYPE_CHECKING:
    import aws_sdk_dynamodb_streams.types.shard_filter_type
    import aws_sdk_dynamodb_streams.types.shard_id


class ShardFilter(TypedDict, closed=True):
    type: NotRequired[
        "aws_sdk_dynamodb_streams.types.shard_filter_type.ShardFilterType"
    ]
...
def serialize_aws_json_1_0(value: ShardFilter) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = (
            aws_sdk_dynamodb_streams.types.shard_filter_type.serialize_aws_json_1_0(
                value["type"]
            )
        )
...
def deserialize_aws_json_1_0(data: dict) -> ShardFilter:
    ...
    if "Type" in data:
        ...
        out["type"] = (
            aws_sdk_dynamodb_streams.types.shard_filter_type.deserialize_aws_json_1_0(
                data["Type"]
            )
        )
    if "ShardId" in data:
