"""Generated from Smithy shape ``com.amazonaws.dynamodbstreams#StringSetAttributeValue``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_dynamodb_streams.types.string_attribute_value

StringSetAttributeValue: TypeAlias = list[
    "aws_sdk_dynamodb_streams.types.string_attribute_value.StringAttributeValue"
]


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: StringSetAttributeValue) -> list:
    return list(value)


def deserialize_aws_json_1_0(data: list) -> StringSetAttributeValue:
    return list(data)
