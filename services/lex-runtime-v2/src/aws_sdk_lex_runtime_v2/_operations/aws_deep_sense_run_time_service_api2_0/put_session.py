...
def build_request(
    ...
    input: aws_sdk_lex_runtime_v2.types.put_session_request.PutSessionRequest,
) -> zapros.Request:
    ...
    if "response_content_type" in input:
        headers["ResponseContentType"] = str(input["response_content_type"])
    import aws_sdk_lex_runtime_v2.types.put_session_request

    body: bytes | None = json.dumps(
        aws_sdk_lex_runtime_v2.types.put_session_request.serialize_json(input)
    ).encode()
