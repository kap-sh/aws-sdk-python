...
if TYPE_CHECKING:
    import capo_lex_runtime_v2.types.slot_hints_intent_map
...
def serialize_json(value: RuntimeHints) -> dict:
    ...
    if "slot_hints" in value:
        ...
        out["slotHints"] = (
            capo_lex_runtime_v2.types.slot_hints_intent_map.serialize_json(
                value["slot_hints"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RuntimeHints:
    ...
    if "slotHints" in data:
        ...
        out["slot_hints"] = (
            capo_lex_runtime_v2.types.slot_hints_intent_map.deserialize_json(
                data["slotHints"]
            )
        )
    return out
