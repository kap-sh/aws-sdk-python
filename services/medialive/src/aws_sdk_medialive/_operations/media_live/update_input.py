...
def build_request(
    ...
    input: aws_sdk_medialive.types.update_input_request.UpdateInputRequest,
) -> zapros.Request:
    ...
    url = url.replace("{InputId}", quote(str(input["input_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_medialive.types.update_input_request

    body: bytes | None = json.dumps(
        aws_sdk_medialive.types.update_input_request.serialize_json(input)
    ).encode()
