"""Generated from Smithy shape ``com.amazonaws.medialive#BatchUpdateSchedule``."""
...
def handle_response(
    ...
    out: aws_sdk_medialive.types.batch_update_schedule_response.BatchUpdateScheduleResponse = aws_sdk_medialive.types.batch_update_schedule_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_medialive.types.batch_update_schedule_request.BatchUpdateScheduleRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/prod/channels/{ChannelId}/schedule"
    url = url.replace("{ChannelId}", quote(str(input_["channel_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_medialive.types.batch_update_schedule_request.serialize_json(input_)
    ).encode()
...
async def async_batch_update_schedule(
