"""Generated from Smithy shape ``com.amazonaws.medialive#RestartChannelPipelines``."""
...
def handle_response(
    ...
    out: aws_sdk_medialive.types.restart_channel_pipelines_response.RestartChannelPipelinesResponse = aws_sdk_medialive.types.restart_channel_pipelines_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_medialive.types.restart_channel_pipelines_request.RestartChannelPipelinesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/prod/channels/{ChannelId}/restartChannelPipelines"
    )
    url = url.replace("{ChannelId}", quote(str(input_["channel_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_medialive.types.restart_channel_pipelines_request.serialize_json(input_)
    ).encode()
...
async def async_restart_channel_pipelines(
