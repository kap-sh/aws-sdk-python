"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateNode``."""
...
def handle_response(
    ...
    out: aws_sdk_medialive.types.update_node_response.UpdateNodeResponse = (
        aws_sdk_medialive.types.update_node_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_medialive.types.update_node_request.UpdateNodeRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/prod/clusters/{ClusterId}/nodes/{NodeId}"
    url = url.replace("{ClusterId}", quote(str(input["cluster_id"]), safe=""))
    url = url.replace("{NodeId}", quote(str(input["node_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_medialive.types.update_node_request

    body: bytes | None = json.dumps(
        aws_sdk_medialive.types.update_node_request.serialize_json(input)
    ).encode()
...
def update_node(
    ...
    aws_sdk_medialive.types.update_node_response.UpdateNodeResponse, zapros.Response
]:
