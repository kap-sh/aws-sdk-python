"""Generated from Smithy shape ``com.amazonaws.medialive#DescribeThumbnails``."""
...
def handle_response(
    ...
    out: aws_sdk_medialive.types.describe_thumbnails_response.DescribeThumbnailsResponse = aws_sdk_medialive.types.describe_thumbnails_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_medialive.types.describe_thumbnails_request.DescribeThumbnailsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/prod/channels/{ChannelId}/thumbnails"
    url = url.replace("{ChannelId}", quote(str(input_["channel_id"]), safe=""))
    params: dict[str, str] = {}
    if "pipeline_id" in input_:
        params["pipelineId"] = str(input_["pipeline_id"])
    if "thumbnail_type" in input_:
        params["thumbnailType"] = str(input_["thumbnail_type"])
...
async def async_describe_thumbnails(
