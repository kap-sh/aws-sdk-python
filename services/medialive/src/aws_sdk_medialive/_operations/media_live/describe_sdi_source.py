"""Generated from Smithy shape ``com.amazonaws.medialive#DescribeSdiSource``."""
...
def handle_response(
    ...
    out: aws_sdk_medialive.types.describe_sdi_source_response.DescribeSdiSourceResponse = aws_sdk_medialive.types.describe_sdi_source_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_medialive.types.describe_sdi_source_request.DescribeSdiSourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/prod/sdiSources/{SdiSourceId}"
    url = url.replace("{SdiSourceId}", quote(str(input_["sdi_source_id"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_sdi_source(
