"""Generated from Smithy shape ``com.amazonaws.medialive#BatchStartResponse``."""
...
class BatchStartResponse(TypedDict, closed=True):
    failed: NotRequired[
...
# --- restJson1 ser/de ---
def serialize_json(value: BatchStartResponse) -> dict:
    out: dict = {}
    if "failed" in value:
        import aws_sdk_medialive.types.__list_of_batch_failed_result_model
...
def deserialize_json(data: dict) -> BatchStartResponse:
    out: BatchStartResponse = {}  # type: ignore[typeddict-item]
    if "failed" in data:
        import aws_sdk_medialive.types.__list_of_batch_failed_result_model
