...
__listOfInput: TypeAlias = list["aws_sdk_medialive.types.input.Input"]
...
def serialize_json(value: __listOfInput) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_medialive.types.input.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfInput:
    ...
    out: __listOfInput = []
    for item in data:
        out.append(aws_sdk_medialive.types.input.deserialize_json(item))
    return out
