...
class AvailBlanking(TypedDict, closed=True):
    ...
    state: NotRequired[
        "aws_sdk_medialive.types.avail_blanking_state.AvailBlankingState"
    ]
...
def serialize_json(value: AvailBlanking) -> dict:
    ...
    if "state" in value:
        ...
        out["state"] = aws_sdk_medialive.types.avail_blanking_state.serialize_json(
            value["state"]
        )
    return out
...
def deserialize_json(data: dict) -> AvailBlanking:
    ...
    if "state" in data:
        ...
        out["state"] = aws_sdk_medialive.types.avail_blanking_state.deserialize_json(
            data["state"]
        )
    return out
