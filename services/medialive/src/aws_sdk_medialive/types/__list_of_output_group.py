...
if TYPE_CHECKING:
    import aws_sdk_medialive.types.output_group

__listOfOutputGroup: TypeAlias = list[
    "aws_sdk_medialive.types.output_group.OutputGroup"
]
...
def serialize_json(value: __listOfOutputGroup) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_medialive.types.output_group.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfOutputGroup:
    ...
    for item in data:
        out.append(aws_sdk_medialive.types.output_group.deserialize_json(item))
    return out
