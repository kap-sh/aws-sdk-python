...
__listOfTransferringInputDeviceSummary: TypeAlias = list[
    "aws_sdk_medialive.types.transferring_input_device_summary.TransferringInputDeviceSummary"
]
...
def serialize_json(value: __listOfTransferringInputDeviceSummary) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_medialive.types.transferring_input_device_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> __listOfTransferringInputDeviceSummary:
    ...
    for item in data:
        out.append(
            aws_sdk_medialive.types.transferring_input_device_summary.deserialize_json(
                item
            )
        )
    return out
