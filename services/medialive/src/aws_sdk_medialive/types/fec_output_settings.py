...
if TYPE_CHECKING:
    import aws_sdk_medialive.types.__integer_min1_max20
    import aws_sdk_medialive.types.__integer_min4_max20
    import aws_sdk_medialive.types.fec_output_include_fec


class FecOutputSettings(TypedDict):
    column_depth: NotRequired[
        "aws_sdk_medialive.types.__integer_min4_max20.__integerMin4Max20"
    ]
    """Parameter D from SMPTE 2022-1. The height of the FEC protection matrix. The number of transport stream packets per column error correction packet. Must be between 4 and 20, inclusive."""
    include_fec: NotRequired[
        "aws_sdk_medialive.types.fec_output_include_fec.FecOutputIncludeFec"
    ]
    """Enables column only or column and row based FEC"""
    row_length: NotRequired[
        "aws_sdk_medialive.types.__integer_min1_max20.__integerMin1Max20"
    ]
...
def serialize_json(value: FecOutputSettings) -> dict:
    ...
    if "include_fec" in value:
        ...
        out["includeFec"] = (
            aws_sdk_medialive.types.fec_output_include_fec.serialize_json(
                value["include_fec"]
            )
        )
...
def deserialize_json(data: dict) -> FecOutputSettings:
    ...
    if "includeFec" in data:
        ...
        out["include_fec"] = (
            aws_sdk_medialive.types.fec_output_include_fec.deserialize_json(
                data["includeFec"]
            )
        )
