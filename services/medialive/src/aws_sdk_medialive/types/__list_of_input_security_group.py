...
def serialize_json(value: __listOfInputSecurityGroup) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_medialive.types.input_security_group.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfInputSecurityGroup:
    ...
    for item in data:
        out.append(aws_sdk_medialive.types.input_security_group.deserialize_json(item))
    return out
