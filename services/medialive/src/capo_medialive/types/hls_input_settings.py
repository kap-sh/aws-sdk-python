...
class HlsInputSettings(TypedDict, closed=True):
    bandwidth: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    """When specified the HLS stream with the m3u8 BANDWIDTH that most closely matches this value will be chosen, otherwise the highest bandwidth stream in the m3u8 will be chosen. The bitrate is specified in bits per second, as in an HLS manifest."""
    buffer_segments: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    """When specified, reading of the HLS input will begin this many buffer segments from the end (most recently written segment). When not specified, the HLS input will begin with the first segment specified in the m3u8."""
    retries: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    """The number of consecutive times that attempts to read a manifest or segment must fail before the input is considered unavailable."""
    retry_interval: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    """The number of seconds between retries when an attempt to read a manifest or segment fails."""
    scte35_source: NotRequired[
        "capo_medialive.types.hls_scte35_source_type.HlsScte35SourceType"
    ]
...
def serialize_json(value: HlsInputSettings) -> dict:
    ...
    if "scte35_source" in value:
        ...
        out["scte35Source"] = (
            capo_medialive.types.hls_scte35_source_type.serialize_json(
                value["scte35_source"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> HlsInputSettings:
    ...
    if "scte35Source" in data:
        ...
        out["scte35_source"] = (
            capo_medialive.types.hls_scte35_source_type.deserialize_json(
                data["scte35Source"]
            )
        )
    return out
