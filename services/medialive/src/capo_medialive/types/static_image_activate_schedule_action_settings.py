...
class StaticImageActivateScheduleActionSettings(TypedDict, closed=True):
    duration: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    """The duration in milliseconds for the image to remain on the video. If omitted or set to 0 the duration is unlimited and the image will remain until it is explicitly deactivated."""
    fade_in: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    ...
    height: NotRequired["capo_medialive.types.__integer_min1.__integerMin1"]
    """The height of the image when inserted into the video, in pixels. The overlay will be scaled up or down to the specified height. Leave blank to use the native height of the overlay."""
    image: NotRequired["capo_medialive.types.input_location.InputLocation"]
    """The location and filename of the image file to overlay on the video. The file must be a 32-bit BMP, PNG, or TGA file, and must not be larger (in pixels) than the input video."""
    image_x: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    """Placement of the left edge of the overlay relative to the left edge of the video frame, in pixels. 0 (the default) is the left edge of the frame. If the placement causes the overlay to extend beyond the right edge of the underlying video, then the overlay is cropped on the right."""
    image_y: NotRequired["capo_medialive.types.__integer_min0.__integerMin0"]
    ...
    width: NotRequired["capo_medialive.types.__integer_min1.__integerMin1"]
...
def serialize_json(value: StaticImageActivateScheduleActionSettings) -> dict:
    ...
    if "image" in value:
        ...
        out["image"] = capo_medialive.types.input_location.serialize_json(
            value["image"]
        )
    if "image_x" in value:
