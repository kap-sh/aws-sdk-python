...
if TYPE_CHECKING:
    import capo_medialive.types.__integer_min25_max2000
    import capo_medialive.types.__string_min1_max256
    import capo_medialive.types.dvb_sdt_output_sdt


class DvbSdtSettings(TypedDict, closed=True):
    output_sdt: NotRequired["capo_medialive.types.dvb_sdt_output_sdt.DvbSdtOutputSdt"]
    """Selects method of inserting SDT information into output stream. The sdtFollow setting copies SDT information from input stream to output stream. The sdtFollowIfPresent setting copies SDT information from input stream to output stream if SDT information is present in the input, otherwise it will fall back on the user-defined values. The sdtManual setting means user will enter the SDT information. The sdtNone setting means output stream will not contain SDT information."""
    rep_interval: NotRequired[
        "capo_medialive.types.__integer_min25_max2000.__integerMin25Max2000"
    ]
...
def serialize_json(value: DvbSdtSettings) -> dict:
    ...
    if "output_sdt" in value:
        ...
        out["outputSdt"] = capo_medialive.types.dvb_sdt_output_sdt.serialize_json(
            value["output_sdt"]
        )
...
def deserialize_json(data: dict) -> DvbSdtSettings:
    ...
    if "outputSdt" in data:
        ...
        out["output_sdt"] = capo_medialive.types.dvb_sdt_output_sdt.deserialize_json(
            data["outputSdt"]
        )
