"""Generated from Smithy shape ``com.amazonaws.medialive#DeleteReservationResponse``."""
...
class DeleteReservationResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: DeleteReservationResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["arn"] = value["arn"]
    if "count" in value:
...
def deserialize_json(data: dict) -> DeleteReservationResponse:
    out: DeleteReservationResponse = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
    if "count" in data:
