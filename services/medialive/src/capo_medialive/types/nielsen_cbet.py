...
if TYPE_CHECKING:
    import capo_medialive.types.__string_min1_max7
...
class NielsenCBET(TypedDict, closed=True):
    ...
    cbet_stepaside: NotRequired[
        "capo_medialive.types.nielsen_watermarks_cbet_stepaside.NielsenWatermarksCbetStepaside"
    ]
    """Determines the method of CBET insertion mode when prior encoding is detected on the same layer."""
    csid: NotRequired["capo_medialive.types.__string_min1_max7.__stringMin1Max7"]
...
def serialize_json(value: NielsenCBET) -> dict:
    ...
    if "cbet_stepaside" in value:
        ...
        out["cbetStepaside"] = (
            capo_medialive.types.nielsen_watermarks_cbet_stepaside.serialize_json(
...
def deserialize_json(data: dict) -> NielsenCBET:
    ...
    if "cbetStepaside" in data:
        ...
        out["cbet_stepaside"] = (
            capo_medialive.types.nielsen_watermarks_cbet_stepaside.deserialize_json(
                data["cbetStepaside"]
            )
        )
    if "csid" in data:
