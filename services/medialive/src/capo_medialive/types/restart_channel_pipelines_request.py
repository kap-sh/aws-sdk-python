...
def serialize_json(value: RestartChannelPipelinesRequest) -> dict:
    ...
    if "pipeline_ids" in value:
        ...
        out["pipelineIds"] = (
            capo_medialive.types.__list_of_channel_pipeline_id_to_restart.serialize_json(
                value["pipeline_ids"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RestartChannelPipelinesRequest:
    ...
    if "pipelineIds" in data:
        ...
        out["pipeline_ids"] = (
            capo_medialive.types.__list_of_channel_pipeline_id_to_restart.deserialize_json(
                data["pipelineIds"]
            )
        )
    return out
