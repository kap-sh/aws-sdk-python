...
if TYPE_CHECKING:
    import capo_medialive.types.input_destination_request

__listOfInputDestinationRequest: TypeAlias = list[
    "capo_medialive.types.input_destination_request.InputDestinationRequest"
]
...
def serialize_json(value: __listOfInputDestinationRequest) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_medialive.types.input_destination_request.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfInputDestinationRequest:
    ...
    for item in data:
        out.append(
            capo_medialive.types.input_destination_request.deserialize_json(item)
        )
    return out
