"""Generated from Smithy shape ``com.amazonaws.medialive#DescribeNetworkResponse``."""
...
class DescribeNetworkResponse(TypedDict, closed=True):
    ...
    ip_pools: NotRequired["capo_medialive.types.__list_of_ip_pool.__listOfIpPool"]
    ...
    routes: NotRequired["capo_medialive.types.__list_of_route.__listOfRoute"]
    """An array of routes that MediaLive Anywhere needs to know about in order to route encoding traffic."""
    state: NotRequired["capo_medialive.types.network_state.NetworkState"]
    """The current state of the Network. Only MediaLive Anywhere can change the state."""


# --- restJson1 ser/de ---
def serialize_json(value: DescribeNetworkResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["arn"] = value["arn"]
    ...
    if "ip_pools" in value:
        ...
        out["ipPools"] = capo_medialive.types.__list_of_ip_pool.serialize_json(
            value["ip_pools"]
        )
    if "name" in value:
        out["name"] = value["name"]
    if "routes" in value:
        ...
        out["routes"] = capo_medialive.types.__list_of_route.serialize_json(
            value["routes"]
        )
    if "state" in value:
        import capo_medialive.types.network_state

        out["state"] = capo_medialive.types.network_state.serialize_json(value["state"])
    return out


def deserialize_json(data: dict) -> DescribeNetworkResponse:
    out: DescribeNetworkResponse = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
