...
if TYPE_CHECKING:
    import capo_medialive.types.__list_of_multiplex_summary
    import capo_medialive.types.__string


class ListMultiplexesResponse(TypedDict, closed=True):
    multiplexes: NotRequired[
        "capo_medialive.types.__list_of_multiplex_summary.__listOfMultiplexSummary"
    ]
    """List of multiplexes."""
...
def serialize_json(value: ListMultiplexesResponse) -> dict:
    ...
    if "multiplexes" in value:
        ...
        out["multiplexes"] = (
            capo_medialive.types.__list_of_multiplex_summary.serialize_json(
                value["multiplexes"]
            )
        )
...
def deserialize_json(data: dict) -> ListMultiplexesResponse:
    ...
    if "multiplexes" in data:
        ...
        out["multiplexes"] = (
            capo_medialive.types.__list_of_multiplex_summary.deserialize_json(
                data["multiplexes"]
            )
        )
