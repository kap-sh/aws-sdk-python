...
if TYPE_CHECKING:
    import capo_medialive.types.__list_of_describe_node_summary
...
class ListNodesResponse(TypedDict, closed=True):
    ...
    nodes: NotRequired[
        "capo_medialive.types.__list_of_describe_node_summary.__listOfDescribeNodeSummary"
    ]
...
def serialize_json(value: ListNodesResponse) -> dict:
    ...
    if "nodes" in value:
        ...
        out["nodes"] = (
            capo_medialive.types.__list_of_describe_node_summary.serialize_json(
                value["nodes"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListNodesResponse:
    ...
    if "nodes" in data:
        ...
        out["nodes"] = (
            capo_medialive.types.__list_of_describe_node_summary.deserialize_json(
                data["nodes"]
            )
        )
    return out
