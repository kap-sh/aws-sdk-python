"""Generated from Smithy shape ``com.amazonaws.medialive#GetCloudWatchAlarmTemplateGroupResponse``."""
...
class GetCloudWatchAlarmTemplateGroupResponse(TypedDict, closed=True):
    arn: NotRequired[
...
# --- restJson1 ser/de ---
def serialize_json(value: GetCloudWatchAlarmTemplateGroupResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["arn"] = value["arn"]
...
def deserialize_json(data: dict) -> GetCloudWatchAlarmTemplateGroupResponse:
    out: GetCloudWatchAlarmTemplateGroupResponse = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
