...
if TYPE_CHECKING:
    import capo_medialive.types.media_package_output_destination_settings

__listOfMediaPackageOutputDestinationSettings: TypeAlias = list[
    "capo_medialive.types.media_package_output_destination_settings.MediaPackageOutputDestinationSettings"
]
...
def serialize_json(value: __listOfMediaPackageOutputDestinationSettings) -> list:
    ...
    for item in value:
        out.append(
            capo_medialive.types.media_package_output_destination_settings.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> __listOfMediaPackageOutputDestinationSettings:
    ...
    for item in data:
        out.append(
            capo_medialive.types.media_package_output_destination_settings.deserialize_json(
                item
            )
        )
    return out
