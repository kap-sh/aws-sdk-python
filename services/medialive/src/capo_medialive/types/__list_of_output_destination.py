...
if TYPE_CHECKING:
    import capo_medialive.types.output_destination

__listOfOutputDestination: TypeAlias = list[
    "capo_medialive.types.output_destination.OutputDestination"
]
...
def serialize_json(value: __listOfOutputDestination) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_medialive.types.output_destination.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfOutputDestination:
    ...
    for item in data:
        out.append(capo_medialive.types.output_destination.deserialize_json(item))
    return out
