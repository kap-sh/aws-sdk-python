...
if TYPE_CHECKING:
    ...
    import capo_medialive.types.__string_max32
    import capo_medialive.types.output_group_settings


class OutputGroup(TypedDict, closed=True):
    name: NotRequired["capo_medialive.types.__string_max32.__stringMax32"]
    """Custom output group name optionally defined by the user."""
    output_group_settings: NotRequired[
        "capo_medialive.types.output_group_settings.OutputGroupSettings"
    ]
    """Settings associated with the output group."""
    outputs: NotRequired["capo_medialive.types.__list_of_output.__listOfOutput"]
...
def serialize_json(value: OutputGroup) -> dict:
    ...
    if "output_group_settings" in value:
        ...
        out["outputGroupSettings"] = (
            capo_medialive.types.output_group_settings.serialize_json(
    ...
    if "outputs" in value:
        ...
        out["outputs"] = capo_medialive.types.__list_of_output.serialize_json(
            value["outputs"]
        )
    return out
...
def deserialize_json(data: dict) -> OutputGroup:
    ...
    if "outputGroupSettings" in data:
        ...
        out["output_group_settings"] = (
            capo_medialive.types.output_group_settings.deserialize_json(
