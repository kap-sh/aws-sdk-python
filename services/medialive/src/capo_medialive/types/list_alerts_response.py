...
class ListAlertsResponse(TypedDict, closed=True):
    alerts: NotRequired[
        "capo_medialive.types.__list_of_channel_alert.__listOfChannelAlert"
    ]
...
def serialize_json(value: ListAlertsResponse) -> dict:
    ...
    if "alerts" in value:
        ...
        out["alerts"] = capo_medialive.types.__list_of_channel_alert.serialize_json(
            value["alerts"]
        )
...
def deserialize_json(data: dict) -> ListAlertsResponse:
    ...
    if "alerts" in data:
        ...
        out["alerts"] = capo_medialive.types.__list_of_channel_alert.deserialize_json(
            data["alerts"]
        )
