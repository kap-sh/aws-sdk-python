"""Generated from Smithy shape ``com.amazonaws.medialive#SdiSourceSummary``."""
...
class SdiSourceSummary(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: SdiSourceSummary) -> dict:
    out: dict = {}
    if "arn" in value:
        out["arn"] = value["arn"]
    if "id" in value:
...
def deserialize_json(data: dict) -> SdiSourceSummary:
    out: SdiSourceSummary = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
    if "id" in data:
        out["id"] = data["id"]
