"""Generated from Smithy shape ``com.amazonaws.medialive#CreateNodeRegistrationScript``."""
...
def handle_response(
    ...
    out: capo_medialive.types.create_node_registration_script_response.CreateNodeRegistrationScriptResponse = capo_medialive.types.create_node_registration_script_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/clusters/{ClusterId}/nodeRegistrationScript"
    url = url.replace("{ClusterId}", quote(str(input_["cluster_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.create_node_registration_script_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_node_registration_script(
