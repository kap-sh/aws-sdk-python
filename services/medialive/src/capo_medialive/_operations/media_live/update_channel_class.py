"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateChannelClass``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_channel_class_response.UpdateChannelClassResponse = capo_medialive.types.update_channel_class_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/channels/{ChannelId}/channelClass"
    url = url.replace("{ChannelId}", quote(str(input_["channel_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_channel_class_request.serialize_json(input_)
    ).encode()
...
async def async_update_channel_class(
