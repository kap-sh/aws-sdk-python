"""Generated from Smithy shape ``com.amazonaws.medialive#RebootInputDevice``."""
...
def handle_response(
    ...
    out: capo_medialive.types.reboot_input_device_response.RebootInputDeviceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/inputDevices/{InputDeviceId}/reboot"
    url = url.replace("{InputDeviceId}", quote(str(input_["input_device_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.reboot_input_device_request.serialize_json(input_)
    ).encode()
...
async def async_reboot_input_device(
