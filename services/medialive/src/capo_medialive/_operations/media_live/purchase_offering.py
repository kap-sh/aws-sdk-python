"""Generated from Smithy shape ``com.amazonaws.medialive#PurchaseOffering``."""
...
def handle_response(
    ...
    out: capo_medialive.types.purchase_offering_response.PurchaseOfferingResponse = (
        capo_medialive.types.purchase_offering_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/offerings/{OfferingId}/purchase"
    url = url.replace("{OfferingId}", quote(str(input_["offering_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.purchase_offering_request.serialize_json(input_)
    ).encode()
