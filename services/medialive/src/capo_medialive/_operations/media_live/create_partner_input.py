"""Generated from Smithy shape ``com.amazonaws.medialive#CreatePartnerInput``."""
...
def handle_response(
    ...
    out: capo_medialive.types.create_partner_input_response.CreatePartnerInputResponse = capo_medialive.types.create_partner_input_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/inputs/{InputId}/partners"
    url = url.replace("{InputId}", quote(str(input_["input_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.create_partner_input_request.serialize_json(input_)
    ).encode()
...
async def async_create_partner_input(
