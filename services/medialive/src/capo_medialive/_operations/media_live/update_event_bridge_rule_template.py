"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateEventBridgeRuleTemplate``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_event_bridge_rule_template_response.UpdateEventBridgeRuleTemplateResponse = capo_medialive.types.update_event_bridge_rule_template_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/eventbridge-rule-templates/{Identifier}"
    url = url.replace("{Identifier}", quote(str(input_["identifier"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_event_bridge_rule_template_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_event_bridge_rule_template(
