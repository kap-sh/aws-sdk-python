"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateCluster``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_cluster_response.UpdateClusterResponse = (
        capo_medialive.types.update_cluster_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/clusters/{ClusterId}"
    url = url.replace("{ClusterId}", quote(str(input_["cluster_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_cluster_request.serialize_json(input_)
    ).encode()
...
def update_cluster(
    ...
    capo_medialive.types.update_cluster_response.UpdateClusterResponse, zapros.Response
]:
