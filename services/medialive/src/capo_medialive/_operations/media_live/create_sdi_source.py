"""Generated from Smithy shape ``com.amazonaws.medialive#CreateSdiSource``."""
...
def handle_response(
    ...
    out: capo_medialive.types.create_sdi_source_response.CreateSdiSourceResponse = (
        capo_medialive.types.create_sdi_source_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/sdiSources"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.create_sdi_source_request.serialize_json(input_)
    ).encode()
...
async def async_create_sdi_source(
