"""Generated from Smithy shape ``com.amazonaws.medialive#UpdateAccountConfiguration``."""
...
def handle_response(
    ...
    out: capo_medialive.types.update_account_configuration_response.UpdateAccountConfigurationResponse = capo_medialive.types.update_account_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/accountConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.update_account_configuration_request.serialize_json(input_)
    ).encode()
