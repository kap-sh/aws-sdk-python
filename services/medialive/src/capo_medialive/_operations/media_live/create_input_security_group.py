"""Generated from Smithy shape ``com.amazonaws.medialive#CreateInputSecurityGroup``."""
...
def handle_response(
    ...
    out: capo_medialive.types.create_input_security_group_response.CreateInputSecurityGroupResponse = capo_medialive.types.create_input_security_group_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/inputSecurityGroups"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.create_input_security_group_request.serialize_json(input_)
    ).encode()
...
async def async_create_input_security_group(
