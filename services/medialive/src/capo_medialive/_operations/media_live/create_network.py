"""Generated from Smithy shape ``com.amazonaws.medialive#CreateNetwork``."""
...
def handle_response(
    ...
    out: capo_medialive.types.create_network_response.CreateNetworkResponse = (
        capo_medialive.types.create_network_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/networks"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.create_network_request.serialize_json(input_)
    ).encode()
...
def create_network(
    ...
    capo_medialive.types.create_network_response.CreateNetworkResponse, zapros.Response
]:
