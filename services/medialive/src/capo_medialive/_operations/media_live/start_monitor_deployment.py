"""Generated from Smithy shape ``com.amazonaws.medialive#StartMonitorDeployment``."""
...
def handle_response(
    ...
    out: capo_medialive.types.start_monitor_deployment_response.StartMonitorDeploymentResponse = capo_medialive.types.start_monitor_deployment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/signal-maps/{Identifier}/monitor-deployment"
    url = url.replace("{Identifier}", quote(str(input_["identifier"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.start_monitor_deployment_request.serialize_json(input_)
    ).encode()
...
async def async_start_monitor_deployment(
