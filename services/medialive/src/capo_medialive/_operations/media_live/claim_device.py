"""Generated from Smithy shape ``com.amazonaws.medialive#ClaimDevice``."""
...
def handle_response(
    ...
    out: capo_medialive.types.claim_device_response.ClaimDeviceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/claimDevice"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.claim_device_request.serialize_json(input_)
    ).encode()
...
def claim_device(
    ...
    capo_medialive.types.claim_device_response.ClaimDeviceResponse, zapros.Response
]:
