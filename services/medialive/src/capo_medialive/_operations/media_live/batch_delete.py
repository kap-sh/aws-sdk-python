"""Generated from Smithy shape ``com.amazonaws.medialive#BatchDelete``."""
...
def handle_response(
    ...
    out: capo_medialive.types.batch_delete_response.BatchDeleteResponse = (
        capo_medialive.types.batch_delete_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/prod/batch/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_medialive.types.batch_delete_request.serialize_json(input_)
    ).encode()
...
def batch_delete(
    ...
    capo_medialive.types.batch_delete_response.BatchDeleteResponse, zapros.Response
]:
