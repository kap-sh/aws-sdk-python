...
class DkimAttributes(TypedDict, closed=True):
    signing_enabled: "capo_pinpoint_email.types.enabled.Enabled"
    ...
    status: NotRequired["capo_pinpoint_email.types.dkim_status.DkimStatus"]
    ...
    tokens: NotRequired["capo_pinpoint_email.types.dns_token_list.DnsTokenList"]
...
def serialize_json(value: DkimAttributes) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_pinpoint_email.types.dkim_status.serialize_json(
            value["status"]
        )
    if "tokens" in value:
        import capo_pinpoint_email.types.dns_token_list

        out["Tokens"] = capo_pinpoint_email.types.dns_token_list.serialize_json(
            value["tokens"]
        )
    return out
