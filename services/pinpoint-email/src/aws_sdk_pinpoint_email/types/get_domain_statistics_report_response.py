...
def serialize_json(value: GetDomainStatisticsReportResponse) -> dict:
    ...
    out["OverallVolume"] = aws_sdk_pinpoint_email.types.overall_volume.serialize_json(
        value["overall_volume"]
    )
    import aws_sdk_pinpoint_email.types.daily_volumes

    out["DailyVolumes"] = aws_sdk_pinpoint_email.types.daily_volumes.serialize_json(
        value["daily_volumes"]
    )
    return out
...
def deserialize_json(data: dict) -> GetDomainStatisticsReportResponse:
    ...
    if "OverallVolume" in data:
        ...
        out["overall_volume"] = (
            aws_sdk_pinpoint_email.types.overall_volume.deserialize_json(
                data["OverallVolume"]
            )
        )
    else:
    ...
    if "DailyVolumes" in data:
        ...
        out["daily_volumes"] = (
            aws_sdk_pinpoint_email.types.daily_volumes.deserialize_json(
                data["DailyVolumes"]
            )
        )
    else:
