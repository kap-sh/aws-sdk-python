...
if TYPE_CHECKING:
    ...
    import aws_sdk_pinpoint_email.types.raw_message
...
class EmailContent(TypedDict):
    ...
    template: NotRequired["aws_sdk_pinpoint_email.types.template.Template"]
...
def serialize_json(value: EmailContent) -> dict:
    ...
    if "simple" in value:
        ...
        out["Simple"] = aws_sdk_pinpoint_email.types.message.serialize_json(
            value["simple"]
        )
    if "raw" in value:
        import aws_sdk_pinpoint_email.types.raw_message

        out["Raw"] = aws_sdk_pinpoint_email.types.raw_message.serialize_json(
            value["raw"]
        )
    if "template" in value:
        import aws_sdk_pinpoint_email.types.template

        out["Template"] = aws_sdk_pinpoint_email.types.template.serialize_json(
            value["template"]
        )
    return out
...
def deserialize_json(data: dict) -> EmailContent:
    ...
    if "Simple" in data:
        ...
        out["simple"] = aws_sdk_pinpoint_email.types.message.deserialize_json(
            data["Simple"]
        )
    if "Raw" in data:
        import aws_sdk_pinpoint_email.types.raw_message

        out["raw"] = aws_sdk_pinpoint_email.types.raw_message.deserialize_json(
            data["Raw"]
        )
    if "Template" in data:
        import aws_sdk_pinpoint_email.types.template

        out["template"] = aws_sdk_pinpoint_email.types.template.deserialize_json(
            data["Template"]
        )
    return out
