...
if TYPE_CHECKING:
    import aws_sdk_pinpoint_email.types.blacklisting_description
    import aws_sdk_pinpoint_email.types.rbl_name
    import aws_sdk_pinpoint_email.types.timestamp


class BlacklistEntry(TypedDict, closed=True):
    rbl_name: NotRequired["aws_sdk_pinpoint_email.types.rbl_name.RblName"]
    """<p>The name of the blacklist that the IP address appears on.</p>"""
    listing_time: NotRequired["aws_sdk_pinpoint_email.types.timestamp.Timestamp"]
    """<p>The time when the blacklisting event occurred, shown in Unix time format.</p>"""
    description: NotRequired[
        "aws_sdk_pinpoint_email.types.blacklisting_description.BlacklistingDescription"
    ]
...
def serialize_json(value: BlacklistEntry) -> dict:
    ...
    if "listing_time" in value:
        ...
        out["ListingTime"] = aws_sdk_pinpoint_email.types.timestamp.serialize_json(
            value["listing_time"]
        )
...
def deserialize_json(data: dict) -> BlacklistEntry:
    ...
    if "ListingTime" in data:
        ...
        out["listing_time"] = aws_sdk_pinpoint_email.types.timestamp.deserialize_json(
            data["ListingTime"]
        )
