...
class DedicatedIp(TypedDict):
    ...
    warmup_percentage: (
        "aws_sdk_pinpoint_email.types.percentage100_wrapper.Percentage100Wrapper"
    )
    ...
    pool_name: NotRequired["aws_sdk_pinpoint_email.types.pool_name.PoolName"]
...
def serialize_json(value: DedicatedIp) -> dict:
    ...
    out["WarmupStatus"] = aws_sdk_pinpoint_email.types.warmup_status.serialize_json(
        value["warmup_status"]
    )
...
def deserialize_json(data: dict) -> DedicatedIp:
    ...
    if "WarmupStatus" in data:
        ...
        out["warmup_status"] = (
            aws_sdk_pinpoint_email.types.warmup_status.deserialize_json(
                data["WarmupStatus"]
            )
        )
    else:
