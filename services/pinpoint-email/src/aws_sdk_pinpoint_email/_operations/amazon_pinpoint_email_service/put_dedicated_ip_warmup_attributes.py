"""Generated from Smithy shape ``com.amazonaws.pinpointemail#PutDedicatedIpWarmupAttributes``."""
...
def handle_response(
    ...
    out: aws_sdk_pinpoint_email.types.put_dedicated_ip_warmup_attributes_response.PutDedicatedIpWarmupAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_pinpoint_email.types.put_dedicated_ip_warmup_attributes_request.PutDedicatedIpWarmupAttributesRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/email/dedicated-ips/{Ip}/warmup"
    url = url.replace("{Ip}", quote(str(input["ip"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_pinpoint_email.types.put_dedicated_ip_warmup_attributes_request

    body: bytes | None = json.dumps(
        aws_sdk_pinpoint_email.types.put_dedicated_ip_warmup_attributes_request.serialize_json(
            input
        )
    ).encode()
...
async def async_put_dedicated_ip_warmup_attributes(
