...
def build_request(
    ...
    input: aws_sdk_pinpoint_email.types.list_domain_deliverability_campaigns_request.ListDomainDeliverabilityCampaignsRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{SubscribedDomain}", quote(str(input["subscribed_domain"]), safe="")
    )
    params: dict[str, str] = {}
    if "start_date" in input:
        params["StartDate"] = str(input["start_date"])
    if "end_date" in input:
        params["EndDate"] = str(input["end_date"])
