"""Generated from Smithy shape ``com.amazonaws.migrationhubstrategy#ImportFileTaskInformation``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import aws_sdk_migrationhubstrategy.types.import_file_task_status
    import aws_sdk_migrationhubstrategy.types.import_s3_bucket
    import aws_sdk_migrationhubstrategy.types.import_s3_key
    import aws_sdk_migrationhubstrategy.types.integer
    import aws_sdk_migrationhubstrategy.types.string
    import aws_sdk_migrationhubstrategy.types.time_stamp


class ImportFileTaskInformation(TypedDict, closed=True):
    id: NotRequired["aws_sdk_migrationhubstrategy.types.string.String"]
    """<p> The ID of the import file task. </p>"""
    status: NotRequired[
        "aws_sdk_migrationhubstrategy.types.import_file_task_status.ImportFileTaskStatus"
    ]
    """<p> Status of import file task. </p>"""
    start_time: NotRequired["aws_sdk_migrationhubstrategy.types.time_stamp.TimeStamp"]
    """<p> Start time of the import task. </p>"""
    input_s3_bucket: NotRequired[
        "aws_sdk_migrationhubstrategy.types.import_s3_bucket.importS3Bucket"
    ]
    """<p> The S3 bucket where the import file is located. </p>"""
    input_s3_key: NotRequired[
        "aws_sdk_migrationhubstrategy.types.import_s3_key.importS3Key"
    ]
    """<p> The Amazon S3 key name of the import file. </p>"""
    status_report_s3_bucket: NotRequired[
        "aws_sdk_migrationhubstrategy.types.import_s3_bucket.importS3Bucket"
    ]
    """<p> The S3 bucket name for status report of import task. </p>"""
    status_report_s3_key: NotRequired[
        "aws_sdk_migrationhubstrategy.types.import_s3_key.importS3Key"
    ]
    """<p> The Amazon S3 key name for status report of import task. The report contains details about whether each record imported successfully or why it did not. </p>"""
    completion_time: NotRequired[
        "aws_sdk_migrationhubstrategy.types.time_stamp.TimeStamp"
    ]
    """<p> The time that the import task completes. </p>"""
    number_of_records_success: NotRequired[
        "aws_sdk_migrationhubstrategy.types.integer.Integer"
    ]
    """<p> The number of records successfully imported. </p>"""
    number_of_records_failed: NotRequired[
        "aws_sdk_migrationhubstrategy.types.integer.Integer"
    ]
    """<p> The number of records that failed to be imported. </p>"""
    import_name: NotRequired["aws_sdk_migrationhubstrategy.types.string.String"]
    """<p> The name of the import task given in <code>StartImportFileTask</code>. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ImportFileTaskInformation) -> dict:
    out: dict = {}
    if "id" in value:
        out["id"] = value["id"]
    if "status" in value:
        out["status"] = value["status"]
    if "start_time" in value:
        import aws_sdk_migrationhubstrategy.types.time_stamp

        out["startTime"] = aws_sdk_migrationhubstrategy.types.time_stamp.serialize_json(
            value["start_time"]
        )
    if "input_s3_bucket" in value:
        out["inputS3Bucket"] = value["input_s3_bucket"]
    if "input_s3_key" in value:
        out["inputS3Key"] = value["input_s3_key"]
    if "status_report_s3_bucket" in value:
        out["statusReportS3Bucket"] = value["status_report_s3_bucket"]
    if "status_report_s3_key" in value:
        out["statusReportS3Key"] = value["status_report_s3_key"]
    if "completion_time" in value:
        import aws_sdk_migrationhubstrategy.types.time_stamp

        out["completionTime"] = (
            aws_sdk_migrationhubstrategy.types.time_stamp.serialize_json(
                value["completion_time"]
            )
        )
    if "number_of_records_success" in value:
        out["numberOfRecordsSuccess"] = value["number_of_records_success"]
    if "number_of_records_failed" in value:
        out["numberOfRecordsFailed"] = value["number_of_records_failed"]
    if "import_name" in value:
        out["importName"] = value["import_name"]
    return out


def deserialize_json(data: dict) -> ImportFileTaskInformation:
    out: ImportFileTaskInformation = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    if "status" in data:
        out["status"] = data["status"]
    if "startTime" in data:
        import aws_sdk_migrationhubstrategy.types.time_stamp

        out["start_time"] = (
            aws_sdk_migrationhubstrategy.types.time_stamp.deserialize_json(
                data["startTime"]
            )
        )
    if "inputS3Bucket" in data:
        out["input_s3_bucket"] = data["inputS3Bucket"]
    if "inputS3Key" in data:
        out["input_s3_key"] = data["inputS3Key"]
    if "statusReportS3Bucket" in data:
        out["status_report_s3_bucket"] = data["statusReportS3Bucket"]
    if "statusReportS3Key" in data:
        out["status_report_s3_key"] = data["statusReportS3Key"]
    if "completionTime" in data:
        import aws_sdk_migrationhubstrategy.types.time_stamp

        out["completion_time"] = (
            aws_sdk_migrationhubstrategy.types.time_stamp.deserialize_json(
                data["completionTime"]
            )
        )
    if "numberOfRecordsSuccess" in data:
        out["number_of_records_success"] = data["numberOfRecordsSuccess"]
    if "numberOfRecordsFailed" in data:
        out["number_of_records_failed"] = data["numberOfRecordsFailed"]
    if "importName" in data:
        out["import_name"] = data["importName"]
    return out
