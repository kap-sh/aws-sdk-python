...
if TYPE_CHECKING:
    import aws_sdk_migrationhubstrategy.types.assessment_target_values
    import aws_sdk_migrationhubstrategy.types.condition
...
class AssessmentTarget(TypedDict, closed=True):
    ...
    values: "aws_sdk_migrationhubstrategy.types.assessment_target_values.AssessmentTargetValues"
...
def serialize_json(value: AssessmentTarget) -> dict:
    ...
    out["values"] = (
        aws_sdk_migrationhubstrategy.types.assessment_target_values.serialize_json(
            value["values"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> AssessmentTarget:
    ...
    if "values" in data:
        ...
        out["values"] = (
            aws_sdk_migrationhubstrategy.types.assessment_target_values.deserialize_json(
                data["values"]
            )
        )
    else:
