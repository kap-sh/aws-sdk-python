...
class GetServerDetailsResponse(TypedDict, closed=True):
    next_token: NotRequired["capo_migrationhubstrategy.types.string.String"]
...
def serialize_json(value: GetServerDetailsResponse) -> dict:
    ...
    if "server_detail" in value:
        ...
        out["serverDetail"] = (
            capo_migrationhubstrategy.types.server_detail.serialize_json(
                value["server_detail"]
            )
        )
...
def deserialize_json(data: dict) -> GetServerDetailsResponse:
    ...
    if "serverDetail" in data:
        ...
        out["server_detail"] = (
            capo_migrationhubstrategy.types.server_detail.deserialize_json(
                data["serverDetail"]
            )
        )
