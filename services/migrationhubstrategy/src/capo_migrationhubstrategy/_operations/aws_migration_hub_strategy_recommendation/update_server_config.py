"""Generated from Smithy shape ``com.amazonaws.migrationhubstrategy#UpdateServerConfig``."""
...
import capo_migrationhubstrategy.types.update_server_config_request
import capo_migrationhubstrategy.types.update_server_config_response
...
) -> capo_migrationhubstrategy.types.update_server_config_response.UpdateServerConfigResponse:
    out: capo_migrationhubstrategy.types.update_server_config_response.UpdateServerConfigResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-server-config/"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_migrationhubstrategy.types.update_server_config_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_server_config(
