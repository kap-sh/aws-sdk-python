"""Generated from Smithy shape ``com.amazonaws.migrationhubstrategy#UpdateApplicationComponentConfig``."""
...
import capo_migrationhubstrategy.types.update_application_component_config_request
import capo_migrationhubstrategy.types.update_application_component_config_response
...
) -> capo_migrationhubstrategy.types.update_application_component_config_response.UpdateApplicationComponentConfigResponse:
    out: capo_migrationhubstrategy.types.update_application_component_config_response.UpdateApplicationComponentConfigResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-applicationcomponent-config/"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_migrationhubstrategy.types.update_application_component_config_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_application_component_config(
