...
class CalculateIsolinesResponse(TypedDict):
    ...
    isoline_geometry_format: "aws_sdk_geo_routes.types.geometry_format.GeometryFormat"
    ...
    isolines: "aws_sdk_geo_routes.types.isoline_list.IsolineList"
    ...
    snapped_destination: NotRequired["aws_sdk_geo_routes.types.position.Position"]
    ...
    snapped_origin: NotRequired["aws_sdk_geo_routes.types.position.Position"]
...
def serialize_json(value: CalculateIsolinesResponse) -> dict:
    ...
    out["Isolines"] = aws_sdk_geo_routes.types.isoline_list.serialize_json(
        value["isolines"]
    )
    if "snapped_destination" in value:
        import aws_sdk_geo_routes.types.position

        out["SnappedDestination"] = aws_sdk_geo_routes.types.position.serialize_json(
            value["snapped_destination"]
        )
    if "snapped_origin" in value:
        import aws_sdk_geo_routes.types.position

        out["SnappedOrigin"] = aws_sdk_geo_routes.types.position.serialize_json(
...
def deserialize_json(data: dict) -> CalculateIsolinesResponse:
    ...
    if "Isolines" in data:
        ...
        out["isolines"] = aws_sdk_geo_routes.types.isoline_list.deserialize_json(
            data["Isolines"]
        )
    else:
    ...
    if "SnappedDestination" in data:
        ...
        out["snapped_destination"] = aws_sdk_geo_routes.types.position.deserialize_json(
            data["SnappedDestination"]
        )
    if "SnappedOrigin" in data:
        import aws_sdk_geo_routes.types.position

        out["snapped_origin"] = aws_sdk_geo_routes.types.position.deserialize_json(
            data["SnappedOrigin"]
        )
    return out
