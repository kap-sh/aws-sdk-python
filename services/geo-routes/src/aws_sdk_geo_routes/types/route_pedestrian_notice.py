...
if TYPE_CHECKING:
    ...
    import aws_sdk_geo_routes.types.route_pedestrian_notice_code


class RoutePedestrianNotice(TypedDict):
    code: "aws_sdk_geo_routes.types.route_pedestrian_notice_code.RoutePedestrianNoticeCode"
...
def serialize_json(value: RoutePedestrianNotice) -> dict:
    ...
    out["Code"] = aws_sdk_geo_routes.types.route_pedestrian_notice_code.serialize_json(
        value["code"]
    )
    if "impact" in value:
        import aws_sdk_geo_routes.types.route_notice_impact
...
def deserialize_json(data: dict) -> RoutePedestrianNotice:
    ...
    if "Code" in data:
        ...
        out["code"] = (
            aws_sdk_geo_routes.types.route_pedestrian_notice_code.deserialize_json(
                data["Code"]
            )
        )
    else:
