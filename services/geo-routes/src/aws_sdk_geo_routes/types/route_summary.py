...
class RouteSummary(TypedDict, closed=True):
    ...
    tolls: NotRequired["aws_sdk_geo_routes.types.route_toll_summary.RouteTollSummary"]
...
def serialize_json(value: RouteSummary) -> dict:
    ...
    if "tolls" in value:
        ...
        out["Tolls"] = aws_sdk_geo_routes.types.route_toll_summary.serialize_json(
            value["tolls"]
        )
    return out
...
def deserialize_json(data: dict) -> RouteSummary:
    ...
    if "Tolls" in data:
        ...
        out["tolls"] = aws_sdk_geo_routes.types.route_toll_summary.deserialize_json(
            data["Tolls"]
        )
    return out
