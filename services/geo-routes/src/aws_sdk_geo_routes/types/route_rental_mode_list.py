...
RouteRentalModeList: TypeAlias = list[
    "aws_sdk_geo_routes.types.route_rental_mode.RouteRentalMode"
]
...
def serialize_json(value: RouteRentalModeList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_geo_routes.types.route_rental_mode.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> RouteRentalModeList:
    ...
    for item in data:
        out.append(aws_sdk_geo_routes.types.route_rental_mode.deserialize_json(item))
    return out
