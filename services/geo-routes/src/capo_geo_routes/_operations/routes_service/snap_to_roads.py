"""Generated from Smithy shape ``com.amazonaws.georoutes#SnapToRoads``."""
...
def handle_response(
    ...
    out: capo_geo_routes.types.snap_to_roads_response.SnapToRoadsResponse = (
        capo_geo_routes.types.snap_to_roads_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/snap-to-roads"
    params: dict[str, str] = {}
    if "key" in input_:
        params["key"] = str(input_["key"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_geo_routes.types.snap_to_roads_request.serialize_json(input_)
    ).encode()
...
def snap_to_roads(
    ...
    capo_geo_routes.types.snap_to_roads_response.SnapToRoadsResponse, zapros.Response
]:
