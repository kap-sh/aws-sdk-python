"""Generated from Smithy shape ``com.amazonaws.georoutes#CalculateIsolines``."""
...
def handle_response(
    ...
    out: capo_geo_routes.types.calculate_isolines_response.CalculateIsolinesResponse = (
        capo_geo_routes.types.calculate_isolines_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/isolines"
    params: dict[str, str] = {}
    if "key" in input_:
        params["key"] = str(input_["key"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_geo_routes.types.calculate_isolines_request.serialize_json(input_)
    ).encode()
...
async def async_calculate_isolines(
