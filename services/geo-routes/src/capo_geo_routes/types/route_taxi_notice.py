...
class RouteTaxiNotice(TypedDict, closed=True):
    code: "capo_geo_routes.types.route_taxi_notice_code.RouteTaxiNoticeCode"
...
def serialize_json(value: RouteTaxiNotice) -> dict:
    ...
    out["Code"] = capo_geo_routes.types.route_taxi_notice_code.serialize_json(
        value["code"]
    )
    if "impact" in value:
        import capo_geo_routes.types.route_notice_impact
