...
class RouteIntermodalTaxiOptions(TypedDict, closed=True):
    allowed_modes: NotRequired[
        "capo_geo_routes.types.route_taxi_mode_list.RouteTaxiModeList"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: RouteIntermodalTaxiOptions) -> dict:
    out: dict = {}
    if "allowed_modes" in value:
        import capo_geo_routes.types.route_taxi_mode_list

        out["AllowedModes"] = capo_geo_routes.types.route_taxi_mode_list.serialize_json(
            value["allowed_modes"]
        )
    ...
    if "excluded_modes" in value:
        ...
        out["ExcludedModes"] = (
            capo_geo_routes.types.route_taxi_mode_list.serialize_json(
...
def deserialize_json(data: dict) -> RouteIntermodalTaxiOptions:
    ...
    if "AllowedModes" in data:
        ...
        out["allowed_modes"] = (
            capo_geo_routes.types.route_taxi_mode_list.deserialize_json(
                data["AllowedModes"]
            )
        )
