...
if TYPE_CHECKING:
    ...
    import capo_geo_routes.types.route_taxi_place_type
...
class RouteTaxiPlace(TypedDict, closed=True):
    ...
    type: NotRequired["capo_geo_routes.types.route_taxi_place_type.RouteTaxiPlaceType"]
...
def serialize_json(value: RouteTaxiPlace) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = capo_geo_routes.types.route_taxi_place_type.serialize_json(
            value["type"]
        )
...
def deserialize_json(data: dict) -> RouteTaxiPlace:
    ...
    if "Type" in data:
        ...
        out["type"] = capo_geo_routes.types.route_taxi_place_type.deserialize_json(
            data["Type"]
        )
