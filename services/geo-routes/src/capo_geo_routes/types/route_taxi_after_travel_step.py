...
if TYPE_CHECKING:
    ...
    import capo_geo_routes.types.route_taxi_after_travel_step_type
    import capo_geo_routes.types.sensitive_string


class RouteTaxiAfterTravelStep(TypedDict, closed=True):
    ...
    type: "capo_geo_routes.types.route_taxi_after_travel_step_type.RouteTaxiAfterTravelStepType"
...
def serialize_json(value: RouteTaxiAfterTravelStep) -> dict:
    ...
    out["Type"] = (
        capo_geo_routes.types.route_taxi_after_travel_step_type.serialize_json(
            value["type"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> RouteTaxiAfterTravelStep:
    ...
    if "Type" in data:
        ...
        out["type"] = (
            capo_geo_routes.types.route_taxi_after_travel_step_type.deserialize_json(
                data["Type"]
            )
        )
    else:
