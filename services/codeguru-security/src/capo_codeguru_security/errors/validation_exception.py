...
class ValidationException_(TypedDict, closed=True):
    ...
    reason: "capo_codeguru_security.types.validation_exception_reason.ValidationExceptionReason"
    """<p>The reason the request failed validation.</p>"""
    field_list: NotRequired[
        "capo_codeguru_security.types.validation_exception_field_list.ValidationExceptionFieldList"
    ]
...
def serialize_json(value: ValidationException_) -> dict:
    ...
    out["reason"] = (
        capo_codeguru_security.types.validation_exception_reason.serialize_json(
            value["reason"]
        )
    )
    if "field_list" in value:
        ...
        out["fieldList"] = (
            capo_codeguru_security.types.validation_exception_field_list.serialize_json(
                value["field_list"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ValidationException_:
    ...
    if "reason" in data:
        ...
        out["reason"] = (
            capo_codeguru_security.types.validation_exception_reason.deserialize_json(
                data["reason"]
            )
        )
    else:
    ...
    if "fieldList" in data:
        ...
        out["field_list"] = (
            capo_codeguru_security.types.validation_exception_field_list.deserialize_json(
                data["fieldList"]
            )
        )
    return out
