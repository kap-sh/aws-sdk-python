...
if TYPE_CHECKING:
    import capo_mgn.types.describe_jobs_request_filters_job_i_ds
    import capo_mgn.types.iso8601_datetime_string


class DescribeJobsRequestFilters(TypedDict, closed=True):
    job_i_ds: NotRequired[
        "capo_mgn.types.describe_jobs_request_filters_job_i_ds.DescribeJobsRequestFiltersJobIDs"
    ]
    ...
    to_date: NotRequired["capo_mgn.types.iso8601_datetime_string.ISO8601DatetimeString"]
...
def serialize_json(value: DescribeJobsRequestFilters) -> dict:
    ...
    if "job_i_ds" in value:
        ...
        out["jobIDs"] = (
            capo_mgn.types.describe_jobs_request_filters_job_i_ds.serialize_json(
                value["job_i_ds"]
            )
        )
...
def deserialize_json(data: dict) -> DescribeJobsRequestFilters:
    ...
    if "jobIDs" in data:
        ...
        out["job_i_ds"] = (
            capo_mgn.types.describe_jobs_request_filters_job_i_ds.deserialize_json(
                data["jobIDs"]
            )
        )
    if "fromDate" in data:
