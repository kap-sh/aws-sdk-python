"""Generated from Smithy shape ``com.amazonaws.mgn#DisconnectFromServiceRequest``."""
...
class DisconnectFromServiceRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: DisconnectFromServiceRequest) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> DisconnectFromServiceRequest:
    out: DisconnectFromServiceRequest = {}  # type: ignore[typeddict-item]
    if "sourceServerID" in data:
        out["source_server_id"] = data["sourceServerID"]
    else:
        raise DeserializationError(
            "DisconnectFromServiceRequest.source_server_id required"
        )
