...
def serialize_json(value: ListNetworkMigrationCodeGenerationSegmentsFilters) -> dict:
    ...
    if "segment_i_ds" in value:
        ...
        out["segmentIDs"] = (
            capo_mgn.types.list_network_migration_code_generation_segments_i_ds_filter.serialize_json(
                value["segment_i_ds"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListNetworkMigrationCodeGenerationSegmentsFilters:
    ...
    if "segmentIDs" in data:
        ...
        out["segment_i_ds"] = (
            capo_mgn.types.list_network_migration_code_generation_segments_i_ds_filter.deserialize_json(
                data["segmentIDs"]
            )
        )
    return out
