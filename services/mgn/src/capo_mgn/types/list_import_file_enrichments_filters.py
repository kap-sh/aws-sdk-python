...
class ListImportFileEnrichmentsFilters(TypedDict, closed=True):
    job_i_ds: NotRequired[
        "capo_mgn.types.import_file_enrichments_i_ds_filter.ImportFileEnrichmentsIDsFilter"
    ]
...
def serialize_json(value: ListImportFileEnrichmentsFilters) -> dict:
    ...
    if "job_i_ds" in value:
        ...
        out["jobIDs"] = (
            capo_mgn.types.import_file_enrichments_i_ds_filter.serialize_json(
                value["job_i_ds"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListImportFileEnrichmentsFilters:
    ...
    if "jobIDs" in data:
        ...
        out["job_i_ds"] = (
            capo_mgn.types.import_file_enrichments_i_ds_filter.deserialize_json(
                data["jobIDs"]
            )
        )
    return out
