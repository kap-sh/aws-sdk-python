...
def serialize_json(value: ListNetworkMigrationDefinitionsRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_mgn.types.list_network_migration_definitions_request_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationDefinitionsRequest:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            capo_mgn.types.list_network_migration_definitions_request_filters.deserialize_json(
                data["filters"]
            )
        )
