...
class ListNetworkMigrationDeploymentsRequest(TypedDict, closed=True):
...
def serialize_json(value: ListNetworkMigrationDeploymentsRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_mgn.types.list_network_migration_deployer_job_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationDeploymentsRequest:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            capo_mgn.types.list_network_migration_deployer_job_filters.deserialize_json(
                data["filters"]
            )
        )
