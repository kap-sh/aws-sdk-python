...
if TYPE_CHECKING:
    ...
    import capo_mgn.types.post_launch_action_execution_status
    import capo_mgn.types.ssm_document
    import capo_mgn.types.ssm_document_type
...
class JobPostLaunchActionsLaunchStatus(TypedDict, closed=True):
    ...
    failure_reason: NotRequired["capo_mgn.types.bounded_string.BoundedString"]
...
def serialize_json(value: JobPostLaunchActionsLaunchStatus) -> dict:
    ...
    if "ssm_document" in value:
        ...
        out["ssmDocument"] = capo_mgn.types.ssm_document.serialize_json(
            value["ssm_document"]
        )
