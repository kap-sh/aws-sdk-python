"""Generated from Smithy shape ``com.amazonaws.mgn#ListNetworkMigrationCodeGenerationSegmentsRequest``."""
...
class ListNetworkMigrationCodeGenerationSegmentsRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: ListNetworkMigrationCodeGenerationSegmentsRequest) -> dict:
    out: dict = {}
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_mgn.types.list_network_migration_code_generation_segments_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationCodeGenerationSegmentsRequest:
    out: ListNetworkMigrationCodeGenerationSegmentsRequest = {}  # type: ignore[typeddict-item]
    if "networkMigrationExecutionID" in data:
        out["network_migration_execution_id"] = data["networkMigrationExecutionID"]
    else:
        raise DeserializationError(
            "ListNetworkMigrationCodeGenerationSegmentsRequest.network_migration_execution_id required"
        )
    if "networkMigrationDefinitionID" in data:
        out["network_migration_definition_id"] = data["networkMigrationDefinitionID"]
    else:
        raise DeserializationError(
            "ListNetworkMigrationCodeGenerationSegmentsRequest.network_migration_definition_id required"
        )
    if "filters" in data:
        import capo_mgn.types.list_network_migration_code_generation_segments_filters

        out["filters"] = (
            capo_mgn.types.list_network_migration_code_generation_segments_filters.deserialize_json(
                data["filters"]
            )
        )
