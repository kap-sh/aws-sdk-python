...
if TYPE_CHECKING:
    ...
    import capo_mgn.types.job_log_event
    import capo_mgn.types.job_log_event_data
...
class JobLog(TypedDict, closed=True):
    ...
    event: NotRequired["capo_mgn.types.job_log_event.JobLogEvent"]
    """<p>Job log event.</p>"""
    event_data: NotRequired["capo_mgn.types.job_log_event_data.JobLogEventData"]
    """<p>Job event data</p>"""
...
def serialize_json(value: JobLog) -> dict:
    ...
    if "event_data" in value:
        ...
        out["eventData"] = capo_mgn.types.job_log_event_data.serialize_json(
            value["event_data"]
        )
    return out
...
def deserialize_json(data: dict) -> JobLog:
    ...
    if "eventData" in data:
        ...
        out["event_data"] = capo_mgn.types.job_log_event_data.deserialize_json(
            data["eventData"]
        )
    return out
