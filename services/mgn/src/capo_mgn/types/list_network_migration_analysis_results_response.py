...
class ListNetworkMigrationAnalysisResultsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_mgn.types.network_migration_analysis_results_list.NetworkMigrationAnalysisResultsList"
    ]
...
def serialize_json(value: ListNetworkMigrationAnalysisResultsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_mgn.types.network_migration_analysis_results_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationAnalysisResultsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            capo_mgn.types.network_migration_analysis_results_list.deserialize_json(
                data["items"]
            )
        )
