...
class ListImportFileEnrichmentsRequest(TypedDict, closed=True):
    filters: NotRequired[
...
def serialize_json(value: ListImportFileEnrichmentsRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_mgn.types.list_import_file_enrichments_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListImportFileEnrichmentsRequest:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            capo_mgn.types.list_import_file_enrichments_filters.deserialize_json(
                data["filters"]
            )
        )
