"""Generated from Smithy shape ``com.amazonaws.mgn#ListConnectorsResponse``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_mgn.types.connectors_list
    import capo_mgn.types.pagination_token


class ListConnectorsResponse(TypedDict, closed=True):
    items: NotRequired["capo_mgn.types.connectors_list.ConnectorsList"]
    """<p>List connectors response items.</p>"""
    next_token: NotRequired["capo_mgn.types.pagination_token.PaginationToken"]
    """<p>List connectors response next token.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListConnectorsResponse) -> dict:
    out: dict = {}
    if "items" in value:
        import capo_mgn.types.connectors_list

        out["items"] = capo_mgn.types.connectors_list.serialize_json(value["items"])
    if "next_token" in value:
        out["nextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListConnectorsResponse:
    out: ListConnectorsResponse = {}  # type: ignore[typeddict-item]
    if "items" in data:
        import capo_mgn.types.connectors_list

        out["items"] = capo_mgn.types.connectors_list.deserialize_json(data["items"])
    if "nextToken" in data:
        out["next_token"] = data["nextToken"]
    return out
