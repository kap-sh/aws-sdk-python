...
if TYPE_CHECKING:
    ...
    import capo_mgn.types.import_error_type
...
class ImportTaskError(TypedDict, closed=True):
    ...
    error_type: NotRequired["capo_mgn.types.import_error_type.ImportErrorType"]
    """<p>Import task error type.</p>"""
    error_data: NotRequired["capo_mgn.types.import_error_data.ImportErrorData"]
...
def serialize_json(value: ImportTaskError) -> dict:
    ...
    if "error_data" in value:
        ...
        out["errorData"] = capo_mgn.types.import_error_data.serialize_json(
            value["error_data"]
        )
    return out
