"""Generated from Smithy shape ``com.amazonaws.mgn#MarkAsArchived``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/MarkAsArchived"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.mark_as_archived_request.serialize_json(input_)
    ).encode()
...
async def async_mark_as_archived(
