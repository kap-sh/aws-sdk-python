...
import capo_mgn.types.network_migration_definition_summaries_list
...
def handle_response(
    ...
    out: capo_mgn.types.list_network_migration_definitions_response.ListNetworkMigrationDefinitionsResponse = capo_mgn.types.list_network_migration_definitions_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/network-migration/ListNetworkMigrationDefinitions"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.list_network_migration_definitions_request.serialize_json(input_)
    ).encode()
...
async def async_list_network_migration_definitions(
