"""Generated from Smithy shape ``com.amazonaws.mgn#DeleteNetworkMigrationDefinition``."""
...
def handle_response(
    ...
    out: capo_mgn.types.delete_network_migration_definition_response.DeleteNetworkMigrationDefinitionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/network-migration/DeleteNetworkMigrationDefinition"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.delete_network_migration_definition_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_network_migration_definition(
