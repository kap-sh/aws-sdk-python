...
def handle_response(response: zapros.Response) -> capo_mgn.types.wave.Wave:
    out: capo_mgn.types.wave.Wave = capo_mgn.types.wave.deserialize_json(
        json.loads(response.read())
    )
    return out


async def async_handle_response(response: zapros.Response) -> capo_mgn.types.wave.Wave:
    out: capo_mgn.types.wave.Wave = capo_mgn.types.wave.deserialize_json(
...
def build_request(
    ...
    input_: capo_mgn.types.create_wave_request.CreateWaveRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        capo_mgn.types.create_wave_request.serialize_json(input_)
    ).encode()
...
def create_wave(
    ...
    input_: capo_mgn.types.create_wave_request.CreateWaveRequest,
) -> tuple[capo_mgn.types.wave.Wave, zapros.Response]:
