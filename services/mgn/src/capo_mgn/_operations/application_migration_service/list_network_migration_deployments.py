"""Generated from Smithy shape ``com.amazonaws.mgn#ListNetworkMigrationDeployments``."""
...
def handle_response(
    ...
    out: capo_mgn.types.list_network_migration_deployer_job_response.ListNetworkMigrationDeployerJobResponse = capo_mgn.types.list_network_migration_deployer_job_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/network-migration/ListNetworkMigrationDeployments"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.list_network_migration_deployments_request.serialize_json(input_)
    ).encode()
...
async def async_list_network_migration_deployments(
