"""Generated from Smithy shape ``com.amazonaws.mgn#TerminateTargetInstances``."""
...
def handle_response(
    ...
    out: capo_mgn.types.terminate_target_instances_response.TerminateTargetInstancesResponse = capo_mgn.types.terminate_target_instances_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/TerminateTargetInstances"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.terminate_target_instances_request.serialize_json(input_)
    ).encode()
...
async def async_terminate_target_instances(
