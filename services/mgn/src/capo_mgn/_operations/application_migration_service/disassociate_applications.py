"""Generated from Smithy shape ``com.amazonaws.mgn#DisassociateApplications``."""
...
def handle_response(
    ...
    out: capo_mgn.types.disassociate_applications_response.DisassociateApplicationsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DisassociateApplications"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.disassociate_applications_request.serialize_json(input_)
    ).encode()
