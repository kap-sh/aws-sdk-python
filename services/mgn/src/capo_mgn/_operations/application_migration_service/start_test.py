"""Generated from Smithy shape ``com.amazonaws.mgn#StartTest``."""
...
def handle_response(
    ...
    out: capo_mgn.types.start_test_response.StartTestResponse = (
        capo_mgn.types.start_test_response.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_mgn.types.start_test_response.StartTestResponse = (
        capo_mgn.types.start_test_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartTest"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.start_test_request.serialize_json(input_)
    ).encode()
...
def start_test(
    ...
    input_: capo_mgn.types.start_test_request.StartTestRequest,
) -> tuple[capo_mgn.types.start_test_response.StartTestResponse, zapros.Response]:
