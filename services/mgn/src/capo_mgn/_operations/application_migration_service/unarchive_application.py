"""Generated from Smithy shape ``com.amazonaws.mgn#UnarchiveApplication``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UnarchiveApplication"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.unarchive_application_request.serialize_json(input_)
    ).encode()
...
async def async_unarchive_application(
