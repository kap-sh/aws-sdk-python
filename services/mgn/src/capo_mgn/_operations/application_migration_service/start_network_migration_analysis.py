...
import capo_mgn.errors.throttling_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            raise capo_mgn.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
        case "ConflictException":
            raise capo_mgn.errors.conflict_exception.ConflictException.from_json(data)
        case "ResourceNotFoundException":
            raise capo_mgn.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
        case "ServiceQuotaExceededException":
            raise capo_mgn.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
        case "ThrottlingException":
            raise capo_mgn.errors.throttling_exception.ThrottlingException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_mgn.types.start_network_migration_analysis_response.StartNetworkMigrationAnalysisResponse = capo_mgn.types.start_network_migration_analysis_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_mgn._auth._sigv4.build_sigv4_auth_scheme("mgn", options.region)
        )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_mgn.types.start_network_migration_analysis_request.serialize_json(input_)
    ).encode()
...
async def async_start_network_migration_analysis(
