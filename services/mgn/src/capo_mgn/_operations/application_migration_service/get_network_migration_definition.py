"""Generated from Smithy shape ``com.amazonaws.mgn#GetNetworkMigrationDefinition``."""
...
def handle_response(
    ...
    out: capo_mgn.types.network_migration_definition.NetworkMigrationDefinition = (
        capo_mgn.types.network_migration_definition.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/network-migration/GetNetworkMigrationDefinition"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.get_network_migration_definition_request.serialize_json(input_)
    ).encode()
...
async def async_get_network_migration_definition(
