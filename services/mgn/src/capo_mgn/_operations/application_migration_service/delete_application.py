"""Generated from Smithy shape ``com.amazonaws.mgn#DeleteApplication``."""
...
def handle_response(
    ...
    out: capo_mgn.types.delete_application_response.DeleteApplicationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteApplication"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.delete_application_request.serialize_json(input_)
    ).encode()
