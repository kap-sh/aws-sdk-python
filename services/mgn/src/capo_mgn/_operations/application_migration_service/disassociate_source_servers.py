"""Generated from Smithy shape ``com.amazonaws.mgn#DisassociateSourceServers``."""
...
def handle_response(
    ...
    out: capo_mgn.types.disassociate_source_servers_response.DisassociateSourceServersResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DisassociateSourceServers"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mgn.types.disassociate_source_servers_request.serialize_json(input_)
    ).encode()
...
async def async_disassociate_source_servers(
