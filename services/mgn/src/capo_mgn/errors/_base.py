...
class mgnError(Exception):
    """Base class for all mgn service errors."""


class ServiceError(mgnError):
...
class SerializationError(mgnError):
    """Raised when a value cannot be serialized."""


class DeserializationError(mgnError):
...
class WaiterFailedError(mgnError):
...
class WaiterTimeoutError(mgnError):
