...
def handle_response(response: zapros.Response) -> aws_sdk_mgn.types.connector.Connector:
    out: aws_sdk_mgn.types.connector.Connector = (
        aws_sdk_mgn.types.connector.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: aws_sdk_mgn.types.connector.Connector = (
        aws_sdk_mgn.types.connector.deserialize_json(json.loads(await response.aread()))
    )
    return out
...
def build_request(
    ...
    input_: aws_sdk_mgn.types.create_connector_request.CreateConnectorRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_mgn.types.create_connector_request.serialize_json(input_)
    ).encode()
...
def create_connector(
    ...
    input_: aws_sdk_mgn.types.create_connector_request.CreateConnectorRequest,
) -> tuple[aws_sdk_mgn.types.connector.Connector, zapros.Response]:
