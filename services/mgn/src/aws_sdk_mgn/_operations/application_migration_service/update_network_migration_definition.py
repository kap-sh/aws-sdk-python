...
def build_request(
    ...
    input_: aws_sdk_mgn.types.update_network_migration_definition_request.UpdateNetworkMigrationDefinitionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/network-migration/UpdateNetworkMigrationDefinition"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_mgn.types.update_network_migration_definition_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_network_migration_definition(
