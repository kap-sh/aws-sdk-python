...
class ListNetworkMigrationExecutionsResponse(TypedDict):
    items: NotRequired[
        "aws_sdk_mgn.types.network_migration_executions_list.NetworkMigrationExecutionsList"
    ]
...
def serialize_json(value: ListNetworkMigrationExecutionsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            aws_sdk_mgn.types.network_migration_executions_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationExecutionsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            aws_sdk_mgn.types.network_migration_executions_list.deserialize_json(
                data["items"]
            )
        )
