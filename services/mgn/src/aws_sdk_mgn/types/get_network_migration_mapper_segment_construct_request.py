...
def deserialize_json(data: dict) -> GetNetworkMigrationMapperSegmentConstructRequest:
    ...
    if "networkMigrationDefinitionID" in data:
        ...
        raise DeserializationError("GetNetworkMigrationMapperSegmentConstructRequest.network_migration_definition_id required")
    if "networkMigrationExecutionID" in data:
        out["network_migration_execution_id"] = data["networkMigrationExecutionID"]
    else:
        raise DeserializationError("GetNetworkMigrationMapperSegmentConstructRequest.network_migration_execution_id required")
    if "segmentID" in data:
        out["segment_id"] = data["segmentID"]
    else:
        raise DeserializationError("GetNetworkMigrationMapperSegmentConstructRequest.segment_id required")
    if "constructID" in data:
        out["construct_id"] = data["constructID"]
    else:
        raise DeserializationError("GetNetworkMigrationMapperSegmentConstructRequest.construct_id required")
    return out