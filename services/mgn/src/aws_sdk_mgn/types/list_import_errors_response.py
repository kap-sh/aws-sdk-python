...
class ListImportErrorsResponse(TypedDict):
    items: NotRequired["aws_sdk_mgn.types.import_errors.ImportErrors"]
...
def serialize_json(value: ListImportErrorsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = aws_sdk_mgn.types.import_errors.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListImportErrorsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = aws_sdk_mgn.types.import_errors.deserialize_json(data["items"])
