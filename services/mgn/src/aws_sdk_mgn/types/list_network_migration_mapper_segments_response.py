...
class ListNetworkMigrationMapperSegmentsResponse(TypedDict):
    items: NotRequired[
        "aws_sdk_mgn.types.network_migration_mapper_segments_list.NetworkMigrationMapperSegmentsList"
    ]
...
def serialize_json(value: ListNetworkMigrationMapperSegmentsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            aws_sdk_mgn.types.network_migration_mapper_segments_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationMapperSegmentsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            aws_sdk_mgn.types.network_migration_mapper_segments_list.deserialize_json(
                data["items"]
            )
        )
