"""Generated from Smithy shape ``com.amazonaws.mgn#VcenterClientList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_mgn.types.vcenter_client

VcenterClientList: TypeAlias = list["aws_sdk_mgn.types.vcenter_client.VcenterClient"]


# --- restJson1 ser/de ---
def serialize_json(value: VcenterClientList) -> list:
    import aws_sdk_mgn.types.vcenter_client

    out: list = []
    for item in value:
        out.append(aws_sdk_mgn.types.vcenter_client.serialize_json(item))
    return out


def deserialize_json(data: list) -> VcenterClientList:
    import aws_sdk_mgn.types.vcenter_client

    out: VcenterClientList = []
    for item in data:
        out.append(aws_sdk_mgn.types.vcenter_client.deserialize_json(item))
    return out
