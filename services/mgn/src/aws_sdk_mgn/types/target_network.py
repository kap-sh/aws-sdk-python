...
class TargetNetwork(TypedDict, closed=True):
    topology: "aws_sdk_mgn.types.target_network_topology.TargetNetworkTopology"
    """<p>The network topology type for the target environment.</p>"""
    inbound_cidr: NotRequired["aws_sdk_mgn.types.cidr.Cidr"]
    """<p>The CIDR block for inbound traffic in the target network.</p>"""
    outbound_cidr: NotRequired["aws_sdk_mgn.types.cidr.Cidr"]
    """<p>The CIDR block for outbound traffic in the target network.</p>"""
    inspection_cidr: NotRequired["aws_sdk_mgn.types.cidr.Cidr"]
