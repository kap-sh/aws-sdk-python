...
NetworkMigrationAnalysesList: TypeAlias = list[
    "aws_sdk_mgn.types.network_migration_analysis_job_details.NetworkMigrationAnalysisJobDetails"
]
...
def serialize_json(value: NetworkMigrationAnalysesList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_mgn.types.network_migration_analysis_job_details.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> NetworkMigrationAnalysesList:
    ...
    for item in data:
        out.append(
            aws_sdk_mgn.types.network_migration_analysis_job_details.deserialize_json(
                item
            )
        )
    return out
