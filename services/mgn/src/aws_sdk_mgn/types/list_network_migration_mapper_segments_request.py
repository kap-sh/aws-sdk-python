"""Generated from Smithy shape ``com.amazonaws.mgn#ListNetworkMigrationMapperSegmentsRequest``."""
...
class ListNetworkMigrationMapperSegmentsRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: ListNetworkMigrationMapperSegmentsRequest) -> dict:
    out: dict = {}
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            aws_sdk_mgn.types.list_network_migration_mapper_segments_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListNetworkMigrationMapperSegmentsRequest:
    out: ListNetworkMigrationMapperSegmentsRequest = {}  # type: ignore[typeddict-item]
    if "networkMigrationExecutionID" in data:
        out["network_migration_execution_id"] = data["networkMigrationExecutionID"]
    else:
        raise DeserializationError(
            "ListNetworkMigrationMapperSegmentsRequest.network_migration_execution_id required"
        )
    if "networkMigrationDefinitionID" in data:
        out["network_migration_definition_id"] = data["networkMigrationDefinitionID"]
    else:
        raise DeserializationError(
            "ListNetworkMigrationMapperSegmentsRequest.network_migration_definition_id required"
        )
    if "filters" in data:
        import aws_sdk_mgn.types.list_network_migration_mapper_segments_filters

        out["filters"] = (
            aws_sdk_mgn.types.list_network_migration_mapper_segments_filters.deserialize_json(
                data["filters"]
            )
        )
