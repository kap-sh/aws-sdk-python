"""Generated from Smithy shape ``com.amazonaws.mgn#NetworkMigrationDeployerJobDetails``."""
...
class NetworkMigrationDeployerJobDetails(TypedDict, closed=True):
    job_id: NotRequired[
...
# --- restJson1 ser/de ---
def serialize_json(value: NetworkMigrationDeployerJobDetails) -> dict:
    out: dict = {}
    if "job_id" in value:
...
def deserialize_json(data: dict) -> NetworkMigrationDeployerJobDetails:
    out: NetworkMigrationDeployerJobDetails = {}  # type: ignore[typeddict-item]
    if "jobID" in data:
