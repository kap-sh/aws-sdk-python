...
SourceServersList: TypeAlias = list["aws_sdk_mgn.types.source_server.SourceServer"]
...
def serialize_json(value: SourceServersList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_mgn.types.source_server.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> SourceServersList:
    ...
    for item in data:
        out.append(aws_sdk_mgn.types.source_server.deserialize_json(item))
    return out
