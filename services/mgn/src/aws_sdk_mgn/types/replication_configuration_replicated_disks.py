...
if TYPE_CHECKING:
    import aws_sdk_mgn.types.replication_configuration_replicated_disk

ReplicationConfigurationReplicatedDisks: TypeAlias = list[
    "aws_sdk_mgn.types.replication_configuration_replicated_disk.ReplicationConfigurationReplicatedDisk"
]
...
def serialize_json(value: ReplicationConfigurationReplicatedDisks) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_mgn.types.replication_configuration_replicated_disk.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ReplicationConfigurationReplicatedDisks:
    ...
    for item in data:
        out.append(
            aws_sdk_mgn.types.replication_configuration_replicated_disk.deserialize_json(
                item
            )
        )
    return out
