"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#UpdateCampaignFlowAssociation``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/campaigns/{id}/flow"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connectcampaignsv2.types.update_campaign_flow_association_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_campaign_flow_association(
