"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#ListCampaigns``."""
...
def handle_response(
    ...
    out: capo_connectcampaignsv2.types.list_campaigns_response.ListCampaignsResponse = (
        capo_connectcampaignsv2.types.list_campaigns_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/campaigns-summary"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connectcampaignsv2.types.list_campaigns_request.serialize_json(input_)
    ).encode()
