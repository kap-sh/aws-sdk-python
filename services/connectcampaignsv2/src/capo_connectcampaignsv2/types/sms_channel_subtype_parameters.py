"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#SmsChannelSubtypeParameters``."""
...
class SmsChannelSubtypeParameters(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: SmsChannelSubtypeParameters) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> SmsChannelSubtypeParameters:
    ...
    if "templateParameters" in data:
        ...
        raise DeserializationError(
            "SmsChannelSubtypeParameters.template_parameters required"
        )
    return out
