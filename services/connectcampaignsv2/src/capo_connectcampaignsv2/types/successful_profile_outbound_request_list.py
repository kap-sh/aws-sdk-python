...
if TYPE_CHECKING:
    import capo_connectcampaignsv2.types.successful_profile_outbound_request
...
def serialize_json(value: SuccessfulProfileOutboundRequestList) -> list:
    ...
    for item in value:
        out.append(
            capo_connectcampaignsv2.types.successful_profile_outbound_request.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> SuccessfulProfileOutboundRequestList:
    ...
    for item in data:
        out.append(
            capo_connectcampaignsv2.types.successful_profile_outbound_request.deserialize_json(
                item
            )
        )
    return out
