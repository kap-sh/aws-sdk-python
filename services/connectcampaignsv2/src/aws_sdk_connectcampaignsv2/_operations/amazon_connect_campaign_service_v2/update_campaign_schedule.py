"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#UpdateCampaignSchedule``."""
...
def build_request(
    ...
    input_: aws_sdk_connectcampaignsv2.types.update_campaign_schedule_request.UpdateCampaignScheduleRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/campaigns/{id}/schedule"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connectcampaignsv2.types.update_campaign_schedule_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_campaign_schedule(
