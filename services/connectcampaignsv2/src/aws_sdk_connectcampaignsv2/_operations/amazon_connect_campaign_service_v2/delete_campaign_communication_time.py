"""Generated from Smithy shape ``com.amazonaws.connectcampaignsv2#DeleteCampaignCommunicationTime``."""
...
def build_request(
    ...
    input_: aws_sdk_connectcampaignsv2.types.delete_campaign_communication_time_request.DeleteCampaignCommunicationTimeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/campaigns/{id}/communication-time"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    if "config" in input_:
        params["config"] = str(input_["config"])
...
async def async_delete_campaign_communication_time(
