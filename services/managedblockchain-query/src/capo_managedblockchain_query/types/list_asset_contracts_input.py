...
if TYPE_CHECKING:
    import capo_managedblockchain_query.types.contract_filter
    import capo_managedblockchain_query.types.next_token


class ListAssetContractsInput(TypedDict, closed=True):
    contract_filter: "capo_managedblockchain_query.types.contract_filter.ContractFilter"
...
def serialize_json(value: ListAssetContractsInput) -> dict:
    ...
    out["contractFilter"] = (
        capo_managedblockchain_query.types.contract_filter.serialize_json(
...
def deserialize_json(data: dict) -> ListAssetContractsInput:
    ...
    if "contractFilter" in data:
        ...
        out["contract_filter"] = (
            capo_managedblockchain_query.types.contract_filter.deserialize_json(
                data["contractFilter"]
            )
        )
    else:
