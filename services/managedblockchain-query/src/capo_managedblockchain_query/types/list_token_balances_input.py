...
class ListTokenBalancesInput(TypedDict, closed=True):
    owner_filter: NotRequired[
        "capo_managedblockchain_query.types.owner_filter.OwnerFilter"
    ]
    ...
    token_filter: "capo_managedblockchain_query.types.token_filter.TokenFilter"
...
def serialize_json(value: ListTokenBalancesInput) -> dict:
    ...
    if "owner_filter" in value:
        ...
        out["ownerFilter"] = (
            capo_managedblockchain_query.types.owner_filter.serialize_json(
                value["owner_filter"]
            )
        )
    import capo_managedblockchain_query.types.token_filter

    out["tokenFilter"] = capo_managedblockchain_query.types.token_filter.serialize_json(
        value["token_filter"]
    )
...
def deserialize_json(data: dict) -> ListTokenBalancesInput:
    ...
    if "ownerFilter" in data:
        ...
        out["owner_filter"] = (
            capo_managedblockchain_query.types.owner_filter.deserialize_json(
                data["ownerFilter"]
            )
        )
    if "tokenFilter" in data:
        ...
        out["token_filter"] = (
            capo_managedblockchain_query.types.token_filter.deserialize_json(
                data["tokenFilter"]
            )
        )
    else:
