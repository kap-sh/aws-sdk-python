...
if TYPE_CHECKING:
    import capo_managedblockchain_query.types.batch_get_token_balance_input_item
...
def serialize_json(value: GetTokenBalanceInputList) -> list:
    ...
    for item in value:
        out.append(
            capo_managedblockchain_query.types.batch_get_token_balance_input_item.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> GetTokenBalanceInputList:
    ...
    for item in data:
        out.append(
            capo_managedblockchain_query.types.batch_get_token_balance_input_item.deserialize_json(
                item
            )
        )
    return out
