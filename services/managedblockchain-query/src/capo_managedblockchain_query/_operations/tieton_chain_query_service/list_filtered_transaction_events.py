...
import capo_managedblockchain_query.types.list_filtered_transaction_events_input
import capo_managedblockchain_query.types.list_filtered_transaction_events_output
...
) -> capo_managedblockchain_query.types.list_filtered_transaction_events_output.ListFilteredTransactionEventsOutput:
    out: capo_managedblockchain_query.types.list_filtered_transaction_events_output.ListFilteredTransactionEventsOutput = capo_managedblockchain_query.types.list_filtered_transaction_events_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_managedblockchain_query._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_managedblockchain_query.types.list_filtered_transaction_events_input.serialize_json(
            input_
        )
    ).encode()
