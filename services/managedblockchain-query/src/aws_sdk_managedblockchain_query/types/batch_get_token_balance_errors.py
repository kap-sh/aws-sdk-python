...
BatchGetTokenBalanceErrors: TypeAlias = list[
    "aws_sdk_managedblockchain_query.types.batch_get_token_balance_error_item.BatchGetTokenBalanceErrorItem"
]
...
def serialize_json(value: BatchGetTokenBalanceErrors) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_managedblockchain_query.types.batch_get_token_balance_error_item.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> BatchGetTokenBalanceErrors:
    ...
    for item in data:
        out.append(
            aws_sdk_managedblockchain_query.types.batch_get_token_balance_error_item.deserialize_json(
                item
            )
        )
    return out
