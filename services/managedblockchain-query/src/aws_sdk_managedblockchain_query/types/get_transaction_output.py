...
class GetTransactionOutput(TypedDict):
    transaction: "aws_sdk_managedblockchain_query.types.transaction.Transaction"
...
def serialize_json(value: GetTransactionOutput) -> dict:
    ...
    out["transaction"] = (
        aws_sdk_managedblockchain_query.types.transaction.serialize_json(
            value["transaction"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> GetTransactionOutput:
    ...
    if "transaction" in data:
        ...
        out["transaction"] = (
            aws_sdk_managedblockchain_query.types.transaction.deserialize_json(
                data["transaction"]
            )
        )
    else:
