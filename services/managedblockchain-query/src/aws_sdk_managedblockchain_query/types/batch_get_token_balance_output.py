...
def serialize_json(value: BatchGetTokenBalanceOutput) -> dict:
    out: dict = {}
    import aws_sdk_managedblockchain_query.types.batch_get_token_balance_output_list
    out["tokenBalances"] = aws_sdk_managedblockchain_query.types.batch_get_token_balance_output_list.serialize_json(value["token_balances"])
    import aws_sdk_managedblockchain_query.types.batch_get_token_balance_errors
    out["errors"] = aws_sdk_managedblockchain_query.types.batch_get_token_balance_errors.serialize_json(value["errors"])
    return out
...
def deserialize_json(data: dict) -> BatchGetTokenBalanceOutput:
    ...
    if "tokenBalances" in data:
        import aws_sdk_managedblockchain_query.types.batch_get_token_balance_output_list
        out["token_balances"] = aws_sdk_managedblockchain_query.types.batch_get_token_balance_output_list.deserialize_json(data["tokenBalances"])
    else:
        raise DeserializationError("BatchGetTokenBalanceOutput.token_balances required")
    if "errors" in data:
        import aws_sdk_managedblockchain_query.types.batch_get_token_balance_errors
        out["errors"] = aws_sdk_managedblockchain_query.types.batch_get_token_balance_errors.deserialize_json(data["errors"])
    else:
