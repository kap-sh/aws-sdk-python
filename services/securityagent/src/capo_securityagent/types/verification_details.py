...
class VerificationDetails(TypedDict, closed=True):
    ...
    dns_txt: NotRequired["capo_securityagent.types.dns_verification.DnsVerification"]
    """<p>The DNS TXT verification details.</p>"""
    http_route: NotRequired[
        "capo_securityagent.types.http_verification.HttpVerification"
    ]
...
def serialize_json(value: VerificationDetails) -> dict:
    ...
    if "dns_txt" in value:
        ...
        out["dnsTxt"] = capo_securityagent.types.dns_verification.serialize_json(
            value["dns_txt"]
        )
    if "http_route" in value:
        import capo_securityagent.types.http_verification

        out["httpRoute"] = capo_securityagent.types.http_verification.serialize_json(
            value["http_route"]
        )
    return out
