...
CodeReviewJobSummaryList: TypeAlias = list[
    "capo_securityagent.types.code_review_job_summary.CodeReviewJobSummary"
]
...
def serialize_json(value: CodeReviewJobSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_securityagent.types.code_review_job_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> CodeReviewJobSummaryList:
    ...
    for item in data:
        out.append(
            capo_securityagent.types.code_review_job_summary.deserialize_json(item)
        )
    return out
