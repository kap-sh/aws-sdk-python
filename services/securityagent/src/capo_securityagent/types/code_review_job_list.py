...
if TYPE_CHECKING:
    import capo_securityagent.types.code_review_job
...
def serialize_json(value: CodeReviewJobList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_securityagent.types.code_review_job.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> CodeReviewJobList:
    ...
    for item in data:
        out.append(capo_securityagent.types.code_review_job.deserialize_json(item))
    return out
