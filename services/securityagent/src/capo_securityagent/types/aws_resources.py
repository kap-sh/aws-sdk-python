...
class AWSResources(TypedDict, closed=True):
    vpcs: NotRequired["capo_securityagent.types.vpc_configs.VpcConfigs"]
    """<p>The VPC configurations associated with the agent space.</p>"""
    log_groups: NotRequired["capo_securityagent.types.log_group_arns.LogGroupArns"]
    """<p>The Amazon Resource Names (ARNs) of the CloudWatch log groups associated with the agent space.</p>"""
    s3_buckets: NotRequired["capo_securityagent.types.s3_bucket_arns.S3BucketArns"]
    """<p>The Amazon Resource Names (ARNs) of the S3 buckets associated with the agent space.</p>"""
    secret_arns: NotRequired["capo_securityagent.types.secret_arns.SecretArns"]
    ...
    lambda_function_arns: NotRequired[
        "capo_securityagent.types.lambda_function_arns.LambdaFunctionArns"
    ]
    """<p>The Amazon Resource Names (ARNs) of the Lambda functions associated with the agent space.</p>"""
    iam_roles: NotRequired["capo_securityagent.types.iam_roles.IamRoles"]
...
def serialize_json(value: AWSResources) -> dict:
    ...
    if "vpcs" in value:
        ...
        out["vpcs"] = capo_securityagent.types.vpc_configs.serialize_json(value["vpcs"])
    if "log_groups" in value:
        import capo_securityagent.types.log_group_arns

        out["logGroups"] = capo_securityagent.types.log_group_arns.serialize_json(
            value["log_groups"]
        )
    if "s3_buckets" in value:
        import capo_securityagent.types.s3_bucket_arns

        out["s3Buckets"] = capo_securityagent.types.s3_bucket_arns.serialize_json(
            value["s3_buckets"]
        )
    if "secret_arns" in value:
        import capo_securityagent.types.secret_arns

        out["secretArns"] = capo_securityagent.types.secret_arns.serialize_json(
            value["secret_arns"]
        )
    if "lambda_function_arns" in value:
        ...
        out["lambdaFunctionArns"] = (
            capo_securityagent.types.lambda_function_arns.serialize_json(
    ...
    if "iam_roles" in value:
        ...
        out["iamRoles"] = capo_securityagent.types.iam_roles.serialize_json(
            value["iam_roles"]
        )
    return out
...
def deserialize_json(data: dict) -> AWSResources:
    ...
    if "lambdaFunctionArns" in data:
        ...
        out["lambda_function_arns"] = (
            capo_securityagent.types.lambda_function_arns.deserialize_json(
