...
class DnsVerification(TypedDict, closed=True):
    ...
    dns_record_type: NotRequired[
        "capo_securityagent.types.dns_record_type.DNSRecordType"
    ]
...
def serialize_json(value: DnsVerification) -> dict:
    ...
    if "dns_record_type" in value:
        ...
        out["dnsRecordType"] = capo_securityagent.types.dns_record_type.serialize_json(
...
def deserialize_json(data: dict) -> DnsVerification:
    ...
    if "dnsRecordType" in data:
        ...
        out["dns_record_type"] = (
            capo_securityagent.types.dns_record_type.deserialize_json(
                data["dnsRecordType"]
            )
        )
    return out
