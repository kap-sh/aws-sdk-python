"""Generated from Smithy shape ``com.amazonaws.securityagent#UpdateTargetDomain``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.update_target_domain_output.UpdateTargetDomainOutput = capo_securityagent.types.update_target_domain_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateTargetDomain"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.update_target_domain_input.serialize_json(input_)
    ).encode()
...
async def async_update_target_domain(
