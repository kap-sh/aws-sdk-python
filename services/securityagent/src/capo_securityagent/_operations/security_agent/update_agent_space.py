"""Generated from Smithy shape ``com.amazonaws.securityagent#UpdateAgentSpace``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.update_agent_space_output.UpdateAgentSpaceOutput = (
        capo_securityagent.types.update_agent_space_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateAgentSpace"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.update_agent_space_input.serialize_json(input_)
    ).encode()
...
async def async_update_agent_space(
