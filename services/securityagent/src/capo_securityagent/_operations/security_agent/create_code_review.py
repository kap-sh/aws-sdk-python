"""Generated from Smithy shape ``com.amazonaws.securityagent#CreateCodeReview``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.create_code_review_output.CreateCodeReviewOutput = (
        capo_securityagent.types.create_code_review_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateCodeReview"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.create_code_review_input.serialize_json(input_)
    ).encode()
