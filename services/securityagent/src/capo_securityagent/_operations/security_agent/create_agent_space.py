"""Generated from Smithy shape ``com.amazonaws.securityagent#CreateAgentSpace``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.create_agent_space_output.CreateAgentSpaceOutput = (
        capo_securityagent.types.create_agent_space_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateAgentSpace"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.create_agent_space_input.serialize_json(input_)
    ).encode()
