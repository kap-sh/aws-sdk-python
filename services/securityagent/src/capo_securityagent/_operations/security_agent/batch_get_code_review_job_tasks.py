"""Generated from Smithy shape ``com.amazonaws.securityagent#BatchGetCodeReviewJobTasks``."""
...
import capo_securityagent.types.code_review_job_task_list
...
def handle_response(
    ...
    out: capo_securityagent.types.batch_get_code_review_job_tasks_output.BatchGetCodeReviewJobTasksOutput = capo_securityagent.types.batch_get_code_review_job_tasks_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/BatchGetCodeReviewJobTasks"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.batch_get_code_review_job_tasks_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_get_code_review_job_tasks(
