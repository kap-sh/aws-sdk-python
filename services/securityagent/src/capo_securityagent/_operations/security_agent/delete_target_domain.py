"""Generated from Smithy shape ``com.amazonaws.securityagent#DeleteTargetDomain``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.delete_target_domain_output.DeleteTargetDomainOutput = capo_securityagent.types.delete_target_domain_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteTargetDomain"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.delete_target_domain_input.serialize_json(input_)
    ).encode()
...
async def async_delete_target_domain(
