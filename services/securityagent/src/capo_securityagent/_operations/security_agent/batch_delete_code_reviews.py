"""Generated from Smithy shape ``com.amazonaws.securityagent#BatchDeleteCodeReviews``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.batch_delete_code_reviews_output.BatchDeleteCodeReviewsOutput = capo_securityagent.types.batch_delete_code_reviews_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/BatchDeleteCodeReviews"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.batch_delete_code_reviews_input.serialize_json(input_)
    ).encode()
...
async def async_batch_delete_code_reviews(
