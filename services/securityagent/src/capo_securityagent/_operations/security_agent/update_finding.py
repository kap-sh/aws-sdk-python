"""Generated from Smithy shape ``com.amazonaws.securityagent#UpdateFinding``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.update_finding_output.UpdateFindingOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateFinding"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.update_finding_input.serialize_json(input_)
    ).encode()
...
def update_finding(
    ...
    capo_securityagent.types.update_finding_output.UpdateFindingOutput, zapros.Response
]:
...
async def async_update_finding(
