"""Generated from Smithy shape ``com.amazonaws.securityagent#BatchGetFindings``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.batch_get_findings_output.BatchGetFindingsOutput = (
        capo_securityagent.types.batch_get_findings_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/BatchGetFindings"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.batch_get_findings_input.serialize_json(input_)
    ).encode()
...
async def async_batch_get_findings(
