"""Generated from Smithy shape ``com.amazonaws.securityagent#UpdateIntegratedResources``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.update_integrated_resources_output.UpdateIntegratedResourcesOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateIntegratedResources"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.update_integrated_resources_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_integrated_resources(
