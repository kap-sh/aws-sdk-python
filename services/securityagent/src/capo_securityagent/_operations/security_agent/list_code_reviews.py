"""Generated from Smithy shape ``com.amazonaws.securityagent#ListCodeReviews``."""
...
def handle_response(
    ...
    out: capo_securityagent.types.list_code_reviews_output.ListCodeReviewsOutput = (
        capo_securityagent.types.list_code_reviews_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListCodeReviews"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityagent.types.list_code_reviews_input.serialize_json(input_)
    ).encode()
