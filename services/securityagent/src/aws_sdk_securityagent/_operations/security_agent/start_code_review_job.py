"""Generated from Smithy shape ``com.amazonaws.securityagent#StartCodeReviewJob``."""
...
def handle_response(
    ...
    out: aws_sdk_securityagent.types.start_code_review_job_output.StartCodeReviewJobOutput = aws_sdk_securityagent.types.start_code_review_job_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_securityagent.types.start_code_review_job_input.StartCodeReviewJobInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/StartCodeReviewJob"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_securityagent.types.start_code_review_job_input.serialize_json(input_)
    ).encode()
...
async def async_start_code_review_job(
