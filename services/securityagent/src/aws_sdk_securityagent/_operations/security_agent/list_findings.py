"""Generated from Smithy shape ``com.amazonaws.securityagent#ListFindings``."""
...
import aws_sdk_securityagent.types.finding_summary_list
...
def handle_response(
    ...
    out: aws_sdk_securityagent.types.list_findings_output.ListFindingsOutput = (
        aws_sdk_securityagent.types.list_findings_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_securityagent.types.list_findings_input.ListFindingsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/ListFindings"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_securityagent.types.list_findings_input.serialize_json(input_)
    ).encode()
...
def list_findings(
    ...
    aws_sdk_securityagent.types.list_findings_output.ListFindingsOutput, zapros.Response
]:
