...
TargetDomainSummaryList: TypeAlias = list[
    "aws_sdk_securityagent.types.target_domain_summary.TargetDomainSummary"
]
...
def serialize_json(value: TargetDomainSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_securityagent.types.target_domain_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> TargetDomainSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_securityagent.types.target_domain_summary.deserialize_json(item)
        )
    return out
