...
DiscoveredEndpointList: TypeAlias = list[
    "aws_sdk_securityagent.types.discovered_endpoint.DiscoveredEndpoint"
]
...
def serialize_json(value: DiscoveredEndpointList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_securityagent.types.discovered_endpoint.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> DiscoveredEndpointList:
    ...
    for item in data:
        out.append(
            aws_sdk_securityagent.types.discovered_endpoint.deserialize_json(item)
        )
    return out
