...
if TYPE_CHECKING:
    ...
    import aws_sdk_securityagent.types.code_review_list


class BatchGetCodeReviewsOutput(TypedDict):
    code_reviews: NotRequired[
        "aws_sdk_securityagent.types.code_review_list.CodeReviewList"
    ]
...
def serialize_json(value: BatchGetCodeReviewsOutput) -> dict:
    ...
    if "code_reviews" in value:
        ...
        out["codeReviews"] = (
            aws_sdk_securityagent.types.code_review_list.serialize_json(
                value["code_reviews"]
            )
        )
...
def deserialize_json(data: dict) -> BatchGetCodeReviewsOutput:
    ...
    if "codeReviews" in data:
        ...
        out["code_reviews"] = (
            aws_sdk_securityagent.types.code_review_list.deserialize_json(
                data["codeReviews"]
            )
        )
    if "notFound" in data:
