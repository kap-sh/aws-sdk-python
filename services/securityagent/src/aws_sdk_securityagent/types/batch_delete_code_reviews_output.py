...
class BatchDeleteCodeReviewsOutput(TypedDict, closed=True):
    ...
    failed: NotRequired[
        "aws_sdk_securityagent.types.delete_code_review_failure_list.DeleteCodeReviewFailureList"
    ]
...
def serialize_json(value: BatchDeleteCodeReviewsOutput) -> dict:
    ...
    if "deleted" in value:
        ...
        out["deleted"] = aws_sdk_securityagent.types.code_review_id_list.serialize_json(
            value["deleted"]
        )
    if "failed" in value:
        import aws_sdk_securityagent.types.delete_code_review_failure_list
        ...
        out["failed"] = (
            aws_sdk_securityagent.types.delete_code_review_failure_list.serialize_json(
                value["failed"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> BatchDeleteCodeReviewsOutput:
    ...
    if "failed" in data:
        ...
        out["failed"] = (
            aws_sdk_securityagent.types.delete_code_review_failure_list.deserialize_json(
                data["failed"]
            )
        )
    return out
