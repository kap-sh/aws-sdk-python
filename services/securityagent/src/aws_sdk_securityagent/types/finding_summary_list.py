...
FindingSummaryList: TypeAlias = list[
    "aws_sdk_securityagent.types.finding_summary.FindingSummary"
]
...
def serialize_json(value: FindingSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_securityagent.types.finding_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> FindingSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_securityagent.types.finding_summary.deserialize_json(item))
    return out
