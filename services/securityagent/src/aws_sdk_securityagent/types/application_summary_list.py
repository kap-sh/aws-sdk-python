...
ApplicationSummaryList: TypeAlias = list[
    "aws_sdk_securityagent.types.application_summary.ApplicationSummary"
]
...
def serialize_json(value: ApplicationSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_securityagent.types.application_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ApplicationSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_securityagent.types.application_summary.deserialize_json(item)
        )
    return out
