...
if TYPE_CHECKING:
    import aws_sdk_securityagent.types.integrated_resource
    import aws_sdk_securityagent.types.provider_resource_capabilities


class IntegratedResourceInputItem(TypedDict):
    resource: "aws_sdk_securityagent.types.integrated_resource.IntegratedResource"
...
def serialize_json(value: IntegratedResourceInputItem) -> dict:
    ...
    out["resource"] = aws_sdk_securityagent.types.integrated_resource.serialize_json(
        value["resource"]
    )
...
def deserialize_json(data: dict) -> IntegratedResourceInputItem:
    ...
    if "resource" in data:
        ...
        out["resource"] = (
            aws_sdk_securityagent.types.integrated_resource.deserialize_json(
                data["resource"]
            )
        )
    else:
