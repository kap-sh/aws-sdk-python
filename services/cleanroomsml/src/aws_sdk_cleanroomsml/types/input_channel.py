...
if TYPE_CHECKING:
    ...
    import aws_sdk_cleanroomsml.types.input_channel_data_source


class InputChannel(TypedDict, closed=True):
    data_source: (
        "aws_sdk_cleanroomsml.types.input_channel_data_source.InputChannelDataSource"
    )
...
def serialize_json(value: InputChannel) -> dict:
    ...
    out["dataSource"] = (
        aws_sdk_cleanroomsml.types.input_channel_data_source.serialize_json(
            value["data_source"]
        )
    )
...
def deserialize_json(data: dict) -> InputChannel:
    ...
    if "dataSource" in data:
        ...
        out["data_source"] = (
            aws_sdk_cleanroomsml.types.input_channel_data_source.deserialize_json(
                data["dataSource"]
            )
        )
    else:
