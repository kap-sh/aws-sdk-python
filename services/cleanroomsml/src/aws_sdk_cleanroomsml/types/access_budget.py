...
class AccessBudget(TypedDict, closed=True):
    ...
    details: (
        "aws_sdk_cleanroomsml.types.access_budget_details_list.AccessBudgetDetailsList"
    )
...
def serialize_json(value: AccessBudget) -> dict:
    ...
    out["details"] = (
        aws_sdk_cleanroomsml.types.access_budget_details_list.serialize_json(
            value["details"]
        )
    )
...
def deserialize_json(data: dict) -> AccessBudget:
    ...
    if "details" in data:
        ...
        out["details"] = (
            aws_sdk_cleanroomsml.types.access_budget_details_list.deserialize_json(
                data["details"]
            )
        )
    else:
