"""Generated from Smithy shape ``com.amazonaws.quicksight#ListUsersIndexCapacity``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.list_users_index_capacity_response.ListUsersIndexCapacityResponse = aws_sdk_quicksight.types.list_users_index_capacity_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.list_users_index_capacity_request.ListUsersIndexCapacityRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/accounts/{awsAccountId}/quick-index/user-capacity"
    )
    url = url.replace("{awsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.list_users_index_capacity_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_users_index_capacity(
