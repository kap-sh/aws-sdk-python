"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateVPCConnection``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.update_vpc_connection_response.UpdateVPCConnectionResponse = aws_sdk_quicksight.types.update_vpc_connection_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.update_vpc_connection_request.UpdateVPCConnectionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{VPCConnectionId}", quote(str(input_["vpc_connection_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.update_vpc_connection_request.serialize_json(input_)
    ).encode()
...
async def async_update_vpc_connection(
