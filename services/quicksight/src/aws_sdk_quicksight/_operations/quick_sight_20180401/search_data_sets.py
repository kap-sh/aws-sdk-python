"""Generated from Smithy shape ``com.amazonaws.quicksight#SearchDataSets``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.search_data_sets_response.SearchDataSetsResponse = (
        aws_sdk_quicksight.types.search_data_sets_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.search_data_sets_request.SearchDataSetsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/search/data-sets"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.search_data_sets_request.serialize_json(input_)
    ).encode()
...
async def async_search_data_sets(
