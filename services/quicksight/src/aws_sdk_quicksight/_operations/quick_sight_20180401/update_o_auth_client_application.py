"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateOAuthClientApplication``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.update_o_auth_client_application_response.UpdateOAuthClientApplicationResponse = aws_sdk_quicksight.types.update_o_auth_client_application_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.update_o_auth_client_application_request.UpdateOAuthClientApplicationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/accounts/{AwsAccountId}/oauth-client-applications/{OAuthClientApplicationId}"
    )
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    url = url.replace(
        "{OAuthClientApplicationId}",
        quote(str(input_["o_auth_client_application_id"]), safe=""),
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.update_o_auth_client_application_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_o_auth_client_application(
