"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateCustomPermissions``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.update_custom_permissions_response.UpdateCustomPermissionsResponse = aws_sdk_quicksight.types.update_custom_permissions_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_quicksight.types.update_custom_permissions_request.UpdateCustomPermissionsRequest,
) -> zapros.Request:
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/accounts/{AwsAccountId}/custom-permissions/{CustomPermissionsName}"
    )
    url = url.replace("{AwsAccountId}", quote(str(input["aws_account_id"]), safe=""))
    url = url.replace(
        "{CustomPermissionsName}", quote(str(input["custom_permissions_name"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.update_custom_permissions_request.serialize_json(input)
    ).encode()
...
async def async_update_custom_permissions(
