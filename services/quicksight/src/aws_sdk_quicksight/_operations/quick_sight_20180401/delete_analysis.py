...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.delete_analysis_request.DeleteAnalysisRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    params: dict[str, str] = {}
    if "recovery_window_in_days" in input_:
        params["recovery-window-in-days"] = str(input_["recovery_window_in_days"])
    params["force-delete-without-recovery"] = str(
        input_.get("force_delete_without_recovery", False)
    )
