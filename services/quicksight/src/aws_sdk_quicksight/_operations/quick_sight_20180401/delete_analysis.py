"""Generated from Smithy shape ``com.amazonaws.quicksight#DeleteAnalysis``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.delete_analysis_response.DeleteAnalysisResponse = (
        aws_sdk_quicksight.types.delete_analysis_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_quicksight.types.delete_analysis_request.DeleteAnalysisRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/analyses/{AnalysisId}"
    url = url.replace("{AwsAccountId}", quote(str(input["aws_account_id"]), safe=""))
    url = url.replace("{AnalysisId}", quote(str(input["analysis_id"]), safe=""))
    params: dict[str, str] = {}
    if "recovery_window_in_days" in input:
        params["recovery-window-in-days"] = str(input["recovery_window_in_days"])
    params["force-delete-without-recovery"] = str(
        input.get("force_delete_without_recovery", False)
    )
