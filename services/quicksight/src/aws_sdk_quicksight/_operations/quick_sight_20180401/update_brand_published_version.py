"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateBrandPublishedVersion``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InternalServerException":
            import aws_sdk_quicksight.errors.internal_server_exception

            raise aws_sdk_quicksight.errors.internal_server_exception.InternalServerException.from_json(
                data
            )
        case "InvalidRequestException":
            import aws_sdk_quicksight.errors.invalid_request_exception

            raise aws_sdk_quicksight.errors.invalid_request_exception.InvalidRequestException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.update_brand_published_version_response.UpdateBrandPublishedVersionResponse = aws_sdk_quicksight.types.update_brand_published_version_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.update_brand_published_version_request.UpdateBrandPublishedVersionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/accounts/{AwsAccountId}/brands/{BrandId}/publishedversion"
    )
    ...
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.update_brand_published_version_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_brand_published_version(
