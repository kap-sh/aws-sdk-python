...
def build_request(
    ...
    input: aws_sdk_quicksight.types.update_flow_permissions_input.UpdateFlowPermissionsInput,
) -> zapros.Request:
    ...
    url = url.replace("{FlowId}", quote(str(input["flow_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_quicksight.types.update_flow_permissions_input

    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.update_flow_permissions_input.serialize_json(input)
    ).encode()
