"""Generated from Smithy shape ``com.amazonaws.quicksight#SearchAnalyses``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.search_analyses_response.SearchAnalysesResponse = (
        aws_sdk_quicksight.types.search_analyses_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.search_analyses_request.SearchAnalysesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/search/analyses"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.search_analyses_request.serialize_json(input_)
    ).encode()
...
async def async_search_analyses(
