"""Generated from Smithy shape ``com.amazonaws.quicksight#PredictQAResults``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.predict_qa_results_response.PredictQAResultsResponse = aws_sdk_quicksight.types.predict_qa_results_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_quicksight.types.predict_qa_results_request.PredictQAResultsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/qa/predict"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_quicksight.types.predict_qa_results_request.serialize_json(input_)
    ).encode()
...
async def async_predict_qa_results(
