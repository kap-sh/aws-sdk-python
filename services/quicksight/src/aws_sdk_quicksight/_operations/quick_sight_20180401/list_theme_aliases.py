"""Generated from Smithy shape ``com.amazonaws.quicksight#ListThemeAliases``."""
...
def handle_response(
    ...
    out: aws_sdk_quicksight.types.list_theme_aliases_response.ListThemeAliasesResponse = aws_sdk_quicksight.types.list_theme_aliases_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_quicksight.types.list_theme_aliases_request.ListThemeAliasesRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/themes/{ThemeId}/aliases"
    url = url.replace("{AwsAccountId}", quote(str(input["aws_account_id"]), safe=""))
    url = url.replace("{ThemeId}", quote(str(input["theme_id"]), safe=""))
    params: dict[str, str] = {}
    if "next_token" in input:
        params["next-token"] = str(input["next_token"])
    if "max_results" in input:
        params["max-result"] = str(input["max_results"])
...
async def async_list_theme_aliases(
