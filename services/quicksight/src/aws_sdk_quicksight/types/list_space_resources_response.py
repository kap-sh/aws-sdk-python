...
class ListSpaceResourcesResponse(TypedDict):
    ...
    space_resources: (
        "aws_sdk_quicksight.types.space_resource_summaries.SpaceResourceSummaries"
    )
...
# --- restJson1 ser/de ---
def serialize_json(value: ListSpaceResourcesResponse) -> dict:
    out: dict = {}
    ...
    out["SpaceResources"] = (
        aws_sdk_quicksight.types.space_resource_summaries.serialize_json(
...
def deserialize_json(data: dict) -> ListSpaceResourcesResponse:
    out: ListSpaceResourcesResponse = {}  # type: ignore[typeddict-item]
    if "spaceId" in data:
        out["space_id"] = data["spaceId"]
    else:
        raise DeserializationError("ListSpaceResourcesResponse.space_id required")
    if "spaceArn" in data:
        out["space_arn"] = data["spaceArn"]
    if "SpaceResources" in data:
        import aws_sdk_quicksight.types.space_resource_summaries

        out["space_resources"] = (
            aws_sdk_quicksight.types.space_resource_summaries.deserialize_json(
                data["SpaceResources"]
            )
        )
    else:
        raise DeserializationError(
            "ListSpaceResourcesResponse.space_resources required"
        )
