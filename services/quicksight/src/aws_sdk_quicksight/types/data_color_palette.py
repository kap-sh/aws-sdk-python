...
class DataColorPalette(TypedDict, closed=True):
    ...
    empty_fill_color: NotRequired["aws_sdk_quicksight.types.hex_color.HexColor"]
...
def serialize_json(value: DataColorPalette) -> dict:
    ...
    if "colors" in value:
        ...
        out["Colors"] = aws_sdk_quicksight.types.color_list.serialize_json(
            value["colors"]
        )
    if "min_max_gradient" in value:
        import aws_sdk_quicksight.types.color_list

        out["MinMaxGradient"] = aws_sdk_quicksight.types.color_list.serialize_json(
...
def deserialize_json(data: dict) -> DataColorPalette:
    ...
    if "Colors" in data:
        ...
        out["colors"] = aws_sdk_quicksight.types.color_list.deserialize_json(
            data["Colors"]
        )
    if "MinMaxGradient" in data:
        import aws_sdk_quicksight.types.color_list

        out["min_max_gradient"] = aws_sdk_quicksight.types.color_list.deserialize_json(
            data["MinMaxGradient"]
        )
