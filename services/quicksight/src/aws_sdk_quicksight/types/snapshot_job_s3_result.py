...
if TYPE_CHECKING:
    ...
    import aws_sdk_quicksight.types.snapshot_job_result_error_info_list
    import aws_sdk_quicksight.types.snapshot_s3_destination_configuration


class SnapshotJobS3Result(TypedDict):
    ...
    s3_uri: NotRequired["aws_sdk_quicksight.types.sensitive_s3_uri.SensitiveS3Uri"]
    """<p>The Amazon S3 Uri.</p>"""
    error_info: NotRequired[
        "aws_sdk_quicksight.types.snapshot_job_result_error_info_list.SnapshotJobResultErrorInfoList"
    ]
...
def serialize_json(value: SnapshotJobS3Result) -> dict:
    ...
    if "error_info" in value:
        ...
        out["ErrorInfo"] = (
            aws_sdk_quicksight.types.snapshot_job_result_error_info_list.serialize_json(
                value["error_info"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> SnapshotJobS3Result:
    ...
    if "ErrorInfo" in data:
        ...
        out["error_info"] = (
            aws_sdk_quicksight.types.snapshot_job_result_error_info_list.deserialize_json(
                data["ErrorInfo"]
            )
        )
    return out
