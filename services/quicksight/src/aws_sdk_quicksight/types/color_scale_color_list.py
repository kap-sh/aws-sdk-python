"""Generated from Smithy shape ``com.amazonaws.quicksight#ColorScaleColorList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_quicksight.types.data_color

ColorScaleColorList: TypeAlias = list["aws_sdk_quicksight.types.data_color.DataColor"]


# --- restJson1 ser/de ---
def serialize_json(value: ColorScaleColorList) -> list:
    import aws_sdk_quicksight.types.data_color

    out: list = []
    for item in value:
        out.append(aws_sdk_quicksight.types.data_color.serialize_json(item))
    return out


def deserialize_json(data: list) -> ColorScaleColorList:
    import aws_sdk_quicksight.types.data_color

    out: ColorScaleColorList = []
    for item in data:
        out.append(aws_sdk_quicksight.types.data_color.deserialize_json(item))
    return out
