...
class RegisterUserRequest(TypedDict):
    ...
    iam_arn: NotRequired["aws_sdk_quicksight.types.string.String"]
...
def serialize_json(value: RegisterUserRequest) -> dict:
    ...
    out["UserRole"] = aws_sdk_quicksight.types.user_role.serialize_json(
        value["user_role"]
    )
    if "iam_arn" in value:
...
def deserialize_json(data: dict) -> RegisterUserRequest:
    ...
    if "UserRole" in data:
        ...
        out["user_role"] = aws_sdk_quicksight.types.user_role.deserialize_json(
            data["UserRole"]
        )
    else:
