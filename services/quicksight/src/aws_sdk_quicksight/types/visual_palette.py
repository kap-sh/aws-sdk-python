...
class VisualPalette(TypedDict, closed=True):
    ...
    color_map: NotRequired[
        "aws_sdk_quicksight.types.data_path_color_list.DataPathColorList"
    ]
...
def serialize_json(value: VisualPalette) -> dict:
    ...
    if "color_map" in value:
        ...
        out["ColorMap"] = aws_sdk_quicksight.types.data_path_color_list.serialize_json(
            value["color_map"]
        )
    return out
...
def deserialize_json(data: dict) -> VisualPalette:
    ...
    if "ColorMap" in data:
        ...
        out["color_map"] = (
            aws_sdk_quicksight.types.data_path_color_list.deserialize_json(
                data["ColorMap"]
            )
        )
    return out
