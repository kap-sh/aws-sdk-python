...
def serialize_json(value: FieldSortOptions) -> dict:
    ...
    if "field_sort" in value:
        ...
        out["FieldSort"] = aws_sdk_quicksight.types.field_sort.serialize_json(
            value["field_sort"]
        )
    if "column_sort" in value:
        import aws_sdk_quicksight.types.column_sort

        out["ColumnSort"] = aws_sdk_quicksight.types.column_sort.serialize_json(
            value["column_sort"]
        )
    return out
...
def deserialize_json(data: dict) -> FieldSortOptions:
    ...
    if "FieldSort" in data:
        ...
        out["field_sort"] = aws_sdk_quicksight.types.field_sort.deserialize_json(
            data["FieldSort"]
        )
    if "ColumnSort" in data:
        import aws_sdk_quicksight.types.column_sort

        out["column_sort"] = aws_sdk_quicksight.types.column_sort.deserialize_json(
            data["ColumnSort"]
        )
    return out
