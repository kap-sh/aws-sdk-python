...
class BatchDeleteTopicReviewedAnswerRequest(TypedDict):
    ...
    answer_ids: NotRequired["aws_sdk_quicksight.types.answer_ids.AnswerIds"]
...
def serialize_json(value: BatchDeleteTopicReviewedAnswerRequest) -> dict:
    ...
    if "answer_ids" in value:
        ...
        out["AnswerIds"] = aws_sdk_quicksight.types.answer_ids.serialize_json(
            value["answer_ids"]
        )
    return out
...
def deserialize_json(data: dict) -> BatchDeleteTopicReviewedAnswerRequest:
    ...
    if "AnswerIds" in data:
        ...
        out["answer_ids"] = aws_sdk_quicksight.types.answer_ids.deserialize_json(
            data["AnswerIds"]
        )
    return out
