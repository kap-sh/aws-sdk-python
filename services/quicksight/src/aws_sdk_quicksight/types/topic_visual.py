...
if TYPE_CHECKING:
    ...
    import aws_sdk_quicksight.types.topic_visuals
    import aws_sdk_quicksight.types.visual_role


class TopicVisual(TypedDict):
    ...
    role: NotRequired["aws_sdk_quicksight.types.visual_role.VisualRole"]
    """<p>The role for the <code>TopicVisual</code>.</p>"""
    ir: NotRequired["aws_sdk_quicksight.types.topic_ir.TopicIR"]
    """<p>The ir for the <code>TopicVisual</code>.</p>"""
    supporting_visuals: NotRequired[
        "aws_sdk_quicksight.types.topic_visuals.TopicVisuals"
    ]
...
def serialize_json(value: TopicVisual) -> dict:
    ...
    if "role" in value:
        ...
        out["Role"] = aws_sdk_quicksight.types.visual_role.serialize_json(value["role"])
    if "ir" in value:
        import aws_sdk_quicksight.types.topic_ir

        out["Ir"] = aws_sdk_quicksight.types.topic_ir.serialize_json(value["ir"])
    if "supporting_visuals" in value:
        import aws_sdk_quicksight.types.topic_visuals

        out["SupportingVisuals"] = (
            aws_sdk_quicksight.types.topic_visuals.serialize_json(
...
def deserialize_json(data: dict) -> TopicVisual:
    ...
    if "Role" in data:
        ...
        out["role"] = aws_sdk_quicksight.types.visual_role.deserialize_json(
            data["Role"]
        )
    if "Ir" in data:
        import aws_sdk_quicksight.types.topic_ir

        out["ir"] = aws_sdk_quicksight.types.topic_ir.deserialize_json(data["Ir"])
    if "SupportingVisuals" in data:
        import aws_sdk_quicksight.types.topic_visuals

        out["supporting_visuals"] = (
            aws_sdk_quicksight.types.topic_visuals.deserialize_json(
