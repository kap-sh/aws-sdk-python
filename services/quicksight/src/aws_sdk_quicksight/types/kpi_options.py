"""Generated from Smithy shape ``com.amazonaws.quicksight#KPIOptions``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import aws_sdk_quicksight.types.comparison_configuration
    import aws_sdk_quicksight.types.font_configuration
    import aws_sdk_quicksight.types.kpi_sparkline_options
    import aws_sdk_quicksight.types.kpi_visual_layout_options
    import aws_sdk_quicksight.types.primary_value_display_type
    import aws_sdk_quicksight.types.progress_bar_options
    import aws_sdk_quicksight.types.secondary_value_options
    import aws_sdk_quicksight.types.trend_arrow_options


class KPIOptions(TypedDict, closed=True):
    progress_bar: NotRequired[
        "aws_sdk_quicksight.types.progress_bar_options.ProgressBarOptions"
    ]
    """<p>The options that determine the presentation of the progress bar of a KPI visual.</p>"""
    trend_arrows: NotRequired[
        "aws_sdk_quicksight.types.trend_arrow_options.TrendArrowOptions"
    ]
    """<p>The options that determine the presentation of trend arrows in a KPI visual.</p>"""
    secondary_value: NotRequired[
        "aws_sdk_quicksight.types.secondary_value_options.SecondaryValueOptions"
    ]
    """<p>The options that determine the presentation of the secondary value of a KPI visual.</p>"""
    comparison: NotRequired[
        "aws_sdk_quicksight.types.comparison_configuration.ComparisonConfiguration"
    ]
    """<p>The comparison configuration of a KPI visual.</p>"""
    primary_value_display_type: NotRequired[
        "aws_sdk_quicksight.types.primary_value_display_type.PrimaryValueDisplayType"
    ]
    """<p>The options that determine the primary value display type.</p>"""
    primary_value_font_configuration: NotRequired[
        "aws_sdk_quicksight.types.font_configuration.FontConfiguration"
    ]
    """<p>The options that determine the primary value font configuration.</p>"""
    secondary_value_font_configuration: NotRequired[
        "aws_sdk_quicksight.types.font_configuration.FontConfiguration"
    ]
    """<p>The options that determine the secondary value font configuration.</p>"""
    sparkline: NotRequired[
        "aws_sdk_quicksight.types.kpi_sparkline_options.KPISparklineOptions"
    ]
    """<p>The options that determine the visibility, color, type, and tooltip visibility of the sparkline of a KPI visual.</p>"""
    visual_layout_options: NotRequired[
        "aws_sdk_quicksight.types.kpi_visual_layout_options.KPIVisualLayoutOptions"
    ]
    """<p>The options that determine the layout a KPI visual.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: KPIOptions) -> dict:
    out: dict = {}
    if "progress_bar" in value:
        import aws_sdk_quicksight.types.progress_bar_options

        out["ProgressBar"] = (
            aws_sdk_quicksight.types.progress_bar_options.serialize_json(
                value["progress_bar"]
            )
        )
    if "trend_arrows" in value:
        import aws_sdk_quicksight.types.trend_arrow_options

        out["TrendArrows"] = (
            aws_sdk_quicksight.types.trend_arrow_options.serialize_json(
                value["trend_arrows"]
            )
        )
    if "secondary_value" in value:
        import aws_sdk_quicksight.types.secondary_value_options

        out["SecondaryValue"] = (
            aws_sdk_quicksight.types.secondary_value_options.serialize_json(
                value["secondary_value"]
            )
        )
    if "comparison" in value:
        import aws_sdk_quicksight.types.comparison_configuration

        out["Comparison"] = (
            aws_sdk_quicksight.types.comparison_configuration.serialize_json(
                value["comparison"]
            )
        )
    if "primary_value_display_type" in value:
        import aws_sdk_quicksight.types.primary_value_display_type

        out["PrimaryValueDisplayType"] = (
            aws_sdk_quicksight.types.primary_value_display_type.serialize_json(
                value["primary_value_display_type"]
            )
        )
    if "primary_value_font_configuration" in value:
        import aws_sdk_quicksight.types.font_configuration

        out["PrimaryValueFontConfiguration"] = (
            aws_sdk_quicksight.types.font_configuration.serialize_json(
                value["primary_value_font_configuration"]
            )
        )
    if "secondary_value_font_configuration" in value:
        import aws_sdk_quicksight.types.font_configuration

        out["SecondaryValueFontConfiguration"] = (
            aws_sdk_quicksight.types.font_configuration.serialize_json(
                value["secondary_value_font_configuration"]
            )
        )
    if "sparkline" in value:
        import aws_sdk_quicksight.types.kpi_sparkline_options

        out["Sparkline"] = (
            aws_sdk_quicksight.types.kpi_sparkline_options.serialize_json(
                value["sparkline"]
            )
        )
    if "visual_layout_options" in value:
        import aws_sdk_quicksight.types.kpi_visual_layout_options

        out["VisualLayoutOptions"] = (
            aws_sdk_quicksight.types.kpi_visual_layout_options.serialize_json(
                value["visual_layout_options"]
            )
        )
    return out


def deserialize_json(data: dict) -> KPIOptions:
    out: KPIOptions = {}  # type: ignore[typeddict-item]
    if "ProgressBar" in data:
        import aws_sdk_quicksight.types.progress_bar_options

        out["progress_bar"] = (
            aws_sdk_quicksight.types.progress_bar_options.deserialize_json(
                data["ProgressBar"]
            )
        )
    if "TrendArrows" in data:
        import aws_sdk_quicksight.types.trend_arrow_options

        out["trend_arrows"] = (
            aws_sdk_quicksight.types.trend_arrow_options.deserialize_json(
                data["TrendArrows"]
            )
        )
    if "SecondaryValue" in data:
        import aws_sdk_quicksight.types.secondary_value_options

        out["secondary_value"] = (
            aws_sdk_quicksight.types.secondary_value_options.deserialize_json(
                data["SecondaryValue"]
            )
        )
    if "Comparison" in data:
        import aws_sdk_quicksight.types.comparison_configuration

        out["comparison"] = (
            aws_sdk_quicksight.types.comparison_configuration.deserialize_json(
                data["Comparison"]
            )
        )
    if "PrimaryValueDisplayType" in data:
        import aws_sdk_quicksight.types.primary_value_display_type

        out["primary_value_display_type"] = (
            aws_sdk_quicksight.types.primary_value_display_type.deserialize_json(
                data["PrimaryValueDisplayType"]
            )
        )
    if "PrimaryValueFontConfiguration" in data:
        import aws_sdk_quicksight.types.font_configuration

        out["primary_value_font_configuration"] = (
            aws_sdk_quicksight.types.font_configuration.deserialize_json(
                data["PrimaryValueFontConfiguration"]
            )
        )
    if "SecondaryValueFontConfiguration" in data:
        import aws_sdk_quicksight.types.font_configuration

        out["secondary_value_font_configuration"] = (
            aws_sdk_quicksight.types.font_configuration.deserialize_json(
                data["SecondaryValueFontConfiguration"]
            )
        )
    if "Sparkline" in data:
        import aws_sdk_quicksight.types.kpi_sparkline_options

        out["sparkline"] = (
            aws_sdk_quicksight.types.kpi_sparkline_options.deserialize_json(
                data["Sparkline"]
            )
        )
    if "VisualLayoutOptions" in data:
        import aws_sdk_quicksight.types.kpi_visual_layout_options

        out["visual_layout_options"] = (
            aws_sdk_quicksight.types.kpi_visual_layout_options.deserialize_json(
                data["VisualLayoutOptions"]
            )
        )
    return out
