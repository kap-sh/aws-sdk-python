...
class OutputColumn(TypedDict, closed=True):
    ...
    type: NotRequired["aws_sdk_quicksight.types.column_data_type.ColumnDataType"]
...
def serialize_json(value: OutputColumn) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = aws_sdk_quicksight.types.column_data_type.serialize_json(
            value["type"]
        )
...
def deserialize_json(data: dict) -> OutputColumn:
    ...
    if "Type" in data:
        ...
        out["type"] = aws_sdk_quicksight.types.column_data_type.deserialize_json(
            data["Type"]
        )
    if "SubType" in data:
        import aws_sdk_quicksight.types.column_data_sub_type
