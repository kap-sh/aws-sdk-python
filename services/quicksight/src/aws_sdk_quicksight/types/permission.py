...
if TYPE_CHECKING:
    import aws_sdk_quicksight.types.actions_list
...
class Permission(TypedDict):
    ...
    principal: (
        "aws_sdk_quicksight.types.permission_principal_string.PermissionPrincipalString"
    )
...
def serialize_json(value: Permission) -> dict:
    ...
    out["Actions"] = aws_sdk_quicksight.types.actions_list.serialize_json(
        value["actions"]
    )
...
def deserialize_json(data: dict) -> Permission:
    ...
    if "Actions" in data:
        ...
        out["actions"] = aws_sdk_quicksight.types.actions_list.deserialize_json(
            data["Actions"]
        )
    else:
