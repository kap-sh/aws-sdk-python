"""Generated from Smithy shape ``com.amazonaws.quicksight#BoxPlotVisual``."""
...
if TYPE_CHECKING:
    import aws_sdk_quicksight.types.box_plot_chart_configuration
...
class BoxPlotVisual(TypedDict, closed=True):
    ...
    chart_configuration: NotRequired[
        "aws_sdk_quicksight.types.box_plot_chart_configuration.BoxPlotChartConfiguration"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: BoxPlotVisual) -> dict:
    out: dict = {}
    ...
    if "chart_configuration" in value:
        ...
        out["ChartConfiguration"] = (
            aws_sdk_quicksight.types.box_plot_chart_configuration.serialize_json(
...
def deserialize_json(data: dict) -> BoxPlotVisual:
    out: BoxPlotVisual = {}  # type: ignore[typeddict-item]
    if "VisualId" in data:
        out["visual_id"] = data["VisualId"]
    else:
        raise DeserializationError("BoxPlotVisual.visual_id required")
    if "Title" in data:
        import aws_sdk_quicksight.types.visual_title_label_options
    ...
    if "ChartConfiguration" in data:
        ...
        out["chart_configuration"] = (
            aws_sdk_quicksight.types.box_plot_chart_configuration.deserialize_json(
