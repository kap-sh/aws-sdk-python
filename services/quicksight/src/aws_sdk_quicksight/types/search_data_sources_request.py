...
class SearchDataSourcesRequest(TypedDict):
...
def serialize_json(value: SearchDataSourcesRequest) -> dict:
    ...
    out["Filters"] = (
        aws_sdk_quicksight.types.data_source_search_filter_list.serialize_json(
            value["filters"]
        )
    )
...
def deserialize_json(data: dict) -> SearchDataSourcesRequest:
    ...
    if "Filters" in data:
        ...
        out["filters"] = (
            aws_sdk_quicksight.types.data_source_search_filter_list.deserialize_json(
                data["Filters"]
            )
        )
    else:
