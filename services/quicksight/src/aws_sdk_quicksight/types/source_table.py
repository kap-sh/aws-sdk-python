...
class SourceTable(TypedDict):
    ...
    data_set: NotRequired["aws_sdk_quicksight.types.parent_data_set.ParentDataSet"]
...
def serialize_json(value: SourceTable) -> dict:
    ...
    if "data_set" in value:
        ...
        out["DataSet"] = aws_sdk_quicksight.types.parent_data_set.serialize_json(
            value["data_set"]
        )
    return out
...
def deserialize_json(data: dict) -> SourceTable:
    ...
    if "DataSet" in data:
        ...
        out["data_set"] = aws_sdk_quicksight.types.parent_data_set.deserialize_json(
            data["DataSet"]
        )
    return out
