"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateSpaceResponse``."""
...
class CreateSpaceResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: CreateSpaceResponse) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> CreateSpaceResponse:
    out: CreateSpaceResponse = {}  # type: ignore[typeddict-item]
    if "spaceId" in data:
        out["space_id"] = data["spaceId"]
    else:
        raise DeserializationError("CreateSpaceResponse.space_id required")
    if "spaceArn" in data:
