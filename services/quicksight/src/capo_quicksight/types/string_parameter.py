...
class StringParameter(TypedDict, closed=True):
    name: "capo_quicksight.types.non_empty_string.NonEmptyString"
    """<p>A display name for a string parameter.</p>"""
    values: "capo_quicksight.types.sensitive_string_list.SensitiveStringList"
...
def serialize_json(value: StringParameter) -> dict:
    ...
    out["Values"] = capo_quicksight.types.sensitive_string_list.serialize_json(
        value["values"]
    )
    return out
