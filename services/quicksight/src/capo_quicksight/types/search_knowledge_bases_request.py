...
class SearchKnowledgeBasesRequest(TypedDict, closed=True):
    ...
    filters: NotRequired[
        "capo_quicksight.types.knowledge_base_search_filters.KnowledgeBaseSearchFilters"
    ]
    """<p>The filters to apply when searching knowledge bases.</p>"""
    sort_by: NotRequired[
        "capo_quicksight.types.knowledge_base_sort_by.KnowledgeBaseSortBy"
    ]
...
def serialize_json(value: SearchKnowledgeBasesRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["Filters"] = (
            capo_quicksight.types.knowledge_base_search_filters.serialize_json(
                value["filters"]
            )
        )
    if "sort_by" in value:
        ...
        out["SortBy"] = capo_quicksight.types.knowledge_base_sort_by.serialize_json(
            value["sort_by"]
        )
    return out
...
def deserialize_json(data: dict) -> SearchKnowledgeBasesRequest:
    ...
    if "Filters" in data:
        ...
        out["filters"] = (
            capo_quicksight.types.knowledge_base_search_filters.deserialize_json(
                data["Filters"]
            )
        )
    if "SortBy" in data:
