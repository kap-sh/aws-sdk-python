...
class DescribeAutomationJobResponse(TypedDict, closed=True):
    ...
    job_status: "capo_quicksight.types.automation_job_status.AutomationJobStatus"
...
def serialize_json(value: DescribeAutomationJobResponse) -> dict:
    ...
    if "started_at" in value:
        ...
        out["StartedAt"] = capo_quicksight.types._prelude.timestamp.serialize_json(
            value["started_at"]
        )
    if "ended_at" in value:
        import capo_quicksight.types._prelude.timestamp

        out["EndedAt"] = capo_quicksight.types._prelude.timestamp.serialize_json(
            value["ended_at"]
        )
    import capo_quicksight.types.automation_job_status

    out["JobStatus"] = capo_quicksight.types.automation_job_status.serialize_json(
        value["job_status"]
    )
...
def deserialize_json(data: dict) -> DescribeAutomationJobResponse:
    ...
    if "JobStatus" in data:
        ...
        out["job_status"] = (
            capo_quicksight.types.automation_job_status.deserialize_json(
                data["JobStatus"]
            )
        )
    else:
