...
class SelfUpgradeRequestDetail(TypedDict, closed=True):
    upgrade_request_id: NotRequired["capo_quicksight.types.string.String"]
    ...
    original_role: NotRequired["capo_quicksight.types.user_role.UserRole"]
    """<p>The original role of the user before the upgrade.</p>"""
    requested_role: NotRequired["capo_quicksight.types.user_role.UserRole"]
    """<p>The role that the user is requesting to upgrade to.</p>"""
    request_note: NotRequired["capo_quicksight.types.string.String"]
    """<p>An optional note explaining the reason for the self-upgrade request.</p>"""
    creation_time: "capo_quicksight.types.long.Long"
    """<p>The time when the self-upgrade request was created.</p>"""
    request_status: NotRequired[
        "capo_quicksight.types.self_upgrade_request_status.SelfUpgradeRequestStatus"
    ]
    """<p>The status of the self-upgrade request.</p>"""
    last_update_attempt_time: "capo_quicksight.types.long.Long"
    """<p>The time of the last update attempt for the self-upgrade request.</p>"""
    last_update_failure_reason: NotRequired["capo_quicksight.types.string.String"]
...
def serialize_json(value: SelfUpgradeRequestDetail) -> dict:
    ...
    if "original_role" in value:
        ...
        out["OriginalRole"] = capo_quicksight.types.user_role.serialize_json(
            value["original_role"]
        )
    if "requested_role" in value:
        import capo_quicksight.types.user_role

        out["RequestedRole"] = capo_quicksight.types.user_role.serialize_json(
    ...
    if "request_status" in value:
        ...
        out["RequestStatus"] = (
            capo_quicksight.types.self_upgrade_request_status.serialize_json(
...
def deserialize_json(data: dict) -> SelfUpgradeRequestDetail:
    ...
    if "RequestStatus" in data:
        ...
        out["request_status"] = (
            capo_quicksight.types.self_upgrade_request_status.deserialize_json(
                data["RequestStatus"]
            )
        )
