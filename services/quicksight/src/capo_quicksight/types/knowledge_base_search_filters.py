...
if TYPE_CHECKING:
    import capo_quicksight.types.knowledge_base_search_filter

KnowledgeBaseSearchFilters: TypeAlias = list[
    "capo_quicksight.types.knowledge_base_search_filter.KnowledgeBaseSearchFilter"
]
...
def serialize_json(value: KnowledgeBaseSearchFilters) -> list:
    ...
    for item in value:
        out.append(
            capo_quicksight.types.knowledge_base_search_filter.serialize_json(item)
        )
    return out
