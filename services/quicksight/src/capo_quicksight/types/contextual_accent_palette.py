...
class ContextualAccentPalette(TypedDict, closed=True):
    connection: NotRequired["capo_quicksight.types.palette.Palette"]
    visualization: NotRequired["capo_quicksight.types.palette.Palette"]
    insight: NotRequired["capo_quicksight.types.palette.Palette"]
    automation: NotRequired["capo_quicksight.types.palette.Palette"]
...
def serialize_json(value: ContextualAccentPalette) -> dict:
    ...
    if "connection" in value:
        ...
        out["Connection"] = capo_quicksight.types.palette.serialize_json(
            value["connection"]
        )
    if "visualization" in value:
        import capo_quicksight.types.palette

        out["Visualization"] = capo_quicksight.types.palette.serialize_json(
            value["visualization"]
        )
    if "insight" in value:
        import capo_quicksight.types.palette

        out["Insight"] = capo_quicksight.types.palette.serialize_json(value["insight"])
    if "automation" in value:
        import capo_quicksight.types.palette

        out["Automation"] = capo_quicksight.types.palette.serialize_json(
            value["automation"]
        )
    return out
