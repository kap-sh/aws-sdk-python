...
class NavbarStyle(TypedDict, closed=True):
    global_navbar: NotRequired["capo_quicksight.types.palette.Palette"]
    """<p>The global navigation bar style.</p>"""
    contextual_navbar: NotRequired["capo_quicksight.types.palette.Palette"]
...
def serialize_json(value: NavbarStyle) -> dict:
    ...
    if "global_navbar" in value:
        ...
        out["GlobalNavbar"] = capo_quicksight.types.palette.serialize_json(
            value["global_navbar"]
        )
    if "contextual_navbar" in value:
        import capo_quicksight.types.palette

        out["ContextualNavbar"] = capo_quicksight.types.palette.serialize_json(
