"""Generated from Smithy shape ``com.amazonaws.quicksight#TreeMapVisual``."""
...
if TYPE_CHECKING:
    ...
    import capo_quicksight.types.tree_map_configuration
...
class TreeMapVisual(TypedDict, closed=True):
    visual_id: (
    ...
    chart_configuration: NotRequired[
        "capo_quicksight.types.tree_map_configuration.TreeMapConfiguration"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: TreeMapVisual) -> dict:
    out: dict = {}
    ...
    if "chart_configuration" in value:
        ...
        out["ChartConfiguration"] = (
            capo_quicksight.types.tree_map_configuration.serialize_json(
...
def deserialize_json(data: dict) -> TreeMapVisual:
    out: TreeMapVisual = {}  # type: ignore[typeddict-item]
    if "VisualId" in data:
        out["visual_id"] = data["VisualId"]
    else:
        raise DeserializationError("TreeMapVisual.visual_id required")
    if "Title" in data:
        import capo_quicksight.types.visual_title_label_options
    ...
    if "ChartConfiguration" in data:
        ...
        out["chart_configuration"] = (
            capo_quicksight.types.tree_map_configuration.deserialize_json(
