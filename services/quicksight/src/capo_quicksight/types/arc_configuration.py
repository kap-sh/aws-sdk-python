...
class ArcConfiguration(TypedDict, closed=True):
    arc_angle: NotRequired["capo_quicksight.types.double.Double"]
    """<p>The option that determines the arc angle of a <code>GaugeChartVisual</code>.</p>"""
    arc_thickness: NotRequired[
        "capo_quicksight.types.arc_thickness_options.ArcThicknessOptions"
    ]
...
def serialize_json(value: ArcConfiguration) -> dict:
    ...
    if "arc_thickness" in value:
        ...
        out["ArcThickness"] = (
            capo_quicksight.types.arc_thickness_options.serialize_json(
                value["arc_thickness"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ArcConfiguration:
    ...
    if "ArcThickness" in data:
        ...
        out["arc_thickness"] = (
            capo_quicksight.types.arc_thickness_options.deserialize_json(
                data["ArcThickness"]
            )
        )
    return out
