...
class TopicIRComparisonMethod(TypedDict, closed=True):
    type: NotRequired[
        "capo_quicksight.types.comparison_method_type.ComparisonMethodType"
    ]
    ...
    window_size: "capo_quicksight.types.integer.Integer"
...
def serialize_json(value: TopicIRComparisonMethod) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = capo_quicksight.types.comparison_method_type.serialize_json(
            value["type"]
        )
    if "period" in value:
        import capo_quicksight.types.topic_time_granularity

        out["Period"] = capo_quicksight.types.topic_time_granularity.serialize_json(
            value["period"]
        )
