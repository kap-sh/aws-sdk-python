...
if TYPE_CHECKING:
    import capo_quicksight.types.asset_bundle_import_job_dashboard_override_tags
...
def serialize_json(value: AssetBundleImportJobDashboardOverrideTagsList) -> list:
    ...
    for item in value:
        out.append(
            capo_quicksight.types.asset_bundle_import_job_dashboard_override_tags.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> AssetBundleImportJobDashboardOverrideTagsList:
    ...
    for item in data:
        out.append(
            capo_quicksight.types.asset_bundle_import_job_dashboard_override_tags.deserialize_json(
                item
            )
        )
    return out
