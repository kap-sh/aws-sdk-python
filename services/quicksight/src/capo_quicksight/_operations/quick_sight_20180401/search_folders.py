"""Generated from Smithy shape ``com.amazonaws.quicksight#SearchFolders``."""
...
import capo_quicksight.types.folder_search_filter_list
import capo_quicksight.types.folder_summary_list
import capo_quicksight.types.search_folders_request
import capo_quicksight.types.search_folders_response
...
) -> capo_quicksight.types.search_folders_response.SearchFoldersResponse:
    out: capo_quicksight.types.search_folders_response.SearchFoldersResponse = (
        capo_quicksight.types.search_folders_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/search/folders"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.search_folders_request.serialize_json(input_)
    ).encode()
...
def search_folders(
    ...
    capo_quicksight.types.search_folders_response.SearchFoldersResponse, zapros.Response
]:
...
async def async_search_folders(
