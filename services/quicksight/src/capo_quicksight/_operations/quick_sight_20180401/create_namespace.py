"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateNamespace``."""
...
) -> capo_quicksight.types.create_namespace_response.CreateNamespaceResponse:
    out: capo_quicksight.types.create_namespace_response.CreateNamespaceResponse = (
        capo_quicksight.types.create_namespace_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.create_namespace_request.serialize_json(input_)
    ).encode()
