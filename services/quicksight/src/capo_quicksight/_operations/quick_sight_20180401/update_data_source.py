...
import capo_quicksight.errors.customer_managed_key_unavailable_exception
...
import capo_quicksight.types.update_data_source_request
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "CustomerManagedKeyUnavailableException":
            raise capo_quicksight.errors.customer_managed_key_unavailable_exception.CustomerManagedKeyUnavailableException.from_json(
                data
            )
...
) -> capo_quicksight.types.update_data_source_response.UpdateDataSourceResponse:
    out: capo_quicksight.types.update_data_source_response.UpdateDataSourceResponse = (
        capo_quicksight.types.update_data_source_response.deserialize_json(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_data_source_request.serialize_json(input_)
    ).encode()
