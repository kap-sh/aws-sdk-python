"""Generated from Smithy shape ``com.amazonaws.quicksight#SearchKnowledgeBases``."""
...
import capo_quicksight.types.search_knowledge_bases_request
import capo_quicksight.types.search_knowledge_bases_response
...
) -> capo_quicksight.types.search_knowledge_bases_response.SearchKnowledgeBasesResponse:
    out: capo_quicksight.types.search_knowledge_bases_response.SearchKnowledgeBasesResponse = capo_quicksight.types.search_knowledge_bases_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/accounts/{AwsAccountId}/search/knowledge-bases"
    )
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.search_knowledge_bases_request.serialize_json(input_)
    ).encode()
...
async def async_search_knowledge_bases(
