"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateFlow``."""
...
) -> capo_quicksight.types.create_flow_response.CreateFlowResponse:
    out: capo_quicksight.types.create_flow_response.CreateFlowResponse = (
        capo_quicksight.types.create_flow_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/flows"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.create_flow_request.serialize_json(input_)
    ).encode()
...
def create_flow(
    ...
    capo_quicksight.types.create_flow_response.CreateFlowResponse, zapros.Response
]:
