"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateActionConnectorPermissions``."""
...
import capo_quicksight.types.update_action_connector_permissions_request
import capo_quicksight.types.update_action_connector_permissions_response
...
) -> capo_quicksight.types.update_action_connector_permissions_response.UpdateActionConnectorPermissionsResponse:
    out: capo_quicksight.types.update_action_connector_permissions_response.UpdateActionConnectorPermissionsResponse = capo_quicksight.types.update_action_connector_permissions_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{ActionConnectorId}", quote(str(input_["action_connector_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_action_connector_permissions_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_action_connector_permissions(
