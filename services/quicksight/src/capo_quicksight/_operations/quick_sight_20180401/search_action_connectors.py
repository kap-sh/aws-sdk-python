"""Generated from Smithy shape ``com.amazonaws.quicksight#SearchActionConnectors``."""
...
import capo_quicksight.types.search_action_connectors_request
import capo_quicksight.types.search_action_connectors_response
...
) -> capo_quicksight.types.search_action_connectors_response.SearchActionConnectorsResponse:
    out: capo_quicksight.types.search_action_connectors_response.SearchActionConnectorsResponse = capo_quicksight.types.search_action_connectors_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/search/action-connectors"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    params["max-results"] = str(input_.get("max_results", 0))
    if "next_token" in input_:
        params["next-token"] = str(input_["next_token"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.search_action_connectors_request.serialize_json(input_)
    ).encode()
...
async def async_search_action_connectors(
