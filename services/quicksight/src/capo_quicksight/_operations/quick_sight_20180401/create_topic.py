"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateTopic``."""
...
) -> capo_quicksight.types.create_topic_response.CreateTopicResponse:
    out: capo_quicksight.types.create_topic_response.CreateTopicResponse = (
        capo_quicksight.types.create_topic_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/topics"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.create_topic_request.serialize_json(input_)
    ).encode()
...
def create_topic(
    ...
    capo_quicksight.types.create_topic_response.CreateTopicResponse, zapros.Response
]:
