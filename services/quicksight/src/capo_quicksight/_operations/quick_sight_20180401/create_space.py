"""Generated from Smithy shape ``com.amazonaws.quicksight#CreateSpace``."""
...
import capo_quicksight.types.create_space_request
import capo_quicksight.types.create_space_response
...
) -> capo_quicksight.types.create_space_response.CreateSpaceResponse:
    out: capo_quicksight.types.create_space_response.CreateSpaceResponse = (
        capo_quicksight.types.create_space_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/accounts/{AwsAccountId}/spaces"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.create_space_request.serialize_json(input_)
    ).encode()
...
def create_space(
    ...
    capo_quicksight.types.create_space_response.CreateSpaceResponse, zapros.Response
]:
