"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateFlow``."""
...
import capo_quicksight.types.update_flow_request
import capo_quicksight.types.update_flow_response
...
) -> capo_quicksight.types.update_flow_response.UpdateFlowResponse:
    out: capo_quicksight.types.update_flow_response.UpdateFlowResponse = (
        capo_quicksight.types.update_flow_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/flows/{FlowId}"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    url = url.replace("{FlowId}", quote(str(input_["flow_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_flow_request.serialize_json(input_)
    ).encode()
...
def update_flow(
    ...
    capo_quicksight.types.update_flow_response.UpdateFlowResponse, zapros.Response
]:
