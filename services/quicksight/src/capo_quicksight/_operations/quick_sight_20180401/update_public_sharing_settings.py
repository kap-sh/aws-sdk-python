"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdatePublicSharingSettings``."""
...
import capo_quicksight.types.update_public_sharing_settings_request
import capo_quicksight.types.update_public_sharing_settings_response
...
) -> capo_quicksight.types.update_public_sharing_settings_response.UpdatePublicSharingSettingsResponse:
    out: capo_quicksight.types.update_public_sharing_settings_response.UpdatePublicSharingSettingsResponse = capo_quicksight.types.update_public_sharing_settings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/public-sharing-settings"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_public_sharing_settings_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_public_sharing_settings(
