"""Generated from Smithy shape ``com.amazonaws.quicksight#UpdateIpRestriction``."""
...
import capo_quicksight.types.update_ip_restriction_request
import capo_quicksight.types.update_ip_restriction_response
...
) -> capo_quicksight.types.update_ip_restriction_response.UpdateIpRestrictionResponse:
    out: capo_quicksight.types.update_ip_restriction_response.UpdateIpRestrictionResponse = capo_quicksight.types.update_ip_restriction_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/ip-restriction"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.update_ip_restriction_request.serialize_json(input_)
    ).encode()
...
async def async_update_ip_restriction(
