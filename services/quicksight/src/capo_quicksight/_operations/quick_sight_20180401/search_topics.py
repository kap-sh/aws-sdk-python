"""Generated from Smithy shape ``com.amazonaws.quicksight#SearchTopics``."""
...
) -> capo_quicksight.types.search_topics_response.SearchTopicsResponse:
    out: capo_quicksight.types.search_topics_response.SearchTopicsResponse = (
        capo_quicksight.types.search_topics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AwsAccountId}/search/topics"
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.search_topics_request.serialize_json(input_)
    ).encode()
...
def search_topics(
    ...
    capo_quicksight.types.search_topics_response.SearchTopicsResponse, zapros.Response
]:
