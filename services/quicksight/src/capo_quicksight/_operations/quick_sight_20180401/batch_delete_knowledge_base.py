"""Generated from Smithy shape ``com.amazonaws.quicksight#BatchDeleteKnowledgeBase``."""
...
import capo_quicksight.errors.invalid_request_exception
...
import capo_quicksight.types.batch_delete_knowledge_base_request
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidRequestException":
            raise capo_quicksight.errors.invalid_request_exception.InvalidRequestException.from_json(
                data
            )
...
) -> capo_quicksight.types.batch_delete_knowledge_base_response.BatchDeleteKnowledgeBaseResponse:
    out: capo_quicksight.types.batch_delete_knowledge_base_response.BatchDeleteKnowledgeBaseResponse = capo_quicksight.types.batch_delete_knowledge_base_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/v1/accounts/{AwsAccountId}/knowledge-bases/batch-delete"
    )
    url = url.replace("{AwsAccountId}", quote(str(input_["aws_account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_quicksight.types.batch_delete_knowledge_base_request.serialize_json(input_)
    ).encode()
...
async def async_batch_delete_knowledge_base(
