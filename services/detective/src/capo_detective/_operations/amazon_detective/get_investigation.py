"""Generated from Smithy shape ``com.amazonaws.detective#GetInvestigation``."""
...
def handle_response(
    ...
    out: capo_detective.types.get_investigation_response.GetInvestigationResponse = (
        capo_detective.types.get_investigation_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/investigations/getInvestigation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.get_investigation_request.serialize_json(input_)
    ).encode()
...
async def async_get_investigation(
