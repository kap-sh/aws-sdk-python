"""Generated from Smithy shape ``com.amazonaws.detective#CreateGraph``."""
...
def handle_response(
    ...
    out: capo_detective.types.create_graph_response.CreateGraphResponse = (
        capo_detective.types.create_graph_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/graph"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.create_graph_request.serialize_json(input_)
    ).encode()
...
def create_graph(
    ...
    capo_detective.types.create_graph_response.CreateGraphResponse, zapros.Response
]:
