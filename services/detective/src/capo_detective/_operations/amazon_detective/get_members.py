"""Generated from Smithy shape ``com.amazonaws.detective#GetMembers``."""
...
def handle_response(
    ...
    out: capo_detective.types.get_members_response.GetMembersResponse = (
        capo_detective.types.get_members_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/graph/members/get"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_detective.types.get_members_request.serialize_json(input_)
    ).encode()
...
def get_members(
    ...
    capo_detective.types.get_members_response.GetMembersResponse, zapros.Response
]:
