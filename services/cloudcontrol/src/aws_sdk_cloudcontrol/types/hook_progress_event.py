...
if TYPE_CHECKING:
    ...
    import aws_sdk_cloudcontrol.types.hook_invocation_point
    import aws_sdk_cloudcontrol.types.hook_status
    import aws_sdk_cloudcontrol.types.hook_type_arn
    import aws_sdk_cloudcontrol.types.status_message
    import aws_sdk_cloudcontrol.types.timestamp
...
class HookProgressEvent(TypedDict, closed=True):
    ...
    hook_type_arn: NotRequired["aws_sdk_cloudcontrol.types.hook_type_arn.HookTypeArn"]
    """<p>The ARN of the Hook being invoked.</p>"""
    invocation_point: NotRequired[
        "aws_sdk_cloudcontrol.types.hook_invocation_point.HookInvocationPoint"
    ]
    """<p>States whether the Hook is invoked before or after resource provisioning.</p>"""
    hook_status: NotRequired["aws_sdk_cloudcontrol.types.hook_status.HookStatus"]
    ...
    hook_event_time: NotRequired["aws_sdk_cloudcontrol.types.timestamp.Timestamp"]
    """<p>The time that the Hook invocation request initiated.</p>"""
    hook_status_message: NotRequired[
        "aws_sdk_cloudcontrol.types.status_message.StatusMessage"
    ]
    """<p>The message explaining the current Hook status.</p>"""
    failure_mode: NotRequired[
        "aws_sdk_cloudcontrol.types.hook_failure_mode.HookFailureMode"
    ]
...
def serialize_aws_json_1_0(value: HookProgressEvent) -> dict:
    ...
    if "hook_event_time" in value:
        ...
        out["HookEventTime"] = (
            aws_sdk_cloudcontrol.types.timestamp.serialize_aws_json_1_0(
...
def deserialize_aws_json_1_0(data: dict) -> HookProgressEvent:
    ...
    if "HookEventTime" in data:
        ...
        out["hook_event_time"] = (
            aws_sdk_cloudcontrol.types.timestamp.deserialize_aws_json_1_0(
                data["HookEventTime"]
            )
        )
