"""Generated from Smithy shape ``com.amazonaws.controltower#ResetLandingZone``."""
...
def handle_response(
    ...
    out: capo_controltower.types.reset_landing_zone_output.ResetLandingZoneOutput = (
        capo_controltower.types.reset_landing_zone_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reset-landingzone"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.reset_landing_zone_input.serialize_json(input_)
    ).encode()
...
async def async_reset_landing_zone(
