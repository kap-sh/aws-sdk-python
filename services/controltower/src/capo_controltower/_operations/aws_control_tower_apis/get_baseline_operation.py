"""Generated from Smithy shape ``com.amazonaws.controltower#GetBaselineOperation``."""
...
import capo_controltower.types.baseline_operation
...
def handle_response(
    ...
    out: capo_controltower.types.get_baseline_operation_output.GetBaselineOperationOutput = capo_controltower.types.get_baseline_operation_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_controltower._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-baseline-operation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.get_baseline_operation_input.serialize_json(input_)
    ).encode()
...
async def async_get_baseline_operation(
