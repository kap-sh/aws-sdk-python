"""Generated from Smithy shape ``com.amazonaws.controltower#GetLandingZoneOperation``."""
...
def handle_response(
    ...
    out: capo_controltower.types.get_landing_zone_operation_output.GetLandingZoneOperationOutput = capo_controltower.types.get_landing_zone_operation_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-landingzone-operation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.get_landing_zone_operation_input.serialize_json(input_)
    ).encode()
...
async def async_get_landing_zone_operation(
