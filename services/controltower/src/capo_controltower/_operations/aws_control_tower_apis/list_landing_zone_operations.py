"""Generated from Smithy shape ``com.amazonaws.controltower#ListLandingZoneOperations``."""
...
def handle_response(
    ...
    out: capo_controltower.types.list_landing_zone_operations_output.ListLandingZoneOperationsOutput = capo_controltower.types.list_landing_zone_operations_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-landingzone-operations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.list_landing_zone_operations_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_landing_zone_operations(
