"""Generated from Smithy shape ``com.amazonaws.controltower#GetControlOperation``."""
...
def handle_response(
    ...
    out: capo_controltower.types.get_control_operation_output.GetControlOperationOutput = capo_controltower.types.get_control_operation_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/get-control-operation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.get_control_operation_input.serialize_json(input_)
    ).encode()
...
async def async_get_control_operation(
