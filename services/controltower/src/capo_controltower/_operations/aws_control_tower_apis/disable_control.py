"""Generated from Smithy shape ``com.amazonaws.controltower#DisableControl``."""
...
def handle_response(
    ...
    out: capo_controltower.types.disable_control_output.DisableControlOutput = (
        capo_controltower.types.disable_control_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/disable-control"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_controltower.types.disable_control_input.serialize_json(input_)
    ).encode()
...
def disable_control(
    ...
    capo_controltower.types.disable_control_output.DisableControlOutput, zapros.Response
]:
...
async def async_disable_control(
