...
if TYPE_CHECKING:
    import capo_controltower.types.enabled_baseline_parameter_summary

EnabledBaselineParameterSummaries: TypeAlias = list[
    "capo_controltower.types.enabled_baseline_parameter_summary.EnabledBaselineParameterSummary"
]
...
def serialize_json(value: EnabledBaselineParameterSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_controltower.types.enabled_baseline_parameter_summary.serialize_json(
                item
            )
        )
    return out
