...
def serialize_json(value: LandingZoneOperationSummary) -> dict:
    ...
    if "operation_type" in value:
        ...
        out["operationType"] = (
            capo_controltower.types.landing_zone_operation_type.serialize_json(
    ...
    if "status" in value:
        ...
        out["status"] = (
            capo_controltower.types.landing_zone_operation_status.serialize_json(
                value["status"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> LandingZoneOperationSummary:
    ...
    if "operationType" in data:
        ...
        out["operation_type"] = (
            capo_controltower.types.landing_zone_operation_type.deserialize_json(
                data["operationType"]
            )
        )
    ...
    if "status" in data:
        ...
        out["status"] = (
            capo_controltower.types.landing_zone_operation_status.deserialize_json(
                data["status"]
            )
        )
    return out
