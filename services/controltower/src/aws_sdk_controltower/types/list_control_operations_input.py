...
class ListControlOperationsInput(TypedDict):
    filter: NotRequired[
...
def serialize_json(value: ListControlOperationsInput) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = (
            aws_sdk_controltower.types.control_operation_filter.serialize_json(
                value["filter"]
            )
        )
...
def deserialize_json(data: dict) -> ListControlOperationsInput:
    ...
    if "filter" in data:
        ...
        out["filter"] = (
            aws_sdk_controltower.types.control_operation_filter.deserialize_json(
                data["filter"]
            )
        )
