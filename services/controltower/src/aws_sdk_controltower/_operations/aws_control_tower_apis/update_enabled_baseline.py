"""Generated from Smithy shape ``com.amazonaws.controltower#UpdateEnabledBaseline``."""
...
import aws_sdk_controltower.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            raise aws_sdk_controltower.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_controltower.types.update_enabled_baseline_output.UpdateEnabledBaselineOutput = aws_sdk_controltower.types.update_enabled_baseline_output.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_controltower._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_controltower._auth._sigv4.build_sigv4_auth_scheme(
                "controltower", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_controltower._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_controltower.types.update_enabled_baseline_input.UpdateEnabledBaselineInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/update-enabled-baseline"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_controltower.types.update_enabled_baseline_input.serialize_json(input_)
    ).encode()
...
async def async_update_enabled_baseline(
