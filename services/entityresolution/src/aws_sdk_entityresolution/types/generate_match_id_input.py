...
class GenerateMatchIdInput(TypedDict):
...
def serialize_json(value: GenerateMatchIdInput) -> dict:
    ...
    out["records"] = aws_sdk_entityresolution.types.record_list.serialize_json(
        value["records"]
    )
    if "processing_type" in value:
        ...
        out["processingType"] = (
            aws_sdk_entityresolution.types.processing_type.serialize_json(
...
def deserialize_json(data: dict) -> GenerateMatchIdInput:
    ...
    if "records" in data:
        ...
        out["records"] = aws_sdk_entityresolution.types.record_list.deserialize_json(
            data["records"]
        )
    else:
    ...
    if "processingType" in data:
        ...
        out["processing_type"] = (
            aws_sdk_entityresolution.types.processing_type.deserialize_json(
                data["processingType"]
            )
        )
    return out
