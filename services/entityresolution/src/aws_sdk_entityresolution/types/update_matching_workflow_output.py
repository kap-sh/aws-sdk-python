"""Generated from Smithy shape ``com.amazonaws.entityresolution#UpdateMatchingWorkflowOutput``."""
...
class UpdateMatchingWorkflowOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateMatchingWorkflowOutput) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> UpdateMatchingWorkflowOutput:
    out: UpdateMatchingWorkflowOutput = {}  # type: ignore[typeddict-item]
    if "workflowName" in data:
        out["workflow_name"] = data["workflowName"]
    else:
        raise DeserializationError(
            "UpdateMatchingWorkflowOutput.workflow_name required"
        )
    ...
    if "inputSourceConfig" in data:
        ...
        raise DeserializationError(
            "UpdateMatchingWorkflowOutput.input_source_config required"
        )
    if "outputSourceConfig" in data:
        ...
        raise DeserializationError(
            "UpdateMatchingWorkflowOutput.output_source_config required"
        )
    if "resolutionTechniques" in data:
        ...
        raise DeserializationError(
            "UpdateMatchingWorkflowOutput.resolution_techniques required"
        )
    ...
    if "roleArn" in data:
        ...
        raise DeserializationError("UpdateMatchingWorkflowOutput.role_arn required")
    return out
