"""Generated from Smithy shape ``com.amazonaws.entityresolution#GetMatchId``."""
...
def handle_response(response: zapros.Response, is_async: bool) -> aws_sdk_entityresolution.types.get_match_id_output.GetMatchIdOutput:
    import aws_sdk_entityresolution.types.get_match_id_output
    out: aws_sdk_entityresolution.types.get_match_id_output.GetMatchIdOutput = aws_sdk_entityresolution.types.get_match_id_output.deserialize_json(json.loads(response.read()))
    return out
...
def build_request(options: OperationOptions | AsyncOperationOptions, input: aws_sdk_entityresolution.types.get_match_id_input.GetMatchIdInput) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/matchingworkflows/{workflowName}/matches"
    url = url.replace("{workflowName}", quote(str(input["workflow_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_entityresolution.types.get_match_id_input
    body: bytes | None = json.dumps(aws_sdk_entityresolution.types.get_match_id_input.serialize_json(input)).encode()
...
def get_match_id(options: OperationOptions, input: aws_sdk_entityresolution.types.get_match_id_input.GetMatchIdInput) -> tuple[aws_sdk_entityresolution.types.get_match_id_output.GetMatchIdOutput, zapros.Response]:
...
async def async_get_match_id(options: AsyncOperationOptions, input: aws_sdk_entityresolution.types.get_match_id_input.GetMatchIdInput) -> tuple[aws_sdk_entityresolution.types.get_match_id_output.GetMatchIdOutput, zapros.Response]:
