...
def handle_response(
    ...
    out: aws_sdk_entityresolution.types.update_id_namespace_output.UpdateIdNamespaceOutput = aws_sdk_entityresolution.types.update_id_namespace_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_entityresolution.types.update_id_namespace_input.UpdateIdNamespaceInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{idNamespaceName}", quote(str(input_["id_namespace_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_entityresolution.types.update_id_namespace_input.serialize_json(input_)
    ).encode()
...
async def async_update_id_namespace(
