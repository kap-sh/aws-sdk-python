"""Generated from Smithy shape ``com.amazonaws.entityresolution#PutPolicy``."""
...
def handle_response(response: zapros.Response, is_async: bool) -> aws_sdk_entityresolution.types.put_policy_output.PutPolicyOutput:
    import aws_sdk_entityresolution.types.put_policy_output
    out: aws_sdk_entityresolution.types.put_policy_output.PutPolicyOutput = aws_sdk_entityresolution.types.put_policy_output.deserialize_json(json.loads(response.read()))
    return out
...
def build_request(options: OperationOptions | AsyncOperationOptions, input: aws_sdk_entityresolution.types.put_policy_input.PutPolicyInput) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/policies/{arn}"
    url = url.replace("{arn}", quote(str(input["arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_entityresolution.types.put_policy_input
    body: bytes | None = json.dumps(aws_sdk_entityresolution.types.put_policy_input.serialize_json(input)).encode()
...
def put_policy(options: OperationOptions, input: aws_sdk_entityresolution.types.put_policy_input.PutPolicyInput) -> tuple[aws_sdk_entityresolution.types.put_policy_output.PutPolicyOutput, zapros.Response]:
...
async def async_put_policy(options: AsyncOperationOptions, input: aws_sdk_entityresolution.types.put_policy_input.PutPolicyInput) -> tuple[aws_sdk_entityresolution.types.put_policy_output.PutPolicyOutput, zapros.Response]:
