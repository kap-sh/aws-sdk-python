...
def build_request(
    ...
    input_: aws_sdk_entityresolution.types.batch_delete_unique_id_input.BatchDeleteUniqueIdInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "input_source" in input_:
        headers["inputSource"] = str(input_["input_source"])
    if "unique_ids" in input_:
        headers["uniqueIds"] = str(input_["unique_ids"])
    body: bytes | None = b""
...
async def async_batch_delete_unique_id(
