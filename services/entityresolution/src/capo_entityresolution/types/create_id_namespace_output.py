"""Generated from Smithy shape ``com.amazonaws.entityresolution#CreateIdNamespaceOutput``."""
...
class CreateIdNamespaceOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: CreateIdNamespaceOutput) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> CreateIdNamespaceOutput:
    out: CreateIdNamespaceOutput = {}  # type: ignore[typeddict-item]
    if "idNamespaceName" in data:
        out["id_namespace_name"] = data["idNamespaceName"]
    else:
        raise DeserializationError("CreateIdNamespaceOutput.id_namespace_name required")
    if "idNamespaceArn" in data:
        out["id_namespace_arn"] = data["idNamespaceArn"]
    else:
        raise DeserializationError("CreateIdNamespaceOutput.id_namespace_arn required")
    ...
    if "type" in data:
        ...
        raise DeserializationError("CreateIdNamespaceOutput.type required")
    if "roleArn" in data:
    ...
    if "createdAt" in data:
        ...
        raise DeserializationError("CreateIdNamespaceOutput.created_at required")
    if "updatedAt" in data:
        ...
        raise DeserializationError("CreateIdNamespaceOutput.updated_at required")
    if "tags" in data:
        import capo_entityresolution.types.tag_map
