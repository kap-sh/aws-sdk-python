"""Generated from Smithy shape ``com.amazonaws.entityresolution#CreateIdNamespace``."""
...
def handle_response(
    ...
    out: capo_entityresolution.types.create_id_namespace_output.CreateIdNamespaceOutput = capo_entityresolution.types.create_id_namespace_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/idnamespaces"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_entityresolution.types.create_id_namespace_input.serialize_json(input_)
    ).encode()
...
async def async_create_id_namespace(
