"""Generated from Smithy shape ``com.amazonaws.entityresolution#CreateMatchingWorkflow``."""
...
def handle_response(
    ...
    out: capo_entityresolution.types.create_matching_workflow_output.CreateMatchingWorkflowOutput = capo_entityresolution.types.create_matching_workflow_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/matchingworkflows"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_entityresolution.types.create_matching_workflow_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_matching_workflow(
