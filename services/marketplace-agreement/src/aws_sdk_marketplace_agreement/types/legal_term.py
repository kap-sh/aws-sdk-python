"""Generated from Smithy shape ``com.amazonaws.marketplaceagreement#LegalTerm``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import aws_sdk_marketplace_agreement.types.document_list
    import aws_sdk_marketplace_agreement.types.term_id
    import aws_sdk_marketplace_agreement.types.unversioned_term_type


class LegalTerm(TypedDict, closed=True):
    type: NotRequired[
        "aws_sdk_marketplace_agreement.types.unversioned_term_type.UnversionedTermType"
    ]
    """<p>Category of the term being updated.</p>"""
    id: NotRequired["aws_sdk_marketplace_agreement.types.term_id.TermId"]
    """<p>The unique identifer for the term.</p>"""
    documents: NotRequired[
        "aws_sdk_marketplace_agreement.types.document_list.DocumentList"
    ]
    """<p>List of references to legal resources proposed to the buyers. An example is the EULA.</p>"""


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: LegalTerm) -> dict:
    out: dict = {}
    if "type" in value:
        out["type"] = value["type"]
    if "id" in value:
        out["id"] = value["id"]
    if "documents" in value:
        import aws_sdk_marketplace_agreement.types.document_list

        out["documents"] = (
            aws_sdk_marketplace_agreement.types.document_list.serialize_aws_json_1_0(
                value["documents"]
            )
        )
    return out


def deserialize_aws_json_1_0(data: dict) -> LegalTerm:
    out: LegalTerm = {}  # type: ignore[typeddict-item]
    if "type" in data:
        out["type"] = data["type"]
    if "id" in data:
        out["id"] = data["id"]
    if "documents" in data:
        import aws_sdk_marketplace_agreement.types.document_list

        out["documents"] = (
            aws_sdk_marketplace_agreement.types.document_list.deserialize_aws_json_1_0(
                data["documents"]
            )
        )
    return out
