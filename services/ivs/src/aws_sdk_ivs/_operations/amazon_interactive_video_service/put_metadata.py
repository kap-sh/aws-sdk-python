"""Generated from Smithy shape ``com.amazonaws.ivs#PutMetadata``."""
...
def build_request(
    ...
    input_: aws_sdk_ivs.types.put_metadata_request.PutMetadataRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/PutMetadata"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_ivs.types.put_metadata_request.serialize_json(input_)
    ).encode()
