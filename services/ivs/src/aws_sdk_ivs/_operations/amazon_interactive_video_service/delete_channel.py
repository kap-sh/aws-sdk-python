"""Generated from Smithy shape ``com.amazonaws.ivs#DeleteChannel``."""
...
def build_request(
    ...
    input_: aws_sdk_ivs.types.delete_channel_request.DeleteChannelRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteChannel"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_ivs.types.delete_channel_request

    body: bytes | None = json.dumps(
        aws_sdk_ivs.types.delete_channel_request.serialize_json(input_)
    ).encode()
