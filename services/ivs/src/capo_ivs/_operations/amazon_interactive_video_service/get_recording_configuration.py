"""Generated from Smithy shape ``com.amazonaws.ivs#GetRecordingConfiguration``."""
...
def handle_response(
    ...
    out: capo_ivs.types.get_recording_configuration_response.GetRecordingConfigurationResponse = capo_ivs.types.get_recording_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetRecordingConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.get_recording_configuration_request.serialize_json(input_)
    ).encode()
...
async def async_get_recording_configuration(
