"""Generated from Smithy shape ``com.amazonaws.ivs#ListStreamKeys``."""
...
def handle_response(
    ...
    out: capo_ivs.types.list_stream_keys_response.ListStreamKeysResponse = (
        capo_ivs.types.list_stream_keys_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListStreamKeys"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.list_stream_keys_request.serialize_json(input_)
    ).encode()
...
def list_stream_keys(
    ...
    capo_ivs.types.list_stream_keys_response.ListStreamKeysResponse, zapros.Response
]:
...
async def async_list_stream_keys(
