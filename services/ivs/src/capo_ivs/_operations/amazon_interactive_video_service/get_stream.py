"""Generated from Smithy shape ``com.amazonaws.ivs#GetStream``."""
...
import capo_ivs.errors.channel_not_broadcasting
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ChannelNotBroadcasting":
            raise capo_ivs.errors.channel_not_broadcasting.ChannelNotBroadcasting.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_ivs.types.get_stream_response.GetStreamResponse = (
        capo_ivs.types.get_stream_response.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_ivs.types.get_stream_response.GetStreamResponse = (
        capo_ivs.types.get_stream_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetStream"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.get_stream_request.serialize_json(input_)
    ).encode()
...
def get_stream(
    ...
    input_: capo_ivs.types.get_stream_request.GetStreamRequest,
) -> tuple[capo_ivs.types.get_stream_response.GetStreamResponse, zapros.Response]:
