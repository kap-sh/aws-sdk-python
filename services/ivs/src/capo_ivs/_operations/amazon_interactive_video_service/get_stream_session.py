"""Generated from Smithy shape ``com.amazonaws.ivs#GetStreamSession``."""
...
def handle_response(
    ...
    out: capo_ivs.types.get_stream_session_response.GetStreamSessionResponse = (
        capo_ivs.types.get_stream_session_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetStreamSession"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.get_stream_session_request.serialize_json(input_)
    ).encode()
...
def get_stream_session(
    ...
    capo_ivs.types.get_stream_session_response.GetStreamSessionResponse, zapros.Response
]:
