"""Generated from Smithy shape ``com.amazonaws.ivs#GetPlaybackRestrictionPolicy``."""
...
def handle_response(
    ...
    out: capo_ivs.types.get_playback_restriction_policy_response.GetPlaybackRestrictionPolicyResponse = capo_ivs.types.get_playback_restriction_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetPlaybackRestrictionPolicy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.get_playback_restriction_policy_request.serialize_json(input_)
    ).encode()
...
async def async_get_playback_restriction_policy(
