"""Generated from Smithy shape ``com.amazonaws.ivs#ListStreamSessions``."""
...
import capo_ivs.types.stream_session_list
...
def handle_response(
    ...
    out: capo_ivs.types.list_stream_sessions_response.ListStreamSessionsResponse = (
        capo_ivs.types.list_stream_sessions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListStreamSessions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.list_stream_sessions_request.serialize_json(input_)
    ).encode()
