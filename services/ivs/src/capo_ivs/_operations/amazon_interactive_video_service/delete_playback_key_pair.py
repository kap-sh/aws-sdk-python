"""Generated from Smithy shape ``com.amazonaws.ivs#DeletePlaybackKeyPair``."""
...
def handle_response(
    ...
    out: capo_ivs.types.delete_playback_key_pair_response.DeletePlaybackKeyPairResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeletePlaybackKeyPair"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs.types.delete_playback_key_pair_request.serialize_json(input_)
    ).encode()
...
async def async_delete_playback_key_pair(
