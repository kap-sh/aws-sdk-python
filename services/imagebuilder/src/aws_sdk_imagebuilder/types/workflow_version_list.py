...
WorkflowVersionList: TypeAlias = list[
    "aws_sdk_imagebuilder.types.workflow_version.WorkflowVersion"
]
...
def serialize_json(value: WorkflowVersionList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_imagebuilder.types.workflow_version.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> WorkflowVersionList:
    ...
    for item in data:
        out.append(aws_sdk_imagebuilder.types.workflow_version.deserialize_json(item))
    return out
