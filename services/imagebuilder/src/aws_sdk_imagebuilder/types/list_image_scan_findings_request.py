...
class ListImageScanFindingsRequest(TypedDict):
    filters: NotRequired[
...
def serialize_json(value: ListImageScanFindingsRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            aws_sdk_imagebuilder.types.image_scan_findings_filter_list.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListImageScanFindingsRequest:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            aws_sdk_imagebuilder.types.image_scan_findings_filter_list.deserialize_json(
                data["filters"]
            )
        )
