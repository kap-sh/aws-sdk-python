...
if TYPE_CHECKING:
    import aws_sdk_imagebuilder.types.lifecycle_execution_resource_status
    import aws_sdk_imagebuilder.types.non_empty_string


class LifecycleExecutionResourceState(TypedDict):
    status: NotRequired[
        "aws_sdk_imagebuilder.types.lifecycle_execution_resource_status.LifecycleExecutionResourceStatus"
    ]
...
def serialize_json(value: LifecycleExecutionResourceState) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = (
            aws_sdk_imagebuilder.types.lifecycle_execution_resource_status.serialize_json(
                value["status"]
            )
        )
    if "reason" in value:
...
def deserialize_json(data: dict) -> LifecycleExecutionResourceState:
    ...
    if "status" in data:
        ...
        out["status"] = (
            aws_sdk_imagebuilder.types.lifecycle_execution_resource_status.deserialize_json(
                data["status"]
            )
        )
    if "reason" in data:
