...
if TYPE_CHECKING:
    import aws_sdk_imagebuilder.types.container_repository_service
...
def serialize_json(value: TargetContainerRepository) -> dict:
    ...
    out["service"] = (
        aws_sdk_imagebuilder.types.container_repository_service.serialize_json(
            value["service"]
        )
    )
...
def deserialize_json(data: dict) -> TargetContainerRepository:
    ...
    if "service" in data:
        ...
        out["service"] = (
            aws_sdk_imagebuilder.types.container_repository_service.deserialize_json(
                data["service"]
            )
        )
    else:
