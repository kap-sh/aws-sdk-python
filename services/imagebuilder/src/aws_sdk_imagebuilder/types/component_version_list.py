...
ComponentVersionList: TypeAlias = list[
    "aws_sdk_imagebuilder.types.component_version.ComponentVersion"
]
...
def serialize_json(value: ComponentVersionList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_imagebuilder.types.component_version.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ComponentVersionList:
    ...
    for item in data:
        out.append(aws_sdk_imagebuilder.types.component_version.deserialize_json(item))
    return out
