...
ImageScanFindingsList: TypeAlias = list[
    "aws_sdk_imagebuilder.types.image_scan_finding.ImageScanFinding"
]
...
def serialize_json(value: ImageScanFindingsList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_imagebuilder.types.image_scan_finding.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ImageScanFindingsList:
    ...
    for item in data:
        out.append(aws_sdk_imagebuilder.types.image_scan_finding.deserialize_json(item))
    return out
