...
ComponentSummaryList: TypeAlias = list[
    "aws_sdk_imagebuilder.types.component_summary.ComponentSummary"
]
...
def serialize_json(value: ComponentSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_imagebuilder.types.component_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ComponentSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_imagebuilder.types.component_summary.deserialize_json(item))
    return out
