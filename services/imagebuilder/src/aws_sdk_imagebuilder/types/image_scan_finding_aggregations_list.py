...
ImageScanFindingAggregationsList: TypeAlias = list[
    "aws_sdk_imagebuilder.types.image_scan_finding_aggregation.ImageScanFindingAggregation"
]
...
def serialize_json(value: ImageScanFindingAggregationsList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_imagebuilder.types.image_scan_finding_aggregation.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ImageScanFindingAggregationsList:
    ...
    for item in data:
        out.append(
            aws_sdk_imagebuilder.types.image_scan_finding_aggregation.deserialize_json(
                item
            )
        )
    return out
