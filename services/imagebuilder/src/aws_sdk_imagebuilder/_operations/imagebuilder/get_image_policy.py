"""Generated from Smithy shape ``com.amazonaws.imagebuilder#GetImagePolicy``."""
...
def handle_response(
    ...
    out: aws_sdk_imagebuilder.types.get_image_policy_response.GetImagePolicyResponse = (
        aws_sdk_imagebuilder.types.get_image_policy_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_imagebuilder.types.get_image_policy_request.GetImagePolicyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetImagePolicy"
    params: dict[str, str] = {}
    if "image_arn" in input_:
        params["imageArn"] = str(input_["image_arn"])
...
async def async_get_image_policy(
