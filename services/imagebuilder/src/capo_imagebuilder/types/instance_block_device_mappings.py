...
if TYPE_CHECKING:
    import capo_imagebuilder.types.instance_block_device_mapping

InstanceBlockDeviceMappings: TypeAlias = list[
    "capo_imagebuilder.types.instance_block_device_mapping.InstanceBlockDeviceMapping"
]
...
def serialize_json(value: InstanceBlockDeviceMappings) -> list:
    ...
    for item in value:
        out.append(
            capo_imagebuilder.types.instance_block_device_mapping.serialize_json(item)
        )
    return out
