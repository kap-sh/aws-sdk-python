...
if TYPE_CHECKING:
    import capo_imagebuilder.types.image_build_message
...
class ListWorkflowStepExecutionsResponse(TypedDict, closed=True):
    ...
    steps: NotRequired[
        "capo_imagebuilder.types.workflow_step_executions_list.WorkflowStepExecutionsList"
    ]
...
def serialize_json(value: ListWorkflowStepExecutionsResponse) -> dict:
    ...
    if "steps" in value:
        ...
        out["steps"] = (
            capo_imagebuilder.types.workflow_step_executions_list.serialize_json(
                value["steps"]
            )
        )
...
def deserialize_json(data: dict) -> ListWorkflowStepExecutionsResponse:
    ...
    if "steps" in data:
        ...
        out["steps"] = (
            capo_imagebuilder.types.workflow_step_executions_list.deserialize_json(
                data["steps"]
            )
        )
