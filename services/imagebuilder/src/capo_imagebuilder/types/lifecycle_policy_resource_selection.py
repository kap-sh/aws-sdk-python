...
class LifecyclePolicyResourceSelection(TypedDict, closed=True):
    recipes: NotRequired[
        "capo_imagebuilder.types.lifecycle_policy_resource_selection_recipes.LifecyclePolicyResourceSelectionRecipes"
    ]
...
def serialize_json(value: LifecyclePolicyResourceSelection) -> dict:
    ...
    if "recipes" in value:
        ...
        out["recipes"] = (
            capo_imagebuilder.types.lifecycle_policy_resource_selection_recipes.serialize_json(
                value["recipes"]
            )
        )
    if "tag_map" in value:
...
def deserialize_json(data: dict) -> LifecyclePolicyResourceSelection:
    ...
    if "recipes" in data:
        ...
        out["recipes"] = (
            capo_imagebuilder.types.lifecycle_policy_resource_selection_recipes.deserialize_json(
                data["recipes"]
            )
        )
    if "tagMap" in data:
