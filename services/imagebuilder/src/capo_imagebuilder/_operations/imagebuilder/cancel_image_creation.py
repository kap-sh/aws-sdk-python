"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CancelImageCreation``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.cancel_image_creation_response.CancelImageCreationResponse = capo_imagebuilder.types.cancel_image_creation_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CancelImageCreation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.cancel_image_creation_request.serialize_json(input_)
    ).encode()
...
async def async_cancel_image_creation(
