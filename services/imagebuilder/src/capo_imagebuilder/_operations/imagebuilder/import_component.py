"""Generated from Smithy shape ``com.amazonaws.imagebuilder#ImportComponent``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.import_component_response.ImportComponentResponse = (
        capo_imagebuilder.types.import_component_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ImportComponent"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.import_component_request.serialize_json(input_)
    ).encode()
...
async def async_import_component(
