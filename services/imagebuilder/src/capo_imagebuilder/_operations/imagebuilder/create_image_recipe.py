"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CreateImageRecipe``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.create_image_recipe_response.CreateImageRecipeResponse = capo_imagebuilder.types.create_image_recipe_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateImageRecipe"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.create_image_recipe_request.serialize_json(input_)
    ).encode()
...
async def async_create_image_recipe(
