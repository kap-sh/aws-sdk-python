"""Generated from Smithy shape ``com.amazonaws.imagebuilder#ImportVmImage``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.import_vm_image_response.ImportVmImageResponse = (
        capo_imagebuilder.types.import_vm_image_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ImportVmImage"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.import_vm_image_request.serialize_json(input_)
    ).encode()
...
async def async_import_vm_image(
