"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CreateContainerRecipe``."""
...
import capo_imagebuilder.errors.invalid_version_number_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidVersionNumberException":
            raise capo_imagebuilder.errors.invalid_version_number_exception.InvalidVersionNumberException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_imagebuilder.types.create_container_recipe_response.CreateContainerRecipeResponse = capo_imagebuilder.types.create_container_recipe_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateContainerRecipe"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.create_container_recipe_request.serialize_json(input_)
    ).encode()
...
async def async_create_container_recipe(
