"""Generated from Smithy shape ``com.amazonaws.imagebuilder#CreateWorkflow``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.create_workflow_response.CreateWorkflowResponse = (
        capo_imagebuilder.types.create_workflow_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateWorkflow"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.create_workflow_request.serialize_json(input_)
    ).encode()
