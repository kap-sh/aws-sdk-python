"""Generated from Smithy shape ``com.amazonaws.imagebuilder#ListContainerRecipes``."""
...
def handle_response(
    ...
    out: capo_imagebuilder.types.list_container_recipes_response.ListContainerRecipesResponse = capo_imagebuilder.types.list_container_recipes_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListContainerRecipes"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_imagebuilder.types.list_container_recipes_request.serialize_json(input_)
    ).encode()
...
async def async_list_container_recipes(
