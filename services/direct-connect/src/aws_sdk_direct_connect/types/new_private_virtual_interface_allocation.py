"""Generated from Smithy shape ``com.amazonaws.directconnect#NewPrivateVirtualInterfaceAllocation``."""
...
class NewPrivateVirtualInterfaceAllocation(TypedDict, closed=True):
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: NewPrivateVirtualInterfaceAllocation) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_1(data: dict) -> NewPrivateVirtualInterfaceAllocation:
    out: NewPrivateVirtualInterfaceAllocation = {}  # type: ignore[typeddict-item]
    if "virtualInterfaceName" in data:
        out["virtual_interface_name"] = data["virtualInterfaceName"]
    else:
        raise DeserializationError(
            "NewPrivateVirtualInterfaceAllocation.virtual_interface_name required"
        )
    if "vlan" in data:
        out["vlan"] = data["vlan"]
    else:
