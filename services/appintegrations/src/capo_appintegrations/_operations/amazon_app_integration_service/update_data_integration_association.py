"""Generated from Smithy shape ``com.amazonaws.appintegrations#UpdateDataIntegrationAssociation``."""
...
def handle_response(
    ...
    out: capo_appintegrations.types.update_data_integration_association_response.UpdateDataIntegrationAssociationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(str(input_["data_integration_identifier"]), safe=""),
    )
    url = url.replace(
        "{DataIntegrationAssociationIdentifier}",
        quote(str(input_["data_integration_association_identifier"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appintegrations.types.update_data_integration_association_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_data_integration_association(
