...
def serialize_json(value: FileConfiguration) -> dict:
    out: dict = {}
    import aws_sdk_appintegrations.types.folder_list
    out["Folders"] = aws_sdk_appintegrations.types.folder_list.serialize_json(value["folders"])
    if "filters" in value:
        import aws_sdk_appintegrations.types.fields_map
        out["Filters"] = aws_sdk_appintegrations.types.fields_map.serialize_json(value["filters"])
    return out
...
def deserialize_json(data: dict) -> FileConfiguration:
    ...
    if "Folders" in data:
        import aws_sdk_appintegrations.types.folder_list
        out["folders"] = aws_sdk_appintegrations.types.folder_list.deserialize_json(data["Folders"])
    else:
        raise DeserializationError("FileConfiguration.folders required")
    if "Filters" in data:
        import aws_sdk_appintegrations.types.fields_map
        out["filters"] = aws_sdk_appintegrations.types.fields_map.deserialize_json(data["Filters"])
    return out