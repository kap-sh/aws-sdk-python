"""Generated from Smithy shape ``com.amazonaws.opensearchserverless#UpdateAccessPolicyRequest``."""
...
class UpdateAccessPolicyRequest(TypedDict):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: UpdateAccessPolicyRequest) -> dict:
    out: dict = {}
    out["type"] = value["type"]
    out["name"] = value["name"]
...
def deserialize_aws_json_1_0(data: dict) -> UpdateAccessPolicyRequest:
    out: UpdateAccessPolicyRequest = {}  # type: ignore[typeddict-item]
    if "type" in data:
        out["type"] = data["type"]
    else:
        raise DeserializationError("UpdateAccessPolicyRequest.type required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("UpdateAccessPolicyRequest.name required")
    if "policyVersion" in data:
        out["policy_version"] = data["policyVersion"]
    else:
        raise DeserializationError("UpdateAccessPolicyRequest.policy_version required")
