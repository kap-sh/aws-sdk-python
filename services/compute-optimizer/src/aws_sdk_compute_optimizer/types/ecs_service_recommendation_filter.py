...
if TYPE_CHECKING:
    import aws_sdk_compute_optimizer.types.ecs_service_recommendation_filter_name
    import aws_sdk_compute_optimizer.types.filter_values


class ECSServiceRecommendationFilter(TypedDict):
    name: NotRequired[
        "aws_sdk_compute_optimizer.types.ecs_service_recommendation_filter_name.ECSServiceRecommendationFilterName"
    ]
...
def serialize_aws_json_1_0(value: ECSServiceRecommendationFilter) -> dict:
    ...
    if "name" in value:
        ...
        out["name"] = (
            aws_sdk_compute_optimizer.types.ecs_service_recommendation_filter_name.serialize_aws_json_1_0(
                value["name"]
            )
        )
    if "values" in value:
...
def deserialize_aws_json_1_0(data: dict) -> ECSServiceRecommendationFilter:
    ...
    if "name" in data:
        ...
        out["name"] = (
            aws_sdk_compute_optimizer.types.ecs_service_recommendation_filter_name.deserialize_aws_json_1_0(
                data["name"]
            )
        )
    if "values" in data:
