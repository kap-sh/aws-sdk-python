...
class EventTriggerLimits(TypedDict, closed=True):
    ...
    periods: NotRequired["capo_customer_profiles.types.periods.Periods"]
...
def serialize_json(value: EventTriggerLimits) -> dict:
    ...
    if "periods" in value:
        ...
        out["Periods"] = capo_customer_profiles.types.periods.serialize_json(
            value["periods"]
        )
    return out
