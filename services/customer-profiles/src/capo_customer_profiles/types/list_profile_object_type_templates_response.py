...
if TYPE_CHECKING:
    import capo_customer_profiles.types.profile_object_type_template_list
    import capo_customer_profiles.types.token


class ListProfileObjectTypeTemplatesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_customer_profiles.types.profile_object_type_template_list.ProfileObjectTypeTemplateList"
    ]
...
def serialize_json(value: ListProfileObjectTypeTemplatesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = (
            capo_customer_profiles.types.profile_object_type_template_list.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListProfileObjectTypeTemplatesResponse:
    ...
    if "Items" in data:
        ...
        out["items"] = (
            capo_customer_profiles.types.profile_object_type_template_list.deserialize_json(
                data["Items"]
            )
        )
