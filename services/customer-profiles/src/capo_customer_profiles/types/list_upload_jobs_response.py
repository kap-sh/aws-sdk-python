...
class ListUploadJobsResponse(TypedDict, closed=True):
    ...
    items: NotRequired["capo_customer_profiles.types.upload_jobs_list.UploadJobsList"]
...
def serialize_json(value: ListUploadJobsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = capo_customer_profiles.types.upload_jobs_list.serialize_json(
            value["items"]
        )
    return out
...
def deserialize_json(data: dict) -> ListUploadJobsResponse:
    ...
    if "Items" in data:
        ...
        out["items"] = capo_customer_profiles.types.upload_jobs_list.deserialize_json(
            data["Items"]
        )
    return out
