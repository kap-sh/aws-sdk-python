"""Generated from Smithy shape ``com.amazonaws.customerprofiles#CreateDomainLayout``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.create_domain_layout_response.CreateDomainLayoutResponse = capo_customer_profiles.types.create_domain_layout_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{LayoutDefinitionName}", quote(str(input_["layout_definition_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.create_domain_layout_request.serialize_json(input_)
    ).encode()
...
async def async_create_domain_layout(
