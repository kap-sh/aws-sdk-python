"""Generated from Smithy shape ``com.amazonaws.customerprofiles#CreateEventStream``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.create_event_stream_response.CreateEventStreamResponse = capo_customer_profiles.types.create_event_stream_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{EventStreamName}", quote(str(input_["event_stream_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.create_event_stream_request.serialize_json(input_)
    ).encode()
...
async def async_create_event_stream(
