"""Generated from Smithy shape ``com.amazonaws.customerprofiles#DeleteIntegration``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.delete_integration_response.DeleteIntegrationResponse = capo_customer_profiles.types.delete_integration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{DomainName}/integrations/delete"
    url = url.replace("{DomainName}", quote(str(input_["domain_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.delete_integration_request.serialize_json(input_)
    ).encode()
