"""Generated from Smithy shape ``com.amazonaws.customerprofiles#UpdateProfile``."""
...
def handle_response(
    ...
    out: capo_customer_profiles.types.update_profile_response.UpdateProfileResponse = (
        capo_customer_profiles.types.update_profile_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{DomainName}/profiles"
    url = url.replace("{DomainName}", quote(str(input_["domain_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_customer_profiles.types.update_profile_request.serialize_json(input_)
    ).encode()
