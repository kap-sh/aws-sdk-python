"""Generated from Smithy shape ``com.amazonaws.customerprofiles#ListAccountIntegrations``."""
...
def handle_response(
    ...
    out: aws_sdk_customer_profiles.types.list_account_integrations_response.ListAccountIntegrationsResponse = aws_sdk_customer_profiles.types.list_account_integrations_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_customer_profiles.types.list_account_integrations_request.ListAccountIntegrationsRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/integrations"
    params: dict[str, str] = {}
    if "next_token" in input:
        params["next-token"] = str(input["next_token"])
    if "max_results" in input:
        params["max-results"] = str(input["max_results"])
    if "include_hidden" in input:
        params["include-hidden"] = str(input["include_hidden"])
...
async def async_list_account_integrations(
