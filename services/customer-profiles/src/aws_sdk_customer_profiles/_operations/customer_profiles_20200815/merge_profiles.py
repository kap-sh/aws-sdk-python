"""Generated from Smithy shape ``com.amazonaws.customerprofiles#MergeProfiles``."""
...
def handle_response(
    ...
    out: aws_sdk_customer_profiles.types.merge_profiles_response.MergeProfilesResponse = aws_sdk_customer_profiles.types.merge_profiles_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_customer_profiles.types.merge_profiles_request.MergeProfilesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/domains/{DomainName}/profiles/objects/merge"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_customer_profiles.types.merge_profiles_request.serialize_json(input_)
    ).encode()
...
async def async_merge_profiles(
