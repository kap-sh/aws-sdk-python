"""Generated from Smithy shape ``com.amazonaws.customerprofiles#GetRecommender``."""
...
def handle_response(
    ...
    out: aws_sdk_customer_profiles.types.get_recommender_response.GetRecommenderResponse = aws_sdk_customer_profiles.types.get_recommender_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_customer_profiles.types.get_recommender_request.GetRecommenderRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{RecommenderName}", quote(str(input_["recommender_name"]), safe="")
    )
    params: dict[str, str] = {}
    if "training_metrics_count" in input_:
        params["training-metrics-count"] = str(input_["training_metrics_count"])
...
async def async_get_recommender(
