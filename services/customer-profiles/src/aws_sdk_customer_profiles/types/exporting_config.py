...
if TYPE_CHECKING:
    import aws_sdk_customer_profiles.types.s3_exporting_config


class ExportingConfig(TypedDict):
    s3_exporting: NotRequired[
        "aws_sdk_customer_profiles.types.s3_exporting_config.S3ExportingConfig"
    ]
...
def serialize_json(value: ExportingConfig) -> dict:
    ...
    if "s3_exporting" in value:
        ...
        out["S3Exporting"] = (
            aws_sdk_customer_profiles.types.s3_exporting_config.serialize_json(
                value["s3_exporting"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ExportingConfig:
    ...
    if "S3Exporting" in data:
        ...
        out["s3_exporting"] = (
            aws_sdk_customer_profiles.types.s3_exporting_config.deserialize_json(
                data["S3Exporting"]
            )
        )
    return out
