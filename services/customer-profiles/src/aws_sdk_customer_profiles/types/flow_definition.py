...
if TYPE_CHECKING:
    import aws_sdk_customer_profiles.types.flow_description
    import aws_sdk_customer_profiles.types.flow_name
    import aws_sdk_customer_profiles.types.kms_arn
    import aws_sdk_customer_profiles.types.source_flow_config
    import aws_sdk_customer_profiles.types.tasks
    import aws_sdk_customer_profiles.types.trigger_config


class FlowDefinition(TypedDict):
    description: NotRequired[
        "aws_sdk_customer_profiles.types.flow_description.FlowDescription"
    ]
...
def serialize_json(value: FlowDefinition) -> dict:
    ...
    out["SourceFlowConfig"] = (
        aws_sdk_customer_profiles.types.source_flow_config.serialize_json(
            value["source_flow_config"]
        )
    )
    import aws_sdk_customer_profiles.types.tasks

    out["Tasks"] = aws_sdk_customer_profiles.types.tasks.serialize_json(value["tasks"])
    import aws_sdk_customer_profiles.types.trigger_config

    out["TriggerConfig"] = (
        aws_sdk_customer_profiles.types.trigger_config.serialize_json(
...
def deserialize_json(data: dict) -> FlowDefinition:
    ...
    if "SourceFlowConfig" in data:
        ...
        out["source_flow_config"] = (
            aws_sdk_customer_profiles.types.source_flow_config.deserialize_json(
    ...
    if "Tasks" in data:
        ...
        out["tasks"] = aws_sdk_customer_profiles.types.tasks.deserialize_json(
            data["Tasks"]
        )
    else:
    ...
    if "TriggerConfig" in data:
        ...
        out["trigger_config"] = (
            aws_sdk_customer_profiles.types.trigger_config.deserialize_json(
                data["TriggerConfig"]
            )
        )
    else:
