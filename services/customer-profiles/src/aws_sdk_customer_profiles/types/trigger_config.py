...
if TYPE_CHECKING:
    import aws_sdk_customer_profiles.types.trigger_properties
    import aws_sdk_customer_profiles.types.trigger_type


class TriggerConfig(TypedDict, closed=True):
    trigger_type: "aws_sdk_customer_profiles.types.trigger_type.TriggerType"
    """<p>Specifies the type of flow trigger. It can be OnDemand, Scheduled, or Event.</p>"""
    trigger_properties: NotRequired[
        "aws_sdk_customer_profiles.types.trigger_properties.TriggerProperties"
    ]
...
def serialize_json(value: TriggerConfig) -> dict:
    ...
    out["TriggerType"] = aws_sdk_customer_profiles.types.trigger_type.serialize_json(
        value["trigger_type"]
    )
    if "trigger_properties" in value:
        ...
        out["TriggerProperties"] = (
            aws_sdk_customer_profiles.types.trigger_properties.serialize_json(
...
def deserialize_json(data: dict) -> TriggerConfig:
    ...
    if "TriggerType" in data:
        ...
        out["trigger_type"] = (
            aws_sdk_customer_profiles.types.trigger_type.deserialize_json(
                data["TriggerType"]
            )
        )
    else:
    ...
    if "TriggerProperties" in data:
        ...
        out["trigger_properties"] = (
            aws_sdk_customer_profiles.types.trigger_properties.deserialize_json(
