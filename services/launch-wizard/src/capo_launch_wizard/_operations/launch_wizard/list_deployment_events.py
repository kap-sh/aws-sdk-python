"""Generated from Smithy shape ``com.amazonaws.launchwizard#ListDeploymentEvents``."""
...
def handle_response(
    ...
    out: capo_launch_wizard.types.list_deployment_events_output.ListDeploymentEventsOutput = capo_launch_wizard.types.list_deployment_events_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listDeploymentEvents"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_launch_wizard.types.list_deployment_events_input.serialize_json(input_)
    ).encode()
...
async def async_list_deployment_events(
