"""Generated from Smithy shape ``com.amazonaws.launchwizard#ListDeployments``."""
...
def handle_response(
    ...
    out: capo_launch_wizard.types.list_deployments_output.ListDeploymentsOutput = (
        capo_launch_wizard.types.list_deployments_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listDeployments"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_launch_wizard.types.list_deployments_input.serialize_json(input_)
    ).encode()
