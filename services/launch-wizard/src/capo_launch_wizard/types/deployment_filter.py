...
class DeploymentFilter(TypedDict, closed=True):
    name: NotRequired[
        "capo_launch_wizard.types.deployment_filter_key.DeploymentFilterKey"
    ]
    """<p>The name of the filter. Filter names are case-sensitive.</p>"""
    values: NotRequired[
        "capo_launch_wizard.types.deployment_filter_values.DeploymentFilterValues"
    ]
...
def serialize_json(value: DeploymentFilter) -> dict:
    ...
    if "name" in value:
        ...
        out["name"] = capo_launch_wizard.types.deployment_filter_key.serialize_json(
            value["name"]
        )
    if "values" in value:
        import capo_launch_wizard.types.deployment_filter_values
        ...
        out["values"] = (
            capo_launch_wizard.types.deployment_filter_values.serialize_json(
                value["values"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> DeploymentFilter:
    ...
    if "values" in data:
        ...
        out["values"] = (
            capo_launch_wizard.types.deployment_filter_values.deserialize_json(
                data["values"]
            )
        )
    return out
