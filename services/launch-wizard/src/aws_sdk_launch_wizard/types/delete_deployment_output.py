...
class DeleteDeploymentOutput(TypedDict):
    status: NotRequired["aws_sdk_launch_wizard.types.deployment_status.DeploymentStatus"]
...
def serialize_json(value: DeleteDeploymentOutput) -> dict:
    ...
    if "status" in value:
        import aws_sdk_launch_wizard.types.deployment_status
        out["status"] = aws_sdk_launch_wizard.types.deployment_status.serialize_json(value["status"])
...
def deserialize_json(data: dict) -> DeleteDeploymentOutput:
    ...
    if "status" in data:
        import aws_sdk_launch_wizard.types.deployment_status
        out["status"] = aws_sdk_launch_wizard.types.deployment_status.deserialize_json(data["status"])
