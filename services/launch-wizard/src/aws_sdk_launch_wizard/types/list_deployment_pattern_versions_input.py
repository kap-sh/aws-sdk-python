...
class ListDeploymentPatternVersionsInput(TypedDict, closed=True):
    ...
    filters: NotRequired["aws_sdk_launch_wizard.types.filter_list.FilterList"]
...
def serialize_json(value: ListDeploymentPatternVersionsInput) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = aws_sdk_launch_wizard.types.filter_list.serialize_json(
            value["filters"]
        )
    return out
...
def deserialize_json(data: dict) -> ListDeploymentPatternVersionsInput:
    ...
    if "filters" in data:
        ...
        out["filters"] = aws_sdk_launch_wizard.types.filter_list.deserialize_json(
            data["filters"]
        )
    return out
