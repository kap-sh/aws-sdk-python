...
AutoshiftSummaries: TypeAlias = list[
    "aws_sdk_arc_zonal_shift.types.autoshift_summary.AutoshiftSummary"
]
...
def serialize_json(value: AutoshiftSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_arc_zonal_shift.types.autoshift_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> AutoshiftSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_arc_zonal_shift.types.autoshift_summary.deserialize_json(item)
        )
    return out
