...
from capo_arc_zonal_shift._auth._identity import (
    Credentials,
    Identity,
)
from capo_arc_zonal_shift._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-arc-zonal-shift[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-arc-zonal-shift[sso]"
        )
