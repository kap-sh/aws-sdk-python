"""Generated from Smithy shape ``com.amazonaws.rekognition#DisassociatedFacesList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_rekognition.types.disassociated_face

DisassociatedFacesList: TypeAlias = list[
    "aws_sdk_rekognition.types.disassociated_face.DisassociatedFace"
]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DisassociatedFacesList) -> list:
    import aws_sdk_rekognition.types.disassociated_face

    out: list = []
    for item in value:
        out.append(
            aws_sdk_rekognition.types.disassociated_face.serialize_aws_json_1_1(item)
        )
    return out


def deserialize_aws_json_1_1(data: list) -> DisassociatedFacesList:
    import aws_sdk_rekognition.types.disassociated_face

    out: DisassociatedFacesList = []
    for item in data:
        out.append(
            aws_sdk_rekognition.types.disassociated_face.deserialize_aws_json_1_1(item)
        )
    return out
