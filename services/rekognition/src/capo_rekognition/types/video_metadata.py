...
class VideoMetadata(TypedDict, closed=True):
    codec: NotRequired["capo_rekognition.types.string.String"]
    ...
    format: NotRequired["capo_rekognition.types.string.String"]
    """<p>Format of the analyzed video. Possible values are MP4, MOV and AVI. </p>"""
    frame_rate: NotRequired["capo_rekognition.types.float.Float"]
    """<p>Number of frames per second in the video.</p>"""
    frame_height: NotRequired["capo_rekognition.types.u_long.ULong"]
    """<p>Vertical pixel dimension of the video.</p>"""
    frame_width: NotRequired["capo_rekognition.types.u_long.ULong"]
    """<p>Horizontal pixel dimension of the video.</p>"""
    color_range: NotRequired["capo_rekognition.types.video_color_range.VideoColorRange"]
...
def serialize_aws_json_1_1(value: VideoMetadata) -> dict:
    ...
    if "color_range" in value:
        ...
        out["ColorRange"] = (
            capo_rekognition.types.video_color_range.serialize_aws_json_1_1(
                value["color_range"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> VideoMetadata:
    ...
    if "ColorRange" in data:
        ...
        out["color_range"] = (
            capo_rekognition.types.video_color_range.deserialize_aws_json_1_1(
                data["ColorRange"]
            )
        )
    return out
