"""Generated from Smithy shape ``com.amazonaws.dynamodb#BackupSummaries``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_dynamodb.types.backup_summary

BackupSummaries: TypeAlias = list["aws_sdk_dynamodb.types.backup_summary.BackupSummary"]


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: BackupSummaries) -> list:
    import aws_sdk_dynamodb.types.backup_summary

    out: list = []
    for item in value:
        out.append(aws_sdk_dynamodb.types.backup_summary.serialize_aws_json_1_0(item))
    return out


def deserialize_aws_json_1_0(data: list) -> BackupSummaries:
    import aws_sdk_dynamodb.types.backup_summary

    out: BackupSummaries = []
    for item in data:
        out.append(aws_sdk_dynamodb.types.backup_summary.deserialize_aws_json_1_0(item))
    return out
