...
class TableAlreadyExistsException_(TypedDict, closed=True):
    message: NotRequired["capo_dynamodb.types.error_message.ErrorMessage"]


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: TableAlreadyExistsException_) -> dict:
    out: dict = {}
    if "message" in value:
...
def deserialize_aws_json_1_0(data: dict) -> TableAlreadyExistsException_:
    out: TableAlreadyExistsException_ = {}  # type: ignore[typeddict-item]
...
class TableAlreadyExistsException(ServiceError):
    ...
    def __init__(self, data: TableAlreadyExistsException_, message: str | None = None):
        super().__init__(
            "client",
