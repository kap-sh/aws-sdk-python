...
if TYPE_CHECKING:
    ...
    import capo_dynamodb.types.backup_summaries


class ListBackupsOutput(TypedDict, closed=True):
    backup_summaries: NotRequired[
        "capo_dynamodb.types.backup_summaries.BackupSummaries"
    ]
    """<p>List of <code>BackupSummary</code> objects.</p>"""
    last_evaluated_backup_arn: NotRequired["capo_dynamodb.types.backup_arn.BackupArn"]
...
def serialize_aws_json_1_0(value: ListBackupsOutput) -> dict:
    ...
    if "backup_summaries" in value:
        ...
        out["BackupSummaries"] = (
            capo_dynamodb.types.backup_summaries.serialize_aws_json_1_0(
...
def deserialize_aws_json_1_0(data: dict) -> ListBackupsOutput:
    ...
    if data.get("BackupSummaries") is not None:
        import capo_dynamodb.types.backup_summaries

        out["backup_summaries"] = (
            capo_dynamodb.types.backup_summaries.deserialize_aws_json_1_0(
                data["BackupSummaries"]
            )
        )
    if data.get("LastEvaluatedBackupArn") is not None:
