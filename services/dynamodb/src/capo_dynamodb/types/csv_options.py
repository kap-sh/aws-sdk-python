...
if TYPE_CHECKING:
    import capo_dynamodb.types.csv_delimiter
    import capo_dynamodb.types.csv_header_list


class CsvOptions(TypedDict, closed=True):
    delimiter: NotRequired["capo_dynamodb.types.csv_delimiter.CsvDelimiter"]
    """<p> The delimiter used for separating items in the CSV file being imported. </p>"""
    header_list: NotRequired["capo_dynamodb.types.csv_header_list.CsvHeaderList"]
...
def serialize_aws_json_1_0(value: CsvOptions) -> dict:
    ...
    if "header_list" in value:
        ...
        out["HeaderList"] = capo_dynamodb.types.csv_header_list.serialize_aws_json_1_0(
            value["header_list"]
        )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> CsvOptions:
    ...
    if "HeaderList" in data:
        ...
        out["header_list"] = (
            capo_dynamodb.types.csv_header_list.deserialize_aws_json_1_0(
                data["HeaderList"]
            )
        )
    return out
