"""Generated from Smithy shape ``com.amazonaws.dynamodb#CreateBackup``."""
...
import capo_dynamodb.errors.continuous_backups_unavailable_exception
import capo_dynamodb.errors.internal_server_error
import capo_dynamodb.errors.invalid_endpoint_exception
import capo_dynamodb.errors.limit_exceeded_exception
import capo_dynamodb.errors.table_in_use_exception
import capo_dynamodb.errors.table_not_found_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "BackupInUseException":
            raise capo_dynamodb.errors.backup_in_use_exception.BackupInUseException.from_aws_json_1_0(
                data, message
            )
        case "ContinuousBackupsUnavailableException":
            raise capo_dynamodb.errors.continuous_backups_unavailable_exception.ContinuousBackupsUnavailableException.from_aws_json_1_0(
                data, message
            )
        ...
        case "TableInUseException":
            raise capo_dynamodb.errors.table_in_use_exception.TableInUseException.from_aws_json_1_0(
                data, message
            )
        case "TableNotFoundException":
            raise capo_dynamodb.errors.table_not_found_exception.TableNotFoundException.from_aws_json_1_0(
                data, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_dynamodb.types.create_backup_output.CreateBackupOutput = (
        capo_dynamodb.types.create_backup_output.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_dynamodb._auth._sigv4.build_sigv4_auth_scheme(
                "dynamodb", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_dynamodb.types.create_backup_input.serialize_aws_json_1_0(input_),
        allow_nan=False,
    ).encode()
...
def create_backup(
    ...
    capo_dynamodb.types.create_backup_output.CreateBackupOutput, zapros.Response
]:
