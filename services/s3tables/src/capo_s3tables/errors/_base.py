...
class S3TablesError(Exception):
    """Base class for all S3Tables service errors."""


class ServiceError(S3TablesError):
...
class SerializationError(S3TablesError):
    """Raised when a value cannot be serialized."""


class DeserializationError(S3TablesError):
...
class WaiterFailedError(S3TablesError):
...
class WaiterTimeoutError(S3TablesError):
