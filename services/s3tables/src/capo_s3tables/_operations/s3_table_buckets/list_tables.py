"""Generated from Smithy shape ``com.amazonaws.s3tables#ListTables``."""
...
def handle_response(
    ...
    out: capo_s3tables.types.list_tables_response.ListTablesResponse = (
        capo_s3tables.types.list_tables_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/tables/{tableBucketARN}"
    url = url.replace(
    ...
    if "max_tables" in input_:
        params["maxTables"] = str(input_["max_tables"])
...
def list_tables(
    ...
    capo_s3tables.types.list_tables_response.ListTablesResponse, zapros.Response
]:
