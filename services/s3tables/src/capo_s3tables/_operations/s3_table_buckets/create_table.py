"""Generated from Smithy shape ``com.amazonaws.s3tables#CreateTable``."""
...
def handle_response(
    ...
    out: capo_s3tables.types.create_table_response.CreateTableResponse = (
        capo_s3tables.types.create_table_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/tables/{tableBucketARN}/{namespace}"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_s3tables.types.create_table_request.serialize_json(input_)
    ).encode()
...
def create_table(
    ...
    capo_s3tables.types.create_table_response.CreateTableResponse, zapros.Response
]:
