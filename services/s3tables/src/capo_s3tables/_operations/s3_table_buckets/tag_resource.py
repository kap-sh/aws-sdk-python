"""Generated from Smithy shape ``com.amazonaws.s3tables#TagResource``."""
...
def handle_response(
    ...
    out: capo_s3tables.types.tag_resource_response.TagResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/tag/{resourceArn}"
    url = url.replace("{resourceArn}", quote(str(input_["resource_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_s3tables.types.tag_resource_request.serialize_json(input_)
    ).encode()
...
def tag_resource(
    ...
    capo_s3tables.types.tag_resource_response.TagResourceResponse, zapros.Response
]:
