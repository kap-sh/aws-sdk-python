"""Generated from Smithy shape ``com.amazonaws.s3tables#RenameTable``."""
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/tables/{tableBucketARN}/{namespace}/{name}/rename"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_s3tables.types.rename_table_request.serialize_json(input_)
    ).encode()
