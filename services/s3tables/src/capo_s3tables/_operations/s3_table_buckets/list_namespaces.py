"""Generated from Smithy shape ``com.amazonaws.s3tables#ListNamespaces``."""
...
import capo_s3tables.errors.conflict_exception
...
from capo_s3tables._services._pipeline import AsyncOperationOptions, OperationOptions
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_s3tables.errors.conflict_exception.ConflictException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_s3tables.types.list_namespaces_response.ListNamespacesResponse = (
        capo_s3tables.types.list_namespaces_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_s3tables._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/namespaces/{tableBucketARN}"
    url = url.replace(
        "{tableBucketARN}", quote(str(input_["table_bucket_arn"]), safe="")
    )
    params: dict[str, str] = {}
    if "prefix" in input_:
        params["prefix"] = str(input_["prefix"])
    if "continuation_token" in input_:
        params["continuationToken"] = str(input_["continuation_token"])
    if "max_namespaces" in input_:
        params["maxNamespaces"] = str(input_["max_namespaces"])
...
def list_namespaces(
    ...
    capo_s3tables.types.list_namespaces_response.ListNamespacesResponse, zapros.Response
]:
