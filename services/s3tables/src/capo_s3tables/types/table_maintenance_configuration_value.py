...
class TableMaintenanceConfigurationValue(TypedDict, closed=True):
    ...
    settings: NotRequired[
        "capo_s3tables.types.table_maintenance_settings.TableMaintenanceSettings"
    ]
...
def serialize_json(value: TableMaintenanceConfigurationValue) -> dict:
    ...
    if "settings" in value:
        ...
        out["settings"] = capo_s3tables.types.table_maintenance_settings.serialize_json(
            value["settings"]
        )
    return out
...
def deserialize_json(data: dict) -> TableMaintenanceConfigurationValue:
    ...
    if "settings" in data:
        ...
        out["settings"] = (
            capo_s3tables.types.table_maintenance_settings.deserialize_json(
                data["settings"]
            )
        )
    return out
