"""Generated from Smithy shape ``com.amazonaws.s3tables#GetTableBucketReplication``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            import aws_sdk_s3tables.errors.access_denied_exception

            raise aws_sdk_s3tables.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_s3tables.types.get_table_bucket_replication_response.GetTableBucketReplicationResponse = aws_sdk_s3tables.types.get_table_bucket_replication_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_s3tables.types.get_table_bucket_replication_request.GetTableBucketReplicationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/table-bucket-replication"
    params: dict[str, str] = {}
    if "table_bucket_arn" in input_:
        params["tableBucketARN"] = str(input_["table_bucket_arn"])
...
async def async_get_table_bucket_replication(
