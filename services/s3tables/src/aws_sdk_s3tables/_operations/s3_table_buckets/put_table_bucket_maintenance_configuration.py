"""Generated from Smithy shape ``com.amazonaws.s3tables#PutTableBucketMaintenanceConfiguration``."""
...
def build_request(
    ...
    input_: aws_sdk_s3tables.types.put_table_bucket_maintenance_configuration_request.PutTableBucketMaintenanceConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/buckets/{tableBucketARN}/maintenance/{type}"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_s3tables.types.put_table_bucket_maintenance_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_table_bucket_maintenance_configuration(
