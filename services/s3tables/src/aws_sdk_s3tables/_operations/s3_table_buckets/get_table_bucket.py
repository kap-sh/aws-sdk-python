"""Generated from Smithy shape ``com.amazonaws.s3tables#GetTableBucket``."""
...
def handle_response(
    ...
    out: aws_sdk_s3tables.types.get_table_bucket_response.GetTableBucketResponse = (
        aws_sdk_s3tables.types.get_table_bucket_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_s3tables.types.get_table_bucket_request.GetTableBucketRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/buckets/{tableBucketARN}"
    url = url.replace(
...
async def async_get_table_bucket(
