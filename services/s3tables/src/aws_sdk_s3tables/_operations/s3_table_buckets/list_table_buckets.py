"""Generated from Smithy shape ``com.amazonaws.s3tables#ListTableBuckets``."""
...
def handle_response(
    ...
    out: aws_sdk_s3tables.types.list_table_buckets_response.ListTableBucketsResponse = (
        aws_sdk_s3tables.types.list_table_buckets_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_s3tables.types.list_table_buckets_request.ListTableBucketsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/buckets"
    params: dict[str, str] = {}
    if "prefix" in input_:
    ...
    if "max_buckets" in input_:
        params["maxBuckets"] = str(input_["max_buckets"])
    if "type" in input_:
...
async def async_list_table_buckets(
