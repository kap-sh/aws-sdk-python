...
if TYPE_CHECKING:
    ...
    import aws_sdk_s3tables.types.table_replication_rules


class TableReplicationConfiguration(TypedDict):
    ...
    rules: "aws_sdk_s3tables.types.table_replication_rules.TableReplicationRules"
...
def serialize_json(value: TableReplicationConfiguration) -> dict:
    ...
    out["rules"] = aws_sdk_s3tables.types.table_replication_rules.serialize_json(
        value["rules"]
    )
    return out
...
def deserialize_json(data: dict) -> TableReplicationConfiguration:
    ...
    if "rules" in data:
        ...
        out["rules"] = aws_sdk_s3tables.types.table_replication_rules.deserialize_json(
            data["rules"]
        )
    else:
