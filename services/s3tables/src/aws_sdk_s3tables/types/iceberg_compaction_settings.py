...
if TYPE_CHECKING:
    import aws_sdk_s3tables.types.iceberg_compaction_strategy
    import aws_sdk_s3tables.types.positive_integer


class IcebergCompactionSettings(TypedDict):
    ...
    strategy: NotRequired[
        "aws_sdk_s3tables.types.iceberg_compaction_strategy.IcebergCompactionStrategy"
    ]
...
def serialize_json(value: IcebergCompactionSettings) -> dict:
    ...
    if "strategy" in value:
        ...
        out["strategy"] = (
            aws_sdk_s3tables.types.iceberg_compaction_strategy.serialize_json(
                value["strategy"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> IcebergCompactionSettings:
    ...
    if "strategy" in data:
        ...
        out["strategy"] = (
            aws_sdk_s3tables.types.iceberg_compaction_strategy.deserialize_json(
                data["strategy"]
            )
        )
    return out
