...
class GetTableRecordExpirationJobStatusResponse(TypedDict, closed=True):
    ...
    metrics: NotRequired[
        "aws_sdk_s3tables.types.table_record_expiration_job_metrics.TableRecordExpirationJobMetrics"
    ]
...
def serialize_json(value: GetTableRecordExpirationJobStatusResponse) -> dict:
    ...
    out["status"] = (
        aws_sdk_s3tables.types.table_record_expiration_job_status.serialize_json(
            value["status"]
        )
    )
    ...
    if "metrics" in value:
        ...
        out["metrics"] = (
            aws_sdk_s3tables.types.table_record_expiration_job_metrics.serialize_json(
                value["metrics"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GetTableRecordExpirationJobStatusResponse:
    ...
    if "status" in data:
        ...
        out["status"] = (
            aws_sdk_s3tables.types.table_record_expiration_job_status.deserialize_json(
                data["status"]
            )
        )
    else:
    ...
    if "metrics" in data:
        ...
        out["metrics"] = (
            aws_sdk_s3tables.types.table_record_expiration_job_metrics.deserialize_json(
                data["metrics"]
            )
        )
    return out
