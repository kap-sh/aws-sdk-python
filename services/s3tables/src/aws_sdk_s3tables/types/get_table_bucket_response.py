"""Generated from Smithy shape ``com.amazonaws.s3tables#GetTableBucketResponse``."""
...
class GetTableBucketResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetTableBucketResponse) -> dict:
    out: dict = {}
    out["arn"] = value["arn"]
    out["name"] = value["name"]
...
def deserialize_json(data: dict) -> GetTableBucketResponse:
    out: GetTableBucketResponse = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("GetTableBucketResponse.arn required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("GetTableBucketResponse.name required")
    if "ownerAccountId" in data:
        out["owner_account_id"] = data["ownerAccountId"]
    else:
        raise DeserializationError("GetTableBucketResponse.owner_account_id required")
    if "createdAt" in data:
        ...
        raise DeserializationError("GetTableBucketResponse.created_at required")
