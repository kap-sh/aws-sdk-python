...
if TYPE_CHECKING:
    import aws_sdk_emr.types.auto_scaling_policy_state_change_reason_code
    import aws_sdk_emr.types.string


class AutoScalingPolicyStateChangeReason(TypedDict):
    code: NotRequired[
        "aws_sdk_emr.types.auto_scaling_policy_state_change_reason_code.AutoScalingPolicyStateChangeReasonCode"
    ]
...
def serialize_aws_json_1_1(value: AutoScalingPolicyStateChangeReason) -> dict:
    ...
    if "code" in value:
        ...
        out["Code"] = (
            aws_sdk_emr.types.auto_scaling_policy_state_change_reason_code.serialize_aws_json_1_1(
                value["code"]
            )
        )
    if "message" in value:
...
def deserialize_aws_json_1_1(data: dict) -> AutoScalingPolicyStateChangeReason:
    ...
    if "Code" in data:
        ...
        out["code"] = (
            aws_sdk_emr.types.auto_scaling_policy_state_change_reason_code.deserialize_aws_json_1_1(
                data["Code"]
            )
        )
    if "Message" in data:
