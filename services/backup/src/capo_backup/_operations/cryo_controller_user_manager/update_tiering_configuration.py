"""Generated from Smithy shape ``com.amazonaws.backup#UpdateTieringConfiguration``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AlreadyExistsException":
            raise capo_backup.errors.already_exists_exception.AlreadyExistsException.from_json(
                data
            )
        ...
        case "LimitExceededException":
            raise capo_backup.errors.limit_exceeded_exception.LimitExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_backup.types.update_tiering_configuration_output.UpdateTieringConfigurationOutput = capo_backup.types.update_tiering_configuration_output.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/tiering-configurations/{TieringConfigurationName}"
    )
    url = url.replace(
        "{TieringConfigurationName}",
        quote(str(input_["tiering_configuration_name"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.update_tiering_configuration_input.serialize_json(input_)
    ).encode()
...
async def async_update_tiering_configuration(
