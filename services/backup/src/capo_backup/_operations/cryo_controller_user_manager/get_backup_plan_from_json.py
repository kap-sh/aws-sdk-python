"""Generated from Smithy shape ``com.amazonaws.backup#GetBackupPlanFromJSON``."""
...
def handle_response(
    ...
    out: capo_backup.types.get_backup_plan_from_json_output.GetBackupPlanFromJSONOutput = capo_backup.types.get_backup_plan_from_json_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/backup/template/json/toPlan"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.get_backup_plan_from_json_input.serialize_json(input_)
    ).encode()
...
async def async_get_backup_plan_from_json(
