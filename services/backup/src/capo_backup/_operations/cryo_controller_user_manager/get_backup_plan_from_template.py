"""Generated from Smithy shape ``com.amazonaws.backup#GetBackupPlanFromTemplate``."""
...
def handle_response(
    ...
    out: capo_backup.types.get_backup_plan_from_template_output.GetBackupPlanFromTemplateOutput = capo_backup.types.get_backup_plan_from_template_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{BackupPlanTemplateId}", quote(str(input_["backup_plan_template_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_backup_plan_from_template(
