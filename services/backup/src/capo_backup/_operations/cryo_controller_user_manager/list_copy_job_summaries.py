"""Generated from Smithy shape ``com.amazonaws.backup#ListCopyJobSummaries``."""
...
import capo_backup.types.copy_job_summary_list
...
def handle_response(
    ...
    out: capo_backup.types.list_copy_job_summaries_output.ListCopyJobSummariesOutput = (
        capo_backup.types.list_copy_job_summaries_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/audit/copy-job-summaries"
    params: dict[str, str] = {}
    ...
    if "message_category" in input_:
        params["MessageCategory"] = str(input_["message_category"])
...
async def async_list_copy_job_summaries(
