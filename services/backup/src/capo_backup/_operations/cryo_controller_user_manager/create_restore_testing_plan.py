"""Generated from Smithy shape ``com.amazonaws.backup#CreateRestoreTestingPlan``."""
...
def handle_response(
    ...
    out: capo_backup.types.create_restore_testing_plan_output.CreateRestoreTestingPlanOutput = capo_backup.types.create_restore_testing_plan_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/restore-testing/plans"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.create_restore_testing_plan_input.serialize_json(input_)
    ).encode()
...
async def async_create_restore_testing_plan(
