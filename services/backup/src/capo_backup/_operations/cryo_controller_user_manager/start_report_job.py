"""Generated from Smithy shape ``com.amazonaws.backup#StartReportJob``."""
...
def handle_response(
    ...
    out: capo_backup.types.start_report_job_output.StartReportJobOutput = (
        capo_backup.types.start_report_job_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/audit/report-jobs/{ReportPlanName}"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_backup.types.start_report_job_input.serialize_json(input_)
    ).encode()
...
def start_report_job(
    ...
    capo_backup.types.start_report_job_output.StartReportJobOutput, zapros.Response
]:
...
async def async_start_report_job(
