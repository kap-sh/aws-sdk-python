"""Generated from Smithy shape ``com.amazonaws.backup#CreateBackupSelection``."""
...
def handle_response(
    ...
    out: capo_backup.types.create_backup_selection_output.CreateBackupSelectionOutput = capo_backup.types.create_backup_selection_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/backup/plans/{BackupPlanId}/selections"
    url = url.replace("{BackupPlanId}", quote(str(input_["backup_plan_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.create_backup_selection_input.serialize_json(input_)
    ).encode()
...
async def async_create_backup_selection(
