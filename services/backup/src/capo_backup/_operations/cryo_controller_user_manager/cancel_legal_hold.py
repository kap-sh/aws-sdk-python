"""Generated from Smithy shape ``com.amazonaws.backup#CancelLegalHold``."""
...
import capo_backup.errors.invalid_resource_state_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidResourceStateException":
            raise capo_backup.errors.invalid_resource_state_exception.InvalidResourceStateException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_backup.types.cancel_legal_hold_output.CancelLegalHoldOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/legal-holds/{LegalHoldId}"
    url = url.replace("{LegalHoldId}", quote(str(input_["legal_hold_id"]), safe=""))
    params: dict[str, str] = {}
    if "cancel_description" in input_:
        params["cancelDescription"] = str(input_["cancel_description"])
    if "retain_record_in_days" in input_:
        params["retainRecordInDays"] = str(input_["retain_record_in_days"])
...
def cancel_legal_hold(
    ...
    capo_backup.types.cancel_legal_hold_output.CancelLegalHoldOutput, zapros.Response
]:
...
async def async_cancel_legal_hold(
