"""Generated from Smithy shape ``com.amazonaws.backup#PutRestoreValidationResult``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/restore-jobs/{RestoreJobId}/validations"
    url = url.replace("{RestoreJobId}", quote(str(input_["restore_job_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_backup.types.put_restore_validation_result_input.serialize_json(input_)
    ).encode()
...
async def async_put_restore_validation_result(
