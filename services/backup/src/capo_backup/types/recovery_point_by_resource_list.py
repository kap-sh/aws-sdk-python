...
RecoveryPointByResourceList: TypeAlias = list[
    "capo_backup.types.recovery_point_by_resource.RecoveryPointByResource"
]
...
def serialize_json(value: RecoveryPointByResourceList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_backup.types.recovery_point_by_resource.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> RecoveryPointByResourceList:
    ...
    for item in data:
        out.append(capo_backup.types.recovery_point_by_resource.deserialize_json(item))
    return out
