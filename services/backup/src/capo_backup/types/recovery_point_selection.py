...
class RecoveryPointSelection(TypedDict, closed=True):
    vault_names: NotRequired["capo_backup.types.vault_names.VaultNames"]
    ...
    resource_identifiers: NotRequired[
        "capo_backup.types.resource_identifiers.ResourceIdentifiers"
    ]
    """<p>These are the resources included in the resource selection (including type of resources and vaults).</p>"""
    date_range: NotRequired["capo_backup.types.date_range.DateRange"]
...
def serialize_json(value: RecoveryPointSelection) -> dict:
    ...
    if "vault_names" in value:
        ...
        out["VaultNames"] = capo_backup.types.vault_names.serialize_json(
            value["vault_names"]
        )
    if "resource_identifiers" in value:
        ...
        out["ResourceIdentifiers"] = (
            capo_backup.types.resource_identifiers.serialize_json(
    ...
    if "date_range" in value:
        ...
        out["DateRange"] = capo_backup.types.date_range.serialize_json(
            value["date_range"]
        )
    return out
...
def deserialize_json(data: dict) -> RecoveryPointSelection:
    ...
    if "ResourceIdentifiers" in data:
        ...
        out["resource_identifiers"] = (
            capo_backup.types.resource_identifiers.deserialize_json(
