...
if TYPE_CHECKING:
    import capo_backup.types.condition_key
    import capo_backup.types.condition_value


class ConditionParameter(TypedDict, closed=True):
    condition_key: NotRequired["capo_backup.types.condition_key.ConditionKey"]
    """<p>The key in a key-value pair. For example, in the tag <code>Department: Accounting</code>, <code>Department</code> is the key.</p>"""
    condition_value: NotRequired["capo_backup.types.condition_value.ConditionValue"]
