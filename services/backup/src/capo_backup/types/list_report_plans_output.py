...
class ListReportPlansOutput(TypedDict, closed=True):
    report_plans: NotRequired["capo_backup.types.report_plan_list.ReportPlanList"]
...
def serialize_json(value: ListReportPlansOutput) -> dict:
    ...
    if "report_plans" in value:
        ...
        out["ReportPlans"] = capo_backup.types.report_plan_list.serialize_json(
            value["report_plans"]
        )
...
def deserialize_json(data: dict) -> ListReportPlansOutput:
    ...
    if "ReportPlans" in data:
        ...
        out["report_plans"] = capo_backup.types.report_plan_list.deserialize_json(
            data["ReportPlans"]
        )
