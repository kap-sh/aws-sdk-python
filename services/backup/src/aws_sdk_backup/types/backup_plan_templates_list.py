...
BackupPlanTemplatesList: TypeAlias = list[
    "aws_sdk_backup.types.backup_plan_templates_list_member.BackupPlanTemplatesListMember"
]
...
def serialize_json(value: BackupPlanTemplatesList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_backup.types.backup_plan_templates_list_member.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BackupPlanTemplatesList:
    ...
    for item in data:
        out.append(
            aws_sdk_backup.types.backup_plan_templates_list_member.deserialize_json(
                item
            )
        )
    return out
