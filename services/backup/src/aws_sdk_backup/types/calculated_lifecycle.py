...
def serialize_json(value: CalculatedLifecycle) -> dict:
    ...
    if "move_to_cold_storage_at" in value:
        ...
        out["MoveToColdStorageAt"] = aws_sdk_backup.types.timestamp.serialize_json(
            value["move_to_cold_storage_at"]
        )
    if "delete_at" in value:
        import aws_sdk_backup.types.timestamp

        out["DeleteAt"] = aws_sdk_backup.types.timestamp.serialize_json(
            value["delete_at"]
        )
    return out
...
def deserialize_json(data: dict) -> CalculatedLifecycle:
    ...
    if "MoveToColdStorageAt" in data:
        ...
        out["move_to_cold_storage_at"] = (
            aws_sdk_backup.types.timestamp.deserialize_json(data["MoveToColdStorageAt"])
        )
    if "DeleteAt" in data:
        import aws_sdk_backup.types.timestamp

        out["delete_at"] = aws_sdk_backup.types.timestamp.deserialize_json(
            data["DeleteAt"]
        )
    return out
