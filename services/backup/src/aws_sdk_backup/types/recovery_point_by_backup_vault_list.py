...
RecoveryPointByBackupVaultList: TypeAlias = list[
    "aws_sdk_backup.types.recovery_point_by_backup_vault.RecoveryPointByBackupVault"
]
...
def serialize_json(value: RecoveryPointByBackupVaultList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_backup.types.recovery_point_by_backup_vault.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> RecoveryPointByBackupVaultList:
    ...
    for item in data:
        out.append(
            aws_sdk_backup.types.recovery_point_by_backup_vault.deserialize_json(item)
        )
    return out
