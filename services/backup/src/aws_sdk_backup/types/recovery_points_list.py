...
RecoveryPointsList: TypeAlias = list[
    "aws_sdk_backup.types.recovery_point_member.RecoveryPointMember"
]
...
def serialize_json(value: RecoveryPointsList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_backup.types.recovery_point_member.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> RecoveryPointsList:
    ...
    for item in data:
        out.append(aws_sdk_backup.types.recovery_point_member.deserialize_json(item))
    return out
