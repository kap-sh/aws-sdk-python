...
class ControlScope(TypedDict):
    compliance_resource_ids: NotRequired["aws_sdk_backup.types.compliance_resource_id_list.ComplianceResourceIdList"]
    """<p>The ID of the only Amazon Web Services resource that you want your control scope to contain.</p>"""
    compliance_resource_types: NotRequired["aws_sdk_backup.types.resource_type_list.ResourceTypeList"]
...
def serialize_json(value: ControlScope) -> dict:
    ...
    if "compliance_resource_ids" in value:
        import aws_sdk_backup.types.compliance_resource_id_list
        out["ComplianceResourceIds"] = aws_sdk_backup.types.compliance_resource_id_list.serialize_json(value["compliance_resource_ids"])
    if "compliance_resource_types" in value:
        import aws_sdk_backup.types.resource_type_list
        out["ComplianceResourceTypes"] = aws_sdk_backup.types.resource_type_list.serialize_json(value["compliance_resource_types"])
    if "tags" in value:
        import aws_sdk_backup.types.string_map
...
def deserialize_json(data: dict) -> ControlScope:
    ...
    if "ComplianceResourceIds" in data:
        import aws_sdk_backup.types.compliance_resource_id_list
        out["compliance_resource_ids"] = aws_sdk_backup.types.compliance_resource_id_list.deserialize_json(data["ComplianceResourceIds"])
    if "ComplianceResourceTypes" in data:
        import aws_sdk_backup.types.resource_type_list
        out["compliance_resource_types"] = aws_sdk_backup.types.resource_type_list.deserialize_json(data["ComplianceResourceTypes"])
    if "Tags" in data:
        import aws_sdk_backup.types.string_map
