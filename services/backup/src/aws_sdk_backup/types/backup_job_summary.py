"""Generated from Smithy shape ``com.amazonaws.backup#BackupJobSummary``."""
...
class BackupJobSummary(TypedDict):
    ...
    state: NotRequired["aws_sdk_backup.types.backup_job_status.BackupJobStatus"]
...
# --- restJson1 ser/de ---
def serialize_json(value: BackupJobSummary) -> dict:
    out: dict = {}
    if "region" in value:
    ...
    if "state" in value:
        import aws_sdk_backup.types.backup_job_status
        out["State"] = aws_sdk_backup.types.backup_job_status.serialize_json(value["state"])
...
def deserialize_json(data: dict) -> BackupJobSummary:
    out: BackupJobSummary = {}  # type: ignore[typeddict-item]
    if "Region" in data:
    ...
    if "State" in data:
        import aws_sdk_backup.types.backup_job_status
        out["state"] = aws_sdk_backup.types.backup_job_status.deserialize_json(data["State"])
