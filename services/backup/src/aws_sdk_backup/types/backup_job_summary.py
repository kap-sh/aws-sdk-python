...
class BackupJobSummary(TypedDict, closed=True):
...
def serialize_json(value: BackupJobSummary) -> dict:
    ...
    if "state" in value:
        ...
        out["State"] = aws_sdk_backup.types.backup_job_status.serialize_json(
            value["state"]
        )
...
def deserialize_json(data: dict) -> BackupJobSummary:
    ...
    if "State" in data:
        ...
        out["state"] = aws_sdk_backup.types.backup_job_status.deserialize_json(
            data["State"]
        )
