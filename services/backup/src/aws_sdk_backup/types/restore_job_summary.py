...
class RestoreJobSummary(TypedDict):
    ...
    state: NotRequired["aws_sdk_backup.types.restore_job_state.RestoreJobState"]
...
def serialize_json(value: RestoreJobSummary) -> dict:
    ...
    if "state" in value:
        ...
        out["State"] = aws_sdk_backup.types.restore_job_state.serialize_json(
            value["state"]
        )
...
def deserialize_json(data: dict) -> RestoreJobSummary:
    ...
    if "State" in data:
        ...
        out["state"] = aws_sdk_backup.types.restore_job_state.deserialize_json(
            data["State"]
        )
