...
def serialize_json(value: AggregatedScanResult) -> dict:
    ...
    if "findings" in value:
        import aws_sdk_backup.types.scan_findings
        out["Findings"] = aws_sdk_backup.types.scan_findings.serialize_json(value["findings"])
    if "last_computed" in value:
        import aws_sdk_backup.types.timestamp
        out["LastComputed"] = aws_sdk_backup.types.timestamp.serialize_json(value["last_computed"])
    return out
...
def deserialize_json(data: dict) -> AggregatedScanResult:
    ...
    if "Findings" in data:
        import aws_sdk_backup.types.scan_findings
        out["findings"] = aws_sdk_backup.types.scan_findings.deserialize_json(data["Findings"])
    if "LastComputed" in data:
        import aws_sdk_backup.types.timestamp
        out["last_computed"] = aws_sdk_backup.types.timestamp.deserialize_json(data["LastComputed"])
    return out