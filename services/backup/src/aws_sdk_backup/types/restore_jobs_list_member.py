"""Generated from Smithy shape ``com.amazonaws.backup#RestoreJobsListMember``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import aws_sdk_backup.types.account_id
    import aws_sdk_backup.types.arn
    import aws_sdk_backup.types.boolean2
    import aws_sdk_backup.types.iam_role_arn
    import aws_sdk_backup.types.long
    import aws_sdk_backup.types.resource_type
    import aws_sdk_backup.types.restore_deletion_status
    import aws_sdk_backup.types.restore_job_creator
    import aws_sdk_backup.types.restore_job_status
    import aws_sdk_backup.types.restore_validation_status
    import aws_sdk_backup.types.string
    import aws_sdk_backup.types.timestamp


class RestoreJobsListMember(TypedDict, closed=True):
    account_id: NotRequired["aws_sdk_backup.types.account_id.AccountId"]
    """<p>The account ID that owns the restore job.</p>"""
    restore_job_id: NotRequired["aws_sdk_backup.types.string.string"]
    """<p>Uniquely identifies the job that restores a recovery point.</p>"""
    recovery_point_arn: NotRequired["aws_sdk_backup.types.arn.ARN"]
    """<p>An ARN that uniquely identifies a recovery point; for example, <code>arn:aws:backup:us-east-1:123456789012:recovery-point:1EB3B5E7-9EB0-435A-A80B-108B488B0D45</code>.</p>"""
    source_resource_arn: NotRequired["aws_sdk_backup.types.arn.ARN"]
    """<p>The Amazon Resource Name (ARN) of the original resource that was backed up. This provides context about what resource is being restored.</p>"""
    backup_vault_arn: NotRequired["aws_sdk_backup.types.arn.ARN"]
    """<p>The Amazon Resource Name (ARN) of the backup vault containing the recovery point being restored. This helps identify vault access policies and permissions.</p>"""
    creation_date: NotRequired["aws_sdk_backup.types.timestamp.timestamp"]
    """<p>The date and time a restore job is created, in Unix format and Coordinated Universal Time (UTC). The value of <code>CreationDate</code> is accurate to milliseconds. For example, the value 1516925490.087 represents Friday, January 26, 2018 12:11:30.087 AM.</p>"""
    completion_date: NotRequired["aws_sdk_backup.types.timestamp.timestamp"]
    """<p>The date and time a job to restore a recovery point is completed, in Unix format and Coordinated Universal Time (UTC). The value of <code>CompletionDate</code> is accurate to milliseconds. For example, the value 1516925490.087 represents Friday, January 26, 2018 12:11:30.087 AM.</p>"""
    status: NotRequired["aws_sdk_backup.types.restore_job_status.RestoreJobStatus"]
    """<p>A status code specifying the state of the job initiated by Backup to restore a recovery point.</p>"""
    status_message: NotRequired["aws_sdk_backup.types.string.string"]
    """<p>A detailed message explaining the status of the job to restore a recovery point.</p>"""
    percent_done: NotRequired["aws_sdk_backup.types.string.string"]
    """<p>Contains an estimated percentage complete of a job at the time the job status was queried.</p>"""
    backup_size_in_bytes: NotRequired["aws_sdk_backup.types.long.Long"]
    """<p>The size, in bytes, of the restored resource.</p>"""
    iam_role_arn: NotRequired["aws_sdk_backup.types.iam_role_arn.IAMRoleArn"]
    """<p>The IAM role ARN used to create the target recovery point; for example, <code>arn:aws:iam::123456789012:role/S3Access</code>.</p>"""
    expected_completion_time_minutes: NotRequired["aws_sdk_backup.types.long.Long"]
    """<p>The amount of time in minutes that a job restoring a recovery point is expected to take.</p>"""
    created_resource_arn: NotRequired["aws_sdk_backup.types.arn.ARN"]
    """<p>An Amazon Resource Name (ARN) that uniquely identifies a resource. The format of the ARN depends on the resource type.</p>"""
    resource_type: NotRequired["aws_sdk_backup.types.resource_type.ResourceType"]
    """<p>The resource type of the listed restore jobs; for example, an Amazon Elastic Block Store (Amazon EBS) volume or an Amazon Relational Database Service (Amazon RDS) database. For Windows Volume Shadow Copy Service (VSS) backups, the only supported resource type is Amazon EC2.</p>"""
    recovery_point_creation_date: NotRequired[
        "aws_sdk_backup.types.timestamp.timestamp"
    ]
    """<p>The date on which a recovery point was created.</p>"""
    is_parent: "aws_sdk_backup.types.boolean2.Boolean2"
    """<p>This is a boolean value indicating whether the restore job is a parent (composite) restore job.</p>"""
    parent_job_id: NotRequired["aws_sdk_backup.types.string.string"]
    """<p>This is the unique identifier of the parent restore job for the selected restore job.</p>"""
    created_by: NotRequired[
        "aws_sdk_backup.types.restore_job_creator.RestoreJobCreator"
    ]
    """<p>Contains identifying information about the creation of a restore job.</p>"""
    validation_status: NotRequired[
        "aws_sdk_backup.types.restore_validation_status.RestoreValidationStatus"
    ]
    """<p>The status of validation run on the indicated restore job.</p>"""
    validation_status_message: NotRequired["aws_sdk_backup.types.string.string"]
    """<p>This describes the status of validation run on the indicated restore job.</p>"""
    deletion_status: NotRequired[
        "aws_sdk_backup.types.restore_deletion_status.RestoreDeletionStatus"
    ]
    """<p>This notes the status of the data generated by the restore test. The status may be <code>Deleting</code>, <code>Failed</code>, or <code>Successful</code>.</p>"""
    deletion_status_message: NotRequired["aws_sdk_backup.types.string.string"]
    """<p>This describes the restore job deletion status.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: RestoreJobsListMember) -> dict:
    out: dict = {}
    if "account_id" in value:
        out["AccountId"] = value["account_id"]
    if "restore_job_id" in value:
        out["RestoreJobId"] = value["restore_job_id"]
    if "recovery_point_arn" in value:
        out["RecoveryPointArn"] = value["recovery_point_arn"]
    if "source_resource_arn" in value:
        out["SourceResourceArn"] = value["source_resource_arn"]
    if "backup_vault_arn" in value:
        out["BackupVaultArn"] = value["backup_vault_arn"]
    if "creation_date" in value:
        import aws_sdk_backup.types.timestamp

        out["CreationDate"] = aws_sdk_backup.types.timestamp.serialize_json(
            value["creation_date"]
        )
    if "completion_date" in value:
        import aws_sdk_backup.types.timestamp

        out["CompletionDate"] = aws_sdk_backup.types.timestamp.serialize_json(
            value["completion_date"]
        )
    if "status" in value:
        import aws_sdk_backup.types.restore_job_status

        out["Status"] = aws_sdk_backup.types.restore_job_status.serialize_json(
            value["status"]
        )
    if "status_message" in value:
        out["StatusMessage"] = value["status_message"]
    if "percent_done" in value:
        out["PercentDone"] = value["percent_done"]
    if "backup_size_in_bytes" in value:
        out["BackupSizeInBytes"] = value["backup_size_in_bytes"]
    if "iam_role_arn" in value:
        out["IamRoleArn"] = value["iam_role_arn"]
    if "expected_completion_time_minutes" in value:
        out["ExpectedCompletionTimeMinutes"] = value["expected_completion_time_minutes"]
    if "created_resource_arn" in value:
        out["CreatedResourceArn"] = value["created_resource_arn"]
    if "resource_type" in value:
        out["ResourceType"] = value["resource_type"]
    if "recovery_point_creation_date" in value:
        import aws_sdk_backup.types.timestamp

        out["RecoveryPointCreationDate"] = (
            aws_sdk_backup.types.timestamp.serialize_json(
                value["recovery_point_creation_date"]
            )
        )
    out["IsParent"] = value.get("is_parent", False)
    if "parent_job_id" in value:
        out["ParentJobId"] = value["parent_job_id"]
    if "created_by" in value:
        import aws_sdk_backup.types.restore_job_creator

        out["CreatedBy"] = aws_sdk_backup.types.restore_job_creator.serialize_json(
            value["created_by"]
        )
    if "validation_status" in value:
        import aws_sdk_backup.types.restore_validation_status

        out["ValidationStatus"] = (
            aws_sdk_backup.types.restore_validation_status.serialize_json(
                value["validation_status"]
            )
        )
    if "validation_status_message" in value:
        out["ValidationStatusMessage"] = value["validation_status_message"]
    if "deletion_status" in value:
        import aws_sdk_backup.types.restore_deletion_status

        out["DeletionStatus"] = (
            aws_sdk_backup.types.restore_deletion_status.serialize_json(
                value["deletion_status"]
            )
        )
    if "deletion_status_message" in value:
        out["DeletionStatusMessage"] = value["deletion_status_message"]
    return out


def deserialize_json(data: dict) -> RestoreJobsListMember:
    out: RestoreJobsListMember = {}  # type: ignore[typeddict-item]
    if "AccountId" in data:
        out["account_id"] = data["AccountId"]
    if "RestoreJobId" in data:
        out["restore_job_id"] = data["RestoreJobId"]
    if "RecoveryPointArn" in data:
        out["recovery_point_arn"] = data["RecoveryPointArn"]
    if "SourceResourceArn" in data:
        out["source_resource_arn"] = data["SourceResourceArn"]
    if "BackupVaultArn" in data:
        out["backup_vault_arn"] = data["BackupVaultArn"]
    if "CreationDate" in data:
        import aws_sdk_backup.types.timestamp

        out["creation_date"] = aws_sdk_backup.types.timestamp.deserialize_json(
            data["CreationDate"]
        )
    if "CompletionDate" in data:
        import aws_sdk_backup.types.timestamp

        out["completion_date"] = aws_sdk_backup.types.timestamp.deserialize_json(
            data["CompletionDate"]
        )
    if "Status" in data:
        import aws_sdk_backup.types.restore_job_status

        out["status"] = aws_sdk_backup.types.restore_job_status.deserialize_json(
            data["Status"]
        )
    if "StatusMessage" in data:
        out["status_message"] = data["StatusMessage"]
    if "PercentDone" in data:
        out["percent_done"] = data["PercentDone"]
    if "BackupSizeInBytes" in data:
        out["backup_size_in_bytes"] = data["BackupSizeInBytes"]
    if "IamRoleArn" in data:
        out["iam_role_arn"] = data["IamRoleArn"]
    if "ExpectedCompletionTimeMinutes" in data:
        out["expected_completion_time_minutes"] = data["ExpectedCompletionTimeMinutes"]
    if "CreatedResourceArn" in data:
        out["created_resource_arn"] = data["CreatedResourceArn"]
    if "ResourceType" in data:
        out["resource_type"] = data["ResourceType"]
    if "RecoveryPointCreationDate" in data:
        import aws_sdk_backup.types.timestamp

        out["recovery_point_creation_date"] = (
            aws_sdk_backup.types.timestamp.deserialize_json(
                data["RecoveryPointCreationDate"]
            )
        )
    if "IsParent" in data:
        out["is_parent"] = data["IsParent"]
    else:
        out["is_parent"] = False
    if "ParentJobId" in data:
        out["parent_job_id"] = data["ParentJobId"]
    if "CreatedBy" in data:
        import aws_sdk_backup.types.restore_job_creator

        out["created_by"] = aws_sdk_backup.types.restore_job_creator.deserialize_json(
            data["CreatedBy"]
        )
    if "ValidationStatus" in data:
        import aws_sdk_backup.types.restore_validation_status

        out["validation_status"] = (
            aws_sdk_backup.types.restore_validation_status.deserialize_json(
                data["ValidationStatus"]
            )
        )
    if "ValidationStatusMessage" in data:
        out["validation_status_message"] = data["ValidationStatusMessage"]
    if "DeletionStatus" in data:
        import aws_sdk_backup.types.restore_deletion_status

        out["deletion_status"] = (
            aws_sdk_backup.types.restore_deletion_status.deserialize_json(
                data["DeletionStatus"]
            )
        )
    if "DeletionStatusMessage" in data:
        out["deletion_status_message"] = data["DeletionStatusMessage"]
    return out
