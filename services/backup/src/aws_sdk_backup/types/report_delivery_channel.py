...
if TYPE_CHECKING:
    import aws_sdk_backup.types.format_list
    import aws_sdk_backup.types.string


class ReportDeliveryChannel(TypedDict):
    s3_bucket_name: "aws_sdk_backup.types.string.string"
    """<p>The unique name of the S3 bucket that receives your reports.</p>"""
    s3_key_prefix: NotRequired["aws_sdk_backup.types.string.string"]
    ...
    formats: NotRequired["aws_sdk_backup.types.format_list.FormatList"]
...
def serialize_json(value: ReportDeliveryChannel) -> dict:
    ...
    if "formats" in value:
        ...
        out["Formats"] = aws_sdk_backup.types.format_list.serialize_json(
            value["formats"]
        )
    return out
...
def deserialize_json(data: dict) -> ReportDeliveryChannel:
    ...
    if "Formats" in data:
        ...
        out["formats"] = aws_sdk_backup.types.format_list.deserialize_json(
            data["Formats"]
        )
    return out
