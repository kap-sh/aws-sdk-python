...
def serialize_json(value: CreateBackupSelectionInput) -> dict:
    out: dict = {}
    import aws_sdk_backup.types.backup_selection
    out["BackupSelection"] = aws_sdk_backup.types.backup_selection.serialize_json(value["backup_selection"])
...
def deserialize_json(data: dict) -> CreateBackupSelectionInput:
    ...
    if "BackupSelection" in data:
        import aws_sdk_backup.types.backup_selection
        out["backup_selection"] = aws_sdk_backup.types.backup_selection.deserialize_json(data["BackupSelection"])
    else:
        raise DeserializationError("CreateBackupSelectionInput.backup_selection required")
