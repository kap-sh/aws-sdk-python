...
def serialize_json(value: CreateBackupPlanInput) -> dict:
    out: dict = {}
    import aws_sdk_backup.types.backup_plan_input
    out["BackupPlan"] = aws_sdk_backup.types.backup_plan_input.serialize_json(value["backup_plan"])
    if "backup_plan_tags" in value:
        import aws_sdk_backup.types.tags
        out["BackupPlanTags"] = aws_sdk_backup.types.tags.serialize_json(value["backup_plan_tags"])
...
def deserialize_json(data: dict) -> CreateBackupPlanInput:
    ...
    if "BackupPlan" in data:
        import aws_sdk_backup.types.backup_plan_input
        out["backup_plan"] = aws_sdk_backup.types.backup_plan_input.deserialize_json(data["BackupPlan"])
    else:
        raise DeserializationError("CreateBackupPlanInput.backup_plan required")
    if "BackupPlanTags" in data:
        import aws_sdk_backup.types.tags
        out["backup_plan_tags"] = aws_sdk_backup.types.tags.deserialize_json(data["BackupPlanTags"])
