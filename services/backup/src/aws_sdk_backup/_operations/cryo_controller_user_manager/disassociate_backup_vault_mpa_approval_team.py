"""Generated from Smithy shape ``com.amazonaws.backup#DisassociateBackupVaultMpaApprovalTeam``."""
...
def build_request(
    ...
    input_: aws_sdk_backup.types.disassociate_backup_vault_mpa_approval_team_input.DisassociateBackupVaultMpaApprovalTeamInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/backup-vaults/{BackupVaultName}/mpaApprovalTeam?delete"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_backup.types.disassociate_backup_vault_mpa_approval_team_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_disassociate_backup_vault_mpa_approval_team(
