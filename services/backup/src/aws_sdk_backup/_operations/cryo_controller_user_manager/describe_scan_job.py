"""Generated from Smithy shape ``com.amazonaws.backup#DescribeScanJob``."""
...
def handle_response(
    ...
    out: aws_sdk_backup.types.describe_scan_job_output.DescribeScanJobOutput = (
        aws_sdk_backup.types.describe_scan_job_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_backup.types.describe_scan_job_input.DescribeScanJobInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/scan/jobs/{ScanJobId}"
    url = url.replace("{ScanJobId}", quote(str(input_["scan_job_id"]), safe=""))
    params: dict[str, str] = {}
...
def describe_scan_job(
    ...
    aws_sdk_backup.types.describe_scan_job_output.DescribeScanJobOutput, zapros.Response
]:
...
async def async_describe_scan_job(
