"""Generated from Smithy shape ``com.amazonaws.backup#StartScanJob``."""
...
def handle_response(
    ...
    out: aws_sdk_backup.types.start_scan_job_output.StartScanJobOutput = (
        aws_sdk_backup.types.start_scan_job_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_backup.types.start_scan_job_input.StartScanJobInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/scan/job"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_backup.types.start_scan_job_input.serialize_json(input_)
    ).encode()
...
def start_scan_job(
    ...
    aws_sdk_backup.types.start_scan_job_output.StartScanJobOutput, zapros.Response
]:
...
async def async_start_scan_job(
