"""Generated from Smithy shape ``com.amazonaws.backup#CreateBackupPlan``."""
...
def handle_response(
    ...
    out: aws_sdk_backup.types.create_backup_plan_output.CreateBackupPlanOutput = (
        aws_sdk_backup.types.create_backup_plan_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_backup.types.create_backup_plan_input.CreateBackupPlanInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/backup/plans"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_backup.types.create_backup_plan_input

    body: bytes | None = json.dumps(
        aws_sdk_backup.types.create_backup_plan_input.serialize_json(input_)
    ).encode()
...
async def async_create_backup_plan(
