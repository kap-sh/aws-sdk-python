"""Generated from Smithy shape ``com.amazonaws.backup#DescribeBackupJob``."""
...
def build_request(
    ...
    input_: aws_sdk_backup.types.describe_backup_job_input.DescribeBackupJobInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/backup-jobs/{BackupJobId}"
    url = url.replace("{BackupJobId}", quote(str(input_["backup_job_id"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_backup_job(
