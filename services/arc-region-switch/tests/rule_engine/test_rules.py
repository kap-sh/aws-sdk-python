import pytest
from capo_arc_region_switch._rule_engine._endpoint_rule_set import EndpointParams, resolve
from capo_arc_region_switch._rule_engine._endpoint_runtime import EndpointError
import re
import zapros
...
def test_control_plane_operation_with_fips_in_us_():
    ...
    params = EndpointParams(Region='us-east-1', UseControlPlaneEndpoint=True, UseFIPS=True)
    result = resolve(params)
    assert result.url == 'https://arc-region-switch-control-plane-fips.us-east-1.api.aws'

def test_control_plane_operation_with_fips_in_cn_():
    """Control plane operation with FIPS in CN returns an error"""
    params = EndpointParams(Region='cn-north-1', UseControlPlaneEndpoint=True, UseFIPS=True)
...
def test_control_plane_operation_with_endpoint_se():
    ...
    params = EndpointParams(Region='us-east-1', UseControlPlaneEndpoint=True, UseFIPS=True, Endpoint='https://amazonaws.com')
    with pytest.raises(EndpointError, match=re.escape('Invalid Configuration: FIPS and custom endpoint are not supported')):
        resolve(params)

def test_control_plane_operation_with_endpoint_se():
    """Control plane operation with endpoint set using FIPS in us-west-2 routes to provided endpoint"""
    params = EndpointParams(Region='us-west-2', UseControlPlaneEndpoint=True, UseFIPS=True, Endpoint='https://amazonaws.com')
    with pytest.raises(EndpointError, match=re.escape('Invalid Configuration: FIPS and custom endpoint are not supported')):
        resolve(params)