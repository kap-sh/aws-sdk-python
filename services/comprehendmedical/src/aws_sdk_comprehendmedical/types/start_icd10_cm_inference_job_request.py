"""Generated from Smithy shape ``com.amazonaws.comprehendmedical#StartICD10CMInferenceJobRequest``."""
...
class StartICD10CMInferenceJobRequest(TypedDict, closed=True):
    input_data_config: (
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: StartICD10CMInferenceJobRequest) -> dict:
    out: dict = {}
    import aws_sdk_comprehendmedical.types.input_data_config
...
def deserialize_aws_json_1_1(data: dict) -> StartICD10CMInferenceJobRequest:
    out: StartICD10CMInferenceJobRequest = {}  # type: ignore[typeddict-item]
    if "InputDataConfig" in data:
        ...
        raise DeserializationError(
            "StartICD10CMInferenceJobRequest.input_data_config required"
        )
    if "OutputDataConfig" in data:
        ...
        raise DeserializationError(
            "StartICD10CMInferenceJobRequest.output_data_config required"
        )
    if "DataAccessRoleArn" in data:
        out["data_access_role_arn"] = data["DataAccessRoleArn"]
    else:
        raise DeserializationError(
            "StartICD10CMInferenceJobRequest.data_access_role_arn required"
        )
    if "JobName" in data:
    ...
    if "LanguageCode" in data:
        ...
        raise DeserializationError(
            "StartICD10CMInferenceJobRequest.language_code required"
        )
    return out
