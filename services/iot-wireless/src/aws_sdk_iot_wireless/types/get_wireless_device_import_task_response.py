"""Generated from Smithy shape ``com.amazonaws.iotwireless#GetWirelessDeviceImportTaskResponse``."""
...
class GetWirelessDeviceImportTaskResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetWirelessDeviceImportTaskResponse) -> dict:
    out: dict = {}
    if "id" in value:
        out["Id"] = value["id"]
    if "arn" in value:
...
def deserialize_json(data: dict) -> GetWirelessDeviceImportTaskResponse:
    out: GetWirelessDeviceImportTaskResponse = {}  # type: ignore[typeddict-item]
    if "Id" in data:
        out["id"] = data["Id"]
    if "Arn" in data:
        out["arn"] = data["Arn"]
