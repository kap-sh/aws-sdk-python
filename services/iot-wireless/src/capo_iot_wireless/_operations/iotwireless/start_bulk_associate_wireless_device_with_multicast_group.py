"""Generated from Smithy shape ``com.amazonaws.iotwireless#StartBulkAssociateWirelessDeviceWithMulticastGroup``."""
...
def handle_response(
    ...
    out: capo_iot_wireless.types.start_bulk_associate_wireless_device_with_multicast_group_response.StartBulkAssociateWirelessDeviceWithMulticastGroupResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/multicast-groups/{Id}/bulk"
    url = url.replace("{Id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_wireless.types.start_bulk_associate_wireless_device_with_multicast_group_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_bulk_associate_wireless_device_with_multicast_group(
