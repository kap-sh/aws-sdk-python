"""Generated from Smithy shape ``com.amazonaws.iotwireless#AssociateMulticastGroupWithFuotaTask``."""
...
def handle_response(
    ...
    out: capo_iot_wireless.types.associate_multicast_group_with_fuota_task_response.AssociateMulticastGroupWithFuotaTaskResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/fuota-tasks/{Id}/multicast-group"
    url = url.replace("{Id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_wireless.types.associate_multicast_group_with_fuota_task_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_associate_multicast_group_with_fuota_task(
