"""Generated from Smithy shape ``com.amazonaws.iotwireless#CreateFuotaTask``."""
...
def handle_response(
    ...
    out: capo_iot_wireless.types.create_fuota_task_response.CreateFuotaTaskResponse = (
        capo_iot_wireless.types.create_fuota_task_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/fuota-tasks"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_wireless.types.create_fuota_task_request.serialize_json(input_)
    ).encode()
...
async def async_create_fuota_task(
