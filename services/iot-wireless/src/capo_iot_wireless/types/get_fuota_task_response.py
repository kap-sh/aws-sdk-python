...
if TYPE_CHECKING:
    ...
    import capo_iot_wireless.types.fuota_task_status
    import capo_iot_wireless.types.lo_ra_wan_fuota_task_get_info
...
class GetFuotaTaskResponse(TypedDict, closed=True):
    ...
    status: NotRequired["capo_iot_wireless.types.fuota_task_status.FuotaTaskStatus"]
    ...
    lo_ra_wan: NotRequired[
        "capo_iot_wireless.types.lo_ra_wan_fuota_task_get_info.LoRaWANFuotaTaskGetInfo"
    ]
...
def serialize_json(value: GetFuotaTaskResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_iot_wireless.types.fuota_task_status.serialize_json(
            value["status"]
        )
    if "name" in value:
        out["Name"] = value["name"]
    ...
    if "lo_ra_wan" in value:
        ...
        out["LoRaWAN"] = (
            capo_iot_wireless.types.lo_ra_wan_fuota_task_get_info.serialize_json(
                value["lo_ra_wan"]
            )
        )
...
def deserialize_json(data: dict) -> GetFuotaTaskResponse:
    ...
    if "Status" in data:
        ...
        out["status"] = capo_iot_wireless.types.fuota_task_status.deserialize_json(
            data["Status"]
        )
    if "Name" in data:
        out["name"] = data["Name"]
    ...
    if "LoRaWAN" in data:
        ...
        out["lo_ra_wan"] = (
            capo_iot_wireless.types.lo_ra_wan_fuota_task_get_info.deserialize_json(
                data["LoRaWAN"]
            )
        )
