...
if TYPE_CHECKING:
    ...
    import capo_iot_wireless.types.wireless_gateway_task_definition_arn
...
class UpdateWirelessGatewayTaskEntry(TypedDict, closed=True):
    ...
    lo_ra_wan: NotRequired[
        "capo_iot_wireless.types.lo_ra_wan_update_gateway_task_entry.LoRaWANUpdateGatewayTaskEntry"
    ]
    """<p>The properties that relate to the LoRaWAN wireless gateway.</p>"""
    arn: NotRequired[
        "capo_iot_wireless.types.wireless_gateway_task_definition_arn.WirelessGatewayTaskDefinitionArn"
    ]
...
def serialize_json(value: UpdateWirelessGatewayTaskEntry) -> dict:
    ...
    if "lo_ra_wan" in value:
        ...
        out["LoRaWAN"] = (
            capo_iot_wireless.types.lo_ra_wan_update_gateway_task_entry.serialize_json(
                value["lo_ra_wan"]
            )
        )
    if "arn" in value:
...
def deserialize_json(data: dict) -> UpdateWirelessGatewayTaskEntry:
    ...
    if "LoRaWAN" in data:
        ...
        out["lo_ra_wan"] = (
            capo_iot_wireless.types.lo_ra_wan_update_gateway_task_entry.deserialize_json(
                data["LoRaWAN"]
            )
        )
    if "Arn" in data:
