...
class SemtechGnssDetail(TypedDict, closed=True):
    ...
    type: NotRequired["capo_iot_wireless.types.position_solver_type.PositionSolverType"]
    """<p>The type of positioning solver used.</p>"""
    status: NotRequired[
        "capo_iot_wireless.types.position_configuration_status.PositionConfigurationStatus"
    ]
    """<p>The status indicating whether the solver is enabled.</p>"""
    fec: NotRequired[
        "capo_iot_wireless.types.position_configuration_fec.PositionConfigurationFec"
    ]
...
def serialize_json(value: SemtechGnssDetail) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = capo_iot_wireless.types.position_solver_type.serialize_json(
            value["type"]
        )
    if "status" in value:
        import capo_iot_wireless.types.position_configuration_status
...
def deserialize_json(data: dict) -> SemtechGnssDetail:
    ...
    if "Type" in data:
        ...
        out["type"] = capo_iot_wireless.types.position_solver_type.deserialize_json(
            data["Type"]
        )
    if "Status" in data:
        import capo_iot_wireless.types.position_configuration_status
