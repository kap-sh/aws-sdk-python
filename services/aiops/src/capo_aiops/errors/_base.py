...
class AIOpsError(Exception):
    """Base class for all AIOps service errors."""


class ServiceError(AIOpsError):
...
class SerializationError(AIOpsError):
    """Raised when a value cannot be serialized."""


class DeserializationError(AIOpsError):
...
class WaiterFailedError(AIOpsError):
...
class WaiterTimeoutError(AIOpsError):
