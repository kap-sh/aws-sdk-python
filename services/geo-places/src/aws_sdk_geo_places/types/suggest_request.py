...
class SuggestRequest(TypedDict):
    ...
    r"""<p> Maximum number of query terms to be returned for use with a search text query. Not supported in <code>ap-southeast-1</code> and <code>ap-southeast-5</code> regions for <a href=\"https://docs.aws.amazon.com/location/latest/developerguide/GrabMaps.html\">GrabMaps</a> customers. </p>"""
    ...
    filter: NotRequired["aws_sdk_geo_places.types.suggest_filter.SuggestFilter"]
...
def serialize_json(value: SuggestRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["Filter"] = aws_sdk_geo_places.types.suggest_filter.serialize_json(
            value["filter"]
        )
    if "additional_features" in value:
        ...
        out["AdditionalFeatures"] = (
            aws_sdk_geo_places.types.suggest_additional_feature_list.serialize_json(
...
def deserialize_json(data: dict) -> SuggestRequest:
    ...
    if "MaxQueryRefinements" in data:
    ...
    if "Filter" in data:
        ...
        out["filter"] = aws_sdk_geo_places.types.suggest_filter.deserialize_json(
            data["Filter"]
        )
    if "AdditionalFeatures" in data:
        ...
        out["additional_features"] = (
            aws_sdk_geo_places.types.suggest_additional_feature_list.deserialize_json(
