...
class SuggestResponse(TypedDict, closed=True):
    ...
    result_items: NotRequired[
        "capo_geo_places.types.suggest_result_item_list.SuggestResultItemList"
    ]
    """<p>List of places or results returned for a query. </p>"""
    query_refinements: NotRequired[
        "capo_geo_places.types.query_refinement_list.QueryRefinementList"
    ]
...
def serialize_json(value: SuggestResponse) -> dict:
    ...
    if "result_items" in value:
        ...
        out["ResultItems"] = (
            capo_geo_places.types.suggest_result_item_list.serialize_json(
                value["result_items"]
            )
        )
    if "query_refinements" in value:
        ...
        out["QueryRefinements"] = (
            capo_geo_places.types.query_refinement_list.serialize_json(
...
def deserialize_json(data: dict) -> SuggestResponse:
    ...
    if "ResultItems" in data:
        ...
        out["result_items"] = (
            capo_geo_places.types.suggest_result_item_list.deserialize_json(
                data["ResultItems"]
            )
        )
    if "QueryRefinements" in data:
        ...
        out["query_refinements"] = (
            capo_geo_places.types.query_refinement_list.deserialize_json(
