...
if TYPE_CHECKING:
    ...
    import aws_sdk_vpc_lattice.types.header_match_name
    import aws_sdk_vpc_lattice.types.header_match_type


class HeaderMatch(TypedDict, closed=True):
    name: "aws_sdk_vpc_lattice.types.header_match_name.HeaderMatchName"
    """<p>The name of the header.</p>"""
    match: "aws_sdk_vpc_lattice.types.header_match_type.HeaderMatchType"
...
def serialize_json(value: HeaderMatch) -> dict:
    ...
    out["match"] = aws_sdk_vpc_lattice.types.header_match_type.serialize_json(
        value["match"]
    )
...
def deserialize_json(data: dict) -> HeaderMatch:
    ...
    if "match" in data:
        ...
        out["match"] = aws_sdk_vpc_lattice.types.header_match_type.deserialize_json(
            data["match"]
        )
    else:
