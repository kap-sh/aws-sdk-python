...
def serialize_json(value: ListServiceNetworkResourceAssociationsResponse) -> dict:
    ...
    out["items"] = (
        aws_sdk_vpc_lattice.types.service_network_resource_association_list.serialize_json(
            value["items"]
        )
    )
...
def deserialize_json(data: dict) -> ListServiceNetworkResourceAssociationsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            aws_sdk_vpc_lattice.types.service_network_resource_association_list.deserialize_json(
                data["items"]
            )
        )
    else:
