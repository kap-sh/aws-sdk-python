...
class ListRulesResponse(TypedDict, closed=True):
    items: "aws_sdk_vpc_lattice.types.rule_summary_list.RuleSummaryList"
...
def serialize_json(value: ListRulesResponse) -> dict:
    ...
    out["items"] = aws_sdk_vpc_lattice.types.rule_summary_list.serialize_json(
        value["items"]
    )
...
def deserialize_json(data: dict) -> ListRulesResponse:
    ...
    if "items" in data:
        ...
        out["items"] = aws_sdk_vpc_lattice.types.rule_summary_list.deserialize_json(
            data["items"]
        )
    else:
