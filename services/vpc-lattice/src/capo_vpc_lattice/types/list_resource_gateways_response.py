...
if TYPE_CHECKING:
    ...
    import capo_vpc_lattice.types.resource_gateway_list


class ListResourceGatewaysResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_vpc_lattice.types.resource_gateway_list.ResourceGatewayList"
    ]
...
def serialize_json(value: ListResourceGatewaysResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_vpc_lattice.types.resource_gateway_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListResourceGatewaysResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_vpc_lattice.types.resource_gateway_list.deserialize_json(
            data["items"]
        )
