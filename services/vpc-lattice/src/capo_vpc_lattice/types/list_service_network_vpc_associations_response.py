...
if TYPE_CHECKING:
    ...
    import capo_vpc_lattice.types.service_network_vpc_association_list


class ListServiceNetworkVpcAssociationsResponse(TypedDict, closed=True):
    items: "capo_vpc_lattice.types.service_network_vpc_association_list.ServiceNetworkVpcAssociationList"
...
def serialize_json(value: ListServiceNetworkVpcAssociationsResponse) -> dict:
    ...
    out["items"] = (
        capo_vpc_lattice.types.service_network_vpc_association_list.serialize_json(
            value["items"]
        )
    )
...
def deserialize_json(data: dict) -> ListServiceNetworkVpcAssociationsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            capo_vpc_lattice.types.service_network_vpc_association_list.deserialize_json(
                data["items"]
            )
        )
    else:
