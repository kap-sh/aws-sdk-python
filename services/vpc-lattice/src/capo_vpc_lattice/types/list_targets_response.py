...
class ListTargetsResponse(TypedDict, closed=True):
    items: "capo_vpc_lattice.types.target_summary_list.TargetSummaryList"
...
def serialize_json(value: ListTargetsResponse) -> dict:
    ...
    out["items"] = capo_vpc_lattice.types.target_summary_list.serialize_json(
        value["items"]
    )
