...
def build_request(
    ...
    input_: aws_sdk_emr_serverless.types.get_dashboard_for_job_run_request.GetDashboardForJobRunRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{jobRunId}", quote(str(input_["job_run_id"]), safe=""))
    params: dict[str, str] = {}
    if "attempt" in input_:
        params["attempt"] = str(input_["attempt"])
    if "access_system_profile_logs" in input_:
        params["accessSystemProfileLogs"] = str(input_["access_system_profile_logs"])
