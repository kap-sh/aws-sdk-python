"""Generated from Smithy shape ``com.amazonaws.emrserverless#GetDashboardForJobRun``."""
...
def handle_response(
    ...
    out: aws_sdk_emr_serverless.types.get_dashboard_for_job_run_response.GetDashboardForJobRunResponse = aws_sdk_emr_serverless.types.get_dashboard_for_job_run_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_emr_serverless.types.get_dashboard_for_job_run_request.GetDashboardForJobRunRequest,
) -> zapros.Request:
    ...
    url = url.replace("{jobRunId}", quote(str(input["job_run_id"]), safe=""))
    params: dict[str, str] = {}
    if "attempt" in input:
        params["attempt"] = str(input["attempt"])
    if "access_system_profile_logs" in input:
        params["accessSystemProfileLogs"] = str(input["access_system_profile_logs"])
...
async def async_get_dashboard_for_job_run(
