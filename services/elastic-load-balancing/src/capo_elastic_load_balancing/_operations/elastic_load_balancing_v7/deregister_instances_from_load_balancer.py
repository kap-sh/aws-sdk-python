"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancing#DeregisterInstancesFromLoadBalancer``."""
...
import capo_elastic_load_balancing.errors.invalid_end_point_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidInstance":
            raise capo_elastic_load_balancing.errors.invalid_end_point_exception.InvalidEndPointException.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DeregisterInstancesFromLoadBalancerResult")
    out: capo_elastic_load_balancing.types.deregister_end_points_output.DeregisterEndPointsOutput = capo_elastic_load_balancing.types.deregister_end_points_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DeregisterInstancesFromLoadBalancer"))
    pairs.append(("Version", "2012-06-01"))
    capo_elastic_load_balancing.types.deregister_end_points_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_deregister_instances_from_load_balancer(
