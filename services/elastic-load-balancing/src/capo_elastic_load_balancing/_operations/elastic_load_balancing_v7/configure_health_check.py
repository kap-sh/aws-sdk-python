"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancing#ConfigureHealthCheck``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ConfigureHealthCheckResult")
    out: capo_elastic_load_balancing.types.configure_health_check_output.ConfigureHealthCheckOutput = capo_elastic_load_balancing.types.configure_health_check_output.deserialize_query(
...
def build_request(
    ...
    capo_elastic_load_balancing.types.configure_health_check_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_configure_health_check(
