"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancing#EnableAvailabilityZonesForLoadBalancer``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("EnableAvailabilityZonesForLoadBalancerResult")
    out: aws_sdk_elastic_load_balancing.types.add_availability_zones_output.AddAvailabilityZonesOutput = aws_sdk_elastic_load_balancing.types.add_availability_zones_output.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_elastic_load_balancing.types.add_availability_zones_input.AddAvailabilityZonesInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
...
async def async_enable_availability_zones_for_load_balancer(
