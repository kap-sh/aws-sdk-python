...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing.types.limit

Limits: TypeAlias = list["aws_sdk_elastic_load_balancing.types.limit.Limit"]
...
def serialize_query(value: Limits, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing.types.limit.serialize_query(
...
def deserialize_query(el: Element) -> Limits:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_elastic_load_balancing.types.limit.deserialize_query(child))
    return out
