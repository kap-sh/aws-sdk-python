...
class DeleteLoadBalancerListenerInput(TypedDict):
    load_balancer_name: (
...
def serialize_query(
    ...
    aws_sdk_elastic_load_balancing.types.ports.serialize_query(
        value["load_balancer_ports"], pairs, f"{prefix}.LoadBalancerPorts"
    )
...
def deserialize_query(el: Element) -> DeleteLoadBalancerListenerInput:
    ...
    if child_load_balancer_ports is not None:
        ...
        out["load_balancer_ports"] = (
            aws_sdk_elastic_load_balancing.types.ports.deserialize_query(
