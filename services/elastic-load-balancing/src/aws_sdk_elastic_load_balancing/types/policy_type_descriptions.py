...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing.types.policy_type_description

PolicyTypeDescriptions: TypeAlias = list[
    "aws_sdk_elastic_load_balancing.types.policy_type_description.PolicyTypeDescription"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing.types.policy_type_description.serialize_query(
...
def deserialize_query(el: Element) -> PolicyTypeDescriptions:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_load_balancing.types.policy_type_description.deserialize_query(
                child
            )
        )
    return out
