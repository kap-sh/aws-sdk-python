...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing.types.tag

TagList: TypeAlias = list["aws_sdk_elastic_load_balancing.types.tag.Tag"]
...
def serialize_query(value: TagList, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing.types.tag.serialize_query(
...
def deserialize_query(el: Element) -> TagList:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_elastic_load_balancing.types.tag.deserialize_query(child))
    return out
