...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing.types.instance_state

InstanceStates: TypeAlias = list[
    "aws_sdk_elastic_load_balancing.types.instance_state.InstanceState"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing.types.instance_state.serialize_query(
...
def deserialize_query(el: Element) -> InstanceStates:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_load_balancing.types.instance_state.deserialize_query(child)
        )
    return out
