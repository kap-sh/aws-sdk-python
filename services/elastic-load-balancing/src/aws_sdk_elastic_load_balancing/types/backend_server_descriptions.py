...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing.types.backend_server_description

BackendServerDescriptions: TypeAlias = list[
    "aws_sdk_elastic_load_balancing.types.backend_server_description.BackendServerDescription"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing.types.backend_server_description.serialize_query(
...
def deserialize_query(el: Element) -> BackendServerDescriptions:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_load_balancing.types.backend_server_description.deserialize_query(
                child
            )
        )
    return out
