...
class DescribeAccountLimitsOutput(TypedDict, closed=True):
    limits: NotRequired["aws_sdk_elastic_load_balancing.types.limits.Limits"]
...
def serialize_query(
    ...
    if "limits" in value:
        ...
        aws_sdk_elastic_load_balancing.types.limits.serialize_query(
...
def deserialize_query(el: Element) -> DescribeAccountLimitsOutput:
    ...
    if child_limits is not None:
        ...
        out["limits"] = aws_sdk_elastic_load_balancing.types.limits.deserialize_query(
            child_limits
        )
