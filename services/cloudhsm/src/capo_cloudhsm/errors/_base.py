...
class CloudHSMError(Exception):
    """Base class for all CloudHSM service errors."""


class ServiceError(CloudHSMError):
...
class SerializationError(CloudHSMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(CloudHSMError):
...
class WaiterFailedError(CloudHSMError):
...
class WaiterTimeoutError(CloudHSMError):
