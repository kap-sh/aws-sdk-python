...
class BatchGetFreeTrialInfoResponse(TypedDict):
    accounts: (
        "aws_sdk_inspector2.types.free_trial_account_info_list.FreeTrialAccountInfoList"
    )
    """<p>An array of objects that provide Amazon Inspector free trial details for each of the requested accounts. </p>"""
    failed_accounts: (
        "aws_sdk_inspector2.types.free_trial_info_error_list.FreeTrialInfoErrorList"
    )
...
def serialize_json(value: BatchGetFreeTrialInfoResponse) -> dict:
    ...
    out["accounts"] = (
        aws_sdk_inspector2.types.free_trial_account_info_list.serialize_json(
            value["accounts"]
        )
    )
    import aws_sdk_inspector2.types.free_trial_info_error_list
    ...
    out["failedAccounts"] = (
        aws_sdk_inspector2.types.free_trial_info_error_list.serialize_json(
...
def deserialize_json(data: dict) -> BatchGetFreeTrialInfoResponse:
    ...
    if "accounts" in data:
        ...
        out["accounts"] = (
            aws_sdk_inspector2.types.free_trial_account_info_list.deserialize_json(
                data["accounts"]
            )
        )
    else:
    ...
    if "failedAccounts" in data:
        ...
        out["failed_accounts"] = (
            aws_sdk_inspector2.types.free_trial_info_error_list.deserialize_json(
                data["failedAccounts"]
            )
        )
    else:
