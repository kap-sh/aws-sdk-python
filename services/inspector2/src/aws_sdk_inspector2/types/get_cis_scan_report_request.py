...
class GetCisScanReportRequest(TypedDict):
    ...
    report_format: "aws_sdk_inspector2.types.cis_report_format.CisReportFormat"
...
def serialize_json(value: GetCisScanReportRequest) -> dict:
    ...
    if "target_accounts" in value:
        ...
        out["targetAccounts"] = (
            aws_sdk_inspector2.types.report_target_accounts.serialize_json(
                value["target_accounts"]
            )
        )
    import aws_sdk_inspector2.types.cis_report_format

    out["reportFormat"] = aws_sdk_inspector2.types.cis_report_format.serialize_json(
...
def deserialize_json(data: dict) -> GetCisScanReportRequest:
    ...
    if "targetAccounts" in data:
        ...
        out["target_accounts"] = (
            aws_sdk_inspector2.types.report_target_accounts.deserialize_json(
                data["targetAccounts"]
            )
        )
    if "reportFormat" in data:
        ...
        out["report_format"] = (
            aws_sdk_inspector2.types.cis_report_format.deserialize_json(
                data["reportFormat"]
            )
        )
    else:
