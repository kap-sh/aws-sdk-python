...
def serialize_json(value: AtigData) -> dict:
    ...
    if "first_seen" in value:
        import aws_sdk_inspector2.types.first_seen
        out["firstSeen"] = aws_sdk_inspector2.types.first_seen.serialize_json(value["first_seen"])
    if "last_seen" in value:
        import aws_sdk_inspector2.types.last_seen
        out["lastSeen"] = aws_sdk_inspector2.types.last_seen.serialize_json(value["last_seen"])
    if "targets" in value:
        import aws_sdk_inspector2.types.targets
        out["targets"] = aws_sdk_inspector2.types.targets.serialize_json(value["targets"])
    if "ttps" in value:
        import aws_sdk_inspector2.types.ttps
...
def deserialize_json(data: dict) -> AtigData:
    ...
    if "firstSeen" in data:
        import aws_sdk_inspector2.types.first_seen
        out["first_seen"] = aws_sdk_inspector2.types.first_seen.deserialize_json(data["firstSeen"])
    if "lastSeen" in data:
        import aws_sdk_inspector2.types.last_seen
        out["last_seen"] = aws_sdk_inspector2.types.last_seen.deserialize_json(data["lastSeen"])
    if "targets" in data:
        import aws_sdk_inspector2.types.targets
        out["targets"] = aws_sdk_inspector2.types.targets.deserialize_json(data["targets"])
    if "ttps" in data:
        import aws_sdk_inspector2.types.ttps
