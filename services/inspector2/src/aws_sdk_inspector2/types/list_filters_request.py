...
class ListFiltersRequest(TypedDict, closed=True):
    arns: NotRequired["aws_sdk_inspector2.types.filter_arn_list.FilterArnList"]
...
def serialize_json(value: ListFiltersRequest) -> dict:
    ...
    if "arns" in value:
        ...
        out["arns"] = aws_sdk_inspector2.types.filter_arn_list.serialize_json(
            value["arns"]
        )
    if "action" in value:
...
def deserialize_json(data: dict) -> ListFiltersRequest:
    ...
    if "arns" in data:
        ...
        out["arns"] = aws_sdk_inspector2.types.filter_arn_list.deserialize_json(
            data["arns"]
        )
    if "action" in data:
