...
def serialize_json(value: AssociateConfigurationRequest) -> dict:
    ...
    import aws_sdk_inspector2.types.code_security_resource
    out["resource"] = aws_sdk_inspector2.types.code_security_resource.serialize_json(value["resource"])
    return out
...
def deserialize_json(data: dict) -> AssociateConfigurationRequest:
    ...
    if "scanConfigurationArn" in data:
        ...
        raise DeserializationError("AssociateConfigurationRequest.scan_configuration_arn required")
    if "resource" in data:
        import aws_sdk_inspector2.types.code_security_resource
        out["resource"] = aws_sdk_inspector2.types.code_security_resource.deserialize_json(data["resource"])
    else:
