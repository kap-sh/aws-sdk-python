...
class CodeSnippetResult(TypedDict, closed=True):
    ...
    code_snippet: NotRequired["aws_sdk_inspector2.types.code_line_list.CodeLineList"]
    """<p>Contains information on the retrieved code snippet.</p>"""
    suggested_fixes: NotRequired[
        "aws_sdk_inspector2.types.suggested_fixes.SuggestedFixes"
    ]
...
def serialize_json(value: CodeSnippetResult) -> dict:
    ...
    if "code_snippet" in value:
        ...
        out["codeSnippet"] = aws_sdk_inspector2.types.code_line_list.serialize_json(
            value["code_snippet"]
        )
    if "suggested_fixes" in value:
        import aws_sdk_inspector2.types.suggested_fixes

        out["suggestedFixes"] = aws_sdk_inspector2.types.suggested_fixes.serialize_json(
...
def deserialize_json(data: dict) -> CodeSnippetResult:
    ...
    if "codeSnippet" in data:
        ...
        out["code_snippet"] = aws_sdk_inspector2.types.code_line_list.deserialize_json(
            data["codeSnippet"]
        )
    if "suggestedFixes" in data:
        ...
        out["suggested_fixes"] = (
            aws_sdk_inspector2.types.suggested_fixes.deserialize_json(
                data["suggestedFixes"]
            )
        )
    return out
