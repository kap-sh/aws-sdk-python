"""Generated from Smithy shape ``com.amazonaws.inspector2#GetCisScanResultDetails``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.get_cis_scan_result_details_response.GetCisScanResultDetailsResponse = capo_inspector2.types.get_cis_scan_result_details_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/cis/scan-result/details/get"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.get_cis_scan_result_details_request.serialize_json(input_)
    ).encode()
...
async def async_get_cis_scan_result_details(
