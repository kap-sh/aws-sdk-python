"""Generated from Smithy shape ``com.amazonaws.inspector2#ListCisScanResultsAggregatedByChecks``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.list_cis_scan_results_aggregated_by_checks_response.ListCisScanResultsAggregatedByChecksResponse = capo_inspector2.types.list_cis_scan_results_aggregated_by_checks_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/cis/scan-result/check/list"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.list_cis_scan_results_aggregated_by_checks_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_cis_scan_results_aggregated_by_checks(
