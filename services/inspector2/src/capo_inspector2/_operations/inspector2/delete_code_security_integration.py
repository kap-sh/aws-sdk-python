"""Generated from Smithy shape ``com.amazonaws.inspector2#DeleteCodeSecurityIntegration``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.delete_code_security_integration_response.DeleteCodeSecurityIntegrationResponse = capo_inspector2.types.delete_code_security_integration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/codesecurity/integration/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.delete_code_security_integration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_code_security_integration(
