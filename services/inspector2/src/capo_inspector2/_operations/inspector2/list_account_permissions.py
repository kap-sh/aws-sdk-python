"""Generated from Smithy shape ``com.amazonaws.inspector2#ListAccountPermissions``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.list_account_permissions_response.ListAccountPermissionsResponse = capo_inspector2.types.list_account_permissions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accountpermissions/list"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.list_account_permissions_request.serialize_json(input_)
    ).encode()
...
async def async_list_account_permissions(
