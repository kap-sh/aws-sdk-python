"""Generated from Smithy shape ``com.amazonaws.inspector2#GetCodeSecurityScan``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.get_code_security_scan_response.GetCodeSecurityScanResponse = capo_inspector2.types.get_code_security_scan_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/codesecurity/scan/get"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.get_code_security_scan_request.serialize_json(input_)
    ).encode()
...
async def async_get_code_security_scan(
