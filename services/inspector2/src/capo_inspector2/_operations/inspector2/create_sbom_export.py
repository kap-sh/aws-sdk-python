"""Generated from Smithy shape ``com.amazonaws.inspector2#CreateSbomExport``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.create_sbom_export_response.CreateSbomExportResponse = (
        capo_inspector2.types.create_sbom_export_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sbomexport/create"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.create_sbom_export_request.serialize_json(input_)
    ).encode()
...
async def async_create_sbom_export(
