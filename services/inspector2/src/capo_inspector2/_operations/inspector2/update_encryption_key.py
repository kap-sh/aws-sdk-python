"""Generated from Smithy shape ``com.amazonaws.inspector2#UpdateEncryptionKey``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.update_encryption_key_response.UpdateEncryptionKeyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/encryptionkey/update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.update_encryption_key_request.serialize_json(input_)
    ).encode()
...
async def async_update_encryption_key(
