"""Generated from Smithy shape ``com.amazonaws.inspector2#UpdateEc2DeepInspectionConfiguration``."""
...
def handle_response(
    ...
    out: capo_inspector2.types.update_ec2_deep_inspection_configuration_response.UpdateEc2DeepInspectionConfigurationResponse = capo_inspector2.types.update_ec2_deep_inspection_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ec2deepinspectionconfiguration/update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_inspector2.types.update_ec2_deep_inspection_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_ec2_deep_inspection_configuration(
