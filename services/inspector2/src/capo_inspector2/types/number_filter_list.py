...
NumberFilterList: TypeAlias = list["capo_inspector2.types.number_filter.NumberFilter"]
...
def serialize_json(value: NumberFilterList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_inspector2.types.number_filter.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> NumberFilterList:
    ...
    for item in data:
        out.append(capo_inspector2.types.number_filter.deserialize_json(item))
    return out
