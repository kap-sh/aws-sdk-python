...
if TYPE_CHECKING:
    import capo_inspector2.types.cvss_score_details


class InspectorScoreDetails(TypedDict, closed=True):
    adjusted_cvss: NotRequired[
        "capo_inspector2.types.cvss_score_details.CvssScoreDetails"
    ]
...
def serialize_json(value: InspectorScoreDetails) -> dict:
    ...
    if "adjusted_cvss" in value:
        ...
        out["adjustedCvss"] = capo_inspector2.types.cvss_score_details.serialize_json(
            value["adjusted_cvss"]
        )
    return out
...
def deserialize_json(data: dict) -> InspectorScoreDetails:
    ...
    if "adjustedCvss" in data:
        ...
        out["adjusted_cvss"] = (
            capo_inspector2.types.cvss_score_details.deserialize_json(
                data["adjustedCvss"]
            )
        )
    return out
