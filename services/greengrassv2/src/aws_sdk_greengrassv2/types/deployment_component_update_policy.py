...
class DeploymentComponentUpdatePolicy(TypedDict):
    ...
    action: NotRequired[
        "aws_sdk_greengrassv2.types.deployment_component_update_policy_action.DeploymentComponentUpdatePolicyAction"
    ]
...
def serialize_json(value: DeploymentComponentUpdatePolicy) -> dict:
    ...
    if "action" in value:
        ...
        out["action"] = (
            aws_sdk_greengrassv2.types.deployment_component_update_policy_action.serialize_json(
                value["action"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> DeploymentComponentUpdatePolicy:
    ...
    if "action" in data:
        ...
        out["action"] = (
            aws_sdk_greengrassv2.types.deployment_component_update_policy_action.deserialize_json(
                data["action"]
            )
        )
    return out
