...
def serialize_json(value: CreateComponentVersionRequest) -> dict:
    ...
    if "inline_recipe" in value:
        ...
        out["inlineRecipe"] = aws_sdk_greengrassv2.types.recipe_blob.serialize_json(
            value["inline_recipe"]
        )
    if "lambda_function" in value:
        ...
        out["lambdaFunction"] = (
            aws_sdk_greengrassv2.types.lambda_function_recipe_source.serialize_json(
...
def deserialize_json(data: dict) -> CreateComponentVersionRequest:
    ...
    if "inlineRecipe" in data:
        ...
        out["inline_recipe"] = aws_sdk_greengrassv2.types.recipe_blob.deserialize_json(
            data["inlineRecipe"]
        )
    if "lambdaFunction" in data:
        ...
        out["lambda_function"] = (
            aws_sdk_greengrassv2.types.lambda_function_recipe_source.deserialize_json(
                data["lambdaFunction"]
            )
        )
    if "tags" in data:
