...
if TYPE_CHECKING:
    import aws_sdk_greengrassv2.types.lambda_event_source_type
    import aws_sdk_greengrassv2.types.topic_string


class LambdaEventSource(TypedDict):
    topic: "aws_sdk_greengrassv2.types.topic_string.TopicString"
    """<p>The topic to which to subscribe to receive event messages.</p>"""
    type: "aws_sdk_greengrassv2.types.lambda_event_source_type.LambdaEventSourceType"
...
def serialize_json(value: LambdaEventSource) -> dict:
    ...
    out["type"] = aws_sdk_greengrassv2.types.lambda_event_source_type.serialize_json(
        value["type"]
    )
    return out
...
def deserialize_json(data: dict) -> LambdaEventSource:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_greengrassv2.types.lambda_event_source_type.deserialize_json(
                data["type"]
            )
        )
    else:
