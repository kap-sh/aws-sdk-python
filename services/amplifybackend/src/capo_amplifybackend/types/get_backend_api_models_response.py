...
class GetBackendAPIModelsResponse(TypedDict, closed=True):
    models: NotRequired["capo_amplifybackend.types.__string.__string"]
    """<p>Stringified JSON of the datastore model.</p>"""
    status: NotRequired["capo_amplifybackend.types.status.Status"]
...
def serialize_json(value: GetBackendAPIModelsResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_amplifybackend.types.status.serialize_json(value["status"])
