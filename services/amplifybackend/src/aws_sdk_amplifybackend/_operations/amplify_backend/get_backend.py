"""Generated from Smithy shape ``com.amazonaws.amplifybackend#GetBackend``."""
...
def handle_response(
    ...
    out: aws_sdk_amplifybackend.types.get_backend_response.GetBackendResponse = (
        aws_sdk_amplifybackend.types.get_backend_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_amplifybackend.types.get_backend_request.GetBackendRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/backend/{AppId}/details"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_amplifybackend.types.get_backend_request.serialize_json(input_)
    ).encode()
