...
GroupSummaryList: TypeAlias = list[
    "aws_sdk_directory_service_data.types.group_summary.GroupSummary"
]
...
def serialize_json(value: GroupSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_directory_service_data.types.group_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> GroupSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_directory_service_data.types.group_summary.deserialize_json(item)
        )
    return out
