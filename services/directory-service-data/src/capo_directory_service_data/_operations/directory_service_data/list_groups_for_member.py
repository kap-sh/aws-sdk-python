"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#ListGroupsForMember``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.list_groups_for_member_result.ListGroupsForMemberResult = capo_directory_service_data.types.list_groups_for_member_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GroupMemberships/ListGroupsForMember"
    params: dict[str, str] = {}
    if "directory_id" in input_:
        params["DirectoryId"] = str(input_["directory_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.list_groups_for_member_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_groups_for_member(
