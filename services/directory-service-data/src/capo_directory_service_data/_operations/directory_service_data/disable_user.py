"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#DisableUser``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.disable_user_result.DisableUserResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Users/DisableUser"
    params: dict[str, str] = {}
    if "directory_id" in input_:
        params["DirectoryId"] = str(input_["directory_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.disable_user_request.serialize_json(input_)
    ).encode()
