"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#RemoveGroupMember``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.remove_group_member_result.RemoveGroupMemberResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GroupMemberships/RemoveGroupMember"
    params: dict[str, str] = {}
    if "directory_id" in input_:
        params["DirectoryId"] = str(input_["directory_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.remove_group_member_request.serialize_json(
            input_
        )
    ).encode()
