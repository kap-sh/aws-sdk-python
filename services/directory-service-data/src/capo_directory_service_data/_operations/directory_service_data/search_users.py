"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#SearchUsers``."""
...
def handle_response(
    ...
    out: capo_directory_service_data.types.search_users_result.SearchUsersResult = (
        capo_directory_service_data.types.search_users_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Users/SearchUsers"
    params: dict[str, str] = {}
    if "directory_id" in input_:
        params["DirectoryId"] = str(input_["directory_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.search_users_request.serialize_json(input_)
    ).encode()
