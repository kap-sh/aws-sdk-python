"""Generated from Smithy shape ``com.amazonaws.directoryservicedata#SearchGroups``."""
...
import capo_directory_service_data.types.group_list
...
def handle_response(
    ...
    out: capo_directory_service_data.types.search_groups_result.SearchGroupsResult = (
        capo_directory_service_data.types.search_groups_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Groups/SearchGroups"
    params: dict[str, str] = {}
    if "directory_id" in input_:
        params["DirectoryId"] = str(input_["directory_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_directory_service_data.types.search_groups_request.serialize_json(input_)
    ).encode()
