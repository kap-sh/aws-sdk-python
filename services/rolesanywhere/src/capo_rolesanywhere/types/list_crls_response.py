...
class ListCrlsResponse(TypedDict, closed=True):
    ...
    crls: NotRequired["capo_rolesanywhere.types.crl_details.CrlDetails"]
...
def serialize_json(value: ListCrlsResponse) -> dict:
    ...
    if "crls" in value:
        ...
        out["crls"] = capo_rolesanywhere.types.crl_details.serialize_json(value["crls"])
    return out
...
def deserialize_json(data: dict) -> ListCrlsResponse:
    ...
    if "crls" in data:
        ...
        out["crls"] = capo_rolesanywhere.types.crl_details.deserialize_json(
            data["crls"]
        )
    return out
