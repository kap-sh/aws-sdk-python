...
TrustAnchorDetails: TypeAlias = list[
    "aws_sdk_rolesanywhere.types.trust_anchor_detail.TrustAnchorDetail"
]
...
def serialize_json(value: TrustAnchorDetails) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_rolesanywhere.types.trust_anchor_detail.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> TrustAnchorDetails:
    ...
    for item in data:
        out.append(
            aws_sdk_rolesanywhere.types.trust_anchor_detail.deserialize_json(item)
        )
    return out
