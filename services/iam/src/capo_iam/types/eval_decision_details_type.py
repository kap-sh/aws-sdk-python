...
if TYPE_CHECKING:
    import capo_iam.types.eval_decision_source_type
    import capo_iam.types.policy_evaluation_decision_type

EvalDecisionDetailsType: TypeAlias = dict[
    "capo_iam.types.eval_decision_source_type.EvalDecisionSourceType",
...
def deserialize_query(el: Element) -> EvalDecisionDetailsType:
    ...
    for entry in el.findall("entry"):
        ...
        value = capo_iam.types.policy_evaluation_decision_type.deserialize_query(
            value_element
        )
        out[key] = value
    return out
