...
class SimulatePolicyResponse(TypedDict, closed=True):
    evaluation_results: NotRequired[
        "capo_iam.types.evaluation_results_list_type.EvaluationResultsListType"
    ]
...
def serialize_query(
    ...
    if "evaluation_results" in value:
        ...
        capo_iam.types.evaluation_results_list_type.serialize_query(
            value["evaluation_results"], pairs, f"{key_prefix}EvaluationResults"
        )
    pairs.append(
        (
...
def deserialize_query(el: Element) -> SimulatePolicyResponse:
    ...
    if child_evaluation_results is not None:
        ...
        out["evaluation_results"] = (
            capo_iam.types.evaluation_results_list_type.deserialize_query(
