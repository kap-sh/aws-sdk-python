...
if TYPE_CHECKING:
    ...
    import capo_iam.types.policy_list_type
    import capo_iam.types.response_marker_type


class ListPoliciesResponse(TypedDict, closed=True):
    policies: NotRequired["capo_iam.types.policy_list_type.policyListType"]
...
# --- awsQuery ser/de ---
def serialize_query(
    value: ListPoliciesResponse, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "policies" in value:
        import capo_iam.types.policy_list_type

        capo_iam.types.policy_list_type.serialize_query(
...
def deserialize_query(el: Element) -> ListPoliciesResponse:
    out: ListPoliciesResponse = {}  # type: ignore[typeddict-item]
    child_policies = el.find("Policies")
    if child_policies is not None:
        import capo_iam.types.policy_list_type

        out["policies"] = capo_iam.types.policy_list_type.deserialize_query(
            child_policies
        )
