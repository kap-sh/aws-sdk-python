...
if TYPE_CHECKING:
    import capo_iam.types.managed_policy_detail

ManagedPolicyDetailListType: TypeAlias = list[
    "capo_iam.types.managed_policy_detail.ManagedPolicyDetail"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_iam.types.managed_policy_detail.serialize_query(
...
def deserialize_query(el: Element) -> ManagedPolicyDetailListType:
    ...
    for child in el.findall("member"):
        out.append(capo_iam.types.managed_policy_detail.deserialize_query(child))
    return out
