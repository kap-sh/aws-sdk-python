...
if TYPE_CHECKING:
    import capo_iam.types.unrecognized_public_key_encoding_message


class UnrecognizedPublicKeyEncodingException_(TypedDict, closed=True):
    message: NotRequired[
        "capo_iam.types.unrecognized_public_key_encoding_message.unrecognizedPublicKeyEncodingMessage"
    ]
...
class UnrecognizedPublicKeyEncodingException(ServiceError):
    ...
    def __init__(
        self, data: UnrecognizedPublicKeyEncodingException_, message: str | None = None
    ):
        super().__init__(
            "client",
