...
import capo_iam.errors.key_pair_mismatch_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "KeyPairMismatch":
            raise capo_iam.errors.key_pair_mismatch_exception.KeyPairMismatchException.from_query(
                error_el, message
            )
        case "LimitExceeded":
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("UploadServerCertificateResult")
    out: capo_iam.types.upload_server_certificate_response.UploadServerCertificateResponse = capo_iam.types.upload_server_certificate_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "UploadServerCertificate"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.upload_server_certificate_request.serialize_query(input_, pairs, "")
