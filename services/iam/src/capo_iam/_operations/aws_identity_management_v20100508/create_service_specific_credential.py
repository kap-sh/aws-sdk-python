...
import capo_iam.errors.service_not_supported_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "NotSupportedService":
            raise capo_iam.errors.service_not_supported_exception.ServiceNotSupportedException.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateServiceSpecificCredentialResult")
    out: capo_iam.types.create_service_specific_credential_response.CreateServiceSpecificCredentialResponse = capo_iam.types.create_service_specific_credential_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateServiceSpecificCredential"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.create_service_specific_credential_request.serialize_query(
        input_, pairs, ""
    )
