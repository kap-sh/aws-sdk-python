...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GetRoleResult")
    out: capo_iam.types.get_role_response.GetRoleResponse = (
        capo_iam.types.get_role_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "GetRole"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.get_role_request.serialize_query(input_, pairs, "")
...
def get_role(
    options: OperationOptions, input_: capo_iam.types.get_role_request.GetRoleRequest
) -> tuple[capo_iam.types.get_role_response.GetRoleResponse, zapros.Response]:
