...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListGroupsResult")
    out: capo_iam.types.list_groups_response.ListGroupsResponse = (
        capo_iam.types.list_groups_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListGroups"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.list_groups_request.serialize_query(input_, pairs, "")
...
def list_groups(
    ...
    input_: capo_iam.types.list_groups_request.ListGroupsRequest,
) -> tuple[capo_iam.types.list_groups_response.ListGroupsResponse, zapros.Response]:
