...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ReportExpired":
            raise capo_iam.errors.credential_report_expired_exception.CredentialReportExpiredException.from_query(
                error_el, message
            )
        case "ReportNotPresent":
            raise capo_iam.errors.credential_report_not_present_exception.CredentialReportNotPresentException.from_query(
                error_el, message
            )
        case "ReportInProgress":
            raise capo_iam.errors.credential_report_not_ready_exception.CredentialReportNotReadyException.from_query(
                error_el, message
            )
        case "ServiceFailure":
...
def handle_response(
    ...
    out: capo_iam.types.get_credential_report_response.GetCredentialReportResponse = (
        capo_iam.types.get_credential_report_response.deserialize_query(
...
def build_request(options: OperationOptions | AsyncOperationOptions) -> zapros.Request:
    ...
    pairs.append(("Action", "GetCredentialReport"))
