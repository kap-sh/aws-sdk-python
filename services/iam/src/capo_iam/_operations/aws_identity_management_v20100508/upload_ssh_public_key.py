...
import capo_iam.errors.duplicate_ssh_public_key_exception
import capo_iam.errors.invalid_public_key_exception
import capo_iam.errors.limit_exceeded_exception
import capo_iam.errors.no_such_entity_exception
import capo_iam.errors.unrecognized_public_key_encoding_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DuplicateSSHPublicKey":
            raise capo_iam.errors.duplicate_ssh_public_key_exception.DuplicateSSHPublicKeyException.from_query(
                error_el, message
            )
        case "InvalidPublicKey":
            raise capo_iam.errors.invalid_public_key_exception.InvalidPublicKeyException.from_query(
                error_el, message
            )
        case "LimitExceeded":
        ...
        case "UnrecognizedPublicKeyEncoding":
            raise capo_iam.errors.unrecognized_public_key_encoding_exception.UnrecognizedPublicKeyEncodingException.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("UploadSSHPublicKeyResult")
    out: capo_iam.types.upload_ssh_public_key_response.UploadSSHPublicKeyResponse = (
        capo_iam.types.upload_ssh_public_key_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "UploadSSHPublicKey"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.upload_ssh_public_key_request.serialize_query(input_, pairs, "")
