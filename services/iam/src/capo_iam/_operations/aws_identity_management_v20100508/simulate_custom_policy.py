...
import capo_iam.errors.policy_evaluation_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PolicyEvaluationException":
            raise capo_iam.errors.policy_evaluation_exception.PolicyEvaluationException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("SimulateCustomPolicyResult")
    out: capo_iam.types.simulate_policy_response.SimulatePolicyResponse = (
        capo_iam.types.simulate_policy_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "SimulateCustomPolicy"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.simulate_custom_policy_request.serialize_query(input_, pairs, "")
...
def simulate_custom_policy(
    ...
    capo_iam.types.simulate_policy_response.SimulatePolicyResponse, zapros.Response
]:
