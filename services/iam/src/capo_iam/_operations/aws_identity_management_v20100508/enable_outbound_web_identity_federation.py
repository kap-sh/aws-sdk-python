...
import capo_iam.errors.feature_enabled_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "FeatureEnabledException":
            raise capo_iam.errors.feature_enabled_exception.FeatureEnabledException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("EnableOutboundWebIdentityFederationResult")
    out: capo_iam.types.enable_outbound_web_identity_federation_response.EnableOutboundWebIdentityFederationResponse = capo_iam.types.enable_outbound_web_identity_federation_response.deserialize_query(
...
def build_request(options: OperationOptions | AsyncOperationOptions) -> zapros.Request:
    ...
    pairs.append(("Action", "EnableOutboundWebIdentityFederation"))
