...
import capo_iam.errors.entity_already_exists_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "EntityAlreadyExists":
            raise capo_iam.errors.entity_already_exists_exception.EntityAlreadyExistsException.from_query(
                error_el, message
            )
        case "LimitExceeded":
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateGroupResult")
    out: capo_iam.types.create_group_response.CreateGroupResponse = (
        capo_iam.types.create_group_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateGroup"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.create_group_request.serialize_query(input_, pairs, "")
...
def create_group(
    ...
    input_: capo_iam.types.create_group_request.CreateGroupRequest,
) -> tuple[capo_iam.types.create_group_response.CreateGroupResponse, zapros.Response]:
