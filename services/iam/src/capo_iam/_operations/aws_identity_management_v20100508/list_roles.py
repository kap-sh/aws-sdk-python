...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListRolesResult")
    out: capo_iam.types.list_roles_response.ListRolesResponse = (
        capo_iam.types.list_roles_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListRoles"))
    pairs.append(("Version", "2010-05-08"))
    capo_iam.types.list_roles_request.serialize_query(input_, pairs, "")
...
def list_roles(
    ...
    input_: capo_iam.types.list_roles_request.ListRolesRequest,
) -> tuple[capo_iam.types.list_roles_response.ListRolesResponse, zapros.Response]:
