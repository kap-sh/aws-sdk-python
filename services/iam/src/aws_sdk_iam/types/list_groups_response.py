"""Generated from Smithy shape ``com.amazonaws.iam#ListGroupsResponse``."""
...
if TYPE_CHECKING:
    ...
    import aws_sdk_iam.types.group_list_type
    import aws_sdk_iam.types.response_marker_type


class ListGroupsResponse(TypedDict, closed=True):
    groups: "aws_sdk_iam.types.group_list_type.groupListType"
...
# --- awsQuery ser/de ---
def serialize_query(
    value: ListGroupsResponse, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_iam.types.group_list_type

    aws_sdk_iam.types.group_list_type.serialize_query(
...
def deserialize_query(el: Element) -> ListGroupsResponse:
    out: ListGroupsResponse = {}  # type: ignore[typeddict-item]
    child_groups = el.find("Groups")
    if child_groups is not None:
        import aws_sdk_iam.types.group_list_type

        out["groups"] = aws_sdk_iam.types.group_list_type.deserialize_query(
            child_groups
        )
    else:
