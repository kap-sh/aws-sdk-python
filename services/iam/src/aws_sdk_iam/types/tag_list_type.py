...
tagListType: TypeAlias = list["aws_sdk_iam.types.tag.Tag"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_iam.types.tag.serialize_query(item, pairs, f"{prefix}.member.{n}")
...
def deserialize_query(el: Element) -> tagListType:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_iam.types.tag.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_iam.types.tag.serialize_query(item, pairs, f"{prefix}.{n}")
