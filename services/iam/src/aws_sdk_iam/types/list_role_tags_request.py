"""Generated from Smithy shape ``com.amazonaws.iam#ListRoleTagsRequest``."""
...
# --- awsQuery ser/de ---
def serialize_query(
    value: ListRoleTagsRequest, pairs: list[tuple[str, str]], prefix: str
) -> None:
...
def deserialize_query(el: Element) -> ListRoleTagsRequest:
    out: ListRoleTagsRequest = {}  # type: ignore[typeddict-item]
    child_role_name = el.find("RoleName")
    if child_role_name is not None:
        out["role_name"] = str(child_role_name.text or "")
    else:
        raise DeserializationError("ListRoleTagsRequest.role_name required")
