"""Generated from Smithy shape ``com.amazonaws.iam#ListAccessKeysResponse``."""
...
if TYPE_CHECKING:
    import aws_sdk_iam.types.access_key_metadata_list_type
    import aws_sdk_iam.types.boolean_type
    import aws_sdk_iam.types.response_marker_type


class ListAccessKeysResponse(TypedDict, closed=True):
    access_key_metadata: (
        "aws_sdk_iam.types.access_key_metadata_list_type.accessKeyMetadataListType"
    )
...
def serialize_query(
    ...
    aws_sdk_iam.types.access_key_metadata_list_type.serialize_query(
...
def deserialize_query(el: Element) -> ListAccessKeysResponse:
    out: ListAccessKeysResponse = {}  # type: ignore[typeddict-item]
    child_access_key_metadata = el.find("AccessKeyMetadata")
    if child_access_key_metadata is not None:
        import aws_sdk_iam.types.access_key_metadata_list_type

        out["access_key_metadata"] = (
            aws_sdk_iam.types.access_key_metadata_list_type.deserialize_query(
                child_access_key_metadata
            )
        )
    else:
        raise DeserializationError(
            "ListAccessKeysResponse.access_key_metadata required"
        )
