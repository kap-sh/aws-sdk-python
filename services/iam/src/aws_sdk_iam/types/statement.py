...
if TYPE_CHECKING:
    import aws_sdk_iam.types.policy_identifier_type
    import aws_sdk_iam.types.policy_source_type
    import aws_sdk_iam.types.position


class Statement(TypedDict, closed=True):
    source_policy_id: NotRequired[
        "aws_sdk_iam.types.policy_identifier_type.PolicyIdentifierType"
    ]
    """<p>The identifier of the policy that was provided as an input.</p>"""
    source_policy_type: NotRequired[
        "aws_sdk_iam.types.policy_source_type.PolicySourceType"
    ]
    """<p>The type of the policy.</p>"""
    start_position: NotRequired["aws_sdk_iam.types.position.Position"]
    """<p>The row and column of the beginning of the <code>Statement</code> in an IAM policy.</p>"""
    end_position: NotRequired["aws_sdk_iam.types.position.Position"]
...
def serialize_query(
    ...
    if "source_policy_type" in value:
        ...
        aws_sdk_iam.types.policy_source_type.serialize_query(
            value["source_policy_type"], pairs, f"{prefix}.SourcePolicyType"
        )
    if "start_position" in value:
        import aws_sdk_iam.types.position

        aws_sdk_iam.types.position.serialize_query(
...
def deserialize_query(el: Element) -> Statement:
    ...
    if child_source_policy_type is not None:
        ...
        out["source_policy_type"] = (
            aws_sdk_iam.types.policy_source_type.deserialize_query(
    ...
    if child_start_position is not None:
        ...
        out["start_position"] = aws_sdk_iam.types.position.deserialize_query(
    ...
    if child_end_position is not None:
        ...
        out["end_position"] = aws_sdk_iam.types.position.deserialize_query(
            child_end_position
        )
    return out
