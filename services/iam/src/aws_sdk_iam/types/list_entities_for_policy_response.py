...
if TYPE_CHECKING:
    ...
    import aws_sdk_iam.types.policy_group_list_type
    import aws_sdk_iam.types.policy_role_list_type
    import aws_sdk_iam.types.policy_user_list_type
    import aws_sdk_iam.types.response_marker_type


class ListEntitiesForPolicyResponse(TypedDict):
    policy_groups: NotRequired[
        "aws_sdk_iam.types.policy_group_list_type.PolicyGroupListType"
    ]
    """<p>A list of IAM groups that the policy is attached to.</p>"""
    policy_users: NotRequired[
        "aws_sdk_iam.types.policy_user_list_type.PolicyUserListType"
    ]
    """<p>A list of IAM users that the policy is attached to.</p>"""
    policy_roles: NotRequired[
        "aws_sdk_iam.types.policy_role_list_type.PolicyRoleListType"
    ]
...
def serialize_query(
    ...
    if "policy_groups" in value:
        ...
        aws_sdk_iam.types.policy_group_list_type.serialize_query(
            value["policy_groups"], pairs, f"{prefix}.PolicyGroups"
        )
    if "policy_users" in value:
        import aws_sdk_iam.types.policy_user_list_type

        aws_sdk_iam.types.policy_user_list_type.serialize_query(
            value["policy_users"], pairs, f"{prefix}.PolicyUsers"
        )
    if "policy_roles" in value:
        import aws_sdk_iam.types.policy_role_list_type

        aws_sdk_iam.types.policy_role_list_type.serialize_query(
...
def deserialize_query(el: Element) -> ListEntitiesForPolicyResponse:
    ...
    if child_policy_groups is not None:
        ...
        out["policy_groups"] = (
            aws_sdk_iam.types.policy_group_list_type.deserialize_query(
                child_policy_groups
            )
        )
    ...
    if child_policy_users is not None:
        ...
        out["policy_users"] = aws_sdk_iam.types.policy_user_list_type.deserialize_query(
            child_policy_users
        )
    ...
    if child_policy_roles is not None:
        ...
        out["policy_roles"] = aws_sdk_iam.types.policy_role_list_type.deserialize_query(
            child_policy_roles
        )
