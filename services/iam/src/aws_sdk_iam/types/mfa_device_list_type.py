...
mfaDeviceListType: TypeAlias = list["aws_sdk_iam.types.mfa_device.MFADevice"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_iam.types.mfa_device.serialize_query(
...
def deserialize_query(el: Element) -> mfaDeviceListType:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_iam.types.mfa_device.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_iam.types.mfa_device.serialize_query(item, pairs, f"{prefix}.{n}")
