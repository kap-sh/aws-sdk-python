...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GetContextKeysForPrincipalPolicyResult")
...
def build_request(
    ...
    input_: aws_sdk_iam.types.get_context_keys_for_principal_policy_request.GetContextKeysForPrincipalPolicyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "GetContextKeysForPrincipalPolicy"))
    pairs.append(("Version", "2010-05-08"))
    import aws_sdk_iam.types.get_context_keys_for_principal_policy_request

    aws_sdk_iam.types.get_context_keys_for_principal_policy_request.serialize_query(
        input_, pairs, ""
    )
