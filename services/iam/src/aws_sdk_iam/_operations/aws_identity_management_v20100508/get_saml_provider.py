...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GetSAMLProviderResult")
    out: aws_sdk_iam.types.get_saml_provider_response.GetSAMLProviderResponse = (
        aws_sdk_iam.types.get_saml_provider_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.get_saml_provider_request.GetSAMLProviderRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "GetSAMLProvider"))
    pairs.append(("Version", "2010-05-08"))
    aws_sdk_iam.types.get_saml_provider_request.serialize_query(input_, pairs, "")
