...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GetAccessKeyLastUsedResult")
    out: aws_sdk_iam.types.get_access_key_last_used_response.GetAccessKeyLastUsedResponse = aws_sdk_iam.types.get_access_key_last_used_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.get_access_key_last_used_request.GetAccessKeyLastUsedRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "GetAccessKeyLastUsed"))
    pairs.append(("Version", "2010-05-08"))
    aws_sdk_iam.types.get_access_key_last_used_request.serialize_query(
        input_, pairs, ""
    )
