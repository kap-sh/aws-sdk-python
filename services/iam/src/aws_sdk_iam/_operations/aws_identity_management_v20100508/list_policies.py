...
import aws_sdk_iam.types.policy_list_type
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListPoliciesResult")
    out: aws_sdk_iam.types.list_policies_response.ListPoliciesResponse = (
        aws_sdk_iam.types.list_policies_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.list_policies_request.ListPoliciesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "ListPolicies"))
    pairs.append(("Version", "2010-05-08"))
    aws_sdk_iam.types.list_policies_request.serialize_query(input_, pairs, "")
