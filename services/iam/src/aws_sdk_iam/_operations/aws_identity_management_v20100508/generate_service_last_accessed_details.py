...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GenerateServiceLastAccessedDetailsResult")
    out: aws_sdk_iam.types.generate_service_last_accessed_details_response.GenerateServiceLastAccessedDetailsResponse = aws_sdk_iam.types.generate_service_last_accessed_details_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.generate_service_last_accessed_details_request.GenerateServiceLastAccessedDetailsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "GenerateServiceLastAccessedDetails"))
    pairs.append(("Version", "2010-05-08"))
    import aws_sdk_iam.types.generate_service_last_accessed_details_request

    aws_sdk_iam.types.generate_service_last_accessed_details_request.serialize_query(
        input_, pairs, ""
    )
