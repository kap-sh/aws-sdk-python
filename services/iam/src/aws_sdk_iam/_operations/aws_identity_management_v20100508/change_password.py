...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "EntityTemporarilyUnmodifiableException":
            import aws_sdk_iam.errors.entity_temporarily_unmodifiable_exception

            raise aws_sdk_iam.errors.entity_temporarily_unmodifiable_exception.EntityTemporarilyUnmodifiableException.from_query(
                root
            )
        case "InvalidUserTypeException":
            import aws_sdk_iam.errors.invalid_user_type_exception

            raise aws_sdk_iam.errors.invalid_user_type_exception.InvalidUserTypeException.from_query(
                root
            )
        ...
        case "PasswordPolicyViolationException":
            import aws_sdk_iam.errors.password_policy_violation_exception

            raise aws_sdk_iam.errors.password_policy_violation_exception.PasswordPolicyViolationException.from_query(
                root
            )
...
def build_request(
    ...
    input_: aws_sdk_iam.types.change_password_request.ChangePasswordRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    aws_sdk_iam.types.change_password_request.serialize_query(input_, pairs, "")
