...
import aws_sdk_iam.errors.malformed_policy_document_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "MalformedPolicyDocumentException":
            raise aws_sdk_iam.errors.malformed_policy_document_exception.MalformedPolicyDocumentException.from_query(
                root
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateRoleResult")
    out: aws_sdk_iam.types.create_role_response.CreateRoleResponse = (
        aws_sdk_iam.types.create_role_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.create_role_request.CreateRoleRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "CreateRole"))
    pairs.append(("Version", "2010-05-08"))
    import aws_sdk_iam.types.create_role_request

    aws_sdk_iam.types.create_role_request.serialize_query(input_, pairs, "")
