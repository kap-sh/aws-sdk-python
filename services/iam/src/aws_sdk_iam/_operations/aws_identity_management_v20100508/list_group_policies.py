...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListGroupPoliciesResult")
    out: aws_sdk_iam.types.list_group_policies_response.ListGroupPoliciesResponse = (
        aws_sdk_iam.types.list_group_policies_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.list_group_policies_request.ListGroupPoliciesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "ListGroupPolicies"))
    pairs.append(("Version", "2010-05-08"))
    aws_sdk_iam.types.list_group_policies_request.serialize_query(input_, pairs, "")
