...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ReportGenerationLimitExceededException":
            import aws_sdk_iam.errors.report_generation_limit_exceeded_exception

            raise aws_sdk_iam.errors.report_generation_limit_exceeded_exception.ReportGenerationLimitExceededException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GenerateOrganizationsAccessReportResult")
    out: aws_sdk_iam.types.generate_organizations_access_report_response.GenerateOrganizationsAccessReportResponse = aws_sdk_iam.types.generate_organizations_access_report_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.generate_organizations_access_report_request.GenerateOrganizationsAccessReportRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "GenerateOrganizationsAccessReport"))
    pairs.append(("Version", "2010-05-08"))
    import aws_sdk_iam.types.generate_organizations_access_report_request

    aws_sdk_iam.types.generate_organizations_access_report_request.serialize_query(
        input_, pairs, ""
    )
