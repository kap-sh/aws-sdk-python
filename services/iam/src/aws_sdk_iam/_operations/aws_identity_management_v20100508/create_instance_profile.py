...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateInstanceProfileResult")
    out: aws_sdk_iam.types.create_instance_profile_response.CreateInstanceProfileResponse = aws_sdk_iam.types.create_instance_profile_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.create_instance_profile_request.CreateInstanceProfileRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "CreateInstanceProfile"))
    pairs.append(("Version", "2010-05-08"))
    import aws_sdk_iam.types.create_instance_profile_request

    aws_sdk_iam.types.create_instance_profile_request.serialize_query(input_, pairs, "")
