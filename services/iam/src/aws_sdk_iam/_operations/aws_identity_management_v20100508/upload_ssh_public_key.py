...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DuplicateSSHPublicKeyException":
            import aws_sdk_iam.errors.duplicate_ssh_public_key_exception

            raise aws_sdk_iam.errors.duplicate_ssh_public_key_exception.DuplicateSSHPublicKeyException.from_query(
                root
            )
        case "InvalidPublicKeyException":
            import aws_sdk_iam.errors.invalid_public_key_exception

            raise aws_sdk_iam.errors.invalid_public_key_exception.InvalidPublicKeyException.from_query(
                root
            )
        ...
        case "UnrecognizedPublicKeyEncodingException":
            import aws_sdk_iam.errors.unrecognized_public_key_encoding_exception

            raise aws_sdk_iam.errors.unrecognized_public_key_encoding_exception.UnrecognizedPublicKeyEncodingException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_iam.types.upload_ssh_public_key_response.UploadSSHPublicKeyResponse = (
        aws_sdk_iam.types.upload_ssh_public_key_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.upload_ssh_public_key_request.UploadSSHPublicKeyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    aws_sdk_iam.types.upload_ssh_public_key_request.serialize_query(input_, pairs, "")
