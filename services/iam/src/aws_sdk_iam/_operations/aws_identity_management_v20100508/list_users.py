...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ListUsersResult")
    out: aws_sdk_iam.types.list_users_response.ListUsersResponse = (
        aws_sdk_iam.types.list_users_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.list_users_request.ListUsersRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "ListUsers"))
    pairs.append(("Version", "2010-05-08"))
    aws_sdk_iam.types.list_users_request.serialize_query(input_, pairs, "")
