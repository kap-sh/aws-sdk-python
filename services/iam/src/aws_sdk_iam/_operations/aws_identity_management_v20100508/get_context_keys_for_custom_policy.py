...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GetContextKeysForCustomPolicyResult")
    out: aws_sdk_iam.types.get_context_keys_for_policy_response.GetContextKeysForPolicyResponse = aws_sdk_iam.types.get_context_keys_for_policy_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.get_context_keys_for_custom_policy_request.GetContextKeysForCustomPolicyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
