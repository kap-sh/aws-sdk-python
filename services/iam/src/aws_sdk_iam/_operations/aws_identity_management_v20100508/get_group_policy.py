...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("GetGroupPolicyResult")
    out: aws_sdk_iam.types.get_group_policy_response.GetGroupPolicyResponse = (
        aws_sdk_iam.types.get_group_policy_response.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_iam.types.get_group_policy_request.GetGroupPolicyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "GetGroupPolicy"))
    pairs.append(("Version", "2010-05-08"))
    aws_sdk_iam.types.get_group_policy_request.serialize_query(input_, pairs, "")
