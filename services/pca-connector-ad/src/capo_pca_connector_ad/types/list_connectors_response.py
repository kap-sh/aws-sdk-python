...
class ListConnectorsResponse(TypedDict, closed=True):
    connectors: NotRequired["capo_pca_connector_ad.types.connector_list.ConnectorList"]
...
def serialize_json(value: ListConnectorsResponse) -> dict:
    ...
    if "connectors" in value:
        ...
        out["Connectors"] = capo_pca_connector_ad.types.connector_list.serialize_json(
            value["connectors"]
        )
