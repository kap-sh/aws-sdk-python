"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListStackInstances``."""
...
import capo_cloudformation.types.list_stack_instances_input
import capo_cloudformation.types.list_stack_instances_output
...
) -> capo_cloudformation.types.list_stack_instances_output.ListStackInstancesOutput:
    root = fromstring(response.read())
    result = root.find("ListStackInstancesResult")
    out: capo_cloudformation.types.list_stack_instances_output.ListStackInstancesOutput = capo_cloudformation.types.list_stack_instances_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListStackInstances"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_stack_instances_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_list_stack_instances(
