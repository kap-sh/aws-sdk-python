"""Generated from Smithy shape ``com.amazonaws.cloudformation#CreateStack``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateStackResult")
    out: capo_cloudformation.types.create_stack_output.CreateStackOutput = (
        capo_cloudformation.types.create_stack_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateStack"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.create_stack_input.serialize_query(input_, pairs, "")
...
def create_stack(
    ...
    capo_cloudformation.types.create_stack_output.CreateStackOutput, zapros.Response
]:
