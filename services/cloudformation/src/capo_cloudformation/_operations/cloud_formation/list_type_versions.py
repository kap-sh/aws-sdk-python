"""Generated from Smithy shape ``com.amazonaws.cloudformation#ListTypeVersions``."""
...
import capo_cloudformation.types.list_type_versions_input
import capo_cloudformation.types.list_type_versions_output
...
) -> capo_cloudformation.types.list_type_versions_output.ListTypeVersionsOutput:
    root = fromstring(response.read())
    result = root.find("ListTypeVersionsResult")
    out: capo_cloudformation.types.list_type_versions_output.ListTypeVersionsOutput = (
        capo_cloudformation.types.list_type_versions_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ListTypeVersions"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.list_type_versions_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_list_type_versions(
