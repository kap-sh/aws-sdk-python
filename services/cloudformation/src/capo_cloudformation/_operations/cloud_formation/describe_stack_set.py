"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackSet``."""
...
import capo_cloudformation.types.describe_stack_set_input
import capo_cloudformation.types.describe_stack_set_output
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeStackSetResult")
    out: capo_cloudformation.types.describe_stack_set_output.DescribeStackSetOutput = (
        capo_cloudformation.types.describe_stack_set_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStackSet"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stack_set_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_set(
