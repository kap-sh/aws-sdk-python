"""Generated from Smithy shape ``com.amazonaws.cloudformation#DetectStackResourceDrift``."""
...
import capo_cloudformation.types.detect_stack_resource_drift_input
import capo_cloudformation.types.detect_stack_resource_drift_output
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DetectStackResourceDriftResult")
    out: capo_cloudformation.types.detect_stack_resource_drift_output.DetectStackResourceDriftOutput = capo_cloudformation.types.detect_stack_resource_drift_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DetectStackResourceDrift"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.detect_stack_resource_drift_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_detect_stack_resource_drift(
