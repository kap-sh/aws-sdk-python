"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStacks``."""
...
import capo_cloudformation.types.describe_stacks_input
import capo_cloudformation.types.describe_stacks_output
...
) -> capo_cloudformation.types.describe_stacks_output.DescribeStacksOutput:
    root = fromstring(response.read())
    result = root.find("DescribeStacksResult")
    out: capo_cloudformation.types.describe_stacks_output.DescribeStacksOutput = (
        capo_cloudformation.types.describe_stacks_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStacks"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stacks_input.serialize_query(input_, pairs, "")
...
async def async_describe_stacks(
