"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeTypeRegistration``."""
...
import capo_cloudformation.types.describe_type_registration_input
import capo_cloudformation.types.describe_type_registration_output
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeTypeRegistrationResult")
    out: capo_cloudformation.types.describe_type_registration_output.DescribeTypeRegistrationOutput = capo_cloudformation.types.describe_type_registration_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeTypeRegistration"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_type_registration_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_type_registration(
