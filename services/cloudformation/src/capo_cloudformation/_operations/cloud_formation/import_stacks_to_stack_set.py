...
import capo_cloudformation.errors.stack_not_found_exception
...
import capo_cloudformation.types.import_stacks_to_stack_set_input
import capo_cloudformation.types.import_stacks_to_stack_set_output
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "StackNotFoundException":
            raise capo_cloudformation.errors.stack_not_found_exception.StackNotFoundException.from_query(
                root
            )
...
) -> capo_cloudformation.types.import_stacks_to_stack_set_output.ImportStacksToStackSetOutput:
    root = fromstring(response.read())
    result = root.find("ImportStacksToStackSetResult")
    out: capo_cloudformation.types.import_stacks_to_stack_set_output.ImportStacksToStackSetOutput = capo_cloudformation.types.import_stacks_to_stack_set_output.deserialize_query(
...
def build_request(
    ...
    capo_cloudformation.types.import_stacks_to_stack_set_input.serialize_query(
        input_, pairs, ""
    )
