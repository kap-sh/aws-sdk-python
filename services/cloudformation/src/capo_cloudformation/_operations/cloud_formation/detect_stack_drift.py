"""Generated from Smithy shape ``com.amazonaws.cloudformation#DetectStackDrift``."""
...
import capo_cloudformation.types.detect_stack_drift_input
import capo_cloudformation.types.detect_stack_drift_output
...
) -> capo_cloudformation.types.detect_stack_drift_output.DetectStackDriftOutput:
    root = fromstring(response.read())
    result = root.find("DetectStackDriftResult")
    out: capo_cloudformation.types.detect_stack_drift_output.DetectStackDriftOutput = (
        capo_cloudformation.types.detect_stack_drift_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DetectStackDrift"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.detect_stack_drift_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_detect_stack_drift(
