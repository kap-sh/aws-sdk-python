"""Generated from Smithy shape ``com.amazonaws.cloudformation#RollbackStack``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "TokenAlreadyExistsException":
            raise capo_cloudformation.errors.token_already_exists_exception.TokenAlreadyExistsException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("RollbackStackResult")
    out: capo_cloudformation.types.rollback_stack_output.RollbackStackOutput = (
        capo_cloudformation.types.rollback_stack_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "RollbackStack"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.rollback_stack_input.serialize_query(input_, pairs, "")
...
def rollback_stack(
    ...
    capo_cloudformation.types.rollback_stack_output.RollbackStackOutput, zapros.Response
]:
...
async def async_rollback_stack(
