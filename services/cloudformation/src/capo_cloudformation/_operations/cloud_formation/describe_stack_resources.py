"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackResources``."""
...
import capo_cloudformation.types.describe_stack_resources_input
import capo_cloudformation.types.describe_stack_resources_output
import capo_cloudformation.types.stack_resources
...
) -> capo_cloudformation.types.describe_stack_resources_output.DescribeStackResourcesOutput:
    root = fromstring(response.read())
    result = root.find("DescribeStackResourcesResult")
    out: capo_cloudformation.types.describe_stack_resources_output.DescribeStackResourcesOutput = capo_cloudformation.types.describe_stack_resources_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStackResources"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stack_resources_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_resources(
