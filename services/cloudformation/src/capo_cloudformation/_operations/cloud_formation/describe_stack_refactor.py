"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackRefactor``."""
...
import capo_cloudformation.errors.stack_refactor_not_found_exception
import capo_cloudformation.types.describe_stack_refactor_input
import capo_cloudformation.types.describe_stack_refactor_output
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "StackRefactorNotFoundException":
            raise capo_cloudformation.errors.stack_refactor_not_found_exception.StackRefactorNotFoundException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeStackRefactorResult")
    out: capo_cloudformation.types.describe_stack_refactor_output.DescribeStackRefactorOutput = capo_cloudformation.types.describe_stack_refactor_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStackRefactor"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stack_refactor_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_refactor(
