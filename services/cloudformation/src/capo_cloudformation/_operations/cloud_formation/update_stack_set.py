"""Generated from Smithy shape ``com.amazonaws.cloudformation#UpdateStackSet``."""
...
import capo_cloudformation.types.update_stack_set_input
import capo_cloudformation.types.update_stack_set_output
...
) -> capo_cloudformation.types.update_stack_set_output.UpdateStackSetOutput:
    root = fromstring(response.read())
    result = root.find("UpdateStackSetResult")
    out: capo_cloudformation.types.update_stack_set_output.UpdateStackSetOutput = (
        capo_cloudformation.types.update_stack_set_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "UpdateStackSet"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.update_stack_set_input.serialize_query(input_, pairs, "")
...
async def async_update_stack_set(
