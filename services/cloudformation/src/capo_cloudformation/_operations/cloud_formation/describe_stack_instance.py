"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackInstance``."""
...
import capo_cloudformation.types.describe_stack_instance_input
import capo_cloudformation.types.describe_stack_instance_output
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeStackInstanceResult")
    out: capo_cloudformation.types.describe_stack_instance_output.DescribeStackInstanceOutput = capo_cloudformation.types.describe_stack_instance_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStackInstance"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stack_instance_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_instance(
