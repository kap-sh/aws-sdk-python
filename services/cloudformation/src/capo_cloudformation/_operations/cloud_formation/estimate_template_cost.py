"""Generated from Smithy shape ``com.amazonaws.cloudformation#EstimateTemplateCost``."""
...
import capo_cloudformation.types.estimate_template_cost_output
...
) -> capo_cloudformation.types.estimate_template_cost_output.EstimateTemplateCostOutput:
    root = fromstring(response.read())
    result = root.find("EstimateTemplateCostResult")
    out: capo_cloudformation.types.estimate_template_cost_output.EstimateTemplateCostOutput = capo_cloudformation.types.estimate_template_cost_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "EstimateTemplateCost"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.estimate_template_cost_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_estimate_template_cost(
