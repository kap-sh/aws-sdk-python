"""Generated from Smithy shape ``com.amazonaws.cloudformation#DescribeStackResourceDrifts``."""
...
import capo_cloudformation.types.describe_stack_resource_drifts_input
import capo_cloudformation.types.describe_stack_resource_drifts_output
import capo_cloudformation.types.stack_resource_drift_status_filters
import capo_cloudformation.types.stack_resource_drifts
...
) -> capo_cloudformation.types.describe_stack_resource_drifts_output.DescribeStackResourceDriftsOutput:
    root = fromstring(response.read())
    result = root.find("DescribeStackResourceDriftsResult")
    out: capo_cloudformation.types.describe_stack_resource_drifts_output.DescribeStackResourceDriftsOutput = capo_cloudformation.types.describe_stack_resource_drifts_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeStackResourceDrifts"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.describe_stack_resource_drifts_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_stack_resource_drifts(
