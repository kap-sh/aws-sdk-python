"""Generated from Smithy shape ``com.amazonaws.cloudformation#DeactivateOrganizationsAccess``."""
...
import capo_cloudformation.errors.operation_not_found_exception
import capo_cloudformation.types.deactivate_organizations_access_input
import capo_cloudformation.types.deactivate_organizations_access_output
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "OperationNotFoundException":
            raise capo_cloudformation.errors.operation_not_found_exception.OperationNotFoundException.from_query(
                root
            )
        case _:
...
) -> capo_cloudformation.types.deactivate_organizations_access_output.DeactivateOrganizationsAccessOutput:
    root = fromstring(response.read())
    result = root.find("DeactivateOrganizationsAccessResult")
    out: capo_cloudformation.types.deactivate_organizations_access_output.DeactivateOrganizationsAccessOutput = capo_cloudformation.types.deactivate_organizations_access_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DeactivateOrganizationsAccess"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.deactivate_organizations_access_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_deactivate_organizations_access(
