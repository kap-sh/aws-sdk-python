"""Generated from Smithy shape ``com.amazonaws.cloudformation#GetTemplate``."""
...
import capo_cloudformation.errors.change_set_not_found_exception
import capo_cloudformation.types.get_template_input
import capo_cloudformation.types.get_template_output
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ChangeSetNotFoundException":
            raise capo_cloudformation.errors.change_set_not_found_exception.ChangeSetNotFoundException.from_query(
                root
            )
        case _:
...
) -> capo_cloudformation.types.get_template_output.GetTemplateOutput:
    root = fromstring(response.read())
    result = root.find("GetTemplateResult")
    out: capo_cloudformation.types.get_template_output.GetTemplateOutput = (
        capo_cloudformation.types.get_template_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "GetTemplate"))
    pairs.append(("Version", "2010-05-15"))
    capo_cloudformation.types.get_template_input.serialize_query(input_, pairs, "")
...
def get_template(
    ...
    capo_cloudformation.types.get_template_output.GetTemplateOutput, zapros.Response
]:
