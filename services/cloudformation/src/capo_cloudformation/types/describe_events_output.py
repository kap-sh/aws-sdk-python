...
class DescribeEventsOutput(TypedDict, closed=True):
    operation_events: NotRequired[
        "capo_cloudformation.types.operation_events.OperationEvents"
    ]
...
def serialize_query(
    ...
    if "operation_events" in value:
        ...
        capo_cloudformation.types.operation_events.serialize_query(
            value["operation_events"], pairs, f"{key_prefix}OperationEvents"
        )
...
def deserialize_query(el: Element) -> DescribeEventsOutput:
    ...
    if child_operation_events is not None:
        ...
        out["operation_events"] = (
            capo_cloudformation.types.operation_events.deserialize_query(
