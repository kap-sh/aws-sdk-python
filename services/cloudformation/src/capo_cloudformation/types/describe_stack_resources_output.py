...
class DescribeStackResourcesOutput(TypedDict, closed=True):
...
def serialize_query(
    ...
    if "stack_resources" in value:
        ...
        capo_cloudformation.types.stack_resources.serialize_query(
            value["stack_resources"], pairs, f"{key_prefix}StackResources"
        )
...
def deserialize_query(el: Element) -> DescribeStackResourcesOutput:
    ...
    if child_stack_resources is not None:
        ...
        out["stack_resources"] = (
            capo_cloudformation.types.stack_resources.deserialize_query(
