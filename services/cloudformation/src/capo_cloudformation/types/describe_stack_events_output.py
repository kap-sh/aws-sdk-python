...
class DescribeStackEventsOutput(TypedDict, closed=True):
    stack_events: NotRequired["capo_cloudformation.types.stack_events.StackEvents"]
...
def serialize_query(
    ...
    if "stack_events" in value:
        ...
        capo_cloudformation.types.stack_events.serialize_query(
            value["stack_events"], pairs, f"{key_prefix}StackEvents"
        )
...
def deserialize_query(el: Element) -> DescribeStackEventsOutput:
    ...
    if child_stack_events is not None:
        ...
        out["stack_events"] = capo_cloudformation.types.stack_events.deserialize_query(
            child_stack_events
        )
