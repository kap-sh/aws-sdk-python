...
if TYPE_CHECKING:
    import capo_cloudformation.types.resource_drift_actual_value
    import capo_cloudformation.types.resource_drift_previous_value
    import capo_cloudformation.types.timestamp


class LiveResourceDrift(TypedDict, closed=True):
    previous_value: NotRequired[
        "capo_cloudformation.types.resource_drift_previous_value.ResourceDriftPreviousValue"
    ]
    """<p>The configuration value from the previous CloudFormation deployment.</p>"""
    actual_value: NotRequired[
        "capo_cloudformation.types.resource_drift_actual_value.ResourceDriftActualValue"
    ]
