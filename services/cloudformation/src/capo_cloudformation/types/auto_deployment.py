...
if TYPE_CHECKING:
    import capo_cloudformation.types.auto_deployment_nullable
    import capo_cloudformation.types.retain_stacks_on_account_removal_nullable
    import capo_cloudformation.types.stack_set_arn_list


class AutoDeployment(TypedDict, closed=True):
    enabled: NotRequired[
        "capo_cloudformation.types.auto_deployment_nullable.AutoDeploymentNullable"
    ]
    ...
    retain_stacks_on_account_removal: NotRequired[
        "capo_cloudformation.types.retain_stacks_on_account_removal_nullable.RetainStacksOnAccountRemovalNullable"
    ]
    ...
    depends_on: NotRequired[
        "capo_cloudformation.types.stack_set_arn_list.StackSetARNList"
    ]
...
def deserialize_query(el: Element) -> AutoDeployment:
    ...
    if child_depends_on is not None:
        ...
        out["depends_on"] = (
            capo_cloudformation.types.stack_set_arn_list.deserialize_query(
                child_depends_on
            )
        )
    return out
