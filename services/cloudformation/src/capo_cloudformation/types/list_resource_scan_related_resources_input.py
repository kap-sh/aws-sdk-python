...
if TYPE_CHECKING:
    import capo_cloudformation.types.boxed_max_results
...
class ListResourceScanRelatedResourcesInput(TypedDict, closed=True):
...
def deserialize_query(el: Element) -> ListResourceScanRelatedResourcesInput:
    ...
    if child_resources is not None:
        ...
        out["resources"] = (
            capo_cloudformation.types.scanned_resource_identifiers.deserialize_query(
                child_resources
            )
        )
