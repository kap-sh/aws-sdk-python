...
class ListResourceScansInput(TypedDict, closed=True):
    ...
    scan_type_filter: NotRequired["capo_cloudformation.types.scan_type.ScanType"]
...
def serialize_query(
    ...
    if "scan_type_filter" in value:
        ...
        capo_cloudformation.types.scan_type.serialize_query(
            value["scan_type_filter"], pairs, f"{key_prefix}ScanTypeFilter"
        )
...
def deserialize_query(el: Element) -> ListResourceScansInput:
    ...
    if child_scan_type_filter is not None:
        ...
        out["scan_type_filter"] = capo_cloudformation.types.scan_type.deserialize_query(
