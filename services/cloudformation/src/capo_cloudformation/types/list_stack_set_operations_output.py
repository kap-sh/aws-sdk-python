...
class ListStackSetOperationsOutput(TypedDict, closed=True):
    summaries: NotRequired[
        "capo_cloudformation.types.stack_set_operation_summaries.StackSetOperationSummaries"
    ]
...
def serialize_query(
    ...
    if "summaries" in value:
        ...
        capo_cloudformation.types.stack_set_operation_summaries.serialize_query(
...
def deserialize_query(el: Element) -> ListStackSetOperationsOutput:
    ...
    if child_summaries is not None:
        ...
        out["summaries"] = (
            capo_cloudformation.types.stack_set_operation_summaries.deserialize_query(
                child_summaries
            )
        )
