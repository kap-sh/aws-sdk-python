...
if TYPE_CHECKING:
    import capo_cloudformation.types.required_activated_type

RequiredActivatedTypes: TypeAlias = list[
    "capo_cloudformation.types.required_activated_type.RequiredActivatedType"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_cloudformation.types.required_activated_type.serialize_query(
...
def deserialize_query(el: Element) -> RequiredActivatedTypes:
    ...
    for child in el.findall("member"):
        out.append(
            capo_cloudformation.types.required_activated_type.deserialize_query(child)
        )
    return out
