...
class ResourceMapping(TypedDict, closed=True):
    source: NotRequired["capo_cloudformation.types.resource_location.ResourceLocation"]
    """<p>The source stack <code>StackName</code> and <code>LogicalResourceId</code> for the resource being refactored.</p>"""
    destination: NotRequired[
        "capo_cloudformation.types.resource_location.ResourceLocation"
    ]
...
def deserialize_query(el: Element) -> ResourceMapping:
    ...
    if child_source is not None:
        ...
        out["source"] = capo_cloudformation.types.resource_location.deserialize_query(
            child_source
        )
    ...
    if child_destination is not None:
        ...
        out["destination"] = (
            capo_cloudformation.types.resource_location.deserialize_query(
                child_destination
            )
        )
    return out
