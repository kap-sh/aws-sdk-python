...
if TYPE_CHECKING:
    import capo_cloudformation.types.resource_definition

ResourceDefinitions: TypeAlias = list[
    "capo_cloudformation.types.resource_definition.ResourceDefinition"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_cloudformation.types.resource_definition.serialize_query(
...
def deserialize_query(el: Element) -> ResourceDefinitions:
    ...
    for child in el.findall("member"):
        out.append(
            capo_cloudformation.types.resource_definition.deserialize_query(child)
        )
    return out
