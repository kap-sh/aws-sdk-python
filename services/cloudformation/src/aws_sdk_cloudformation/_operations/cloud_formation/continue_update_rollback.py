"""Generated from Smithy shape ``com.amazonaws.cloudformation#ContinueUpdateRollback``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ContinueUpdateRollbackResult")
    out: aws_sdk_cloudformation.types.continue_update_rollback_output.ContinueUpdateRollbackOutput = aws_sdk_cloudformation.types.continue_update_rollback_output.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_cloudformation.types.continue_update_rollback_input.ContinueUpdateRollbackInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
...
async def async_continue_update_rollback(
