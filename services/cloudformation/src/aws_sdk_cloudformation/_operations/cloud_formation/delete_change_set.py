"""Generated from Smithy shape ``com.amazonaws.cloudformation#DeleteChangeSet``."""
...
import aws_sdk_cloudformation.errors.invalid_change_set_status_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "InvalidChangeSetStatusException":
            raise aws_sdk_cloudformation.errors.invalid_change_set_status_exception.InvalidChangeSetStatusException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DeleteChangeSetResult")
    out: aws_sdk_cloudformation.types.delete_change_set_output.DeleteChangeSetOutput = (
        aws_sdk_cloudformation.types.delete_change_set_output.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_cloudformation.types.delete_change_set_input.DeleteChangeSetInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "DeleteChangeSet"))
    pairs.append(("Version", "2010-05-15"))
    aws_sdk_cloudformation.types.delete_change_set_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_delete_change_set(
