"""Generated from Smithy shape ``com.amazonaws.cloudformation#PublishType``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("PublishTypeResult")
    out: aws_sdk_cloudformation.types.publish_type_output.PublishTypeOutput = (
        aws_sdk_cloudformation.types.publish_type_output.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_cloudformation.types.publish_type_input.PublishTypeInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "PublishType"))
    pairs.append(("Version", "2010-05-15"))
    import aws_sdk_cloudformation.types.publish_type_input

    aws_sdk_cloudformation.types.publish_type_input.serialize_query(input_, pairs, "")
...
def publish_type(
    ...
    aws_sdk_cloudformation.types.publish_type_output.PublishTypeOutput, zapros.Response
]:
