"""Generated from Smithy shape ``com.amazonaws.cloudformation#CreateStackSet``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "CreatedButModifiedException":
            import aws_sdk_cloudformation.errors.created_but_modified_exception

            raise aws_sdk_cloudformation.errors.created_but_modified_exception.CreatedButModifiedException.from_query(
                root
            )
        ...
        case "NameAlreadyExistsException":
            import aws_sdk_cloudformation.errors.name_already_exists_exception

            raise aws_sdk_cloudformation.errors.name_already_exists_exception.NameAlreadyExistsException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateStackSetResult")
    out: aws_sdk_cloudformation.types.create_stack_set_output.CreateStackSetOutput = (
        aws_sdk_cloudformation.types.create_stack_set_output.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_cloudformation.types.create_stack_set_input.CreateStackSetInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "CreateStackSet"))
    pairs.append(("Version", "2010-05-15"))
    import aws_sdk_cloudformation.types.create_stack_set_input

    aws_sdk_cloudformation.types.create_stack_set_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_create_stack_set(
