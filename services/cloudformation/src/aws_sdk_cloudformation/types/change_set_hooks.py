"""Generated from Smithy shape ``com.amazonaws.cloudformation#ChangeSetHooks``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_cloudformation._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_cloudformation.types.change_set_hook

ChangeSetHooks: TypeAlias = list[
    "aws_sdk_cloudformation.types.change_set_hook.ChangeSetHook"
]


# --- awsQuery ser/de ---
def serialize_query(
    value: ChangeSetHooks, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_cloudformation.types.change_set_hook

    for n, item in enumerate(value, 1):
        aws_sdk_cloudformation.types.change_set_hook.serialize_query(
            item, pairs, f"{prefix}.member.{n}"
        )


def deserialize_query(el: Element) -> ChangeSetHooks:
    import aws_sdk_cloudformation.types.change_set_hook

    out: ChangeSetHooks = []
    for child in el.findall("member"):
        out.append(
            aws_sdk_cloudformation.types.change_set_hook.deserialize_query(child)
        )
    return out


def serialize_query_flat(
    value: ChangeSetHooks, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_cloudformation.types.change_set_hook

    for n, item in enumerate(value, 1):
        aws_sdk_cloudformation.types.change_set_hook.serialize_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_query_flat(parent: Element, tag: str) -> ChangeSetHooks:
    import aws_sdk_cloudformation.types.change_set_hook

    out: ChangeSetHooks = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_cloudformation.types.change_set_hook.deserialize_query(child)
        )
    return out
