"""Generated from Smithy shape ``com.amazonaws.cloudformation#HookResultSummaries``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_cloudformation._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_cloudformation.types.hook_result_summary

HookResultSummaries: TypeAlias = list[
    "aws_sdk_cloudformation.types.hook_result_summary.HookResultSummary"
]


# --- awsQuery ser/de ---
def serialize_query(
    value: HookResultSummaries, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_cloudformation.types.hook_result_summary

    for n, item in enumerate(value, 1):
        aws_sdk_cloudformation.types.hook_result_summary.serialize_query(
            item, pairs, f"{prefix}.member.{n}"
        )


def deserialize_query(el: Element) -> HookResultSummaries:
    import aws_sdk_cloudformation.types.hook_result_summary

    out: HookResultSummaries = []
    for child in el.findall("member"):
        out.append(
            aws_sdk_cloudformation.types.hook_result_summary.deserialize_query(child)
        )
    return out


def serialize_query_flat(
    value: HookResultSummaries, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_cloudformation.types.hook_result_summary

    for n, item in enumerate(value, 1):
        aws_sdk_cloudformation.types.hook_result_summary.serialize_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_query_flat(parent: Element, tag: str) -> HookResultSummaries:
    import aws_sdk_cloudformation.types.hook_result_summary

    out: HookResultSummaries = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_cloudformation.types.hook_result_summary.deserialize_query(child)
        )
    return out
