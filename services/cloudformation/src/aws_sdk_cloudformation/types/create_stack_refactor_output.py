"""Generated from Smithy shape ``com.amazonaws.cloudformation#CreateStackRefactorOutput``."""
...
class CreateStackRefactorOutput(TypedDict, closed=True):
...
# --- awsQuery ser/de ---
def serialize_query(
    value: CreateStackRefactorOutput, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "stack_refactor_id" in value:
        pairs.append((f"{prefix}.StackRefactorId", str(value["stack_refactor_id"])))


def deserialize_query(el: Element) -> CreateStackRefactorOutput:
    out: CreateStackRefactorOutput = {}  # type: ignore[typeddict-item]
