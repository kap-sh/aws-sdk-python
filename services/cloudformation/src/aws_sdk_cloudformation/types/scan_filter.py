...
if TYPE_CHECKING:
    import aws_sdk_cloudformation.types.resource_type_filters


class ScanFilter(TypedDict):
    types: NotRequired[
...
def serialize_query(
    ...
    if "types" in value:
        ...
        aws_sdk_cloudformation.types.resource_type_filters.serialize_query(
...
def deserialize_query(el: Element) -> ScanFilter:
    ...
    if child_types is not None:
        ...
        out["types"] = (
            aws_sdk_cloudformation.types.resource_type_filters.deserialize_query(
                child_types
            )
        )
    return out
