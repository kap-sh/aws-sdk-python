...
if TYPE_CHECKING:
    import aws_sdk_cloudformation.types.category
...
class TypeFilters(TypedDict, closed=True):
    ...
    type_name_prefix: NotRequired[
        "aws_sdk_cloudformation.types.type_name_prefix.TypeNamePrefix"
    ]
...
def serialize_query(
    ...
    if "category" in value:
        ...
        aws_sdk_cloudformation.types.category.serialize_query(
...
def deserialize_query(el: Element) -> TypeFilters:
    ...
    if child_category is not None:
        ...
        out["category"] = aws_sdk_cloudformation.types.category.deserialize_query(
            child_category
        )
