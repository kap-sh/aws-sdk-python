...
if TYPE_CHECKING:
    import capo_ssm.types.maintenance_window_filter

MaintenanceWindowFilterList: TypeAlias = list[
    "capo_ssm.types.maintenance_window_filter.MaintenanceWindowFilter"
]
...
def serialize_aws_json_1_1(value: MaintenanceWindowFilterList) -> list:
    ...
    for item in value:
        out.append(
            capo_ssm.types.maintenance_window_filter.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> MaintenanceWindowFilterList:
    ...
    for item in data:
        out.append(
            capo_ssm.types.maintenance_window_filter.deserialize_aws_json_1_1(item)
        )
    return out
