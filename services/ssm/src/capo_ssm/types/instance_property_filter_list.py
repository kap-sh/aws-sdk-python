...
if TYPE_CHECKING:
    import capo_ssm.types.instance_property_filter

InstancePropertyFilterList: TypeAlias = list[
    "capo_ssm.types.instance_property_filter.InstancePropertyFilter"
]
...
def serialize_aws_json_1_1(value: InstancePropertyFilterList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_ssm.types.instance_property_filter.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> InstancePropertyFilterList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_ssm.types.instance_property_filter.deserialize_aws_json_1_1(item)
        )
    return out
