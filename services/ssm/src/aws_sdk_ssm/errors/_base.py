...
class SSMError(Exception):
    """Base class for all SSM service errors."""


class ServiceError(SSMError):
...
class SerializationError(SSMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(SSMError):
...
class WaiterFailedError(SSMError):
...
class WaiterTimeoutError(SSMError):
