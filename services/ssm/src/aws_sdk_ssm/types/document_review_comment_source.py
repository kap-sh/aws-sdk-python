...
if TYPE_CHECKING:
    import aws_sdk_ssm.types.document_review_comment
    import aws_sdk_ssm.types.document_review_comment_type


class DocumentReviewCommentSource(TypedDict):
    type: NotRequired[
        "aws_sdk_ssm.types.document_review_comment_type.DocumentReviewCommentType"
    ]
    """<p>The type of information added to a review request. Currently, only the value <code>Comment</code> is supported.</p>"""
    content: NotRequired[
        "aws_sdk_ssm.types.document_review_comment.DocumentReviewComment"
    ]
...
def serialize_aws_json_1_1(value: DocumentReviewCommentSource) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = (
            aws_sdk_ssm.types.document_review_comment_type.serialize_aws_json_1_1(
                value["type"]
            )
        )
    if "content" in value:
...
def deserialize_aws_json_1_1(data: dict) -> DocumentReviewCommentSource:
    ...
    if "Type" in data:
        ...
        out["type"] = (
            aws_sdk_ssm.types.document_review_comment_type.deserialize_aws_json_1_1(
                data["Type"]
            )
        )
    if "Content" in data:
