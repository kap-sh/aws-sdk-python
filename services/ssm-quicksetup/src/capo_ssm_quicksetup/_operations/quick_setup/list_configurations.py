"""Generated from Smithy shape ``com.amazonaws.ssmquicksetup#ListConfigurations``."""
...
import capo_ssm_quicksetup.errors.resource_not_found_exception
import capo_ssm_quicksetup.errors.throttling_exception
import capo_ssm_quicksetup.errors.validation_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            raise capo_ssm_quicksetup.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
        ...
        case "ValidationException":
            raise capo_ssm_quicksetup.errors.validation_exception.ValidationException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_ssm_quicksetup.types.list_configurations_output.ListConfigurationsOutput = capo_ssm_quicksetup.types.list_configurations_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listConfigurations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_quicksetup.types.list_configurations_input.serialize_json(input_)
    ).encode()
