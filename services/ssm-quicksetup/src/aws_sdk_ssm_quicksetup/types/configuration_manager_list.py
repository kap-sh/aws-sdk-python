...
ConfigurationManagerList: TypeAlias = list[
    "aws_sdk_ssm_quicksetup.types.configuration_manager_summary.ConfigurationManagerSummary"
]
...
def serialize_json(value: ConfigurationManagerList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_ssm_quicksetup.types.configuration_manager_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ConfigurationManagerList:
    ...
    for item in data:
        out.append(
            aws_sdk_ssm_quicksetup.types.configuration_manager_summary.deserialize_json(
                item
            )
        )
    return out
