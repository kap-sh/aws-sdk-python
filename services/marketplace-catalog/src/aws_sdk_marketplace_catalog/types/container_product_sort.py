"""Generated from Smithy shape ``com.amazonaws.marketplacecatalog#ContainerProductSort``."""
...
if TYPE_CHECKING:
    import aws_sdk_marketplace_catalog.types.container_product_sort_by
    import aws_sdk_marketplace_catalog.types.sort_order


class ContainerProductSort(TypedDict, closed=True):
    sort_by: NotRequired[
        "aws_sdk_marketplace_catalog.types.container_product_sort_by.ContainerProductSortBy"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: ContainerProductSort) -> dict:
    out: dict = {}
    if "sort_by" in value:
        import aws_sdk_marketplace_catalog.types.container_product_sort_by

        out["SortBy"] = (
            aws_sdk_marketplace_catalog.types.container_product_sort_by.serialize_json(
                value["sort_by"]
            )
        )
...
def deserialize_json(data: dict) -> ContainerProductSort:
    ...
    if "SortBy" in data:
        ...
        out["sort_by"] = (
            aws_sdk_marketplace_catalog.types.container_product_sort_by.deserialize_json(
                data["SortBy"]
            )
        )
