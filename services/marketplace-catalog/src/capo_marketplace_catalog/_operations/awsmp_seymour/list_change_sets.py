"""Generated from Smithy shape ``com.amazonaws.marketplacecatalog#ListChangeSets``."""
...
def handle_response(
    ...
    out: capo_marketplace_catalog.types.list_change_sets_response.ListChangeSetsResponse = capo_marketplace_catalog.types.list_change_sets_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListChangeSets"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_marketplace_catalog.types.list_change_sets_request.serialize_json(input_)
    ).encode()
