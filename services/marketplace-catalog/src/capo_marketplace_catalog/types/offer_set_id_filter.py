...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.offer_set_id_filter_value_list


class OfferSetIdFilter(TypedDict, closed=True):
    value_list: NotRequired[
        "capo_marketplace_catalog.types.offer_set_id_filter_value_list.OfferSetIdFilterValueList"
    ]
...
def serialize_json(value: OfferSetIdFilter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = (
            capo_marketplace_catalog.types.offer_set_id_filter_value_list.serialize_json(
                value["value_list"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> OfferSetIdFilter:
    ...
    if "ValueList" in data:
        ...
        out["value_list"] = (
            capo_marketplace_catalog.types.offer_set_id_filter_value_list.deserialize_json(
                data["ValueList"]
            )
        )
    return out
