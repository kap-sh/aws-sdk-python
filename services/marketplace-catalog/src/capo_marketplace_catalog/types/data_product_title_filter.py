...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.data_product_title_filter_value_list
...
class DataProductTitleFilter(TypedDict, closed=True):
    value_list: NotRequired[
        "capo_marketplace_catalog.types.data_product_title_filter_value_list.DataProductTitleFilterValueList"
    ]
...
def serialize_json(value: DataProductTitleFilter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = (
            capo_marketplace_catalog.types.data_product_title_filter_value_list.serialize_json(
                value["value_list"]
            )
        )
...
def deserialize_json(data: dict) -> DataProductTitleFilter:
    ...
    if "ValueList" in data:
        ...
        out["value_list"] = (
            capo_marketplace_catalog.types.data_product_title_filter_value_list.deserialize_json(
                data["ValueList"]
            )
        )
