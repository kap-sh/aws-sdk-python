...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.ami_product_sort_by
    import capo_marketplace_catalog.types.sort_order


class AmiProductSort(TypedDict, closed=True):
    sort_by: NotRequired[
        "capo_marketplace_catalog.types.ami_product_sort_by.AmiProductSortBy"
    ]
...
def serialize_json(value: AmiProductSort) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["SortBy"] = (
            capo_marketplace_catalog.types.ami_product_sort_by.serialize_json(
                value["sort_by"]
            )
        )
...
def deserialize_json(data: dict) -> AmiProductSort:
    ...
    if "SortBy" in data:
        ...
        out["sort_by"] = (
            capo_marketplace_catalog.types.ami_product_sort_by.deserialize_json(
                data["SortBy"]
            )
        )
