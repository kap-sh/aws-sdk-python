...
class ResaleAuthorizationLastModifiedDateFilter(TypedDict, closed=True):
    date_range: NotRequired[
        "capo_marketplace_catalog.types.resale_authorization_last_modified_date_filter_date_range.ResaleAuthorizationLastModifiedDateFilterDateRange"
    ]
...
def serialize_json(value: ResaleAuthorizationLastModifiedDateFilter) -> dict:
    ...
    if "date_range" in value:
        ...
        out["DateRange"] = (
            capo_marketplace_catalog.types.resale_authorization_last_modified_date_filter_date_range.serialize_json(
                value["date_range"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ResaleAuthorizationLastModifiedDateFilter:
    ...
    if "DateRange" in data:
        ...
        out["date_range"] = (
            capo_marketplace_catalog.types.resale_authorization_last_modified_date_filter_date_range.deserialize_json(
                data["DateRange"]
            )
        )
    return out
