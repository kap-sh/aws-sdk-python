...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.offer_set_sort_by
    import capo_marketplace_catalog.types.sort_order


class OfferSetSort(TypedDict, closed=True):
    sort_by: NotRequired[
        "capo_marketplace_catalog.types.offer_set_sort_by.OfferSetSortBy"
    ]
...
def serialize_json(value: OfferSetSort) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["SortBy"] = capo_marketplace_catalog.types.offer_set_sort_by.serialize_json(
            value["sort_by"]
        )
...
def deserialize_json(data: dict) -> OfferSetSort:
    ...
    if "SortBy" in data:
        ...
        out["sort_by"] = (
            capo_marketplace_catalog.types.offer_set_sort_by.deserialize_json(
                data["SortBy"]
            )
        )
