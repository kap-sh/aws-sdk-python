...
if TYPE_CHECKING:
    import capo_marketplace_catalog.types.filter_name
    import capo_marketplace_catalog.types.value_list


class Filter(TypedDict, closed=True):
    name: NotRequired["capo_marketplace_catalog.types.filter_name.FilterName"]
    ...
    value_list: NotRequired["capo_marketplace_catalog.types.value_list.ValueList"]
...
def serialize_json(value: Filter) -> dict:
    ...
    if "value_list" in value:
        ...
        out["ValueList"] = capo_marketplace_catalog.types.value_list.serialize_json(
            value["value_list"]
        )
    return out
...
def deserialize_json(data: dict) -> Filter:
    ...
    if "ValueList" in data:
        ...
        out["value_list"] = capo_marketplace_catalog.types.value_list.deserialize_json(
            data["ValueList"]
        )
    return out
