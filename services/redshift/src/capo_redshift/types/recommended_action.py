...
class RecommendedAction(TypedDict, closed=True):
    ...
    database: NotRequired["capo_redshift.types.string.String"]
    """<p>The database name to perform the action on. Only applicable if the type of command is SQL.</p>"""
    command: NotRequired["capo_redshift.types.string.String"]
    """<p>The command to run.</p>"""
    type: NotRequired[
        "capo_redshift.types.recommended_action_type.RecommendedActionType"
    ]
...
def deserialize_query(el: Element) -> RecommendedAction:
    ...
    if child_type is not None:
        ...
        out["type"] = capo_redshift.types.recommended_action_type.deserialize_query(
            child_type
        )
    return out
