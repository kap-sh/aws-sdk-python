...
if TYPE_CHECKING:
    import capo_redshift.types.availability_zone

AvailabilityZoneList: TypeAlias = list[
    "capo_redshift.types.availability_zone.AvailabilityZone"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_redshift.types.availability_zone.serialize_query(
...
def deserialize_query(el: Element) -> AvailabilityZoneList:
    ...
    for child in el.findall("AvailabilityZone"):
        out.append(capo_redshift.types.availability_zone.deserialize_query(child))
    return out
