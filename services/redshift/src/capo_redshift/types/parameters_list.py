...
ParametersList: TypeAlias = list["capo_redshift.types.parameter.Parameter"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_redshift.types.parameter.serialize_query(
...
def deserialize_query(el: Element) -> ParametersList:
    ...
    for child in el.findall("Parameter"):
        out.append(capo_redshift.types.parameter.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        capo_redshift.types.parameter.serialize_query(item, pairs, f"{prefix}.{n}")
