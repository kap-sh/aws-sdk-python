"""Generated from Smithy shape ``com.amazonaws.redshift#UnauthorizedOperation``."""
...
# --- awsQuery ser/de ---
def serialize_query(
    value: UnauthorizedOperation_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.message", str(value["message"])))


def deserialize_query(el: Element) -> UnauthorizedOperation_:
...
class UnauthorizedOperation(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.redshift#UnauthorizedOperation``."""
    ...
    @classmethod
    def from_query(cls, el: Element) -> "UnauthorizedOperation":
