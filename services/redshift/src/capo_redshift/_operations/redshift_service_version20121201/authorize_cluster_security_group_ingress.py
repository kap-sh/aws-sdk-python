"""Generated from Smithy shape ``com.amazonaws.redshift#AuthorizeClusterSecurityGroupIngress``."""
...
import capo_redshift.errors.authorization_already_exists_fault
import capo_redshift.errors.authorization_quota_exceeded_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AuthorizationAlreadyExistsFault":
            raise capo_redshift.errors.authorization_already_exists_fault.AuthorizationAlreadyExistsFault.from_query(
                root
            )
        case "AuthorizationQuotaExceededFault":
            raise capo_redshift.errors.authorization_quota_exceeded_fault.AuthorizationQuotaExceededFault.from_query(
                root
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("AuthorizeClusterSecurityGroupIngressResult")
    out: capo_redshift.types.authorize_cluster_security_group_ingress_result.AuthorizeClusterSecurityGroupIngressResult = capo_redshift.types.authorize_cluster_security_group_ingress_result.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "AuthorizeClusterSecurityGroupIngress"))
    pairs.append(("Version", "2012-12-01"))
    capo_redshift.types.authorize_cluster_security_group_ingress_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_authorize_cluster_security_group_ingress(
