"""Generated from Smithy shape ``com.amazonaws.redshift#DescribeClusterDbRevisions``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeClusterDbRevisionsResult")
    out: capo_redshift.types.cluster_db_revisions_message.ClusterDbRevisionsMessage = (
        capo_redshift.types.cluster_db_revisions_message.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeClusterDbRevisions"))
    pairs.append(("Version", "2012-12-01"))
    capo_redshift.types.describe_cluster_db_revisions_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_cluster_db_revisions(
