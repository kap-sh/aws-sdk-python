"""Generated from Smithy shape ``com.amazonaws.redshift#DescribeTableRestoreStatus``."""
...
import capo_redshift.errors.table_restore_not_found_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "TableRestoreNotFoundFault":
            raise capo_redshift.errors.table_restore_not_found_fault.TableRestoreNotFoundFault.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeTableRestoreStatusResult")
    out: capo_redshift.types.table_restore_status_message.TableRestoreStatusMessage = (
        capo_redshift.types.table_restore_status_message.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeTableRestoreStatus"))
    pairs.append(("Version", "2012-12-01"))
    capo_redshift.types.describe_table_restore_status_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_table_restore_status(
