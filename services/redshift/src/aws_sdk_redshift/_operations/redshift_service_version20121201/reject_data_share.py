"""Generated from Smithy shape ``com.amazonaws.redshift#RejectDataShare``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("RejectDataShareResult")
...
def build_request(
    ...
    input_: aws_sdk_redshift.types.reject_data_share_message.RejectDataShareMessage,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "RejectDataShare"))
    pairs.append(("Version", "2012-12-01"))
    aws_sdk_redshift.types.reject_data_share_message.serialize_query(input_, pairs, "")
...
async def async_reject_data_share(
