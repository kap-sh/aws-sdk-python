...
EventList: TypeAlias = list["aws_sdk_redshift.types.event.Event"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_redshift.types.event.serialize_query(item, pairs, f"{prefix}.Event.{n}")
...
def deserialize_query(el: Element) -> EventList:
    ...
    for child in el.findall("Event"):
        out.append(aws_sdk_redshift.types.event.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_redshift.types.event.serialize_query(item, pairs, f"{prefix}.{n}")
