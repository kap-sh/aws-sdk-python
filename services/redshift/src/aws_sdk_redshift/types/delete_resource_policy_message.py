"""Generated from Smithy shape ``com.amazonaws.redshift#DeleteResourcePolicyMessage``."""
...
class DeleteResourcePolicyMessage(TypedDict, closed=True):
...
# --- awsQuery ser/de ---
def serialize_query(
    value: DeleteResourcePolicyMessage, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "resource_arn" in value:
        pairs.append((f"{prefix}.ResourceArn", str(value["resource_arn"])))


def deserialize_query(el: Element) -> DeleteResourcePolicyMessage:
    out: DeleteResourcePolicyMessage = {}  # type: ignore[typeddict-item]
