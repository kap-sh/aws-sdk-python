...
if TYPE_CHECKING:
    import aws_sdk_redshift.types.deferred_maintenance_window

DeferredMaintenanceWindowsList: TypeAlias = list[
    "aws_sdk_redshift.types.deferred_maintenance_window.DeferredMaintenanceWindow"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_redshift.types.deferred_maintenance_window.serialize_query(
...
def deserialize_query(el: Element) -> DeferredMaintenanceWindowsList:
    ...
    for child in el.findall("DeferredMaintenanceWindow"):
        out.append(
            aws_sdk_redshift.types.deferred_maintenance_window.deserialize_query(child)
        )
    return out
