...
class ScheduledActionType(TypedDict):
    resize_cluster: NotRequired[
        "aws_sdk_redshift.types.resize_cluster_message.ResizeClusterMessage"
    ]
    """<p>An action that runs a <code>ResizeCluster</code> API operation. </p>"""
    pause_cluster: NotRequired[
        "aws_sdk_redshift.types.pause_cluster_message.PauseClusterMessage"
    ]
    """<p>An action that runs a <code>PauseCluster</code> API operation. </p>"""
    resume_cluster: NotRequired[
        "aws_sdk_redshift.types.resume_cluster_message.ResumeClusterMessage"
    ]
...
def serialize_query(
    ...
    if "resize_cluster" in value:
        ...
        aws_sdk_redshift.types.resize_cluster_message.serialize_query(
            value["resize_cluster"], pairs, f"{prefix}.ResizeCluster"
        )
    if "pause_cluster" in value:
        import aws_sdk_redshift.types.pause_cluster_message

        aws_sdk_redshift.types.pause_cluster_message.serialize_query(
            value["pause_cluster"], pairs, f"{prefix}.PauseCluster"
        )
    if "resume_cluster" in value:
        import aws_sdk_redshift.types.resume_cluster_message

        aws_sdk_redshift.types.resume_cluster_message.serialize_query(
...
def deserialize_query(el: Element) -> ScheduledActionType:
    ...
    if child_resize_cluster is not None:
        ...
        out["resize_cluster"] = (
            aws_sdk_redshift.types.resize_cluster_message.deserialize_query(
    ...
    if child_pause_cluster is not None:
        ...
        out["pause_cluster"] = (
            aws_sdk_redshift.types.pause_cluster_message.deserialize_query(
                child_pause_cluster
            )
        )
    ...
    if child_resume_cluster is not None:
        ...
        out["resume_cluster"] = (
            aws_sdk_redshift.types.resume_cluster_message.deserialize_query(
