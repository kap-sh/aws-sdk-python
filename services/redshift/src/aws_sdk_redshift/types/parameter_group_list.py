...
ParameterGroupList: TypeAlias = list[
    "aws_sdk_redshift.types.cluster_parameter_group.ClusterParameterGroup"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_redshift.types.cluster_parameter_group.serialize_query(
...
def deserialize_query(el: Element) -> ParameterGroupList:
    ...
    for child in el.findall("ClusterParameterGroup"):
        out.append(
            aws_sdk_redshift.types.cluster_parameter_group.deserialize_query(child)
        )
    return out
