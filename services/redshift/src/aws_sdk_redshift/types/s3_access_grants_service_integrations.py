...
if TYPE_CHECKING:
    import aws_sdk_redshift.types.s3_access_grants_scope_union

S3AccessGrantsServiceIntegrations: TypeAlias = list[
    "aws_sdk_redshift.types.s3_access_grants_scope_union.S3AccessGrantsScopeUnion"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_redshift.types.s3_access_grants_scope_union.serialize_query(
...
def deserialize_query(el: Element) -> S3AccessGrantsServiceIntegrations:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_redshift.types.s3_access_grants_scope_union.deserialize_query(child)
        )
    return out
