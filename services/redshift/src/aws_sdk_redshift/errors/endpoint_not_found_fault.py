"""Generated from Smithy shape ``com.amazonaws.redshift#EndpointNotFoundFault``."""
...
class EndpointNotFoundFault_(TypedDict, closed=True):
    message: NotRequired["aws_sdk_redshift.types.exception_message.ExceptionMessage"]


# --- awsQuery ser/de ---
def serialize_query(
    value: EndpointNotFoundFault_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.message", str(value["message"])))


def deserialize_query(el: Element) -> EndpointNotFoundFault_:
    out: EndpointNotFoundFault_ = {}  # type: ignore[typeddict-item]
...
class EndpointNotFoundFault(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.redshift#EndpointNotFoundFault``."""
