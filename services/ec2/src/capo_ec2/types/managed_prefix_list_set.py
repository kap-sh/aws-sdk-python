"""Generated from Smithy shape ``com.amazonaws.ec2#ManagedPrefixListSet``."""

from typing import TYPE_CHECKING, TypeAlias

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.managed_prefix_list

ManagedPrefixListSet: TypeAlias = list[
    "capo_ec2.types.managed_prefix_list.ManagedPrefixList"
]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: ManagedPrefixListSet, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if not value:
        return
    for n, item in enumerate(value, 1):
        import capo_ec2.types.managed_prefix_list

        capo_ec2.types.managed_prefix_list.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(el: Element) -> ManagedPrefixListSet:
    import capo_ec2.types.managed_prefix_list

    out: ManagedPrefixListSet = []
    for child in el.findall("item"):
        out.append(capo_ec2.types.managed_prefix_list.deserialize_ec2_query(child))
    return out


def deserialize_ec2_query_flat(parent: Element, tag: str) -> ManagedPrefixListSet:
    import capo_ec2.types.managed_prefix_list

    out: ManagedPrefixListSet = []
    for child in parent.findall(tag):
        out.append(capo_ec2.types.managed_prefix_list.deserialize_ec2_query(child))
    return out
