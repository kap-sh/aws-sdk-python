...
def serialize_ec2_query(
    value: SecurityGroupIdList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    for n, item in enumerate(value, 1):
        pairs.append((f"{prefix}.{n}", str(item)))


def deserialize_ec2_query(el: Element) -> SecurityGroupIdList:
...
def deserialize_ec2_query_flat(parent: Element, tag: str) -> SecurityGroupIdList:
    out: SecurityGroupIdList = []
