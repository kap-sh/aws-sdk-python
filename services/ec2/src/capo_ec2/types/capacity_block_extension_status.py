"""Generated from Smithy shape ``com.amazonaws.ec2#CapacityBlockExtensionStatus``."""

from typing import Literal, TypeAlias, cast

from capo_ec2._protocol.xml import Element

CapacityBlockExtensionStatus: TypeAlias = Literal[
    "payment-pending",
    "payment-failed",
    "payment-succeeded",
]


# --- ec2Query ser/de ---
def to_ec2_query_text(value: CapacityBlockExtensionStatus) -> str:
    return value


def from_ec2_query_text(text: str) -> CapacityBlockExtensionStatus:
    return cast(CapacityBlockExtensionStatus, text)


def serialize_ec2_query(
    value: CapacityBlockExtensionStatus, pairs: list[tuple[str, str]], prefix: str
) -> None:
    pairs.append((prefix, to_ec2_query_text(value)))


def deserialize_ec2_query(el: Element) -> CapacityBlockExtensionStatus:
    return from_ec2_query_text(el.text or "")
