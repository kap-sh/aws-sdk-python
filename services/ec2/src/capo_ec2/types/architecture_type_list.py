...
def serialize_ec2_query(
    value: ArchitectureTypeList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if not value:
        return
...
def deserialize_ec2_query(el: Element) -> ArchitectureTypeList:
    import capo_ec2.types.architecture_type

    out: ArchitectureTypeList = []
    for child in el.findall("item"):
        out.append(capo_ec2.types.architecture_type.deserialize_ec2_query(child))
    return out


def deserialize_ec2_query_flat(parent: Element, tag: str) -> ArchitectureTypeList:
    import capo_ec2.types.architecture_type
