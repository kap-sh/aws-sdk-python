...
Phase1DHGroupNumbersRequestList: TypeAlias = list[
    "capo_ec2.types.phase1_dh_group_numbers_request_list_value.Phase1DHGroupNumbersRequestListValue"
]
...
def serialize_ec2_query(
    ...
    for n, item in enumerate(value, 1):
        ...
        capo_ec2.types.phase1_dh_group_numbers_request_list_value.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(el: Element) -> Phase1DHGroupNumbersRequestList:
    import capo_ec2.types.phase1_dh_group_numbers_request_list_value

    out: Phase1DHGroupNumbersRequestList = []
    for child in el.findall("item"):
        out.append(
            capo_ec2.types.phase1_dh_group_numbers_request_list_value.deserialize_ec2_query(
                child
            )
        )
    return out
