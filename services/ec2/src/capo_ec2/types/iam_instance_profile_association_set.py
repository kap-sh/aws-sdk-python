"""Generated from Smithy shape ``com.amazonaws.ec2#IamInstanceProfileAssociationSet``."""

from typing import TYPE_CHECKING, TypeAlias

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.iam_instance_profile_association

IamInstanceProfileAssociationSet: TypeAlias = list[
    "capo_ec2.types.iam_instance_profile_association.IamInstanceProfileAssociation"
]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: IamInstanceProfileAssociationSet, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if not value:
        pairs.append((prefix, ""))
        return
    for n, item in enumerate(value, 1):
        import capo_ec2.types.iam_instance_profile_association

        capo_ec2.types.iam_instance_profile_association.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(el: Element) -> IamInstanceProfileAssociationSet:
    import capo_ec2.types.iam_instance_profile_association

    out: IamInstanceProfileAssociationSet = []
    for child in el.findall("item"):
        out.append(
            capo_ec2.types.iam_instance_profile_association.deserialize_ec2_query(child)
        )
    return out


def deserialize_ec2_query_flat(
    parent: Element, tag: str
) -> IamInstanceProfileAssociationSet:
    import capo_ec2.types.iam_instance_profile_association

    out: IamInstanceProfileAssociationSet = []
    for child in parent.findall(tag):
        out.append(
            capo_ec2.types.iam_instance_profile_association.deserialize_ec2_query(child)
        )
    return out
