...
class DescribeAccountAttributesRequest(TypedDict, closed=True):
    ...
    attribute_names: NotRequired[
        "capo_ec2.types.account_attribute_name_string_list.AccountAttributeNameStringList"
    ]
...
def deserialize_ec2_query(el: Element) -> DescribeAccountAttributesRequest:
    ...
    child_attribute_names = el.find("attributeName")
    if child_attribute_names is not None:
        import capo_ec2.types.account_attribute_name_string_list

        out["attribute_names"] = (
            capo_ec2.types.account_attribute_name_string_list.deserialize_ec2_query(
