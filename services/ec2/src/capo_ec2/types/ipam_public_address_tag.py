"""Generated from Smithy shape ``com.amazonaws.ec2#IpamPublicAddressTag``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.string


class IpamPublicAddressTag(TypedDict, closed=True):
    key: NotRequired["capo_ec2.types.string.String"]
    """<p>The tag's key.</p>"""
    value: NotRequired["capo_ec2.types.string.String"]
    """<p>The tag's value.</p>"""


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: IpamPublicAddressTag, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "key" in value:
        pairs.append((f"{prefix}.Key", str(value["key"])))
    if "value" in value:
        pairs.append((f"{prefix}.Value", str(value["value"])))


def deserialize_ec2_query(el: Element) -> IpamPublicAddressTag:
    out: IpamPublicAddressTag = {}  # type: ignore[typeddict-item]
    child_key = el.find("Key")
    if child_key is not None:
        out["key"] = str(child_key.text or "")
    child_value = el.find("Value")
    if child_value is not None:
        out["value"] = str(child_value.text or "")
    return out
