...
if TYPE_CHECKING:
    ...
    import capo_ec2.types.traffic_mirror_session_id_list
    import capo_ec2.types.traffic_mirroring_max_results


class DescribeTrafficMirrorSessionsRequest(TypedDict, closed=True):
    traffic_mirror_session_ids: NotRequired[
        "capo_ec2.types.traffic_mirror_session_id_list.TrafficMirrorSessionIdList"
    ]
...
def serialize_ec2_query(
    ...
    if "traffic_mirror_session_ids" in value:
        ...
        capo_ec2.types.traffic_mirror_session_id_list.serialize_ec2_query(
...
def deserialize_ec2_query(el: Element) -> DescribeTrafficMirrorSessionsRequest:
    ...
    child_traffic_mirror_session_ids = el.find("TrafficMirrorSessionId")
    if child_traffic_mirror_session_ids is not None:
        import capo_ec2.types.traffic_mirror_session_id_list

        out["traffic_mirror_session_ids"] = (
            capo_ec2.types.traffic_mirror_session_id_list.deserialize_ec2_query(
                child_traffic_mirror_session_ids
            )
        )
