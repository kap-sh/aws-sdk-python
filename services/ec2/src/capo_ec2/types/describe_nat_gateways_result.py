...
class DescribeNatGatewaysResult(TypedDict, closed=True):
    nat_gateways: NotRequired["capo_ec2.types.nat_gateway_list.NatGatewayList"]
...
def deserialize_ec2_query(el: Element) -> DescribeNatGatewaysResult:
    ...
    child_nat_gateways = el.find("natGatewaySet")
    if child_nat_gateways is not None:
        import capo_ec2.types.nat_gateway_list

        out["nat_gateways"] = capo_ec2.types.nat_gateway_list.deserialize_ec2_query(
            child_nat_gateways
        )
