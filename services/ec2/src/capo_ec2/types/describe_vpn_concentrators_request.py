...
class DescribeVpnConcentratorsRequest(TypedDict, closed=True):
    vpn_concentrator_ids: NotRequired[
        "capo_ec2.types.vpn_concentrator_id_string_list.VpnConcentratorIdStringList"
    ]
...
def deserialize_ec2_query(el: Element) -> DescribeVpnConcentratorsRequest:
    ...
    child_vpn_concentrator_ids = el.find("VpnConcentratorId")
    if child_vpn_concentrator_ids is not None:
        import capo_ec2.types.vpn_concentrator_id_string_list

        out["vpn_concentrator_ids"] = (
            capo_ec2.types.vpn_concentrator_id_string_list.deserialize_ec2_query(
                child_vpn_concentrator_ids
            )
        )
