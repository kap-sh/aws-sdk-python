"""Generated from Smithy shape ``com.amazonaws.ec2#IpamPrefixListResolverTargetSet``."""

from typing import TYPE_CHECKING, TypeAlias

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.ipam_prefix_list_resolver_target

IpamPrefixListResolverTargetSet: TypeAlias = list[
    "capo_ec2.types.ipam_prefix_list_resolver_target.IpamPrefixListResolverTarget"
]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: IpamPrefixListResolverTargetSet, pairs: list[tuple[str, str]], prefix: str
) -> None:
    for n, item in enumerate(value, 1):
        import capo_ec2.types.ipam_prefix_list_resolver_target

        capo_ec2.types.ipam_prefix_list_resolver_target.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> IpamPrefixListResolverTargetSet:
    import capo_ec2.types.ipam_prefix_list_resolver_target

    out: IpamPrefixListResolverTargetSet = []
    for child in parent.findall(tag):
        out.append(
            capo_ec2.types.ipam_prefix_list_resolver_target.deserialize_ec2_query(child)
        )
    return out
