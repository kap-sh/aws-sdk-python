"""Generated from Smithy shape ``com.amazonaws.ec2#IpamRoutingPolicyRegistrationDeltaSet``."""

from typing import TYPE_CHECKING, TypeAlias

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.ipam_routing_policy_registration_delta

IpamRoutingPolicyRegistrationDeltaSet: TypeAlias = list[
    "capo_ec2.types.ipam_routing_policy_registration_delta.IpamRoutingPolicyRegistrationDelta"
]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: IpamRoutingPolicyRegistrationDeltaSet,
    pairs: list[tuple[str, str]],
    prefix: str,
) -> None:
    if not value:
        return
    for n, item in enumerate(value, 1):
        import capo_ec2.types.ipam_routing_policy_registration_delta

        capo_ec2.types.ipam_routing_policy_registration_delta.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(el: Element) -> IpamRoutingPolicyRegistrationDeltaSet:
    import capo_ec2.types.ipam_routing_policy_registration_delta

    out: IpamRoutingPolicyRegistrationDeltaSet = []
    for child in el.findall("item"):
        out.append(
            capo_ec2.types.ipam_routing_policy_registration_delta.deserialize_ec2_query(
                child
            )
        )
    return out


def deserialize_ec2_query_flat(
    parent: Element, tag: str
) -> IpamRoutingPolicyRegistrationDeltaSet:
    import capo_ec2.types.ipam_routing_policy_registration_delta

    out: IpamRoutingPolicyRegistrationDeltaSet = []
    for child in parent.findall(tag):
        out.append(
            capo_ec2.types.ipam_routing_policy_registration_delta.deserialize_ec2_query(
                child
            )
        )
    return out
