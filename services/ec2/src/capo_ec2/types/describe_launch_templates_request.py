...
if TYPE_CHECKING:
    ...
    import capo_ec2.types.describe_launch_templates_max_results
...
class DescribeLaunchTemplatesRequest(TypedDict, closed=True):
    dry_run: NotRequired["capo_ec2.types.boolean.Boolean"]
    """<p>Checks whether you have the required permissions for the action, without actually making the request, and provides an error response. If you have the required permissions, the error response is <code>DryRunOperation</code>. Otherwise, it is <code>UnauthorizedOperation</code>.</p>"""
    launch_template_ids: NotRequired[
        "capo_ec2.types.launch_template_id_string_list.LaunchTemplateIdStringList"
    ]
    """<p>One or more launch template IDs.</p>"""
    launch_template_names: NotRequired[
        "capo_ec2.types.launch_template_name_string_list.LaunchTemplateNameStringList"
    ]
    ...
    max_results: NotRequired[
        "capo_ec2.types.describe_launch_templates_max_results.DescribeLaunchTemplatesMaxResults"
    ]
...
def serialize_ec2_query(
    ...
    if "launch_template_ids" in value:
        ...
        capo_ec2.types.launch_template_id_string_list.serialize_ec2_query(
            value["launch_template_ids"], pairs, f"{key_prefix}LaunchTemplateId"
        )
    if "launch_template_names" in value:
        import capo_ec2.types.launch_template_name_string_list

        capo_ec2.types.launch_template_name_string_list.serialize_ec2_query(
            value["launch_template_names"], pairs, f"{key_prefix}LaunchTemplateName"
        )
    if "filters" in value:
        import capo_ec2.types.filter_list
