...
if TYPE_CHECKING:
    ...
    import capo_ec2.types.describe_secondary_interfaces_max_results
...
class DescribeSecondaryInterfacesRequest(TypedDict, closed=True):
    ...
    max_results: NotRequired[
        "capo_ec2.types.describe_secondary_interfaces_max_results.DescribeSecondaryInterfacesMaxResults"
    ]
    """<p>The maximum number of results to return with a single call. To retrieve the remaining results, make another call with the returned <code>nextToken</code> value.</p>"""
    next_token: NotRequired["capo_ec2.types.string.String"]
    """<p>The token for the next page of results.</p>"""
    secondary_interface_ids: NotRequired[
        "capo_ec2.types.secondary_interface_id_list.SecondaryInterfaceIdList"
    ]
...
def serialize_ec2_query(
    ...
    if "secondary_interface_ids" in value:
        ...
        capo_ec2.types.secondary_interface_id_list.serialize_ec2_query(
            value["secondary_interface_ids"], pairs, f"{key_prefix}SecondaryInterfaceId"
        )
...
def deserialize_ec2_query(el: Element) -> DescribeSecondaryInterfacesRequest:
    ...
    child_secondary_interface_ids = el.find("SecondaryInterfaceId")
    if child_secondary_interface_ids is not None:
        import capo_ec2.types.secondary_interface_id_list

        out["secondary_interface_ids"] = (
            capo_ec2.types.secondary_interface_id_list.deserialize_ec2_query(
                child_secondary_interface_ids
            )
        )
    return out
