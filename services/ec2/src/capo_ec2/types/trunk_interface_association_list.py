"""Generated from Smithy shape ``com.amazonaws.ec2#TrunkInterfaceAssociationList``."""

from typing import TYPE_CHECKING, TypeAlias

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.trunk_interface_association

TrunkInterfaceAssociationList: TypeAlias = list[
    "capo_ec2.types.trunk_interface_association.TrunkInterfaceAssociation"
]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: TrunkInterfaceAssociationList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    for n, item in enumerate(value, 1):
        import capo_ec2.types.trunk_interface_association

        capo_ec2.types.trunk_interface_association.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> TrunkInterfaceAssociationList:
    import capo_ec2.types.trunk_interface_association

    out: TrunkInterfaceAssociationList = []
    for child in parent.findall(tag):
        out.append(
            capo_ec2.types.trunk_interface_association.deserialize_ec2_query(child)
        )
    return out
