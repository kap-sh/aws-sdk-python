...
if TYPE_CHECKING:
    import capo_ec2.types.hour
    import capo_ec2.types.week_day


class InstanceEventWindowTimeRangeRequest(TypedDict, closed=True):
    start_week_day: NotRequired["capo_ec2.types.week_day.WeekDay"]
    """<p>The day on which the time range begins.</p>"""
    start_hour: NotRequired["capo_ec2.types.hour.Hour"]
    """<p>The hour when the time range begins.</p>"""
    end_week_day: NotRequired["capo_ec2.types.week_day.WeekDay"]
    """<p>The day on which the time range ends.</p>"""
    end_hour: NotRequired["capo_ec2.types.hour.Hour"]
    """<p>The hour when the time range ends.</p>"""


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: InstanceEventWindowTimeRangeRequest,
...
def deserialize_ec2_query(el: Element) -> InstanceEventWindowTimeRangeRequest:
    out: InstanceEventWindowTimeRangeRequest = {}  # type: ignore[typeddict-item]
