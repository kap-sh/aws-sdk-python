...
if TYPE_CHECKING:
    ...
    import capo_ec2.types.describe_capacity_manager_data_exports_request_max_results
    import capo_ec2.types.filter_list
    import capo_ec2.types.string


class DescribeCapacityManagerDataExportsRequest(TypedDict, closed=True):
    capacity_manager_data_export_ids: NotRequired[
        "capo_ec2.types.capacity_manager_data_export_id_set.CapacityManagerDataExportIdSet"
    ]
    """<p> The IDs of the data export configurations to describe. If not specified, all export configurations are returned. </p>"""
    max_results: NotRequired[
        "capo_ec2.types.describe_capacity_manager_data_exports_request_max_results.DescribeCapacityManagerDataExportsRequestMaxResults"
    ]
...
def deserialize_ec2_query(el: Element) -> DescribeCapacityManagerDataExportsRequest:
    ...
    child_capacity_manager_data_export_ids = el.find("CapacityManagerDataExportId")
    if child_capacity_manager_data_export_ids is not None:
        import capo_ec2.types.capacity_manager_data_export_id_set

        out["capacity_manager_data_export_ids"] = (
            capo_ec2.types.capacity_manager_data_export_id_set.deserialize_ec2_query(
                child_capacity_manager_data_export_ids
            )
        )
