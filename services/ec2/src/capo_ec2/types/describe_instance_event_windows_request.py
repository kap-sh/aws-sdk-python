...
if TYPE_CHECKING:
    ...
    import capo_ec2.types.result_range
    import capo_ec2.types.string


class DescribeInstanceEventWindowsRequest(TypedDict, closed=True):
    dry_run: NotRequired["capo_ec2.types.boolean.Boolean"]
    """<p>Checks whether you have the required permissions for the action, without actually making the request, and provides an error response. If you have the required permissions, the error response is <code>DryRunOperation</code>. Otherwise, it is <code>UnauthorizedOperation</code>.</p>"""
    instance_event_window_ids: NotRequired[
        "capo_ec2.types.instance_event_window_id_set.InstanceEventWindowIdSet"
    ]
    ...
    max_results: NotRequired["capo_ec2.types.result_range.ResultRange"]
...
def serialize_ec2_query(
    ...
    if "instance_event_window_ids" in value:
        ...
        capo_ec2.types.instance_event_window_id_set.serialize_ec2_query(
...
def deserialize_ec2_query(el: Element) -> DescribeInstanceEventWindowsRequest:
    ...
    child_instance_event_window_ids = el.find("InstanceEventWindowId")
    if child_instance_event_window_ids is not None:
        import capo_ec2.types.instance_event_window_id_set

        out["instance_event_window_ids"] = (
            capo_ec2.types.instance_event_window_id_set.deserialize_ec2_query(
                child_instance_event_window_ids
            )
        )
