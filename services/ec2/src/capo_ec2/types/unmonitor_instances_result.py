"""Generated from Smithy shape ``com.amazonaws.ec2#UnmonitorInstancesResult``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import capo_ec2.types.instance_monitoring_list


class UnmonitorInstancesResult(TypedDict, closed=True):
    instance_monitorings: NotRequired[
        "capo_ec2.types.instance_monitoring_list.InstanceMonitoringList"
    ]
    """<p>The monitoring information.</p>"""


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: UnmonitorInstancesResult, pairs: list[tuple[str, str]], prefix: str
) -> None:
    key_prefix = f"{prefix}." if prefix else ""
    if "instance_monitorings" in value:
        import capo_ec2.types.instance_monitoring_list

        capo_ec2.types.instance_monitoring_list.serialize_ec2_query(
            value["instance_monitorings"], pairs, f"{key_prefix}InstancesSet"
        )


def deserialize_ec2_query(el: Element) -> UnmonitorInstancesResult:
    out: UnmonitorInstancesResult = {}  # type: ignore[typeddict-item]
    child_instance_monitorings = el.find("instancesSet")
    if child_instance_monitorings is not None:
        import capo_ec2.types.instance_monitoring_list

        out["instance_monitorings"] = (
            capo_ec2.types.instance_monitoring_list.deserialize_ec2_query(
                child_instance_monitorings
            )
        )
    return out
