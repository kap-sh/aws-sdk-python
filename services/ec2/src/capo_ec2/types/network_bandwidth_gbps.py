...
class NetworkBandwidthGbps(TypedDict, closed=True):
...
def serialize_ec2_query(
    value: NetworkBandwidthGbps, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "min" in value:
...
def deserialize_ec2_query(el: Element) -> NetworkBandwidthGbps:
    out: NetworkBandwidthGbps = {}  # type: ignore[typeddict-item]
