...
class DescribeIpamResourceDiscoveriesRequest(TypedDict, closed=True):
...
def serialize_ec2_query(
    value: DescribeIpamResourceDiscoveriesRequest,
    ...
    if "ipam_resource_discovery_ids" in value:
        import capo_ec2.types.value_string_list

        capo_ec2.types.value_string_list.serialize_ec2_query(
            value["ipam_resource_discovery_ids"],
            pairs,
...
def deserialize_ec2_query(el: Element) -> DescribeIpamResourceDiscoveriesRequest:
    out: DescribeIpamResourceDiscoveriesRequest = {}  # type: ignore[typeddict-item]
    child_dry_run = el.find("DryRun")
    if child_dry_run is not None:
        out["dry_run"] = (child_dry_run.text or "").lower() == "true"
    child_ipam_resource_discovery_ids = el.find("IpamResourceDiscoveryId")
    if child_ipam_resource_discovery_ids is not None:
        import capo_ec2.types.value_string_list

        out["ipam_resource_discovery_ids"] = (
            capo_ec2.types.value_string_list.deserialize_ec2_query(
                child_ipam_resource_discovery_ids
            )
        )
