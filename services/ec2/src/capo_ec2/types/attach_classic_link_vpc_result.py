...
def serialize_ec2_query(
    value: AttachClassicLinkVpcResult, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "return" in value:
        pairs.append((f"{prefix}.Return", "true" if value["return"] else "false"))


def deserialize_ec2_query(el: Element) -> AttachClassicLinkVpcResult:
    out: AttachClassicLinkVpcResult = {}  # type: ignore[typeddict-item]
