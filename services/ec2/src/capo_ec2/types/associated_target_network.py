...
class AssociatedTargetNetwork(TypedDict, closed=True):
    network_id: NotRequired["capo_ec2.types.string.String"]
    """<p>The ID of the subnet.</p>"""
    network_type: NotRequired[
        "capo_ec2.types.associated_network_type.AssociatedNetworkType"
    ]
...
def deserialize_ec2_query(el: Element) -> AssociatedTargetNetwork:
    ...
    if child_network_type is not None:
        ...
        out["network_type"] = (
            capo_ec2.types.associated_network_type.deserialize_ec2_query(
                child_network_type
            )
        )
    return out
