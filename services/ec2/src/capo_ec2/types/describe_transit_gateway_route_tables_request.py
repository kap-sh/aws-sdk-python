...
if TYPE_CHECKING:
    ...
    import capo_ec2.types.transit_gateway_route_table_id_string_list


class DescribeTransitGatewayRouteTablesRequest(TypedDict, closed=True):
    transit_gateway_route_table_ids: NotRequired[
        "capo_ec2.types.transit_gateway_route_table_id_string_list.TransitGatewayRouteTableIdStringList"
    ]
...
def serialize_ec2_query(
    value: DescribeTransitGatewayRouteTablesRequest,
    pairs: list[tuple[str, str]],
    prefix: str,
) -> None:
    key_prefix = f"{prefix}." if prefix else ""
    if "transit_gateway_route_table_ids" in value:
        import capo_ec2.types.transit_gateway_route_table_id_string_list

        capo_ec2.types.transit_gateway_route_table_id_string_list.serialize_ec2_query(
            value["transit_gateway_route_table_ids"],
            pairs,
            f"{key_prefix}TransitGatewayRouteTableIds",
        )
    if "filters" in value:
        import capo_ec2.types.filter_list
...
def deserialize_ec2_query(el: Element) -> DescribeTransitGatewayRouteTablesRequest:
    out: DescribeTransitGatewayRouteTablesRequest = {}  # type: ignore[typeddict-item]
    child_transit_gateway_route_table_ids = el.find("TransitGatewayRouteTableIds")
    if child_transit_gateway_route_table_ids is not None:
        import capo_ec2.types.transit_gateway_route_table_id_string_list

        out["transit_gateway_route_table_ids"] = (
            capo_ec2.types.transit_gateway_route_table_id_string_list.deserialize_ec2_query(
                child_transit_gateway_route_table_ids
            )
        )
