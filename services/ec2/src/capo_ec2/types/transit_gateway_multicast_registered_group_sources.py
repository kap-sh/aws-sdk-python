...
class TransitGatewayMulticastRegisteredGroupSources(TypedDict, closed=True):
...
def serialize_ec2_query(
    value: TransitGatewayMulticastRegisteredGroupSources,
    ...
    if "registered_network_interface_ids" in value:
        ...
        capo_ec2.types.value_string_list.serialize_ec2_query(
            ...
            pairs,
            f"{key_prefix}RegisteredNetworkInterfaceIds",
        )
    if "group_ip_address" in value:
        pairs.append((f"{key_prefix}GroupIpAddress", str(value["group_ip_address"])))


def deserialize_ec2_query(el: Element) -> TransitGatewayMulticastRegisteredGroupSources:
    out: TransitGatewayMulticastRegisteredGroupSources = {}  # type: ignore[typeddict-item]
