...
class EnableFastSnapshotRestoreStateErrorItem(TypedDict, closed=True):
    ...
    error: NotRequired[
        "capo_ec2.types.enable_fast_snapshot_restore_state_error.EnableFastSnapshotRestoreStateError"
    ]
    """<p>The error.</p>"""
...
def deserialize_ec2_query(el: Element) -> EnableFastSnapshotRestoreStateErrorItem:
    ...
    if child_error is not None:
        ...
        out["error"] = (
            capo_ec2.types.enable_fast_snapshot_restore_state_error.deserialize_ec2_query(
                child_error
            )
        )
    return out
