...
class CreateLaunchTemplateResult(TypedDict, closed=True):
    launch_template: NotRequired["capo_ec2.types.launch_template.LaunchTemplate"]
    """<p>Information about the launch template.</p>"""
    warning: NotRequired["capo_ec2.types.validation_warning.ValidationWarning"]
...
def deserialize_ec2_query(el: Element) -> CreateLaunchTemplateResult:
    ...
    if child_warning is not None:
        ...
        out["warning"] = capo_ec2.types.validation_warning.deserialize_ec2_query(
            child_warning
        )
    return out
