...
import capo_ec2.types.key_pair
...
def handle_response(response: zapros.Response) -> capo_ec2.types.key_pair.KeyPair:
    out: capo_ec2.types.key_pair.KeyPair = (
        capo_ec2.types.key_pair.deserialize_ec2_query(fromstring(response.read()))
    )
    return out
...
) -> capo_ec2.types.key_pair.KeyPair:
    out: capo_ec2.types.key_pair.KeyPair = (
        capo_ec2.types.key_pair.deserialize_ec2_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateKeyPair"))
    pairs.append(("Version", "2016-11-15"))
    capo_ec2.types.create_key_pair_request.serialize_ec2_query(input_, pairs, "")
...
def create_key_pair(
    ...
    input_: capo_ec2.types.create_key_pair_request.CreateKeyPairRequest,
) -> tuple[capo_ec2.types.key_pair.KeyPair, zapros.Response]:
