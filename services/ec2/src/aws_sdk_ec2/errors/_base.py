...
class EC2Error(Exception):
    """Base class for all EC2 service errors."""


class ServiceError(EC2Error):
...
class SerializationError(EC2Error):
    """Raised when a value cannot be serialized."""


class DeserializationError(EC2Error):
...
class WaiterFailedError(EC2Error):
...
class WaiterTimeoutError(EC2Error):
