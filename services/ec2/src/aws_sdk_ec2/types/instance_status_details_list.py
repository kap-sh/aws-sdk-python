...
def serialize_ec2_query(
    ...
    for n, item in enumerate(value, 1):
        ...
        aws_sdk_ec2.types.instance_status_details.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> InstanceStatusDetailsList:
    import aws_sdk_ec2.types.instance_status_details

    out: InstanceStatusDetailsList = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_ec2.types.instance_status_details.deserialize_ec2_query(child)
        )
    return out
