...
def serialize_ec2_query(
    value: ModifyVpcEndpointResult, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "return" in value:
        pairs.append((f"{prefix}.Return", "true" if value["return"] else "false"))


def deserialize_ec2_query(el: Element) -> ModifyVpcEndpointResult:
    out: ModifyVpcEndpointResult = {}  # type: ignore[typeddict-item]
