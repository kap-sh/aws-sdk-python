...
def serialize_ec2_query(
    ...
    if "fast_restored" in value:
        pairs.append(
            (f"{prefix}.FastRestored", "true" if value["fast_restored"] else "false")
        )
    ...
    if "attachments" in value:
        ...
        aws_sdk_ec2.types.volume_attachment_list.serialize_ec2_query(
...
def deserialize_ec2_query(el: Element) -> Volume:
    ...
    if el.find("AttachmentSet") is not None:
        ...
        out["attachments"] = (
            aws_sdk_ec2.types.volume_attachment_list.deserialize_ec2_query(
                el, "AttachmentSet"
            )
        )
