"""Generated from Smithy shape ``com.amazonaws.ec2#HostList``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_ec2.types.host

HostList: TypeAlias = list["aws_sdk_ec2.types.host.Host"]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: HostList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    for n, item in enumerate(value, 1):
        import aws_sdk_ec2.types.host

        aws_sdk_ec2.types.host.serialize_ec2_query(item, pairs, f"{prefix}.{n}")


def deserialize_ec2_query(parent: Element, tag: str) -> HostList:
    import aws_sdk_ec2.types.host

    out: HostList = []
    for child in parent.findall(tag):
        out.append(aws_sdk_ec2.types.host.deserialize_ec2_query(child))
    return out
