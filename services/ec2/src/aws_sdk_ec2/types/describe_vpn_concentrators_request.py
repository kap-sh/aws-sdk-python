...
class DescribeVpnConcentratorsRequest(TypedDict, closed=True):
...
def serialize_ec2_query(
    value: DescribeVpnConcentratorsRequest, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "vpn_concentrator_ids" in value:
        import aws_sdk_ec2.types.vpn_concentrator_id_string_list

        aws_sdk_ec2.types.vpn_concentrator_id_string_list.serialize_ec2_query(
            value["vpn_concentrator_ids"], pairs, f"{prefix}.VpnConcentratorIds"
        )
    if "filters" in value:
        import aws_sdk_ec2.types.filter_list
...
def deserialize_ec2_query(el: Element) -> DescribeVpnConcentratorsRequest:
    ...
    if el.find("VpnConcentratorIds") is not None:
        ...
        out["vpn_concentrator_ids"] = (
            aws_sdk_ec2.types.vpn_concentrator_id_string_list.deserialize_ec2_query(
