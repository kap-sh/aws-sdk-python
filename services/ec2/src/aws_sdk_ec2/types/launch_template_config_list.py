...
def serialize_ec2_query(
    ...
    for n, item in enumerate(value, 1):
        ...
        aws_sdk_ec2.types.launch_template_config.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> LaunchTemplateConfigList:
    import aws_sdk_ec2.types.launch_template_config

    out: LaunchTemplateConfigList = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_ec2.types.launch_template_config.deserialize_ec2_query(child)
        )
    return out
