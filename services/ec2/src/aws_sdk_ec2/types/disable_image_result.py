...
def serialize_ec2_query(
    value: DisableImageResult, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "return" in value:
        pairs.append((f"{prefix}.Return", "true" if value["return"] else "false"))


def deserialize_ec2_query(el: Element) -> DisableImageResult:
    out: DisableImageResult = {}  # type: ignore[typeddict-item]
