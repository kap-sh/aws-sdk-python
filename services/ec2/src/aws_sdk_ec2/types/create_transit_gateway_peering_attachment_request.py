...
def serialize_ec2_query(
    ...
    if "peer_account_id" in value:
        pairs.append((f"{prefix}.PeerAccountId", str(value["peer_account_id"])))
    if "peer_region" in value:
        pairs.append((f"{prefix}.PeerRegion", str(value["peer_region"])))
    if "options" in value:
        import aws_sdk_ec2.types.create_transit_gateway_peering_attachment_request_options

        aws_sdk_ec2.types.create_transit_gateway_peering_attachment_request_options.serialize_ec2_query(
...
def deserialize_ec2_query(el: Element) -> CreateTransitGatewayPeeringAttachmentRequest:
    ...
    if child_options is not None:
        ...
        out["options"] = (
            aws_sdk_ec2.types.create_transit_gateway_peering_attachment_request_options.deserialize_ec2_query(
                child_options
            )
        )
