"""Generated from Smithy shape ``com.amazonaws.ec2#VolumeList``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_ec2.types.volume

VolumeList: TypeAlias = list["aws_sdk_ec2.types.volume.Volume"]


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: VolumeList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    for n, item in enumerate(value, 1):
        import aws_sdk_ec2.types.volume

        aws_sdk_ec2.types.volume.serialize_ec2_query(item, pairs, f"{prefix}.{n}")


def deserialize_ec2_query(parent: Element, tag: str) -> VolumeList:
    import aws_sdk_ec2.types.volume

    out: VolumeList = []
    for child in parent.findall(tag):
        out.append(aws_sdk_ec2.types.volume.deserialize_ec2_query(child))
    return out
