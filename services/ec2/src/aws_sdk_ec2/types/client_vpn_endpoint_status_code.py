"""Generated from Smithy shape ``com.amazonaws.ec2#ClientVpnEndpointStatusCode``."""

from typing import Literal, TypeAlias, cast
from aws_sdk_ec2.errors import DeserializationError
from aws_sdk_ec2._protocol.xml import Element

ClientVpnEndpointStatusCode: TypeAlias = Literal[
    "pending-associate",
    "available",
    "deleting",
    "deleted",
    "pending",
]


# --- ec2Query ser/de ---
_VALUES: frozenset[str] = frozenset(
    (
        "pending-associate",
        "available",
        "deleting",
        "deleted",
        "pending",
    )
)


_VALUES: frozenset[str] = frozenset(
    (
        "pending-associate",
        "available",
        "deleting",
        "deleted",
        "pending",
    )
)


def to_ec2_query_text(value: ClientVpnEndpointStatusCode) -> str:
    return value


def from_ec2_query_text(text: str) -> ClientVpnEndpointStatusCode:
    if text not in _VALUES:
        raise DeserializationError(
            f"unknown ClientVpnEndpointStatusCode value: {text!r}"
        )
    return cast(ClientVpnEndpointStatusCode, text)


def serialize_ec2_query(
    value: ClientVpnEndpointStatusCode, pairs: list[tuple[str, str]], prefix: str
) -> None:
    pairs.append((prefix, to_ec2_query_text(value)))


def deserialize_ec2_query(el: Element) -> ClientVpnEndpointStatusCode:
    return from_ec2_query_text(el.text or "")
