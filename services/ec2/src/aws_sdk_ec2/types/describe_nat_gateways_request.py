...
class DescribeNatGatewaysRequest(TypedDict, closed=True):
...
def serialize_ec2_query(
    value: DescribeNatGatewaysRequest, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "dry_run" in value:
    ...
    if "nat_gateway_ids" in value:
        import aws_sdk_ec2.types.nat_gateway_id_string_list

        aws_sdk_ec2.types.nat_gateway_id_string_list.serialize_ec2_query(
            value["nat_gateway_ids"], pairs, f"{prefix}.NatGatewayIds"
        )
    if "next_token" in value:
        pairs.append((f"{prefix}.NextToken", str(value["next_token"])))


def deserialize_ec2_query(el: Element) -> DescribeNatGatewaysRequest:
    out: DescribeNatGatewaysRequest = {}  # type: ignore[typeddict-item]
    ...
    if el.find("NatGatewayIds") is not None:
        ...
        out["nat_gateway_ids"] = (
            aws_sdk_ec2.types.nat_gateway_id_string_list.deserialize_ec2_query(
                el, "NatGatewayIds"
            )
        )
