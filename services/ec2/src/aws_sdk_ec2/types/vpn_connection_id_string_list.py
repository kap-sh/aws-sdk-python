...
def serialize_ec2_query(
    value: VpnConnectionIdStringList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    for n, item in enumerate(value, 1):
        pairs.append((f"{prefix}.{n}", str(item)))


def deserialize_ec2_query(parent: Element, tag: str) -> VpnConnectionIdStringList:
    out: VpnConnectionIdStringList = []
