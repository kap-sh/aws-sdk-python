...
def serialize_ec2_query(
    ...
    for n, item in enumerate(value, 1):
        ...
        aws_sdk_ec2.types.create_fleet_instance.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> CreateFleetInstancesSet:
    import aws_sdk_ec2.types.create_fleet_instance

    out: CreateFleetInstancesSet = []
    for child in parent.findall(tag):
        out.append(aws_sdk_ec2.types.create_fleet_instance.deserialize_ec2_query(child))
    return out
