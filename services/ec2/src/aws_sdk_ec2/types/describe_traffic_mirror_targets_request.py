...
def serialize_ec2_query(
    ...
    if "traffic_mirror_target_ids" in value:
        ...
        aws_sdk_ec2.types.traffic_mirror_target_id_list.serialize_ec2_query(
            value["traffic_mirror_target_ids"],
            pairs,
            f"{prefix}.TrafficMirrorTargetIds",
        )
    if "dry_run" in value:
...
def deserialize_ec2_query(el: Element) -> DescribeTrafficMirrorTargetsRequest:
    ...
    if el.find("TrafficMirrorTargetIds") is not None:
        import aws_sdk_ec2.types.traffic_mirror_target_id_list

        out["traffic_mirror_target_ids"] = (
            aws_sdk_ec2.types.traffic_mirror_target_id_list.deserialize_ec2_query(
                el, "TrafficMirrorTargetIds"
            )
        )
