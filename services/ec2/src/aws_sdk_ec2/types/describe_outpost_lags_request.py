...
def serialize_ec2_query(
    value: DescribeOutpostLagsRequest, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "outpost_lag_ids" in value:
        import aws_sdk_ec2.types.outpost_lag_id_set

        aws_sdk_ec2.types.outpost_lag_id_set.serialize_ec2_query(
            value["outpost_lag_ids"], pairs, f"{prefix}.OutpostLagIds"
        )
    if "filters" in value:
        import aws_sdk_ec2.types.filter_list
...
def deserialize_ec2_query(el: Element) -> DescribeOutpostLagsRequest:
    ...
    if el.find("OutpostLagIds") is not None:
        import aws_sdk_ec2.types.outpost_lag_id_set

        out["outpost_lag_ids"] = (
            aws_sdk_ec2.types.outpost_lag_id_set.deserialize_ec2_query(
                el, "OutpostLagIds"
            )
        )
