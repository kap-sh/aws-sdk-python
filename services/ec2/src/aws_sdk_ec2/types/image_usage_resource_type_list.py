...
def serialize_ec2_query(
    ...
    for n, item in enumerate(value, 1):
        ...
        aws_sdk_ec2.types.image_usage_resource_type.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> ImageUsageResourceTypeList:
    import aws_sdk_ec2.types.image_usage_resource_type

    out: ImageUsageResourceTypeList = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_ec2.types.image_usage_resource_type.deserialize_ec2_query(child)
        )
    return out
