...
class IpamPolicyAllocationRuleRequest(TypedDict, closed=True):
...
def serialize_ec2_query(
    value: IpamPolicyAllocationRuleRequest, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "source_ipam_pool_id" in value:
        pairs.append((f"{prefix}.SourceIpamPoolId", str(value["source_ipam_pool_id"])))


def deserialize_ec2_query(el: Element) -> IpamPolicyAllocationRuleRequest:
    out: IpamPolicyAllocationRuleRequest = {}  # type: ignore[typeddict-item]
