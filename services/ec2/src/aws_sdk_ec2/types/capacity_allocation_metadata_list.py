...
def serialize_ec2_query(
    ...
    for n, item in enumerate(value, 1):
        ...
        aws_sdk_ec2.types.capacity_allocation_metadata_entry.serialize_ec2_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_ec2_query(parent: Element, tag: str) -> CapacityAllocationMetadataList:
    import aws_sdk_ec2.types.capacity_allocation_metadata_entry

    out: CapacityAllocationMetadataList = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_ec2.types.capacity_allocation_metadata_entry.deserialize_ec2_query(
                child
            )
        )
    return out
