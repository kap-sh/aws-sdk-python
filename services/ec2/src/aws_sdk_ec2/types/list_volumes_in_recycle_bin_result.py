...
class ListVolumesInRecycleBinResult(TypedDict, closed=True):
    volumes: NotRequired[
...
def serialize_ec2_query(
    ...
    if "volumes" in value:
        ...
        aws_sdk_ec2.types.volume_recycle_bin_info_list.serialize_ec2_query(
...
def deserialize_ec2_query(el: Element) -> ListVolumesInRecycleBinResult:
    out: ListVolumesInRecycleBinResult = {}  # type: ignore[typeddict-item]
    if el.find("VolumeSet") is not None:
        import aws_sdk_ec2.types.volume_recycle_bin_info_list

        out["volumes"] = (
            aws_sdk_ec2.types.volume_recycle_bin_info_list.deserialize_ec2_query(
                el, "VolumeSet"
            )
        )
