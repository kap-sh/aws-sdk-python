...
def serialize_ec2_query(
    ...
    if "bundle_ids" in value:
        ...
        aws_sdk_ec2.types.bundle_id_string_list.serialize_ec2_query(
            value["bundle_ids"], pairs, f"{prefix}.BundleIds"
        )
    if "dry_run" in value:
...
def deserialize_ec2_query(el: Element) -> DescribeBundleTasksRequest:
    ...
    if el.find("BundleIds") is not None:
        ...
        out["bundle_ids"] = (
            aws_sdk_ec2.types.bundle_id_string_list.deserialize_ec2_query(
                el, "BundleIds"
            )
        )
