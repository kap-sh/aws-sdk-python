"""Generated from Smithy shape ``com.amazonaws.ec2#DescribeVerifiedAccessInstanceLoggingConfigurationsResult``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

from aws_sdk_ec2._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_ec2.types.next_token
    import aws_sdk_ec2.types.verified_access_instance_logging_configuration_list


class DescribeVerifiedAccessInstanceLoggingConfigurationsResult(TypedDict):
    logging_configurations: NotRequired[
        "aws_sdk_ec2.types.verified_access_instance_logging_configuration_list.VerifiedAccessInstanceLoggingConfigurationList"
    ]
    """<p>The logging configuration for the Verified Access instances.</p>"""
    next_token: NotRequired["aws_sdk_ec2.types.next_token.NextToken"]
    """<p>The token to use to retrieve the next page of results. This value is <code>null</code> when there are no more results to return.</p>"""


# --- ec2Query ser/de ---
def serialize_ec2_query(
    value: DescribeVerifiedAccessInstanceLoggingConfigurationsResult,
    pairs: list[tuple[str, str]],
    prefix: str,
) -> None:
    if "logging_configurations" in value:
        import aws_sdk_ec2.types.verified_access_instance_logging_configuration_list

        aws_sdk_ec2.types.verified_access_instance_logging_configuration_list.serialize_ec2_query(
            value["logging_configurations"], pairs, f"{prefix}.LoggingConfigurationSet"
        )
    if "next_token" in value:
        pairs.append((f"{prefix}.NextToken", str(value["next_token"])))


def deserialize_ec2_query(
    el: Element,
) -> DescribeVerifiedAccessInstanceLoggingConfigurationsResult:
    out: DescribeVerifiedAccessInstanceLoggingConfigurationsResult = {}  # type: ignore[typeddict-item]
    if el.find("LoggingConfigurationSet") is not None:
        import aws_sdk_ec2.types.verified_access_instance_logging_configuration_list

        out["logging_configurations"] = (
            aws_sdk_ec2.types.verified_access_instance_logging_configuration_list.deserialize_ec2_query(
                el, "LoggingConfigurationSet"
            )
        )
    child_next_token = el.find("NextToken")
    if child_next_token is not None:
        out["next_token"] = str(child_next_token.text or "")
    return out
