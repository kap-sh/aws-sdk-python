...
if TYPE_CHECKING:
    ...
    import aws_sdk_emr_containers.types.template_parameter_data_type


class TemplateParameterConfiguration(TypedDict):
    type: NotRequired[
        "aws_sdk_emr_containers.types.template_parameter_data_type.TemplateParameterDataType"
    ]
    """<p>The type of the job template parameter. Allowed values are: ‘STRING’, ‘NUMBER’.</p>"""
    default_value: NotRequired["aws_sdk_emr_containers.types.string1024.String1024"]
...
def serialize_json(value: TemplateParameterConfiguration) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = (
            aws_sdk_emr_containers.types.template_parameter_data_type.serialize_json(
                value["type"]
            )
        )
...
def deserialize_json(data: dict) -> TemplateParameterConfiguration:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_emr_containers.types.template_parameter_data_type.deserialize_json(
                data["type"]
            )
        )
