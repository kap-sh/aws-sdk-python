...
class FMSError(Exception):
    """Base class for all FMS service errors."""


class ServiceError(FMSError):
...
class SerializationError(FMSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(FMSError):
...
class WaiterFailedError(FMSError):
...
class WaiterTimeoutError(FMSError):
