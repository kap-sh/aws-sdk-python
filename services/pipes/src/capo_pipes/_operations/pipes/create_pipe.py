"""Generated from Smithy shape ``com.amazonaws.pipes#CreatePipe``."""
...
import capo_pipes.errors.conflict_exception
import capo_pipes.errors.internal_exception
import capo_pipes.errors.not_found_exception
import capo_pipes.errors.service_quota_exceeded_exception
...
from capo_pipes._services._pipeline import AsyncOperationOptions, OperationOptions
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ConflictException":
            raise capo_pipes.errors.conflict_exception.ConflictException.from_json(data)
        case "InternalException":
            raise capo_pipes.errors.internal_exception.InternalException.from_json(data)
        case "NotFoundException":
            raise capo_pipes.errors.not_found_exception.NotFoundException.from_json(
                data
            )
        case "ServiceQuotaExceededException":
            raise capo_pipes.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_pipes.types.create_pipe_response.CreatePipeResponse = (
        capo_pipes.types.create_pipe_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_pipes._auth._sigv4.build_sigv4_auth_scheme("pipes", options.region)
        )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/pipes/{Name}"
    url = url.replace("{Name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_pipes.types.create_pipe_request.serialize_json(input_)
    ).encode()
...
def create_pipe(
    ...
    input_: capo_pipes.types.create_pipe_request.CreatePipeRequest,
) -> tuple[capo_pipes.types.create_pipe_response.CreatePipeResponse, zapros.Response]:
