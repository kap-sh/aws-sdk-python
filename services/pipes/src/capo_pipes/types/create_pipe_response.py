"""Generated from Smithy shape ``com.amazonaws.pipes#CreatePipeResponse``."""
...
class CreatePipeResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: CreatePipeResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["Arn"] = value["arn"]
    if "name" in value:
...
def deserialize_json(data: dict) -> CreatePipeResponse:
    out: CreatePipeResponse = {}  # type: ignore[typeddict-item]
    if "Arn" in data:
        out["arn"] = data["Arn"]
    if "Name" in data:
        out["name"] = data["Name"]
