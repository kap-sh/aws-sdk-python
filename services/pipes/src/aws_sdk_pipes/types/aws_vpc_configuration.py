...
if TYPE_CHECKING:
    import aws_sdk_pipes.types.assign_public_ip
    import aws_sdk_pipes.types.security_groups
...
class AwsVpcConfiguration(TypedDict, closed=True):
    ...
    assign_public_ip: NotRequired["aws_sdk_pipes.types.assign_public_ip.AssignPublicIp"]
...
def serialize_json(value: AwsVpcConfiguration) -> dict:
    ...
    out["Subnets"] = aws_sdk_pipes.types.subnets.serialize_json(value["subnets"])
    if "security_groups" in value:
        import aws_sdk_pipes.types.security_groups

        out["SecurityGroups"] = aws_sdk_pipes.types.security_groups.serialize_json(
...
def deserialize_json(data: dict) -> AwsVpcConfiguration:
    ...
    if "Subnets" in data:
        ...
        out["subnets"] = aws_sdk_pipes.types.subnets.deserialize_json(data["Subnets"])
    else:
        raise DeserializationError("AwsVpcConfiguration.subnets required")
    if "SecurityGroups" in data:
        import aws_sdk_pipes.types.security_groups

        out["security_groups"] = aws_sdk_pipes.types.security_groups.deserialize_json(
            data["SecurityGroups"]
        )
