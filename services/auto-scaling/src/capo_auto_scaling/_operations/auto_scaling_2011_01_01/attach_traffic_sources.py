"""Generated from Smithy shape ``com.amazonaws.autoscaling#AttachTrafficSources``."""
...
import capo_auto_scaling.errors.instance_refresh_in_progress_fault
import capo_auto_scaling.errors.resource_contention_fault
import capo_auto_scaling.errors.service_linked_role_failure
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "InstanceRefreshInProgressFault":
            raise capo_auto_scaling.errors.instance_refresh_in_progress_fault.InstanceRefreshInProgressFault.from_query(
                root
            )
        ...
        case "ServiceLinkedRoleFailure":
            raise capo_auto_scaling.errors.service_linked_role_failure.ServiceLinkedRoleFailure.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("AttachTrafficSourcesResult")
    out: capo_auto_scaling.types.attach_traffic_sources_result_type.AttachTrafficSourcesResultType = capo_auto_scaling.types.attach_traffic_sources_result_type.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "AttachTrafficSources"))
    pairs.append(("Version", "2011-01-01"))
    capo_auto_scaling.types.attach_traffic_sources_type.serialize_query(
        input_, pairs, ""
    )
...
async def async_attach_traffic_sources(
