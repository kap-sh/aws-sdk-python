"""Generated from Smithy shape ``com.amazonaws.autoscaling#CompleteLifecycleActionType``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_auto_scaling._protocol.xml import Element

if TYPE_CHECKING:
    import capo_auto_scaling.types.ascii_string_max_len255
    import capo_auto_scaling.types.lifecycle_action_result
    import capo_auto_scaling.types.lifecycle_action_token
    import capo_auto_scaling.types.resource_name
    import capo_auto_scaling.types.xml_string_max_len19


class CompleteLifecycleActionType(TypedDict, closed=True):
    lifecycle_hook_name: NotRequired[
        "capo_auto_scaling.types.ascii_string_max_len255.AsciiStringMaxLen255"
    ]
    """<p>The name of the lifecycle hook.</p>"""
    auto_scaling_group_name: NotRequired[
        "capo_auto_scaling.types.resource_name.ResourceName"
    ]
    """<p>The name of the Auto Scaling group.</p>"""
    lifecycle_action_token: NotRequired[
        "capo_auto_scaling.types.lifecycle_action_token.LifecycleActionToken"
    ]
    """<p>A universally unique identifier (UUID) that identifies a specific lifecycle action associated with an instance. Amazon EC2 Auto Scaling sends this token to the notification target you specified when you created the lifecycle hook.</p>"""
    lifecycle_action_result: NotRequired[
        "capo_auto_scaling.types.lifecycle_action_result.LifecycleActionResult"
    ]
    """<p>The action for the group to take. You can specify either <code>CONTINUE</code> or <code>ABANDON</code>.</p>"""
    instance_id: NotRequired[
        "capo_auto_scaling.types.xml_string_max_len19.XmlStringMaxLen19"
    ]
    """<p>The ID of the instance.</p>"""


# --- awsQuery ser/de ---
def serialize_query(
    value: CompleteLifecycleActionType, pairs: list[tuple[str, str]], prefix: str
) -> None:
    key_prefix = f"{prefix}." if prefix else ""
    if "lifecycle_hook_name" in value:
        pairs.append(
            (f"{key_prefix}LifecycleHookName", str(value["lifecycle_hook_name"]))
        )
    if "auto_scaling_group_name" in value:
        pairs.append(
            (f"{key_prefix}AutoScalingGroupName", str(value["auto_scaling_group_name"]))
        )
    if "lifecycle_action_token" in value:
        pairs.append(
            (f"{key_prefix}LifecycleActionToken", str(value["lifecycle_action_token"]))
        )
    if "lifecycle_action_result" in value:
        pairs.append(
            (
                f"{key_prefix}LifecycleActionResult",
                str(value["lifecycle_action_result"]),
            )
        )
    if "instance_id" in value:
        pairs.append((f"{key_prefix}InstanceId", str(value["instance_id"])))


def deserialize_query(el: Element) -> CompleteLifecycleActionType:
    out: CompleteLifecycleActionType = {}  # type: ignore[typeddict-item]
    child_lifecycle_hook_name = el.find("LifecycleHookName")
    if child_lifecycle_hook_name is not None:
        out["lifecycle_hook_name"] = str(child_lifecycle_hook_name.text or "")
    child_auto_scaling_group_name = el.find("AutoScalingGroupName")
    if child_auto_scaling_group_name is not None:
        out["auto_scaling_group_name"] = str(child_auto_scaling_group_name.text or "")
    child_lifecycle_action_token = el.find("LifecycleActionToken")
    if child_lifecycle_action_token is not None:
        out["lifecycle_action_token"] = str(child_lifecycle_action_token.text or "")
    child_lifecycle_action_result = el.find("LifecycleActionResult")
    if child_lifecycle_action_result is not None:
        out["lifecycle_action_result"] = str(child_lifecycle_action_result.text or "")
    child_instance_id = el.find("InstanceId")
    if child_instance_id is not None:
        out["instance_id"] = str(child_instance_id.text or "")
    return out
