...
AutoScalingGroups: TypeAlias = list[
    "capo_auto_scaling.types.auto_scaling_group.AutoScalingGroup"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.auto_scaling_group.serialize_query(
...
def deserialize_query(el: Element) -> AutoScalingGroups:
    ...
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.auto_scaling_group.deserialize_query(child))
    return out
