...
if TYPE_CHECKING:
    import capo_auto_scaling.types.traffic_source_state

TrafficSourceStates: TypeAlias = list[
    "capo_auto_scaling.types.traffic_source_state.TrafficSourceState"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.traffic_source_state.serialize_query(
...
def deserialize_query(el: Element) -> TrafficSourceStates:
    ...
    for child in el.findall("member"):
        out.append(
            capo_auto_scaling.types.traffic_source_state.deserialize_query(child)
        )
    return out
