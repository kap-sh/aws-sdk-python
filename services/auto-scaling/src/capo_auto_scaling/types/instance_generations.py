...
if TYPE_CHECKING:
    import capo_auto_scaling.types.instance_generation

InstanceGenerations: TypeAlias = list[
    "capo_auto_scaling.types.instance_generation.InstanceGeneration"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.instance_generation.serialize_query(
...
def deserialize_query(el: Element) -> InstanceGenerations:
    ...
    for child in el.findall("member"):
        out.append(capo_auto_scaling.types.instance_generation.deserialize_query(child))
    return out
