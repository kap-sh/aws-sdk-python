...
class DescribeLoadBalancerTargetGroupsResponse(TypedDict, closed=True):
    load_balancer_target_groups: NotRequired[
        "capo_auto_scaling.types.load_balancer_target_group_states.LoadBalancerTargetGroupStates"
    ]
...
def deserialize_query(el: Element) -> DescribeLoadBalancerTargetGroupsResponse:
    ...
    if child_load_balancer_target_groups is not None:
        ...
        out["load_balancer_target_groups"] = (
            capo_auto_scaling.types.load_balancer_target_group_states.deserialize_query(
