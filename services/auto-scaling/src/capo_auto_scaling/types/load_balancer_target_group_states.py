...
if TYPE_CHECKING:
    import capo_auto_scaling.types.load_balancer_target_group_state

LoadBalancerTargetGroupStates: TypeAlias = list[
    "capo_auto_scaling.types.load_balancer_target_group_state.LoadBalancerTargetGroupState"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_auto_scaling.types.load_balancer_target_group_state.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerTargetGroupStates:
    ...
    for child in el.findall("member"):
        out.append(
            capo_auto_scaling.types.load_balancer_target_group_state.deserialize_query(
                child
            )
        )
    return out
