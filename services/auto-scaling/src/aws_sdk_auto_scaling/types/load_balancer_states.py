...
if TYPE_CHECKING:
    import aws_sdk_auto_scaling.types.load_balancer_state

LoadBalancerStates: TypeAlias = list[
    "aws_sdk_auto_scaling.types.load_balancer_state.LoadBalancerState"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_auto_scaling.types.load_balancer_state.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerStates:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_auto_scaling.types.load_balancer_state.deserialize_query(child)
        )
    return out
