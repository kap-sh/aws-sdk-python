...
if TYPE_CHECKING:
    import aws_sdk_auto_scaling.types.instance_collection

InstanceCollections: TypeAlias = list[
    "aws_sdk_auto_scaling.types.instance_collection.InstanceCollection"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_auto_scaling.types.instance_collection.serialize_query(
...
def deserialize_query(el: Element) -> InstanceCollections:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_auto_scaling.types.instance_collection.deserialize_query(child)
        )
    return out
