...
if TYPE_CHECKING:
    import aws_sdk_auto_scaling.types.launch_instances_error

LaunchInstancesErrors: TypeAlias = list[
    "aws_sdk_auto_scaling.types.launch_instances_error.LaunchInstancesError"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_auto_scaling.types.launch_instances_error.serialize_query(
...
def deserialize_query(el: Element) -> LaunchInstancesErrors:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_auto_scaling.types.launch_instances_error.deserialize_query(child)
        )
    return out
