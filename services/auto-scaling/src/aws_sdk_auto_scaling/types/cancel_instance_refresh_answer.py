"""Generated from Smithy shape ``com.amazonaws.autoscaling#CancelInstanceRefreshAnswer``."""
...
class CancelInstanceRefreshAnswer(TypedDict, closed=True):
...
# --- awsQuery ser/de ---
def serialize_query(
    value: CancelInstanceRefreshAnswer, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "instance_refresh_id" in value:
        pairs.append((f"{prefix}.InstanceRefreshId", str(value["instance_refresh_id"])))


def deserialize_query(el: Element) -> CancelInstanceRefreshAnswer:
    out: CancelInstanceRefreshAnswer = {}  # type: ignore[typeddict-item]
