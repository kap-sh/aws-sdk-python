...
if TYPE_CHECKING:
    import aws_sdk_auto_scaling.types.ebs
    import aws_sdk_auto_scaling.types.no_device
...
class BlockDeviceMapping(TypedDict, closed=True):
    ...
    ebs: NotRequired["aws_sdk_auto_scaling.types.ebs.Ebs"]
    """<p>Information to attach an EBS volume to an instance at launch.</p>"""
    no_device: NotRequired["aws_sdk_auto_scaling.types.no_device.NoDevice"]
...
def serialize_query(
    ...
    if "ebs" in value:
        ...
        aws_sdk_auto_scaling.types.ebs.serialize_query(
            value["ebs"], pairs, f"{prefix}.Ebs"
        )
    if "no_device" in value:
        pairs.append((f"{prefix}.NoDevice", "true" if value["no_device"] else "false"))
...
def deserialize_query(el: Element) -> BlockDeviceMapping:
    ...
    if child_ebs is not None:
        ...
        out["ebs"] = aws_sdk_auto_scaling.types.ebs.deserialize_query(child_ebs)
