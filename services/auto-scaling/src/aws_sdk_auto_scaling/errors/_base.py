...
class AutoScalingError(Exception):
    """Base class for all AutoScaling service errors."""


class ServiceError(AutoScalingError):
...
class SerializationError(AutoScalingError):
    """Raised when a value cannot be serialized."""


class DeserializationError(AutoScalingError):
...
class WaiterFailedError(AutoScalingError):
...
class WaiterTimeoutError(AutoScalingError):
