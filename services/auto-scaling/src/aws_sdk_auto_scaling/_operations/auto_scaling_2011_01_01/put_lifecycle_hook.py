"""Generated from Smithy shape ``com.amazonaws.autoscaling#PutLifecycleHook``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("PutLifecycleHookResult")
    out: aws_sdk_auto_scaling.types.put_lifecycle_hook_answer.PutLifecycleHookAnswer = (
        aws_sdk_auto_scaling.types.put_lifecycle_hook_answer.deserialize_query(
...
def build_request(
    ...
    input: aws_sdk_auto_scaling.types.put_lifecycle_hook_type.PutLifecycleHookType,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    aws_sdk_auto_scaling.types.put_lifecycle_hook_type.serialize_query(input, pairs, "")
...
async def async_put_lifecycle_hook(
