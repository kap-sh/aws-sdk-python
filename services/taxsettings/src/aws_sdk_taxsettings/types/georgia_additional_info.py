...
class GeorgiaAdditionalInfo(TypedDict, closed=True):
...
def serialize_json(value: GeorgiaAdditionalInfo) -> dict:
    ...
    out["personType"] = aws_sdk_taxsettings.types.person_type.serialize_json(
        value["person_type"]
    )
    return out
...
def deserialize_json(data: dict) -> GeorgiaAdditionalInfo:
    ...
    if "personType" in data:
        ...
        out["person_type"] = aws_sdk_taxsettings.types.person_type.deserialize_json(
            data["personType"]
        )
    else:
