...
TaxExemptionTypes: TypeAlias = list[
    "aws_sdk_taxsettings.types.tax_exemption_type.TaxExemptionType"
]
...
def serialize_json(value: TaxExemptionTypes) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_taxsettings.types.tax_exemption_type.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> TaxExemptionTypes:
    ...
    for item in data:
        out.append(aws_sdk_taxsettings.types.tax_exemption_type.deserialize_json(item))
    return out
