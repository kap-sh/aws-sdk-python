"""Generated from Smithy shape ``com.amazonaws.kafka#DescribeClusterOperation``."""
...
def handle_response(
    ...
    out: aws_sdk_kafka.types.describe_cluster_operation_response.DescribeClusterOperationResponse = aws_sdk_kafka.types.describe_cluster_operation_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_kafka.types.describe_cluster_operation_request.DescribeClusterOperationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/operations/{ClusterOperationArn}"
    url = url.replace(
...
async def async_describe_cluster_operation(
