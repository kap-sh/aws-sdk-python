"""Generated from Smithy shape ``com.amazonaws.kafka#DescribeClusterOperationV2``."""
...
def handle_response(
    ...
    out: aws_sdk_kafka.types.describe_cluster_operation_v2_response.DescribeClusterOperationV2Response = aws_sdk_kafka.types.describe_cluster_operation_v2_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_kafka.types.describe_cluster_operation_v2_request.DescribeClusterOperationV2Request,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/api/v2/operations/{ClusterOperationArn}"
    url = url.replace(
        "{ClusterOperationArn}", quote(str(input_["cluster_operation_arn"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_describe_cluster_operation_v2(
