"""Generated from Smithy shape ``com.amazonaws.kafka#UpdateBrokerType``."""
...
def handle_response(
    ...
    out: aws_sdk_kafka.types.update_broker_type_response.UpdateBrokerTypeResponse = (
        aws_sdk_kafka.types.update_broker_type_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_kafka.types.update_broker_type_request.UpdateBrokerTypeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/nodes/type"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_kafka.types.update_broker_type_request.serialize_json(input_)
    ).encode()
...
async def async_update_broker_type(
