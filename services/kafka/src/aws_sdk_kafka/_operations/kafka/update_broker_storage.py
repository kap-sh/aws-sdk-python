"""Generated from Smithy shape ``com.amazonaws.kafka#UpdateBrokerStorage``."""
...
def handle_response(
    ...
    out: aws_sdk_kafka.types.update_broker_storage_response.UpdateBrokerStorageResponse = aws_sdk_kafka.types.update_broker_storage_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_kafka.types.update_broker_storage_request.UpdateBrokerStorageRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/nodes/storage"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_kafka.types.update_broker_storage_request.serialize_json(input_)
    ).encode()
...
async def async_update_broker_storage(
