...
if TYPE_CHECKING:
    import aws_sdk_kafka.types.broker_ebs_volume_info

__listOfBrokerEBSVolumeInfo: TypeAlias = list[
    "aws_sdk_kafka.types.broker_ebs_volume_info.BrokerEBSVolumeInfo"
]
...
def serialize_json(value: __listOfBrokerEBSVolumeInfo) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_kafka.types.broker_ebs_volume_info.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfBrokerEBSVolumeInfo:
    ...
    for item in data:
        out.append(aws_sdk_kafka.types.broker_ebs_volume_info.deserialize_json(item))
    return out
