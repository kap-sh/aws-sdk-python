...
__listOfConfiguration: TypeAlias = list[
    "aws_sdk_kafka.types.configuration.Configuration"
]
...
def serialize_json(value: __listOfConfiguration) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_kafka.types.configuration.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfConfiguration:
    ...
    for item in data:
        out.append(aws_sdk_kafka.types.configuration.deserialize_json(item))
    return out
