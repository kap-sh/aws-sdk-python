...
if TYPE_CHECKING:
    import capo_kafka.types.rebalancing_status


class Rebalancing(TypedDict, closed=True):
    status: NotRequired["capo_kafka.types.rebalancing_status.RebalancingStatus"]
...
def serialize_json(value: Rebalancing) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_kafka.types.rebalancing_status.serialize_json(
            value["status"]
        )
    return out
...
def deserialize_json(data: dict) -> Rebalancing:
    ...
    if "status" in data:
        ...
        out["status"] = capo_kafka.types.rebalancing_status.deserialize_json(
            data["status"]
        )
    return out
