...
class DescribeTopicPartitionsResponse(TypedDict, closed=True):
    partitions: NotRequired[
        "capo_kafka.types.__list_of_topic_partition_info.__listOfTopicPartitionInfo"
    ]
...
def serialize_json(value: DescribeTopicPartitionsResponse) -> dict:
    ...
    if "partitions" in value:
        ...
        out["partitions"] = (
            capo_kafka.types.__list_of_topic_partition_info.serialize_json(
                value["partitions"]
            )
        )
...
def deserialize_json(data: dict) -> DescribeTopicPartitionsResponse:
    ...
    if "partitions" in data:
        ...
        out["partitions"] = (
            capo_kafka.types.__list_of_topic_partition_info.deserialize_json(
                data["partitions"]
            )
        )
