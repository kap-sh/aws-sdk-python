...
class CompatibleKafkaVersion(TypedDict, closed=True):
    source_version: NotRequired["capo_kafka.types.__string.__string"]
    """<p>An Apache Kafka version.</p>"""
    target_versions: NotRequired["capo_kafka.types.__list_of__string.__listOf__string"]
...
def serialize_json(value: CompatibleKafkaVersion) -> dict:
    ...
    if "target_versions" in value:
        ...
        out["targetVersions"] = capo_kafka.types.__list_of__string.serialize_json(
