...
class EncryptionInTransit(TypedDict, closed=True):
    client_broker: NotRequired["capo_kafka.types.client_broker.ClientBroker"]
    ...
    in_cluster: NotRequired["capo_kafka.types.__boolean.__boolean"]
...
def serialize_json(value: EncryptionInTransit) -> dict:
    ...
    if "client_broker" in value:
        ...
        out["clientBroker"] = capo_kafka.types.client_broker.serialize_json(
            value["client_broker"]
        )
