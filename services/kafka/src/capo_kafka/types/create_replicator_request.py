...
class CreateReplicatorRequest(TypedDict, closed=True):
    description: NotRequired["capo_kafka.types.__string_max1024.__stringMax1024"]
...
def serialize_json(value: CreateReplicatorRequest) -> dict:
    ...
    if "kafka_clusters" in value:
        ...
        out["kafkaClusters"] = capo_kafka.types.__list_of_kafka_cluster.serialize_json(
    ...
    if "replication_info_list" in value:
        ...
        out["replicationInfoList"] = (
            capo_kafka.types.__list_of_replication_info.serialize_json(
...
def deserialize_json(data: dict) -> CreateReplicatorRequest:
    ...
    if "kafkaClusters" in data:
        ...
        out["kafka_clusters"] = (
            capo_kafka.types.__list_of_kafka_cluster.deserialize_json(
                data["kafkaClusters"]
            )
        )
    if "replicationInfoList" in data:
        ...
        out["replication_info_list"] = (
            capo_kafka.types.__list_of_replication_info.deserialize_json(
