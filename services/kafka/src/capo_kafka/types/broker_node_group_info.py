...
class BrokerNodeGroupInfo(TypedDict, closed=True):
    broker_az_distribution: NotRequired[
        "capo_kafka.types.broker_az_distribution.BrokerAZDistribution"
    ]
    ...
    storage_info: NotRequired["capo_kafka.types.storage_info.StorageInfo"]
    """<p>Contains information about storage volumes attached to MSK broker nodes.</p>"""
    connectivity_info: NotRequired[
        "capo_kafka.types.connectivity_info.ConnectivityInfo"
    ]
    """<p>Information about the broker access configuration.</p>"""
    zone_ids: NotRequired["capo_kafka.types.__list_of__string.__listOf__string"]
...
def serialize_json(value: BrokerNodeGroupInfo) -> dict:
    ...
    if "broker_az_distribution" in value:
        ...
        out["brokerAZDistribution"] = (
            capo_kafka.types.broker_az_distribution.serialize_json(
    ...
    if "storage_info" in value:
        ...
        out["storageInfo"] = capo_kafka.types.storage_info.serialize_json(
            value["storage_info"]
        )
    ...
    if "zone_ids" in value:
        ...
        out["zoneIds"] = capo_kafka.types.__list_of__string.serialize_json(
            value["zone_ids"]
        )
    return out
...
def deserialize_json(data: dict) -> BrokerNodeGroupInfo:
    ...
    if "brokerAZDistribution" in data:
        ...
        out["broker_az_distribution"] = (
            capo_kafka.types.broker_az_distribution.deserialize_json(
