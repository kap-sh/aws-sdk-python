...
if TYPE_CHECKING:
    import capo_kafka.types.replication_info

__listOfReplicationInfo: TypeAlias = list[
    "capo_kafka.types.replication_info.ReplicationInfo"
]
...
def serialize_json(value: __listOfReplicationInfo) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_kafka.types.replication_info.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfReplicationInfo:
    ...
    for item in data:
        out.append(capo_kafka.types.replication_info.deserialize_json(item))
    return out
