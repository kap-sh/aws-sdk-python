"""Generated from Smithy shape ``com.amazonaws.kafka#RejectClientVpcConnection``."""
...
def handle_response(
    ...
    out: capo_kafka.types.reject_client_vpc_connection_response.RejectClientVpcConnectionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/client-vpc-connection"
    url = url.replace("{ClusterArn}", quote(str(input_["cluster_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.reject_client_vpc_connection_request.serialize_json(input_)
    ).encode()
...
async def async_reject_client_vpc_connection(
