"""Generated from Smithy shape ``com.amazonaws.kafka#CreateTopic``."""
...
import capo_kafka.errors.cluster_connectivity_exception
import capo_kafka.errors.conflict_exception
import capo_kafka.errors.controller_moved_exception
import capo_kafka.errors.forbidden_exception
import capo_kafka.errors.group_subscribed_to_topic_exception
import capo_kafka.errors.internal_server_error_exception
import capo_kafka.errors.kafka_request_exception
import capo_kafka.errors.kafka_timeout_exception
import capo_kafka.errors.not_controller_exception
import capo_kafka.errors.reassignment_in_progress_exception
import capo_kafka.errors.service_unavailable_exception
import capo_kafka.errors.too_many_requests_exception
import capo_kafka.errors.topic_exists_exception
import capo_kafka.errors.unauthorized_exception
import capo_kafka.errors.unknown_topic_or_partition_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ClusterConnectivityException":
            raise capo_kafka.errors.cluster_connectivity_exception.ClusterConnectivityException.from_json(
                data
            )
        case "ConflictException":
            raise capo_kafka.errors.conflict_exception.ConflictException.from_json(data)
        case "ControllerMovedException":
            raise capo_kafka.errors.controller_moved_exception.ControllerMovedException.from_json(
                data
            )
        ...
        case "GroupSubscribedToTopicException":
            raise capo_kafka.errors.group_subscribed_to_topic_exception.GroupSubscribedToTopicException.from_json(
                data
            )
        ...
        case "KafkaRequestException":
            raise capo_kafka.errors.kafka_request_exception.KafkaRequestException.from_json(
                data
            )
        case "KafkaTimeoutException":
            raise capo_kafka.errors.kafka_timeout_exception.KafkaTimeoutException.from_json(
                data
            )
        case "NotControllerException":
            raise capo_kafka.errors.not_controller_exception.NotControllerException.from_json(
                data
            )
        case "ReassignmentInProgressException":
            raise capo_kafka.errors.reassignment_in_progress_exception.ReassignmentInProgressException.from_json(
                data
            )
        ...
        case "TopicExistsException":
            raise capo_kafka.errors.topic_exists_exception.TopicExistsException.from_json(
                data
            )
        ...
        case "UnknownTopicOrPartitionException":
            raise capo_kafka.errors.unknown_topic_or_partition_exception.UnknownTopicOrPartitionException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_kafka.types.create_topic_response.CreateTopicResponse = (
        capo_kafka.types.create_topic_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/topics"
    url = url.replace("{ClusterArn}", quote(str(input_["cluster_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.create_topic_request.serialize_json(input_)
    ).encode()
...
def create_topic(
    ...
    input_: capo_kafka.types.create_topic_request.CreateTopicRequest,
) -> tuple[capo_kafka.types.create_topic_response.CreateTopicResponse, zapros.Response]:
