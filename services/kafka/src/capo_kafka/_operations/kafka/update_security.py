"""Generated from Smithy shape ``com.amazonaws.kafka#UpdateSecurity``."""
...
def handle_response(
    ...
    out: capo_kafka.types.update_security_response.UpdateSecurityResponse = (
        capo_kafka.types.update_security_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/security"
    url = url.replace("{ClusterArn}", quote(str(input_["cluster_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.update_security_request.serialize_json(input_)
    ).encode()
...
def update_security(
    ...
    capo_kafka.types.update_security_response.UpdateSecurityResponse, zapros.Response
]:
...
async def async_update_security(
