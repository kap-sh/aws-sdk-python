"""Generated from Smithy shape ``com.amazonaws.kafka#CreateReplicator``."""
...
def handle_response(
    ...
    out: capo_kafka.types.create_replicator_response.CreateReplicatorResponse = (
        capo_kafka.types.create_replicator_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/replication/v1/replicators"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.create_replicator_request.serialize_json(input_)
    ).encode()
...
async def async_create_replicator(
