"""Generated from Smithy shape ``com.amazonaws.kafka#CreateCluster``."""
...
def handle_response(
    ...
    out: capo_kafka.types.create_cluster_response.CreateClusterResponse = (
        capo_kafka.types.create_cluster_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.create_cluster_request.serialize_json(input_)
    ).encode()
...
def create_cluster(
    ...
    capo_kafka.types.create_cluster_response.CreateClusterResponse, zapros.Response
]:
