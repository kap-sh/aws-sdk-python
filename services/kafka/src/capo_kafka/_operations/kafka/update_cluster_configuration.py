"""Generated from Smithy shape ``com.amazonaws.kafka#UpdateClusterConfiguration``."""
...
def handle_response(
    ...
    out: capo_kafka.types.update_cluster_configuration_response.UpdateClusterConfigurationResponse = capo_kafka.types.update_cluster_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/clusters/{ClusterArn}/configuration"
    url = url.replace("{ClusterArn}", quote(str(input_["cluster_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_kafka.types.update_cluster_configuration_request.serialize_json(input_)
    ).encode()
...
async def async_update_cluster_configuration(
