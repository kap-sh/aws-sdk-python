...
if TYPE_CHECKING:
    import capo_rum.types.query_filter_key
    import capo_rum.types.query_filter_value_list


class QueryFilter(TypedDict, closed=True):
    name: NotRequired["capo_rum.types.query_filter_key.QueryFilterKey"]
    ...
    values: NotRequired["capo_rum.types.query_filter_value_list.QueryFilterValueList"]
...
def serialize_json(value: QueryFilter) -> dict:
    ...
    if "values" in value:
        ...
        out["Values"] = capo_rum.types.query_filter_value_list.serialize_json(
            value["values"]
        )
    return out
