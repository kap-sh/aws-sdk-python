...
def get_signer(
...
) -> aws_sdk_rum._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_rum._auth._sigv4.build_sigv4_auth_scheme("rum", options.region)
        )
        if sigv4_config is not None:
            return aws_sdk_rum._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_rum.types.delete_rum_metrics_destination_request.DeleteRumMetricsDestinationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
...
async def async_delete_rum_metrics_destination(
