...
class RUMError(Exception):
    """Base class for all RUM service errors."""


class ServiceError(RUMError):
...
class SerializationError(RUMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(RUMError):
...
class WaiterFailedError(RUMError):
...
class WaiterTimeoutError(RUMError):
