...
def serialize_json(value: PutRumEventsRequest) -> dict:
    ...
    import aws_sdk_rum.types.app_monitor_details
    out["AppMonitorDetails"] = aws_sdk_rum.types.app_monitor_details.serialize_json(value["app_monitor_details"])
    import aws_sdk_rum.types.user_details
    out["UserDetails"] = aws_sdk_rum.types.user_details.serialize_json(value["user_details"])
    import aws_sdk_rum.types.rum_event_list
    out["RumEvents"] = aws_sdk_rum.types.rum_event_list.serialize_json(value["rum_events"])
    if "alias" in value:
...
def deserialize_json(data: dict) -> PutRumEventsRequest:
    ...
    if "AppMonitorDetails" in data:
        import aws_sdk_rum.types.app_monitor_details
        out["app_monitor_details"] = aws_sdk_rum.types.app_monitor_details.deserialize_json(data["AppMonitorDetails"])
    else:
        raise DeserializationError("PutRumEventsRequest.app_monitor_details required")
    if "UserDetails" in data:
        import aws_sdk_rum.types.user_details
        out["user_details"] = aws_sdk_rum.types.user_details.deserialize_json(data["UserDetails"])
    else:
        raise DeserializationError("PutRumEventsRequest.user_details required")
    if "RumEvents" in data:
        import aws_sdk_rum.types.rum_event_list
        out["rum_events"] = aws_sdk_rum.types.rum_event_list.deserialize_json(data["RumEvents"])
    else:
