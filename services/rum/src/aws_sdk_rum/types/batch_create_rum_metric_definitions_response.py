...
class BatchCreateRumMetricDefinitionsResponse(TypedDict):
    ...
    metric_definitions: NotRequired["aws_sdk_rum.types.metric_definitions.MetricDefinitions"]
...
def serialize_json(value: BatchCreateRumMetricDefinitionsResponse) -> dict:
    out: dict = {}
    import aws_sdk_rum.types.batch_create_rum_metric_definitions_errors
    out["Errors"] = aws_sdk_rum.types.batch_create_rum_metric_definitions_errors.serialize_json(value["errors"])
    if "metric_definitions" in value:
        import aws_sdk_rum.types.metric_definitions
        out["MetricDefinitions"] = aws_sdk_rum.types.metric_definitions.serialize_json(value["metric_definitions"])
    return out
...
def deserialize_json(data: dict) -> BatchCreateRumMetricDefinitionsResponse:
    ...
    if "Errors" in data:
        import aws_sdk_rum.types.batch_create_rum_metric_definitions_errors
        out["errors"] = aws_sdk_rum.types.batch_create_rum_metric_definitions_errors.deserialize_json(data["Errors"])
    else:
        raise DeserializationError("BatchCreateRumMetricDefinitionsResponse.errors required")
    if "MetricDefinitions" in data:
        import aws_sdk_rum.types.metric_definitions
        out["metric_definitions"] = aws_sdk_rum.types.metric_definitions.deserialize_json(data["MetricDefinitions"])
    return out