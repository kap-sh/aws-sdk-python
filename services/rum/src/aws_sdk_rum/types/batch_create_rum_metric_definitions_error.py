...
class BatchCreateRumMetricDefinitionsError(TypedDict):
...
def serialize_json(value: BatchCreateRumMetricDefinitionsError) -> dict:
    out: dict = {}
    import aws_sdk_rum.types.metric_definition_request
    out["MetricDefinition"] = aws_sdk_rum.types.metric_definition_request.serialize_json(value["metric_definition"])
...
def deserialize_json(data: dict) -> BatchCreateRumMetricDefinitionsError:
    ...
    if "MetricDefinition" in data:
        import aws_sdk_rum.types.metric_definition_request
        out["metric_definition"] = aws_sdk_rum.types.metric_definition_request.deserialize_json(data["MetricDefinition"])
    else:
        raise DeserializationError("BatchCreateRumMetricDefinitionsError.metric_definition required")
    if "ErrorCode" in data:
        out["error_code"] = data["ErrorCode"]
    else:
        raise DeserializationError("BatchCreateRumMetricDefinitionsError.error_code required")
    if "ErrorMessage" in data:
        out["error_message"] = data["ErrorMessage"]
    else:
        raise DeserializationError("BatchCreateRumMetricDefinitionsError.error_message required")
    return out