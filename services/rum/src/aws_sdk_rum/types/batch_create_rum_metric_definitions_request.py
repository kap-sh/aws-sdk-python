...
class BatchCreateRumMetricDefinitionsRequest(TypedDict):
...
def serialize_json(value: BatchCreateRumMetricDefinitionsRequest) -> dict:
    ...
    import aws_sdk_rum.types.metric_definitions_request
    out["MetricDefinitions"] = aws_sdk_rum.types.metric_definitions_request.serialize_json(value["metric_definitions"])
    return out
...
def deserialize_json(data: dict) -> BatchCreateRumMetricDefinitionsRequest:
    ...
    if "Destination" in data:
        ...
        raise DeserializationError("BatchCreateRumMetricDefinitionsRequest.destination required")
    if "DestinationArn" in data:
        out["destination_arn"] = data["DestinationArn"]
    if "MetricDefinitions" in data:
        import aws_sdk_rum.types.metric_definitions_request
        out["metric_definitions"] = aws_sdk_rum.types.metric_definitions_request.deserialize_json(data["MetricDefinitions"])
    else:
        raise DeserializationError("BatchCreateRumMetricDefinitionsRequest.metric_definitions required")
    return out