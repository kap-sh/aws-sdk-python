...
MetricDefinitions: TypeAlias = list["aws_sdk_rum.types.metric_definition.MetricDefinition"]
...
def serialize_json(value: MetricDefinitions) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_rum.types.metric_definition.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> MetricDefinitions:
    ...
    for item in data:
        out.append(aws_sdk_rum.types.metric_definition.deserialize_json(item))
    return out