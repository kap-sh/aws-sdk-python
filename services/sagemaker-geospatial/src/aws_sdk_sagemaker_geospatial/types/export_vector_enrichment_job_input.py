...
class ExportVectorEnrichmentJobInput(TypedDict):
...
def serialize_json(value: ExportVectorEnrichmentJobInput) -> dict:
    ...
    out["OutputConfig"] = (
        aws_sdk_sagemaker_geospatial.types.export_vector_enrichment_job_output_config.serialize_json(
            value["output_config"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ExportVectorEnrichmentJobInput:
    ...
    if "OutputConfig" in data:
        ...
        out["output_config"] = (
            aws_sdk_sagemaker_geospatial.types.export_vector_enrichment_job_output_config.deserialize_json(
                data["OutputConfig"]
            )
        )
    else:
