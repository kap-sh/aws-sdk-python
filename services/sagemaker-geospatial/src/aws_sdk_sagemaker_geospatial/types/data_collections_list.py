...
DataCollectionsList: TypeAlias = list[
    "aws_sdk_sagemaker_geospatial.types.raster_data_collection_metadata.RasterDataCollectionMetadata"
]
...
def serialize_json(value: DataCollectionsList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_sagemaker_geospatial.types.raster_data_collection_metadata.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> DataCollectionsList:
    ...
    for item in data:
        out.append(
            aws_sdk_sagemaker_geospatial.types.raster_data_collection_metadata.deserialize_json(
                item
            )
        )
    return out
