...
class ExportVectorEnrichmentJobOutputConfig(TypedDict, closed=True):
    s3_data: "capo_sagemaker_geospatial.types.vector_enrichment_job_s3_data.VectorEnrichmentJobS3Data"
...
def serialize_json(value: ExportVectorEnrichmentJobOutputConfig) -> dict:
    ...
    out["S3Data"] = (
        capo_sagemaker_geospatial.types.vector_enrichment_job_s3_data.serialize_json(
            value["s3_data"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ExportVectorEnrichmentJobOutputConfig:
    ...
    if "S3Data" in data:
        ...
        out["s3_data"] = (
            capo_sagemaker_geospatial.types.vector_enrichment_job_s3_data.deserialize_json(
                data["S3Data"]
            )
        )
    else:
