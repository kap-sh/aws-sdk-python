...
class OutputResolutionResamplingInput(TypedDict, closed=True):
    user_defined: "capo_sagemaker_geospatial.types.user_defined.UserDefined"
...
def serialize_json(value: OutputResolutionResamplingInput) -> dict:
    ...
    out["UserDefined"] = capo_sagemaker_geospatial.types.user_defined.serialize_json(
        value["user_defined"]
    )
    return out
...
def deserialize_json(data: dict) -> OutputResolutionResamplingInput:
    ...
    if "UserDefined" in data:
        ...
        out["user_defined"] = (
            capo_sagemaker_geospatial.types.user_defined.deserialize_json(
                data["UserDefined"]
            )
        )
    else:
