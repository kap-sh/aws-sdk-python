"""Generated from Smithy shape ``com.amazonaws.sagemakergeospatial#StopVectorEnrichmentJob``."""
...
def handle_response(
    ...
    out: capo_sagemaker_geospatial.types.stop_vector_enrichment_job_output.StopVectorEnrichmentJobOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/vector-enrichment-jobs/stop"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sagemaker_geospatial.types.stop_vector_enrichment_job_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_stop_vector_enrichment_job(
