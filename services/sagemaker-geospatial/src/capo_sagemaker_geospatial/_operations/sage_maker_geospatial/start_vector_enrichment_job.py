"""Generated from Smithy shape ``com.amazonaws.sagemakergeospatial#StartVectorEnrichmentJob``."""
...
def handle_response(
    ...
    out: capo_sagemaker_geospatial.types.start_vector_enrichment_job_output.StartVectorEnrichmentJobOutput = capo_sagemaker_geospatial.types.start_vector_enrichment_job_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/vector-enrichment-jobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sagemaker_geospatial.types.start_vector_enrichment_job_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_vector_enrichment_job(
