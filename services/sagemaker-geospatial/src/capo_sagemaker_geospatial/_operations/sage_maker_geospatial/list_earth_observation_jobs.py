"""Generated from Smithy shape ``com.amazonaws.sagemakergeospatial#ListEarthObservationJobs``."""
...
def handle_response(
    ...
    out: capo_sagemaker_geospatial.types.list_earth_observation_job_output.ListEarthObservationJobOutput = capo_sagemaker_geospatial.types.list_earth_observation_job_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-earth-observation-jobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sagemaker_geospatial.types.list_earth_observation_job_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_earth_observation_jobs(
