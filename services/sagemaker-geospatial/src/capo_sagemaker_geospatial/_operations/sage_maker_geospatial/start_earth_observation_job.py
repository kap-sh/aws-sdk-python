"""Generated from Smithy shape ``com.amazonaws.sagemakergeospatial#StartEarthObservationJob``."""
...
def handle_response(
    ...
    out: capo_sagemaker_geospatial.types.start_earth_observation_job_output.StartEarthObservationJobOutput = capo_sagemaker_geospatial.types.start_earth_observation_job_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/earth-observation-jobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sagemaker_geospatial.types.start_earth_observation_job_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_earth_observation_job(
