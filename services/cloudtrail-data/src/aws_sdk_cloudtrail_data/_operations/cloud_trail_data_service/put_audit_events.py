...
def build_request(
    ...
    input: aws_sdk_cloudtrail_data.types.put_audit_events_request.PutAuditEventsRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "channel_arn" in input:
        params["channelArn"] = str(input["channel_arn"])
    if "external_id" in input:
        params["externalId"] = str(input["external_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_cloudtrail_data.types.put_audit_events_request

    body: bytes | None = json.dumps(
        aws_sdk_cloudtrail_data.types.put_audit_events_request.serialize_json(input)
    ).encode()
