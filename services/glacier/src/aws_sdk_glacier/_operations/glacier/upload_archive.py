...
def handle_response(
    ...
    out: aws_sdk_glacier.types.archive_creation_output.ArchiveCreationOutput = {}  # type: ignore[typeddict-item]
    ...
    if "x-amz-archive-id" in response.headers:
        out["archive_id"] = str(response.headers["x-amz-archive-id"])
    return out
...
def build_request(
    ...
    input: aws_sdk_glacier.types.upload_archive_input.UploadArchiveInput,
) -> zapros.Request:
    ...
    if "checksum" in input:
        headers["x-amz-sha256-tree-hash"] = str(input["checksum"])
    body = input["body"]
...
def upload_archive(
    ...
    aws_sdk_glacier.types.archive_creation_output.ArchiveCreationOutput, zapros.Response
]:
