...
def handle_response(
    ...
    out: aws_sdk_glacier.types.initiate_vault_lock_output.InitiateVaultLockOutput = {}  # type: ignore[typeddict-item]
    if "x-amz-lock-id" in response.headers:
        out["lock_id"] = str(response.headers["x-amz-lock-id"])
    return out
...
def build_request(
    ...
    input_: aws_sdk_glacier.types.initiate_vault_lock_input.InitiateVaultLockInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "policy" in input_:
        body: bytes | None = json.dumps(
            aws_sdk_glacier.types.vault_lock_policy.serialize_json(input_["policy"])
        ).encode()
