"""Generated from Smithy shape ``com.amazonaws.glacier#PurchaseProvisionedCapacity``."""
...
def handle_response(
    ...
    out: aws_sdk_glacier.types.purchase_provisioned_capacity_output.PurchaseProvisionedCapacityOutput = {}  # type: ignore[typeddict-item]
    if "x-amz-capacity-id" in response.headers:
        out["capacity_id"] = str(response.headers["x-amz-capacity-id"])
    return out
...
def build_request(
    ...
    input_: aws_sdk_glacier.types.purchase_provisioned_capacity_input.PurchaseProvisionedCapacityInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/{accountId}/provisioned-capacity"
...
async def async_purchase_provisioned_capacity(
