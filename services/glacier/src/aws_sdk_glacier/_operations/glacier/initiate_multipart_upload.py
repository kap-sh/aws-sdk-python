"""Generated from Smithy shape ``com.amazonaws.glacier#InitiateMultipartUpload``."""
...
def handle_response(
    ...
    out: aws_sdk_glacier.types.initiate_multipart_upload_output.InitiateMultipartUploadOutput = {}  # type: ignore[typeddict-item]
    if "Location" in response.headers:
        out["location"] = str(response.headers["Location"])
    if "x-amz-multipart-upload-id" in response.headers:
        out["upload_id"] = str(response.headers["x-amz-multipart-upload-id"])
    return out
...
def build_request(
    ...
    input: aws_sdk_glacier.types.initiate_multipart_upload_input.InitiateMultipartUploadInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/{accountId}/vaults/{vaultName}/multipart-uploads"
    ...
    if "archive_description" in input:
        headers["x-amz-archive-description"] = str(input["archive_description"])
    if "part_size" in input:
        headers["x-amz-part-size"] = str(input["part_size"])
    body: bytes | None = b""
...
async def async_initiate_multipart_upload(
