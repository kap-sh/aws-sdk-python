...
def build_request(
    ...
    input_: aws_sdk_glacier.types.initiate_multipart_upload_input.InitiateMultipartUploadInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "archive_description" in input_:
        headers["x-amz-archive-description"] = str(input_["archive_description"])
    if "part_size" in input_:
        headers["x-amz-part-size"] = str(input_["part_size"])
    body: bytes | None = b""
