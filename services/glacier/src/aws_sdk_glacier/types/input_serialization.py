...
if TYPE_CHECKING:
    import aws_sdk_glacier.types.csv_input


class InputSerialization(TypedDict, closed=True):
    csv: NotRequired["aws_sdk_glacier.types.csv_input.CSVInput"]
...
def serialize_json(value: InputSerialization) -> dict:
    ...
    if "csv" in value:
        ...
        out["csv"] = aws_sdk_glacier.types.csv_input.serialize_json(value["csv"])
    return out
...
def deserialize_json(data: dict) -> InputSerialization:
    ...
    if "csv" in data:
        ...
        out["csv"] = aws_sdk_glacier.types.csv_input.deserialize_json(data["csv"])
    return out
