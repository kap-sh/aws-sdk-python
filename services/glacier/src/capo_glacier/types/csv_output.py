...
class CSVOutput(TypedDict, closed=True):
    quote_fields: NotRequired["capo_glacier.types.quote_fields.QuoteFields"]
    """<p>A value that indicates whether all output fields should be contained within quotation marks.</p>"""
    quote_escape_character: NotRequired["capo_glacier.types.string.string"]
    """<p>A single character used for escaping the quotation-mark character inside an already escaped value.</p>"""
    record_delimiter: NotRequired["capo_glacier.types.string.string"]
    """<p>A value used to separate individual records from each other.</p>"""
    field_delimiter: NotRequired["capo_glacier.types.string.string"]
    """<p>A value used to separate individual fields from each other within a record.</p>"""
    quote_character: NotRequired["capo_glacier.types.string.string"]
...
def serialize_json(value: CSVOutput) -> dict:
    ...
    if "quote_fields" in value:
        ...
        out["QuoteFields"] = capo_glacier.types.quote_fields.serialize_json(
            value["quote_fields"]
        )
