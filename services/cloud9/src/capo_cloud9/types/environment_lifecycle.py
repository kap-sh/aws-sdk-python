...
class EnvironmentLifecycle(TypedDict, closed=True):
    status: NotRequired[
        "capo_cloud9.types.environment_lifecycle_status.EnvironmentLifecycleStatus"
    ]
    ...
    reason: NotRequired["capo_cloud9.types.string.String"]
    """<p>Any informational message about the lifecycle state of the environment.</p>"""
    failure_resource: NotRequired["capo_cloud9.types.string.String"]
...
def serialize_aws_json_1_1(value: EnvironmentLifecycle) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = (
            capo_cloud9.types.environment_lifecycle_status.serialize_aws_json_1_1(
                value["status"]
            )
        )
    if "reason" in value:
...
def deserialize_aws_json_1_1(data: dict) -> EnvironmentLifecycle:
    ...
    if "status" in data:
        ...
        out["status"] = (
            capo_cloud9.types.environment_lifecycle_status.deserialize_aws_json_1_1(
                data["status"]
            )
        )
    if "reason" in data:
