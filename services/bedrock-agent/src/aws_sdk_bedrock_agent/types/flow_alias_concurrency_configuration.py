...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent.types.concurrency_type


class FlowAliasConcurrencyConfiguration(TypedDict):
    type: "aws_sdk_bedrock_agent.types.concurrency_type.ConcurrencyType"
...
def serialize_json(value: FlowAliasConcurrencyConfiguration) -> dict:
    ...
    out["type"] = aws_sdk_bedrock_agent.types.concurrency_type.serialize_json(
        value["type"]
    )
...
def deserialize_json(data: dict) -> FlowAliasConcurrencyConfiguration:
    ...
    if "type" in data:
        ...
        out["type"] = aws_sdk_bedrock_agent.types.concurrency_type.deserialize_json(
            data["type"]
        )
    else:
