...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent.types.key
    import aws_sdk_bedrock_agent.types.metadata_attribute_value


class MetadataAttribute(TypedDict):
    key: "aws_sdk_bedrock_agent.types.key.Key"
    """<p>The key of the metadata attribute.</p>"""
    value: "aws_sdk_bedrock_agent.types.metadata_attribute_value.MetadataAttributeValue"
...
def serialize_json(value: MetadataAttribute) -> dict:
    ...
    out["value"] = aws_sdk_bedrock_agent.types.metadata_attribute_value.serialize_json(
        value["value"]
    )
    return out
...
def deserialize_json(data: dict) -> MetadataAttribute:
    ...
    if "value" in data:
        ...
        out["value"] = (
            aws_sdk_bedrock_agent.types.metadata_attribute_value.deserialize_json(
                data["value"]
            )
        )
    else:
