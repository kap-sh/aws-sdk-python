...
class IngestKnowledgeBaseDocumentsRequest(TypedDict, closed=True):
    ...
    documents: (
        "aws_sdk_bedrock_agent.types.knowledge_base_documents.KnowledgeBaseDocuments"
    )
...
def serialize_json(value: IngestKnowledgeBaseDocumentsRequest) -> dict:
    ...
    out["documents"] = (
        aws_sdk_bedrock_agent.types.knowledge_base_documents.serialize_json(
            value["documents"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> IngestKnowledgeBaseDocumentsRequest:
    ...
    if "documents" in data:
        ...
        out["documents"] = (
            aws_sdk_bedrock_agent.types.knowledge_base_documents.deserialize_json(
                data["documents"]
            )
        )
    else:
