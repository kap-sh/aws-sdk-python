...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent.types.confluence_auth_type
    import aws_sdk_bedrock_agent.types.confluence_host_type
    import aws_sdk_bedrock_agent.types.https_url
    import aws_sdk_bedrock_agent.types.secret_arn


class ConfluenceSourceConfiguration(TypedDict):
    host_url: "aws_sdk_bedrock_agent.types.https_url.HttpsUrl"
    """<p>The Confluence host URL or instance URL.</p>"""
    host_type: "aws_sdk_bedrock_agent.types.confluence_host_type.ConfluenceHostType"
    """<p>The supported host type, whether online/cloud or server/on-premises.</p>"""
    auth_type: "aws_sdk_bedrock_agent.types.confluence_auth_type.ConfluenceAuthType"
...
def serialize_json(value: ConfluenceSourceConfiguration) -> dict:
    ...
    out["hostType"] = aws_sdk_bedrock_agent.types.confluence_host_type.serialize_json(
        value["host_type"]
    )
    import aws_sdk_bedrock_agent.types.confluence_auth_type

    out["authType"] = aws_sdk_bedrock_agent.types.confluence_auth_type.serialize_json(
        value["auth_type"]
    )
...
def deserialize_json(data: dict) -> ConfluenceSourceConfiguration:
    ...
    if "hostType" in data:
        ...
        out["host_type"] = (
            aws_sdk_bedrock_agent.types.confluence_host_type.deserialize_json(
                data["hostType"]
            )
        )
    else:
    ...
    if "authType" in data:
        ...
        out["auth_type"] = (
            aws_sdk_bedrock_agent.types.confluence_auth_type.deserialize_json(
                data["authType"]
            )
        )
    else:
