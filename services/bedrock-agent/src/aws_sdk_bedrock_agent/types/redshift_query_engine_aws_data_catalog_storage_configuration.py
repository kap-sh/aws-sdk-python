...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent.types.aws_data_catalog_table_names


class RedshiftQueryEngineAwsDataCatalogStorageConfiguration(TypedDict):
    table_names: "aws_sdk_bedrock_agent.types.aws_data_catalog_table_names.AwsDataCatalogTableNames"
...
def serialize_json(
    ...
    out["tableNames"] = (
        aws_sdk_bedrock_agent.types.aws_data_catalog_table_names.serialize_json(
            value["table_names"]
        )
    )
    return out
...
def deserialize_json(
    ...
    if "tableNames" in data:
        ...
        out["table_names"] = (
            aws_sdk_bedrock_agent.types.aws_data_catalog_table_names.deserialize_json(
                data["tableNames"]
            )
        )
    else:
