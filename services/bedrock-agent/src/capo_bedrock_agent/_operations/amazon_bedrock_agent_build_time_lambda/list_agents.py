"""Generated from Smithy shape ``com.amazonaws.bedrockagent#ListAgents``."""
...
import capo_bedrock_agent.types.agent_summaries
...
def handle_response(
    ...
    out: capo_bedrock_agent.types.list_agents_response.ListAgentsResponse = (
        capo_bedrock_agent.types.list_agents_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/agents/"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agent.types.list_agents_request.serialize_json(input_)
    ).encode()
...
def list_agents(
    ...
    capo_bedrock_agent.types.list_agents_response.ListAgentsResponse, zapros.Response
]:
