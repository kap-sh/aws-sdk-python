"""Generated from Smithy shape ``com.amazonaws.bedrockagent#UpdateAgentCollaborator``."""
...
def handle_response(
    ...
    out: capo_bedrock_agent.types.update_agent_collaborator_response.UpdateAgentCollaboratorResponse = capo_bedrock_agent.types.update_agent_collaborator_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{collaboratorId}", quote(input_["collaborator_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agent.types.update_agent_collaborator_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_update_agent_collaborator(
