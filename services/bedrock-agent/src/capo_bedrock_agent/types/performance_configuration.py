...
class PerformanceConfiguration(TypedDict, closed=True):
    latency: (
        "capo_bedrock_agent.types.performance_config_latency.PerformanceConfigLatency"
    )
...
def serialize_json(value: PerformanceConfiguration) -> dict:
    ...
    out["latency"] = capo_bedrock_agent.types.performance_config_latency.serialize_json(
...
def deserialize_json(data: dict) -> PerformanceConfiguration:
    ...
    if "latency" in data:
        ...
        out["latency"] = (
            capo_bedrock_agent.types.performance_config_latency.deserialize_json(
                data["latency"]
            )
        )
    else:
