...
if TYPE_CHECKING:
    import capo_bedrock_agent.types.query_generation_table

QueryGenerationTables: TypeAlias = list[
    "capo_bedrock_agent.types.query_generation_table.QueryGenerationTable"
]
...
def serialize_json(value: QueryGenerationTables) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_bedrock_agent.types.query_generation_table.serialize_json(item))
    return out
