...
from aws_sdk_kinesis_video_webrtc_storage._rule_engine._endpoint_rule_set import (
    EndpointParams,
    resolve,
)
...
from aws_sdk_kinesis_video_webrtc_storage.errors import (
    UnknownServiceError,
)
...
def get_signer(
...
) -> aws_sdk_kinesis_video_webrtc_storage._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_kinesis_video_webrtc_storage._auth._sigv4.build_sigv4_auth_scheme(
                "kinesisvideo", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_kinesis_video_webrtc_storage._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_kinesis_video_webrtc_storage.types.join_storage_session_as_viewer_input.JoinStorageSessionAsViewerInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_kinesis_video_webrtc_storage.types.join_storage_session_as_viewer_input.serialize_json(
            input_
        )
    ).encode()
