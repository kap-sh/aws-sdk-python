...
if TYPE_CHECKING:
    ...
    import aws_sdk_codebuild.types.docker_server_status
...
class DockerServer(TypedDict):
    ...
    status: NotRequired[
        "aws_sdk_codebuild.types.docker_server_status.DockerServerStatus"
    ]
...
def serialize_aws_json_1_1(value: DockerServer) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = (
            aws_sdk_codebuild.types.docker_server_status.serialize_aws_json_1_1(
                value["status"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> DockerServer:
    ...
    if "status" in data:
        ...
        out["status"] = (
            aws_sdk_codebuild.types.docker_server_status.deserialize_aws_json_1_1(
                data["status"]
            )
        )
    return out
