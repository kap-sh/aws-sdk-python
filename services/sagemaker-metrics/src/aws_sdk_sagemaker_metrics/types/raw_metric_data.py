...
if TYPE_CHECKING:
    ...
    import aws_sdk_sagemaker_metrics.types.step
...
class RawMetricData(TypedDict, closed=True):
    ...
    step: NotRequired["aws_sdk_sagemaker_metrics.types.step.Step"]
    """<p>The metric step (epoch). </p>"""
    value: NotRequired["aws_sdk_sagemaker_metrics.types.double.Double"]
...
def serialize_json(value: RawMetricData) -> dict:
    ...
    if "timestamp" in value:
        ...
        out["Timestamp"] = aws_sdk_sagemaker_metrics.types.timestamp.serialize_json(
            value["timestamp"]
        )
    if "step" in value:
        out["Step"] = value["step"]
...
def deserialize_json(data: dict) -> RawMetricData:
    ...
    if "Timestamp" in data:
        ...
        out["timestamp"] = aws_sdk_sagemaker_metrics.types.timestamp.deserialize_json(
            data["Timestamp"]
        )
    if "Step" in data:
        out["step"] = data["Step"]
