...
class DLMError(Exception):
    """Base class for all DLM service errors."""


class ServiceError(DLMError):
...
class SerializationError(DLMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(DLMError):
...
class WaiterFailedError(DLMError):
...
class WaiterTimeoutError(DLMError):
