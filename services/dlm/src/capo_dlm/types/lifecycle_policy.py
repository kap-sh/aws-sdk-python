...
if TYPE_CHECKING:
    ...
    import capo_dlm.types.status_message
...
class LifecyclePolicy(TypedDict, closed=True):
    ...
    status_message: NotRequired["capo_dlm.types.status_message.StatusMessage"]
    ...
    date_created: NotRequired["capo_dlm.types.timestamp.Timestamp"]
    """<p>The local date and time when the lifecycle policy was created.</p>"""
    date_modified: NotRequired["capo_dlm.types.timestamp.Timestamp"]
    ...
    """<p>The tags.</p>"""
    policy_arn: NotRequired["capo_dlm.types.policy_arn.PolicyArn"]
...
def serialize_json(value: LifecyclePolicy) -> dict:
    ...
    if "date_created" in value:
        ...
        out["DateCreated"] = capo_dlm.types.timestamp.serialize_json(
            value["date_created"]
        )
    if "date_modified" in value:
        import capo_dlm.types.timestamp

        out["DateModified"] = capo_dlm.types.timestamp.serialize_json(
            value["date_modified"]
        )
