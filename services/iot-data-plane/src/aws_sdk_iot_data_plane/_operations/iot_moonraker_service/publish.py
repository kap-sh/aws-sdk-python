...
def build_request(
    ...
    input: aws_sdk_iot_data_plane.types.publish_request.PublishRequest,
) -> zapros.Request:
    ...
    url = url.replace("{topic}", quote(str(input["topic"]), safe=""))
    params: dict[str, str] = {}
    params["qos"] = str(input.get("qos", 0))
    params["retain"] = str(input.get("retain", False))
    if "content_type" in input:
        params["contentType"] = str(input["content_type"])
    if "response_topic" in input:
        params["responseTopic"] = str(input["response_topic"])
    params["messageExpiry"] = str(input.get("message_expiry", 0))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "user_properties" in input:
        headers["x-amz-mqtt5-user-properties"] = str(input["user_properties"])
    ...
    if "correlation_data" in input:
        headers["x-amz-mqtt5-correlation-data"] = str(input["correlation_data"])
    if "payload" in input:
        import aws_sdk_iot_data_plane.types.payload

        body: bytes | None = json.dumps(
            aws_sdk_iot_data_plane.types.payload.serialize_json(input["payload"])
        ).encode()
