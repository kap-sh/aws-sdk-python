...
def resolve(p: EndpointParams) -> Endpoint:  # type: ignore
    ...
    if p.Region is not None:
        ...
        if _locals["PartitionResult"] is not None:
            if string_equals(p.Region, interpolate("us-east-1", p, _locals)):
                if p.UseFIPS is False:
                    if p.UseDualStack is False:
                        return Endpoint(
                            url=interpolate("https://sdb.amazonaws.com", p, _locals),
                            properties={},
                            headers={},
                        )
            if p.UseFIPS is True:
            ...
            if p.UseFIPS is True:
                if p.UseDualStack is False:
                    if (
                        ...
                        return Endpoint(
                            url=interpolate(
                                "https://sdb-fips.{Region}.{PartitionResult#dnsSuffix}",
                                p,
                                _locals,
                            ),
                            properties={},
            ...
            if p.UseFIPS is False:
                if p.UseDualStack is True:
                    if True is get_attr(
                        ...
                        return Endpoint(
                            url=interpolate(
                                "https://sdb.{Region}.{PartitionResult#dualStackDnsSuffix}",
                                p,
                                _locals,
                            ),
                            properties={},
            ...
            return Endpoint(
                url=interpolate(
                    "https://sdb.{Region}.{PartitionResult#dnsSuffix}", p, _locals
                ),
                properties={},
                headers={},
            )
