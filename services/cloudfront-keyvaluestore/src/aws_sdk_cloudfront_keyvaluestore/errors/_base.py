...
class CloudFrontKeyValueStoreError(Exception):
    """Base class for all CloudFrontKeyValueStore service errors."""

class ServiceError(CloudFrontKeyValueStoreError):
    ...
    """
    code: str | None = None
    def __init__(self, fault: Literal["client", "server"], *, is_throttling_error: bool, is_retryable: bool, code: str | None = None, message: str | None = None) -> None:
...
class SerializationError(CloudFrontKeyValueStoreError):
    """Raised when a value cannot be serialized."""

class DeserializationError(CloudFrontKeyValueStoreError):
...
class UnknownServiceError(ServiceError):
    ...
    """
    def __init__(self, *, code: str | None, message: str | None, response: object) -> None:
        ...
        super().__init__(_fault, is_throttling_error=_throttling, is_retryable=_retryable, code=code, message=message)
        self.response = response

class WaiterFailedError(CloudFrontKeyValueStoreError):
...
class WaiterTimeoutError(CloudFrontKeyValueStoreError):
    ...
    def __init__(self, waiter_name: str, max_wait_time: float) -> None:
        super().__init__(f"waiter {waiter_name} timed out after {max_wait_time} seconds")
        self.waiter_name = waiter_name
