...
def build_request(
    ...
    input: aws_sdk_outposts.types.list_assets_input.ListAssetsInput,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "host_id_filter" in input:
        params["HostIdFilter"] = str(input["host_id_filter"])
    if "max_results" in input:
        params["MaxResults"] = str(input["max_results"])
    if "next_token" in input:
        params["NextToken"] = str(input["next_token"])
    if "status_filter" in input:
        params["StatusFilter"] = str(input["status_filter"])
    if "asset_type_filter" in input:
        params["AssetTypeFilter"] = str(input["asset_type_filter"])
