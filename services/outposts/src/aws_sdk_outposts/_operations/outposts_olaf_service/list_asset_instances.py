"""Generated from Smithy shape ``com.amazonaws.outposts#ListAssetInstances``."""
...
def handle_response(
    ...
    out: aws_sdk_outposts.types.list_asset_instances_output.ListAssetInstancesOutput = (
        aws_sdk_outposts.types.list_asset_instances_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_outposts.types.list_asset_instances_input.ListAssetInstancesInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/outposts/{OutpostIdentifier}/assetInstances"
    url = url.replace(
        "{OutpostIdentifier}", quote(str(input_["outpost_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    if "asset_id_filter" in input_:
        params["AssetIdFilter"] = str(input_["asset_id_filter"])
    if "instance_type_filter" in input_:
        params["InstanceTypeFilter"] = str(input_["instance_type_filter"])
    if "account_id_filter" in input_:
        params["AccountIdFilter"] = str(input_["account_id_filter"])
    if "aws_service_filter" in input_:
        params["AwsServiceFilter"] = str(input_["aws_service_filter"])
...
async def async_list_asset_instances(
