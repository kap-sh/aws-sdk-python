...
def build_request(
    ...
    input: aws_sdk_outposts.types.list_catalog_items_input.ListCatalogItemsInput,
) -> zapros.Request:
    ...
    if "item_class_filter" in input:
        params["ItemClassFilter"] = str(input["item_class_filter"])
    if "supported_storage_filter" in input:
        params["SupportedStorageFilter"] = str(input["supported_storage_filter"])
    if "ec2_family_filter" in input:
        params["EC2FamilyFilter"] = str(input["ec2_family_filter"])
