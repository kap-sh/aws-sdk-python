...
if TYPE_CHECKING:
    import capo_outposts.types.order_summary_list_definition
    import capo_outposts.types.token


class ListOrdersOutput(TypedDict, closed=True):
    orders: NotRequired[
        "capo_outposts.types.order_summary_list_definition.OrderSummaryListDefinition"
    ]
...
def serialize_json(value: ListOrdersOutput) -> dict:
    ...
    if "orders" in value:
        ...
        out["Orders"] = (
            capo_outposts.types.order_summary_list_definition.serialize_json(
                value["orders"]
            )
        )
...
def deserialize_json(data: dict) -> ListOrdersOutput:
    ...
    if "Orders" in data:
        ...
        out["orders"] = (
            capo_outposts.types.order_summary_list_definition.deserialize_json(
                data["Orders"]
            )
        )
