...
class ScheduledQueryInsights(TypedDict, closed=True):
    mode: "capo_timestream_query.types.scheduled_query_insights_mode.ScheduledQueryInsightsMode"
...
def serialize_aws_json_1_0(value: ScheduledQueryInsights) -> dict:
    ...
    out["Mode"] = (
        capo_timestream_query.types.scheduled_query_insights_mode.serialize_aws_json_1_0(
            value["mode"]
        )
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> ScheduledQueryInsights:
    ...
    if "Mode" in data:
        ...
        out["mode"] = (
            capo_timestream_query.types.scheduled_query_insights_mode.deserialize_aws_json_1_0(
                data["Mode"]
            )
        )
    else:
