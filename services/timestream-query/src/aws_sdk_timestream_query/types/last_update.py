...
class LastUpdate(TypedDict, closed=True):
    ...
    status: NotRequired[
        "aws_sdk_timestream_query.types.last_update_status.LastUpdateStatus"
    ]
    """<p>The status of the last update. Can be either <code>PENDING</code>, <code>FAILED</code>, or <code>SUCCEEDED</code>.</p>"""
    status_message: NotRequired["aws_sdk_timestream_query.types.string.String"]
...
def serialize_aws_json_1_0(value: LastUpdate) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            aws_sdk_timestream_query.types.last_update_status.serialize_aws_json_1_0(
                value["status"]
            )
        )
...
def deserialize_aws_json_1_0(data: dict) -> LastUpdate:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            aws_sdk_timestream_query.types.last_update_status.deserialize_aws_json_1_0(
                data["Status"]
            )
        )
