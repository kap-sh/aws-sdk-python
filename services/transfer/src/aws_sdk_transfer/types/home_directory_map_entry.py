...
if TYPE_CHECKING:
    import aws_sdk_transfer.types.map_entry
    import aws_sdk_transfer.types.map_target
    import aws_sdk_transfer.types.map_type


class HomeDirectoryMapEntry(TypedDict):
    entry: "aws_sdk_transfer.types.map_entry.MapEntry"
    """<p>Represents an entry for <code>HomeDirectoryMappings</code>.</p>"""
    target: "aws_sdk_transfer.types.map_target.MapTarget"
    """<p>Represents the map target that is used in a <code>HomeDirectoryMapEntry</code>.</p>"""
    type: NotRequired["aws_sdk_transfer.types.map_type.MapType"]
...
def serialize_aws_json_1_1(value: HomeDirectoryMapEntry) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = aws_sdk_transfer.types.map_type.serialize_aws_json_1_1(
            value["type"]
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> HomeDirectoryMapEntry:
    ...
    if "Type" in data:
        ...
        out["type"] = aws_sdk_transfer.types.map_type.deserialize_aws_json_1_1(
            data["Type"]
        )
    return out
