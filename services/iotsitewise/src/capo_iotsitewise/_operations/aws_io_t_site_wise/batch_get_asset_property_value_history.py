"""Generated from Smithy shape ``com.amazonaws.iotsitewise#BatchGetAssetPropertyValueHistory``."""
...
def handle_response(
    ...
    out: capo_iotsitewise.types.batch_get_asset_property_value_history_response.BatchGetAssetPropertyValueHistoryResponse = capo_iotsitewise.types.batch_get_asset_property_value_history_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/properties/batch/history"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iotsitewise.types.batch_get_asset_property_value_history_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_get_asset_property_value_history(
