"""Generated from Smithy shape ``com.amazonaws.iotsitewise#UpdateGatewayCapabilityConfiguration``."""
...
def handle_response(
    ...
    out: capo_iotsitewise.types.update_gateway_capability_configuration_response.UpdateGatewayCapabilityConfigurationResponse = capo_iotsitewise.types.update_gateway_capability_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/20200301/gateways/{gatewayId}/capability"
    url = url.replace("{gatewayId}", quote(str(input_["gateway_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iotsitewise.types.update_gateway_capability_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_gateway_capability_configuration(
