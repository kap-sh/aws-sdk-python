"""Generated from Smithy shape ``com.amazonaws.iotsitewise#ListAssetRelationships``."""
...
import capo_iotsitewise.types.asset_relationship_summaries
...
def handle_response(
    ...
    out: capo_iotsitewise.types.list_asset_relationships_response.ListAssetRelationshipsResponse = capo_iotsitewise.types.list_asset_relationships_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/assets/{assetId}/assetRelationships"
    url = url.replace("{assetId}", quote(str(input_["asset_id"]), safe=""))
    params: dict[str, str] = {}
    if "traversal_type" in input_:
        params["traversalType"] = str(input_["traversal_type"])
...
async def async_list_asset_relationships(
