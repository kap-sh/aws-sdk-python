"""Generated from Smithy shape ``com.amazonaws.iotsitewise#BatchAssociateProjectAssets``."""
...
def handle_response(
    ...
    out: capo_iotsitewise.types.batch_associate_project_assets_response.BatchAssociateProjectAssetsResponse = capo_iotsitewise.types.batch_associate_project_assets_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/projects/{projectId}/assets/associate"
    url = url.replace("{projectId}", quote(str(input_["project_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iotsitewise.types.batch_associate_project_assets_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_associate_project_assets(
