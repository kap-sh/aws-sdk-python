...
if TYPE_CHECKING:
    ...
    import capo_iotsitewise.types.monitor_error_message


class MonitorErrorDetails(TypedDict, closed=True):
    code: NotRequired["capo_iotsitewise.types.monitor_error_code.MonitorErrorCode"]
    """<p>The error code.</p>"""
    message: NotRequired[
        "capo_iotsitewise.types.monitor_error_message.MonitorErrorMessage"
    ]
...
def serialize_json(value: MonitorErrorDetails) -> dict:
    ...
    if "code" in value:
        ...
        out["code"] = capo_iotsitewise.types.monitor_error_code.serialize_json(
            value["code"]
        )
    if "message" in value:
