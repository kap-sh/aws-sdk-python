...
class BatchGetAssetPropertyAggregatesErrorInfo(TypedDict, closed=True):
    error_code: "capo_iotsitewise.types.batch_get_asset_property_aggregates_error_code.BatchGetAssetPropertyAggregatesErrorCode"
    """<p>The error code.</p>"""
...
def serialize_json(value: BatchGetAssetPropertyAggregatesErrorInfo) -> dict:
    ...
    out["errorCode"] = (
        capo_iotsitewise.types.batch_get_asset_property_aggregates_error_code.serialize_json(
            value["error_code"]
        )
    )
    import capo_iotsitewise.types.timestamp
...
def deserialize_json(data: dict) -> BatchGetAssetPropertyAggregatesErrorInfo:
    ...
    if "errorCode" in data:
        ...
        out["error_code"] = (
            capo_iotsitewise.types.batch_get_asset_property_aggregates_error_code.deserialize_json(
                data["errorCode"]
            )
        )
    else:
