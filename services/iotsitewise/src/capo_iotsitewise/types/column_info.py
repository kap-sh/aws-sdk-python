...
if TYPE_CHECKING:
    import capo_iotsitewise.types.column_type
    import capo_iotsitewise.types.string


class ColumnInfo(TypedDict, closed=True):
    name: NotRequired["capo_iotsitewise.types.string.String"]
    """<p>The name of the column description.</p>"""
    type: NotRequired["capo_iotsitewise.types.column_type.ColumnType"]
...
def serialize_json(value: ColumnInfo) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = capo_iotsitewise.types.column_type.serialize_json(value["type"])
    return out
...
def deserialize_json(data: dict) -> ColumnInfo:
    ...
    if "type" in data:
        ...
        out["type"] = capo_iotsitewise.types.column_type.deserialize_json(data["type"])
    return out
