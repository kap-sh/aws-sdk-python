...
class Source(TypedDict, closed=True):
    arn: NotRequired["capo_iotsitewise.types.string.String"]
    """<p>Contains the ARN of the dataset. If the source is Kendra, it's the ARN of the Kendra index.</p>"""
    location: NotRequired["capo_iotsitewise.types.location.Location"]
...
def serialize_json(value: Source) -> dict:
    ...
    if "location" in value:
        ...
        out["location"] = capo_iotsitewise.types.location.serialize_json(
            value["location"]
        )
    return out
