...
class BatchPutAssetPropertyValueRequest(TypedDict):
...
def serialize_json(value: BatchPutAssetPropertyValueRequest) -> dict:
    ...
    out["entries"] = (
        aws_sdk_iotsitewise.types.put_asset_property_value_entries.serialize_json(
            value["entries"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> BatchPutAssetPropertyValueRequest:
    ...
    if "entries" in data:
        ...
        out["entries"] = (
            aws_sdk_iotsitewise.types.put_asset_property_value_entries.deserialize_json(
                data["entries"]
            )
        )
    else:
