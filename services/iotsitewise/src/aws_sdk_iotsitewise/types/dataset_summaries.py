...
DatasetSummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.dataset_summary.DatasetSummary"
]
...
def serialize_json(value: DatasetSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iotsitewise.types.dataset_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> DatasetSummaries:
    ...
    for item in data:
        out.append(aws_sdk_iotsitewise.types.dataset_summary.deserialize_json(item))
    return out
