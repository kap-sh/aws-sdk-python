...
class CreateBulkImportJobResponse(TypedDict):
    job_id: "aws_sdk_iotsitewise.types.id.ID"
    """<p>The ID of the job.</p>"""
    job_name: "aws_sdk_iotsitewise.types.name.Name"
    """<p>The unique name that helps identify the job request.</p>"""
    job_status: "aws_sdk_iotsitewise.types.job_status.JobStatus"
...
def serialize_json(value: CreateBulkImportJobResponse) -> dict:
    ...
    out["jobStatus"] = aws_sdk_iotsitewise.types.job_status.serialize_json(
        value["job_status"]
    )
    return out
...
def deserialize_json(data: dict) -> CreateBulkImportJobResponse:
    ...
    if "jobStatus" in data:
        ...
        out["job_status"] = aws_sdk_iotsitewise.types.job_status.deserialize_json(
            data["jobStatus"]
        )
    else:
