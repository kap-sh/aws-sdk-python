"""Generated from Smithy shape ``com.amazonaws.iotsitewise#BatchGetAssetPropertyValueHistoryRequest``."""
...
class BatchGetAssetPropertyValueHistoryRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: BatchGetAssetPropertyValueHistoryRequest) -> dict:
    out: dict = {}
    import aws_sdk_iotsitewise.types.batch_get_asset_property_value_history_entries

    out["entries"] = (
        aws_sdk_iotsitewise.types.batch_get_asset_property_value_history_entries.serialize_json(
            value["entries"]
        )
    )
...
def deserialize_json(data: dict) -> BatchGetAssetPropertyValueHistoryRequest:
    ...
    if "entries" in data:
        ...
        out["entries"] = (
            aws_sdk_iotsitewise.types.batch_get_asset_property_value_history_entries.deserialize_json(
                data["entries"]
            )
        )
    else:
