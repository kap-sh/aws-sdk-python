...
ExecutionSummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.execution_summary.ExecutionSummary"
]
...
def serialize_json(value: ExecutionSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iotsitewise.types.execution_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ExecutionSummaries:
    ...
    for item in data:
        out.append(aws_sdk_iotsitewise.types.execution_summary.deserialize_json(item))
    return out
