...
AssetModelPropertySummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.asset_model_property_summary.AssetModelPropertySummary"
]
...
def serialize_json(value: AssetModelPropertySummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_iotsitewise.types.asset_model_property_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AssetModelPropertySummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_iotsitewise.types.asset_model_property_summary.deserialize_json(
                item
            )
        )
    return out
