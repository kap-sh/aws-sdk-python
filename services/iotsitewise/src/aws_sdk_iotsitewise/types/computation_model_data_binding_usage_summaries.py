...
ComputationModelDataBindingUsageSummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.computation_model_data_binding_usage_summary.ComputationModelDataBindingUsageSummary"
]
...
def serialize_json(value: ComputationModelDataBindingUsageSummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_iotsitewise.types.computation_model_data_binding_usage_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ComputationModelDataBindingUsageSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_iotsitewise.types.computation_model_data_binding_usage_summary.deserialize_json(
                item
            )
        )
    return out
