...
GatewaySummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.gateway_summary.GatewaySummary"
]
...
def serialize_json(value: GatewaySummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iotsitewise.types.gateway_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> GatewaySummaries:
    ...
    for item in data:
        out.append(aws_sdk_iotsitewise.types.gateway_summary.deserialize_json(item))
    return out
