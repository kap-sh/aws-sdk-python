...
AssetModelSummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.asset_model_summary.AssetModelSummary"
]
...
def serialize_json(value: AssetModelSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iotsitewise.types.asset_model_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> AssetModelSummaries:
    ...
    for item in data:
        out.append(aws_sdk_iotsitewise.types.asset_model_summary.deserialize_json(item))
    return out
