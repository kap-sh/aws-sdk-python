...
ActionSummaries: TypeAlias = list[
    "aws_sdk_iotsitewise.types.action_summary.ActionSummary"
]
...
def serialize_json(value: ActionSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iotsitewise.types.action_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ActionSummaries:
    ...
    out: ActionSummaries = []
    for item in data:
        out.append(aws_sdk_iotsitewise.types.action_summary.deserialize_json(item))
    return out
