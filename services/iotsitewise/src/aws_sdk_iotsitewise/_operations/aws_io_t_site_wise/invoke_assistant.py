...
from aws_sdk_iotsitewise._protocol.eventstream import (
    MessageDecoder,
...
def handle_response(
    ...
    _union_deser = aws_sdk_iotsitewise.types.response_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_iotsitewise.types.invoke_assistant_response.InvokeAssistantResponse = {
...
def build_request(
    ...
    input_: aws_sdk_iotsitewise.types.invoke_assistant_request.InvokeAssistantRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_iotsitewise.types.invoke_assistant_request.serialize_json(input_)
    ).encode()
