"""Generated from Smithy shape ``com.amazonaws.iotsitewise#UpdatePortal``."""
...
def handle_response(
    ...
    out: aws_sdk_iotsitewise.types.update_portal_response.UpdatePortalResponse = (
        aws_sdk_iotsitewise.types.update_portal_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iotsitewise.types.update_portal_request.UpdatePortalRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/portals/{portalId}"
    url = url.replace("{portalId}", quote(str(input_["portal_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_iotsitewise.types.update_portal_request.serialize_json(input_)
    ).encode()
