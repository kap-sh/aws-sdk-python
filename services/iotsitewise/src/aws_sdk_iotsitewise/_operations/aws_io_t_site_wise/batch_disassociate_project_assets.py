"""Generated from Smithy shape ``com.amazonaws.iotsitewise#BatchDisassociateProjectAssets``."""
...
def handle_response(
    ...
    out: aws_sdk_iotsitewise.types.batch_disassociate_project_assets_response.BatchDisassociateProjectAssetsResponse = aws_sdk_iotsitewise.types.batch_disassociate_project_assets_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iotsitewise.types.batch_disassociate_project_assets_request.BatchDisassociateProjectAssetsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/projects/{projectId}/assets/disassociate"
    url = url.replace("{projectId}", quote(str(input_["project_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iotsitewise.types.batch_disassociate_project_assets_request

    body: bytes | None = json.dumps(
        aws_sdk_iotsitewise.types.batch_disassociate_project_assets_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_disassociate_project_assets(
