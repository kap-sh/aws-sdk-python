"""Generated from Smithy shape ``com.amazonaws.iotsitewise#CreateGateway``."""
...
def handle_response(
    ...
    out: aws_sdk_iotsitewise.types.create_gateway_response.CreateGatewayResponse = (
        aws_sdk_iotsitewise.types.create_gateway_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iotsitewise.types.create_gateway_request.CreateGatewayRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/20200301/gateways"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iotsitewise.types.create_gateway_request

    body: bytes | None = json.dumps(
        aws_sdk_iotsitewise.types.create_gateway_request.serialize_json(input_)
    ).encode()
