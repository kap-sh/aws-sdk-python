"""Generated from Smithy shape ``com.amazonaws.iotsitewise#UpdateAccessPolicy``."""
...
def handle_response(
    ...
    out: aws_sdk_iotsitewise.types.update_access_policy_response.UpdateAccessPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iotsitewise.types.update_access_policy_request.UpdateAccessPolicyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/access-policies/{accessPolicyId}"
    url = url.replace(
        "{accessPolicyId}", quote(str(input_["access_policy_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_iotsitewise.types.update_access_policy_request.serialize_json(input_)
    ).encode()
