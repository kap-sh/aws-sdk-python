"""Generated from Smithy shape ``com.amazonaws.rds#DeleteDBClusterEndpoint``."""
...
import aws_sdk_rds.errors.db_cluster_endpoint_not_found_fault
...
import aws_sdk_rds.types.db_cluster_endpoint
import aws_sdk_rds.types.delete_db_cluster_endpoint_message
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DBClusterEndpointNotFoundFault":
            raise aws_sdk_rds.errors.db_cluster_endpoint_not_found_fault.DBClusterEndpointNotFoundFault.from_query(
                root
            )
...
) -> aws_sdk_rds.types.db_cluster_endpoint.DBClusterEndpoint:
    root = fromstring(response.read())
    result = root.find("DeleteDBClusterEndpointResult")
    out: aws_sdk_rds.types.db_cluster_endpoint.DBClusterEndpoint = (
        aws_sdk_rds.types.db_cluster_endpoint.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_rds.types.delete_db_cluster_endpoint_message.DeleteDBClusterEndpointMessage,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "DeleteDBClusterEndpoint"))
    pairs.append(("Version", "2014-10-31"))
    import aws_sdk_rds.types.delete_db_cluster_endpoint_message

    aws_sdk_rds.types.delete_db_cluster_endpoint_message.serialize_query(
        input_, pairs, ""
    )
...
def delete_db_cluster_endpoint(
    ...
    input_: aws_sdk_rds.types.delete_db_cluster_endpoint_message.DeleteDBClusterEndpointMessage,
) -> tuple[aws_sdk_rds.types.db_cluster_endpoint.DBClusterEndpoint, zapros.Response]:
...
async def async_delete_db_cluster_endpoint(
