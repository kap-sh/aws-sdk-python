...
DBShardGroupsList: TypeAlias = list["aws_sdk_rds.types.db_shard_group.DBShardGroup"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.db_shard_group.serialize_query(
...
def deserialize_query(el: Element) -> DBShardGroupsList:
    ...
    for child in el.findall("DBShardGroup"):
        out.append(aws_sdk_rds.types.db_shard_group.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.db_shard_group.serialize_query(item, pairs, f"{prefix}.{n}")
