...
if TYPE_CHECKING:
    import aws_sdk_rds.types.user_auth_config_info

UserAuthConfigInfoList: TypeAlias = list[
    "aws_sdk_rds.types.user_auth_config_info.UserAuthConfigInfo"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.user_auth_config_info.serialize_query(
...
def deserialize_query(el: Element) -> UserAuthConfigInfoList:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_rds.types.user_auth_config_info.deserialize_query(child))
    return out
