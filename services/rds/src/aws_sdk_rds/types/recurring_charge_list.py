...
if TYPE_CHECKING:
    import aws_sdk_rds.types.recurring_charge

RecurringChargeList: TypeAlias = list[
    "aws_sdk_rds.types.recurring_charge.RecurringCharge"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.recurring_charge.serialize_query(
...
def deserialize_query(el: Element) -> RecurringChargeList:
    ...
    for child in el.findall("RecurringCharge"):
        out.append(aws_sdk_rds.types.recurring_charge.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.recurring_charge.serialize_query(item, pairs, f"{prefix}.{n}")
