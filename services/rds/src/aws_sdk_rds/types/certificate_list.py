...
CertificateList: TypeAlias = list["aws_sdk_rds.types.certificate.Certificate"]
...
def deserialize_query(el: Element) -> CertificateList:
    ...
    for child in el.findall("Certificate"):
        out.append(aws_sdk_rds.types.certificate.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.certificate.serialize_query(item, pairs, f"{prefix}.{n}")
