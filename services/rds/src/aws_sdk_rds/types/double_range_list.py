...
if TYPE_CHECKING:
    import aws_sdk_rds.types.double_range

DoubleRangeList: TypeAlias = list["aws_sdk_rds.types.double_range.DoubleRange"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.double_range.serialize_query(
...
def deserialize_query(el: Element) -> DoubleRangeList:
    ...
    for child in el.findall("DoubleRange"):
        out.append(aws_sdk_rds.types.double_range.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.double_range.serialize_query(item, pairs, f"{prefix}.{n}")
