...
if TYPE_CHECKING:
    import aws_sdk_rds.types.db_snapshot_tenant_database

DBSnapshotTenantDatabasesList: TypeAlias = list[
    "aws_sdk_rds.types.db_snapshot_tenant_database.DBSnapshotTenantDatabase"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.db_snapshot_tenant_database.serialize_query(
...
def deserialize_query(el: Element) -> DBSnapshotTenantDatabasesList:
    ...
    for child in el.findall("DBSnapshotTenantDatabase"):
        out.append(
            aws_sdk_rds.types.db_snapshot_tenant_database.deserialize_query(child)
        )
    return out
