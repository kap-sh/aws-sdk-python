...
DBClusterList: TypeAlias = list["aws_sdk_rds.types.db_cluster.DBCluster"]
...
def deserialize_query(el: Element) -> DBClusterList:
    ...
    for child in el.findall("DBCluster"):
        out.append(aws_sdk_rds.types.db_cluster.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.db_cluster.serialize_query(item, pairs, f"{prefix}.{n}")
