...
IntegrationList: TypeAlias = list["aws_sdk_rds.types.integration.Integration"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.integration.serialize_query(
...
def deserialize_query(el: Element) -> IntegrationList:
    ...
    for child in el.findall("Integration"):
        out.append(aws_sdk_rds.types.integration.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.integration.serialize_query(item, pairs, f"{prefix}.{n}")
