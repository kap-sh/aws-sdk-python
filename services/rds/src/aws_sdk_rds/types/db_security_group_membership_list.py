...
if TYPE_CHECKING:
    import aws_sdk_rds.types.db_security_group_membership

DBSecurityGroupMembershipList: TypeAlias = list[
    "aws_sdk_rds.types.db_security_group_membership.DBSecurityGroupMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.db_security_group_membership.serialize_query(
...
def deserialize_query(el: Element) -> DBSecurityGroupMembershipList:
    ...
    for child in el.findall("DBSecurityGroup"):
        out.append(
            aws_sdk_rds.types.db_security_group_membership.deserialize_query(child)
        )
    return out
