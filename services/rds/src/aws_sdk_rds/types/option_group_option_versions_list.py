...
if TYPE_CHECKING:
    import aws_sdk_rds.types.option_version

OptionGroupOptionVersionsList: TypeAlias = list[
    "aws_sdk_rds.types.option_version.OptionVersion"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.option_version.serialize_query(
...
def deserialize_query(el: Element) -> OptionGroupOptionVersionsList:
    ...
    for child in el.findall("OptionVersion"):
        out.append(aws_sdk_rds.types.option_version.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.option_version.serialize_query(item, pairs, f"{prefix}.{n}")
