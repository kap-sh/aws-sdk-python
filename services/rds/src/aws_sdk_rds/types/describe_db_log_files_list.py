...
if TYPE_CHECKING:
    import aws_sdk_rds.types.describe_db_log_files_details

DescribeDBLogFilesList: TypeAlias = list[
    "aws_sdk_rds.types.describe_db_log_files_details.DescribeDBLogFilesDetails"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.describe_db_log_files_details.serialize_query(
...
def deserialize_query(el: Element) -> DescribeDBLogFilesList:
    ...
    for child in el.findall("DescribeDBLogFilesDetails"):
        out.append(
            aws_sdk_rds.types.describe_db_log_files_details.deserialize_query(child)
        )
    return out
