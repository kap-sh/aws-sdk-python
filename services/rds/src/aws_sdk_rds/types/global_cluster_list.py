"""Generated from Smithy shape ``com.amazonaws.rds#GlobalClusterList``."""
...
GlobalClusterList: TypeAlias = list["aws_sdk_rds.types.global_cluster.GlobalCluster"]
...
def deserialize_query(el: Element) -> GlobalClusterList:
    ...
    for child in el.findall("GlobalClusterMember"):
        out.append(aws_sdk_rds.types.global_cluster.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.global_cluster.serialize_query(item, pairs, f"{prefix}.{n}")
