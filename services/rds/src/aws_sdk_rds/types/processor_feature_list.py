...
if TYPE_CHECKING:
    import aws_sdk_rds.types.processor_feature

ProcessorFeatureList: TypeAlias = list[
    "aws_sdk_rds.types.processor_feature.ProcessorFeature"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.processor_feature.serialize_query(
...
def deserialize_query(el: Element) -> ProcessorFeatureList:
    ...
    for child in el.findall("ProcessorFeature"):
        out.append(aws_sdk_rds.types.processor_feature.deserialize_query(child))
    return out
