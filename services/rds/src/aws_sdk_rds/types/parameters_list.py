...
if TYPE_CHECKING:
    import aws_sdk_rds.types.parameter

ParametersList: TypeAlias = list["aws_sdk_rds.types.parameter.Parameter"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.parameter.serialize_query(
...
def deserialize_query(el: Element) -> ParametersList:
    ...
    for child in el.findall("Parameter"):
        out.append(aws_sdk_rds.types.parameter.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_rds.types.parameter.serialize_query(item, pairs, f"{prefix}.{n}")
