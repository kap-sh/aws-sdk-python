...
class RDSError(Exception):
    """Base class for all RDS service errors."""


class ServiceError(RDSError):
...
class SerializationError(RDSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(RDSError):
...
class WaiterFailedError(RDSError):
...
class WaiterTimeoutError(RDSError):
