"""Generated from Smithy shape ``com.amazonaws.rds#DescribeDBClusterBacktracks``."""
...
import capo_rds.types.db_cluster_backtrack_message
import capo_rds.types.describe_db_cluster_backtracks_message
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DBClusterBacktrackNotFoundFault":
            raise capo_rds.errors.db_cluster_backtrack_not_found_fault.DBClusterBacktrackNotFoundFault.from_query(
                error_el, message
            )
...
) -> capo_rds.types.db_cluster_backtrack_message.DBClusterBacktrackMessage:
    root = fromstring(response.read())
    result = root.find("DescribeDBClusterBacktracksResult")
    out: capo_rds.types.db_cluster_backtrack_message.DBClusterBacktrackMessage = (
        capo_rds.types.db_cluster_backtrack_message.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeDBClusterBacktracks"))
    pairs.append(("Version", "2014-10-31"))
    capo_rds.types.describe_db_cluster_backtracks_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_db_cluster_backtracks(
