"""Generated from Smithy shape ``com.amazonaws.rds#CreateOptionGroup``."""
...
import capo_rds.errors.option_group_already_exists_fault
import capo_rds.errors.option_group_quota_exceeded_fault
import capo_rds.types.create_option_group_message
import capo_rds.types.create_option_group_result
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "OptionGroupAlreadyExistsFault":
            raise capo_rds.errors.option_group_already_exists_fault.OptionGroupAlreadyExistsFault.from_query(
                error_el, message
            )
        case "OptionGroupQuotaExceededFault":
            raise capo_rds.errors.option_group_quota_exceeded_fault.OptionGroupQuotaExceededFault.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateOptionGroupResult")
    out: capo_rds.types.create_option_group_result.CreateOptionGroupResult = (
        capo_rds.types.create_option_group_result.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "CreateOptionGroup"))
    pairs.append(("Version", "2014-10-31"))
    capo_rds.types.create_option_group_message.serialize_query(input_, pairs, "")
...
def create_option_group(
    ...
    capo_rds.types.create_option_group_result.CreateOptionGroupResult, zapros.Response
]:
...
async def async_create_option_group(
