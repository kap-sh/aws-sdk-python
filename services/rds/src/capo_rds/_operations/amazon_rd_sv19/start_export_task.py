...
import capo_rds.errors.export_task_already_exists_fault
import capo_rds.errors.iam_role_missing_permissions_fault
import capo_rds.errors.iam_role_not_found_fault
import capo_rds.errors.invalid_export_only_fault
import capo_rds.errors.invalid_export_source_state_fault
import capo_rds.errors.invalid_s3_bucket_fault
...
import capo_rds.types.start_export_task_message
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ExportTaskAlreadyExistsFault":
            raise capo_rds.errors.export_task_already_exists_fault.ExportTaskAlreadyExistsFault.from_query(
                root
            )
        case "IamRoleMissingPermissionsFault":
            raise capo_rds.errors.iam_role_missing_permissions_fault.IamRoleMissingPermissionsFault.from_query(
                root
            )
        case "IamRoleNotFoundFault":
            raise capo_rds.errors.iam_role_not_found_fault.IamRoleNotFoundFault.from_query(
                root
            )
        case "InvalidExportOnlyFault":
            raise capo_rds.errors.invalid_export_only_fault.InvalidExportOnlyFault.from_query(
                root
            )
        case "InvalidExportSourceStateFault":
            raise capo_rds.errors.invalid_export_source_state_fault.InvalidExportSourceStateFault.from_query(
                root
            )
...
def handle_response(response: zapros.Response) -> capo_rds.types.export_task.ExportTask:
    root = fromstring(response.read())
    result = root.find("StartExportTaskResult")
    out: capo_rds.types.export_task.ExportTask = (
        capo_rds.types.export_task.deserialize_query(
...
def build_request(
    ...
    capo_rds.types.start_export_task_message.serialize_query(input_, pairs, "")
...
def start_export_task(
    ...
    input_: capo_rds.types.start_export_task_message.StartExportTaskMessage,
) -> tuple[capo_rds.types.export_task.ExportTask, zapros.Response]:
