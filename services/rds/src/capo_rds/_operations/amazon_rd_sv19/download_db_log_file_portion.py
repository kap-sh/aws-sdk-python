"""Generated from Smithy shape ``com.amazonaws.rds#DownloadDBLogFilePortion``."""
...
import capo_rds.errors.db_instance_not_ready_fault
import capo_rds.errors.db_log_file_not_found_fault
import capo_rds.types.download_db_log_file_portion_details
import capo_rds.types.download_db_log_file_portion_message
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "DBInstanceNotReady":
            raise capo_rds.errors.db_instance_not_ready_fault.DBInstanceNotReadyFault.from_query(
                error_el, message
            )
        case "DBLogFileNotFoundFault":
            raise capo_rds.errors.db_log_file_not_found_fault.DBLogFileNotFoundFault.from_query(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_rds.types.download_db_log_file_portion_details.DownloadDBLogFilePortionDetails = capo_rds.types.download_db_log_file_portion_details.deserialize_query(
...
def build_request(
    ...
    capo_rds.types.download_db_log_file_portion_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_download_db_log_file_portion(
