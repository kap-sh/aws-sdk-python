...
import capo_rds.errors.authorization_already_exists_fault
import capo_rds.errors.authorization_quota_exceeded_fault
import capo_rds.errors.db_security_group_not_found_fault
import capo_rds.errors.invalid_db_security_group_state_fault
import capo_rds.types.authorize_db_security_group_ingress_message
import capo_rds.types.authorize_db_security_group_ingress_result
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AuthorizationAlreadyExists":
            raise capo_rds.errors.authorization_already_exists_fault.AuthorizationAlreadyExistsFault.from_query(
                error_el, message
            )
        case "AuthorizationQuotaExceeded":
            raise capo_rds.errors.authorization_quota_exceeded_fault.AuthorizationQuotaExceededFault.from_query(
                error_el, message
            )
...
) -> capo_rds.types.authorize_db_security_group_ingress_result.AuthorizeDBSecurityGroupIngressResult:
    root = fromstring(response.read())
    result = root.find("AuthorizeDBSecurityGroupIngressResult")
    out: capo_rds.types.authorize_db_security_group_ingress_result.AuthorizeDBSecurityGroupIngressResult = capo_rds.types.authorize_db_security_group_ingress_result.deserialize_query(
...
def build_request(
    ...
    capo_rds.types.authorize_db_security_group_ingress_message.serialize_query(
        input_, pairs, ""
    )
