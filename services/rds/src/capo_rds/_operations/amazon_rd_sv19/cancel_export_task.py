"""Generated from Smithy shape ``com.amazonaws.rds#CancelExportTask``."""
...
import capo_rds.errors.invalid_export_task_state_fault
import capo_rds.types.cancel_export_task_message
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ExportTaskNotFound":
            raise capo_rds.errors.export_task_not_found_fault.ExportTaskNotFoundFault.from_query(
                error_el, message
            )
        case "InvalidExportTaskStateFault":
            raise capo_rds.errors.invalid_export_task_state_fault.InvalidExportTaskStateFault.from_query(
                error_el, message
            )
        case _:
...
def handle_response(response: zapros.Response) -> capo_rds.types.export_task.ExportTask:
    root = fromstring(response.read())
    result = root.find("CancelExportTaskResult")
...
def build_request(
    ...
    capo_rds.types.cancel_export_task_message.serialize_query(input_, pairs, "")
