...
if TYPE_CHECKING:
    import capo_rds.types.valid_volume_options

ValidVolumeOptionsList: TypeAlias = list[
    "capo_rds.types.valid_volume_options.ValidVolumeOptions"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.valid_volume_options.serialize_query(
...
def deserialize_query(el: Element) -> ValidVolumeOptionsList:
    ...
    for child in el.findall("member"):
        out.append(capo_rds.types.valid_volume_options.deserialize_query(child))
    return out
