...
if TYPE_CHECKING:
    import capo_rds.types.upgrade_target

ValidUpgradeTargetList: TypeAlias = list["capo_rds.types.upgrade_target.UpgradeTarget"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.upgrade_target.serialize_query(
...
def deserialize_query(el: Element) -> ValidUpgradeTargetList:
    ...
    for child in el.findall("UpgradeTarget"):
        out.append(capo_rds.types.upgrade_target.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.upgrade_target.serialize_query(item, pairs, f"{prefix}.{n}")
