"""Generated from Smithy shape ``com.amazonaws.rds#CreateGlobalClusterResult``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from capo_rds._protocol.xml import Element

if TYPE_CHECKING:
    import capo_rds.types.global_cluster


class CreateGlobalClusterResult(TypedDict, closed=True):
    global_cluster: NotRequired["capo_rds.types.global_cluster.GlobalCluster"]


# --- awsQuery ser/de ---
def serialize_query(
    value: CreateGlobalClusterResult, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "global_cluster" in value:
        import capo_rds.types.global_cluster

        capo_rds.types.global_cluster.serialize_query(
            value["global_cluster"], pairs, f"{prefix}.GlobalCluster"
        )


def deserialize_query(el: Element) -> CreateGlobalClusterResult:
    out: CreateGlobalClusterResult = {}  # type: ignore[typeddict-item]
    child_global_cluster = el.find("GlobalCluster")
    if child_global_cluster is not None:
        import capo_rds.types.global_cluster

        out["global_cluster"] = capo_rds.types.global_cluster.deserialize_query(
            child_global_cluster
        )
    return out
