...
if TYPE_CHECKING:
    import capo_rds.types.db_major_engine_version

DBMajorEngineVersionsList: TypeAlias = list[
    "capo_rds.types.db_major_engine_version.DBMajorEngineVersion"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.db_major_engine_version.serialize_query(
...
def deserialize_query(el: Element) -> DBMajorEngineVersionsList:
    ...
    for child in el.findall("DBMajorEngineVersion"):
        out.append(capo_rds.types.db_major_engine_version.deserialize_query(child))
    return out
