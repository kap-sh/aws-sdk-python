...
if TYPE_CHECKING:
    import capo_rds.types.option_group_membership

OptionGroupMembershipList: TypeAlias = list[
    "capo_rds.types.option_group_membership.OptionGroupMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.option_group_membership.serialize_query(
...
def deserialize_query(el: Element) -> OptionGroupMembershipList:
    ...
    for child in el.findall("OptionGroupMembership"):
        out.append(capo_rds.types.option_group_membership.deserialize_query(child))
    return out
