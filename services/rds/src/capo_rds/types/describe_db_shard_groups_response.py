...
class DescribeDBShardGroupsResponse(TypedDict, closed=True):
    db_shard_groups: NotRequired[
        "capo_rds.types.db_shard_groups_list.DBShardGroupsList"
    ]
...
def serialize_query(
    ...
    if "db_shard_groups" in value:
        ...
        capo_rds.types.db_shard_groups_list.serialize_query(
            value["db_shard_groups"], pairs, f"{key_prefix}DBShardGroups"
        )
    if "marker" in value:
...
def deserialize_query(el: Element) -> DescribeDBShardGroupsResponse:
    ...
    if child_db_shard_groups is not None:
        ...
        out["db_shard_groups"] = capo_rds.types.db_shard_groups_list.deserialize_query(
