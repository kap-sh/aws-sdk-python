...
if TYPE_CHECKING:
    import capo_rds.types.domain_membership

DomainMembershipList: TypeAlias = list[
    "capo_rds.types.domain_membership.DomainMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.domain_membership.serialize_query(
...
def deserialize_query(el: Element) -> DomainMembershipList:
    ...
    for child in el.findall("DomainMembership"):
        out.append(capo_rds.types.domain_membership.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.domain_membership.serialize_query(item, pairs, f"{prefix}.{n}")
