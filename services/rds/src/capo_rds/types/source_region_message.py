...
class SourceRegionMessage(TypedDict, closed=True):
    ...
    source_regions: NotRequired["capo_rds.types.source_region_list.SourceRegionList"]
...
def serialize_query(
    ...
    if "source_regions" in value:
        ...
        capo_rds.types.source_region_list.serialize_query(
            value["source_regions"], pairs, f"{key_prefix}SourceRegions"
        )
...
def deserialize_query(el: Element) -> SourceRegionMessage:
    ...
    if child_source_regions is not None:
        ...
        out["source_regions"] = capo_rds.types.source_region_list.deserialize_query(
