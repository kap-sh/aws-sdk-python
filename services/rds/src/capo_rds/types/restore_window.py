...
class RestoreWindow(TypedDict, closed=True):
    earliest_time: NotRequired["capo_rds.types.t_stamp.TStamp"]
    """<p>The earliest time you can restore an instance to.</p>"""
    latest_time: NotRequired["capo_rds.types.t_stamp.TStamp"]
...
def deserialize_query(el: Element) -> RestoreWindow:
    ...
    if child_earliest_time is not None:
        ...
        out["earliest_time"] = capo_rds.types.t_stamp.deserialize_query(
            child_earliest_time
        )
    ...
    if child_latest_time is not None:
        ...
        out["latest_time"] = capo_rds.types.t_stamp.deserialize_query(child_latest_time)
    return out
