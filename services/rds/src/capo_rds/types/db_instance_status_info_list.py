...
if TYPE_CHECKING:
    import capo_rds.types.db_instance_status_info

DBInstanceStatusInfoList: TypeAlias = list[
    "capo_rds.types.db_instance_status_info.DBInstanceStatusInfo"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.db_instance_status_info.serialize_query(
...
def deserialize_query(el: Element) -> DBInstanceStatusInfoList:
    ...
    for child in el.findall("DBInstanceStatusInfo"):
        out.append(capo_rds.types.db_instance_status_info.deserialize_query(child))
    return out
