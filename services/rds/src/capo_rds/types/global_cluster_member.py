...
class GlobalClusterMember(TypedDict, closed=True):
    ...
    readers: NotRequired["capo_rds.types.readers_arn_list.ReadersArnList"]
    """<p>The Amazon Resource Name (ARN) for each read-only secondary cluster associated with the global cluster.</p>"""
    is_writer: NotRequired["capo_rds.types.boolean.Boolean"]
...
def deserialize_query(el: Element) -> GlobalClusterMember:
    ...
    if child_readers is not None:
        ...
        out["readers"] = capo_rds.types.readers_arn_list.deserialize_query(
            child_readers
        )
    ...
    if child_synchronization_status is not None:
        ...
        out["synchronization_status"] = (
            capo_rds.types.global_cluster_member_synchronization_status.deserialize_query(
