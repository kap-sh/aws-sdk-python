...
if TYPE_CHECKING:
    import capo_rds.types.blue_green_deployment_task

BlueGreenDeploymentTaskList: TypeAlias = list[
    "capo_rds.types.blue_green_deployment_task.BlueGreenDeploymentTask"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_rds.types.blue_green_deployment_task.serialize_query(
...
def deserialize_query(el: Element) -> BlueGreenDeploymentTaskList:
    ...
    for child in el.findall("member"):
        out.append(capo_rds.types.blue_green_deployment_task.deserialize_query(child))
    return out
