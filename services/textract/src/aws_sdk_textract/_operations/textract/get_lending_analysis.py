"""Generated from Smithy shape ``com.amazonaws.textract#GetLendingAnalysis``."""
...
def build_request(
    ...
    input_: aws_sdk_textract.types.get_lending_analysis_request.GetLendingAnalysisRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    headers["X-Amz-Target"] = "Textract.GetLendingAnalysis"
    body: bytes | None = json.dumps(
        aws_sdk_textract.types.get_lending_analysis_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_get_lending_analysis(
