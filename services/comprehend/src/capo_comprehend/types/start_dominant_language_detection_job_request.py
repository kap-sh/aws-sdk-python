"""Generated from Smithy shape ``com.amazonaws.comprehend#StartDominantLanguageDetectionJobRequest``."""
...
class StartDominantLanguageDetectionJobRequest(TypedDict, closed=True):
    ...
    r"""<p>Configuration parameters for an optional private Virtual Private Cloud (VPC) containing the resources you are using for your dominant language detection job. For more information, see <a href=\"https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html\">Amazon VPC</a>. </p>"""
    tags: NotRequired["capo_comprehend.types.tag_list.TagList"]
    r"""<p>Tags to associate with the dominant language detection job. A tag is a key-value pair that adds metadata to a resource used by Amazon Comprehend. For example, a tag with \"Sales\" as the key might be added to a resource to indicate its use by the sales department.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: StartDominantLanguageDetectionJobRequest) -> dict:
    out: dict = {}
    import capo_comprehend.types.input_data_config
...
def deserialize_aws_json_1_1(data: dict) -> StartDominantLanguageDetectionJobRequest:
    out: StartDominantLanguageDetectionJobRequest = {}  # type: ignore[typeddict-item]
    if "InputDataConfig" in data:
        ...
        raise DeserializationError(
            "StartDominantLanguageDetectionJobRequest.input_data_config required"
        )
    if "OutputDataConfig" in data:
        ...
        raise DeserializationError(
            "StartDominantLanguageDetectionJobRequest.output_data_config required"
        )
    if "DataAccessRoleArn" in data:
        out["data_access_role_arn"] = data["DataAccessRoleArn"]
    else:
        raise DeserializationError(
            "StartDominantLanguageDetectionJobRequest.data_access_role_arn required"
        )
    if "JobName" in data:
