...
class ListEndpointsRequest(TypedDict, closed=True):
    filter: NotRequired["capo_comprehend.types.endpoint_filter.EndpointFilter"]
...
def serialize_aws_json_1_1(value: ListEndpointsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["Filter"] = capo_comprehend.types.endpoint_filter.serialize_aws_json_1_1(
            value["filter"]
        )
...
def deserialize_aws_json_1_1(data: dict) -> ListEndpointsRequest:
    ...
    if "Filter" in data:
        ...
        out["filter"] = capo_comprehend.types.endpoint_filter.deserialize_aws_json_1_1(
            data["Filter"]
        )
