...
EngineVersionsSummaryList: TypeAlias = list[
    "capo_m2.types.engine_versions_summary.EngineVersionsSummary"
]
...
def serialize_json(value: EngineVersionsSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_m2.types.engine_versions_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> EngineVersionsSummaryList:
    ...
    for item in data:
        out.append(capo_m2.types.engine_versions_summary.deserialize_json(item))
    return out
