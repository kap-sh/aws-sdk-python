...
class m2Error(Exception):
    """Base class for all m2 service errors."""


class ServiceError(m2Error):
...
class SerializationError(m2Error):
    """Raised when a value cannot be serialized."""


class DeserializationError(m2Error):
...
class WaiterFailedError(m2Error):
...
class WaiterTimeoutError(m2Error):
