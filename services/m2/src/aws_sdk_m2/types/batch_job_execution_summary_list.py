...
BatchJobExecutionSummaryList: TypeAlias = list[
    "aws_sdk_m2.types.batch_job_execution_summary.BatchJobExecutionSummary"
]
...
def serialize_json(value: BatchJobExecutionSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_m2.types.batch_job_execution_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> BatchJobExecutionSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_m2.types.batch_job_execution_summary.deserialize_json(item))
    return out
