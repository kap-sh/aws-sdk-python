...
def serialize_json(value: GetDiscoveredSchemaRequest) -> dict:
    ...
    if "events" in value:
        ...
        out["Events"] = (
            capo_schemas.types.__list_of_get_discovered_schema_version_item_input.serialize_json(
                value["events"]
            )
        )
    if "type" in value:
...
def deserialize_json(data: dict) -> GetDiscoveredSchemaRequest:
    ...
    if "Events" in data:
        ...
        out["events"] = (
            capo_schemas.types.__list_of_get_discovered_schema_version_item_input.deserialize_json(
                data["Events"]
            )
        )
    if "Type" in data:
