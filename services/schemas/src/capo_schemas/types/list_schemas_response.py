...
class ListSchemasResponse(TypedDict, closed=True):
    ...
    schemas: NotRequired[
        "capo_schemas.types.__list_of_schema_summary.__listOfSchemaSummary"
    ]
...
def serialize_json(value: ListSchemasResponse) -> dict:
    ...
    if "schemas" in value:
        ...
        out["Schemas"] = capo_schemas.types.__list_of_schema_summary.serialize_json(
            value["schemas"]
        )
    return out
...
def deserialize_json(data: dict) -> ListSchemasResponse:
    ...
    if "Schemas" in data:
        ...
        out["schemas"] = capo_schemas.types.__list_of_schema_summary.deserialize_json(
            data["Schemas"]
        )
    return out
