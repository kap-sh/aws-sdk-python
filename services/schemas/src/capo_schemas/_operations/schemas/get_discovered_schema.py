"""Generated from Smithy shape ``com.amazonaws.schemas#GetDiscoveredSchema``."""
...
def handle_response(
    ...
    out: capo_schemas.types.get_discovered_schema_response.GetDiscoveredSchemaResponse = capo_schemas.types.get_discovered_schema_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/discover"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_schemas.types.get_discovered_schema_request.serialize_json(input_)
    ).encode()
...
async def async_get_discovered_schema(
