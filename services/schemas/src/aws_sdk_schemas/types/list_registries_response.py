...
class ListRegistriesResponse(TypedDict, closed=True):
    ...
    registries: NotRequired[
        "aws_sdk_schemas.types.__list_of_registry_summary.__listOfRegistrySummary"
    ]
...
def serialize_json(value: ListRegistriesResponse) -> dict:
    ...
    if "registries" in value:
        ...
        out["Registries"] = (
            aws_sdk_schemas.types.__list_of_registry_summary.serialize_json(
                value["registries"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListRegistriesResponse:
    ...
    if "Registries" in data:
        ...
        out["registries"] = (
            aws_sdk_schemas.types.__list_of_registry_summary.deserialize_json(
                data["Registries"]
            )
        )
    return out
