"""Generated from Smithy shape ``com.amazonaws.schemas#UpdateDiscoverer``."""
...
def handle_response(
    ...
    out: aws_sdk_schemas.types.update_discoverer_response.UpdateDiscovererResponse = (
        aws_sdk_schemas.types.update_discoverer_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_schemas.types.update_discoverer_request.UpdateDiscovererRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/discoverers/id/{DiscovererId}"
    url = url.replace("{DiscovererId}", quote(str(input_["discoverer_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_schemas.types.update_discoverer_request

    body: bytes | None = json.dumps(
        aws_sdk_schemas.types.update_discoverer_request.serialize_json(input_)
    ).encode()
...
async def async_update_discoverer(
