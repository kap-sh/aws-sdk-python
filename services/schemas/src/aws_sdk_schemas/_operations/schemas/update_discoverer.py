...
def build_request(
    ...
    input: aws_sdk_schemas.types.update_discoverer_request.UpdateDiscovererRequest,
) -> zapros.Request:
    ...
    url = url.replace("{DiscovererId}", quote(str(input["discoverer_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_schemas.types.update_discoverer_request

    body: bytes | None = json.dumps(
        aws_sdk_schemas.types.update_discoverer_request.serialize_json(input)
    ).encode()
