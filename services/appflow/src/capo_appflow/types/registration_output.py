...
class RegistrationOutput(TypedDict, closed=True):
    ...
    status: NotRequired["capo_appflow.types.execution_status.ExecutionStatus"]
...
def serialize_json(value: RegistrationOutput) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_appflow.types.execution_status.serialize_json(
            value["status"]
        )
    return out
