...
class AggregationConfig(TypedDict, closed=True):
    aggregation_type: NotRequired["capo_appflow.types.aggregation_type.AggregationType"]
    """<p> Specifies whether Amazon AppFlow aggregates the flow records into a single file, or leave them unaggregated. </p>"""
    target_file_size: NotRequired["capo_appflow.types.long.Long"]
...
def serialize_json(value: AggregationConfig) -> dict:
    ...
    if "aggregation_type" in value:
        ...
        out["aggregationType"] = capo_appflow.types.aggregation_type.serialize_json(
