...
class AppflowError(Exception):
    """Base class for all Appflow service errors."""


class ServiceError(AppflowError):
...
class SerializationError(AppflowError):
    """Raised when a value cannot be serialized."""


class DeserializationError(AppflowError):
...
class WaiterFailedError(AppflowError):
...
class WaiterTimeoutError(AppflowError):
