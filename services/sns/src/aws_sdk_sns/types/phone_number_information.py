"""Generated from Smithy shape ``com.amazonaws.sns#PhoneNumberInformation``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from aws_sdk_sns._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_sns.types.iso2_country_code
    import aws_sdk_sns.types.number_capability_list
    import aws_sdk_sns.types.phone_number
    import aws_sdk_sns.types.route_type
    import aws_sdk_sns.types.string
    import aws_sdk_sns.types.timestamp


class PhoneNumberInformation(TypedDict, closed=True):
    created_at: NotRequired["aws_sdk_sns.types.timestamp.Timestamp"]
    """<p>The date and time when the phone number was created.</p>"""
    phone_number: NotRequired["aws_sdk_sns.types.phone_number.PhoneNumber"]
    """<p>The phone number.</p>"""
    status: NotRequired["aws_sdk_sns.types.string.String"]
    """<p>The status of the phone number.</p>"""
    iso2_country_code: NotRequired[
        "aws_sdk_sns.types.iso2_country_code.Iso2CountryCode"
    ]
    """<p>The two-character code for the country or region, in ISO 3166-1 alpha-2 format.</p>"""
    route_type: NotRequired["aws_sdk_sns.types.route_type.RouteType"]
    """<p>The list of supported routes.</p>"""
    number_capabilities: NotRequired[
        "aws_sdk_sns.types.number_capability_list.NumberCapabilityList"
    ]
    """<p>The capabilities of each phone number.</p>"""


# --- awsQuery ser/de ---
def serialize_query(
    value: PhoneNumberInformation, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "created_at" in value:
        import aws_sdk_sns.types.timestamp

        aws_sdk_sns.types.timestamp.serialize_query(
            value["created_at"], pairs, f"{prefix}.CreatedAt"
        )
    if "phone_number" in value:
        pairs.append((f"{prefix}.PhoneNumber", str(value["phone_number"])))
    if "status" in value:
        pairs.append((f"{prefix}.Status", str(value["status"])))
    if "iso2_country_code" in value:
        pairs.append((f"{prefix}.Iso2CountryCode", str(value["iso2_country_code"])))
    if "route_type" in value:
        import aws_sdk_sns.types.route_type

        aws_sdk_sns.types.route_type.serialize_query(
            value["route_type"], pairs, f"{prefix}.RouteType"
        )
    if "number_capabilities" in value:
        import aws_sdk_sns.types.number_capability_list

        aws_sdk_sns.types.number_capability_list.serialize_query(
            value["number_capabilities"], pairs, f"{prefix}.NumberCapabilities"
        )


def deserialize_query(el: Element) -> PhoneNumberInformation:
    out: PhoneNumberInformation = {}  # type: ignore[typeddict-item]
    child_created_at = el.find("CreatedAt")
    if child_created_at is not None:
        import aws_sdk_sns.types.timestamp

        out["created_at"] = aws_sdk_sns.types.timestamp.deserialize_query(
            child_created_at
        )
    child_phone_number = el.find("PhoneNumber")
    if child_phone_number is not None:
        out["phone_number"] = str(child_phone_number.text or "")
    child_status = el.find("Status")
    if child_status is not None:
        out["status"] = str(child_status.text or "")
    child_iso2_country_code = el.find("Iso2CountryCode")
    if child_iso2_country_code is not None:
        out["iso2_country_code"] = str(child_iso2_country_code.text or "")
    child_route_type = el.find("RouteType")
    if child_route_type is not None:
        import aws_sdk_sns.types.route_type

        out["route_type"] = aws_sdk_sns.types.route_type.deserialize_query(
            child_route_type
        )
    child_number_capabilities = el.find("NumberCapabilities")
    if child_number_capabilities is not None:
        import aws_sdk_sns.types.number_capability_list

        out["number_capabilities"] = (
            aws_sdk_sns.types.number_capability_list.deserialize_query(
                child_number_capabilities
            )
        )
    return out
