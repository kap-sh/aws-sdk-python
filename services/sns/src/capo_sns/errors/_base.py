...
class SNSError(Exception):
    """Base class for all SNS service errors."""


class ServiceError(SNSError):
...
class SerializationError(SNSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(SNSError):
...
class WaiterFailedError(SNSError):
...
class WaiterTimeoutError(SNSError):
