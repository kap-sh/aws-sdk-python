"""Generated from Smithy shape ``com.amazonaws.sns#KMSOptInRequired``."""
...
# --- awsQuery ser/de ---
def serialize_query(
    value: KMSOptInRequired_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.message", str(value["message"])))


def deserialize_query(el: Element) -> KMSOptInRequired_:
...
class KMSOptInRequired(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.sns#KMSOptInRequired``."""
