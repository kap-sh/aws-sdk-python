"""Generated from Smithy shape ``com.amazonaws.sns#SetTopicAttributesInput``."""
...
class SetTopicAttributesInput(TypedDict, closed=True):
...
# --- awsQuery ser/de ---
def serialize_query(
    value: SetTopicAttributesInput, pairs: list[tuple[str, str]], prefix: str
) -> None:
...
def deserialize_query(el: Element) -> SetTopicAttributesInput:
    out: SetTopicAttributesInput = {}  # type: ignore[typeddict-item]
    child_topic_arn = el.find("TopicArn")
    if child_topic_arn is not None:
        out["topic_arn"] = str(child_topic_arn.text or "")
    else:
        raise DeserializationError("SetTopicAttributesInput.topic_arn required")
    ...
    if child_attribute_name is not None:
        ...
        raise DeserializationError("SetTopicAttributesInput.attribute_name required")
