...
ListOfEndpoints: TypeAlias = list["capo_sns.types.endpoint.Endpoint"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_sns.types.endpoint.serialize_query(item, pairs, f"{prefix}.member.{n}")
...
def deserialize_query(el: Element) -> ListOfEndpoints:
    ...
    for child in el.findall("member"):
        out.append(capo_sns.types.endpoint.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    if not value:
        ...
        return
    for n, item in enumerate(value, 1):
        capo_sns.types.endpoint.serialize_query(item, pairs, f"{prefix}.{n}")
