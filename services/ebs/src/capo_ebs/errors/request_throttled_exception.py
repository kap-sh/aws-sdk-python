...
class RequestThrottledException_(TypedDict, closed=True):
    ...
    reason: NotRequired[
        "capo_ebs.types.request_throttled_exception_reason.RequestThrottledExceptionReason"
    ]
...
def serialize_json(value: RequestThrottledException_) -> dict:
    ...
    if "reason" in value:
        ...
        out["Reason"] = (
            capo_ebs.types.request_throttled_exception_reason.serialize_json(
                value["reason"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RequestThrottledException_:
    ...
    if "Reason" in data:
        ...
        out["reason"] = (
            capo_ebs.types.request_throttled_exception_reason.deserialize_json(
                data["Reason"]
            )
        )
    return out
