...
class EBSError(Exception):
    """Base class for all EBS service errors."""


class ServiceError(EBSError):
...
class SerializationError(EBSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(EBSError):
...
class WaiterFailedError(EBSError):
...
class WaiterTimeoutError(EBSError):
