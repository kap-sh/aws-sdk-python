"""Generated from Smithy shape ``com.amazonaws.ebs#ListChangedBlocks``."""
...
def handle_response(
    ...
    out: aws_sdk_ebs.types.list_changed_blocks_response.ListChangedBlocksResponse = (
        aws_sdk_ebs.types.list_changed_blocks_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_ebs._auth._sigv4.build_sigv4_auth_scheme("ebs", options.region)
        )
        if sigv4_config is not None:
            return aws_sdk_ebs._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input: aws_sdk_ebs.types.list_changed_blocks_request.ListChangedBlocksRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/snapshots/{SecondSnapshotId}/changedblocks"
    url = url.replace(
        "{SecondSnapshotId}", quote(str(input["second_snapshot_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "first_snapshot_id" in input:
        params["firstSnapshotId"] = str(input["first_snapshot_id"])
    if "next_token" in input:
        params["pageToken"] = str(input["next_token"])
    if "max_results" in input:
        params["maxResults"] = str(input["max_results"])
    if "starting_block_index" in input:
        params["startingBlockIndex"] = str(input["starting_block_index"])
...
async def async_list_changed_blocks(
