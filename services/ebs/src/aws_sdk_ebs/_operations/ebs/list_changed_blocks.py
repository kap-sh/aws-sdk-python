...
def build_request(
    ...
    input_: aws_sdk_ebs.types.list_changed_blocks_request.ListChangedBlocksRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{SecondSnapshotId}", quote(str(input_["second_snapshot_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "first_snapshot_id" in input_:
        params["firstSnapshotId"] = str(input_["first_snapshot_id"])
