...
class RegisterScalableTargetRequest(TypedDict):
    ...
    r"""<p>The maximum value that you plan to scale out to. When a scaling policy is in effect, Application Auto Scaling can scale out (expand) as needed to the maximum capacity limit in response to changing demand. This property is required when registering a new scalable target.</p> <p>Although you can specify a large maximum capacity, note that service quotas might impose lower limits. Each service has its own default quotas for the maximum capacity of the resource. If you want to specify a higher limit, you can request an increase. For more information, consult the documentation for that service. For information about the default quotas for each service, see <a href=\"https://docs.aws.amazon.com/general/latest/gr/aws-service-information.html\">Service endpoints and quotas</a> in the <i>Amazon Web Services General Reference</i>.</p>"""
    role_arn: NotRequired[
        "aws_sdk_application_auto_scaling.types.resource_id_max_len1600.ResourceIdMaxLen1600"
    ]
    r"""<p>This parameter is required for services that do not support service-linked roles (such as Amazon EMR), and it must specify the ARN of an IAM role that allows Application Auto Scaling to modify the scalable target on your behalf. </p> <p>If the service supports service-linked roles, Application Auto Scaling uses a service-linked role, which it creates if it does not yet exist. For more information, see <a href=\"https://docs.aws.amazon.com/autoscaling/application/userguide/security_iam_service-with-iam.html\">How Application Auto Scaling works with IAM</a>.</p>"""
...
def serialize_aws_json_1_1(value: RegisterScalableTargetRequest) -> dict:
    ...
    if "tags" in value:
        ...
        out["Tags"] = (
            aws_sdk_application_auto_scaling.types.tag_map.serialize_aws_json_1_1(
                value["tags"]
            )
        )
    return out
