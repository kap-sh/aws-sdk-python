...
from capo_connect_contact_lens._auth._identity import (
    Credentials,
    Identity,
)
from capo_connect_contact_lens._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-connect-contact-lens[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-connect-contact-lens[sso]"
        )
