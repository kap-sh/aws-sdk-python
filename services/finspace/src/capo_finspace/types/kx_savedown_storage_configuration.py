...
if TYPE_CHECKING:
    import capo_finspace.types.kx_savedown_storage_size
    import capo_finspace.types.kx_savedown_storage_type
    import capo_finspace.types.kx_volume_name


class KxSavedownStorageConfiguration(TypedDict, closed=True):
    type: NotRequired[
        "capo_finspace.types.kx_savedown_storage_type.KxSavedownStorageType"
    ]
    ...
    size: NotRequired[
        "capo_finspace.types.kx_savedown_storage_size.KxSavedownStorageSize"
    ]
...
def serialize_json(value: KxSavedownStorageConfiguration) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = capo_finspace.types.kx_savedown_storage_type.serialize_json(
            value["type"]
        )
    if "size" in value:
        out["size"] = value["size"]
