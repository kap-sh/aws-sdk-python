"""Generated from Smithy shape ``com.amazonaws.finspace#UpdateKxEnvironmentNetworkResponse``."""
...
class UpdateKxEnvironmentNetworkResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateKxEnvironmentNetworkResponse) -> dict:
    out: dict = {}
    if "name" in value:
        out["name"] = value["name"]
...
def deserialize_json(data: dict) -> UpdateKxEnvironmentNetworkResponse:
    out: UpdateKxEnvironmentNetworkResponse = {}  # type: ignore[typeddict-item]
    if "name" in data:
        out["name"] = data["name"]
