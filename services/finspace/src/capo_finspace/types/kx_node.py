...
class KxNode(TypedDict, closed=True):
    node_id: NotRequired[
        "capo_finspace.types.kx_cluster_node_id_string.KxClusterNodeIdString"
    ]
    ...
    launch_time: NotRequired["capo_finspace.types.timestamp.Timestamp"]
    ...
    status: NotRequired["capo_finspace.types.kx_node_status.KxNodeStatus"]
...
def serialize_json(value: KxNode) -> dict:
    ...
    if "launch_time" in value:
        ...
        out["launchTime"] = capo_finspace.types.timestamp.serialize_json(
            value["launch_time"]
        )
    if "status" in value:
        import capo_finspace.types.kx_node_status

        out["status"] = capo_finspace.types.kx_node_status.serialize_json(
            value["status"]
        )
    return out
