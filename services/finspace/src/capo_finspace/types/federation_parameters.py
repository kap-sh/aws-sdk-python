...
if TYPE_CHECKING:
    ...
    import capo_finspace.types.federation_provider_name
    import capo_finspace.types.saml_metadata_document
    import capo_finspace.types.url
    import capo_finspace.types.urn


class FederationParameters(TypedDict, closed=True):
    saml_metadata_document: NotRequired[
        "capo_finspace.types.saml_metadata_document.SamlMetadataDocument"
    ]
    """<p>SAML 2.0 Metadata document from identity provider (IdP).</p>"""
    saml_metadata_url: NotRequired["capo_finspace.types.url.url"]
    """<p>Provide the metadata URL from your SAML 2.0 compliant identity provider (IdP).</p>"""
    application_call_back_url: NotRequired["capo_finspace.types.url.url"]
    """<p>The redirect or sign-in URL that should be entered into the SAML 2.0 compliant identity provider configuration (IdP).</p>"""
    federation_urn: NotRequired["capo_finspace.types.urn.urn"]
    ...
    federation_provider_name: NotRequired[
        "capo_finspace.types.federation_provider_name.FederationProviderName"
    ]
    """<p>Name of the identity provider (IdP).</p>"""
    attribute_map: NotRequired["capo_finspace.types.attribute_map.AttributeMap"]
...
def serialize_json(value: FederationParameters) -> dict:
    ...
    if "attribute_map" in value:
        ...
        out["attributeMap"] = capo_finspace.types.attribute_map.serialize_json(
            value["attribute_map"]
        )
    return out
