...
class S3VectorsError(Exception):
    """Base class for all S3Vectors service errors."""


class ServiceError(S3VectorsError):
...
class SerializationError(S3VectorsError):
    """Raised when a value cannot be serialized."""


class DeserializationError(S3VectorsError):
...
class WaiterFailedError(S3VectorsError):
...
class WaiterTimeoutError(S3VectorsError):
