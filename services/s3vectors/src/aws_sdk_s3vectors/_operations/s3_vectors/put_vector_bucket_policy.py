"""Generated from Smithy shape ``com.amazonaws.s3vectors#PutVectorBucketPolicy``."""
...
def handle_response(
    ...
    out: aws_sdk_s3vectors.types.put_vector_bucket_policy_output.PutVectorBucketPolicyOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_s3vectors.types.put_vector_bucket_policy_input.PutVectorBucketPolicyInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/PutVectorBucketPolicy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_s3vectors.types.put_vector_bucket_policy_input

    body: bytes | None = json.dumps(
        aws_sdk_s3vectors.types.put_vector_bucket_policy_input.serialize_json(input_)
    ).encode()
...
async def async_put_vector_bucket_policy(
