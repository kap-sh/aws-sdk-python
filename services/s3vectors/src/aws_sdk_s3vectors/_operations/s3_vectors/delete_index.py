"""Generated from Smithy shape ``com.amazonaws.s3vectors#DeleteIndex``."""
...
def build_request(
    ...
    input_: aws_sdk_s3vectors.types.delete_index_input.DeleteIndexInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteIndex"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_s3vectors.types.delete_index_input

    body: bytes | None = json.dumps(
        aws_sdk_s3vectors.types.delete_index_input.serialize_json(input_)
    ).encode()
...
def delete_index(
    ...
    aws_sdk_s3vectors.types.delete_index_output.DeleteIndexOutput, zapros.Response
]:
