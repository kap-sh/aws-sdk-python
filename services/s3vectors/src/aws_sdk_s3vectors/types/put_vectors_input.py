...
class PutVectorsInput(TypedDict):
...
def serialize_json(value: PutVectorsInput) -> dict:
    ...
    out["vectors"] = aws_sdk_s3vectors.types.put_vectors_input_list.serialize_json(
        value["vectors"]
    )
    return out
...
def deserialize_json(data: dict) -> PutVectorsInput:
    ...
    if "vectors" in data:
        ...
        out["vectors"] = (
            aws_sdk_s3vectors.types.put_vectors_input_list.deserialize_json(
                data["vectors"]
            )
        )
    else:
