...
def serialize_json(value: GetVectorBucketOutput) -> dict:
    out: dict = {}
    import aws_sdk_s3vectors.types.vector_bucket
    out["vectorBucket"] = aws_sdk_s3vectors.types.vector_bucket.serialize_json(value["vector_bucket"])
    return out
...
def deserialize_json(data: dict) -> GetVectorBucketOutput:
    ...
    if "vectorBucket" in data:
        import aws_sdk_s3vectors.types.vector_bucket
        out["vector_bucket"] = aws_sdk_s3vectors.types.vector_bucket.deserialize_json(data["vectorBucket"])
    else:
