...
class CreateIndexInput(TypedDict):
    ...
    metadata_configuration: NotRequired["aws_sdk_s3vectors.types.metadata_configuration.MetadataConfiguration"]
...
def serialize_json(value: CreateIndexInput) -> dict:
    ...
    import aws_sdk_s3vectors.types.data_type
    out["dataType"] = aws_sdk_s3vectors.types.data_type.serialize_json(value["data_type"])
    out["dimension"] = value["dimension"]
    import aws_sdk_s3vectors.types.distance_metric
    out["distanceMetric"] = aws_sdk_s3vectors.types.distance_metric.serialize_json(value["distance_metric"])
    if "metadata_configuration" in value:
        import aws_sdk_s3vectors.types.metadata_configuration
        out["metadataConfiguration"] = aws_sdk_s3vectors.types.metadata_configuration.serialize_json(value["metadata_configuration"])
...
def deserialize_json(data: dict) -> CreateIndexInput:
    ...
    if "dataType" in data:
        import aws_sdk_s3vectors.types.data_type
        out["data_type"] = aws_sdk_s3vectors.types.data_type.deserialize_json(data["dataType"])
    else:
    ...
    if "distanceMetric" in data:
        import aws_sdk_s3vectors.types.distance_metric
        out["distance_metric"] = aws_sdk_s3vectors.types.distance_metric.deserialize_json(data["distanceMetric"])
    else:
        raise DeserializationError("CreateIndexInput.distance_metric required")
    if "metadataConfiguration" in data:
        import aws_sdk_s3vectors.types.metadata_configuration
        out["metadata_configuration"] = aws_sdk_s3vectors.types.metadata_configuration.deserialize_json(data["metadataConfiguration"])
