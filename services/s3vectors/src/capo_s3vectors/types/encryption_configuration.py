...
class EncryptionConfiguration(TypedDict, closed=True):
    ...
    kms_key_arn: NotRequired["capo_s3vectors.types.kms_key_arn.KmsKeyArn"]
...
def serialize_json(value: EncryptionConfiguration) -> dict:
    ...
    out["sseType"] = capo_s3vectors.types.sse_type.serialize_json(
        value.get("sse_type", "AES256")
    )
