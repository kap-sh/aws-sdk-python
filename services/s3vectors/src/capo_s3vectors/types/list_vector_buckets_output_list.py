...
ListVectorBucketsOutputList: TypeAlias = list[
    "capo_s3vectors.types.vector_bucket_summary.VectorBucketSummary"
]
...
def serialize_json(value: ListVectorBucketsOutputList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_s3vectors.types.vector_bucket_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ListVectorBucketsOutputList:
    ...
    for item in data:
        out.append(capo_s3vectors.types.vector_bucket_summary.deserialize_json(item))
    return out
