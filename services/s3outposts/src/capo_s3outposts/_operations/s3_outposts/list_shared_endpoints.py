"""Generated from Smithy shape ``com.amazonaws.s3outposts#ListSharedEndpoints``."""
...
def handle_response(
    ...
    out: capo_s3outposts.types.list_shared_endpoints_result.ListSharedEndpointsResult = capo_s3outposts.types.list_shared_endpoints_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/S3Outposts/ListSharedEndpoints"
    params: dict[str, str] = {}
    if "next_token" in input_:
        params["nextToken"] = str(input_["next_token"])
    params["maxResults"] = str(input_.get("max_results", 0))
    if "outpost_id" in input_:
        params["outpostId"] = str(input_["outpost_id"])
...
async def async_list_shared_endpoints(
