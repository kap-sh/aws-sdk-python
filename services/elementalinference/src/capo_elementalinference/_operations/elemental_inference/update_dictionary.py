"""Generated from Smithy shape ``com.amazonaws.elementalinference#UpdateDictionary``."""
...
def handle_response(
    ...
    out: capo_elementalinference.types.update_dictionary_response.UpdateDictionaryResponse = capo_elementalinference.types.update_dictionary_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/dictionary/{id}"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elementalinference.types.update_dictionary_request.serialize_json(input_)
    ).encode()
...
async def async_update_dictionary(
