"""Generated from Smithy shape ``com.amazonaws.elementalinference#CreateDictionary``."""
...
import capo_elementalinference.errors.conflict_exception
import capo_elementalinference.errors.internal_server_error_exception
import capo_elementalinference.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_elementalinference.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ServiceQuotaExceededException":
            raise capo_elementalinference.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_elementalinference.types.create_dictionary_response.CreateDictionaryResponse = capo_elementalinference.types.create_dictionary_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/dictionary"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_elementalinference.types.create_dictionary_request.serialize_json(input_)
    ).encode()
...
async def async_create_dictionary(
