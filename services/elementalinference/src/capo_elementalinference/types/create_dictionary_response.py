"""Generated from Smithy shape ``com.amazonaws.elementalinference#CreateDictionaryResponse``."""
...
class CreateDictionaryResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: CreateDictionaryResponse) -> dict:
    out: dict = {}
    out["name"] = value["name"]
    out["arn"] = value["arn"]
    out["id"] = value["id"]
...
def deserialize_json(data: dict) -> CreateDictionaryResponse:
    out: CreateDictionaryResponse = {}  # type: ignore[typeddict-item]
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("CreateDictionaryResponse.name required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("CreateDictionaryResponse.arn required")
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("CreateDictionaryResponse.id required")
    if "language" in data:
        import capo_elementalinference.types.dictionary_language
        ...
        raise DeserializationError("CreateDictionaryResponse.language required")
    if "status" in data:
        import capo_elementalinference.types.dictionary_status
        ...
        raise DeserializationError("CreateDictionaryResponse.status required")
