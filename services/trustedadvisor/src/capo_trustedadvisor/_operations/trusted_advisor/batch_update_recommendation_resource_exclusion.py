"""Generated from Smithy shape ``com.amazonaws.trustedadvisor#BatchUpdateRecommendationResourceExclusion``."""
...
def handle_response(
    ...
    out: capo_trustedadvisor.types.batch_update_recommendation_resource_exclusion_response.BatchUpdateRecommendationResourceExclusionResponse = capo_trustedadvisor.types.batch_update_recommendation_resource_exclusion_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/batch-update-recommendation-resource-exclusion"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_trustedadvisor.types.batch_update_recommendation_resource_exclusion_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_update_recommendation_resource_exclusion(
