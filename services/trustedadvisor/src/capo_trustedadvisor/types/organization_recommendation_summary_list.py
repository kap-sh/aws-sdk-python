...
OrganizationRecommendationSummaryList: TypeAlias = list[
    "capo_trustedadvisor.types.organization_recommendation_summary.OrganizationRecommendationSummary"
]
...
def serialize_json(value: OrganizationRecommendationSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_trustedadvisor.types.organization_recommendation_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> OrganizationRecommendationSummaryList:
    ...
    for item in data:
        out.append(
            capo_trustedadvisor.types.organization_recommendation_summary.deserialize_json(
                item
            )
        )
    return out
