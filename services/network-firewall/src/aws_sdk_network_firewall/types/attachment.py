...
if TYPE_CHECKING:
    import aws_sdk_network_firewall.types.attachment_status
    import aws_sdk_network_firewall.types.az_subnet
    import aws_sdk_network_firewall.types.endpoint_id
    import aws_sdk_network_firewall.types.status_message


class Attachment(TypedDict, closed=True):
    subnet_id: NotRequired["aws_sdk_network_firewall.types.az_subnet.AzSubnet"]
    """<p>The unique identifier of the subnet that you've specified to be used for a firewall endpoint. </p>"""
    endpoint_id: NotRequired["aws_sdk_network_firewall.types.endpoint_id.EndpointId"]
    ...
    status: NotRequired[
        "aws_sdk_network_firewall.types.attachment_status.AttachmentStatus"
    ]
...
def serialize_aws_json_1_0(value: Attachment) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            aws_sdk_network_firewall.types.attachment_status.serialize_aws_json_1_0(
                value["status"]
            )
        )
...
def deserialize_aws_json_1_0(data: dict) -> Attachment:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            aws_sdk_network_firewall.types.attachment_status.deserialize_aws_json_1_0(
                data["Status"]
            )
        )
