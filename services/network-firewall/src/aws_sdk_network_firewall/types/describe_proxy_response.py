"""Generated from Smithy shape ``com.amazonaws.networkfirewall#DescribeProxyResponse``."""
...
if TYPE_CHECKING:
    import aws_sdk_network_firewall.types.describe_proxy_resource
    import aws_sdk_network_firewall.types.update_token


class DescribeProxyResponse(TypedDict, closed=True):
    proxy: NotRequired[
        "aws_sdk_network_firewall.types.describe_proxy_resource.DescribeProxyResource"
    ]
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: DescribeProxyResponse) -> dict:
    out: dict = {}
    if "proxy" in value:
        import aws_sdk_network_firewall.types.describe_proxy_resource

        out["Proxy"] = (
            aws_sdk_network_firewall.types.describe_proxy_resource.serialize_aws_json_1_0(
                value["proxy"]
            )
        )
...
def deserialize_aws_json_1_0(data: dict) -> DescribeProxyResponse:
    out: DescribeProxyResponse = {}  # type: ignore[typeddict-item]
    if "Proxy" in data:
        import aws_sdk_network_firewall.types.describe_proxy_resource

        out["proxy"] = (
            aws_sdk_network_firewall.types.describe_proxy_resource.deserialize_aws_json_1_0(
                data["Proxy"]
            )
        )
