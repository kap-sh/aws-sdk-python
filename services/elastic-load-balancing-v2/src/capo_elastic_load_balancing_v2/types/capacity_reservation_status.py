...
class CapacityReservationStatus(TypedDict, closed=True):
    code: NotRequired[
        "capo_elastic_load_balancing_v2.types.capacity_reservation_state_enum.CapacityReservationStateEnum"
    ]
    """<p>The status code.</p>"""
...
def deserialize_query(el: Element) -> CapacityReservationStatus:
    ...
    if child_code is not None:
        ...
        out["code"] = (
            capo_elastic_load_balancing_v2.types.capacity_reservation_state_enum.deserialize_query(
                child_code
            )
        )
