...
LoadBalancers: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.load_balancer.LoadBalancer"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.load_balancer.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancers:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.load_balancer.deserialize_query(child)
        )
    return out
