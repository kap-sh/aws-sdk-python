...
Listeners: TypeAlias = list["capo_elastic_load_balancing_v2.types.listener.Listener"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.listener.serialize_query(
...
def deserialize_query(el: Element) -> Listeners:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.listener.deserialize_query(child)
        )
    return out
