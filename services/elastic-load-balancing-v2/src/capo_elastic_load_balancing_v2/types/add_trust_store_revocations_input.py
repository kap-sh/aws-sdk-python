...
def serialize_query(
    ...
    if "revocation_contents" in value:
        ...
        capo_elastic_load_balancing_v2.types.revocation_contents.serialize_query(
            value["revocation_contents"], pairs, f"{key_prefix}RevocationContents"
        )
...
def deserialize_query(el: Element) -> AddTrustStoreRevocationsInput:
    ...
    if child_revocation_contents is not None:
        ...
        out["revocation_contents"] = (
            capo_elastic_load_balancing_v2.types.revocation_contents.deserialize_query(
