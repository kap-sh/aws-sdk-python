...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.query_string_key_value_pair

QueryStringKeyValuePairList: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.query_string_key_value_pair.QueryStringKeyValuePair"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.query_string_key_value_pair.serialize_query(
...
def deserialize_query(el: Element) -> QueryStringKeyValuePairList:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.query_string_key_value_pair.deserialize_query(
                child
            )
        )
    return out
