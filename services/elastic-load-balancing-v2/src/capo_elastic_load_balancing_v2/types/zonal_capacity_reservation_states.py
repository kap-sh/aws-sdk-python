...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.zonal_capacity_reservation_state

ZonalCapacityReservationStates: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.zonal_capacity_reservation_state.ZonalCapacityReservationState"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.zonal_capacity_reservation_state.serialize_query(
...
def deserialize_query(el: Element) -> ZonalCapacityReservationStates:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.zonal_capacity_reservation_state.deserialize_query(
                child
            )
        )
    return out
