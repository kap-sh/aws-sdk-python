...
class QueryStringKeyValuePair(TypedDict, closed=True):
    key: NotRequired["capo_elastic_load_balancing_v2.types.string_value.StringValue"]
    """<p>The key. You can omit the key.</p>"""
    value: NotRequired["capo_elastic_load_balancing_v2.types.string_value.StringValue"]
    """<p>The value.</p>"""


# --- awsQuery ser/de ---
def serialize_query(
    value: QueryStringKeyValuePair, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "key" in value:
...
def deserialize_query(el: Element) -> QueryStringKeyValuePair:
