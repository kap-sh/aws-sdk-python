...
if TYPE_CHECKING:
    import capo_elastic_load_balancing_v2.types.target_health_description

TargetHealthDescriptions: TypeAlias = list[
    "capo_elastic_load_balancing_v2.types.target_health_description.TargetHealthDescription"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_load_balancing_v2.types.target_health_description.serialize_query(
...
def deserialize_query(el: Element) -> TargetHealthDescriptions:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_load_balancing_v2.types.target_health_description.deserialize_query(
                child
            )
        )
    return out
