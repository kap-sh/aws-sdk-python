"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#ModifyListener``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("ModifyListenerResult")
    out: capo_elastic_load_balancing_v2.types.modify_listener_output.ModifyListenerOutput = capo_elastic_load_balancing_v2.types.modify_listener_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "ModifyListener"))
    pairs.append(("Version", "2015-12-01"))
    capo_elastic_load_balancing_v2.types.modify_listener_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_modify_listener(
