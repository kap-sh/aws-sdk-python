"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#DescribeTrustStoreRevocations``."""
...
import capo_elastic_load_balancing_v2.errors.revocation_id_not_found_exception
import capo_elastic_load_balancing_v2.errors.trust_store_not_found_exception
import capo_elastic_load_balancing_v2.types.describe_trust_store_revocation_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "RevocationIdNotFoundException":
            raise capo_elastic_load_balancing_v2.errors.revocation_id_not_found_exception.RevocationIdNotFoundException.from_query(
                root
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DescribeTrustStoreRevocationsResult")
    out: capo_elastic_load_balancing_v2.types.describe_trust_store_revocations_output.DescribeTrustStoreRevocationsOutput = capo_elastic_load_balancing_v2.types.describe_trust_store_revocations_output.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DescribeTrustStoreRevocations"))
    pairs.append(("Version", "2015-12-01"))
    capo_elastic_load_balancing_v2.types.describe_trust_store_revocations_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_trust_store_revocations(
