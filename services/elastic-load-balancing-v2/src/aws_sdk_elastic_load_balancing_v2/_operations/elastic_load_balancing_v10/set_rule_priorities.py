"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#SetRulePriorities``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PriorityInUseException":
            import aws_sdk_elastic_load_balancing_v2.errors.priority_in_use_exception

            raise aws_sdk_elastic_load_balancing_v2.errors.priority_in_use_exception.PriorityInUseException.from_query(
                root
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("SetRulePrioritiesResult")
    out: aws_sdk_elastic_load_balancing_v2.types.set_rule_priorities_output.SetRulePrioritiesOutput = aws_sdk_elastic_load_balancing_v2.types.set_rule_priorities_output.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_elastic_load_balancing_v2.types.set_rule_priorities_input.SetRulePrioritiesInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "SetRulePriorities"))
    pairs.append(("Version", "2015-12-01"))
    import aws_sdk_elastic_load_balancing_v2.types.set_rule_priorities_input

    aws_sdk_elastic_load_balancing_v2.types.set_rule_priorities_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_set_rule_priorities(
