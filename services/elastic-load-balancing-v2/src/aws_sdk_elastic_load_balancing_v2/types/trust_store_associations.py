...
TrustStoreAssociations: TypeAlias = list[
    "aws_sdk_elastic_load_balancing_v2.types.trust_store_association.TrustStoreAssociation"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing_v2.types.trust_store_association.serialize_query(
...
def deserialize_query(el: Element) -> TrustStoreAssociations:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_load_balancing_v2.types.trust_store_association.deserialize_query(
                child
            )
        )
    return out
