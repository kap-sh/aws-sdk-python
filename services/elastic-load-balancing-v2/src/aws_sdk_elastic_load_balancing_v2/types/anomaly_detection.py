...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing_v2.types.anomaly_result_enum
    import aws_sdk_elastic_load_balancing_v2.types.mitigation_in_effect_enum


class AnomalyDetection(TypedDict, closed=True):
    result: NotRequired[
        "aws_sdk_elastic_load_balancing_v2.types.anomaly_result_enum.AnomalyResultEnum"
    ]
    """<p>The latest anomaly detection result.</p>"""
    mitigation_in_effect: NotRequired[
        "aws_sdk_elastic_load_balancing_v2.types.mitigation_in_effect_enum.MitigationInEffectEnum"
    ]
...
def serialize_query(
    ...
    if "result" in value:
        ...
        aws_sdk_elastic_load_balancing_v2.types.anomaly_result_enum.serialize_query(
            value["result"], pairs, f"{prefix}.Result"
        )
    if "mitigation_in_effect" in value:
        import aws_sdk_elastic_load_balancing_v2.types.mitigation_in_effect_enum

        aws_sdk_elastic_load_balancing_v2.types.mitigation_in_effect_enum.serialize_query(
...
def deserialize_query(el: Element) -> AnomalyDetection:
    ...
    if child_result is not None:
        ...
        out["result"] = (
            aws_sdk_elastic_load_balancing_v2.types.anomaly_result_enum.deserialize_query(
                child_result
            )
        )
    ...
    if child_mitigation_in_effect is not None:
        ...
        out["mitigation_in_effect"] = (
            aws_sdk_elastic_load_balancing_v2.types.mitigation_in_effect_enum.deserialize_query(
