...
if TYPE_CHECKING:
    import aws_sdk_elastic_load_balancing_v2.types.action

Actions: TypeAlias = list["aws_sdk_elastic_load_balancing_v2.types.action.Action"]
...
def serialize_query(value: Actions, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_load_balancing_v2.types.action.serialize_query(
...
def deserialize_query(el: Element) -> Actions:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_load_balancing_v2.types.action.deserialize_query(child)
        )
    return out
