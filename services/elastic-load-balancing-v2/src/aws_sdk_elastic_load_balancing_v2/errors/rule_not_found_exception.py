"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#RuleNotFoundException``."""
...
class RuleNotFoundException_(TypedDict, closed=True):
    message: NotRequired[
...
# --- awsQuery ser/de ---
def serialize_query(
    value: RuleNotFoundException_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.Message", str(value["message"])))


def deserialize_query(el: Element) -> RuleNotFoundException_:
    out: RuleNotFoundException_ = {}  # type: ignore[typeddict-item]
...
class RuleNotFoundException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticloadbalancingv2#RuleNotFoundException``."""
