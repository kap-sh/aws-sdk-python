"""Generated from Smithy shape ``com.amazonaws.elasticloadbalancingv2#InsufficientCapacityException``."""
...
# --- awsQuery ser/de ---
def serialize_query(
    value: InsufficientCapacityException_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.Message", str(value["message"])))


def deserialize_query(el: Element) -> InsufficientCapacityException_:
...
class InsufficientCapacityException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticloadbalancingv2#InsufficientCapacityException``."""
