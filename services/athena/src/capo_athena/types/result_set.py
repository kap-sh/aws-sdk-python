"""Generated from Smithy shape ``com.amazonaws.athena#ResultSet``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_athena.types.result_set_metadata
    import capo_athena.types.row_list


class ResultSet(TypedDict, closed=True):
    rows: NotRequired["capo_athena.types.row_list.RowList"]
    """<p>The rows in the table.</p>"""
    result_set_metadata: NotRequired[
        "capo_athena.types.result_set_metadata.ResultSetMetadata"
    ]
    """<p>The metadata that describes the column structure and data types of a table of query results.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: ResultSet) -> dict:
    out: dict = {}
    if "rows" in value:
        import capo_athena.types.row_list

        out["Rows"] = capo_athena.types.row_list.serialize_aws_json_1_1(value["rows"])
    if "result_set_metadata" in value:
        import capo_athena.types.result_set_metadata

        out["ResultSetMetadata"] = (
            capo_athena.types.result_set_metadata.serialize_aws_json_1_1(
                value["result_set_metadata"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> ResultSet:
    out: ResultSet = {}  # type: ignore[typeddict-item]
    if "Rows" in data:
        import capo_athena.types.row_list

        out["rows"] = capo_athena.types.row_list.deserialize_aws_json_1_1(data["Rows"])
    if "ResultSetMetadata" in data:
        import capo_athena.types.result_set_metadata

        out["result_set_metadata"] = (
            capo_athena.types.result_set_metadata.deserialize_aws_json_1_1(
                data["ResultSetMetadata"]
            )
        )
    return out
