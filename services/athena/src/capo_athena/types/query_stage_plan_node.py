"""Generated from Smithy shape ``com.amazonaws.athena#QueryStagePlanNode``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_athena.types.query_stage_plan_nodes
    import capo_athena.types.string
    import capo_athena.types.string_list


class QueryStagePlanNode(TypedDict, closed=True):
    name: NotRequired["capo_athena.types.string.String"]
    """<p>Name of the query stage plan that describes the operation this stage is performing as part of query execution.</p>"""
    identifier: NotRequired["capo_athena.types.string.String"]
    """<p>Information about the operation this query stage plan node is performing.</p>"""
    children: NotRequired[
        "capo_athena.types.query_stage_plan_nodes.QueryStagePlanNodes"
    ]
    """<p>Stage plan information such as name, identifier, sub plans, and remote sources of child plan nodes/</p>"""
    remote_sources: NotRequired["capo_athena.types.string_list.StringList"]
    """<p>Source plan node IDs.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: QueryStagePlanNode) -> dict:
    out: dict = {}
    if "name" in value:
        out["Name"] = value["name"]
    if "identifier" in value:
        out["Identifier"] = value["identifier"]
    if "children" in value:
        import capo_athena.types.query_stage_plan_nodes

        out["Children"] = (
            capo_athena.types.query_stage_plan_nodes.serialize_aws_json_1_1(
                value["children"]
            )
        )
    if "remote_sources" in value:
        import capo_athena.types.string_list

        out["RemoteSources"] = capo_athena.types.string_list.serialize_aws_json_1_1(
            value["remote_sources"]
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> QueryStagePlanNode:
    out: QueryStagePlanNode = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    if "Identifier" in data:
        out["identifier"] = data["Identifier"]
    if "Children" in data:
        import capo_athena.types.query_stage_plan_nodes

        out["children"] = (
            capo_athena.types.query_stage_plan_nodes.deserialize_aws_json_1_1(
                data["Children"]
            )
        )
    if "RemoteSources" in data:
        import capo_athena.types.string_list

        out["remote_sources"] = capo_athena.types.string_list.deserialize_aws_json_1_1(
            data["RemoteSources"]
        )
    return out
