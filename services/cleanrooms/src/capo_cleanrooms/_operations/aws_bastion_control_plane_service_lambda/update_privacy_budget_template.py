...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_privacy_budget_template_output.UpdatePrivacyBudgetTemplateOutput = capo_cleanrooms.types.update_privacy_budget_template_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(str(input_["privacy_budget_template_identifier"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_privacy_budget_template_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_privacy_budget_template(
