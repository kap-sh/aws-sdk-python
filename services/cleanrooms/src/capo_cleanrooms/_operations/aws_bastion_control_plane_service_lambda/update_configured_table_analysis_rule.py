"""Generated from Smithy shape ``com.amazonaws.cleanrooms#UpdateConfiguredTableAnalysisRule``."""
...
import capo_cleanrooms.errors.access_denied_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            raise capo_cleanrooms.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
        case "ConflictException":
            raise capo_cleanrooms.errors.conflict_exception.ConflictException.from_json(
                data
            )
        case "InternalServerException":
            raise capo_cleanrooms.errors.internal_server_exception.InternalServerException.from_json(
                data
            )
        ...
        case "ThrottlingException":
            raise capo_cleanrooms.errors.throttling_exception.ThrottlingException.from_json(
                data
            )
        case "ValidationException":
            raise capo_cleanrooms.errors.validation_exception.ValidationException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_configured_table_analysis_rule_output.UpdateConfiguredTableAnalysisRuleOutput = capo_cleanrooms.types.update_configured_table_analysis_rule_output.deserialize_json(
...
def get_signer(
...
) -> capo_cleanrooms._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_cleanrooms._auth._sigv4.build_sigv4_auth_scheme(
                "cleanrooms", options.region
            )
        )
        if sigv4_config is not None:
            return capo_cleanrooms._auth._signers.SigV4Signer(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(str(input_["configured_table_identifier"]), safe=""),
    )
    url = url.replace(
        "{analysisRuleType}", quote(str(input_["analysis_rule_type"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_configured_table_analysis_rule_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_configured_table_analysis_rule(
