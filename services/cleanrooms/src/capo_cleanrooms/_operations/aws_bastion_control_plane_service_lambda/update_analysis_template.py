...
def handle_response(
    ...
    out: capo_cleanrooms.types.update_analysis_template_output.UpdateAnalysisTemplateOutput = capo_cleanrooms.types.update_analysis_template_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(str(input_["analysis_template_identifier"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_cleanrooms.types.update_analysis_template_input.serialize_json(input_)
    ).encode()
...
async def async_update_analysis_template(
