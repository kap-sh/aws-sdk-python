...
class UpdateCollaborationChangeRequestInput(TypedDict):
...
def serialize_json(value: UpdateCollaborationChangeRequestInput) -> dict:
    ...
    out["action"] = aws_sdk_cleanrooms.types.change_request_action.serialize_json(
        value["action"]
    )
    return out
...
def deserialize_json(data: dict) -> UpdateCollaborationChangeRequestInput:
    ...
    if "action" in data:
        ...
        out["action"] = aws_sdk_cleanrooms.types.change_request_action.deserialize_json(
            data["action"]
        )
    else:
