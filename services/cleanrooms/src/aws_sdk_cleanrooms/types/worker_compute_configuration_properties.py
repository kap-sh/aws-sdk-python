...
if TYPE_CHECKING:
    import aws_sdk_cleanrooms.types.spark_properties


class _WorkerComputeConfigurationProperties_spark(TypedDict):
    spark: "aws_sdk_cleanrooms.types.spark_properties.SparkProperties"
...
def serialize_json(value: WorkerComputeConfigurationProperties) -> dict:
    if "spark" in value:
        ...
        return {
            "spark": aws_sdk_cleanrooms.types.spark_properties.serialize_json(
                value["spark"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> WorkerComputeConfigurationProperties:
    if "spark" in data:
        ...
        return {
            "spark": aws_sdk_cleanrooms.types.spark_properties.deserialize_json(
                data["spark"]
            )
        }
    else:
