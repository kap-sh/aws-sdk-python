...
if TYPE_CHECKING:
    import aws_sdk_cleanrooms.types.protected_query_output


class ProtectedQueryResult(TypedDict):
    output: "aws_sdk_cleanrooms.types.protected_query_output.ProtectedQueryOutput"
...
def serialize_json(value: ProtectedQueryResult) -> dict:
    ...
    out["output"] = aws_sdk_cleanrooms.types.protected_query_output.serialize_json(
        value["output"]
    )
    return out
...
def deserialize_json(data: dict) -> ProtectedQueryResult:
    ...
    if "output" in data:
        ...
        out["output"] = (
            aws_sdk_cleanrooms.types.protected_query_output.deserialize_json(
                data["output"]
            )
        )
    else:
