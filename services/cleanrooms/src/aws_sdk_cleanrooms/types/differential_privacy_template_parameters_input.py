"""Generated from Smithy shape ``com.amazonaws.cleanrooms#DifferentialPrivacyTemplateParametersInput``."""
...
class DifferentialPrivacyTemplateParametersInput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: DifferentialPrivacyTemplateParametersInput) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> DifferentialPrivacyTemplateParametersInput:
    out: DifferentialPrivacyTemplateParametersInput = {}  # type: ignore[typeddict-item]
    if "epsilon" in data:
        out["epsilon"] = data["epsilon"]
    else:
        raise DeserializationError(
            "DifferentialPrivacyTemplateParametersInput.epsilon required"
        )
    if "usersNoisePerQuery" in data:
        out["users_noise_per_query"] = data["usersNoisePerQuery"]
    else:
        raise DeserializationError(
            "DifferentialPrivacyTemplateParametersInput.users_noise_per_query required"
        )
    return out
