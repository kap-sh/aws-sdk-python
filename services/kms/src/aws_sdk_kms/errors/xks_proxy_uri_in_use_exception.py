...
class XksProxyUriInUseException_(TypedDict, closed=True):
    message: NotRequired["aws_sdk_kms.types.error_message_type.ErrorMessageType"]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: XksProxyUriInUseException_) -> dict:
    out: dict = {}
    if "message" in value:
...
def deserialize_aws_json_1_1(data: dict) -> XksProxyUriInUseException_:
    out: XksProxyUriInUseException_ = {}  # type: ignore[typeddict-item]
    if "message" in data:
...
class XksProxyUriInUseException(ServiceError):
    ...
    @classmethod
    def from_aws_json_1_1(cls, data: dict) -> "XksProxyUriInUseException":
