...
import capo_kms.errors.incorrect_key_exception
import capo_kms.errors.invalid_ciphertext_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "IncorrectKeyException":
            raise capo_kms.errors.incorrect_key_exception.IncorrectKeyException.from_aws_json_1_1(
                data, message
            )
        case "InvalidCiphertextException":
            raise capo_kms.errors.invalid_ciphertext_exception.InvalidCiphertextException.from_aws_json_1_1(
                data, message
            )
...
def handle_response(
    ...
    out: capo_kms.types.decrypt_response.DecryptResponse = (
        capo_kms.types.decrypt_response.deserialize_aws_json_1_1(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        ...
        if endpoint_scheme is not None or not name_to_schema:
            sigv4_config = capo_kms._auth._sigv4.build_sigv4_auth_scheme(
                "kms", options.region, endpoint_scheme
            )
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_kms.types.decrypt_request.serialize_aws_json_1_1(input_), allow_nan=False
    ).encode()
...
def decrypt(
    options: OperationOptions, input_: capo_kms.types.decrypt_request.DecryptRequest
) -> tuple[capo_kms.types.decrypt_response.DecryptResponse, zapros.Response]:
