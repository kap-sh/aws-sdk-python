...
class uxcError(Exception):
    """Base class for all uxc service errors."""


class ServiceError(uxcError):
...
class SerializationError(uxcError):
    """Raised when a value cannot be serialized."""


class DeserializationError(uxcError):
...
class WaiterFailedError(uxcError):
...
class WaiterTimeoutError(uxcError):
