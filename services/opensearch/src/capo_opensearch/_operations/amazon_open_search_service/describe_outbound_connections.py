"""Generated from Smithy shape ``com.amazonaws.opensearch#DescribeOutboundConnections``."""
...
import capo_opensearch.errors.invalid_pagination_token_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidPaginationTokenException":
            raise capo_opensearch.errors.invalid_pagination_token_exception.InvalidPaginationTokenException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_opensearch.types.describe_outbound_connections_response.DescribeOutboundConnectionsResponse = capo_opensearch.types.describe_outbound_connections_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/2021-01-01/opensearch/cc/outboundConnection/search"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.describe_outbound_connections_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_describe_outbound_connections(
