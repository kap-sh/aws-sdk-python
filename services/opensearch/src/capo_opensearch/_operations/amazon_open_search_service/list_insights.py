"""Generated from Smithy shape ``com.amazonaws.opensearch#ListInsights``."""
...
def handle_response(
    ...
    out: capo_opensearch.types.list_insights_response.ListInsightsResponse = (
        capo_opensearch.types.list_insights_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/opensearch/insights"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.list_insights_request.serialize_json(input_)
    ).encode()
...
def list_insights(
    ...
    capo_opensearch.types.list_insights_response.ListInsightsResponse, zapros.Response
]:
