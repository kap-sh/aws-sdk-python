"""Generated from Smithy shape ``com.amazonaws.opensearch#StartServiceSoftwareUpdate``."""
...
def handle_response(
    ...
    out: capo_opensearch.types.start_service_software_update_response.StartServiceSoftwareUpdateResponse = capo_opensearch.types.start_service_software_update_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/2021-01-01/opensearch/serviceSoftwareUpdate/start"
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_opensearch.types.start_service_software_update_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_service_software_update(
