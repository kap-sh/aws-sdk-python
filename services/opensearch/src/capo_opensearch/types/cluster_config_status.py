...
class ClusterConfigStatus(TypedDict, closed=True):
    options: "capo_opensearch.types.cluster_config.ClusterConfig"
...
def serialize_json(value: ClusterConfigStatus) -> dict:
    ...
    out["Options"] = capo_opensearch.types.cluster_config.serialize_json(
        value["options"]
    )
    import capo_opensearch.types.option_status
...
def deserialize_json(data: dict) -> ClusterConfigStatus:
    ...
    if "Options" in data:
        ...
        out["options"] = capo_opensearch.types.cluster_config.deserialize_json(
            data["Options"]
        )
    else:
