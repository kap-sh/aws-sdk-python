...
if TYPE_CHECKING:
    import capo_opensearch.types.connection_status_message
...
class InboundConnectionStatus(TypedDict, closed=True):
    status_code: NotRequired[
        "capo_opensearch.types.inbound_connection_status_code.InboundConnectionStatusCode"
    ]
    ...
    message: NotRequired[
        "capo_opensearch.types.connection_status_message.ConnectionStatusMessage"
    ]
...
def serialize_json(value: InboundConnectionStatus) -> dict:
    ...
    if "status_code" in value:
        ...
        out["StatusCode"] = (
            capo_opensearch.types.inbound_connection_status_code.serialize_json(
                value["status_code"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> InboundConnectionStatus:
    ...
    if "StatusCode" in data:
        ...
        out["status_code"] = (
            capo_opensearch.types.inbound_connection_status_code.deserialize_json(
                data["StatusCode"]
            )
        )
    if "Message" in data:
