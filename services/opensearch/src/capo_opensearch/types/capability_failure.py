...
if TYPE_CHECKING:
    import capo_opensearch.types.capability_failure_details
    import capo_opensearch.types.capability_failure_reason


class CapabilityFailure(TypedDict, closed=True):
    reason: NotRequired[
        "capo_opensearch.types.capability_failure_reason.CapabilityFailureReason"
    ]
    """<p>The reason for the capability failure. Possible values: <code>KMS_KEY_INSUFFICIENT_PERMISSION</code>.</p>"""
    details: NotRequired[
        "capo_opensearch.types.capability_failure_details.CapabilityFailureDetails"
    ]
...
def serialize_json(value: CapabilityFailure) -> dict:
    ...
    if "reason" in value:
        ...
        out["reason"] = capo_opensearch.types.capability_failure_reason.serialize_json(
            value["reason"]
        )
    if "details" in value:
...
def deserialize_json(data: dict) -> CapabilityFailure:
    ...
    if "reason" in data:
        ...
        out["reason"] = (
            capo_opensearch.types.capability_failure_reason.deserialize_json(
                data["reason"]
            )
        )
    if "details" in data:
