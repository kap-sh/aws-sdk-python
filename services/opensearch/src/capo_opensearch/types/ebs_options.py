...
class EBSOptions(TypedDict, closed=True):
    ebs_enabled: NotRequired["capo_opensearch.types.boolean.Boolean"]
    """<p>Indicates whether EBS volumes are attached to data nodes in an OpenSearch Service domain.</p>"""
    volume_type: NotRequired["capo_opensearch.types.volume_type.VolumeType"]
    """<p>Specifies the type of EBS volumes attached to data nodes.</p>"""
    volume_size: NotRequired["capo_opensearch.types.integer_class.IntegerClass"]
    """<p>Specifies the size (in GiB) of EBS volumes attached to data nodes.</p>"""
    iops: NotRequired["capo_opensearch.types.integer_class.IntegerClass"]
    ...
    throughput: NotRequired["capo_opensearch.types.integer_class.IntegerClass"]
...
def serialize_json(value: EBSOptions) -> dict:
    ...
    if "volume_type" in value:
        ...
        out["VolumeType"] = capo_opensearch.types.volume_type.serialize_json(
            value["volume_type"]
        )
...
def deserialize_json(data: dict) -> EBSOptions:
    ...
    if "VolumeType" in data:
        ...
        out["volume_type"] = capo_opensearch.types.volume_type.deserialize_json(
            data["VolumeType"]
        )
