"""Generated from Smithy shape ``com.amazonaws.opensearch#CreatePackage``."""
...
def handle_response(
    ...
    out: aws_sdk_opensearch.types.create_package_response.CreatePackageResponse = (
        aws_sdk_opensearch.types.create_package_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_opensearch.types.create_package_request.CreatePackageRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/packages"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_opensearch.types.create_package_request

    body: bytes | None = json.dumps(
        aws_sdk_opensearch.types.create_package_request.serialize_json(input_)
    ).encode()
...
async def async_create_package(
