...
def build_request(
    ...
    input: aws_sdk_opensearch.types.describe_instance_type_limits_request.DescribeInstanceTypeLimitsRequest,
) -> zapros.Request:
    ...
    url = url.replace("{InstanceType}", quote(str(input["instance_type"]), safe=""))
    url = url.replace("{EngineVersion}", quote(str(input["engine_version"]), safe=""))
    params: dict[str, str] = {}
    if "domain_name" in input:
        params["domainName"] = str(input["domain_name"])
