"""Generated from Smithy shape ``com.amazonaws.opensearch#CreateDomain``."""
...
def build_request(
    ...
    input_: aws_sdk_opensearch.types.create_domain_request.CreateDomainRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/opensearch/domain"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_opensearch.types.create_domain_request

    body: bytes | None = json.dumps(
        aws_sdk_opensearch.types.create_domain_request.serialize_json(input_)
    ).encode()
