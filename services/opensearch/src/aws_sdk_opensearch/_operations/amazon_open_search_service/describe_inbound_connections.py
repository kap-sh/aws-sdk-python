"""Generated from Smithy shape ``com.amazonaws.opensearch#DescribeInboundConnections``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidPaginationTokenException":
            import aws_sdk_opensearch.errors.invalid_pagination_token_exception

            raise aws_sdk_opensearch.errors.invalid_pagination_token_exception.InvalidPaginationTokenException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_opensearch.types.describe_inbound_connections_response.DescribeInboundConnectionsResponse = aws_sdk_opensearch.types.describe_inbound_connections_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_opensearch.types.describe_inbound_connections_request.DescribeInboundConnectionsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/2021-01-01/opensearch/cc/inboundConnection/search"
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_opensearch.types.describe_inbound_connections_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_describe_inbound_connections(
