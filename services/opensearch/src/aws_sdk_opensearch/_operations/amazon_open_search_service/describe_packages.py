"""Generated from Smithy shape ``com.amazonaws.opensearch#DescribePackages``."""
...
import aws_sdk_opensearch.types.package_details_list
...
def handle_response(
    ...
    out: aws_sdk_opensearch.types.describe_packages_response.DescribePackagesResponse = aws_sdk_opensearch.types.describe_packages_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_opensearch.types.describe_packages_request.DescribePackagesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/packages/describe"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_opensearch.types.describe_packages_request.serialize_json(input_)
    ).encode()
...
async def async_describe_packages(
