"""Generated from Smithy shape ``com.amazonaws.opensearch#UpgradeDomain``."""
...
def handle_response(
    ...
    out: aws_sdk_opensearch.types.upgrade_domain_response.UpgradeDomainResponse = (
        aws_sdk_opensearch.types.upgrade_domain_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_opensearch.types.upgrade_domain_request.UpgradeDomainRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2021-01-01/opensearch/upgradeDomain"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_opensearch.types.upgrade_domain_request

    body: bytes | None = json.dumps(
        aws_sdk_opensearch.types.upgrade_domain_request.serialize_json(input_)
    ).encode()
...
async def async_upgrade_domain(
