...
def serialize_json(value: AuthorizedPrincipalList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_opensearch.types.authorized_principal.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> AuthorizedPrincipalList:
    ...
    for item in data:
        out.append(aws_sdk_opensearch.types.authorized_principal.deserialize_json(item))
    return out
