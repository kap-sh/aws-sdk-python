...
if TYPE_CHECKING:
    import aws_sdk_opensearch.types.insight_entity_type
    import aws_sdk_opensearch.types.insight_entity_value


class InsightEntity(TypedDict):
    type: "aws_sdk_opensearch.types.insight_entity_type.InsightEntityType"
    """<p>The type of the entity. Possible values are <code>Account</code> and <code>DomainName</code>.</p>"""
    value: NotRequired[
        "aws_sdk_opensearch.types.insight_entity_value.InsightEntityValue"
    ]
...
def serialize_json(value: InsightEntity) -> dict:
    ...
    out["Type"] = aws_sdk_opensearch.types.insight_entity_type.serialize_json(
        value["type"]
    )
    if "value" in value:
...
def deserialize_json(data: dict) -> InsightEntity:
    ...
    if "Type" in data:
        ...
        out["type"] = aws_sdk_opensearch.types.insight_entity_type.deserialize_json(
            data["Type"]
        )
    else:
