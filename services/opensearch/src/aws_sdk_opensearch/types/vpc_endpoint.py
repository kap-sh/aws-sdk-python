...
class VpcEndpoint(TypedDict):
    ...
    vpc_endpoint_owner: NotRequired["aws_sdk_opensearch.types.aws_account.AWSAccount"]
    """<p>The creator of the endpoint.</p>"""
    domain_arn: NotRequired["aws_sdk_opensearch.types.domain_arn.DomainArn"]
    ...
    status: NotRequired[
        "aws_sdk_opensearch.types.vpc_endpoint_status.VpcEndpointStatus"
    ]
...
def serialize_json(value: VpcEndpoint) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = aws_sdk_opensearch.types.vpc_endpoint_status.serialize_json(
            value["status"]
        )
...
def deserialize_json(data: dict) -> VpcEndpoint:
    ...
    if "Status" in data:
        ...
        out["status"] = aws_sdk_opensearch.types.vpc_endpoint_status.deserialize_json(
            data["Status"]
        )
    if "Endpoint" in data:
