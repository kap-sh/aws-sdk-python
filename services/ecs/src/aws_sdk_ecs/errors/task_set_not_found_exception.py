"""Generated from Smithy shape ``com.amazonaws.ecs#TaskSetNotFoundException``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from aws_sdk_ecs.errors import ServiceError

if TYPE_CHECKING:
    import aws_sdk_ecs.types.string


class TaskSetNotFoundException_(TypedDict, closed=True):
    message: NotRequired["aws_sdk_ecs.types.string.String"]
    """<p> Message that describes the cause of the exception.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: TaskSetNotFoundException_) -> dict:
    out: dict = {}
    if "message" in value:
        out["message"] = value["message"]
    return out


def deserialize_aws_json_1_1(data: dict) -> TaskSetNotFoundException_:
    out: TaskSetNotFoundException_ = {}  # type: ignore[typeddict-item]
    if "message" in data:
        out["message"] = data["message"]
    return out


class TaskSetNotFoundException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.ecs#TaskSetNotFoundException``."""

    code: str | None = "TaskSetNotFoundException"

    def __init__(self, data: TaskSetNotFoundException_):
        super().__init__(
            "client",
            is_throttling_error=False,
            is_retryable=False,
            code="TaskSetNotFoundException",
        )
        self.data = data

    @classmethod
    def from_aws_json_1_1(cls, data: dict) -> "TaskSetNotFoundException":
        return cls(deserialize_aws_json_1_1(data))
