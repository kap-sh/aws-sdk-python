"""Generated from Smithy shape ``com.amazonaws.ecs#DeploymentController``."""
...
if TYPE_CHECKING:
    import aws_sdk_ecs.types.deployment_controller_type


class DeploymentController(TypedDict, closed=True):
    type: "aws_sdk_ecs.types.deployment_controller_type.DeploymentControllerType"
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DeploymentController) -> dict:
    out: dict = {}
    import aws_sdk_ecs.types.deployment_controller_type

    out["type"] = aws_sdk_ecs.types.deployment_controller_type.serialize_aws_json_1_1(
        value["type"]
    )
    return out


def deserialize_aws_json_1_1(data: dict) -> DeploymentController:
    out: DeploymentController = {}  # type: ignore[typeddict-item]
    if "type" in data:
        import aws_sdk_ecs.types.deployment_controller_type

        out["type"] = (
            aws_sdk_ecs.types.deployment_controller_type.deserialize_aws_json_1_1(
                data["type"]
            )
        )
    else:
        raise DeserializationError("DeploymentController.type required")
    return out
