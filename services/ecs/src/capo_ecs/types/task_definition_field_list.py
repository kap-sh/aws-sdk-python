...
if TYPE_CHECKING:
    import capo_ecs.types.task_definition_field

TaskDefinitionFieldList: TypeAlias = list[
    "capo_ecs.types.task_definition_field.TaskDefinitionField"
]
...
def serialize_aws_json_1_1(value: TaskDefinitionFieldList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_ecs.types.task_definition_field.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> TaskDefinitionFieldList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(capo_ecs.types.task_definition_field.deserialize_aws_json_1_1(item))
    return out
