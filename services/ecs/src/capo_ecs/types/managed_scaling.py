...
if TYPE_CHECKING:
    import capo_ecs.types.managed_scaling_instance_warmup_period
    import capo_ecs.types.managed_scaling_status
    import capo_ecs.types.managed_scaling_step_size
    import capo_ecs.types.managed_scaling_target_capacity


class ManagedScaling(TypedDict, closed=True):
    status: NotRequired["capo_ecs.types.managed_scaling_status.ManagedScalingStatus"]
    """<p>Determines whether to use managed scaling for the capacity provider.</p>"""
    target_capacity: NotRequired[
        "capo_ecs.types.managed_scaling_target_capacity.ManagedScalingTargetCapacity"
    ]
    ...
    minimum_scaling_step_size: NotRequired[
        "capo_ecs.types.managed_scaling_step_size.ManagedScalingStepSize"
    ]
    ...
    instance_warmup_period: NotRequired[
        "capo_ecs.types.managed_scaling_instance_warmup_period.ManagedScalingInstanceWarmupPeriod"
    ]
...
def serialize_aws_json_1_1(value: ManagedScaling) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_ecs.types.managed_scaling_status.serialize_aws_json_1_1(
            value["status"]
        )
