...
class ListTasksResponse(TypedDict, closed=True):
    task_arns: NotRequired["capo_ecs.types.string_list.StringList"]
...
def serialize_aws_json_1_1(value: ListTasksResponse) -> dict:
    ...
    if "task_arns" in value:
        ...
        out["taskArns"] = capo_ecs.types.string_list.serialize_aws_json_1_1(
            value["task_arns"]
        )
...
def deserialize_aws_json_1_1(data: dict) -> ListTasksResponse:
    ...
    if data.get("taskArns") is not None:
        import capo_ecs.types.string_list

        out["task_arns"] = capo_ecs.types.string_list.deserialize_aws_json_1_1(
            data["taskArns"]
        )
