...
if TYPE_CHECKING:
    import capo_ecs.types.deployment_lifecycle_hook_detail

DeploymentLifecycleHookDetailList: TypeAlias = list[
    "capo_ecs.types.deployment_lifecycle_hook_detail.DeploymentLifecycleHookDetail"
]
...
def serialize_aws_json_1_1(value: DeploymentLifecycleHookDetailList) -> list:
    ...
    for item in value:
        out.append(
            capo_ecs.types.deployment_lifecycle_hook_detail.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> DeploymentLifecycleHookDetailList:
    ...
    for item in data:
        if item is None:
            continue
        out.append(
            capo_ecs.types.deployment_lifecycle_hook_detail.deserialize_aws_json_1_1(
                item
            )
        )
    return out
