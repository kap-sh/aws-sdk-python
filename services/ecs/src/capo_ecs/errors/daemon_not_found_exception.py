...
class DaemonNotFoundException_(TypedDict, closed=True):
    message: NotRequired["capo_ecs.types.string.String"]
    """<p> Message that describes the cause of the exception.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: DaemonNotFoundException_) -> dict:
    out: dict = {}
    if "message" in value:
...
class DaemonNotFoundException(ServiceError):
    ...
    def __init__(self, data: DaemonNotFoundException_, message: str | None = None):
        super().__init__(
            "client",
