...
def build_request(
    ...
    input: aws_sdk_appsync.types.update_type_request.UpdateTypeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{typeName}", quote(str(input["type_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_appsync.types.update_type_request

    body: bytes | None = json.dumps(
        aws_sdk_appsync.types.update_type_request.serialize_json(input)
    ).encode()
