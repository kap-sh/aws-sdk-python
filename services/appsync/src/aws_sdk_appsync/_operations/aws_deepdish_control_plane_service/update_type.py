"""Generated from Smithy shape ``com.amazonaws.appsync#UpdateType``."""
...
def handle_response(
    ...
    out: aws_sdk_appsync.types.update_type_response.UpdateTypeResponse = (
        aws_sdk_appsync.types.update_type_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_appsync.types.update_type_request.UpdateTypeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apis/{apiId}/types/{typeName}"
    url = url.replace("{apiId}", quote(str(input_["api_id"]), safe=""))
    url = url.replace("{typeName}", quote(str(input_["type_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_appsync.types.update_type_request

    body: bytes | None = json.dumps(
        aws_sdk_appsync.types.update_type_request.serialize_json(input_)
    ).encode()
...
def update_type(
    ...
    aws_sdk_appsync.types.update_type_response.UpdateTypeResponse, zapros.Response
]:
