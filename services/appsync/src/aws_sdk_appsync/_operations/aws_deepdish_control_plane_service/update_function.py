"""Generated from Smithy shape ``com.amazonaws.appsync#UpdateFunction``."""
...
def handle_response(
    ...
    out: aws_sdk_appsync.types.update_function_response.UpdateFunctionResponse = (
        aws_sdk_appsync.types.update_function_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_appsync.types.update_function_request.UpdateFunctionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apis/{apiId}/functions/{functionId}"
    url = url.replace("{apiId}", quote(str(input_["api_id"]), safe=""))
    url = url.replace("{functionId}", quote(str(input_["function_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_appsync.types.update_function_request

    body: bytes | None = json.dumps(
        aws_sdk_appsync.types.update_function_request.serialize_json(input_)
    ).encode()
