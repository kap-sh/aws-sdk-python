...
def build_request(
    ...
    input: aws_sdk_appsync.types.update_function_request.UpdateFunctionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{functionId}", quote(str(input["function_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_appsync.types.update_function_request

    body: bytes | None = json.dumps(
        aws_sdk_appsync.types.update_function_request.serialize_json(input)
    ).encode()
