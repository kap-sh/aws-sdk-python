"""Generated from Smithy shape ``com.amazonaws.appsync#CreateDomainName``."""
...
def handle_response(
    ...
    out: aws_sdk_appsync.types.create_domain_name_response.CreateDomainNameResponse = (
        aws_sdk_appsync.types.create_domain_name_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_appsync.types.create_domain_name_request.CreateDomainNameRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/domainnames"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_appsync.types.create_domain_name_request

    body: bytes | None = json.dumps(
        aws_sdk_appsync.types.create_domain_name_request.serialize_json(input_)
    ).encode()
