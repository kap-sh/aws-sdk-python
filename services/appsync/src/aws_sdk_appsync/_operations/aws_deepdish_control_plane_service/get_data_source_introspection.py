...
def build_request(
    ...
    input_: aws_sdk_appsync.types.get_data_source_introspection_request.GetDataSourceIntrospectionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{introspectionId}", quote(str(input_["introspection_id"]), safe="")
    )
    params: dict[str, str] = {}
    params["includeModelsSDL"] = str(input_.get("include_models_sdl", False))
