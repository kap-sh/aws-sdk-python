...
SourceApiAssociationSummaryList: TypeAlias = list[
    "aws_sdk_appsync.types.source_api_association_summary.SourceApiAssociationSummary"
]
...
def serialize_json(value: SourceApiAssociationSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_appsync.types.source_api_association_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> SourceApiAssociationSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_appsync.types.source_api_association_summary.deserialize_json(item)
        )
    return out
