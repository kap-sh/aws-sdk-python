...
if TYPE_CHECKING:
    import aws_sdk_appsync.types.lambda_config
    import aws_sdk_appsync.types.string


class Integration(TypedDict, closed=True):
    data_source_name: "aws_sdk_appsync.types.string.String"
    """<p>The unique name of the data source that has been configured on the API.</p>"""
    lambda_config: NotRequired["aws_sdk_appsync.types.lambda_config.LambdaConfig"]
...
def serialize_json(value: Integration) -> dict:
    ...
    if "lambda_config" in value:
        ...
        out["lambdaConfig"] = aws_sdk_appsync.types.lambda_config.serialize_json(
            value["lambda_config"]
        )
    return out
...
def deserialize_json(data: dict) -> Integration:
    ...
    if "lambdaConfig" in data:
        ...
        out["lambda_config"] = aws_sdk_appsync.types.lambda_config.deserialize_json(
            data["lambdaConfig"]
        )
    return out
