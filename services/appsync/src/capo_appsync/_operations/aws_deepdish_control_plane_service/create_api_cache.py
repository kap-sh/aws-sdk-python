"""Generated from Smithy shape ``com.amazonaws.appsync#CreateApiCache``."""
...
def handle_response(
    ...
    out: capo_appsync.types.create_api_cache_response.CreateApiCacheResponse = (
        capo_appsync.types.create_api_cache_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apis/{apiId}/ApiCaches"
    url = url.replace("{apiId}", quote(str(input_["api_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_appsync.types.create_api_cache_request.serialize_json(input_)
    ).encode()
...
def create_api_cache(
    ...
    capo_appsync.types.create_api_cache_response.CreateApiCacheResponse, zapros.Response
]:
...
async def async_create_api_cache(
