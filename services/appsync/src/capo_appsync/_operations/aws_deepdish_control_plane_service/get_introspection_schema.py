"""Generated from Smithy shape ``com.amazonaws.appsync#GetIntrospectionSchema``."""
...
import capo_appsync.errors.graph_ql_schema_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "GraphQLSchemaException":
            raise capo_appsync.errors.graph_ql_schema_exception.GraphQLSchemaException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_appsync.types.get_introspection_schema_response.GetIntrospectionSchemaResponse = {
        "schema": capo_appsync.types.blob.deserialize_json(json.loads(response.read()))
...
async def async_handle_response(
    ...
    out: capo_appsync.types.get_introspection_schema_response.GetIntrospectionSchemaResponse = {
        "schema": capo_appsync.types.blob.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apis/{apiId}/schema"
    url = url.replace("{apiId}", quote(str(input_["api_id"]), safe=""))
    params: dict[str, str] = {}
    if "format" in input_:
        params["format"] = str(input_["format"])
    if "include_directives" in input_:
        params["includeDirectives"] = str(input_["include_directives"])
...
async def async_get_introspection_schema(
