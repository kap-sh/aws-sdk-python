...
class AppSyncError(Exception):
    """Base class for all AppSync service errors."""


class ServiceError(AppSyncError):
...
class SerializationError(AppSyncError):
    """Raised when a value cannot be serialized."""


class DeserializationError(AppSyncError):
...
class WaiterFailedError(AppSyncError):
...
class WaiterTimeoutError(AppSyncError):
