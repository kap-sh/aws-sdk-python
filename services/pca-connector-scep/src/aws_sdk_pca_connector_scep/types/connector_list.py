...
ConnectorList: TypeAlias = list[
    "aws_sdk_pca_connector_scep.types.connector_summary.ConnectorSummary"
]
...
def serialize_json(value: ConnectorList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_pca_connector_scep.types.connector_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ConnectorList:
    ...
    for item in data:
        out.append(
            aws_sdk_pca_connector_scep.types.connector_summary.deserialize_json(item)
        )
    return out
