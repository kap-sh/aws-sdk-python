...
class BatchRemoveChannelRoleFromAccessorsInput(TypedDict):
...
def deserialize_json(data: dict) -> BatchRemoveChannelRoleFromAccessorsInput:
    ...
    if "accessorIds" in data:
        ...
        raise DeserializationError("BatchRemoveChannelRoleFromAccessorsInput.accessor_ids required")
    if "channelRole" in data:
        import aws_sdk_repostspace.types.channel_role
        out["channel_role"] = aws_sdk_repostspace.types.channel_role.deserialize_json(data["channelRole"])
    else:
        raise DeserializationError("BatchRemoveChannelRoleFromAccessorsInput.channel_role required")
    return out