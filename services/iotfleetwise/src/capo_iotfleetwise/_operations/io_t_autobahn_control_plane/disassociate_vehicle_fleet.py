"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#DisassociateVehicleFleet``."""
...
def handle_response(
    ...
    out: capo_iotfleetwise.types.disassociate_vehicle_fleet_response.DisassociateVehicleFleetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/vehicles/{vehicleName}/disassociate"
    url = url.replace("{vehicleName}", quote(str(input_["vehicle_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.DisassociateVehicleFleet"
    body: bytes | None = json.dumps(
        capo_iotfleetwise.types.disassociate_vehicle_fleet_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_disassociate_vehicle_fleet(
