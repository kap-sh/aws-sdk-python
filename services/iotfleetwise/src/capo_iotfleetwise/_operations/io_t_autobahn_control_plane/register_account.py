"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#RegisterAccount``."""
...
def handle_response(
    ...
    out: capo_iotfleetwise.types.register_account_response.RegisterAccountResponse = (
        capo_iotfleetwise.types.register_account_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/account/registration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.RegisterAccount"
    body: bytes | None = json.dumps(
        capo_iotfleetwise.types.register_account_request.serialize_aws_json_1_0(input_)
    ).encode()
