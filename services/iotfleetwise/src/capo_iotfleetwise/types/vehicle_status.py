...
if TYPE_CHECKING:
    ...
    import capo_iotfleetwise.types.vehicle_state


class VehicleStatus(TypedDict, closed=True):
    campaign_name: NotRequired["capo_iotfleetwise.types.campaign_name.campaignName"]
    ...
    status: NotRequired["capo_iotfleetwise.types.vehicle_state.VehicleState"]
...
def serialize_aws_json_1_0(value: VehicleStatus) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_iotfleetwise.types.vehicle_state.serialize_aws_json_1_0(
            value["status"]
        )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> VehicleStatus:
    ...
    if "status" in data:
        ...
        out["status"] = capo_iotfleetwise.types.vehicle_state.deserialize_aws_json_1_0(
            data["status"]
        )
    return out
