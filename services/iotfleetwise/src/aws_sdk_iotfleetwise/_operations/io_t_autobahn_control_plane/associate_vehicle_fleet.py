"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#AssociateVehicleFleet``."""
...
def handle_response(
    ...
    out: aws_sdk_iotfleetwise.types.associate_vehicle_fleet_response.AssociateVehicleFleetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_iotfleetwise.types.associate_vehicle_fleet_request.AssociateVehicleFleetRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/vehicles/{vehicleName}/associate"
    url = url.replace("{vehicleName}", quote(str(input["vehicle_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.AssociateVehicleFleet"
    import aws_sdk_iotfleetwise.types.associate_vehicle_fleet_request

    body: bytes | None = json.dumps(
        aws_sdk_iotfleetwise.types.associate_vehicle_fleet_request.serialize_aws_json_1_0(
            input
        )
    ).encode()
...
async def async_associate_vehicle_fleet(
