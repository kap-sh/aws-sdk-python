"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#CreateStateTemplate``."""
...
def handle_response(
    ...
    out: aws_sdk_iotfleetwise.types.create_state_template_response.CreateStateTemplateResponse = aws_sdk_iotfleetwise.types.create_state_template_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_iotfleetwise.types.create_state_template_request.CreateStateTemplateRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/state-templates/{name}"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.CreateStateTemplate"
    body: bytes | None = json.dumps(
        aws_sdk_iotfleetwise.types.create_state_template_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_create_state_template(
