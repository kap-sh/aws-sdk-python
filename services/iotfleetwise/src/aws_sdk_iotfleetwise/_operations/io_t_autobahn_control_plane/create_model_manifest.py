"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#CreateModelManifest``."""
...
def handle_response(
    ...
    out: aws_sdk_iotfleetwise.types.create_model_manifest_response.CreateModelManifestResponse = aws_sdk_iotfleetwise.types.create_model_manifest_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_iotfleetwise.types.create_model_manifest_request.CreateModelManifestRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/model-manifests/{name}"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.CreateModelManifest"
    import aws_sdk_iotfleetwise.types.create_model_manifest_request

    body: bytes | None = json.dumps(
        aws_sdk_iotfleetwise.types.create_model_manifest_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_create_model_manifest(
