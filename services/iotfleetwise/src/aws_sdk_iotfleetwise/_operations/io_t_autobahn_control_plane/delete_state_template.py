"""Generated from Smithy shape ``com.amazonaws.iotfleetwise#DeleteStateTemplate``."""
...
def handle_response(
    ...
    out: aws_sdk_iotfleetwise.types.delete_state_template_response.DeleteStateTemplateResponse = aws_sdk_iotfleetwise.types.delete_state_template_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_iotfleetwise.types.delete_state_template_request.DeleteStateTemplateRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/state-templates/{identifier}"
    url = url.replace("{identifier}", quote(str(input_["identifier"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "IoTAutobahnControlPlane.DeleteStateTemplate"
    body: bytes | None = b""
...
async def async_delete_state_template(
