...
class ListControlsRequest(TypedDict):
    ...
    filter: NotRequired["aws_sdk_controlcatalog.types.control_filter.ControlFilter"]
...
def serialize_json(value: ListControlsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["Filter"] = aws_sdk_controlcatalog.types.control_filter.serialize_json(
            value["filter"]
        )
    return out
...
def deserialize_json(data: dict) -> ListControlsRequest:
    ...
    if "Filter" in data:
        ...
        out["filter"] = aws_sdk_controlcatalog.types.control_filter.deserialize_json(
            data["Filter"]
        )
    return out
