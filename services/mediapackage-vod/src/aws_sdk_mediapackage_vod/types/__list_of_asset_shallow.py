...
__listOfAssetShallow: TypeAlias = list[
    "aws_sdk_mediapackage_vod.types.asset_shallow.AssetShallow"
]
...
def serialize_json(value: __listOfAssetShallow) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_mediapackage_vod.types.asset_shallow.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfAssetShallow:
    ...
    for item in data:
        out.append(aws_sdk_mediapackage_vod.types.asset_shallow.deserialize_json(item))
    return out
