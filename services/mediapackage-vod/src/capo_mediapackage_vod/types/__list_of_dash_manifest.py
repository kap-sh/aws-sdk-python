...
if TYPE_CHECKING:
    import capo_mediapackage_vod.types.dash_manifest

__listOfDashManifest: TypeAlias = list[
    "capo_mediapackage_vod.types.dash_manifest.DashManifest"
]
...
def serialize_json(value: __listOfDashManifest) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mediapackage_vod.types.dash_manifest.serialize_json(item))
    return out
