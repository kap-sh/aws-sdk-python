"""Generated from Smithy shape ``com.amazonaws.mediapackagevod#MssEncryption``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_mediapackage_vod.types.speke_key_provider


class MssEncryption(TypedDict, closed=True):
    speke_key_provider: NotRequired[
        "capo_mediapackage_vod.types.speke_key_provider.SpekeKeyProvider"
    ]


# --- restJson1 ser/de ---
def serialize_json(value: MssEncryption) -> dict:
    out: dict = {}
    if "speke_key_provider" in value:
        import capo_mediapackage_vod.types.speke_key_provider

        out["spekeKeyProvider"] = (
            capo_mediapackage_vod.types.speke_key_provider.serialize_json(
                value["speke_key_provider"]
            )
        )
    return out


def deserialize_json(data: dict) -> MssEncryption:
    out: MssEncryption = {}  # type: ignore[typeddict-item]
    if "spekeKeyProvider" in data:
        import capo_mediapackage_vod.types.speke_key_provider

        out["speke_key_provider"] = (
            capo_mediapackage_vod.types.speke_key_provider.deserialize_json(
                data["spekeKeyProvider"]
            )
        )
    return out
