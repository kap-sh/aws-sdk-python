"""Generated from Smithy shape ``com.amazonaws.mediapackagevod#ConfigureLogs``."""
...
import capo_mediapackage_vod.types.configure_logs_request
import capo_mediapackage_vod.types.configure_logs_response
...
) -> capo_mediapackage_vod.types.configure_logs_response.ConfigureLogsResponse:
    out: capo_mediapackage_vod.types.configure_logs_response.ConfigureLogsResponse = (
        capo_mediapackage_vod.types.configure_logs_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/packaging_groups/{Id}/configure_logs"
    url = url.replace("{Id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediapackage_vod.types.configure_logs_request.serialize_json(input_)
    ).encode()
