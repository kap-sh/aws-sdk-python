"""Generated from Smithy shape ``com.amazonaws.gamelift#Event``."""
...
if TYPE_CHECKING:
    ...
    import capo_gamelift.types.event_count
...
class Event(TypedDict, closed=True):
    ...
    event_code: NotRequired["capo_gamelift.types.event_code.EventCode"]
    ...
    event_time: NotRequired["capo_gamelift.types.timestamp.Timestamp"]
    r"""<p>Time stamp indicating when this event occurred. Format is a number expressed in Unix time as milliseconds (for example <code>\"1469498468.057\"</code>).</p>"""
    pre_signed_log_url: NotRequired[
        "capo_gamelift.types.non_zero_and_max_string.NonZeroAndMaxString"
    ]
    ...
    count: NotRequired["capo_gamelift.types.event_count.EventCount"]
...
def serialize_aws_json_1_1(value: Event) -> dict:
    ...
    if "event_code" in value:
        import capo_gamelift.types.event_code

        out["EventCode"] = capo_gamelift.types.event_code.serialize_aws_json_1_1(
            value["event_code"]
        )
    if "message" in value:
    ...
    if "event_time" in value:
        ...
        out["EventTime"] = capo_gamelift.types.timestamp.serialize_aws_json_1_1(
            value["event_time"]
        )
    if "pre_signed_log_url" in value:
        out["PreSignedLogUrl"] = value["pre_signed_log_url"]
    if "count" in value:
