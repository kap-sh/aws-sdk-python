"""Generated from Smithy shape ``com.amazonaws.efs#DescribeAccountPreferences``."""
...
import capo_efs.types.describe_account_preferences_request
import capo_efs.types.describe_account_preferences_response
...
) -> capo_efs.types.describe_account_preferences_response.DescribeAccountPreferencesResponse:
    out: capo_efs.types.describe_account_preferences_response.DescribeAccountPreferencesResponse = capo_efs.types.describe_account_preferences_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2015-02-01/account-preferences"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_efs.types.describe_account_preferences_request.serialize_json(input_)
    ).encode()
...
async def async_describe_account_preferences(
