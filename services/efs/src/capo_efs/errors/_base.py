...
class EFSError(Exception):
    """Base class for all EFS service errors."""


class ServiceError(EFSError):
...
class SerializationError(EFSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(EFSError):
...
class WaiterFailedError(EFSError):
...
class WaiterTimeoutError(EFSError):
