...
ParticipantReplicaList: TypeAlias = list[
    "aws_sdk_ivs_realtime.types.participant_replica.ParticipantReplica"
]
...
def serialize_json(value: ParticipantReplicaList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_ivs_realtime.types.participant_replica.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ParticipantReplicaList:
    ...
    for item in data:
        out.append(
            aws_sdk_ivs_realtime.types.participant_replica.deserialize_json(item)
        )
    return out
