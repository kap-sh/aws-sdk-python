"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#GetStageSession``."""
...
def handle_response(
    ...
    out: aws_sdk_ivs_realtime.types.get_stage_session_response.GetStageSessionResponse = aws_sdk_ivs_realtime.types.get_stage_session_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_ivs_realtime.types.get_stage_session_request.GetStageSessionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetStageSession"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_ivs_realtime.types.get_stage_session_request.serialize_json(input_)
    ).encode()
...
async def async_get_stage_session(
