"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#CreateStage``."""
...
def handle_response(
    ...
    out: aws_sdk_ivs_realtime.types.create_stage_response.CreateStageResponse = (
        aws_sdk_ivs_realtime.types.create_stage_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_ivs_realtime.types.create_stage_request.CreateStageRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/CreateStage"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_ivs_realtime.types.create_stage_request

    body: bytes | None = json.dumps(
        aws_sdk_ivs_realtime.types.create_stage_request.serialize_json(input_)
    ).encode()
