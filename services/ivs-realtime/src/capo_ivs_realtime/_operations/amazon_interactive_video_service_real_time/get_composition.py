"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#GetComposition``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.get_composition_response.GetCompositionResponse = (
        capo_ivs_realtime.types.get_composition_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetComposition"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.get_composition_request.serialize_json(input_)
    ).encode()
