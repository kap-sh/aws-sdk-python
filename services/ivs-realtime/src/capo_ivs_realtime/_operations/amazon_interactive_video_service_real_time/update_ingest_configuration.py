"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#UpdateIngestConfiguration``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateIngestConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.update_ingest_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_ingest_configuration(
