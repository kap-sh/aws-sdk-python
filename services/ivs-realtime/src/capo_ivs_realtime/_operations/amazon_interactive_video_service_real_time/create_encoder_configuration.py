"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#CreateEncoderConfiguration``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.create_encoder_configuration_response.CreateEncoderConfigurationResponse = capo_ivs_realtime.types.create_encoder_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateEncoderConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.create_encoder_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_encoder_configuration(
