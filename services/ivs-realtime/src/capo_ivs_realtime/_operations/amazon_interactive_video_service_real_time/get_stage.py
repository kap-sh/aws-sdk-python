"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#GetStage``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.get_stage_response.GetStageResponse = (
        capo_ivs_realtime.types.get_stage_response.deserialize_json(
...
def build_request(
    ...
    input_: capo_ivs_realtime.types.get_stage_request.GetStageRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetStage"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.get_stage_request.serialize_json(input_)
    ).encode()
...
def get_stage(
    ...
    capo_ivs_realtime.types.get_stage_response.GetStageResponse, zapros.Response
]:
