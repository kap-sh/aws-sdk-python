"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#DeleteStorageConfiguration``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.delete_storage_configuration_response.DeleteStorageConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteStorageConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.delete_storage_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_storage_configuration(
