"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#DeletePublicKey``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.delete_public_key_response.DeletePublicKeyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeletePublicKey"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.delete_public_key_request.serialize_json(input_)
    ).encode()
...
async def async_delete_public_key(
