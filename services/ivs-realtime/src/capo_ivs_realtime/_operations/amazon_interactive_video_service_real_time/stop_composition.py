"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#StopComposition``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.stop_composition_response.StopCompositionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StopComposition"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.stop_composition_request.serialize_json(input_)
    ).encode()
...
async def async_stop_composition(
