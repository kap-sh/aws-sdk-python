"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#CreateParticipantToken``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.create_participant_token_response.CreateParticipantTokenResponse = capo_ivs_realtime.types.create_participant_token_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateParticipantToken"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.create_participant_token_request.serialize_json(input_)
    ).encode()
...
async def async_create_participant_token(
