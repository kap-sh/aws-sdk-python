"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#ListEncoderConfigurations``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.list_encoder_configurations_response.ListEncoderConfigurationsResponse = capo_ivs_realtime.types.list_encoder_configurations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListEncoderConfigurations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.list_encoder_configurations_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_encoder_configurations(
