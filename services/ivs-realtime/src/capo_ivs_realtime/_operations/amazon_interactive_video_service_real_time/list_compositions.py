"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#ListCompositions``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.list_compositions_response.ListCompositionsResponse = (
        capo_ivs_realtime.types.list_compositions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListCompositions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.list_compositions_request.serialize_json(input_)
    ).encode()
...
async def async_list_compositions(
