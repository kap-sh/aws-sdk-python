"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#ListStageSessions``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.list_stage_sessions_response.ListStageSessionsResponse = capo_ivs_realtime.types.list_stage_sessions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListStageSessions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.list_stage_sessions_request.serialize_json(input_)
    ).encode()
...
async def async_list_stage_sessions(
