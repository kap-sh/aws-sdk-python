"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#GetStorageConfiguration``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.get_storage_configuration_response.GetStorageConfigurationResponse = capo_ivs_realtime.types.get_storage_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetStorageConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.get_storage_configuration_request.serialize_json(input_)
    ).encode()
...
async def async_get_storage_configuration(
