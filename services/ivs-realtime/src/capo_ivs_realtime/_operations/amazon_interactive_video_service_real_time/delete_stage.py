"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#DeleteStage``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.delete_stage_response.DeleteStageResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteStage"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.delete_stage_request.serialize_json(input_)
    ).encode()
...
def delete_stage(
    ...
    capo_ivs_realtime.types.delete_stage_response.DeleteStageResponse, zapros.Response
]:
