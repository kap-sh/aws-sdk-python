"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#ListIngestConfigurations``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.list_ingest_configurations_response.ListIngestConfigurationsResponse = capo_ivs_realtime.types.list_ingest_configurations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListIngestConfigurations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.list_ingest_configurations_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_ingest_configurations(
