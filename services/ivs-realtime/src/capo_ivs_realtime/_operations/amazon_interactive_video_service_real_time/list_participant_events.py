"""Generated from Smithy shape ``com.amazonaws.ivsrealtime#ListParticipantEvents``."""
...
def handle_response(
    ...
    out: capo_ivs_realtime.types.list_participant_events_response.ListParticipantEventsResponse = capo_ivs_realtime.types.list_participant_events_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListParticipantEvents"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ivs_realtime.types.list_participant_events_request.serialize_json(input_)
    ).encode()
...
async def async_list_participant_events(
