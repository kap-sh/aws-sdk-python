...
if TYPE_CHECKING:
    ...
    import aws_sdk_securityhub.types.volume_mount_list


class ContainerDetails(TypedDict):
    ...
    image_name: NotRequired["aws_sdk_securityhub.types.non_empty_string.NonEmptyString"]
    """<p>The name of the container image related to a finding.</p>"""
    launched_at: NotRequired[
        "aws_sdk_securityhub.types.non_empty_string.NonEmptyString"
    ]
    """<p>Indicates when the container started.</p> <p>For more information about the validation and formatting of timestamp fields in Security Hub CSPM, see <a href=\"https://docs.aws.amazon.com/securityhub/1.0/APIReference/Welcome.html#timestamps\">Timestamps</a>.</p>"""
    volume_mounts: NotRequired[
        "aws_sdk_securityhub.types.volume_mount_list.VolumeMountList"
    ]
...
def serialize_json(value: ContainerDetails) -> dict:
    ...
    if "volume_mounts" in value:
        ...
        out["VolumeMounts"] = (
            aws_sdk_securityhub.types.volume_mount_list.serialize_json(
                value["volume_mounts"]
            )
        )
...
def deserialize_json(data: dict) -> ContainerDetails:
    ...
    if "VolumeMounts" in data:
        ...
        out["volume_mounts"] = (
            aws_sdk_securityhub.types.volume_mount_list.deserialize_json(
                data["VolumeMounts"]
            )
        )
