...
class GetFindingsV2Request(TypedDict):
    filters: NotRequired[
...
def serialize_json(value: GetFindingsV2Request) -> dict:
    ...
    if "scopes" in value:
        ...
        out["Scopes"] = aws_sdk_securityhub.types.finding_scopes.serialize_json(
            value["scopes"]
        )
...
def deserialize_json(data: dict) -> GetFindingsV2Request:
    ...
    if "Scopes" in data:
        ...
        out["scopes"] = aws_sdk_securityhub.types.finding_scopes.deserialize_json(
            data["Scopes"]
        )
