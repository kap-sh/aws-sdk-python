...
def build_request(
    ...
    input_: aws_sdk_securityhub.types.update_standards_control_request.UpdateStandardsControlRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{StandardsControlArn+}", quote(str(input_["standards_control_arn"]), safe="/")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_securityhub.types.update_standards_control_request.serialize_json(
            input_
        )
    ).encode()
