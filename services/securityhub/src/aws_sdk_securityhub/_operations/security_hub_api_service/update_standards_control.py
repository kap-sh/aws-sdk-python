"""Generated from Smithy shape ``com.amazonaws.securityhub#UpdateStandardsControl``."""
...
def handle_response(
    ...
    out: aws_sdk_securityhub.types.update_standards_control_response.UpdateStandardsControlResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_securityhub.types.update_standards_control_request.UpdateStandardsControlRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/standards/control/{StandardsControlArn+}"
    url = url.replace(
        "{StandardsControlArn+}", quote(str(input["standards_control_arn"]), safe="/")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_securityhub.types.update_standards_control_request.serialize_json(input)
    ).encode()
...
async def async_update_standards_control(
