"""Generated from Smithy shape ``com.amazonaws.securityhub#UpdateActionTarget``."""
...
def handle_response(
    ...
    out: aws_sdk_securityhub.types.update_action_target_response.UpdateActionTargetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_securityhub.types.update_action_target_request.UpdateActionTargetRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/actionTargets/{ActionTargetArn+}"
    url = url.replace(
        "{ActionTargetArn+}", quote(str(input_["action_target_arn"]), safe="/")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_securityhub.types.update_action_target_request.serialize_json(input_)
    ).encode()
...
async def async_update_action_target(
