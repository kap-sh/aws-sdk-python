"""Generated from Smithy shape ``com.amazonaws.securityhub#UpdateConnectorV2``."""
...
def handle_response(
    ...
    out: aws_sdk_securityhub.types.update_connector_v2_response.UpdateConnectorV2Response = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_securityhub.types.update_connector_v2_request.UpdateConnectorV2Request,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/connectorsv2/{ConnectorId+}"
    url = url.replace("{ConnectorId+}", quote(str(input_["connector_id"]), safe="/"))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_securityhub.types.update_connector_v2_request.serialize_json(input_)
    ).encode()
...
async def async_update_connector_v2(
