"""Generated from Smithy shape ``com.amazonaws.securityhub#DisableImportFindingsForProduct``."""
...
def handle_response(
    ...
    out: aws_sdk_securityhub.types.disable_import_findings_for_product_response.DisableImportFindingsForProductResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_securityhub.types.disable_import_findings_for_product_request.DisableImportFindingsForProductRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/productSubscriptions/{ProductSubscriptionArn+}"
    url = url.replace(
        "{ProductSubscriptionArn+}",
        quote(str(input_["product_subscription_arn"]), safe="/"),
    )
    params: dict[str, str] = {}
...
async def async_disable_import_findings_for_product(
