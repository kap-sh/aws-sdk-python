"""Generated from Smithy shape ``com.amazonaws.securityhub#GetFindingAggregator``."""
...
def handle_response(
    ...
    out: aws_sdk_securityhub.types.get_finding_aggregator_response.GetFindingAggregatorResponse = aws_sdk_securityhub.types.get_finding_aggregator_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_securityhub.types.get_finding_aggregator_request.GetFindingAggregatorRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/findingAggregator/get/{FindingAggregatorArn+}"
    url = url.replace(
        "{FindingAggregatorArn+}", quote(str(input["finding_aggregator_arn"]), safe="/")
    )
    params: dict[str, str] = {}
...
async def async_get_finding_aggregator(
