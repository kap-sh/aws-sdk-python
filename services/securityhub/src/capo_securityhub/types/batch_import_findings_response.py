...
class BatchImportFindingsResponse(TypedDict, closed=True):
    ...
    success_count: NotRequired["capo_securityhub.types.integer.Integer"]
    """<p>The number of findings that were successfully imported.</p>"""
    failed_findings: NotRequired[
        "capo_securityhub.types.import_findings_error_list.ImportFindingsErrorList"
    ]
...
def serialize_json(value: BatchImportFindingsResponse) -> dict:
    ...
    if "failed_findings" in value:
        ...
        out["FailedFindings"] = (
            capo_securityhub.types.import_findings_error_list.serialize_json(
...
def deserialize_json(data: dict) -> BatchImportFindingsResponse:
    ...
    if "FailedFindings" in data:
        ...
        out["failed_findings"] = (
            capo_securityhub.types.import_findings_error_list.deserialize_json(
                data["FailedFindings"]
            )
        )
    return out
