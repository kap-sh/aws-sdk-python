...
class AwsEcsTaskDetails(TypedDict, closed=True):
    ...
    started_at: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    ...
    started_by: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    ...
    group: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p>The name of the task group that's associated with the task. </p>"""
    volumes: NotRequired[
        "capo_securityhub.types.aws_ecs_task_volume_details_list.AwsEcsTaskVolumeDetailsList"
    ]
    """<p>Details about the data volume that is used in a task definition. </p>"""
    containers: NotRequired[
        "capo_securityhub.types.aws_ecs_container_details_list.AwsEcsContainerDetailsList"
    ]
...
def serialize_json(value: AwsEcsTaskDetails) -> dict:
    ...
    if "volumes" in value:
        ...
        out["Volumes"] = (
            capo_securityhub.types.aws_ecs_task_volume_details_list.serialize_json(
                value["volumes"]
            )
        )
    if "containers" in value:
        ...
        out["Containers"] = (
            capo_securityhub.types.aws_ecs_container_details_list.serialize_json(
                value["containers"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AwsEcsTaskDetails:
    ...
    if "Volumes" in data:
        ...
        out["volumes"] = (
            capo_securityhub.types.aws_ecs_task_volume_details_list.deserialize_json(
                data["Volumes"]
            )
        )
    if "Containers" in data:
        ...
        out["containers"] = (
            capo_securityhub.types.aws_ecs_container_details_list.deserialize_json(
                data["Containers"]
            )
        )
    return out
