...
if TYPE_CHECKING:
    import capo_securityhub.types.custom_data_identifiers_detections

CustomDataIdentifiersDetectionsList: TypeAlias = list[
    "capo_securityhub.types.custom_data_identifiers_detections.CustomDataIdentifiersDetections"
]
...
def serialize_json(value: CustomDataIdentifiersDetectionsList) -> list:
    ...
    for item in value:
        out.append(
            capo_securityhub.types.custom_data_identifiers_detections.serialize_json(
                item
            )
        )
    return out
