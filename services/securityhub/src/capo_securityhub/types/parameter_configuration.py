...
if TYPE_CHECKING:
    import capo_securityhub.types.parameter_value
    import capo_securityhub.types.parameter_value_type


class ParameterConfiguration(TypedDict, closed=True):
    value_type: NotRequired[
        "capo_securityhub.types.parameter_value_type.ParameterValueType"
    ]
    ...
    value: NotRequired["capo_securityhub.types.parameter_value.ParameterValue"]
...
def serialize_json(value: ParameterConfiguration) -> dict:
    ...
    if "value_type" in value:
        ...
        out["ValueType"] = capo_securityhub.types.parameter_value_type.serialize_json(
            value["value_type"]
        )
    if "value" in value:
        import capo_securityhub.types.parameter_value

        out["Value"] = capo_securityhub.types.parameter_value.serialize_json(
            value["value"]
        )
    return out
...
def deserialize_json(data: dict) -> ParameterConfiguration:
    ...
    if "ValueType" in data:
        ...
        out["value_type"] = (
            capo_securityhub.types.parameter_value_type.deserialize_json(
                data["ValueType"]
            )
        )
    if "Value" in data:
        ...
        out["value"] = capo_securityhub.types.parameter_value.deserialize_json(
            data["Value"]
        )
    return out
