...
class AwsStepFunctionStateMachineLoggingConfigurationDetails(TypedDict, closed=True):
    destinations: NotRequired[
        "capo_securityhub.types.aws_step_function_state_machine_logging_configuration_destinations_list.AwsStepFunctionStateMachineLoggingConfigurationDestinationsList"
    ]
    """<p> An array of objects that describes where your execution history events will be logged. </p>"""
    include_execution_data: NotRequired["capo_securityhub.types.boolean.Boolean"]
    """<p> Determines whether execution data is included in your log. When set to false, data is excluded. </p>"""
    level: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
...
def serialize_json(
    ...
    if "destinations" in value:
        ...
        out["Destinations"] = (
            capo_securityhub.types.aws_step_function_state_machine_logging_configuration_destinations_list.serialize_json(
                value["destinations"]
            )
        )
...
def deserialize_json(
    ...
    if "Destinations" in data:
        ...
        out["destinations"] = (
            capo_securityhub.types.aws_step_function_state_machine_logging_configuration_destinations_list.deserialize_json(
                data["Destinations"]
            )
        )
