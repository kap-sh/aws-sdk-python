...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_waf_regional_rule_group_rules_list
    import capo_securityhub.types.non_empty_string


class AwsWafRegionalRuleGroupDetails(TypedDict, closed=True):
    ...
    rules: NotRequired[
        "capo_securityhub.types.aws_waf_regional_rule_group_rules_list.AwsWafRegionalRuleGroupRulesList"
    ]
...
def serialize_json(value: AwsWafRegionalRuleGroupDetails) -> dict:
    ...
    if "rules" in value:
        ...
        out["Rules"] = (
            capo_securityhub.types.aws_waf_regional_rule_group_rules_list.serialize_json(
                value["rules"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AwsWafRegionalRuleGroupDetails:
    ...
    if "Rules" in data:
        ...
        out["rules"] = (
            capo_securityhub.types.aws_waf_regional_rule_group_rules_list.deserialize_json(
                data["Rules"]
            )
        )
    return out
