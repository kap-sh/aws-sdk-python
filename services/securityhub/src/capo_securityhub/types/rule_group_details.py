...
if TYPE_CHECKING:
    import capo_securityhub.types.rule_group_source
    import capo_securityhub.types.rule_group_variables


class RuleGroupDetails(TypedDict, closed=True):
    rule_variables: NotRequired[
        "capo_securityhub.types.rule_group_variables.RuleGroupVariables"
    ]
    """<p>Additional settings to use in the specified rules.</p>"""
    rules_source: NotRequired[
        "capo_securityhub.types.rule_group_source.RuleGroupSource"
    ]
...
def serialize_json(value: RuleGroupDetails) -> dict:
    ...
    if "rule_variables" in value:
        ...
        out["RuleVariables"] = (
            capo_securityhub.types.rule_group_variables.serialize_json(
    ...
    if "rules_source" in value:
        ...
        out["RulesSource"] = capo_securityhub.types.rule_group_source.serialize_json(
            value["rules_source"]
        )
    return out
...
def deserialize_json(data: dict) -> RuleGroupDetails:
    ...
    if "RuleVariables" in data:
        ...
        out["rule_variables"] = (
            capo_securityhub.types.rule_group_variables.deserialize_json(
                data["RuleVariables"]
            )
        )
    if "RulesSource" in data:
        ...
        out["rules_source"] = capo_securityhub.types.rule_group_source.deserialize_json(
            data["RulesSource"]
        )
    return out
