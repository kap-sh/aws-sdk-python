...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_security_finding_list
    import capo_securityhub.types.next_token


class GetFindingsResponse(TypedDict, closed=True):
    findings: NotRequired[
        "capo_securityhub.types.aws_security_finding_list.AwsSecurityFindingList"
    ]
...
def serialize_json(value: GetFindingsResponse) -> dict:
    ...
    if "findings" in value:
        ...
        out["Findings"] = (
            capo_securityhub.types.aws_security_finding_list.serialize_json(
                value["findings"]
            )
        )
...
def deserialize_json(data: dict) -> GetFindingsResponse:
    ...
    if "Findings" in data:
        ...
        out["findings"] = (
            capo_securityhub.types.aws_security_finding_list.deserialize_json(
                data["Findings"]
            )
        )
