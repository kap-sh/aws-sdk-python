...
class AwsS3AccountPublicAccessBlockDetails(TypedDict, closed=True):
    block_public_acls: NotRequired["capo_securityhub.types.boolean.Boolean"]
    """<p>Indicates whether to reject calls to update an S3 bucket if the calls include a public access control list (ACL).</p>"""
    block_public_policy: NotRequired["capo_securityhub.types.boolean.Boolean"]
    ...
    ignore_public_acls: NotRequired["capo_securityhub.types.boolean.Boolean"]
    """<p>Indicates whether Amazon S3 ignores public ACLs that are associated with an S3 bucket.</p>"""
    restrict_public_buckets: NotRequired["capo_securityhub.types.boolean.Boolean"]
