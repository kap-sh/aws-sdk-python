...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_backup_backup_plan_backup_plan_details
    import capo_securityhub.types.non_empty_string


class AwsBackupBackupPlanDetails(TypedDict, closed=True):
    backup_plan: NotRequired[
        "capo_securityhub.types.aws_backup_backup_plan_backup_plan_details.AwsBackupBackupPlanBackupPlanDetails"
    ]
...
def serialize_json(value: AwsBackupBackupPlanDetails) -> dict:
    ...
    if "backup_plan" in value:
        ...
        out["BackupPlan"] = (
            capo_securityhub.types.aws_backup_backup_plan_backup_plan_details.serialize_json(
                value["backup_plan"]
            )
        )
...
def deserialize_json(data: dict) -> AwsBackupBackupPlanDetails:
    ...
    if "BackupPlan" in data:
        ...
        out["backup_plan"] = (
            capo_securityhub.types.aws_backup_backup_plan_backup_plan_details.deserialize_json(
                data["BackupPlan"]
            )
        )
