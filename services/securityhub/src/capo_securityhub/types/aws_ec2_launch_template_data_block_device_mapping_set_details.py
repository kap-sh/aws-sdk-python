...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_ec2_launch_template_data_block_device_mapping_set_ebs_details
...
class AwsEc2LaunchTemplateDataBlockDeviceMappingSetDetails(TypedDict, closed=True):
    ...
    """<p> The device name. </p>"""
    ebs: NotRequired[
        "capo_securityhub.types.aws_ec2_launch_template_data_block_device_mapping_set_ebs_details.AwsEc2LaunchTemplateDataBlockDeviceMappingSetEbsDetails"
    ]
    """<p> Parameters used to automatically set up Amazon EBS volumes when the instance is launched. </p>"""
    no_device: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
...
def serialize_json(value: AwsEc2LaunchTemplateDataBlockDeviceMappingSetDetails) -> dict:
    ...
    if "ebs" in value:
        ...
        out["Ebs"] = (
            capo_securityhub.types.aws_ec2_launch_template_data_block_device_mapping_set_ebs_details.serialize_json(
                value["ebs"]
            )
        )
...
def deserialize_json(
    ...
    if "Ebs" in data:
        ...
        out["ebs"] = (
            capo_securityhub.types.aws_ec2_launch_template_data_block_device_mapping_set_ebs_details.deserialize_json(
                data["Ebs"]
            )
        )
    if "NoDevice" in data:
