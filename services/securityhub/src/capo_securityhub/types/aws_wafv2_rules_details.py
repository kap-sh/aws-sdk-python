...
class AwsWafv2RulesDetails(TypedDict, closed=True):
    action: NotRequired[
        "capo_securityhub.types.aws_wafv2_rules_action_details.AwsWafv2RulesActionDetails"
    ]
...
def serialize_json(value: AwsWafv2RulesDetails) -> dict:
    ...
    if "action" in value:
        ...
        out["Action"] = (
            capo_securityhub.types.aws_wafv2_rules_action_details.serialize_json(
                value["action"]
            )
        )
    if "name" in value:
...
def deserialize_json(data: dict) -> AwsWafv2RulesDetails:
    ...
    if "Action" in data:
        ...
        out["action"] = (
            capo_securityhub.types.aws_wafv2_rules_action_details.deserialize_json(
                data["Action"]
            )
        )
    if "Name" in data:
