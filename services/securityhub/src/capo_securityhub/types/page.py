...
class Page(TypedDict, closed=True):
    page_number: NotRequired["capo_securityhub.types.long.Long"]
    """<p>The page number of the page that contains the sensitive data.</p>"""
    line_range: NotRequired["capo_securityhub.types.range.Range"]
    ...
    offset_range: NotRequired["capo_securityhub.types.range.Range"]
...
def serialize_json(value: Page) -> dict:
    ...
    if "line_range" in value:
        ...
        out["LineRange"] = capo_securityhub.types.range.serialize_json(
            value["line_range"]
        )
    if "offset_range" in value:
        import capo_securityhub.types.range

        out["OffsetRange"] = capo_securityhub.types.range.serialize_json(
            value["offset_range"]
        )
    return out
