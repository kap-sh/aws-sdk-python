"""Generated from Smithy shape ``com.amazonaws.securityhub#AwsWafRateBasedRuleMatchPredicate``."""
...
class AwsWafRateBasedRuleMatchPredicate(TypedDict, closed=True):
    data_id: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p>The unique identifier for the predicate.</p>"""
    negated: NotRequired["capo_securityhub.types.boolean.Boolean"]
...
# --- restJson1 ser/de ---
def serialize_json(value: AwsWafRateBasedRuleMatchPredicate) -> dict:
    out: dict = {}
    if "data_id" in value:
...
def deserialize_json(data: dict) -> AwsWafRateBasedRuleMatchPredicate:
    out: AwsWafRateBasedRuleMatchPredicate = {}  # type: ignore[typeddict-item]
    if "DataId" in data:
