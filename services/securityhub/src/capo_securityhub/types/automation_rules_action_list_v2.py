...
if TYPE_CHECKING:
    import capo_securityhub.types.automation_rules_action_v2

AutomationRulesActionListV2: TypeAlias = list[
    "capo_securityhub.types.automation_rules_action_v2.AutomationRulesActionV2"
]
...
def serialize_json(value: AutomationRulesActionListV2) -> list:
    ...
    for item in value:
        out.append(
            capo_securityhub.types.automation_rules_action_v2.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AutomationRulesActionListV2:
    ...
    for item in data:
        out.append(
            capo_securityhub.types.automation_rules_action_v2.deserialize_json(item)
        )
    return out
