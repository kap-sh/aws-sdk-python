...
class BatchUpdateFindingsV2UnprocessedFinding(TypedDict, closed=True):
    ...
    error_code: NotRequired[
        "capo_securityhub.types.batch_update_findings_v2_unprocessed_finding_error_code.BatchUpdateFindingsV2UnprocessedFindingErrorCode"
    ]
...
def serialize_json(value: BatchUpdateFindingsV2UnprocessedFinding) -> dict:
    ...
    if "error_code" in value:
        ...
        out["ErrorCode"] = (
            capo_securityhub.types.batch_update_findings_v2_unprocessed_finding_error_code.serialize_json(
                value["error_code"]
            )
        )
...
def deserialize_json(data: dict) -> BatchUpdateFindingsV2UnprocessedFinding:
    ...
    if "ErrorCode" in data:
        ...
        out["error_code"] = (
            capo_securityhub.types.batch_update_findings_v2_unprocessed_finding_error_code.deserialize_json(
                data["ErrorCode"]
            )
        )
