...
class NetworkPathComponent(TypedDict, closed=True):
    component_id: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    ...
    egress: NotRequired["capo_securityhub.types.network_header.NetworkHeader"]
    """<p>Information about the component that comes after the current component in the network path.</p>"""
    ingress: NotRequired["capo_securityhub.types.network_header.NetworkHeader"]
...
def serialize_json(value: NetworkPathComponent) -> dict:
    ...
    if "egress" in value:
        ...
        out["Egress"] = capo_securityhub.types.network_header.serialize_json(
            value["egress"]
        )
    if "ingress" in value:
        import capo_securityhub.types.network_header

        out["Ingress"] = capo_securityhub.types.network_header.serialize_json(
            value["ingress"]
        )
    return out
