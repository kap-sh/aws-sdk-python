...
class AwsEc2NetworkAclEntry(TypedDict, closed=True):
    ...
    egress: NotRequired["capo_securityhub.types.boolean.Boolean"]
    """<p>Whether the rule is an egress rule. An egress rule is a rule that applies to traffic that leaves the subnet.</p>"""
    icmp_type_code: NotRequired["capo_securityhub.types.icmp_type_code.IcmpTypeCode"]
    ...
    port_range: NotRequired["capo_securityhub.types.port_range_from_to.PortRangeFromTo"]
    ...
    rule_action: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
    """<p>Whether the rule is used to allow access or deny access.</p>"""
    rule_number: NotRequired["capo_securityhub.types.integer.Integer"]
...
def serialize_json(value: AwsEc2NetworkAclEntry) -> dict:
    ...
    if "icmp_type_code" in value:
        ...
        out["IcmpTypeCode"] = capo_securityhub.types.icmp_type_code.serialize_json(
    ...
    if "port_range" in value:
        ...
        out["PortRange"] = capo_securityhub.types.port_range_from_to.serialize_json(
            value["port_range"]
        )
