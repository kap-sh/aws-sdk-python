...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_amazon_mq_broker_users_details

AwsAmazonMqBrokerUsersList: TypeAlias = list[
    "capo_securityhub.types.aws_amazon_mq_broker_users_details.AwsAmazonMqBrokerUsersDetails"
]
...
def serialize_json(value: AwsAmazonMqBrokerUsersList) -> list:
    ...
    for item in value:
        out.append(
            capo_securityhub.types.aws_amazon_mq_broker_users_details.serialize_json(
                item
            )
        )
    return out
