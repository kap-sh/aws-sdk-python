"""Generated from Smithy shape ``com.amazonaws.securityhub#GetFindingsTrendsV2Request``."""
...
# --- restJson1 ser/de ---
def serialize_json(value: GetFindingsTrendsV2Request) -> dict:
    out: dict = {}
    if "filters" in value:
        import capo_securityhub.types.findings_trends_filters

        out["Filters"] = capo_securityhub.types.findings_trends_filters.serialize_json(
            value["filters"]
        )
    if "start_time" in value:
        import capo_securityhub.types.timestamp

        out["StartTime"] = capo_securityhub.types.timestamp.serialize_json(
            value["start_time"]
        )
    if "end_time" in value:
        import capo_securityhub.types.timestamp

        out["EndTime"] = capo_securityhub.types.timestamp.serialize_json(
            value["end_time"]
        )
...
def deserialize_json(data: dict) -> GetFindingsTrendsV2Request:
    ...
    if "Filters" in data:
        ...
        out["filters"] = (
            capo_securityhub.types.findings_trends_filters.deserialize_json(
                data["Filters"]
            )
        )
