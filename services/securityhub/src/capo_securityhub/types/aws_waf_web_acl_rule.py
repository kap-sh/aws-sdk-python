...
class AwsWafWebAclRule(TypedDict, closed=True):
    action: NotRequired["capo_securityhub.types.waf_action.WafAction"]
    ...
    excluded_rules: NotRequired[
        "capo_securityhub.types.waf_excluded_rule_list.WafExcludedRuleList"
    ]
    """<p>Rules to exclude from a rule group.</p>"""
    override_action: NotRequired[
        "capo_securityhub.types.waf_override_action.WafOverrideAction"
    ]
...
def serialize_json(value: AwsWafWebAclRule) -> dict:
    ...
    if "action" in value:
        ...
        out["Action"] = capo_securityhub.types.waf_action.serialize_json(
            value["action"]
        )
    if "excluded_rules" in value:
        ...
        out["ExcludedRules"] = (
            capo_securityhub.types.waf_excluded_rule_list.serialize_json(
    ...
    if "override_action" in value:
        ...
        out["OverrideAction"] = (
            capo_securityhub.types.waf_override_action.serialize_json(
...
def deserialize_json(data: dict) -> AwsWafWebAclRule:
    ...
    if "ExcludedRules" in data:
        ...
        out["excluded_rules"] = (
            capo_securityhub.types.waf_excluded_rule_list.deserialize_json(
                data["ExcludedRules"]
            )
        )
    if "OverrideAction" in data:
        ...
        out["override_action"] = (
            capo_securityhub.types.waf_override_action.deserialize_json(
                data["OverrideAction"]
            )
        )
    if "Priority" in data:
