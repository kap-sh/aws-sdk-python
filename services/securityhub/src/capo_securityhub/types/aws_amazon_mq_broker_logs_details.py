...
class AwsAmazonMqBrokerLogsDetails(TypedDict, closed=True):
    ...
    pending: NotRequired[
        "capo_securityhub.types.aws_amazon_mq_broker_logs_pending_details.AwsAmazonMqBrokerLogsPendingDetails"
    ]
...
def serialize_json(value: AwsAmazonMqBrokerLogsDetails) -> dict:
    ...
    if "pending" in value:
        ...
        out["Pending"] = (
            capo_securityhub.types.aws_amazon_mq_broker_logs_pending_details.serialize_json(
                value["pending"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AwsAmazonMqBrokerLogsDetails:
    ...
    if "Pending" in data:
        ...
        out["pending"] = (
            capo_securityhub.types.aws_amazon_mq_broker_logs_pending_details.deserialize_json(
                data["Pending"]
            )
        )
    return out
