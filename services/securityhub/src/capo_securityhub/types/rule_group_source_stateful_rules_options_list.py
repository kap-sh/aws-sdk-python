...
if TYPE_CHECKING:
    import capo_securityhub.types.rule_group_source_stateful_rules_options_details

RuleGroupSourceStatefulRulesOptionsList: TypeAlias = list[
    "capo_securityhub.types.rule_group_source_stateful_rules_options_details.RuleGroupSourceStatefulRulesOptionsDetails"
]
...
def serialize_json(value: RuleGroupSourceStatefulRulesOptionsList) -> list:
    ...
    for item in value:
        out.append(
            capo_securityhub.types.rule_group_source_stateful_rules_options_details.serialize_json(
                item
            )
        )
    return out
