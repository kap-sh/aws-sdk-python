...
if TYPE_CHECKING:
    import capo_securityhub.types.aws_s3_bucket_notification_configuration_s3_key_filter_rule_name
    import capo_securityhub.types.non_empty_string


class AwsS3BucketNotificationConfigurationS3KeyFilterRule(TypedDict, closed=True):
    name: NotRequired[
        "capo_securityhub.types.aws_s3_bucket_notification_configuration_s3_key_filter_rule_name.AwsS3BucketNotificationConfigurationS3KeyFilterRuleName"
    ]
...
def serialize_json(value: AwsS3BucketNotificationConfigurationS3KeyFilterRule) -> dict:
    ...
    if "name" in value:
        ...
        out["Name"] = (
            capo_securityhub.types.aws_s3_bucket_notification_configuration_s3_key_filter_rule_name.serialize_json(
                value["name"]
            )
        )
    if "value" in value:
...
def deserialize_json(data: dict) -> AwsS3BucketNotificationConfigurationS3KeyFilterRule:
    ...
    if "Name" in data:
        ...
        out["name"] = (
            capo_securityhub.types.aws_s3_bucket_notification_configuration_s3_key_filter_rule_name.deserialize_json(
                data["Name"]
            )
        )
    if "Value" in data:
