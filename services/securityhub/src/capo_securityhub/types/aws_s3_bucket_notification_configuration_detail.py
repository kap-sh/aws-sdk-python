...
class AwsS3BucketNotificationConfigurationDetail(TypedDict, closed=True):
    events: NotRequired[
        "capo_securityhub.types.aws_s3_bucket_notification_configuration_events.AwsS3BucketNotificationConfigurationEvents"
    ]
    """<p>The list of events that trigger a notification.</p>"""
    filter: NotRequired[
        "capo_securityhub.types.aws_s3_bucket_notification_configuration_filter.AwsS3BucketNotificationConfigurationFilter"
    ]
    """<p>The filters that determine which S3 buckets generate notifications.</p>"""
    destination: NotRequired["capo_securityhub.types.non_empty_string.NonEmptyString"]
...
def serialize_json(value: AwsS3BucketNotificationConfigurationDetail) -> dict:
    ...
    if "events" in value:
        ...
        out["Events"] = (
            capo_securityhub.types.aws_s3_bucket_notification_configuration_events.serialize_json(
                value["events"]
            )
        )
    if "filter" in value:
        ...
        out["Filter"] = (
            capo_securityhub.types.aws_s3_bucket_notification_configuration_filter.serialize_json(
                value["filter"]
            )
        )
...
def deserialize_json(data: dict) -> AwsS3BucketNotificationConfigurationDetail:
    ...
    if "Events" in data:
        ...
        out["events"] = (
            capo_securityhub.types.aws_s3_bucket_notification_configuration_events.deserialize_json(
                data["Events"]
            )
        )
    if "Filter" in data:
        ...
        out["filter"] = (
            capo_securityhub.types.aws_s3_bucket_notification_configuration_filter.deserialize_json(
                data["Filter"]
            )
        )
