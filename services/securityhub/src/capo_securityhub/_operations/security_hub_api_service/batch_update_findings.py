"""Generated from Smithy shape ``com.amazonaws.securityhub#BatchUpdateFindings``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.batch_update_findings_response.BatchUpdateFindingsResponse = capo_securityhub.types.batch_update_findings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findings/batchupdate"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.batch_update_findings_request.serialize_json(input_)
    ).encode()
...
async def async_batch_update_findings(
