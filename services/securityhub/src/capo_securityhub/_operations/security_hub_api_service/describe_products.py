"""Generated from Smithy shape ``com.amazonaws.securityhub#DescribeProducts``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.describe_products_response.DescribeProductsResponse = (
        capo_securityhub.types.describe_products_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/products"
    params: dict[str, str] = {}
    if "next_token" in input_:
        params["NextToken"] = str(input_["next_token"])
    if "max_results" in input_:
        params["MaxResults"] = str(input_["max_results"])
    if "product_arn" in input_:
        params["ProductArn"] = str(input_["product_arn"])
...
async def async_describe_products(
