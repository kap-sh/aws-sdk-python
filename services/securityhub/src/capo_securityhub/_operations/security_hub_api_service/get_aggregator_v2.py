"""Generated from Smithy shape ``com.amazonaws.securityhub#GetAggregatorV2``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.get_aggregator_v2_response.GetAggregatorV2Response = (
        capo_securityhub.types.get_aggregator_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/aggregatorv2/get/{AggregatorV2Arn+}"
    url = url.replace(
        "{AggregatorV2Arn+}", quote(str(input_["aggregator_v2_arn"]), safe="/")
    )
    params: dict[str, str] = {}
...
async def async_get_aggregator_v2(
