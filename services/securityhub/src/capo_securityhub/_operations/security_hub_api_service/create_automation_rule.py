"""Generated from Smithy shape ``com.amazonaws.securityhub#CreateAutomationRule``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.create_automation_rule_response.CreateAutomationRuleResponse = capo_securityhub.types.create_automation_rule_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/automationrules/create"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.create_automation_rule_request.serialize_json(input_)
    ).encode()
