"""Generated from Smithy shape ``com.amazonaws.securityhub#DisassociateMembers``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.disassociate_members_response.DisassociateMembersResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/members/disassociate"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.disassociate_members_request.serialize_json(input_)
    ).encode()
