"""Generated from Smithy shape ``com.amazonaws.securityhub#DeleteInvitations``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.delete_invitations_response.DeleteInvitationsResponse = capo_securityhub.types.delete_invitations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/invitations/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.delete_invitations_request.serialize_json(input_)
    ).encode()
...
async def async_delete_invitations(
