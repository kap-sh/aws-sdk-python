"""Generated from Smithy shape ``com.amazonaws.securityhub#BatchUpdateFindingsV2``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.batch_update_findings_v2_response.BatchUpdateFindingsV2Response = capo_securityhub.types.batch_update_findings_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findingsv2/batchupdatev2"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.batch_update_findings_v2_request.serialize_json(input_)
    ).encode()
...
async def async_batch_update_findings_v2(
