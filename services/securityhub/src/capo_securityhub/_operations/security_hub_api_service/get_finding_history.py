"""Generated from Smithy shape ``com.amazonaws.securityhub#GetFindingHistory``."""
...
def handle_response(
    ...
    out: capo_securityhub.types.get_finding_history_response.GetFindingHistoryResponse = capo_securityhub.types.get_finding_history_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findingHistory/get"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securityhub.types.get_finding_history_request.serialize_json(input_)
    ).encode()
...
async def async_get_finding_history(
