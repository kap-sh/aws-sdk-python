...
if TYPE_CHECKING:
    import aws_sdk_bedrock.types.evaluation_dataset
    import aws_sdk_bedrock.types.evaluation_metric_names
...
class EvaluationDatasetMetricConfig(TypedDict):
    ...
    metric_names: "aws_sdk_bedrock.types.evaluation_metric_names.EvaluationMetricNames"
...
def serialize_json(value: EvaluationDatasetMetricConfig) -> dict:
    ...
    out["taskType"] = aws_sdk_bedrock.types.evaluation_task_type.serialize_json(
        value["task_type"]
    )
    import aws_sdk_bedrock.types.evaluation_dataset

    out["dataset"] = aws_sdk_bedrock.types.evaluation_dataset.serialize_json(
        value["dataset"]
    )
    import aws_sdk_bedrock.types.evaluation_metric_names

    out["metricNames"] = aws_sdk_bedrock.types.evaluation_metric_names.serialize_json(
        value["metric_names"]
    )
    return out
...
def deserialize_json(data: dict) -> EvaluationDatasetMetricConfig:
    ...
    if "taskType" in data:
        ...
        out["task_type"] = aws_sdk_bedrock.types.evaluation_task_type.deserialize_json(
            data["taskType"]
        )
    else:
    ...
    if "dataset" in data:
        ...
        out["dataset"] = aws_sdk_bedrock.types.evaluation_dataset.deserialize_json(
            data["dataset"]
        )
    else:
    ...
    if "metricNames" in data:
        ...
        out["metric_names"] = (
            aws_sdk_bedrock.types.evaluation_metric_names.deserialize_json(
                data["metricNames"]
            )
        )
    else:
