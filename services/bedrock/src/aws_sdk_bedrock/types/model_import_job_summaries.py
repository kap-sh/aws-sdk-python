...
ModelImportJobSummaries: TypeAlias = list[
    "aws_sdk_bedrock.types.model_import_job_summary.ModelImportJobSummary"
]
...
def serialize_json(value: ModelImportJobSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_bedrock.types.model_import_job_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ModelImportJobSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock.types.model_import_job_summary.deserialize_json(item)
        )
    return out
