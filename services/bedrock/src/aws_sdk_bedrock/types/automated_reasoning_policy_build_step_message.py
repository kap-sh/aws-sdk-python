...
if TYPE_CHECKING:
    import aws_sdk_bedrock.types.automated_reasoning_policy_build_message_type


class AutomatedReasoningPolicyBuildStepMessage(TypedDict):
    message: "str"
    """<p>The content of the message, describing what occurred during the build step.</p>"""
    message_type: "aws_sdk_bedrock.types.automated_reasoning_policy_build_message_type.AutomatedReasoningPolicyBuildMessageType"
...
def serialize_json(value: AutomatedReasoningPolicyBuildStepMessage) -> dict:
    ...
    out["messageType"] = (
        aws_sdk_bedrock.types.automated_reasoning_policy_build_message_type.serialize_json(
            value["message_type"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> AutomatedReasoningPolicyBuildStepMessage:
    ...
    if "messageType" in data:
        ...
        out["message_type"] = (
            aws_sdk_bedrock.types.automated_reasoning_policy_build_message_type.deserialize_json(
                data["messageType"]
            )
        )
    else:
