...
if TYPE_CHECKING:
    import aws_sdk_bedrock.types.guardrail_managed_words_config

GuardrailManagedWordListsConfig: TypeAlias = list[
    "aws_sdk_bedrock.types.guardrail_managed_words_config.GuardrailManagedWordsConfig"
]
...
def serialize_json(value: GuardrailManagedWordListsConfig) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock.types.guardrail_managed_words_config.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> GuardrailManagedWordListsConfig:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock.types.guardrail_managed_words_config.deserialize_json(item)
        )
    return out
