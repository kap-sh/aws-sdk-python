...
if TYPE_CHECKING:
    import aws_sdk_bedrock.types.query_transformation_type


class QueryTransformationConfiguration(TypedDict, closed=True):
    type: "aws_sdk_bedrock.types.query_transformation_type.QueryTransformationType"
...
def serialize_json(value: QueryTransformationConfiguration) -> dict:
    ...
    out["type"] = aws_sdk_bedrock.types.query_transformation_type.serialize_json(
        value["type"]
    )
    return out
...
def deserialize_json(data: dict) -> QueryTransformationConfiguration:
    ...
    if "type" in data:
        ...
        out["type"] = aws_sdk_bedrock.types.query_transformation_type.deserialize_json(
            data["type"]
        )
    else:
