...
ImportedModelSummaryList: TypeAlias = list[
    "aws_sdk_bedrock.types.imported_model_summary.ImportedModelSummary"
]
...
def serialize_json(value: ImportedModelSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_bedrock.types.imported_model_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ImportedModelSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_bedrock.types.imported_model_summary.deserialize_json(item))
    return out
