...
if TYPE_CHECKING:
    import aws_sdk_bedrock.types.custom_metric_instructions
    import aws_sdk_bedrock.types.metric_name
...
class CustomMetricDefinition(TypedDict, closed=True):
    ...
    instructions: (
        "aws_sdk_bedrock.types.custom_metric_instructions.CustomMetricInstructions"
    )
...
def serialize_json(value: CustomMetricDefinition) -> dict:
    ...
    if "rating_scale" in value:
        ...
        out["ratingScale"] = aws_sdk_bedrock.types.rating_scale.serialize_json(
            value["rating_scale"]
        )
    return out
...
def deserialize_json(data: dict) -> CustomMetricDefinition:
    ...
    if "ratingScale" in data:
        ...
        out["rating_scale"] = aws_sdk_bedrock.types.rating_scale.deserialize_json(
            data["ratingScale"]
        )
    return out
