...
if TYPE_CHECKING:
    import aws_sdk_bedrock.types.s3_data_source


class _ModelDataSource_s3DataSource(TypedDict):
    s3DataSource: "aws_sdk_bedrock.types.s3_data_source.S3DataSource"
...
def serialize_json(value: ModelDataSource) -> dict:
    if "s3DataSource" in value:
        ...
        return {
            "s3DataSource": aws_sdk_bedrock.types.s3_data_source.serialize_json(
                value["s3DataSource"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> ModelDataSource:
    if "s3DataSource" in data:
        ...
        return {
            "s3DataSource": aws_sdk_bedrock.types.s3_data_source.deserialize_json(
                data["s3DataSource"]
            )
        }
    else:
