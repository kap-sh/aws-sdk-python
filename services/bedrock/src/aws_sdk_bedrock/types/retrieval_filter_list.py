...
RetrievalFilterList: TypeAlias = list[
    "aws_sdk_bedrock.types.retrieval_filter.RetrievalFilter"
]
...
def serialize_json(value: RetrievalFilterList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_bedrock.types.retrieval_filter.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> RetrievalFilterList:
    ...
    for item in data:
        out.append(aws_sdk_bedrock.types.retrieval_filter.deserialize_json(item))
    return out
