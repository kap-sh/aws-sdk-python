...
class CreateInferenceProfileRequest(TypedDict):
...
def serialize_json(value: CreateInferenceProfileRequest) -> dict:
    ...
    out["modelSource"] = (
        aws_sdk_bedrock.types.inference_profile_model_source.serialize_json(
            value["model_source"]
        )
    )
    if "tags" in value:
...
def deserialize_json(data: dict) -> CreateInferenceProfileRequest:
    ...
    if "modelSource" in data:
        ...
        out["model_source"] = (
            aws_sdk_bedrock.types.inference_profile_model_source.deserialize_json(
                data["modelSource"]
            )
        )
    else:
