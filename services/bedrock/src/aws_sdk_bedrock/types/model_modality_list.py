"""Generated from Smithy shape ``com.amazonaws.bedrock#ModelModalityList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_bedrock.types.model_modality

ModelModalityList: TypeAlias = list[
    "aws_sdk_bedrock.types.model_modality.ModelModality"
]


# --- restJson1 ser/de ---
def serialize_json(value: ModelModalityList) -> list:
    import aws_sdk_bedrock.types.model_modality

    out: list = []
    for item in value:
        out.append(aws_sdk_bedrock.types.model_modality.serialize_json(item))
    return out


def deserialize_json(data: list) -> ModelModalityList:
    import aws_sdk_bedrock.types.model_modality

    out: ModelModalityList = []
    for item in data:
        out.append(aws_sdk_bedrock.types.model_modality.deserialize_json(item))
    return out
