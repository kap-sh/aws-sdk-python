...
import capo_bedrock.errors.resource_in_use_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceInUseException":
            raise capo_bedrock.errors.resource_in_use_exception.ResourceInUseException.from_json(
                data, message
            )
...
def handle_response(
    ...
    out: capo_bedrock.types.start_automated_reasoning_policy_test_workflow_response.StartAutomatedReasoningPolicyTestWorkflowResponse = capo_bedrock.types.start_automated_reasoning_policy_test_workflow_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{buildWorkflowId}", quote(input_["build_workflow_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock.types.start_automated_reasoning_policy_test_workflow_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
