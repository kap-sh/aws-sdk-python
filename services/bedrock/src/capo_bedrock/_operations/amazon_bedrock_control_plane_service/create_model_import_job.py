"""Generated from Smithy shape ``com.amazonaws.bedrock#CreateModelImportJob``."""
...
def handle_response(
    ...
    out: capo_bedrock.types.create_model_import_job_response.CreateModelImportJobResponse = capo_bedrock.types.create_model_import_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/model-import-jobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock.types.create_model_import_job_request.serialize_json(input_)
    ).encode()
...
async def async_create_model_import_job(
