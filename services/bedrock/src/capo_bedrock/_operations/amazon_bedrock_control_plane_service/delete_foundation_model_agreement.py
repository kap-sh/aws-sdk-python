"""Generated from Smithy shape ``com.amazonaws.bedrock#DeleteFoundationModelAgreement``."""
...
def handle_response(
    ...
    out: capo_bedrock.types.delete_foundation_model_agreement_response.DeleteFoundationModelAgreementResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-foundation-model-agreement"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock.types.delete_foundation_model_agreement_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
