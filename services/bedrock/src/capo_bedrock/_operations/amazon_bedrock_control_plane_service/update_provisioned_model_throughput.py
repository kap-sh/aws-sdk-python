"""Generated from Smithy shape ``com.amazonaws.bedrock#UpdateProvisionedModelThroughput``."""
...
def handle_response(
    ...
    out: capo_bedrock.types.update_provisioned_model_throughput_response.UpdateProvisionedModelThroughputResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{provisionedModelId}", quote(input_["provisioned_model_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock.types.update_provisioned_model_throughput_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
