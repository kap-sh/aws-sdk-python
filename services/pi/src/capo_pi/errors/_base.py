...
class PIError(Exception):
    """Base class for all PI service errors."""


class ServiceError(PIError):
...
class SerializationError(PIError):
    """Raised when a value cannot be serialized."""


class DeserializationError(PIError):
...
class WaiterFailedError(PIError):
...
class WaiterTimeoutError(PIError):
