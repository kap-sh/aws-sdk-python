"""Generated from Smithy shape ``com.amazonaws.medicalimaging#StartDICOMImportJob``."""
...
def handle_response(
    ...
    out: aws_sdk_medical_imaging.types.start_dicom_import_job_response.StartDICOMImportJobResponse = aws_sdk_medical_imaging.types.start_dicom_import_job_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_medical_imaging.types.start_dicom_import_job_request.StartDICOMImportJobRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/startDICOMImportJob/datastore/{datastoreId}"
    url = url.replace("{datastoreId}", quote(str(input["datastore_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_medical_imaging.types.start_dicom_import_job_request

    body: bytes | None = json.dumps(
        aws_sdk_medical_imaging.types.start_dicom_import_job_request.serialize_json(
            input
        )
    ).encode()
...
async def async_start_dicom_import_job(
