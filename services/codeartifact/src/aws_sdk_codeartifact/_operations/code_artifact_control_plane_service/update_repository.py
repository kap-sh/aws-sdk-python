"""Generated from Smithy shape ``com.amazonaws.codeartifact#UpdateRepository``."""
...
def handle_response(
    ...
    out: aws_sdk_codeartifact.types.update_repository_result.UpdateRepositoryResult = (
        aws_sdk_codeartifact.types.update_repository_result.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_codeartifact.types.update_repository_request.UpdateRepositoryRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/repository"
    params: dict[str, str] = {}
    if "domain" in input_:
    ...
    body: bytes | None = json.dumps(
        aws_sdk_codeartifact.types.update_repository_request.serialize_json(input_)
    ).encode()
...
async def async_update_repository(
