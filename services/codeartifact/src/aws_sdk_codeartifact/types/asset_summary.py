...
if TYPE_CHECKING:
    import aws_sdk_codeartifact.types.asset_hashes
...
class AssetSummary(TypedDict, closed=True):
    ...
    hashes: NotRequired["aws_sdk_codeartifact.types.asset_hashes.AssetHashes"]
...
def serialize_json(value: AssetSummary) -> dict:
    ...
    if "hashes" in value:
        ...
        out["hashes"] = aws_sdk_codeartifact.types.asset_hashes.serialize_json(
            value["hashes"]
        )
    return out
...
def deserialize_json(data: dict) -> AssetSummary:
    ...
    if "hashes" in data:
        ...
        out["hashes"] = aws_sdk_codeartifact.types.asset_hashes.deserialize_json(
            data["hashes"]
        )
    return out
