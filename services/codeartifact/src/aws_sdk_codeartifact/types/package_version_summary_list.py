...
PackageVersionSummaryList: TypeAlias = list[
    "aws_sdk_codeartifact.types.package_version_summary.PackageVersionSummary"
]
...
def serialize_json(value: PackageVersionSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_codeartifact.types.package_version_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> PackageVersionSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_codeartifact.types.package_version_summary.deserialize_json(item)
        )
    return out
