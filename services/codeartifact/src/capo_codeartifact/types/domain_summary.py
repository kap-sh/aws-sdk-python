...
class DomainSummary(TypedDict, closed=True):
    name: NotRequired["capo_codeartifact.types.domain_name.DomainName"]
    """<p> The name of the domain. </p>"""
    owner: NotRequired["capo_codeartifact.types.account_id.AccountId"]
    """<p> The 12-digit account number of the Amazon Web Services account that owns the domain. It does not include dashes or spaces. </p>"""
    arn: NotRequired["capo_codeartifact.types.arn.Arn"]
    """<p> The ARN of the domain. </p>"""
    status: NotRequired["capo_codeartifact.types.domain_status.DomainStatus"]
...
def serialize_json(value: DomainSummary) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_codeartifact.types.domain_status.serialize_json(
            value["status"]
        )
