...
class RepositoryExternalConnectionInfo(TypedDict, closed=True):
    ...
    package_format: NotRequired["capo_codeartifact.types.package_format.PackageFormat"]
    ...
    status: NotRequired[
        "capo_codeartifact.types.external_connection_status.ExternalConnectionStatus"
    ]
...
def serialize_json(value: RepositoryExternalConnectionInfo) -> dict:
    ...
    if "package_format" in value:
        ...
        out["packageFormat"] = capo_codeartifact.types.package_format.serialize_json(
    ...
    if "status" in value:
        ...
        out["status"] = (
            capo_codeartifact.types.external_connection_status.serialize_json(
                value["status"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RepositoryExternalConnectionInfo:
    ...
    if "status" in data:
        ...
        out["status"] = (
            capo_codeartifact.types.external_connection_status.deserialize_json(
                data["status"]
            )
        )
    return out
