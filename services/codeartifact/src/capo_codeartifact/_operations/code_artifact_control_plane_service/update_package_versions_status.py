"""Generated from Smithy shape ``com.amazonaws.codeartifact#UpdatePackageVersionsStatus``."""
...
def handle_response(
    ...
    out: capo_codeartifact.types.update_package_versions_status_result.UpdatePackageVersionsStatusResult = capo_codeartifact.types.update_package_versions_status_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/package/versions/update_status"
    params: dict[str, str] = {}
    if "domain" in input_:
    ...
    body: bytes | None = json.dumps(
        capo_codeartifact.types.update_package_versions_status_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_package_versions_status(
