"""Generated from Smithy shape ``com.amazonaws.codeartifact#CopyPackageVersions``."""
...
def handle_response(
    ...
    out: capo_codeartifact.types.copy_package_versions_result.CopyPackageVersionsResult = capo_codeartifact.types.copy_package_versions_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/package/versions/copy"
    params: dict[str, str] = {}
    if "domain" in input_:
        params["domain"] = str(input_["domain"])
    if "domain_owner" in input_:
        params["domain-owner"] = str(input_["domain_owner"])
    if "source_repository" in input_:
        params["source-repository"] = str(input_["source_repository"])
    if "destination_repository" in input_:
        params["destination-repository"] = str(input_["destination_repository"])
    if "format" in input_:
    ...
    body: bytes | None = json.dumps(
        capo_codeartifact.types.copy_package_versions_request.serialize_json(input_)
    ).encode()
...
async def async_copy_package_versions(
