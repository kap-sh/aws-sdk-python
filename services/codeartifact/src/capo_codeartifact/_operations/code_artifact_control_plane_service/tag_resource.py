"""Generated from Smithy shape ``com.amazonaws.codeartifact#TagResource``."""
...
def handle_response(
    ...
    out: capo_codeartifact.types.tag_resource_result.TagResourceResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/tag"
    params: dict[str, str] = {}
    if "resource_arn" in input_:
        params["resourceArn"] = str(input_["resource_arn"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codeartifact.types.tag_resource_request.serialize_json(input_)
    ).encode()
...
def tag_resource(
    ...
    capo_codeartifact.types.tag_resource_result.TagResourceResult, zapros.Response
]:
