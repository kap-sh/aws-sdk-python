...
if TYPE_CHECKING:
    import capo_connecthealth.types.error_message
    import capo_connecthealth.types.post_stream_artifact_generation_status
    import capo_connecthealth.types.uri


class ArtifactDetails(TypedDict, closed=True):
    output_location: NotRequired["capo_connecthealth.types.uri.Uri"]
    """<p/>"""
    status: NotRequired[
        "capo_connecthealth.types.post_stream_artifact_generation_status.PostStreamArtifactGenerationStatus"
    ]
    """<p>The generation status of the artifact</p>"""
    failure_reason: NotRequired["capo_connecthealth.types.error_message.ErrorMessage"]
...
def serialize_json(value: ArtifactDetails) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = (
            capo_connecthealth.types.post_stream_artifact_generation_status.serialize_json(
                value["status"]
            )
        )
...
def deserialize_json(data: dict) -> ArtifactDetails:
    ...
    if "status" in data:
        ...
        out["status"] = (
            capo_connecthealth.types.post_stream_artifact_generation_status.deserialize_json(
                data["status"]
            )
        )
