...
if TYPE_CHECKING:
    import capo_connecthealth.types.medical_scribe_channel_definition

MedicalScribeChannelDefinitions: TypeAlias = list[
    "capo_connecthealth.types.medical_scribe_channel_definition.MedicalScribeChannelDefinition"
]
...
def serialize_json(value: MedicalScribeChannelDefinitions) -> list:
    ...
    for item in value:
        out.append(
            capo_connecthealth.types.medical_scribe_channel_definition.serialize_json(
                item
            )
        )
    return out
