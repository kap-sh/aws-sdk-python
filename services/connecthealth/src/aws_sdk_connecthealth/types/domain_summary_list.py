...
DomainSummaryList: TypeAlias = list[
    "aws_sdk_connecthealth.types.domain_summary.DomainSummary"
]
...
def serialize_json(value: DomainSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_connecthealth.types.domain_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> DomainSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_connecthealth.types.domain_summary.deserialize_json(item))
    return out
