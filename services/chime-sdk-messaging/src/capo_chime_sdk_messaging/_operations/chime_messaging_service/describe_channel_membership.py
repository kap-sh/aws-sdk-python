"""Generated from Smithy shape ``com.amazonaws.chimesdkmessaging#DescribeChannelMembership``."""
...
def handle_response(
    ...
    out: capo_chime_sdk_messaging.types.describe_channel_membership_response.DescribeChannelMembershipResponse = capo_chime_sdk_messaging.types.describe_channel_membership_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/channels/{ChannelArn}/memberships/{MemberArn}"
    url = url.replace("{ChannelArn}", quote(str(input_["channel_arn"]), safe=""))
    url = url.replace("{MemberArn}", quote(str(input_["member_arn"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_channel_membership(
