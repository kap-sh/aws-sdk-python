...
if TYPE_CHECKING:
    import aws_sdk_chime_sdk_messaging.types.lambda_configuration
...
def serialize_json(value: ProcessorConfiguration) -> dict:
    ...
    out["Lambda"] = (
        aws_sdk_chime_sdk_messaging.types.lambda_configuration.serialize_json(
            value["lambda"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ProcessorConfiguration:
    ...
    if "Lambda" in data:
        ...
        out["lambda"] = (
            aws_sdk_chime_sdk_messaging.types.lambda_configuration.deserialize_json(
                data["Lambda"]
            )
        )
    else:
