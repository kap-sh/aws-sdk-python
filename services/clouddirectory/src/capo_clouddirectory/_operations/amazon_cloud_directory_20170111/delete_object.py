"""Generated from Smithy shape ``com.amazonaws.clouddirectory#DeleteObject``."""
...
import capo_clouddirectory.errors.object_not_detached_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ObjectNotDetachedException":
            raise capo_clouddirectory.errors.object_not_detached_exception.ObjectNotDetachedException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.delete_object_response.DeleteObjectResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/object/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["directory_arn"])
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.delete_object_request.serialize_json(input_)
    ).encode()
