"""Generated from Smithy shape ``com.amazonaws.clouddirectory#AttachToIndex``."""
...
import capo_clouddirectory.errors.indexed_attribute_missing_exception
...
import capo_clouddirectory.errors.link_name_already_in_use_exception
import capo_clouddirectory.errors.not_index_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "IndexedAttributeMissingException":
            raise capo_clouddirectory.errors.indexed_attribute_missing_exception.IndexedAttributeMissingException.from_json(
                data
            )
        ...
        case "LinkNameAlreadyInUseException":
            raise capo_clouddirectory.errors.link_name_already_in_use_exception.LinkNameAlreadyInUseException.from_json(
                data
            )
        case "NotIndexException":
            raise capo_clouddirectory.errors.not_index_exception.NotIndexException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.attach_to_index_response.AttachToIndexResponse = (
        capo_clouddirectory.types.attach_to_index_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/index/attach"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["directory_arn"])
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.attach_to_index_request.serialize_json(input_)
    ).encode()
...
async def async_attach_to_index(
