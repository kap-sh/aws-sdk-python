"""Generated from Smithy shape ``com.amazonaws.clouddirectory#DetachObject``."""
...
import capo_clouddirectory.errors.not_node_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "NotNodeException":
            raise capo_clouddirectory.errors.not_node_exception.NotNodeException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.detach_object_response.DetachObjectResponse = (
        capo_clouddirectory.types.detach_object_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/object/detach"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["directory_arn"])
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.detach_object_request.serialize_json(input_)
    ).encode()
