"""Generated from Smithy shape ``com.amazonaws.clouddirectory#DeleteFacet``."""
...
import capo_clouddirectory.errors.facet_in_use_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "FacetInUseException":
            raise capo_clouddirectory.errors.facet_in_use_exception.FacetInUseException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_clouddirectory.types.delete_facet_response.DeleteFacetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/facet/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "schema_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["schema_arn"])
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.delete_facet_request.serialize_json(input_)
    ).encode()
...
def delete_facet(
    ...
    capo_clouddirectory.types.delete_facet_response.DeleteFacetResponse, zapros.Response
]:
