"""Generated from Smithy shape ``com.amazonaws.clouddirectory#UpdateObjectAttributes``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.update_object_attributes_response.UpdateObjectAttributesResponse = capo_clouddirectory.types.update_object_attributes_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/object/update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["directory_arn"])
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.update_object_attributes_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_object_attributes(
