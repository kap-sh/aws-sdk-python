"""Generated from Smithy shape ``com.amazonaws.clouddirectory#DetachPolicy``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.detach_policy_response.DetachPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/policy/detach"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "directory_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["directory_arn"])
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.detach_policy_request.serialize_json(input_)
    ).encode()
