"""Generated from Smithy shape ``com.amazonaws.clouddirectory#ListPublishedSchemaArns``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.list_published_schema_arns_response.ListPublishedSchemaArnsResponse = capo_clouddirectory.types.list_published_schema_arns_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/schema/published"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.list_published_schema_arns_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_published_schema_arns(
