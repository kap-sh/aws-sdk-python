"""Generated from Smithy shape ``com.amazonaws.clouddirectory#ListAppliedSchemaArns``."""
...
def handle_response(
    ...
    out: capo_clouddirectory.types.list_applied_schema_arns_response.ListAppliedSchemaArnsResponse = capo_clouddirectory.types.list_applied_schema_arns_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/schema/applied"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_clouddirectory.types.list_applied_schema_arns_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_applied_schema_arns(
