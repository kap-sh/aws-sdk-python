...
class PolicyToPath(TypedDict, closed=True):
    ...
    policies: NotRequired[
        "capo_clouddirectory.types.policy_attachment_list.PolicyAttachmentList"
    ]
...
def serialize_json(value: PolicyToPath) -> dict:
    ...
    if "policies" in value:
        ...
        out["Policies"] = (
            capo_clouddirectory.types.policy_attachment_list.serialize_json(
                value["policies"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> PolicyToPath:
    ...
    if "Policies" in data:
        ...
        out["policies"] = (
            capo_clouddirectory.types.policy_attachment_list.deserialize_json(
                data["Policies"]
            )
        )
    return out
