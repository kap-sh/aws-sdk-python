"""Generated from Smithy shape ``com.amazonaws.clouddirectory#CreateFacet``."""
...
import aws_sdk_clouddirectory.errors.facet_already_exists_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "FacetAlreadyExistsException":
            raise aws_sdk_clouddirectory.errors.facet_already_exists_exception.FacetAlreadyExistsException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_clouddirectory.types.create_facet_response.CreateFacetResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_clouddirectory.types.create_facet_request.CreateFacetRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/facet/create"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "schema_arn" in input_:
        headers["x-amz-data-partition"] = str(input_["schema_arn"])
    body: bytes | None = json.dumps(
        aws_sdk_clouddirectory.types.create_facet_request.serialize_json(input_)
    ).encode()
