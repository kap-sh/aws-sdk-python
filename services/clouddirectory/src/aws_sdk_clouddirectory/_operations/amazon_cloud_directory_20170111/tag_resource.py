"""Generated from Smithy shape ``com.amazonaws.clouddirectory#TagResource``."""
...
def handle_response(
    ...
    out: aws_sdk_clouddirectory.types.tag_resource_response.TagResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_clouddirectory.types.tag_resource_request.TagResourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/amazonclouddirectory/2017-01-11/tags/add"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_clouddirectory.types.tag_resource_request

    body: bytes | None = json.dumps(
        aws_sdk_clouddirectory.types.tag_resource_request.serialize_json(input_)
    ).encode()
