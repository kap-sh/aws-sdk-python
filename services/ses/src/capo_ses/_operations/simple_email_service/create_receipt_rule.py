"""Generated from Smithy shape ``com.amazonaws.ses#CreateReceiptRule``."""
...
import capo_ses.errors.already_exists_exception
...
import capo_ses.types.create_receipt_rule_request
import capo_ses.types.create_receipt_rule_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AlreadyExistsException":
            raise capo_ses.errors.already_exists_exception.AlreadyExistsException.from_query(
                root
            )
...
) -> capo_ses.types.create_receipt_rule_response.CreateReceiptRuleResponse:
    root = fromstring(response.read())
    result = root.find("CreateReceiptRuleResult")
    out: capo_ses.types.create_receipt_rule_response.CreateReceiptRuleResponse = (
        capo_ses.types.create_receipt_rule_response.deserialize_query(
...
def build_request(
    ...
    input_: capo_ses.types.create_receipt_rule_request.CreateReceiptRuleRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "CreateReceiptRule"))
    pairs.append(("Version", "2010-12-01"))
    capo_ses.types.create_receipt_rule_request.serialize_query(input_, pairs, "")
...
async def async_create_receipt_rule(
