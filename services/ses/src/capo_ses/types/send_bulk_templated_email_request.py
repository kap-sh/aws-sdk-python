"""Generated from Smithy shape ``com.amazonaws.ses#SendBulkTemplatedEmailRequest``."""
...
if TYPE_CHECKING:
    ...
    import capo_ses.types.bulk_email_destination_list
...
class SendBulkTemplatedEmailRequest(TypedDict, closed=True):
    ...
    default_tags: NotRequired["capo_ses.types.message_tag_list.MessageTagList"]
    ...
    template: "capo_ses.types.template_name.TemplateName"
    """<p>The template to use when sending this email.</p>"""
    template_arn: NotRequired["capo_ses.types.amazon_resource_name.AmazonResourceName"]
    """<p>The ARN of the template to use when sending this email.</p>"""
    default_template_data: "capo_ses.types.template_data.TemplateData"
    ...
    destinations: "capo_ses.types.bulk_email_destination_list.BulkEmailDestinationList"
...
# --- awsQuery ser/de ---
def serialize_query(
    value: SendBulkTemplatedEmailRequest, pairs: list[tuple[str, str]], prefix: str
) -> None:
    ...
    if "default_tags" in value:
        import capo_ses.types.message_tag_list

        capo_ses.types.message_tag_list.serialize_query(
            value["default_tags"], pairs, f"{prefix}.DefaultTags"
        )
    ...
    pairs.append((f"{prefix}.DefaultTemplateData", str(value["default_template_data"])))
    import capo_ses.types.bulk_email_destination_list

    capo_ses.types.bulk_email_destination_list.serialize_query(
        value["destinations"], pairs, f"{prefix}.Destinations"
    )


def deserialize_query(el: Element) -> SendBulkTemplatedEmailRequest:
    out: SendBulkTemplatedEmailRequest = {}  # type: ignore[typeddict-item]
    child_source = el.find("Source")
    if child_source is not None:
        out["source"] = str(child_source.text or "")
    else:
        raise DeserializationError("SendBulkTemplatedEmailRequest.source required")
    ...
    child_default_tags = el.find("DefaultTags")
    if child_default_tags is not None:
        import capo_ses.types.message_tag_list

        out["default_tags"] = capo_ses.types.message_tag_list.deserialize_query(
            child_default_tags
        )
    ...
    if child_template is not None:
        ...
        raise DeserializationError("SendBulkTemplatedEmailRequest.template required")
    child_template_arn = el.find("TemplateArn")
    if child_template_arn is not None:
        out["template_arn"] = str(child_template_arn.text or "")
    child_default_template_data = el.find("DefaultTemplateData")
    if child_default_template_data is not None:
        out["default_template_data"] = str(child_default_template_data.text or "")
    else:
        raise DeserializationError(
            "SendBulkTemplatedEmailRequest.default_template_data required"
        )
    ...
    if child_destinations is not None:
        ...
        out["destinations"] = (
            capo_ses.types.bulk_email_destination_list.deserialize_query(
                child_destinations
            )
        )
    else:
        raise DeserializationError(
            "SendBulkTemplatedEmailRequest.destinations required"
        )
    return out
