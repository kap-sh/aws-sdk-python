...
if TYPE_CHECKING:
    import capo_ses.types.bounced_recipient_info

BouncedRecipientInfoList: TypeAlias = list[
    "capo_ses.types.bounced_recipient_info.BouncedRecipientInfo"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_ses.types.bounced_recipient_info.serialize_query(
...
def deserialize_query(el: Element) -> BouncedRecipientInfoList:
    ...
    for child in el.findall("member"):
        out.append(capo_ses.types.bounced_recipient_info.deserialize_query(child))
    return out
