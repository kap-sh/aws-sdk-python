...
if TYPE_CHECKING:
    import aws_sdk_ses.types.counter
    import aws_sdk_ses.types.timestamp


class SendDataPoint(TypedDict):
    timestamp: NotRequired["aws_sdk_ses.types.timestamp.Timestamp"]
    """<p>Time of the data point.</p>"""
    delivery_attempts: "aws_sdk_ses.types.counter.Counter"
    """<p>Number of emails that have been sent.</p>"""
    bounces: "aws_sdk_ses.types.counter.Counter"
    """<p>Number of emails that have bounced.</p>"""
    complaints: "aws_sdk_ses.types.counter.Counter"
    """<p>Number of unwanted emails that were rejected by recipients.</p>"""
    rejects: "aws_sdk_ses.types.counter.Counter"
...
def deserialize_query(el: Element) -> SendDataPoint:
    ...
    if child_timestamp is not None:
        ...
        out["timestamp"] = aws_sdk_ses.types.timestamp.deserialize_query(
            child_timestamp
        )
