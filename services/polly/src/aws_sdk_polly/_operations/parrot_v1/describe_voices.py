...
def build_request(
    ...
    input: aws_sdk_polly.types.describe_voices_input.DescribeVoicesInput,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "engine" in input:
        params["Engine"] = str(input["engine"])
    if "language_code" in input:
        params["LanguageCode"] = str(input["language_code"])
    params["IncludeAdditionalLanguageCodes"] = str(
        input.get("include_additional_language_codes", False)
    )
