...
class PollyError(Exception):
    """Base class for all Polly service errors."""


class ServiceError(PollyError):
...
class SerializationError(PollyError):
    """Raised when a value cannot be serialized."""


class DeserializationError(PollyError):
...
class WaiterFailedError(PollyError):
...
class WaiterTimeoutError(PollyError):
