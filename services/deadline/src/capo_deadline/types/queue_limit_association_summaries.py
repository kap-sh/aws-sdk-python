...
QueueLimitAssociationSummaries: TypeAlias = list[
    "capo_deadline.types.queue_limit_association_summary.QueueLimitAssociationSummary"
]
...
def serialize_json(value: QueueLimitAssociationSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_deadline.types.queue_limit_association_summary.serialize_json(item)
        )
    return out
