...
if TYPE_CHECKING:
    import capo_deadline.types.batch_get_step_error_code
...
class BatchGetStepError(TypedDict, closed=True):
    ...
    code: "capo_deadline.types.batch_get_step_error_code.BatchGetStepErrorCode"
    """<p>The error code.</p>"""
...
def serialize_json(value: BatchGetStepError) -> dict:
    ...
    out["code"] = capo_deadline.types.batch_get_step_error_code.serialize_json(
        value["code"]
    )
...
def deserialize_json(data: dict) -> BatchGetStepError:
    ...
    if "code" in data:
        ...
        out["code"] = capo_deadline.types.batch_get_step_error_code.deserialize_json(
            data["code"]
        )
    else:
