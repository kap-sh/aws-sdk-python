"""Generated from Smithy shape ``com.amazonaws.deadline#CreateLicenseEndpoint``."""
...
def handle_response(
    ...
    out: capo_deadline.types.create_license_endpoint_response.CreateLicenseEndpointResponse = capo_deadline.types.create_license_endpoint_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2023-10-12/license-endpoints"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "client_token" in input_:
        headers["X-Amz-Client-Token"] = str(input_["client_token"])
    body: bytes | None = json.dumps(
        capo_deadline.types.create_license_endpoint_request.serialize_json(input_)
    ).encode()
...
async def async_create_license_endpoint(
