"""Generated from Smithy shape ``com.amazonaws.deadline#SearchTasks``."""
...
def handle_response(
    ...
    out: capo_deadline.types.search_tasks_response.SearchTasksResponse = (
        capo_deadline.types.search_tasks_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2023-10-12/farms/{farmId}/search/tasks"
    url = url.replace("{farmId}", quote(str(input_["farm_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_deadline.types.search_tasks_request.serialize_json(input_)
    ).encode()
...
def search_tasks(
    ...
    capo_deadline.types.search_tasks_response.SearchTasksResponse, zapros.Response
]:
