"""Generated from Smithy shape ``com.amazonaws.deadline#GetVolume``."""
...
def handle_response(
    ...
    out: capo_deadline.types.get_volume_response.GetVolumeResponse = (
        capo_deadline.types.get_volume_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/2023-10-12/farms/{farmId}/fleets/{fleetId}/volumes/{volumeId}"
    )
    url = url.replace("{farmId}", quote(str(input_["farm_id"]), safe=""))
    url = url.replace("{fleetId}", quote(str(input_["fleet_id"]), safe=""))
    url = url.replace("{volumeId}", quote(str(input_["volume_id"]), safe=""))
    params: dict[str, str] = {}
...
def get_volume(
    ...
    input_: capo_deadline.types.get_volume_request.GetVolumeRequest,
) -> tuple[capo_deadline.types.get_volume_response.GetVolumeResponse, zapros.Response]:
