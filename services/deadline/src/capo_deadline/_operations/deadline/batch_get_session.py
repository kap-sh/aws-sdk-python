"""Generated from Smithy shape ``com.amazonaws.deadline#BatchGetSession``."""
...
def handle_response(
    ...
    out: capo_deadline.types.batch_get_session_response.BatchGetSessionResponse = (
        capo_deadline.types.batch_get_session_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2023-10-12/batch-get-session"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_deadline.types.batch_get_session_request.serialize_json(input_)
    ).encode()
...
async def async_batch_get_session(
