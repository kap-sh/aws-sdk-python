"""Generated from Smithy shape ``com.amazonaws.deadline#SearchSteps``."""
...
def handle_response(
    ...
    out: capo_deadline.types.search_steps_response.SearchStepsResponse = (
        capo_deadline.types.search_steps_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/2023-10-12/farms/{farmId}/search/steps"
    url = url.replace("{farmId}", quote(str(input_["farm_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_deadline.types.search_steps_request.serialize_json(input_)
    ).encode()
...
def search_steps(
    ...
    capo_deadline.types.search_steps_response.SearchStepsResponse, zapros.Response
]:
