...
if TYPE_CHECKING:
    import aws_sdk_deadline.types.worker_attribute_capability

WorkerAttributeCapabilityList: TypeAlias = list[
    "aws_sdk_deadline.types.worker_attribute_capability.WorkerAttributeCapability"
]
...
def serialize_json(value: WorkerAttributeCapabilityList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_deadline.types.worker_attribute_capability.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> WorkerAttributeCapabilityList:
    ...
    for item in data:
        out.append(
            aws_sdk_deadline.types.worker_attribute_capability.deserialize_json(item)
        )
    return out
