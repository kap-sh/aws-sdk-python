...
class QueueLimitAssociationSummary(TypedDict):
    ...
    status: "aws_sdk_deadline.types.queue_limit_association_status.QueueLimitAssociationStatus"
...
def serialize_json(value: QueueLimitAssociationSummary) -> dict:
    ...
    out["status"] = (
        aws_sdk_deadline.types.queue_limit_association_status.serialize_json(
            value["status"]
        )
    )
    import aws_sdk_deadline.types.created_at
...
def deserialize_json(data: dict) -> QueueLimitAssociationSummary:
    ...
    if "status" in data:
        ...
        out["status"] = (
            aws_sdk_deadline.types.queue_limit_association_status.deserialize_json(
                data["status"]
            )
        )
    else:
