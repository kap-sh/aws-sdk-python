...
class ListQueuesResponse(TypedDict):
    queues: "aws_sdk_deadline.types.queue_summaries.QueueSummaries"
...
def serialize_json(value: ListQueuesResponse) -> dict:
    ...
    out["queues"] = aws_sdk_deadline.types.queue_summaries.serialize_json(
        value["queues"]
    )
...
def deserialize_json(data: dict) -> ListQueuesResponse:
    ...
    if "queues" in data:
        ...
        out["queues"] = aws_sdk_deadline.types.queue_summaries.deserialize_json(
            data["queues"]
        )
    else:
