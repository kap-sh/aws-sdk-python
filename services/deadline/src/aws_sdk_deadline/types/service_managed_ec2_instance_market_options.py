"""Generated from Smithy shape ``com.amazonaws.deadline#ServiceManagedEc2InstanceMarketOptions``."""

from typing import TYPE_CHECKING, TypedDict

from aws_sdk_deadline.errors import DeserializationError

if TYPE_CHECKING:
    import aws_sdk_deadline.types.ec2_market_type


class ServiceManagedEc2InstanceMarketOptions(TypedDict):
    type: "aws_sdk_deadline.types.ec2_market_type.Ec2MarketType"
    """<p>The Amazon EC2 instance type.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ServiceManagedEc2InstanceMarketOptions) -> dict:
    out: dict = {}
    import aws_sdk_deadline.types.ec2_market_type

    out["type"] = aws_sdk_deadline.types.ec2_market_type.serialize_json(value["type"])
    return out


def deserialize_json(data: dict) -> ServiceManagedEc2InstanceMarketOptions:
    out: ServiceManagedEc2InstanceMarketOptions = {}  # type: ignore[typeddict-item]
    if "type" in data:
        import aws_sdk_deadline.types.ec2_market_type

        out["type"] = aws_sdk_deadline.types.ec2_market_type.deserialize_json(
            data["type"]
        )
    else:
        raise DeserializationError(
            "ServiceManagedEc2InstanceMarketOptions.type required"
        )
    return out
