...
class ListSessionsResponse(TypedDict, closed=True):
    sessions: "aws_sdk_deadline.types.session_summaries.SessionSummaries"
...
def serialize_json(value: ListSessionsResponse) -> dict:
    ...
    out["sessions"] = aws_sdk_deadline.types.session_summaries.serialize_json(
        value["sessions"]
    )
...
def deserialize_json(data: dict) -> ListSessionsResponse:
    ...
    if "sessions" in data:
        ...
        out["sessions"] = aws_sdk_deadline.types.session_summaries.deserialize_json(
            data["sessions"]
        )
    else:
