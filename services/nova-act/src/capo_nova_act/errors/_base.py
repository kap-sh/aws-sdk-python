...
class NovaActError(Exception):
    """Base class for all NovaAct service errors."""


class ServiceError(NovaActError):
...
class SerializationError(NovaActError):
    """Raised when a value cannot be serialized."""


class DeserializationError(NovaActError):
...
class WaiterFailedError(NovaActError):
...
class WaiterTimeoutError(NovaActError):
