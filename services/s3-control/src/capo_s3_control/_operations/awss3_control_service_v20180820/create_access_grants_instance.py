"""Generated from Smithy shape ``com.amazonaws.s3control#CreateAccessGrantsInstance``."""
...
def handle_response(
    ...
    out: capo_s3_control.types.create_access_grants_instance_result.CreateAccessGrantsInstanceResult = capo_s3_control.types.create_access_grants_instance_result.deserialize_xml(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v20180820/accessgrantsinstance"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "account_id" in input_:
        headers["x-amz-account-id"] = str(input_["account_id"])
    root = Element("CreateAccessGrantsInstanceRequest")
...
async def async_create_access_grants_instance(
