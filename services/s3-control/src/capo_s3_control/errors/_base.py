...
class S3ControlError(Exception):
    """Base class for all S3Control service errors."""


class ServiceError(S3ControlError):
...
class SerializationError(S3ControlError):
    """Raised when a value cannot be serialized."""


class DeserializationError(S3ControlError):
...
class WaiterFailedError(S3ControlError):
...
class WaiterTimeoutError(S3ControlError):
