...
if TYPE_CHECKING:
    import capo_s3_control.types.access_key_id
    import capo_s3_control.types.expiration
    import capo_s3_control.types.secret_access_key
    import capo_s3_control.types.session_token


class Credentials(TypedDict, closed=True):
    access_key_id: NotRequired["capo_s3_control.types.access_key_id.AccessKeyId"]
    ...
    secret_access_key: NotRequired[
        "capo_s3_control.types.secret_access_key.SecretAccessKey"
    ]
    ...
    session_token: NotRequired["capo_s3_control.types.session_token.SessionToken"]
    """<p>The Amazon Web Services STS temporary credential that S3 Access Grants vends to grantees and client applications. </p>"""
    expiration: NotRequired["capo_s3_control.types.expiration.Expiration"]
...
def serialize_xml(value: Credentials, parent: Element, tag: str) -> None:
    ...
    if "expiration" in value:
        ...
        capo_s3_control.types.expiration.serialize_xml(
...
def deserialize_xml(el: Element) -> Credentials:
    ...
    if child_expiration is not None:
        ...
        out["expiration"] = capo_s3_control.types.expiration.deserialize_xml(
            child_expiration
        )
    return out
