...
if TYPE_CHECKING:
    import capo_s3_control.types.prefix_level_storage_metrics


class PrefixLevel(TypedDict, closed=True):
    storage_metrics: (
        "capo_s3_control.types.prefix_level_storage_metrics.PrefixLevelStorageMetrics"
    )
...
def serialize_xml(value: PrefixLevel, parent: Element, tag: str) -> None:
    ...
    capo_s3_control.types.prefix_level_storage_metrics.serialize_xml(
...
def deserialize_xml(el: Element) -> PrefixLevel:
    ...
    if child_storage_metrics is not None:
        ...
        out["storage_metrics"] = (
            capo_s3_control.types.prefix_level_storage_metrics.deserialize_xml(
