...
if TYPE_CHECKING:
    import capo_s3_control.types.s3_update_object_encryption_ssekms


class ObjectEncryption(TypedDict, closed=True):
    ssekms: NotRequired[
        "capo_s3_control.types.s3_update_object_encryption_ssekms.S3UpdateObjectEncryptionSSEKMS"
    ]
...
def serialize_xml(value: ObjectEncryption, parent: Element, tag: str) -> None:
    ...
    if "ssekms" in value:
        ...
        capo_s3_control.types.s3_update_object_encryption_ssekms.serialize_xml(
...
def deserialize_xml(el: Element) -> ObjectEncryption:
    ...
    if child_ssekms is not None:
        ...
        out["ssekms"] = (
            capo_s3_control.types.s3_update_object_encryption_ssekms.deserialize_xml(
                child_ssekms
            )
        )
    return out
