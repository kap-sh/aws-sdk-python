...
class GetBucketVersioningResult(TypedDict, closed=True):
    status: NotRequired[
        "capo_s3_control.types.bucket_versioning_status.BucketVersioningStatus"
    ]
    """<p>The versioning state of the S3 on Outposts bucket.</p>"""
    mfa_delete: NotRequired["capo_s3_control.types.mfa_delete_status.MFADeleteStatus"]
...
def serialize_xml(value: GetBucketVersioningResult, parent: Element, tag: str) -> None:
    ...
    if "status" in value:
        ...
        capo_s3_control.types.bucket_versioning_status.serialize_xml(
            value["status"], el, "Status"
        )
    if "mfa_delete" in value:
        import capo_s3_control.types.mfa_delete_status

        capo_s3_control.types.mfa_delete_status.serialize_xml(
...
def deserialize_xml(el: Element) -> GetBucketVersioningResult:
    ...
    if child_status is not None:
        ...
        out["status"] = capo_s3_control.types.bucket_versioning_status.deserialize_xml(
            child_status
        )
    ...
    if child_mfa_delete is not None:
        ...
        out["mfa_delete"] = capo_s3_control.types.mfa_delete_status.deserialize_xml(
            child_mfa_delete
        )
    return out
