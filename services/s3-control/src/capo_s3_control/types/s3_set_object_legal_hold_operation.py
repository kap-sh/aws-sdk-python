...
if TYPE_CHECKING:
    import capo_s3_control.types.s3_object_lock_legal_hold


class S3SetObjectLegalHoldOperation(TypedDict, closed=True):
    legal_hold: "capo_s3_control.types.s3_object_lock_legal_hold.S3ObjectLockLegalHold"
...
def serialize_xml(
    ...
    capo_s3_control.types.s3_object_lock_legal_hold.serialize_xml(
...
def deserialize_xml(el: Element) -> S3SetObjectLegalHoldOperation:
    ...
    if child_legal_hold is not None:
        ...
        out["legal_hold"] = (
            capo_s3_control.types.s3_object_lock_legal_hold.deserialize_xml(
                child_legal_hold
            )
        )
    else:
