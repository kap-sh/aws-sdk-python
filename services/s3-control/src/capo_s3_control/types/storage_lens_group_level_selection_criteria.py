...
if TYPE_CHECKING:
    import capo_s3_control.types.storage_lens_group_level_exclude
    import capo_s3_control.types.storage_lens_group_level_include


class StorageLensGroupLevelSelectionCriteria(TypedDict, closed=True):
    include: NotRequired[
        "capo_s3_control.types.storage_lens_group_level_include.StorageLensGroupLevelInclude"
    ]
    """<p> Indicates which Storage Lens group ARNs to include in the Storage Lens group aggregation. </p>"""
    exclude: NotRequired[
        "capo_s3_control.types.storage_lens_group_level_exclude.StorageLensGroupLevelExclude"
    ]
...
def serialize_xml(
    ...
    if "include" in value:
        ...
        capo_s3_control.types.storage_lens_group_level_include.serialize_xml(
            value["include"], el, "Include"
        )
    if "exclude" in value:
        import capo_s3_control.types.storage_lens_group_level_exclude

        capo_s3_control.types.storage_lens_group_level_exclude.serialize_xml(
...
def deserialize_xml(el: Element) -> StorageLensGroupLevelSelectionCriteria:
    ...
    if child_include is not None:
        ...
        out["include"] = (
            capo_s3_control.types.storage_lens_group_level_include.deserialize_xml(
                child_include
            )
        )
    ...
    if child_exclude is not None:
        ...
        out["exclude"] = (
            capo_s3_control.types.storage_lens_group_level_exclude.deserialize_xml(
                child_exclude
            )
        )
    return out
