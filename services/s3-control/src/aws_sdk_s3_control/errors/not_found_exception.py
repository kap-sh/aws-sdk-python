"""Generated from Smithy shape ``com.amazonaws.s3control#NotFoundException``."""
...
# --- restXml ser/de ---
def serialize_xml(value: NotFoundException_, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    if "message" in value:
        SubElement(el, "Message").text = str(value["message"])


def deserialize_xml(el: Element) -> NotFoundException_:
...
class NotFoundException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.s3control#NotFoundException``."""
    ...
    @classmethod
    def from_xml(cls, el: Element) -> "NotFoundException":
