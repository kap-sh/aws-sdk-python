...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "BucketAlreadyExists":
            import aws_sdk_s3_control.errors.bucket_already_exists

            raise aws_sdk_s3_control.errors.bucket_already_exists.BucketAlreadyExists.from_xml(
                root
            )
        case "BucketAlreadyOwnedByYou":
            import aws_sdk_s3_control.errors.bucket_already_owned_by_you

            raise aws_sdk_s3_control.errors.bucket_already_owned_by_you.BucketAlreadyOwnedByYou.from_xml(
                root
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_s3_control.types.create_bucket_result.CreateBucketResult = (
        aws_sdk_s3_control.types.create_bucket_result.deserialize_xml(
...
def build_request(
    ...
    input: aws_sdk_s3_control.types.create_bucket_request.CreateBucketRequest,
) -> zapros.Request:
    endpoint = resolve(  # noqa: F841
        EndpointParams(
            ...
            OutpostId=input.get("outpost_id"),
    ...
    headers["x-amz-bucket-object-lock-enabled"] = str(
        input.get("object_lock_enabled_for_bucket", False)
    )
    if "outpost_id" in input:
        headers["x-amz-outpost-id"] = str(input["outpost_id"])
...
def create_bucket(
    ...
    aws_sdk_s3_control.types.create_bucket_result.CreateBucketResult, zapros.Response
]:
