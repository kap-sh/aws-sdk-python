...
def handle_response(
    ...
    out: aws_sdk_s3_control.types.list_access_grants_result.ListAccessGrantsResult = (
        aws_sdk_s3_control.types.list_access_grants_result.deserialize_xml(
...
def build_request(
    ...
    input: aws_sdk_s3_control.types.list_access_grants_request.ListAccessGrantsRequest,
) -> zapros.Request:
    ...
    if "grantee_type" in input:
        params["granteetype"] = str(input["grantee_type"])
    if "grantee_identifier" in input:
        params["granteeidentifier"] = str(input["grantee_identifier"])
    if "permission" in input:
        params["permission"] = str(input["permission"])
    if "grant_scope" in input:
        params["grantscope"] = str(input["grant_scope"])
    if "application_arn" in input:
        params["application_arn"] = str(input["application_arn"])
