...
if TYPE_CHECKING:
    import aws_sdk_s3_control.types.metrics_status
    import aws_sdk_s3_control.types.replication_time_value


class Metrics(TypedDict, closed=True):
    status: "aws_sdk_s3_control.types.metrics_status.MetricsStatus"
    """<p>Specifies whether replication metrics are enabled. </p>"""
    event_threshold: NotRequired[
        "aws_sdk_s3_control.types.replication_time_value.ReplicationTimeValue"
    ]
...
def serialize_xml(value: Metrics, parent: Element, tag: str) -> None:
    ...
    aws_sdk_s3_control.types.metrics_status.serialize_xml(value["status"], el, "Status")
...
def deserialize_xml(el: Element) -> Metrics:
    ...
    if child_status is not None:
        ...
        out["status"] = aws_sdk_s3_control.types.metrics_status.deserialize_xml(
            child_status
        )
    else:
    ...
    if child_event_threshold is not None:
        ...
        out["event_threshold"] = (
            aws_sdk_s3_control.types.replication_time_value.deserialize_xml(
