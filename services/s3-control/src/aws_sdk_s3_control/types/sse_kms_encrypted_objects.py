...
if TYPE_CHECKING:
    import aws_sdk_s3_control.types.sse_kms_encrypted_objects_status


class SseKmsEncryptedObjects(TypedDict):
    status: "aws_sdk_s3_control.types.sse_kms_encrypted_objects_status.SseKmsEncryptedObjectsStatus"
...
def serialize_xml(value: SseKmsEncryptedObjects, parent: Element, tag: str) -> None:
    ...
    aws_sdk_s3_control.types.sse_kms_encrypted_objects_status.serialize_xml(
...
def deserialize_xml(el: Element) -> SseKmsEncryptedObjects:
    ...
    if child_status is not None:
        ...
        out["status"] = (
            aws_sdk_s3_control.types.sse_kms_encrypted_objects_status.deserialize_xml(
                child_status
            )
        )
    else:
