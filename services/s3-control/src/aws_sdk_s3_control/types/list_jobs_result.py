...
class ListJobsResult(TypedDict, closed=True):
    ...
    jobs: NotRequired[
        "aws_sdk_s3_control.types.job_list_descriptor_list.JobListDescriptorList"
    ]
...
def serialize_xml(value: ListJobsResult, parent: Element, tag: str) -> None:
    ...
    if "jobs" in value:
        ...
        aws_sdk_s3_control.types.job_list_descriptor_list.serialize_xml(
            value["jobs"], el, "Jobs"
        )
...
def deserialize_xml(el: Element) -> ListJobsResult:
    ...
    if child_jobs is not None:
        ...
        out["jobs"] = aws_sdk_s3_control.types.job_list_descriptor_list.deserialize_xml(
            child_jobs
        )
    return out
