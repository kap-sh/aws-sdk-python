...
if TYPE_CHECKING:
    import aws_sdk_s3_control.types.s3_job_manifest_generator


class _JobManifestGenerator_S3JobManifestGenerator(TypedDict):
    S3JobManifestGenerator: (
        "aws_sdk_s3_control.types.s3_job_manifest_generator.S3JobManifestGenerator"
    )
...
def serialize_xml(value: JobManifestGenerator, parent: Element, tag: str) -> None:
    ...
    if "S3JobManifestGenerator" in value:
        ...
        aws_sdk_s3_control.types.s3_job_manifest_generator.serialize_xml(
...
def deserialize_xml(el: Element) -> JobManifestGenerator:
    for child in el:
        if child.tag == "S3JobManifestGenerator":
            ...
            return {
                "S3JobManifestGenerator": aws_sdk_s3_control.types.s3_job_manifest_generator.deserialize_xml(
                    child
                )
            }
