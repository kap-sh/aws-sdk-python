...
class ServiceHealth(TypedDict, closed=True):
    service_name: NotRequired["capo_devops_guru.types.service_name.ServiceName"]
    """<p>The name of the Amazon Web Services service.</p>"""
    insight: NotRequired[
        "capo_devops_guru.types.service_insight_health.ServiceInsightHealth"
    ]
...
def serialize_json(value: ServiceHealth) -> dict:
    ...
    if "service_name" in value:
        ...
        out["ServiceName"] = capo_devops_guru.types.service_name.serialize_json(
            value["service_name"]
        )
    if "insight" in value:
        import capo_devops_guru.types.service_insight_health

        out["Insight"] = capo_devops_guru.types.service_insight_health.serialize_json(
            value["insight"]
        )
