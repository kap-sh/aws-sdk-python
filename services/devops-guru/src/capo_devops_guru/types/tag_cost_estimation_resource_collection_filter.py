...
class TagCostEstimationResourceCollectionFilter(TypedDict, closed=True):
    ...
    tag_values: (
        "capo_devops_guru.types.cost_estimation_tag_values.CostEstimationTagValues"
    )
...
def serialize_json(value: TagCostEstimationResourceCollectionFilter) -> dict:
    ...
    out["TagValues"] = capo_devops_guru.types.cost_estimation_tag_values.serialize_json(
        value["tag_values"]
    )
    return out
...
def deserialize_json(data: dict) -> TagCostEstimationResourceCollectionFilter:
    ...
    if "TagValues" in data:
        ...
        out["tag_values"] = (
            capo_devops_guru.types.cost_estimation_tag_values.deserialize_json(
                data["TagValues"]
            )
        )
    else:
