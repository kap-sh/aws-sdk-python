"""Generated from Smithy shape ``com.amazonaws.devopsguru#UpdateTagCollectionFilter``."""
...
class UpdateTagCollectionFilter(TypedDict, closed=True):
    ...
    tag_values: "capo_devops_guru.types.update_tag_values.UpdateTagValues"
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateTagCollectionFilter) -> dict:
    out: dict = {}
    out["AppBoundaryKey"] = value["app_boundary_key"]
    import capo_devops_guru.types.update_tag_values

    out["TagValues"] = capo_devops_guru.types.update_tag_values.serialize_json(
        value["tag_values"]
    )
    return out
...
def deserialize_json(data: dict) -> UpdateTagCollectionFilter:
    ...
    if "TagValues" in data:
        ...
        raise DeserializationError("UpdateTagCollectionFilter.tag_values required")
    return out
