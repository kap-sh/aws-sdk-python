...
if TYPE_CHECKING:
    import capo_devops_guru.types.cloud_formation_collection
    import capo_devops_guru.types.tag_collections


class ResourceCollection(TypedDict, closed=True):
    cloud_formation: NotRequired[
        "capo_devops_guru.types.cloud_formation_collection.CloudFormationCollection"
    ]
...
def serialize_json(value: ResourceCollection) -> dict:
    ...
    if "cloud_formation" in value:
        ...
        out["CloudFormation"] = (
            capo_devops_guru.types.cloud_formation_collection.serialize_json(
    ...
    if "tags" in value:
        ...
        out["Tags"] = capo_devops_guru.types.tag_collections.serialize_json(
            value["tags"]
        )
    return out
...
def deserialize_json(data: dict) -> ResourceCollection:
    ...
    if "CloudFormation" in data:
        ...
        out["cloud_formation"] = (
            capo_devops_guru.types.cloud_formation_collection.deserialize_json(
                data["CloudFormation"]
            )
        )
    if "Tags" in data:
        ...
        out["tags"] = capo_devops_guru.types.tag_collections.deserialize_json(
            data["Tags"]
        )
    return out
