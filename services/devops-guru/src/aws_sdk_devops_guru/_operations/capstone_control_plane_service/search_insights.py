"""Generated from Smithy shape ``com.amazonaws.devopsguru#SearchInsights``."""
...
def handle_response(
    ...
    out: aws_sdk_devops_guru.types.search_insights_response.SearchInsightsResponse = (
        aws_sdk_devops_guru.types.search_insights_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_devops_guru.types.search_insights_request.SearchInsightsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/insights/search"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_devops_guru.types.search_insights_request.serialize_json(input_)
    ).encode()
...
async def async_search_insights(
