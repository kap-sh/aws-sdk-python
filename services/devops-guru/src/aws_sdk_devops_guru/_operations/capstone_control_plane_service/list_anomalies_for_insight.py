"""Generated from Smithy shape ``com.amazonaws.devopsguru#ListAnomaliesForInsight``."""
...
def handle_response(
    ...
    out: aws_sdk_devops_guru.types.list_anomalies_for_insight_response.ListAnomaliesForInsightResponse = aws_sdk_devops_guru.types.list_anomalies_for_insight_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_devops_guru.types.list_anomalies_for_insight_request.ListAnomaliesForInsightRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/anomalies/insight/{InsightId}"
    url = url.replace("{InsightId}", quote(str(input_["insight_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_devops_guru.types.list_anomalies_for_insight_request

    body: bytes | None = json.dumps(
        aws_sdk_devops_guru.types.list_anomalies_for_insight_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_anomalies_for_insight(
