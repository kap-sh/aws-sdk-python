...
if TYPE_CHECKING:
    import aws_sdk_waf_regional.types.action
    import aws_sdk_waf_regional.types.http_request
    import aws_sdk_waf_regional.types.resource_id
    import aws_sdk_waf_regional.types.sample_weight
...
class SampledHTTPRequest(TypedDict):
    ...
    timestamp: NotRequired["aws_sdk_waf_regional.types.timestamp.Timestamp"]
    """<p>The time at which AWS WAF received the request from your AWS resource, in Unix time format (in seconds).</p>"""
    action: NotRequired["aws_sdk_waf_regional.types.action.Action"]
...
def serialize_aws_json_1_1(value: SampledHTTPRequest) -> dict:
    ...
    out["Request"] = aws_sdk_waf_regional.types.http_request.serialize_aws_json_1_1(
        value["request"]
    )
    ...
    if "timestamp" in value:
        ...
        out["Timestamp"] = aws_sdk_waf_regional.types.timestamp.serialize_aws_json_1_1(
            value["timestamp"]
        )
    if "action" in value:
...
def deserialize_aws_json_1_1(data: dict) -> SampledHTTPRequest:
    ...
    if "Request" in data:
        ...
        out["request"] = (
            aws_sdk_waf_regional.types.http_request.deserialize_aws_json_1_1(
                data["Request"]
            )
        )
    else:
    ...
    if "Timestamp" in data:
        ...
        out["timestamp"] = (
            aws_sdk_waf_regional.types.timestamp.deserialize_aws_json_1_1(
                data["Timestamp"]
            )
        )
    if "Action" in data:
