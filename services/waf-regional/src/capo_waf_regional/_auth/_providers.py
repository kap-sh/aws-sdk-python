...
from capo_waf_regional._auth._identity import (
    Credentials,
    Identity,
)
from capo_waf_regional._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-waf-regional[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-waf-regional[sso]"
        )
