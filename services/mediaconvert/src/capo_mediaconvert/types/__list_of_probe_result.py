...
if TYPE_CHECKING:
    import capo_mediaconvert.types.probe_result

__listOfProbeResult: TypeAlias = list[
    "capo_mediaconvert.types.probe_result.ProbeResult"
]
...
def serialize_json(value: __listOfProbeResult) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mediaconvert.types.probe_result.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfProbeResult:
    ...
    for item in data:
        out.append(capo_mediaconvert.types.probe_result.deserialize_json(item))
    return out
