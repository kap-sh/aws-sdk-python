...
class Deinterlacer(TypedDict, closed=True):
    algorithm: NotRequired[
        "capo_mediaconvert.types.deinterlace_algorithm.DeinterlaceAlgorithm"
    ]
    ...
    control: NotRequired[
        "capo_mediaconvert.types.deinterlacer_control.DeinterlacerControl"
    ]
    """- When set to NORMAL (default), the deinterlacer does not convert frames that are tagged in metadata as progressive. It will only convert those that are tagged as some other type. - When set to FORCE_ALL_FRAMES, the deinterlacer converts every frame to progressive - even those that are already tagged as progressive. Turn Force mode on only if there is a good chance that the metadata has tagged frames as progressive when they are not progressive. Do not turn on otherwise; processing frames that are already progressive into progressive will probably result in lower quality video."""
    mode: NotRequired["capo_mediaconvert.types.deinterlacer_mode.DeinterlacerMode"]
...
def serialize_json(value: Deinterlacer) -> dict:
    ...
    if "algorithm" in value:
        ...
        out["algorithm"] = capo_mediaconvert.types.deinterlace_algorithm.serialize_json(
            value["algorithm"]
        )
    if "control" in value:
        import capo_mediaconvert.types.deinterlacer_control

        out["control"] = capo_mediaconvert.types.deinterlacer_control.serialize_json(
            value["control"]
        )
    if "mode" in value:
        import capo_mediaconvert.types.deinterlacer_mode

        out["mode"] = capo_mediaconvert.types.deinterlacer_mode.serialize_json(
            value["mode"]
        )
    return out
...
def deserialize_json(data: dict) -> Deinterlacer:
    ...
    if "algorithm" in data:
        ...
        out["algorithm"] = (
            capo_mediaconvert.types.deinterlace_algorithm.deserialize_json(
                data["algorithm"]
            )
        )
    if "control" in data:
