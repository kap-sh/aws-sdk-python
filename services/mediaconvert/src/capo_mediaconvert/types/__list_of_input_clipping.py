...
if TYPE_CHECKING:
    import capo_mediaconvert.types.input_clipping

__listOfInputClipping: TypeAlias = list[
    "capo_mediaconvert.types.input_clipping.InputClipping"
]
...
def serialize_json(value: __listOfInputClipping) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mediaconvert.types.input_clipping.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfInputClipping:
    ...
    for item in data:
        out.append(capo_mediaconvert.types.input_clipping.deserialize_json(item))
    return out
