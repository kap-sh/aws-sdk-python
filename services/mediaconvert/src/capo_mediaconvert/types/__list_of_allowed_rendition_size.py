...
if TYPE_CHECKING:
    import capo_mediaconvert.types.allowed_rendition_size

__listOfAllowedRenditionSize: TypeAlias = list[
    "capo_mediaconvert.types.allowed_rendition_size.AllowedRenditionSize"
]
...
def serialize_json(value: __listOfAllowedRenditionSize) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mediaconvert.types.allowed_rendition_size.serialize_json(item))
    return out
