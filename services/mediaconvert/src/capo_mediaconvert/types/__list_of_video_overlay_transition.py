...
if TYPE_CHECKING:
    import capo_mediaconvert.types.video_overlay_transition

__listOfVideoOverlayTransition: TypeAlias = list[
    "capo_mediaconvert.types.video_overlay_transition.VideoOverlayTransition"
]
...
def serialize_json(value: __listOfVideoOverlayTransition) -> list:
    ...
    for item in value:
        out.append(
            capo_mediaconvert.types.video_overlay_transition.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> __listOfVideoOverlayTransition:
    ...
    for item in data:
        out.append(
            capo_mediaconvert.types.video_overlay_transition.deserialize_json(item)
        )
    return out
