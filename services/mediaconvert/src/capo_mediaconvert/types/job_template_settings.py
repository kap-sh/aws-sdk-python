...
if TYPE_CHECKING:
    ...
    import capo_mediaconvert.types.__list_of_input_template
...
class JobTemplateSettings(TypedDict, closed=True):
    ...
    inputs: NotRequired[
        "capo_mediaconvert.types.__list_of_input_template.__listOfInputTemplate"
    ]
...
def serialize_json(value: JobTemplateSettings) -> dict:
    ...
    if "inputs" in value:
        ...
        out["inputs"] = capo_mediaconvert.types.__list_of_input_template.serialize_json(
            value["inputs"]
        )
...
def deserialize_json(data: dict) -> JobTemplateSettings:
    ...
    if "inputs" in data:
        ...
        out["inputs"] = (
            capo_mediaconvert.types.__list_of_input_template.deserialize_json(
                data["inputs"]
            )
        )
