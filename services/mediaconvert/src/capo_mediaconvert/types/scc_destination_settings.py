...
class SccDestinationSettings(TypedDict, closed=True):
    framerate: NotRequired[
        "capo_mediaconvert.types.scc_destination_framerate.SccDestinationFramerate"
    ]
...
def serialize_json(value: SccDestinationSettings) -> dict:
    ...
    if "framerate" in value:
        ...
        out["framerate"] = (
            capo_mediaconvert.types.scc_destination_framerate.serialize_json(
                value["framerate"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> SccDestinationSettings:
    ...
    if "framerate" in data:
        ...
        out["framerate"] = (
            capo_mediaconvert.types.scc_destination_framerate.deserialize_json(
                data["framerate"]
            )
        )
    return out
