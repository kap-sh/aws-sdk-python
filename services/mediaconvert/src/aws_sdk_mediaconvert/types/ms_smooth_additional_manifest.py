"""Generated from Smithy shape ``com.amazonaws.mediaconvert#MsSmoothAdditionalManifest``."""
...
class MsSmoothAdditionalManifest(TypedDict, closed=True):
    manifest_name_modifier: NotRequired[
        "aws_sdk_mediaconvert.types.__string_min1.__stringMin1"
    ]
    r"""Specify a name modifier that the service adds to the name of this manifest to make it different from the file names of the other main manifests in the output group. For example, say that the default main manifest for your Microsoft Smooth group is film-name.ismv. If you enter \"-no-premium\" for this setting, then the file name the service generates for this top-level manifest is film-name-no-premium.ismv."""
...
# --- restJson1 ser/de ---
def serialize_json(value: MsSmoothAdditionalManifest) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> MsSmoothAdditionalManifest:
    out: MsSmoothAdditionalManifest = {}  # type: ignore[typeddict-item]
