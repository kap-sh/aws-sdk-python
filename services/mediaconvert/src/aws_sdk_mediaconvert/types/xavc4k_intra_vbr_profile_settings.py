...
if TYPE_CHECKING:
    import aws_sdk_mediaconvert.types.xavc4k_intra_vbr_profile_class


class Xavc4kIntraVbrProfileSettings(TypedDict):
    xavc_class: NotRequired[
        "aws_sdk_mediaconvert.types.xavc4k_intra_vbr_profile_class.Xavc4kIntraVbrProfileClass"
    ]
...
def serialize_json(value: Xavc4kIntraVbrProfileSettings) -> dict:
    ...
    if "xavc_class" in value:
        ...
        out["xavcClass"] = (
            aws_sdk_mediaconvert.types.xavc4k_intra_vbr_profile_class.serialize_json(
                value["xavc_class"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> Xavc4kIntraVbrProfileSettings:
    ...
    if "xavcClass" in data:
        ...
        out["xavc_class"] = (
            aws_sdk_mediaconvert.types.xavc4k_intra_vbr_profile_class.deserialize_json(
                data["xavcClass"]
            )
        )
    return out
