...
if TYPE_CHECKING:
    import aws_sdk_mediaconvert.types.automated_abr_settings


class AutomatedEncodingSettings(TypedDict):
    abr_settings: NotRequired[
        "aws_sdk_mediaconvert.types.automated_abr_settings.AutomatedAbrSettings"
    ]
...
def serialize_json(value: AutomatedEncodingSettings) -> dict:
    ...
    if "abr_settings" in value:
        ...
        out["abrSettings"] = (
            aws_sdk_mediaconvert.types.automated_abr_settings.serialize_json(
                value["abr_settings"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AutomatedEncodingSettings:
    ...
    if "abrSettings" in data:
        ...
        out["abr_settings"] = (
            aws_sdk_mediaconvert.types.automated_abr_settings.deserialize_json(
                data["abrSettings"]
            )
        )
    return out
