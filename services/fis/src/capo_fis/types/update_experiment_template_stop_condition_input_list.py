...
if TYPE_CHECKING:
    import capo_fis.types.update_experiment_template_stop_condition_input

UpdateExperimentTemplateStopConditionInputList: TypeAlias = list[
    "capo_fis.types.update_experiment_template_stop_condition_input.UpdateExperimentTemplateStopConditionInput"
]
...
def serialize_json(value: UpdateExperimentTemplateStopConditionInputList) -> list:
    ...
    for item in value:
        out.append(
            capo_fis.types.update_experiment_template_stop_condition_input.serialize_json(
                item
            )
        )
    return out
