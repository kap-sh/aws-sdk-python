...
class ExperimentTemplateTarget(TypedDict, closed=True):
    ...
    filters: NotRequired[
        "capo_fis.types.experiment_template_target_filter_list.ExperimentTemplateTargetFilterList"
    ]
    """<p>The filters to apply to identify target resources using specific attributes.</p>"""
    selection_mode: NotRequired[
        "capo_fis.types.experiment_template_target_selection_mode.ExperimentTemplateTargetSelectionMode"
    ]
...
def serialize_json(value: ExperimentTemplateTarget) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_fis.types.experiment_template_target_filter_list.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ExperimentTemplateTarget:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            capo_fis.types.experiment_template_target_filter_list.deserialize_json(
                data["filters"]
            )
        )
