...
class fisError(Exception):
    """Base class for all fis service errors."""


class ServiceError(fisError):
...
class SerializationError(fisError):
    """Raised when a value cannot be serialized."""


class DeserializationError(fisError):
...
class WaiterFailedError(fisError):
...
class WaiterTimeoutError(fisError):
