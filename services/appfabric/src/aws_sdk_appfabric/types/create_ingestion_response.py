...
if TYPE_CHECKING:
    import aws_sdk_appfabric.types.ingestion


class CreateIngestionResponse(TypedDict, closed=True):
    ingestion: "aws_sdk_appfabric.types.ingestion.Ingestion"
...
def serialize_json(value: CreateIngestionResponse) -> dict:
    ...
    out["ingestion"] = aws_sdk_appfabric.types.ingestion.serialize_json(
        value["ingestion"]
    )
    return out
...
def deserialize_json(data: dict) -> CreateIngestionResponse:
    ...
    if "ingestion" in data:
        ...
        out["ingestion"] = aws_sdk_appfabric.types.ingestion.deserialize_json(
            data["ingestion"]
        )
    else:
