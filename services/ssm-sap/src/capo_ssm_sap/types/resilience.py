...
class Resilience(TypedDict, closed=True):
    ...
    hsr_replication_mode: NotRequired[
        "capo_ssm_sap.types.replication_mode.ReplicationMode"
    ]
    """<p>The replication mode of the component.</p>"""
    hsr_operation_mode: NotRequired["capo_ssm_sap.types.operation_mode.OperationMode"]
    """<p>The operation mode of the component.</p>"""
    cluster_status: NotRequired["capo_ssm_sap.types.cluster_status.ClusterStatus"]
...
def serialize_json(value: Resilience) -> dict:
    ...
    if "hsr_replication_mode" in value:
        ...
        out["HsrReplicationMode"] = capo_ssm_sap.types.replication_mode.serialize_json(
            value["hsr_replication_mode"]
        )
    if "hsr_operation_mode" in value:
        import capo_ssm_sap.types.operation_mode

        out["HsrOperationMode"] = capo_ssm_sap.types.operation_mode.serialize_json(
            value["hsr_operation_mode"]
        )
    if "cluster_status" in value:
        import capo_ssm_sap.types.cluster_status

        out["ClusterStatus"] = capo_ssm_sap.types.cluster_status.serialize_json(
...
def deserialize_json(data: dict) -> Resilience:
    ...
    if "HsrReplicationMode" in data:
        ...
        out["hsr_replication_mode"] = (
            capo_ssm_sap.types.replication_mode.deserialize_json(
