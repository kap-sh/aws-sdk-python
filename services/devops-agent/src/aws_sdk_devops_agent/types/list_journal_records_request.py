...
class ListJournalRecordsRequest(TypedDict):
    ...
    order: "aws_sdk_devops_agent.types.order_type.OrderType"
...
def serialize_json(value: ListJournalRecordsRequest) -> dict:
    ...
    out["order"] = aws_sdk_devops_agent.types.order_type.serialize_json(
...
def deserialize_json(data: dict) -> ListJournalRecordsRequest:
    ...
    if "order" in data:
        ...
        out["order"] = aws_sdk_devops_agent.types.order_type.deserialize_json(
            data["order"]
        )
    else:
        out["order"] = "DESC"
