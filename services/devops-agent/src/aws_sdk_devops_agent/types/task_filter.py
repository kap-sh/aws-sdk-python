...
if TYPE_CHECKING:
    ...
    import aws_sdk_devops_agent.types.priority_list
    import aws_sdk_devops_agent.types.resource_id
    import aws_sdk_devops_agent.types.task_status_list
...
class TaskFilter(TypedDict):
    ...
    priority: NotRequired["aws_sdk_devops_agent.types.priority_list.PriorityList"]
    """<p>Filter by priority (single value only)</p>"""
    status: NotRequired["aws_sdk_devops_agent.types.task_status_list.TaskStatusList"]
    """<p>Filter by status (single value only)</p>"""
    task_type: NotRequired["aws_sdk_devops_agent.types.task_type_list.TaskTypeList"]
    """<p>Filter by task type (single value only)</p>"""
    primary_task_id: NotRequired["aws_sdk_devops_agent.types.resource_id.ResourceId"]
...
def serialize_json(value: TaskFilter) -> dict:
    ...
    if "priority" in value:
        ...
        out["priority"] = aws_sdk_devops_agent.types.priority_list.serialize_json(
            value["priority"]
        )
    if "status" in value:
        import aws_sdk_devops_agent.types.task_status_list

        out["status"] = aws_sdk_devops_agent.types.task_status_list.serialize_json(
            value["status"]
        )
    if "task_type" in value:
        import aws_sdk_devops_agent.types.task_type_list

        out["taskType"] = aws_sdk_devops_agent.types.task_type_list.serialize_json(
            value["task_type"]
        )
...
def deserialize_json(data: dict) -> TaskFilter:
    ...
    if "priority" in data:
        ...
        out["priority"] = aws_sdk_devops_agent.types.priority_list.deserialize_json(
            data["priority"]
        )
    if "status" in data:
        import aws_sdk_devops_agent.types.task_status_list

        out["status"] = aws_sdk_devops_agent.types.task_status_list.deserialize_json(
            data["status"]
        )
    if "taskType" in data:
        import aws_sdk_devops_agent.types.task_type_list

        out["task_type"] = aws_sdk_devops_agent.types.task_type_list.deserialize_json(
            data["taskType"]
        )
