...
def serialize_json(value: PendingMessage) -> dict:
    ...
    out["message"] = aws_sdk_devops_agent.types.message.serialize_json(value["message"])
    return out
...
def deserialize_json(data: dict) -> PendingMessage:
    ...
    if "message" in data:
        ...
        out["message"] = aws_sdk_devops_agent.types.message.deserialize_json(
            data["message"]
        )
    else:
