...
class ListExecutionsResponse(TypedDict, closed=True):
    executions: "aws_sdk_devops_agent.types.execution_list.ExecutionList"
...
def serialize_json(value: ListExecutionsResponse) -> dict:
    ...
    out["executions"] = aws_sdk_devops_agent.types.execution_list.serialize_json(
        value["executions"]
    )
...
def deserialize_json(data: dict) -> ListExecutionsResponse:
    ...
    if "executions" in data:
        ...
        out["executions"] = aws_sdk_devops_agent.types.execution_list.deserialize_json(
            data["executions"]
        )
    else:
