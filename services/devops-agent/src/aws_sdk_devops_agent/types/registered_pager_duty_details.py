...
if TYPE_CHECKING:
    import aws_sdk_devops_agent.types.pager_duty_scopes_list


class RegisteredPagerDutyDetails(TypedDict):
    scopes: "aws_sdk_devops_agent.types.pager_duty_scopes_list.PagerDutyScopesList"
...
def serialize_json(value: RegisteredPagerDutyDetails) -> dict:
    ...
    out["scopes"] = aws_sdk_devops_agent.types.pager_duty_scopes_list.serialize_json(
        value["scopes"]
    )
    return out
...
def deserialize_json(data: dict) -> RegisteredPagerDutyDetails:
    ...
    if "scopes" in data:
        ...
        out["scopes"] = (
            aws_sdk_devops_agent.types.pager_duty_scopes_list.deserialize_json(
                data["scopes"]
            )
        )
    else:
