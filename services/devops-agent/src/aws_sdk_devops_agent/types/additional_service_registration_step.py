...
def serialize_json(value: AdditionalServiceRegistrationStep) -> dict:
    if "oauth" in value:
        ...
        return {
            "oauth": aws_sdk_devops_agent.types.o_auth_additional_step_details.serialize_json(
                value["oauth"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> AdditionalServiceRegistrationStep:
    if "oauth" in data:
        ...
        return {
            "oauth": aws_sdk_devops_agent.types.o_auth_additional_step_details.deserialize_json(
                data["oauth"]
            )
        }
    else:
