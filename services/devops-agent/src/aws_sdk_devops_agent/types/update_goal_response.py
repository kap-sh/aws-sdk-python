...
class UpdateGoalResponse(TypedDict):
    goal: "aws_sdk_devops_agent.types.goal.Goal"
...
def serialize_json(value: UpdateGoalResponse) -> dict:
    ...
    out["goal"] = aws_sdk_devops_agent.types.goal.serialize_json(value["goal"])
    return out
...
def deserialize_json(data: dict) -> UpdateGoalResponse:
    ...
    if "goal" in data:
        ...
        out["goal"] = aws_sdk_devops_agent.types.goal.deserialize_json(data["goal"])
    else:
