...
class ListRecommendationsRequest(TypedDict, closed=True):
    ...
    task_id: NotRequired["aws_sdk_devops_agent.types.resource_id.ResourceId"]
    """<p>Optional task ID to filter recommendations by specific task</p>"""
    goal_id: NotRequired["aws_sdk_devops_agent.types.resource_id.ResourceId"]
...
def serialize_json(value: ListRecommendationsRequest) -> dict:
    ...
    if "priority" in value:
        ...
        out["priority"] = (
            aws_sdk_devops_agent.types.recommendation_priority.serialize_json(
                value["priority"]
            )
        )
