"""Generated from Smithy shape ``com.amazonaws.devopsagent#ListExecutions``."""
...
import aws_sdk_devops_agent.types.execution_list
...
def handle_response(
    ...
    out: aws_sdk_devops_agent.types.list_executions_response.ListExecutionsResponse = (
        aws_sdk_devops_agent.types.list_executions_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_devops_agent.types.list_executions_request.ListExecutionsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/journal/agent-space/{agentSpaceId}/executions"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_devops_agent.types.list_executions_request.serialize_json(input_)
    ).encode()
