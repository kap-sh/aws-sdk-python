"""Generated from Smithy shape ``com.amazonaws.devopsagent#AssociateService``."""
...
import aws_sdk_devops_agent.types.associate_service_input
import aws_sdk_devops_agent.types.associate_service_output
...
) -> aws_sdk_devops_agent.types.associate_service_output.AssociateServiceOutput:
    out: aws_sdk_devops_agent.types.associate_service_output.AssociateServiceOutput = (
        aws_sdk_devops_agent.types.associate_service_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_devops_agent.types.associate_service_input.AssociateServiceInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/agentspaces/{agentSpaceId}/associations"
    url = url.replace("{agentSpaceId}", quote(str(input_["agent_space_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_devops_agent.types.associate_service_input.serialize_json(input_)
    ).encode()
...
async def async_associate_service(
