"""Generated from Smithy shape ``com.amazonaws.devopsagent#DeregisterService``."""
...
def handle_response(
    ...
    out: aws_sdk_devops_agent.types.deregister_service_output.DeregisterServiceOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_devops_agent.types.deregister_service_input.DeregisterServiceInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/services/{serviceId}"
    url = url.replace("{serviceId}", quote(str(input_["service_id"]), safe=""))
    params: dict[str, str] = {}
...
async def async_deregister_service(
