"""Generated from Smithy shape ``com.amazonaws.devopsagent#CreateBacklogTask``."""
...
def handle_response(
    ...
    out: aws_sdk_devops_agent.types.create_backlog_task_response.CreateBacklogTaskResponse = aws_sdk_devops_agent.types.create_backlog_task_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_devops_agent.types.create_backlog_task_request.CreateBacklogTaskRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/backlog/agent-space/{agentSpaceId}/tasks"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_devops_agent.types.create_backlog_task_request.serialize_json(input_)
    ).encode()
...
async def async_create_backlog_task(
