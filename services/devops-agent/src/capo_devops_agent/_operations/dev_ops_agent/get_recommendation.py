"""Generated from Smithy shape ``com.amazonaws.devopsagent#GetRecommendation``."""
...
def handle_response(
    ...
    out: capo_devops_agent.types.get_recommendation_response.GetRecommendationResponse = capo_devops_agent.types.get_recommendation_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{recommendationId}", quote(str(input_["recommendation_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "recommendation_version" in input_:
        params["recommendationVersion"] = str(input_["recommendation_version"])
...
async def async_get_recommendation(
