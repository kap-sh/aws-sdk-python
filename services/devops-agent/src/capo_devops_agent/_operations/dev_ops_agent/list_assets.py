"""Generated from Smithy shape ``com.amazonaws.devopsagent#ListAssets``."""
...
import capo_devops_agent.types.asset_list
...
def handle_response(
    ...
    out: capo_devops_agent.types.list_assets_response.ListAssetsResponse = (
        capo_devops_agent.types.list_assets_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/asset/agent-space/{agentSpaceId}/assets"
    ...
    if "updated_after" in input_:
        params["updatedAfter"] = str(input_["updated_after"])
    if "updated_before" in input_:
        params["updatedBefore"] = str(input_["updated_before"])
...
def list_assets(
    ...
    capo_devops_agent.types.list_assets_response.ListAssetsResponse, zapros.Response
]:
