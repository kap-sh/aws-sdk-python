"""Generated from Smithy shape ``com.amazonaws.devopsagent#UpdateGoal``."""
...
def handle_response(
    ...
    out: capo_devops_agent.types.update_goal_response.UpdateGoalResponse = (
        capo_devops_agent.types.update_goal_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/backlog/agent-space/{agentSpaceId}/goals/{goalId}"
    )
    url = url.replace("{agentSpaceId}", quote(str(input_["agent_space_id"]), safe=""))
    url = url.replace("{goalId}", quote(str(input_["goal_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_devops_agent.types.update_goal_request.serialize_json(input_)
    ).encode()
...
def update_goal(
    ...
    capo_devops_agent.types.update_goal_response.UpdateGoalResponse, zapros.Response
]:
