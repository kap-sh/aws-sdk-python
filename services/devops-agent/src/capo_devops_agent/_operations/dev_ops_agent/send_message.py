...
import capo_devops_agent.types.send_message_events
...
from capo_devops_agent._protocol.eventstream import (
    MessageDecoder,
...
def handle_response(
    ...
    _union_deser = capo_devops_agent.types.send_message_events.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: capo_devops_agent.types.send_message_response.SendMessageResponse = {
        "events": cast(Any, raw_stream_to_events(_iter, _message_decoder, _union_deser))
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/agents/agent-space/{agentSpaceId}/chat/sendMessage"
    )
    url = url.replace("{agentSpaceId}", quote(str(input_["agent_space_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_devops_agent.types.send_message_request.serialize_json(input_)
    ).encode()
...
def send_message(
    ...
    capo_devops_agent.types.send_message_response.SendMessageResponse, zapros.Response
]:
