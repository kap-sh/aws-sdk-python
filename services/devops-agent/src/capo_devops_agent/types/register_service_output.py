...
class RegisterServiceOutput(TypedDict, closed=True):
    service_id: NotRequired["capo_devops_agent.types.service_id.ServiceId"]
    ...
    additional_step: NotRequired[
        "capo_devops_agent.types.additional_service_registration_step.AdditionalServiceRegistrationStep"
    ]
...
def serialize_json(value: RegisterServiceOutput) -> dict:
    ...
    if "additional_step" in value:
        ...
        out["additionalStep"] = (
            capo_devops_agent.types.additional_service_registration_step.serialize_json(
...
def deserialize_json(data: dict) -> RegisterServiceOutput:
    ...
    if "additionalStep" in data:
        ...
        out["additional_step"] = (
            capo_devops_agent.types.additional_service_registration_step.deserialize_json(
                data["additionalStep"]
            )
        )
