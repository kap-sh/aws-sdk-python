...
AssetFileSummaryList: TypeAlias = list[
    "capo_devops_agent.types.asset_file_summary.AssetFileSummary"
]
...
def serialize_json(value: AssetFileSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_devops_agent.types.asset_file_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> AssetFileSummaryList:
    ...
    for item in data:
        out.append(capo_devops_agent.types.asset_file_summary.deserialize_json(item))
    return out
