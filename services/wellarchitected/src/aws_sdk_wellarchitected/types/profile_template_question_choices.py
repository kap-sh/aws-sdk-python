...
if TYPE_CHECKING:
    import aws_sdk_wellarchitected.types.profile_template_choice

ProfileTemplateQuestionChoices: TypeAlias = list[
    "aws_sdk_wellarchitected.types.profile_template_choice.ProfileTemplateChoice"
]
...
def serialize_json(value: ProfileTemplateQuestionChoices) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_wellarchitected.types.profile_template_choice.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ProfileTemplateQuestionChoices:
    ...
    for item in data:
        out.append(
            aws_sdk_wellarchitected.types.profile_template_choice.deserialize_json(item)
        )
    return out
