"""Generated from Smithy shape ``com.amazonaws.wellarchitected#CreateLensShare``."""
...
def handle_response(
    ...
    out: aws_sdk_wellarchitected.types.create_lens_share_output.CreateLensShareOutput = aws_sdk_wellarchitected.types.create_lens_share_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_wellarchitected.types.create_lens_share_input.CreateLensShareInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/lenses/{LensAlias}/shares"
    url = url.replace("{LensAlias}", quote(str(input_["lens_alias"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_wellarchitected.types.create_lens_share_input.serialize_json(input_)
    ).encode()
...
async def async_create_lens_share(
