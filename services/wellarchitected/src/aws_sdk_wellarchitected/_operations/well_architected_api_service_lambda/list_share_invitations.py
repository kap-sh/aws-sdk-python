...
def build_request(
    ...
    input: aws_sdk_wellarchitected.types.list_share_invitations_input.ListShareInvitationsInput,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "workload_name_prefix" in input:
        params["WorkloadNamePrefix"] = str(input["workload_name_prefix"])
    if "lens_name_prefix" in input:
        params["LensNamePrefix"] = str(input["lens_name_prefix"])
    if "share_resource_type" in input:
        params["ShareResourceType"] = str(input["share_resource_type"])
    if "next_token" in input:
        params["NextToken"] = str(input["next_token"])
    if "max_results" in input:
        params["MaxResults"] = str(input["max_results"])
    if "profile_name_prefix" in input:
        params["ProfileNamePrefix"] = str(input["profile_name_prefix"])
    if "template_name_prefix" in input:
        params["TemplateNamePrefix"] = str(input["template_name_prefix"])
