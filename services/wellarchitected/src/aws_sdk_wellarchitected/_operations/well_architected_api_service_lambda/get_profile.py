"""Generated from Smithy shape ``com.amazonaws.wellarchitected#GetProfile``."""
...
def handle_response(
    ...
    out: aws_sdk_wellarchitected.types.get_profile_output.GetProfileOutput = (
        aws_sdk_wellarchitected.types.get_profile_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_wellarchitected.types.get_profile_input.GetProfileInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/profiles/{ProfileArn}"
    url = url.replace("{ProfileArn}", quote(str(input_["profile_arn"]), safe=""))
    params: dict[str, str] = {}
    if "profile_version" in input_:
        params["ProfileVersion"] = str(input_["profile_version"])
...
def get_profile(
    ...
    aws_sdk_wellarchitected.types.get_profile_output.GetProfileOutput, zapros.Response
]:
