"""Generated from Smithy shape ``com.amazonaws.wellarchitected#ListTemplateShares``."""
...
import aws_sdk_wellarchitected.types.template_share_summaries
...
def handle_response(
    ...
    out: aws_sdk_wellarchitected.types.list_template_shares_output.ListTemplateSharesOutput = aws_sdk_wellarchitected.types.list_template_shares_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_wellarchitected.types.list_template_shares_input.ListTemplateSharesInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/templates/shares/{TemplateArn}"
    url = url.replace("{TemplateArn}", quote(str(input_["template_arn"]), safe=""))
    params: dict[str, str] = {}
    if "shared_with_prefix" in input_:
        params["SharedWithPrefix"] = str(input_["shared_with_prefix"])
...
async def async_list_template_shares(
