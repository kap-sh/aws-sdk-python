"""Generated from Smithy shape ``com.amazonaws.wellarchitected#ListLensReviews``."""
...
def handle_response(
    ...
    out: aws_sdk_wellarchitected.types.list_lens_reviews_output.ListLensReviewsOutput = aws_sdk_wellarchitected.types.list_lens_reviews_output.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_wellarchitected.types.list_lens_reviews_input.ListLensReviewsInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/workloads/{WorkloadId}/lensReviews"
    url = url.replace("{WorkloadId}", quote(str(input["workload_id"]), safe=""))
    params: dict[str, str] = {}
    if "milestone_number" in input:
        params["MilestoneNumber"] = str(input["milestone_number"])
...
async def async_list_lens_reviews(
