"""Generated from Smithy shape ``com.amazonaws.wellarchitected#CreateWorkloadShare``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            import aws_sdk_wellarchitected.errors.resource_not_found_exception

            raise aws_sdk_wellarchitected.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
        case "ServiceQuotaExceededException":
            import aws_sdk_wellarchitected.errors.service_quota_exceeded_exception

            raise aws_sdk_wellarchitected.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_wellarchitected.types.create_workload_share_output.CreateWorkloadShareOutput = aws_sdk_wellarchitected.types.create_workload_share_output.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_wellarchitected._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_wellarchitected._auth._sigv4.build_sigv4_auth_scheme(
                "wellarchitected", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_wellarchitected._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input: aws_sdk_wellarchitected.types.create_workload_share_input.CreateWorkloadShareInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/workloads/{WorkloadId}/shares"
    url = url.replace("{WorkloadId}", quote(str(input["workload_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_wellarchitected.types.create_workload_share_input

    body: bytes | None = json.dumps(
        aws_sdk_wellarchitected.types.create_workload_share_input.serialize_json(input)
    ).encode()
...
async def async_create_workload_share(
