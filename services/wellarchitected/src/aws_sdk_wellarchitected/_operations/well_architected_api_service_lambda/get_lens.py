"""Generated from Smithy shape ``com.amazonaws.wellarchitected#GetLens``."""
...
def handle_response(
    ...
    out: aws_sdk_wellarchitected.types.get_lens_output.GetLensOutput = (
        aws_sdk_wellarchitected.types.get_lens_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_wellarchitected.types.get_lens_input.GetLensInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/lenses/{LensAlias}"
    url = url.replace("{LensAlias}", quote(str(input_["lens_alias"]), safe=""))
    params: dict[str, str] = {}
    if "lens_version" in input_:
        params["LensVersion"] = str(input_["lens_version"])
...
def get_lens(
    ...
    aws_sdk_wellarchitected.types.get_lens_output.GetLensOutput, zapros.Response
]:
