"""Generated from Smithy shape ``com.amazonaws.wellarchitected#CreateProfileShare``."""
...
import capo_wellarchitected.errors.conflict_exception
...
import capo_wellarchitected.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_wellarchitected.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ServiceQuotaExceededException":
            raise capo_wellarchitected.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_wellarchitected.types.create_profile_share_output.CreateProfileShareOutput = capo_wellarchitected.types.create_profile_share_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/profiles/{ProfileArn}/shares"
    url = url.replace("{ProfileArn}", quote(str(input_["profile_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.create_profile_share_input.serialize_json(input_)
    ).encode()
...
async def async_create_profile_share(
