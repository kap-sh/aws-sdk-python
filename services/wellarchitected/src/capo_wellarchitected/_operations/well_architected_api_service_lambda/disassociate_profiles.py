"""Generated from Smithy shape ``com.amazonaws.wellarchitected#DisassociateProfiles``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workloads/{WorkloadId}/disassociateProfiles"
    url = url.replace("{WorkloadId}", quote(str(input_["workload_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.disassociate_profiles_input.serialize_json(input_)
    ).encode()
...
async def async_disassociate_profiles(
