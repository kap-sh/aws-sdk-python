"""Generated from Smithy shape ``com.amazonaws.wellarchitected#DisassociateLenses``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workloads/{WorkloadId}/disassociateLenses"
    url = url.replace("{WorkloadId}", quote(str(input_["workload_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.disassociate_lenses_input.serialize_json(input_)
    ).encode()
...
async def async_disassociate_lenses(
