"""Generated from Smithy shape ``com.amazonaws.wellarchitected#ListCheckSummaries``."""
...
def handle_response(
    ...
    out: capo_wellarchitected.types.list_check_summaries_output.ListCheckSummariesOutput = capo_wellarchitected.types.list_check_summaries_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workloads/{WorkloadId}/checkSummaries"
    url = url.replace("{WorkloadId}", quote(str(input_["workload_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wellarchitected.types.list_check_summaries_input.serialize_json(input_)
    ).encode()
...
async def async_list_check_summaries(
