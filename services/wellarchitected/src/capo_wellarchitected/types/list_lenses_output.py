...
class ListLensesOutput(TypedDict, closed=True):
    lens_summaries: NotRequired[
        "capo_wellarchitected.types.lens_summaries.LensSummaries"
    ]
...
def serialize_json(value: ListLensesOutput) -> dict:
    ...
    if "lens_summaries" in value:
        ...
        out["LensSummaries"] = capo_wellarchitected.types.lens_summaries.serialize_json(
...
def deserialize_json(data: dict) -> ListLensesOutput:
    ...
    if "LensSummaries" in data:
        ...
        out["lens_summaries"] = (
            capo_wellarchitected.types.lens_summaries.deserialize_json(
                data["LensSummaries"]
            )
        )
