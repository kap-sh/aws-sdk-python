...
class GetLensOutput(TypedDict, closed=True):
    lens: NotRequired["capo_wellarchitected.types.lens.Lens"]
...
def serialize_json(value: GetLensOutput) -> dict:
    ...
    if "lens" in value:
        ...
        out["Lens"] = capo_wellarchitected.types.lens.serialize_json(value["lens"])
    return out
...
def deserialize_json(data: dict) -> GetLensOutput:
    ...
    if "Lens" in data:
        ...
        out["lens"] = capo_wellarchitected.types.lens.deserialize_json(data["Lens"])
    return out
