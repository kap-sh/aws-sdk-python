...
class drsError(Exception):
    """Base class for all drs service errors."""
...
class SerializationError(drsError):
    """Raised when a value cannot be serialized."""


class DeserializationError(drsError):
...
class WaiterFailedError(drsError):
...
class WaiterTimeoutError(drsError):
