...
class LifeCycleLastLaunch(TypedDict):
    initiated: NotRequired[
        "aws_sdk_drs.types.life_cycle_last_launch_initiated.LifeCycleLastLaunchInitiated"
    ]
...
def serialize_json(value: LifeCycleLastLaunch) -> dict:
    ...
    if "initiated" in value:
        ...
        out["initiated"] = (
            aws_sdk_drs.types.life_cycle_last_launch_initiated.serialize_json(
                value["initiated"]
            )
        )
    if "status" in value:
...
def deserialize_json(data: dict) -> LifeCycleLastLaunch:
    ...
    if "initiated" in data:
        ...
        out["initiated"] = (
            aws_sdk_drs.types.life_cycle_last_launch_initiated.deserialize_json(
                data["initiated"]
            )
        )
    if "status" in data:
