...
def deserialize_json(data: dict) -> AssociateSourceNetworkStackRequest:
    ...
    if "sourceNetworkID" in data:
        ...
        raise DeserializationError("AssociateSourceNetworkStackRequest.source_network_id required")
    if "cfnStackName" in data:
        out["cfn_stack_name"] = data["cfnStackName"]
    else:
        raise DeserializationError("AssociateSourceNetworkStackRequest.cfn_stack_name required")
    return out