"""Generated from Smithy shape ``com.amazonaws.drs#UpdateFailbackReplicationConfigurationRequest``."""
...
class UpdateFailbackReplicationConfigurationRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateFailbackReplicationConfigurationRequest) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> UpdateFailbackReplicationConfigurationRequest:
    out: UpdateFailbackReplicationConfigurationRequest = {}  # type: ignore[typeddict-item]
    if "recoveryInstanceID" in data:
        out["recovery_instance_id"] = data["recoveryInstanceID"]
    else:
        raise DeserializationError(
            "UpdateFailbackReplicationConfigurationRequest.recovery_instance_id required"
        )
    if "name" in data:
        out["name"] = data["name"]
