...
class DescribeSourceServersRequestFilters(TypedDict):
    source_server_i_ds: NotRequired["aws_sdk_drs.types.describe_source_servers_request_filters_i_ds.DescribeSourceServersRequestFiltersIDs"]
...
def serialize_json(value: DescribeSourceServersRequestFilters) -> dict:
    ...
    if "source_server_i_ds" in value:
        import aws_sdk_drs.types.describe_source_servers_request_filters_i_ds
        out["sourceServerIDs"] = aws_sdk_drs.types.describe_source_servers_request_filters_i_ds.serialize_json(value["source_server_i_ds"])
    if "hardware_id" in value:
        out["hardwareId"] = value["hardware_id"]
    if "staging_account_i_ds" in value:
        import aws_sdk_drs.types.account_i_ds
        out["stagingAccountIDs"] = aws_sdk_drs.types.account_i_ds.serialize_json(value["staging_account_i_ds"])
    return out
...
def deserialize_json(data: dict) -> DescribeSourceServersRequestFilters:
    ...
    if "sourceServerIDs" in data:
        import aws_sdk_drs.types.describe_source_servers_request_filters_i_ds
        out["source_server_i_ds"] = aws_sdk_drs.types.describe_source_servers_request_filters_i_ds.deserialize_json(data["sourceServerIDs"])
    if "hardwareId" in data:
        out["hardware_id"] = data["hardwareId"]
    if "stagingAccountIDs" in data:
        import aws_sdk_drs.types.account_i_ds
        out["staging_account_i_ds"] = aws_sdk_drs.types.account_i_ds.deserialize_json(data["stagingAccountIDs"])
    return out