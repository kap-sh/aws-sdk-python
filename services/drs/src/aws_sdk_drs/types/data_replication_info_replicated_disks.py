...
DataReplicationInfoReplicatedDisks: TypeAlias = list[
    "aws_sdk_drs.types.data_replication_info_replicated_disk.DataReplicationInfoReplicatedDisk"
]
...
def serialize_json(value: DataReplicationInfoReplicatedDisks) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_drs.types.data_replication_info_replicated_disk.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> DataReplicationInfoReplicatedDisks:
    ...
    for item in data:
        out.append(
            aws_sdk_drs.types.data_replication_info_replicated_disk.deserialize_json(
                item
            )
        )
    return out
