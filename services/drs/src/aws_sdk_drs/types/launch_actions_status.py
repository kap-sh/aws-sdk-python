...
class LaunchActionsStatus(TypedDict):
    ssm_agent_discovery_datetime: NotRequired["aws_sdk_drs.types.iso8601_datetime_string.ISO8601DatetimeString"]
...
def deserialize_json(data: dict) -> LaunchActionsStatus:
    ...
    if "runs" in data:
        import aws_sdk_drs.types.launch_action_runs
        out["runs"] = aws_sdk_drs.types.launch_action_runs.deserialize_json(data["runs"])
    return out