...
class LaunchActionsRequestFilters(TypedDict):
...
def serialize_json(value: LaunchActionsRequestFilters) -> dict:
    ...
    if "action_ids" in value:
        import aws_sdk_drs.types.launch_action_ids
        out["actionIds"] = aws_sdk_drs.types.launch_action_ids.serialize_json(value["action_ids"])
    return out
...
def deserialize_json(data: dict) -> LaunchActionsRequestFilters:
    ...
    if "actionIds" in data:
        import aws_sdk_drs.types.launch_action_ids
        out["action_ids"] = aws_sdk_drs.types.launch_action_ids.deserialize_json(data["actionIds"])
    return out