...
class DescribeRecoverySnapshotsResponse(TypedDict, closed=True):
    items: NotRequired[
        "aws_sdk_drs.types.recovery_snapshots_list.RecoverySnapshotsList"
    ]
...
def serialize_json(value: DescribeRecoverySnapshotsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = aws_sdk_drs.types.recovery_snapshots_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> DescribeRecoverySnapshotsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = aws_sdk_drs.types.recovery_snapshots_list.deserialize_json(
            data["items"]
        )
