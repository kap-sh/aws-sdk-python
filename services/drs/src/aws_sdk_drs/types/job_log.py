...
class JobLog(TypedDict):
    log_date_time: NotRequired["aws_sdk_drs.types.iso8601_datetime_string.ISO8601DatetimeString"]
...
def serialize_json(value: JobLog) -> dict:
    ...
    if "event_data" in value:
        import aws_sdk_drs.types.job_log_event_data
        out["eventData"] = aws_sdk_drs.types.job_log_event_data.serialize_json(value["event_data"])
    return out
...
def deserialize_json(data: dict) -> JobLog:
    ...
    if "eventData" in data:
        import aws_sdk_drs.types.job_log_event_data
        out["event_data"] = aws_sdk_drs.types.job_log_event_data.deserialize_json(data["eventData"])
    return out