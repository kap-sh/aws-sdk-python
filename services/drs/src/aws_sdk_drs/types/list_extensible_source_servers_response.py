...
class ListExtensibleSourceServersResponse(TypedDict):
    items: NotRequired["aws_sdk_drs.types.staging_source_servers_list.StagingSourceServersList"]
...
def serialize_json(value: ListExtensibleSourceServersResponse) -> dict:
    ...
    if "items" in value:
        import aws_sdk_drs.types.staging_source_servers_list
        out["items"] = aws_sdk_drs.types.staging_source_servers_list.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListExtensibleSourceServersResponse:
    ...
    if "items" in data:
        import aws_sdk_drs.types.staging_source_servers_list
        out["items"] = aws_sdk_drs.types.staging_source_servers_list.deserialize_json(data["items"])
