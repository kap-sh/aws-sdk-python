...
class DescribeSourceServersResponse(TypedDict):
    items: NotRequired["aws_sdk_drs.types.source_servers_list.SourceServersList"]
...
def serialize_json(value: DescribeSourceServersResponse) -> dict:
    ...
    if "items" in value:
        import aws_sdk_drs.types.source_servers_list
        out["items"] = aws_sdk_drs.types.source_servers_list.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> DescribeSourceServersResponse:
    ...
    if "items" in data:
        import aws_sdk_drs.types.source_servers_list
        out["items"] = aws_sdk_drs.types.source_servers_list.deserialize_json(data["items"])
