...
class ListLaunchActionsResponse(TypedDict, closed=True):
    items: NotRequired["capo_drs.types.launch_actions.LaunchActions"]
...
def serialize_json(value: ListLaunchActionsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_drs.types.launch_actions.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListLaunchActionsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_drs.types.launch_actions.deserialize_json(data["items"])
