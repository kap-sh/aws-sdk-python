...
SourceServersList: TypeAlias = list["capo_drs.types.source_server.SourceServer"]
...
def serialize_json(value: SourceServersList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_drs.types.source_server.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> SourceServersList:
    ...
    for item in data:
        out.append(capo_drs.types.source_server.deserialize_json(item))
    return out
