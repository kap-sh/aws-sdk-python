...
class DataReplicationInitiation(TypedDict, closed=True):
    ...
    steps: NotRequired[
        "capo_drs.types.data_replication_initiation_steps.DataReplicationInitiationSteps"
    ]
...
def serialize_json(value: DataReplicationInitiation) -> dict:
    ...
    if "steps" in value:
        ...
        out["steps"] = capo_drs.types.data_replication_initiation_steps.serialize_json(
            value["steps"]
        )
    return out
...
def deserialize_json(data: dict) -> DataReplicationInitiation:
    ...
    if "steps" in data:
        ...
        out["steps"] = (
            capo_drs.types.data_replication_initiation_steps.deserialize_json(
                data["steps"]
            )
        )
    return out
