...
if TYPE_CHECKING:
    import capo_drs.types.replication_configuration_replicated_disk

ReplicationConfigurationReplicatedDisks: TypeAlias = list[
    "capo_drs.types.replication_configuration_replicated_disk.ReplicationConfigurationReplicatedDisk"
]
...
def serialize_json(value: ReplicationConfigurationReplicatedDisks) -> list:
    ...
    for item in value:
        out.append(
            capo_drs.types.replication_configuration_replicated_disk.serialize_json(
                item
            )
        )
    return out
