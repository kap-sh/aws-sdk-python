"""Generated from Smithy shape ``com.amazonaws.neptune#ModifyGlobalClusterResult``."""
...
class ModifyGlobalClusterResult(TypedDict, closed=True):
    global_cluster: NotRequired["capo_neptune.types.global_cluster.GlobalCluster"]


# --- awsQuery ser/de ---
def serialize_query(
    value: ModifyGlobalClusterResult, pairs: list[tuple[str, str]], prefix: str
) -> None:
...
def deserialize_query(el: Element) -> ModifyGlobalClusterResult:
    out: ModifyGlobalClusterResult = {}  # type: ignore[typeddict-item]
