...
if TYPE_CHECKING:
    import capo_neptune.types.db_cluster_member

DBClusterMemberList: TypeAlias = list[
    "capo_neptune.types.db_cluster_member.DBClusterMember"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_neptune.types.db_cluster_member.serialize_query(
...
def deserialize_query(el: Element) -> DBClusterMemberList:
    ...
    for child in el.findall("DBClusterMember"):
        out.append(capo_neptune.types.db_cluster_member.deserialize_query(child))
    return out
