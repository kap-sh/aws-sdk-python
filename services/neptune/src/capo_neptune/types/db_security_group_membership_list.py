...
if TYPE_CHECKING:
    import capo_neptune.types.db_security_group_membership

DBSecurityGroupMembershipList: TypeAlias = list[
    "capo_neptune.types.db_security_group_membership.DBSecurityGroupMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_neptune.types.db_security_group_membership.serialize_query(
...
def deserialize_query(el: Element) -> DBSecurityGroupMembershipList:
    ...
    for child in el.findall("DBSecurityGroup"):
        out.append(
            capo_neptune.types.db_security_group_membership.deserialize_query(child)
        )
    return out
