...
class ModifyDBClusterMessage(TypedDict, closed=True):
    ...
    new_db_cluster_identifier: NotRequired["capo_neptune.types.string.String"]
    ...
    r"""<p>The configuration setting for the log types to be enabled for export to CloudWatch Logs for a specific DB cluster. See <a href=\"https://docs.aws.amazon.com/neptune/latest/userguide/cloudwatch-logs.html#cloudwatch-logs-cli\">Using the CLI to publish Neptune audit logs to CloudWatch Logs</a>.</p>"""
    ...
    db_instance_parameter_group_name: NotRequired["capo_neptune.types.string.String"]
...
def serialize_query(
    ...
    if "db_instance_parameter_group_name" in value:
        pairs.append(
            (
                f"{key_prefix}DBInstanceParameterGroupName",
