"""Generated from Smithy shape ``com.amazonaws.neptune#RemoveSourceIdentifierFromSubscription``."""
...
import capo_neptune.errors.source_not_found_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "SourceNotFoundFault":
            raise capo_neptune.errors.source_not_found_fault.SourceNotFoundFault.from_query(
                root
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("RemoveSourceIdentifierFromSubscriptionResult")
    out: capo_neptune.types.remove_source_identifier_from_subscription_result.RemoveSourceIdentifierFromSubscriptionResult = capo_neptune.types.remove_source_identifier_from_subscription_result.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "RemoveSourceIdentifierFromSubscription"))
    pairs.append(("Version", "2014-10-31"))
    capo_neptune.types.remove_source_identifier_from_subscription_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_remove_source_identifier_from_subscription(
