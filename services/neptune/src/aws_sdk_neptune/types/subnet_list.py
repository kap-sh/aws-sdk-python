...
if TYPE_CHECKING:
    import aws_sdk_neptune.types.subnet

SubnetList: TypeAlias = list["aws_sdk_neptune.types.subnet.Subnet"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_neptune.types.subnet.serialize_query(
...
def deserialize_query(el: Element) -> SubnetList:
    ...
    for child in el.findall("Subnet"):
        out.append(aws_sdk_neptune.types.subnet.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_neptune.types.subnet.serialize_query(item, pairs, f"{prefix}.{n}")
