...
def build_request(
    ...
    input_: aws_sdk_braket.types.delete_spending_limit_request.DeleteSpendingLimitRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{spendingLimitArn}", quote(str(input_["spending_limit_arn"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_delete_spending_limit(
