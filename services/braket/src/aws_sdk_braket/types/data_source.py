...
def serialize_json(value: DataSource) -> dict:
    out: dict = {}
    import aws_sdk_braket.types.s3_data_source
    out["s3DataSource"] = aws_sdk_braket.types.s3_data_source.serialize_json(value["s3_data_source"])
    return out
...
def deserialize_json(data: dict) -> DataSource:
    ...
    if "s3DataSource" in data:
        import aws_sdk_braket.types.s3_data_source
        out["s3_data_source"] = aws_sdk_braket.types.s3_data_source.deserialize_json(data["s3DataSource"])
    else:
