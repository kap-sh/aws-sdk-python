...
def handle_response(
    ...
    out: aws_sdk_backupsearch.types.get_search_result_export_job_output.GetSearchResultExportJobOutput = aws_sdk_backupsearch.types.get_search_result_export_job_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_backupsearch.types.get_search_result_export_job_input.GetSearchResultExportJobInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{ExportJobIdentifier}", quote(str(input_["export_job_identifier"]), safe="")
    )
    params: dict[str, str] = {}
