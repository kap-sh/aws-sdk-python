...
class SearchJobBackupsResult(TypedDict, closed=True):
    status: NotRequired["capo_backupsearch.types.search_job_state.SearchJobState"]
    """<p>This is the status of the search job backup result.</p>"""
    status_message: NotRequired["str"]
    """<p>This is the status message included with the results.</p>"""
    resource_type: NotRequired["capo_backupsearch.types.resource_type.ResourceType"]
...
def serialize_json(value: SearchJobBackupsResult) -> dict:
    ...
    if "resource_type" in value:
        ...
        out["ResourceType"] = capo_backupsearch.types.resource_type.serialize_json(
            value["resource_type"]
        )
