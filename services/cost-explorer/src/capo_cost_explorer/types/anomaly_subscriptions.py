...
AnomalySubscriptions: TypeAlias = list[
    "capo_cost_explorer.types.anomaly_subscription.AnomalySubscription"
]
...
def serialize_aws_json_1_1(value: AnomalySubscriptions) -> list:
    ...
    for item in value:
        out.append(
            capo_cost_explorer.types.anomaly_subscription.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> AnomalySubscriptions:
    ...
    for item in data:
        out.append(
            capo_cost_explorer.types.anomaly_subscription.deserialize_aws_json_1_1(item)
        )
    return out
