...
class GetApproximateUsageRecordsResponse(TypedDict, closed=True):
    services: NotRequired[
        "capo_cost_explorer.types.approximate_usage_records_per_service.ApproximateUsageRecordsPerService"
    ]
    """<p>The service metadata for the service or services in the response.</p>"""
    total_records: "capo_cost_explorer.types.non_negative_long.NonNegativeLong"
    """<p>The total number of usage records for all services in the services list.</p>"""
    lookback_period: NotRequired["capo_cost_explorer.types.date_interval.DateInterval"]
...
def serialize_aws_json_1_1(value: GetApproximateUsageRecordsResponse) -> dict:
    ...
    if "services" in value:
        ...
        out["Services"] = (
            capo_cost_explorer.types.approximate_usage_records_per_service.serialize_aws_json_1_1(
                value["services"]
            )
        )
...
def deserialize_aws_json_1_1(data: dict) -> GetApproximateUsageRecordsResponse:
    ...
    if "Services" in data:
        ...
        out["services"] = (
            capo_cost_explorer.types.approximate_usage_records_per_service.deserialize_aws_json_1_1(
                data["Services"]
            )
        )
