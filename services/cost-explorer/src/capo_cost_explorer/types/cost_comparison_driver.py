...
class CostComparisonDriver(TypedDict, closed=True):
    cost_selector: NotRequired["capo_cost_explorer.types.expression.Expression"]
    metrics: NotRequired[
        "capo_cost_explorer.types.comparison_metrics.ComparisonMetrics"
    ]
    """<p>A mapping of metric names to their comparison values.</p>"""
    cost_drivers: NotRequired["capo_cost_explorer.types.cost_drivers.CostDrivers"]
...
def serialize_aws_json_1_1(value: CostComparisonDriver) -> dict:
    ...
    if "cost_drivers" in value:
        ...
        out["CostDrivers"] = (
            capo_cost_explorer.types.cost_drivers.serialize_aws_json_1_1(
                value["cost_drivers"]
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> CostComparisonDriver:
    ...
    if "CostDrivers" in data:
        ...
        out["cost_drivers"] = (
            capo_cost_explorer.types.cost_drivers.deserialize_aws_json_1_1(
                data["CostDrivers"]
            )
        )
    return out
