...
def build_request(
    ...
    input: aws_sdk_managedblockchain.types.update_member_input.UpdateMemberInput,
) -> zapros.Request:
    ...
    url = url.replace("{MemberId}", quote(str(input["member_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_managedblockchain.types.update_member_input

    body: bytes | None = json.dumps(
        aws_sdk_managedblockchain.types.update_member_input.serialize_json(input)
    ).encode()
