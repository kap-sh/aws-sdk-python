"""Generated from Smithy shape ``com.amazonaws.managedblockchain#GetMember``."""
...
def handle_response(
    ...
    out: capo_managedblockchain.types.get_member_output.GetMemberOutput = (
        capo_managedblockchain.types.get_member_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{NetworkId}/members/{MemberId}"
    url = url.replace("{NetworkId}", quote(str(input_["network_id"]), safe=""))
    url = url.replace("{MemberId}", quote(str(input_["member_id"]), safe=""))
    params: dict[str, str] = {}
...
def get_member(
    ...
    capo_managedblockchain.types.get_member_output.GetMemberOutput, zapros.Response
]:
