...
class ListNodesOutput(TypedDict, closed=True):
    nodes: NotRequired["capo_managedblockchain.types.node_summary_list.NodeSummaryList"]
...
def serialize_json(value: ListNodesOutput) -> dict:
    ...
    if "nodes" in value:
        ...
        out["Nodes"] = capo_managedblockchain.types.node_summary_list.serialize_json(
            value["nodes"]
        )
...
def deserialize_json(data: dict) -> ListNodesOutput:
    ...
    if "Nodes" in data:
        ...
        out["nodes"] = capo_managedblockchain.types.node_summary_list.deserialize_json(
            data["Nodes"]
        )
