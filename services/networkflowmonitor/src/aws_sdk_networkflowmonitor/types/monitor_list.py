...
MonitorList: TypeAlias = list[
    "aws_sdk_networkflowmonitor.types.monitor_summary.MonitorSummary"
]
...
def serialize_json(value: MonitorList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_networkflowmonitor.types.monitor_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> MonitorList:
    ...
    for item in data:
        out.append(
            aws_sdk_networkflowmonitor.types.monitor_summary.deserialize_json(item)
        )
    return out
