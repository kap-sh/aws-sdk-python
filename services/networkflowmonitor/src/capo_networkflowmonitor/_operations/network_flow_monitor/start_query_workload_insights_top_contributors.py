"""Generated from Smithy shape ``com.amazonaws.networkflowmonitor#StartQueryWorkloadInsightsTopContributors``."""
...
def handle_response(
    ...
    out: capo_networkflowmonitor.types.start_query_workload_insights_top_contributors_output.StartQueryWorkloadInsightsTopContributorsOutput = capo_networkflowmonitor.types.start_query_workload_insights_top_contributors_output.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/workloadInsights/{scopeId}/topContributorsQueries"
    )
    url = url.replace("{scopeId}", quote(str(input_["scope_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_networkflowmonitor.types.start_query_workload_insights_top_contributors_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_query_workload_insights_top_contributors(
