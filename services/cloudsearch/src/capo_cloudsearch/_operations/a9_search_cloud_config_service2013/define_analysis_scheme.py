"""Generated from Smithy shape ``com.amazonaws.cloudsearch#DefineAnalysisScheme``."""
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("DefineAnalysisSchemeResult")
    out: capo_cloudsearch.types.define_analysis_scheme_response.DefineAnalysisSchemeResponse = capo_cloudsearch.types.define_analysis_scheme_response.deserialize_query(
...
def build_request(
    ...
    pairs.append(("Action", "DefineAnalysisScheme"))
    pairs.append(("Version", "2013-01-01"))
    capo_cloudsearch.types.define_analysis_scheme_request.serialize_query(
        input_, pairs, ""
    )
...
async def async_define_analysis_scheme(
