...
class DescribeDomainsRequest(TypedDict, closed=True):
    domain_names: NotRequired["capo_cloudsearch.types.domain_name_list.DomainNameList"]
...
def serialize_query(
    ...
    if "domain_names" in value:
        ...
        capo_cloudsearch.types.domain_name_list.serialize_query(
...
def deserialize_query(el: Element) -> DescribeDomainsRequest:
    ...
    if child_domain_names is not None:
        ...
        out["domain_names"] = capo_cloudsearch.types.domain_name_list.deserialize_query(
            child_domain_names
        )
    return out
