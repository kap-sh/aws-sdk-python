...
class ListDomainNamesResponse(TypedDict, closed=True):
    domain_names: NotRequired["capo_cloudsearch.types.domain_name_map.DomainNameMap"]
...
def deserialize_query(el: Element) -> ListDomainNamesResponse:
    ...
    if child_domain_names is not None:
        ...
        out["domain_names"] = capo_cloudsearch.types.domain_name_map.deserialize_query(
            child_domain_names
        )
    return out
