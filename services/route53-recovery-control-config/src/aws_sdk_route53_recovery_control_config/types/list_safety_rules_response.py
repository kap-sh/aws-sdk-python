...
class ListSafetyRulesResponse(TypedDict):
    ...
    safety_rules: NotRequired[
        "aws_sdk_route53_recovery_control_config.types.__list_of_rule.__listOfRule"
    ]
...
def serialize_json(value: ListSafetyRulesResponse) -> dict:
    ...
    if "safety_rules" in value:
        ...
        out["SafetyRules"] = (
            aws_sdk_route53_recovery_control_config.types.__list_of_rule.serialize_json(
                value["safety_rules"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListSafetyRulesResponse:
    ...
    if "SafetyRules" in data:
        ...
        out["safety_rules"] = (
            aws_sdk_route53_recovery_control_config.types.__list_of_rule.deserialize_json(
                data["SafetyRules"]
            )
        )
    return out
