"""Generated from Smithy shape ``com.amazonaws.route53recoverycontrolconfig#CreateRoutingControl``."""
...
import aws_sdk_route53_recovery_control_config.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            raise aws_sdk_route53_recovery_control_config.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_route53_recovery_control_config.types.create_routing_control_response.CreateRoutingControlResponse = aws_sdk_route53_recovery_control_config.types.create_routing_control_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53_recovery_control_config.types.create_routing_control_request.CreateRoutingControlRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/routingcontrol"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_route53_recovery_control_config.types.create_routing_control_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_routing_control(
