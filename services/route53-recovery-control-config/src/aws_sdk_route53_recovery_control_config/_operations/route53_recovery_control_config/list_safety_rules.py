"""Generated from Smithy shape ``com.amazonaws.route53recoverycontrolconfig#ListSafetyRules``."""
...
import aws_sdk_route53_recovery_control_config.types.__list_of_rule
...
def handle_response(
    ...
    out: aws_sdk_route53_recovery_control_config.types.list_safety_rules_response.ListSafetyRulesResponse = aws_sdk_route53_recovery_control_config.types.list_safety_rules_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53_recovery_control_config.types.list_safety_rules_request.ListSafetyRulesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/controlpanel/{ControlPanelArn}/safetyrules"
    url = url.replace(
        "{ControlPanelArn}", quote(str(input_["control_panel_arn"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_list_safety_rules(
