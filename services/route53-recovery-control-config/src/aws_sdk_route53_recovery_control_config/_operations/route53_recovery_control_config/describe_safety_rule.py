"""Generated from Smithy shape ``com.amazonaws.route53recoverycontrolconfig#DescribeSafetyRule``."""
...
def handle_response(
    ...
    out: aws_sdk_route53_recovery_control_config.types.describe_safety_rule_response.DescribeSafetyRuleResponse = aws_sdk_route53_recovery_control_config.types.describe_safety_rule_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53_recovery_control_config.types.describe_safety_rule_request.DescribeSafetyRuleRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/safetyrule/{SafetyRuleArn}"
    url = url.replace("{SafetyRuleArn}", quote(str(input_["safety_rule_arn"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_safety_rule(
