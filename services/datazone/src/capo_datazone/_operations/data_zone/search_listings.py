"""Generated from Smithy shape ``com.amazonaws.datazone#SearchListings``."""
...
import capo_datazone.types.aggregation_output_list
...
import capo_datazone.types.search_result_items
...
def handle_response(
    ...
    out: capo_datazone.types.search_listings_output.SearchListingsOutput = (
        capo_datazone.types.search_listings_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/listings/search"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.search_listings_input.serialize_json(input_)
    ).encode()
...
def search_listings(
    ...
    capo_datazone.types.search_listings_output.SearchListingsOutput, zapros.Response
]:
