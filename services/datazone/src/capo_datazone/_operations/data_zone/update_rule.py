"""Generated from Smithy shape ``com.amazonaws.datazone#UpdateRule``."""
...
def handle_response(
    ...
    out: capo_datazone.types.update_rule_output.UpdateRuleOutput = (
        capo_datazone.types.update_rule_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/rules/{identifier}"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.update_rule_input.serialize_json(input_)
    ).encode()
...
def update_rule(
    ...
    input_: capo_datazone.types.update_rule_input.UpdateRuleInput,
) -> tuple[capo_datazone.types.update_rule_output.UpdateRuleOutput, zapros.Response]:
