"""Generated from Smithy shape ``com.amazonaws.datazone#SearchTypes``."""
...
import capo_datazone.types.search_types_result_items
...
def handle_response(
    ...
    out: capo_datazone.types.search_types_output.SearchTypesOutput = (
        capo_datazone.types.search_types_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/types-search"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.search_types_input.serialize_json(input_)
    ).encode()
...
def search_types(
    ...
    input_: capo_datazone.types.search_types_input.SearchTypesInput,
) -> tuple[capo_datazone.types.search_types_output.SearchTypesOutput, zapros.Response]:
