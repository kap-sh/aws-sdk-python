"""Generated from Smithy shape ``com.amazonaws.datazone#ListLineageEvents``."""
...
def handle_response(
    ...
    out: capo_datazone.types.list_lineage_events_output.ListLineageEventsOutput = (
        capo_datazone.types.list_lineage_events_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/lineage/events"
    url = url.replace(
    ...
    if "timestamp_after" in input_:
        params["timestampAfter"] = str(input_["timestamp_after"])
    if "timestamp_before" in input_:
        params["timestampBefore"] = str(input_["timestamp_before"])
    if "processing_status" in input_:
        params["processingStatus"] = str(input_["processing_status"])
...
async def async_list_lineage_events(
