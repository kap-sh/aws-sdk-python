"""Generated from Smithy shape ``com.amazonaws.datazone#DeleteProjectMembership``."""
...
def handle_response(
    ...
    out: capo_datazone.types.delete_project_membership_output.DeleteProjectMembershipOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{projectIdentifier}", quote(str(input_["project_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.delete_project_membership_input.serialize_json(input_)
    ).encode()
...
async def async_delete_project_membership(
