"""Generated from Smithy shape ``com.amazonaws.datazone#CreateFormType``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_form_type_output.CreateFormTypeOutput = (
        capo_datazone.types.create_form_type_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/form-types"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.create_form_type_input.serialize_json(input_)
    ).encode()
...
def create_form_type(
    ...
    capo_datazone.types.create_form_type_output.CreateFormTypeOutput, zapros.Response
]:
...
async def async_create_form_type(
