"""Generated from Smithy shape ``com.amazonaws.datazone#CreateAccountPool``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_account_pool_output.CreateAccountPoolOutput = (
        capo_datazone.types.create_account_pool_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/account-pools"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.create_account_pool_input.serialize_json(input_)
    ).encode()
...
async def async_create_account_pool(
