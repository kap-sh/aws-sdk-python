"""Generated from Smithy shape ``com.amazonaws.datazone#RejectPredictions``."""
...
def handle_response(
    ...
    out: capo_datazone.types.reject_predictions_output.RejectPredictionsOutput = (
        capo_datazone.types.reject_predictions_output.deserialize_json(
...
def build_request(
    ...
    params: dict[str, str] = {}
    if "revision" in input_:
        params["revision"] = str(input_["revision"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.reject_predictions_input.serialize_json(input_)
    ).encode()
...
async def async_reject_predictions(
