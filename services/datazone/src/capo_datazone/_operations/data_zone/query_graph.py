"""Generated from Smithy shape ``com.amazonaws.datazone#QueryGraph``."""
...
def handle_response(
    ...
    out: capo_datazone.types.query_graph_output.QueryGraphOutput = (
        capo_datazone.types.query_graph_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/graph/query"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.query_graph_input.serialize_json(input_)
    ).encode()
...
def query_graph(
    ...
    input_: capo_datazone.types.query_graph_input.QueryGraphInput,
) -> tuple[capo_datazone.types.query_graph_output.QueryGraphOutput, zapros.Response]:
