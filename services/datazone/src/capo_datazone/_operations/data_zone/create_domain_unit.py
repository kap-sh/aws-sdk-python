"""Generated from Smithy shape ``com.amazonaws.datazone#CreateDomainUnit``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_domain_unit_output.CreateDomainUnitOutput = (
        capo_datazone.types.create_domain_unit_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/domain-units"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.create_domain_unit_input.serialize_json(input_)
    ).encode()
...
async def async_create_domain_unit(
