"""Generated from Smithy shape ``com.amazonaws.datazone#UpdateUserProfile``."""
...
def handle_response(
    ...
    out: capo_datazone.types.update_user_profile_output.UpdateUserProfileOutput = (
        capo_datazone.types.update_user_profile_output.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{userIdentifier}", quote(str(input_["user_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_datazone.types.update_user_profile_input.serialize_json(input_)
    ).encode()
