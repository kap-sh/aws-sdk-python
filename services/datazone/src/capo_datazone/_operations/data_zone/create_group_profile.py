"""Generated from Smithy shape ``com.amazonaws.datazone#CreateGroupProfile``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_group_profile_output.CreateGroupProfileOutput = (
        capo_datazone.types.create_group_profile_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/group-profiles"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.create_group_profile_input.serialize_json(input_)
    ).encode()
...
async def async_create_group_profile(
