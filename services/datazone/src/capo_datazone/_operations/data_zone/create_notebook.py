"""Generated from Smithy shape ``com.amazonaws.datazone#CreateNotebook``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_notebook_output.CreateNotebookOutput = (
        capo_datazone.types.create_notebook_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/notebooks"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.create_notebook_input.serialize_json(input_)
    ).encode()
...
def create_notebook(
    ...
    capo_datazone.types.create_notebook_output.CreateNotebookOutput, zapros.Response
]:
...
async def async_create_notebook(
