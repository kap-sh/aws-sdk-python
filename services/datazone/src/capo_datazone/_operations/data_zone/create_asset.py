"""Generated from Smithy shape ``com.amazonaws.datazone#CreateAsset``."""
...
def handle_response(
    ...
    out: capo_datazone.types.create_asset_output.CreateAssetOutput = (
        capo_datazone.types.create_asset_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/assets"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_datazone.types.create_asset_input.serialize_json(input_)
    ).encode()
...
def create_asset(
    ...
    input_: capo_datazone.types.create_asset_input.CreateAssetInput,
) -> tuple[capo_datazone.types.create_asset_output.CreateAssetOutput, zapros.Response]:
