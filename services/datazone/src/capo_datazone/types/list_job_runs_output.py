...
class ListJobRunsOutput(TypedDict, closed=True):
    items: NotRequired["capo_datazone.types.job_run_summaries.JobRunSummaries"]
...
def serialize_json(value: ListJobRunsOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_datazone.types.job_run_summaries.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListJobRunsOutput:
    ...
    if "items" in data:
        ...
        out["items"] = capo_datazone.types.job_run_summaries.deserialize_json(
            data["items"]
        )
