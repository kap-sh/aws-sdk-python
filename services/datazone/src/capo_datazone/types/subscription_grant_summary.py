"""Generated from Smithy shape ``com.amazonaws.datazone#SubscriptionGrantSummary``."""
...
class SubscriptionGrantSummary(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: SubscriptionGrantSummary) -> dict:
    out: dict = {}
    out["id"] = value["id"]
...
def deserialize_json(data: dict) -> SubscriptionGrantSummary:
    out: SubscriptionGrantSummary = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("SubscriptionGrantSummary.id required")
    if "createdBy" in data:
        out["created_by"] = data["createdBy"]
    else:
        raise DeserializationError("SubscriptionGrantSummary.created_by required")
    if "updatedBy" in data:
        out["updated_by"] = data["updatedBy"]
    if "domainId" in data:
        out["domain_id"] = data["domainId"]
    else:
        raise DeserializationError("SubscriptionGrantSummary.domain_id required")
    if "createdAt" in data:
        ...
        raise DeserializationError("SubscriptionGrantSummary.created_at required")
    if "updatedAt" in data:
        ...
        raise DeserializationError("SubscriptionGrantSummary.updated_at required")
    ...
    if "subscriptionTargetId" in data:
        ...
        raise DeserializationError(
            "SubscriptionGrantSummary.subscription_target_id required"
        )
    if "grantedEntity" in data:
        ...
        raise DeserializationError("SubscriptionGrantSummary.granted_entity required")
    if "status" in data:
        import capo_datazone.types.subscription_grant_overall_status
        ...
        raise DeserializationError("SubscriptionGrantSummary.status required")
    if "assets" in data:
        import capo_datazone.types.subscribed_assets
