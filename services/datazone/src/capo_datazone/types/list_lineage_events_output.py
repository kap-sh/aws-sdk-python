...
class ListLineageEventsOutput(TypedDict, closed=True):
    items: NotRequired[
        "capo_datazone.types.lineage_event_summaries.LineageEventSummaries"
    ]
...
def serialize_json(value: ListLineageEventsOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_datazone.types.lineage_event_summaries.serialize_json(
            value["items"]
        )
