...
if TYPE_CHECKING:
    ...
    import capo_datazone.types.project_summaries


class ListProjectsOutput(TypedDict, closed=True):
    items: NotRequired["capo_datazone.types.project_summaries.ProjectSummaries"]
...
def serialize_json(value: ListProjectsOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_datazone.types.project_summaries.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListProjectsOutput:
    ...
    if "items" in data:
        ...
        out["items"] = capo_datazone.types.project_summaries.deserialize_json(
            data["items"]
        )
