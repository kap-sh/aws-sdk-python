...
class DataSourceRunLineageSummary(TypedDict, closed=True):
    import_status: NotRequired[
        "capo_datazone.types.lineage_import_status.LineageImportStatus"
    ]
...
def serialize_json(value: DataSourceRunLineageSummary) -> dict:
    ...
    if "import_status" in value:
        ...
        out["importStatus"] = capo_datazone.types.lineage_import_status.serialize_json(
            value["import_status"]
        )
    return out
...
def deserialize_json(data: dict) -> DataSourceRunLineageSummary:
    ...
    if "importStatus" in data:
        ...
        out["import_status"] = (
            capo_datazone.types.lineage_import_status.deserialize_json(
                data["importStatus"]
            )
        )
    return out
