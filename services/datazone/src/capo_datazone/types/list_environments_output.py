...
if TYPE_CHECKING:
    import capo_datazone.types.environment_summaries
    import capo_datazone.types.pagination_token


class ListEnvironmentsOutput(TypedDict, closed=True):
    items: "capo_datazone.types.environment_summaries.EnvironmentSummaries"
...
def serialize_json(value: ListEnvironmentsOutput) -> dict:
    ...
    out["items"] = capo_datazone.types.environment_summaries.serialize_json(
        value["items"]
    )
...
def deserialize_json(data: dict) -> ListEnvironmentsOutput:
    ...
    if "items" in data:
        ...
        out["items"] = capo_datazone.types.environment_summaries.deserialize_json(
            data["items"]
        )
    else:
