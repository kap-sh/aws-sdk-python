...
class VpcPropertiesPatch(TypedDict, closed=True):
    vpc_id: NotRequired["capo_datazone.types.vpc_id.VpcId"]
...
def serialize_json(value: VpcPropertiesPatch) -> dict:
    ...
    if "subnet_ids" in value:
        ...
        out["subnetIds"] = (
            capo_datazone.types.vpc_connection_subnet_id_list.serialize_json(
                value["subnet_ids"]
            )
        )
