...
class ListProjectProfilesOutput(TypedDict, closed=True):
    items: NotRequired[
        "capo_datazone.types.project_profile_summaries.ProjectProfileSummaries"
    ]
...
def serialize_json(value: ListProjectProfilesOutput) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_datazone.types.project_profile_summaries.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListProjectProfilesOutput:
    ...
    if "items" in data:
        ...
        out["items"] = capo_datazone.types.project_profile_summaries.deserialize_json(
            data["items"]
        )
