...
if TYPE_CHECKING:
    import capo_datazone.types.lineage_node_summaries
    import capo_datazone.types.pagination_token


class ListLineageNodeHistoryOutput(TypedDict, closed=True):
    nodes: NotRequired[
        "capo_datazone.types.lineage_node_summaries.LineageNodeSummaries"
    ]
...
def serialize_json(value: ListLineageNodeHistoryOutput) -> dict:
    ...
    if "nodes" in value:
        ...
        out["nodes"] = capo_datazone.types.lineage_node_summaries.serialize_json(
            value["nodes"]
        )
...
def deserialize_json(data: dict) -> ListLineageNodeHistoryOutput:
    ...
    if "nodes" in data:
        ...
        out["nodes"] = capo_datazone.types.lineage_node_summaries.deserialize_json(
            data["nodes"]
        )
