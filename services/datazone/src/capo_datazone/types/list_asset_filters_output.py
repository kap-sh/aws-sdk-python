...
class ListAssetFiltersOutput(TypedDict, closed=True):
    items: "capo_datazone.types.asset_filters.AssetFilters"
...
def serialize_json(value: ListAssetFiltersOutput) -> dict:
    ...
    out["items"] = capo_datazone.types.asset_filters.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListAssetFiltersOutput:
    ...
    if "items" in data:
        ...
        out["items"] = capo_datazone.types.asset_filters.deserialize_json(data["items"])
    else:
