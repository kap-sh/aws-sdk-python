...
if TYPE_CHECKING:
    import capo_datazone.types.s3_permission

S3Permissions: TypeAlias = list["capo_datazone.types.s3_permission.S3Permission"]
...
def serialize_json(value: S3Permissions) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_datazone.types.s3_permission.serialize_json(item))
    return out
