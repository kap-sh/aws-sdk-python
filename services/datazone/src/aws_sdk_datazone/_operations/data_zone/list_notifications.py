...
def build_request(
    ...
    input: aws_sdk_datazone.types.list_notifications_input.ListNotificationsInput,
) -> zapros.Request:
    ...
    if "after_timestamp" in input:
        params["afterTimestamp"] = str(input["after_timestamp"])
    if "before_timestamp" in input:
        params["beforeTimestamp"] = str(input["before_timestamp"])
    if "subjects" in input:
        params["subjects"] = str(input["subjects"])
