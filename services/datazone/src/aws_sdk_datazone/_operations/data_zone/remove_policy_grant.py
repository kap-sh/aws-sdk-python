"""Generated from Smithy shape ``com.amazonaws.datazone#RemovePolicyGrant``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.remove_policy_grant_output.RemovePolicyGrantOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.remove_policy_grant_input.RemovePolicyGrantInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.remove_policy_grant_input.serialize_json(input_)
    ).encode()
...
async def async_remove_policy_grant(
