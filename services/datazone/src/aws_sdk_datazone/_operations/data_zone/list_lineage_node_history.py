...
def build_request(
    ...
    input: aws_sdk_datazone.types.list_lineage_node_history_input.ListLineageNodeHistoryInput,
) -> zapros.Request:
    ...
    if "direction" in input:
        params["direction"] = str(input["direction"])
    if "event_timestamp_gte" in input:
        params["timestampGTE"] = str(input["event_timestamp_gte"])
    if "event_timestamp_lte" in input:
        params["timestampLTE"] = str(input["event_timestamp_lte"])
