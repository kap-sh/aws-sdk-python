"""Generated from Smithy shape ``com.amazonaws.datazone#StartNotebookExport``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.start_notebook_export_output.StartNotebookExportOutput = aws_sdk_datazone.types.start_notebook_export_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.start_notebook_export_input.StartNotebookExportInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/notebook-exports"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.start_notebook_export_input.serialize_json(input_)
    ).encode()
...
async def async_start_notebook_export(
