"""Generated from Smithy shape ``com.amazonaws.datazone#ListSubscriptionGrants``."""
...
import aws_sdk_datazone.types.subscription_grants
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.list_subscription_grants_output.ListSubscriptionGrantsOutput = aws_sdk_datazone.types.list_subscription_grants_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.list_subscription_grants_input.ListSubscriptionGrantsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/subscription-grants"
    )
    url = url.replace(
    ...
    if "subscription_target_id" in input_:
        params["subscriptionTargetId"] = str(input_["subscription_target_id"])
    ...
    if "subscription_id" in input_:
        params["subscriptionId"] = str(input_["subscription_id"])
...
async def async_list_subscription_grants(
