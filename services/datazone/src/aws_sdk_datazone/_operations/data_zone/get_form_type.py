"""Generated from Smithy shape ``com.amazonaws.datazone#GetFormType``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.get_form_type_output.GetFormTypeOutput = (
        aws_sdk_datazone.types.get_form_type_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.get_form_type_input.GetFormTypeInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{formTypeIdentifier}", quote(str(input_["form_type_identifier"]), safe="")
    )
    params: dict[str, str] = {}
...
def get_form_type(
    ...
    aws_sdk_datazone.types.get_form_type_output.GetFormTypeOutput, zapros.Response
]:
