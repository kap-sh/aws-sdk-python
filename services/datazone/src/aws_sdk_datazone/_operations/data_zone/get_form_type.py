...
def build_request(
    ...
    input: aws_sdk_datazone.types.get_form_type_input.GetFormTypeInput,
) -> zapros.Request:
    ...
    url = url.replace(
        "{formTypeIdentifier}", quote(str(input["form_type_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    if "revision" in input:
        params["revision"] = str(input["revision"])
