...
def handle_response(
    ...
    out: aws_sdk_datazone.types.post_lineage_event_output.PostLineageEventOutput = (
        aws_sdk_datazone.types.post_lineage_event_output.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_datazone.types.post_lineage_event_input.PostLineageEventInput,
) -> zapros.Request:
    ...
    if "client_token" in input:
        headers["Client-Token"] = str(input["client_token"])
    if "event" in input:
        import aws_sdk_datazone.types.lineage_event

        body: bytes | None = json.dumps(
            aws_sdk_datazone.types.lineage_event.serialize_json(input["event"])
        ).encode()
