"""Generated from Smithy shape ``com.amazonaws.datazone#ListProjects``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.list_projects_output.ListProjectsOutput = (
        aws_sdk_datazone.types.list_projects_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.list_projects_input.ListProjectsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/projects"
    url = url.replace(
        "{domainIdentifier}", quote(str(input_["domain_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    if "user_identifier" in input_:
        params["userIdentifier"] = str(input_["user_identifier"])
    if "group_identifier" in input_:
        params["groupIdentifier"] = str(input_["group_identifier"])
    if "name" in input_:
        params["name"] = str(input_["name"])
    if "project_category" in input_:
        params["projectCategory"] = str(input_["project_category"])
...
def list_projects(
    ...
    aws_sdk_datazone.types.list_projects_output.ListProjectsOutput, zapros.Response
]:
