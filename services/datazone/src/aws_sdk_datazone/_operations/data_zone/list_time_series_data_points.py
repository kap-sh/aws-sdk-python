...
def build_request(
    ...
    input: aws_sdk_datazone.types.list_time_series_data_points_input.ListTimeSeriesDataPointsInput,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "form_name" in input:
        params["formName"] = str(input["form_name"])
    if "started_at" in input:
        params["startedAt"] = str(input["started_at"])
    if "ended_at" in input:
        params["endedAt"] = str(input["ended_at"])
