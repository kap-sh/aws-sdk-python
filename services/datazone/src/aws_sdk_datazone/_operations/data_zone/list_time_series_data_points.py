"""Generated from Smithy shape ``com.amazonaws.datazone#ListTimeSeriesDataPoints``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.list_time_series_data_points_output.ListTimeSeriesDataPointsOutput = aws_sdk_datazone.types.list_time_series_data_points_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.list_time_series_data_points_input.ListTimeSeriesDataPointsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    params: dict[str, str] = {}
    if "form_name" in input_:
        params["formName"] = str(input_["form_name"])
    if "started_at" in input_:
        params["startedAt"] = str(input_["started_at"])
    if "ended_at" in input_:
        params["endedAt"] = str(input_["ended_at"])
...
async def async_list_time_series_data_points(
