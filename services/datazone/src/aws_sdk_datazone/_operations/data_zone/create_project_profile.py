"""Generated from Smithy shape ``com.amazonaws.datazone#CreateProjectProfile``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.create_project_profile_output.CreateProjectProfileOutput = aws_sdk_datazone.types.create_project_profile_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.create_project_profile_input.CreateProjectProfileInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/project-profiles"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.create_project_profile_input.serialize_json(input_)
    ).encode()
...
async def async_create_project_profile(
