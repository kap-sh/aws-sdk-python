"""Generated from Smithy shape ``com.amazonaws.datazone#SearchUserProfiles``."""
...
import aws_sdk_datazone.types.user_profile_summaries
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.search_user_profiles_output.SearchUserProfilesOutput = (
        aws_sdk_datazone.types.search_user_profiles_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.search_user_profiles_input.SearchUserProfilesInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/search-user-profiles"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.search_user_profiles_input.serialize_json(input_)
    ).encode()
...
async def async_search_user_profiles(
