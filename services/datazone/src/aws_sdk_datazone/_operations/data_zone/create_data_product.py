"""Generated from Smithy shape ``com.amazonaws.datazone#CreateDataProduct``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.create_data_product_output.CreateDataProductOutput = (
        aws_sdk_datazone.types.create_data_product_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.create_data_product_input.CreateDataProductInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/data-products"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.create_data_product_input.serialize_json(input_)
    ).encode()
...
async def async_create_data_product(
