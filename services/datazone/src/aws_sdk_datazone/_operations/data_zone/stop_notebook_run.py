"""Generated from Smithy shape ``com.amazonaws.datazone#StopNotebookRun``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.stop_notebook_run_output.StopNotebookRunOutput = (
        aws_sdk_datazone.types.stop_notebook_run_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.stop_notebook_run_input.StopNotebookRunInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/v2/domains/{domainIdentifier}/notebook-runs/{identifier}/stop"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.stop_notebook_run_input.serialize_json(input_)
    ).encode()
...
async def async_stop_notebook_run(
