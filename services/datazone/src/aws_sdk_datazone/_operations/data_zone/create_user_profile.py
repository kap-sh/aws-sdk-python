"""Generated from Smithy shape ``com.amazonaws.datazone#CreateUserProfile``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.create_user_profile_output.CreateUserProfileOutput = (
        aws_sdk_datazone.types.create_user_profile_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.create_user_profile_input.CreateUserProfileInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/user-profiles"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.create_user_profile_input.serialize_json(input_)
    ).encode()
...
async def async_create_user_profile(
