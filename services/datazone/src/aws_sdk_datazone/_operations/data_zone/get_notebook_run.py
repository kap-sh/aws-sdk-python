"""Generated from Smithy shape ``com.amazonaws.datazone#GetNotebookRun``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.get_notebook_run_output.GetNotebookRunOutput = (
        aws_sdk_datazone.types.get_notebook_run_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.get_notebook_run_input.GetNotebookRunInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/v2/domains/{domainIdentifier}/notebook-runs/{identifier}"
    )
    url = url.replace(
...
def get_notebook_run(
    ...
    aws_sdk_datazone.types.get_notebook_run_output.GetNotebookRunOutput, zapros.Response
]:
...
async def async_get_notebook_run(
