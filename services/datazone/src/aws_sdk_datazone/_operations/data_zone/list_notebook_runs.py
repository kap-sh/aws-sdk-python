...
def build_request(
    ...
    input: aws_sdk_datazone.types.list_notebook_runs_input.ListNotebookRunsInput,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "owning_project_identifier" in input:
        params["owningProjectIdentifier"] = str(input["owning_project_identifier"])
    if "notebook_identifier" in input:
        params["notebookIdentifier"] = str(input["notebook_identifier"])
    if "status" in input:
        params["status"] = str(input["status"])
    if "schedule_identifier" in input:
        params["scheduleIdentifier"] = str(input["schedule_identifier"])
