"""Generated from Smithy shape ``com.amazonaws.datazone#ListNotebookRuns``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.list_notebook_runs_output.ListNotebookRunsOutput = (
        aws_sdk_datazone.types.list_notebook_runs_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.list_notebook_runs_input.ListNotebookRunsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/notebook-runs"
    url = url.replace(
        "{domainIdentifier}", quote(str(input_["domain_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    if "owning_project_identifier" in input_:
        params["owningProjectIdentifier"] = str(input_["owning_project_identifier"])
    if "notebook_identifier" in input_:
        params["notebookIdentifier"] = str(input_["notebook_identifier"])
    if "status" in input_:
        params["status"] = str(input_["status"])
    if "schedule_identifier" in input_:
        params["scheduleIdentifier"] = str(input_["schedule_identifier"])
...
async def async_list_notebook_runs(
