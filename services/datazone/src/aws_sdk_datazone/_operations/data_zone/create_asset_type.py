"""Generated from Smithy shape ``com.amazonaws.datazone#CreateAssetType``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.create_asset_type_output.CreateAssetTypeOutput = (
        aws_sdk_datazone.types.create_asset_type_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.create_asset_type_input.CreateAssetTypeInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domains/{domainIdentifier}/asset-types"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.create_asset_type_input.serialize_json(input_)
    ).encode()
...
async def async_create_asset_type(
