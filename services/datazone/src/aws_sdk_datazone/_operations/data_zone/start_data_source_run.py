"""Generated from Smithy shape ``com.amazonaws.datazone#StartDataSourceRun``."""
...
def handle_response(
    ...
    out: aws_sdk_datazone.types.start_data_source_run_output.StartDataSourceRunOutput = aws_sdk_datazone.types.start_data_source_run_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_datazone.types.start_data_source_run_input.StartDataSourceRunInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{dataSourceIdentifier}", quote(str(input_["data_source_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_datazone.types.start_data_source_run_input.serialize_json(input_)
    ).encode()
...
async def async_start_data_source_run(
