...
def serialize_json(value: AddPolicyGrantInput) -> dict:
    out: dict = {}
    import aws_sdk_datazone.types.managed_policy_type
    out["policyType"] = aws_sdk_datazone.types.managed_policy_type.serialize_json(value["policy_type"])
    import aws_sdk_datazone.types.policy_grant_principal
    out["principal"] = aws_sdk_datazone.types.policy_grant_principal.serialize_json(value["principal"])
    import aws_sdk_datazone.types.policy_grant_detail
    out["detail"] = aws_sdk_datazone.types.policy_grant_detail.serialize_json(value["detail"])
...
def deserialize_json(data: dict) -> AddPolicyGrantInput:
    ...
    if "policyType" in data:
        import aws_sdk_datazone.types.managed_policy_type
        out["policy_type"] = aws_sdk_datazone.types.managed_policy_type.deserialize_json(data["policyType"])
    else:
        raise DeserializationError("AddPolicyGrantInput.policy_type required")
    if "principal" in data:
        import aws_sdk_datazone.types.policy_grant_principal
        out["principal"] = aws_sdk_datazone.types.policy_grant_principal.deserialize_json(data["principal"])
    else:
        raise DeserializationError("AddPolicyGrantInput.principal required")
    if "detail" in data:
        import aws_sdk_datazone.types.policy_grant_detail
        out["detail"] = aws_sdk_datazone.types.policy_grant_detail.deserialize_json(data["detail"])
    else:
