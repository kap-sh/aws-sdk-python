"""Generated from Smithy shape ``com.amazonaws.datazone#UpdateSubscriptionRequestOutput``."""
...
class UpdateSubscriptionRequestOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateSubscriptionRequestOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
...
def deserialize_json(data: dict) -> UpdateSubscriptionRequestOutput:
    out: UpdateSubscriptionRequestOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("UpdateSubscriptionRequestOutput.id required")
    if "createdBy" in data:
        out["created_by"] = data["createdBy"]
    else:
        raise DeserializationError(
            "UpdateSubscriptionRequestOutput.created_by required"
        )
    ...
    if "domainId" in data:
        ...
        raise DeserializationError("UpdateSubscriptionRequestOutput.domain_id required")
    if "status" in data:
        import aws_sdk_datazone.types.subscription_request_status
        ...
        raise DeserializationError("UpdateSubscriptionRequestOutput.status required")
    if "createdAt" in data:
        ...
        raise DeserializationError(
            "UpdateSubscriptionRequestOutput.created_at required"
        )
    if "updatedAt" in data:
        ...
        raise DeserializationError(
            "UpdateSubscriptionRequestOutput.updated_at required"
        )
    if "requestReason" in data:
        out["request_reason"] = data["requestReason"]
    else:
        raise DeserializationError(
            "UpdateSubscriptionRequestOutput.request_reason required"
        )
    if "subscribedPrincipals" in data:
        ...
        raise DeserializationError(
            "UpdateSubscriptionRequestOutput.subscribed_principals required"
        )
    if "subscribedListings" in data:
        ...
        raise DeserializationError(
            "UpdateSubscriptionRequestOutput.subscribed_listings required"
        )
