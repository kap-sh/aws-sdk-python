"""Generated from Smithy shape ``com.amazonaws.datazone#CreateConnectionOutput``."""
...
# --- restJson1 ser/de ---
def serialize_json(value: CreateConnectionOutput) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> CreateConnectionOutput:
    out: CreateConnectionOutput = {}  # type: ignore[typeddict-item]
    if "connectionId" in data:
        out["connection_id"] = data["connectionId"]
    else:
        raise DeserializationError("CreateConnectionOutput.connection_id required")
    ...
    if "domainId" in data:
        ...
        raise DeserializationError("CreateConnectionOutput.domain_id required")
    if "domainUnitId" in data:
        out["domain_unit_id"] = data["domainUnitId"]
    else:
        raise DeserializationError("CreateConnectionOutput.domain_unit_id required")
    if "environmentId" in data:
        out["environment_id"] = data["environmentId"]
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("CreateConnectionOutput.name required")
    if "physicalEndpoints" in data:
        ...
        raise DeserializationError("CreateConnectionOutput.physical_endpoints required")
    ...
    if "type" in data:
        ...
        raise DeserializationError("CreateConnectionOutput.type required")
    if "scope" in data:
        import aws_sdk_datazone.types.connection_scope
