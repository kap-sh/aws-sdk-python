"""Generated from Smithy shape ``com.amazonaws.datazone#EnvironmentBlueprintConfigurationItem``."""
...
class EnvironmentBlueprintConfigurationItem(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: EnvironmentBlueprintConfigurationItem) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> EnvironmentBlueprintConfigurationItem:
    out: EnvironmentBlueprintConfigurationItem = {}  # type: ignore[typeddict-item]
    if "domainId" in data:
        out["domain_id"] = data["domainId"]
    else:
        raise DeserializationError(
            "EnvironmentBlueprintConfigurationItem.domain_id required"
        )
    if "environmentBlueprintId" in data:
        out["environment_blueprint_id"] = data["environmentBlueprintId"]
    else:
        raise DeserializationError(
            "EnvironmentBlueprintConfigurationItem.environment_blueprint_id required"
        )
