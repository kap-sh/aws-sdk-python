"""Generated from Smithy shape ``com.amazonaws.datazone#EnvironmentActionSummary``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

from aws_sdk_datazone.errors import DeserializationError

if TYPE_CHECKING:
    import aws_sdk_datazone.types.action_parameters
    import aws_sdk_datazone.types.domain_id
    import aws_sdk_datazone.types.environment_action_id
    import aws_sdk_datazone.types.environment_id


class EnvironmentActionSummary(TypedDict, closed=True):
    domain_id: "aws_sdk_datazone.types.domain_id.DomainId"
    """<p>The Amazon DataZone domain ID of the environment action.</p>"""
    environment_id: "aws_sdk_datazone.types.environment_id.EnvironmentId"
    """<p>The environment ID of the environment action.</p>"""
    id: "aws_sdk_datazone.types.environment_action_id.EnvironmentActionId"
    """<p>The ID of the environment action.</p>"""
    name: "str"
    """<p>The name of the environment action.</p>"""
    parameters: "aws_sdk_datazone.types.action_parameters.ActionParameters"
    """<p>The parameters of the environment action.</p>"""
    description: NotRequired["str"]
    """<p>The environment action description.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: EnvironmentActionSummary) -> dict:
    out: dict = {}
    out["domainId"] = value["domain_id"]
    out["environmentId"] = value["environment_id"]
    out["id"] = value["id"]
    out["name"] = value["name"]
    import aws_sdk_datazone.types.action_parameters

    out["parameters"] = aws_sdk_datazone.types.action_parameters.serialize_json(
        value["parameters"]
    )
    if "description" in value:
        out["description"] = value["description"]
    return out


def deserialize_json(data: dict) -> EnvironmentActionSummary:
    out: EnvironmentActionSummary = {}  # type: ignore[typeddict-item]
    if "domainId" in data:
        out["domain_id"] = data["domainId"]
    else:
        raise DeserializationError("EnvironmentActionSummary.domain_id required")
    if "environmentId" in data:
        out["environment_id"] = data["environmentId"]
    else:
        raise DeserializationError("EnvironmentActionSummary.environment_id required")
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("EnvironmentActionSummary.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("EnvironmentActionSummary.name required")
    if "parameters" in data:
        import aws_sdk_datazone.types.action_parameters

        out["parameters"] = aws_sdk_datazone.types.action_parameters.deserialize_json(
            data["parameters"]
        )
    else:
        raise DeserializationError("EnvironmentActionSummary.parameters required")
    if "description" in data:
        out["description"] = data["description"]
    return out
