...
class _ActionParameters_awsConsoleLink(TypedDict):
    awsConsoleLink: "aws_sdk_datazone.types.aws_console_link_parameters.AwsConsoleLinkParameters"
...
def serialize_json(value: ActionParameters) -> dict:
    if "awsConsoleLink" in value:
        import aws_sdk_datazone.types.aws_console_link_parameters
        return {"awsConsoleLink": aws_sdk_datazone.types.aws_console_link_parameters.serialize_json(value["awsConsoleLink"])}
    else:
...
def deserialize_json(data: dict) -> ActionParameters:
    if "awsConsoleLink" in data:
        import aws_sdk_datazone.types.aws_console_link_parameters
        return {"awsConsoleLink": aws_sdk_datazone.types.aws_console_link_parameters.deserialize_json(data["awsConsoleLink"])}
    else:
