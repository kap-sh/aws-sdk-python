...
if TYPE_CHECKING:
    import aws_sdk_datazone.types.metadata_generation_target_type
...
def serialize_json(value: MetadataGenerationRunTarget) -> dict:
    ...
    out["type"] = aws_sdk_datazone.types.metadata_generation_target_type.serialize_json(
        value["type"]
    )
...
def deserialize_json(data: dict) -> MetadataGenerationRunTarget:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_datazone.types.metadata_generation_target_type.deserialize_json(
                data["type"]
            )
        )
    else:
