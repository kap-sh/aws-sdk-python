...
EnvironmentBlueprintSummaries: TypeAlias = list[
    "aws_sdk_datazone.types.environment_blueprint_summary.EnvironmentBlueprintSummary"
]
...
def serialize_json(value: EnvironmentBlueprintSummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_datazone.types.environment_blueprint_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> EnvironmentBlueprintSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_datazone.types.environment_blueprint_summary.deserialize_json(item)
        )
    return out
