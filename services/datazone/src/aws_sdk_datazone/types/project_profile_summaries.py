...
ProjectProfileSummaries: TypeAlias = list[
    "aws_sdk_datazone.types.project_profile_summary.ProjectProfileSummary"
]
...
def serialize_json(value: ProjectProfileSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_datazone.types.project_profile_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ProjectProfileSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_datazone.types.project_profile_summary.deserialize_json(item)
        )
    return out
