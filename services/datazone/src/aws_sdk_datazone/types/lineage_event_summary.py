...
class LineageEventSummary(TypedDict):
    ...
    event_summary: NotRequired["aws_sdk_datazone.types.event_summary.EventSummary"]
...
def serialize_json(value: LineageEventSummary) -> dict:
    ...
    if "event_time" in value:
        ...
        out["eventTime"] = aws_sdk_datazone.types._prelude.timestamp.serialize_json(
            value["event_time"]
        )
    if "event_summary" in value:
        import aws_sdk_datazone.types.event_summary

        out["eventSummary"] = aws_sdk_datazone.types.event_summary.serialize_json(
            value["event_summary"]
        )
...
def deserialize_json(data: dict) -> LineageEventSummary:
    ...
    if "eventTime" in data:
        ...
        out["event_time"] = aws_sdk_datazone.types._prelude.timestamp.deserialize_json(
            data["eventTime"]
        )
    if "eventSummary" in data:
        import aws_sdk_datazone.types.event_summary

        out["event_summary"] = aws_sdk_datazone.types.event_summary.deserialize_json(
            data["eventSummary"]
        )
