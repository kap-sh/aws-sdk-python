...
if TYPE_CHECKING:
    ...
    import aws_sdk_datazone.types.subscribed_listing_item


class SubscribedListing(TypedDict):
    ...
    name: "aws_sdk_datazone.types.listing_name.ListingName"
    """<p>The name of the published asset for which the subscription grant is created.</p>"""
    description: "aws_sdk_datazone.types.description.Description"
    """<p>The description of the published asset for which the subscription grant is created.</p>"""
    item: "aws_sdk_datazone.types.subscribed_listing_item.SubscribedListingItem"
    """<p>The published asset for which the subscription grant is created.</p>"""
    owner_project_id: "aws_sdk_datazone.types.project_id.ProjectId"
...
def serialize_json(value: SubscribedListing) -> dict:
    ...
    out["item"] = aws_sdk_datazone.types.subscribed_listing_item.serialize_json(
        value["item"]
    )
...
def deserialize_json(data: dict) -> SubscribedListing:
    ...
    if "item" in data:
        ...
        out["item"] = aws_sdk_datazone.types.subscribed_listing_item.deserialize_json(
            data["item"]
        )
    else:
