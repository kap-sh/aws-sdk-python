...
class ListRouterInputsRequest(TypedDict):
...
def serialize_json(value: ListRouterInputsRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            aws_sdk_mediaconnect.types.router_input_filter_list.serialize_json(
                value["filters"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListRouterInputsRequest:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            aws_sdk_mediaconnect.types.router_input_filter_list.deserialize_json(
                data["filters"]
            )
        )
    return out
