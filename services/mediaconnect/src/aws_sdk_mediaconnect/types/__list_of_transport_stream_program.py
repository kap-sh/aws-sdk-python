"""Generated from Smithy shape ``com.amazonaws.mediaconnect#__listOfTransportStreamProgram``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_mediaconnect.types.transport_stream_program

__listOfTransportStreamProgram: TypeAlias = list[
    "aws_sdk_mediaconnect.types.transport_stream_program.TransportStreamProgram"
]


# --- restJson1 ser/de ---
def serialize_json(value: __listOfTransportStreamProgram) -> list:
    import aws_sdk_mediaconnect.types.transport_stream_program

    out: list = []
    for item in value:
        out.append(
            aws_sdk_mediaconnect.types.transport_stream_program.serialize_json(item)
        )
    return out


def deserialize_json(data: list) -> __listOfTransportStreamProgram:
    import aws_sdk_mediaconnect.types.transport_stream_program

    out: __listOfTransportStreamProgram = []
    for item in data:
        out.append(
            aws_sdk_mediaconnect.types.transport_stream_program.deserialize_json(item)
        )
    return out
