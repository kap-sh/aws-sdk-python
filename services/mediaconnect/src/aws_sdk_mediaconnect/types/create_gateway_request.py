...
class CreateGatewayRequest(TypedDict):
    egress_cidr_blocks: NotRequired["aws_sdk_mediaconnect.types.__list_of_string.__listOfString"]
    ...
    networks: NotRequired["aws_sdk_mediaconnect.types.__list_of_gateway_network.__listOfGatewayNetwork"]
...
def serialize_json(value: CreateGatewayRequest) -> dict:
    ...
    if "egress_cidr_blocks" in value:
        import aws_sdk_mediaconnect.types.__list_of_string
        out["egressCidrBlocks"] = aws_sdk_mediaconnect.types.__list_of_string.serialize_json(value["egress_cidr_blocks"])
    if "name" in value:
        out["name"] = value["name"]
    if "networks" in value:
        import aws_sdk_mediaconnect.types.__list_of_gateway_network
        out["networks"] = aws_sdk_mediaconnect.types.__list_of_gateway_network.serialize_json(value["networks"])
    return out
...
def deserialize_json(data: dict) -> CreateGatewayRequest:
    ...
    if "egressCidrBlocks" in data:
        import aws_sdk_mediaconnect.types.__list_of_string
        out["egress_cidr_blocks"] = aws_sdk_mediaconnect.types.__list_of_string.deserialize_json(data["egressCidrBlocks"])
    if "name" in data:
        out["name"] = data["name"]
    if "networks" in data:
        import aws_sdk_mediaconnect.types.__list_of_gateway_network
        out["networks"] = aws_sdk_mediaconnect.types.__list_of_gateway_network.deserialize_json(data["networks"])
    return out