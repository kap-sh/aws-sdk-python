"""Generated from Smithy shape ``com.amazonaws.mediaconnect#RouterInputTypeList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_mediaconnect.types.router_input_type

RouterInputTypeList: TypeAlias = list[
    "aws_sdk_mediaconnect.types.router_input_type.RouterInputType"
]


# --- restJson1 ser/de ---
def serialize_json(value: RouterInputTypeList) -> list:
    import aws_sdk_mediaconnect.types.router_input_type

    out: list = []
    for item in value:
        out.append(aws_sdk_mediaconnect.types.router_input_type.serialize_json(item))
    return out


def deserialize_json(data: list) -> RouterInputTypeList:
    import aws_sdk_mediaconnect.types.router_input_type

    out: RouterInputTypeList = []
    for item in data:
        out.append(aws_sdk_mediaconnect.types.router_input_type.deserialize_json(item))
    return out
