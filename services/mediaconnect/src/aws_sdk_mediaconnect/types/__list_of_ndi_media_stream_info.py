...
if TYPE_CHECKING:
    import aws_sdk_mediaconnect.types.ndi_media_stream_info

__listOfNdiMediaStreamInfo: TypeAlias = list[
    "aws_sdk_mediaconnect.types.ndi_media_stream_info.NdiMediaStreamInfo"
]
...
def serialize_json(value: __listOfNdiMediaStreamInfo) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_mediaconnect.types.ndi_media_stream_info.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> __listOfNdiMediaStreamInfo:
    ...
    for item in data:
        out.append(
            aws_sdk_mediaconnect.types.ndi_media_stream_info.deserialize_json(item)
        )
    return out
