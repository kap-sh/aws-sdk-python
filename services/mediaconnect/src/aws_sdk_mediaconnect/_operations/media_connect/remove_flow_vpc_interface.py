"""Generated from Smithy shape ``com.amazonaws.mediaconnect#RemoveFlowVpcInterface``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.remove_flow_vpc_interface_response.RemoveFlowVpcInterfaceResponse = aws_sdk_mediaconnect.types.remove_flow_vpc_interface_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.remove_flow_vpc_interface_request.RemoveFlowVpcInterfaceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{VpcInterfaceName}", quote(str(input_["vpc_interface_name"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_remove_flow_vpc_interface(
