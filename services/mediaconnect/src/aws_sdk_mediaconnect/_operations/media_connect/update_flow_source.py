"""Generated from Smithy shape ``com.amazonaws.mediaconnect#UpdateFlowSource``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.update_flow_source_response.UpdateFlowSourceResponse = aws_sdk_mediaconnect.types.update_flow_source_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_mediaconnect.types.update_flow_source_request.UpdateFlowSourceRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/flows/{FlowArn}/source/{SourceArn}"
    url = url.replace("{FlowArn}", quote(str(input["flow_arn"]), safe=""))
    url = url.replace("{SourceArn}", quote(str(input["source_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_mediaconnect.types.update_flow_source_request

    body: bytes | None = json.dumps(
        aws_sdk_mediaconnect.types.update_flow_source_request.serialize_json(input)
    ).encode()
...
async def async_update_flow_source(
