"""Generated from Smithy shape ``com.amazonaws.mediaconnect#DescribeOffering``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.describe_offering_response.DescribeOfferingResponse = aws_sdk_mediaconnect.types.describe_offering_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_mediaconnect.types.describe_offering_request.DescribeOfferingRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/offerings/{OfferingArn}"
    url = url.replace("{OfferingArn}", quote(str(input["offering_arn"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_offering(
