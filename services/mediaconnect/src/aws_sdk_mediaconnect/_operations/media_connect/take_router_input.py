...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.take_router_input_response.TakeRouterInputResponse = aws_sdk_mediaconnect.types.take_router_input_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_mediaconnect.types.take_router_input_request.TakeRouterInputRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{RouterOutputArn}", quote(str(input["router_output_arn"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_mediaconnect.types.take_router_input_request.serialize_json(input)
    ).encode()
