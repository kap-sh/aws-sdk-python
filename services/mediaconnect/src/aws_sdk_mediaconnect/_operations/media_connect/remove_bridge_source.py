"""Generated from Smithy shape ``com.amazonaws.mediaconnect#RemoveBridgeSource``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.remove_bridge_source_response.RemoveBridgeSourceResponse = aws_sdk_mediaconnect.types.remove_bridge_source_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.remove_bridge_source_request.RemoveBridgeSourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/bridges/{BridgeArn}/sources/{SourceName}"
    url = url.replace("{BridgeArn}", quote(str(input_["bridge_arn"]), safe=""))
    url = url.replace("{SourceName}", quote(str(input_["source_name"]), safe=""))
    params: dict[str, str] = {}
...
async def async_remove_bridge_source(
