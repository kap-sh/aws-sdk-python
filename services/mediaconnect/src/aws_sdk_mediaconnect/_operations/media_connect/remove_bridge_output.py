"""Generated from Smithy shape ``com.amazonaws.mediaconnect#RemoveBridgeOutput``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.remove_bridge_output_response.RemoveBridgeOutputResponse = aws_sdk_mediaconnect.types.remove_bridge_output_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.remove_bridge_output_request.RemoveBridgeOutputRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/bridges/{BridgeArn}/outputs/{OutputName}"
    url = url.replace("{BridgeArn}", quote(str(input_["bridge_arn"]), safe=""))
    url = url.replace("{OutputName}", quote(str(input_["output_name"]), safe=""))
    params: dict[str, str] = {}
...
async def async_remove_bridge_output(
