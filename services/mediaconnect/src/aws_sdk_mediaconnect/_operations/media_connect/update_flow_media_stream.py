"""Generated from Smithy shape ``com.amazonaws.mediaconnect#UpdateFlowMediaStream``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.update_flow_media_stream_response.UpdateFlowMediaStreamResponse = aws_sdk_mediaconnect.types.update_flow_media_stream_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.update_flow_media_stream_request.UpdateFlowMediaStreamRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/flows/{FlowArn}/mediaStreams/{MediaStreamName}"
    )
    url = url.replace("{FlowArn}", quote(str(input_["flow_arn"]), safe=""))
    url = url.replace(
        "{MediaStreamName}", quote(str(input_["media_stream_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_mediaconnect.types.update_flow_media_stream_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_flow_media_stream(
