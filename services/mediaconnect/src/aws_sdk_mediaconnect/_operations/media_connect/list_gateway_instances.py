"""Generated from Smithy shape ``com.amazonaws.mediaconnect#ListGatewayInstances``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.list_gateway_instances_response.ListGatewayInstancesResponse = aws_sdk_mediaconnect.types.list_gateway_instances_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.list_gateway_instances_request.ListGatewayInstancesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/gateway-instances"
    params: dict[str, str] = {}
    if "filter_arn" in input_:
        params["filterArn"] = str(input_["filter_arn"])
...
async def async_list_gateway_instances(
