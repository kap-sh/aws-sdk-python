"""Generated from Smithy shape ``com.amazonaws.mediaconnect#AddFlowSources``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.add_flow_sources_response.AddFlowSourcesResponse = (
        aws_sdk_mediaconnect.types.add_flow_sources_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.add_flow_sources_request.AddFlowSourcesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/flows/{FlowArn}/source"
    url = url.replace("{FlowArn}", quote(str(input_["flow_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_mediaconnect.types.add_flow_sources_request.serialize_json(input_)
    ).encode()
...
async def async_add_flow_sources(
