"""Generated from Smithy shape ``com.amazonaws.mediaconnect#UpdateFlowOutput``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.update_flow_output_response.UpdateFlowOutputResponse = aws_sdk_mediaconnect.types.update_flow_output_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.update_flow_output_request.UpdateFlowOutputRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/flows/{FlowArn}/outputs/{OutputArn}"
    url = url.replace("{FlowArn}", quote(str(input_["flow_arn"]), safe=""))
    url = url.replace("{OutputArn}", quote(str(input_["output_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_mediaconnect.types.update_flow_output_request

    body: bytes | None = json.dumps(
        aws_sdk_mediaconnect.types.update_flow_output_request.serialize_json(input_)
    ).encode()
...
async def async_update_flow_output(
