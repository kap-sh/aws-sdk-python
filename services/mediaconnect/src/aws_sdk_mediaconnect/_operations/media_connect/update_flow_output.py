...
def build_request(
    ...
    input: aws_sdk_mediaconnect.types.update_flow_output_request.UpdateFlowOutputRequest,
) -> zapros.Request:
    ...
    url = url.replace("{OutputArn}", quote(str(input["output_arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_mediaconnect.types.update_flow_output_request

    body: bytes | None = json.dumps(
        aws_sdk_mediaconnect.types.update_flow_output_request.serialize_json(input)
    ).encode()
