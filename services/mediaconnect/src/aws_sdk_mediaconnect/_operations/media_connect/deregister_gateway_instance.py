"""Generated from Smithy shape ``com.amazonaws.mediaconnect#DeregisterGatewayInstance``."""
...
def handle_response(
    ...
    out: aws_sdk_mediaconnect.types.deregister_gateway_instance_response.DeregisterGatewayInstanceResponse = aws_sdk_mediaconnect.types.deregister_gateway_instance_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_mediaconnect.types.deregister_gateway_instance_request.DeregisterGatewayInstanceRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/gateway-instances/{GatewayInstanceArn}"
    url = url.replace(
        "{GatewayInstanceArn}", quote(str(input["gateway_instance_arn"]), safe="")
    )
    params: dict[str, str] = {}
    if "force" in input:
        params["force"] = str(input["force"])
...
async def async_deregister_gateway_instance(
