...
def build_request(
    ...
    input_: aws_sdk_mediaconnect.types.deregister_gateway_instance_request.DeregisterGatewayInstanceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{GatewayInstanceArn}", quote(str(input_["gateway_instance_arn"]), safe="")
    )
    params: dict[str, str] = {}
    if "force" in input_:
