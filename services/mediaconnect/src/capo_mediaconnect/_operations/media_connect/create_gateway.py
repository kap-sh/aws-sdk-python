"""Generated from Smithy shape ``com.amazonaws.mediaconnect#CreateGateway``."""
...
import capo_mediaconnect.errors.create_gateway420_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "CreateGateway420Exception":
            raise capo_mediaconnect.errors.create_gateway420_exception.CreateGateway420Exception.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_mediaconnect.types.create_gateway_response.CreateGatewayResponse = (
        capo_mediaconnect.types.create_gateway_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/gateways"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.create_gateway_request.serialize_json(input_)
    ).encode()
