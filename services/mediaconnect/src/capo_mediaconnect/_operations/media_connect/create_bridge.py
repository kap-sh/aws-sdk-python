"""Generated from Smithy shape ``com.amazonaws.mediaconnect#CreateBridge``."""
...
import capo_mediaconnect.errors.create_bridge420_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "CreateBridge420Exception":
            raise capo_mediaconnect.errors.create_bridge420_exception.CreateBridge420Exception.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_mediaconnect.types.create_bridge_response.CreateBridgeResponse = (
        capo_mediaconnect.types.create_bridge_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/bridges"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.create_bridge_request.serialize_json(input_)
    ).encode()
...
def create_bridge(
    ...
    capo_mediaconnect.types.create_bridge_response.CreateBridgeResponse, zapros.Response
]:
