"""Generated from Smithy shape ``com.amazonaws.mediaconnect#TakeRouterInput``."""
...
def handle_response(
    ...
    out: capo_mediaconnect.types.take_router_input_response.TakeRouterInputResponse = (
        capo_mediaconnect.types.take_router_input_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/routerOutput/takeRouterInput/{RouterOutputArn}"
    )
    url = url.replace(
        "{RouterOutputArn}", quote(str(input_["router_output_arn"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mediaconnect.types.take_router_input_request.serialize_json(input_)
    ).encode()
...
async def async_take_router_input(
