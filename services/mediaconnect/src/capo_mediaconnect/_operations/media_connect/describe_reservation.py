"""Generated from Smithy shape ``com.amazonaws.mediaconnect#DescribeReservation``."""
...
def handle_response(
    ...
    out: capo_mediaconnect.types.describe_reservation_response.DescribeReservationResponse = capo_mediaconnect.types.describe_reservation_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/reservations/{ReservationArn}"
    url = url.replace(
        "{ReservationArn}", quote(str(input_["reservation_arn"]), safe="")
    )
    params: dict[str, str] = {}
