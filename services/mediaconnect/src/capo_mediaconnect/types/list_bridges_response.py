...
if TYPE_CHECKING:
    import capo_mediaconnect.types.__list_of_listed_bridge


class ListBridgesResponse(TypedDict, closed=True):
    bridges: NotRequired[
        "capo_mediaconnect.types.__list_of_listed_bridge.__listOfListedBridge"
    ]
...
def serialize_json(value: ListBridgesResponse) -> dict:
    ...
    if "bridges" in value:
        ...
        out["bridges"] = capo_mediaconnect.types.__list_of_listed_bridge.serialize_json(
            value["bridges"]
        )
...
def deserialize_json(data: dict) -> ListBridgesResponse:
    ...
    if "bridges" in data:
        ...
        out["bridges"] = (
            capo_mediaconnect.types.__list_of_listed_bridge.deserialize_json(
                data["bridges"]
            )
        )
