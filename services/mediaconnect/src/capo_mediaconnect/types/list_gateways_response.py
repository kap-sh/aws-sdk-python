...
class ListGatewaysResponse(TypedDict, closed=True):
    gateways: NotRequired[
        "capo_mediaconnect.types.__list_of_listed_gateway.__listOfListedGateway"
    ]
...
def serialize_json(value: ListGatewaysResponse) -> dict:
    ...
    if "gateways" in value:
        ...
        out["gateways"] = (
            capo_mediaconnect.types.__list_of_listed_gateway.serialize_json(
                value["gateways"]
            )
        )
...
def deserialize_json(data: dict) -> ListGatewaysResponse:
    ...
    if "gateways" in data:
        ...
        out["gateways"] = (
            capo_mediaconnect.types.__list_of_listed_gateway.deserialize_json(
                data["gateways"]
            )
        )
