...
class Fmtp(TypedDict, closed=True):
    ...
    colorimetry: NotRequired["capo_mediaconnect.types.colorimetry.Colorimetry"]
    ...
    range: NotRequired["capo_mediaconnect.types.range.Range"]
    ...
    tcs: NotRequired["capo_mediaconnect.types.tcs.Tcs"]
    """<p>The transfer characteristic system (TCS) that is used in the video. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: Fmtp) -> dict:
    out: dict = {}
    ...
    if "colorimetry" in value:
        ...
        out["colorimetry"] = capo_mediaconnect.types.colorimetry.serialize_json(
            value["colorimetry"]
        )
    ...
    if "range" in value:
        ...
        out["range"] = capo_mediaconnect.types.range.serialize_json(value["range"])
    ...
    if "tcs" in value:
        ...
        out["tcs"] = capo_mediaconnect.types.tcs.serialize_json(value["tcs"])
    return out


def deserialize_json(data: dict) -> Fmtp:
    out: Fmtp = {}  # type: ignore[typeddict-item]
