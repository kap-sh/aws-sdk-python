...
class FailoverConfig(TypedDict, closed=True):
    failover_mode: NotRequired["capo_mediaconnect.types.failover_mode.FailoverMode"]
    ...
    source_priority: NotRequired[
        "capo_mediaconnect.types.source_priority.SourcePriority"
    ]
...
def serialize_json(value: FailoverConfig) -> dict:
    ...
    if "failover_mode" in value:
        ...
        out["failoverMode"] = capo_mediaconnect.types.failover_mode.serialize_json(
            value["failover_mode"]
        )
    ...
    if "source_priority" in value:
        ...
        out["sourcePriority"] = capo_mediaconnect.types.source_priority.serialize_json(
...
def deserialize_json(data: dict) -> FailoverConfig:
    ...
    if "sourcePriority" in data:
        ...
        out["source_priority"] = (
            capo_mediaconnect.types.source_priority.deserialize_json(
                data["sourcePriority"]
            )
        )
    if "state" in data:
