...
class ListEntitlementsResponse(TypedDict, closed=True):
    entitlements: NotRequired[
        "capo_mediaconnect.types.__list_of_listed_entitlement.__listOfListedEntitlement"
    ]
...
def serialize_json(value: ListEntitlementsResponse) -> dict:
    ...
    if "entitlements" in value:
        ...
        out["entitlements"] = (
            capo_mediaconnect.types.__list_of_listed_entitlement.serialize_json(
                value["entitlements"]
            )
        )
...
def deserialize_json(data: dict) -> ListEntitlementsResponse:
    ...
    if "entitlements" in data:
        ...
        out["entitlements"] = (
            capo_mediaconnect.types.__list_of_listed_entitlement.deserialize_json(
                data["entitlements"]
            )
        )
