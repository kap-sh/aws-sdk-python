...
if TYPE_CHECKING:
    import capo_kafkaconnect.types.auto_scaling_description
    import capo_kafkaconnect.types.provisioned_capacity_description


class CapacityDescription(TypedDict, closed=True):
    auto_scaling: NotRequired[
        "capo_kafkaconnect.types.auto_scaling_description.AutoScalingDescription"
    ]
    """<p>Describes the connector's auto scaling capacity.</p>"""
    provisioned_capacity: NotRequired[
        "capo_kafkaconnect.types.provisioned_capacity_description.ProvisionedCapacityDescription"
    ]
...
def serialize_json(value: CapacityDescription) -> dict:
    ...
    if "auto_scaling" in value:
        ...
        out["autoScaling"] = (
            capo_kafkaconnect.types.auto_scaling_description.serialize_json(
                value["auto_scaling"]
            )
        )
    if "provisioned_capacity" in value:
        ...
        out["provisionedCapacity"] = (
            capo_kafkaconnect.types.provisioned_capacity_description.serialize_json(
...
def deserialize_json(data: dict) -> CapacityDescription:
    ...
    if "autoScaling" in data:
        ...
        out["auto_scaling"] = (
            capo_kafkaconnect.types.auto_scaling_description.deserialize_json(
                data["autoScaling"]
            )
        )
    if "provisionedCapacity" in data:
        ...
        out["provisioned_capacity"] = (
            capo_kafkaconnect.types.provisioned_capacity_description.deserialize_json(
