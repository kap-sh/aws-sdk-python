...
if TYPE_CHECKING:
    import capo_codestar_connections.types.repository_sync_event_list
    import capo_codestar_connections.types.repository_sync_status
...
class RepositorySyncAttempt(TypedDict, closed=True):
    ...
    status: (
        "capo_codestar_connections.types.repository_sync_status.RepositorySyncStatus"
    )
    ...
    events: "capo_codestar_connections.types.repository_sync_event_list.RepositorySyncEventList"
...
def serialize_aws_json_1_0(value: RepositorySyncAttempt) -> dict:
    ...
    out["Status"] = (
        capo_codestar_connections.types.repository_sync_status.serialize_aws_json_1_0(
            value["status"]
        )
    )
    import capo_codestar_connections.types.repository_sync_event_list
    ...
    out["Events"] = (
        capo_codestar_connections.types.repository_sync_event_list.serialize_aws_json_1_0(
            value["events"]
        )
    )
    return out
...
def deserialize_aws_json_1_0(data: dict) -> RepositorySyncAttempt:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            capo_codestar_connections.types.repository_sync_status.deserialize_aws_json_1_0(
                data["Status"]
            )
        )
    else:
    ...
    if "Events" in data:
        ...
        out["events"] = (
            capo_codestar_connections.types.repository_sync_event_list.deserialize_aws_json_1_0(
                data["Events"]
            )
        )
    else:
