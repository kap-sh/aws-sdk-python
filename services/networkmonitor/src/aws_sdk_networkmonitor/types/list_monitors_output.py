...
class ListMonitorsOutput(TypedDict, closed=True):
    monitors: "aws_sdk_networkmonitor.types.monitor_list.MonitorList"
...
def serialize_json(value: ListMonitorsOutput) -> dict:
    ...
    out["monitors"] = aws_sdk_networkmonitor.types.monitor_list.serialize_json(
        value["monitors"]
    )
...
def deserialize_json(data: dict) -> ListMonitorsOutput:
    ...
    if "monitors" in data:
        ...
        out["monitors"] = aws_sdk_networkmonitor.types.monitor_list.deserialize_json(
            data["monitors"]
        )
    else:
