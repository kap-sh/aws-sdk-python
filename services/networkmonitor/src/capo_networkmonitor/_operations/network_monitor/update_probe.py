"""Generated from Smithy shape ``com.amazonaws.networkmonitor#UpdateProbe``."""
...
import capo_networkmonitor.errors.resource_not_found_exception
import capo_networkmonitor.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            raise capo_networkmonitor.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
        case "ServiceQuotaExceededException":
            raise capo_networkmonitor.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_networkmonitor.types.update_probe_output.UpdateProbeOutput = (
        capo_networkmonitor.types.update_probe_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_networkmonitor._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/monitors/{monitorName}/probes/{probeId}"
    url = url.replace("{monitorName}", quote(str(input_["monitor_name"]), safe=""))
    url = url.replace("{probeId}", quote(str(input_["probe_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_networkmonitor.types.update_probe_input.serialize_json(input_)
    ).encode()
...
def update_probe(
    ...
    capo_networkmonitor.types.update_probe_output.UpdateProbeOutput, zapros.Response
]:
