...
class CreateMonitorProbeInput(TypedDict, closed=True):
    ...
    probe_tags: NotRequired["capo_networkmonitor.types.tag_map.TagMap"]
    """<p>The list of key-value pairs created and assigned to the monitor.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: CreateMonitorProbeInput) -> dict:
    out: dict = {}
    ...
    if "probe_tags" in value:
        import capo_networkmonitor.types.tag_map

        out["probeTags"] = capo_networkmonitor.types.tag_map.serialize_json(
            value["probe_tags"]
        )
    return out
