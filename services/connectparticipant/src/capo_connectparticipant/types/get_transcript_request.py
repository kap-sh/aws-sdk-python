...
class GetTranscriptRequest(TypedDict, closed=True):
    contact_id: NotRequired["capo_connectparticipant.types.contact_id.ContactId"]
    """<p>The contactId from the current contact chain for which transcript is needed.</p>"""
    max_results: NotRequired["capo_connectparticipant.types.max_results.MaxResults"]
    ...
    sort_order: NotRequired["capo_connectparticipant.types.sort_key.SortKey"]
...
def serialize_json(value: GetTranscriptRequest) -> dict:
    ...
    if "scan_direction" in value:
        ...
        out["ScanDirection"] = (
            capo_connectparticipant.types.scan_direction.serialize_json(
    ...
    if "sort_order" in value:
        ...
        out["SortOrder"] = capo_connectparticipant.types.sort_key.serialize_json(
            value["sort_order"]
        )
    if "start_position" in value:
        ...
        out["StartPosition"] = (
            capo_connectparticipant.types.start_position.serialize_json(
...
def deserialize_json(data: dict) -> GetTranscriptRequest:
    ...
    if "ScanDirection" in data:
        ...
        out["scan_direction"] = (
            capo_connectparticipant.types.scan_direction.deserialize_json(
                data["ScanDirection"]
            )
        )
    ...
    if "StartPosition" in data:
        ...
        out["start_position"] = (
            capo_connectparticipant.types.start_position.deserialize_json(
                data["StartPosition"]
            )
        )
    return out
