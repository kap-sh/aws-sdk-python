"""Generated from Smithy shape ``com.amazonaws.appstream#UpdateStackResult``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import aws_sdk_appstream.types.stack


class UpdateStackResult(TypedDict, closed=True):
    stack: NotRequired["aws_sdk_appstream.types.stack.Stack"]
    """<p>Information about the stack.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: UpdateStackResult) -> dict:
    out: dict = {}
    if "stack" in value:
        import aws_sdk_appstream.types.stack

        out["Stack"] = aws_sdk_appstream.types.stack.serialize_aws_json_1_1(
            value["stack"]
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> UpdateStackResult:
    out: UpdateStackResult = {}  # type: ignore[typeddict-item]
    if "Stack" in data:
        import aws_sdk_appstream.types.stack

        out["stack"] = aws_sdk_appstream.types.stack.deserialize_aws_json_1_1(
            data["Stack"]
        )
    return out
