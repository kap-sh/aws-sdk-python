...
class CreateUpdatedImageRequest(TypedDict):
    existing_image_name: NotRequired["aws_sdk_appstream.types.name.Name"]
    """<p>The name of the image to update.</p>"""
    new_image_name: NotRequired["aws_sdk_appstream.types.name.Name"]
    """<p>The name of the new image. The name must be unique within the AWS account and Region.</p>"""
    new_image_description: NotRequired[
        "aws_sdk_appstream.types.description.Description"
    ]
    """<p>The description to display for the new image.</p>"""
    new_image_display_name: NotRequired[
        "aws_sdk_appstream.types.display_name.DisplayName"
    ]
    """<p>The name to display for the new image.</p>"""
    new_image_tags: NotRequired["aws_sdk_appstream.types.tags.Tags"]
...
def serialize_aws_json_1_1(value: CreateUpdatedImageRequest) -> dict:
    ...
    if "new_image_tags" in value:
        ...
        out["newImageTags"] = aws_sdk_appstream.types.tags.serialize_aws_json_1_1(
...
def deserialize_aws_json_1_1(data: dict) -> CreateUpdatedImageRequest:
    ...
    if "newImageTags" in data:
        ...
        out["new_image_tags"] = aws_sdk_appstream.types.tags.deserialize_aws_json_1_1(
            data["newImageTags"]
        )
    if "dryRun" in data:
