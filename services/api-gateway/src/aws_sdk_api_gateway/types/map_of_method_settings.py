"""Generated from Smithy shape ``com.amazonaws.apigateway#MapOfMethodSettings``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_api_gateway.types.method_setting
    import aws_sdk_api_gateway.types.string

MapOfMethodSettings: TypeAlias = dict[
    "aws_sdk_api_gateway.types.string.String",
    "aws_sdk_api_gateway.types.method_setting.MethodSetting",
]


# --- restJson1 ser/de ---
def serialize_json(input_to_serialize: MapOfMethodSettings) -> dict:
    out: dict = {}
    for key, value in input_to_serialize.items():
        import aws_sdk_api_gateway.types.method_setting

        out[key] = aws_sdk_api_gateway.types.method_setting.serialize_json(value)
    return out


def deserialize_json(data: dict) -> MapOfMethodSettings:
    out: MapOfMethodSettings = {}
    for key, value in data.items():
        import aws_sdk_api_gateway.types.method_setting

        out[key] = aws_sdk_api_gateway.types.method_setting.deserialize_json(value)
    return out
