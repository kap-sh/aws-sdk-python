"""Generated from Smithy shape ``com.amazonaws.apigateway#MapOfMethodSnapshot``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_api_gateway.types.method_snapshot
    import aws_sdk_api_gateway.types.string

MapOfMethodSnapshot: TypeAlias = dict[
    "aws_sdk_api_gateway.types.string.String",
    "aws_sdk_api_gateway.types.method_snapshot.MethodSnapshot",
]


# --- restJson1 ser/de ---
def serialize_json(input_to_serialize: MapOfMethodSnapshot) -> dict:
    out: dict = {}
    for key, value in input_to_serialize.items():
        import aws_sdk_api_gateway.types.method_snapshot

        out[key] = aws_sdk_api_gateway.types.method_snapshot.serialize_json(value)
    return out


def deserialize_json(data: dict) -> MapOfMethodSnapshot:
    out: MapOfMethodSnapshot = {}
    for key, value in data.items():
        import aws_sdk_api_gateway.types.method_snapshot

        out[key] = aws_sdk_api_gateway.types.method_snapshot.deserialize_json(value)
    return out
