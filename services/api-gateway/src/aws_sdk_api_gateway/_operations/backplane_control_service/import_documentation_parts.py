...
def build_request(
    ...
    input: aws_sdk_api_gateway.types.import_documentation_parts_request.ImportDocumentationPartsRequest,
) -> zapros.Request:
    ...
    params["failonwarnings"] = str(input.get("fail_on_warnings", False))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "body" in input:
        import aws_sdk_api_gateway.types.blob

        body: bytes | None = json.dumps(
            aws_sdk_api_gateway.types.blob.serialize_json(input["body"])
        ).encode()
