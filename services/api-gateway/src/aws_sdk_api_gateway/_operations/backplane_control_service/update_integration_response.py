"""Generated from Smithy shape ``com.amazonaws.apigateway#UpdateIntegrationResponse``."""
...
def handle_response(
    ...
    out: aws_sdk_api_gateway.types.integration_response.IntegrationResponse = (
        aws_sdk_api_gateway.types.integration_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_api_gateway.types.update_integration_response_request.UpdateIntegrationResponseRequest,
) -> zapros.Request:
    ...
    url = url.replace("{httpMethod}", quote(str(input["http_method"]), safe=""))
    url = url.replace("{statusCode}", quote(str(input["status_code"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_api_gateway.types.update_integration_response_request

    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.update_integration_response_request.serialize_json(
            input
        )
    ).encode()
...
def update_integration_response(
    ...
    aws_sdk_api_gateway.types.integration_response.IntegrationResponse, zapros.Response
]:
...
async def async_update_integration_response(
