"""Generated from Smithy shape ``com.amazonaws.apigateway#UpdateDeployment``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceUnavailableException":
            import aws_sdk_api_gateway.errors.service_unavailable_exception

            raise aws_sdk_api_gateway.errors.service_unavailable_exception.ServiceUnavailableException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_api_gateway.types.deployment.Deployment = (
        aws_sdk_api_gateway.types.deployment.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_api_gateway._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_api_gateway._auth._sigv4.build_sigv4_auth_scheme(
                "apigateway", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_api_gateway._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.update_deployment_request.UpdateDeploymentRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/restapis/{restApiId}/deployments/{deploymentId}"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.update_deployment_request.serialize_json(input_)
    ).encode()
...
def update_deployment(
    ...
    input_: aws_sdk_api_gateway.types.update_deployment_request.UpdateDeploymentRequest,
) -> tuple[aws_sdk_api_gateway.types.deployment.Deployment, zapros.Response]:
