"""Generated from Smithy shape ``com.amazonaws.apigateway#ImportRestApi``."""
...
import aws_sdk_api_gateway.types.api_status
...
def handle_response(
    ...
    out: aws_sdk_api_gateway.types.rest_api.RestApi = (
        aws_sdk_api_gateway.types.rest_api.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: aws_sdk_api_gateway.types.rest_api.RestApi = (
        aws_sdk_api_gateway.types.rest_api.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.import_rest_api_request.ImportRestApiRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/restapis?mode=import"
    params: dict[str, str] = {}
    params["failonwarnings"] = str(input_.get("fail_on_warnings", False))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "body" in input_:
        body: bytes | None = json.dumps(
            aws_sdk_api_gateway.types.blob.serialize_json(input_["body"])
        ).encode()
...
def import_rest_api(
    ...
    input_: aws_sdk_api_gateway.types.import_rest_api_request.ImportRestApiRequest,
) -> tuple[aws_sdk_api_gateway.types.rest_api.RestApi, zapros.Response]:
...
async def async_import_rest_api(
