"""Generated from Smithy shape ``com.amazonaws.apigateway#CreateStage``."""
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.create_stage_request.CreateStageRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/restapis/{restApiId}/stages"
    url = url.replace("{restApiId}", quote(str(input_["rest_api_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.create_stage_request.serialize_json(input_)
    ).encode()
