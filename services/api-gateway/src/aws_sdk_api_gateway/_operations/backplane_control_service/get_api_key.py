"""Generated from Smithy shape ``com.amazonaws.apigateway#GetApiKey``."""
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.get_api_key_request.GetApiKeyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/apikeys/{apiKey}"
    url = url.replace("{apiKey}", quote(str(input_["api_key"]), safe=""))
    params: dict[str, str] = {}
    if "include_value" in input_:
        params["includeValue"] = str(input_["include_value"])
