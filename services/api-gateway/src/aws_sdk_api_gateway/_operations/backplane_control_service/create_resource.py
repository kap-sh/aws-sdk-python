"""Generated from Smithy shape ``com.amazonaws.apigateway#CreateResource``."""
...
def handle_response(
    ...
    out: aws_sdk_api_gateway.types.resource.Resource = (
        aws_sdk_api_gateway.types.resource.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: aws_sdk_api_gateway.types.resource.Resource = (
        aws_sdk_api_gateway.types.resource.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.create_resource_request.CreateResourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/restapis/{restApiId}/resources/{parentId}"
    url = url.replace("{restApiId}", quote(str(input_["rest_api_id"]), safe=""))
    url = url.replace("{parentId}", quote(str(input_["parent_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_api_gateway.types.create_resource_request

    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.create_resource_request.serialize_json(input_)
    ).encode()
...
def create_resource(
    ...
    input_: aws_sdk_api_gateway.types.create_resource_request.CreateResourceRequest,
) -> tuple[aws_sdk_api_gateway.types.resource.Resource, zapros.Response]:
