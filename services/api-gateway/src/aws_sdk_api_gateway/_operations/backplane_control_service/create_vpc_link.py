"""Generated from Smithy shape ``com.amazonaws.apigateway#CreateVpcLink``."""
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.create_vpc_link_request.CreateVpcLinkRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/vpclinks"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_api_gateway.types.create_vpc_link_request

    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.create_vpc_link_request.serialize_json(input_)
    ).encode()
...
async def async_create_vpc_link(
