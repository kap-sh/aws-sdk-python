"""Generated from Smithy shape ``com.amazonaws.apigateway#UpdateVpcLink``."""
...
def build_request(
    ...
    input_: aws_sdk_api_gateway.types.update_vpc_link_request.UpdateVpcLinkRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/vpclinks/{vpcLinkId}"
    url = url.replace("{vpcLinkId}", quote(str(input_["vpc_link_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_api_gateway.types.update_vpc_link_request

    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.update_vpc_link_request.serialize_json(input_)
    ).encode()
...
def update_vpc_link(
    ...
    input_: aws_sdk_api_gateway.types.update_vpc_link_request.UpdateVpcLinkRequest,
) -> tuple[aws_sdk_api_gateway.types.vpc_link.VpcLink, zapros.Response]:
...
async def async_update_vpc_link(
