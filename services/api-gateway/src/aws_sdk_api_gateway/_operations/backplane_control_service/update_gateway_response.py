...
def build_request(
    ...
    input: aws_sdk_api_gateway.types.update_gateway_response_request.UpdateGatewayResponseRequest,
) -> zapros.Request:
    ...
    url = url.replace("{responseType}", quote(str(input["response_type"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_api_gateway.types.update_gateway_response_request

    body: bytes | None = json.dumps(
        aws_sdk_api_gateway.types.update_gateway_response_request.serialize_json(input)
    ).encode()
