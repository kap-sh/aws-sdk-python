...
ListOfAuthorizer: TypeAlias = list["capo_api_gateway.types.authorizer.Authorizer"]
...
def serialize_json(value: ListOfAuthorizer) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_api_gateway.types.authorizer.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ListOfAuthorizer:
    ...
    for item in data:
        out.append(capo_api_gateway.types.authorizer.deserialize_json(item))
    return out
