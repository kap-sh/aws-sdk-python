...
class ClientCertificates(TypedDict, closed=True):
    items: NotRequired[
        "capo_api_gateway.types.list_of_client_certificate.ListOfClientCertificate"
    ]
...
def serialize_json(value: ClientCertificates) -> dict:
    ...
    if "items" in value:
        ...
        out["item"] = capo_api_gateway.types.list_of_client_certificate.serialize_json(
            value["items"]
        )
    return out
...
def deserialize_json(data: dict) -> ClientCertificates:
    ...
    if "item" in data:
        ...
        out["items"] = (
            capo_api_gateway.types.list_of_client_certificate.deserialize_json(
                data["item"]
            )
        )
    return out
