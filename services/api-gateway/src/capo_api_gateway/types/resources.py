...
class Resources(TypedDict, closed=True):
    items: NotRequired["capo_api_gateway.types.list_of_resource.ListOfResource"]
...
def serialize_json(value: Resources) -> dict:
    ...
    if "items" in value:
        ...
        out["item"] = capo_api_gateway.types.list_of_resource.serialize_json(
            value["items"]
        )
    return out
...
def deserialize_json(data: dict) -> Resources:
    ...
    if "item" in data:
        ...
        out["items"] = capo_api_gateway.types.list_of_resource.deserialize_json(
            data["item"]
        )
    return out
