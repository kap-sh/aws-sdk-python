"""Generated from Smithy shape ``com.amazonaws.apigateway#CreateUsagePlan``."""
...
def handle_response(
    ...
    out: capo_api_gateway.types.usage_plan.UsagePlan = (
        capo_api_gateway.types.usage_plan.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_api_gateway.types.usage_plan.UsagePlan = (
        capo_api_gateway.types.usage_plan.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/usageplans"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_api_gateway.types.create_usage_plan_request.serialize_json(input_)
    ).encode()
...
def create_usage_plan(
    ...
    input_: capo_api_gateway.types.create_usage_plan_request.CreateUsagePlanRequest,
) -> tuple[capo_api_gateway.types.usage_plan.UsagePlan, zapros.Response]:
...
async def async_create_usage_plan(
