"""Generated from Smithy shape ``com.amazonaws.apigateway#CreateUsagePlanKey``."""
...
def handle_response(
    ...
    out: capo_api_gateway.types.usage_plan_key.UsagePlanKey = (
        capo_api_gateway.types.usage_plan_key.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/usageplans/{usagePlanId}/keys"
    url = url.replace("{usagePlanId}", quote(str(input_["usage_plan_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_api_gateway.types.create_usage_plan_key_request.serialize_json(input_)
    ).encode()
...
def create_usage_plan_key(
    ...
    input_: capo_api_gateway.types.create_usage_plan_key_request.CreateUsagePlanKeyRequest,
) -> tuple[capo_api_gateway.types.usage_plan_key.UsagePlanKey, zapros.Response]:
...
async def async_create_usage_plan_key(
