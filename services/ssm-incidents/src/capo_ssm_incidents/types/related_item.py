...
class RelatedItem(TypedDict, closed=True):
    identifier: "capo_ssm_incidents.types.item_identifier.ItemIdentifier"
    """<p>Details about the related item.</p>"""
    title: NotRequired["str"]
    """<p>The title of the related item.</p>"""
    generated_id: NotRequired["capo_ssm_incidents.types.generated_id.GeneratedId"]
...
def serialize_json(value: RelatedItem) -> dict:
    ...
    out["identifier"] = capo_ssm_incidents.types.item_identifier.serialize_json(
        value["identifier"]
    )
    if "title" in value:
...
def deserialize_json(data: dict) -> RelatedItem:
    ...
    if "identifier" in data:
        ...
        out["identifier"] = capo_ssm_incidents.types.item_identifier.deserialize_json(
            data["identifier"]
        )
    else:
