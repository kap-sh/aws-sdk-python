"""Generated from Smithy shape ``com.amazonaws.ssmincidents#CreateReplicationSet``."""
...
import capo_ssm_incidents.errors.conflict_exception
import capo_ssm_incidents.errors.internal_server_exception
import capo_ssm_incidents.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_ssm_incidents.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ServiceQuotaExceededException":
            raise capo_ssm_incidents.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.create_replication_set_output.CreateReplicationSetOutput = capo_ssm_incidents.types.create_replication_set_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/createReplicationSet"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.create_replication_set_input.serialize_json(input_)
    ).encode()
...
async def async_create_replication_set(
