"""Generated from Smithy shape ``com.amazonaws.ssmincidents#ListResponsePlans``."""
...
import capo_ssm_incidents.types.response_plan_summary_list
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.list_response_plans_output.ListResponsePlansOutput = (
        capo_ssm_incidents.types.list_response_plans_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listResponsePlans"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.list_response_plans_input.serialize_json(input_)
    ).encode()
...
async def async_list_response_plans(
