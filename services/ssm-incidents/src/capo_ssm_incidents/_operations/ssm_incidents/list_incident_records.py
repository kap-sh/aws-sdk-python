"""Generated from Smithy shape ``com.amazonaws.ssmincidents#ListIncidentRecords``."""
...
def handle_response(
    ...
    out: capo_ssm_incidents.types.list_incident_records_output.ListIncidentRecordsOutput = capo_ssm_incidents.types.list_incident_records_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listIncidentRecords"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ssm_incidents.types.list_incident_records_input.serialize_json(input_)
    ).encode()
...
async def async_list_incident_records(
