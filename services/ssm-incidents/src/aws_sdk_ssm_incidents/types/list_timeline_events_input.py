...
class ListTimelineEventsInput(TypedDict):
    ...
    filters: NotRequired["aws_sdk_ssm_incidents.types.filter_list.FilterList"]
    ...
    sort_order: NotRequired["aws_sdk_ssm_incidents.types.sort_order.SortOrder"]
...
def serialize_json(value: ListTimelineEventsInput) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = aws_sdk_ssm_incidents.types.filter_list.serialize_json(
            value["filters"]
        )
    if "sort_by" in value:
...
def deserialize_json(data: dict) -> ListTimelineEventsInput:
    ...
    if "filters" in data:
        ...
        out["filters"] = aws_sdk_ssm_incidents.types.filter_list.deserialize_json(
            data["filters"]
        )
    if "sortBy" in data:
