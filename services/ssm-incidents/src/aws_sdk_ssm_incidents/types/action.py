...
def serialize_json(value: Action) -> dict:
    if "ssmAutomation" in value:
        import aws_sdk_ssm_incidents.types.ssm_automation
        return {"ssmAutomation": aws_sdk_ssm_incidents.types.ssm_automation.serialize_json(value["ssmAutomation"])}
    else:
...
def deserialize_json(data: dict) -> Action:
    if "ssmAutomation" in data:
        import aws_sdk_ssm_incidents.types.ssm_automation
        return {"ssmAutomation": aws_sdk_ssm_incidents.types.ssm_automation.deserialize_json(data["ssmAutomation"])}
    else:
