...
AutomationExecutionSet: TypeAlias = list[
    "aws_sdk_ssm_incidents.types.automation_execution.AutomationExecution"
]
...
def serialize_json(value: AutomationExecutionSet) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_ssm_incidents.types.automation_execution.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AutomationExecutionSet:
    ...
    for item in data:
        out.append(
            aws_sdk_ssm_incidents.types.automation_execution.deserialize_json(item)
        )
    return out
