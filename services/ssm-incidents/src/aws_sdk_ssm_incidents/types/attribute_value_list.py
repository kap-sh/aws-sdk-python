...
AttributeValueList: TypeAlias = _AttributeValueList_stringValues | _AttributeValueList_integerValues

# --- restJson1 ser/de ---
def serialize_json(value: AttributeValueList) -> dict:
    if "stringValues" in value:
        import aws_sdk_ssm_incidents.types.string_list
        return {"stringValues": aws_sdk_ssm_incidents.types.string_list.serialize_json(value["stringValues"])}
    elif "integerValues" in value:
        import aws_sdk_ssm_incidents.types.integer_list
        return {"integerValues": aws_sdk_ssm_incidents.types.integer_list.serialize_json(value["integerValues"])}
    else:
...
def deserialize_json(data: dict) -> AttributeValueList:
    if "stringValues" in data:
        import aws_sdk_ssm_incidents.types.string_list
        return {"stringValues": aws_sdk_ssm_incidents.types.string_list.deserialize_json(data["stringValues"])}
    elif "integerValues" in data:
        import aws_sdk_ssm_incidents.types.integer_list
        return {"integerValues": aws_sdk_ssm_incidents.types.integer_list.deserialize_json(data["integerValues"])}
    else:
