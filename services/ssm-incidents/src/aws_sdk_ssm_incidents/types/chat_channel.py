...
def serialize_json(value: ChatChannel) -> dict:
    if "empty" in value:
        import aws_sdk_ssm_incidents.types.empty_chat_channel
        return {"empty": aws_sdk_ssm_incidents.types.empty_chat_channel.serialize_json(value["empty"])}
    elif "chatbotSns" in value:
        import aws_sdk_ssm_incidents.types.chatbot_sns_configuration_set
        return {"chatbotSns": aws_sdk_ssm_incidents.types.chatbot_sns_configuration_set.serialize_json(value["chatbotSns"])}
    else:
...
def deserialize_json(data: dict) -> ChatChannel:
    if "empty" in data:
        import aws_sdk_ssm_incidents.types.empty_chat_channel
        return {"empty": aws_sdk_ssm_incidents.types.empty_chat_channel.deserialize_json(data["empty"])}
    elif "chatbotSns" in data:
        import aws_sdk_ssm_incidents.types.chatbot_sns_configuration_set
        return {"chatbotSns": aws_sdk_ssm_incidents.types.chatbot_sns_configuration_set.deserialize_json(data["chatbotSns"])}
    else:
