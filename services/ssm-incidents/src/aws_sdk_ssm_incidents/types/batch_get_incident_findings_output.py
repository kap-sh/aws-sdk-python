...
def serialize_json(value: BatchGetIncidentFindingsOutput) -> dict:
    ...
    out["findings"] = aws_sdk_ssm_incidents.types.finding_list.serialize_json(
        value["findings"]
    )
    import aws_sdk_ssm_incidents.types.batch_get_incident_findings_error_list

    out["errors"] = (
        aws_sdk_ssm_incidents.types.batch_get_incident_findings_error_list.serialize_json(
            value["errors"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> BatchGetIncidentFindingsOutput:
    ...
    if "findings" in data:
        ...
        out["findings"] = aws_sdk_ssm_incidents.types.finding_list.deserialize_json(
            data["findings"]
        )
    else:
    ...
    if "errors" in data:
        ...
        out["errors"] = (
            aws_sdk_ssm_incidents.types.batch_get_incident_findings_error_list.deserialize_json(
                data["errors"]
            )
        )
    else:
