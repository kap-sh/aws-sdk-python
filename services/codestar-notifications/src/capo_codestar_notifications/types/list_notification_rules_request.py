...
def serialize_json(value: ListNotificationRulesRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["Filters"] = (
            capo_codestar_notifications.types.list_notification_rules_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListNotificationRulesRequest:
    ...
    if "Filters" in data:
        ...
        out["filters"] = (
            capo_codestar_notifications.types.list_notification_rules_filters.deserialize_json(
                data["Filters"]
            )
        )
