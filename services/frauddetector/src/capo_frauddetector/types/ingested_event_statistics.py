...
if TYPE_CHECKING:
    import capo_frauddetector.types.long
    import capo_frauddetector.types.time


class IngestedEventStatistics(TypedDict, closed=True):
    number_of_events: NotRequired["capo_frauddetector.types.long.Long"]
    """<p>The number of stored events.</p>"""
    event_data_size_in_bytes: NotRequired["capo_frauddetector.types.long.Long"]
    """<p>The total size of the stored events.</p>"""
    least_recent_event: NotRequired["capo_frauddetector.types.time.time"]
    """<p>The oldest stored event.</p>"""
    most_recent_event: NotRequired["capo_frauddetector.types.time.time"]
