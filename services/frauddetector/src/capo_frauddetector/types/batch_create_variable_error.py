...
if TYPE_CHECKING:
    import capo_frauddetector.types.integer2
...
class BatchCreateVariableError(TypedDict, closed=True):
    ...
    """<p>The name.</p>"""
    code: "capo_frauddetector.types.integer2.Integer2"
    """<p>The error code. </p>"""
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: BatchCreateVariableError) -> dict:
    out: dict = {}
    if "name" in value:
        out["name"] = value["name"]
...
def deserialize_aws_json_1_1(data: dict) -> BatchCreateVariableError:
    out: BatchCreateVariableError = {}  # type: ignore[typeddict-item]
    if "name" in data:
        out["name"] = data["name"]
    if "code" in data:
        out["code"] = data["code"]
