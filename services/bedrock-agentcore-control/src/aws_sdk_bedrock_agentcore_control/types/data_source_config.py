...
def serialize_json(value: DataSourceConfig) -> dict:
    if "cloudWatchLogs" in value:
        ...
        return {
            "cloudWatchLogs": aws_sdk_bedrock_agentcore_control.types.cloud_watch_logs_input_config.serialize_json(
...
def deserialize_json(data: dict) -> DataSourceConfig:
    if "cloudWatchLogs" in data:
        ...
        return {
            "cloudWatchLogs": aws_sdk_bedrock_agentcore_control.types.cloud_watch_logs_input_config.deserialize_json(
                data["cloudWatchLogs"]
            )
        }
    else:
