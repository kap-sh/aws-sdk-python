...
def serialize_json(value: AuthorizationData) -> dict:
    if "oauth2" in value:
        import aws_sdk_bedrock_agentcore_control.types.o_auth2_authorization_data
        return {"oauth2": aws_sdk_bedrock_agentcore_control.types.o_auth2_authorization_data.serialize_json(value["oauth2"])}
    else:
...
def deserialize_json(data: dict) -> AuthorizationData:
    if "oauth2" in data:
        import aws_sdk_bedrock_agentcore_control.types.o_auth2_authorization_data
        return {"oauth2": aws_sdk_bedrock_agentcore_control.types.o_auth2_authorization_data.deserialize_json(data["oauth2"])}
    else:
