"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#SecurityGroupIds``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_bedrock_agentcore_control.types.security_group_identifier

SecurityGroupIds: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.security_group_identifier.SecurityGroupIdentifier"
]


# --- restJson1 ser/de ---
def serialize_json(value: SecurityGroupIds) -> list:
    return list(value)


def deserialize_json(data: list) -> SecurityGroupIds:
    return list(data)
