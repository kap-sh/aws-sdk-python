...
class A2aDescriptor(TypedDict):
    agent_card: NotRequired[
        "aws_sdk_bedrock_agentcore_control.types.agent_card_definition.AgentCardDefinition"
    ]
...
def serialize_json(value: A2aDescriptor) -> dict:
    ...
    if "agent_card" in value:
        ...
        out["agentCard"] = (
            aws_sdk_bedrock_agentcore_control.types.agent_card_definition.serialize_json(
                value["agent_card"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> A2aDescriptor:
    ...
    if "agentCard" in data:
        ...
        out["agent_card"] = (
            aws_sdk_bedrock_agentcore_control.types.agent_card_definition.deserialize_json(
                data["agentCard"]
            )
        )
    return out
