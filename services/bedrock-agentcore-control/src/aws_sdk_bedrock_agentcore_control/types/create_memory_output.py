...
class CreateMemoryOutput(TypedDict):
...
def serialize_json(value: CreateMemoryOutput) -> dict:
    ...
    if "memory" in value:
        import aws_sdk_bedrock_agentcore_control.types.memory
        out["memory"] = aws_sdk_bedrock_agentcore_control.types.memory.serialize_json(value["memory"])
    return out
...
def deserialize_json(data: dict) -> CreateMemoryOutput:
    ...
    if "memory" in data:
        import aws_sdk_bedrock_agentcore_control.types.memory
        out["memory"] = aws_sdk_bedrock_agentcore_control.types.memory.deserialize_json(data["memory"])
    return out