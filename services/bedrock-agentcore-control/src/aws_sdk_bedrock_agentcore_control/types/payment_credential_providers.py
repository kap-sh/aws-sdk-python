...
PaymentCredentialProviders: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.payment_credential_provider_item.PaymentCredentialProviderItem"
]
...
def serialize_json(value: PaymentCredentialProviders) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.payment_credential_provider_item.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> PaymentCredentialProviders:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.payment_credential_provider_item.deserialize_json(
                item
            )
        )
    return out
