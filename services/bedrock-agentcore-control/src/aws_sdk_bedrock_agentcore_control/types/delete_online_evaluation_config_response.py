...
def deserialize_json(data: dict) -> DeleteOnlineEvaluationConfigResponse:
    ...
    if "onlineEvaluationConfigArn" in data:
        ...
        raise DeserializationError("DeleteOnlineEvaluationConfigResponse.online_evaluation_config_arn required")
    if "onlineEvaluationConfigId" in data:
        out["online_evaluation_config_id"] = data["onlineEvaluationConfigId"]
    else:
        raise DeserializationError("DeleteOnlineEvaluationConfigResponse.online_evaluation_config_id required")
    if "status" in data:
        import aws_sdk_bedrock_agentcore_control.types.online_evaluation_config_status
        out["status"] = aws_sdk_bedrock_agentcore_control.types.online_evaluation_config_status.deserialize_json(data["status"])
    else:
        raise DeserializationError("DeleteOnlineEvaluationConfigResponse.status required")
    return out