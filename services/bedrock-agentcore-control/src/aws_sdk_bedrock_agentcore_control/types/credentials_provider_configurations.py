...
CredentialsProviderConfigurations: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.credentials_provider_configuration.CredentialsProviderConfiguration"
]
...
def serialize_json(value: CredentialsProviderConfigurations) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.credentials_provider_configuration.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> CredentialsProviderConfigurations:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.credentials_provider_configuration.deserialize_json(
                item
            )
        )
    return out
