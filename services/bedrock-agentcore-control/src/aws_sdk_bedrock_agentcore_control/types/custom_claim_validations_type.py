...
CustomClaimValidationsType: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.custom_claim_validation_type.CustomClaimValidationType"
]
...
def serialize_json(value: CustomClaimValidationsType) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.custom_claim_validation_type.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> CustomClaimValidationsType:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.custom_claim_validation_type.deserialize_json(
                item
            )
        )
    return out
