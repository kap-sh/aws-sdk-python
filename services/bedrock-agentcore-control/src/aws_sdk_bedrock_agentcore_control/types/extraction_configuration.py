...
ExtractionConfiguration: TypeAlias = _ExtractionConfiguration_customExtractionConfiguration

# --- restJson1 ser/de ---
def serialize_json(value: ExtractionConfiguration) -> dict:
    if "customExtractionConfiguration" in value:
        import aws_sdk_bedrock_agentcore_control.types.custom_extraction_configuration
        return {"customExtractionConfiguration": aws_sdk_bedrock_agentcore_control.types.custom_extraction_configuration.serialize_json(value["customExtractionConfiguration"])}
    else:
...
def deserialize_json(data: dict) -> ExtractionConfiguration:
    if "customExtractionConfiguration" in data:
        import aws_sdk_bedrock_agentcore_control.types.custom_extraction_configuration
        return {"customExtractionConfiguration": aws_sdk_bedrock_agentcore_control.types.custom_extraction_configuration.deserialize_json(data["customExtractionConfiguration"])}
    else:
