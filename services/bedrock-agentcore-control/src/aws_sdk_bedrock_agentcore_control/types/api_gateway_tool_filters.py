...
ApiGatewayToolFilters: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.api_gateway_tool_filter.ApiGatewayToolFilter"
]
...
def serialize_json(value: ApiGatewayToolFilters) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.api_gateway_tool_filter.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ApiGatewayToolFilters:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.api_gateway_tool_filter.deserialize_json(
                item
            )
        )
    return out
