...
def serialize_json(value: Code) -> dict:
    if "s3" in value:
        import aws_sdk_bedrock_agentcore_control.types.s3_location
        return {"s3": aws_sdk_bedrock_agentcore_control.types.s3_location.serialize_json(value["s3"])}
    else:
...
def deserialize_json(data: dict) -> Code:
    if "s3" in data:
        import aws_sdk_bedrock_agentcore_control.types.s3_location
        return {"s3": aws_sdk_bedrock_agentcore_control.types.s3_location.deserialize_json(data["s3"])}
    else:
