...
def serialize_json(value: EvaluatorConfig) -> dict:
    if "llmAsAJudge" in value:
        import aws_sdk_bedrock_agentcore_control.types.llm_as_a_judge_evaluator_config
        return {"llmAsAJudge": aws_sdk_bedrock_agentcore_control.types.llm_as_a_judge_evaluator_config.serialize_json(value["llmAsAJudge"])}
    elif "codeBased" in value:
        import aws_sdk_bedrock_agentcore_control.types.code_based_evaluator_config
        return {"codeBased": aws_sdk_bedrock_agentcore_control.types.code_based_evaluator_config.serialize_json(value["codeBased"])}
    else:
...
def deserialize_json(data: dict) -> EvaluatorConfig:
    if "llmAsAJudge" in data:
        import aws_sdk_bedrock_agentcore_control.types.llm_as_a_judge_evaluator_config
        return {"llmAsAJudge": aws_sdk_bedrock_agentcore_control.types.llm_as_a_judge_evaluator_config.deserialize_json(data["llmAsAJudge"])}
    elif "codeBased" in data:
        import aws_sdk_bedrock_agentcore_control.types.code_based_evaluator_config
        return {"codeBased": aws_sdk_bedrock_agentcore_control.types.code_based_evaluator_config.deserialize_json(data["codeBased"])}
    else:
