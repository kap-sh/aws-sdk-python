...
class CodeInterpreterNetworkConfiguration(TypedDict):
    ...
    vpc_config: NotRequired["aws_sdk_bedrock_agentcore_control.types.vpc_config.VpcConfig"]
...
def serialize_json(value: CodeInterpreterNetworkConfiguration) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore_control.types.code_interpreter_network_mode
    out["networkMode"] = aws_sdk_bedrock_agentcore_control.types.code_interpreter_network_mode.serialize_json(value.get("network_mode", 'SANDBOX'))
    if "vpc_config" in value:
        import aws_sdk_bedrock_agentcore_control.types.vpc_config
        out["vpcConfig"] = aws_sdk_bedrock_agentcore_control.types.vpc_config.serialize_json(value["vpc_config"])
    return out
...
def deserialize_json(data: dict) -> CodeInterpreterNetworkConfiguration:
    ...
    if "networkMode" in data:
        import aws_sdk_bedrock_agentcore_control.types.code_interpreter_network_mode
        out["network_mode"] = aws_sdk_bedrock_agentcore_control.types.code_interpreter_network_mode.deserialize_json(data["networkMode"])
    else:
        out["network_mode"] = 'SANDBOX'
    if "vpcConfig" in data:
        import aws_sdk_bedrock_agentcore_control.types.vpc_config
        out["vpc_config"] = aws_sdk_bedrock_agentcore_control.types.vpc_config.deserialize_json(data["vpcConfig"])
    return out