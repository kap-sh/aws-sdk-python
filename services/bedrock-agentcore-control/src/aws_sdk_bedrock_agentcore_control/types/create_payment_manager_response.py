...
class CreatePaymentManagerResponse(TypedDict):
    payment_manager_arn: "aws_sdk_bedrock_agentcore_control.types.payment_manager_arn.PaymentManagerArn"
...
def serialize_json(value: CreatePaymentManagerResponse) -> dict:
    ...
    import aws_sdk_bedrock_agentcore_control.types.payment_manager_status
    out["status"] = aws_sdk_bedrock_agentcore_control.types.payment_manager_status.serialize_json(value["status"])
    if "tags" in value:
        import aws_sdk_bedrock_agentcore_control.types.tags_map
...
def deserialize_json(data: dict) -> CreatePaymentManagerResponse:
    ...
    if "paymentManagerArn" in data:
        ...
        raise DeserializationError("CreatePaymentManagerResponse.payment_manager_arn required")
    if "paymentManagerId" in data:
        out["payment_manager_id"] = data["paymentManagerId"]
    else:
        raise DeserializationError("CreatePaymentManagerResponse.payment_manager_id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
    ...
    if "authorizerType" in data:
        ...
        raise DeserializationError("CreatePaymentManagerResponse.authorizer_type required")
    ...
    if "status" in data:
        import aws_sdk_bedrock_agentcore_control.types.payment_manager_status
        out["status"] = aws_sdk_bedrock_agentcore_control.types.payment_manager_status.deserialize_json(data["status"])
    else:
