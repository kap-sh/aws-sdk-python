...
def serialize_json(value: Filter) -> dict:
    ...
    out["operator"] = (
        aws_sdk_bedrock_agentcore_control.types.filter_operator.serialize_json(
            value["operator"]
        )
    )
    import aws_sdk_bedrock_agentcore_control.types.filter_value

    out["value"] = aws_sdk_bedrock_agentcore_control.types.filter_value.serialize_json(
        value["value"]
    )
    return out
...
def deserialize_json(data: dict) -> Filter:
    ...
    if "operator" in data:
        ...
        out["operator"] = (
            aws_sdk_bedrock_agentcore_control.types.filter_operator.deserialize_json(
                data["operator"]
            )
        )
    else:
    ...
    if "value" in data:
        ...
        out["value"] = (
            aws_sdk_bedrock_agentcore_control.types.filter_value.deserialize_json(
                data["value"]
            )
        )
    else:
