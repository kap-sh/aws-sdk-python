...
RegistryRecordSummaryList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.registry_record_summary.RegistryRecordSummary"
]
...
def serialize_json(value: RegistryRecordSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.registry_record_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> RegistryRecordSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.registry_record_summary.deserialize_json(
                item
            )
        )
    return out
