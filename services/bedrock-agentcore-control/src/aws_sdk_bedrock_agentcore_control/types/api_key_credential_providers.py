...
ApiKeyCredentialProviders: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.api_key_credential_provider_item.ApiKeyCredentialProviderItem"
]
...
def serialize_json(value: ApiKeyCredentialProviders) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.api_key_credential_provider_item.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ApiKeyCredentialProviders:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.api_key_credential_provider_item.deserialize_json(
                item
            )
        )
    return out
