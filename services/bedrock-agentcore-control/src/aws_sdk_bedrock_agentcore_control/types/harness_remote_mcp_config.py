...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agentcore_control.types.harness_remote_mcp_url
    import aws_sdk_bedrock_agentcore_control.types.http_headers_map


class HarnessRemoteMcpConfig(TypedDict, closed=True):
    url: "aws_sdk_bedrock_agentcore_control.types.harness_remote_mcp_url.HarnessRemoteMcpUrl"
    """<p>URL of the MCP endpoint.</p>"""
    headers: NotRequired[
        "aws_sdk_bedrock_agentcore_control.types.http_headers_map.HttpHeadersMap"
    ]
...
def serialize_json(value: HarnessRemoteMcpConfig) -> dict:
    ...
    if "headers" in value:
        ...
        out["headers"] = (
            aws_sdk_bedrock_agentcore_control.types.http_headers_map.serialize_json(
                value["headers"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> HarnessRemoteMcpConfig:
    ...
    if "headers" in data:
        ...
        out["headers"] = (
            aws_sdk_bedrock_agentcore_control.types.http_headers_map.deserialize_json(
                data["headers"]
            )
        )
    return out
