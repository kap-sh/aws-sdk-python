...
class ListGatewaysResponse(TypedDict, closed=True):
    items: "aws_sdk_bedrock_agentcore_control.types.gateway_summaries.GatewaySummaries"
...
def serialize_json(value: ListGatewaysResponse) -> dict:
    ...
    out["items"] = (
        aws_sdk_bedrock_agentcore_control.types.gateway_summaries.serialize_json(
            value["items"]
        )
    )
...
def deserialize_json(data: dict) -> ListGatewaysResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            aws_sdk_bedrock_agentcore_control.types.gateway_summaries.deserialize_json(
                data["items"]
            )
        )
    else:
