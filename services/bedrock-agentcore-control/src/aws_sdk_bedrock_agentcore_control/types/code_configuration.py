...
def serialize_json(value: CodeConfiguration) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore_control.types.code
    out["code"] = aws_sdk_bedrock_agentcore_control.types.code.serialize_json(value["code"])
    import aws_sdk_bedrock_agentcore_control.types.agent_managed_runtime_type
    out["runtime"] = aws_sdk_bedrock_agentcore_control.types.agent_managed_runtime_type.serialize_json(value["runtime"])
    import aws_sdk_bedrock_agentcore_control.types.entry_points
    out["entryPoint"] = aws_sdk_bedrock_agentcore_control.types.entry_points.serialize_json(value["entry_point"])
    return out
...
def deserialize_json(data: dict) -> CodeConfiguration:
    ...
    if "code" in data:
        import aws_sdk_bedrock_agentcore_control.types.code
        out["code"] = aws_sdk_bedrock_agentcore_control.types.code.deserialize_json(data["code"])
    else:
        raise DeserializationError("CodeConfiguration.code required")
    if "runtime" in data:
        import aws_sdk_bedrock_agentcore_control.types.agent_managed_runtime_type
        out["runtime"] = aws_sdk_bedrock_agentcore_control.types.agent_managed_runtime_type.deserialize_json(data["runtime"])
    else:
        raise DeserializationError("CodeConfiguration.runtime required")
    if "entryPoint" in data:
        import aws_sdk_bedrock_agentcore_control.types.entry_points
        out["entry_point"] = aws_sdk_bedrock_agentcore_control.types.entry_points.deserialize_json(data["entryPoint"])
    else:
