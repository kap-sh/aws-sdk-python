...
class GatewayInterceptorConfiguration(TypedDict):
    ...
    input_configuration: NotRequired["aws_sdk_bedrock_agentcore_control.types.interceptor_input_configuration.InterceptorInputConfiguration"]
...
def serialize_json(value: GatewayInterceptorConfiguration) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore_control.types.interceptor_configuration
    out["interceptor"] = aws_sdk_bedrock_agentcore_control.types.interceptor_configuration.serialize_json(value["interceptor"])
    import aws_sdk_bedrock_agentcore_control.types.gateway_interception_points
    out["interceptionPoints"] = aws_sdk_bedrock_agentcore_control.types.gateway_interception_points.serialize_json(value["interception_points"])
    if "input_configuration" in value:
        import aws_sdk_bedrock_agentcore_control.types.interceptor_input_configuration
        out["inputConfiguration"] = aws_sdk_bedrock_agentcore_control.types.interceptor_input_configuration.serialize_json(value["input_configuration"])
    return out
...
def deserialize_json(data: dict) -> GatewayInterceptorConfiguration:
    ...
    if "interceptor" in data:
        import aws_sdk_bedrock_agentcore_control.types.interceptor_configuration
        out["interceptor"] = aws_sdk_bedrock_agentcore_control.types.interceptor_configuration.deserialize_json(data["interceptor"])
    else:
        raise DeserializationError("GatewayInterceptorConfiguration.interceptor required")
    if "interceptionPoints" in data:
        import aws_sdk_bedrock_agentcore_control.types.gateway_interception_points
        out["interception_points"] = aws_sdk_bedrock_agentcore_control.types.gateway_interception_points.deserialize_json(data["interceptionPoints"])
    else:
        raise DeserializationError("GatewayInterceptorConfiguration.interception_points required")
    if "inputConfiguration" in data:
        import aws_sdk_bedrock_agentcore_control.types.interceptor_input_configuration
        out["input_configuration"] = aws_sdk_bedrock_agentcore_control.types.interceptor_input_configuration.deserialize_json(data["inputConfiguration"])
    return out