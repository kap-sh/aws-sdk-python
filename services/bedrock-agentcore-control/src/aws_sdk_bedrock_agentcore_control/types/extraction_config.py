...
def serialize_json(value: ExtractionConfig) -> dict:
    if "llmExtractionConfig" in value:
        import aws_sdk_bedrock_agentcore_control.types.llm_extraction_config
        return {"llmExtractionConfig": aws_sdk_bedrock_agentcore_control.types.llm_extraction_config.serialize_json(value["llmExtractionConfig"])}
    else:
...
def deserialize_json(data: dict) -> ExtractionConfig:
    if "llmExtractionConfig" in data:
        import aws_sdk_bedrock_agentcore_control.types.llm_extraction_config
        return {"llmExtractionConfig": aws_sdk_bedrock_agentcore_control.types.llm_extraction_config.deserialize_json(data["llmExtractionConfig"])}
    else:
