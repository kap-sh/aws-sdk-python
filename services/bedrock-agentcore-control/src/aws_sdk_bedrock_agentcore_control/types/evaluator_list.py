...
EvaluatorList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.evaluator_reference.EvaluatorReference"
]
...
def serialize_json(value: EvaluatorList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.evaluator_reference.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> EvaluatorList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.evaluator_reference.deserialize_json(
                item
            )
        )
    return out
