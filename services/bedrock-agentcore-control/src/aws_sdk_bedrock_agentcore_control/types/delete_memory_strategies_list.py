...
DeleteMemoryStrategiesList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.delete_memory_strategy_input.DeleteMemoryStrategyInput"
]
...
def serialize_json(value: DeleteMemoryStrategiesList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.delete_memory_strategy_input.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> DeleteMemoryStrategiesList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.delete_memory_strategy_input.deserialize_json(
                item
            )
        )
    return out
