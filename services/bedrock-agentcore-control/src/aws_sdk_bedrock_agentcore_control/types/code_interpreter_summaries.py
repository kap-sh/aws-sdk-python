...
CodeInterpreterSummaries: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.code_interpreter_summary.CodeInterpreterSummary"
]
...
def serialize_json(value: CodeInterpreterSummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.code_interpreter_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> CodeInterpreterSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.code_interpreter_summary.deserialize_json(
                item
            )
        )
    return out
