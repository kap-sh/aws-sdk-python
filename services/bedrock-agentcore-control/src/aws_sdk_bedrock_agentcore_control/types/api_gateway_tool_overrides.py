...
ApiGatewayToolOverrides: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.api_gateway_tool_override.ApiGatewayToolOverride"
]
...
def serialize_json(value: ApiGatewayToolOverrides) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.api_gateway_tool_override.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ApiGatewayToolOverrides:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.api_gateway_tool_override.deserialize_json(
                item
            )
        )
    return out
