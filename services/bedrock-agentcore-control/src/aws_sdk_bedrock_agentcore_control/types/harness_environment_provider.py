...
HarnessEnvironmentProvider: TypeAlias = _HarnessEnvironmentProvider_agentCoreRuntimeEnvironment

# --- restJson1 ser/de ---
def serialize_json(value: HarnessEnvironmentProvider) -> dict:
    if "agentCoreRuntimeEnvironment" in value:
        import aws_sdk_bedrock_agentcore_control.types.harness_agent_core_runtime_environment
        return {"agentCoreRuntimeEnvironment": aws_sdk_bedrock_agentcore_control.types.harness_agent_core_runtime_environment.serialize_json(value["agentCoreRuntimeEnvironment"])}
    else:
...
def deserialize_json(data: dict) -> HarnessEnvironmentProvider:
    if "agentCoreRuntimeEnvironment" in data:
        import aws_sdk_bedrock_agentcore_control.types.harness_agent_core_runtime_environment
        return {"agentCoreRuntimeEnvironment": aws_sdk_bedrock_agentcore_control.types.harness_agent_core_runtime_environment.deserialize_json(data["agentCoreRuntimeEnvironment"])}
    else:
        raise DeserializationError("HarnessEnvironmentProvider: no recognized variant key")