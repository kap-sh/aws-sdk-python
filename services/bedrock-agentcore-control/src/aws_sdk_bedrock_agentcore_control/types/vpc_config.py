...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agentcore_control.types.security_groups
    import aws_sdk_bedrock_agentcore_control.types.subnets


class VpcConfig(TypedDict):
    security_groups: (
        "aws_sdk_bedrock_agentcore_control.types.security_groups.SecurityGroups"
    )
    """<p>The security groups associated with the VPC configuration.</p>"""
    subnets: "aws_sdk_bedrock_agentcore_control.types.subnets.Subnets"
...
def serialize_json(value: VpcConfig) -> dict:
    ...
    out["securityGroups"] = (
        aws_sdk_bedrock_agentcore_control.types.security_groups.serialize_json(
            value["security_groups"]
        )
    )
    import aws_sdk_bedrock_agentcore_control.types.subnets

    out["subnets"] = aws_sdk_bedrock_agentcore_control.types.subnets.serialize_json(
        value["subnets"]
    )
...
def deserialize_json(data: dict) -> VpcConfig:
    ...
    if "securityGroups" in data:
        ...
        out["security_groups"] = (
            aws_sdk_bedrock_agentcore_control.types.security_groups.deserialize_json(
                data["securityGroups"]
            )
        )
    else:
    ...
    if "subnets" in data:
        ...
        out["subnets"] = (
            aws_sdk_bedrock_agentcore_control.types.subnets.deserialize_json(
                data["subnets"]
            )
        )
    else:
