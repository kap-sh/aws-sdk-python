...
class BrowserEnterprisePolicy(TypedDict):
    location: "aws_sdk_bedrock_agentcore_control.types.resource_location.ResourceLocation"
    """<p>The location of the enterprise policy file.</p>"""
    type: NotRequired["aws_sdk_bedrock_agentcore_control.types.browser_enterprise_policy_type.BrowserEnterprisePolicyType"]
...
def serialize_json(value: BrowserEnterprisePolicy) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore_control.types.resource_location
    out["location"] = aws_sdk_bedrock_agentcore_control.types.resource_location.serialize_json(value["location"])
    if "type" in value:
        import aws_sdk_bedrock_agentcore_control.types.browser_enterprise_policy_type
        out["type"] = aws_sdk_bedrock_agentcore_control.types.browser_enterprise_policy_type.serialize_json(value["type"])
    return out
...
def deserialize_json(data: dict) -> BrowserEnterprisePolicy:
    ...
    if "location" in data:
        import aws_sdk_bedrock_agentcore_control.types.resource_location
        out["location"] = aws_sdk_bedrock_agentcore_control.types.resource_location.deserialize_json(data["location"])
    else:
        raise DeserializationError("BrowserEnterprisePolicy.location required")
    if "type" in data:
        import aws_sdk_bedrock_agentcore_control.types.browser_enterprise_policy_type
        out["type"] = aws_sdk_bedrock_agentcore_control.types.browser_enterprise_policy_type.deserialize_json(data["type"])
    return out