...
GatewayInterceptionPoints: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.gateway_interception_point.GatewayInterceptionPoint"
]
...
def serialize_json(value: GatewayInterceptionPoints) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.gateway_interception_point.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> GatewayInterceptionPoints:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.gateway_interception_point.deserialize_json(
                item
            )
        )
    return out
