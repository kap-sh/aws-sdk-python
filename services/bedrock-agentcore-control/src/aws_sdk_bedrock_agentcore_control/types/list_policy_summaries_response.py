...
class ListPolicySummariesResponse(TypedDict):
    policies: (
        "aws_sdk_bedrock_agentcore_control.types.policy_summary_list.PolicySummaryList"
    )
...
def serialize_json(value: ListPolicySummariesResponse) -> dict:
    ...
    out["policies"] = (
        aws_sdk_bedrock_agentcore_control.types.policy_summary_list.serialize_json(
            value["policies"]
        )
    )
...
def deserialize_json(data: dict) -> ListPolicySummariesResponse:
    ...
    if "policies" in data:
        ...
        out["policies"] = (
            aws_sdk_bedrock_agentcore_control.types.policy_summary_list.deserialize_json(
                data["policies"]
            )
        )
    else:
