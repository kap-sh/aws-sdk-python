...
def serialize_json(value: SummaryOverrideConfigurationInput) -> dict:
    ...
    if "consolidation" in value:
        ...
        out["consolidation"] = (
            aws_sdk_bedrock_agentcore_control.types.summary_override_consolidation_configuration_input.serialize_json(
                value["consolidation"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> SummaryOverrideConfigurationInput:
    ...
    if "consolidation" in data:
        ...
        out["consolidation"] = (
            aws_sdk_bedrock_agentcore_control.types.summary_override_consolidation_configuration_input.deserialize_json(
                data["consolidation"]
            )
        )
    return out
