...
AgentRuntimes: TypeAlias = list[
    "aws_sdk_bedrock_agentcore_control.types.agent_runtime.AgentRuntime"
]
...
def serialize_json(value: AgentRuntimes) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.agent_runtime.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AgentRuntimes:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore_control.types.agent_runtime.deserialize_json(item)
        )
    return out
