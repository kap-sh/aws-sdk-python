...
class CreatePaymentConnectorResponse(TypedDict):
...
def serialize_json(value: CreatePaymentConnectorResponse) -> dict:
    ...
    out["name"] = value["name"]
    import aws_sdk_bedrock_agentcore_control.types.payment_connector_type
    out["type"] = aws_sdk_bedrock_agentcore_control.types.payment_connector_type.serialize_json(value["type"])
    import aws_sdk_bedrock_agentcore_control.types.credentials_provider_configurations
    out["credentialProviderConfigurations"] = aws_sdk_bedrock_agentcore_control.types.credentials_provider_configurations.serialize_json(value["credential_provider_configurations"])
    import aws_sdk_bedrock_agentcore_control.types.date_timestamp
    out["createdAt"] = aws_sdk_bedrock_agentcore_control.types.date_timestamp.serialize_json(value["created_at"])
    import aws_sdk_bedrock_agentcore_control.types.payment_connector_status
    out["status"] = aws_sdk_bedrock_agentcore_control.types.payment_connector_status.serialize_json(value["status"])
    return out
...
def deserialize_json(data: dict) -> CreatePaymentConnectorResponse:
    ...
    if "paymentConnectorId" in data:
        ...
        raise DeserializationError("CreatePaymentConnectorResponse.payment_connector_id required")
    if "paymentManagerId" in data:
        out["payment_manager_id"] = data["paymentManagerId"]
    else:
        raise DeserializationError("CreatePaymentConnectorResponse.payment_manager_id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
    ...
    if "type" in data:
        import aws_sdk_bedrock_agentcore_control.types.payment_connector_type
        out["type"] = aws_sdk_bedrock_agentcore_control.types.payment_connector_type.deserialize_json(data["type"])
    else:
        raise DeserializationError("CreatePaymentConnectorResponse.type required")
    if "credentialProviderConfigurations" in data:
        import aws_sdk_bedrock_agentcore_control.types.credentials_provider_configurations
        out["credential_provider_configurations"] = aws_sdk_bedrock_agentcore_control.types.credentials_provider_configurations.deserialize_json(data["credentialProviderConfigurations"])
    else:
        raise DeserializationError("CreatePaymentConnectorResponse.credential_provider_configurations required")
    ...
    if "status" in data:
        import aws_sdk_bedrock_agentcore_control.types.payment_connector_status
        out["status"] = aws_sdk_bedrock_agentcore_control.types.payment_connector_status.deserialize_json(data["status"])
    else:
