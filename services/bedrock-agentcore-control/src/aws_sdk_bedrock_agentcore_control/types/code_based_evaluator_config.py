...
def serialize_json(value: CodeBasedEvaluatorConfig) -> dict:
    if "lambdaConfig" in value:
        import aws_sdk_bedrock_agentcore_control.types.lambda_evaluator_config
        return {"lambdaConfig": aws_sdk_bedrock_agentcore_control.types.lambda_evaluator_config.serialize_json(value["lambdaConfig"])}
    else:
...
def deserialize_json(data: dict) -> CodeBasedEvaluatorConfig:
    if "lambdaConfig" in data:
        import aws_sdk_bedrock_agentcore_control.types.lambda_evaluator_config
        return {"lambdaConfig": aws_sdk_bedrock_agentcore_control.types.lambda_evaluator_config.deserialize_json(data["lambdaConfig"])}
    else:
        raise DeserializationError("CodeBasedEvaluatorConfig: no recognized variant key")