...
def serialize_json(value: GatewayProtocolConfiguration) -> dict:
    if "mcp" in value:
        ...
        return {
            "mcp": aws_sdk_bedrock_agentcore_control.types.mcp_gateway_configuration.serialize_json(
                value["mcp"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> GatewayProtocolConfiguration:
    if "mcp" in data:
        ...
        return {
            "mcp": aws_sdk_bedrock_agentcore_control.types.mcp_gateway_configuration.deserialize_json(
                data["mcp"]
            )
        }
    else:
