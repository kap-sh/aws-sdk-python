...
def serialize_json(value: DataSourceType) -> dict:
    if "inlineExamples" in value:
        ...
        return {
            "inlineExamples": aws_sdk_bedrock_agentcore_control.types.inline_examples_source.serialize_json(
        ...
        return {
            "s3Source": aws_sdk_bedrock_agentcore_control.types.s3_source.serialize_json(
                value["s3Source"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> DataSourceType:
    if "inlineExamples" in data:
        ...
        return {
            "inlineExamples": aws_sdk_bedrock_agentcore_control.types.inline_examples_source.deserialize_json(
                data["inlineExamples"]
            )
        }
        ...
        return {
            "s3Source": aws_sdk_bedrock_agentcore_control.types.s3_source.deserialize_json(
                data["s3Source"]
            )
        }
    else:
