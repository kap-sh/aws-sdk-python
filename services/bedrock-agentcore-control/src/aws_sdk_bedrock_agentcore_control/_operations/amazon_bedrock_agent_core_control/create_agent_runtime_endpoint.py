...
def handle_response(
    ...
    out: aws_sdk_bedrock_agentcore_control.types.create_agent_runtime_endpoint_response.CreateAgentRuntimeEndpointResponse = aws_sdk_bedrock_agentcore_control.types.create_agent_runtime_endpoint_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agentcore_control.types.create_agent_runtime_endpoint_request.CreateAgentRuntimeEndpointRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{agentRuntimeId}", quote(str(input_["agent_runtime_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_bedrock_agentcore_control.types.create_agent_runtime_endpoint_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_agent_runtime_endpoint(
