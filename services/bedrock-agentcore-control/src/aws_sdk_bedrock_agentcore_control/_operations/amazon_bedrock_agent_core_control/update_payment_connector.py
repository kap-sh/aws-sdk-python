...
) -> aws_sdk_bedrock_agentcore_control.types.update_payment_connector_response.UpdatePaymentConnectorResponse:
    import aws_sdk_bedrock_agentcore_control.types.update_payment_connector_response

    out: aws_sdk_bedrock_agentcore_control.types.update_payment_connector_response.UpdatePaymentConnectorResponse = aws_sdk_bedrock_agentcore_control.types.update_payment_connector_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agentcore_control.types.update_payment_connector_request.UpdatePaymentConnectorRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/payments/managers/{paymentManagerId}/connectors/{paymentConnectorId}"
    )
    url = url.replace(
        "{paymentManagerId}", quote(str(input_["payment_manager_id"]), safe="")
    )
    url = url.replace(
        "{paymentConnectorId}", quote(str(input_["payment_connector_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_bedrock_agentcore_control.types.update_payment_connector_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_payment_connector(
