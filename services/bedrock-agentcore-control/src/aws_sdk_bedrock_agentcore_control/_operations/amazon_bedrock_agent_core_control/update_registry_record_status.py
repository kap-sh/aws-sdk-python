...
def handle_response(
    ...
    out: aws_sdk_bedrock_agentcore_control.types.update_registry_record_status_response.UpdateRegistryRecordStatusResponse = aws_sdk_bedrock_agentcore_control.types.update_registry_record_status_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agentcore_control.types.update_registry_record_status_request.UpdateRegistryRecordStatusRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/registries/{registryId}/records/{recordId}/status"
    )
    url = url.replace("{registryId}", quote(str(input_["registry_id"]), safe=""))
    url = url.replace("{recordId}", quote(str(input_["record_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_bedrock_agentcore_control.types.update_registry_record_status_request

    body: bytes | None = json.dumps(
        aws_sdk_bedrock_agentcore_control.types.update_registry_record_status_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_registry_record_status(
