...
class Rule(TypedDict, closed=True):
    sampling_config: (
        "capo_bedrock_agentcore_control.types.sampling_config.SamplingConfig"
    )
    """<p> The sampling configuration that determines what percentage of agent traces to evaluate. </p>"""
    filters: NotRequired["capo_bedrock_agentcore_control.types.filter_list.FilterList"]
    ...
    session_config: NotRequired[
        "capo_bedrock_agentcore_control.types.session_config.SessionConfig"
    ]
...
def serialize_json(value: Rule) -> dict:
    ...
    out["samplingConfig"] = (
        capo_bedrock_agentcore_control.types.sampling_config.serialize_json(
    ...
    if "filters" in value:
        ...
        out["filters"] = (
            capo_bedrock_agentcore_control.types.filter_list.serialize_json(
                value["filters"]
            )
        )
    if "session_config" in value:
        ...
        out["sessionConfig"] = (
            capo_bedrock_agentcore_control.types.session_config.serialize_json(
...
def deserialize_json(data: dict) -> Rule:
    ...
    if data.get("samplingConfig") is not None:
        ...
        out["sampling_config"] = (
            capo_bedrock_agentcore_control.types.sampling_config.deserialize_json(
                data["samplingConfig"]
            )
        )
    else:
    ...
    if data.get("filters") is not None:
        ...
        out["filters"] = (
            capo_bedrock_agentcore_control.types.filter_list.deserialize_json(
                data["filters"]
            )
        )
    if data.get("sessionConfig") is not None:
        ...
        out["session_config"] = (
            capo_bedrock_agentcore_control.types.session_config.deserialize_json(
                data["sessionConfig"]
            )
        )
    return out
