...
class Certificate(TypedDict, closed=True):
    location: (
        "capo_bedrock_agentcore_control.types.certificate_location.CertificateLocation"
    )
...
def serialize_json(value: Certificate) -> dict:
    ...
    out["location"] = (
        capo_bedrock_agentcore_control.types.certificate_location.serialize_json(
            value["location"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> Certificate:
    ...
    if data.get("location") is not None:
        ...
        out["location"] = (
            capo_bedrock_agentcore_control.types.certificate_location.deserialize_json(
                data["location"]
            )
        )
    else:
