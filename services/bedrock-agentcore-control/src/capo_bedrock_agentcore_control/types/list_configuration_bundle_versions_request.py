...
class ListConfigurationBundleVersionsRequest(TypedDict, closed=True):
...
def serialize_json(value: ListConfigurationBundleVersionsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = (
            capo_bedrock_agentcore_control.types.version_filter.serialize_json(
                value["filter"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListConfigurationBundleVersionsRequest:
    ...
    if "filter" in data:
        ...
        out["filter"] = (
            capo_bedrock_agentcore_control.types.version_filter.deserialize_json(
                data["filter"]
            )
        )
    return out
