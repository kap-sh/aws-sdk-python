...
PaymentManagerSummaries: TypeAlias = list[
    "capo_bedrock_agentcore_control.types.payment_manager_summary.PaymentManagerSummary"
]
...
def serialize_json(value: PaymentManagerSummaries) -> list:
    ...
    for item in value:
        out.append(
            capo_bedrock_agentcore_control.types.payment_manager_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> PaymentManagerSummaries:
    ...
    for item in data:
        out.append(
            capo_bedrock_agentcore_control.types.payment_manager_summary.deserialize_json(
                item
            )
        )
    return out
