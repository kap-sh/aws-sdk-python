...
class StringListValidation(TypedDict, closed=True):
    allowed_values: NotRequired[
        "capo_bedrock_agentcore_control.types.allowed_string_list_values_list.AllowedStringListValuesList"
    ]
...
def serialize_json(value: StringListValidation) -> dict:
    ...
    if "allowed_values" in value:
        ...
        out["allowedValues"] = (
            capo_bedrock_agentcore_control.types.allowed_string_list_values_list.serialize_json(
...
def deserialize_json(data: dict) -> StringListValidation:
    ...
    if "allowedValues" in data:
        ...
        out["allowed_values"] = (
            capo_bedrock_agentcore_control.types.allowed_string_list_values_list.deserialize_json(
                data["allowedValues"]
            )
        )
    if "maxItems" in data:
