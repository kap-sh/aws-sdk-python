"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#CreateGatewayTarget``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.create_gateway_target_response.CreateGatewayTargetResponse = capo_bedrock_agentcore_control.types.create_gateway_target_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/gateways/{gatewayIdentifier}/targets/"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.create_gateway_target_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_gateway_target(
