...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.update_policy_response.UpdatePolicyResponse = capo_bedrock_agentcore_control.types.update_policy_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{policyId}", quote(input_["policy_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.update_policy_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
