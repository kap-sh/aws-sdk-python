"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#GetWorkloadIdentity``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.get_workload_identity_response.GetWorkloadIdentityResponse = capo_bedrock_agentcore_control.types.get_workload_identity_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identities/GetWorkloadIdentity"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.get_workload_identity_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_workload_identity(
