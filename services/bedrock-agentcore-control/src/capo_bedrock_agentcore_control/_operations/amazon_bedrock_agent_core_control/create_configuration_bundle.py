"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#CreateConfigurationBundle``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.create_configuration_bundle_response.CreateConfigurationBundleResponse = capo_bedrock_agentcore_control.types.create_configuration_bundle_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/configuration-bundles/create"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.create_configuration_bundle_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_configuration_bundle(
