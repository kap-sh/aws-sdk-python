"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#UpdateConfigurationBundle``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.update_configuration_bundle_response.UpdateConfigurationBundleResponse = capo_bedrock_agentcore_control.types.update_configuration_bundle_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/configuration-bundles/{bundleId}"
    url = url.replace("{bundleId}", quote(str(input_["bundle_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.update_configuration_bundle_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_configuration_bundle(
