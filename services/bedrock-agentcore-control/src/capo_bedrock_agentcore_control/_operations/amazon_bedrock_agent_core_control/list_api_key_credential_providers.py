"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#ListApiKeyCredentialProviders``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.list_api_key_credential_providers_response.ListApiKeyCredentialProvidersResponse = capo_bedrock_agentcore_control.types.list_api_key_credential_providers_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identities/ListApiKeyCredentialProviders"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.list_api_key_credential_providers_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_list_api_key_credential_providers(
