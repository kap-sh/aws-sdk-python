"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#UpdateHarness``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.update_harness_response.UpdateHarnessResponse = capo_bedrock_agentcore_control.types.update_harness_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{harnessId}", quote(str(input_["harness_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.update_harness_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_harness(
