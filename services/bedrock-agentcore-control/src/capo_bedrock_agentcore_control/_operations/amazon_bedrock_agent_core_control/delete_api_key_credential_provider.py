"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#DeleteApiKeyCredentialProvider``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.delete_api_key_credential_provider_response.DeleteApiKeyCredentialProviderResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identities/DeleteApiKeyCredentialProvider"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.delete_api_key_credential_provider_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_delete_api_key_credential_provider(
