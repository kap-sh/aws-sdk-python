"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#AddDatasetExamples``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.add_dataset_examples_response.AddDatasetExamplesResponse = capo_bedrock_agentcore_control.types.add_dataset_examples_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/datasets/{datasetId}/examples/add"
    url = url.replace("{datasetId}", quote(str(input_["dataset_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.add_dataset_examples_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_add_dataset_examples(
