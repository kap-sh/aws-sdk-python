"""Generated from Smithy shape ``com.amazonaws.bedrockagentcorecontrol#GetOauth2CredentialProvider``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore_control.types.get_oauth2_credential_provider_response.GetOauth2CredentialProviderResponse = capo_bedrock_agentcore_control.types.get_oauth2_credential_provider_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identities/GetOauth2CredentialProvider"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore_control.types.get_oauth2_credential_provider_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_oauth2_credential_provider(
