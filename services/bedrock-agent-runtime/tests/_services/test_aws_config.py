...
import pytest
from aws_sdk_bedrock_agent_runtime._services._aws_config import (
    _env_bool,
    _load_profile,
    _profile_bool,
    load_aws_settings,
...
def test_endpoint_service_env_beats_global_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AWS_ENDPOINT_URL_BEDROCK_AGENT_RUNTIME", "https://svc.example")
...
def test_load_profile_parses_services_block(tmp_path: Path) -> None:
    ...
    assert (
        services["bedrock_agent_runtime"]["endpoint_url"] == "https://localstack:4566"
    )
