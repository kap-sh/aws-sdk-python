...
class RetrievalResultContent(TypedDict, closed=True):
    type: NotRequired[
        "capo_bedrock_agent_runtime.types.retrieval_result_content_type.RetrievalResultContentType"
    ]
    ...
    video: NotRequired["capo_bedrock_agent_runtime.types.video_segment.VideoSegment"]
    """<p>Video segment information when the retrieval result contains video content.</p>"""
    audio: NotRequired["capo_bedrock_agent_runtime.types.audio_segment.AudioSegment"]
    """<p>Audio segment information when the retrieval result contains audio content.</p>"""
    row: NotRequired[
        "capo_bedrock_agent_runtime.types.retrieval_result_content_row.RetrievalResultContentRow"
    ]
...
def serialize_json(value: RetrievalResultContent) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = (
            capo_bedrock_agent_runtime.types.retrieval_result_content_type.serialize_json(
                value["type"]
            )
        )
    ...
    if "video" in value:
        ...
        out["video"] = capo_bedrock_agent_runtime.types.video_segment.serialize_json(
            value["video"]
        )
    if "audio" in value:
        import capo_bedrock_agent_runtime.types.audio_segment

        out["audio"] = capo_bedrock_agent_runtime.types.audio_segment.serialize_json(
            value["audio"]
        )
    if "row" in value:
        import capo_bedrock_agent_runtime.types.retrieval_result_content_row
        ...
        out["row"] = (
            capo_bedrock_agent_runtime.types.retrieval_result_content_row.serialize_json(
                value["row"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RetrievalResultContent:
    ...
    if data.get("type") is not None:
        ...
        out["type"] = (
            capo_bedrock_agent_runtime.types.retrieval_result_content_type.deserialize_json(
                data["type"]
            )
        )
    ...
    if data.get("row") is not None:
        ...
        out["row"] = (
            capo_bedrock_agent_runtime.types.retrieval_result_content_row.deserialize_json(
                data["row"]
            )
        )
    return out
