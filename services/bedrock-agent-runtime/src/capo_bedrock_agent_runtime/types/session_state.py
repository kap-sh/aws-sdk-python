...
class SessionState(TypedDict, closed=True):
    ...
    r"""<p>Contains attributes that persist across a session and the values of those attributes. If <code>sessionAttributes</code> are passed to a supervisor agent in <a href=\"https://docs.aws.amazon.com/bedrock/latest/userguide/agents-multi-agent-collaboration.html\">multi-agent collaboration</a>, it will be forwarded to all agent collaborators.</p>"""
    ...
    r"""<p>The identifier of the invocation of an action. This value must match the <code>invocationId</code> returned in the <code>InvokeAgent</code> response for the action whose results are provided in the <code>returnControlInvocationResults</code> field. For more information, see <a href=\"https://docs.aws.amazon.com/bedrock/latest/userguide/agents-returncontrol.html\">Return control to the agent developer</a> and <a href=\"https://docs.aws.amazon.com/bedrock/latest/userguide/agents-session-state.html\">Control session context</a>.</p>"""
    ...
    knowledge_base_configurations: NotRequired[
        "capo_bedrock_agent_runtime.types.knowledge_base_configurations.KnowledgeBaseConfigurations"
    ]
...
def serialize_json(value: SessionState) -> dict:
    ...
    if "knowledge_base_configurations" in value:
        ...
        out["knowledgeBaseConfigurations"] = (
            capo_bedrock_agent_runtime.types.knowledge_base_configurations.serialize_json(
...
def deserialize_json(data: dict) -> SessionState:
    ...
    if data.get("knowledgeBaseConfigurations") is not None:
        ...
        out["knowledge_base_configurations"] = (
            capo_bedrock_agent_runtime.types.knowledge_base_configurations.deserialize_json(
