...
class FlowExecutionError(TypedDict, closed=True):
    node_name: NotRequired["capo_bedrock_agent_runtime.types.node_name.NodeName"]
    """<p>The name of the node in the flow where the error occurred (if applicable).</p>"""
    error: NotRequired[
        "capo_bedrock_agent_runtime.types.flow_execution_error_type.FlowExecutionErrorType"
    ]
...
def serialize_json(value: FlowExecutionError) -> dict:
    ...
    if "error" in value:
        ...
        out["error"] = (
            capo_bedrock_agent_runtime.types.flow_execution_error_type.serialize_json(
                value["error"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> FlowExecutionError:
    ...
    if data.get("error") is not None:
        ...
        out["error"] = (
            capo_bedrock_agent_runtime.types.flow_execution_error_type.deserialize_json(
                data["error"]
            )
        )
