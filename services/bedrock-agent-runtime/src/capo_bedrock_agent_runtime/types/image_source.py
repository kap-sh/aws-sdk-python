...
if TYPE_CHECKING:
    import capo_bedrock_agent_runtime.types.s3_location
...
class _ImageSource_s3Location(TypedDict, closed=True):
    s3Location: "capo_bedrock_agent_runtime.types.s3_location.S3Location"
...
def serialize_json(value: ImageSource) -> dict:
    if "bytes" in value:
        ...
        return {
            "s3Location": capo_bedrock_agent_runtime.types.s3_location.serialize_json(
                value["s3Location"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> ImageSource:
    if data.get("bytes") is not None:
        ...
        return {
            "s3Location": capo_bedrock_agent_runtime.types.s3_location.deserialize_json(
                data["s3Location"]
            )
        }
    else:
