...
def handle_response(
    ...
    _union_deser = (
        capo_bedrock_agent_runtime.types.flow_response_stream.deserialize_event_json
    )
    _iter = cast(Any, response.iter_bytes())
    out: capo_bedrock_agent_runtime.types.invoke_flow_response.InvokeFlowResponse = {
    ...
    if "x-amz-bedrock-flow-execution-id" in response.headers:
        out["execution_id"] = response.headers["x-amz-bedrock-flow-execution-id"]
    return out
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_bedrock_agent_runtime.types.invoke_flow_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
