"""Generated from Smithy shape ``com.amazonaws.bedrockagentruntime#RetrieveAndGenerate``."""
...
def handle_response(
    ...
    out: capo_bedrock_agent_runtime.types.retrieve_and_generate_response.RetrieveAndGenerateResponse = capo_bedrock_agent_runtime.types.retrieve_and_generate_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/retrieveAndGenerate"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agent_runtime.types.retrieve_and_generate_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_retrieve_and_generate(
