"""Generated from Smithy shape ``com.amazonaws.bedrockagentruntime#GetInvocationStep``."""
...
def handle_response(
    ...
    out: capo_bedrock_agent_runtime.types.get_invocation_step_response.GetInvocationStepResponse = capo_bedrock_agent_runtime.types.get_invocation_step_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{invocationStepId}", quote(str(input_["invocation_step_id"]), safe="")
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_bedrock_agent_runtime.types.get_invocation_step_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_get_invocation_step(
