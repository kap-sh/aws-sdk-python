...
def handle_response(
    ...
    _union_deser = aws_sdk_bedrock_agent_runtime.types.inline_agent_response_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_bedrock_agent_runtime.types.invoke_inline_agent_response.InvokeInlineAgentResponse = {
        "completion": cast(
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agent_runtime.types.invoke_inline_agent_request.InvokeInlineAgentRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/agents/{sessionId}"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_bedrock_agent_runtime.types.invoke_inline_agent_request.serialize_json(
            input_
        )
    ).encode()
