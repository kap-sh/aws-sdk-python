...
GuardrailManagedWordList: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.guardrail_managed_word.GuardrailManagedWord"
]
...
def serialize_json(value: GuardrailManagedWordList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.guardrail_managed_word.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> GuardrailManagedWordList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.guardrail_managed_word.deserialize_json(
                item
            )
        )
    return out
