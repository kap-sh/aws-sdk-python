...
FlowTraceNodeInputExecutionChain: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.flow_trace_node_input_execution_chain_item.FlowTraceNodeInputExecutionChainItem"
]
...
def serialize_json(value: FlowTraceNodeInputExecutionChain) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.flow_trace_node_input_execution_chain_item.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> FlowTraceNodeInputExecutionChain:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.flow_trace_node_input_execution_chain_item.deserialize_json(
                item
            )
        )
    return out
