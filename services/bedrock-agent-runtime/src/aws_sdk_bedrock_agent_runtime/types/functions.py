...
Functions: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.function_definition.FunctionDefinition"
]
...
def serialize_json(value: Functions) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.function_definition.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> Functions:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.function_definition.deserialize_json(
                item
            )
        )
    return out
