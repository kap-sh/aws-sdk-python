...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent_runtime.types.rerank_document_type
...
class RerankDocument(TypedDict, closed=True):
    ...
    text_document: NotRequired[
        "aws_sdk_bedrock_agent_runtime.types.rerank_text_document.RerankTextDocument"
    ]
...
def serialize_json(value: RerankDocument) -> dict:
    ...
    out["type"] = (
        aws_sdk_bedrock_agent_runtime.types.rerank_document_type.serialize_json(
            value["type"]
        )
    )
    if "text_document" in value:
        ...
        out["textDocument"] = (
            aws_sdk_bedrock_agent_runtime.types.rerank_text_document.serialize_json(
                value["text_document"]
            )
        )
...
def deserialize_json(data: dict) -> RerankDocument:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_bedrock_agent_runtime.types.rerank_document_type.deserialize_json(
                data["type"]
            )
        )
    else:
    ...
    if "textDocument" in data:
        ...
        out["text_document"] = (
            aws_sdk_bedrock_agent_runtime.types.rerank_text_document.deserialize_json(
                data["textDocument"]
            )
        )
