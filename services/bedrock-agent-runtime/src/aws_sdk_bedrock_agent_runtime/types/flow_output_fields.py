...
FlowOutputFields: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.flow_output_field.FlowOutputField"
]
...
def serialize_json(value: FlowOutputFields) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.flow_output_field.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> FlowOutputFields:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.flow_output_field.deserialize_json(item)
        )
    return out
