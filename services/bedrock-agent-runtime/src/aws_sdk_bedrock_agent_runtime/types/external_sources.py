...
ExternalSources: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.external_source.ExternalSource"
]
...
def serialize_json(value: ExternalSources) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.external_source.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ExternalSources:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.external_source.deserialize_json(item)
        )
    return out
