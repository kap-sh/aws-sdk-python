...
FlowExecutionEvents: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.flow_execution_event.FlowExecutionEvent"
]
...
def serialize_json(value: FlowExecutionEvents) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.flow_execution_event.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> FlowExecutionEvents:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.flow_execution_event.deserialize_json(
                item
            )
        )
    return out
