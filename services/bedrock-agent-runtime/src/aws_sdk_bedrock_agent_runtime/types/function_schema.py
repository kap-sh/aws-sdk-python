"""Generated from Smithy shape ``com.amazonaws.bedrockagentruntime#FunctionSchema``."""

from typing import TYPE_CHECKING, TypeAlias, TypedDict
from aws_sdk_bedrock_agent_runtime.errors import DeserializationError, SerializationError
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent_runtime.types.functions

class _FunctionSchema_functions(TypedDict):
    functions: "aws_sdk_bedrock_agent_runtime.types.functions.Functions"
...
def serialize_json(value: FunctionSchema) -> dict:
    if "functions" in value:
        import aws_sdk_bedrock_agent_runtime.types.functions
        return {"functions": aws_sdk_bedrock_agent_runtime.types.functions.serialize_json(value["functions"])}
    else:
...
def deserialize_json(data: dict) -> FunctionSchema:
    if "functions" in data:
        import aws_sdk_bedrock_agent_runtime.types.functions
        return {"functions": aws_sdk_bedrock_agent_runtime.types.functions.deserialize_json(data["functions"])}
    else:
