...
class GuardrailTopicPolicyAssessment(TypedDict, closed=True):
    topics: NotRequired[
        "aws_sdk_bedrock_agent_runtime.types.guardrail_topic_list.GuardrailTopicList"
    ]
...
def serialize_json(value: GuardrailTopicPolicyAssessment) -> dict:
    ...
    if "topics" in value:
        ...
        out["topics"] = (
            aws_sdk_bedrock_agent_runtime.types.guardrail_topic_list.serialize_json(
                value["topics"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GuardrailTopicPolicyAssessment:
    ...
    if "topics" in data:
        ...
        out["topics"] = (
            aws_sdk_bedrock_agent_runtime.types.guardrail_topic_list.deserialize_json(
                data["topics"]
            )
        )
    return out
