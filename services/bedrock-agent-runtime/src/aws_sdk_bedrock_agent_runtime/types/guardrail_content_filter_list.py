...
GuardrailContentFilterList: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.guardrail_content_filter.GuardrailContentFilter"
]
...
def serialize_json(value: GuardrailContentFilterList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.guardrail_content_filter.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> GuardrailContentFilterList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.guardrail_content_filter.deserialize_json(
                item
            )
        )
    return out
