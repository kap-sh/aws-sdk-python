...
GuardrailTopicList: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.guardrail_topic.GuardrailTopic"
]
...
def serialize_json(value: GuardrailTopicList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.guardrail_topic.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> GuardrailTopicList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.guardrail_topic.deserialize_json(item)
        )
    return out
