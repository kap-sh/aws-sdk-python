...
class FlowTraceConditionNodeResultEvent(TypedDict):
    ...
    satisfied_conditions: "aws_sdk_bedrock_agent_runtime.types.flow_trace_conditions.FlowTraceConditions"
...
def serialize_json(value: FlowTraceConditionNodeResultEvent) -> dict:
    ...
    import aws_sdk_bedrock_agent_runtime.types.flow_trace_conditions
    out["satisfiedConditions"] = aws_sdk_bedrock_agent_runtime.types.flow_trace_conditions.serialize_json(value["satisfied_conditions"])
    return out
...
def deserialize_json(data: dict) -> FlowTraceConditionNodeResultEvent:
    ...
    if "nodeName" in data:
        ...
        raise DeserializationError("FlowTraceConditionNodeResultEvent.node_name required")
    if "timestamp" in data:
        import aws_sdk_bedrock_agent_runtime.types.date_timestamp
        out["timestamp"] = aws_sdk_bedrock_agent_runtime.types.date_timestamp.deserialize_json(data["timestamp"])
    else:
        raise DeserializationError("FlowTraceConditionNodeResultEvent.timestamp required")
    if "satisfiedConditions" in data:
        import aws_sdk_bedrock_agent_runtime.types.flow_trace_conditions
        out["satisfied_conditions"] = aws_sdk_bedrock_agent_runtime.types.flow_trace_conditions.deserialize_json(data["satisfiedConditions"])
    else:
        raise DeserializationError("FlowTraceConditionNodeResultEvent.satisfied_conditions required")
    return out