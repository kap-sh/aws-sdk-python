...
BedrockSessionContentBlocks: TypeAlias = list[
    "aws_sdk_bedrock_agent_runtime.types.bedrock_session_content_block.BedrockSessionContentBlock"
]
...
def serialize_json(value: BedrockSessionContentBlocks) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.bedrock_session_content_block.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> BedrockSessionContentBlocks:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agent_runtime.types.bedrock_session_content_block.deserialize_json(
                item
            )
        )
    return out
