...
class FlowExecutionError(TypedDict, closed=True):
    ...
    error: NotRequired[
        "aws_sdk_bedrock_agent_runtime.types.flow_execution_error_type.FlowExecutionErrorType"
    ]
...
def serialize_json(value: FlowExecutionError) -> dict:
    ...
    if "error" in value:
        ...
        out["error"] = (
            aws_sdk_bedrock_agent_runtime.types.flow_execution_error_type.serialize_json(
                value["error"]
            )
        )
    if "message" in value:
...
def deserialize_json(data: dict) -> FlowExecutionError:
    ...
    if "error" in data:
        ...
        out["error"] = (
            aws_sdk_bedrock_agent_runtime.types.flow_execution_error_type.deserialize_json(
                data["error"]
            )
        )
    if "message" in data:
