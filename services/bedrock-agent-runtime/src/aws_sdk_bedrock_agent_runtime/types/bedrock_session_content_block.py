...
BedrockSessionContentBlock: TypeAlias = (
    _BedrockSessionContentBlock_text | _BedrockSessionContentBlock_image
)
...
def serialize_json(value: BedrockSessionContentBlock) -> dict:
    if "text" in value:
        ...
        return {
            "image": aws_sdk_bedrock_agent_runtime.types.image_block.serialize_json(
                value["image"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> BedrockSessionContentBlock:
    if "text" in data:
        ...
        return {
            "image": aws_sdk_bedrock_agent_runtime.types.image_block.deserialize_json(
                data["image"]
            )
        }
    else:
        raise DeserializationError(
            "BedrockSessionContentBlock: no recognized variant key"
        )
