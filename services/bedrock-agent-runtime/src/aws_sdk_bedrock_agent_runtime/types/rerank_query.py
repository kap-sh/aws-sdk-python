...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent_runtime.types.rerank_query_content_type
...
def serialize_json(value: RerankQuery) -> dict:
    ...
    out["type"] = (
        aws_sdk_bedrock_agent_runtime.types.rerank_query_content_type.serialize_json(
            value["type"]
        )
    )
    import aws_sdk_bedrock_agent_runtime.types.rerank_text_document
...
def deserialize_json(data: dict) -> RerankQuery:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_bedrock_agent_runtime.types.rerank_query_content_type.deserialize_json(
                data["type"]
            )
        )
    else:
