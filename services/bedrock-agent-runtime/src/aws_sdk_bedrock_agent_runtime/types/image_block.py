...
def serialize_json(value: ImageBlock) -> dict:
    ...
    out["format"] = aws_sdk_bedrock_agent_runtime.types.image_format.serialize_json(
        value["format"]
    )
    import aws_sdk_bedrock_agent_runtime.types.image_source

    out["source"] = aws_sdk_bedrock_agent_runtime.types.image_source.serialize_json(
        value["source"]
    )
    return out
...
def deserialize_json(data: dict) -> ImageBlock:
    ...
    if "format" in data:
        ...
        out["format"] = (
            aws_sdk_bedrock_agent_runtime.types.image_format.deserialize_json(
                data["format"]
            )
        )
    else:
    ...
    if "source" in data:
        ...
        out["source"] = (
            aws_sdk_bedrock_agent_runtime.types.image_source.deserialize_json(
                data["source"]
            )
        )
    else:
