...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agent_runtime.types.parameter_description
    import aws_sdk_bedrock_agent_runtime.types.parameter_type


class ParameterDetail(TypedDict, closed=True):
    description: NotRequired[
        "aws_sdk_bedrock_agent_runtime.types.parameter_description.ParameterDescription"
    ]
    """<p> A description of the parameter. Helps the foundation model determine how to elicit the parameters from the user. </p>"""
    type: "aws_sdk_bedrock_agent_runtime.types.parameter_type.ParameterType"
...
def serialize_json(value: ParameterDetail) -> dict:
    ...
    out["type"] = aws_sdk_bedrock_agent_runtime.types.parameter_type.serialize_json(
        value["type"]
    )
...
def deserialize_json(data: dict) -> ParameterDetail:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_bedrock_agent_runtime.types.parameter_type.deserialize_json(
                data["type"]
            )
        )
    else:
