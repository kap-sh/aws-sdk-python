...
def serialize_json(value: FilePart) -> dict:
    ...
    if "files" in value:
        import aws_sdk_bedrock_agent_runtime.types.output_files
        out["files"] = aws_sdk_bedrock_agent_runtime.types.output_files.serialize_json(value["files"])
    return out
...
def deserialize_json(data: dict) -> FilePart:
    ...
    if "files" in data:
        import aws_sdk_bedrock_agent_runtime.types.output_files
        out["files"] = aws_sdk_bedrock_agent_runtime.types.output_files.deserialize_json(data["files"])
    return out