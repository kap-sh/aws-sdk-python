...
class NodeInputExecutionChainItem(TypedDict):
    ...
    type: (
        "aws_sdk_bedrock_agent_runtime.types.flow_control_node_type.FlowControlNodeType"
    )
...
def serialize_json(value: NodeInputExecutionChainItem) -> dict:
    ...
    out["type"] = (
        aws_sdk_bedrock_agent_runtime.types.flow_control_node_type.serialize_json(
            value["type"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> NodeInputExecutionChainItem:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_bedrock_agent_runtime.types.flow_control_node_type.deserialize_json(
                data["type"]
            )
        )
    else:
