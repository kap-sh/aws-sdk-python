"""Generated from Smithy shape ``com.amazonaws.bedrockagentruntime#NodeFailureEvent``."""

from typing import TYPE_CHECKING, TypedDict

from aws_sdk_bedrock_agent_runtime.errors import DeserializationError

if TYPE_CHECKING:
    import aws_sdk_bedrock_agent_runtime.types.date_timestamp
    import aws_sdk_bedrock_agent_runtime.types.node_error_code
    import aws_sdk_bedrock_agent_runtime.types.node_name


class NodeFailureEvent(TypedDict):
    node_name: "aws_sdk_bedrock_agent_runtime.types.node_name.NodeName"
    """<p>The name of the node where the failure occurred.</p>"""
    timestamp: "aws_sdk_bedrock_agent_runtime.types.date_timestamp.DateTimestamp"
    """<p>The timestamp when the node failure occurred.</p>"""
    error_code: "aws_sdk_bedrock_agent_runtime.types.node_error_code.NodeErrorCode"
    """<p>The error code that identifies the type of failure that occurred at the node.</p>"""
    error_message: "str"
    """<p>A descriptive message that provides details about the node failure.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: NodeFailureEvent) -> dict:
    out: dict = {}
    out["nodeName"] = value["node_name"]
    import aws_sdk_bedrock_agent_runtime.types.date_timestamp

    out["timestamp"] = (
        aws_sdk_bedrock_agent_runtime.types.date_timestamp.serialize_json(
            value["timestamp"]
        )
    )
    import aws_sdk_bedrock_agent_runtime.types.node_error_code

    out["errorCode"] = (
        aws_sdk_bedrock_agent_runtime.types.node_error_code.serialize_json(
            value["error_code"]
        )
    )
    out["errorMessage"] = value["error_message"]
    return out


def deserialize_json(data: dict) -> NodeFailureEvent:
    out: NodeFailureEvent = {}  # type: ignore[typeddict-item]
    if "nodeName" in data:
        out["node_name"] = data["nodeName"]
    else:
        raise DeserializationError("NodeFailureEvent.node_name required")
    if "timestamp" in data:
        import aws_sdk_bedrock_agent_runtime.types.date_timestamp

        out["timestamp"] = (
            aws_sdk_bedrock_agent_runtime.types.date_timestamp.deserialize_json(
                data["timestamp"]
            )
        )
    else:
        raise DeserializationError("NodeFailureEvent.timestamp required")
    if "errorCode" in data:
        import aws_sdk_bedrock_agent_runtime.types.node_error_code

        out["error_code"] = (
            aws_sdk_bedrock_agent_runtime.types.node_error_code.deserialize_json(
                data["errorCode"]
            )
        )
    else:
        raise DeserializationError("NodeFailureEvent.error_code required")
    if "errorMessage" in data:
        out["error_message"] = data["errorMessage"]
    else:
        raise DeserializationError("NodeFailureEvent.error_message required")
    return out
