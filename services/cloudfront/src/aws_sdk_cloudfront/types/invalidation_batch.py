...
if TYPE_CHECKING:
    import aws_sdk_cloudfront.types.paths
    import aws_sdk_cloudfront.types.string


class InvalidationBatch(TypedDict):
    paths: "aws_sdk_cloudfront.types.paths.Paths"
...
def serialize_xml(value: InvalidationBatch, parent: Element, tag: str) -> None:
    ...
    aws_sdk_cloudfront.types.paths.serialize_xml(value["paths"], el, "Paths")
...
def deserialize_xml(el: Element) -> InvalidationBatch:
    ...
    if child_paths is not None:
        ...
        out["paths"] = aws_sdk_cloudfront.types.paths.deserialize_xml(child_paths)
    else:
