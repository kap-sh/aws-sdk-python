"""Generated from Smithy shape ``com.amazonaws.cloudfront#KeyValueStoreList``."""
...
if TYPE_CHECKING:
    ...
    import aws_sdk_cloudfront.types.key_value_store_summary_list
    import aws_sdk_cloudfront.types.string


class KeyValueStoreList(TypedDict, closed=True):
    ...
    items: NotRequired[
        "aws_sdk_cloudfront.types.key_value_store_summary_list.KeyValueStoreSummaryList"
    ]
    """<p>The items of the key value store list.</p>"""


# --- restXml ser/de ---
def serialize_xml(value: KeyValueStoreList, parent: Element, tag: str) -> None:
    ...
    if "items" in value:
        ...
        aws_sdk_cloudfront.types.key_value_store_summary_list.serialize_xml(
            value["items"], el, "Items"
        )


def deserialize_xml(el: Element) -> KeyValueStoreList:
    out: KeyValueStoreList = {}  # type: ignore[typeddict-item]
    ...
    if child_max_items is not None:
        ...
        raise DeserializationError("KeyValueStoreList.max_items required")
    child_quantity = el.find("Quantity")
    if child_quantity is not None:
        out["quantity"] = int(child_quantity.text or "")
    else:
        raise DeserializationError("KeyValueStoreList.quantity required")
    child_items = el.find("Items")
    if child_items is not None:
        import aws_sdk_cloudfront.types.key_value_store_summary_list

        out["items"] = (
            aws_sdk_cloudfront.types.key_value_store_summary_list.deserialize_xml(
                child_items
            )
        )
    return out
