...
if TYPE_CHECKING:
    import aws_sdk_cloudfront.types.continuous_deployment_policy_type
    import aws_sdk_cloudfront.types.continuous_deployment_single_header_config
    import aws_sdk_cloudfront.types.continuous_deployment_single_weight_config


class TrafficConfig(TypedDict, closed=True):
    single_weight_config: NotRequired[
        "aws_sdk_cloudfront.types.continuous_deployment_single_weight_config.ContinuousDeploymentSingleWeightConfig"
    ]
    """<p>Contains the percentage of traffic to send to the staging distribution.</p>"""
    single_header_config: NotRequired[
        "aws_sdk_cloudfront.types.continuous_deployment_single_header_config.ContinuousDeploymentSingleHeaderConfig"
    ]
    """<p>Determines which HTTP requests are sent to the staging distribution.</p>"""
    type: "aws_sdk_cloudfront.types.continuous_deployment_policy_type.ContinuousDeploymentPolicyType"
...
def serialize_xml(value: TrafficConfig, parent: Element, tag: str) -> None:
    ...
    if "single_weight_config" in value:
        ...
        aws_sdk_cloudfront.types.continuous_deployment_single_weight_config.serialize_xml(
            value["single_weight_config"], el, "SingleWeightConfig"
        )
    if "single_header_config" in value:
        import aws_sdk_cloudfront.types.continuous_deployment_single_header_config

        aws_sdk_cloudfront.types.continuous_deployment_single_header_config.serialize_xml(
            value["single_header_config"], el, "SingleHeaderConfig"
        )
    import aws_sdk_cloudfront.types.continuous_deployment_policy_type

    aws_sdk_cloudfront.types.continuous_deployment_policy_type.serialize_xml(
        value["type"], el, "Type"
    )
...
def deserialize_xml(el: Element) -> TrafficConfig:
    ...
    if child_single_weight_config is not None:
        ...
        out["single_weight_config"] = (
            aws_sdk_cloudfront.types.continuous_deployment_single_weight_config.deserialize_xml(
    ...
    if child_single_header_config is not None:
        ...
        out["single_header_config"] = (
            aws_sdk_cloudfront.types.continuous_deployment_single_header_config.deserialize_xml(
    ...
    if child_type is not None:
        ...
        out["type"] = (
            aws_sdk_cloudfront.types.continuous_deployment_policy_type.deserialize_xml(
                child_type
            )
        )
    else:
