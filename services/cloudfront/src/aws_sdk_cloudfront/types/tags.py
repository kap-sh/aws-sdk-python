...
if TYPE_CHECKING:
    import aws_sdk_cloudfront.types.tag_list


class Tags(TypedDict, closed=True):
    items: NotRequired["aws_sdk_cloudfront.types.tag_list.TagList"]
...
def serialize_xml(value: Tags, parent: Element, tag: str) -> None:
    ...
    if "items" in value:
        ...
        aws_sdk_cloudfront.types.tag_list.serialize_xml(value["items"], el, "Items")
...
def deserialize_xml(el: Element) -> Tags:
    ...
    if child_items is not None:
        ...
        out["items"] = aws_sdk_cloudfront.types.tag_list.deserialize_xml(child_items)
    return out
