"""Generated from Smithy shape ``com.amazonaws.cloudfront#AssociateAlias``."""
...
def build_request(
    ...
    input_: aws_sdk_cloudfront.types.associate_alias_request.AssociateAliasRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{TargetDistributionId}", quote(str(input_["target_distribution_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "alias" in input_:
...
async def async_associate_alias(
