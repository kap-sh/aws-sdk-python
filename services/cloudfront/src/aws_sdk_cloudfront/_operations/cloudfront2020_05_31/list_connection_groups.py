"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListConnectionGroups``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.list_connection_groups_result.ListConnectionGroupsResult = aws_sdk_cloudfront.types.list_connection_groups_result.deserialize_xml(
...
def build_request(
    ...
    input: aws_sdk_cloudfront.types.list_connection_groups_request.ListConnectionGroupsRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/2020-05-31/connection-groups"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    root = Element("ListConnectionGroupsRequest")
    if "association_filter" in input:
        ...
        aws_sdk_cloudfront.types.connection_group_association_filter.serialize_xml(
            input["association_filter"], root, "AssociationFilter"
        )
    if "marker" in input:
...
async def async_list_connection_groups(
