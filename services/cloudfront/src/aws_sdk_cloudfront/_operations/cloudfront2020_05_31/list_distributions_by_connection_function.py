"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListDistributionsByConnectionFunction``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.list_distributions_by_connection_function_result.ListDistributionsByConnectionFunctionResult = {
        "distribution_list": aws_sdk_cloudfront.types.distribution_list.deserialize_xml(
...
def build_request(
    ...
    input_: aws_sdk_cloudfront.types.list_distributions_by_connection_function_request.ListDistributionsByConnectionFunctionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2020-05-31/distributionsByConnectionFunction"
    params: dict[str, str] = {}
    if "marker" in input_:
        params["Marker"] = str(input_["marker"])
    if "max_items" in input_:
        params["MaxItems"] = str(input_["max_items"])
    if "connection_function_identifier" in input_:
        params["ConnectionFunctionIdentifier"] = str(
            input_["connection_function_identifier"]
        )
...
async def async_list_distributions_by_connection_function(
