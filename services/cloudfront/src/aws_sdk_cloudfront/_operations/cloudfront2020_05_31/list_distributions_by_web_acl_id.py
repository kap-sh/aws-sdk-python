"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListDistributionsByWebACLId``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.list_distributions_by_web_acl_id_result.ListDistributionsByWebACLIdResult = {
...
def build_request(
    ...
    input_: aws_sdk_cloudfront.types.list_distributions_by_web_acl_id_request.ListDistributionsByWebACLIdRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2020-05-31/distributionsByWebACLId/{WebACLId}"
    url = url.replace("{WebACLId}", quote(str(input_["web_acl_id"]), safe=""))
    params: dict[str, str] = {}
    if "marker" in input_:
...
async def async_list_distributions_by_web_acl_id(
