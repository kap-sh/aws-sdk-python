...
def build_request(
    ...
    input: aws_sdk_cloudfront.types.list_distributions_by_anycast_ip_list_id_request.ListDistributionsByAnycastIpListIdRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{AnycastIpListId}", quote(str(input["anycast_ip_list_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "marker" in input:
