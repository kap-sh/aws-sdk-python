"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListDistributionsByAnycastIpListId``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.list_distributions_by_anycast_ip_list_id_result.ListDistributionsByAnycastIpListIdResult = {
...
def build_request(
    ...
    input_: aws_sdk_cloudfront.types.list_distributions_by_anycast_ip_list_id_request.ListDistributionsByAnycastIpListIdRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{AnycastIpListId}", quote(str(input_["anycast_ip_list_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "marker" in input_:
...
async def async_list_distributions_by_anycast_ip_list_id(
