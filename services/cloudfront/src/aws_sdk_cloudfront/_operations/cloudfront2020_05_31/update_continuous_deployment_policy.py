"""Generated from Smithy shape ``com.amazonaws.cloudfront#UpdateContinuousDeploymentPolicy``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.update_continuous_deployment_policy_result.UpdateContinuousDeploymentPolicyResult = {
...
def build_request(
    ...
    input: aws_sdk_cloudfront.types.update_continuous_deployment_policy_request.UpdateContinuousDeploymentPolicyRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/2020-05-31/continuous-deployment-policy/{Id}"
    url = url.replace("{Id}", quote(str(input["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "if_match" in input:
        headers["If-Match"] = str(input["if_match"])
    if "continuous_deployment_policy_config" in input:
...
async def async_update_continuous_deployment_policy(
