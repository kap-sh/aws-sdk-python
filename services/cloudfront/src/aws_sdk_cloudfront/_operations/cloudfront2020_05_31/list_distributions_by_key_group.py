"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListDistributionsByKeyGroup``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.list_distributions_by_key_group_result.ListDistributionsByKeyGroupResult = {
...
def build_request(
    ...
    input_: aws_sdk_cloudfront.types.list_distributions_by_key_group_request.ListDistributionsByKeyGroupRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/2020-05-31/distributionsByKeyGroupId/{KeyGroupId}"
    )
    url = url.replace("{KeyGroupId}", quote(str(input_["key_group_id"]), safe=""))
    params: dict[str, str] = {}
    if "marker" in input_:
...
async def async_list_distributions_by_key_group(
