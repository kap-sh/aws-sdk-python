"""Generated from Smithy shape ``com.amazonaws.cloudfront#VerifyDnsConfiguration``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.verify_dns_configuration_result.VerifyDnsConfigurationResult = aws_sdk_cloudfront.types.verify_dns_configuration_result.deserialize_xml(
...
def build_request(
    ...
    input: aws_sdk_cloudfront.types.verify_dns_configuration_request.VerifyDnsConfigurationRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/2020-05-31/verify-dns-configuration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    root = Element("VerifyDnsConfigurationRequest")
    if "domain" in input:
        SubElement(root, "Domain").text = str(input["domain"])
    if "identifier" in input:
        SubElement(root, "Identifier").text = str(input["identifier"])
...
async def async_verify_dns_configuration(
