"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListConflictingAliases``."""
...
def handle_response(
    ...
    out: aws_sdk_cloudfront.types.list_conflicting_aliases_result.ListConflictingAliasesResult = {
        "conflicting_aliases_list": aws_sdk_cloudfront.types.conflicting_aliases_list.deserialize_xml(
...
def build_request(
    ...
    input_: aws_sdk_cloudfront.types.list_conflicting_aliases_request.ListConflictingAliasesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2020-05-31/conflicting-alias"
    params: dict[str, str] = {}
    if "distribution_id" in input_:
        params["DistributionId"] = str(input_["distribution_id"])
    if "alias" in input_:
...
async def async_list_conflicting_aliases(
