...
def handle_response(
    ...
    out: capo_cloudfront.types.verify_dns_configuration_result.VerifyDnsConfigurationResult = capo_cloudfront.types.verify_dns_configuration_result.deserialize_xml(
...
def build_request(
    ...
    if "domain" in input_:
        SubElement(root, "Domain").text = input_["domain"]
    if "identifier" in input_:
        SubElement(root, "Identifier").text = input_["identifier"]
