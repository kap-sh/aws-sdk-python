...
import capo_cloudfront.types.conflicting_aliases_list
...
STATUS_CODE_TO_CODE = {400: "InvalidArgument", 404: "NoSuchDistribution"}
...
def handle_response(
    ...
    out: capo_cloudfront.types.list_conflicting_aliases_result.ListConflictingAliasesResult = {
        "conflicting_aliases_list": capo_cloudfront.types.conflicting_aliases_list.deserialize_xml(
...
def build_request(
    ...
    if "distribution_id" in input_:
        params.append(("DistributionId", input_["distribution_id"]))
    if "alias" in input_:
