"""Generated from Smithy shape ``com.amazonaws.cloudfront#ListDistributionsByConnectionMode``."""
...
def handle_response(
    ...
    out: capo_cloudfront.types.list_distributions_by_connection_mode_result.ListDistributionsByConnectionModeResult = {
...
def build_request(
    ...
    url = url.replace(
        "{ConnectionMode}",
        quote(
            capo_cloudfront.types.connection_mode.to_xml_text(
                input_["connection_mode"]
            ),
            safe="",
        ),
    )
...
async def async_list_distributions_by_connection_mode(
