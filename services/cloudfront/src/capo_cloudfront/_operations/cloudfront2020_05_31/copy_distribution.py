...
def handle_response(
    ...
    out: capo_cloudfront.types.copy_distribution_result.CopyDistributionResult = {
...
def build_request(
    ...
    url = url.replace(
        "{PrimaryDistributionId}", quote(input_["primary_distribution_id"], safe="")
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "staging" in input_:
        headers["Staging"] = "true" if input_["staging"] else "false"
