...
import capo_cloudfront.errors.cache_policy_already_exists
import capo_cloudfront.errors.inconsistent_quantities
import capo_cloudfront.errors.invalid_argument
import capo_cloudfront.errors.too_many_cache_policies
import capo_cloudfront.errors.too_many_cookies_in_cache_policy
import capo_cloudfront.errors.too_many_headers_in_cache_policy
import capo_cloudfront.errors.too_many_query_strings_in_cache_policy
...
STATUS_CODE_TO_CODE = {403: "AccessDenied", 409: "CachePolicyAlreadyExists"}
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "CachePolicyAlreadyExists":
            raise capo_cloudfront.errors.cache_policy_already_exists.CachePolicyAlreadyExists.from_xml(
                error_el, message
            )
        ...
        case "TooManyCachePolicies":
            raise capo_cloudfront.errors.too_many_cache_policies.TooManyCachePolicies.from_xml(
                error_el, message
            )
        case "TooManyCookiesInCachePolicy":
            raise capo_cloudfront.errors.too_many_cookies_in_cache_policy.TooManyCookiesInCachePolicy.from_xml(
                error_el, message
            )
        case "TooManyHeadersInCachePolicy":
            raise capo_cloudfront.errors.too_many_headers_in_cache_policy.TooManyHeadersInCachePolicy.from_xml(
                error_el, message
            )
        case "TooManyQueryStringsInCachePolicy":
            raise capo_cloudfront.errors.too_many_query_strings_in_cache_policy.TooManyQueryStringsInCachePolicy.from_xml(
                error_el, message
            )
        case _:
...
def handle_response(
    ...
    out: capo_cloudfront.types.create_cache_policy_result.CreateCachePolicyResult = {
        "cache_policy": capo_cloudfront.types.cache_policy.deserialize_xml(
