...
def handle_response(
    ...
    out: capo_cloudfront.types.list_cache_policies_result.ListCachePoliciesResult = {
        "cache_policy_list": capo_cloudfront.types.cache_policy_list.deserialize_xml(
...
def build_request(
    ...
    if "type" in input_:
        params.append(
            (
                "Type",
                capo_cloudfront.types.cache_policy_type.to_xml_text(input_["type"]),
            )
        )
    if "marker" in input_:
