...
if TYPE_CHECKING:
    import capo_cloudfront.types.cache_policy_summary_list
...
class CachePolicyList(TypedDict, closed=True):
    ...
    items: NotRequired[
        "capo_cloudfront.types.cache_policy_summary_list.CachePolicySummaryList"
    ]
...
def serialize_xml(value: CachePolicyList, parent: Element, tag: str) -> None:
    ...
    if "items" in value:
        ...
        capo_cloudfront.types.cache_policy_summary_list.serialize_xml(
...
def deserialize_xml(el: Element) -> CachePolicyList:
    ...
    if child_items is not None:
        ...
        out["items"] = capo_cloudfront.types.cache_policy_summary_list.deserialize_xml(
            child_items
        )
    return out
