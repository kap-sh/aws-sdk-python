...
if TYPE_CHECKING:
    import capo_cloudfront.types.distribution_id_owner

DistributionIdOwnerItemList: TypeAlias = list[
    "capo_cloudfront.types.distribution_id_owner.DistributionIdOwner"
]
...
def serialize_xml(
    ...
    for item in value:
        ...
        capo_cloudfront.types.distribution_id_owner.serialize_xml(
...
def deserialize_xml(el: Element) -> DistributionIdOwnerItemList:
    ...
    for child in el.findall("DistributionIdOwner"):
        out.append(capo_cloudfront.types.distribution_id_owner.deserialize_xml(child))
    return out
...
def serialize_xml_flat(
    ...
    for item in value:
        ...
        capo_cloudfront.types.distribution_id_owner.serialize_xml(item, parent, tag)
