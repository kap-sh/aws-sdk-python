...
if TYPE_CHECKING:
    import capo_cloudfront.types.status_codes


class OriginGroupFailoverCriteria(TypedDict, closed=True):
    status_codes: "capo_cloudfront.types.status_codes.StatusCodes"
...
def serialize_xml(
    ...
    capo_cloudfront.types.status_codes.serialize_xml(
...
def deserialize_xml(el: Element) -> OriginGroupFailoverCriteria:
    ...
    if child_status_codes is not None:
        ...
        out["status_codes"] = capo_cloudfront.types.status_codes.deserialize_xml(
            child_status_codes
        )
    else:
