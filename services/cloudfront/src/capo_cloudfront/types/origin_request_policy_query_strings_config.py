...
if TYPE_CHECKING:
    import capo_cloudfront.types.origin_request_policy_query_string_behavior
    import capo_cloudfront.types.query_string_names


class OriginRequestPolicyQueryStringsConfig(TypedDict, closed=True):
    query_string_behavior: "capo_cloudfront.types.origin_request_policy_query_string_behavior.OriginRequestPolicyQueryStringBehavior"
    ...
    query_strings: NotRequired[
        "capo_cloudfront.types.query_string_names.QueryStringNames"
    ]
...
def serialize_xml(
    ...
    capo_cloudfront.types.origin_request_policy_query_string_behavior.serialize_xml(
        value["query_string_behavior"], el, "QueryStringBehavior"
    )
    if "query_strings" in value:
        import capo_cloudfront.types.query_string_names

        capo_cloudfront.types.query_string_names.serialize_xml(
...
def deserialize_xml(el: Element) -> OriginRequestPolicyQueryStringsConfig:
    ...
    if child_query_string_behavior is not None:
        ...
        out["query_string_behavior"] = (
            capo_cloudfront.types.origin_request_policy_query_string_behavior.deserialize_xml(
    ...
    if child_query_strings is not None:
        ...
        out["query_strings"] = capo_cloudfront.types.query_string_names.deserialize_xml(
            child_query_strings
        )
    return out
