...
if TYPE_CHECKING:
    import capo_cloudfront.types.key_pair_ids
    import capo_cloudfront.types.string


class KGKeyPairIds(TypedDict, closed=True):
    key_group_id: NotRequired["capo_cloudfront.types.string.string"]
    """<p>The identifier of the key group that contains the public keys.</p>"""
    key_pair_ids: NotRequired["capo_cloudfront.types.key_pair_ids.KeyPairIds"]
...
def serialize_xml(value: KGKeyPairIds, parent: Element, tag: str) -> None:
    ...
    if "key_pair_ids" in value:
        ...
        capo_cloudfront.types.key_pair_ids.serialize_xml(
...
def deserialize_xml(el: Element) -> KGKeyPairIds:
    ...
    if child_key_pair_ids is not None:
        ...
        out["key_pair_ids"] = capo_cloudfront.types.key_pair_ids.deserialize_xml(
            child_key_pair_ids
        )
    return out
