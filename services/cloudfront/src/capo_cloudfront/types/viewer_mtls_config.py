...
if TYPE_CHECKING:
    import capo_cloudfront.types.trust_store_config
    import capo_cloudfront.types.viewer_mtls_mode


class ViewerMtlsConfig(TypedDict, closed=True):
    mode: "capo_cloudfront.types.viewer_mtls_mode.ViewerMtlsMode"
    """<p>The viewer mTLS mode.</p>"""
    trust_store_config: NotRequired[
        "capo_cloudfront.types.trust_store_config.TrustStoreConfig"
    ]
...
def serialize_xml(value: ViewerMtlsConfig, parent: Element, tag: str) -> None:
    ...
    capo_cloudfront.types.viewer_mtls_mode.serialize_xml(
        value.get("mode", "required"), el, "Mode"
    )
    if "trust_store_config" in value:
        import capo_cloudfront.types.trust_store_config

        capo_cloudfront.types.trust_store_config.serialize_xml(
...
def deserialize_xml(el: Element) -> ViewerMtlsConfig:
    ...
    if child_mode is not None:
        ...
        out["mode"] = capo_cloudfront.types.viewer_mtls_mode.deserialize_xml(child_mode)
    else:
        out["mode"] = "required"
    ...
    if child_trust_store_config is not None:
        ...
        out["trust_store_config"] = (
            capo_cloudfront.types.trust_store_config.deserialize_xml(
