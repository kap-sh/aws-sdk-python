...
if TYPE_CHECKING:
    import capo_cloudfront.types.lambda_function_association

LambdaFunctionAssociationList: TypeAlias = list[
    "capo_cloudfront.types.lambda_function_association.LambdaFunctionAssociation"
]
...
def serialize_xml(
    ...
    for item in value:
        ...
        capo_cloudfront.types.lambda_function_association.serialize_xml(
...
def deserialize_xml(el: Element) -> LambdaFunctionAssociationList:
    ...
    for child in el.findall("LambdaFunctionAssociation"):
        out.append(
            capo_cloudfront.types.lambda_function_association.deserialize_xml(child)
        )
    return out
