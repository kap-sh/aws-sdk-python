"""Generated from Smithy shape ``com.amazonaws.cloudfront#UnsupportedOperation``."""
...
# --- restXml ser/de ---
def serialize_xml(value: UnsupportedOperation_, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    if "message" in value:
        SubElement(el, "Message").text = str(value["message"])


def deserialize_xml(el: Element) -> UnsupportedOperation_:
...
class UnsupportedOperation(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.cloudfront#UnsupportedOperation``."""
