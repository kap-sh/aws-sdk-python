"""Generated from Smithy shape ``com.amazonaws.rdsdata#RollbackTransaction``."""
...
def handle_response(
    ...
    out: aws_sdk_rds_data.types.rollback_transaction_response.RollbackTransactionResponse = aws_sdk_rds_data.types.rollback_transaction_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_rds_data.types.rollback_transaction_request.RollbackTransactionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/RollbackTransaction"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_rds_data.types.rollback_transaction_request

    body: bytes | None = json.dumps(
        aws_sdk_rds_data.types.rollback_transaction_request.serialize_json(input_)
    ).encode()
...
async def async_rollback_transaction(
