"""Generated from Smithy shape ``com.amazonaws.rdsdata#ExecuteSql``."""
...
def handle_response(
    ...
    out: capo_rds_data.types.execute_sql_response.ExecuteSqlResponse = (
        capo_rds_data.types.execute_sql_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_rds_data._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ExecuteSql"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_rds_data.types.execute_sql_request.serialize_json(input_)
    ).encode()
...
def execute_sql(
    ...
    capo_rds_data.types.execute_sql_response.ExecuteSqlResponse, zapros.Response
]:
