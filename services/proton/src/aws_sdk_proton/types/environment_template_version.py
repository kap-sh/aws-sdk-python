"""Generated from Smithy shape ``com.amazonaws.proton#EnvironmentTemplateVersion``."""
...
class EnvironmentTemplateVersion(TypedDict, closed=True):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: EnvironmentTemplateVersion) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_0(data: dict) -> EnvironmentTemplateVersion:
    out: EnvironmentTemplateVersion = {}  # type: ignore[typeddict-item]
    if "templateName" in data:
        out["template_name"] = data["templateName"]
    else:
        raise DeserializationError("EnvironmentTemplateVersion.template_name required")
    if "majorVersion" in data:
        out["major_version"] = data["majorVersion"]
    else:
        raise DeserializationError("EnvironmentTemplateVersion.major_version required")
    if "minorVersion" in data:
        out["minor_version"] = data["minorVersion"]
    else:
        raise DeserializationError("EnvironmentTemplateVersion.minor_version required")
    if "recommendedMinorVersion" in data:
        out["recommended_minor_version"] = data["recommendedMinorVersion"]
    if "status" in data:
        out["status"] = data["status"]
    else:
        raise DeserializationError("EnvironmentTemplateVersion.status required")
    ...
    if "arn" in data:
        ...
        raise DeserializationError("EnvironmentTemplateVersion.arn required")
    if "createdAt" in data:
        ...
        raise DeserializationError("EnvironmentTemplateVersion.created_at required")
    if "lastModifiedAt" in data:
        ...
        raise DeserializationError(
            "EnvironmentTemplateVersion.last_modified_at required"
        )
    if "schema" in data:
