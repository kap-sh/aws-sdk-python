...
def handle_response(
    ...
    out: aws_sdk_bedrock_runtime.types.apply_guardrail_response.ApplyGuardrailResponse = aws_sdk_bedrock_runtime.types.apply_guardrail_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_bedrock_runtime.types.apply_guardrail_request.ApplyGuardrailRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{guardrailVersion}", quote(str(input["guardrail_version"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_bedrock_runtime.types.apply_guardrail_request.serialize_json(input)
    ).encode()
...
async def async_apply_guardrail(
