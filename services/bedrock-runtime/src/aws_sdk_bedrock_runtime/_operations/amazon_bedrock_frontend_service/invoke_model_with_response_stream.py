...
def handle_response(
    ...
    _union_deser = aws_sdk_bedrock_runtime.types.response_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_bedrock_runtime.types.invoke_model_with_response_stream_response.InvokeModelWithResponseStreamResponse = {
        "body": cast(Any, raw_stream_to_events(_iter, _message_decoder, _union_deser))
    }  # type: ignore[reportAssignmentType]
    out["content_type"] = str(response.headers["X-Amzn-Bedrock-Content-Type"])
...
def build_request(
    ...
    input_: aws_sdk_bedrock_runtime.types.invoke_model_with_response_stream_request.InvokeModelWithResponseStreamRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "accept" in input_:
        headers["X-Amzn-Bedrock-Accept"] = str(input_["accept"])
    if "trace" in input_:
