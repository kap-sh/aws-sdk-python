...
def handle_response(
    ...
    _union_deser = (
        aws_sdk_bedrock_runtime.types.converse_stream_output.deserialize_event_json
    )
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_bedrock_runtime.types.converse_stream_response.ConverseStreamResponse = {
...
def build_request(
    ...
    input_: aws_sdk_bedrock_runtime.types.converse_stream_request.ConverseStreamRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/model/{modelId}/converse-stream"
    url = url.replace("{modelId}", quote(str(input_["model_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_bedrock_runtime.types.converse_stream_request.serialize_json(input_)
    ).encode()
