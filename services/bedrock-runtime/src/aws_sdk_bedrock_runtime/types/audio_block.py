...
def serialize_json(value: AudioBlock) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_runtime.types.audio_format
    out["format"] = aws_sdk_bedrock_runtime.types.audio_format.serialize_json(value["format"])
    import aws_sdk_bedrock_runtime.types.audio_source
    out["source"] = aws_sdk_bedrock_runtime.types.audio_source.serialize_json(value["source"])
    if "error" in value:
        import aws_sdk_bedrock_runtime.types.error_block
        out["error"] = aws_sdk_bedrock_runtime.types.error_block.serialize_json(value["error"])
    return out
...
def deserialize_json(data: dict) -> AudioBlock:
    ...
    if "format" in data:
        import aws_sdk_bedrock_runtime.types.audio_format
        out["format"] = aws_sdk_bedrock_runtime.types.audio_format.deserialize_json(data["format"])
    else:
        raise DeserializationError("AudioBlock.format required")
    if "source" in data:
        import aws_sdk_bedrock_runtime.types.audio_source
        out["source"] = aws_sdk_bedrock_runtime.types.audio_source.deserialize_json(data["source"])
    else:
        raise DeserializationError("AudioBlock.source required")
    if "error" in data:
        import aws_sdk_bedrock_runtime.types.error_block
        out["error"] = aws_sdk_bedrock_runtime.types.error_block.deserialize_json(data["error"])
    return out