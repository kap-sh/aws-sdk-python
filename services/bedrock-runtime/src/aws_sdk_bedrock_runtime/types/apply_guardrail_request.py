...
class ApplyGuardrailRequest(TypedDict):
    ...
    output_scope: NotRequired["aws_sdk_bedrock_runtime.types.guardrail_output_scope.GuardrailOutputScope"]
...
def serialize_json(value: ApplyGuardrailRequest) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_runtime.types.guardrail_content_source
    out["source"] = aws_sdk_bedrock_runtime.types.guardrail_content_source.serialize_json(value["source"])
    import aws_sdk_bedrock_runtime.types.guardrail_content_block_list
    out["content"] = aws_sdk_bedrock_runtime.types.guardrail_content_block_list.serialize_json(value["content"])
    if "output_scope" in value:
        import aws_sdk_bedrock_runtime.types.guardrail_output_scope
        out["outputScope"] = aws_sdk_bedrock_runtime.types.guardrail_output_scope.serialize_json(value["output_scope"])
    return out
...
def deserialize_json(data: dict) -> ApplyGuardrailRequest:
    ...
    if "source" in data:
        import aws_sdk_bedrock_runtime.types.guardrail_content_source
        out["source"] = aws_sdk_bedrock_runtime.types.guardrail_content_source.deserialize_json(data["source"])
    else:
        raise DeserializationError("ApplyGuardrailRequest.source required")
    if "content" in data:
        import aws_sdk_bedrock_runtime.types.guardrail_content_block_list
        out["content"] = aws_sdk_bedrock_runtime.types.guardrail_content_block_list.deserialize_json(data["content"])
    else:
        raise DeserializationError("ApplyGuardrailRequest.content required")
    if "outputScope" in data:
        import aws_sdk_bedrock_runtime.types.guardrail_output_scope
        out["output_scope"] = aws_sdk_bedrock_runtime.types.guardrail_output_scope.deserialize_json(data["outputScope"])
    return out