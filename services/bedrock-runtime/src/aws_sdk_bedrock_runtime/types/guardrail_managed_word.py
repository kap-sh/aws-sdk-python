...
if TYPE_CHECKING:
    import aws_sdk_bedrock_runtime.types.guardrail_managed_word_type
    import aws_sdk_bedrock_runtime.types.guardrail_word_policy_action


class GuardrailManagedWord(TypedDict, closed=True):
    match: "str"
    """<p>The match for the managed word.</p>"""
    type: "aws_sdk_bedrock_runtime.types.guardrail_managed_word_type.GuardrailManagedWordType"
    """<p>The type for the managed word.</p>"""
    action: "aws_sdk_bedrock_runtime.types.guardrail_word_policy_action.GuardrailWordPolicyAction"
...
def serialize_json(value: GuardrailManagedWord) -> dict:
    ...
    out["type"] = (
        aws_sdk_bedrock_runtime.types.guardrail_managed_word_type.serialize_json(
            value["type"]
        )
    )
    import aws_sdk_bedrock_runtime.types.guardrail_word_policy_action
    ...
    out["action"] = (
        aws_sdk_bedrock_runtime.types.guardrail_word_policy_action.serialize_json(
            value["action"]
        )
    )
...
def deserialize_json(data: dict) -> GuardrailManagedWord:
    ...
    if "type" in data:
        ...
        out["type"] = (
            aws_sdk_bedrock_runtime.types.guardrail_managed_word_type.deserialize_json(
                data["type"]
            )
        )
    else:
    ...
    if "action" in data:
        ...
        out["action"] = (
            aws_sdk_bedrock_runtime.types.guardrail_word_policy_action.deserialize_json(
                data["action"]
            )
        )
    else:
