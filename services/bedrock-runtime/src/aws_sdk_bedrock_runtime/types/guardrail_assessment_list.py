...
GuardrailAssessmentList: TypeAlias = list[
    "aws_sdk_bedrock_runtime.types.guardrail_assessment.GuardrailAssessment"
]
...
def serialize_json(value: GuardrailAssessmentList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_runtime.types.guardrail_assessment.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> GuardrailAssessmentList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_runtime.types.guardrail_assessment.deserialize_json(item)
        )
    return out
