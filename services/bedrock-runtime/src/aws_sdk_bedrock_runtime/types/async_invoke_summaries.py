...
AsyncInvokeSummaries: TypeAlias = list[
    "aws_sdk_bedrock_runtime.types.async_invoke_summary.AsyncInvokeSummary"
]
...
def serialize_json(value: AsyncInvokeSummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_runtime.types.async_invoke_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AsyncInvokeSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_runtime.types.async_invoke_summary.deserialize_json(item)
        )
    return out
