...
class AsyncInvokeSummary(TypedDict):
    ...
    client_request_token: NotRequired["aws_sdk_bedrock_runtime.types.async_invoke_idempotency_token.AsyncInvokeIdempotencyToken"]
    """<p>The invocation's idempotency token.</p>"""
    status: NotRequired["aws_sdk_bedrock_runtime.types.async_invoke_status.AsyncInvokeStatus"]
    """<p>The invocation's status.</p>"""
    failure_message: NotRequired["aws_sdk_bedrock_runtime.types.async_invoke_message.AsyncInvokeMessage"]
...
def serialize_json(value: AsyncInvokeSummary) -> dict:
    ...
    if "status" in value:
        import aws_sdk_bedrock_runtime.types.async_invoke_status
        out["status"] = aws_sdk_bedrock_runtime.types.async_invoke_status.serialize_json(value["status"])
    if "failure_message" in value:
        out["failureMessage"] = value["failure_message"]
    import aws_sdk_bedrock_runtime.types.timestamp
    out["submitTime"] = aws_sdk_bedrock_runtime.types.timestamp.serialize_json(value["submit_time"])
    if "last_modified_time" in value:
        import aws_sdk_bedrock_runtime.types.timestamp
        out["lastModifiedTime"] = aws_sdk_bedrock_runtime.types.timestamp.serialize_json(value["last_modified_time"])
    if "end_time" in value:
        import aws_sdk_bedrock_runtime.types.timestamp
        out["endTime"] = aws_sdk_bedrock_runtime.types.timestamp.serialize_json(value["end_time"])
    import aws_sdk_bedrock_runtime.types.async_invoke_output_data_config
    out["outputDataConfig"] = aws_sdk_bedrock_runtime.types.async_invoke_output_data_config.serialize_json(value["output_data_config"])
    return out
...
def deserialize_json(data: dict) -> AsyncInvokeSummary:
    ...
    if "status" in data:
        import aws_sdk_bedrock_runtime.types.async_invoke_status
        out["status"] = aws_sdk_bedrock_runtime.types.async_invoke_status.deserialize_json(data["status"])
    if "failureMessage" in data:
        out["failure_message"] = data["failureMessage"]
    if "submitTime" in data:
        import aws_sdk_bedrock_runtime.types.timestamp
        out["submit_time"] = aws_sdk_bedrock_runtime.types.timestamp.deserialize_json(data["submitTime"])
    else:
        raise DeserializationError("AsyncInvokeSummary.submit_time required")
    if "lastModifiedTime" in data:
        import aws_sdk_bedrock_runtime.types.timestamp
        out["last_modified_time"] = aws_sdk_bedrock_runtime.types.timestamp.deserialize_json(data["lastModifiedTime"])
    if "endTime" in data:
        import aws_sdk_bedrock_runtime.types.timestamp
        out["end_time"] = aws_sdk_bedrock_runtime.types.timestamp.deserialize_json(data["endTime"])
    if "outputDataConfig" in data:
        import aws_sdk_bedrock_runtime.types.async_invoke_output_data_config
        out["output_data_config"] = aws_sdk_bedrock_runtime.types.async_invoke_output_data_config.deserialize_json(data["outputDataConfig"])
    else:
