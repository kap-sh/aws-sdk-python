...
from aws_sdk_bedrock_runtime._auth._sigv4 import SigV4AuthContext, sign_sigv4
from aws_sdk_bedrock_runtime._auth._identity import BearerToken, Credentials, Identity
from aws_sdk_bedrock_runtime._auth._providers import IdentityProvider
...
class HttpBearerSigner(Signer[BearerToken]):
    ...
    async def asign(self, req: Request) -> Request:
        ...
        return Request(req.url, req.method, headers, body=req.body or b"", context=req.context)
    def sign(self, req: Request) -> Request:
        t = self.provider.resolve_identity()
        headers = req.headers.copy()
        headers["Authorization"] = f"Bearer {t['token']}"
