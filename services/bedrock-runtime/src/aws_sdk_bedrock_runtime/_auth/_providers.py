...
from aws_sdk_bedrock_runtime._auth._identity import (
    BearerToken,
    Credentials,
    Identity,
)
...
class ProfileCredentialsProvider(CredentialsProvider):
    ...
    def __init__(self, profile: str | None = None, credentials_file: Path | None = None, config_file: Path | None = None) -> None:
        self._profile = profile or os.environ.get("AWS_PROFILE", "default")
        self._cred_file = credentials_file or Path(os.environ.get("AWS_SHARED_CREDENTIALS_FILE") or Path.home() / ".aws" / "credentials")
        self._cfg_file = config_file or Path(os.environ.get("AWS_CONFIG_FILE") or Path.home() / ".aws" / "config")
    def resolve_identity(self) -> Credentials:
        section = self._load_profile()
        ak = section.get("aws_access_key_id")
        sk = section.get("aws_secret_access_key")
        if not ak or not sk:
            raise IdentityNotFound(f"profile {self._profile!r}: missing aws_access_key_id/aws_secret_access_key")
        out: Credentials = {"access_key": ak, "secret_key": sk}
    ...
    def _load_profile(self) -> dict[str, str]:
        ...
        if self._cfg_file.is_file():
            ...
            key = "default" if self._profile == "default" else f"profile {self._profile}"
        ...
        if not merged:
            raise IdentityNotFound(f"profile {self._profile!r} not found in {self._cred_file} or {self._cfg_file}")
        return merged

class SsoTokenCacheProvider(BearerTokenProvider):
    """Read a token cached by `aws sso login` from ~/.aws/sso/cache/."""
    def __init__(self, cache_dir: Path | None = None, session_name: str | None = None) -> None:
...
def default_aws_credentials_chain() -> IdentityProvider[Credentials]:
    return CachedProvider(ChainedProvider(EnvCredentialsProvider(), ProfileCredentialsProvider()))
