...
class StartReadSetImportJobRequest(TypedDict):
...
def serialize_json(value: StartReadSetImportJobRequest) -> dict:
    ...
    out["sources"] = (
        aws_sdk_omics.types.start_read_set_import_job_source_list.serialize_json(
            value["sources"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> StartReadSetImportJobRequest:
    ...
    if "sources" in data:
        ...
        out["sources"] = (
            aws_sdk_omics.types.start_read_set_import_job_source_list.deserialize_json(
                data["sources"]
            )
        )
    else:
