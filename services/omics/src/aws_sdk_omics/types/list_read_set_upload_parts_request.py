...
class ListReadSetUploadPartsRequest(TypedDict):
...
def serialize_json(value: ListReadSetUploadPartsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = (
            aws_sdk_omics.types.read_set_upload_part_list_filter.serialize_json(
                value["filter"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListReadSetUploadPartsRequest:
    ...
    if "filter" in data:
        ...
        out["filter"] = (
            aws_sdk_omics.types.read_set_upload_part_list_filter.deserialize_json(
                data["filter"]
            )
        )
    return out
