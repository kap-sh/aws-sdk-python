...
ConfigurationList: TypeAlias = list[
    "aws_sdk_omics.types.configuration_list_item.ConfigurationListItem"
]
...
def serialize_json(value: ConfigurationList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_omics.types.configuration_list_item.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ConfigurationList:
    ...
    for item in data:
        out.append(aws_sdk_omics.types.configuration_list_item.deserialize_json(item))
    return out
