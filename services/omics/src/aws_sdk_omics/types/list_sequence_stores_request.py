...
class ListSequenceStoresRequest(TypedDict):
    ...
    filter: NotRequired["aws_sdk_omics.types.sequence_store_filter.SequenceStoreFilter"]
    """<p>A filter to apply to the list.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListSequenceStoresRequest) -> dict:
    out: dict = {}
    if "filter" in value:
        import aws_sdk_omics.types.sequence_store_filter

        out["filter"] = aws_sdk_omics.types.sequence_store_filter.serialize_json(
            value["filter"]
        )
    return out
...
def deserialize_json(data: dict) -> ListSequenceStoresRequest:
    ...
    if "filter" in data:
        ...
        out["filter"] = aws_sdk_omics.types.sequence_store_filter.deserialize_json(
            data["filter"]
        )
    return out
