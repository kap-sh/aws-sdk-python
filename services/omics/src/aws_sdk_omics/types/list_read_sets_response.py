...
if TYPE_CHECKING:
    ...
    import aws_sdk_omics.types.read_set_list


class ListReadSetsResponse(TypedDict):
    next_token: NotRequired["aws_sdk_omics.types.next_token.NextToken"]
    """<p>A pagination token that's included if more results are available.</p>"""
    read_sets: "aws_sdk_omics.types.read_set_list.ReadSetList"
...
def serialize_json(value: ListReadSetsResponse) -> dict:
    ...
    out["readSets"] = aws_sdk_omics.types.read_set_list.serialize_json(
        value["read_sets"]
    )
    return out
...
def deserialize_json(data: dict) -> ListReadSetsResponse:
    ...
    if "readSets" in data:
        ...
        out["read_sets"] = aws_sdk_omics.types.read_set_list.deserialize_json(
            data["readSets"]
        )
    else:
