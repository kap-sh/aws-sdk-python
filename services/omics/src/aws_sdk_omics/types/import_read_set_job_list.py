...
ImportReadSetJobList: TypeAlias = list[
    "aws_sdk_omics.types.import_read_set_job_item.ImportReadSetJobItem"
]
...
def serialize_json(value: ImportReadSetJobList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_omics.types.import_read_set_job_item.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ImportReadSetJobList:
    ...
    for item in data:
        out.append(aws_sdk_omics.types.import_read_set_job_item.deserialize_json(item))
    return out
