...
class ListReferenceImportJobsRequest(TypedDict):
...
def serialize_json(value: ListReferenceImportJobsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = aws_sdk_omics.types.import_reference_filter.serialize_json(
            value["filter"]
        )
    return out
...
def deserialize_json(data: dict) -> ListReferenceImportJobsRequest:
    ...
    if "filter" in data:
        ...
        out["filter"] = aws_sdk_omics.types.import_reference_filter.deserialize_json(
            data["filter"]
        )
    return out
