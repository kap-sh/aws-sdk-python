"""Generated from Smithy shape ``com.amazonaws.omics#FormatOptions``."""

from typing import TYPE_CHECKING, TypeAlias

from typing_extensions import TypedDict

from aws_sdk_omics.errors import DeserializationError, SerializationError

if TYPE_CHECKING:
    import aws_sdk_omics.types.tsv_options
    import aws_sdk_omics.types.vcf_options


class _FormatOptions_tsvOptions(TypedDict, closed=True):
    tsvOptions: "aws_sdk_omics.types.tsv_options.TsvOptions"


class _FormatOptions_vcfOptions(TypedDict, closed=True):
    vcfOptions: "aws_sdk_omics.types.vcf_options.VcfOptions"


FormatOptions: TypeAlias = _FormatOptions_tsvOptions | _FormatOptions_vcfOptions


# --- restJson1 ser/de ---
def serialize_json(value: FormatOptions) -> dict:
    if "tsvOptions" in value:
        import aws_sdk_omics.types.tsv_options

        return {
            "tsvOptions": aws_sdk_omics.types.tsv_options.serialize_json(
                value["tsvOptions"]
            )
        }
    elif "vcfOptions" in value:
        import aws_sdk_omics.types.vcf_options

        return {
            "vcfOptions": aws_sdk_omics.types.vcf_options.serialize_json(
                value["vcfOptions"]
            )
        }
    else:
        raise SerializationError("FormatOptions: no variant present")


def deserialize_json(data: dict) -> FormatOptions:
    if "tsvOptions" in data:
        import aws_sdk_omics.types.tsv_options

        return {
            "tsvOptions": aws_sdk_omics.types.tsv_options.deserialize_json(
                data["tsvOptions"]
            )
        }
    elif "vcfOptions" in data:
        import aws_sdk_omics.types.vcf_options

        return {
            "vcfOptions": aws_sdk_omics.types.vcf_options.deserialize_json(
                data["vcfOptions"]
            )
        }
    else:
        raise DeserializationError("FormatOptions: no recognized variant key")
