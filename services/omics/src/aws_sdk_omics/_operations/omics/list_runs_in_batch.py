...
def handle_response(
    ...
    out: aws_sdk_omics.types.list_runs_in_batch_response.ListRunsInBatchResponse = (
        aws_sdk_omics.types.list_runs_in_batch_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_omics.types.list_runs_in_batch_request.ListRunsInBatchRequest,
) -> zapros.Request:
    ...
    url = url.replace("{batchId}", quote(str(input["batch_id"]), safe=""))
    params: dict[str, str] = {}
    ...
    if "submission_status" in input:
        params["submissionStatus"] = str(input["submission_status"])
    if "run_setting_id" in input:
        params["runSettingId"] = str(input["run_setting_id"])
    if "run_id" in input:
