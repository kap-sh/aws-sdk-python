"""Generated from Smithy shape ``com.amazonaws.omics#CreateAnnotationStoreVersion``."""
...
def handle_response(
    ...
    out: capo_omics.types.create_annotation_store_version_response.CreateAnnotationStoreVersionResponse = capo_omics.types.create_annotation_store_version_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/annotationStore/{name}/version"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.create_annotation_store_version_request.serialize_json(input_)
    ).encode()
...
async def async_create_annotation_store_version(
