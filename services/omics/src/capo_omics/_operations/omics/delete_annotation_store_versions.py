"""Generated from Smithy shape ``com.amazonaws.omics#DeleteAnnotationStoreVersions``."""
...
def handle_response(
    ...
    out: capo_omics.types.delete_annotation_store_versions_response.DeleteAnnotationStoreVersionsResponse = capo_omics.types.delete_annotation_store_versions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/annotationStore/{name}/versions/delete"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    params["force"] = str(input_.get("force", False))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.delete_annotation_store_versions_request.serialize_json(input_)
    ).encode()
...
async def async_delete_annotation_store_versions(
