"""Generated from Smithy shape ``com.amazonaws.omics#ListRunsInBatch``."""
...
def handle_response(
    ...
    out: capo_omics.types.list_runs_in_batch_response.ListRunsInBatchResponse = (
        capo_omics.types.list_runs_in_batch_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/runBatch/{batchId}/run"
    url = url.replace("{batchId}", quote(str(input_["batch_id"]), safe=""))
    params: dict[str, str] = {}
    ...
    if "submission_status" in input_:
        params["submissionStatus"] = str(input_["submission_status"])
    if "run_setting_id" in input_:
        params["runSettingId"] = str(input_["run_setting_id"])
    if "run_id" in input_:
        params["runId"] = str(input_["run_id"])
...
async def async_list_runs_in_batch(
