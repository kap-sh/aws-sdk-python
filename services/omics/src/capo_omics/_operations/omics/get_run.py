"""Generated from Smithy shape ``com.amazonaws.omics#GetRun``."""
...
import capo_omics.errors.conflict_exception
import capo_omics.errors.internal_server_exception
import capo_omics.errors.request_timeout_exception
import capo_omics.errors.resource_not_found_exception
import capo_omics.errors.service_quota_exceeded_exception
import capo_omics.errors.throttling_exception
import capo_omics.errors.validation_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_omics.errors.conflict_exception.ConflictException.from_json(data)
        ...
        case "ServiceQuotaExceededException":
            raise capo_omics.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_omics.types.get_run_response.GetRunResponse = (
        capo_omics.types.get_run_response.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_omics.types.get_run_response.GetRunResponse = (
        capo_omics.types.get_run_response.deserialize_json(
...
def build_request(
    ...
    input_: capo_omics.types.get_run_request.GetRunRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/run/{id}"
...
def get_run(
    options: OperationOptions, input_: capo_omics.types.get_run_request.GetRunRequest
) -> tuple[capo_omics.types.get_run_response.GetRunResponse, zapros.Response]:
