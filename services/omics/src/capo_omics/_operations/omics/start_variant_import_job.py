"""Generated from Smithy shape ``com.amazonaws.omics#StartVariantImportJob``."""
...
def handle_response(
    ...
    out: capo_omics.types.start_variant_import_response.StartVariantImportResponse = (
        capo_omics.types.start_variant_import_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/import/variant"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_omics.types.start_variant_import_request.serialize_json(input_)
    ).encode()
...
async def async_start_variant_import_job(
