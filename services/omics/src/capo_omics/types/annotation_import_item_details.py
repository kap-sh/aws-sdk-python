...
if TYPE_CHECKING:
    import capo_omics.types.annotation_import_item_detail
...
def serialize_json(value: AnnotationImportItemDetails) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_omics.types.annotation_import_item_detail.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> AnnotationImportItemDetails:
    ...
    for item in data:
        out.append(
            capo_omics.types.annotation_import_item_detail.deserialize_json(item)
        )
    return out
