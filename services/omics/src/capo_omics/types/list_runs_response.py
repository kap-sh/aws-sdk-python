...
if TYPE_CHECKING:
    import capo_omics.types.run_list
    import capo_omics.types.run_list_token


class ListRunsResponse(TypedDict, closed=True):
    items: NotRequired["capo_omics.types.run_list.RunList"]
    """<p>A list of runs.</p>"""
    next_token: NotRequired["capo_omics.types.run_list_token.RunListToken"]
...
def serialize_json(value: ListRunsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_omics.types.run_list.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListRunsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_omics.types.run_list.deserialize_json(data["items"])
