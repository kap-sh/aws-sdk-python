...
class ListRunTasksResponse(TypedDict, closed=True):
    items: NotRequired["capo_omics.types.task_list.TaskList"]
    """<p>A list of tasks.</p>"""
    next_token: NotRequired["capo_omics.types.task_list_token.TaskListToken"]
...
def serialize_json(value: ListRunTasksResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_omics.types.task_list.serialize_json(value["items"])
