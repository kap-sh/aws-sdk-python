...
VariantStoreItems: TypeAlias = list[
    "capo_omics.types.variant_store_item.VariantStoreItem"
]
...
def serialize_json(value: VariantStoreItems) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_omics.types.variant_store_item.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> VariantStoreItems:
    ...
    for item in data:
        out.append(capo_omics.types.variant_store_item.deserialize_json(item))
    return out
