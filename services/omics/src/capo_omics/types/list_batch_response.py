...
class ListBatchResponse(TypedDict, closed=True):
    items: NotRequired["capo_omics.types.batch_list.BatchList"]
...
def serialize_json(value: ListBatchResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_omics.types.batch_list.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListBatchResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_omics.types.batch_list.deserialize_json(data["items"])
