...
if TYPE_CHECKING:
    import capo_omics.types.workflow_list
    import capo_omics.types.workflow_list_token


class ListWorkflowsResponse(TypedDict, closed=True):
    items: NotRequired["capo_omics.types.workflow_list.WorkflowList"]
    """<p>A list of workflow items.</p>"""
    next_token: NotRequired["capo_omics.types.workflow_list_token.WorkflowListToken"]
...
def serialize_json(value: ListWorkflowsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_omics.types.workflow_list.serialize_json(value["items"])
...
def deserialize_json(data: dict) -> ListWorkflowsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_omics.types.workflow_list.deserialize_json(data["items"])
