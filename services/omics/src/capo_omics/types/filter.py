...
class Filter(TypedDict, closed=True):
    resource_arns: NotRequired["capo_omics.types.arn_list.ArnList"]
    """<p>Filter based on the Amazon Resource Number (ARN) of the resource. You can specify up to 10 values.</p>"""
    status: NotRequired["capo_omics.types.status_list.StatusList"]
    """<p>Filter based on the resource status. You can specify up to 10 values.</p>"""
    type: NotRequired["capo_omics.types.type_list.TypeList"]
...
def serialize_json(value: Filter) -> dict:
    ...
    if "resource_arns" in value:
        ...
        out["resourceArns"] = capo_omics.types.arn_list.serialize_json(
            value["resource_arns"]
        )
    if "status" in value:
        import capo_omics.types.status_list

        out["status"] = capo_omics.types.status_list.serialize_json(value["status"])
    if "type" in value:
        import capo_omics.types.type_list

        out["type"] = capo_omics.types.type_list.serialize_json(value["type"])
    return out
