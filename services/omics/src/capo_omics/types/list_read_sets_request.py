...
class ListReadSetsRequest(TypedDict, closed=True):
    ...
    filter: NotRequired["capo_omics.types.read_set_filter.ReadSetFilter"]
...
def serialize_json(value: ListReadSetsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["filter"] = capo_omics.types.read_set_filter.serialize_json(value["filter"])
    return out
...
def deserialize_json(data: dict) -> ListReadSetsRequest:
    ...
    if "filter" in data:
        ...
        out["filter"] = capo_omics.types.read_set_filter.deserialize_json(
            data["filter"]
        )
    return out
