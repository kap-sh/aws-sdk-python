...
if TYPE_CHECKING:
    import capo_cloudwatch.types.partial_failure

BatchFailures: TypeAlias = list["capo_cloudwatch.types.partial_failure.PartialFailure"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_cloudwatch.types.partial_failure.serialize_query(
...
def deserialize_query(el: Element) -> BatchFailures:
    ...
    for child in el.findall("member"):
        out.append(capo_cloudwatch.types.partial_failure.deserialize_query(child))
    return out
...
def serialize_aws_json_1_0(value: BatchFailures) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_cloudwatch.types.partial_failure.serialize_aws_json_1_0(item))
    return out
