"""Generated from Smithy shape ``com.amazonaws.cloudwatch#MetricStreamEntries``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_cloudwatch._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_cloudwatch.types.metric_stream_entry

MetricStreamEntries: TypeAlias = list[
    "aws_sdk_cloudwatch.types.metric_stream_entry.MetricStreamEntry"
]


# --- awsQuery ser/de ---
def serialize_query(
    value: MetricStreamEntries, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_cloudwatch.types.metric_stream_entry

    for n, item in enumerate(value, 1):
        aws_sdk_cloudwatch.types.metric_stream_entry.serialize_query(
            item, pairs, f"{prefix}.member.{n}"
        )


def deserialize_query(el: Element) -> MetricStreamEntries:
    import aws_sdk_cloudwatch.types.metric_stream_entry

    out: MetricStreamEntries = []
    for child in el.findall("member"):
        out.append(
            aws_sdk_cloudwatch.types.metric_stream_entry.deserialize_query(child)
        )
    return out


def serialize_query_flat(
    value: MetricStreamEntries, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_cloudwatch.types.metric_stream_entry

    for n, item in enumerate(value, 1):
        aws_sdk_cloudwatch.types.metric_stream_entry.serialize_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_query_flat(parent: Element, tag: str) -> MetricStreamEntries:
    import aws_sdk_cloudwatch.types.metric_stream_entry

    out: MetricStreamEntries = []
    for child in parent.findall(tag):
        out.append(
            aws_sdk_cloudwatch.types.metric_stream_entry.deserialize_query(child)
        )
    return out


# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: MetricStreamEntries) -> list:
    import aws_sdk_cloudwatch.types.metric_stream_entry

    out: list = []
    for item in value:
        out.append(
            aws_sdk_cloudwatch.types.metric_stream_entry.serialize_aws_json_1_0(item)
        )
    return out


def deserialize_aws_json_1_0(data: list) -> MetricStreamEntries:
    import aws_sdk_cloudwatch.types.metric_stream_entry

    out: MetricStreamEntries = []
    for item in data:
        out.append(
            aws_sdk_cloudwatch.types.metric_stream_entry.deserialize_aws_json_1_0(item)
        )
    return out
