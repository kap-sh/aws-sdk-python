...
if TYPE_CHECKING:
    import aws_sdk_cloudwatch.types.metric
    import aws_sdk_cloudwatch.types.period
    import aws_sdk_cloudwatch.types.standard_unit
    import aws_sdk_cloudwatch.types.stat


class MetricStat(TypedDict):
    metric: NotRequired["aws_sdk_cloudwatch.types.metric.Metric"]
    ...
    stat: NotRequired["aws_sdk_cloudwatch.types.stat.Stat"]
...
def serialize_aws_json_1_0(value: MetricStat) -> dict:
    ...
    if "metric" in value:
        ...
        out["Metric"] = aws_sdk_cloudwatch.types.metric.serialize_aws_json_1_0(
            value["metric"]
        )
    if "period" in value:
...
def deserialize_aws_json_1_0(data: dict) -> MetricStat:
    ...
    if "Metric" in data:
        ...
        out["metric"] = aws_sdk_cloudwatch.types.metric.deserialize_aws_json_1_0(
            data["Metric"]
        )
    if "Period" in data:
...
def serialize_query(
    ...
    if "metric" in value:
        ...
        aws_sdk_cloudwatch.types.metric.serialize_query(
...
def deserialize_query(el: Element) -> MetricStat:
    ...
    if child_metric is not None:
        ...
        out["metric"] = aws_sdk_cloudwatch.types.metric.deserialize_query(child_metric)
