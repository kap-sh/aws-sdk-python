...
AlarmTypes: TypeAlias = list["aws_sdk_cloudwatch.types.alarm_type.AlarmType"]
...
def deserialize_query(el: Element) -> AlarmTypes:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_cloudwatch.types.alarm_type.deserialize_query(child))
    return out
...
def serialize_aws_json_1_0(value: AlarmTypes) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_cloudwatch.types.alarm_type.serialize_aws_json_1_0(item))
    return out
...
def deserialize_aws_json_1_0(data: list) -> AlarmTypes:
    ...
    out: AlarmTypes = []
    for item in data:
        out.append(aws_sdk_cloudwatch.types.alarm_type.deserialize_aws_json_1_0(item))
    return out
