...
Statistics: TypeAlias = list["aws_sdk_cloudwatch.types.statistic.Statistic"]
...
def deserialize_query(el: Element) -> Statistics:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_cloudwatch.types.statistic.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_cloudwatch.types.statistic.serialize_query(item, pairs, f"{prefix}.{n}")
...
def serialize_aws_json_1_0(value: Statistics) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_cloudwatch.types.statistic.serialize_aws_json_1_0(item))
    return out
...
def deserialize_aws_json_1_0(data: list) -> Statistics:
    ...
    out: Statistics = []
    for item in data:
        out.append(aws_sdk_cloudwatch.types.statistic.deserialize_aws_json_1_0(item))
    return out
