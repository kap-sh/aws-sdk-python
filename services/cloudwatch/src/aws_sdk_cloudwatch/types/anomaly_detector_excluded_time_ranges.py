...
if TYPE_CHECKING:
    import aws_sdk_cloudwatch.types.range

AnomalyDetectorExcludedTimeRanges: TypeAlias = list[
    "aws_sdk_cloudwatch.types.range.Range"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_cloudwatch.types.range.serialize_query(
...
def deserialize_query(el: Element) -> AnomalyDetectorExcludedTimeRanges:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_cloudwatch.types.range.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_cloudwatch.types.range.serialize_query(item, pairs, f"{prefix}.{n}")
...
def serialize_aws_json_1_0(value: AnomalyDetectorExcludedTimeRanges) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_cloudwatch.types.range.serialize_aws_json_1_0(item))
    return out
...
def deserialize_aws_json_1_0(data: list) -> AnomalyDetectorExcludedTimeRanges:
    ...
    out: AnomalyDetectorExcludedTimeRanges = []
    for item in data:
        out.append(aws_sdk_cloudwatch.types.range.deserialize_aws_json_1_0(item))
    return out
