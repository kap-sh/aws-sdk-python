"""Generated from Smithy shape ``com.amazonaws.cloudwatch#DescribeAlarmHistory``."""
...
import aws_sdk_cloudwatch.errors.invalid_next_token
import aws_sdk_cloudwatch.types.alarm_history_items
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "InvalidNextToken":
            raise aws_sdk_cloudwatch.errors.invalid_next_token.InvalidNextToken.from_aws_json_1_0(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_cloudwatch.types.describe_alarm_history_output.DescribeAlarmHistoryOutput = aws_sdk_cloudwatch.types.describe_alarm_history_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_cloudwatch.types.describe_alarm_history_input.DescribeAlarmHistoryInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    headers["X-Amz-Target"] = "GraniteServiceVersion20100801.DescribeAlarmHistory"
    pairs: list[tuple[str, str]] = []
    pairs.append(("Action", "DescribeAlarmHistory"))
    pairs.append(("Version", "2010-08-01"))
    import aws_sdk_cloudwatch.types.describe_alarm_history_input

    aws_sdk_cloudwatch.types.describe_alarm_history_input.serialize_query(
        input_, pairs, ""
    )
...
async def async_describe_alarm_history(
