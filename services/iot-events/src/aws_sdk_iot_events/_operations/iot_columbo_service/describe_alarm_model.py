...
def build_request(
    ...
    input: aws_sdk_iot_events.types.describe_alarm_model_request.DescribeAlarmModelRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{alarmModelName}", quote(str(input["alarm_model_name"]), safe="")
    )
    params: dict[str, str] = {}
    if "alarm_model_version" in input:
        params["version"] = str(input["alarm_model_version"])
