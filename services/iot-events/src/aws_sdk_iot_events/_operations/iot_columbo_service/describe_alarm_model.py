"""Generated from Smithy shape ``com.amazonaws.iotevents#DescribeAlarmModel``."""
...
def handle_response(
    ...
    out: aws_sdk_iot_events.types.describe_alarm_model_response.DescribeAlarmModelResponse = aws_sdk_iot_events.types.describe_alarm_model_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iot_events.types.describe_alarm_model_request.DescribeAlarmModelRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/alarm-models/{alarmModelName}"
    url = url.replace(
        "{alarmModelName}", quote(str(input_["alarm_model_name"]), safe="")
    )
    params: dict[str, str] = {}
    if "alarm_model_version" in input_:
        params["version"] = str(input_["alarm_model_version"])
...
async def async_describe_alarm_model(
