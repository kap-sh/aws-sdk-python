"""Generated from Smithy shape ``com.amazonaws.iotevents#UpdateInput``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            import aws_sdk_iot_events.errors.resource_not_found_exception

            raise aws_sdk_iot_events.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_iot_events.types.update_input_response.UpdateInputResponse = (
        aws_sdk_iot_events.types.update_input_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_iot_events.types.update_input_request.UpdateInputRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/inputs/{inputName}"
    url = url.replace("{inputName}", quote(str(input["input_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot_events.types.update_input_request

    body: bytes | None = json.dumps(
        aws_sdk_iot_events.types.update_input_request.serialize_json(input)
    ).encode()
...
def update_input(
    ...
    aws_sdk_iot_events.types.update_input_response.UpdateInputResponse, zapros.Response
]:
