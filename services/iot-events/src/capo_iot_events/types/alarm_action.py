...
AlarmAction = TypedDict(
    ...
    {
        "sns": NotRequired[
            "capo_iot_events.types.sns_topic_publish_action.SNSTopicPublishAction"
        ],
        "iot_topic_publish": NotRequired[
            "capo_iot_events.types.iot_topic_publish_action.IotTopicPublishAction"
        ],
        "lambda": NotRequired["capo_iot_events.types.lambda_action.LambdaAction"],
        "iot_events": NotRequired[
            "capo_iot_events.types.iot_events_action.IotEventsAction"
        ],
        "sqs": NotRequired["capo_iot_events.types.sqs_action.SqsAction"],
        "firehose": NotRequired["capo_iot_events.types.firehose_action.FirehoseAction"],
        "dynamo_db": NotRequired[
            "capo_iot_events.types.dynamo_db_action.DynamoDBAction"
        ],
        "dynamo_d_bv2": NotRequired[
            "capo_iot_events.types.dynamo_d_bv2_action.DynamoDBv2Action"
        ],
        "iot_site_wise": NotRequired[
            "capo_iot_events.types.iot_site_wise_action.IotSiteWiseAction"
        ],
    },
    closed=True,
)
...
def serialize_json(value: AlarmAction) -> dict:
    ...
    if "sns" in value:
        ...
        out["sns"] = capo_iot_events.types.sns_topic_publish_action.serialize_json(
            value["sns"]
        )
    if "iot_topic_publish" in value:
        ...
        out["iotTopicPublish"] = (
            capo_iot_events.types.iot_topic_publish_action.serialize_json(
    ...
    if "lambda" in value:
        ...
        out["lambda"] = capo_iot_events.types.lambda_action.serialize_json(
            value["lambda"]
        )
    if "iot_events" in value:
        import capo_iot_events.types.iot_events_action

        out["iotEvents"] = capo_iot_events.types.iot_events_action.serialize_json(
            value["iot_events"]
        )
    if "sqs" in value:
        import capo_iot_events.types.sqs_action

        out["sqs"] = capo_iot_events.types.sqs_action.serialize_json(value["sqs"])
    if "firehose" in value:
        import capo_iot_events.types.firehose_action

        out["firehose"] = capo_iot_events.types.firehose_action.serialize_json(
            value["firehose"]
        )
    if "dynamo_db" in value:
        import capo_iot_events.types.dynamo_db_action

        out["dynamoDB"] = capo_iot_events.types.dynamo_db_action.serialize_json(
            value["dynamo_db"]
        )
    if "dynamo_d_bv2" in value:
        import capo_iot_events.types.dynamo_d_bv2_action

        out["dynamoDBv2"] = capo_iot_events.types.dynamo_d_bv2_action.serialize_json(
            value["dynamo_d_bv2"]
        )
    if "iot_site_wise" in value:
        import capo_iot_events.types.iot_site_wise_action

        out["iotSiteWise"] = capo_iot_events.types.iot_site_wise_action.serialize_json(
            value["iot_site_wise"]
        )
    return out
...
def deserialize_json(data: dict) -> AlarmAction:
    ...
    if "iotTopicPublish" in data:
        ...
        out["iot_topic_publish"] = (
            capo_iot_events.types.iot_topic_publish_action.deserialize_json(
    ...
    if "dynamoDBv2" in data:
        ...
        out["dynamo_d_bv2"] = (
            capo_iot_events.types.dynamo_d_bv2_action.deserialize_json(
                data["dynamoDBv2"]
            )
        )
    if "iotSiteWise" in data:
        ...
        out["iot_site_wise"] = (
            capo_iot_events.types.iot_site_wise_action.deserialize_json(
                data["iotSiteWise"]
            )
        )
    return out
