"""Generated from Smithy shape ``com.amazonaws.iotevents#ListInputRoutings``."""
...
import capo_iot_events.types.routed_resources
...
def handle_response(
    ...
    out: capo_iot_events.types.list_input_routings_response.ListInputRoutingsResponse = capo_iot_events.types.list_input_routings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/input-routings"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_events.types.list_input_routings_request.serialize_json(input_)
    ).encode()
...
async def async_list_input_routings(
