"""Generated from Smithy shape ``com.amazonaws.iotevents#StartDetectorModelAnalysis``."""
...
def handle_response(
    ...
    out: capo_iot_events.types.start_detector_model_analysis_response.StartDetectorModelAnalysisResponse = capo_iot_events.types.start_detector_model_analysis_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/analysis/detector-models"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_events.types.start_detector_model_analysis_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_detector_model_analysis(
