...
def serialize_json(value: CreateTaskInput) -> dict:
    ...
    out["targets"] = aws_sdk_snow_device_management.types.target_list.serialize_json(
        value["targets"]
    )
    import aws_sdk_snow_device_management.types.command

    out["command"] = aws_sdk_snow_device_management.types.command.serialize_json(
        value["command"]
    )
...
def deserialize_json(data: dict) -> CreateTaskInput:
    ...
    if "command" in data:
        ...
        out["command"] = aws_sdk_snow_device_management.types.command.deserialize_json(
            data["command"]
        )
    else:
