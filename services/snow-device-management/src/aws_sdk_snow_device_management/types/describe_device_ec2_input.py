...
class DescribeDeviceEc2Input(TypedDict):
...
def serialize_json(value: DescribeDeviceEc2Input) -> dict:
    out: dict = {}
    import aws_sdk_snow_device_management.types.instance_ids_list
    out["instanceIds"] = aws_sdk_snow_device_management.types.instance_ids_list.serialize_json(value["instance_ids"])
    return out
...
def deserialize_json(data: dict) -> DescribeDeviceEc2Input:
    ...
    if "instanceIds" in data:
        import aws_sdk_snow_device_management.types.instance_ids_list
        out["instance_ids"] = aws_sdk_snow_device_management.types.instance_ids_list.deserialize_json(data["instanceIds"])
    else:
