...
from aws_sdk_snow_device_management.errors import DeserializationError, SerializationError
if TYPE_CHECKING:
...
def serialize_json(value: Command) -> dict:
    if "unlock" in value:
        import aws_sdk_snow_device_management.types.unlock
        return {"unlock": aws_sdk_snow_device_management.types.unlock.serialize_json(value["unlock"])}
    elif "reboot" in value:
        import aws_sdk_snow_device_management.types.reboot
        return {"reboot": aws_sdk_snow_device_management.types.reboot.serialize_json(value["reboot"])}
    else:
...
def deserialize_json(data: dict) -> Command:
    if "unlock" in data:
        import aws_sdk_snow_device_management.types.unlock
        return {"unlock": aws_sdk_snow_device_management.types.unlock.deserialize_json(data["unlock"])}
    elif "reboot" in data:
        import aws_sdk_snow_device_management.types.reboot
        return {"reboot": aws_sdk_snow_device_management.types.reboot.deserialize_json(data["reboot"])}
    else:
