...
TaskSummaryList: TypeAlias = list[
    "aws_sdk_snow_device_management.types.task_summary.TaskSummary"
]
...
def serialize_json(value: TaskSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_snow_device_management.types.task_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> TaskSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_snow_device_management.types.task_summary.deserialize_json(item)
        )
    return out
