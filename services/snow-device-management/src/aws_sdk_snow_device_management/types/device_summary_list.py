...
DeviceSummaryList: TypeAlias = list[
    "aws_sdk_snow_device_management.types.device_summary.DeviceSummary"
]
...
def serialize_json(value: DeviceSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_snow_device_management.types.device_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> DeviceSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_snow_device_management.types.device_summary.deserialize_json(item)
        )
    return out
