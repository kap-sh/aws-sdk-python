"""Generated from Smithy shape ``com.amazonaws.snowdevicemanagement#CreateTask``."""
...
import capo_snow_device_management.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            raise capo_snow_device_management.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_snow_device_management.types.create_task_output.CreateTaskOutput = (
        capo_snow_device_management.types.create_task_output.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_snow_device_management._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/task"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_snow_device_management.types.create_task_input.serialize_json(input_)
    ).encode()
