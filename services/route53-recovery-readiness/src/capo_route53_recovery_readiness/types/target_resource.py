...
class TargetResource(TypedDict, closed=True):
    nlb_resource: NotRequired[
        "capo_route53_recovery_readiness.types.nlb_resource.NLBResource"
    ]
    """<p>The Network Load Balancer Resource.</p>"""
    r53_resource: NotRequired[
        "capo_route53_recovery_readiness.types.r53_resource_record.R53ResourceRecord"
    ]
...
def serialize_json(value: TargetResource) -> dict:
    ...
    if "nlb_resource" in value:
        ...
        out["nLBResource"] = (
            capo_route53_recovery_readiness.types.nlb_resource.serialize_json(
                value["nlb_resource"]
            )
        )
    if "r53_resource" in value:
        ...
        out["r53Resource"] = (
            capo_route53_recovery_readiness.types.r53_resource_record.serialize_json(
                value["r53_resource"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> TargetResource:
    ...
    if "nLBResource" in data:
        ...
        out["nlb_resource"] = (
            capo_route53_recovery_readiness.types.nlb_resource.deserialize_json(
                data["nLBResource"]
            )
        )
    if "r53Resource" in data:
        ...
        out["r53_resource"] = (
            capo_route53_recovery_readiness.types.r53_resource_record.deserialize_json(
                data["r53Resource"]
            )
        )
    return out
