...
__listOfRecoveryGroupOutput: TypeAlias = list[
    "aws_sdk_route53_recovery_readiness.types.recovery_group_output.RecoveryGroupOutput"
]
...
def serialize_json(value: __listOfRecoveryGroupOutput) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_route53_recovery_readiness.types.recovery_group_output.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> __listOfRecoveryGroupOutput:
    ...
    for item in data:
        out.append(
            aws_sdk_route53_recovery_readiness.types.recovery_group_output.deserialize_json(
                item
            )
        )
    return out
