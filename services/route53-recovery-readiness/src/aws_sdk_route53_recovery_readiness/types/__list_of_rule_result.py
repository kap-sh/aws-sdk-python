...
__listOfRuleResult: TypeAlias = list[
    "aws_sdk_route53_recovery_readiness.types.rule_result.RuleResult"
]
...
def serialize_json(value: __listOfRuleResult) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_route53_recovery_readiness.types.rule_result.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> __listOfRuleResult:
    ...
    for item in data:
        out.append(
            aws_sdk_route53_recovery_readiness.types.rule_result.deserialize_json(item)
        )
    return out
