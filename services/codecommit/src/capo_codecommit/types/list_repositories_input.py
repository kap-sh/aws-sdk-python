...
class ListRepositoriesInput(TypedDict, closed=True):
    ...
    sort_by: NotRequired["capo_codecommit.types.sort_by_enum.SortByEnum"]
    """<p>The criteria used to sort the results of a list repositories operation.</p>"""
    order: NotRequired["capo_codecommit.types.order_enum.OrderEnum"]
...
def serialize_aws_json_1_1(value: ListRepositoriesInput) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["sortBy"] = capo_codecommit.types.sort_by_enum.serialize_aws_json_1_1(
            value["sort_by"]
        )
    if "order" in value:
        import capo_codecommit.types.order_enum

        out["order"] = capo_codecommit.types.order_enum.serialize_aws_json_1_1(
            value["order"]
        )
    return out
