...
if TYPE_CHECKING:
    import aws_sdk_connect.types.routing_profile_queue_config_summary

RoutingProfileQueueConfigSummaryList: TypeAlias = list[
    "aws_sdk_connect.types.routing_profile_queue_config_summary.RoutingProfileQueueConfigSummary"
]
...
def serialize_json(value: RoutingProfileQueueConfigSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_connect.types.routing_profile_queue_config_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> RoutingProfileQueueConfigSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_connect.types.routing_profile_queue_config_summary.deserialize_json(
                item
            )
        )
    return out
