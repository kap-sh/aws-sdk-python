...
if TYPE_CHECKING:
    import aws_sdk_connect.types.security_profile_search_summary

SecurityProfilesSearchSummaryList: TypeAlias = list[
    "aws_sdk_connect.types.security_profile_search_summary.SecurityProfileSearchSummary"
]
...
def serialize_json(value: SecurityProfilesSearchSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_connect.types.security_profile_search_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> SecurityProfilesSearchSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_connect.types.security_profile_search_summary.deserialize_json(item)
        )
    return out
