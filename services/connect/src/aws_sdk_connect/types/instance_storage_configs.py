...
def serialize_json(value: InstanceStorageConfigs) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_connect.types.instance_storage_config.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> InstanceStorageConfigs:
    ...
    for item in data:
        out.append(aws_sdk_connect.types.instance_storage_config.deserialize_json(item))
    return out
