...
if TYPE_CHECKING:
    import aws_sdk_connect.types.searchable_segment_attributes_criteria

SearchableSegmentAttributesCriteriaList: TypeAlias = list[
    "aws_sdk_connect.types.searchable_segment_attributes_criteria.SearchableSegmentAttributesCriteria"
]
...
def serialize_json(value: SearchableSegmentAttributesCriteriaList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_connect.types.searchable_segment_attributes_criteria.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> SearchableSegmentAttributesCriteriaList:
    ...
    for item in data:
        out.append(
            aws_sdk_connect.types.searchable_segment_attributes_criteria.deserialize_json(
                item
            )
        )
    return out
