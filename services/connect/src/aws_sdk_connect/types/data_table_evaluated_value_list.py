...
if TYPE_CHECKING:
    import aws_sdk_connect.types.data_table_evaluated_value

DataTableEvaluatedValueList: TypeAlias = list[
    "aws_sdk_connect.types.data_table_evaluated_value.DataTableEvaluatedValue"
]
...
def serialize_json(value: DataTableEvaluatedValueList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_connect.types.data_table_evaluated_value.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> DataTableEvaluatedValueList:
    ...
    for item in data:
        out.append(
            aws_sdk_connect.types.data_table_evaluated_value.deserialize_json(item)
        )
    return out
