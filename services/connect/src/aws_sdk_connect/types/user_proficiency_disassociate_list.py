...
if TYPE_CHECKING:
    import aws_sdk_connect.types.user_proficiency_disassociate

UserProficiencyDisassociateList: TypeAlias = list[
    "aws_sdk_connect.types.user_proficiency_disassociate.UserProficiencyDisassociate"
]
...
def serialize_json(value: UserProficiencyDisassociateList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_connect.types.user_proficiency_disassociate.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> UserProficiencyDisassociateList:
    ...
    for item in data:
        out.append(
            aws_sdk_connect.types.user_proficiency_disassociate.deserialize_json(item)
        )
    return out
