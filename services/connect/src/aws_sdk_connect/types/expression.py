...
if TYPE_CHECKING:
    import aws_sdk_connect.types.attribute_condition
    import aws_sdk_connect.types.expressions


class Expression(TypedDict, closed=True):
    attribute_condition: NotRequired[
        "aws_sdk_connect.types.attribute_condition.AttributeCondition"
    ]
    """<p>An object to specify the predefined attribute condition.</p>"""
    and_expression: NotRequired["aws_sdk_connect.types.expressions.Expressions"]
    """<p>List of routing expressions which will be AND-ed together.</p>"""
    or_expression: NotRequired["aws_sdk_connect.types.expressions.Expressions"]
...
def serialize_json(value: Expression) -> dict:
    ...
    if "attribute_condition" in value:
        ...
        out["AttributeCondition"] = (
            aws_sdk_connect.types.attribute_condition.serialize_json(
    ...
    if "and_expression" in value:
        ...
        out["AndExpression"] = aws_sdk_connect.types.expressions.serialize_json(
            value["and_expression"]
        )
    if "or_expression" in value:
        import aws_sdk_connect.types.expressions

        out["OrExpression"] = aws_sdk_connect.types.expressions.serialize_json(
            value["or_expression"]
        )
...
def deserialize_json(data: dict) -> Expression:
    ...
    if "AttributeCondition" in data:
        ...
        out["attribute_condition"] = (
            aws_sdk_connect.types.attribute_condition.deserialize_json(
    ...
    if "AndExpression" in data:
        ...
        out["and_expression"] = aws_sdk_connect.types.expressions.deserialize_json(
            data["AndExpression"]
        )
    if "OrExpression" in data:
        import aws_sdk_connect.types.expressions

        out["or_expression"] = aws_sdk_connect.types.expressions.deserialize_json(
            data["OrExpression"]
        )
