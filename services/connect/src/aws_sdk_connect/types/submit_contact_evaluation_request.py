...
class SubmitContactEvaluationRequest(TypedDict):
...
def serialize_json(value: SubmitContactEvaluationRequest) -> dict:
    ...
    if "submitted_by" in value:
        ...
        out["SubmittedBy"] = aws_sdk_connect.types.evaluator_user_union.serialize_json(
            value["submitted_by"]
        )
    return out
...
def deserialize_json(data: dict) -> SubmitContactEvaluationRequest:
    ...
    if "SubmittedBy" in data:
        ...
        out["submitted_by"] = (
            aws_sdk_connect.types.evaluator_user_union.deserialize_json(
                data["SubmittedBy"]
            )
        )
    return out
