"""Generated from Smithy shape ``com.amazonaws.connect#BatchCreateDataTableValueFailureResult``."""
...
class BatchCreateDataTableValueFailureResult(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: BatchCreateDataTableValueFailureResult) -> dict:
    out: dict = {}
    import aws_sdk_connect.types.primary_values_set
...
def deserialize_json(data: dict) -> BatchCreateDataTableValueFailureResult:
    out: BatchCreateDataTableValueFailureResult = {}  # type: ignore[typeddict-item]
    if "PrimaryValues" in data:
        ...
        raise DeserializationError(
            "BatchCreateDataTableValueFailureResult.primary_values required"
        )
    if "AttributeName" in data:
        out["attribute_name"] = data["AttributeName"]
    else:
        raise DeserializationError(
            "BatchCreateDataTableValueFailureResult.attribute_name required"
        )
    if "Message" in data:
        out["message"] = data["Message"]
    else:
        raise DeserializationError(
            "BatchCreateDataTableValueFailureResult.message required"
        )
    return out
