...
class DescribeDataTableResponse(TypedDict):
    data_table: "aws_sdk_connect.types.data_table.DataTable"
...
def serialize_json(value: DescribeDataTableResponse) -> dict:
    ...
    out["DataTable"] = aws_sdk_connect.types.data_table.serialize_json(
        value["data_table"]
    )
    return out
...
def deserialize_json(data: dict) -> DescribeDataTableResponse:
    ...
    if "DataTable" in data:
        ...
        out["data_table"] = aws_sdk_connect.types.data_table.deserialize_json(
            data["DataTable"]
        )
    else:
