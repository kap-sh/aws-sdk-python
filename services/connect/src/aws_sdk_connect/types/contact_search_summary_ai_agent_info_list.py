...
if TYPE_CHECKING:
    import aws_sdk_connect.types.contact_search_summary_ai_agent_info

ContactSearchSummaryAiAgentInfoList: TypeAlias = list[
    "aws_sdk_connect.types.contact_search_summary_ai_agent_info.ContactSearchSummaryAiAgentInfo"
]
...
def serialize_json(value: ContactSearchSummaryAiAgentInfoList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_connect.types.contact_search_summary_ai_agent_info.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ContactSearchSummaryAiAgentInfoList:
    ...
    for item in data:
        out.append(
            aws_sdk_connect.types.contact_search_summary_ai_agent_info.deserialize_json(
                item
            )
        )
    return out
