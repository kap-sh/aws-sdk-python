"""Generated from Smithy shape ``com.amazonaws.connect#HoursOfOperationSearchCriteria``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.hours_of_operation_search_condition_list
    import aws_sdk_connect.types.string_condition


class HoursOfOperationSearchCriteria(TypedDict, closed=True):
    or_conditions: NotRequired[
        "aws_sdk_connect.types.hours_of_operation_search_condition_list.HoursOfOperationSearchConditionList"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: HoursOfOperationSearchCriteria) -> dict:
    out: dict = {}
    if "or_conditions" in value:
        import aws_sdk_connect.types.hours_of_operation_search_condition_list

        out["OrConditions"] = (
            aws_sdk_connect.types.hours_of_operation_search_condition_list.serialize_json(
                value["or_conditions"]
            )
        )
...
def deserialize_json(data: dict) -> HoursOfOperationSearchCriteria:
    ...
    if "OrConditions" in data:
        ...
        out["or_conditions"] = (
            aws_sdk_connect.types.hours_of_operation_search_condition_list.deserialize_json(
                data["OrConditions"]
            )
        )
