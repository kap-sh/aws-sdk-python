"""Generated from Smithy shape ``com.amazonaws.connect#DisassociateLambdaFunction``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.disassociate_lambda_function_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.disassociate_lambda_function_request.DisassociateLambdaFunctionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/instance/{InstanceId}/lambda-function"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    if "function_arn" in input_:
        params["functionArn"] = str(input_["function_arn"])
...
async def async_disassociate_lambda_function(
