"""Generated from Smithy shape ``com.amazonaws.connect#UpdateUserHierarchyGroupName``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_user_hierarchy_group_name_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_user_hierarchy_group_name_request.UpdateUserHierarchyGroupNameRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{HierarchyGroupId}", quote(str(input_["hierarchy_group_id"]), safe="")
    )
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_user_hierarchy_group_name_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_user_hierarchy_group_name(
