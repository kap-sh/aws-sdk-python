"""Generated from Smithy shape ``com.amazonaws.connect#StartOutboundChatContact``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.start_outbound_chat_contact_request
    import aws_sdk_connect.types.start_outbound_chat_contact_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.start_outbound_chat_contact_response.StartOutboundChatContactResponse = aws_sdk_connect.types.start_outbound_chat_contact_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.start_outbound_chat_contact_request.StartOutboundChatContactRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/contact/outbound-chat"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.start_outbound_chat_contact_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.start_outbound_chat_contact_request.serialize_json(input_)
    ).encode()
...
async def async_start_outbound_chat_contact(
