"""Generated from Smithy shape ``com.amazonaws.connect#UpdateQueueMaxContacts``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_queue_max_contacts_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_queue_max_contacts_request.UpdateQueueMaxContactsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/queues/{InstanceId}/{QueueId}/max-contacts"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_queue_max_contacts_request.serialize_json(input_)
    ).encode()
...
async def async_update_queue_max_contacts(
