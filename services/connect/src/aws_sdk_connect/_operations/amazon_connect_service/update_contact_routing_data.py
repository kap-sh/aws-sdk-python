"""Generated from Smithy shape ``com.amazonaws.connect#UpdateContactRoutingData``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_contact_routing_data_request
    import aws_sdk_connect.types.update_contact_routing_data_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.update_contact_routing_data_response.UpdateContactRoutingDataResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_connect.types.update_contact_routing_data_request.UpdateContactRoutingDataRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/contacts/{InstanceId}/{ContactId}/routing-data"
    url = url.replace("{InstanceId}", quote(str(input["instance_id"]), safe=""))
    url = url.replace("{ContactId}", quote(str(input["contact_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.update_contact_routing_data_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_contact_routing_data_request.serialize_json(input)
    ).encode()
...
async def async_update_contact_routing_data(
