"""Generated from Smithy shape ``com.amazonaws.connect#SearchVocabularies``."""
...
import aws_sdk_connect.types.search_vocabularies_request
import aws_sdk_connect.types.search_vocabularies_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.search_vocabularies_response.SearchVocabulariesResponse = aws_sdk_connect.types.search_vocabularies_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.search_vocabularies_request.SearchVocabulariesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/vocabulary-summary/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.search_vocabularies_request.serialize_json(input_)
    ).encode()
...
async def async_search_vocabularies(
