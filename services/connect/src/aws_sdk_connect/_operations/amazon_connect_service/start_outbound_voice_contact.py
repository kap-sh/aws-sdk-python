"""Generated from Smithy shape ``com.amazonaws.connect#StartOutboundVoiceContact``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.start_outbound_voice_contact_request
    import aws_sdk_connect.types.start_outbound_voice_contact_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DestinationNotAllowedException":
            import aws_sdk_connect.errors.destination_not_allowed_exception

            raise aws_sdk_connect.errors.destination_not_allowed_exception.DestinationNotAllowedException.from_json(
                data
            )
        ...
        case "OutboundContactNotPermittedException":
            import aws_sdk_connect.errors.outbound_contact_not_permitted_exception

            raise aws_sdk_connect.errors.outbound_contact_not_permitted_exception.OutboundContactNotPermittedException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_connect.types.start_outbound_voice_contact_response.StartOutboundVoiceContactResponse = aws_sdk_connect.types.start_outbound_voice_contact_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_connect.types.start_outbound_voice_contact_request.StartOutboundVoiceContactRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/contact/outbound-voice"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.start_outbound_voice_contact_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.start_outbound_voice_contact_request.serialize_json(input)
    ).encode()
...
async def async_start_outbound_voice_contact(
