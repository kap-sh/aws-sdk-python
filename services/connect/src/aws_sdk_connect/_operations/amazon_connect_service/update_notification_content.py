"""Generated from Smithy shape ``com.amazonaws.connect#UpdateNotificationContent``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_notification_content_request
    import aws_sdk_connect.types.update_notification_content_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.update_notification_content_response.UpdateNotificationContentResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_connect.types.update_notification_content_request.UpdateNotificationContentRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/notifications/{InstanceId}/{NotificationId}"
    url = url.replace("{InstanceId}", quote(str(input["instance_id"]), safe=""))
    url = url.replace("{NotificationId}", quote(str(input["notification_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.update_notification_content_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_notification_content_request.serialize_json(input)
    ).encode()
...
async def async_update_notification_content(
