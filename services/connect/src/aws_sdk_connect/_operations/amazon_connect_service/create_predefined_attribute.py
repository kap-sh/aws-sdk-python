"""Generated from Smithy shape ``com.amazonaws.connect#CreatePredefinedAttribute``."""
...
import aws_sdk_connect.types.create_predefined_attribute_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.create_predefined_attribute_request.CreatePredefinedAttributeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/predefined-attributes/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.create_predefined_attribute_request.serialize_json(input_)
    ).encode()
...
async def async_create_predefined_attribute(
