...
def build_request(
    ...
    input: aws_sdk_connect.types.update_agent_status_request.UpdateAgentStatusRequest,
) -> zapros.Request:
    ...
    url = url.replace("{AgentStatusId}", quote(str(input["agent_status_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.update_agent_status_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_agent_status_request.serialize_json(input)
    ).encode()
