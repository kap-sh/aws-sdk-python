"""Generated from Smithy shape ``com.amazonaws.connect#UpdateAgentStatus``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_agent_status_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_agent_status_request.UpdateAgentStatusRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/agent-status/{InstanceId}/{AgentStatusId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace("{AgentStatusId}", quote(str(input_["agent_status_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.update_agent_status_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_agent_status_request.serialize_json(input_)
    ).encode()
...
async def async_update_agent_status(
