"""Generated from Smithy shape ``com.amazonaws.connect#GetContactAttributes``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.get_contact_attributes_request
    import aws_sdk_connect.types.get_contact_attributes_response
...
) -> aws_sdk_connect.types.get_contact_attributes_response.GetContactAttributesResponse:
    import aws_sdk_connect.types.get_contact_attributes_response

    out: aws_sdk_connect.types.get_contact_attributes_response.GetContactAttributesResponse = aws_sdk_connect.types.get_contact_attributes_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.get_contact_attributes_request.GetContactAttributesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/contact/attributes/{InstanceId}/{InitialContactId}"
    )
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace(
        "{InitialContactId}", quote(str(input_["initial_contact_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_contact_attributes(
