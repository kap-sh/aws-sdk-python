"""Generated from Smithy shape ``com.amazonaws.connect#UpdateRoutingProfileQueues``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_routing_profile_queues_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_routing_profile_queues_request.UpdateRoutingProfileQueuesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_routing_profile_queues_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_routing_profile_queues(
