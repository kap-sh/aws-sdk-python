"""Generated from Smithy shape ``com.amazonaws.connect#SearchPrompts``."""
...
import aws_sdk_connect.types.search_prompts_request
import aws_sdk_connect.types.search_prompts_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.search_prompts_response.SearchPromptsResponse = (
        aws_sdk_connect.types.search_prompts_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.search_prompts_request.SearchPromptsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/search-prompts"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.search_prompts_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.search_prompts_request.serialize_json(input_)
    ).encode()
...
def search_prompts(
    ...
    aws_sdk_connect.types.search_prompts_response.SearchPromptsResponse, zapros.Response
]:
...
async def async_search_prompts(
