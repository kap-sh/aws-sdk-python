"""Generated from Smithy shape ``com.amazonaws.connect#UpdateSecurityProfile``."""
...
import aws_sdk_connect.types.update_security_profile_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_security_profile_request.UpdateSecurityProfileRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/security-profiles/{InstanceId}/{SecurityProfileId}"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_security_profile_request.serialize_json(input_)
    ).encode()
