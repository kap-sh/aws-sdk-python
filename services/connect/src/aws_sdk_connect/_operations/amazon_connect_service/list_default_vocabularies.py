"""Generated from Smithy shape ``com.amazonaws.connect#ListDefaultVocabularies``."""
...
import aws_sdk_connect.types.list_default_vocabularies_request
import aws_sdk_connect.types.list_default_vocabularies_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.list_default_vocabularies_response.ListDefaultVocabulariesResponse = aws_sdk_connect.types.list_default_vocabularies_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.list_default_vocabularies_request.ListDefaultVocabulariesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/default-vocabulary-summary/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.list_default_vocabularies_request.serialize_json(input_)
    ).encode()
...
async def async_list_default_vocabularies(
