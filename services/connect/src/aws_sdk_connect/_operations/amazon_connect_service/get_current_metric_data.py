"""Generated from Smithy shape ``com.amazonaws.connect#GetCurrentMetricData``."""
...
import aws_sdk_connect.types.current_metric_sort_criteria_max_one
import aws_sdk_connect.types.current_metrics
import aws_sdk_connect.types.filters
import aws_sdk_connect.types.get_current_metric_data_request
import aws_sdk_connect.types.get_current_metric_data_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.get_current_metric_data_response.GetCurrentMetricDataResponse = aws_sdk_connect.types.get_current_metric_data_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.get_current_metric_data_request.GetCurrentMetricDataRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/metrics/current/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.get_current_metric_data_request.serialize_json(input_)
    ).encode()
...
async def async_get_current_metric_data(
