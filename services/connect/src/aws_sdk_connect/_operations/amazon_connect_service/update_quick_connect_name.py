"""Generated from Smithy shape ``com.amazonaws.connect#UpdateQuickConnectName``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_quick_connect_name_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_quick_connect_name_request.UpdateQuickConnectNameRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/quick-connects/{InstanceId}/{QuickConnectId}/name"
    )
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace(
        "{QuickConnectId}", quote(str(input_["quick_connect_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_quick_connect_name_request.serialize_json(input_)
    ).encode()
...
async def async_update_quick_connect_name(
