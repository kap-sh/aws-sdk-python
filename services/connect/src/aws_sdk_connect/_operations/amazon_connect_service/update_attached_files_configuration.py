"""Generated from Smithy shape ``com.amazonaws.connect#UpdateAttachedFilesConfiguration``."""
...
import aws_sdk_connect.types.update_attached_files_configuration_request
import aws_sdk_connect.types.update_attached_files_configuration_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.update_attached_files_configuration_response.UpdateAttachedFilesConfigurationResponse = aws_sdk_connect.types.update_attached_files_configuration_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_attached_files_configuration_request.UpdateAttachedFilesConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/attached-files-configurations/{InstanceId}/{AttachmentScope}"
    )
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace(
        "{AttachmentScope}", quote(str(input_["attachment_scope"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_attached_files_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_attached_files_configuration(
