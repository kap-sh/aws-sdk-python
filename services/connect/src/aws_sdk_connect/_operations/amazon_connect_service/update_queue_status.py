"""Generated from Smithy shape ``com.amazonaws.connect#UpdateQueueStatus``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_queue_status_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_queue_status_request.UpdateQueueStatusRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/queues/{InstanceId}/{QueueId}/status"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_queue_status_request.serialize_json(input_)
    ).encode()
...
async def async_update_queue_status(
