"""Generated from Smithy shape ``com.amazonaws.connect#UpdateEmailAddressMetadata``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.update_email_address_metadata_request
    import aws_sdk_connect.types.update_email_address_metadata_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.update_email_address_metadata_response.UpdateEmailAddressMetadataResponse = aws_sdk_connect.types.update_email_address_metadata_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.update_email_address_metadata_request.UpdateEmailAddressMetadataRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/email-addresses/{InstanceId}/{EmailAddressId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace(
        "{EmailAddressId}", quote(str(input_["email_address_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.update_email_address_metadata_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_email_address_metadata(
