"""Generated from Smithy shape ``com.amazonaws.connect#StartWebRTCContact``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.start_web_rtc_contact_request
    import aws_sdk_connect.types.start_web_rtc_contact_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.start_web_rtc_contact_response.StartWebRTCContactResponse = aws_sdk_connect.types.start_web_rtc_contact_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.start_web_rtc_contact_request.StartWebRTCContactRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/contact/webrtc"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_connect.types.start_web_rtc_contact_request

    body: bytes | None = json.dumps(
        aws_sdk_connect.types.start_web_rtc_contact_request.serialize_json(input_)
    ).encode()
...
async def async_start_web_rtc_contact(
