"""Generated from Smithy shape ``com.amazonaws.connect#StartContactMediaProcessing``."""
...
import aws_sdk_connect.types.start_contact_media_processing_request
import aws_sdk_connect.types.start_contact_media_processing_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.start_contact_media_processing_response.StartContactMediaProcessingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_connect.types.start_contact_media_processing_request.StartContactMediaProcessingRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/contact/start-contact-media-processing"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_connect.types.start_contact_media_processing_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_contact_media_processing(
