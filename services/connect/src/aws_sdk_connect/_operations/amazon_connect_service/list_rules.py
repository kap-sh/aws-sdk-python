"""Generated from Smithy shape ``com.amazonaws.connect#ListRules``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.list_rules_request
    import aws_sdk_connect.types.list_rules_response
...
def handle_response(
    ...
    out: aws_sdk_connect.types.list_rules_response.ListRulesResponse = (
        aws_sdk_connect.types.list_rules_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_connect.types.list_rules_request.ListRulesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/rules/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    if "publish_status" in input_:
        params["publishStatus"] = str(input_["publish_status"])
    if "event_source_name" in input_:
        params["eventSourceName"] = str(input_["event_source_name"])
...
def list_rules(
    ...
    aws_sdk_connect.types.list_rules_response.ListRulesResponse, zapros.Response
]:
