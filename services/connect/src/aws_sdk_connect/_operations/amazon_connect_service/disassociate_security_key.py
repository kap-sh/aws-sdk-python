"""Generated from Smithy shape ``com.amazonaws.connect#DisassociateSecurityKey``."""
...
if TYPE_CHECKING:
    import aws_sdk_connect.types.disassociate_security_key_request
...
def build_request(
    ...
    input_: aws_sdk_connect.types.disassociate_security_key_request.DisassociateSecurityKeyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/instance/{InstanceId}/security-key/{AssociationId}"
    )
...
async def async_disassociate_security_key(
