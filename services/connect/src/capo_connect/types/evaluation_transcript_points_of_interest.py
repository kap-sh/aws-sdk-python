...
if TYPE_CHECKING:
    import capo_connect.types.evaluation_transcript_point_of_interest

EvaluationTranscriptPointsOfInterest: TypeAlias = list[
    "capo_connect.types.evaluation_transcript_point_of_interest.EvaluationTranscriptPointOfInterest"
]
...
def serialize_json(value: EvaluationTranscriptPointsOfInterest) -> list:
    ...
    for item in value:
        out.append(
            capo_connect.types.evaluation_transcript_point_of_interest.serialize_json(
                item
            )
        )
    return out
