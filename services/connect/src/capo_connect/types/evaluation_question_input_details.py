...
if TYPE_CHECKING:
    import capo_connect.types.evaluation_transcript_type


class EvaluationQuestionInputDetails(TypedDict, closed=True):
    transcript_type: NotRequired[
        "capo_connect.types.evaluation_transcript_type.EvaluationTranscriptType"
    ]
    """<p>Transcript type.</p>"""
...
def serialize_json(value: EvaluationQuestionInputDetails) -> dict:
    ...
    if "transcript_type" in value:
        ...
        out["TranscriptType"] = (
            capo_connect.types.evaluation_transcript_type.serialize_json(
...
def deserialize_json(data: dict) -> EvaluationQuestionInputDetails:
    ...
    if "TranscriptType" in data:
        ...
        out["transcript_type"] = (
            capo_connect.types.evaluation_transcript_type.deserialize_json(
                data["TranscriptType"]
            )
        )
    return out
