...
class GetCurrentMetricDataResponse(TypedDict, closed=True):
    ...
    metric_results: NotRequired[
        "capo_connect.types.current_metric_results.CurrentMetricResults"
    ]
    """<p>Information about the real-time metrics.</p>"""
    data_snapshot_time: NotRequired["capo_connect.types.timestamp.Timestamp"]
...
def serialize_json(value: GetCurrentMetricDataResponse) -> dict:
    ...
    if "metric_results" in value:
        ...
        out["MetricResults"] = capo_connect.types.current_metric_results.serialize_json(
            value["metric_results"]
        )
    if "data_snapshot_time" in value:
        import capo_connect.types.timestamp

        out["DataSnapshotTime"] = capo_connect.types.timestamp.serialize_json(
...
def deserialize_json(data: dict) -> GetCurrentMetricDataResponse:
    ...
    if "MetricResults" in data:
        ...
        out["metric_results"] = (
            capo_connect.types.current_metric_results.deserialize_json(
                data["MetricResults"]
            )
        )
