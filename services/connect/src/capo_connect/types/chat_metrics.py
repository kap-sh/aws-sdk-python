...
class ChatMetrics(TypedDict, closed=True):
    chat_contact_metrics: NotRequired[
        "capo_connect.types.chat_contact_metrics.ChatContactMetrics"
    ]
    """<p>Information about the overall participant interactions at the contact level.</p>"""
    agent_metrics: NotRequired[
        "capo_connect.types.participant_metrics.ParticipantMetrics"
    ]
...
def serialize_json(value: ChatMetrics) -> dict:
    ...
    if "chat_contact_metrics" in value:
        ...
        out["ChatContactMetrics"] = (
            capo_connect.types.chat_contact_metrics.serialize_json(
    ...
    if "agent_metrics" in value:
        ...
        out["AgentMetrics"] = capo_connect.types.participant_metrics.serialize_json(
            value["agent_metrics"]
        )
    if "customer_metrics" in value:
        import capo_connect.types.participant_metrics

        out["CustomerMetrics"] = capo_connect.types.participant_metrics.serialize_json(
...
def deserialize_json(data: dict) -> ChatMetrics:
    ...
    if "ChatContactMetrics" in data:
        ...
        out["chat_contact_metrics"] = (
            capo_connect.types.chat_contact_metrics.deserialize_json(
    ...
    if "CustomerMetrics" in data:
        ...
        out["customer_metrics"] = (
            capo_connect.types.participant_metrics.deserialize_json(
