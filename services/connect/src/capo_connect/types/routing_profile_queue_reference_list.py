...
RoutingProfileQueueReferenceList: TypeAlias = list[
    "capo_connect.types.routing_profile_queue_reference.RoutingProfileQueueReference"
]
...
def serialize_json(value: RoutingProfileQueueReferenceList) -> list:
    ...
    for item in value:
        out.append(
            capo_connect.types.routing_profile_queue_reference.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> RoutingProfileQueueReferenceList:
    ...
    for item in data:
        out.append(
            capo_connect.types.routing_profile_queue_reference.deserialize_json(item)
        )
    return out
