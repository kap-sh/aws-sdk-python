...
class AgentStatusReference(TypedDict, closed=True):
    status_start_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>The start timestamp of the agent's status.</p>"""
    status_arn: NotRequired["capo_connect.types.arn.ARN"]
    """<p>The Amazon Resource Name (ARN) of the agent's status.</p>"""
    status_name: NotRequired["capo_connect.types.agent_status_name.AgentStatusName"]
...
def serialize_json(value: AgentStatusReference) -> dict:
    ...
    if "status_start_timestamp" in value:
        ...
        out["StatusStartTimestamp"] = capo_connect.types.timestamp.serialize_json(
