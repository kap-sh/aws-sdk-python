...
NotificationSummaryList: TypeAlias = list[
    "capo_connect.types.notification.Notification"
]
...
def serialize_json(value: NotificationSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_connect.types.notification.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> NotificationSummaryList:
    ...
    for item in data:
        out.append(capo_connect.types.notification.deserialize_json(item))
    return out
