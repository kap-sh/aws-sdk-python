...
if TYPE_CHECKING:
    import capo_connect.types.evaluation_arn
    import capo_connect.types.export_location
    import capo_connect.types.form_id
    import capo_connect.types.status
    import capo_connect.types.timestamp


class ContactEvaluation(TypedDict, closed=True):
    form_id: NotRequired["capo_connect.types.form_id.FormId"]
    """<p>The <code>FormId</code> of the contact evaluation.</p>"""
    evaluation_arn: NotRequired["capo_connect.types.evaluation_arn.EvaluationArn"]
    """<p>The Amazon Resource Name for the evaluation form. It is always present.</p>"""
    status: NotRequired["capo_connect.types.status.Status"]
    ...
    end_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>The date and time when the evaluation was submitted, in UTC time.</p>"""
    delete_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>The date and time when the evaluation was deleted, in UTC time.</p>"""
    export_location: NotRequired["capo_connect.types.export_location.ExportLocation"]
...
def serialize_json(value: ContactEvaluation) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_connect.types.status.serialize_json(value["status"])
    ...
    if "end_timestamp" in value:
        ...
        out["EndTimestamp"] = capo_connect.types.timestamp.serialize_json(
            value["end_timestamp"]
        )
    if "delete_timestamp" in value:
        import capo_connect.types.timestamp

        out["DeleteTimestamp"] = capo_connect.types.timestamp.serialize_json(
