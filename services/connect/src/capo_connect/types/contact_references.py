...
if TYPE_CHECKING:
    import capo_connect.types.reference
    import capo_connect.types.reference_key

ContactReferences: TypeAlias = dict[
    "capo_connect.types.reference_key.ReferenceKey",
    "capo_connect.types.reference.Reference",
]
...
def serialize_json(input_to_serialize: ContactReferences) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[key] = capo_connect.types.reference.serialize_json(value)
    return out
...
def deserialize_json(data: dict) -> ContactReferences:
    ...
    for key, value in data.items():
        ...
        out[key] = capo_connect.types.reference.deserialize_json(value)
    return out
