...
if TYPE_CHECKING:
    ...
    import capo_connect.types.workspace_search_condition_list


class WorkspaceSearchCriteria(TypedDict, closed=True):
    or_conditions: NotRequired[
        "capo_connect.types.workspace_search_condition_list.WorkspaceSearchConditionList"
    ]
...
def serialize_json(value: WorkspaceSearchCriteria) -> dict:
    ...
    if "or_conditions" in value:
        ...
        out["OrConditions"] = (
            capo_connect.types.workspace_search_condition_list.serialize_json(
                value["or_conditions"]
            )
        )
...
def deserialize_json(data: dict) -> WorkspaceSearchCriteria:
    ...
    if "OrConditions" in data:
        ...
        out["or_conditions"] = (
            capo_connect.types.workspace_search_condition_list.deserialize_json(
                data["OrConditions"]
            )
        )
