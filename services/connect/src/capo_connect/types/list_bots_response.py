...
if TYPE_CHECKING:
    import capo_connect.types.lex_bot_config_list
    import capo_connect.types.next_token


class ListBotsResponse(TypedDict, closed=True):
    lex_bots: NotRequired["capo_connect.types.lex_bot_config_list.LexBotConfigList"]
...
def serialize_json(value: ListBotsResponse) -> dict:
    ...
    if "lex_bots" in value:
        ...
        out["LexBots"] = capo_connect.types.lex_bot_config_list.serialize_json(
            value["lex_bots"]
        )
...
def deserialize_json(data: dict) -> ListBotsResponse:
    ...
    if "LexBots" in data:
        ...
        out["lex_bots"] = capo_connect.types.lex_bot_config_list.deserialize_json(
            data["LexBots"]
        )
