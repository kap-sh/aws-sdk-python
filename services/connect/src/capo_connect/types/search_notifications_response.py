...
if TYPE_CHECKING:
    ...
    import capo_connect.types.notification_search_summary_list


class SearchNotificationsResponse(TypedDict, closed=True):
    notifications: NotRequired[
        "capo_connect.types.notification_search_summary_list.NotificationSearchSummaryList"
    ]
...
def serialize_json(value: SearchNotificationsResponse) -> dict:
    ...
    if "notifications" in value:
        ...
        out["Notifications"] = (
            capo_connect.types.notification_search_summary_list.serialize_json(
                value["notifications"]
            )
        )
...
def deserialize_json(data: dict) -> SearchNotificationsResponse:
    ...
    if "Notifications" in data:
        ...
        out["notifications"] = (
            capo_connect.types.notification_search_summary_list.deserialize_json(
                data["Notifications"]
            )
        )
