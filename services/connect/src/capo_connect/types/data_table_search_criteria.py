...
if TYPE_CHECKING:
    import capo_connect.types.data_table_search_condition_list
    import capo_connect.types.string_condition


class DataTableSearchCriteria(TypedDict, closed=True):
    or_conditions: NotRequired[
        "capo_connect.types.data_table_search_condition_list.DataTableSearchConditionList"
    ]
...
def serialize_json(value: DataTableSearchCriteria) -> dict:
    ...
    if "or_conditions" in value:
        ...
        out["OrConditions"] = (
            capo_connect.types.data_table_search_condition_list.serialize_json(
                value["or_conditions"]
            )
        )
...
def deserialize_json(data: dict) -> DataTableSearchCriteria:
    ...
    if "OrConditions" in data:
        ...
        out["or_conditions"] = (
            capo_connect.types.data_table_search_condition_list.deserialize_json(
                data["OrConditions"]
            )
        )
