...
class SearchDataTablesResponse(TypedDict, closed=True):
    data_tables: NotRequired["capo_connect.types.data_table_list.DataTableList"]
...
def serialize_json(value: SearchDataTablesResponse) -> dict:
    ...
    if "data_tables" in value:
        ...
        out["DataTables"] = capo_connect.types.data_table_list.serialize_json(
            value["data_tables"]
        )
...
def deserialize_json(data: dict) -> SearchDataTablesResponse:
    ...
    if "DataTables" in data:
        ...
        out["data_tables"] = capo_connect.types.data_table_list.deserialize_json(
            data["DataTables"]
        )
