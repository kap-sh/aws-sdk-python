...
def serialize_json(value: WorkspaceSearchConditionList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_connect.types.workspace_search_criteria.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> WorkspaceSearchConditionList:
    ...
    for item in data:
        out.append(capo_connect.types.workspace_search_criteria.deserialize_json(item))
    return out
