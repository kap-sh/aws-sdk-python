...
class HoursOfOperationOverrideConfig(TypedDict, closed=True):
    day: NotRequired["capo_connect.types.override_days.OverrideDays"]
    """<p>The day that the hours of operation override applies to.</p>"""
    start_time: NotRequired["capo_connect.types.override_time_slice.OverrideTimeSlice"]
    """<p>The start time when your contact center opens if overrides are applied.</p>"""
    end_time: NotRequired["capo_connect.types.override_time_slice.OverrideTimeSlice"]
...
def serialize_json(value: HoursOfOperationOverrideConfig) -> dict:
    ...
    if "day" in value:
        ...
        out["Day"] = capo_connect.types.override_days.serialize_json(value["day"])
    if "start_time" in value:
        import capo_connect.types.override_time_slice

        out["StartTime"] = capo_connect.types.override_time_slice.serialize_json(
            value["start_time"]
        )
    if "end_time" in value:
        import capo_connect.types.override_time_slice

        out["EndTime"] = capo_connect.types.override_time_slice.serialize_json(
            value["end_time"]
        )
    return out
