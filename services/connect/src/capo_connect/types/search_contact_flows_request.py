"""Generated from Smithy shape ``com.amazonaws.connect#SearchContactFlowsRequest``."""
...
if TYPE_CHECKING:
    ...
    import capo_connect.types.contact_flow_search_filter
    import capo_connect.types.instance_id
    import capo_connect.types.max_result100
    import capo_connect.types.next_token2500


class SearchContactFlowsRequest(TypedDict, closed=True):
    ...
    search_filter: NotRequired[
        "capo_connect.types.contact_flow_search_filter.ContactFlowSearchFilter"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: SearchContactFlowsRequest) -> dict:
    out: dict = {}
    ...
    if "search_filter" in value:
        ...
        out["SearchFilter"] = (
            capo_connect.types.contact_flow_search_filter.serialize_json(
                value["search_filter"]
            )
        )
    if "search_criteria" in value:
        ...
        out["SearchCriteria"] = (
            capo_connect.types.contact_flow_search_criteria.serialize_json(
...
def deserialize_json(data: dict) -> SearchContactFlowsRequest:
    out: SearchContactFlowsRequest = {}  # type: ignore[typeddict-item]
    if "InstanceId" in data:
        out["instance_id"] = data["InstanceId"]
    else:
        raise DeserializationError("SearchContactFlowsRequest.instance_id required")
    ...
    if "SearchFilter" in data:
        ...
        out["search_filter"] = (
            capo_connect.types.contact_flow_search_filter.deserialize_json(
                data["SearchFilter"]
            )
        )
    if "SearchCriteria" in data:
        ...
        out["search_criteria"] = (
            capo_connect.types.contact_flow_search_criteria.deserialize_json(
                data["SearchCriteria"]
            )
        )
    return out
