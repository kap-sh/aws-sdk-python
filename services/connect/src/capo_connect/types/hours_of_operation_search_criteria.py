...
class HoursOfOperationSearchCriteria(TypedDict, closed=True):
    or_conditions: NotRequired[
        "capo_connect.types.hours_of_operation_search_condition_list.HoursOfOperationSearchConditionList"
    ]
...
def serialize_json(value: HoursOfOperationSearchCriteria) -> dict:
    ...
    if "or_conditions" in value:
        ...
        out["OrConditions"] = (
            capo_connect.types.hours_of_operation_search_condition_list.serialize_json(
                value["or_conditions"]
            )
        )
...
def deserialize_json(data: dict) -> HoursOfOperationSearchCriteria:
    ...
    if "OrConditions" in data:
        ...
        out["or_conditions"] = (
            capo_connect.types.hours_of_operation_search_condition_list.deserialize_json(
                data["OrConditions"]
            )
        )
