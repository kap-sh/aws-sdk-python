...
if TYPE_CHECKING:
    import capo_connect.types.current_metric_sort_criteria

CurrentMetricSortCriteriaMaxOne: TypeAlias = list[
    "capo_connect.types.current_metric_sort_criteria.CurrentMetricSortCriteria"
]
...
def serialize_json(value: CurrentMetricSortCriteriaMaxOne) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_connect.types.current_metric_sort_criteria.serialize_json(item))
    return out
