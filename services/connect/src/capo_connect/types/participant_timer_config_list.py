...
if TYPE_CHECKING:
    import capo_connect.types.participant_timer_configuration

ParticipantTimerConfigList: TypeAlias = list[
    "capo_connect.types.participant_timer_configuration.ParticipantTimerConfiguration"
]
...
def serialize_json(value: ParticipantTimerConfigList) -> list:
    ...
    for item in value:
        out.append(
            capo_connect.types.participant_timer_configuration.serialize_json(item)
        )
    return out
