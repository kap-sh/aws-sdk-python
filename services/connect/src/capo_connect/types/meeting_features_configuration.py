...
if TYPE_CHECKING:
    import capo_connect.types.audio_features


class MeetingFeaturesConfiguration(TypedDict, closed=True):
    audio: NotRequired["capo_connect.types.audio_features.AudioFeatures"]
...
def serialize_json(value: MeetingFeaturesConfiguration) -> dict:
    ...
    if "audio" in value:
        ...
        out["Audio"] = capo_connect.types.audio_features.serialize_json(value["audio"])
    return out
...
def deserialize_json(data: dict) -> MeetingFeaturesConfiguration:
    ...
    if "Audio" in data:
        ...
        out["audio"] = capo_connect.types.audio_features.deserialize_json(data["Audio"])
    return out
