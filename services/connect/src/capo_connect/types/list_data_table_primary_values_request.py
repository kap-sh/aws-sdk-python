"""Generated from Smithy shape ``com.amazonaws.connect#ListDataTablePrimaryValuesRequest``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_connect.types.data_table_id
    import capo_connect.types.instance_id
    import capo_connect.types.max_result1000
    import capo_connect.types.next_token
    import capo_connect.types.primary_attribute_value_filters
    import capo_connect.types.record_ids


class ListDataTablePrimaryValuesRequest(TypedDict, closed=True):
    instance_id: "capo_connect.types.instance_id.InstanceId"
    """<p>The unique identifier for the Amazon Connect instance.</p>"""
    data_table_id: "capo_connect.types.data_table_id.DataTableId"
    """<p>The unique identifier for the data table whose primary values should be listed.</p>"""
    record_ids: NotRequired["capo_connect.types.record_ids.RecordIds"]
    """<p>Optional list of specific record IDs to retrieve. Used for CloudFormation to effectively describe records by ID. If NextToken is provided, this parameter is ignored.</p>"""
    primary_attribute_values: NotRequired[
        "capo_connect.types.primary_attribute_value_filters.PrimaryAttributeValueFilters"
    ]
    """<p>Optional filter to retrieve primary values matching specific criteria.</p>"""
    next_token: NotRequired["capo_connect.types.next_token.NextToken"]
    """<p>Specify the pagination token from a previous request to retrieve the next page of results.</p>"""
    max_results: NotRequired["capo_connect.types.max_result1000.MaxResult1000"]
    """<p>The maximum number of data table primary values to return in one page of results.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListDataTablePrimaryValuesRequest) -> dict:
    out: dict = {}
    if "record_ids" in value:
        import capo_connect.types.record_ids

        out["RecordIds"] = capo_connect.types.record_ids.serialize_json(
            value["record_ids"]
        )
    if "primary_attribute_values" in value:
        import capo_connect.types.primary_attribute_value_filters

        out["PrimaryAttributeValues"] = (
            capo_connect.types.primary_attribute_value_filters.serialize_json(
                value["primary_attribute_values"]
            )
        )
    return out


def deserialize_json(data: dict) -> ListDataTablePrimaryValuesRequest:
    out: ListDataTablePrimaryValuesRequest = {}  # type: ignore[typeddict-item]
    if "RecordIds" in data:
        import capo_connect.types.record_ids

        out["record_ids"] = capo_connect.types.record_ids.deserialize_json(
            data["RecordIds"]
        )
    if "PrimaryAttributeValues" in data:
        import capo_connect.types.primary_attribute_value_filters

        out["primary_attribute_values"] = (
            capo_connect.types.primary_attribute_value_filters.deserialize_json(
                data["PrimaryAttributeValues"]
            )
        )
    return out
