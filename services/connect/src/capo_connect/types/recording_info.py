...
if TYPE_CHECKING:
    import capo_connect.types.fragment_number
    import capo_connect.types.media_stream_type
    import capo_connect.types.participant_type
    import capo_connect.types.recording_deletion_reason
    import capo_connect.types.recording_location
    import capo_connect.types.recording_status
    import capo_connect.types.storage_type
    import capo_connect.types.timestamp
    import capo_connect.types.unprocessed_transcript_location


class RecordingInfo(TypedDict, closed=True):
    storage_type: NotRequired["capo_connect.types.storage_type.StorageType"]
    """<p>Where the recording/transcript is stored.</p>"""
    location: NotRequired["capo_connect.types.recording_location.RecordingLocation"]
    """<p>The location, in Amazon S3, for the recording/transcript.</p>"""
    media_stream_type: NotRequired[
        "capo_connect.types.media_stream_type.MediaStreamType"
    ]
    ...
    fragment_start_number: NotRequired[
        "capo_connect.types.fragment_number.FragmentNumber"
    ]
    ...
    start_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>When the conversation of the last leg of the recording started in UTC time.</p>"""
    stop_timestamp: NotRequired["capo_connect.types.timestamp.Timestamp"]
    """<p>When the conversation of the last leg of recording stopped in UTC time.</p>"""
    status: NotRequired["capo_connect.types.recording_status.RecordingStatus"]
    """<p>The status of the recording/transcript.</p>"""
    deletion_reason: NotRequired[
        "capo_connect.types.recording_deletion_reason.RecordingDeletionReason"
    ]
    ...
    unprocessed_transcript_location: NotRequired[
        "capo_connect.types.unprocessed_transcript_location.UnprocessedTranscriptLocation"
    ]
...
def serialize_json(value: RecordingInfo) -> dict:
    ...
    if "storage_type" in value:
        ...
        out["StorageType"] = capo_connect.types.storage_type.serialize_json(
            value["storage_type"]
        )
    ...
    if "media_stream_type" in value:
        ...
        out["MediaStreamType"] = capo_connect.types.media_stream_type.serialize_json(
    ...
    if "start_timestamp" in value:
        ...
        out["StartTimestamp"] = capo_connect.types.timestamp.serialize_json(
            value["start_timestamp"]
        )
    if "stop_timestamp" in value:
        import capo_connect.types.timestamp

        out["StopTimestamp"] = capo_connect.types.timestamp.serialize_json(
            value["stop_timestamp"]
        )
    if "status" in value:
        import capo_connect.types.recording_status

        out["Status"] = capo_connect.types.recording_status.serialize_json(
            value["status"]
        )
...
def deserialize_json(data: dict) -> RecordingInfo:
    ...
    if "MediaStreamType" in data:
        ...
        out["media_stream_type"] = (
            capo_connect.types.media_stream_type.deserialize_json(
