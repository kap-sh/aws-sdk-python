...
if TYPE_CHECKING:
    import capo_connect.types.persistent_connection_config

PersistentConnectionConfigs: TypeAlias = list[
    "capo_connect.types.persistent_connection_config.PersistentConnectionConfig"
]
...
def serialize_json(value: PersistentConnectionConfigs) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_connect.types.persistent_connection_config.serialize_json(item))
    return out
