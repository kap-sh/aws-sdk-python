...
class SearchQueuesRequest(TypedDict, closed=True):
    ...
    search_filter: NotRequired[
        "capo_connect.types.queue_search_filter.QueueSearchFilter"
    ]
...
def serialize_json(value: SearchQueuesRequest) -> dict:
    ...
    if "search_filter" in value:
        ...
        out["SearchFilter"] = capo_connect.types.queue_search_filter.serialize_json(
            value["search_filter"]
        )
    if "search_criteria" in value:
        import capo_connect.types.queue_search_criteria

        out["SearchCriteria"] = capo_connect.types.queue_search_criteria.serialize_json(
...
def deserialize_json(data: dict) -> SearchQueuesRequest:
    ...
    if "SearchCriteria" in data:
        ...
        out["search_criteria"] = (
            capo_connect.types.queue_search_criteria.deserialize_json(
                data["SearchCriteria"]
            )
        )
    return out
