...
if TYPE_CHECKING:
    import capo_connect.types.meeting_feature_status


class AudioFeatures(TypedDict, closed=True):
    echo_reduction: NotRequired[
        "capo_connect.types.meeting_feature_status.MeetingFeatureStatus"
    ]
...
def serialize_json(value: AudioFeatures) -> dict:
    ...
    if "echo_reduction" in value:
        ...
        out["EchoReduction"] = capo_connect.types.meeting_feature_status.serialize_json(
...
def deserialize_json(data: dict) -> AudioFeatures:
    ...
    if "EchoReduction" in data:
        ...
        out["echo_reduction"] = (
            capo_connect.types.meeting_feature_status.deserialize_json(
                data["EchoReduction"]
            )
        )
    return out
