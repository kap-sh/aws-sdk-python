"""Generated from Smithy shape ``com.amazonaws.connect#DisassociateSecurityProfiles``."""
...
import capo_connect.errors.conditional_operation_failed_exception
...
import capo_connect.types.disassociate_security_profiles_request
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConditionalOperationFailedException":
            raise capo_connect.errors.conditional_operation_failed_exception.ConditionalOperationFailedException.from_json(
                data
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/disassociate-security-profiles/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.disassociate_security_profiles_request.serialize_json(input_)
    ).encode()
...
async def async_disassociate_security_profiles(
