"""Generated from Smithy shape ``com.amazonaws.connect#ListContactFlows``."""
...
import capo_connect.types.contact_flow_types
import capo_connect.types.list_contact_flows_request
import capo_connect.types.list_contact_flows_response
...
def handle_response(
    ...
    out: capo_connect.types.list_contact_flows_response.ListContactFlowsResponse = (
        capo_connect.types.list_contact_flows_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact-flows-summary/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    if "contact_flow_types" in input_:
        params["contactFlowTypes"] = str(input_["contact_flow_types"])
...
async def async_list_contact_flows(
