"""Generated from Smithy shape ``com.amazonaws.connect#AssociateApprovedOrigin``."""
...
import capo_connect.types.associate_approved_origin_request
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/instance/{InstanceId}/approved-origin"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_approved_origin_request.serialize_json(input_)
    ).encode()
...
async def async_associate_approved_origin(
