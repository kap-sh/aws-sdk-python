"""Generated from Smithy shape ``com.amazonaws.connect#SearchDataTables``."""
...
import capo_connect.types.data_table_list
import capo_connect.types.data_table_search_criteria
import capo_connect.types.data_table_search_filter
import capo_connect.types.search_data_tables_request
import capo_connect.types.search_data_tables_response
...
) -> capo_connect.types.search_data_tables_response.SearchDataTablesResponse:
    out: capo_connect.types.search_data_tables_response.SearchDataTablesResponse = (
        capo_connect.types.search_data_tables_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-data-tables"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.search_data_tables_request.serialize_json(input_)
    ).encode()
...
async def async_search_data_tables(
