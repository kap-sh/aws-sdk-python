"""Generated from Smithy shape ``com.amazonaws.connect#TagContact``."""
...
import capo_connect.types.tag_contact_request
import capo_connect.types.tag_contact_response
...
def handle_response(
    ...
    out: capo_connect.types.tag_contact_response.TagContactResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/tags"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.tag_contact_request.serialize_json(input_)
    ).encode()
...
def tag_contact(
    ...
    input_: capo_connect.types.tag_contact_request.TagContactRequest,
) -> tuple[capo_connect.types.tag_contact_response.TagContactResponse, zapros.Response]:
