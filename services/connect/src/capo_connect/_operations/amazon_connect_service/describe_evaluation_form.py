"""Generated from Smithy shape ``com.amazonaws.connect#DescribeEvaluationForm``."""
...
def handle_response(
    ...
    out: capo_connect.types.describe_evaluation_form_response.DescribeEvaluationFormResponse = capo_connect.types.describe_evaluation_form_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/evaluation-forms/{InstanceId}/{EvaluationFormId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace(
        "{EvaluationFormId}", quote(str(input_["evaluation_form_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "evaluation_form_version" in input_:
        params["version"] = str(input_["evaluation_form_version"])
...
async def async_describe_evaluation_form(
