"""Generated from Smithy shape ``com.amazonaws.connect#AssociateSecurityKey``."""
...
import capo_connect.types.associate_security_key_request
import capo_connect.types.associate_security_key_response
...
def handle_response(
    ...
    out: capo_connect.types.associate_security_key_response.AssociateSecurityKeyResponse = capo_connect.types.associate_security_key_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/instance/{InstanceId}/security-key"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_security_key_request.serialize_json(input_)
    ).encode()
...
async def async_associate_security_key(
