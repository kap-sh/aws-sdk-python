"""Generated from Smithy shape ``com.amazonaws.connect#PutUserStatus``."""
...
import capo_connect.types.put_user_status_request
import capo_connect.types.put_user_status_response
...
def handle_response(
    ...
    out: capo_connect.types.put_user_status_response.PutUserStatusResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/users/{InstanceId}/{UserId}/status"
    ...
    body: bytes | None = json.dumps(
        capo_connect.types.put_user_status_request.serialize_json(input_)
    ).encode()
...
def put_user_status(
    ...
    capo_connect.types.put_user_status_response.PutUserStatusResponse, zapros.Response
]:
...
async def async_put_user_status(
