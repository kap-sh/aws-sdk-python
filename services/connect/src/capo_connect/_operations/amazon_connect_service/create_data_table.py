"""Generated from Smithy shape ``com.amazonaws.connect#CreateDataTable``."""
...
import capo_connect.types.create_data_table_request
import capo_connect.types.create_data_table_response
...
) -> capo_connect.types.create_data_table_response.CreateDataTableResponse:
    out: capo_connect.types.create_data_table_response.CreateDataTableResponse = (
        capo_connect.types.create_data_table_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/data-tables/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_data_table_request.serialize_json(input_)
    ).encode()
...
async def async_create_data_table(
