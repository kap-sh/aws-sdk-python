"""Generated from Smithy shape ``com.amazonaws.connect#StopContact``."""
...
import capo_connect.errors.contact_not_found_exception
import capo_connect.errors.internal_service_exception
import capo_connect.errors.invalid_active_region_exception
...
import capo_connect.types.stop_contact_request
import capo_connect.types.stop_contact_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ContactNotFoundException":
            raise capo_connect.errors.contact_not_found_exception.ContactNotFoundException.from_json(
                data
            )
        ...
        case "InvalidActiveRegionException":
            raise capo_connect.errors.invalid_active_region_exception.InvalidActiveRegionException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_connect.types.stop_contact_response.StopContactResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/stop"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.stop_contact_request.serialize_json(input_)
    ).encode()
...
def stop_contact(
    ...
    capo_connect.types.stop_contact_response.StopContactResponse, zapros.Response
]:
