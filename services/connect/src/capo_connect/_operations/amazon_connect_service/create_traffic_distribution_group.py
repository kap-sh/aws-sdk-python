"""Generated from Smithy shape ``com.amazonaws.connect#CreateTrafficDistributionGroup``."""
...
import capo_connect.errors.resource_not_ready_exception
import capo_connect.errors.service_quota_exceeded_exception
import capo_connect.errors.throttling_exception
import capo_connect.types.create_traffic_distribution_group_request
import capo_connect.types.create_traffic_distribution_group_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceConflictException":
            raise capo_connect.errors.resource_conflict_exception.ResourceConflictException.from_json(
                data
            )
        ...
        case "ResourceNotReadyException":
            raise capo_connect.errors.resource_not_ready_exception.ResourceNotReadyException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_connect.types.create_traffic_distribution_group_response.CreateTrafficDistributionGroupResponse = capo_connect.types.create_traffic_distribution_group_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/traffic-distribution-group"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_traffic_distribution_group_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_traffic_distribution_group(
