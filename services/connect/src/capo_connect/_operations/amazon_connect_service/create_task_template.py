"""Generated from Smithy shape ``com.amazonaws.connect#CreateTaskTemplate``."""
...
import capo_connect.errors.property_validation_exception
...
import capo_connect.types.create_task_template_request
import capo_connect.types.create_task_template_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PropertyValidationException":
            raise capo_connect.errors.property_validation_exception.PropertyValidationException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_connect.types.create_task_template_response.CreateTaskTemplateResponse = (
        capo_connect.types.create_task_template_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/instance/{InstanceId}/task/template"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_task_template_request.serialize_json(input_)
    ).encode()
...
async def async_create_task_template(
