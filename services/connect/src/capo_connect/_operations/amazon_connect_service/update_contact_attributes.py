"""Generated from Smithy shape ``com.amazonaws.connect#UpdateContactAttributes``."""
...
import capo_connect.types.update_contact_attributes_request
import capo_connect.types.update_contact_attributes_response
...
def handle_response(
    ...
    out: capo_connect.types.update_contact_attributes_response.UpdateContactAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/attributes"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.update_contact_attributes_request.serialize_json(input_)
    ).encode()
...
async def async_update_contact_attributes(
