"""Generated from Smithy shape ``com.amazonaws.connect#SendOutboundEmail``."""
...
def handle_response(
    ...
    out: capo_connect.types.send_outbound_email_response.SendOutboundEmailResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/instance/{InstanceId}/outbound-email"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.send_outbound_email_request.serialize_json(input_)
    ).encode()
...
async def async_send_outbound_email(
