"""Generated from Smithy shape ``com.amazonaws.connect#SearchUsers``."""
...
import capo_connect.types.search_users_request
import capo_connect.types.search_users_response
import capo_connect.types.user_search_criteria
import capo_connect.types.user_search_filter
...
def handle_response(
    ...
    out: capo_connect.types.search_users_response.SearchUsersResponse = (
        capo_connect.types.search_users_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-users"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.search_users_request.serialize_json(input_)
    ).encode()
...
def search_users(
    ...
    capo_connect.types.search_users_response.SearchUsersResponse, zapros.Response
]:
