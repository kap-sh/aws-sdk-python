"""Generated from Smithy shape ``com.amazonaws.connect#AssociateContactWithUser``."""
...
import capo_connect.types.associate_contact_with_user_request
import capo_connect.types.associate_contact_with_user_response
...
def handle_response(
    ...
    out: capo_connect.types.associate_contact_with_user_response.AssociateContactWithUserResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contacts/{InstanceId}/{ContactId}/associate-user"
    ...
    body: bytes | None = json.dumps(
        capo_connect.types.associate_contact_with_user_request.serialize_json(input_)
    ).encode()
...
async def async_associate_contact_with_user(
