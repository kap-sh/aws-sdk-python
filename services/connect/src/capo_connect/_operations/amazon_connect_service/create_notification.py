"""Generated from Smithy shape ``com.amazonaws.connect#CreateNotification``."""
...
def handle_response(
    ...
    out: capo_connect.types.create_notification_response.CreateNotificationResponse = (
        capo_connect.types.create_notification_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/notifications/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_notification_request.serialize_json(input_)
    ).encode()
