"""Generated from Smithy shape ``com.amazonaws.connect#CreateContactFlow``."""
...
import capo_connect.errors.invalid_contact_flow_exception
...
import capo_connect.types.create_contact_flow_request
import capo_connect.types.create_contact_flow_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidContactFlowException":
            raise capo_connect.errors.invalid_contact_flow_exception.InvalidContactFlowException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_connect.types.create_contact_flow_response.CreateContactFlowResponse = (
        capo_connect.types.create_contact_flow_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact-flows/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_contact_flow_request.serialize_json(input_)
    ).encode()
...
async def async_create_contact_flow(
