"""Generated from Smithy shape ``com.amazonaws.connect#CreateEmailAddress``."""
...
import capo_connect.types.create_email_address_request
...
def handle_response(
    ...
    out: capo_connect.types.create_email_address_response.CreateEmailAddressResponse = (
        capo_connect.types.create_email_address_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/email-addresses/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_email_address_request.serialize_json(input_)
    ).encode()
...
async def async_create_email_address(
