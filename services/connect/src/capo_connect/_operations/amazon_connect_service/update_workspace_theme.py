"""Generated from Smithy shape ``com.amazonaws.connect#UpdateWorkspaceTheme``."""
...
import capo_connect.types.update_workspace_theme_request
import capo_connect.types.update_workspace_theme_response
...
def handle_response(
    ...
    out: capo_connect.types.update_workspace_theme_response.UpdateWorkspaceThemeResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{InstanceId}/{WorkspaceId}/theme"
    ...
    body: bytes | None = json.dumps(
        capo_connect.types.update_workspace_theme_request.serialize_json(input_)
    ).encode()
...
async def async_update_workspace_theme(
