"""Generated from Smithy shape ``com.amazonaws.connect#StartTaskContact``."""
...
import capo_connect.types.start_task_contact_request
import capo_connect.types.start_task_contact_response
...
def handle_response(
    ...
    out: capo_connect.types.start_task_contact_response.StartTaskContactResponse = (
        capo_connect.types.start_task_contact_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/task"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.start_task_contact_request.serialize_json(input_)
    ).encode()
...
async def async_start_task_contact(
