"""Generated from Smithy shape ``com.amazonaws.connect#ReplicateInstance``."""
...
import capo_connect.types.replicate_instance_request
import capo_connect.types.replicate_instance_response
...
def handle_response(
    ...
    out: capo_connect.types.replicate_instance_response.ReplicateInstanceResponse = (
        capo_connect.types.replicate_instance_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/instance/{InstanceId}/replicate"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.replicate_instance_request.serialize_json(input_)
    ).encode()
...
async def async_replicate_instance(
