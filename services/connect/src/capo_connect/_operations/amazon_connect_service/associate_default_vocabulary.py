"""Generated from Smithy shape ``com.amazonaws.connect#AssociateDefaultVocabulary``."""
...
import capo_connect.types.associate_default_vocabulary_request
import capo_connect.types.associate_default_vocabulary_response
...
def handle_response(
    ...
    out: capo_connect.types.associate_default_vocabulary_response.AssociateDefaultVocabularyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/default-vocabulary/{InstanceId}/{LanguageCode}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    url = url.replace("{LanguageCode}", quote(str(input_["language_code"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_default_vocabulary_request.serialize_json(input_)
    ).encode()
...
async def async_associate_default_vocabulary(
