"""Generated from Smithy shape ``com.amazonaws.connect#SearchContactFlows``."""
...
import capo_connect.types.contact_flow_search_summary_list
import capo_connect.types.search_contact_flows_request
import capo_connect.types.search_contact_flows_response
...
def handle_response(
    ...
    out: capo_connect.types.search_contact_flows_response.SearchContactFlowsResponse = (
        capo_connect.types.search_contact_flows_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/search-contact-flows"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.search_contact_flows_request.serialize_json(input_)
    ).encode()
...
async def async_search_contact_flows(
