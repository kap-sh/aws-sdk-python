"""Generated from Smithy shape ``com.amazonaws.connect#AssociatePhoneNumberContactFlow``."""
...
import capo_connect.types.associate_phone_number_contact_flow_request
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/phone-number/{PhoneNumberId}/contact-flow"
    url = url.replace("{PhoneNumberId}", quote(str(input_["phone_number_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.associate_phone_number_contact_flow_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_associate_phone_number_contact_flow(
