"""Generated from Smithy shape ``com.amazonaws.connect#CreateHoursOfOperation``."""
...
import capo_connect.types.create_hours_of_operation_response
...
def handle_response(
    ...
    out: capo_connect.types.create_hours_of_operation_response.CreateHoursOfOperationResponse = capo_connect.types.create_hours_of_operation_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/hours-of-operations/{InstanceId}"
    url = url.replace("{InstanceId}", quote(str(input_["instance_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.create_hours_of_operation_request.serialize_json(input_)
    ).encode()
...
async def async_create_hours_of_operation(
