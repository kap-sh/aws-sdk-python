"""Generated from Smithy shape ``com.amazonaws.connect#UpdateContactFlowName``."""
...
import capo_connect.types.update_contact_flow_name_request
import capo_connect.types.update_contact_flow_name_response
...
def handle_response(
    ...
    out: capo_connect.types.update_contact_flow_name_response.UpdateContactFlowNameResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact-flows/{InstanceId}/{ContactFlowId}/name"
    ...
    body: bytes | None = json.dumps(
        capo_connect.types.update_contact_flow_name_request.serialize_json(input_)
    ).encode()
...
async def async_update_contact_flow_name(
