"""Generated from Smithy shape ``com.amazonaws.connect#StopContactStreaming``."""
...
import capo_connect.types.stop_contact_streaming_request
import capo_connect.types.stop_contact_streaming_response
...
def handle_response(
    ...
    out: capo_connect.types.stop_contact_streaming_response.StopContactStreamingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/contact/stop-streaming"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.stop_contact_streaming_request.serialize_json(input_)
    ).encode()
...
async def async_stop_contact_streaming(
