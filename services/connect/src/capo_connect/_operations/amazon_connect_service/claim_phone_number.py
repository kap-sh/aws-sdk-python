"""Generated from Smithy shape ``com.amazonaws.connect#ClaimPhoneNumber``."""
...
import capo_connect.types.claim_phone_number_request
import capo_connect.types.claim_phone_number_response
...
def handle_response(
    ...
    out: capo_connect.types.claim_phone_number_response.ClaimPhoneNumberResponse = (
        capo_connect.types.claim_phone_number_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/phone-number/claim"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_connect.types.claim_phone_number_request.serialize_json(input_)
    ).encode()
...
async def async_claim_phone_number(
