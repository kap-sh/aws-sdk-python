...
class ResourceSharePermissionDetail(TypedDict):
    ...
    permission: NotRequired["aws_sdk_ram.types.string.String"]
    ...
    status: NotRequired["aws_sdk_ram.types.permission_status.PermissionStatus"]
...
# --- restJson1 ser/de ---
def serialize_json(value: ResourceSharePermissionDetail) -> dict:
    out: dict = {}
    if "arn" in value:
        out["arn"] = value["arn"]
    if "version" in value:
    ...
    if "status" in value:
        ...
        out["status"] = aws_sdk_ram.types.permission_status.serialize_json(
            value["status"]
        )
    if "tags" in value:
        import aws_sdk_ram.types.tag_list
...
def deserialize_json(data: dict) -> ResourceSharePermissionDetail:
    out: ResourceSharePermissionDetail = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
    if "version" in data:
    ...
    if "status" in data:
        ...
        out["status"] = aws_sdk_ram.types.permission_status.deserialize_json(
            data["status"]
        )
    if "tags" in data:
        import aws_sdk_ram.types.tag_list
