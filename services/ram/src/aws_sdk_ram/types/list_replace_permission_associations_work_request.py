...
class ListReplacePermissionAssociationsWorkRequest(TypedDict):
...
def serialize_json(value: ListReplacePermissionAssociationsWorkRequest) -> dict:
    ...
    if "work_ids" in value:
        ...
        out["workIds"] = (
            aws_sdk_ram.types.replace_permission_associations_work_id_list.serialize_json(
                value["work_ids"]
            )
        )
    if "status" in value:
...
def deserialize_json(data: dict) -> ListReplacePermissionAssociationsWorkRequest:
    ...
    if "workIds" in data:
        ...
        out["work_ids"] = (
            aws_sdk_ram.types.replace_permission_associations_work_id_list.deserialize_json(
                data["workIds"]
            )
        )
    if "status" in data:
