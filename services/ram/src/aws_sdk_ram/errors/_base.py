...
class RAMError(Exception):
    """Base class for all RAM service errors."""


class ServiceError(RAMError):
...
class SerializationError(RAMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(RAMError):
...
class WaiterFailedError(RAMError):
...
class WaiterTimeoutError(RAMError):
