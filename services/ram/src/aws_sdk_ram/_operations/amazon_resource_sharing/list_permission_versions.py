"""Generated from Smithy shape ``com.amazonaws.ram#ListPermissionVersions``."""
...
def handle_response(
    ...
    out: aws_sdk_ram.types.list_permission_versions_response.ListPermissionVersionsResponse = aws_sdk_ram.types.list_permission_versions_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_ram.types.list_permission_versions_request.ListPermissionVersionsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/listpermissionversions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_ram.types.list_permission_versions_request

    body: bytes | None = json.dumps(
        aws_sdk_ram.types.list_permission_versions_request.serialize_json(input_)
    ).encode()
...
async def async_list_permission_versions(
