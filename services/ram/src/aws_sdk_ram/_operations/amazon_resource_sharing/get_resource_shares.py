"""Generated from Smithy shape ``com.amazonaws.ram#GetResourceShares``."""
...
import aws_sdk_ram.types.resource_share_list
...
def handle_response(
    ...
    out: aws_sdk_ram.types.get_resource_shares_response.GetResourceSharesResponse = (
        aws_sdk_ram.types.get_resource_shares_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_ram.types.get_resource_shares_request.GetResourceSharesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/getresourceshares"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_ram.types.get_resource_shares_request

    body: bytes | None = json.dumps(
        aws_sdk_ram.types.get_resource_shares_request.serialize_json(input_)
    ).encode()
...
async def async_get_resource_shares(
