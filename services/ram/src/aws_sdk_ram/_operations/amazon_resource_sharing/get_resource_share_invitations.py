"""Generated from Smithy shape ``com.amazonaws.ram#GetResourceShareInvitations``."""
...
def build_request(
    ...
    input_: aws_sdk_ram.types.get_resource_share_invitations_request.GetResourceShareInvitationsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/getresourceshareinvitations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_ram.types.get_resource_share_invitations_request.serialize_json(input_)
    ).encode()
...
async def async_get_resource_share_invitations(
