"""Generated from Smithy shape ``com.amazonaws.ram#ListSourceAssociations``."""
...
def handle_response(
    ...
    out: capo_ram.types.list_source_associations_response.ListSourceAssociationsResponse = capo_ram.types.list_source_associations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listsourceassociations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.list_source_associations_request.serialize_json(input_)
    ).encode()
...
async def async_list_source_associations(
