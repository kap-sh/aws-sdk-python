"""Generated from Smithy shape ``com.amazonaws.ram#ReplacePermissionAssociations``."""
...
def handle_response(
    ...
    out: capo_ram.types.replace_permission_associations_response.ReplacePermissionAssociationsResponse = capo_ram.types.replace_permission_associations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/replacepermissionassociations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.replace_permission_associations_request.serialize_json(input_)
    ).encode()
...
async def async_replace_permission_associations(
