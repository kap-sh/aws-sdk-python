"""Generated from Smithy shape ``com.amazonaws.ram#SetDefaultPermissionVersion``."""
...
def handle_response(
    ...
    out: capo_ram.types.set_default_permission_version_response.SetDefaultPermissionVersionResponse = capo_ram.types.set_default_permission_version_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/setdefaultpermissionversion"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.set_default_permission_version_request.serialize_json(input_)
    ).encode()
...
async def async_set_default_permission_version(
