"""Generated from Smithy shape ``com.amazonaws.ram#ListResourceTypes``."""
...
def handle_response(
    ...
    out: capo_ram.types.list_resource_types_response.ListResourceTypesResponse = (
        capo_ram.types.list_resource_types_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/listresourcetypes"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_ram.types.list_resource_types_request.serialize_json(input_)
    ).encode()
...
async def async_list_resource_types(
