"""Generated from Smithy shape ``com.amazonaws.wafv2#DeleteRuleGroupRequest``."""
...
def deserialize_aws_json_1_1(data: dict) -> DeleteRuleGroupRequest:
    ...
    if "Name" in data:
        ...
        raise DeserializationError("DeleteRuleGroupRequest.name required")
    if "Scope" in data:
        import capo_wafv2.types.scope

        out["scope"] = capo_wafv2.types.scope.deserialize_aws_json_1_1(data["Scope"])
    else:
        raise DeserializationError("DeleteRuleGroupRequest.scope required")
    if "Id" in data:
        out["id"] = data["Id"]
    else:
        raise DeserializationError("DeleteRuleGroupRequest.id required")
    if "LockToken" in data:
        out["lock_token"] = data["LockToken"]
    else:
        raise DeserializationError("DeleteRuleGroupRequest.lock_token required")
    return out
