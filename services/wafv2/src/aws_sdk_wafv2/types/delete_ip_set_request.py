"""Generated from Smithy shape ``com.amazonaws.wafv2#DeleteIPSetRequest``."""
...
def deserialize_aws_json_1_1(data: dict) -> DeleteIPSetRequest:
    ...
    if "Name" in data:
        ...
        raise DeserializationError("DeleteIPSetRequest.name required")
    if "Scope" in data:
        import aws_sdk_wafv2.types.scope

        out["scope"] = aws_sdk_wafv2.types.scope.deserialize_aws_json_1_1(data["Scope"])
    else:
        raise DeserializationError("DeleteIPSetRequest.scope required")
    if "Id" in data:
        out["id"] = data["Id"]
    else:
        raise DeserializationError("DeleteIPSetRequest.id required")
    if "LockToken" in data:
        out["lock_token"] = data["LockToken"]
    else:
        raise DeserializationError("DeleteIPSetRequest.lock_token required")
    return out
