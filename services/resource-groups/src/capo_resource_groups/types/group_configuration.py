...
class GroupConfiguration(TypedDict, closed=True):
    ...
    status: NotRequired[
        "capo_resource_groups.types.group_configuration_status.GroupConfigurationStatus"
    ]
    """<p>The current status of an attempt to update the group configuration.</p>"""
    failure_reason: NotRequired[
        "capo_resource_groups.types.group_configuration_failure_reason.GroupConfigurationFailureReason"
    ]
...
def serialize_json(value: GroupConfiguration) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            capo_resource_groups.types.group_configuration_status.serialize_json(
                value["status"]
            )
        )
...
def deserialize_json(data: dict) -> GroupConfiguration:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            capo_resource_groups.types.group_configuration_status.deserialize_json(
                data["Status"]
            )
        )
