...
class ListGroupsInput(TypedDict):
    filters: NotRequired[
...
def serialize_json(value: ListGroupsInput) -> dict:
    ...
    if "filters" in value:
        ...
        out["Filters"] = aws_sdk_resource_groups.types.group_filter_list.serialize_json(
            value["filters"]
        )
    return out
...
def deserialize_json(data: dict) -> ListGroupsInput:
    ...
    if "Filters" in data:
        ...
        out["filters"] = (
            aws_sdk_resource_groups.types.group_filter_list.deserialize_json(
                data["Filters"]
            )
        )
    return out
