...
from capo_app_mesh._auth._identity import (
    Credentials,
    Identity,
)
from capo_app_mesh._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-app-mesh[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-app-mesh[sso]"
        )
