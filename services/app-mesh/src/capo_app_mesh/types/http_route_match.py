...
if TYPE_CHECKING:
    ...
    import capo_app_mesh.types.http_scheme
...
class HttpRouteMatch(TypedDict, closed=True):
    ...
    scheme: NotRequired["capo_app_mesh.types.http_scheme.HttpScheme"]
    """<p>The client request scheme to match on. Specify only one. Applicable only for HTTP2 routes.</p>"""
    headers: NotRequired["capo_app_mesh.types.http_route_headers.HttpRouteHeaders"]
...
def serialize_json(value: HttpRouteMatch) -> dict:
    ...
    if "headers" in value:
        ...
        out["headers"] = capo_app_mesh.types.http_route_headers.serialize_json(
            value["headers"]
        )
    if "port" in value:
        out["port"] = value["port"]
