...
if TYPE_CHECKING:
    import capo_app_mesh.types.http_path_exact
    import capo_app_mesh.types.http_path_regex


class HttpPathMatch(TypedDict, closed=True):
    exact: NotRequired["capo_app_mesh.types.http_path_exact.HttpPathExact"]
    """<p>The exact path to match on.</p>"""
    regex: NotRequired["capo_app_mesh.types.http_path_regex.HttpPathRegex"]
