...
if TYPE_CHECKING:
    ...
    import capo_app_mesh.types.virtual_gateway_health_check_interval_millis
    import capo_app_mesh.types.virtual_gateway_health_check_threshold
    import capo_app_mesh.types.virtual_gateway_health_check_timeout_millis
    import capo_app_mesh.types.virtual_gateway_port_protocol


class VirtualGatewayHealthCheckPolicy(TypedDict, closed=True):
    timeout_millis: "capo_app_mesh.types.virtual_gateway_health_check_timeout_millis.VirtualGatewayHealthCheckTimeoutMillis"
    """<p>The amount of time to wait when receiving a response from the health check, in milliseconds.</p>"""
    interval_millis: "capo_app_mesh.types.virtual_gateway_health_check_interval_millis.VirtualGatewayHealthCheckIntervalMillis"
    ...
    healthy_threshold: "capo_app_mesh.types.virtual_gateway_health_check_threshold.VirtualGatewayHealthCheckThreshold"
    """<p>The number of consecutive successful health checks that must occur before declaring the listener healthy.</p>"""
    unhealthy_threshold: "capo_app_mesh.types.virtual_gateway_health_check_threshold.VirtualGatewayHealthCheckThreshold"
