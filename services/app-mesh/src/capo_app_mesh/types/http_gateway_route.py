...
class HttpGatewayRoute(TypedDict, closed=True):
    match: "capo_app_mesh.types.http_gateway_route_match.HttpGatewayRouteMatch"
    """<p>An object that represents the criteria for determining a request match.</p>"""
    action: "capo_app_mesh.types.http_gateway_route_action.HttpGatewayRouteAction"
...
def serialize_json(value: HttpGatewayRoute) -> dict:
    ...
    out["match"] = capo_app_mesh.types.http_gateway_route_match.serialize_json(
        value["match"]
    )
    import capo_app_mesh.types.http_gateway_route_action

    out["action"] = capo_app_mesh.types.http_gateway_route_action.serialize_json(
        value["action"]
    )
    return out
