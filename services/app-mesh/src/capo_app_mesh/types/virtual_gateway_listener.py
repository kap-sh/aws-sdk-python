...
class VirtualGatewayListener(TypedDict, closed=True):
    health_check: NotRequired[
        "capo_app_mesh.types.virtual_gateway_health_check_policy.VirtualGatewayHealthCheckPolicy"
    ]
    """<p>The health check information for the listener.</p>"""
    port_mapping: (
        "capo_app_mesh.types.virtual_gateway_port_mapping.VirtualGatewayPortMapping"
    )
    """<p>The port mapping information for the listener.</p>"""
    tls: NotRequired[
        "capo_app_mesh.types.virtual_gateway_listener_tls.VirtualGatewayListenerTls"
    ]
    ...
    connection_pool: NotRequired[
        "capo_app_mesh.types.virtual_gateway_connection_pool.VirtualGatewayConnectionPool"
    ]
...
def serialize_json(value: VirtualGatewayListener) -> dict:
    ...
    if "health_check" in value:
        ...
        out["healthCheck"] = (
            capo_app_mesh.types.virtual_gateway_health_check_policy.serialize_json(
                value["health_check"]
            )
        )
    import capo_app_mesh.types.virtual_gateway_port_mapping
    ...
    out["portMapping"] = (
        capo_app_mesh.types.virtual_gateway_port_mapping.serialize_json(
            value["port_mapping"]
        )
    )
    if "tls" in value:
        ...
        out["tls"] = capo_app_mesh.types.virtual_gateway_listener_tls.serialize_json(
            value["tls"]
        )
    if "connection_pool" in value:
        ...
        out["connectionPool"] = (
            capo_app_mesh.types.virtual_gateway_connection_pool.serialize_json(
...
def deserialize_json(data: dict) -> VirtualGatewayListener:
    ...
    if "healthCheck" in data:
        ...
        out["health_check"] = (
            capo_app_mesh.types.virtual_gateway_health_check_policy.deserialize_json(
                data["healthCheck"]
            )
        )
    if "portMapping" in data:
        ...
        out["port_mapping"] = (
            capo_app_mesh.types.virtual_gateway_port_mapping.deserialize_json(
                data["portMapping"]
            )
        )
    else:
    ...
    if "connectionPool" in data:
        ...
        out["connection_pool"] = (
            capo_app_mesh.types.virtual_gateway_connection_pool.deserialize_json(
                data["connectionPool"]
            )
        )
    return out
