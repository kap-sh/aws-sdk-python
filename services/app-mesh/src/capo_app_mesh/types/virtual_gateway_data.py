...
class VirtualGatewayData(TypedDict, closed=True):
    ...
    status: "capo_app_mesh.types.virtual_gateway_status.VirtualGatewayStatus"
...
def serialize_json(value: VirtualGatewayData) -> dict:
    ...
    out["status"] = capo_app_mesh.types.virtual_gateway_status.serialize_json(
        value["status"]
    )
    return out
