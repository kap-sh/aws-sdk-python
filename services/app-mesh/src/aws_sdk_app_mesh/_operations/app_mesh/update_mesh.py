"""Generated from Smithy shape ``com.amazonaws.appmesh#UpdateMesh``."""
...
def handle_response(response: zapros.Response, is_async: bool) -> aws_sdk_app_mesh.types.update_mesh_output.UpdateMeshOutput:
    import aws_sdk_app_mesh.types.mesh_data
    out: aws_sdk_app_mesh.types.update_mesh_output.UpdateMeshOutput = {"mesh": aws_sdk_app_mesh.types.mesh_data.deserialize_json(json.loads(response.read()))}  # type: ignore[typeddict-item]
    return out
...
def build_request(options: OperationOptions | AsyncOperationOptions, input: aws_sdk_app_mesh.types.update_mesh_input.UpdateMeshInput) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v20190125/meshes/{meshName}"
    url = url.replace("{meshName}", quote(str(input["mesh_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_app_mesh.types.update_mesh_input
    body: bytes | None = json.dumps(aws_sdk_app_mesh.types.update_mesh_input.serialize_json(input)).encode()
...
def update_mesh(options: OperationOptions, input: aws_sdk_app_mesh.types.update_mesh_input.UpdateMeshInput) -> tuple[aws_sdk_app_mesh.types.update_mesh_output.UpdateMeshOutput, zapros.Response]:
...
async def async_update_mesh(options: AsyncOperationOptions, input: aws_sdk_app_mesh.types.update_mesh_input.UpdateMeshInput) -> tuple[aws_sdk_app_mesh.types.update_mesh_output.UpdateMeshOutput, zapros.Response]:
