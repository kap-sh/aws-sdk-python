...
if TYPE_CHECKING:
    import aws_sdk_app_mesh.types.grpc_route_metadata_match_method
    import aws_sdk_app_mesh.types.header_name


class GrpcRouteMetadata(TypedDict):
    ...
    match: NotRequired[
        "aws_sdk_app_mesh.types.grpc_route_metadata_match_method.GrpcRouteMetadataMatchMethod"
    ]
...
def serialize_json(value: GrpcRouteMetadata) -> dict:
    ...
    if "match" in value:
        ...
        out["match"] = (
            aws_sdk_app_mesh.types.grpc_route_metadata_match_method.serialize_json(
                value["match"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GrpcRouteMetadata:
    ...
    if "match" in data:
        ...
        out["match"] = (
            aws_sdk_app_mesh.types.grpc_route_metadata_match_method.deserialize_json(
                data["match"]
            )
        )
    return out
