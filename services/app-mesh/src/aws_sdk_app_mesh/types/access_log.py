...
def serialize_json(value: AccessLog) -> dict:
    if "file" in value:
        import aws_sdk_app_mesh.types.file_access_log
        return {"file": aws_sdk_app_mesh.types.file_access_log.serialize_json(value["file"])}
    else:
...
def deserialize_json(data: dict) -> AccessLog:
    if "file" in data:
        import aws_sdk_app_mesh.types.file_access_log
        return {"file": aws_sdk_app_mesh.types.file_access_log.deserialize_json(data["file"])}
    else:
