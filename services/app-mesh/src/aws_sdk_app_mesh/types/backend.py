...
class _Backend_virtualService(TypedDict):
    virtualService: "aws_sdk_app_mesh.types.virtual_service_backend.VirtualServiceBackend"
...
def serialize_json(value: Backend) -> dict:
    if "virtualService" in value:
        import aws_sdk_app_mesh.types.virtual_service_backend
        return {"virtualService": aws_sdk_app_mesh.types.virtual_service_backend.serialize_json(value["virtualService"])}
    else:
...
def deserialize_json(data: dict) -> Backend:
    if "virtualService" in data:
        import aws_sdk_app_mesh.types.virtual_service_backend
        return {"virtualService": aws_sdk_app_mesh.types.virtual_service_backend.deserialize_json(data["virtualService"])}
    else:
