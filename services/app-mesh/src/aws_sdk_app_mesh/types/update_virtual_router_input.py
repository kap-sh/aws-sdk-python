...
if TYPE_CHECKING:
    ...
    import aws_sdk_app_mesh.types.virtual_router_spec


class UpdateVirtualRouterInput(TypedDict):
    ...
    spec: "aws_sdk_app_mesh.types.virtual_router_spec.VirtualRouterSpec"
...
def serialize_json(value: UpdateVirtualRouterInput) -> dict:
    ...
    out["spec"] = aws_sdk_app_mesh.types.virtual_router_spec.serialize_json(
        value["spec"]
    )
...
def deserialize_json(data: dict) -> UpdateVirtualRouterInput:
    ...
    if "spec" in data:
        ...
        out["spec"] = aws_sdk_app_mesh.types.virtual_router_spec.deserialize_json(
            data["spec"]
        )
    else:
