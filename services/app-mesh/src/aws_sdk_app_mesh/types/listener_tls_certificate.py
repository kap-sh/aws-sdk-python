...
if TYPE_CHECKING:
    import aws_sdk_app_mesh.types.listener_tls_acm_certificate
    import aws_sdk_app_mesh.types.listener_tls_file_certificate
    import aws_sdk_app_mesh.types.listener_tls_sds_certificate


class _ListenerTlsCertificate_acm(TypedDict):
    acm: "aws_sdk_app_mesh.types.listener_tls_acm_certificate.ListenerTlsAcmCertificate"


class _ListenerTlsCertificate_file(TypedDict):
    file: "aws_sdk_app_mesh.types.listener_tls_file_certificate.ListenerTlsFileCertificate"


class _ListenerTlsCertificate_sds(TypedDict):
...
def serialize_json(value: ListenerTlsCertificate) -> dict:
    if "acm" in value:
        ...
        return {
            "acm": aws_sdk_app_mesh.types.listener_tls_acm_certificate.serialize_json(
                value["acm"]
            )
        }
    elif "file" in value:
...
def deserialize_json(data: dict) -> ListenerTlsCertificate:
    if "acm" in data:
        ...
        return {
            "acm": aws_sdk_app_mesh.types.listener_tls_acm_certificate.deserialize_json(
                data["acm"]
            )
        }
    elif "file" in data:
