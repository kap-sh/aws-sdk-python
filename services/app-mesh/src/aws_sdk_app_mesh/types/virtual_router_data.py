...
class VirtualRouterData(TypedDict):
    ...
    status: "aws_sdk_app_mesh.types.virtual_router_status.VirtualRouterStatus"
...
def serialize_json(value: VirtualRouterData) -> dict:
    ...
    out["status"] = aws_sdk_app_mesh.types.virtual_router_status.serialize_json(
        value["status"]
    )
    return out
...
def deserialize_json(data: dict) -> VirtualRouterData:
    ...
    if "status" in data:
        ...
        out["status"] = aws_sdk_app_mesh.types.virtual_router_status.deserialize_json(
            data["status"]
        )
    else:
