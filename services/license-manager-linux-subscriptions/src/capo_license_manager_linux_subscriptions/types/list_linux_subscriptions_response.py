...
class ListLinuxSubscriptionsResponse(TypedDict, closed=True):
    subscriptions: NotRequired[
        "capo_license_manager_linux_subscriptions.types.subscription_list.SubscriptionList"
    ]
...
def serialize_json(value: ListLinuxSubscriptionsResponse) -> dict:
    ...
    if "subscriptions" in value:
        ...
        out["Subscriptions"] = (
            capo_license_manager_linux_subscriptions.types.subscription_list.serialize_json(
                value["subscriptions"]
            )
        )
...
def deserialize_json(data: dict) -> ListLinuxSubscriptionsResponse:
    ...
    if "Subscriptions" in data:
        ...
        out["subscriptions"] = (
            capo_license_manager_linux_subscriptions.types.subscription_list.deserialize_json(
                data["Subscriptions"]
            )
        )
