...
class ListLinuxSubscriptionInstancesRequest(TypedDict, closed=True):
    filters: NotRequired[
...
def serialize_json(value: ListLinuxSubscriptionInstancesRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["Filters"] = (
            capo_license_manager_linux_subscriptions.types.filter_list.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListLinuxSubscriptionInstancesRequest:
    ...
    if "Filters" in data:
        ...
        out["filters"] = (
            capo_license_manager_linux_subscriptions.types.filter_list.deserialize_json(
                data["Filters"]
            )
        )
