"""Generated from Smithy shape ``com.amazonaws.licensemanagerlinuxsubscriptions#GetRegisteredSubscriptionProvider``."""
...
import capo_license_manager_linux_subscriptions.errors.resource_not_found_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            raise capo_license_manager_linux_subscriptions.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_license_manager_linux_subscriptions.types.get_registered_subscription_provider_response.GetRegisteredSubscriptionProviderResponse = capo_license_manager_linux_subscriptions.types.get_registered_subscription_provider_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/subscription/GetRegisteredSubscriptionProvider"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_linux_subscriptions.types.get_registered_subscription_provider_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_get_registered_subscription_provider(
