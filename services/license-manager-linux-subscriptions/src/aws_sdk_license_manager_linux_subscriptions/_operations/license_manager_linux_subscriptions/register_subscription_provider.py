"""Generated from Smithy shape ``com.amazonaws.licensemanagerlinuxsubscriptions#RegisterSubscriptionProvider``."""
...
from aws_sdk_license_manager_linux_subscriptions._rule_engine._endpoint_rule_set import (
    EndpointParams,
    resolve,
)
...
from aws_sdk_license_manager_linux_subscriptions.errors import (
    UnknownServiceError,
)
...
def handle_response(
    ...
    out: aws_sdk_license_manager_linux_subscriptions.types.register_subscription_provider_response.RegisterSubscriptionProviderResponse = aws_sdk_license_manager_linux_subscriptions.types.register_subscription_provider_response.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_license_manager_linux_subscriptions._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_license_manager_linux_subscriptions._auth._sigv4.build_sigv4_auth_scheme(
                "license-manager-linux-subscriptions", options.region
            )
        )
        if sigv4_config is not None:
            return (
                aws_sdk_license_manager_linux_subscriptions._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_license_manager_linux_subscriptions.types.register_subscription_provider_request.RegisterSubscriptionProviderRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/subscription/RegisterSubscriptionProvider"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_license_manager_linux_subscriptions.types.register_subscription_provider_request

    body: bytes | None = json.dumps(
        aws_sdk_license_manager_linux_subscriptions.types.register_subscription_provider_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_register_subscription_provider(
