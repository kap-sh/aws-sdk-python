"""Generated from Smithy shape ``com.amazonaws.mq#UpdateUser``."""
...
import capo_mq.errors.conflict_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_mq.errors.conflict_exception.ConflictException.from_json(data)
...
def handle_response(
    ...
    out: capo_mq.types.update_user_response.UpdateUserResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/brokers/{BrokerId}/users/{Username}"
    url = url.replace("{BrokerId}", quote(str(input_["broker_id"]), safe=""))
    url = url.replace("{Username}", quote(str(input_["username"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_mq.types.update_user_request.serialize_json(input_)
    ).encode()
...
def update_user(
    ...
    input_: capo_mq.types.update_user_request.UpdateUserRequest,
) -> tuple[capo_mq.types.update_user_response.UpdateUserResponse, zapros.Response]:
