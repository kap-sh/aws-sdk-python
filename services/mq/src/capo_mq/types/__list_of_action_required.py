...
if TYPE_CHECKING:
    import capo_mq.types.action_required

__listOfActionRequired: TypeAlias = list["capo_mq.types.action_required.ActionRequired"]
...
def serialize_json(value: __listOfActionRequired) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_mq.types.action_required.serialize_json(item))
    return out
