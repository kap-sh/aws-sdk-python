...
class DescribeUserResponse(TypedDict, closed=True):
    ...
    pending: NotRequired["capo_mq.types.user_pending_changes.UserPendingChanges"]
    """<p>The status of the changes pending for the ActiveMQ user.</p>"""
    username: NotRequired["capo_mq.types.__string.__string"]
...
def serialize_json(value: DescribeUserResponse) -> dict:
    ...
    if "pending" in value:
        ...
        out["pending"] = capo_mq.types.user_pending_changes.serialize_json(
            value["pending"]
        )
