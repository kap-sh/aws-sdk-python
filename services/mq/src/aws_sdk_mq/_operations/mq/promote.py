...
def handle_response(
    ...
    out: aws_sdk_mq.types.promote_response.PromoteResponse = (
        aws_sdk_mq.types.promote_response.deserialize_json(json.loads(response.read()))
    )
    return out
...
def get_signer(
...
) -> aws_sdk_mq._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_mq._auth._sigv4.build_sigv4_auth_scheme("mq", options.region)
        )
        if sigv4_config is not None:
            return aws_sdk_mq._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input: aws_sdk_mq.types.promote_request.PromoteRequest,
) -> zapros.Request:
    ...
    url = url.replace("{BrokerId}", quote(str(input["broker_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_mq.types.promote_request

    body: bytes | None = json.dumps(
        aws_sdk_mq.types.promote_request.serialize_json(input)
    ).encode()
...
def promote(
    options: OperationOptions, input: aws_sdk_mq.types.promote_request.PromoteRequest
) -> tuple[aws_sdk_mq.types.promote_response.PromoteResponse, zapros.Response]:
