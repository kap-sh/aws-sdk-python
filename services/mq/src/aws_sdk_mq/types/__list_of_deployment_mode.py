...
__listOfDeploymentMode: TypeAlias = list[
    "aws_sdk_mq.types.deployment_mode.DeploymentMode"
]
...
def serialize_json(value: __listOfDeploymentMode) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_mq.types.deployment_mode.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfDeploymentMode:
    ...
    for item in data:
        out.append(aws_sdk_mq.types.deployment_mode.deserialize_json(item))
    return out
