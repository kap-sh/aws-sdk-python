...
if TYPE_CHECKING:
    ...
    import aws_sdk_mq.types.sanitization_warning_reason


class SanitizationWarning(TypedDict):
    attribute_name: NotRequired["aws_sdk_mq.types.__string.__string"]
    """<p>The name of the configuration attribute that has been sanitized.</p>"""
    element_name: NotRequired["aws_sdk_mq.types.__string.__string"]
    """<p>The name of the configuration element that has been sanitized.</p>"""
    reason: NotRequired[
        "aws_sdk_mq.types.sanitization_warning_reason.SanitizationWarningReason"
    ]
...
def serialize_json(value: SanitizationWarning) -> dict:
    ...
    if "reason" in value:
        ...
        out["reason"] = aws_sdk_mq.types.sanitization_warning_reason.serialize_json(
            value["reason"]
        )
    return out
...
def deserialize_json(data: dict) -> SanitizationWarning:
    ...
    if "reason" in data:
        ...
        out["reason"] = aws_sdk_mq.types.sanitization_warning_reason.deserialize_json(
            data["reason"]
        )
    return out
