...
from capo_rbin._auth._identity import (
    Credentials,
    Identity,
)
from capo_rbin._services._aws_config import _load_profile, active_profile, config_file

# The SSO, assume-role and web-identity providers call other AWS services, so
# they need the `sso` extra: `pip install capo-rbin[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-rbin[sso]"
        )
