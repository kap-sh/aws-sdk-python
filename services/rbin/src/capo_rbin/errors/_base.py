...
class rbinError(Exception):
    """Base class for all rbin service errors."""


class ServiceError(rbinError):
...
class SerializationError(rbinError):
    """Raised when a value cannot be serialized."""


class DeserializationError(rbinError):
...
class WaiterFailedError(rbinError):
...
class WaiterTimeoutError(rbinError):
