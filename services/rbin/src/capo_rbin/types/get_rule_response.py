"""Generated from Smithy shape ``com.amazonaws.rbin#GetRuleResponse``."""
...
class GetRuleResponse(TypedDict, closed=True):
    ...
    status: NotRequired["capo_rbin.types.rule_status.RuleStatus"]
    ...
    lock_end_time: NotRequired["capo_rbin.types.time_stamp.TimeStamp"]
...
# --- restJson1 ser/de ---
def serialize_json(value: GetRuleResponse) -> dict:
    out: dict = {}
    ...
    if "status" in value:
        ...
        out["Status"] = capo_rbin.types.rule_status.serialize_json(value["status"])
    ...
    if "lock_end_time" in value:
        ...
        out["LockEndTime"] = capo_rbin.types.time_stamp.serialize_json(
            value["lock_end_time"]
        )
...
def deserialize_json(data: dict) -> GetRuleResponse:
    ...
    if "LockEndTime" in data:
        ...
        out["lock_end_time"] = capo_rbin.types.time_stamp.deserialize_json(
            data["LockEndTime"]
        )
    if "RuleArn" in data:
