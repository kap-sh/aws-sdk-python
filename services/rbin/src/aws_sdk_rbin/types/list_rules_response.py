...
class ListRulesResponse(TypedDict):
    rules: NotRequired["aws_sdk_rbin.types.rule_summary_list.RuleSummaryList"]
...
def serialize_json(value: ListRulesResponse) -> dict:
    ...
    if "rules" in value:
        ...
        out["Rules"] = aws_sdk_rbin.types.rule_summary_list.serialize_json(
            value["rules"]
        )
...
def deserialize_json(data: dict) -> ListRulesResponse:
    ...
    if "Rules" in data:
        ...
        out["rules"] = aws_sdk_rbin.types.rule_summary_list.deserialize_json(
            data["Rules"]
        )
