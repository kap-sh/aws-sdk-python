...
def handle_response(
    ...
    out: aws_sdk_lex_model_building_service.types.get_migrations_response.GetMigrationsResponse = aws_sdk_lex_model_building_service.types.get_migrations_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_lex_model_building_service.types.get_migrations_request.GetMigrationsRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "sort_by_attribute" in input:
        params["sortByAttribute"] = str(input["sort_by_attribute"])
    if "sort_by_order" in input:
        params["sortByOrder"] = str(input["sort_by_order"])
    if "v1_bot_name_contains" in input:
        params["v1BotNameContains"] = str(input["v1_bot_name_contains"])
    if "migration_status_equals" in input:
        params["migrationStatusEquals"] = str(input["migration_status_equals"])
