...
def build_request(
    ...
    input_: aws_sdk_lex_model_building_service.types.get_migrations_request.GetMigrationsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "sort_by_attribute" in input_:
        params["sortByAttribute"] = str(input_["sort_by_attribute"])
    if "sort_by_order" in input_:
        params["sortByOrder"] = str(input_["sort_by_order"])
    if "v1_bot_name_contains" in input_:
        params["v1BotNameContains"] = str(input_["v1_bot_name_contains"])
    if "migration_status_equals" in input_:
        params["migrationStatusEquals"] = str(input_["migration_status_equals"])
