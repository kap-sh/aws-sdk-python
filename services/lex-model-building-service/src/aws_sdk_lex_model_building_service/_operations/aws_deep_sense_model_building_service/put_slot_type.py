"""Generated from Smithy shape ``com.amazonaws.lexmodelbuildingservice#PutSlotType``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_model_building_service.types.put_slot_type_response.PutSlotTypeResponse = aws_sdk_lex_model_building_service.types.put_slot_type_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lex_model_building_service.types.put_slot_type_request.PutSlotTypeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/slottypes/{name}/versions/$LATEST"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_lex_model_building_service.types.put_slot_type_request.serialize_json(
            input_
        )
    ).encode()
