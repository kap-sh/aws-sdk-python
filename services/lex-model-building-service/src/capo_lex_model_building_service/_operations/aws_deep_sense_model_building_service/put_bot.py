"""Generated from Smithy shape ``com.amazonaws.lexmodelbuildingservice#PutBot``."""
...
def handle_response(
    ...
    out: capo_lex_model_building_service.types.put_bot_response.PutBotResponse = (
        capo_lex_model_building_service.types.put_bot_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{name}/versions/$LATEST"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_model_building_service.types.put_bot_request.serialize_json(input_)
    ).encode()
