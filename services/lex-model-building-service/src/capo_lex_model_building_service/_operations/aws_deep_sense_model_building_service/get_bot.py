"""Generated from Smithy shape ``com.amazonaws.lexmodelbuildingservice#GetBot``."""
...
def handle_response(
    ...
    out: capo_lex_model_building_service.types.get_bot_response.GetBotResponse = (
        capo_lex_model_building_service.types.get_bot_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{name}/versions/{versionOrAlias}"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    url = url.replace(
        "{versionOrAlias}", quote(str(input_["version_or_alias"]), safe="")
    )
    params: dict[str, str] = {}
