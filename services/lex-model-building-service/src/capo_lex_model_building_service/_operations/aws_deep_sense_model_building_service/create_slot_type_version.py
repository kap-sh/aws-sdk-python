"""Generated from Smithy shape ``com.amazonaws.lexmodelbuildingservice#CreateSlotTypeVersion``."""
...
import capo_lex_model_building_service.errors.not_found_exception
import capo_lex_model_building_service.errors.precondition_failed_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PreconditionFailedException":
            raise capo_lex_model_building_service.errors.precondition_failed_exception.PreconditionFailedException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_lex_model_building_service.types.create_slot_type_version_response.CreateSlotTypeVersionResponse = capo_lex_model_building_service.types.create_slot_type_version_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/slottypes/{name}/versions"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_model_building_service.types.create_slot_type_version_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_slot_type_version(
