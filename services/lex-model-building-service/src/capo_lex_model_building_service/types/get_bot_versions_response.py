...
class GetBotVersionsResponse(TypedDict, closed=True):
    bots: NotRequired[
        "capo_lex_model_building_service.types.bot_metadata_list.BotMetadataList"
    ]
...
def serialize_json(value: GetBotVersionsResponse) -> dict:
    ...
    if "bots" in value:
        ...
        out["bots"] = (
            capo_lex_model_building_service.types.bot_metadata_list.serialize_json(
                value["bots"]
            )
        )
...
def deserialize_json(data: dict) -> GetBotVersionsResponse:
    ...
    if "bots" in data:
        ...
        out["bots"] = (
            capo_lex_model_building_service.types.bot_metadata_list.deserialize_json(
                data["bots"]
            )
        )
