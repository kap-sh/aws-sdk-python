...
class GetIntentVersionsResponse(TypedDict, closed=True):
    intents: NotRequired[
        "capo_lex_model_building_service.types.intent_metadata_list.IntentMetadataList"
    ]
...
def serialize_json(value: GetIntentVersionsResponse) -> dict:
    ...
    if "intents" in value:
        ...
        out["intents"] = (
            capo_lex_model_building_service.types.intent_metadata_list.serialize_json(
                value["intents"]
            )
        )
...
def deserialize_json(data: dict) -> GetIntentVersionsResponse:
    ...
    if "intents" in data:
        ...
        out["intents"] = (
            capo_lex_model_building_service.types.intent_metadata_list.deserialize_json(
                data["intents"]
            )
        )
