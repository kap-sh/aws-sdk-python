"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#CreateUserJourney``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.create_user_journey_response.CreateUserJourneyResponse = capo_resiliencehubv2.types.create_user_journey_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/create-user-journey"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.create_user_journey_request.serialize_json(input_)
    ).encode()
...
async def async_create_user_journey(
