"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#ListServiceEvents``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.list_service_events_response.ListServiceEventsResponse = capo_resiliencehubv2.types.list_service_events_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/list-service-events"
    params: dict[str, str] = {}
    if "service_arn" in input_:
        params["serviceArn"] = str(input_["service_arn"])
    if "event_types" in input_:
        params["eventTypes"] = str(input_["event_types"])
...
async def async_list_service_events(
