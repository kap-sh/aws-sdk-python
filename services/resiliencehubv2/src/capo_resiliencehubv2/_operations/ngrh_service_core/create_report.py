"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#CreateReport``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.create_report_response.CreateReportResponse = (
        capo_resiliencehubv2.types.create_report_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/create-report"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.create_report_request.serialize_json(input_)
    ).encode()
