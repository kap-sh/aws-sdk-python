"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#UpdateDependency``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.update_dependency_response.UpdateDependencyResponse = capo_resiliencehubv2.types.update_dependency_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/update-dependency"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.update_dependency_request.serialize_json(input_)
    ).encode()
...
async def async_update_dependency(
