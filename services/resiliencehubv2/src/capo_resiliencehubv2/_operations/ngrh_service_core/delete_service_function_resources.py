"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#DeleteServiceFunctionResources``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.delete_service_function_resources_response.DeleteServiceFunctionResourcesResponse = capo_resiliencehubv2.types.delete_service_function_resources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/delete-service-function-resources"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.delete_service_function_resources_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_service_function_resources(
