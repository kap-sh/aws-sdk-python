"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#DeletePolicy``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.delete_policy_response.DeletePolicyResponse = (
        capo_resiliencehubv2.types.delete_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/delete-policy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.delete_policy_request.serialize_json(input_)
    ).encode()
