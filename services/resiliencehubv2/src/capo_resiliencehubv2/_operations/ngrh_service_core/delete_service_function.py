"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#DeleteServiceFunction``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.delete_service_function_response.DeleteServiceFunctionResponse = capo_resiliencehubv2.types.delete_service_function_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/delete-function"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.delete_service_function_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_service_function(
