"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#DeleteAssertion``."""
...
def handle_response(
    ...
    out: capo_resiliencehubv2.types.delete_assertion_response.DeleteAssertionResponse = capo_resiliencehubv2.types.delete_assertion_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/delete-assertion"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehubv2.types.delete_assertion_request.serialize_json(input_)
    ).encode()
...
async def async_delete_assertion(
