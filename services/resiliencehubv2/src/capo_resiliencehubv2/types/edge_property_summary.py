...
class EdgePropertySummary(TypedDict, closed=True):
    topology_type: NotRequired["capo_resiliencehubv2.types.topology_type.TopologyType"]
...
def serialize_json(value: EdgePropertySummary) -> dict:
    ...
    if "topology_type" in value:
        ...
        out["topologyType"] = capo_resiliencehubv2.types.topology_type.serialize_json(
            value["topology_type"]
        )
    if "label" in value:
...
def deserialize_json(data: dict) -> EdgePropertySummary:
    ...
    if "topologyType" in data:
        ...
        out["topology_type"] = (
            capo_resiliencehubv2.types.topology_type.deserialize_json(
                data["topologyType"]
            )
        )
    if "label" in data:
