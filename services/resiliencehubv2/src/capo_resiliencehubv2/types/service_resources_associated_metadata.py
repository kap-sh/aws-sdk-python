...
class ServiceResourcesAssociatedMetadata(TypedDict, closed=True):
    resource_count: NotRequired["int"]
    """<p>The number of resources associated.</p>"""
    resource_types: NotRequired[
        "capo_resiliencehubv2.types.resource_type_list.ResourceTypeList"
    ]
...
def serialize_json(value: ServiceResourcesAssociatedMetadata) -> dict:
    ...
    if "resource_types" in value:
        ...
        out["resourceTypes"] = (
            capo_resiliencehubv2.types.resource_type_list.serialize_json(
...
def deserialize_json(data: dict) -> ServiceResourcesAssociatedMetadata:
    ...
    if "resourceTypes" in data:
        ...
        out["resource_types"] = (
            capo_resiliencehubv2.types.resource_type_list.deserialize_json(
                data["resourceTypes"]
            )
        )
    return out
