"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#StartFailureModeAssessment``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            import aws_sdk_resiliencehubv2.errors.conflict_exception

            raise aws_sdk_resiliencehubv2.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ThrottlingException":
            import aws_sdk_resiliencehubv2.errors.throttling_exception

            raise aws_sdk_resiliencehubv2.errors.throttling_exception.ThrottlingException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_resiliencehubv2.types.start_failure_mode_assessment_response.StartFailureModeAssessmentResponse = aws_sdk_resiliencehubv2.types.start_failure_mode_assessment_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehubv2.types.start_failure_mode_assessment_request.StartFailureModeAssessmentRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/start-failure-mode-assessment"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_resiliencehubv2.types.start_failure_mode_assessment_request

    body: bytes | None = json.dumps(
        aws_sdk_resiliencehubv2.types.start_failure_mode_assessment_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_failure_mode_assessment(
