...
def build_request(
    ...
    input: aws_sdk_resiliencehubv2.types.list_failure_mode_findings_request.ListFailureModeFindingsRequest,
) -> zapros.Request:
    ...
    if "severity" in input:
        params["severity"] = str(input["severity"])
    if "failure_category" in input:
        params["failureCategory"] = str(input["failure_category"])
    if "status" in input:
