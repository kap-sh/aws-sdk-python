"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#ListFailureModeFindings``."""
...
def handle_response(
    ...
    out: aws_sdk_resiliencehubv2.types.list_failure_mode_findings_response.ListFailureModeFindingsResponse = aws_sdk_resiliencehubv2.types.list_failure_mode_findings_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehubv2.types.list_failure_mode_findings_request.ListFailureModeFindingsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/list-failure-mode-findings"
    params: dict[str, str] = {}
    if "service_arn" in input_:
        params["serviceArn"] = str(input_["service_arn"])
    if "severity" in input_:
        params["severity"] = str(input_["severity"])
    if "failure_category" in input_:
        params["failureCategory"] = str(input_["failure_category"])
    if "status" in input_:
...
async def async_list_failure_mode_findings(
