"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#UpdateFailureModeFinding``."""
...
def handle_response(
    ...
    out: aws_sdk_resiliencehubv2.types.update_failure_mode_finding_response.UpdateFailureModeFindingResponse = aws_sdk_resiliencehubv2.types.update_failure_mode_finding_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehubv2.types.update_failure_mode_finding_request.UpdateFailureModeFindingRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/update-failure-mode-finding"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_resiliencehubv2.types.update_failure_mode_finding_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_failure_mode_finding(
