"""Generated from Smithy shape ``com.amazonaws.resiliencehubv2#ListDependencies``."""
...
def handle_response(
    ...
    out: aws_sdk_resiliencehubv2.types.list_dependencies_response.ListDependenciesResponse = aws_sdk_resiliencehubv2.types.list_dependencies_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehubv2.types.list_dependencies_request.ListDependenciesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/list-dependencies"
    params: dict[str, str] = {}
    if "service_arn" in input_:
        params["serviceArn"] = str(input_["service_arn"])
    if "query_range_start_time" in input_:
        params["queryRangeStartTime"] = str(input_["query_range_start_time"])
    if "query_range_end_time" in input_:
        params["queryRangeEndTime"] = str(input_["query_range_end_time"])
    if "query_range_granularity" in input_:
        params["queryRangeGranularity"] = str(input_["query_range_granularity"])
...
async def async_list_dependencies(
