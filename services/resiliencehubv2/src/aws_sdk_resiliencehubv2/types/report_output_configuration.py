...
if TYPE_CHECKING:
    import aws_sdk_resiliencehubv2.types.s3_report_output_configuration


class _ReportOutputConfiguration_s3(TypedDict):
    s3: "aws_sdk_resiliencehubv2.types.s3_report_output_configuration.S3ReportOutputConfiguration"
...
def serialize_json(value: ReportOutputConfiguration) -> dict:
    if "s3" in value:
        ...
        return {
            "s3": aws_sdk_resiliencehubv2.types.s3_report_output_configuration.serialize_json(
                value["s3"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> ReportOutputConfiguration:
    if "s3" in data:
        ...
        return {
            "s3": aws_sdk_resiliencehubv2.types.s3_report_output_configuration.deserialize_json(
                data["s3"]
            )
        }
    else:
