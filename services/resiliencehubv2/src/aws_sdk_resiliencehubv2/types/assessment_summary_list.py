...
AssessmentSummaryList: TypeAlias = list[
    "aws_sdk_resiliencehubv2.types.assessment_summary.AssessmentSummary"
]
...
def serialize_json(value: AssessmentSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_resiliencehubv2.types.assessment_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AssessmentSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_resiliencehubv2.types.assessment_summary.deserialize_json(item)
        )
    return out
