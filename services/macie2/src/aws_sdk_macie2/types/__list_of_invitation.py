...
__listOfInvitation: TypeAlias = list["aws_sdk_macie2.types.invitation.Invitation"]
...
def serialize_json(value: __listOfInvitation) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_macie2.types.invitation.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfInvitation:
    ...
    for item in data:
        out.append(aws_sdk_macie2.types.invitation.deserialize_json(item))
    return out
