"""Generated from Smithy shape ``com.amazonaws.macie2#ListJobsSortCriteria``."""
...
if TYPE_CHECKING:
    import aws_sdk_macie2.types.list_jobs_sort_attribute_name
    import aws_sdk_macie2.types.order_by


class ListJobsSortCriteria(TypedDict, closed=True):
    attribute_name: NotRequired[
        "aws_sdk_macie2.types.list_jobs_sort_attribute_name.ListJobsSortAttributeName"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: ListJobsSortCriteria) -> dict:
    out: dict = {}
    if "attribute_name" in value:
        import aws_sdk_macie2.types.list_jobs_sort_attribute_name

        out["attributeName"] = (
            aws_sdk_macie2.types.list_jobs_sort_attribute_name.serialize_json(
...
def deserialize_json(data: dict) -> ListJobsSortCriteria:
    ...
    if "attributeName" in data:
        ...
        out["attribute_name"] = (
            aws_sdk_macie2.types.list_jobs_sort_attribute_name.deserialize_json(
                data["attributeName"]
            )
        )
    if "orderBy" in data:
