...
class Page(TypedDict, closed=True):
    line_range: NotRequired["aws_sdk_macie2.types.range.Range"]
    """<p>Reserved for future use.</p>"""
    offset_range: NotRequired["aws_sdk_macie2.types.range.Range"]
    """<p>Reserved for future use.</p>"""
    page_number: NotRequired["aws_sdk_macie2.types.__long.__long"]
...
def serialize_json(value: Page) -> dict:
    ...
    if "line_range" in value:
        ...
        out["lineRange"] = aws_sdk_macie2.types.range.serialize_json(
            value["line_range"]
        )
    if "offset_range" in value:
        import aws_sdk_macie2.types.range

        out["offsetRange"] = aws_sdk_macie2.types.range.serialize_json(
            value["offset_range"]
        )
...
def deserialize_json(data: dict) -> Page:
    ...
    if "lineRange" in data:
        ...
        out["line_range"] = aws_sdk_macie2.types.range.deserialize_json(
            data["lineRange"]
        )
    if "offsetRange" in data:
        import aws_sdk_macie2.types.range

        out["offset_range"] = aws_sdk_macie2.types.range.deserialize_json(
            data["offsetRange"]
        )
