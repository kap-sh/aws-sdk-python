...
# --- restJson1 ser/de ---
def serialize_json(value: CreateAllowListResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["arn"] = value["arn"]
    if "id" in value:
...
def deserialize_json(data: dict) -> CreateAllowListResponse:
    out: CreateAllowListResponse = {}  # type: ignore[typeddict-item]
    if "arn" in data:
        out["arn"] = data["arn"]
    if "id" in data:
        out["id"] = data["id"]
