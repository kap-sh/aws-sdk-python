...
class ListClassificationJobsResponse(TypedDict):
    items: NotRequired["aws_sdk_macie2.types.__list_of_job_summary.__listOfJobSummary"]
...
def serialize_json(value: ListClassificationJobsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = aws_sdk_macie2.types.__list_of_job_summary.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListClassificationJobsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = aws_sdk_macie2.types.__list_of_job_summary.deserialize_json(
            data["items"]
        )
