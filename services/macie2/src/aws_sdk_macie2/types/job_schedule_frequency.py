...
if TYPE_CHECKING:
    ...
    import aws_sdk_macie2.types.monthly_schedule
...
class JobScheduleFrequency(TypedDict, closed=True):
    ...
    monthly_schedule: NotRequired[
        "aws_sdk_macie2.types.monthly_schedule.MonthlySchedule"
    ]
    """<p>Specifies a monthly recurrence pattern for running the job.</p>"""
    weekly_schedule: NotRequired["aws_sdk_macie2.types.weekly_schedule.WeeklySchedule"]
...
def serialize_json(value: JobScheduleFrequency) -> dict:
    ...
    if "daily_schedule" in value:
        ...
        out["dailySchedule"] = aws_sdk_macie2.types.daily_schedule.serialize_json(
            value["daily_schedule"]
        )
    if "monthly_schedule" in value:
        import aws_sdk_macie2.types.monthly_schedule

        out["monthlySchedule"] = aws_sdk_macie2.types.monthly_schedule.serialize_json(
            value["monthly_schedule"]
        )
    if "weekly_schedule" in value:
        import aws_sdk_macie2.types.weekly_schedule

        out["weeklySchedule"] = aws_sdk_macie2.types.weekly_schedule.serialize_json(
...
def deserialize_json(data: dict) -> JobScheduleFrequency:
    ...
    if "dailySchedule" in data:
        ...
        out["daily_schedule"] = aws_sdk_macie2.types.daily_schedule.deserialize_json(
            data["dailySchedule"]
        )
    if "monthlySchedule" in data:
        import aws_sdk_macie2.types.monthly_schedule

        out["monthly_schedule"] = (
            aws_sdk_macie2.types.monthly_schedule.deserialize_json(
    ...
    if "weeklySchedule" in data:
        ...
        out["weekly_schedule"] = aws_sdk_macie2.types.weekly_schedule.deserialize_json(
            data["weeklySchedule"]
        )
    return out
