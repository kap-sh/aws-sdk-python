...
class ListManagedDataIdentifiersResponse(TypedDict):
    items: NotRequired[
        "aws_sdk_macie2.types.__list_of_managed_data_identifier_summary.__listOfManagedDataIdentifierSummary"
    ]
...
def serialize_json(value: ListManagedDataIdentifiersResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            aws_sdk_macie2.types.__list_of_managed_data_identifier_summary.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListManagedDataIdentifiersResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            aws_sdk_macie2.types.__list_of_managed_data_identifier_summary.deserialize_json(
                data["items"]
            )
        )
