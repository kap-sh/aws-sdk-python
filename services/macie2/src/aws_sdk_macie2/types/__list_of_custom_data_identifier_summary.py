"""Generated from Smithy shape ``com.amazonaws.macie2#__listOfCustomDataIdentifierSummary``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_macie2.types.custom_data_identifier_summary

__listOfCustomDataIdentifierSummary: TypeAlias = list[
    "aws_sdk_macie2.types.custom_data_identifier_summary.CustomDataIdentifierSummary"
]


# --- restJson1 ser/de ---
def serialize_json(value: __listOfCustomDataIdentifierSummary) -> list:
    import aws_sdk_macie2.types.custom_data_identifier_summary

    out: list = []
    for item in value:
        out.append(
            aws_sdk_macie2.types.custom_data_identifier_summary.serialize_json(item)
        )
    return out


def deserialize_json(data: list) -> __listOfCustomDataIdentifierSummary:
    import aws_sdk_macie2.types.custom_data_identifier_summary

    out: __listOfCustomDataIdentifierSummary = []
    for item in data:
        out.append(
            aws_sdk_macie2.types.custom_data_identifier_summary.deserialize_json(item)
        )
    return out
