"""Generated from Smithy shape ``com.amazonaws.macie2#CreateAllowList``."""
...
def handle_response(
    ...
    out: aws_sdk_macie2.types.create_allow_list_response.CreateAllowListResponse = (
        aws_sdk_macie2.types.create_allow_list_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_macie2.types.create_allow_list_request.CreateAllowListRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/allow-lists"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_macie2.types.create_allow_list_request.serialize_json(input_)
    ).encode()
...
async def async_create_allow_list(
