"""Generated from Smithy shape ``com.amazonaws.macie2#PutFindingsPublicationConfiguration``."""
...
def handle_response(
    ...
    out: aws_sdk_macie2.types.put_findings_publication_configuration_response.PutFindingsPublicationConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_macie2.types.put_findings_publication_configuration_request.PutFindingsPublicationConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/findings-publication-configuration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_macie2.types.put_findings_publication_configuration_request

    body: bytes | None = json.dumps(
        aws_sdk_macie2.types.put_findings_publication_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_findings_publication_configuration(
