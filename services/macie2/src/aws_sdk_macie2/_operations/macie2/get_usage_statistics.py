"""Generated from Smithy shape ``com.amazonaws.macie2#GetUsageStatistics``."""
...
def handle_response(
    ...
    out: aws_sdk_macie2.types.get_usage_statistics_response.GetUsageStatisticsResponse = aws_sdk_macie2.types.get_usage_statistics_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_macie2.types.get_usage_statistics_request.GetUsageStatisticsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/usage/statistics"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_macie2.types.get_usage_statistics_request

    body: bytes | None = json.dumps(
        aws_sdk_macie2.types.get_usage_statistics_request.serialize_json(input_)
    ).encode()
...
async def async_get_usage_statistics(
