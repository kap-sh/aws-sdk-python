"""Generated from Smithy shape ``com.amazonaws.macie2#BatchUpdateAutomatedDiscoveryAccounts``."""
...
import aws_sdk_macie2.types.__list_of_automated_discovery_account_update_error
...
def handle_response(
    ...
    out: aws_sdk_macie2.types.batch_update_automated_discovery_accounts_response.BatchUpdateAutomatedDiscoveryAccountsResponse = aws_sdk_macie2.types.batch_update_automated_discovery_accounts_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_macie2.types.batch_update_automated_discovery_accounts_request.BatchUpdateAutomatedDiscoveryAccountsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/automated-discovery/accounts"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_macie2.types.batch_update_automated_discovery_accounts_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_update_automated_discovery_accounts(
