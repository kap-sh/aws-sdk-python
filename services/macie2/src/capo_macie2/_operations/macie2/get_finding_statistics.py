"""Generated from Smithy shape ``com.amazonaws.macie2#GetFindingStatistics``."""
...
def handle_response(
    ...
    out: capo_macie2.types.get_finding_statistics_response.GetFindingStatisticsResponse = capo_macie2.types.get_finding_statistics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findings/statistics"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.get_finding_statistics_request.serialize_json(input_)
    ).encode()
...
async def async_get_finding_statistics(
