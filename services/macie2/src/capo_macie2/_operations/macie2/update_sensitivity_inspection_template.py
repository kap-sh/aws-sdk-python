"""Generated from Smithy shape ``com.amazonaws.macie2#UpdateSensitivityInspectionTemplate``."""
...
def handle_response(
    ...
    out: capo_macie2.types.update_sensitivity_inspection_template_response.UpdateSensitivityInspectionTemplateResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/templates/sensitivity-inspections/{id}"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.update_sensitivity_inspection_template_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_sensitivity_inspection_template(
