"""Generated from Smithy shape ``com.amazonaws.macie2#UpdateResourceProfileDetections``."""
...
def handle_response(
    ...
    out: capo_macie2.types.update_resource_profile_detections_response.UpdateResourceProfileDetectionsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/resource-profiles/detections"
    params: dict[str, str] = {}
    if "resource_arn" in input_:
        params["resourceArn"] = str(input_["resource_arn"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.update_resource_profile_detections_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_resource_profile_detections(
