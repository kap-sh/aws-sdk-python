"""Generated from Smithy shape ``com.amazonaws.macie2#CreateSampleFindings``."""
...
def handle_response(
    ...
    out: capo_macie2.types.create_sample_findings_response.CreateSampleFindingsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findings/sample"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.create_sample_findings_request.serialize_json(input_)
    ).encode()
...
async def async_create_sample_findings(
