"""Generated from Smithy shape ``com.amazonaws.macie2#DeclineInvitations``."""
...
def handle_response(
    ...
    out: capo_macie2.types.decline_invitations_response.DeclineInvitationsResponse = (
        capo_macie2.types.decline_invitations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/invitations/decline"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.decline_invitations_request.serialize_json(input_)
    ).encode()
...
async def async_decline_invitations(
