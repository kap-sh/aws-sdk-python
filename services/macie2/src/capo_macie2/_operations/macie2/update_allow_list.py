"""Generated from Smithy shape ``com.amazonaws.macie2#UpdateAllowList``."""
...
def handle_response(
    ...
    out: capo_macie2.types.update_allow_list_response.UpdateAllowListResponse = (
        capo_macie2.types.update_allow_list_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/allow-lists/{id}"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.update_allow_list_request.serialize_json(input_)
    ).encode()
...
async def async_update_allow_list(
