"""Generated from Smithy shape ``com.amazonaws.macie2#GetFindings``."""
...
def handle_response(
    ...
    out: capo_macie2.types.get_findings_response.GetFindingsResponse = (
        capo_macie2.types.get_findings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findings/describe"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.get_findings_request.serialize_json(input_)
    ).encode()
...
def get_findings(
    ...
    capo_macie2.types.get_findings_response.GetFindingsResponse, zapros.Response
]:
