"""Generated from Smithy shape ``com.amazonaws.macie2#EnableMacie``."""
...
def handle_response(
    ...
    out: capo_macie2.types.enable_macie_response.EnableMacieResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/macie"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.enable_macie_request.serialize_json(input_)
    ).encode()
...
def enable_macie(
    ...
    capo_macie2.types.enable_macie_response.EnableMacieResponse, zapros.Response
]:
