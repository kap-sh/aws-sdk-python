"""Generated from Smithy shape ``com.amazonaws.macie2#UpdateClassificationScope``."""
...
def handle_response(
    ...
    out: capo_macie2.types.update_classification_scope_response.UpdateClassificationScopeResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/classification-scopes/{id}"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.update_classification_scope_request.serialize_json(input_)
    ).encode()
...
async def async_update_classification_scope(
