"""Generated from Smithy shape ``com.amazonaws.macie2#GetBucketStatistics``."""
...
def handle_response(
    ...
    out: capo_macie2.types.get_bucket_statistics_response.GetBucketStatisticsResponse = capo_macie2.types.get_bucket_statistics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/datasources/s3/statistics"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.get_bucket_statistics_request.serialize_json(input_)
    ).encode()
...
async def async_get_bucket_statistics(
