"""Generated from Smithy shape ``com.amazonaws.macie2#BatchGetCustomDataIdentifiers``."""
...
def handle_response(
    ...
    out: capo_macie2.types.batch_get_custom_data_identifiers_response.BatchGetCustomDataIdentifiersResponse = capo_macie2.types.batch_get_custom_data_identifiers_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/custom-data-identifiers/get"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.batch_get_custom_data_identifiers_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_get_custom_data_identifiers(
