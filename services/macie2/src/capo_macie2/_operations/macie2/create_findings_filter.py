"""Generated from Smithy shape ``com.amazonaws.macie2#CreateFindingsFilter``."""
...
def handle_response(
    ...
    out: capo_macie2.types.create_findings_filter_response.CreateFindingsFilterResponse = capo_macie2.types.create_findings_filter_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/findingsfilters"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.create_findings_filter_request.serialize_json(input_)
    ).encode()
...
async def async_create_findings_filter(
