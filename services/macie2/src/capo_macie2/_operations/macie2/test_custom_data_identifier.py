"""Generated from Smithy shape ``com.amazonaws.macie2#TestCustomDataIdentifier``."""
...
def handle_response(
    ...
    out: capo_macie2.types.test_custom_data_identifier_response.TestCustomDataIdentifierResponse = capo_macie2.types.test_custom_data_identifier_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/custom-data-identifiers/test"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.test_custom_data_identifier_request.serialize_json(input_)
    ).encode()
...
async def async_test_custom_data_identifier(
