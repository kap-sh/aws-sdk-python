"""Generated from Smithy shape ``com.amazonaws.macie2#UpdateMemberSession``."""
...
def handle_response(
    ...
    out: capo_macie2.types.update_member_session_response.UpdateMemberSessionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/macie/members/{id}"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.update_member_session_request.serialize_json(input_)
    ).encode()
...
async def async_update_member_session(
