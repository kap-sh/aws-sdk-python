"""Generated from Smithy shape ``com.amazonaws.macie2#UpdateRevealConfiguration``."""
...
def handle_response(
    ...
    out: capo_macie2.types.update_reveal_configuration_response.UpdateRevealConfigurationResponse = capo_macie2.types.update_reveal_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/reveal-configuration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.update_reveal_configuration_request.serialize_json(input_)
    ).encode()
...
async def async_update_reveal_configuration(
