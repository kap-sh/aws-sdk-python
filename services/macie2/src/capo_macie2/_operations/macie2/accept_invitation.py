"""Generated from Smithy shape ``com.amazonaws.macie2#AcceptInvitation``."""
...
def handle_response(
    ...
    out: capo_macie2.types.accept_invitation_response.AcceptInvitationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/invitations/accept"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_macie2.types.accept_invitation_request.serialize_json(input_)
    ).encode()
...
async def async_accept_invitation(
