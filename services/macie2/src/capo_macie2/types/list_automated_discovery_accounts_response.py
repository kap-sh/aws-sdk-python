...
class ListAutomatedDiscoveryAccountsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_macie2.types.__list_of_automated_discovery_account.__listOfAutomatedDiscoveryAccount"
    ]
...
def serialize_json(value: ListAutomatedDiscoveryAccountsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_macie2.types.__list_of_automated_discovery_account.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListAutomatedDiscoveryAccountsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            capo_macie2.types.__list_of_automated_discovery_account.deserialize_json(
                data["items"]
            )
        )
