...
def serialize_json(value: BatchUpdateAutomatedDiscoveryAccountsRequest) -> dict:
    ...
    if "accounts" in value:
        ...
        out["accounts"] = (
            capo_macie2.types.__list_of_automated_discovery_account_update.serialize_json(
                value["accounts"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> BatchUpdateAutomatedDiscoveryAccountsRequest:
    ...
    if "accounts" in data:
        ...
        out["accounts"] = (
            capo_macie2.types.__list_of_automated_discovery_account_update.deserialize_json(
                data["accounts"]
            )
        )
    return out
