...
if TYPE_CHECKING:
    import capo_macie2.types.tag_criterion_pair_for_job

__listOfTagCriterionPairForJob: TypeAlias = list[
    "capo_macie2.types.tag_criterion_pair_for_job.TagCriterionPairForJob"
]
...
def serialize_json(value: __listOfTagCriterionPairForJob) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_macie2.types.tag_criterion_pair_for_job.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> __listOfTagCriterionPairForJob:
    ...
    for item in data:
        out.append(capo_macie2.types.tag_criterion_pair_for_job.deserialize_json(item))
    return out
