...
class ReplicationDetails(TypedDict, closed=True):
    replicated: NotRequired["capo_macie2.types.__boolean.__boolean"]
    """<p>Specifies whether the bucket is configured to replicate one or more objects to any destination.</p>"""
    replicated_externally: NotRequired["capo_macie2.types.__boolean.__boolean"]
...
def serialize_json(value: ReplicationDetails) -> dict:
    ...
    if "replication_accounts" in value:
        ...
        out["replicationAccounts"] = capo_macie2.types.__list_of__string.serialize_json(
