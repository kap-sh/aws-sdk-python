...
class SessionContext(TypedDict, closed=True):
    attributes: NotRequired[
        "capo_macie2.types.session_context_attributes.SessionContextAttributes"
    ]
    ...
    session_issuer: NotRequired["capo_macie2.types.session_issuer.SessionIssuer"]
...
def serialize_json(value: SessionContext) -> dict:
    ...
    if "attributes" in value:
        ...
        out["attributes"] = capo_macie2.types.session_context_attributes.serialize_json(
            value["attributes"]
        )
    if "session_issuer" in value:
        import capo_macie2.types.session_issuer

        out["sessionIssuer"] = capo_macie2.types.session_issuer.serialize_json(
...
def deserialize_json(data: dict) -> SessionContext:
    ...
    if "attributes" in data:
        ...
        out["attributes"] = (
            capo_macie2.types.session_context_attributes.deserialize_json(
                data["attributes"]
            )
        )
