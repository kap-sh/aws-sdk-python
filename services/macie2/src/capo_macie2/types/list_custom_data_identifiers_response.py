...
class ListCustomDataIdentifiersResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_macie2.types.__list_of_custom_data_identifier_summary.__listOfCustomDataIdentifierSummary"
    ]
...
def serialize_json(value: ListCustomDataIdentifiersResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_macie2.types.__list_of_custom_data_identifier_summary.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListCustomDataIdentifiersResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            capo_macie2.types.__list_of_custom_data_identifier_summary.deserialize_json(
                data["items"]
            )
        )
