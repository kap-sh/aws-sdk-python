...
class TagCriterionForJob(TypedDict, closed=True):
    ...
    tag_values: NotRequired[
        "capo_macie2.types.__list_of_tag_criterion_pair_for_job.__listOfTagCriterionPairForJob"
    ]
...
def serialize_json(value: TagCriterionForJob) -> dict:
    ...
    if "tag_values" in value:
        ...
        out["tagValues"] = (
            capo_macie2.types.__list_of_tag_criterion_pair_for_job.serialize_json(
                value["tag_values"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> TagCriterionForJob:
    ...
    if "tagValues" in data:
        ...
        out["tag_values"] = (
            capo_macie2.types.__list_of_tag_criterion_pair_for_job.deserialize_json(
                data["tagValues"]
            )
        )
    return out
