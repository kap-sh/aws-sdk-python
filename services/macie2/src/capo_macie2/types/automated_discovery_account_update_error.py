...
class AutomatedDiscoveryAccountUpdateError(TypedDict, closed=True):
    ...
    error_code: NotRequired[
        "capo_macie2.types.automated_discovery_account_update_error_code.AutomatedDiscoveryAccountUpdateErrorCode"
    ]
...
def serialize_json(value: AutomatedDiscoveryAccountUpdateError) -> dict:
    ...
    if "error_code" in value:
        ...
        out["errorCode"] = (
            capo_macie2.types.automated_discovery_account_update_error_code.serialize_json(
                value["error_code"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AutomatedDiscoveryAccountUpdateError:
    ...
    if "errorCode" in data:
        ...
        out["error_code"] = (
            capo_macie2.types.automated_discovery_account_update_error_code.deserialize_json(
                data["errorCode"]
            )
        )
    return out
