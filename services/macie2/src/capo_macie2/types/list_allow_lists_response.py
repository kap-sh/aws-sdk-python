...
if TYPE_CHECKING:
    import capo_macie2.types.__list_of_allow_list_summary
    import capo_macie2.types.__string


class ListAllowListsResponse(TypedDict, closed=True):
    allow_lists: NotRequired[
        "capo_macie2.types.__list_of_allow_list_summary.__listOfAllowListSummary"
    ]
...
def serialize_json(value: ListAllowListsResponse) -> dict:
    ...
    if "allow_lists" in value:
        ...
        out["allowLists"] = (
            capo_macie2.types.__list_of_allow_list_summary.serialize_json(
                value["allow_lists"]
            )
        )
...
def deserialize_json(data: dict) -> ListAllowListsResponse:
    ...
    if "allowLists" in data:
        ...
        out["allow_lists"] = (
            capo_macie2.types.__list_of_allow_list_summary.deserialize_json(
                data["allowLists"]
            )
        )
