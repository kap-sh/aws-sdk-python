...
class GetSensitiveDataOccurrencesResponse(TypedDict, closed=True):
    error: NotRequired["capo_macie2.types.__string.__string"]
    ...
    sensitive_data_occurrences: NotRequired[
        "capo_macie2.types.sensitive_data_occurrences.SensitiveDataOccurrences"
    ]
    ...
    status: NotRequired["capo_macie2.types.reveal_request_status.RevealRequestStatus"]
...
def serialize_json(value: GetSensitiveDataOccurrencesResponse) -> dict:
    ...
    if "sensitive_data_occurrences" in value:
        ...
        out["sensitiveDataOccurrences"] = (
            capo_macie2.types.sensitive_data_occurrences.serialize_json(
    ...
    if "status" in value:
        ...
        out["status"] = capo_macie2.types.reveal_request_status.serialize_json(
            value["status"]
        )
    return out
...
def deserialize_json(data: dict) -> GetSensitiveDataOccurrencesResponse:
    ...
    if "sensitiveDataOccurrences" in data:
        ...
        out["sensitive_data_occurrences"] = (
            capo_macie2.types.sensitive_data_occurrences.deserialize_json(
