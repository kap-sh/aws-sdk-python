...
class TagScopeTerm(TypedDict, closed=True):
    ...
    tag_values: NotRequired[
        "capo_macie2.types.__list_of_tag_value_pair.__listOfTagValuePair"
    ]
    ...
    target: NotRequired["capo_macie2.types.tag_target.TagTarget"]
...
def serialize_json(value: TagScopeTerm) -> dict:
    ...
    if "tag_values" in value:
        ...
        out["tagValues"] = capo_macie2.types.__list_of_tag_value_pair.serialize_json(
            value["tag_values"]
        )
    if "target" in value:
        import capo_macie2.types.tag_target

        out["target"] = capo_macie2.types.tag_target.serialize_json(value["target"])
    return out
