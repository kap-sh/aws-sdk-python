...
class AllowListCriteria(TypedDict, closed=True):
    regex: NotRequired[
    ...
    s3_words_list: NotRequired["capo_macie2.types.s3_words_list.S3WordsList"]
...
def serialize_json(value: AllowListCriteria) -> dict:
    ...
    if "s3_words_list" in value:
        ...
        out["s3WordsList"] = capo_macie2.types.s3_words_list.serialize_json(
            value["s3_words_list"]
        )
    return out
