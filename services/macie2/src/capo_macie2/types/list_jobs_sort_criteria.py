...
class ListJobsSortCriteria(TypedDict, closed=True):
    attribute_name: NotRequired[
        "capo_macie2.types.list_jobs_sort_attribute_name.ListJobsSortAttributeName"
    ]
...
def serialize_json(value: ListJobsSortCriteria) -> dict:
    ...
    if "attribute_name" in value:
        ...
        out["attributeName"] = (
            capo_macie2.types.list_jobs_sort_attribute_name.serialize_json(
...
def deserialize_json(data: dict) -> ListJobsSortCriteria:
    ...
    if "attributeName" in data:
        ...
        out["attribute_name"] = (
            capo_macie2.types.list_jobs_sort_attribute_name.deserialize_json(
                data["attributeName"]
            )
        )
    if "orderBy" in data:
