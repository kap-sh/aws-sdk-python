...
class AppRunnerError(Exception):
    """Base class for all AppRunner service errors."""


class ServiceError(AppRunnerError):
...
class SerializationError(AppRunnerError):
    """Raised when a value cannot be serialized."""


class DeserializationError(AppRunnerError):
...
class WaiterFailedError(AppRunnerError):
...
class WaiterTimeoutError(AppRunnerError):
