...
EventList: TypeAlias = list["capo_docdb.types.event.Event"]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.event.serialize_query(item, pairs, f"{prefix}.Event.{n}")
...
def deserialize_query(el: Element) -> EventList:
    ...
    for child in el.findall("Event"):
        out.append(capo_docdb.types.event.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.event.serialize_query(item, pairs, f"{prefix}.{n}")
