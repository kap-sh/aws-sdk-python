...
DBSubnetGroups: TypeAlias = list["capo_docdb.types.db_subnet_group.DBSubnetGroup"]
...
def deserialize_query(el: Element) -> DBSubnetGroups:
    ...
    for child in el.findall("DBSubnetGroup"):
        out.append(capo_docdb.types.db_subnet_group.deserialize_query(child))
    return out
...
def serialize_query_flat(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.db_subnet_group.serialize_query(item, pairs, f"{prefix}.{n}")
