...
if TYPE_CHECKING:
    import capo_docdb.types.db_cluster_snapshot_attribute

DBClusterSnapshotAttributeList: TypeAlias = list[
    "capo_docdb.types.db_cluster_snapshot_attribute.DBClusterSnapshotAttribute"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_docdb.types.db_cluster_snapshot_attribute.serialize_query(
...
def deserialize_query(el: Element) -> DBClusterSnapshotAttributeList:
    ...
    for child in el.findall("DBClusterSnapshotAttribute"):
        out.append(
            capo_docdb.types.db_cluster_snapshot_attribute.deserialize_query(child)
        )
    return out
