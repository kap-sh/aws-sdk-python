...
class Event(TypedDict, closed=True):
    ...
    date: NotRequired["capo_docdb.types.t_stamp.TStamp"]
    """<p>Specifies the date and time of the event.</p>"""
    source_arn: NotRequired["capo_docdb.types.string.String"]
...
def serialize_query(value: Event, pairs: list[tuple[str, str]], prefix: str) -> None:
    ...
    if "date" in value:
        ...
        capo_docdb.types.t_stamp.serialize_query(value["date"], pairs, f"{prefix}.Date")
...
def deserialize_query(el: Element) -> Event:
    ...
    if child_date is not None:
        ...
        out["date"] = capo_docdb.types.t_stamp.deserialize_query(child_date)
