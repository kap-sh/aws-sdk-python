...
class CertificateMessage(TypedDict, closed=True):
    certificates: NotRequired["capo_docdb.types.certificate_list.CertificateList"]
...
def serialize_query(
    ...
    if "certificates" in value:
        ...
        capo_docdb.types.certificate_list.serialize_query(
...
def deserialize_query(el: Element) -> CertificateMessage:
    ...
    if child_certificates is not None:
        ...
        out["certificates"] = capo_docdb.types.certificate_list.deserialize_query(
            child_certificates
        )
