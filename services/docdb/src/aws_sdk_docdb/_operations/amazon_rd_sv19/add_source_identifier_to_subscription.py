"""Generated from Smithy shape ``com.amazonaws.docdb#AddSourceIdentifierToSubscription``."""
...
import aws_sdk_docdb.errors.source_not_found_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "SourceNotFoundFault":
            raise aws_sdk_docdb.errors.source_not_found_fault.SourceNotFoundFault.from_query(
                root
            )
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("AddSourceIdentifierToSubscriptionResult")
    out: aws_sdk_docdb.types.add_source_identifier_to_subscription_result.AddSourceIdentifierToSubscriptionResult = aws_sdk_docdb.types.add_source_identifier_to_subscription_result.deserialize_query(
...
def build_request(
    ...
    input_: aws_sdk_docdb.types.add_source_identifier_to_subscription_message.AddSourceIdentifierToSubscriptionMessage,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    pairs.append(("Action", "AddSourceIdentifierToSubscription"))
    pairs.append(("Version", "2014-10-31"))
    aws_sdk_docdb.types.add_source_identifier_to_subscription_message.serialize_query(
        input_, pairs, ""
    )
...
async def async_add_source_identifier_to_subscription(
