"""Generated from Smithy shape ``com.amazonaws.docdb#EventCategoriesMapList``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_docdb._protocol.xml import Element

if TYPE_CHECKING:
    import aws_sdk_docdb.types.event_categories_map

EventCategoriesMapList: TypeAlias = list[
    "aws_sdk_docdb.types.event_categories_map.EventCategoriesMap"
]


# --- awsQuery ser/de ---
def serialize_query(
    value: EventCategoriesMapList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_docdb.types.event_categories_map

    for n, item in enumerate(value, 1):
        aws_sdk_docdb.types.event_categories_map.serialize_query(
            item, pairs, f"{prefix}.EventCategoriesMap.{n}"
        )


def deserialize_query(el: Element) -> EventCategoriesMapList:
    import aws_sdk_docdb.types.event_categories_map

    out: EventCategoriesMapList = []
    for child in el.findall("EventCategoriesMap"):
        out.append(aws_sdk_docdb.types.event_categories_map.deserialize_query(child))
    return out


def serialize_query_flat(
    value: EventCategoriesMapList, pairs: list[tuple[str, str]], prefix: str
) -> None:
    import aws_sdk_docdb.types.event_categories_map

    for n, item in enumerate(value, 1):
        aws_sdk_docdb.types.event_categories_map.serialize_query(
            item, pairs, f"{prefix}.{n}"
        )


def deserialize_query_flat(parent: Element, tag: str) -> EventCategoriesMapList:
    import aws_sdk_docdb.types.event_categories_map

    out: EventCategoriesMapList = []
    for child in parent.findall(tag):
        out.append(aws_sdk_docdb.types.event_categories_map.deserialize_query(child))
    return out
