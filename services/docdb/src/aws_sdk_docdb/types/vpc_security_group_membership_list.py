...
if TYPE_CHECKING:
    import aws_sdk_docdb.types.vpc_security_group_membership

VpcSecurityGroupMembershipList: TypeAlias = list[
    "aws_sdk_docdb.types.vpc_security_group_membership.VpcSecurityGroupMembership"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_docdb.types.vpc_security_group_membership.serialize_query(
...
def deserialize_query(el: Element) -> VpcSecurityGroupMembershipList:
    ...
    for child in el.findall("VpcSecurityGroupMembership"):
        out.append(
            aws_sdk_docdb.types.vpc_security_group_membership.deserialize_query(child)
        )
    return out
