"""Generated from Smithy shape ``com.amazonaws.workspacesthinclient#DeregisterDevice``."""
...
def handle_response(
    ...
    out: aws_sdk_workspaces_thin_client.types.deregister_device_response.DeregisterDeviceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_workspaces_thin_client.types.deregister_device_request.DeregisterDeviceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/deregister-device/{id}"
    url = url.replace("{id}", quote(str(input_["id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_workspaces_thin_client.types.deregister_device_request.serialize_json(
            input_
        )
    ).encode()
