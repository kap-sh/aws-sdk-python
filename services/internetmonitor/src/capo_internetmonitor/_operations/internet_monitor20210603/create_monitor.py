"""Generated from Smithy shape ``com.amazonaws.internetmonitor#CreateMonitor``."""
...
import capo_internetmonitor.errors.conflict_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_internetmonitor.errors.conflict_exception.ConflictException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_internetmonitor.types.create_monitor_output.CreateMonitorOutput = (
        capo_internetmonitor.types.create_monitor_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v20210603/Monitors"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_internetmonitor.types.create_monitor_input.serialize_json(input_)
    ).encode()
