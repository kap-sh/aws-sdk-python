...
InternetEventsList: TypeAlias = list[
    "aws_sdk_internetmonitor.types.internet_event_summary.InternetEventSummary"
]
...
def serialize_json(value: InternetEventsList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_internetmonitor.types.internet_event_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> InternetEventsList:
    ...
    for item in data:
        out.append(
            aws_sdk_internetmonitor.types.internet_event_summary.deserialize_json(item)
        )
    return out
