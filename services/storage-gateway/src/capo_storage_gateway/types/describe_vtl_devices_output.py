...
class DescribeVTLDevicesOutput(TypedDict, closed=True):
    gateway_arn: NotRequired["capo_storage_gateway.types.gateway_arn.GatewayARN"]
    vtl_devices: NotRequired["capo_storage_gateway.types.vtl_devices.VTLDevices"]
...
def serialize_aws_json_1_1(value: DescribeVTLDevicesOutput) -> dict:
    ...
    if "vtl_devices" in value:
        ...
        out["VTLDevices"] = (
            capo_storage_gateway.types.vtl_devices.serialize_aws_json_1_1(
                value["vtl_devices"]
            )
        )
    if "marker" in value:
...
def deserialize_aws_json_1_1(data: dict) -> DescribeVTLDevicesOutput:
    ...
    if "VTLDevices" in data:
        ...
        out["vtl_devices"] = (
            capo_storage_gateway.types.vtl_devices.deserialize_aws_json_1_1(
                data["VTLDevices"]
            )
        )
    if "Marker" in data:
