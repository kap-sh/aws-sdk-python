...
if TYPE_CHECKING:
    import aws_sdk_ssm_guiconnect.types.s3_buckets


class RecordingDestinations(TypedDict):
    s3_buckets: "aws_sdk_ssm_guiconnect.types.s3_buckets.S3Buckets"
...
def serialize_json(value: RecordingDestinations) -> dict:
    ...
    out["S3Buckets"] = aws_sdk_ssm_guiconnect.types.s3_buckets.serialize_json(
        value["s3_buckets"]
    )
    return out
...
def deserialize_json(data: dict) -> RecordingDestinations:
    ...
    if "S3Buckets" in data:
        ...
        out["s3_buckets"] = aws_sdk_ssm_guiconnect.types.s3_buckets.deserialize_json(
            data["S3Buckets"]
        )
    else:
