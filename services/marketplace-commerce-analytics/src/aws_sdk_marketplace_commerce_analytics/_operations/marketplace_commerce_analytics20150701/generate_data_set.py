"""Generated from Smithy shape ``com.amazonaws.marketplacecommerceanalytics#GenerateDataSet``."""
...
from aws_sdk_marketplace_commerce_analytics._rule_engine._endpoint_rule_set import (
    EndpointParams,
    resolve,
)
...
from aws_sdk_marketplace_commerce_analytics.errors import (
    UnknownServiceError,
)
...
def get_signer(
...
) -> aws_sdk_marketplace_commerce_analytics._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_marketplace_commerce_analytics._auth._sigv4.build_sigv4_auth_scheme(
                "marketplacecommerceanalytics", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_marketplace_commerce_analytics._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_marketplace_commerce_analytics.types.generate_data_set_request.GenerateDataSetRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_marketplace_commerce_analytics.types.generate_data_set_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_generate_data_set(
