"""Generated from Smithy shape ``com.amazonaws.mwaaserverless#DeleteWorkflow``."""
...
def handle_response(
    ...
    out: capo_mwaa_serverless.types.delete_workflow_response.DeleteWorkflowResponse = (
        capo_mwaa_serverless.types.delete_workflow_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workflows/{WorkflowArn}"
    url = url.replace("{WorkflowArn}", quote(str(input_["workflow_arn"]), safe=""))
    params: dict[str, str] = {}
    if "workflow_version" in input_:
        params["workflowVersion"] = str(input_["workflow_version"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AmazonMWAAServerless.DeleteWorkflow"
    body: bytes | None = b""
