...
class Segment(TypedDict):
    ...
    scte: NotRequired["aws_sdk_mediapackagev2.types.scte.Scte"]
    """<p>The SCTE configuration options in the segment settings.</p>"""
    encryption: NotRequired["aws_sdk_mediapackagev2.types.encryption.Encryption"]
...
def serialize_json(value: Segment) -> dict:
    ...
    if "scte" in value:
        ...
        out["Scte"] = aws_sdk_mediapackagev2.types.scte.serialize_json(value["scte"])
    if "encryption" in value:
        import aws_sdk_mediapackagev2.types.encryption

        out["Encryption"] = aws_sdk_mediapackagev2.types.encryption.serialize_json(
            value["encryption"]
        )
    return out
...
def deserialize_json(data: dict) -> Segment:
    ...
    if "Scte" in data:
        ...
        out["scte"] = aws_sdk_mediapackagev2.types.scte.deserialize_json(data["Scte"])
    if "Encryption" in data:
        import aws_sdk_mediapackagev2.types.encryption

        out["encryption"] = aws_sdk_mediapackagev2.types.encryption.deserialize_json(
            data["Encryption"]
        )
    return out
