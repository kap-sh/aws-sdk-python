...
class ListChannelGroupsResponse(TypedDict):
    items: NotRequired[
        "aws_sdk_mediapackagev2.types.channel_groups_list.ChannelGroupsList"
    ]
...
def serialize_json(value: ListChannelGroupsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = aws_sdk_mediapackagev2.types.channel_groups_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListChannelGroupsResponse:
    ...
    if "Items" in data:
        ...
        out["items"] = (
            aws_sdk_mediapackagev2.types.channel_groups_list.deserialize_json(
                data["Items"]
            )
        )
