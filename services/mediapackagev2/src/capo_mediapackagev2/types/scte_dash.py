...
class ScteDash(TypedDict, closed=True):
    ad_marker_dash: NotRequired["capo_mediapackagev2.types.ad_marker_dash.AdMarkerDash"]
...
def serialize_json(value: ScteDash) -> dict:
    ...
    if "ad_marker_dash" in value:
        ...
        out["AdMarkerDash"] = capo_mediapackagev2.types.ad_marker_dash.serialize_json(
...
def deserialize_json(data: dict) -> ScteDash:
    ...
    if "AdMarkerDash" in data:
        ...
        out["ad_marker_dash"] = (
            capo_mediapackagev2.types.ad_marker_dash.deserialize_json(
                data["AdMarkerDash"]
            )
        )
