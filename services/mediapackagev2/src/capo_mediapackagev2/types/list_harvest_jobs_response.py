...
class ListHarvestJobsResponse(TypedDict, closed=True):
    items: NotRequired["capo_mediapackagev2.types.harvest_jobs_list.HarvestJobsList"]
...
def serialize_json(value: ListHarvestJobsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = capo_mediapackagev2.types.harvest_jobs_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListHarvestJobsResponse:
    ...
    if "Items" in data:
        ...
        out["items"] = capo_mediapackagev2.types.harvest_jobs_list.deserialize_json(
            data["Items"]
        )
