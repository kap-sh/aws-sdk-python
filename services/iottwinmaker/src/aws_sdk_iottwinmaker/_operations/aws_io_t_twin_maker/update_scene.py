...
def build_request(
    ...
    input: aws_sdk_iottwinmaker.types.update_scene_request.UpdateSceneRequest,
) -> zapros.Request:
    ...
    url = url.replace("{sceneId}", quote(str(input["scene_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iottwinmaker.types.update_scene_request

    body: bytes | None = json.dumps(
        aws_sdk_iottwinmaker.types.update_scene_request.serialize_json(input)
    ).encode()
