"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#UpdateScene``."""
...
def handle_response(
    ...
    out: aws_sdk_iottwinmaker.types.update_scene_response.UpdateSceneResponse = (
        aws_sdk_iottwinmaker.types.update_scene_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iottwinmaker.types.update_scene_request.UpdateSceneRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/scenes/{sceneId}"
    url = url.replace("{workspaceId}", quote(str(input_["workspace_id"]), safe=""))
    url = url.replace("{sceneId}", quote(str(input_["scene_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iottwinmaker.types.update_scene_request

    body: bytes | None = json.dumps(
        aws_sdk_iottwinmaker.types.update_scene_request.serialize_json(input_)
    ).encode()
