"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#GetPropertyValue``."""
...
import aws_sdk_iottwinmaker.errors.connector_failure_exception
import aws_sdk_iottwinmaker.errors.connector_timeout_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConnectorFailureException":
            raise aws_sdk_iottwinmaker.errors.connector_failure_exception.ConnectorFailureException.from_json(
                data
            )
        case "ConnectorTimeoutException":
            raise aws_sdk_iottwinmaker.errors.connector_timeout_exception.ConnectorTimeoutException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_iottwinmaker.types.get_property_value_response.GetPropertyValueResponse = aws_sdk_iottwinmaker.types.get_property_value_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iottwinmaker.types.get_property_value_request.GetPropertyValueRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/entity-properties/value"
    url = url.replace("{workspaceId}", quote(str(input_["workspace_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_iottwinmaker.types.get_property_value_request.serialize_json(input_)
    ).encode()
