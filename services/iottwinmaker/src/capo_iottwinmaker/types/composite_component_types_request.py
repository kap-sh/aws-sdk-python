...
if TYPE_CHECKING:
    import capo_iottwinmaker.types.composite_component_type_request
    import capo_iottwinmaker.types.name

CompositeComponentTypesRequest: TypeAlias = dict[
    "capo_iottwinmaker.types.name.Name",
    "capo_iottwinmaker.types.composite_component_type_request.CompositeComponentTypeRequest",
]
...
def serialize_json(input_to_serialize: CompositeComponentTypesRequest) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[key] = (
            capo_iottwinmaker.types.composite_component_type_request.serialize_json(
                value
            )
        )
    return out
...
def deserialize_json(data: dict) -> CompositeComponentTypesRequest:
    ...
    for key, value in data.items():
        ...
        out[key] = (
            capo_iottwinmaker.types.composite_component_type_request.deserialize_json(
                value
            )
        )
    return out
