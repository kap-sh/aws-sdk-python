...
if TYPE_CHECKING:
    import capo_iottwinmaker.types.composite_component_type_response
    import capo_iottwinmaker.types.name

CompositeComponentTypesResponse: TypeAlias = dict[
    "capo_iottwinmaker.types.name.Name",
    "capo_iottwinmaker.types.composite_component_type_response.CompositeComponentTypeResponse",
]
...
def serialize_json(input_to_serialize: CompositeComponentTypesResponse) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[key] = (
            capo_iottwinmaker.types.composite_component_type_response.serialize_json(
                value
            )
        )
    return out
...
def deserialize_json(data: dict) -> CompositeComponentTypesResponse:
    ...
    for key, value in data.items():
        ...
        out[key] = (
            capo_iottwinmaker.types.composite_component_type_response.deserialize_json(
                value
            )
        )
    return out
