"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#CreateEntity``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.create_entity_response.CreateEntityResponse = (
        capo_iottwinmaker.types.create_entity_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/entities"
    url = url.replace("{workspaceId}", quote(str(input_["workspace_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.create_entity_request.serialize_json(input_)
    ).encode()
...
def create_entity(
    ...
    capo_iottwinmaker.types.create_entity_response.CreateEntityResponse, zapros.Response
]:
