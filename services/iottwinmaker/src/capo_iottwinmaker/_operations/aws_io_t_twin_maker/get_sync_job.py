"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#GetSyncJob``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.get_sync_job_response.GetSyncJobResponse = (
        capo_iottwinmaker.types.get_sync_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/sync-jobs/{syncSource}"
    url = url.replace("{syncSource}", quote(str(input_["sync_source"]), safe=""))
    params: dict[str, str] = {}
    if "workspace_id" in input_:
        params["workspace"] = str(input_["workspace_id"])
...
def get_sync_job(
    ...
    capo_iottwinmaker.types.get_sync_job_response.GetSyncJobResponse, zapros.Response
]:
