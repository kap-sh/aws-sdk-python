"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#CreateMetadataTransferJob``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.create_metadata_transfer_job_response.CreateMetadataTransferJobResponse = capo_iottwinmaker.types.create_metadata_transfer_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/metadata-transfer-jobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.create_metadata_transfer_job_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_metadata_transfer_job(
