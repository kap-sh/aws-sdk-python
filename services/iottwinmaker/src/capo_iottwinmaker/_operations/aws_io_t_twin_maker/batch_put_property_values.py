"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#BatchPutPropertyValues``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.batch_put_property_values_response.BatchPutPropertyValuesResponse = capo_iottwinmaker.types.batch_put_property_values_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/entity-properties"
    url = url.replace("{workspaceId}", quote(str(input_["workspace_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.batch_put_property_values_request.serialize_json(input_)
    ).encode()
...
async def async_batch_put_property_values(
