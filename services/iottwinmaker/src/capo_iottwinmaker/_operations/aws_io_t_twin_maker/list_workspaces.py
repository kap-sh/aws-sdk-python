"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#ListWorkspaces``."""
...
import capo_iottwinmaker.types.workspace_summaries
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.list_workspaces_response.ListWorkspacesResponse = (
        capo_iottwinmaker.types.list_workspaces_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/workspaces-list"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.list_workspaces_request.serialize_json(input_)
    ).encode()
