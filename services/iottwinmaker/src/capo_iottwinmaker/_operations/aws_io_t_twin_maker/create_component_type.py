"""Generated from Smithy shape ``com.amazonaws.iottwinmaker#CreateComponentType``."""
...
def handle_response(
    ...
    out: capo_iottwinmaker.types.create_component_type_response.CreateComponentTypeResponse = capo_iottwinmaker.types.create_component_type_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{componentTypeId}", quote(str(input_["component_type_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iottwinmaker.types.create_component_type_request.serialize_json(input_)
    ).encode()
...
async def async_create_component_type(
