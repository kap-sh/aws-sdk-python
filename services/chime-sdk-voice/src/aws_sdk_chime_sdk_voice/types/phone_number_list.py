...
PhoneNumberList: TypeAlias = list[
    "aws_sdk_chime_sdk_voice.types.phone_number.PhoneNumber"
]
...
def serialize_json(value: PhoneNumberList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_chime_sdk_voice.types.phone_number.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PhoneNumberList:
    ...
    out: PhoneNumberList = []
    for item in data:
        out.append(aws_sdk_chime_sdk_voice.types.phone_number.deserialize_json(item))
    return out
