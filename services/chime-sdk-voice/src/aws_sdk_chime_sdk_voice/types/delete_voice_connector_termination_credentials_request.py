...
class DeleteVoiceConnectorTerminationCredentialsRequest(TypedDict):
...
def serialize_json(value: DeleteVoiceConnectorTerminationCredentialsRequest) -> dict:
    ...
    out["Usernames"] = (
        aws_sdk_chime_sdk_voice.types.sensitive_string_list.serialize_json(
            value["usernames"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> DeleteVoiceConnectorTerminationCredentialsRequest:
    ...
    if "Usernames" in data:
        ...
        out["usernames"] = (
            aws_sdk_chime_sdk_voice.types.sensitive_string_list.deserialize_json(
                data["Usernames"]
            )
        )
    else:
