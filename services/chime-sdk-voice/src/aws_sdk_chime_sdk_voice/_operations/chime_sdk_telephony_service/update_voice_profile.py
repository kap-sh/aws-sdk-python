...
def build_request(
    ...
    input_: aws_sdk_chime_sdk_voice.types.update_voice_profile_request.UpdateVoiceProfileRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{VoiceProfileId}", quote(str(input_["voice_profile_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_chime_sdk_voice.types.update_voice_profile_request.serialize_json(
            input_
        )
    ).encode()
