"""Generated from Smithy shape ``com.amazonaws.chimesdkvoice#UpdateVoiceProfileDomain``."""
...
def handle_response(
    ...
    out: aws_sdk_chime_sdk_voice.types.update_voice_profile_domain_response.UpdateVoiceProfileDomainResponse = aws_sdk_chime_sdk_voice.types.update_voice_profile_domain_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_chime_sdk_voice.types.update_voice_profile_domain_request.UpdateVoiceProfileDomainRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/voice-profile-domains/{VoiceProfileDomainId}"
    url = url.replace(
        "{VoiceProfileDomainId}", quote(str(input_["voice_profile_domain_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_chime_sdk_voice.types.update_voice_profile_domain_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_voice_profile_domain(
