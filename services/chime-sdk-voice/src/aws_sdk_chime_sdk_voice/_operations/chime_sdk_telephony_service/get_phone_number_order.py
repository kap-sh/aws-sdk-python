"""Generated from Smithy shape ``com.amazonaws.chimesdkvoice#GetPhoneNumberOrder``."""
...
def handle_response(
    ...
    out: aws_sdk_chime_sdk_voice.types.get_phone_number_order_response.GetPhoneNumberOrderResponse = aws_sdk_chime_sdk_voice.types.get_phone_number_order_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_chime_sdk_voice.types.get_phone_number_order_request.GetPhoneNumberOrderRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/phone-number-orders/{PhoneNumberOrderId}"
    url = url.replace(
        "{PhoneNumberOrderId}", quote(str(input_["phone_number_order_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_phone_number_order(
