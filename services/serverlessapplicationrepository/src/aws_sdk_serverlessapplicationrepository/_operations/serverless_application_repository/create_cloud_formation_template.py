...
from aws_sdk_serverlessapplicationrepository._rule_engine._endpoint_rule_set import (
    EndpointParams,
    resolve,
)
...
from aws_sdk_serverlessapplicationrepository.errors import (
    UnknownServiceError,
)
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "NotFoundException":
            import aws_sdk_serverlessapplicationrepository.errors.not_found_exception

            raise aws_sdk_serverlessapplicationrepository.errors.not_found_exception.NotFoundException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_serverlessapplicationrepository.types.create_cloud_formation_template_response.CreateCloudFormationTemplateResponse = aws_sdk_serverlessapplicationrepository.types.create_cloud_formation_template_response.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_serverlessapplicationrepository._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_serverlessapplicationrepository._auth._sigv4.build_sigv4_auth_scheme(
                "serverlessrepo", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_serverlessapplicationrepository._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_serverlessapplicationrepository.types.create_cloud_formation_template_request.CreateCloudFormationTemplateRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_serverlessapplicationrepository.types.create_cloud_formation_template_request.serialize_json(
            input_
        )
    ).encode()
