...
Resources: TypeAlias = list[
    "aws_sdk_service_catalog_appregistry.types.resource_info.ResourceInfo"
]
...
def serialize_json(value: Resources) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_service_catalog_appregistry.types.resource_info.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> Resources:
    ...
    for item in data:
        out.append(
            aws_sdk_service_catalog_appregistry.types.resource_info.deserialize_json(
                item
            )
        )
    return out
