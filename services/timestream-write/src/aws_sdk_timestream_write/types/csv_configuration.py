...
if TYPE_CHECKING:
    ...
    import aws_sdk_timestream_write.types.string_value1
    import aws_sdk_timestream_write.types.string_value256


class CsvConfiguration(TypedDict):
    column_separator: NotRequired[
        "aws_sdk_timestream_write.types.string_value1.StringValue1"
    ]
    """<p>Column separator can be one of comma (','), pipe ('|), semicolon (';'), tab('/t'), or blank space (' '). </p>"""
    escape_char: NotRequired[
        "aws_sdk_timestream_write.types.string_value1.StringValue1"
    ]
    ...
    trim_white_space: NotRequired["aws_sdk_timestream_write.types.boolean.Boolean"]
