...
class DataBrewError(Exception):
    """Base class for all DataBrew service errors."""


class ServiceError(DataBrewError):
...
class SerializationError(DataBrewError):
    """Raised when a value cannot be serialized."""


class DeserializationError(DataBrewError):
...
class WaiterFailedError(DataBrewError):
...
class WaiterTimeoutError(DataBrewError):
