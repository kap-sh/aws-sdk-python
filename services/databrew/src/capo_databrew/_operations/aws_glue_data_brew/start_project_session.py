"""Generated from Smithy shape ``com.amazonaws.databrew#StartProjectSession``."""
...
import capo_databrew.types.start_project_session_request
import capo_databrew.types.start_project_session_response
...
) -> capo_databrew.types.start_project_session_response.StartProjectSessionResponse:
    out: capo_databrew.types.start_project_session_response.StartProjectSessionResponse = capo_databrew.types.start_project_session_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/projects/{Name}/startProjectSession"
    url = url.replace("{Name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_databrew.types.start_project_session_request.serialize_json(input_)
    ).encode()
...
async def async_start_project_session(
