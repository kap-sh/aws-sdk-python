"""Generated from Smithy shape ``com.amazonaws.databrew#SendProjectSessionAction``."""
...
import capo_databrew.types.send_project_session_action_request
import capo_databrew.types.send_project_session_action_response
...
) -> capo_databrew.types.send_project_session_action_response.SendProjectSessionActionResponse:
    out: capo_databrew.types.send_project_session_action_response.SendProjectSessionActionResponse = capo_databrew.types.send_project_session_action_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/projects/{Name}/sendProjectSessionAction"
    url = url.replace("{Name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_databrew.types.send_project_session_action_request.serialize_json(input_)
    ).encode()
...
async def async_send_project_session_action(
