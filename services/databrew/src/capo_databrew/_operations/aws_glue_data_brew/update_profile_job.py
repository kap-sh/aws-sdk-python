"""Generated from Smithy shape ``com.amazonaws.databrew#UpdateProfileJob``."""
...
import capo_databrew.errors.access_denied_exception
...
import capo_databrew.types.update_profile_job_request
import capo_databrew.types.update_profile_job_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            raise capo_databrew.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
...
) -> capo_databrew.types.update_profile_job_response.UpdateProfileJobResponse:
    out: capo_databrew.types.update_profile_job_response.UpdateProfileJobResponse = (
        capo_databrew.types.update_profile_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/profileJobs/{Name}"
    url = url.replace("{Name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_databrew.types.update_profile_job_request.serialize_json(input_)
    ).encode()
...
async def async_update_profile_job(
