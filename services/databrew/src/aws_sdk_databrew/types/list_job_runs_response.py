...
class ListJobRunsResponse(TypedDict, closed=True):
    job_runs: "aws_sdk_databrew.types.job_run_list.JobRunList"
...
def serialize_json(value: ListJobRunsResponse) -> dict:
    ...
    out["JobRuns"] = aws_sdk_databrew.types.job_run_list.serialize_json(
        value["job_runs"]
    )
...
def deserialize_json(data: dict) -> ListJobRunsResponse:
    ...
    if "JobRuns" in data:
        ...
        out["job_runs"] = aws_sdk_databrew.types.job_run_list.deserialize_json(
            data["JobRuns"]
        )
    else:
