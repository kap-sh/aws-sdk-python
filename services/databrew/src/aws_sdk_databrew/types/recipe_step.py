...
if TYPE_CHECKING:
    import aws_sdk_databrew.types.condition_expression_list
    import aws_sdk_databrew.types.recipe_action


class RecipeStep(TypedDict, closed=True):
    action: "aws_sdk_databrew.types.recipe_action.RecipeAction"
    """<p>The particular action to be performed in the recipe step.</p>"""
    condition_expressions: NotRequired[
        "aws_sdk_databrew.types.condition_expression_list.ConditionExpressionList"
    ]
...
def serialize_json(value: RecipeStep) -> dict:
    ...
    out["Action"] = aws_sdk_databrew.types.recipe_action.serialize_json(value["action"])
    if "condition_expressions" in value:
        import aws_sdk_databrew.types.condition_expression_list

        out["ConditionExpressions"] = (
            aws_sdk_databrew.types.condition_expression_list.serialize_json(
...
def deserialize_json(data: dict) -> RecipeStep:
    ...
    if "Action" in data:
        ...
        out["action"] = aws_sdk_databrew.types.recipe_action.deserialize_json(
            data["Action"]
        )
    else:
    ...
    if "ConditionExpressions" in data:
        ...
        out["condition_expressions"] = (
            aws_sdk_databrew.types.condition_expression_list.deserialize_json(
