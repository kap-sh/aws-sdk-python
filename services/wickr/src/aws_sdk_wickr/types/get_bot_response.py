...
class GetBotResponse(TypedDict):
    ...
    pubkey: NotRequired["aws_sdk_wickr.types.generic_string.GenericString"]
...
def serialize_json(value: GetBotResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = aws_sdk_wickr.types.bot_status.serialize_json(value["status"])
...
def deserialize_json(data: dict) -> GetBotResponse:
    ...
    if "status" in data:
        ...
        out["status"] = aws_sdk_wickr.types.bot_status.deserialize_json(data["status"])
    if "groupId" in data:
