...
def build_request(
    ...
    input: aws_sdk_wickr.types.update_guest_user_request.UpdateGuestUserRequest,
) -> zapros.Request:
    ...
    url = url.replace("{usernameHash}", quote(str(input["username_hash"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_wickr.types.update_guest_user_request

    body: bytes | None = json.dumps(
        aws_sdk_wickr.types.update_guest_user_request.serialize_json(input)
    ).encode()
