...
def handle_response(
    ...
    out: aws_sdk_wickr.types.update_guest_user_response.UpdateGuestUserResponse = (
        aws_sdk_wickr.types.update_guest_user_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_wickr.types.update_guest_user_request.UpdateGuestUserRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{usernameHash}", quote(str(input_["username_hash"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_wickr.types.update_guest_user_request

    body: bytes | None = json.dumps(
        aws_sdk_wickr.types.update_guest_user_request.serialize_json(input_)
    ).encode()
