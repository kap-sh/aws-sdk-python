"""Generated from Smithy shape ``com.amazonaws.wickr#RegisterOpentdfConfig``."""
...
def handle_response(
    ...
    out: aws_sdk_wickr.types.register_opentdf_config_response.RegisterOpentdfConfigResponse = aws_sdk_wickr.types.register_opentdf_config_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_wickr.types.register_opentdf_config_request.RegisterOpentdfConfigRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/tdf"
    url = url.replace("{networkId}", quote(str(input["network_id"]), safe=""))
    params: dict[str, str] = {}
    if "dry_run" in input:
        params["dryRun"] = str(input["dry_run"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_wickr.types.register_opentdf_config_request

    body: bytes | None = json.dumps(
        aws_sdk_wickr.types.register_opentdf_config_request.serialize_json(input)
    ).encode()
...
async def async_register_opentdf_config(
