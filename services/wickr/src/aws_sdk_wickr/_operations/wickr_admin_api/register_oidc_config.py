"""Generated from Smithy shape ``com.amazonaws.wickr#RegisterOidcConfig``."""
...
def handle_response(
    ...
    out: aws_sdk_wickr.types.register_oidc_config_response.RegisterOidcConfigResponse = aws_sdk_wickr.types.register_oidc_config_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_wickr.types.register_oidc_config_request.RegisterOidcConfigRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/oidc/save"
    url = url.replace("{networkId}", quote(str(input_["network_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_wickr.types.register_oidc_config_request.serialize_json(input_)
    ).encode()
...
async def async_register_oidc_config(
