"""Generated from Smithy shape ``com.amazonaws.wickr#BatchLookupUserUname``."""
...
def handle_response(
    ...
    out: aws_sdk_wickr.types.batch_lookup_user_uname_response.BatchLookupUserUnameResponse = aws_sdk_wickr.types.batch_lookup_user_uname_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_wickr.types.batch_lookup_user_uname_request.BatchLookupUserUnameRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/users/uname-lookup"
    url = url.replace("{networkId}", quote(str(input["network_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "client_token" in input:
        headers["X-Client-Token"] = str(input["client_token"])
    import aws_sdk_wickr.types.batch_lookup_user_uname_request

    body: bytes | None = json.dumps(
        aws_sdk_wickr.types.batch_lookup_user_uname_request.serialize_json(input)
    ).encode()
...
async def async_batch_lookup_user_uname(
