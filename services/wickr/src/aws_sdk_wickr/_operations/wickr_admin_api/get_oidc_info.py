...
def build_request(
    ...
    input_: aws_sdk_wickr.types.get_oidc_info_request.GetOidcInfoRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "code" in input_:
        params["code"] = str(input_["code"])
    if "grant_type" in input_:
        params["grantType"] = str(input_["grant_type"])
    if "redirect_uri" in input_:
        params["redirectUri"] = str(input_["redirect_uri"])
    if "url" in input_:
        params["url"] = str(input_["url"])
    if "client_secret" in input_:
        params["clientSecret"] = str(input_["client_secret"])
    if "code_verifier" in input_:
        params["codeVerifier"] = str(input_["code_verifier"])
    if "certificate" in input_:
        params["certificate"] = str(input_["certificate"])
