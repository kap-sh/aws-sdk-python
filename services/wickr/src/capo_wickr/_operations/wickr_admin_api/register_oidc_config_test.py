"""Generated from Smithy shape ``com.amazonaws.wickr#RegisterOidcConfigTest``."""
...
def handle_response(
    ...
    out: capo_wickr.types.register_oidc_config_test_response.RegisterOidcConfigTestResponse = capo_wickr.types.register_oidc_config_test_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/oidc/test"
    url = url.replace("{networkId}", quote(str(input_["network_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wickr.types.register_oidc_config_test_request.serialize_json(input_)
    ).encode()
...
async def async_register_oidc_config_test(
