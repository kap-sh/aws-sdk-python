"""Generated from Smithy shape ``com.amazonaws.wickr#UpdateNetworkSettings``."""
...
def handle_response(
    ...
    out: capo_wickr.types.update_network_settings_response.UpdateNetworkSettingsResponse = capo_wickr.types.update_network_settings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}/settings"
    url = url.replace("{networkId}", quote(str(input_["network_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_wickr.types.update_network_settings_request.serialize_json(input_)
    ).encode()
...
async def async_update_network_settings(
