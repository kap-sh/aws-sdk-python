"""Generated from Smithy shape ``com.amazonaws.wickr#UpdateNetwork``."""
...
def handle_response(
    ...
    out: capo_wickr.types.update_network_response.UpdateNetworkResponse = (
        capo_wickr.types.update_network_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/networks/{networkId}"
    ...
    body: bytes | None = json.dumps(
        capo_wickr.types.update_network_request.serialize_json(input_)
    ).encode()
...
def update_network(
    ...
    capo_wickr.types.update_network_response.UpdateNetworkResponse, zapros.Response
]:
...
async def async_update_network(
