"""Generated from Smithy shape ``com.amazonaws.applicationsignals#BatchGetServiceLevelObjectiveBudgetReport``."""
...
def handle_response(
    ...
    out: capo_application_signals.types.batch_get_service_level_objective_budget_report_output.BatchGetServiceLevelObjectiveBudgetReportOutput = capo_application_signals.types.batch_get_service_level_objective_budget_report_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/budget-report"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_application_signals.types.batch_get_service_level_objective_budget_report_input.serialize_json(
            input_
        )
    ).encode()
