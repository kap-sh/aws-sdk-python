...
AuditTargets: TypeAlias = list["aws_sdk_application_signals.types.audit_target.AuditTarget"]
...
def deserialize_json(data: list) -> AuditTargets:
    ...
    out: AuditTargets = []
    for item in data:
        out.append(aws_sdk_application_signals.types.audit_target.deserialize_json(item))
    return out