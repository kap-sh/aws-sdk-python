...
AuditFindings: TypeAlias = list["aws_sdk_application_signals.types.audit_finding.AuditFinding"]
...
def deserialize_json(data: list) -> AuditFindings:
    ...
    out: AuditFindings = []
    for item in data:
        out.append(aws_sdk_application_signals.types.audit_finding.deserialize_json(item))
    return out