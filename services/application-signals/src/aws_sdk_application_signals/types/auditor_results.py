...
AuditorResults: TypeAlias = list[
    "aws_sdk_application_signals.types.auditor_result.AuditorResult"
]
...
def serialize_json(value: AuditorResults) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_application_signals.types.auditor_result.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AuditorResults:
    ...
    for item in data:
        out.append(
            aws_sdk_application_signals.types.auditor_result.deserialize_json(item)
        )
    return out
