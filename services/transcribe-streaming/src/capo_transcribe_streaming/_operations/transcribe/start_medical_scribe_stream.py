...
def handle_response(
    ...
    _union_deser = capo_transcribe_streaming.types.medical_scribe_result_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: capo_transcribe_streaming.types.start_medical_scribe_stream_response.StartMedicalScribeStreamResponse = {
        "result_stream": cast(
...
def build_request(
    ...
    body = capo_transcribe_streaming._iter.map_sync_iterator(
        input_["input_stream"],
        capo_transcribe_streaming.types.medical_scribe_input_stream.serialize_event_json,
    )
