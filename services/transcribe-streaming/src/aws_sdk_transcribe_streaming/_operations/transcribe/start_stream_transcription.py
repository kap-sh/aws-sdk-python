...
def handle_response(
    ...
    _union_deser = aws_sdk_transcribe_streaming.types.transcript_result_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_transcribe_streaming.types.start_stream_transcription_response.StartStreamTranscriptionResponse = {
...
def build_request(
    ...
    input_: aws_sdk_transcribe_streaming.types.start_stream_transcription_request.StartStreamTranscriptionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
