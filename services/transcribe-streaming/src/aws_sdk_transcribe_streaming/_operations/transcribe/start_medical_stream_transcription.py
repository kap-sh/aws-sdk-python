...
def handle_response(
    ...
    _union_deser = aws_sdk_transcribe_streaming.types.medical_transcript_result_stream.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_transcribe_streaming.types.start_medical_stream_transcription_response.StartMedicalStreamTranscriptionResponse = {
    ...
    if "x-amzn-transcribe-specialty" in response.headers:
        out["specialty"] = (
            aws_sdk_transcribe_streaming.types.specialty.deserialize_json(
                response.headers["x-amzn-transcribe-specialty"]
            )
        )
    if "x-amzn-transcribe-type" in response.headers:
        out["type"] = aws_sdk_transcribe_streaming.types.type.deserialize_json(
    ...
    if "x-amzn-transcribe-content-identification-type" in response.headers:
        out["content_identification_type"] = (
            aws_sdk_transcribe_streaming.types.medical_content_identification_type.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_transcribe_streaming.types.start_medical_stream_transcription_request.StartMedicalStreamTranscriptionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
