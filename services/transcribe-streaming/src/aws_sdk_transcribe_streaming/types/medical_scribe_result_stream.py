...
def serialize_event_json(value: _MedicalScribeResultStream) -> bytes:
    match value:
        case {"TranscriptEvent": payload}:
            ...
            return aws_sdk_transcribe_streaming.types.medical_scribe_transcript_event.serialize_event_json(
                payload
            )
...
def deserialize_event_json(message: Message) -> _MedicalScribeResultStream:
    ...
    if message_type == "error":
        ...
        raise ValueError(
            f"MedicalScribeResultStream: unrecognized error-type {error_type!r}"
        )
    ...
    match event_type:
        case "TranscriptEvent":
            ...
            return {
                "TranscriptEvent": aws_sdk_transcribe_streaming.types.medical_scribe_transcript_event.deserialize_event_json(
                    message
                )
            }
        case _:
