...
# --- restJson1 ser/de ---
def serialize_json(value: MedicalScribeResultStream) -> dict:
    if "TranscriptEvent" in value:
        import aws_sdk_transcribe_streaming.types.medical_scribe_transcript_event

        return {
            "TranscriptEvent": aws_sdk_transcribe_streaming.types.medical_scribe_transcript_event.serialize_json(
        ...
        raise SerializationError("MedicalScribeResultStream: no variant present")


def deserialize_json(data: dict) -> MedicalScribeResultStream:
    if "TranscriptEvent" in data:
        import aws_sdk_transcribe_streaming.types.medical_scribe_transcript_event

        return {
            "TranscriptEvent": aws_sdk_transcribe_streaming.types.medical_scribe_transcript_event.deserialize_json(
        ...
        raise DeserializationError(
            "MedicalScribeResultStream: no recognized variant key"
        )
