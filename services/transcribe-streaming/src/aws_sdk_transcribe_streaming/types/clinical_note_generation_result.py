...
if TYPE_CHECKING:
    import aws_sdk_transcribe_streaming.types.clinical_note_generation_status
    import aws_sdk_transcribe_streaming.types.string
    import aws_sdk_transcribe_streaming.types.uri


class ClinicalNoteGenerationResult(TypedDict):
    clinical_note_output_location: NotRequired[
        "aws_sdk_transcribe_streaming.types.uri.Uri"
    ]
    """<p>Holds the Amazon S3 URI for the output Clinical Note. </p>"""
    transcript_output_location: NotRequired[
        "aws_sdk_transcribe_streaming.types.uri.Uri"
    ]
    """<p>Holds the Amazon S3 URI for the output Transcript. </p>"""
    status: NotRequired[
        "aws_sdk_transcribe_streaming.types.clinical_note_generation_status.ClinicalNoteGenerationStatus"
    ]
    ...
    failure_reason: NotRequired["aws_sdk_transcribe_streaming.types.string.String"]
...
def serialize_json(value: ClinicalNoteGenerationResult) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            aws_sdk_transcribe_streaming.types.clinical_note_generation_status.serialize_json(
                value["status"]
            )
        )
...
def deserialize_json(data: dict) -> ClinicalNoteGenerationResult:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            aws_sdk_transcribe_streaming.types.clinical_note_generation_status.deserialize_json(
                data["Status"]
            )
        )
