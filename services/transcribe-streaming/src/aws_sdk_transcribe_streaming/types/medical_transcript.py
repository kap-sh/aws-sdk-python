...
if TYPE_CHECKING:
    import aws_sdk_transcribe_streaming.types.medical_result_list


class MedicalTranscript(TypedDict, closed=True):
    results: NotRequired[
        "aws_sdk_transcribe_streaming.types.medical_result_list.MedicalResultList"
    ]
...
def serialize_json(value: MedicalTranscript) -> dict:
    ...
    if "results" in value:
        ...
        out["Results"] = (
            aws_sdk_transcribe_streaming.types.medical_result_list.serialize_json(
                value["results"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> MedicalTranscript:
    ...
    if "Results" in data:
        ...
        out["results"] = (
            aws_sdk_transcribe_streaming.types.medical_result_list.deserialize_json(
                data["Results"]
            )
        )
    return out
