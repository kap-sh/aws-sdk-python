...
if TYPE_CHECKING:
    import aws_sdk_transcribe_streaming.types.bucket_name
    import aws_sdk_transcribe_streaming.types.medical_scribe_note_template


class ClinicalNoteGenerationSettings(TypedDict):
    output_bucket_name: "aws_sdk_transcribe_streaming.types.bucket_name.BucketName"
    ...
    note_template: NotRequired[
        "aws_sdk_transcribe_streaming.types.medical_scribe_note_template.MedicalScribeNoteTemplate"
    ]
...
def serialize_json(value: ClinicalNoteGenerationSettings) -> dict:
    ...
    if "note_template" in value:
        ...
        out["NoteTemplate"] = (
            aws_sdk_transcribe_streaming.types.medical_scribe_note_template.serialize_json(
                value["note_template"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ClinicalNoteGenerationSettings:
    ...
    if "NoteTemplate" in data:
        ...
        out["note_template"] = (
            aws_sdk_transcribe_streaming.types.medical_scribe_note_template.deserialize_json(
                data["NoteTemplate"]
            )
        )
    return out
