"""Generated from Smithy shape ``com.amazonaws.resourcegroupstaggingapi#ListRequiredTags``."""
...
import aws_sdk_resource_groups_tagging_api.errors.pagination_token_expired_exception
...
from aws_sdk_resource_groups_tagging_api._rule_engine._endpoint_rule_set import (
    EndpointParams,
    resolve,
)
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PaginationTokenExpiredException":
            raise aws_sdk_resource_groups_tagging_api.errors.pagination_token_expired_exception.PaginationTokenExpiredException.from_aws_json_1_1(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_resource_groups_tagging_api.types.list_required_tags_output.ListRequiredTagsOutput = aws_sdk_resource_groups_tagging_api.types.list_required_tags_output.deserialize_aws_json_1_1(
...
def get_signer(
...
) -> aws_sdk_resource_groups_tagging_api._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_resource_groups_tagging_api._auth._sigv4.build_sigv4_auth_scheme(
                "tagging", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_resource_groups_tagging_api._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_resource_groups_tagging_api.types.list_required_tags_input.ListRequiredTagsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/ListRequiredTags"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "ResourceGroupsTaggingAPI_20170126.ListRequiredTags"
    body: bytes | None = json.dumps(
        aws_sdk_resource_groups_tagging_api.types.list_required_tags_input.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_list_required_tags(
