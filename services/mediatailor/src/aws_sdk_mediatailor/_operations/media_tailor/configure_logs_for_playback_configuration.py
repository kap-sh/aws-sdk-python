"""Generated from Smithy shape ``com.amazonaws.mediatailor#ConfigureLogsForPlaybackConfiguration``."""
...
def handle_response(
    ...
    out: aws_sdk_mediatailor.types.configure_logs_for_playback_configuration_response.ConfigureLogsForPlaybackConfigurationResponse = aws_sdk_mediatailor.types.configure_logs_for_playback_configuration_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mediatailor.types.configure_logs_for_playback_configuration_request.ConfigureLogsForPlaybackConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/configureLogs/playbackConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_mediatailor.types.configure_logs_for_playback_configuration_request

    body: bytes | None = json.dumps(
        aws_sdk_mediatailor.types.configure_logs_for_playback_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_configure_logs_for_playback_configuration(
