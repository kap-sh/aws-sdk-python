"""Generated from Smithy shape ``com.amazonaws.mediatailor#TagResource``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "BadRequestException":
            import aws_sdk_mediatailor.errors.bad_request_exception

            raise aws_sdk_mediatailor.errors.bad_request_exception.BadRequestException.from_json(
                data
            )
        case _:
...
def build_request(
    ...
    input_: aws_sdk_mediatailor.types.tag_resource_request.TagResourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_mediatailor.types.tag_resource_request.serialize_json(input_)
    ).encode()
