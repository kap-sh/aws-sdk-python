"""Generated from Smithy shape ``com.amazonaws.mediatailor#GetChannelSchedule``."""
...
def handle_response(
    ...
    out: capo_mediatailor.types.get_channel_schedule_response.GetChannelScheduleResponse = capo_mediatailor.types.get_channel_schedule_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/channel/{ChannelName}/schedule"
    url = url.replace("{ChannelName}", quote(str(input_["channel_name"]), safe=""))
    params: dict[str, str] = {}
    if "duration_minutes" in input_:
        params["durationMinutes"] = str(input_["duration_minutes"])
    if "max_results" in input_:
        params["maxResults"] = str(input_["max_results"])
    if "next_token" in input_:
        params["nextToken"] = str(input_["next_token"])
    if "audience" in input_:
        params["audience"] = str(input_["audience"])
...
async def async_get_channel_schedule(
