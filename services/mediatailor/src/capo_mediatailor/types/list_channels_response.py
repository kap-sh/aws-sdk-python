...
if TYPE_CHECKING:
    import capo_mediatailor.types.__list_of_channel
    import capo_mediatailor.types.__string


class ListChannelsResponse(TypedDict, closed=True):
    items: NotRequired["capo_mediatailor.types.__list_of_channel.__listOfChannel"]
...
def serialize_json(value: ListChannelsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = capo_mediatailor.types.__list_of_channel.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> ListChannelsResponse:
    ...
    if "Items" in data:
        ...
        out["items"] = capo_mediatailor.types.__list_of_channel.deserialize_json(
            data["Items"]
        )
