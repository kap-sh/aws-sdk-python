...
if TYPE_CHECKING:
    import capo_sts.types.decoded_message_type


class DecodeAuthorizationMessageResponse(TypedDict, closed=True):
    decoded_message: NotRequired[
        "capo_sts.types.decoded_message_type.decodedMessageType"
    ]
...
def serialize_query(
    ...
    if "decoded_message" in value:
        pairs.append((f"{key_prefix}DecodedMessage", str(value["decoded_message"])))
