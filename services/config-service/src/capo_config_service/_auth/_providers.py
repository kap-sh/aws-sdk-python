...
from capo_config_service._auth._identity import (
    Credentials,
    Identity,
)
...
# they need the `sso` extra: `pip install capo-config-service[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-config-service[sso]"
        )
