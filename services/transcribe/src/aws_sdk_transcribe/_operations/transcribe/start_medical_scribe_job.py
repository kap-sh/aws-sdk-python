"""Generated from Smithy shape ``com.amazonaws.transcribe#StartMedicalScribeJob``."""
...
def handle_response(
    ...
    out: aws_sdk_transcribe.types.start_medical_scribe_job_response.StartMedicalScribeJobResponse = aws_sdk_transcribe.types.start_medical_scribe_job_response.deserialize_aws_json_1_1(
...
def build_request(
    ...
    input_: aws_sdk_transcribe.types.start_medical_scribe_job_request.StartMedicalScribeJobRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/medicalscribejobs/{MedicalScribeJobName}"
    url = url.replace(
        "{MedicalScribeJobName}", quote(str(input_["medical_scribe_job_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Transcribe.StartMedicalScribeJob"
    import aws_sdk_transcribe.types.start_medical_scribe_job_request

    body: bytes | None = json.dumps(
        aws_sdk_transcribe.types.start_medical_scribe_job_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_start_medical_scribe_job(
