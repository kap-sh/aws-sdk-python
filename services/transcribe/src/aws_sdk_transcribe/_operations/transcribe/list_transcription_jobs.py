"""Generated from Smithy shape ``com.amazonaws.transcribe#ListTranscriptionJobs``."""
...
def handle_response(
    ...
    out: aws_sdk_transcribe.types.list_transcription_jobs_response.ListTranscriptionJobsResponse = aws_sdk_transcribe.types.list_transcription_jobs_response.deserialize_aws_json_1_1(
...
def build_request(
    ...
    input: aws_sdk_transcribe.types.list_transcription_jobs_request.ListTranscriptionJobsRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/transcriptionjobs"
    params: dict[str, str] = {}
    if "status" in input:
        params["Status"] = str(input["status"])
    if "job_name_contains" in input:
        params["JobNameContains"] = str(input["job_name_contains"])
...
async def async_list_transcription_jobs(
