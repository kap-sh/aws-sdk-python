"""Generated from Smithy shape ``com.amazonaws.xray#PutTelemetryRecords``."""
...
import capo_xray.types.put_telemetry_records_request
import capo_xray.types.put_telemetry_records_result
...
) -> capo_xray.types.put_telemetry_records_result.PutTelemetryRecordsResult:
    out: capo_xray.types.put_telemetry_records_result.PutTelemetryRecordsResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/TelemetryRecords"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.put_telemetry_records_request.serialize_json(input_)
    ).encode()
...
async def async_put_telemetry_records(
