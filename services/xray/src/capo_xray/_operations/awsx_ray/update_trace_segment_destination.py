"""Generated from Smithy shape ``com.amazonaws.xray#UpdateTraceSegmentDestination``."""
...
import capo_xray.types.update_trace_segment_destination_request
import capo_xray.types.update_trace_segment_destination_result
...
) -> capo_xray.types.update_trace_segment_destination_result.UpdateTraceSegmentDestinationResult:
    out: capo_xray.types.update_trace_segment_destination_result.UpdateTraceSegmentDestinationResult = capo_xray.types.update_trace_segment_destination_result.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_xray._auth._sigv4.build_sigv4_auth_scheme("xray", options.region)
        )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateTraceSegmentDestination"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.update_trace_segment_destination_request.serialize_json(input_)
    ).encode()
...
async def async_update_trace_segment_destination(
