"""Generated from Smithy shape ``com.amazonaws.xray#GetInsight``."""
...
import capo_xray.types.get_insight_request
import capo_xray.types.get_insight_result
...
) -> capo_xray.types.get_insight_result.GetInsightResult:
    out: capo_xray.types.get_insight_result.GetInsightResult = (
        capo_xray.types.get_insight_result.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_xray.types.get_insight_result.GetInsightResult = (
        capo_xray.types.get_insight_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/Insight"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.get_insight_request.serialize_json(input_)
    ).encode()
...
def get_insight(
    ...
    input_: capo_xray.types.get_insight_request.GetInsightRequest,
) -> tuple[capo_xray.types.get_insight_result.GetInsightResult, zapros.Response]:
