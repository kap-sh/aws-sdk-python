"""Generated from Smithy shape ``com.amazonaws.xray#GetGroup``."""
...
import capo_xray.types.get_group_request
import capo_xray.types.get_group_result
...
) -> capo_xray.types.get_group_result.GetGroupResult:
    out: capo_xray.types.get_group_result.GetGroupResult = (
        capo_xray.types.get_group_result.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_xray.types.get_group_result.GetGroupResult = (
        capo_xray.types.get_group_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetGroup"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_xray.types.get_group_request.serialize_json(input_)
    ).encode()
...
def get_group(
    options: OperationOptions, input_: capo_xray.types.get_group_request.GetGroupRequest
) -> tuple[capo_xray.types.get_group_result.GetGroupResult, zapros.Response]:
