...
class GraphLink(TypedDict, closed=True):
    reference_type: NotRequired["capo_xray.types.string.String"]
    """<p> Relationship of a trace to the corresponding service. </p>"""
    source_trace_id: NotRequired["capo_xray.types.string.String"]
    """<p> Source trace of a link relationship. </p>"""
    destination_trace_ids: NotRequired["capo_xray.types.trace_id_list.TraceIdList"]
...
def serialize_json(value: GraphLink) -> dict:
    ...
    if "destination_trace_ids" in value:
        ...
        out["DestinationTraceIds"] = capo_xray.types.trace_id_list.serialize_json(
