...
if TYPE_CHECKING:
    import capo_xray.types.indexing_rule_list
    import capo_xray.types.string


class GetIndexingRulesResult(TypedDict, closed=True):
    indexing_rules: NotRequired["capo_xray.types.indexing_rule_list.IndexingRuleList"]
...
def serialize_json(value: GetIndexingRulesResult) -> dict:
    ...
    if "indexing_rules" in value:
        ...
        out["IndexingRules"] = capo_xray.types.indexing_rule_list.serialize_json(
...
def deserialize_json(data: dict) -> GetIndexingRulesResult:
    ...
    if "IndexingRules" in data:
        ...
        out["indexing_rules"] = capo_xray.types.indexing_rule_list.deserialize_json(
            data["IndexingRules"]
        )
