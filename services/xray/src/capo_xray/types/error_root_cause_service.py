...
class ErrorRootCauseService(TypedDict, closed=True):
    ...
    entity_path: NotRequired[
        "capo_xray.types.error_root_cause_entity_path.ErrorRootCauseEntityPath"
    ]
...
def serialize_json(value: ErrorRootCauseService) -> dict:
    ...
    if "entity_path" in value:
        ...
        out["EntityPath"] = capo_xray.types.error_root_cause_entity_path.serialize_json(
            value["entity_path"]
        )
...
def deserialize_json(data: dict) -> ErrorRootCauseService:
    ...
    if "EntityPath" in data:
        ...
        out["entity_path"] = (
            capo_xray.types.error_root_cause_entity_path.deserialize_json(
                data["EntityPath"]
            )
        )
    if "Inferred" in data:
