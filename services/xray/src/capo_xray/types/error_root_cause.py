...
if TYPE_CHECKING:
    import capo_xray.types.error_root_cause_services
    import capo_xray.types.nullable_boolean


class ErrorRootCause(TypedDict, closed=True):
    services: NotRequired[
        "capo_xray.types.error_root_cause_services.ErrorRootCauseServices"
    ]
...
def serialize_json(value: ErrorRootCause) -> dict:
    ...
    if "services" in value:
        ...
        out["Services"] = capo_xray.types.error_root_cause_services.serialize_json(
            value["services"]
        )
...
def deserialize_json(data: dict) -> ErrorRootCause:
    ...
    if "Services" in data:
        ...
        out["services"] = capo_xray.types.error_root_cause_services.deserialize_json(
            data["Services"]
        )
