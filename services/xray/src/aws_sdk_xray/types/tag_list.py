...
TagList: TypeAlias = list["aws_sdk_xray.types.tag.Tag"]
...
def serialize_json(value: TagList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_xray.types.tag.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> TagList:
    ...
    out: TagList = []
    for item in data:
        out.append(aws_sdk_xray.types.tag.deserialize_json(item))
    return out
