...
class IndexingRule(TypedDict):
    ...
    rule: NotRequired["aws_sdk_xray.types.indexing_rule_value.IndexingRuleValue"]
...
def serialize_json(value: IndexingRule) -> dict:
    ...
    if "rule" in value:
        ...
        out["Rule"] = aws_sdk_xray.types.indexing_rule_value.serialize_json(
            value["rule"]
        )
    return out
...
def deserialize_json(data: dict) -> IndexingRule:
    ...
    if "Rule" in data:
        ...
        out["rule"] = aws_sdk_xray.types.indexing_rule_value.deserialize_json(
            data["Rule"]
        )
    return out
