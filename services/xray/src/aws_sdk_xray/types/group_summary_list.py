...
GroupSummaryList: TypeAlias = list["aws_sdk_xray.types.group_summary.GroupSummary"]
...
def serialize_json(value: GroupSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_xray.types.group_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> GroupSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_xray.types.group_summary.deserialize_json(item))
    return out
