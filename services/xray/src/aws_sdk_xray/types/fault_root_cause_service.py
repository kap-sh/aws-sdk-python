...
class FaultRootCauseService(TypedDict):
    ...
    entity_path: NotRequired[
        "aws_sdk_xray.types.fault_root_cause_entity_path.FaultRootCauseEntityPath"
    ]
...
def serialize_json(value: FaultRootCauseService) -> dict:
    ...
    if "entity_path" in value:
        ...
        out["EntityPath"] = (
            aws_sdk_xray.types.fault_root_cause_entity_path.serialize_json(
                value["entity_path"]
            )
        )
...
def deserialize_json(data: dict) -> FaultRootCauseService:
    ...
    if "EntityPath" in data:
        ...
        out["entity_path"] = (
            aws_sdk_xray.types.fault_root_cause_entity_path.deserialize_json(
                data["EntityPath"]
            )
        )
    if "Inferred" in data:
