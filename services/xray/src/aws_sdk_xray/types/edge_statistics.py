...
if TYPE_CHECKING:
    import aws_sdk_xray.types.error_statistics
    import aws_sdk_xray.types.fault_statistics
    import aws_sdk_xray.types.nullable_double
    import aws_sdk_xray.types.nullable_long


class EdgeStatistics(TypedDict):
    ok_count: NotRequired["aws_sdk_xray.types.nullable_long.NullableLong"]
    """<p>The number of requests that completed with a 2xx Success status code.</p>"""
    error_statistics: NotRequired["aws_sdk_xray.types.error_statistics.ErrorStatistics"]
    """<p>Information about requests that failed with a 4xx Client Error status code.</p>"""
    fault_statistics: NotRequired["aws_sdk_xray.types.fault_statistics.FaultStatistics"]
    """<p>Information about requests that failed with a 5xx Server Error status code.</p>"""
    total_count: NotRequired["aws_sdk_xray.types.nullable_long.NullableLong"]
    """<p>The total number of completed requests.</p>"""
    total_response_time: NotRequired[
...
def serialize_json(value: EdgeStatistics) -> dict:
    ...
    if "error_statistics" in value:
        ...
        out["ErrorStatistics"] = aws_sdk_xray.types.error_statistics.serialize_json(
            value["error_statistics"]
        )
    if "fault_statistics" in value:
        import aws_sdk_xray.types.fault_statistics

        out["FaultStatistics"] = aws_sdk_xray.types.fault_statistics.serialize_json(
...
def deserialize_json(data: dict) -> EdgeStatistics:
    ...
    if "ErrorStatistics" in data:
        ...
        out["error_statistics"] = aws_sdk_xray.types.error_statistics.deserialize_json(
            data["ErrorStatistics"]
        )
    if "FaultStatistics" in data:
        import aws_sdk_xray.types.fault_statistics

        out["fault_statistics"] = aws_sdk_xray.types.fault_statistics.deserialize_json(
