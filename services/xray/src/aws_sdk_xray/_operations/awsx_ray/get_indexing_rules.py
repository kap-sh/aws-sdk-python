"""Generated from Smithy shape ``com.amazonaws.xray#GetIndexingRules``."""
...
def handle_response(
    ...
    out: aws_sdk_xray.types.get_indexing_rules_result.GetIndexingRulesResult = (
        aws_sdk_xray.types.get_indexing_rules_result.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_xray.types.get_indexing_rules_request.GetIndexingRulesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetIndexingRules"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_xray.types.get_indexing_rules_request

    body: bytes | None = json.dumps(
        aws_sdk_xray.types.get_indexing_rules_request.serialize_json(input_)
    ).encode()
...
def get_indexing_rules(
    ...
    aws_sdk_xray.types.get_indexing_rules_result.GetIndexingRulesResult, zapros.Response
]:
...
async def async_get_indexing_rules(
