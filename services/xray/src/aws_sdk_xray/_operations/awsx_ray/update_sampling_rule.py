"""Generated from Smithy shape ``com.amazonaws.xray#UpdateSamplingRule``."""
...
def handle_response(
    ...
    out: aws_sdk_xray.types.update_sampling_rule_result.UpdateSamplingRuleResult = (
        aws_sdk_xray.types.update_sampling_rule_result.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_xray.types.update_sampling_rule_request.UpdateSamplingRuleRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateSamplingRule"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_xray.types.update_sampling_rule_request.serialize_json(input_)
    ).encode()
...
async def async_update_sampling_rule(
