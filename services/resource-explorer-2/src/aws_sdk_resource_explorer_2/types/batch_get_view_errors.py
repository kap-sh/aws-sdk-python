...
BatchGetViewErrors: TypeAlias = list[
    "aws_sdk_resource_explorer_2.types.batch_get_view_error.BatchGetViewError"
]
...
def serialize_json(value: BatchGetViewErrors) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_resource_explorer_2.types.batch_get_view_error.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BatchGetViewErrors:
    ...
    for item in data:
        out.append(
            aws_sdk_resource_explorer_2.types.batch_get_view_error.deserialize_json(
                item
            )
        )
    return out
