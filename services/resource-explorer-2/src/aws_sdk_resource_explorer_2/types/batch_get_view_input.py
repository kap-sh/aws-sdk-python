...
class BatchGetViewInput(TypedDict):
    view_arns: NotRequired["aws_sdk_resource_explorer_2.types.view_arn_list.ViewArnList"]
...
def serialize_json(value: BatchGetViewInput) -> dict:
    ...
    if "view_arns" in value:
        import aws_sdk_resource_explorer_2.types.view_arn_list
        out["ViewArns"] = aws_sdk_resource_explorer_2.types.view_arn_list.serialize_json(value["view_arns"])
    return out
...
def deserialize_json(data: dict) -> BatchGetViewInput:
    ...
    if "ViewArns" in data:
        import aws_sdk_resource_explorer_2.types.view_arn_list
        out["view_arns"] = aws_sdk_resource_explorer_2.types.view_arn_list.deserialize_json(data["ViewArns"])
    return out