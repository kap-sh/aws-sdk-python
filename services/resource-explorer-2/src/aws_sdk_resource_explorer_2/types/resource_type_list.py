...
ResourceTypeList: TypeAlias = list[
    "aws_sdk_resource_explorer_2.types.supported_resource_type.SupportedResourceType"
]
...
def serialize_json(value: ResourceTypeList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_resource_explorer_2.types.supported_resource_type.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ResourceTypeList:
    ...
    for item in data:
        out.append(
            aws_sdk_resource_explorer_2.types.supported_resource_type.deserialize_json(
                item
            )
        )
    return out
