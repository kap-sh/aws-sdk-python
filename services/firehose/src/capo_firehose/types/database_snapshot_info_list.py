...
if TYPE_CHECKING:
    import capo_firehose.types.database_snapshot_info

DatabaseSnapshotInfoList: TypeAlias = list[
    "capo_firehose.types.database_snapshot_info.DatabaseSnapshotInfo"
]
...
def serialize_aws_json_1_1(value: DatabaseSnapshotInfoList) -> list:
    ...
    for item in value:
        out.append(
            capo_firehose.types.database_snapshot_info.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> DatabaseSnapshotInfoList:
    ...
    for item in data:
        out.append(
            capo_firehose.types.database_snapshot_info.deserialize_aws_json_1_1(item)
        )
    return out
