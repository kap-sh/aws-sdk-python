...
if TYPE_CHECKING:
    import capo_firehose.types.database_include_or_exclude_list


class DatabaseList(TypedDict, closed=True):
    include: NotRequired[
        "capo_firehose.types.database_include_or_exclude_list.DatabaseIncludeOrExcludeList"
    ]
...
def serialize_aws_json_1_1(value: DatabaseList) -> dict:
    ...
    if "include" in value:
        ...
        out["Include"] = (
            capo_firehose.types.database_include_or_exclude_list.serialize_aws_json_1_1(
                value["include"]
            )
        )
    if "exclude" in value:
...
def deserialize_aws_json_1_1(data: dict) -> DatabaseList:
    ...
    if "Include" in data:
        ...
        out["include"] = (
            capo_firehose.types.database_include_or_exclude_list.deserialize_aws_json_1_1(
                data["Include"]
            )
        )
    if "Exclude" in data:
