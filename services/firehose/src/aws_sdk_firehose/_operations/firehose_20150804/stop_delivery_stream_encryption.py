"""Generated from Smithy shape ``com.amazonaws.firehose#StopDeliveryStreamEncryption``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            import aws_sdk_firehose.errors.resource_not_found_exception

            raise aws_sdk_firehose.errors.resource_not_found_exception.ResourceNotFoundException.from_aws_json_1_1(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_firehose.types.stop_delivery_stream_encryption_output.StopDeliveryStreamEncryptionOutput = {}  # type: ignore[typeddict-item]
    return out
...
def get_signer(
...
) -> aws_sdk_firehose._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_firehose._auth._sigv4.build_sigv4_auth_scheme(
                "firehose", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_firehose._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_firehose.types.stop_delivery_stream_encryption_input.StopDeliveryStreamEncryptionInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_firehose.types.stop_delivery_stream_encryption_input.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_stop_delivery_stream_encryption(
