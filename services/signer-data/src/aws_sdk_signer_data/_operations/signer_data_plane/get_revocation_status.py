...
from aws_sdk_signer_data.errors import UnknownServiceError
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            ...
            raise aws_sdk_signer_data.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
        case "InternalServiceErrorException":
            import aws_sdk_signer_data.errors.internal_service_error_exception

            raise aws_sdk_signer_data.errors.internal_service_error_exception.InternalServiceErrorException.from_json(
                data
            )
        case "TooManyRequestsException":
            import aws_sdk_signer_data.errors.too_many_requests_exception

            raise aws_sdk_signer_data.errors.too_many_requests_exception.TooManyRequestsException.from_json(
                data
            )
        case "ValidationException":
            import aws_sdk_signer_data.errors.validation_exception

            raise aws_sdk_signer_data.errors.validation_exception.ValidationException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_signer_data.types.get_revocation_status_response.GetRevocationStatusResponse = aws_sdk_signer_data.types.get_revocation_status_response.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_signer_data._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_signer_data._auth._sigv4.build_sigv4_auth_scheme(
                "signer", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_signer_data._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_signer_data.types.get_revocation_status_request.GetRevocationStatusRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
