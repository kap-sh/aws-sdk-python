"""Generated from Smithy shape ``com.amazonaws.appconfig#StopDeployment``."""
...
def handle_response(
    ...
    out: capo_appconfig.types.deployment.Deployment = (
        capo_appconfig.types.deployment.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_appconfig.types.deployment.Deployment = (
        capo_appconfig.types.deployment.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{DeploymentNumber}", quote(str(input_["deployment_number"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "allow_revert" in input_:
        headers["Allow-Revert"] = str(input_["allow_revert"])
    body: bytes | None = b""
...
def stop_deployment(
    ...
    input_: capo_appconfig.types.stop_deployment_request.StopDeploymentRequest,
) -> tuple[capo_appconfig.types.deployment.Deployment, zapros.Response]:
...
async def async_stop_deployment(
