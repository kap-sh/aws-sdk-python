...
class ExtensionAssociations(TypedDict, closed=True):
    items: NotRequired[
        "capo_appconfig.types.extension_association_summaries.ExtensionAssociationSummaries"
    ]
...
def serialize_json(value: ExtensionAssociations) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = (
            capo_appconfig.types.extension_association_summaries.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ExtensionAssociations:
    ...
    if "Items" in data:
        ...
        out["items"] = (
            capo_appconfig.types.extension_association_summaries.deserialize_json(
                data["Items"]
            )
        )
