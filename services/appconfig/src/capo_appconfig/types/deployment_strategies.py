...
class DeploymentStrategies(TypedDict, closed=True):
    items: NotRequired[
        "capo_appconfig.types.deployment_strategy_list.DeploymentStrategyList"
    ]
...
def serialize_json(value: DeploymentStrategies) -> dict:
    ...
    if "items" in value:
        ...
        out["Items"] = capo_appconfig.types.deployment_strategy_list.serialize_json(
            value["items"]
        )
...
def deserialize_json(data: dict) -> DeploymentStrategies:
    ...
    if "Items" in data:
        ...
        out["items"] = capo_appconfig.types.deployment_strategy_list.deserialize_json(
            data["Items"]
        )
