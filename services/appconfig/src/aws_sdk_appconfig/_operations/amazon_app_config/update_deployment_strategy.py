...
def build_request(
    ...
    input_: aws_sdk_appconfig.types.update_deployment_strategy_request.UpdateDeploymentStrategyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{DeploymentStrategyId}", quote(str(input_["deployment_strategy_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_appconfig.types.update_deployment_strategy_request.serialize_json(
            input_
        )
    ).encode()
