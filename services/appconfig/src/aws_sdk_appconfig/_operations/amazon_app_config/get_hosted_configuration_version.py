"""Generated from Smithy shape ``com.amazonaws.appconfig#GetHostedConfigurationVersion``."""
...
def handle_response(
    ...
    out: aws_sdk_appconfig.types.hosted_configuration_version.HostedConfigurationVersion = {
        "content": aws_sdk_appconfig.types.blob.deserialize_json(
            json.loads(response.read())
        )
    }  # type: ignore[typeddict-item]
    if "Application-Id" in response.headers:
        out["application_id"] = str(response.headers["Application-Id"])
    if "Configuration-Profile-Id" in response.headers:
        out["configuration_profile_id"] = str(
            response.headers["Configuration-Profile-Id"]
        )
    out["version_number"] = int(response.headers["Version-Number"])
    if "Description" in response.headers:
        out["description"] = str(response.headers["Description"])
    if "Content-Type" in response.headers:
        out["content_type"] = str(response.headers["Content-Type"])
    if "VersionLabel" in response.headers:
        out["version_label"] = str(response.headers["VersionLabel"])
    if "KmsKeyArn" in response.headers:
        out["kms_key_arn"] = str(response.headers["KmsKeyArn"])
    return out
...
def build_request(
    ...
    input_: aws_sdk_appconfig.types.get_hosted_configuration_version_request.GetHostedConfigurationVersionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
...
async def async_get_hosted_configuration_version(
