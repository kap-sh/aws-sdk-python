...
def build_request(
    ...
    input: aws_sdk_appconfig.types.stop_deployment_request.StopDeploymentRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{DeploymentNumber}", quote(str(input["deployment_number"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "allow_revert" in input:
        headers["Allow-Revert"] = str(input["allow_revert"])
    body: bytes | None = b""
