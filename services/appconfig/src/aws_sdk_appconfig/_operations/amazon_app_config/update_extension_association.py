"""Generated from Smithy shape ``com.amazonaws.appconfig#UpdateExtensionAssociation``."""
...
def handle_response(
    ...
    out: aws_sdk_appconfig.types.extension_association.ExtensionAssociation = (
        aws_sdk_appconfig.types.extension_association.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_appconfig.types.update_extension_association_request.UpdateExtensionAssociationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/extensionassociations/{ExtensionAssociationId}"
    url = url.replace(
        "{ExtensionAssociationId}",
        quote(str(input_["extension_association_id"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_appconfig.types.update_extension_association_request.serialize_json(
            input_
        )
    ).encode()
...
def update_extension_association(
    ...
    aws_sdk_appconfig.types.extension_association.ExtensionAssociation, zapros.Response
]:
...
async def async_update_extension_association(
