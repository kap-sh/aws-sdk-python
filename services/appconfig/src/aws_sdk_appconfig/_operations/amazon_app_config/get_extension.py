"""Generated from Smithy shape ``com.amazonaws.appconfig#GetExtension``."""
...
def handle_response(
    ...
    out: aws_sdk_appconfig.types.extension.Extension = (
        aws_sdk_appconfig.types.extension.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: aws_sdk_appconfig.types.extension.Extension = (
        aws_sdk_appconfig.types.extension.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_appconfig.types.get_extension_request.GetExtensionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/extensions/{ExtensionIdentifier}"
    url = url.replace(
        "{ExtensionIdentifier}", quote(str(input_["extension_identifier"]), safe="")
    )
    params: dict[str, str] = {}
    if "version_number" in input_:
        params["version_number"] = str(input_["version_number"])
...
def get_extension(
    ...
    input_: aws_sdk_appconfig.types.get_extension_request.GetExtensionRequest,
) -> tuple[aws_sdk_appconfig.types.extension.Extension, zapros.Response]:
