...
class ACMPCAError(Exception):
    """Base class for all ACMPCA service errors."""


class ServiceError(ACMPCAError):
...
class SerializationError(ACMPCAError):
    """Raised when a value cannot be serialized."""


class DeserializationError(ACMPCAError):
...
class WaiterFailedError(ACMPCAError):
...
class WaiterTimeoutError(ACMPCAError):
