...
class EKSAuthError(Exception):
    """Base class for all EKSAuth service errors."""


class ServiceError(EKSAuthError):
...
class SerializationError(EKSAuthError):
    """Raised when a value cannot be serialized."""


class DeserializationError(EKSAuthError):
...
class WaiterFailedError(EKSAuthError):
...
class WaiterTimeoutError(EKSAuthError):
