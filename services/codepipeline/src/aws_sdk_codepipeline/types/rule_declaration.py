...
if TYPE_CHECKING:
    ...
    import aws_sdk_codepipeline.types.rule_timeout
    import aws_sdk_codepipeline.types.rule_type_id


class RuleDeclaration(TypedDict):
    name: "aws_sdk_codepipeline.types.rule_name.RuleName"
    """<p>The name of the rule that is created for the condition, such as <code>VariableCheck</code>.</p>"""
    rule_type_id: "aws_sdk_codepipeline.types.rule_type_id.RuleTypeId"
    ...
    timeout_in_minutes: NotRequired[
        "aws_sdk_codepipeline.types.rule_timeout.RuleTimeout"
    ]
...
def serialize_aws_json_1_1(value: RuleDeclaration) -> dict:
    ...
    out["ruleTypeId"] = aws_sdk_codepipeline.types.rule_type_id.serialize_aws_json_1_1(
        value["rule_type_id"]
    )
