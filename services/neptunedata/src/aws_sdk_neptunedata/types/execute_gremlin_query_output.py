...
if TYPE_CHECKING:
    import aws_sdk_neptunedata.types.gremlin_query_status_attributes

class ExecuteGremlinQueryOutput(TypedDict):
    request_id: NotRequired["str"]
    """<p>The unique identifier of the Gremlin query.</p>"""
    status: NotRequired["aws_sdk_neptunedata.types.gremlin_query_status_attributes.GremlinQueryStatusAttributes"]
...
def serialize_json(value: ExecuteGremlinQueryOutput) -> dict:
    ...
    if "status" in value:
        import aws_sdk_neptunedata.types.gremlin_query_status_attributes
        out["status"] = aws_sdk_neptunedata.types.gremlin_query_status_attributes.serialize_json(value["status"])
    if "result" in value:
...
def deserialize_json(data: dict) -> ExecuteGremlinQueryOutput:
    ...
    if "status" in data:
        import aws_sdk_neptunedata.types.gremlin_query_status_attributes
        out["status"] = aws_sdk_neptunedata.types.gremlin_query_status_attributes.deserialize_json(data["status"])
    if "result" in data:
