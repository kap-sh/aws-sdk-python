...
class ExecuteOpenCypherExplainQueryInput(TypedDict):
...
def serialize_json(value: ExecuteOpenCypherExplainQueryInput) -> dict:
    ...
    import aws_sdk_neptunedata.types.open_cypher_explain_mode
    out["explain"] = aws_sdk_neptunedata.types.open_cypher_explain_mode.serialize_json(value["explain_mode"])
    return out
...
def deserialize_json(data: dict) -> ExecuteOpenCypherExplainQueryInput:
    ...
    if "query" in data:
        ...
        raise DeserializationError("ExecuteOpenCypherExplainQueryInput.open_cypher_query required")
    if "parameters" in data:
        out["parameters"] = data["parameters"]
    if "explain" in data:
        import aws_sdk_neptunedata.types.open_cypher_explain_mode
        out["explain_mode"] = aws_sdk_neptunedata.types.open_cypher_explain_mode.deserialize_json(data["explain"])
    else:
        raise DeserializationError("ExecuteOpenCypherExplainQueryInput.explain_mode required")
    return out