...
def deserialize_json(data: dict) -> ConcurrentModificationException_:
    ...
    if "detailedMessage" in data:
        ...
        raise DeserializationError("ConcurrentModificationException_.detailed_message required")
    if "requestId" in data:
        out["request_id"] = data["requestId"]
    else:
        raise DeserializationError("ConcurrentModificationException_.request_id required")
    if "code" in data:
        out["code"] = data["code"]
    else:
...
class ConcurrentModificationException(ServiceError):
    ...
    def __init__(self, data: ConcurrentModificationException_):
        super().__init__('server', is_throttling_error=False, is_retryable=True, code='ConcurrentModificationException')
        self.data = data
