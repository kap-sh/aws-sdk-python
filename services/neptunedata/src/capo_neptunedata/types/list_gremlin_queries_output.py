...
class ListGremlinQueriesOutput(TypedDict, closed=True):
    ...
    queries: NotRequired["capo_neptunedata.types.gremlin_queries.GremlinQueries"]
...
def serialize_json(value: ListGremlinQueriesOutput) -> dict:
    ...
    if "queries" in value:
        ...
        out["queries"] = capo_neptunedata.types.gremlin_queries.serialize_json(
            value["queries"]
        )
    return out
...
def deserialize_json(data: dict) -> ListGremlinQueriesOutput:
    ...
    if "queries" in data:
        ...
        out["queries"] = capo_neptunedata.types.gremlin_queries.deserialize_json(
            data["queries"]
        )
    return out
