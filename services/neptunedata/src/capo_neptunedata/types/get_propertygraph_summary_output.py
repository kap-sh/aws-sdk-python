...
class GetPropertygraphSummaryOutput(TypedDict, closed=True):
    status_code: NotRequired["int"]
    """<p>The HTTP return code of the request. If the request succeeded, the code is 200.</p>"""
    payload: NotRequired[
        "capo_neptunedata.types.propertygraph_summary_value_map.PropertygraphSummaryValueMap"
    ]
...
def serialize_json(value: GetPropertygraphSummaryOutput) -> dict:
    ...
    if "payload" in value:
        ...
        out["payload"] = (
            capo_neptunedata.types.propertygraph_summary_value_map.serialize_json(
                value["payload"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GetPropertygraphSummaryOutput:
    ...
    if "payload" in data:
        ...
        out["payload"] = (
            capo_neptunedata.types.propertygraph_summary_value_map.deserialize_json(
                data["payload"]
            )
        )
    return out
