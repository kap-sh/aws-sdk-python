...
class GetSparqlStreamOutput(TypedDict, closed=True):
    last_event_id: "capo_neptunedata.types.string_valued_map.StringValuedMap"
    ...
    records: "capo_neptunedata.types.sparql_records_list.SparqlRecordsList"
...
def serialize_json(value: GetSparqlStreamOutput) -> dict:
    ...
    out["lastEventId"] = capo_neptunedata.types.string_valued_map.serialize_json(
        value["last_event_id"]
    )
    ...
    out["records"] = capo_neptunedata.types.sparql_records_list.serialize_json(
        value["records"]
    )
