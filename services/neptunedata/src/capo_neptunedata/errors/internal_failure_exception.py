"""Generated from Smithy shape ``com.amazonaws.neptunedata#InternalFailureException``."""
...
def deserialize_json(data: dict) -> InternalFailureException_:
    ...
    if "detailedMessage" in data:
        ...
        raise DeserializationError(
            "InternalFailureException_.detailed_message required"
        )
    if "requestId" in data:
        out["request_id"] = data["requestId"]
    else:
        raise DeserializationError("InternalFailureException_.request_id required")
    if "code" in data:
        out["code"] = data["code"]
    else:
        raise DeserializationError("InternalFailureException_.code required")
    return out


class InternalFailureException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.neptunedata#InternalFailureException``."""
