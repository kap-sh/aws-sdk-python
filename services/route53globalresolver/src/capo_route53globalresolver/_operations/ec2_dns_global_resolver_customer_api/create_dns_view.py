"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#CreateDNSView``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.create_dns_view_output.CreateDNSViewOutput = (
        capo_route53globalresolver.types.create_dns_view_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/dns-views/{globalResolverId}"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.create_dns_view_input.serialize_json(input_)
    ).encode()
...
async def async_create_dns_view(
