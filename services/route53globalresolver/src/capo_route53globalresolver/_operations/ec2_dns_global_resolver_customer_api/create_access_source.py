"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#CreateAccessSource``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.create_access_source_output.CreateAccessSourceOutput = capo_route53globalresolver.types.create_access_source_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/access-sources"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.create_access_source_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_access_source(
