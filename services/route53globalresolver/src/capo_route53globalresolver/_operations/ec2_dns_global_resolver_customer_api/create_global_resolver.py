"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#CreateGlobalResolver``."""
...
def handle_response(
    ...
    out: capo_route53globalresolver.types.create_global_resolver_output.CreateGlobalResolverOutput = capo_route53globalresolver.types.create_global_resolver_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/global-resolver"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_route53globalresolver.types.create_global_resolver_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_global_resolver(
