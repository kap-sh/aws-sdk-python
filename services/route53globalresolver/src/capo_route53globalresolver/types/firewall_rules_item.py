"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#FirewallRulesItem``."""
...
class FirewallRulesItem(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: FirewallRulesItem) -> dict:
    out: dict = {}
    import capo_route53globalresolver.types.firewall_rule_action
...
def deserialize_json(data: dict) -> FirewallRulesItem:
    out: FirewallRulesItem = {}  # type: ignore[typeddict-item]
    if "action" in data:
        import capo_route53globalresolver.types.firewall_rule_action
        ...
        raise DeserializationError("FirewallRulesItem.action required")
    ...
    if "createdAt" in data:
        ...
        raise DeserializationError("FirewallRulesItem.created_at required")
    ...
    if "id" in data:
        ...
        raise DeserializationError("FirewallRulesItem.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("FirewallRulesItem.name required")
    if "priority" in data:
        out["priority"] = data["priority"]
    else:
        raise DeserializationError("FirewallRulesItem.priority required")
    if "dnsViewId" in data:
        out["dns_view_id"] = data["dnsViewId"]
    else:
        raise DeserializationError("FirewallRulesItem.dns_view_id required")
    ...
    if "status" in data:
        ...
        raise DeserializationError("FirewallRulesItem.status required")
    if "updatedAt" in data:
        ...
        raise DeserializationError("FirewallRulesItem.updated_at required")
    return out
