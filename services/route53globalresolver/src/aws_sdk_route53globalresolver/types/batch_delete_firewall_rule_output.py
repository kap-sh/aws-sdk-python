...
def serialize_json(value: BatchDeleteFirewallRuleOutput) -> dict:
    ...
    out["failures"] = (
        aws_sdk_route53globalresolver.types.batch_delete_firewall_rule_output_items.serialize_json(
            value["failures"]
        )
    )
    import aws_sdk_route53globalresolver.types.batch_delete_firewall_rule_output_items
...
def deserialize_json(data: dict) -> BatchDeleteFirewallRuleOutput:
    ...
    if "failures" in data:
        ...
        out["failures"] = (
            aws_sdk_route53globalresolver.types.batch_delete_firewall_rule_output_items.deserialize_json(
                data["failures"]
            )
        )
    else:
