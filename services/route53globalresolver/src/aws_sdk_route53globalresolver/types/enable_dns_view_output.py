"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#EnableDNSViewOutput``."""
...
class EnableDNSViewOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: EnableDNSViewOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["arn"] = value["arn"]
...
def deserialize_json(data: dict) -> EnableDNSViewOutput:
    out: EnableDNSViewOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("EnableDNSViewOutput.id required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("EnableDNSViewOutput.arn required")
    ...
    if "dnssecValidation" in data:
        ...
        raise DeserializationError("EnableDNSViewOutput.dnssec_validation required")
    if "ednsClientSubnet" in data:
        ...
        raise DeserializationError("EnableDNSViewOutput.edns_client_subnet required")
    if "firewallRulesFailOpen" in data:
        ...
        raise DeserializationError(
            "EnableDNSViewOutput.firewall_rules_fail_open required"
        )
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("EnableDNSViewOutput.name required")
    if "description" in data:
        out["description"] = data["description"]
    if "globalResolverId" in data:
        out["global_resolver_id"] = data["globalResolverId"]
    else:
        raise DeserializationError("EnableDNSViewOutput.global_resolver_id required")
    if "createdAt" in data:
        ...
        raise DeserializationError("EnableDNSViewOutput.created_at required")
    if "updatedAt" in data:
        ...
        raise DeserializationError("EnableDNSViewOutput.updated_at required")
    if "status" in data:
        import aws_sdk_route53globalresolver.types.profile_resource_status
        ...
        raise DeserializationError("EnableDNSViewOutput.status required")
    return out
