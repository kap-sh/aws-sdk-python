"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#BatchUpdateFirewallRule``."""
...
def handle_response(
    ...
    out: aws_sdk_route53globalresolver.types.batch_update_firewall_rule_output.BatchUpdateFirewallRuleOutput = aws_sdk_route53globalresolver.types.batch_update_firewall_rule_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53globalresolver.types.batch_update_firewall_rule_input.BatchUpdateFirewallRuleInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/firewall-rules/batch-update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_route53globalresolver.types.batch_update_firewall_rule_input

    body: bytes | None = json.dumps(
        aws_sdk_route53globalresolver.types.batch_update_firewall_rule_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_update_firewall_rule(
