"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#DeleteAccessSource``."""
...
def handle_response(
    ...
    out: aws_sdk_route53globalresolver.types.delete_access_source_output.DeleteAccessSourceOutput = aws_sdk_route53globalresolver.types.delete_access_source_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53globalresolver.types.delete_access_source_input.DeleteAccessSourceInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/access-sources/{accessSourceId}"
    url = url.replace(
        "{accessSourceId}", quote(str(input_["access_source_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_delete_access_source(
