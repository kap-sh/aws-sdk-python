"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#UpdateFirewallRule``."""
...
def handle_response(
    ...
    out: aws_sdk_route53globalresolver.types.update_firewall_rule_output.UpdateFirewallRuleOutput = aws_sdk_route53globalresolver.types.update_firewall_rule_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53globalresolver.types.update_firewall_rule_input.UpdateFirewallRuleInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/firewall-rules/{firewallRuleId}"
    url = url.replace(
        "{firewallRuleId}", quote(str(input_["firewall_rule_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_route53globalresolver.types.update_firewall_rule_input.serialize_json(
            input_
        )
    ).encode()
