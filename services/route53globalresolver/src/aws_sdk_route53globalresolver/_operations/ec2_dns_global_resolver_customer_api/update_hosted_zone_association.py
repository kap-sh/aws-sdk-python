"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#UpdateHostedZoneAssociation``."""
...
def handle_response(
    ...
    out: aws_sdk_route53globalresolver.types.update_hosted_zone_association_output.UpdateHostedZoneAssociationOutput = aws_sdk_route53globalresolver.types.update_hosted_zone_association_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_route53globalresolver.types.update_hosted_zone_association_input.UpdateHostedZoneAssociationInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/hosted-zone-associations/{hostedZoneAssociationId}"
    )
    url = url.replace(
        "{hostedZoneAssociationId}",
        quote(str(input_["hosted_zone_association_id"]), safe=""),
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_route53globalresolver.types.update_hosted_zone_association_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_hosted_zone_association(
