"""Generated from Smithy shape ``com.amazonaws.route53globalresolver#UpdateAccessToken``."""
...
def handle_response(
    ...
    out: aws_sdk_route53globalresolver.types.update_access_token_output.UpdateAccessTokenOutput = aws_sdk_route53globalresolver.types.update_access_token_output.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_route53globalresolver.types.update_access_token_input.UpdateAccessTokenInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/tokens/{accessTokenId}"
    url = url.replace("{accessTokenId}", quote(str(input["access_token_id"]), safe=""))
    params: dict[str, str] = {}
...
async def async_update_access_token(
