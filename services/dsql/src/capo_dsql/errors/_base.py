...
class DSQLError(Exception):
    """Base class for all DSQL service errors."""


class ServiceError(DSQLError):
...
class SerializationError(DSQLError):
    """Raised when a value cannot be serialized."""


class DeserializationError(DSQLError):
...
class WaiterFailedError(DSQLError):
...
class WaiterTimeoutError(DSQLError):
