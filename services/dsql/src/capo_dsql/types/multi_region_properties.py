...
if TYPE_CHECKING:
    ...
    import capo_dsql.types.region


class MultiRegionProperties(TypedDict, closed=True):
    witness_region: NotRequired["capo_dsql.types.region.Region"]
    ...
    clusters: NotRequired["capo_dsql.types.cluster_arn_list.ClusterArnList"]
...
def serialize_json(value: MultiRegionProperties) -> dict:
    ...
    if "clusters" in value:
        ...
        out["clusters"] = capo_dsql.types.cluster_arn_list.serialize_json(
            value["clusters"]
        )
    return out
