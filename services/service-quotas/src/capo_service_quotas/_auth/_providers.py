...
from capo_service_quotas._auth._identity import (
    Credentials,
    Identity,
)
from capo_service_quotas._services._aws_config import (
    _load_profile,
    active_profile,
    config_file,
)
...
# they need the `sso` extra: `pip install capo-service-quotas[sso]`. Without it the imports
...
def require_dependency(module: object | None, package: str) -> None:
    if module is None:
        raise MissingDependencyError(
            ...
            "installed; reinstall with the sso feature enabled: capo-service-quotas[sso]"
        )
