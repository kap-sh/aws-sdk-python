...
if TYPE_CHECKING:
    ...
    import capo_service_quotas.types.report_message
    import capo_service_quotas.types.report_status


class StartQuotaUtilizationReportResponse(TypedDict, closed=True):
    report_id: NotRequired["capo_service_quotas.types.report_id.ReportId"]
    ...
    status: NotRequired["capo_service_quotas.types.report_status.ReportStatus"]
    """<p>The current status of the report generation. The status will be <code>PENDING</code> when the report is first initiated.</p>"""
    message: NotRequired["capo_service_quotas.types.report_message.ReportMessage"]
...
def serialize_aws_json_1_1(value: StartQuotaUtilizationReportResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_service_quotas.types.report_status.serialize_aws_json_1_1(
            value["status"]
        )
    if "message" in value:
...
def deserialize_aws_json_1_1(data: dict) -> StartQuotaUtilizationReportResponse:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            capo_service_quotas.types.report_status.deserialize_aws_json_1_1(
                data["Status"]
            )
        )
    if "Message" in data:
