"""Generated from Smithy shape ``com.amazonaws.lakeformation#UpdateDataCellsFilter``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.update_data_cells_filter_response.UpdateDataCellsFilterResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateDataCellsFilter"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.update_data_cells_filter_request.serialize_json(input_)
    ).encode()
...
async def async_update_data_cells_filter(
