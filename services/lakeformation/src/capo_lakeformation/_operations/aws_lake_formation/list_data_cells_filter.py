"""Generated from Smithy shape ``com.amazonaws.lakeformation#ListDataCellsFilter``."""
...
import capo_lakeformation.types.data_cells_filter_list
...
def handle_response(
    ...
    out: capo_lakeformation.types.list_data_cells_filter_response.ListDataCellsFilterResponse = capo_lakeformation.types.list_data_cells_filter_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListDataCellsFilter"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.list_data_cells_filter_request.serialize_json(input_)
    ).encode()
...
async def async_list_data_cells_filter(
