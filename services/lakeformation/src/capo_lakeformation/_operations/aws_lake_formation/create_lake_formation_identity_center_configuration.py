"""Generated from Smithy shape ``com.amazonaws.lakeformation#CreateLakeFormationIdentityCenterConfiguration``."""
...
import capo_lakeformation.errors.already_exists_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "AlreadyExistsException":
            raise capo_lakeformation.errors.already_exists_exception.AlreadyExistsException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_lakeformation.types.create_lake_formation_identity_center_configuration_response.CreateLakeFormationIdentityCenterConfigurationResponse = capo_lakeformation.types.create_lake_formation_identity_center_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateLakeFormationIdentityCenterConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.create_lake_formation_identity_center_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_lake_formation_identity_center_configuration(
