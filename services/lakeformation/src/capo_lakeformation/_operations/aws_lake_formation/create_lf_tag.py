"""Generated from Smithy shape ``com.amazonaws.lakeformation#CreateLFTag``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.create_lf_tag_response.CreateLFTagResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateLFTag"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.create_lf_tag_request.serialize_json(input_)
    ).encode()
...
def create_lf_tag(
    ...
    capo_lakeformation.types.create_lf_tag_response.CreateLFTagResponse, zapros.Response
]:
