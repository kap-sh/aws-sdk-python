"""Generated from Smithy shape ``com.amazonaws.lakeformation#DescribeResource``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.describe_resource_response.DescribeResourceResponse = capo_lakeformation.types.describe_resource_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DescribeResource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.describe_resource_request.serialize_json(input_)
    ).encode()
