"""Generated from Smithy shape ``com.amazonaws.lakeformation#SearchTablesByLFTags``."""
...
import capo_lakeformation.types.table_lf_tags_list
...
def handle_response(
    ...
    out: capo_lakeformation.types.search_tables_by_lf_tags_response.SearchTablesByLFTagsResponse = capo_lakeformation.types.search_tables_by_lf_tags_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/SearchTablesByLFTags"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.search_tables_by_lf_tags_request.serialize_json(input_)
    ).encode()
...
async def async_search_tables_by_lf_tags(
