"""Generated from Smithy shape ``com.amazonaws.lakeformation#CancelTransaction``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.cancel_transaction_response.CancelTransactionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CancelTransaction"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.cancel_transaction_request.serialize_json(input_)
    ).encode()
...
async def async_cancel_transaction(
