"""Generated from Smithy shape ``com.amazonaws.lakeformation#DeleteLakeFormationOptIn``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.delete_lake_formation_opt_in_response.DeleteLakeFormationOptInResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteLakeFormationOptIn"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.delete_lake_formation_opt_in_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_lake_formation_opt_in(
