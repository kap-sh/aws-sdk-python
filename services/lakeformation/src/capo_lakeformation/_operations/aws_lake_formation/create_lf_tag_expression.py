"""Generated from Smithy shape ``com.amazonaws.lakeformation#CreateLFTagExpression``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.create_lf_tag_expression_response.CreateLFTagExpressionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateLFTagExpression"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.create_lf_tag_expression_request.serialize_json(input_)
    ).encode()
...
async def async_create_lf_tag_expression(
