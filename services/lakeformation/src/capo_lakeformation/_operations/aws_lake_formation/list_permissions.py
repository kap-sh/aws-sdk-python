"""Generated from Smithy shape ``com.amazonaws.lakeformation#ListPermissions``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.list_permissions_response.ListPermissionsResponse = (
        capo_lakeformation.types.list_permissions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListPermissions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.list_permissions_request.serialize_json(input_)
    ).encode()
...
async def async_list_permissions(
