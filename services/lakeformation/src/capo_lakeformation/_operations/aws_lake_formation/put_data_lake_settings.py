"""Generated from Smithy shape ``com.amazonaws.lakeformation#PutDataLakeSettings``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.put_data_lake_settings_response.PutDataLakeSettingsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/PutDataLakeSettings"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.put_data_lake_settings_request.serialize_json(input_)
    ).encode()
...
async def async_put_data_lake_settings(
