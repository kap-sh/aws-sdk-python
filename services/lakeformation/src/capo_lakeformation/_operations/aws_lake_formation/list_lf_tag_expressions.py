"""Generated from Smithy shape ``com.amazonaws.lakeformation#ListLFTagExpressions``."""
...
import capo_lakeformation.types.lf_tag_expressions_list
...
def handle_response(
    ...
    out: capo_lakeformation.types.list_lf_tag_expressions_response.ListLFTagExpressionsResponse = capo_lakeformation.types.list_lf_tag_expressions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListLFTagExpressions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.list_lf_tag_expressions_request.serialize_json(input_)
    ).encode()
...
async def async_list_lf_tag_expressions(
