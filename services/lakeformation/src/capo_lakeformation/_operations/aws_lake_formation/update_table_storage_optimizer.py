"""Generated from Smithy shape ``com.amazonaws.lakeformation#UpdateTableStorageOptimizer``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.update_table_storage_optimizer_response.UpdateTableStorageOptimizerResponse = capo_lakeformation.types.update_table_storage_optimizer_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateTableStorageOptimizer"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.update_table_storage_optimizer_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_table_storage_optimizer(
