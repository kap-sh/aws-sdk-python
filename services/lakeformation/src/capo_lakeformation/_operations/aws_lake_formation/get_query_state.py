"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetQueryState``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.get_query_state_response.GetQueryStateResponse = (
        capo_lakeformation.types.get_query_state_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetQueryState"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.get_query_state_request.serialize_json(input_)
    ).encode()
...
async def async_get_query_state(
