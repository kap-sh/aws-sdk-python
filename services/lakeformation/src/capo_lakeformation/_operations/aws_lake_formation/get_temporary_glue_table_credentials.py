"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetTemporaryGlueTableCredentials``."""
...
import capo_lakeformation.errors.permission_type_mismatch_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PermissionTypeMismatchException":
            raise capo_lakeformation.errors.permission_type_mismatch_exception.PermissionTypeMismatchException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_lakeformation.types.get_temporary_glue_table_credentials_response.GetTemporaryGlueTableCredentialsResponse = capo_lakeformation.types.get_temporary_glue_table_credentials_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetTemporaryGlueTableCredentials"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.get_temporary_glue_table_credentials_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_get_temporary_glue_table_credentials(
