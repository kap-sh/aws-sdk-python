"""Generated from Smithy shape ``com.amazonaws.lakeformation#GrantPermissions``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.grant_permissions_response.GrantPermissionsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GrantPermissions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.grant_permissions_request.serialize_json(input_)
    ).encode()
...
async def async_grant_permissions(
