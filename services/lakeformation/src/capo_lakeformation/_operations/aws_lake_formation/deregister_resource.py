"""Generated from Smithy shape ``com.amazonaws.lakeformation#DeregisterResource``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.deregister_resource_response.DeregisterResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeregisterResource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.deregister_resource_request.serialize_json(input_)
    ).encode()
...
async def async_deregister_resource(
