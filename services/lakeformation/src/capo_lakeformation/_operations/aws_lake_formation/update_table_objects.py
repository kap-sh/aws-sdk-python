"""Generated from Smithy shape ``com.amazonaws.lakeformation#UpdateTableObjects``."""
...
import capo_lakeformation.errors.resource_not_ready_exception
import capo_lakeformation.errors.transaction_canceled_exception
import capo_lakeformation.errors.transaction_commit_in_progress_exception
import capo_lakeformation.errors.transaction_committed_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotReadyException":
            raise capo_lakeformation.errors.resource_not_ready_exception.ResourceNotReadyException.from_json(
                data
            )
        case "TransactionCanceledException":
            raise capo_lakeformation.errors.transaction_canceled_exception.TransactionCanceledException.from_json(
                data
            )
        case "TransactionCommitInProgressException":
            raise capo_lakeformation.errors.transaction_commit_in_progress_exception.TransactionCommitInProgressException.from_json(
                data
            )
        case "TransactionCommittedException":
            raise capo_lakeformation.errors.transaction_committed_exception.TransactionCommittedException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_lakeformation.types.update_table_objects_response.UpdateTableObjectsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateTableObjects"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.update_table_objects_request.serialize_json(input_)
    ).encode()
...
async def async_update_table_objects(
