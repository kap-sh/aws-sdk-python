"""Generated from Smithy shape ``com.amazonaws.lakeformation#UpdateLFTag``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.update_lf_tag_response.UpdateLFTagResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateLFTag"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.update_lf_tag_request.serialize_json(input_)
    ).encode()
...
def update_lf_tag(
    ...
    capo_lakeformation.types.update_lf_tag_response.UpdateLFTagResponse, zapros.Response
]:
