"""Generated from Smithy shape ``com.amazonaws.lakeformation#StartQueryPlanning``."""
...
def handle_response(
    ...
    out: capo_lakeformation.types.start_query_planning_response.StartQueryPlanningResponse = capo_lakeformation.types.start_query_planning_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StartQueryPlanning"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.start_query_planning_request.serialize_json(input_)
    ).encode()
...
async def async_start_query_planning(
