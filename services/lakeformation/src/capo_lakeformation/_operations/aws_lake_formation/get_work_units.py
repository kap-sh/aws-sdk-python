"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetWorkUnits``."""
...
import capo_lakeformation.errors.work_units_not_ready_yet_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "WorkUnitsNotReadyYetException":
            raise capo_lakeformation.errors.work_units_not_ready_yet_exception.WorkUnitsNotReadyYetException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_lakeformation.types.get_work_units_response.GetWorkUnitsResponse = (
        capo_lakeformation.types.get_work_units_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetWorkUnits"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lakeformation.types.get_work_units_request.serialize_json(input_)
    ).encode()
...
async def async_get_work_units(
