"""Generated from Smithy shape ``com.amazonaws.lakeformation#CreateLFTagRequest``."""
...
class CreateLFTagRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: CreateLFTagRequest) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> CreateLFTagRequest:
    out: CreateLFTagRequest = {}  # type: ignore[typeddict-item]
    if "CatalogId" in data:
        out["catalog_id"] = data["CatalogId"]
    if "TagKey" in data:
        out["tag_key"] = data["TagKey"]
    else:
        raise DeserializationError("CreateLFTagRequest.tag_key required")
    if "TagValues" in data:
        ...
        raise DeserializationError("CreateLFTagRequest.tag_values required")
    return out
