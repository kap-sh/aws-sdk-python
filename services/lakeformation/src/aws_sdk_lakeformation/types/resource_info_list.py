...
ResourceInfoList: TypeAlias = list[
    "aws_sdk_lakeformation.types.resource_info.ResourceInfo"
]
...
def serialize_json(value: ResourceInfoList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_lakeformation.types.resource_info.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ResourceInfoList:
    ...
    for item in data:
        out.append(aws_sdk_lakeformation.types.resource_info.deserialize_json(item))
    return out
