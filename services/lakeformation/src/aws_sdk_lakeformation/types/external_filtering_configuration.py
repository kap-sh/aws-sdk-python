...
if TYPE_CHECKING:
    import aws_sdk_lakeformation.types.enable_status
...
def serialize_json(value: ExternalFilteringConfiguration) -> dict:
    ...
    out["Status"] = aws_sdk_lakeformation.types.enable_status.serialize_json(
        value["status"]
    )
    import aws_sdk_lakeformation.types.scope_targets

    out["AuthorizedTargets"] = aws_sdk_lakeformation.types.scope_targets.serialize_json(
...
def deserialize_json(data: dict) -> ExternalFilteringConfiguration:
    ...
    if "Status" in data:
        ...
        out["status"] = aws_sdk_lakeformation.types.enable_status.deserialize_json(
            data["Status"]
        )
    else:
    ...
    if "AuthorizedTargets" in data:
        ...
        out["authorized_targets"] = (
            aws_sdk_lakeformation.types.scope_targets.deserialize_json(
