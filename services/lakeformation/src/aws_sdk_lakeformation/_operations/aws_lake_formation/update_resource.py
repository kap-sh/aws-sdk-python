"""Generated from Smithy shape ``com.amazonaws.lakeformation#UpdateResource``."""
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.update_resource_response.UpdateResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.update_resource_request.UpdateResourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateResource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lakeformation.types.update_resource_request

    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.update_resource_request.serialize_json(input_)
    ).encode()
