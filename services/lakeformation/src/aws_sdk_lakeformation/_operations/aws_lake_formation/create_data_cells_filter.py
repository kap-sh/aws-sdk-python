"""Generated from Smithy shape ``com.amazonaws.lakeformation#CreateDataCellsFilter``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNumberLimitExceededException":
            import aws_sdk_lakeformation.errors.resource_number_limit_exceeded_exception

            raise aws_sdk_lakeformation.errors.resource_number_limit_exceeded_exception.ResourceNumberLimitExceededException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.create_data_cells_filter_response.CreateDataCellsFilterResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.create_data_cells_filter_request.CreateDataCellsFilterRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/CreateDataCellsFilter"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lakeformation.types.create_data_cells_filter_request

    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.create_data_cells_filter_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_data_cells_filter(
