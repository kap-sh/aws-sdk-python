"""Generated from Smithy shape ``com.amazonaws.lakeformation#BatchGrantPermissions``."""
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.batch_grant_permissions_response.BatchGrantPermissionsResponse = aws_sdk_lakeformation.types.batch_grant_permissions_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.batch_grant_permissions_request.BatchGrantPermissionsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/BatchGrantPermissions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lakeformation.types.batch_grant_permissions_request

    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.batch_grant_permissions_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_grant_permissions(
