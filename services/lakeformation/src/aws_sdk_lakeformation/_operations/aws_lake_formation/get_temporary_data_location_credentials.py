"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetTemporaryDataLocationCredentials``."""
...
import aws_sdk_lakeformation.errors.conflict_exception
import aws_sdk_lakeformation.errors.entity_not_found_exception
import aws_sdk_lakeformation.errors.glue_encryption_exception
...
import aws_sdk_lakeformation.types.temporary_credentials
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise aws_sdk_lakeformation.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "GlueEncryptionException":
            raise aws_sdk_lakeformation.errors.glue_encryption_exception.GlueEncryptionException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.get_temporary_data_location_credentials_response.GetTemporaryDataLocationCredentialsResponse = aws_sdk_lakeformation.types.get_temporary_data_location_credentials_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.get_temporary_data_location_credentials_request.GetTemporaryDataLocationCredentialsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetTemporaryDataLocationCredentials"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.get_temporary_data_location_credentials_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_get_temporary_data_location_credentials(
