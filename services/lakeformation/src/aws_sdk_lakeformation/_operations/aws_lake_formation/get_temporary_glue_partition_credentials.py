"""Generated from Smithy shape ``com.amazonaws.lakeformation#GetTemporaryGluePartitionCredentials``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "PermissionTypeMismatchException":
            import aws_sdk_lakeformation.errors.permission_type_mismatch_exception

            raise aws_sdk_lakeformation.errors.permission_type_mismatch_exception.PermissionTypeMismatchException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.get_temporary_glue_partition_credentials_response.GetTemporaryGluePartitionCredentialsResponse = aws_sdk_lakeformation.types.get_temporary_glue_partition_credentials_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.get_temporary_glue_partition_credentials_request.GetTemporaryGluePartitionCredentialsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/GetTemporaryGluePartitionCredentials"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lakeformation.types.get_temporary_glue_partition_credentials_request

    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.get_temporary_glue_partition_credentials_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_get_temporary_glue_partition_credentials(
