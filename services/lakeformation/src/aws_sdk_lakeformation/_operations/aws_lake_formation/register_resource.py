"""Generated from Smithy shape ``com.amazonaws.lakeformation#RegisterResource``."""
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.register_resource_response.RegisterResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.register_resource_request.RegisterResourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/RegisterResource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lakeformation.types.register_resource_request

    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.register_resource_request.serialize_json(input_)
    ).encode()
