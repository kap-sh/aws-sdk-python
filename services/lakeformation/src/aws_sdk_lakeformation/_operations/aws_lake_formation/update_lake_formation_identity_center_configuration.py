"""Generated from Smithy shape ``com.amazonaws.lakeformation#UpdateLakeFormationIdentityCenterConfiguration``."""
...
def handle_response(
    ...
    out: aws_sdk_lakeformation.types.update_lake_formation_identity_center_configuration_response.UpdateLakeFormationIdentityCenterConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_lakeformation.types.update_lake_formation_identity_center_configuration_request.UpdateLakeFormationIdentityCenterConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateLakeFormationIdentityCenterConfiguration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lakeformation.types.update_lake_formation_identity_center_configuration_request

    body: bytes | None = json.dumps(
        aws_sdk_lakeformation.types.update_lake_formation_identity_center_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_lake_formation_identity_center_configuration(
