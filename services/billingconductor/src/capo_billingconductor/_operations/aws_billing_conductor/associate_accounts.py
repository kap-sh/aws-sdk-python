"""Generated from Smithy shape ``com.amazonaws.billingconductor#AssociateAccounts``."""
...
def handle_response(
    ...
    out: capo_billingconductor.types.associate_accounts_output.AssociateAccountsOutput = capo_billingconductor.types.associate_accounts_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/associate-accounts"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_billingconductor.types.associate_accounts_input.serialize_json(input_)
    ).encode()
...
async def async_associate_accounts(
