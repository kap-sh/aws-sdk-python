"""Generated from Smithy shape ``com.amazonaws.billingconductor#CreateCustomLineItem``."""
...
def handle_response(
    ...
    out: capo_billingconductor.types.create_custom_line_item_output.CreateCustomLineItemOutput = capo_billingconductor.types.create_custom_line_item_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/create-custom-line-item"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "client_token" in input_:
        headers["X-Amzn-Client-Token"] = str(input_["client_token"])
    body: bytes | None = json.dumps(
        capo_billingconductor.types.create_custom_line_item_input.serialize_json(input_)
    ).encode()
...
async def async_create_custom_line_item(
