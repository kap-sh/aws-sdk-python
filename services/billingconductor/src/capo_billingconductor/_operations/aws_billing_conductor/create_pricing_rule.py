"""Generated from Smithy shape ``com.amazonaws.billingconductor#CreatePricingRule``."""
...
def handle_response(
    ...
    out: capo_billingconductor.types.create_pricing_rule_output.CreatePricingRuleOutput = capo_billingconductor.types.create_pricing_rule_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/create-pricing-rule"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "client_token" in input_:
        headers["X-Amzn-Client-Token"] = str(input_["client_token"])
    body: bytes | None = json.dumps(
        capo_billingconductor.types.create_pricing_rule_input.serialize_json(input_)
    ).encode()
...
async def async_create_pricing_rule(
