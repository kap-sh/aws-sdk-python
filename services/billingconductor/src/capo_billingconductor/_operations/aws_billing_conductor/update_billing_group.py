"""Generated from Smithy shape ``com.amazonaws.billingconductor#UpdateBillingGroup``."""
...
def handle_response(
    ...
    out: capo_billingconductor.types.update_billing_group_output.UpdateBillingGroupOutput = capo_billingconductor.types.update_billing_group_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-billing-group"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_billingconductor.types.update_billing_group_input.serialize_json(input_)
    ).encode()
