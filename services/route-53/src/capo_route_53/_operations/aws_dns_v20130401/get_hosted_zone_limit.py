...
import capo_route_53.errors.hosted_zone_not_private
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "HostedZoneNotPrivate":
            raise capo_route_53.errors.hosted_zone_not_private.HostedZoneNotPrivate.from_xml(
                error_el, message
            )
        case "InvalidInput":
...
def handle_response(
    ...
    out: capo_route_53.types.get_hosted_zone_limit_response.GetHostedZoneLimitResponse = capo_route_53.types.get_hosted_zone_limit_response.deserialize_xml(
...
def build_request(
    ...
    url = url.replace(
        "{Type}",
        quote(
            capo_route_53.types.hosted_zone_limit_type.to_xml_text(input_["type"]),
            safe="",
        ),
    )
