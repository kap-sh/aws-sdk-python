...
def handle_response(
    ...
    out: capo_route_53.types.list_resource_record_sets_response.ListResourceRecordSetsResponse = capo_route_53.types.list_resource_record_sets_response.deserialize_xml(
...
def build_request(
    ...
    if "start_record_name" in input_:
        params.append(("name", input_["start_record_name"]))
    if "start_record_type" in input_:
        params.append(
            (
                "type",
                capo_route_53.types.rr_type.to_xml_text(input_["start_record_type"]),
            )
        )
    if "start_record_identifier" in input_:
        params.append(("identifier", input_["start_record_identifier"]))
