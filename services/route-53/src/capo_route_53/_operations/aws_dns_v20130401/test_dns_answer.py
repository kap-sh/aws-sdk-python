...
def handle_response(
    ...
    out: capo_route_53.types.test_dns_answer_response.TestDNSAnswerResponse = (
        capo_route_53.types.test_dns_answer_response.deserialize_xml(
...
def build_request(
    ...
    if "record_name" in input_:
        params.append(("recordname", input_["record_name"]))
    if "record_type" in input_:
        params.append(
            (
                "recordtype",
                capo_route_53.types.rr_type.to_xml_text(input_["record_type"]),
            )
        )
    if "resolver_ip" in input_:
        params.append(("resolverip", input_["resolver_ip"]))
    if "edns0_client_subnet_ip" in input_:
        params.append(("edns0clientsubnetip", input_["edns0_client_subnet_ip"]))
    if "edns0_client_subnet_mask" in input_:
        params.append(("edns0clientsubnetmask", input_["edns0_client_subnet_mask"]))
...
def test_dns_answer(
    ...
    capo_route_53.types.test_dns_answer_response.TestDNSAnswerResponse, zapros.Response
]:
