...
if TYPE_CHECKING:
    import capo_route_53.types.accelerated_recovery_status
    import capo_route_53.types.hosted_zone_failure_reasons


class HostedZoneFeatures(TypedDict, closed=True):
    accelerated_recovery_status: NotRequired[
        "capo_route_53.types.accelerated_recovery_status.AcceleratedRecoveryStatus"
    ]
    """<p>The current status of accelerated recovery for the hosted zone.</p>"""
    failure_reasons: NotRequired[
        "capo_route_53.types.hosted_zone_failure_reasons.HostedZoneFailureReasons"
    ]
...
def serialize_xml(value: HostedZoneFeatures, parent: Element, tag: str) -> None:
    ...
    if "accelerated_recovery_status" in value:
        ...
        capo_route_53.types.accelerated_recovery_status.serialize_xml(
            value["accelerated_recovery_status"], el, "AcceleratedRecoveryStatus"
        )
    if "failure_reasons" in value:
        import capo_route_53.types.hosted_zone_failure_reasons

        capo_route_53.types.hosted_zone_failure_reasons.serialize_xml(
...
def deserialize_xml(el: Element) -> HostedZoneFeatures:
    ...
    if child_accelerated_recovery_status is not None:
        ...
        out["accelerated_recovery_status"] = (
            capo_route_53.types.accelerated_recovery_status.deserialize_xml(
    ...
    if child_failure_reasons is not None:
        ...
        out["failure_reasons"] = (
            capo_route_53.types.hosted_zone_failure_reasons.deserialize_xml(
