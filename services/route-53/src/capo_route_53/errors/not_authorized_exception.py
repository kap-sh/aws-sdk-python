"""Generated from Smithy shape ``com.amazonaws.route53#NotAuthorizedException``."""
...
# --- restXml ser/de ---
def serialize_xml(value: NotAuthorizedException_, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    if "message" in value:
        SubElement(el, "message").text = str(value["message"])


def deserialize_xml(el: Element) -> NotAuthorizedException_:
...
class NotAuthorizedException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.route53#NotAuthorizedException``."""
    ...
    @classmethod
    def from_xml(cls, el: Element) -> "NotAuthorizedException":
