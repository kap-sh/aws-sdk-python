...
if TYPE_CHECKING:
    import aws_sdk_route_53.types.vpc_id
    import aws_sdk_route_53.types.vpc_region


class VPC(TypedDict, closed=True):
    vpc_region: NotRequired["aws_sdk_route_53.types.vpc_region.VPCRegion"]
    """<p>(Private hosted zones only) The region that an Amazon VPC was created in.</p>"""
    vpc_id: NotRequired["aws_sdk_route_53.types.vpc_id.VPCId"]
...
def serialize_xml(value: VPC, parent: Element, tag: str) -> None:
    ...
    if "vpc_region" in value:
        ...
        aws_sdk_route_53.types.vpc_region.serialize_xml(
...
def deserialize_xml(el: Element) -> VPC:
    ...
    if child_vpc_region is not None:
        ...
        out["vpc_region"] = aws_sdk_route_53.types.vpc_region.deserialize_xml(
            child_vpc_region
        )
