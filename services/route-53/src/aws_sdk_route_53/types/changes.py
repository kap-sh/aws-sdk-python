"""Generated from Smithy shape ``com.amazonaws.route53#Changes``."""

from typing import TYPE_CHECKING, TypeAlias

from aws_sdk_route_53._protocol.xml import Element, SubElement

if TYPE_CHECKING:
    import aws_sdk_route_53.types.change

Changes: TypeAlias = list["aws_sdk_route_53.types.change.Change"]


# --- restXml ser/de ---
def serialize_xml(value: Changes, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    for item in value:
        import aws_sdk_route_53.types.change

        aws_sdk_route_53.types.change.serialize_xml(item, el, "Change")


def deserialize_xml(el: Element) -> Changes:
    import aws_sdk_route_53.types.change

    out: Changes = []
    for child in el.findall("Change"):
        out.append(aws_sdk_route_53.types.change.deserialize_xml(child))
    return out


def serialize_xml_flat(value: Changes, parent: Element, tag: str) -> None:
    """Variant used by parent structures with ``@xmlFlattened`` on the referencing member. Items emitted directly under ``parent``."""
    for item in value:
        import aws_sdk_route_53.types.change

        aws_sdk_route_53.types.change.serialize_xml(item, parent, tag)


def deserialize_xml_flat(parent: Element, tag: str) -> Changes:
    import aws_sdk_route_53.types.change

    out: Changes = []
    for child in parent.findall(tag):
        out.append(aws_sdk_route_53.types.change.deserialize_xml(child))
    return out
