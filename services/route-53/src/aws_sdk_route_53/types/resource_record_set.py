"""Generated from Smithy shape ``com.amazonaws.route53#ResourceRecordSet``."""
...
if TYPE_CHECKING:
    import aws_sdk_route_53.types.alias_target
    import aws_sdk_route_53.types.cidr_routing_config
    import aws_sdk_route_53.types.dns_name
    import aws_sdk_route_53.types.geo_location
    import aws_sdk_route_53.types.geo_proximity_location
    import aws_sdk_route_53.types.health_check_id
    import aws_sdk_route_53.types.resource_record_set_failover
    import aws_sdk_route_53.types.resource_record_set_identifier
    import aws_sdk_route_53.types.resource_record_set_multi_value_answer
    import aws_sdk_route_53.types.resource_record_set_region
    import aws_sdk_route_53.types.resource_record_set_weight
    import aws_sdk_route_53.types.resource_records
...
class ResourceRecordSet(TypedDict, closed=True):
    ...
    multi_value_answer: NotRequired[
        "aws_sdk_route_53.types.resource_record_set_multi_value_answer.ResourceRecordSetMultiValueAnswer"
    ]
    ...
    ttl: NotRequired["aws_sdk_route_53.types.ttl.TTL"]
    ...
    resource_records: NotRequired[
        "aws_sdk_route_53.types.resource_records.ResourceRecords"
    ]
    ...
    cidr_routing_config: NotRequired[
        "aws_sdk_route_53.types.cidr_routing_config.CidrRoutingConfig"
    ]
    geo_proximity_location: NotRequired[
        "aws_sdk_route_53.types.geo_proximity_location.GeoProximityLocation"
    ]
...
# --- restXml ser/de ---
def serialize_xml(value: ResourceRecordSet, parent: Element, tag: str) -> None:
    el = SubElement(parent, tag)
    SubElement(el, "Name").text = str(value["name"])
    import aws_sdk_route_53.types.rr_type

    aws_sdk_route_53.types.rr_type.serialize_xml(value["type"], el, "Type")
    if "set_identifier" in value:
        SubElement(el, "SetIdentifier").text = str(value["set_identifier"])
    if "weight" in value:
        SubElement(el, "Weight").text = str(value["weight"])
    if "region" in value:
        import aws_sdk_route_53.types.resource_record_set_region

        aws_sdk_route_53.types.resource_record_set_region.serialize_xml(
            value["region"], el, "Region"
        )
    if "geo_location" in value:
        import aws_sdk_route_53.types.geo_location

        aws_sdk_route_53.types.geo_location.serialize_xml(
            value["geo_location"], el, "GeoLocation"
        )
    if "failover" in value:
        import aws_sdk_route_53.types.resource_record_set_failover

        aws_sdk_route_53.types.resource_record_set_failover.serialize_xml(
            value["failover"], el, "Failover"
        )
    if "multi_value_answer" in value:
        SubElement(el, "MultiValueAnswer").text = (
            "true" if value["multi_value_answer"] else "false"
        )
    if "ttl" in value:
        SubElement(el, "TTL").text = str(value["ttl"])
    if "resource_records" in value:
        import aws_sdk_route_53.types.resource_records

        aws_sdk_route_53.types.resource_records.serialize_xml(
            value["resource_records"], el, "ResourceRecords"
        )
    if "alias_target" in value:
        import aws_sdk_route_53.types.alias_target

        aws_sdk_route_53.types.alias_target.serialize_xml(
            value["alias_target"], el, "AliasTarget"
        )
    if "health_check_id" in value:
        SubElement(el, "HealthCheckId").text = str(value["health_check_id"])
    if "traffic_policy_instance_id" in value:
        SubElement(el, "TrafficPolicyInstanceId").text = str(
            value["traffic_policy_instance_id"]
        )
    if "cidr_routing_config" in value:
        import aws_sdk_route_53.types.cidr_routing_config

        aws_sdk_route_53.types.cidr_routing_config.serialize_xml(
            value["cidr_routing_config"], el, "CidrRoutingConfig"
        )
    if "geo_proximity_location" in value:
        import aws_sdk_route_53.types.geo_proximity_location

        aws_sdk_route_53.types.geo_proximity_location.serialize_xml(
            value["geo_proximity_location"], el, "GeoProximityLocation"
        )


def deserialize_xml(el: Element) -> ResourceRecordSet:
    out: ResourceRecordSet = {}  # type: ignore[typeddict-item]
    child_name = el.find("Name")
    if child_name is not None:
        out["name"] = str(child_name.text or "")
    else:
        raise DeserializationError("ResourceRecordSet.name required")
    ...
    if child_type is not None:
        ...
        raise DeserializationError("ResourceRecordSet.type required")
    child_set_identifier = el.find("SetIdentifier")
    if child_set_identifier is not None:
        out["set_identifier"] = str(child_set_identifier.text or "")
    ...
    if child_region is not None:
        ...
        out["region"] = (
            aws_sdk_route_53.types.resource_record_set_region.deserialize_xml(
                child_region
            )
        )
    child_geo_location = el.find("GeoLocation")
    if child_geo_location is not None:
        import aws_sdk_route_53.types.geo_location

        out["geo_location"] = aws_sdk_route_53.types.geo_location.deserialize_xml(
            child_geo_location
        )
    child_failover = el.find("Failover")
    if child_failover is not None:
        import aws_sdk_route_53.types.resource_record_set_failover

        out["failover"] = (
            aws_sdk_route_53.types.resource_record_set_failover.deserialize_xml(
                child_failover
            )
        )
    child_multi_value_answer = el.find("MultiValueAnswer")
    if child_multi_value_answer is not None:
        out["multi_value_answer"] = (
            child_multi_value_answer.text or ""
        ).lower() == "true"
    child_ttl = el.find("TTL")
    if child_ttl is not None:
        out["ttl"] = int(child_ttl.text or "")
    child_resource_records = el.find("ResourceRecords")
    if child_resource_records is not None:
        import aws_sdk_route_53.types.resource_records

        out["resource_records"] = (
            aws_sdk_route_53.types.resource_records.deserialize_xml(
                child_resource_records
            )
        )
    child_alias_target = el.find("AliasTarget")
    if child_alias_target is not None:
        import aws_sdk_route_53.types.alias_target

        out["alias_target"] = aws_sdk_route_53.types.alias_target.deserialize_xml(
            child_alias_target
        )
    child_health_check_id = el.find("HealthCheckId")
    if child_health_check_id is not None:
        out["health_check_id"] = str(child_health_check_id.text or "")
    child_traffic_policy_instance_id = el.find("TrafficPolicyInstanceId")
    if child_traffic_policy_instance_id is not None:
        out["traffic_policy_instance_id"] = str(
            child_traffic_policy_instance_id.text or ""
        )
    child_cidr_routing_config = el.find("CidrRoutingConfig")
    if child_cidr_routing_config is not None:
        import aws_sdk_route_53.types.cidr_routing_config

        out["cidr_routing_config"] = (
            aws_sdk_route_53.types.cidr_routing_config.deserialize_xml(
                child_cidr_routing_config
            )
        )
    child_geo_proximity_location = el.find("GeoProximityLocation")
    if child_geo_proximity_location is not None:
        import aws_sdk_route_53.types.geo_proximity_location

        out["geo_proximity_location"] = (
            aws_sdk_route_53.types.geo_proximity_location.deserialize_xml(
                child_geo_proximity_location
            )
        )
    return out
