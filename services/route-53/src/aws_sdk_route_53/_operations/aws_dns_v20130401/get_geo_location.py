...
def build_request(
    ...
    input: aws_sdk_route_53.types.get_geo_location_request.GetGeoLocationRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "continent_code" in input:
        params["continentcode"] = str(input["continent_code"])
    if "country_code" in input:
        params["countrycode"] = str(input["country_code"])
    if "subdivision_code" in input:
        params["subdivisioncode"] = str(input["subdivision_code"])
