...
def build_request(
    ...
    input: aws_sdk_route_53.types.create_hosted_zone_request.CreateHostedZoneRequest,
) -> zapros.Request:
    ...
    if "hosted_zone_config" in input:
        import aws_sdk_route_53.types.hosted_zone_config

        aws_sdk_route_53.types.hosted_zone_config.serialize_xml(
            input["hosted_zone_config"], root, "HostedZoneConfig"
        )
    if "delegation_set_id" in input:
        SubElement(root, "DelegationSetId").text = str(input["delegation_set_id"])
