"""Generated from Smithy shape ``com.amazonaws.route53#ListHostedZonesByName``."""
...
def build_request(
    ...
    input: aws_sdk_route_53.types.list_hosted_zones_by_name_request.ListHostedZonesByNameRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/2013-04-01/hostedzonesbyname"
    params: dict[str, str] = {}
    if "dns_name" in input:
        params["dnsname"] = str(input["dns_name"])
    if "hosted_zone_id" in input:
        params["hostedzoneid"] = str(input["hosted_zone_id"])
...
async def async_list_hosted_zones_by_name(
