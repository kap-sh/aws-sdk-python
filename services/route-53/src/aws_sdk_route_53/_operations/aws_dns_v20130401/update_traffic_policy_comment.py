"""Generated from Smithy shape ``com.amazonaws.route53#UpdateTrafficPolicyComment``."""
...
if TYPE_CHECKING:
    import aws_sdk_route_53.types.update_traffic_policy_comment_request
    import aws_sdk_route_53.types.update_traffic_policy_comment_response
...
) -> aws_sdk_route_53.types.update_traffic_policy_comment_response.UpdateTrafficPolicyCommentResponse:
    import aws_sdk_route_53.types.update_traffic_policy_comment_response

    out: aws_sdk_route_53.types.update_traffic_policy_comment_response.UpdateTrafficPolicyCommentResponse = aws_sdk_route_53.types.update_traffic_policy_comment_response.deserialize_xml(
...
def build_request(
    ...
    input_: aws_sdk_route_53.types.update_traffic_policy_comment_request.UpdateTrafficPolicyCommentRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/2013-04-01/trafficpolicy/{Id}/{Version}"
    url = url.replace("{Id}", quote(str(input_["id"]), safe=""))
    url = url.replace("{Version}", quote(str(input_["version"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    root = Element("UpdateTrafficPolicyCommentRequest")
...
async def async_update_traffic_policy_comment(
