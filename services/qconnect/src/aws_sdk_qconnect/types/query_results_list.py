...
QueryResultsList: TypeAlias = list["aws_sdk_qconnect.types.result_data.ResultData"]
...
def serialize_json(value: QueryResultsList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_qconnect.types.result_data.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> QueryResultsList:
    ...
    for item in data:
        out.append(aws_sdk_qconnect.types.result_data.deserialize_json(item))
    return out
