...
if TYPE_CHECKING:
    import aws_sdk_qconnect.types.seed_urls


class UrlConfiguration(TypedDict, closed=True):
    seed_urls: NotRequired["aws_sdk_qconnect.types.seed_urls.SeedUrls"]
...
def serialize_json(value: UrlConfiguration) -> dict:
    ...
    if "seed_urls" in value:
        ...
        out["seedUrls"] = aws_sdk_qconnect.types.seed_urls.serialize_json(
            value["seed_urls"]
        )
    return out
...
def deserialize_json(data: dict) -> UrlConfiguration:
    ...
    if "seedUrls" in data:
        ...
        out["seed_urls"] = aws_sdk_qconnect.types.seed_urls.deserialize_json(
            data["seedUrls"]
        )
    return out
