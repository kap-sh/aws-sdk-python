...
if TYPE_CHECKING:
    import aws_sdk_qconnect.types.guardrail_topics_config


class AIGuardrailTopicPolicyConfig(TypedDict):
    topics_config: (
        "aws_sdk_qconnect.types.guardrail_topics_config.GuardrailTopicsConfig"
    )
...
def serialize_json(value: AIGuardrailTopicPolicyConfig) -> dict:
    ...
    out["topicsConfig"] = aws_sdk_qconnect.types.guardrail_topics_config.serialize_json(
        value["topics_config"]
    )
    return out
...
def deserialize_json(data: dict) -> AIGuardrailTopicPolicyConfig:
    ...
    if "topicsConfig" in data:
        ...
        out["topics_config"] = (
            aws_sdk_qconnect.types.guardrail_topics_config.deserialize_json(
                data["topicsConfig"]
            )
        )
    else:
