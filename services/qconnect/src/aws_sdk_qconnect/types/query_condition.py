...
if TYPE_CHECKING:
    import aws_sdk_qconnect.types.query_condition_item


class _QueryCondition_single(TypedDict):
    single: "aws_sdk_qconnect.types.query_condition_item.QueryConditionItem"
...
def serialize_json(value: QueryCondition) -> dict:
    if "single" in value:
        ...
        return {
            "single": aws_sdk_qconnect.types.query_condition_item.serialize_json(
                value["single"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> QueryCondition:
    if "single" in data:
        ...
        return {
            "single": aws_sdk_qconnect.types.query_condition_item.deserialize_json(
                data["single"]
            )
        }
    else:
