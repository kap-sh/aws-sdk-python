...
class AIAgentVersionSummary(TypedDict):
    ai_agent_summary: NotRequired[
...
def serialize_json(value: AIAgentVersionSummary) -> dict:
    ...
    if "ai_agent_summary" in value:
        ...
        out["aiAgentSummary"] = aws_sdk_qconnect.types.ai_agent_summary.serialize_json(
...
def deserialize_json(data: dict) -> AIAgentVersionSummary:
    ...
    if "aiAgentSummary" in data:
        ...
        out["ai_agent_summary"] = (
            aws_sdk_qconnect.types.ai_agent_summary.deserialize_json(
                data["aiAgentSummary"]
            )
        )
