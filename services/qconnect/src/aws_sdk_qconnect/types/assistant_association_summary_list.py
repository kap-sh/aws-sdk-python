...
if TYPE_CHECKING:
    import aws_sdk_qconnect.types.assistant_association_summary

AssistantAssociationSummaryList: TypeAlias = list[
    "aws_sdk_qconnect.types.assistant_association_summary.AssistantAssociationSummary"
]
...
def serialize_json(value: AssistantAssociationSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_qconnect.types.assistant_association_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AssistantAssociationSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_qconnect.types.assistant_association_summary.deserialize_json(item)
        )
    return out
