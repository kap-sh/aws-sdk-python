"""Generated from Smithy shape ``com.amazonaws.qconnect#SearchContentRequest``."""
...
class SearchContentRequest(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: SearchContentRequest) -> dict:
    out: dict = {}
    import capo_qconnect.types.search_expression

    out["searchExpression"] = capo_qconnect.types.search_expression.serialize_json(
...
def deserialize_json(data: dict) -> SearchContentRequest:
    ...
    if "searchExpression" in data:
        ...
        out["search_expression"] = (
            capo_qconnect.types.search_expression.deserialize_json(
                data["searchExpression"]
            )
        )
    else:
        raise DeserializationError("SearchContentRequest.search_expression required")
    return out
