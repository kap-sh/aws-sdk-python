...
if TYPE_CHECKING:
    ...
    import capo_qconnect.types.quick_response_search_results_list


class SearchQuickResponsesResponse(TypedDict, closed=True):
    results: "capo_qconnect.types.quick_response_search_results_list.QuickResponseSearchResultsList"
...
def serialize_json(value: SearchQuickResponsesResponse) -> dict:
    ...
    out["results"] = (
        capo_qconnect.types.quick_response_search_results_list.serialize_json(
            value["results"]
        )
    )
...
def deserialize_json(data: dict) -> SearchQuickResponsesResponse:
    ...
    if "results" in data:
        ...
        out["results"] = (
            capo_qconnect.types.quick_response_search_results_list.deserialize_json(
                data["results"]
            )
        )
    else:
