...
SessionSummaries: TypeAlias = list["capo_qconnect.types.session_summary.SessionSummary"]
...
def serialize_json(value: SessionSummaries) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_qconnect.types.session_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> SessionSummaries:
    ...
    for item in data:
        out.append(capo_qconnect.types.session_summary.deserialize_json(item))
    return out
