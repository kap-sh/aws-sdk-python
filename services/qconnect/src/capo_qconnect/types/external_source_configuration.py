...
if TYPE_CHECKING:
    import capo_qconnect.types.configuration
    import capo_qconnect.types.external_source


class ExternalSourceConfiguration(TypedDict, closed=True):
    source: "capo_qconnect.types.external_source.ExternalSource"
    """<p>The type of the external data source.</p>"""
    configuration: "capo_qconnect.types.configuration.Configuration"
...
def serialize_json(value: ExternalSourceConfiguration) -> dict:
    ...
    out["configuration"] = capo_qconnect.types.configuration.serialize_json(
        value["configuration"]
    )
    return out
...
def deserialize_json(data: dict) -> ExternalSourceConfiguration:
    ...
    if "configuration" in data:
        ...
        out["configuration"] = capo_qconnect.types.configuration.deserialize_json(
            data["configuration"]
        )
    else:
