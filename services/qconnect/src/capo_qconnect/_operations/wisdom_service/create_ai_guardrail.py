"""Generated from Smithy shape ``com.amazonaws.qconnect#CreateAIGuardrail``."""
...
def handle_response(
    ...
    out: capo_qconnect.types.create_ai_guardrail_response.CreateAIGuardrailResponse = (
        capo_qconnect.types.create_ai_guardrail_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/assistants/{assistantId}/aiguardrails"
    url = url.replace("{assistantId}", quote(str(input_["assistant_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qconnect.types.create_ai_guardrail_request.serialize_json(input_)
    ).encode()
...
async def async_create_ai_guardrail(
