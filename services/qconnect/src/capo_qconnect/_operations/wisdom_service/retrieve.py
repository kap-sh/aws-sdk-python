"""Generated from Smithy shape ``com.amazonaws.qconnect#Retrieve``."""
...
def handle_response(
    ...
    out: capo_qconnect.types.retrieve_response.RetrieveResponse = (
        capo_qconnect.types.retrieve_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/assistants/{assistantId}/retrieve"
    url = url.replace("{assistantId}", quote(str(input_["assistant_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qconnect.types.retrieve_request.serialize_json(input_)
    ).encode()
...
def retrieve(
    ...
    input_: capo_qconnect.types.retrieve_request.RetrieveRequest,
) -> tuple[capo_qconnect.types.retrieve_response.RetrieveResponse, zapros.Response]:
