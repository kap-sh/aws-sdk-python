"""Generated from Smithy shape ``com.amazonaws.qconnect#ListModels``."""
...
def handle_response(
    ...
    out: capo_qconnect.types.list_models_response.ListModelsResponse = (
        capo_qconnect.types.list_models_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/assistants/{assistantId}/models"
    url = url.replace("{assistantId}", quote(str(input_["assistant_id"]), safe=""))
    params: dict[str, str] = {}
    if "ai_prompt_type" in input_:
        params["aiPromptType"] = str(input_["ai_prompt_type"])
    if "model_lifecycle" in input_:
        params["modelLifecycle"] = str(input_["model_lifecycle"])
...
def list_models(
    ...
    capo_qconnect.types.list_models_response.ListModelsResponse, zapros.Response
]:
