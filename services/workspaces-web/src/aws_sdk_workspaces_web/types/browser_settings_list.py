...
BrowserSettingsList: TypeAlias = list[
    "aws_sdk_workspaces_web.types.browser_settings_summary.BrowserSettingsSummary"
]
...
def serialize_json(value: BrowserSettingsList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_workspaces_web.types.browser_settings_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BrowserSettingsList:
    ...
    for item in data:
        out.append(
            aws_sdk_workspaces_web.types.browser_settings_summary.deserialize_json(item)
        )
    return out
