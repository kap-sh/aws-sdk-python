...
if TYPE_CHECKING:
    import aws_sdk_workspaces_web.types.s3_log_configuration


class LogConfiguration(TypedDict, closed=True):
    s3: NotRequired[
        "aws_sdk_workspaces_web.types.s3_log_configuration.S3LogConfiguration"
    ]
...
def serialize_json(value: LogConfiguration) -> dict:
    ...
    if "s3" in value:
        ...
        out["s3"] = aws_sdk_workspaces_web.types.s3_log_configuration.serialize_json(
            value["s3"]
        )
    return out
...
def deserialize_json(data: dict) -> LogConfiguration:
    ...
    if "s3" in data:
        ...
        out["s3"] = aws_sdk_workspaces_web.types.s3_log_configuration.deserialize_json(
            data["s3"]
        )
    return out
