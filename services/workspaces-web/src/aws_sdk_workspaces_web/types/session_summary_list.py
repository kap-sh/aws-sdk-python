...
SessionSummaryList: TypeAlias = list[
    "aws_sdk_workspaces_web.types.session_summary.SessionSummary"
]
...
def serialize_json(value: SessionSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_workspaces_web.types.session_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> SessionSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_workspaces_web.types.session_summary.deserialize_json(item))
    return out
