...
class CookieSynchronizationConfiguration(TypedDict, closed=True):
    ...
    blocklist: NotRequired[
        "aws_sdk_workspaces_web.types.cookie_specifications.CookieSpecifications"
    ]
...
def serialize_json(value: CookieSynchronizationConfiguration) -> dict:
    ...
    out["allowlist"] = (
        aws_sdk_workspaces_web.types.cookie_specifications.serialize_json(
            value["allowlist"]
        )
    )
...
def deserialize_json(data: dict) -> CookieSynchronizationConfiguration:
    ...
    if "allowlist" in data:
        ...
        out["allowlist"] = (
            aws_sdk_workspaces_web.types.cookie_specifications.deserialize_json(
                data["allowlist"]
            )
        )
    else:
