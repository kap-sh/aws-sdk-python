...
class ListTrustStoresResponse(TypedDict, closed=True):
    trust_stores: NotRequired[
        "capo_workspaces_web.types.trust_store_summary_list.TrustStoreSummaryList"
    ]
...
def serialize_json(value: ListTrustStoresResponse) -> dict:
    ...
    if "trust_stores" in value:
        ...
        out["trustStores"] = (
            capo_workspaces_web.types.trust_store_summary_list.serialize_json(
                value["trust_stores"]
            )
        )
...
def deserialize_json(data: dict) -> ListTrustStoresResponse:
    ...
    if "trustStores" in data:
        ...
        out["trust_stores"] = (
            capo_workspaces_web.types.trust_store_summary_list.deserialize_json(
                data["trustStores"]
            )
        )
