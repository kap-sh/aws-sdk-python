"""Generated from Smithy shape ``com.amazonaws.workspacesweb#UpdateUserSettings``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.update_user_settings_response.UpdateUserSettingsResponse = capo_workspaces_web.types.update_user_settings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/userSettings/{userSettingsArn+}"
    url = url.replace(
        "{userSettingsArn+}", quote(str(input_["user_settings_arn"]), safe="/")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_workspaces_web.types.update_user_settings_request.serialize_json(input_)
    ).encode()
