"""Generated from Smithy shape ``com.amazonaws.workspacesweb#CreateIdentityProvider``."""
...
def handle_response(
    ...
    out: capo_workspaces_web.types.create_identity_provider_response.CreateIdentityProviderResponse = capo_workspaces_web.types.create_identity_provider_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identityProviders"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_workspaces_web.types.create_identity_provider_request.serialize_json(
            input_
        )
    ).encode()
