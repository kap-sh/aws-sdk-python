...
def serialize_json(value: AudioConcatenationConfiguration) -> dict:
    ...
    out["State"] = (
        aws_sdk_chime_sdk_media_pipelines.types.audio_artifacts_concatenation_state.serialize_json(
            value["state"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> AudioConcatenationConfiguration:
    ...
    if "State" in data:
        ...
        out["state"] = (
            aws_sdk_chime_sdk_media_pipelines.types.audio_artifacts_concatenation_state.deserialize_json(
                data["State"]
            )
        )
    else:
