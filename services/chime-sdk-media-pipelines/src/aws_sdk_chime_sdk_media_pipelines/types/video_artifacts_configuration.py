...
class VideoArtifactsConfiguration(TypedDict):
    ...
    mux_type: NotRequired[
        "aws_sdk_chime_sdk_media_pipelines.types.video_mux_type.VideoMuxType"
    ]
...
def serialize_json(value: VideoArtifactsConfiguration) -> dict:
    ...
    if "mux_type" in value:
        ...
        out["MuxType"] = (
            aws_sdk_chime_sdk_media_pipelines.types.video_mux_type.serialize_json(
                value["mux_type"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> VideoArtifactsConfiguration:
    ...
    if "MuxType" in data:
        ...
        out["mux_type"] = (
            aws_sdk_chime_sdk_media_pipelines.types.video_mux_type.deserialize_json(
                data["MuxType"]
            )
        )
    return out
