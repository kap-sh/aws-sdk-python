...
class TargetCloseDateFilter(TypedDict, closed=True):
    after_target_close_date: NotRequired["capo_partnercentral_selling.types.date.Date"]
    """<p>Filters opportunities with a target close date after this date. Use the <code>YYYY-MM-DD</code> format.</p>"""
    before_target_close_date: NotRequired["capo_partnercentral_selling.types.date.Date"]
