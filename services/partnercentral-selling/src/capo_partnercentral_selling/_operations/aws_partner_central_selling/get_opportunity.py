"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#GetOpportunity``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.get_opportunity_response.GetOpportunityResponse = capo_partnercentral_selling.types.get_opportunity_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetOpportunity"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.GetOpportunity"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.get_opportunity_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_get_opportunity(
