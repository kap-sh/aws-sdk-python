"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#StopResourceSnapshotJob``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/StopResourceSnapshotJob"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.StopResourceSnapshotJob"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.stop_resource_snapshot_job_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_stop_resource_snapshot_job(
