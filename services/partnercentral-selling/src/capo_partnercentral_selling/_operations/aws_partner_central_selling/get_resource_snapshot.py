"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#GetResourceSnapshot``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.get_resource_snapshot_response.GetResourceSnapshotResponse = capo_partnercentral_selling.types.get_resource_snapshot_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetResourceSnapshot"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.GetResourceSnapshot"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.get_resource_snapshot_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_get_resource_snapshot(
