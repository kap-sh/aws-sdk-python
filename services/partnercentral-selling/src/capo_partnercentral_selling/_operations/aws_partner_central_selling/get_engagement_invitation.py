"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#GetEngagementInvitation``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.get_engagement_invitation_response.GetEngagementInvitationResponse = capo_partnercentral_selling.types.get_engagement_invitation_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetEngagementInvitation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.GetEngagementInvitation"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.get_engagement_invitation_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_get_engagement_invitation(
