"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#ListEngagementInvitations``."""
...
import capo_partnercentral_selling.types.engagement_invitation_summaries
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.list_engagement_invitations_response.ListEngagementInvitationsResponse = capo_partnercentral_selling.types.list_engagement_invitations_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListEngagementInvitations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.ListEngagementInvitations"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.list_engagement_invitations_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_list_engagement_invitations(
