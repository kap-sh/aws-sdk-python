"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#UpdateOpportunity``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.update_opportunity_response.UpdateOpportunityResponse = capo_partnercentral_selling.types.update_opportunity_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateOpportunity"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.UpdateOpportunity"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.update_opportunity_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_update_opportunity(
