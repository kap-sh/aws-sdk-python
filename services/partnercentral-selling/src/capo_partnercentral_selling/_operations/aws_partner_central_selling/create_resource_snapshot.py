"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#CreateResourceSnapshot``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.create_resource_snapshot_response.CreateResourceSnapshotResponse = capo_partnercentral_selling.types.create_resource_snapshot_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateResourceSnapshot"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.CreateResourceSnapshot"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.create_resource_snapshot_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_create_resource_snapshot(
