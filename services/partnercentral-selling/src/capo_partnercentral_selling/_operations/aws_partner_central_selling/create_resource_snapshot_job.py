"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#CreateResourceSnapshotJob``."""
...
def handle_response(
    ...
    out: capo_partnercentral_selling.types.create_resource_snapshot_job_response.CreateResourceSnapshotJobResponse = capo_partnercentral_selling.types.create_resource_snapshot_job_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateResourceSnapshotJob"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.CreateResourceSnapshotJob"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.create_resource_snapshot_job_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_create_resource_snapshot_job(
