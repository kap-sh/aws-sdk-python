"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#AcceptEngagementInvitation``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/AcceptEngagementInvitation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.AcceptEngagementInvitation"
    body: bytes | None = json.dumps(
        capo_partnercentral_selling.types.accept_engagement_invitation_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_accept_engagement_invitation(
