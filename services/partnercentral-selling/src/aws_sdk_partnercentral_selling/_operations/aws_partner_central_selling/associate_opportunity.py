"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#AssociateOpportunity``."""
...
def build_request(
    ...
    input_: aws_sdk_partnercentral_selling.types.associate_opportunity_request.AssociateOpportunityRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/AssociateOpportunity"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.AssociateOpportunity"
    import aws_sdk_partnercentral_selling.types.associate_opportunity_request

    body: bytes | None = json.dumps(
        aws_sdk_partnercentral_selling.types.associate_opportunity_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_associate_opportunity(
