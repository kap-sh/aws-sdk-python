"""Generated from Smithy shape ``com.amazonaws.partnercentralselling#ListEngagements``."""
...
def handle_response(
    ...
    out: aws_sdk_partnercentral_selling.types.list_engagements_response.ListEngagementsResponse = aws_sdk_partnercentral_selling.types.list_engagements_response.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_partnercentral_selling.types.list_engagements_request.ListEngagementsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/ListEngagements"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "AWSPartnerCentralSelling.ListEngagements"
    import aws_sdk_partnercentral_selling.types.list_engagements_request

    body: bytes | None = json.dumps(
        aws_sdk_partnercentral_selling.types.list_engagements_request.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_list_engagements(
