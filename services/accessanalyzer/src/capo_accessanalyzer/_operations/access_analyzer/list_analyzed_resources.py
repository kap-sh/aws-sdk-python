"""Generated from Smithy shape ``com.amazonaws.accessanalyzer#ListAnalyzedResources``."""
...
def handle_response(
    ...
    out: capo_accessanalyzer.types.list_analyzed_resources_response.ListAnalyzedResourcesResponse = capo_accessanalyzer.types.list_analyzed_resources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/analyzed-resource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_accessanalyzer.types.list_analyzed_resources_request.serialize_json(input_)
    ).encode()
...
async def async_list_analyzed_resources(
