...
class Access(TypedDict, closed=True):
    actions: "capo_accessanalyzer.types.actions_list.ActionsList"
    ...
    resources: "capo_accessanalyzer.types.resources_list.ResourcesList"
...
def serialize_json(value: Access) -> dict:
    ...
    out["actions"] = capo_accessanalyzer.types.actions_list.serialize_json(
        value.get("actions", [])
    )
    import capo_accessanalyzer.types.resources_list

    out["resources"] = capo_accessanalyzer.types.resources_list.serialize_json(
        value.get("resources", [])
    )
    return out
