...
class GetArchiveRuleResponse(TypedDict):
    archive_rule: "aws_sdk_accessanalyzer.types.archive_rule_summary.ArchiveRuleSummary"
...
def serialize_json(value: GetArchiveRuleResponse) -> dict:
    ...
    out["archiveRule"] = (
        aws_sdk_accessanalyzer.types.archive_rule_summary.serialize_json(
            value["archive_rule"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> GetArchiveRuleResponse:
    ...
    if "archiveRule" in data:
        ...
        out["archive_rule"] = (
            aws_sdk_accessanalyzer.types.archive_rule_summary.deserialize_json(
                data["archiveRule"]
            )
        )
    else:
