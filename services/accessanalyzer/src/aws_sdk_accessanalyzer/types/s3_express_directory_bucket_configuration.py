...
class S3ExpressDirectoryBucketConfiguration(TypedDict, closed=True):
    ...
    access_points: NotRequired[
        "aws_sdk_accessanalyzer.types.s3_express_directory_access_point_configurations_map.S3ExpressDirectoryAccessPointConfigurationsMap"
    ]
...
def serialize_json(value: S3ExpressDirectoryBucketConfiguration) -> dict:
    ...
    if "access_points" in value:
        ...
        out["accessPoints"] = (
            aws_sdk_accessanalyzer.types.s3_express_directory_access_point_configurations_map.serialize_json(
                value["access_points"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> S3ExpressDirectoryBucketConfiguration:
    ...
    if "accessPoints" in data:
        ...
        out["access_points"] = (
            aws_sdk_accessanalyzer.types.s3_express_directory_access_point_configurations_map.deserialize_json(
                data["accessPoints"]
            )
        )
    return out
