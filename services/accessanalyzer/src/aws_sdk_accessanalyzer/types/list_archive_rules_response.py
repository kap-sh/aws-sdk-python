...
if TYPE_CHECKING:
    import aws_sdk_accessanalyzer.types.archive_rules_list
...
def serialize_json(value: ListArchiveRulesResponse) -> dict:
    ...
    out["archiveRules"] = (
        aws_sdk_accessanalyzer.types.archive_rules_list.serialize_json(
            value["archive_rules"]
        )
    )
...
def deserialize_json(data: dict) -> ListArchiveRulesResponse:
    ...
    if "archiveRules" in data:
        ...
        out["archive_rules"] = (
            aws_sdk_accessanalyzer.types.archive_rules_list.deserialize_json(
                data["archiveRules"]
            )
        )
    else:
