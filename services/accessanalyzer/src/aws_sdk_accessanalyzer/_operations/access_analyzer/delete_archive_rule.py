"""Generated from Smithy shape ``com.amazonaws.accessanalyzer#DeleteArchiveRule``."""
...
if TYPE_CHECKING:
    import aws_sdk_accessanalyzer.types.delete_archive_rule_request
...
def build_request(
    ...
    input: aws_sdk_accessanalyzer.types.delete_archive_rule_request.DeleteArchiveRuleRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/analyzer/{analyzerName}/archive-rule/{ruleName}"
    url = url.replace("{analyzerName}", quote(str(input["analyzer_name"]), safe=""))
    url = url.replace("{ruleName}", quote(str(input["rule_name"]), safe=""))
    params: dict[str, str] = {}
...
async def async_delete_archive_rule(
