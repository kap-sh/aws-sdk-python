"""Generated from Smithy shape ``com.amazonaws.accessanalyzer#StartResourceScan``."""
...
def build_request(
    ...
    input_: aws_sdk_accessanalyzer.types.start_resource_scan_request.StartResourceScanRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/resource/scan"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_accessanalyzer.types.start_resource_scan_request

    body: bytes | None = json.dumps(
        aws_sdk_accessanalyzer.types.start_resource_scan_request.serialize_json(input_)
    ).encode()
