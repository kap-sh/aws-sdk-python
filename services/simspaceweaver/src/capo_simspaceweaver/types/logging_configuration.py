...
class LoggingConfiguration(TypedDict, closed=True):
    destinations: NotRequired[
        "capo_simspaceweaver.types.log_destinations.LogDestinations"
    ]
...
def serialize_json(value: LoggingConfiguration) -> dict:
    ...
    if "destinations" in value:
        ...
        out["Destinations"] = capo_simspaceweaver.types.log_destinations.serialize_json(
            value["destinations"]
        )
    return out
...
def deserialize_json(data: dict) -> LoggingConfiguration:
    ...
    if "Destinations" in data:
        ...
        out["destinations"] = (
            capo_simspaceweaver.types.log_destinations.deserialize_json(
                data["Destinations"]
            )
        )
    return out
