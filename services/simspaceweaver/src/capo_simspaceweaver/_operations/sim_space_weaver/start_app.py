"""Generated from Smithy shape ``com.amazonaws.simspaceweaver#StartApp``."""
...
import capo_simspaceweaver.errors.conflict_exception
import capo_simspaceweaver.errors.internal_server_exception
import capo_simspaceweaver.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_simspaceweaver.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ServiceQuotaExceededException":
            raise capo_simspaceweaver.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_simspaceweaver.types.start_app_output.StartAppOutput = (
        capo_simspaceweaver.types.start_app_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/startapp"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_simspaceweaver.types.start_app_input.serialize_json(input_)
    ).encode()
...
def start_app(
    ...
    input_: capo_simspaceweaver.types.start_app_input.StartAppInput,
) -> tuple[capo_simspaceweaver.types.start_app_output.StartAppOutput, zapros.Response]:
