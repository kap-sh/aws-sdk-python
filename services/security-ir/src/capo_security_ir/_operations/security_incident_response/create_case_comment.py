"""Generated from Smithy shape ``com.amazonaws.securityir#CreateCaseComment``."""
...
def handle_response(
    ...
    out: capo_security_ir.types.create_case_comment_response.CreateCaseCommentResponse = capo_security_ir.types.create_case_comment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/cases/{caseId}/create-comment"
    url = url.replace("{caseId}", quote(str(input_["case_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.create_case_comment_request.serialize_json(input_)
    ).encode()
...
async def async_create_case_comment(
