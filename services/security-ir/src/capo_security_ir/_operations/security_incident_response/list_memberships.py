"""Generated from Smithy shape ``com.amazonaws.securityir#ListMemberships``."""
...
import capo_security_ir.types.list_membership_items
...
def handle_response(
    ...
    out: capo_security_ir.types.list_memberships_response.ListMembershipsResponse = (
        capo_security_ir.types.list_memberships_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/memberships"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_security_ir.types.list_memberships_request.serialize_json(input_)
    ).encode()
