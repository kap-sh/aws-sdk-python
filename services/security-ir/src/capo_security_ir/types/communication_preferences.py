...
if TYPE_CHECKING:
    import capo_security_ir.types.communication_type

CommunicationPreferences: TypeAlias = list[
    "capo_security_ir.types.communication_type.CommunicationType"
]
...
def serialize_json(value: CommunicationPreferences) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_security_ir.types.communication_type.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> CommunicationPreferences:
    ...
    for item in data:
        out.append(capo_security_ir.types.communication_type.deserialize_json(item))
    return out
