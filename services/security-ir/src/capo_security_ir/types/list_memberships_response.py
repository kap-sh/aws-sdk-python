...
class ListMembershipsResponse(TypedDict, closed=True):
    ...
    items: NotRequired[
        "capo_security_ir.types.list_membership_items.ListMembershipItems"
    ]
...
def serialize_json(value: ListMembershipsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_security_ir.types.list_membership_items.serialize_json(
            value["items"]
        )
    return out
...
def deserialize_json(data: dict) -> ListMembershipsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_security_ir.types.list_membership_items.deserialize_json(
            data["items"]
        )
    return out
