...
def serialize_json(value: BatchGetMemberAccountDetailsRequest) -> dict:
    out: dict = {}
    import aws_sdk_security_ir.types.aws_account_ids
    out["accountIds"] = aws_sdk_security_ir.types.aws_account_ids.serialize_json(value["account_ids"])
    return out
...
def deserialize_json(data: dict) -> BatchGetMemberAccountDetailsRequest:
    ...
    if "accountIds" in data:
        import aws_sdk_security_ir.types.aws_account_ids
        out["account_ids"] = aws_sdk_security_ir.types.aws_account_ids.deserialize_json(data["accountIds"])
    else:
        raise DeserializationError("BatchGetMemberAccountDetailsRequest.account_ids required")
    return out