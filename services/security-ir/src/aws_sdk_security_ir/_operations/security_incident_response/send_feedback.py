...
def build_request(
    ...
    input: aws_sdk_security_ir.types.send_feedback_request.SendFeedbackRequest,
) -> zapros.Request:
    ...
    url = url.replace("{resultId}", quote(str(input["result_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_security_ir.types.send_feedback_request

    body: bytes | None = json.dumps(
        aws_sdk_security_ir.types.send_feedback_request.serialize_json(input)
    ).encode()
