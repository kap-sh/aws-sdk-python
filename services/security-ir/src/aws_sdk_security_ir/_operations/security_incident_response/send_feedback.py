"""Generated from Smithy shape ``com.amazonaws.securityir#SendFeedback``."""
...
def handle_response(
    ...
    out: aws_sdk_security_ir.types.send_feedback_response.SendFeedbackResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_security_ir.types.send_feedback_request.SendFeedbackRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{resultId}", quote(str(input_["result_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_security_ir.types.send_feedback_request

    body: bytes | None = json.dumps(
        aws_sdk_security_ir.types.send_feedback_request.serialize_json(input_)
    ).encode()
