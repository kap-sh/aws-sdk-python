"""Generated from Smithy shape ``com.amazonaws.securityir#UpdateResolverType``."""
...
def handle_response(
    ...
    out: aws_sdk_security_ir.types.update_resolver_type_response.UpdateResolverTypeResponse = aws_sdk_security_ir.types.update_resolver_type_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_security_ir.types.update_resolver_type_request.UpdateResolverTypeRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/cases/{caseId}/update-resolver-type"
    url = url.replace("{caseId}", quote(str(input_["case_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_security_ir.types.update_resolver_type_request.serialize_json(input_)
    ).encode()
...
async def async_update_resolver_type(
