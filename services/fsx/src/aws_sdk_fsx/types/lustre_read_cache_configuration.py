...
class LustreReadCacheConfiguration(TypedDict, closed=True):
    ...
    size_gi_b: NotRequired["aws_sdk_fsx.types.storage_capacity.StorageCapacity"]
    """<p> Required if <code>SizingMode</code> is set to <code>USER_PROVISIONED</code>. Specifies the size of the file system's SSD read cache, in gibibytes (GiB). </p> <p>The SSD read cache size is distributed across provisioned file servers in your file system. Intelligent-Tiering file systems support a minimum of 32 GiB and maximum of 131072 GiB for SSD read cache size for every 4,000 MB/s of throughput capacity provisioned.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: LustreReadCacheConfiguration) -> dict:
    out: dict = {}
    if "sizing_mode" in value:
        import aws_sdk_fsx.types.lustre_read_cache_sizing_mode

        out["SizingMode"] = (
            aws_sdk_fsx.types.lustre_read_cache_sizing_mode.serialize_aws_json_1_1(
                value["sizing_mode"]
            )
        )
...
def deserialize_aws_json_1_1(data: dict) -> LustreReadCacheConfiguration:
    ...
    if "SizingMode" in data:
        ...
        out["sizing_mode"] = (
            aws_sdk_fsx.types.lustre_read_cache_sizing_mode.deserialize_aws_json_1_1(
                data["SizingMode"]
            )
        )
    if "SizeGiB" in data:
