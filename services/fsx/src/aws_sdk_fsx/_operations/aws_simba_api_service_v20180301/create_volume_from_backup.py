"""Generated from Smithy shape ``com.amazonaws.fsx#CreateVolumeFromBackup``."""
...
def build_request(
    ...
    input_: aws_sdk_fsx.types.create_volume_from_backup_request.CreateVolumeFromBackupRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    headers["X-Amz-Target"] = "AWSSimbaAPIService_v20180301.CreateVolumeFromBackup"
    import aws_sdk_fsx.types.create_volume_from_backup_request

    body: bytes | None = json.dumps(
        aws_sdk_fsx.types.create_volume_from_backup_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_create_volume_from_backup(
