...
class FSxError(Exception):
    """Base class for all FSx service errors."""


class ServiceError(FSxError):
...
class SerializationError(FSxError):
    """Raised when a value cannot be serialized."""


class DeserializationError(FSxError):
...
class WaiterFailedError(FSxError):
...
class WaiterTimeoutError(FSxError):
