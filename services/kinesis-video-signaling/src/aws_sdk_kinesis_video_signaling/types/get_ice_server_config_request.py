...
class GetIceServerConfigRequest(TypedDict):
    channel_arn: NotRequired[
        "aws_sdk_kinesis_video_signaling.types.resource_arn.ResourceARN"
    ]
    """<p>The ARN of the signaling channel to be used for the peer-to-peer connection between configured peers. </p>"""
    client_id: NotRequired["aws_sdk_kinesis_video_signaling.types.client_id.ClientId"]
    """<p>Unique identifier for the viewer. Must be unique within the signaling channel.</p>"""
    service: NotRequired["aws_sdk_kinesis_video_signaling.types.service.Service"]
...
def serialize_json(value: GetIceServerConfigRequest) -> dict:
    ...
    if "service" in value:
        ...
        out["Service"] = aws_sdk_kinesis_video_signaling.types.service.serialize_json(
            value["service"]
        )
...
def deserialize_json(data: dict) -> GetIceServerConfigRequest:
    ...
    if "Service" in data:
        ...
        out["service"] = aws_sdk_kinesis_video_signaling.types.service.deserialize_json(
            data["Service"]
        )
    if "Username" in data:
