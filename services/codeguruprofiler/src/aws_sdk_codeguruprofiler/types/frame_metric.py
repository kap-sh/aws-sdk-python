...
class FrameMetric(TypedDict, closed=True):
    ...
    thread_states: "aws_sdk_codeguruprofiler.types.thread_states.ThreadStates"
...
def serialize_json(value: FrameMetric) -> dict:
    ...
    out["threadStates"] = aws_sdk_codeguruprofiler.types.thread_states.serialize_json(
        value["thread_states"]
    )
    return out
...
def deserialize_json(data: dict) -> FrameMetric:
    ...
    if "threadStates" in data:
        ...
        out["thread_states"] = (
            aws_sdk_codeguruprofiler.types.thread_states.deserialize_json(
                data["threadStates"]
            )
        )
    else:
