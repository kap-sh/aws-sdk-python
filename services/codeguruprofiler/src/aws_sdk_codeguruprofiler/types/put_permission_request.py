...
class PutPermissionRequest(TypedDict, closed=True):
    ...
    revision_id: NotRequired["aws_sdk_codeguruprofiler.types.revision_id.RevisionId"]
...
def serialize_json(value: PutPermissionRequest) -> dict:
    ...
    out["principals"] = aws_sdk_codeguruprofiler.types.principals.serialize_json(
        value["principals"]
    )
...
def deserialize_json(data: dict) -> PutPermissionRequest:
    ...
    if "principals" in data:
        ...
        out["principals"] = aws_sdk_codeguruprofiler.types.principals.deserialize_json(
            data["principals"]
        )
    else:
