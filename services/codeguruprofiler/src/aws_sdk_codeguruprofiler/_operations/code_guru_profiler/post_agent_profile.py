...
def handle_response(
    ...
    out: aws_sdk_codeguruprofiler.types.post_agent_profile_response.PostAgentProfileResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_codeguruprofiler.types.post_agent_profile_request.PostAgentProfileRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "profile_token" in input:
        params["profileToken"] = str(input["profile_token"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "content_type" in input:
        headers["Content-Type"] = str(input["content_type"])
    if "agent_profile" in input:
