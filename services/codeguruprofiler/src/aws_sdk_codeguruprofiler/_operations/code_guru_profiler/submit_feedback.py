...
def handle_response(
    ...
    out: aws_sdk_codeguruprofiler.types.submit_feedback_response.SubmitFeedbackResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_codeguruprofiler.types.submit_feedback_request.SubmitFeedbackRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{anomalyInstanceId}", quote(str(input_["anomaly_instance_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_codeguruprofiler.types.submit_feedback_request.serialize_json(input_)
    ).encode()
