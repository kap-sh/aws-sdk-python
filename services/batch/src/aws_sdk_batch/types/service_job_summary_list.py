...
ServiceJobSummaryList: TypeAlias = list[
    "aws_sdk_batch.types.service_job_summary.ServiceJobSummary"
]
...
def serialize_json(value: ServiceJobSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_batch.types.service_job_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ServiceJobSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_batch.types.service_job_summary.deserialize_json(item))
    return out
