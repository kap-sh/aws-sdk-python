...
class EvaluateOnExit(TypedDict):
    ...
    on_exit_code: NotRequired["aws_sdk_batch.types.string.String"]
    ...
    action: NotRequired["aws_sdk_batch.types.retry_action.RetryAction"]
...
def serialize_json(value: EvaluateOnExit) -> dict:
    ...
    if "action" in value:
        ...
        out["action"] = aws_sdk_batch.types.retry_action.serialize_json(value["action"])
    return out
...
def deserialize_json(data: dict) -> EvaluateOnExit:
    ...
    if "action" in data:
        ...
        out["action"] = aws_sdk_batch.types.retry_action.deserialize_json(
            data["action"]
        )
    return out
