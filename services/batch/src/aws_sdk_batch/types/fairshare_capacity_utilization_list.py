...
if TYPE_CHECKING:
    import aws_sdk_batch.types.fairshare_capacity_utilization

FairshareCapacityUtilizationList: TypeAlias = list[
    "aws_sdk_batch.types.fairshare_capacity_utilization.FairshareCapacityUtilization"
]
...
def serialize_json(value: FairshareCapacityUtilizationList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_batch.types.fairshare_capacity_utilization.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> FairshareCapacityUtilizationList:
    ...
    for item in data:
        out.append(
            aws_sdk_batch.types.fairshare_capacity_utilization.deserialize_json(item)
        )
    return out
