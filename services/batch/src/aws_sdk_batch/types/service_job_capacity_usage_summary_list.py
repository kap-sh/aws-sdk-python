...
if TYPE_CHECKING:
    import aws_sdk_batch.types.service_job_capacity_usage_summary

ServiceJobCapacityUsageSummaryList: TypeAlias = list[
    "aws_sdk_batch.types.service_job_capacity_usage_summary.ServiceJobCapacityUsageSummary"
]
...
def serialize_json(value: ServiceJobCapacityUsageSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_batch.types.service_job_capacity_usage_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ServiceJobCapacityUsageSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_batch.types.service_job_capacity_usage_summary.deserialize_json(
                item
            )
        )
    return out
