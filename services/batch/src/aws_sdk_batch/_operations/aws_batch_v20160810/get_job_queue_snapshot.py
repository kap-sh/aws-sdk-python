"""Generated from Smithy shape ``com.amazonaws.batch#GetJobQueueSnapshot``."""
...
def handle_response(
    ...
    out: aws_sdk_batch.types.get_job_queue_snapshot_response.GetJobQueueSnapshotResponse = aws_sdk_batch.types.get_job_queue_snapshot_response.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_batch._auth._sigv4.build_sigv4_auth_scheme(
                "batch", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_batch._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_batch.types.get_job_queue_snapshot_request.GetJobQueueSnapshotRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/getjobqueuesnapshot"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_batch.types.get_job_queue_snapshot_request.serialize_json(input_)
    ).encode()
...
async def async_get_job_queue_snapshot(
