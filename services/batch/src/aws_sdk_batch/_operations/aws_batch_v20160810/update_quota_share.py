"""Generated from Smithy shape ``com.amazonaws.batch#UpdateQuotaShare``."""
...
def handle_response(
    ...
    out: aws_sdk_batch.types.update_quota_share_response.UpdateQuotaShareResponse = (
        aws_sdk_batch.types.update_quota_share_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_batch.types.update_quota_share_request.UpdateQuotaShareRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/updatequotashare"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_batch.types.update_quota_share_request

    body: bytes | None = json.dumps(
        aws_sdk_batch.types.update_quota_share_request.serialize_json(input_)
    ).encode()
...
async def async_update_quota_share(
