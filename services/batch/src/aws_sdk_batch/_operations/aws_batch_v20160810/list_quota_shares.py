"""Generated from Smithy shape ``com.amazonaws.batch#ListQuotaShares``."""
...
def handle_response(
    ...
    out: aws_sdk_batch.types.list_quota_shares_response.ListQuotaSharesResponse = (
        aws_sdk_batch.types.list_quota_shares_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_batch.types.list_quota_shares_request.ListQuotaSharesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/listquotashares"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_batch.types.list_quota_shares_request.serialize_json(input_)
    ).encode()
...
async def async_list_quota_shares(
