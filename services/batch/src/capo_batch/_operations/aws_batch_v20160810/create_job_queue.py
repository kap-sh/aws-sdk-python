"""Generated from Smithy shape ``com.amazonaws.batch#CreateJobQueue``."""
...
def handle_response(
    ...
    out: capo_batch.types.create_job_queue_response.CreateJobQueueResponse = (
        capo_batch.types.create_job_queue_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/createjobqueue"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.create_job_queue_request.serialize_json(input_)
    ).encode()
...
def create_job_queue(
    ...
    capo_batch.types.create_job_queue_response.CreateJobQueueResponse, zapros.Response
]:
...
async def async_create_job_queue(
