"""Generated from Smithy shape ``com.amazonaws.batch#DeleteSchedulingPolicy``."""
...
def handle_response(
    ...
    out: capo_batch.types.delete_scheduling_policy_response.DeleteSchedulingPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/deleteschedulingpolicy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.delete_scheduling_policy_request.serialize_json(input_)
    ).encode()
...
async def async_delete_scheduling_policy(
