"""Generated from Smithy shape ``com.amazonaws.batch#CreateComputeEnvironment``."""
...
def handle_response(
    ...
    out: capo_batch.types.create_compute_environment_response.CreateComputeEnvironmentResponse = capo_batch.types.create_compute_environment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/createcomputeenvironment"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.create_compute_environment_request.serialize_json(input_)
    ).encode()
...
async def async_create_compute_environment(
