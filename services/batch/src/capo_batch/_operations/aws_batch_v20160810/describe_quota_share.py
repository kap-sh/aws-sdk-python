"""Generated from Smithy shape ``com.amazonaws.batch#DescribeQuotaShare``."""
...
def handle_response(
    ...
    out: capo_batch.types.describe_quota_share_response.DescribeQuotaShareResponse = (
        capo_batch.types.describe_quota_share_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/describequotashare"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.describe_quota_share_request.serialize_json(input_)
    ).encode()
...
async def async_describe_quota_share(
