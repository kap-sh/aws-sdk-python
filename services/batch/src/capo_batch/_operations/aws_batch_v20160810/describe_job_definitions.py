"""Generated from Smithy shape ``com.amazonaws.batch#DescribeJobDefinitions``."""
...
def handle_response(
    ...
    out: capo_batch.types.describe_job_definitions_response.DescribeJobDefinitionsResponse = capo_batch.types.describe_job_definitions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/describejobdefinitions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.describe_job_definitions_request.serialize_json(input_)
    ).encode()
...
async def async_describe_job_definitions(
