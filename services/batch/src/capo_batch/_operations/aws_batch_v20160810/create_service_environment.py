"""Generated from Smithy shape ``com.amazonaws.batch#CreateServiceEnvironment``."""
...
def handle_response(
    ...
    out: capo_batch.types.create_service_environment_response.CreateServiceEnvironmentResponse = capo_batch.types.create_service_environment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/createserviceenvironment"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.create_service_environment_request.serialize_json(input_)
    ).encode()
...
async def async_create_service_environment(
