"""Generated from Smithy shape ``com.amazonaws.batch#ListJobs``."""
...
def handle_response(
    ...
    out: capo_batch.types.list_jobs_response.ListJobsResponse = (
        capo_batch.types.list_jobs_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/listjobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.list_jobs_request.serialize_json(input_)
    ).encode()
...
def list_jobs(
    ...
    input_: capo_batch.types.list_jobs_request.ListJobsRequest,
) -> tuple[capo_batch.types.list_jobs_response.ListJobsResponse, zapros.Response]:
