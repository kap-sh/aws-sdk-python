"""Generated from Smithy shape ``com.amazonaws.batch#TerminateServiceJob``."""
...
def handle_response(
    ...
    out: capo_batch.types.terminate_service_job_response.TerminateServiceJobResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/terminateservicejob"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.terminate_service_job_request.serialize_json(input_)
    ).encode()
...
async def async_terminate_service_job(
