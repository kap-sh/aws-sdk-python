"""Generated from Smithy shape ``com.amazonaws.batch#DeleteConsumableResource``."""
...
def handle_response(
    ...
    out: capo_batch.types.delete_consumable_resource_response.DeleteConsumableResourceResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/deleteconsumableresource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.delete_consumable_resource_request.serialize_json(input_)
    ).encode()
...
async def async_delete_consumable_resource(
