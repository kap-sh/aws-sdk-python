"""Generated from Smithy shape ``com.amazonaws.batch#ListJobsByConsumableResource``."""
...
import capo_batch.errors.client_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ClientException":
            raise capo_batch.errors.client_exception.ClientException.from_json(data)
        case "ServerException":
            raise capo_batch.errors.server_exception.ServerException.from_json(data)
        case _:
...
def handle_response(
    ...
    out: capo_batch.types.list_jobs_by_consumable_resource_response.ListJobsByConsumableResourceResponse = capo_batch.types.list_jobs_by_consumable_resource_response.deserialize_json(
...
def get_signer(
...
) -> capo_batch._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_batch._auth._sigv4.build_sigv4_auth_scheme("batch", options.region)
        )
        if sigv4_config is not None:
            return capo_batch._auth._signers.SigV4Signer(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/listjobsbyconsumableresource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.list_jobs_by_consumable_resource_request.serialize_json(input_)
    ).encode()
...
async def async_list_jobs_by_consumable_resource(
