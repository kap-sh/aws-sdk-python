"""Generated from Smithy shape ``com.amazonaws.batch#CreateConsumableResource``."""
...
def handle_response(
    ...
    out: capo_batch.types.create_consumable_resource_response.CreateConsumableResourceResponse = capo_batch.types.create_consumable_resource_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/createconsumableresource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_batch.types.create_consumable_resource_request.serialize_json(input_)
    ).encode()
...
async def async_create_consumable_resource(
