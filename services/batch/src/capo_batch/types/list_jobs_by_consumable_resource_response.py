...
class ListJobsByConsumableResourceResponse(TypedDict, closed=True):
    jobs: NotRequired[
        "capo_batch.types.list_jobs_by_consumable_resource_summary_list.ListJobsByConsumableResourceSummaryList"
    ]
...
def serialize_json(value: ListJobsByConsumableResourceResponse) -> dict:
    ...
    if "jobs" in value:
        ...
        out["jobs"] = (
            capo_batch.types.list_jobs_by_consumable_resource_summary_list.serialize_json(
                value["jobs"]
            )
        )
...
def deserialize_json(data: dict) -> ListJobsByConsumableResourceResponse:
    ...
    if "jobs" in data:
        ...
        out["jobs"] = (
            capo_batch.types.list_jobs_by_consumable_resource_summary_list.deserialize_json(
                data["jobs"]
            )
        )
