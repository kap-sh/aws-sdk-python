...
class RetryStrategy(TypedDict, closed=True):
    ...
    evaluate_on_exit: NotRequired[
        "capo_batch.types.evaluate_on_exit_list.EvaluateOnExitList"
    ]
...
def serialize_json(value: RetryStrategy) -> dict:
    ...
    if "evaluate_on_exit" in value:
        ...
        out["evaluateOnExit"] = capo_batch.types.evaluate_on_exit_list.serialize_json(
...
def deserialize_json(data: dict) -> RetryStrategy:
    ...
    if "evaluateOnExit" in data:
        ...
        out["evaluate_on_exit"] = (
            capo_batch.types.evaluate_on_exit_list.deserialize_json(
                data["evaluateOnExit"]
            )
        )
    return out
