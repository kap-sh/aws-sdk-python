...
class FrontOfQuotaSharesDetail(TypedDict, closed=True):
    quota_shares: NotRequired[
        "capo_batch.types.front_of_quota_shares_job_summary_map.FrontOfQuotaSharesJobSummaryMap"
    ]
...
def serialize_json(value: FrontOfQuotaSharesDetail) -> dict:
    ...
    if "quota_shares" in value:
        ...
        out["quotaShares"] = (
            capo_batch.types.front_of_quota_shares_job_summary_map.serialize_json(
                value["quota_shares"]
            )
        )
...
def deserialize_json(data: dict) -> FrontOfQuotaSharesDetail:
    ...
    if "quotaShares" in data:
        ...
        out["quota_shares"] = (
            capo_batch.types.front_of_quota_shares_job_summary_map.deserialize_json(
                data["quotaShares"]
            )
        )
