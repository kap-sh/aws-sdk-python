"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#PushConfig``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_iot_managed_integrations.types.ota_task_abort_config
    import capo_iot_managed_integrations.types.ota_task_execution_rollout_config
    import capo_iot_managed_integrations.types.ota_task_timeout_config


class PushConfig(TypedDict, closed=True):
    abort_config: NotRequired[
        "capo_iot_managed_integrations.types.ota_task_abort_config.OtaTaskAbortConfig"
    ]
    """<p>Structure representing one abort config.</p>"""
    rollout_config: NotRequired[
        "capo_iot_managed_integrations.types.ota_task_execution_rollout_config.OtaTaskExecutionRolloutConfig"
    ]
    """<p>Structure representing one rollout config.</p>"""
    timeout_config: NotRequired[
        "capo_iot_managed_integrations.types.ota_task_timeout_config.OtaTaskTimeoutConfig"
    ]
    """<p>Structure representing one timeout config.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: PushConfig) -> dict:
    out: dict = {}
    if "abort_config" in value:
        import capo_iot_managed_integrations.types.ota_task_abort_config

        out["AbortConfig"] = (
            capo_iot_managed_integrations.types.ota_task_abort_config.serialize_json(
                value["abort_config"]
            )
        )
    if "rollout_config" in value:
        import capo_iot_managed_integrations.types.ota_task_execution_rollout_config

        out["RolloutConfig"] = (
            capo_iot_managed_integrations.types.ota_task_execution_rollout_config.serialize_json(
                value["rollout_config"]
            )
        )
    if "timeout_config" in value:
        import capo_iot_managed_integrations.types.ota_task_timeout_config

        out["TimeoutConfig"] = (
            capo_iot_managed_integrations.types.ota_task_timeout_config.serialize_json(
                value["timeout_config"]
            )
        )
    return out


def deserialize_json(data: dict) -> PushConfig:
    out: PushConfig = {}  # type: ignore[typeddict-item]
    if "AbortConfig" in data:
        import capo_iot_managed_integrations.types.ota_task_abort_config

        out["abort_config"] = (
            capo_iot_managed_integrations.types.ota_task_abort_config.deserialize_json(
                data["AbortConfig"]
            )
        )
    if "RolloutConfig" in data:
        import capo_iot_managed_integrations.types.ota_task_execution_rollout_config

        out["rollout_config"] = (
            capo_iot_managed_integrations.types.ota_task_execution_rollout_config.deserialize_json(
                data["RolloutConfig"]
            )
        )
    if "TimeoutConfig" in data:
        import capo_iot_managed_integrations.types.ota_task_timeout_config

        out["timeout_config"] = (
            capo_iot_managed_integrations.types.ota_task_timeout_config.deserialize_json(
                data["TimeoutConfig"]
            )
        )
    return out
