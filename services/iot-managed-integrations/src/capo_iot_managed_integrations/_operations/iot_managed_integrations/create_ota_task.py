"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#CreateOtaTask``."""
...
def handle_response(
    ...
    out: capo_iot_managed_integrations.types.create_ota_task_response.CreateOtaTaskResponse = capo_iot_managed_integrations.types.create_ota_task_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ota-tasks"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_managed_integrations.types.create_ota_task_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_ota_task(
