"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#CreateProvisioningProfile``."""
...
def handle_response(
    ...
    out: capo_iot_managed_integrations.types.create_provisioning_profile_response.CreateProvisioningProfileResponse = capo_iot_managed_integrations.types.create_provisioning_profile_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/provisioning-profiles"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot_managed_integrations.types.create_provisioning_profile_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_provisioning_profile(
