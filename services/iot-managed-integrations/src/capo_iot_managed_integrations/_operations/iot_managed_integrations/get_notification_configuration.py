"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#GetNotificationConfiguration``."""
...
def handle_response(
    ...
    out: capo_iot_managed_integrations.types.get_notification_configuration_response.GetNotificationConfigurationResponse = capo_iot_managed_integrations.types.get_notification_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/notification-configurations/{EventType}"
    url = url.replace("{EventType}", quote(str(input_["event_type"]), safe=""))
    params: dict[str, str] = {}
...
async def async_get_notification_configuration(
