...
class SendManagedThingCommandRequest(TypedDict):
    managed_thing_id: (
        "aws_sdk_iot_managed_integrations.types.managed_thing_id.ManagedThingId"
    )
    """<p>The id of the device.</p>"""
    endpoints: (
        "aws_sdk_iot_managed_integrations.types.command_endpoints.CommandEndpoints"
    )
...
def serialize_json(value: SendManagedThingCommandRequest) -> dict:
    ...
    out["Endpoints"] = (
        aws_sdk_iot_managed_integrations.types.command_endpoints.serialize_json(
            value["endpoints"]
        )
    )
...
def deserialize_json(data: dict) -> SendManagedThingCommandRequest:
    ...
    if "Endpoints" in data:
        ...
        out["endpoints"] = (
            aws_sdk_iot_managed_integrations.types.command_endpoints.deserialize_json(
                data["Endpoints"]
            )
        )
    else:
