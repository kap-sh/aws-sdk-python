...
if TYPE_CHECKING:
    import aws_sdk_iot_managed_integrations.types.capability_actions
...
class CommandCapability(TypedDict, closed=True):
    ...
    actions: (
        "aws_sdk_iot_managed_integrations.types.capability_actions.CapabilityActions"
    )
...
def serialize_json(value: CommandCapability) -> dict:
    ...
    out["actions"] = (
        aws_sdk_iot_managed_integrations.types.capability_actions.serialize_json(
            value["actions"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> CommandCapability:
    ...
    if "actions" in data:
        ...
        out["actions"] = (
            aws_sdk_iot_managed_integrations.types.capability_actions.deserialize_json(
                data["actions"]
            )
        )
    else:
