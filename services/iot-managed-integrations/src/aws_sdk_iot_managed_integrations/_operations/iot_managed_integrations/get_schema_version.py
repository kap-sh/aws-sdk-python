"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#GetSchemaVersion``."""
...
def handle_response(
    ...
    out: aws_sdk_iot_managed_integrations.types.get_schema_version_response.GetSchemaVersionResponse = aws_sdk_iot_managed_integrations.types.get_schema_version_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iot_managed_integrations.types.get_schema_version_request.GetSchemaVersionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/schema-versions/{Type}/{SchemaVersionedId}"
    url = url.replace("{Type}", quote(str(input_["type"]), safe=""))
    url = url.replace(
        "{SchemaVersionedId}", quote(str(input_["schema_versioned_id"]), safe="")
    )
    params: dict[str, str] = {}
    if "format" in input_:
...
async def async_get_schema_version(
