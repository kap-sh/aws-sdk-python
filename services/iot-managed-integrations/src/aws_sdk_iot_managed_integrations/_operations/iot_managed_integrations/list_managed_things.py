...
def build_request(
    ...
    input: aws_sdk_iot_managed_integrations.types.list_managed_things_request.ListManagedThingsRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "owner_filter" in input:
        params["OwnerFilter"] = str(input["owner_filter"])
    if "credential_locker_filter" in input:
        params["CredentialLockerFilter"] = str(input["credential_locker_filter"])
    if "role_filter" in input:
        params["RoleFilter"] = str(input["role_filter"])
    if "parent_controller_identifier_filter" in input:
        params["ParentControllerIdentifierFilter"] = str(
            input["parent_controller_identifier_filter"]
        )
    if "connector_policy_id_filter" in input:
        params["ConnectorPolicyIdFilter"] = str(input["connector_policy_id_filter"])
    if "connector_destination_id_filter" in input:
        params["ConnectorDestinationIdFilter"] = str(
            input["connector_destination_id_filter"]
        )
    if "connector_device_id_filter" in input:
        params["ConnectorDeviceIdFilter"] = str(input["connector_device_id_filter"])
    if "serial_number_filter" in input:
        params["SerialNumberFilter"] = str(input["serial_number_filter"])
    if "provisioning_status_filter" in input:
        params["ProvisioningStatusFilter"] = str(input["provisioning_status_filter"])
