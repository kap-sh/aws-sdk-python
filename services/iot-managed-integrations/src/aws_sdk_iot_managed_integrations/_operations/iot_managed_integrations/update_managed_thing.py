"""Generated from Smithy shape ``com.amazonaws.iotmanagedintegrations#UpdateManagedThing``."""
...
def build_request(
    ...
    input_: aws_sdk_iot_managed_integrations.types.update_managed_thing_request.UpdateManagedThingRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/managed-things/{Identifier}"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_iot_managed_integrations.types.update_managed_thing_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_managed_thing(
