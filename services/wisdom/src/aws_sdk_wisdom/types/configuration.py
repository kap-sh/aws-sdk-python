...
class _Configuration_connectConfiguration(TypedDict):
    connectConfiguration: "aws_sdk_wisdom.types.connect_configuration.ConnectConfiguration"
...
def serialize_json(value: Configuration) -> dict:
    if "connectConfiguration" in value:
        import aws_sdk_wisdom.types.connect_configuration
        return {"connectConfiguration": aws_sdk_wisdom.types.connect_configuration.serialize_json(value["connectConfiguration"])}
    else:
...
def deserialize_json(data: dict) -> Configuration:
    if "connectConfiguration" in data:
        import aws_sdk_wisdom.types.connect_configuration
        return {"connectConfiguration": aws_sdk_wisdom.types.connect_configuration.deserialize_json(data["connectConfiguration"])}
    else:
