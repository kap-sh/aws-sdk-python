...
KnowledgeBaseList: TypeAlias = list[
    "aws_sdk_wisdom.types.knowledge_base_summary.KnowledgeBaseSummary"
]
...
def serialize_json(value: KnowledgeBaseList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_wisdom.types.knowledge_base_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> KnowledgeBaseList:
    ...
    for item in data:
        out.append(aws_sdk_wisdom.types.knowledge_base_summary.deserialize_json(item))
    return out
