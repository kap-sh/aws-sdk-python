...
AssistantAssociationOutputData: TypeAlias = _AssistantAssociationOutputData_knowledgeBaseAssociation

# --- restJson1 ser/de ---
def serialize_json(value: AssistantAssociationOutputData) -> dict:
    if "knowledgeBaseAssociation" in value:
        import aws_sdk_wisdom.types.knowledge_base_association_data
        return {"knowledgeBaseAssociation": aws_sdk_wisdom.types.knowledge_base_association_data.serialize_json(value["knowledgeBaseAssociation"])}
    else:
...
def deserialize_json(data: dict) -> AssistantAssociationOutputData:
    if "knowledgeBaseAssociation" in data:
        import aws_sdk_wisdom.types.knowledge_base_association_data
        return {"knowledgeBaseAssociation": aws_sdk_wisdom.types.knowledge_base_association_data.deserialize_json(data["knowledgeBaseAssociation"])}
    else:
        raise DeserializationError("AssistantAssociationOutputData: no recognized variant key")