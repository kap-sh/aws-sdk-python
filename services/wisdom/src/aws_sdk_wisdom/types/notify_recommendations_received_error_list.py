...
if TYPE_CHECKING:
    import aws_sdk_wisdom.types.notify_recommendations_received_error

NotifyRecommendationsReceivedErrorList: TypeAlias = list[
    "aws_sdk_wisdom.types.notify_recommendations_received_error.NotifyRecommendationsReceivedError"
]
...
def serialize_json(value: NotifyRecommendationsReceivedErrorList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_wisdom.types.notify_recommendations_received_error.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> NotifyRecommendationsReceivedErrorList:
    ...
    for item in data:
        out.append(
            aws_sdk_wisdom.types.notify_recommendations_received_error.deserialize_json(
                item
            )
        )
    return out
