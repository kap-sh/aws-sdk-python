...
if TYPE_CHECKING:
    ...
    import capo_wisdom.types.priority
    import capo_wisdom.types.quick_response_query_operator
    import capo_wisdom.types.quick_response_query_value_list


class QuickResponseQueryField(TypedDict, closed=True):
    name: "capo_wisdom.types.non_empty_string.NonEmptyString"
    """<p>The name of the attribute to query the quick responses by.</p>"""
    values: (
        "capo_wisdom.types.quick_response_query_value_list.QuickResponseQueryValueList"
    )
    """<p>The values of the attribute to query the quick responses by.</p>"""
    operator: (
        "capo_wisdom.types.quick_response_query_operator.QuickResponseQueryOperator"
    )
    ...
    priority: NotRequired["capo_wisdom.types.priority.Priority"]
...
def serialize_json(value: QuickResponseQueryField) -> dict:
    ...
    out["values"] = capo_wisdom.types.quick_response_query_value_list.serialize_json(
        value["values"]
    )
...
def deserialize_json(data: dict) -> QuickResponseQueryField:
    ...
    if "values" in data:
        ...
        out["values"] = (
            capo_wisdom.types.quick_response_query_value_list.deserialize_json(
                data["values"]
            )
        )
    else:
