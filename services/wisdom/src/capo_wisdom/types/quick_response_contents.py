...
if TYPE_CHECKING:
    import capo_wisdom.types.quick_response_content_provider


class QuickResponseContents(TypedDict, closed=True):
    plain_text: NotRequired[
        "capo_wisdom.types.quick_response_content_provider.QuickResponseContentProvider"
    ]
...
def serialize_json(value: QuickResponseContents) -> dict:
    ...
    if "plain_text" in value:
        ...
        out["plainText"] = (
            capo_wisdom.types.quick_response_content_provider.serialize_json(
                value["plain_text"]
            )
        )
...
def deserialize_json(data: dict) -> QuickResponseContents:
    ...
    if "plainText" in data:
        ...
        out["plain_text"] = (
            capo_wisdom.types.quick_response_content_provider.deserialize_json(
                data["plainText"]
            )
        )
    if "markdown" in data:
