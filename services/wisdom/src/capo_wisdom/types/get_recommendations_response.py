...
class GetRecommendationsResponse(TypedDict, closed=True):
    recommendations: "capo_wisdom.types.recommendation_list.RecommendationList"
    """<p>The recommendations.</p>"""
    triggers: NotRequired[
        "capo_wisdom.types.recommendation_trigger_list.RecommendationTriggerList"
    ]
...
def serialize_json(value: GetRecommendationsResponse) -> dict:
    ...
    out["recommendations"] = capo_wisdom.types.recommendation_list.serialize_json(
        value["recommendations"]
    )
    if "triggers" in value:
        import capo_wisdom.types.recommendation_trigger_list

        out["triggers"] = capo_wisdom.types.recommendation_trigger_list.serialize_json(
            value["triggers"]
        )
    return out
...
def deserialize_json(data: dict) -> GetRecommendationsResponse:
    ...
    if "triggers" in data:
        ...
        out["triggers"] = (
            capo_wisdom.types.recommendation_trigger_list.deserialize_json(
                data["triggers"]
            )
        )
    return out
