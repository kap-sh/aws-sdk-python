...
class ListEndpointAccessResponse(TypedDict):
    next_token: NotRequired["str"]
    """<p>When <code>nextToken</code> is returned, there are more results available. The value of <code>nextToken</code> is a unique pagination token for each page. Make the call again using the returned token to retrieve the next page.</p>"""
    endpoints: (
        "aws_sdk_redshift_serverless.types.endpoint_access_list.EndpointAccessList"
    )
...
def serialize_aws_json_1_1(value: ListEndpointAccessResponse) -> dict:
    ...
    out["endpoints"] = (
        aws_sdk_redshift_serverless.types.endpoint_access_list.serialize_aws_json_1_1(
            value["endpoints"]
        )
    )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> ListEndpointAccessResponse:
    ...
    if "endpoints" in data:
        ...
        out["endpoints"] = (
            aws_sdk_redshift_serverless.types.endpoint_access_list.deserialize_aws_json_1_1(
                data["endpoints"]
            )
        )
    else:
