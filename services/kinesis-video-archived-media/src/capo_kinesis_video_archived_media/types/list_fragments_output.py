...
class ListFragmentsOutput(TypedDict, closed=True):
    fragments: NotRequired[
        "capo_kinesis_video_archived_media.types.fragment_list.FragmentList"
    ]
...
def serialize_json(value: ListFragmentsOutput) -> dict:
    ...
    if "fragments" in value:
        ...
        out["Fragments"] = (
            capo_kinesis_video_archived_media.types.fragment_list.serialize_json(
                value["fragments"]
            )
        )
...
def deserialize_json(data: dict) -> ListFragmentsOutput:
    ...
    if "Fragments" in data:
        ...
        out["fragments"] = (
            capo_kinesis_video_archived_media.types.fragment_list.deserialize_json(
                data["Fragments"]
            )
        )
