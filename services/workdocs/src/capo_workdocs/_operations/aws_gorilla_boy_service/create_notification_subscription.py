"""Generated from Smithy shape ``com.amazonaws.workdocs#CreateNotificationSubscription``."""
...
import capo_workdocs.errors.too_many_subscriptions_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "TooManySubscriptionsException":
            raise capo_workdocs.errors.too_many_subscriptions_exception.TooManySubscriptionsException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_workdocs.types.create_notification_subscription_response.CreateNotificationSubscriptionResponse = capo_workdocs.types.create_notification_subscription_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{OrganizationId}", quote(str(input_["organization_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_workdocs.types.create_notification_subscription_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_notification_subscription(
