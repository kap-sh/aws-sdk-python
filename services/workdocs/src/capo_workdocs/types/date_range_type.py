...
class DateRangeType(TypedDict, closed=True):
    start_value: NotRequired["capo_workdocs.types.timestamp_type.TimestampType"]
    """<p>Timestamp range start value (in epochs)</p>"""
    end_value: NotRequired["capo_workdocs.types.timestamp_type.TimestampType"]
...
def serialize_json(value: DateRangeType) -> dict:
    ...
    if "start_value" in value:
        ...
        out["StartValue"] = capo_workdocs.types.timestamp_type.serialize_json(
            value["start_value"]
        )
    if "end_value" in value:
        import capo_workdocs.types.timestamp_type

        out["EndValue"] = capo_workdocs.types.timestamp_type.serialize_json(
            value["end_value"]
        )
    return out
