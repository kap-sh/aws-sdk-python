...
GroupMetadataList: TypeAlias = list["capo_workdocs.types.group_metadata.GroupMetadata"]
...
def serialize_json(value: GroupMetadataList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_workdocs.types.group_metadata.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> GroupMetadataList:
    ...
    for item in data:
        out.append(capo_workdocs.types.group_metadata.deserialize_json(item))
    return out
