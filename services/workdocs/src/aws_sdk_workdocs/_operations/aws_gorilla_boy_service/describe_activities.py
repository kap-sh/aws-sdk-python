...
def build_request(
    ...
    input: aws_sdk_workdocs.types.describe_activities_request.DescribeActivitiesRequest,
) -> zapros.Request:
    ...
    if "activity_types" in input:
        params["activityTypes"] = str(input["activity_types"])
    ...
    params["includeIndirectActivities"] = str(
        input.get("include_indirect_activities", False)
    )
    if "limit" in input:
