"""Generated from Smithy shape ``com.amazonaws.workdocs#DescribeActivities``."""
...
def handle_response(
    ...
    out: aws_sdk_workdocs.types.describe_activities_response.DescribeActivitiesResponse = aws_sdk_workdocs.types.describe_activities_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_workdocs.types.describe_activities_request.DescribeActivitiesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/api/v1/activities"
    params: dict[str, str] = {}
    ...
    if "activity_types" in input_:
        params["activityTypes"] = str(input_["activity_types"])
    if "resource_id" in input_:
        params["resourceId"] = str(input_["resource_id"])
    if "user_id" in input_:
        params["userId"] = str(input_["user_id"])
    params["includeIndirectActivities"] = str(
        input_.get("include_indirect_activities", False)
    )
    if "limit" in input_:
...
async def async_describe_activities(
