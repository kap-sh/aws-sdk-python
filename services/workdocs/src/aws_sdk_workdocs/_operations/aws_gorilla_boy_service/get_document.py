"""Generated from Smithy shape ``com.amazonaws.workdocs#GetDocument``."""
...
def handle_response(
    ...
    out: aws_sdk_workdocs.types.get_document_response.GetDocumentResponse = (
        aws_sdk_workdocs.types.get_document_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_workdocs.types.get_document_request.GetDocumentRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/api/v1/documents/{DocumentId}"
    url = url.replace("{DocumentId}", quote(str(input_["document_id"]), safe=""))
    params: dict[str, str] = {}
    params["includeCustomMetadata"] = str(input_.get("include_custom_metadata", False))
...
def get_document(
    ...
    aws_sdk_workdocs.types.get_document_response.GetDocumentResponse, zapros.Response
]:
