...
OrganizationUserList: TypeAlias = list["aws_sdk_workdocs.types.user.User"]
...
def serialize_json(value: OrganizationUserList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_workdocs.types.user.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> OrganizationUserList:
    ...
    for item in data:
        out.append(aws_sdk_workdocs.types.user.deserialize_json(item))
    return out
