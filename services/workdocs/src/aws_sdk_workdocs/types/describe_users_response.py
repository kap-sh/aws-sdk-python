...
class DescribeUsersResponse(TypedDict, closed=True):
    users: NotRequired[
        "aws_sdk_workdocs.types.organization_user_list.OrganizationUserList"
    ]
    """<p>The users.</p>"""
    total_number_of_users: NotRequired["aws_sdk_workdocs.types.size_type.SizeType"]
...
def serialize_json(value: DescribeUsersResponse) -> dict:
    ...
    if "users" in value:
        ...
        out["Users"] = aws_sdk_workdocs.types.organization_user_list.serialize_json(
            value["users"]
        )
...
def deserialize_json(data: dict) -> DescribeUsersResponse:
    ...
    if "Users" in data:
        ...
        out["users"] = aws_sdk_workdocs.types.organization_user_list.deserialize_json(
            data["Users"]
        )
