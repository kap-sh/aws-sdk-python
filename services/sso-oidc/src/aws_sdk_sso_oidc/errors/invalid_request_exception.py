...
class InvalidRequestException_(TypedDict, closed=True):
    ...
    reason: NotRequired[
        "aws_sdk_sso_oidc.types.invalid_request_exception_reason.InvalidRequestExceptionReason"
    ]
...
def serialize_json(value: InvalidRequestException_) -> dict:
    ...
    if "reason" in value:
        ...
        out["reason"] = (
            aws_sdk_sso_oidc.types.invalid_request_exception_reason.serialize_json(
                value["reason"]
            )
        )
...
def deserialize_json(data: dict) -> InvalidRequestException_:
    ...
    if "reason" in data:
        ...
        out["reason"] = (
            aws_sdk_sso_oidc.types.invalid_request_exception_reason.deserialize_json(
                data["reason"]
            )
        )
