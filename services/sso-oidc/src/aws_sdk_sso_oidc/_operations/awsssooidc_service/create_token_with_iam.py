"""Generated from Smithy shape ``com.amazonaws.ssooidc#CreateTokenWithIAM``."""
...
def build_request(
    ...
    input_: aws_sdk_sso_oidc.types.create_token_with_iam_request.CreateTokenWithIAMRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/token?aws_iam=t"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_sso_oidc.types.create_token_with_iam_request

    body: bytes | None = json.dumps(
        aws_sdk_sso_oidc.types.create_token_with_iam_request.serialize_json(input_)
    ).encode()
...
async def async_create_token_with_iam(
