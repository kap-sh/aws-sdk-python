...
if TYPE_CHECKING:
    ...
    import aws_sdk_sso_oidc.types.interval_in_seconds
    import aws_sdk_sso_oidc.types.uri
    import aws_sdk_sso_oidc.types.user_code


class StartDeviceAuthorizationResponse(TypedDict):
    device_code: NotRequired["aws_sdk_sso_oidc.types.device_code.DeviceCode"]
    """<p>The short-lived code that is used by the device when polling for a session token.</p>"""
    user_code: NotRequired["aws_sdk_sso_oidc.types.user_code.UserCode"]
    """<p>A one-time user verification code. This is needed to authorize an in-use device.</p>"""
    verification_uri: NotRequired["aws_sdk_sso_oidc.types.uri.URI"]
    """<p>The URI of the verification page that takes the <code>userCode</code> to authorize the device.</p>"""
    verification_uri_complete: NotRequired["aws_sdk_sso_oidc.types.uri.URI"]
    ...
    interval: "aws_sdk_sso_oidc.types.interval_in_seconds.IntervalInSeconds"
