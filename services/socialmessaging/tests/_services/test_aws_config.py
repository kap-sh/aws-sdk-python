...
import pytest
from aws_sdk_socialmessaging._services._aws_config import (
    _env_bool,
    _load_profile,
    _profile_bool,
    load_aws_settings,
...
def clean_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    for var in (
        ...
        "AWS_REGION",
        "AWS_ENDPOINT_URL",
        "AWS_ENDPOINT_URL_SOCIAL_MESSAGING",
...
def test_endpoint_service_env_beats_global_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AWS_ENDPOINT_URL_SOCIAL_MESSAGING", "https://svc.example")
...
def test_load_profile_parses_services_block(tmp_path: Path) -> None:
    ...
    assert services["social_messaging"]["endpoint_url"] == "https://localstack:4566"
