"""Generated from Smithy shape ``com.amazonaws.socialmessaging#GetWhatsAppFlowPreview``."""
...
def handle_response(
    ...
    out: capo_socialmessaging.types.get_whats_app_flow_preview_output.GetWhatsAppFlowPreviewOutput = capo_socialmessaging.types.get_whats_app_flow_preview_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/flow/preview"
    params: dict[str, str] = {}
    if "id" in input_:
        params["id"] = str(input_["id"])
    if "flow_id" in input_:
        params["flowId"] = str(input_["flow_id"])
    if "invalidate" in input_:
        params["invalidate"] = str(input_["invalidate"])
...
async def async_get_whats_app_flow_preview(
