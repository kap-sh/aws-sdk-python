"""Generated from Smithy shape ``com.amazonaws.socialmessaging#DeprecateWhatsAppFlow``."""
...
def handle_response(
    ...
    out: capo_socialmessaging.types.deprecate_whats_app_flow_output.DeprecateWhatsAppFlowOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/flow/deprecate"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_socialmessaging.types.deprecate_whats_app_flow_input.serialize_json(input_)
    ).encode()
...
async def async_deprecate_whats_app_flow(
