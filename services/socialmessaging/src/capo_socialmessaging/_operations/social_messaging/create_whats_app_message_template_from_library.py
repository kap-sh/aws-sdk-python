"""Generated from Smithy shape ``com.amazonaws.socialmessaging#CreateWhatsAppMessageTemplateFromLibrary``."""
...
def handle_response(
    ...
    out: capo_socialmessaging.types.create_whats_app_message_template_from_library_output.CreateWhatsAppMessageTemplateFromLibraryOutput = capo_socialmessaging.types.create_whats_app_message_template_from_library_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/template/create"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_socialmessaging.types.create_whats_app_message_template_from_library_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_whats_app_message_template_from_library(
