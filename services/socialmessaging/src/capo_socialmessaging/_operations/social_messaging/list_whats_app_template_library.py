"""Generated from Smithy shape ``com.amazonaws.socialmessaging#ListWhatsAppTemplateLibrary``."""
...
def handle_response(
    ...
    out: capo_socialmessaging.types.list_whats_app_template_library_output.ListWhatsAppTemplateLibraryOutput = capo_socialmessaging.types.list_whats_app_template_library_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/template/library"
    params: dict[str, str] = {}
    if "id" in input_:
        params["id"] = str(input_["id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_socialmessaging.types.list_whats_app_template_library_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_whats_app_template_library(
