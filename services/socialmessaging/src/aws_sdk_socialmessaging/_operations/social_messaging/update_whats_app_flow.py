"""Generated from Smithy shape ``com.amazonaws.socialmessaging#UpdateWhatsAppFlow``."""
...
def handle_response(
    ...
    out: aws_sdk_socialmessaging.types.update_whats_app_flow_output.UpdateWhatsAppFlowOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_socialmessaging.types.update_whats_app_flow_input.UpdateWhatsAppFlowInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/flow/update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_socialmessaging.types.update_whats_app_flow_input.serialize_json(input_)
    ).encode()
...
async def async_update_whats_app_flow(
