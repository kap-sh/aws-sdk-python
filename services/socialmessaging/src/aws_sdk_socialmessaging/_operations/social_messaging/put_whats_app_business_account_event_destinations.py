"""Generated from Smithy shape ``com.amazonaws.socialmessaging#PutWhatsAppBusinessAccountEventDestinations``."""
...
def handle_response(
    ...
    out: aws_sdk_socialmessaging.types.put_whats_app_business_account_event_destinations_output.PutWhatsAppBusinessAccountEventDestinationsOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_socialmessaging.types.put_whats_app_business_account_event_destinations_input.PutWhatsAppBusinessAccountEventDestinationsInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/waba/eventdestinations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_socialmessaging.types.put_whats_app_business_account_event_destinations_input

    body: bytes | None = json.dumps(
        aws_sdk_socialmessaging.types.put_whats_app_business_account_event_destinations_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_whats_app_business_account_event_destinations(
