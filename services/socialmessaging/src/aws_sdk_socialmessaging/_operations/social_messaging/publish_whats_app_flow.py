"""Generated from Smithy shape ``com.amazonaws.socialmessaging#PublishWhatsAppFlow``."""
...
def handle_response(
    ...
    out: aws_sdk_socialmessaging.types.publish_whats_app_flow_output.PublishWhatsAppFlowOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_socialmessaging.types.publish_whats_app_flow_input.PublishWhatsAppFlowInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/flow/publish"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_socialmessaging.types.publish_whats_app_flow_input

    body: bytes | None = json.dumps(
        aws_sdk_socialmessaging.types.publish_whats_app_flow_input.serialize_json(input)
    ).encode()
...
async def async_publish_whats_app_flow(
