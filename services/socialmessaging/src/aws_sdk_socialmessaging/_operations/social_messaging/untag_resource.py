"""Generated from Smithy shape ``com.amazonaws.socialmessaging#UntagResource``."""
...
def handle_response(
    ...
    out: aws_sdk_socialmessaging.types.untag_resource_output.UntagResourceOutput = (
        aws_sdk_socialmessaging.types.untag_resource_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_socialmessaging.types.untag_resource_input.UntagResourceInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/tags/untag-resource"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_socialmessaging.types.untag_resource_input

    body: bytes | None = json.dumps(
        aws_sdk_socialmessaging.types.untag_resource_input.serialize_json(input_)
    ).encode()
