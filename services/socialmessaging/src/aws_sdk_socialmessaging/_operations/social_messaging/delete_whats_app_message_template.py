"""Generated from Smithy shape ``com.amazonaws.socialmessaging#DeleteWhatsAppMessageTemplate``."""
...
def handle_response(
    ...
    out: aws_sdk_socialmessaging.types.delete_whats_app_message_template_output.DeleteWhatsAppMessageTemplateOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_socialmessaging.types.delete_whats_app_message_template_input.DeleteWhatsAppMessageTemplateInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/whatsapp/template"
    params: dict[str, str] = {}
    if "meta_template_id" in input_:
        params["metaTemplateId"] = str(input_["meta_template_id"])
    if "delete_all_languages" in input_:
        params["deleteAllTemplates"] = str(input_["delete_all_languages"])
    if "id" in input_:
        params["id"] = str(input_["id"])
    if "template_name" in input_:
        params["templateName"] = str(input_["template_name"])
...
async def async_delete_whats_app_message_template(
