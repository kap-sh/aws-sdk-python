...
def build_request(
    ...
    input: aws_sdk_socialmessaging.types.delete_whats_app_message_template_input.DeleteWhatsAppMessageTemplateInput,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "meta_template_id" in input:
        params["metaTemplateId"] = str(input["meta_template_id"])
    if "delete_all_languages" in input:
        params["deleteAllTemplates"] = str(input["delete_all_languages"])
    if "id" in input:
        params["id"] = str(input["id"])
    if "template_name" in input:
        params["templateName"] = str(input["template_name"])
