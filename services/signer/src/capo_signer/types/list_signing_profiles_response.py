...
class ListSigningProfilesResponse(TypedDict, closed=True):
    profiles: NotRequired["capo_signer.types.signing_profiles.SigningProfiles"]
...
def serialize_json(value: ListSigningProfilesResponse) -> dict:
    ...
    if "profiles" in value:
        ...
        out["profiles"] = capo_signer.types.signing_profiles.serialize_json(
            value["profiles"]
        )
...
def deserialize_json(data: dict) -> ListSigningProfilesResponse:
    ...
    if "profiles" in data:
        ...
        out["profiles"] = capo_signer.types.signing_profiles.deserialize_json(
            data["profiles"]
        )
