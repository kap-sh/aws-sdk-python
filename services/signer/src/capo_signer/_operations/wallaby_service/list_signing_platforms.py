"""Generated from Smithy shape ``com.amazonaws.signer#ListSigningPlatforms``."""
...
def handle_response(
    ...
    out: capo_signer.types.list_signing_platforms_response.ListSigningPlatformsResponse = capo_signer.types.list_signing_platforms_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/signing-platforms"
    params: dict[str, str] = {}
    if "category" in input_:
        params["category"] = str(input_["category"])
    if "partner" in input_:
        params["partner"] = str(input_["partner"])
    if "target" in input_:
...
async def async_list_signing_platforms(
