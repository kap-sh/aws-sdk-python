"""Generated from Smithy shape ``com.amazonaws.signer#PutSigningProfile``."""
...
def handle_response(
    ...
    out: aws_sdk_signer.types.put_signing_profile_response.PutSigningProfileResponse = (
        aws_sdk_signer.types.put_signing_profile_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_signer.types.put_signing_profile_request.PutSigningProfileRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/signing-profiles/{profileName}"
    url = url.replace("{profileName}", quote(str(input["profile_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_signer.types.put_signing_profile_request

    body: bytes | None = json.dumps(
        aws_sdk_signer.types.put_signing_profile_request.serialize_json(input)
    ).encode()
...
async def async_put_signing_profile(
