...
class SigningProfileRevocationRecord(TypedDict):
    revocation_effective_from: NotRequired["aws_sdk_signer.types.timestamp.Timestamp"]
    """<p>The time when revocation becomes effective.</p>"""
    revoked_at: NotRequired["aws_sdk_signer.types.timestamp.Timestamp"]
    """<p>The time when the signing profile was revoked.</p>"""
    revoked_by: NotRequired["aws_sdk_signer.types.string.String"]
...
def serialize_json(value: SigningProfileRevocationRecord) -> dict:
    ...
    if "revocation_effective_from" in value:
        ...
        out["revocationEffectiveFrom"] = aws_sdk_signer.types.timestamp.serialize_json(
            value["revocation_effective_from"]
        )
    if "revoked_at" in value:
        import aws_sdk_signer.types.timestamp

        out["revokedAt"] = aws_sdk_signer.types.timestamp.serialize_json(
            value["revoked_at"]
        )
...
def deserialize_json(data: dict) -> SigningProfileRevocationRecord:
    ...
    if "revocationEffectiveFrom" in data:
        ...
        out["revocation_effective_from"] = (
            aws_sdk_signer.types.timestamp.deserialize_json(
    ...
    if "revokedAt" in data:
        ...
        out["revoked_at"] = aws_sdk_signer.types.timestamp.deserialize_json(
            data["revokedAt"]
        )
