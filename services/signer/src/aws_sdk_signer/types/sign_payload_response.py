...
class SignPayloadResponse(TypedDict):
    ...
    metadata: NotRequired["aws_sdk_signer.types.metadata.Metadata"]
    """<p>Information including the signing profile ARN and the signing job ID.</p>"""
    signature: NotRequired["aws_sdk_signer.types.blob.Blob"]
...
def serialize_json(value: SignPayloadResponse) -> dict:
    ...
    if "metadata" in value:
        ...
        out["metadata"] = aws_sdk_signer.types.metadata.serialize_json(
            value["metadata"]
        )
    if "signature" in value:
        import aws_sdk_signer.types.blob

        out["signature"] = aws_sdk_signer.types.blob.serialize_json(value["signature"])
    return out
...
def deserialize_json(data: dict) -> SignPayloadResponse:
    ...
    if "metadata" in data:
        ...
        out["metadata"] = aws_sdk_signer.types.metadata.deserialize_json(
            data["metadata"]
        )
    if "signature" in data:
        import aws_sdk_signer.types.blob

        out["signature"] = aws_sdk_signer.types.blob.deserialize_json(data["signature"])
    return out
