...
if TYPE_CHECKING:
    import aws_sdk_signer.types.integer
    import aws_sdk_signer.types.validity_type


class SignatureValidityPeriod(TypedDict):
    value: "aws_sdk_signer.types.integer.Integer"
    """<p>The numerical value of the time unit for signature validity.</p>"""
    type: NotRequired["aws_sdk_signer.types.validity_type.ValidityType"]
...
def serialize_json(value: SignatureValidityPeriod) -> dict:
    ...
    if "type" in value:
        ...
        out["type"] = aws_sdk_signer.types.validity_type.serialize_json(value["type"])
    return out
...
def deserialize_json(data: dict) -> SignatureValidityPeriod:
    ...
    if "type" in data:
        ...
        out["type"] = aws_sdk_signer.types.validity_type.deserialize_json(data["type"])
    return out
