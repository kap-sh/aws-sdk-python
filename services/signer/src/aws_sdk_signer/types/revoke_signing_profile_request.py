...
class RevokeSigningProfileRequest(TypedDict):
...
def serialize_json(value: RevokeSigningProfileRequest) -> dict:
    ...
    out["effectiveTime"] = aws_sdk_signer.types.timestamp.serialize_json(
...
def deserialize_json(data: dict) -> RevokeSigningProfileRequest:
    ...
    if "effectiveTime" in data:
        ...
        out["effective_time"] = aws_sdk_signer.types.timestamp.deserialize_json(
            data["effectiveTime"]
        )
    else:
        raise DeserializationError(
