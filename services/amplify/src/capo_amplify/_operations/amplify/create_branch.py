"""Generated from Smithy shape ``com.amazonaws.amplify#CreateBranch``."""
...
def handle_response(
    ...
    out: capo_amplify.types.create_branch_result.CreateBranchResult = (
        capo_amplify.types.create_branch_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/apps/{appId}/branches"
    url = url.replace("{appId}", quote(str(input_["app_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_amplify.types.create_branch_request.serialize_json(input_)
    ).encode()
...
def create_branch(
    ...
    input_: capo_amplify.types.create_branch_request.CreateBranchRequest,
) -> tuple[capo_amplify.types.create_branch_result.CreateBranchResult, zapros.Response]:
