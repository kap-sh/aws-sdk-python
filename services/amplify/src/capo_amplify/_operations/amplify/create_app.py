"""Generated from Smithy shape ``com.amazonaws.amplify#CreateApp``."""
...
import capo_amplify.errors.dependent_service_failure_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "DependentServiceFailureException":
            raise capo_amplify.errors.dependent_service_failure_exception.DependentServiceFailureException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_amplify.types.create_app_result.CreateAppResult = (
        capo_amplify.types.create_app_result.deserialize_json(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_amplify._auth._sigv4.build_sigv4_auth_scheme(
...
def build_request(
    ...
    input_: capo_amplify.types.create_app_request.CreateAppRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/apps"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_amplify.types.create_app_request.serialize_json(input_)
    ).encode()
...
def create_app(
    ...
    input_: capo_amplify.types.create_app_request.CreateAppRequest,
) -> tuple[capo_amplify.types.create_app_result.CreateAppResult, zapros.Response]:
