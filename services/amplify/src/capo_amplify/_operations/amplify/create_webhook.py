"""Generated from Smithy shape ``com.amazonaws.amplify#CreateWebhook``."""
...
def handle_response(
    ...
    out: capo_amplify.types.create_webhook_result.CreateWebhookResult = (
        capo_amplify.types.create_webhook_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/apps/{appId}/webhooks"
    url = url.replace("{appId}", quote(str(input_["app_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_amplify.types.create_webhook_request.serialize_json(input_)
    ).encode()
...
def create_webhook(
    ...
    capo_amplify.types.create_webhook_result.CreateWebhookResult, zapros.Response
]:
