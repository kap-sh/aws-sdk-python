...
class CreateDeploymentRequest(TypedDict, closed=True):
    ...
    file_map: NotRequired["aws_sdk_amplify.types.file_map.FileMap"]
...
def serialize_json(value: CreateDeploymentRequest) -> dict:
    ...
    if "file_map" in value:
        ...
        out["fileMap"] = aws_sdk_amplify.types.file_map.serialize_json(
            value["file_map"]
        )
    return out
...
def deserialize_json(data: dict) -> CreateDeploymentRequest:
    ...
    if "fileMap" in data:
        ...
        out["file_map"] = aws_sdk_amplify.types.file_map.deserialize_json(
            data["fileMap"]
        )
    return out
