...
BackendEnvironments: TypeAlias = list[
    "aws_sdk_amplify.types.backend_environment.BackendEnvironment"
]
...
def serialize_json(value: BackendEnvironments) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_amplify.types.backend_environment.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> BackendEnvironments:
    ...
    for item in data:
        out.append(aws_sdk_amplify.types.backend_environment.deserialize_json(item))
    return out
