...
def build_request(
    ...
    input: aws_sdk_amplify.types.update_webhook_request.UpdateWebhookRequest,
) -> zapros.Request:
    ...
    url = url.replace("{webhookId}", quote(str(input["webhook_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_amplify.types.update_webhook_request

    body: bytes | None = json.dumps(
        aws_sdk_amplify.types.update_webhook_request.serialize_json(input)
    ).encode()
