"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#DescribeBotReplica``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.describe_bot_replica_response.DescribeBotReplicaResponse = aws_sdk_lex_models_v2.types.describe_bot_replica_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_lex_models_v2.types.describe_bot_replica_request.DescribeBotReplicaRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/replicas/{replicaRegion}"
    url = url.replace("{botId}", quote(str(input["bot_id"]), safe=""))
    url = url.replace("{replicaRegion}", quote(str(input["replica_region"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_bot_replica(
