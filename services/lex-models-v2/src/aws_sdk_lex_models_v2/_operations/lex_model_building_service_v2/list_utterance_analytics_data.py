"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListUtteranceAnalyticsData``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.list_utterance_analytics_data_response.ListUtteranceAnalyticsDataResponse = aws_sdk_lex_models_v2.types.list_utterance_analytics_data_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lex_models_v2.types.list_utterance_analytics_data_request.ListUtteranceAnalyticsDataRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/analytics/utterances"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_lex_models_v2.types.list_utterance_analytics_data_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_utterance_analytics_data(
