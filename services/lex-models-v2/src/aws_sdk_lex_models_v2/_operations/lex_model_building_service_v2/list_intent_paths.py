"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListIntentPaths``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.list_intent_paths_response.ListIntentPathsResponse = aws_sdk_lex_models_v2.types.list_intent_paths_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lex_models_v2.types.list_intent_paths_request.ListIntentPathsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/analytics/intentpaths"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_lex_models_v2.types.list_intent_paths_request.serialize_json(input_)
    ).encode()
...
async def async_list_intent_paths(
