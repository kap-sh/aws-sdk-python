"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#CreateBotVersion``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.create_bot_version_response.CreateBotVersionResponse = aws_sdk_lex_models_v2.types.create_bot_version_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lex_models_v2.types.create_bot_version_request.CreateBotVersionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/botversions"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_lex_models_v2.types.create_bot_version_request.serialize_json(input_)
    ).encode()
...
async def async_create_bot_version(
