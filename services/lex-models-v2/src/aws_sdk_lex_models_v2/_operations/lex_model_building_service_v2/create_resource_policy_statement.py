"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#CreateResourcePolicyStatement``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            import aws_sdk_lex_models_v2.errors.resource_not_found_exception

            raise aws_sdk_lex_models_v2.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.create_resource_policy_statement_response.CreateResourcePolicyStatementResponse = aws_sdk_lex_models_v2.types.create_resource_policy_statement_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_lex_models_v2.types.create_resource_policy_statement_request.CreateResourcePolicyStatementRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/policy/{resourceArn}/statements"
    url = url.replace("{resourceArn}", quote(str(input["resource_arn"]), safe=""))
    params: dict[str, str] = {}
    if "expected_revision_id" in input:
        params["expectedRevisionId"] = str(input["expected_revision_id"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lex_models_v2.types.create_resource_policy_statement_request

    body: bytes | None = json.dumps(
        aws_sdk_lex_models_v2.types.create_resource_policy_statement_request.serialize_json(
            input
        )
    ).encode()
...
async def async_create_resource_policy_statement(
