...
def build_request(
    ...
    input: aws_sdk_lex_models_v2.types.update_export_request.UpdateExportRequest,
) -> zapros.Request:
    ...
    url = url.replace("{exportId}", quote(str(input["export_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_lex_models_v2.types.update_export_request

    body: bytes | None = json.dumps(
        aws_sdk_lex_models_v2.types.update_export_request.serialize_json(input)
    ).encode()
