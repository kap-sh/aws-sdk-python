"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#UpdateExport``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.update_export_response.UpdateExportResponse = (
        aws_sdk_lex_models_v2.types.update_export_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lex_models_v2.types.update_export_request.UpdateExportRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/exports/{exportId}"
    url = url.replace("{exportId}", quote(str(input_["export_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_lex_models_v2.types.update_export_request.serialize_json(input_)
    ).encode()
