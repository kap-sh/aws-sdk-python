"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#DeleteBotAnalyzerRecommendation``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.delete_bot_analyzer_recommendation_response.DeleteBotAnalyzerRecommendationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_lex_models_v2.types.delete_bot_analyzer_recommendation_request.DeleteBotAnalyzerRecommendationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/botanalyzer/{botAnalyzerRequestId}"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    url = url.replace(
        "{botAnalyzerRequestId}", quote(str(input_["bot_analyzer_request_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_delete_bot_analyzer_recommendation(
