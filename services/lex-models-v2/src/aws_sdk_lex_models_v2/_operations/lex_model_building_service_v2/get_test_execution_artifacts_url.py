"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#GetTestExecutionArtifactsUrl``."""
...
def handle_response(
    ...
    out: aws_sdk_lex_models_v2.types.get_test_execution_artifacts_url_response.GetTestExecutionArtifactsUrlResponse = aws_sdk_lex_models_v2.types.get_test_execution_artifacts_url_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lex_models_v2.types.get_test_execution_artifacts_url_request.GetTestExecutionArtifactsUrlRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/testexecutions/{testExecutionId}/artifacturl"
    url = url.replace(
        "{testExecutionId}", quote(str(input_["test_execution_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_test_execution_artifacts_url(
