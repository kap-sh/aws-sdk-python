...
class ListUtteranceAnalyticsDataRequest(TypedDict):
...
def serialize_json(value: ListUtteranceAnalyticsDataRequest) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["sortBy"] = (
            aws_sdk_lex_models_v2.types.utterance_data_sort_by.serialize_json(
                value["sort_by"]
            )
        )
    if "filters" in value:
...
def deserialize_json(data: dict) -> ListUtteranceAnalyticsDataRequest:
    ...
    if "sortBy" in data:
        ...
        out["sort_by"] = (
            aws_sdk_lex_models_v2.types.utterance_data_sort_by.deserialize_json(
                data["sortBy"]
            )
        )
    if "filters" in data:
