...
# --- restJson1 ser/de ---
def serialize_json(value: AnalyticsIntentFilter) -> dict:
    out: dict = {}
    import aws_sdk_lex_models_v2.types.analytics_intent_filter_name

    out["name"] = (
        aws_sdk_lex_models_v2.types.analytics_intent_filter_name.serialize_json(
            value["name"]
        )
    )
    import aws_sdk_lex_models_v2.types.analytics_filter_operator
...
def deserialize_json(data: dict) -> AnalyticsIntentFilter:
    out: AnalyticsIntentFilter = {}  # type: ignore[typeddict-item]
    if "name" in data:
        import aws_sdk_lex_models_v2.types.analytics_intent_filter_name

        out["name"] = (
            aws_sdk_lex_models_v2.types.analytics_intent_filter_name.deserialize_json(
                data["name"]
            )
        )
    else:
        raise DeserializationError("AnalyticsIntentFilter.name required")
    if "operator" in data:
        import aws_sdk_lex_models_v2.types.analytics_filter_operator
        ...
        raise DeserializationError("AnalyticsIntentFilter.operator required")
    if "values" in data:
        import aws_sdk_lex_models_v2.types.analytics_filter_values
        ...
        raise DeserializationError("AnalyticsIntentFilter.values required")
    return out
