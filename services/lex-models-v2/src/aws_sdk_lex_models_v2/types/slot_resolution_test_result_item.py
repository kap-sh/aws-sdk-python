...
if TYPE_CHECKING:
    import aws_sdk_lex_models_v2.types.slot_resolution_test_result_item_counts
...
def serialize_json(value: SlotResolutionTestResultItem) -> dict:
    ...
    out["resultCounts"] = (
        aws_sdk_lex_models_v2.types.slot_resolution_test_result_item_counts.serialize_json(
            value["result_counts"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> SlotResolutionTestResultItem:
    ...
    if "resultCounts" in data:
        ...
        out["result_counts"] = (
            aws_sdk_lex_models_v2.types.slot_resolution_test_result_item_counts.deserialize_json(
                data["resultCounts"]
            )
        )
    else:
