...
class ListTestExecutionsRequest(TypedDict):
    sort_by: NotRequired[
...
def serialize_json(value: ListTestExecutionsRequest) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["sortBy"] = (
            aws_sdk_lex_models_v2.types.test_execution_sort_by.serialize_json(
                value["sort_by"]
            )
        )
...
def deserialize_json(data: dict) -> ListTestExecutionsRequest:
    ...
    if "sortBy" in data:
        ...
        out["sort_by"] = (
            aws_sdk_lex_models_v2.types.test_execution_sort_by.deserialize_json(
                data["sortBy"]
            )
        )
