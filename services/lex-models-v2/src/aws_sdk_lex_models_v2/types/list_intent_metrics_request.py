...
class ListIntentMetricsRequest(TypedDict):
    ...
    metrics: (
        "aws_sdk_lex_models_v2.types.analytics_intent_metrics.AnalyticsIntentMetrics"
    )
...
def serialize_json(value: ListIntentMetricsRequest) -> dict:
    ...
    out["metrics"] = (
        aws_sdk_lex_models_v2.types.analytics_intent_metrics.serialize_json(
            value["metrics"]
        )
    )
    if "bin_by" in value:
    ...
    if "group_by" in value:
        ...
        out["groupBy"] = (
            aws_sdk_lex_models_v2.types.analytics_intent_group_by_list.serialize_json(
                value["group_by"]
            )
        )
    if "filters" in value:
        ...
        out["filters"] = (
            aws_sdk_lex_models_v2.types.analytics_intent_filters.serialize_json(
                value["filters"]
            )
        )
...
def deserialize_json(data: dict) -> ListIntentMetricsRequest:
    ...
    if "metrics" in data:
        ...
        out["metrics"] = (
            aws_sdk_lex_models_v2.types.analytics_intent_metrics.deserialize_json(
                data["metrics"]
            )
        )
    else:
    ...
    if "groupBy" in data:
        ...
        out["group_by"] = (
            aws_sdk_lex_models_v2.types.analytics_intent_group_by_list.deserialize_json(
                data["groupBy"]
            )
        )
    if "filters" in data:
        ...
        out["filters"] = (
            aws_sdk_lex_models_v2.types.analytics_intent_filters.deserialize_json(
                data["filters"]
            )
        )
