...
if TYPE_CHECKING:
    ...
    import aws_sdk_lex_models_v2.types.import_filter_name
...
class ImportFilter(TypedDict, closed=True):
    ...
    operator: "aws_sdk_lex_models_v2.types.import_filter_operator.ImportFilterOperator"
...
def serialize_json(value: ImportFilter) -> dict:
    ...
    out["name"] = aws_sdk_lex_models_v2.types.import_filter_name.serialize_json(
        value["name"]
    )
    import aws_sdk_lex_models_v2.types.filter_values
    ...
    out["operator"] = aws_sdk_lex_models_v2.types.import_filter_operator.serialize_json(
        value["operator"]
    )
    return out
...
def deserialize_json(data: dict) -> ImportFilter:
    ...
    if "name" in data:
        ...
        out["name"] = aws_sdk_lex_models_v2.types.import_filter_name.deserialize_json(
            data["name"]
        )
    else:
    ...
    if "operator" in data:
        ...
        out["operator"] = (
            aws_sdk_lex_models_v2.types.import_filter_operator.deserialize_json(
                data["operator"]
            )
        )
    else:
