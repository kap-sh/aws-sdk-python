...
if TYPE_CHECKING:
    import aws_sdk_lex_models_v2.types.deepgram_model_id
    import aws_sdk_lex_models_v2.types.secrets_manager_secret_arn


class DeepgramSpeechModelConfig(TypedDict):
    api_token_secret_arn: (
        "aws_sdk_lex_models_v2.types.secrets_manager_secret_arn.SecretsManagerSecretArn"
    )
    """<p>The Amazon Resource Name (ARN) of the Secrets Manager secret that contains the Deepgram API token.</p>"""
    model_id: NotRequired[
        "aws_sdk_lex_models_v2.types.deepgram_model_id.DeepgramModelId"
    ]
