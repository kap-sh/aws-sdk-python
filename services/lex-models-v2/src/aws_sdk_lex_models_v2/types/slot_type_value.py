...
def serialize_json(value: SlotTypeValue) -> dict:
    ...
    if "sample_value" in value:
        ...
        out["sampleValue"] = aws_sdk_lex_models_v2.types.sample_value.serialize_json(
            value["sample_value"]
        )
    if "synonyms" in value:
        import aws_sdk_lex_models_v2.types.synonym_list

        out["synonyms"] = aws_sdk_lex_models_v2.types.synonym_list.serialize_json(
            value["synonyms"]
        )
    return out
...
def deserialize_json(data: dict) -> SlotTypeValue:
    ...
    if "sampleValue" in data:
        ...
        out["sample_value"] = aws_sdk_lex_models_v2.types.sample_value.deserialize_json(
            data["sampleValue"]
        )
    if "synonyms" in data:
        import aws_sdk_lex_models_v2.types.synonym_list

        out["synonyms"] = aws_sdk_lex_models_v2.types.synonym_list.deserialize_json(
            data["synonyms"]
        )
    return out
