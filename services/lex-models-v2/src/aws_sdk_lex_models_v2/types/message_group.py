...
if TYPE_CHECKING:
    ...
    import aws_sdk_lex_models_v2.types.message_variations_list


class MessageGroup(TypedDict, closed=True):
    message: "aws_sdk_lex_models_v2.types.message.Message"
    """<p>The primary message that Amazon Lex should send to the user.</p>"""
    variations: NotRequired[
        "aws_sdk_lex_models_v2.types.message_variations_list.MessageVariationsList"
    ]
...
def serialize_json(value: MessageGroup) -> dict:
    ...
    out["message"] = aws_sdk_lex_models_v2.types.message.serialize_json(
        value["message"]
    )
    if "variations" in value:
        ...
        out["variations"] = (
            aws_sdk_lex_models_v2.types.message_variations_list.serialize_json(
                value["variations"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> MessageGroup:
    ...
    if "message" in data:
        ...
        out["message"] = aws_sdk_lex_models_v2.types.message.deserialize_json(
            data["message"]
        )
    else:
    ...
    if "variations" in data:
        ...
        out["variations"] = (
            aws_sdk_lex_models_v2.types.message_variations_list.deserialize_json(
                data["variations"]
            )
        )
    return out
