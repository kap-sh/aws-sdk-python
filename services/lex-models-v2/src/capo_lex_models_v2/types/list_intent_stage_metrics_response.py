...
class ListIntentStageMetricsResponse(TypedDict, closed=True):
    ...
    results: NotRequired[
        "capo_lex_models_v2.types.analytics_intent_stage_results.AnalyticsIntentStageResults"
    ]
...
def serialize_json(value: ListIntentStageMetricsResponse) -> dict:
    ...
    if "results" in value:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_intent_stage_results.serialize_json(
                value["results"]
            )
        )
...
def deserialize_json(data: dict) -> ListIntentStageMetricsResponse:
    ...
    if "results" in data:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_intent_stage_results.deserialize_json(
                data["results"]
            )
        )
