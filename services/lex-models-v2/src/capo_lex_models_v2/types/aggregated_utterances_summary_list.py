...
if TYPE_CHECKING:
    import capo_lex_models_v2.types.aggregated_utterances_summary

AggregatedUtterancesSummaryList: TypeAlias = list[
    "capo_lex_models_v2.types.aggregated_utterances_summary.AggregatedUtterancesSummary"
]
...
def serialize_json(value: AggregatedUtterancesSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_lex_models_v2.types.aggregated_utterances_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AggregatedUtterancesSummaryList:
    ...
    for item in data:
        out.append(
            capo_lex_models_v2.types.aggregated_utterances_summary.deserialize_json(
                item
            )
        )
    return out
