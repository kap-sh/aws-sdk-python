...
if TYPE_CHECKING:
    ...
    import capo_lex_models_v2.types.analytics_session_filter_name


class AnalyticsSessionFilter(TypedDict, closed=True):
    name: "capo_lex_models_v2.types.analytics_session_filter_name.AnalyticsSessionFilterName"
...
# --- restJson1 ser/de ---
def serialize_json(value: AnalyticsSessionFilter) -> dict:
    out: dict = {}
    import capo_lex_models_v2.types.analytics_session_filter_name

    out["name"] = capo_lex_models_v2.types.analytics_session_filter_name.serialize_json(
        value["name"]
    )
    import capo_lex_models_v2.types.analytics_filter_operator
...
def deserialize_json(data: dict) -> AnalyticsSessionFilter:
    out: AnalyticsSessionFilter = {}  # type: ignore[typeddict-item]
    if "name" in data:
        import capo_lex_models_v2.types.analytics_session_filter_name

        out["name"] = (
            capo_lex_models_v2.types.analytics_session_filter_name.deserialize_json(
                data["name"]
            )
        )
    else:
        raise DeserializationError("AnalyticsSessionFilter.name required")
    if "operator" in data:
        import capo_lex_models_v2.types.analytics_filter_operator
        ...
        raise DeserializationError("AnalyticsSessionFilter.operator required")
    if "values" in data:
        import capo_lex_models_v2.types.analytics_filter_values
        ...
        raise DeserializationError("AnalyticsSessionFilter.values required")
    return out
