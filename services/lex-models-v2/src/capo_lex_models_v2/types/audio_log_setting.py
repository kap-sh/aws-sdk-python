...
if TYPE_CHECKING:
    import capo_lex_models_v2.types.audio_log_destination
...
class AudioLogSetting(TypedDict, closed=True):
    ...
    destination: "capo_lex_models_v2.types.audio_log_destination.AudioLogDestination"
...
def serialize_json(value: AudioLogSetting) -> dict:
    ...
    out["destination"] = capo_lex_models_v2.types.audio_log_destination.serialize_json(
        value["destination"]
    )
...
def deserialize_json(data: dict) -> AudioLogSetting:
    ...
    if "destination" in data:
        ...
        out["destination"] = (
            capo_lex_models_v2.types.audio_log_destination.deserialize_json(
                data["destination"]
            )
        )
    else:
