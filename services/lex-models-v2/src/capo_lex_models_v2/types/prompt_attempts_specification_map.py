...
PromptAttemptsSpecificationMap: TypeAlias = dict[
    "capo_lex_models_v2.types.prompt_attempt.PromptAttempt",
    "capo_lex_models_v2.types.prompt_attempt_specification.PromptAttemptSpecification",
]
...
def serialize_json(input_to_serialize: PromptAttemptsSpecificationMap) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[capo_lex_models_v2.types.prompt_attempt.serialize_json(key)] = (
            capo_lex_models_v2.types.prompt_attempt_specification.serialize_json(value)
        )
    return out
...
def deserialize_json(data: dict) -> PromptAttemptsSpecificationMap:
    ...
    for key, value in data.items():
        ...
        out[capo_lex_models_v2.types.prompt_attempt.deserialize_json(key)] = (
            capo_lex_models_v2.types.prompt_attempt_specification.deserialize_json(
                value
            )
        )
    return out
