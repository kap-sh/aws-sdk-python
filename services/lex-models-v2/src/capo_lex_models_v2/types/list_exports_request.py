...
class ListExportsRequest(TypedDict, closed=True):
    ...
    sort_by: NotRequired["capo_lex_models_v2.types.export_sort_by.ExportSortBy"]
    ...
    filters: NotRequired["capo_lex_models_v2.types.export_filters.ExportFilters"]
...
def serialize_json(value: ListExportsRequest) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["sortBy"] = capo_lex_models_v2.types.export_sort_by.serialize_json(
            value["sort_by"]
        )
    if "filters" in value:
        import capo_lex_models_v2.types.export_filters

        out["filters"] = capo_lex_models_v2.types.export_filters.serialize_json(
            value["filters"]
        )
...
def deserialize_json(data: dict) -> ListExportsRequest:
    ...
    if "sortBy" in data:
        ...
        out["sort_by"] = capo_lex_models_v2.types.export_sort_by.deserialize_json(
            data["sortBy"]
        )
    if "filters" in data:
        import capo_lex_models_v2.types.export_filters

        out["filters"] = capo_lex_models_v2.types.export_filters.deserialize_json(
            data["filters"]
        )
