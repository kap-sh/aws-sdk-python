...
class AnalyticsSessionMetricResult(TypedDict, closed=True):
    name: NotRequired[
        "capo_lex_models_v2.types.analytics_session_metric_name.AnalyticsSessionMetricName"
    ]
...
def serialize_json(value: AnalyticsSessionMetricResult) -> dict:
    ...
    if "name" in value:
        ...
        out["name"] = (
            capo_lex_models_v2.types.analytics_session_metric_name.serialize_json(
                value["name"]
            )
        )
