...
if TYPE_CHECKING:
    ...
    import capo_lex_models_v2.types.analytics_session_metric_name
    import capo_lex_models_v2.types.analytics_sort_order


class AnalyticsSessionMetric(TypedDict, closed=True):
    name: "capo_lex_models_v2.types.analytics_session_metric_name.AnalyticsSessionMetricName"
...
# --- restJson1 ser/de ---
def serialize_json(value: AnalyticsSessionMetric) -> dict:
    out: dict = {}
    import capo_lex_models_v2.types.analytics_session_metric_name

    out["name"] = capo_lex_models_v2.types.analytics_session_metric_name.serialize_json(
        value["name"]
    )
    import capo_lex_models_v2.types.analytics_metric_statistic
...
def deserialize_json(data: dict) -> AnalyticsSessionMetric:
    out: AnalyticsSessionMetric = {}  # type: ignore[typeddict-item]
    if "name" in data:
        import capo_lex_models_v2.types.analytics_session_metric_name

        out["name"] = (
            capo_lex_models_v2.types.analytics_session_metric_name.deserialize_json(
                data["name"]
            )
        )
    else:
        raise DeserializationError("AnalyticsSessionMetric.name required")
    if "statistic" in data:
        ...
        raise DeserializationError("AnalyticsSessionMetric.statistic required")
    if "order" in data:
        import capo_lex_models_v2.types.analytics_sort_order
