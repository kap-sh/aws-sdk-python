...
if TYPE_CHECKING:
    ...
    import capo_lex_models_v2.types.slot_filter_name
    import capo_lex_models_v2.types.slot_filter_operator


class SlotFilter(TypedDict, closed=True):
    name: "capo_lex_models_v2.types.slot_filter_name.SlotFilterName"
    ...
    operator: "capo_lex_models_v2.types.slot_filter_operator.SlotFilterOperator"
...
def serialize_json(value: SlotFilter) -> dict:
    ...
    out["name"] = capo_lex_models_v2.types.slot_filter_name.serialize_json(
        value["name"]
    )
    import capo_lex_models_v2.types.filter_values
    ...
    out["operator"] = capo_lex_models_v2.types.slot_filter_operator.serialize_json(
        value["operator"]
    )
    return out
...
def deserialize_json(data: dict) -> SlotFilter:
    ...
    if "name" in data:
        ...
        out["name"] = capo_lex_models_v2.types.slot_filter_name.deserialize_json(
            data["name"]
        )
    else:
    ...
    if "operator" in data:
        ...
        out["operator"] = (
            capo_lex_models_v2.types.slot_filter_operator.deserialize_json(
                data["operator"]
            )
        )
    else:
