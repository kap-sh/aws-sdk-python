...
if TYPE_CHECKING:
    import capo_lex_models_v2.types.aggregated_utterances_sort_attribute
    import capo_lex_models_v2.types.sort_order


class AggregatedUtterancesSortBy(TypedDict, closed=True):
    attribute: "capo_lex_models_v2.types.aggregated_utterances_sort_attribute.AggregatedUtterancesSortAttribute"
...
def serialize_json(value: AggregatedUtterancesSortBy) -> dict:
    ...
    out["attribute"] = (
        capo_lex_models_v2.types.aggregated_utterances_sort_attribute.serialize_json(
            value["attribute"]
        )
    )
    import capo_lex_models_v2.types.sort_order
...
def deserialize_json(data: dict) -> AggregatedUtterancesSortBy:
    ...
    if "attribute" in data:
        ...
        out["attribute"] = (
            capo_lex_models_v2.types.aggregated_utterances_sort_attribute.deserialize_json(
                data["attribute"]
            )
        )
    else:
