...
class ListSessionAnalyticsDataResponse(TypedDict, closed=True):
    ...
    sessions: NotRequired[
        "capo_lex_models_v2.types.session_specifications.SessionSpecifications"
    ]
...
def serialize_json(value: ListSessionAnalyticsDataResponse) -> dict:
    ...
    if "sessions" in value:
        ...
        out["sessions"] = (
            capo_lex_models_v2.types.session_specifications.serialize_json(
                value["sessions"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListSessionAnalyticsDataResponse:
    ...
    if "sessions" in data:
        ...
        out["sessions"] = (
            capo_lex_models_v2.types.session_specifications.deserialize_json(
                data["sessions"]
            )
        )
    return out
