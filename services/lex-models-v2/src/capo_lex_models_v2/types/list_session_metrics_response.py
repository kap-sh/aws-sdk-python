...
class ListSessionMetricsResponse(TypedDict, closed=True):
    ...
    results: NotRequired[
        "capo_lex_models_v2.types.analytics_session_results.AnalyticsSessionResults"
    ]
...
def serialize_json(value: ListSessionMetricsResponse) -> dict:
    ...
    if "results" in value:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_session_results.serialize_json(
                value["results"]
            )
        )
...
def deserialize_json(data: dict) -> ListSessionMetricsResponse:
    ...
    if "results" in data:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_session_results.deserialize_json(
                data["results"]
            )
        )
