...
if TYPE_CHECKING:
    import capo_lex_models_v2.types.slot_sort_attribute
    import capo_lex_models_v2.types.sort_order


class SlotSortBy(TypedDict, closed=True):
    attribute: "capo_lex_models_v2.types.slot_sort_attribute.SlotSortAttribute"
...
def serialize_json(value: SlotSortBy) -> dict:
    ...
    out["attribute"] = capo_lex_models_v2.types.slot_sort_attribute.serialize_json(
        value["attribute"]
    )
    import capo_lex_models_v2.types.sort_order
...
def deserialize_json(data: dict) -> SlotSortBy:
    ...
    if "attribute" in data:
        ...
        out["attribute"] = (
            capo_lex_models_v2.types.slot_sort_attribute.deserialize_json(
                data["attribute"]
            )
        )
    else:
