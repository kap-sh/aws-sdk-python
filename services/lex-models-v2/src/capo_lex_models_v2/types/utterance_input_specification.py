...
class UtteranceInputSpecification(TypedDict, closed=True):
    ...
    audio_input: NotRequired[
        "capo_lex_models_v2.types.utterance_audio_input_specification.UtteranceAudioInputSpecification"
    ]
...
def serialize_json(value: UtteranceInputSpecification) -> dict:
    ...
    if "audio_input" in value:
        ...
        out["audioInput"] = (
            capo_lex_models_v2.types.utterance_audio_input_specification.serialize_json(
                value["audio_input"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> UtteranceInputSpecification:
    ...
    if "audioInput" in data:
        ...
        out["audio_input"] = (
            capo_lex_models_v2.types.utterance_audio_input_specification.deserialize_json(
                data["audioInput"]
            )
        )
    return out
