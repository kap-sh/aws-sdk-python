...
class ListUtteranceMetricsResponse(TypedDict, closed=True):
    ...
    results: NotRequired[
        "capo_lex_models_v2.types.analytics_utterance_results.AnalyticsUtteranceResults"
    ]
...
def serialize_json(value: ListUtteranceMetricsResponse) -> dict:
    ...
    if "results" in value:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_utterance_results.serialize_json(
                value["results"]
            )
        )
...
def deserialize_json(data: dict) -> ListUtteranceMetricsResponse:
    ...
    if "results" in data:
        ...
        out["results"] = (
            capo_lex_models_v2.types.analytics_utterance_results.deserialize_json(
                data["results"]
            )
        )
