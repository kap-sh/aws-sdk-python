...
class UpdateTestSetResponse(TypedDict, closed=True):
    ...
    modality: NotRequired["capo_lex_models_v2.types.test_set_modality.TestSetModality"]
    """<p>Indicates whether audio or text is used for the updated test set.</p>"""
    status: NotRequired["capo_lex_models_v2.types.test_set_status.TestSetStatus"]
    ...
    num_turns: NotRequired["capo_lex_models_v2.types.count.Count"]
...
def serialize_json(value: UpdateTestSetResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_lex_models_v2.types.test_set_status.serialize_json(
            value["status"]
        )
