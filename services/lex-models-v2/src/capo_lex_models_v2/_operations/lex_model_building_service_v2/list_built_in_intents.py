"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListBuiltInIntents``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_built_in_intents_response.ListBuiltInIntentsResponse = capo_lex_models_v2.types.list_built_in_intents_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/builtins/locales/{localeId}/intents"
    url = url.replace("{localeId}", quote(str(input_["locale_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_built_in_intents_request.serialize_json(input_)
    ).encode()
...
async def async_list_built_in_intents(
