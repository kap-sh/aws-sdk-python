"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#UpdateBot``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.update_bot_response.UpdateBotResponse = (
        capo_lex_models_v2.types.update_bot_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.update_bot_request.serialize_json(input_)
    ).encode()
...
def update_bot(
    ...
    capo_lex_models_v2.types.update_bot_response.UpdateBotResponse, zapros.Response
]:
