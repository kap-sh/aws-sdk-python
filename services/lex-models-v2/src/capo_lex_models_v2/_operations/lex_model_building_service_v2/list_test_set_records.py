"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListTestSetRecords``."""
...
import capo_lex_models_v2.types.test_set_turn_record_list
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_test_set_records_response.ListTestSetRecordsResponse = capo_lex_models_v2.types.list_test_set_records_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/testsets/{testSetId}/records"
    url = url.replace("{testSetId}", quote(str(input_["test_set_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_test_set_records_request.serialize_json(input_)
    ).encode()
...
async def async_list_test_set_records(
