"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#ListIntentStageMetrics``."""
...
import capo_lex_models_v2.types.analytics_intent_stage_results
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.list_intent_stage_metrics_response.ListIntentStageMetricsResponse = capo_lex_models_v2.types.list_intent_stage_metrics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/analytics/intentstagemetrics"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.list_intent_stage_metrics_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_intent_stage_metrics(
