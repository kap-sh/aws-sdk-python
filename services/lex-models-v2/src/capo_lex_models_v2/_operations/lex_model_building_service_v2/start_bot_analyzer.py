"""Generated from Smithy shape ``com.amazonaws.lexmodelsv2#StartBotAnalyzer``."""
...
def handle_response(
    ...
    out: capo_lex_models_v2.types.start_bot_analyzer_response.StartBotAnalyzerResponse = capo_lex_models_v2.types.start_bot_analyzer_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/bots/{botId}/botanalyzer"
    url = url.replace("{botId}", quote(str(input_["bot_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_lex_models_v2.types.start_bot_analyzer_request.serialize_json(input_)
    ).encode()
...
async def async_start_bot_analyzer(
