...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            import aws_sdk_geo_maps.errors.resource_not_found_exception

            raise aws_sdk_geo_maps.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_geo_maps.types.get_tile_response.GetTileResponse = {
        "blob": response.read()
...
def build_request(
    ...
    input: aws_sdk_geo_maps.types.get_tile_request.GetTileRequest,
) -> zapros.Request:
    ...
    url = url.replace("{Tileset}", quote(str(input["tileset"]), safe=""))
    url = url.replace("{Z}", quote(str(input["z"]), safe=""))
    url = url.replace("{X}", quote(str(input["x"]), safe=""))
    url = url.replace("{Y}", quote(str(input["y"]), safe=""))
    params: dict[str, str] = {}
    if "additional_features" in input:
        params["additional-features"] = str(input["additional_features"])
    if "key" in input:
...
def get_tile(
    ...
    input: aws_sdk_geo_maps.types.get_tile_request.GetTileRequest,
) -> tuple[aws_sdk_geo_maps.types.get_tile_response.GetTileResponse, zapros.Response]:
