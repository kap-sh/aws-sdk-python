...
class GeoMapsError(Exception):
    """Base class for all GeoMaps service errors."""


class ServiceError(GeoMapsError):
...
class SerializationError(GeoMapsError):
    """Raised when a value cannot be serialized."""


class DeserializationError(GeoMapsError):
...
class WaiterFailedError(GeoMapsError):
...
class WaiterTimeoutError(GeoMapsError):
