...
class StreamSelection(TypedDict, closed=True):
    ...
    stream_order: NotRequired["aws_sdk_mediapackage.types.stream_order.StreamOrder"]
...
def serialize_json(value: StreamSelection) -> dict:
    ...
    if "stream_order" in value:
        ...
        out["streamOrder"] = aws_sdk_mediapackage.types.stream_order.serialize_json(
            value["stream_order"]
        )
    return out
...
def deserialize_json(data: dict) -> StreamSelection:
    ...
    if "streamOrder" in data:
        ...
        out["stream_order"] = aws_sdk_mediapackage.types.stream_order.deserialize_json(
            data["streamOrder"]
        )
    return out
