"""Generated from Smithy shape ``com.amazonaws.licensemanagerusersubscriptions#RegisterIdentityProvider``."""
...
def handle_response(
    ...
    out: capo_license_manager_user_subscriptions.types.register_identity_provider_response.RegisterIdentityProviderResponse = capo_license_manager_user_subscriptions.types.register_identity_provider_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identity-provider/RegisterIdentityProvider"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_user_subscriptions.types.register_identity_provider_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_register_identity_provider(
