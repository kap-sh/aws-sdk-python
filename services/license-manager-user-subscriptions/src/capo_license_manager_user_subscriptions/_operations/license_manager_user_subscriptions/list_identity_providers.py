"""Generated from Smithy shape ``com.amazonaws.licensemanagerusersubscriptions#ListIdentityProviders``."""
...
def handle_response(
    ...
    out: capo_license_manager_user_subscriptions.types.list_identity_providers_response.ListIdentityProvidersResponse = capo_license_manager_user_subscriptions.types.list_identity_providers_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identity-provider/ListIdentityProviders"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_license_manager_user_subscriptions.types.list_identity_providers_request.serialize_json(
            input_
        )
    ).encode()
