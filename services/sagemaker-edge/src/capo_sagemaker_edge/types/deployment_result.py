...
class DeploymentResult(TypedDict, closed=True):
    ...
    deployment_status: NotRequired["capo_sagemaker_edge.types.entity_name.EntityName"]
    """<p>Returns the bucket error code.</p>"""
    deployment_status_message: NotRequired["capo_sagemaker_edge.types.string.String"]
    """<p>Returns the detailed error message.</p>"""
    deployment_start_time: NotRequired["capo_sagemaker_edge.types.timestamp.Timestamp"]
    """<p>The timestamp of when the deployment was started on the agent.</p>"""
    deployment_end_time: NotRequired["capo_sagemaker_edge.types.timestamp.Timestamp"]
    """<p>The timestamp of when the deployment was ended, and the agent got the deployment results.</p>"""
    deployment_models: NotRequired[
        "capo_sagemaker_edge.types.deployment_models.DeploymentModels"
    ]
...
def serialize_json(value: DeploymentResult) -> dict:
    ...
    if "deployment_start_time" in value:
        ...
        out["DeploymentStartTime"] = capo_sagemaker_edge.types.timestamp.serialize_json(
            value["deployment_start_time"]
        )
    if "deployment_end_time" in value:
        import capo_sagemaker_edge.types.timestamp

        out["DeploymentEndTime"] = capo_sagemaker_edge.types.timestamp.serialize_json(
    ...
    if "deployment_models" in value:
        ...
        out["DeploymentModels"] = (
            capo_sagemaker_edge.types.deployment_models.serialize_json(
...
def deserialize_json(data: dict) -> DeploymentResult:
    ...
    if "DeploymentModels" in data:
        ...
        out["deployment_models"] = (
            capo_sagemaker_edge.types.deployment_models.deserialize_json(
