"""Generated from Smithy shape ``com.amazonaws.memorydb#DescribeSubnetGroups``."""
...
def build_request(
    ...
    input_: aws_sdk_memorydb.types.describe_subnet_groups_request.DescribeSubnetGroupsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    headers["X-Amz-Target"] = "AmazonMemoryDB.DescribeSubnetGroups"
    import aws_sdk_memorydb.types.describe_subnet_groups_request

    body: bytes | None = json.dumps(
        aws_sdk_memorydb.types.describe_subnet_groups_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
...
async def async_describe_subnet_groups(
