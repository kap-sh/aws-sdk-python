...
class ListSourceRepositoriesResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_codecatalyst.types.list_source_repositories_items.ListSourceRepositoriesItems"
    ]
...
def serialize_json(value: ListSourceRepositoriesResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_codecatalyst.types.list_source_repositories_items.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListSourceRepositoriesResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            capo_codecatalyst.types.list_source_repositories_items.deserialize_json(
                data["items"]
            )
        )
