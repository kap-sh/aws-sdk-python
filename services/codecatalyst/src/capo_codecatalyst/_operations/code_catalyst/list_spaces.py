"""Generated from Smithy shape ``com.amazonaws.codecatalyst#ListSpaces``."""
...
def handle_response(
    ...
    out: capo_codecatalyst.types.list_spaces_response.ListSpacesResponse = (
        capo_codecatalyst.types.list_spaces_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/spaces"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codecatalyst.types.list_spaces_request.serialize_json(input_)
    ).encode()
...
def list_spaces(
    ...
    capo_codecatalyst.types.list_spaces_response.ListSpacesResponse, zapros.Response
]:
