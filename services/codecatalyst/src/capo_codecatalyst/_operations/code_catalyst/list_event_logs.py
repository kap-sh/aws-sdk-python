"""Generated from Smithy shape ``com.amazonaws.codecatalyst#ListEventLogs``."""
...
def handle_response(
    ...
    out: capo_codecatalyst.types.list_event_logs_response.ListEventLogsResponse = (
        capo_codecatalyst.types.list_event_logs_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/spaces/{spaceName}/eventLogs"
    url = url.replace("{spaceName}", quote(str(input_["space_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_codecatalyst.types.list_event_logs_request.serialize_json(input_)
    ).encode()
...
async def async_list_event_logs(
