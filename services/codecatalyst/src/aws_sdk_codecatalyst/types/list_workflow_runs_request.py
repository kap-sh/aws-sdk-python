...
class ListWorkflowRunsRequest(TypedDict, closed=True):
    ...
    sort_by: NotRequired[
        "aws_sdk_codecatalyst.types.workflow_run_sort_criteria_list.WorkflowRunSortCriteriaList"
    ]
...
def serialize_json(value: ListWorkflowRunsRequest) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["sortBy"] = (
            aws_sdk_codecatalyst.types.workflow_run_sort_criteria_list.serialize_json(
                value["sort_by"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> ListWorkflowRunsRequest:
    ...
    if "sortBy" in data:
        ...
        out["sort_by"] = (
            aws_sdk_codecatalyst.types.workflow_run_sort_criteria_list.deserialize_json(
                data["sortBy"]
            )
        )
    return out
