...
def handle_response(
    ...
    out: aws_sdk_cognito_identity_provider.types.confirm_forgot_password_response.ConfirmForgotPasswordResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_cognito_identity_provider.types.confirm_forgot_password_request.ConfirmForgotPasswordRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_cognito_identity_provider.types.confirm_forgot_password_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
