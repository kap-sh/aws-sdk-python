...
def serialize_aws_json_1_1(value: VerifySoftwareTokenResponse) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            aws_sdk_cognito_identity_provider.types.verify_software_token_response_type.serialize_aws_json_1_1(
                value["status"]
            )
        )
    if "session" in value:
...
def deserialize_aws_json_1_1(data: dict) -> VerifySoftwareTokenResponse:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            aws_sdk_cognito_identity_provider.types.verify_software_token_response_type.deserialize_aws_json_1_1(
                data["Status"]
            )
        )
    if "Session" in data:
