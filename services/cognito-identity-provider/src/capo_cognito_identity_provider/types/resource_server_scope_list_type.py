...
if TYPE_CHECKING:
    import capo_cognito_identity_provider.types.resource_server_scope_type

ResourceServerScopeListType: TypeAlias = list[
    "capo_cognito_identity_provider.types.resource_server_scope_type.ResourceServerScopeType"
]
...
def serialize_aws_json_1_1(value: ResourceServerScopeListType) -> list:
    ...
    for item in value:
        out.append(
            capo_cognito_identity_provider.types.resource_server_scope_type.serialize_aws_json_1_1(
                item
            )
        )
    return out
