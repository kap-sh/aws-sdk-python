...
class EventRiskType(TypedDict, closed=True):
    risk_decision: NotRequired[
        "capo_cognito_identity_provider.types.risk_decision_type.RiskDecisionType"
    ]
    ...
    risk_level: NotRequired[
        "capo_cognito_identity_provider.types.risk_level_type.RiskLevelType"
    ]
...
def serialize_aws_json_1_1(value: EventRiskType) -> dict:
    ...
    if "risk_decision" in value:
        ...
        out["RiskDecision"] = (
            capo_cognito_identity_provider.types.risk_decision_type.serialize_aws_json_1_1(
                value["risk_decision"]
            )
        )
    if "risk_level" in value:
        ...
        out["RiskLevel"] = (
            capo_cognito_identity_provider.types.risk_level_type.serialize_aws_json_1_1(
                value["risk_level"]
            )
        )
...
def deserialize_aws_json_1_1(data: dict) -> EventRiskType:
    ...
    if "RiskDecision" in data:
        ...
        out["risk_decision"] = (
            capo_cognito_identity_provider.types.risk_decision_type.deserialize_aws_json_1_1(
                data["RiskDecision"]
            )
        )
    if "RiskLevel" in data:
        ...
        out["risk_level"] = (
            capo_cognito_identity_provider.types.risk_level_type.deserialize_aws_json_1_1(
                data["RiskLevel"]
            )
        )
