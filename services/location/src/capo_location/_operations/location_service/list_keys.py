"""Generated from Smithy shape ``com.amazonaws.location#ListKeys``."""
...
def handle_response(
    ...
    out: capo_location.types.list_keys_response.ListKeysResponse = (
        capo_location.types.list_keys_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/list-keys"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.list_keys_request.serialize_json(input_)
    ).encode()
...
def list_keys(
    ...
    input_: capo_location.types.list_keys_request.ListKeysRequest,
) -> tuple[capo_location.types.list_keys_response.ListKeysResponse, zapros.Response]:
