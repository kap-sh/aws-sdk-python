"""Generated from Smithy shape ``com.amazonaws.location#BatchUpdateDevicePosition``."""
...
def handle_response(
    ...
    out: capo_location.types.batch_update_device_position_response.BatchUpdateDevicePositionResponse = capo_location.types.batch_update_device_position_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/tracking/v0/trackers/{TrackerName}/positions"
    url = url.replace("{TrackerName}", quote(str(input_["tracker_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.batch_update_device_position_request.serialize_json(input_)
    ).encode()
...
async def async_batch_update_device_position(
