"""Generated from Smithy shape ``com.amazonaws.location#GetJob``."""
...
def handle_response(
    ...
    out: capo_location.types.get_job_response.GetJobResponse = (
        capo_location.types.get_job_response.deserialize_json(
...
def build_request(
    ...
    input_: capo_location.types.get_job_request.GetJobRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/jobs/{JobId}"
...
def get_job(
    options: OperationOptions, input_: capo_location.types.get_job_request.GetJobRequest
) -> tuple[capo_location.types.get_job_response.GetJobResponse, zapros.Response]:
