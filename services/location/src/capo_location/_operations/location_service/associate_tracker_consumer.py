"""Generated from Smithy shape ``com.amazonaws.location#AssociateTrackerConsumer``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_location.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ServiceQuotaExceededException":
            raise capo_location.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_location.types.associate_tracker_consumer_response.AssociateTrackerConsumerResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/tracking/v0/trackers/{TrackerName}/consumers"
    url = url.replace("{TrackerName}", quote(str(input_["tracker_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.associate_tracker_consumer_request.serialize_json(input_)
    ).encode()
...
async def async_associate_tracker_consumer(
