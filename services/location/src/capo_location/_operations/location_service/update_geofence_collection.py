"""Generated from Smithy shape ``com.amazonaws.location#UpdateGeofenceCollection``."""
...
def handle_response(
    ...
    out: capo_location.types.update_geofence_collection_response.UpdateGeofenceCollectionResponse = capo_location.types.update_geofence_collection_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/geofencing/v0/collections/{CollectionName}"
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        capo_location.types.update_geofence_collection_request.serialize_json(input_)
    ).encode()
...
async def async_update_geofence_collection(
