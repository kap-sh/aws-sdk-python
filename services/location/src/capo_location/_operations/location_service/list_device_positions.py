"""Generated from Smithy shape ``com.amazonaws.location#ListDevicePositions``."""
...
def handle_response(
    ...
    out: capo_location.types.list_device_positions_response.ListDevicePositionsResponse = capo_location.types.list_device_positions_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/tracking/v0/trackers/{TrackerName}/list-positions"
    )
    url = url.replace("{TrackerName}", quote(str(input_["tracker_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_location.types.list_device_positions_request.serialize_json(input_)
    ).encode()
...
async def async_list_device_positions(
