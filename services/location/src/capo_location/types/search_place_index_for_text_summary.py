...
class SearchPlaceIndexForTextSummary(TypedDict, closed=True):
    ...
    result_b_box: NotRequired["capo_location.types.bounding_box.BoundingBox"]
...
# --- restJson1 ser/de ---
def serialize_json(value: SearchPlaceIndexForTextSummary) -> dict:
    out: dict = {}
    out["Text"] = value["text"]
    ...
    if "result_b_box" in value:
        import capo_location.types.bounding_box

        out["ResultBBox"] = capo_location.types.bounding_box.serialize_json(
            value["result_b_box"]
        )
...
def deserialize_json(data: dict) -> SearchPlaceIndexForTextSummary:
    out: SearchPlaceIndexForTextSummary = {}  # type: ignore[typeddict-item]
    if "Text" in data:
        out["text"] = data["Text"]
    else:
        raise DeserializationError("SearchPlaceIndexForTextSummary.text required")
    ...
    if "ResultBBox" in data:
        import capo_location.types.bounding_box

        out["result_b_box"] = capo_location.types.bounding_box.deserialize_json(
            data["ResultBBox"]
        )
    if "DataSource" in data:
        ...
        raise DeserializationError(
            "SearchPlaceIndexForTextSummary.data_source required"
        )
    if "Language" in data:
