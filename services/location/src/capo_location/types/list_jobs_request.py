...
class ListJobsRequest(TypedDict, closed=True):
    filter: NotRequired["capo_location.types.jobs_filter.JobsFilter"]
...
def serialize_json(value: ListJobsRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["Filter"] = capo_location.types.jobs_filter.serialize_json(value["filter"])
...
def deserialize_json(data: dict) -> ListJobsRequest:
    ...
    if "Filter" in data:
        ...
        out["filter"] = capo_location.types.jobs_filter.deserialize_json(data["Filter"])
