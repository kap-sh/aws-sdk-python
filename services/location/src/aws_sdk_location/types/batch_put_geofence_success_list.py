...
BatchPutGeofenceSuccessList: TypeAlias = list[
    "aws_sdk_location.types.batch_put_geofence_success.BatchPutGeofenceSuccess"
]
...
def serialize_json(value: BatchPutGeofenceSuccessList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_location.types.batch_put_geofence_success.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BatchPutGeofenceSuccessList:
    ...
    for item in data:
        out.append(
            aws_sdk_location.types.batch_put_geofence_success.deserialize_json(item)
        )
    return out
