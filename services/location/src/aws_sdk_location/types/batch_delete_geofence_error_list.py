...
BatchDeleteGeofenceErrorList: TypeAlias = list[
    "aws_sdk_location.types.batch_delete_geofence_error.BatchDeleteGeofenceError"
]
...
def serialize_json(value: BatchDeleteGeofenceErrorList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_location.types.batch_delete_geofence_error.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BatchDeleteGeofenceErrorList:
    ...
    for item in data:
        out.append(
            aws_sdk_location.types.batch_delete_geofence_error.deserialize_json(item)
        )
    return out
