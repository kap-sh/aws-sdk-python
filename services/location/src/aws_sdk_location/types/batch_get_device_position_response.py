...
def serialize_json(value: BatchGetDevicePositionResponse) -> dict:
    ...
    out["Errors"] = (
        aws_sdk_location.types.batch_get_device_position_error_list.serialize_json(
            value["errors"]
        )
    )
    import aws_sdk_location.types.device_position_list
...
def deserialize_json(data: dict) -> BatchGetDevicePositionResponse:
    ...
    if "Errors" in data:
        ...
        out["errors"] = (
            aws_sdk_location.types.batch_get_device_position_error_list.deserialize_json(
                data["Errors"]
            )
        )
    else:
