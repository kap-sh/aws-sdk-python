...
def serialize_json(value: BatchPutGeofenceRequest) -> dict:
    ...
    out["Entries"] = (
        aws_sdk_location.types.batch_put_geofence_request_entry_list.serialize_json(
            value["entries"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> BatchPutGeofenceRequest:
    ...
    if "Entries" in data:
        ...
        out["entries"] = (
            aws_sdk_location.types.batch_put_geofence_request_entry_list.deserialize_json(
                data["Entries"]
            )
        )
    else:
