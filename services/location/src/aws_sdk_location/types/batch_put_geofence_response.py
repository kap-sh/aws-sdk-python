...
class BatchPutGeofenceResponse(TypedDict):
    ...
    errors: "aws_sdk_location.types.batch_put_geofence_error_list.BatchPutGeofenceErrorList"
...
def serialize_json(value: BatchPutGeofenceResponse) -> dict:
    out: dict = {}
    import aws_sdk_location.types.batch_put_geofence_success_list
    out["Successes"] = aws_sdk_location.types.batch_put_geofence_success_list.serialize_json(value["successes"])
    import aws_sdk_location.types.batch_put_geofence_error_list
    out["Errors"] = aws_sdk_location.types.batch_put_geofence_error_list.serialize_json(value["errors"])
    return out
...
def deserialize_json(data: dict) -> BatchPutGeofenceResponse:
    ...
    if "Successes" in data:
        import aws_sdk_location.types.batch_put_geofence_success_list
        out["successes"] = aws_sdk_location.types.batch_put_geofence_success_list.deserialize_json(data["Successes"])
    else:
        raise DeserializationError("BatchPutGeofenceResponse.successes required")
    if "Errors" in data:
        import aws_sdk_location.types.batch_put_geofence_error_list
        out["errors"] = aws_sdk_location.types.batch_put_geofence_error_list.deserialize_json(data["Errors"])
    else:
