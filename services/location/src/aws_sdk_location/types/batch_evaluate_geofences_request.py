...
class BatchEvaluateGeofencesRequest(TypedDict):
...
def serialize_json(value: BatchEvaluateGeofencesRequest) -> dict:
    out: dict = {}
    import aws_sdk_location.types.device_position_update_list
    out["DevicePositionUpdates"] = aws_sdk_location.types.device_position_update_list.serialize_json(value["device_position_updates"])
    return out
...
def deserialize_json(data: dict) -> BatchEvaluateGeofencesRequest:
    ...
    if "DevicePositionUpdates" in data:
        import aws_sdk_location.types.device_position_update_list
        out["device_position_updates"] = aws_sdk_location.types.device_position_update_list.deserialize_json(data["DevicePositionUpdates"])
    else:
        raise DeserializationError("BatchEvaluateGeofencesRequest.device_position_updates required")
    return out