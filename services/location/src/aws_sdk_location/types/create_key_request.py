...
def serialize_json(value: CreateKeyRequest) -> dict:
    ...
    import aws_sdk_location.types.api_key_restrictions
    out["Restrictions"] = aws_sdk_location.types.api_key_restrictions.serialize_json(value["restrictions"])
    if "description" in value:
        out["Description"] = value["description"]
    if "expire_time" in value:
        import aws_sdk_location.types.timestamp
        out["ExpireTime"] = aws_sdk_location.types.timestamp.serialize_json(value["expire_time"])
...
def deserialize_json(data: dict) -> CreateKeyRequest:
    ...
    if "Restrictions" in data:
        import aws_sdk_location.types.api_key_restrictions
        out["restrictions"] = aws_sdk_location.types.api_key_restrictions.deserialize_json(data["Restrictions"])
    else:
    ...
    if "ExpireTime" in data:
        import aws_sdk_location.types.timestamp
        out["expire_time"] = aws_sdk_location.types.timestamp.deserialize_json(data["ExpireTime"])
    if "NoExpiry" in data:
