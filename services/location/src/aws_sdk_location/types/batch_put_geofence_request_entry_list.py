...
BatchPutGeofenceRequestEntryList: TypeAlias = list[
    "aws_sdk_location.types.batch_put_geofence_request_entry.BatchPutGeofenceRequestEntry"
]
...
def serialize_json(value: BatchPutGeofenceRequestEntryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_location.types.batch_put_geofence_request_entry.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BatchPutGeofenceRequestEntryList:
    ...
    for item in data:
        out.append(
            aws_sdk_location.types.batch_put_geofence_request_entry.deserialize_json(
                item
            )
        )
    return out
