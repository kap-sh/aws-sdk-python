...
def serialize_json(value: BatchDeleteDevicePositionHistoryResponse) -> dict:
    out: dict = {}
    import aws_sdk_location.types.batch_delete_device_position_history_error_list
    out["Errors"] = aws_sdk_location.types.batch_delete_device_position_history_error_list.serialize_json(value["errors"])
    return out
...
def deserialize_json(data: dict) -> BatchDeleteDevicePositionHistoryResponse:
    ...
    if "Errors" in data:
        import aws_sdk_location.types.batch_delete_device_position_history_error_list
        out["errors"] = aws_sdk_location.types.batch_delete_device_position_history_error_list.deserialize_json(data["Errors"])
    else:
        raise DeserializationError("BatchDeleteDevicePositionHistoryResponse.errors required")
    return out