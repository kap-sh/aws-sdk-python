...
class BatchDeleteDevicePositionHistoryError(TypedDict):
...
def serialize_json(value: BatchDeleteDevicePositionHistoryError) -> dict:
    ...
    import aws_sdk_location.types.batch_item_error
    out["Error"] = aws_sdk_location.types.batch_item_error.serialize_json(value["error"])
    return out
...
def deserialize_json(data: dict) -> BatchDeleteDevicePositionHistoryError:
    ...
    if "DeviceId" in data:
        ...
        raise DeserializationError("BatchDeleteDevicePositionHistoryError.device_id required")
    if "Error" in data:
        import aws_sdk_location.types.batch_item_error
        out["error"] = aws_sdk_location.types.batch_item_error.deserialize_json(data["Error"])
    else:
        raise DeserializationError("BatchDeleteDevicePositionHistoryError.error required")
    return out