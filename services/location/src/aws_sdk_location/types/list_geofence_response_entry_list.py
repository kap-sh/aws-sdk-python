...
ListGeofenceResponseEntryList: TypeAlias = list[
    "aws_sdk_location.types.list_geofence_response_entry.ListGeofenceResponseEntry"
]
...
def serialize_json(value: ListGeofenceResponseEntryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_location.types.list_geofence_response_entry.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ListGeofenceResponseEntryList:
    ...
    for item in data:
        out.append(
            aws_sdk_location.types.list_geofence_response_entry.deserialize_json(item)
        )
    return out
