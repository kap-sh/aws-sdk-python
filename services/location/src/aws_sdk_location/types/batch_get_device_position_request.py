...
class BatchGetDevicePositionRequest(TypedDict):
...
def serialize_json(value: BatchGetDevicePositionRequest) -> dict:
    out: dict = {}
    import aws_sdk_location.types.id_list
    out["DeviceIds"] = aws_sdk_location.types.id_list.serialize_json(value["device_ids"])
    return out
...
def deserialize_json(data: dict) -> BatchGetDevicePositionRequest:
    ...
    if "DeviceIds" in data:
        import aws_sdk_location.types.id_list
        out["device_ids"] = aws_sdk_location.types.id_list.deserialize_json(data["DeviceIds"])
    else:
