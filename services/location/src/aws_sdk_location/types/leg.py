...
if TYPE_CHECKING:
    import aws_sdk_location.types.leg_geometry
...
class Leg(TypedDict, closed=True):
    ...
    geometry: NotRequired["aws_sdk_location.types.leg_geometry.LegGeometry"]
    """<p>Contains the calculated route's path as a linestring geometry.</p>"""
    steps: "aws_sdk_location.types.step_list.StepList"
...
def serialize_json(value: Leg) -> dict:
    ...
    if "geometry" in value:
        ...
        out["Geometry"] = aws_sdk_location.types.leg_geometry.serialize_json(
            value["geometry"]
        )
    import aws_sdk_location.types.step_list

    out["Steps"] = aws_sdk_location.types.step_list.serialize_json(value["steps"])
    return out
...
def deserialize_json(data: dict) -> Leg:
    ...
    if "Geometry" in data:
        ...
        out["geometry"] = aws_sdk_location.types.leg_geometry.deserialize_json(
            data["Geometry"]
        )
    if "Steps" in data:
        import aws_sdk_location.types.step_list

        out["steps"] = aws_sdk_location.types.step_list.deserialize_json(data["Steps"])
    else:
