...
class CalculateRouteMatrixResponse(TypedDict, closed=True):
    ...
    snapped_departure_positions: NotRequired[
        "aws_sdk_location.types.position_list.PositionList"
    ]
...
def serialize_json(value: CalculateRouteMatrixResponse) -> dict:
    ...
    out["RouteMatrix"] = aws_sdk_location.types.route_matrix.serialize_json(
        value["route_matrix"]
    )
    if "snapped_departure_positions" in value:
        ...
        out["SnappedDeparturePositions"] = (
            aws_sdk_location.types.position_list.serialize_json(
    ...
    out["Summary"] = (
        aws_sdk_location.types.calculate_route_matrix_summary.serialize_json(
            value["summary"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> CalculateRouteMatrixResponse:
    ...
    if "RouteMatrix" in data:
        ...
        out["route_matrix"] = aws_sdk_location.types.route_matrix.deserialize_json(
            data["RouteMatrix"]
        )
    else:
    ...
    if "Summary" in data:
        ...
        out["summary"] = (
            aws_sdk_location.types.calculate_route_matrix_summary.deserialize_json(
                data["Summary"]
            )
        )
    else:
