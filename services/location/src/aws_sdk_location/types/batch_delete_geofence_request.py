...
def serialize_json(value: BatchDeleteGeofenceRequest) -> dict:
    out: dict = {}
    import aws_sdk_location.types.id_list
    out["GeofenceIds"] = aws_sdk_location.types.id_list.serialize_json(value["geofence_ids"])
    return out
...
def deserialize_json(data: dict) -> BatchDeleteGeofenceRequest:
    ...
    if "GeofenceIds" in data:
        import aws_sdk_location.types.id_list
        out["geofence_ids"] = aws_sdk_location.types.id_list.deserialize_json(data["GeofenceIds"])
    else:
