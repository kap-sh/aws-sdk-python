...
def build_request(
    ...
    input_: aws_sdk_location.types.put_geofence_request.PutGeofenceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{CollectionName}", quote(str(input_["collection_name"]), safe="")
    )
    url = url.replace("{GeofenceId}", quote(str(input_["geofence_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_location.types.put_geofence_request

    body: bytes | None = json.dumps(
        aws_sdk_location.types.put_geofence_request.serialize_json(input_)
    ).encode()
