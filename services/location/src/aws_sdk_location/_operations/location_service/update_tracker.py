"""Generated from Smithy shape ``com.amazonaws.location#UpdateTracker``."""
...
def handle_response(
    ...
    out: aws_sdk_location.types.update_tracker_response.UpdateTrackerResponse = (
        aws_sdk_location.types.update_tracker_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.update_tracker_request.UpdateTrackerRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/tracking/v0/trackers/{TrackerName}"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_location.types.update_tracker_request.serialize_json(input_)
    ).encode()
