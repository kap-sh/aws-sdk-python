"""Generated from Smithy shape ``com.amazonaws.location#ListTrackers``."""
...
import aws_sdk_location.types.list_trackers_response_entry_list
...
def handle_response(
    ...
    out: aws_sdk_location.types.list_trackers_response.ListTrackersResponse = (
        aws_sdk_location.types.list_trackers_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.list_trackers_request.ListTrackersRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/tracking/v0/list-trackers"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_location.types.list_trackers_request.serialize_json(input_)
    ).encode()
...
def list_trackers(
    ...
    aws_sdk_location.types.list_trackers_response.ListTrackersResponse, zapros.Response
]:
