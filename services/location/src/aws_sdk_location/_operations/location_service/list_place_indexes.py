"""Generated from Smithy shape ``com.amazonaws.location#ListPlaceIndexes``."""
...
def handle_response(
    ...
    out: aws_sdk_location.types.list_place_indexes_response.ListPlaceIndexesResponse = (
        aws_sdk_location.types.list_place_indexes_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.list_place_indexes_request.ListPlaceIndexesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/places/v0/list-indexes"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_location.types.list_place_indexes_request

    body: bytes | None = json.dumps(
        aws_sdk_location.types.list_place_indexes_request.serialize_json(input_)
    ).encode()
...
async def async_list_place_indexes(
