"""Generated from Smithy shape ``com.amazonaws.location#CreateKey``."""
...
def handle_response(
    ...
    out: aws_sdk_location.types.create_key_response.CreateKeyResponse = (
        aws_sdk_location.types.create_key_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.create_key_request.CreateKeyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/keys"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_location.types.create_key_request.serialize_json(input_)
    ).encode()
...
def create_key(
    ...
    aws_sdk_location.types.create_key_response.CreateKeyResponse, zapros.Response
]:
