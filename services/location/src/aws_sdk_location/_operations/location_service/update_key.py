"""Generated from Smithy shape ``com.amazonaws.location#UpdateKey``."""
...
def handle_response(
    ...
    out: aws_sdk_location.types.update_key_response.UpdateKeyResponse = (
        aws_sdk_location.types.update_key_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.update_key_request.UpdateKeyRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/keys/{KeyName}"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_location.types.update_key_request.serialize_json(input_)
    ).encode()
...
def update_key(
    ...
    aws_sdk_location.types.update_key_response.UpdateKeyResponse, zapros.Response
]:
