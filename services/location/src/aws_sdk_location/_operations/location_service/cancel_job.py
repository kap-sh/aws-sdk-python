"""Generated from Smithy shape ``com.amazonaws.location#CancelJob``."""
...
def handle_response(
    ...
    out: aws_sdk_location.types.cancel_job_response.CancelJobResponse = (
        aws_sdk_location.types.cancel_job_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.cancel_job_request.CancelJobRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/jobs/cancel-job"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_location.types.cancel_job_request

    body: bytes | None = json.dumps(
        aws_sdk_location.types.cancel_job_request.serialize_json(input_)
    ).encode()
...
def cancel_job(
    ...
    aws_sdk_location.types.cancel_job_response.CancelJobResponse, zapros.Response
]:
