"""Generated from Smithy shape ``com.amazonaws.location#StartJob``."""
...
def handle_response(
    ...
    out: aws_sdk_location.types.start_job_response.StartJobResponse = (
        aws_sdk_location.types.start_job_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_location.types.start_job_request.StartJobRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/metadata/v0/jobs"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_location.types.start_job_request

    body: bytes | None = json.dumps(
        aws_sdk_location.types.start_job_request.serialize_json(input_)
    ).encode()
...
def start_job(
    ...
    input_: aws_sdk_location.types.start_job_request.StartJobRequest,
) -> tuple[aws_sdk_location.types.start_job_response.StartJobResponse, zapros.Response]:
