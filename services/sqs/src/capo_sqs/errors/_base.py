...
class SQSError(Exception):
    """Base class for all SQS service errors."""


class ServiceError(SQSError):
...
class SerializationError(SQSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(SQSError):
...
class WaiterFailedError(SQSError):
...
class WaiterTimeoutError(SQSError):
