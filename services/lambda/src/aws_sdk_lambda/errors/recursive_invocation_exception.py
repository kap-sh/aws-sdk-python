"""Generated from Smithy shape ``com.amazonaws.lambda#RecursiveInvocationException``."""

from typing import TYPE_CHECKING, TypedDict
from typing_extensions import NotRequired
from aws_sdk_lambda.errors import ServiceError

if TYPE_CHECKING:
    import aws_sdk_lambda.types.string


class RecursiveInvocationException_(TypedDict):
    type: NotRequired["aws_sdk_lambda.types.string.String"]
    """<p>The exception type.</p>"""
    message: NotRequired["aws_sdk_lambda.types.string.String"]
    """<p>The exception message.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: RecursiveInvocationException_) -> dict:
    out: dict = {}
    if "type" in value:
        out["Type"] = value["type"]
    if "message" in value:
        out["Message"] = value["message"]
    return out


def deserialize_json(data: dict) -> RecursiveInvocationException_:
    out: RecursiveInvocationException_ = {}  # type: ignore[typeddict-item]
    if "Type" in data:
        out["type"] = data["Type"]
    if "Message" in data:
        out["message"] = data["Message"]
    return out


class RecursiveInvocationException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.lambda#RecursiveInvocationException``."""

    code: str | None = "RecursiveInvocationException"

    def __init__(self, data: RecursiveInvocationException_):
        super().__init__(
            "client",
            is_throttling_error=False,
            is_retryable=False,
            code="RecursiveInvocationException",
        )
        self.data = data

    @classmethod
    def from_json(cls, data: dict) -> "RecursiveInvocationException":
        return cls(deserialize_json(data))
