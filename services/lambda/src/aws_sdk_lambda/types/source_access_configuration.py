...
if TYPE_CHECKING:
    import aws_sdk_lambda.types.source_access_type
    import aws_sdk_lambda.types.uri


class SourceAccessConfiguration(TypedDict):
    type: NotRequired["aws_sdk_lambda.types.source_access_type.SourceAccessType"]
    ...
    uri: NotRequired["aws_sdk_lambda.types.uri.URI"]
...
def serialize_json(value: SourceAccessConfiguration) -> dict:
    ...
    if "type" in value:
        ...
        out["Type"] = aws_sdk_lambda.types.source_access_type.serialize_json(
            value["type"]
        )
    if "uri" in value:
        out["URI"] = value["uri"]
...
def deserialize_json(data: dict) -> SourceAccessConfiguration:
    ...
    if "Type" in data:
        ...
        out["type"] = aws_sdk_lambda.types.source_access_type.deserialize_json(
            data["Type"]
        )
    if "URI" in data:
        out["uri"] = data["URI"]
