...
def build_request(
    ...
    input: aws_sdk_lambda.types.list_function_versions_by_capacity_provider_request.ListFunctionVersionsByCapacityProviderRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{CapacityProviderName}", quote(str(input["capacity_provider_name"]), safe="")
    )
    params: dict[str, str] = {}
    if "marker" in input:
