...
def build_request(
    ...
    input_: aws_sdk_lambda.types.list_event_source_mappings_request.ListEventSourceMappingsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    if "event_source_arn" in input_:
        params["EventSourceArn"] = str(input_["event_source_arn"])
    if "function_name" in input_:
        params["FunctionName"] = str(input_["function_name"])
    if "marker" in input_:
