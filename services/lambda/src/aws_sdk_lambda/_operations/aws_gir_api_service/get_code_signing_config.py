"""Generated from Smithy shape ``com.amazonaws.lambda#GetCodeSigningConfig``."""
...
def handle_response(
    ...
    out: aws_sdk_lambda.types.get_code_signing_config_response.GetCodeSigningConfigResponse = aws_sdk_lambda.types.get_code_signing_config_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_lambda.types.get_code_signing_config_request.GetCodeSigningConfigRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{CodeSigningConfigArn}", quote(str(input_["code_signing_config_arn"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_code_signing_config(
