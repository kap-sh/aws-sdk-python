...
def handle_response(
    ...
    out: capo_lambda.types.send_durable_execution_callback_success_response.SendDurableExecutionCallbackSuccessResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace("{CallbackId}", quote(input_["callback_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "result" in input_:
        body: bytes | None = input_["result"]
