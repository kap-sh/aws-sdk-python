...
class CodeSigningPolicies(TypedDict, closed=True):
    untrusted_artifact_on_deployment: NotRequired[
        "capo_lambda.types.code_signing_policy.CodeSigningPolicy"
    ]
...
def serialize_json(value: CodeSigningPolicies) -> dict:
    ...
    if "untrusted_artifact_on_deployment" in value:
        ...
        out["UntrustedArtifactOnDeployment"] = (
            capo_lambda.types.code_signing_policy.serialize_json(
...
def deserialize_json(data: dict) -> CodeSigningPolicies:
    ...
    if data.get("UntrustedArtifactOnDeployment") is not None:
        ...
        out["untrusted_artifact_on_deployment"] = (
            capo_lambda.types.code_signing_policy.deserialize_json(
