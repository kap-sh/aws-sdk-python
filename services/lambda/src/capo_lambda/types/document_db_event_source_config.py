...
if TYPE_CHECKING:
    import capo_lambda.types.collection_name
    import capo_lambda.types.database_name
    import capo_lambda.types.full_document


class DocumentDBEventSourceConfig(TypedDict, closed=True):
    database_name: NotRequired["capo_lambda.types.database_name.DatabaseName"]
    """<p> The name of the database to consume within the DocumentDB cluster. </p>"""
    collection_name: NotRequired["capo_lambda.types.collection_name.CollectionName"]
    ...
    full_document: NotRequired["capo_lambda.types.full_document.FullDocument"]
...
def serialize_json(value: DocumentDBEventSourceConfig) -> dict:
    ...
    if "full_document" in value:
        ...
        out["FullDocument"] = capo_lambda.types.full_document.serialize_json(
            value["full_document"]
        )
    return out
