...
if TYPE_CHECKING:
    import aws_sdk_tnb.types.list_sol_network_package_metadata
...
class ListSolNetworkPackageInfo(TypedDict):
    ...
    vnf_pkg_ids: NotRequired["aws_sdk_tnb.types.vnf_pkg_id_list.VnfPkgIdList"]
    """<p>Identifies the function package for the function package descriptor referenced by the onboarded network package.</p>"""
    metadata: "aws_sdk_tnb.types.list_sol_network_package_metadata.ListSolNetworkPackageMetadata"
...
def serialize_json(value: ListSolNetworkPackageInfo) -> dict:
    ...
    out["metadata"] = (
        aws_sdk_tnb.types.list_sol_network_package_metadata.serialize_json(
            value["metadata"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ListSolNetworkPackageInfo:
    ...
    if "metadata" in data:
        ...
        out["metadata"] = (
            aws_sdk_tnb.types.list_sol_network_package_metadata.deserialize_json(
                data["metadata"]
            )
        )
    else:
