...
class ValidateSolFunctionPackageContentOutput(TypedDict, closed=True):
    ...
    metadata: "capo_tnb.types.validate_sol_function_package_content_metadata.ValidateSolFunctionPackageContentMetadata"
...
def serialize_json(value: ValidateSolFunctionPackageContentOutput) -> dict:
    ...
    out["metadata"] = (
        capo_tnb.types.validate_sol_function_package_content_metadata.serialize_json(
            value["metadata"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ValidateSolFunctionPackageContentOutput:
    ...
    if "metadata" in data:
        ...
        out["metadata"] = (
            capo_tnb.types.validate_sol_function_package_content_metadata.deserialize_json(
                data["metadata"]
            )
        )
    else:
