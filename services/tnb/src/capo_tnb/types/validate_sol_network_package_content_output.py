...
class ValidateSolNetworkPackageContentOutput(TypedDict, closed=True):
    ...
    metadata: "capo_tnb.types.validate_sol_network_package_content_metadata.ValidateSolNetworkPackageContentMetadata"
...
def serialize_json(value: ValidateSolNetworkPackageContentOutput) -> dict:
    ...
    out["metadata"] = (
        capo_tnb.types.validate_sol_network_package_content_metadata.serialize_json(
            value["metadata"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> ValidateSolNetworkPackageContentOutput:
    ...
    if "metadata" in data:
        ...
        out["metadata"] = (
            capo_tnb.types.validate_sol_network_package_content_metadata.deserialize_json(
                data["metadata"]
            )
        )
    else:
