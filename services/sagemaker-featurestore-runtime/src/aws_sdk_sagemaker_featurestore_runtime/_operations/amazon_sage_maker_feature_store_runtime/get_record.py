...
def build_request(
    ...
    input: aws_sdk_sagemaker_featurestore_runtime.types.get_record_request.GetRecordRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "record_identifier_value_as_string" in input:
        params["RecordIdentifierValueAsString"] = str(
            input["record_identifier_value_as_string"]
        )
    if "feature_names" in input:
        params["FeatureName"] = str(input["feature_names"])
    if "expiration_time_response" in input:
        params["ExpirationTimeResponse"] = str(input["expiration_time_response"])
