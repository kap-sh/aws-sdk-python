...
if TYPE_CHECKING:
    import aws_sdk_s3files.types.import_trigger


class ImportDataRule(TypedDict):
    prefix: "str"
    ...
    trigger: "aws_sdk_s3files.types.import_trigger.ImportTrigger"
...
def serialize_json(value: ImportDataRule) -> dict:
    ...
    out["trigger"] = aws_sdk_s3files.types.import_trigger.serialize_json(
        value["trigger"]
    )
...
def deserialize_json(data: dict) -> ImportDataRule:
    ...
    if "trigger" in data:
        ...
        out["trigger"] = aws_sdk_s3files.types.import_trigger.deserialize_json(
            data["trigger"]
        )
    else:
