...
AccessPoints: TypeAlias = list[
    "aws_sdk_s3files.types.list_access_points_description.ListAccessPointsDescription"
]
...
def serialize_json(value: AccessPoints) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_s3files.types.list_access_points_description.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> AccessPoints:
    ...
    for item in data:
        out.append(
            aws_sdk_s3files.types.list_access_points_description.deserialize_json(item)
        )
    return out
