"""Generated from Smithy shape ``com.amazonaws.s3files#GetAccessPoint``."""
...
def handle_response(
    ...
    out: aws_sdk_s3files.types.get_access_point_response.GetAccessPointResponse = (
        aws_sdk_s3files.types.get_access_point_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_s3files.types.get_access_point_request.GetAccessPointRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/access-points/{accessPointId}"
    url = url.replace("{accessPointId}", quote(str(input_["access_point_id"]), safe=""))
    params: dict[str, str] = {}
