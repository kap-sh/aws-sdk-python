...
class S3FilesError(Exception):
    """Base class for all S3Files service errors."""


class ServiceError(S3FilesError):
...
class SerializationError(S3FilesError):
    """Raised when a value cannot be serialized."""


class DeserializationError(S3FilesError):
...
class WaiterFailedError(S3FilesError):
...
class WaiterTimeoutError(S3FilesError):
