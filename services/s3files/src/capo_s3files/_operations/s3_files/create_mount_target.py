"""Generated from Smithy shape ``com.amazonaws.s3files#CreateMountTarget``."""
...
def handle_response(
    ...
    out: capo_s3files.types.create_mount_target_response.CreateMountTargetResponse = (
        capo_s3files.types.create_mount_target_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/mount-targets"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_s3files.types.create_mount_target_request.serialize_json(input_)
    ).encode()
