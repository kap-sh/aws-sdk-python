"""Generated from Smithy shape ``com.amazonaws.s3files#GetMountTargetResponse``."""
...
class GetMountTargetResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetMountTargetResponse) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> GetMountTargetResponse:
    out: GetMountTargetResponse = {}  # type: ignore[typeddict-item]
    if "availabilityZoneId" in data:
        out["availability_zone_id"] = data["availabilityZoneId"]
    if "ownerId" in data:
        out["owner_id"] = data["ownerId"]
    else:
        raise DeserializationError("GetMountTargetResponse.owner_id required")
    if "mountTargetId" in data:
        out["mount_target_id"] = data["mountTargetId"]
    else:
        raise DeserializationError("GetMountTargetResponse.mount_target_id required")
    if "fileSystemId" in data:
        out["file_system_id"] = data["fileSystemId"]
    if "subnetId" in data:
        out["subnet_id"] = data["subnetId"]
    else:
        raise DeserializationError("GetMountTargetResponse.subnet_id required")
