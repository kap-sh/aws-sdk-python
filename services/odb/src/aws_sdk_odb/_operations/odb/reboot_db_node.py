"""Generated from Smithy shape ``com.amazonaws.odb#RebootDbNode``."""
...
def handle_response(
    ...
    out: aws_sdk_odb.types.reboot_db_node_output.RebootDbNodeOutput = (
        aws_sdk_odb.types.reboot_db_node_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_odb.types.reboot_db_node_input.RebootDbNodeInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{cloudVmClusterId}", quote(str(input_["cloud_vm_cluster_id"]), safe="")
    )
    url = url.replace("{dbNodeId}", quote(str(input_["db_node_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Odb.RebootDbNode"
    body: bytes | None = b""
...
def reboot_db_node(
    ...
    input_: aws_sdk_odb.types.reboot_db_node_input.RebootDbNodeInput,
) -> tuple[aws_sdk_odb.types.reboot_db_node_output.RebootDbNodeOutput, zapros.Response]:
...
async def async_reboot_db_node(
