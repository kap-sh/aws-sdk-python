"""Generated from Smithy shape ``com.amazonaws.odb#UpdateAutonomousDatabaseBackup``."""
...
def handle_response(
    ...
    out: aws_sdk_odb.types.update_autonomous_database_backup_output.UpdateAutonomousDatabaseBackupOutput = aws_sdk_odb.types.update_autonomous_database_backup_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input: aws_sdk_odb.types.update_autonomous_database_backup_input.UpdateAutonomousDatabaseBackupInput,
) -> zapros.Request:
    ...
    url = url.replace(
        ...
        quote(str(input["autonomous_database_backup_id"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Odb.UpdateAutonomousDatabaseBackup"
    import aws_sdk_odb.types.update_autonomous_database_backup_input

    body: bytes | None = json.dumps(
        aws_sdk_odb.types.update_autonomous_database_backup_input.serialize_aws_json_1_0(
            input
        )
    ).encode()
...
async def async_update_autonomous_database_backup(
