"""Generated from Smithy shape ``com.amazonaws.odb#UpdateOdbPeeringConnection``."""
...
def handle_response(
    ...
    out: aws_sdk_odb.types.update_odb_peering_connection_output.UpdateOdbPeeringConnectionOutput = aws_sdk_odb.types.update_odb_peering_connection_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_odb.types.update_odb_peering_connection_input.UpdateOdbPeeringConnectionInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        ...
        quote(str(input_["odb_peering_connection_id"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Odb.UpdateOdbPeeringConnection"
    import aws_sdk_odb.types.update_odb_peering_connection_input

    body: bytes | None = json.dumps(
        aws_sdk_odb.types.update_odb_peering_connection_input.serialize_aws_json_1_0(
            input_
        )
    ).encode()
...
async def async_update_odb_peering_connection(
