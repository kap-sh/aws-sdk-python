"""Generated from Smithy shape ``com.amazonaws.odb#UpdateOdbNetwork``."""
...
def handle_response(
    ...
    out: aws_sdk_odb.types.update_odb_network_output.UpdateOdbNetworkOutput = (
        aws_sdk_odb.types.update_odb_network_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input_: aws_sdk_odb.types.update_odb_network_input.UpdateOdbNetworkInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{odbNetworkId}", quote(str(input_["odb_network_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Odb.UpdateOdbNetwork"
    import aws_sdk_odb.types.update_odb_network_input

    body: bytes | None = json.dumps(
        aws_sdk_odb.types.update_odb_network_input.serialize_aws_json_1_0(input_)
    ).encode()
...
def update_odb_network(
    ...
    aws_sdk_odb.types.update_odb_network_output.UpdateOdbNetworkOutput, zapros.Response
]:
...
async def async_update_odb_network(
