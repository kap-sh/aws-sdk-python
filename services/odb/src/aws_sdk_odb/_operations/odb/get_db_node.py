"""Generated from Smithy shape ``com.amazonaws.odb#GetDbNode``."""
...
def handle_response(
    ...
    out: aws_sdk_odb.types.get_db_node_output.GetDbNodeOutput = (
        aws_sdk_odb.types.get_db_node_output.deserialize_aws_json_1_0(
...
def build_request(
    ...
    input: aws_sdk_odb.types.get_db_node_input.GetDbNodeInput,
) -> zapros.Request:
    ...
    url = url.replace("{dbNodeId}", quote(str(input["db_node_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    headers["X-Amz-Target"] = "Odb.GetDbNode"
    body: bytes | None = b""
...
def get_db_node(
    options: OperationOptions, input: aws_sdk_odb.types.get_db_node_input.GetDbNodeInput
) -> tuple[aws_sdk_odb.types.get_db_node_output.GetDbNodeOutput, zapros.Response]:
