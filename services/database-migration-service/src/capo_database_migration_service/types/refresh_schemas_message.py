"""Generated from Smithy shape ``com.amazonaws.databasemigrationservice#RefreshSchemasMessage``."""
...
class RefreshSchemasMessage(TypedDict, closed=True):
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: RefreshSchemasMessage) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_1(data: dict) -> RefreshSchemasMessage:
    out: RefreshSchemasMessage = {}  # type: ignore[typeddict-item]
    if "EndpointArn" in data:
        out["endpoint_arn"] = data["EndpointArn"]
    else:
        raise DeserializationError("RefreshSchemasMessage.endpoint_arn required")
    if "ReplicationInstanceArn" in data:
        out["replication_instance_arn"] = data["ReplicationInstanceArn"]
    else:
        raise DeserializationError(
            "RefreshSchemasMessage.replication_instance_arn required"
        )
    return out
