...
def handle_response(
    ...
    out: aws_sdk_cloudwatch_logs.types.get_log_object_response.GetLogObjectResponse = {}  # type: ignore[typeddict-item]
    out["field_stream"] = cast(
        Any,
        aws_sdk_cloudwatch_logs._iter.map_sync_iterator(
            _messages,
            aws_sdk_cloudwatch_logs.types.get_log_object_response_stream.deserialize_event_aws_json_1_1,
        ),
    )
    return out
...
def build_request(
    ...
    input_: aws_sdk_cloudwatch_logs.types.get_log_object_request.GetLogObjectRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_cloudwatch_logs.types.get_log_object_request.serialize_aws_json_1_1(
            input_
        )
    ).encode()
