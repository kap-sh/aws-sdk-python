...
def handle_response(
    ...
    out: capo_cloudwatch_logs.types.start_live_tail_response.StartLiveTailResponse = {}  # type: ignore[typeddict-item]
    out["response_stream"] = cast(
        Any,
        capo_cloudwatch_logs._iter.map_sync_iterator(
            _messages,
            capo_cloudwatch_logs.types.start_live_tail_response_stream.deserialize_event_aws_json_1_1,
        ),
    )
    return out
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_cloudwatch_logs.types.start_live_tail_request.serialize_aws_json_1_1(
            input_
        ),
        allow_nan=False,
    ).encode()
