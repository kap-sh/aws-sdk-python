...
class ListDataIntegrationFlowsResponse(TypedDict):
    flows: (
        "aws_sdk_supplychain.types.data_integration_flow_list.DataIntegrationFlowList"
    )
...
def serialize_json(value: ListDataIntegrationFlowsResponse) -> dict:
    ...
    out["flows"] = aws_sdk_supplychain.types.data_integration_flow_list.serialize_json(
        value["flows"]
    )
...
def deserialize_json(data: dict) -> ListDataIntegrationFlowsResponse:
    ...
    if "flows" in data:
        ...
        out["flows"] = (
            aws_sdk_supplychain.types.data_integration_flow_list.deserialize_json(
                data["flows"]
            )
        )
    else:
