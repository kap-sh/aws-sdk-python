...
InstanceStateList: TypeAlias = list[
    "aws_sdk_supplychain.types.instance_state.InstanceState"
]
...
def serialize_json(value: InstanceStateList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_supplychain.types.instance_state.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> InstanceStateList:
    ...
    for item in data:
        out.append(aws_sdk_supplychain.types.instance_state.deserialize_json(item))
    return out
