...
if TYPE_CHECKING:
    import aws_sdk_supplychain.types.data_integration_flow_file_type


class DataIntegrationFlowS3Options(TypedDict):
    file_type: NotRequired[
        "aws_sdk_supplychain.types.data_integration_flow_file_type.DataIntegrationFlowFileType"
    ]
...
def serialize_json(value: DataIntegrationFlowS3Options) -> dict:
    ...
    if "file_type" in value:
        ...
        out["fileType"] = (
            aws_sdk_supplychain.types.data_integration_flow_file_type.serialize_json(
                value["file_type"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> DataIntegrationFlowS3Options:
    ...
    if "fileType" in data:
        ...
        out["file_type"] = (
            aws_sdk_supplychain.types.data_integration_flow_file_type.deserialize_json(
                data["fileType"]
            )
        )
    return out
