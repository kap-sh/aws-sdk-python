...
class DataIntegrationFlowDatasetOptions(TypedDict, closed=True):
    load_type: NotRequired[
        "capo_supplychain.types.data_integration_flow_load_type.DataIntegrationFlowLoadType"
    ]
    ...
    dedupe_strategy: NotRequired[
        "capo_supplychain.types.data_integration_flow_dedupe_strategy.DataIntegrationFlowDedupeStrategy"
    ]
...
def serialize_json(value: DataIntegrationFlowDatasetOptions) -> dict:
    ...
    if "load_type" in value:
        ...
        out["loadType"] = (
            capo_supplychain.types.data_integration_flow_load_type.serialize_json(
                value["load_type"]
            )
        )
    ...
    if "dedupe_strategy" in value:
        ...
        out["dedupeStrategy"] = (
            capo_supplychain.types.data_integration_flow_dedupe_strategy.serialize_json(
...
def deserialize_json(data: dict) -> DataIntegrationFlowDatasetOptions:
    ...
    if "loadType" in data:
        ...
        out["load_type"] = (
            capo_supplychain.types.data_integration_flow_load_type.deserialize_json(
                data["loadType"]
            )
        )
    ...
    if "dedupeStrategy" in data:
        ...
        out["dedupe_strategy"] = (
            capo_supplychain.types.data_integration_flow_dedupe_strategy.deserialize_json(
                data["dedupeStrategy"]
            )
        )
    return out
