"""Generated from Smithy shape ``com.amazonaws.pinpoint#EventStartCondition``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

if TYPE_CHECKING:
    import aws_sdk_pinpoint.types.__string
    import aws_sdk_pinpoint.types.event_filter


class EventStartCondition(TypedDict):
    event_filter: NotRequired["aws_sdk_pinpoint.types.event_filter.EventFilter"]
    segment_id: NotRequired["aws_sdk_pinpoint.types.__string.__string"]


# --- restJson1 ser/de ---
def serialize_json(value: EventStartCondition) -> dict:
    out: dict = {}
    if "event_filter" in value:
        import aws_sdk_pinpoint.types.event_filter

        out["EventFilter"] = aws_sdk_pinpoint.types.event_filter.serialize_json(
            value["event_filter"]
        )
    if "segment_id" in value:
        out["SegmentId"] = value["segment_id"]
    return out


def deserialize_json(data: dict) -> EventStartCondition:
    out: EventStartCondition = {}  # type: ignore[typeddict-item]
    if "EventFilter" in data:
        import aws_sdk_pinpoint.types.event_filter

        out["event_filter"] = aws_sdk_pinpoint.types.event_filter.deserialize_json(
            data["EventFilter"]
        )
    if "SegmentId" in data:
        out["segment_id"] = data["SegmentId"]
    return out
