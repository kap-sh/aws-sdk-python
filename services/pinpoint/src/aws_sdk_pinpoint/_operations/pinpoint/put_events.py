"""Generated from Smithy shape ``com.amazonaws.pinpoint#PutEvents``."""
...
def handle_response(
    ...
    out: aws_sdk_pinpoint.types.put_events_response.PutEventsResponse = {
        "events_response": aws_sdk_pinpoint.types.events_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_pinpoint.types.put_events_request.PutEventsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/events"
    url = url.replace("{ApplicationId}", quote(str(input_["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "events_request" in input_:
        import aws_sdk_pinpoint.types.events_request

        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.events_request.serialize_json(
                input_["events_request"]
            )
        ).encode()
...
def put_events(
    ...
    aws_sdk_pinpoint.types.put_events_response.PutEventsResponse, zapros.Response
]:
