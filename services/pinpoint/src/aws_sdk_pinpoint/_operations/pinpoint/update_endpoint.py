"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdateEndpoint``."""
...
def handle_response(
    ...
    out: aws_sdk_pinpoint.types.update_endpoint_response.UpdateEndpointResponse = {
...
def build_request(
    ...
    input: aws_sdk_pinpoint.types.update_endpoint_request.UpdateEndpointRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/endpoints/{EndpointId}"
    url = url.replace("{ApplicationId}", quote(str(input["application_id"]), safe=""))
    url = url.replace("{EndpointId}", quote(str(input["endpoint_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "endpoint_request" in input:
        import aws_sdk_pinpoint.types.endpoint_request

        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.endpoint_request.serialize_json(
                input["endpoint_request"]
            )
        ).encode()
