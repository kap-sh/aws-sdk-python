"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdateApnsSandboxChannel``."""
...
def handle_response(
    ...
    out: aws_sdk_pinpoint.types.update_apns_sandbox_channel_response.UpdateApnsSandboxChannelResponse = {
        "apns_sandbox_channel_response": aws_sdk_pinpoint.types.apns_sandbox_channel_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_pinpoint.types.update_apns_sandbox_channel_request.UpdateApnsSandboxChannelRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/channels/apns_sandbox"
    url = url.replace("{ApplicationId}", quote(str(input["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "apns_sandbox_channel_request" in input:
        ...
        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.apns_sandbox_channel_request.serialize_json(
                input["apns_sandbox_channel_request"]
            )
        ).encode()
...
async def async_update_apns_sandbox_channel(
