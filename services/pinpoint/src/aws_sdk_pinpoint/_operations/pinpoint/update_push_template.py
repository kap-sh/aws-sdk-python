"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdatePushTemplate``."""
...
def handle_response(
    ...
    out: aws_sdk_pinpoint.types.update_push_template_response.UpdatePushTemplateResponse = {
        "message_body": aws_sdk_pinpoint.types.message_body.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_pinpoint._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_pinpoint._auth._sigv4.build_sigv4_auth_scheme(
                "mobiletargeting", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_pinpoint._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input: aws_sdk_pinpoint.types.update_push_template_request.UpdatePushTemplateRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/v1/templates/{TemplateName}/push"
    url = url.replace("{TemplateName}", quote(str(input["template_name"]), safe=""))
    params: dict[str, str] = {}
    if "create_new_version" in input:
        params["create-new-version"] = str(input["create_new_version"])
    if "version" in input:
        params["version"] = str(input["version"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "push_notification_template_request" in input:
        ...
        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.push_notification_template_request.serialize_json(
                input["push_notification_template_request"]
            )
        ).encode()
...
async def async_update_push_template(
