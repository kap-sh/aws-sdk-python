"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdateApnsVoipSandboxChannel``."""
...
def build_request(
    ...
    input: aws_sdk_pinpoint.types.update_apns_voip_sandbox_channel_request.UpdateApnsVoipSandboxChannelRequest,
) -> zapros.Request:
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/channels/apns_voip_sandbox"
    )
    url = url.replace("{ApplicationId}", quote(str(input["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "apns_voip_sandbox_channel_request" in input:
        ...
        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.apns_voip_sandbox_channel_request.serialize_json(
                input["apns_voip_sandbox_channel_request"]
            )
        ).encode()
...
async def async_update_apns_voip_sandbox_channel(
