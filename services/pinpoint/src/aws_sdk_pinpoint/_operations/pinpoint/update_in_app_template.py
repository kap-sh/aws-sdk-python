...
def build_request(
    ...
    input_: aws_sdk_pinpoint.types.update_in_app_template_request.UpdateInAppTemplateRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    params: dict[str, str] = {}
    if "create_new_version" in input_:
        params["create-new-version"] = str(input_["create_new_version"])
    if "version" in input_:
        params["version"] = str(input_["version"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "in_app_template_request" in input_:
        import aws_sdk_pinpoint.types.in_app_template_request

        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.in_app_template_request.serialize_json(
                input_["in_app_template_request"]
            )
        ).encode()
