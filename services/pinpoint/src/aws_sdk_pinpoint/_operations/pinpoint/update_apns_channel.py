"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdateApnsChannel``."""
...
def handle_response(
    ...
    out: aws_sdk_pinpoint.types.update_apns_channel_response.UpdateApnsChannelResponse = {
        "apns_channel_response": aws_sdk_pinpoint.types.apns_channel_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_pinpoint.types.update_apns_channel_request.UpdateApnsChannelRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/channels/apns"
    url = url.replace("{ApplicationId}", quote(str(input_["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "apns_channel_request" in input_:
        import aws_sdk_pinpoint.types.apns_channel_request

        body: bytes | None = json.dumps(
            aws_sdk_pinpoint.types.apns_channel_request.serialize_json(
                input_["apns_channel_request"]
            )
        ).encode()
...
async def async_update_apns_channel(
