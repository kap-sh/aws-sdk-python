...
class OpenHours(TypedDict, closed=True):
    email: NotRequired[
        "capo_pinpoint.types.map_of_list_of_open_hours_rules.MapOfListOfOpenHoursRules"
    ]
...
def serialize_json(value: OpenHours) -> dict:
    ...
    if "email" in value:
        ...
        out["EMAIL"] = (
            capo_pinpoint.types.map_of_list_of_open_hours_rules.serialize_json(
                value["email"]
            )
        )
    if "sms" in value:
        ...
        out["SMS"] = capo_pinpoint.types.map_of_list_of_open_hours_rules.serialize_json(
            value["sms"]
        )
    if "push" in value:
        import capo_pinpoint.types.map_of_list_of_open_hours_rules
...
def deserialize_json(data: dict) -> OpenHours:
    ...
    if "EMAIL" in data:
        ...
        out["email"] = (
            capo_pinpoint.types.map_of_list_of_open_hours_rules.deserialize_json(
                data["EMAIL"]
            )
        )
    if "SMS" in data:
