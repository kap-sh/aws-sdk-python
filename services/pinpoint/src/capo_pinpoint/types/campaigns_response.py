...
if TYPE_CHECKING:
    ...
    import capo_pinpoint.types.list_of_campaign_response


class CampaignsResponse(TypedDict, closed=True):
    item: NotRequired[
        "capo_pinpoint.types.list_of_campaign_response.ListOfCampaignResponse"
    ]
...
def serialize_json(value: CampaignsResponse) -> dict:
    ...
    if "item" in value:
        ...
        out["Item"] = capo_pinpoint.types.list_of_campaign_response.serialize_json(
            value["item"]
        )
...
def deserialize_json(data: dict) -> CampaignsResponse:
    ...
    if "Item" in data:
        ...
        out["item"] = capo_pinpoint.types.list_of_campaign_response.deserialize_json(
            data["Item"]
        )
