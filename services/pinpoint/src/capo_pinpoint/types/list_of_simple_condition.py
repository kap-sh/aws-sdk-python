...
ListOfSimpleCondition: TypeAlias = list[
    "capo_pinpoint.types.simple_condition.SimpleCondition"
]
...
def serialize_json(value: ListOfSimpleCondition) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_pinpoint.types.simple_condition.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ListOfSimpleCondition:
    ...
    for item in data:
        out.append(capo_pinpoint.types.simple_condition.deserialize_json(item))
    return out
