...
if TYPE_CHECKING:
    ...
    import capo_pinpoint.types.delivery_status


class EndpointMessageResult(TypedDict, closed=True):
    address: NotRequired["capo_pinpoint.types.__string.__string"]
    """<p>The endpoint address that the message was delivered to.</p>"""
    delivery_status: NotRequired["capo_pinpoint.types.delivery_status.DeliveryStatus"]
    ...
    message_id: NotRequired["capo_pinpoint.types.__string.__string"]
    ...
    status_message: NotRequired["capo_pinpoint.types.__string.__string"]
    """<p>The status message for delivering the message.</p>"""
    updated_token: NotRequired["capo_pinpoint.types.__string.__string"]
...
def serialize_json(value: EndpointMessageResult) -> dict:
    ...
    if "delivery_status" in value:
        ...
        out["DeliveryStatus"] = capo_pinpoint.types.delivery_status.serialize_json(
...
def deserialize_json(data: dict) -> EndpointMessageResult:
    ...
    if "DeliveryStatus" in data:
        ...
        out["delivery_status"] = capo_pinpoint.types.delivery_status.deserialize_json(
            data["DeliveryStatus"]
        )
