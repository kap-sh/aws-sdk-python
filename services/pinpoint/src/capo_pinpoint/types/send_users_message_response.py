...
class SendUsersMessageResponse(TypedDict, closed=True):
    ...
    result: NotRequired[
        "capo_pinpoint.types.map_of_map_of_endpoint_message_result.MapOfMapOfEndpointMessageResult"
    ]
...
def serialize_json(value: SendUsersMessageResponse) -> dict:
    ...
    if "result" in value:
        ...
        out["Result"] = (
            capo_pinpoint.types.map_of_map_of_endpoint_message_result.serialize_json(
                value["result"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> SendUsersMessageResponse:
    ...
    if "Result" in data:
        ...
        out["result"] = (
            capo_pinpoint.types.map_of_map_of_endpoint_message_result.deserialize_json(
                data["Result"]
            )
        )
    return out
