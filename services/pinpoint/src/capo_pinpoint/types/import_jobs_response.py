...
if TYPE_CHECKING:
    ...
    import capo_pinpoint.types.list_of_import_job_response


class ImportJobsResponse(TypedDict, closed=True):
    item: NotRequired[
        "capo_pinpoint.types.list_of_import_job_response.ListOfImportJobResponse"
    ]
...
def serialize_json(value: ImportJobsResponse) -> dict:
    ...
    if "item" in value:
        ...
        out["Item"] = capo_pinpoint.types.list_of_import_job_response.serialize_json(
            value["item"]
        )
...
def deserialize_json(data: dict) -> ImportJobsResponse:
    ...
    if "Item" in data:
        ...
        out["item"] = capo_pinpoint.types.list_of_import_job_response.deserialize_json(
            data["Item"]
        )
