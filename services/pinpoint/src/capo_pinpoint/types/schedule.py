...
class Schedule(TypedDict, closed=True):
    end_time: NotRequired["capo_pinpoint.types.__string.__string"]
    ...
    frequency: NotRequired["capo_pinpoint.types.frequency.Frequency"]
    """<p>Specifies how often the campaign is sent or whether the campaign is sent in response to a specific event.</p>"""
    is_local_time: NotRequired["capo_pinpoint.types.__boolean.__boolean"]
    ...
    start_time: NotRequired["capo_pinpoint.types.__string.__string"]
...
def serialize_json(value: Schedule) -> dict:
    ...
    if "frequency" in value:
        ...
        out["Frequency"] = capo_pinpoint.types.frequency.serialize_json(
            value["frequency"]
        )
