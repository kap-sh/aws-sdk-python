...
if TYPE_CHECKING:
    ...
    import capo_pinpoint.types.list_of_application_response


class ApplicationsResponse(TypedDict, closed=True):
    item: NotRequired[
        "capo_pinpoint.types.list_of_application_response.ListOfApplicationResponse"
    ]
...
def serialize_json(value: ApplicationsResponse) -> dict:
    ...
    if "item" in value:
        ...
        out["Item"] = capo_pinpoint.types.list_of_application_response.serialize_json(
            value["item"]
        )
...
def deserialize_json(data: dict) -> ApplicationsResponse:
    ...
    if "Item" in data:
        ...
        out["item"] = capo_pinpoint.types.list_of_application_response.deserialize_json(
            data["Item"]
        )
