...
class SimpleEmail(TypedDict, closed=True):
    html_part: NotRequired["capo_pinpoint.types.simple_email_part.SimpleEmailPart"]
    ...
    subject: NotRequired["capo_pinpoint.types.simple_email_part.SimpleEmailPart"]
    """<p>The subject line, or title, of the email.</p>"""
    text_part: NotRequired["capo_pinpoint.types.simple_email_part.SimpleEmailPart"]
...
def serialize_json(value: SimpleEmail) -> dict:
    ...
    if "html_part" in value:
        ...
        out["HtmlPart"] = capo_pinpoint.types.simple_email_part.serialize_json(
            value["html_part"]
        )
    if "subject" in value:
        import capo_pinpoint.types.simple_email_part

        out["Subject"] = capo_pinpoint.types.simple_email_part.serialize_json(
            value["subject"]
        )
    if "text_part" in value:
        import capo_pinpoint.types.simple_email_part

        out["TextPart"] = capo_pinpoint.types.simple_email_part.serialize_json(
            value["text_part"]
        )
    if "headers" in value:
        import capo_pinpoint.types.list_of_message_header
