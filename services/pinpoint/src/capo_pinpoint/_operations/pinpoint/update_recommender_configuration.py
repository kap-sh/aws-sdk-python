"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdateRecommenderConfiguration``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.update_recommender_configuration_response.UpdateRecommenderConfigurationResponse = {
        "recommender_configuration_response": capo_pinpoint.types.recommender_configuration_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/recommenders/{RecommenderId}"
    url = url.replace("{RecommenderId}", quote(str(input_["recommender_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "update_recommender_configuration" in input_:
...
async def async_update_recommender_configuration(
