"""Generated from Smithy shape ``com.amazonaws.pinpoint#SendMessages``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.send_messages_response.SendMessagesResponse = {
        "message_response": capo_pinpoint.types.message_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/messages"
    url = url.replace("{ApplicationId}", quote(str(input_["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "message_request" in input_:
        body: bytes | None = json.dumps(
            capo_pinpoint.types.message_request.serialize_json(
                input_["message_request"]
            )
        ).encode()
...
def send_messages(
    ...
    capo_pinpoint.types.send_messages_response.SendMessagesResponse, zapros.Response
]:
