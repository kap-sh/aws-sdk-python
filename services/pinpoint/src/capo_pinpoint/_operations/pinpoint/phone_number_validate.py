"""Generated from Smithy shape ``com.amazonaws.pinpoint#PhoneNumberValidate``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.phone_number_validate_response.PhoneNumberValidateResponse = {
        "number_validate_response": capo_pinpoint.types.number_validate_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/phone/number/validate"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "number_validate_request" in input_:
        body: bytes | None = json.dumps(
            capo_pinpoint.types.number_validate_request.serialize_json(
                input_["number_validate_request"]
            )
        ).encode()
...
async def async_phone_number_validate(
