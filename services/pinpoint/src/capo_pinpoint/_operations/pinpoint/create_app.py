"""Generated from Smithy shape ``com.amazonaws.pinpoint#CreateApp``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.create_app_response.CreateAppResponse = {
...
def build_request(
    ...
    if "create_application_request" in input_:
        body: bytes | None = json.dumps(
            capo_pinpoint.types.create_application_request.serialize_json(
                input_["create_application_request"]
            )
        ).encode()
...
def create_app(
    ...
    input_: capo_pinpoint.types.create_app_request.CreateAppRequest,
) -> tuple[capo_pinpoint.types.create_app_response.CreateAppResponse, zapros.Response]:
