"""Generated from Smithy shape ``com.amazonaws.pinpoint#UpdateJourneyState``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.update_journey_state_response.UpdateJourneyStateResponse = {
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/journeys/{JourneyId}/state"
    )
    ...
    if "journey_state_request" in input_:
        body: bytes | None = json.dumps(
            capo_pinpoint.types.journey_state_request.serialize_json(
                input_["journey_state_request"]
            )
        ).encode()
...
async def async_update_journey_state(
