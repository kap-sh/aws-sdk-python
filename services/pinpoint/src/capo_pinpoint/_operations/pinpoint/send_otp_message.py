"""Generated from Smithy shape ``com.amazonaws.pinpoint#SendOTPMessage``."""
...
def handle_response(
    ...
    out: capo_pinpoint.types.send_otp_message_response.SendOTPMessageResponse = {
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/apps/{ApplicationId}/otp"
    url = url.replace("{ApplicationId}", quote(str(input_["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "send_otp_message_request_parameters" in input_:
...
async def async_send_otp_message(
