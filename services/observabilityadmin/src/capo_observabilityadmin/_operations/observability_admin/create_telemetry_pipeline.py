"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#CreateTelemetryPipeline``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.create_telemetry_pipeline_output.CreateTelemetryPipelineOutput = capo_observabilityadmin.types.create_telemetry_pipeline_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/CreateTelemetryPipeline"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.create_telemetry_pipeline_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_telemetry_pipeline(
