"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#GetTelemetryRule``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.get_telemetry_rule_output.GetTelemetryRuleOutput = capo_observabilityadmin.types.get_telemetry_rule_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/GetTelemetryRule"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.get_telemetry_rule_input.serialize_json(input_)
    ).encode()
...
async def async_get_telemetry_rule(
