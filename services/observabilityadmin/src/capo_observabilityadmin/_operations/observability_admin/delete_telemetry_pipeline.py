"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#DeleteTelemetryPipeline``."""
...
def handle_response(
    ...
    out: capo_observabilityadmin.types.delete_telemetry_pipeline_output.DeleteTelemetryPipelineOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteTelemetryPipeline"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.delete_telemetry_pipeline_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_telemetry_pipeline(
