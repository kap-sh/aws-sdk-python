"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#DeleteS3TableIntegration``."""
...
import capo_observabilityadmin.errors.invalid_state_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidStateException":
            raise capo_observabilityadmin.errors.invalid_state_exception.InvalidStateException.from_json(
                data
            )
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteS3TableIntegration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_observabilityadmin.types.delete_s3_table_integration_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_s3_table_integration(
