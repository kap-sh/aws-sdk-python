...
if TYPE_CHECKING:
    import capo_observabilityadmin.types.telemetry_pipeline


class GetTelemetryPipelineOutput(TypedDict, closed=True):
    pipeline: NotRequired[
        "capo_observabilityadmin.types.telemetry_pipeline.TelemetryPipeline"
    ]
...
def serialize_json(value: GetTelemetryPipelineOutput) -> dict:
    ...
    if "pipeline" in value:
        ...
        out["Pipeline"] = (
            capo_observabilityadmin.types.telemetry_pipeline.serialize_json(
                value["pipeline"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GetTelemetryPipelineOutput:
    ...
    if "Pipeline" in data:
        ...
        out["pipeline"] = (
            capo_observabilityadmin.types.telemetry_pipeline.deserialize_json(
                data["Pipeline"]
            )
        )
    return out
