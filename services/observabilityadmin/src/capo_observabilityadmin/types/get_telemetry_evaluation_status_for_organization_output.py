...
if TYPE_CHECKING:
    import capo_observabilityadmin.types.failure_reason
...
class GetTelemetryEvaluationStatusForOrganizationOutput(TypedDict, closed=True):
    status: NotRequired["capo_observabilityadmin.types.status.Status"]
    """<p> The onboarding status of the telemetry config feature for the organization. </p>"""
    failure_reason: NotRequired[
        "capo_observabilityadmin.types.failure_reason.FailureReason"
    ]
...
def serialize_json(value: GetTelemetryEvaluationStatusForOrganizationOutput) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_observabilityadmin.types.status.serialize_json(
            value["status"]
        )
