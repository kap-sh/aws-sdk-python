...
class WAFLoggingParameters(TypedDict, closed=True):
    redacted_fields: NotRequired[
        "capo_observabilityadmin.types.redacted_fields.RedactedFields"
    ]
    """<p> The fields to redact from WAF logs to protect sensitive information. </p>"""
    logging_filter: NotRequired[
        "capo_observabilityadmin.types.logging_filter.LoggingFilter"
    ]
    """<p> A filter configuration that determines which WAF log records to include or exclude. </p>"""
    log_type: NotRequired["capo_observabilityadmin.types.waf_log_type.WAFLogType"]
...
def serialize_json(value: WAFLoggingParameters) -> dict:
    ...
    if "redacted_fields" in value:
        ...
        out["RedactedFields"] = (
            capo_observabilityadmin.types.redacted_fields.serialize_json(
    ...
    if "logging_filter" in value:
        ...
        out["LoggingFilter"] = (
            capo_observabilityadmin.types.logging_filter.serialize_json(
    ...
    if "log_type" in value:
        ...
        out["LogType"] = capo_observabilityadmin.types.waf_log_type.serialize_json(
            value["log_type"]
        )
    return out
...
def deserialize_json(data: dict) -> WAFLoggingParameters:
    ...
    if "RedactedFields" in data:
        ...
        out["redacted_fields"] = (
            capo_observabilityadmin.types.redacted_fields.deserialize_json(
                data["RedactedFields"]
            )
        )
    if "LoggingFilter" in data:
        ...
        out["logging_filter"] = (
            capo_observabilityadmin.types.logging_filter.deserialize_json(
                data["LoggingFilter"]
            )
        )
    if "LogType" in data:
