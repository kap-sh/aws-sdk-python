"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#StartTelemetryEvaluation``."""
...
def build_request(
    ...
    input_: aws_sdk_observabilityadmin.types.start_telemetry_evaluation_input.StartTelemetryEvaluationInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/StartTelemetryEvaluation"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_observabilityadmin.types.start_telemetry_evaluation_input

    body: bytes | None = json.dumps(
        aws_sdk_observabilityadmin.types.start_telemetry_evaluation_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_telemetry_evaluation(
