"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#UpdateTelemetryPipeline``."""
...
def handle_response(
    ...
    out: aws_sdk_observabilityadmin.types.update_telemetry_pipeline_output.UpdateTelemetryPipelineOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_observabilityadmin.types.update_telemetry_pipeline_input.UpdateTelemetryPipelineInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/UpdateTelemetryPipeline"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_observabilityadmin.types.update_telemetry_pipeline_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_telemetry_pipeline(
