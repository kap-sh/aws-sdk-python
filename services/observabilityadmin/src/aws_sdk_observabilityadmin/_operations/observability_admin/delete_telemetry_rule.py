"""Generated from Smithy shape ``com.amazonaws.observabilityadmin#DeleteTelemetryRule``."""
...
def build_request(
    ...
    input_: aws_sdk_observabilityadmin.types.delete_telemetry_rule_input.DeleteTelemetryRuleInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/DeleteTelemetryRule"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_observabilityadmin.types.delete_telemetry_rule_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_telemetry_rule(
