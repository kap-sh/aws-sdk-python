...
class DateColumnStatisticsData(TypedDict, closed=True):
    minimum_value: NotRequired["capo_glue.types.timestamp.Timestamp"]
    """<p>The lowest value in the column.</p>"""
    maximum_value: NotRequired["capo_glue.types.timestamp.Timestamp"]
...
def serialize_aws_json_1_1(value: DateColumnStatisticsData) -> dict:
    ...
    if "minimum_value" in value:
        ...
        out["MinimumValue"] = capo_glue.types.timestamp.serialize_aws_json_1_1(
            value["minimum_value"]
        )
    if "maximum_value" in value:
        import capo_glue.types.timestamp

        out["MaximumValue"] = capo_glue.types.timestamp.serialize_aws_json_1_1(
            value["maximum_value"]
        )
...
def deserialize_aws_json_1_1(data: dict) -> DateColumnStatisticsData:
    ...
    if "MinimumValue" in data:
        ...
        out["minimum_value"] = capo_glue.types.timestamp.deserialize_aws_json_1_1(
            data["MinimumValue"]
        )
    if "MaximumValue" in data:
        import capo_glue.types.timestamp

        out["maximum_value"] = capo_glue.types.timestamp.deserialize_aws_json_1_1(
            data["MaximumValue"]
        )
