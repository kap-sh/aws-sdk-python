"""Generated from Smithy shape ``com.amazonaws.glue#CatalogDeltaSource``."""
...
class CatalogDeltaSource(TypedDict, closed=True):
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: CatalogDeltaSource) -> dict:
    out: dict = {}
    out["Name"] = value["name"]
...
def deserialize_aws_json_1_1(data: dict) -> CatalogDeltaSource:
    out: CatalogDeltaSource = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    else:
        raise DeserializationError("CatalogDeltaSource.name required")
    if "Database" in data:
        out["database"] = data["Database"]
    else:
        raise DeserializationError("CatalogDeltaSource.database required")
    if "Table" in data:
        out["table"] = data["Table"]
    else:
        raise DeserializationError("CatalogDeltaSource.table required")
