"""Generated from Smithy shape ``com.amazonaws.glue#S3DeltaCatalogTarget``."""
...
class S3DeltaCatalogTarget(TypedDict, closed=True):
    ...
    """<p>Specifies whether to automatically enable data quality evaluation for the S3 Delta catalog target. When set to <code>true</code>, data quality checks are performed automatically during the write operation.</p>"""
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: S3DeltaCatalogTarget) -> dict:
    out: dict = {}
    out["Name"] = value["name"]
    import capo_glue.types.one_input
...
def deserialize_aws_json_1_1(data: dict) -> S3DeltaCatalogTarget:
    out: S3DeltaCatalogTarget = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    else:
        raise DeserializationError("S3DeltaCatalogTarget.name required")
    if "Inputs" in data:
        import capo_glue.types.one_input
        ...
        raise DeserializationError("S3DeltaCatalogTarget.inputs required")
    ...
    if "Table" in data:
        ...
        raise DeserializationError("S3DeltaCatalogTarget.table required")
    if "Database" in data:
        out["database"] = data["Database"]
    else:
        raise DeserializationError("S3DeltaCatalogTarget.database required")
