...
RegistryListDefinition: TypeAlias = list[
    "capo_glue.types.registry_list_item.RegistryListItem"
]
...
def serialize_aws_json_1_1(value: RegistryListDefinition) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_glue.types.registry_list_item.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> RegistryListDefinition:
    ...
    for item in data:
        out.append(capo_glue.types.registry_list_item.deserialize_aws_json_1_1(item))
    return out
