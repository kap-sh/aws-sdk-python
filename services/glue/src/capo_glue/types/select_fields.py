"""Generated from Smithy shape ``com.amazonaws.glue#SelectFields``."""
...
class SelectFields(TypedDict, closed=True):
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: SelectFields) -> dict:
    out: dict = {}
    out["Name"] = value["name"]
    import capo_glue.types.one_input
...
def deserialize_aws_json_1_1(data: dict) -> SelectFields:
    out: SelectFields = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    else:
        raise DeserializationError("SelectFields.name required")
    if "Inputs" in data:
        import capo_glue.types.one_input
        ...
        raise DeserializationError("SelectFields.inputs required")
    if "Paths" in data:
        import capo_glue.types.glue_studio_path_list
        ...
        raise DeserializationError("SelectFields.paths required")
    return out
