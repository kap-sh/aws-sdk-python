...
if TYPE_CHECKING:
    import capo_glue.types.batch_stop_job_run_error

BatchStopJobRunErrorList: TypeAlias = list[
    "capo_glue.types.batch_stop_job_run_error.BatchStopJobRunError"
]
...
def serialize_aws_json_1_1(value: BatchStopJobRunErrorList) -> list:
    ...
    for item in value:
        out.append(
            capo_glue.types.batch_stop_job_run_error.serialize_aws_json_1_1(item)
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> BatchStopJobRunErrorList:
    ...
    for item in data:
        out.append(
            capo_glue.types.batch_stop_job_run_error.deserialize_aws_json_1_1(item)
        )
    return out
