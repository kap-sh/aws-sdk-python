"""Generated from Smithy shape ``com.amazonaws.glue#S3CatalogIcebergSource``."""
...
class S3CatalogIcebergSource(TypedDict, closed=True):
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: S3CatalogIcebergSource) -> dict:
    out: dict = {}
    out["Name"] = value["name"]
...
def deserialize_aws_json_1_1(data: dict) -> S3CatalogIcebergSource:
    out: S3CatalogIcebergSource = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    else:
        raise DeserializationError("S3CatalogIcebergSource.name required")
    if "Database" in data:
        out["database"] = data["Database"]
    else:
        raise DeserializationError("S3CatalogIcebergSource.database required")
    if "Table" in data:
        out["table"] = data["Table"]
    else:
        raise DeserializationError("S3CatalogIcebergSource.table required")
