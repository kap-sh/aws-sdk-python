"""Generated from Smithy shape ``com.amazonaws.glue#CatalogKafkaSource``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

from aws_sdk_glue.errors import DeserializationError

if TYPE_CHECKING:
    import aws_sdk_glue.types.boxed_boolean
    import aws_sdk_glue.types.boxed_positive_int
    import aws_sdk_glue.types.enclosed_in_string_property
    import aws_sdk_glue.types.kafka_streaming_source_options
    import aws_sdk_glue.types.node_name
    import aws_sdk_glue.types.streaming_data_preview_options


class CatalogKafkaSource(TypedDict):
    name: "aws_sdk_glue.types.node_name.NodeName"
    """<p>The name of the data store.</p>"""
    window_size: NotRequired["aws_sdk_glue.types.boxed_positive_int.BoxedPositiveInt"]
    """<p>The amount of time to spend processing each micro batch.</p>"""
    detect_schema: NotRequired["aws_sdk_glue.types.boxed_boolean.BoxedBoolean"]
    """<p>Whether to automatically determine the schema from the incoming data.</p>"""
    table: "aws_sdk_glue.types.enclosed_in_string_property.EnclosedInStringProperty"
    """<p>The name of the table in the database to read from.</p>"""
    database: "aws_sdk_glue.types.enclosed_in_string_property.EnclosedInStringProperty"
    """<p>The name of the database to read from.</p>"""
    streaming_options: NotRequired[
        "aws_sdk_glue.types.kafka_streaming_source_options.KafkaStreamingSourceOptions"
    ]
    """<p>Specifies the streaming options.</p>"""
    data_preview_options: NotRequired[
        "aws_sdk_glue.types.streaming_data_preview_options.StreamingDataPreviewOptions"
    ]
    """<p>Specifies options related to data preview for viewing a sample of your data.</p>"""


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: CatalogKafkaSource) -> dict:
    out: dict = {}
    out["Name"] = value["name"]
    if "window_size" in value:
        out["WindowSize"] = value["window_size"]
    if "detect_schema" in value:
        out["DetectSchema"] = value["detect_schema"]
    out["Table"] = value["table"]
    out["Database"] = value["database"]
    if "streaming_options" in value:
        import aws_sdk_glue.types.kafka_streaming_source_options

        out["StreamingOptions"] = (
            aws_sdk_glue.types.kafka_streaming_source_options.serialize_aws_json_1_1(
                value["streaming_options"]
            )
        )
    if "data_preview_options" in value:
        import aws_sdk_glue.types.streaming_data_preview_options

        out["DataPreviewOptions"] = (
            aws_sdk_glue.types.streaming_data_preview_options.serialize_aws_json_1_1(
                value["data_preview_options"]
            )
        )
    return out


def deserialize_aws_json_1_1(data: dict) -> CatalogKafkaSource:
    out: CatalogKafkaSource = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    else:
        raise DeserializationError("CatalogKafkaSource.name required")
    if "WindowSize" in data:
        out["window_size"] = data["WindowSize"]
    if "DetectSchema" in data:
        out["detect_schema"] = data["DetectSchema"]
    if "Table" in data:
        out["table"] = data["Table"]
    else:
        raise DeserializationError("CatalogKafkaSource.table required")
    if "Database" in data:
        out["database"] = data["Database"]
    else:
        raise DeserializationError("CatalogKafkaSource.database required")
    if "StreamingOptions" in data:
        import aws_sdk_glue.types.kafka_streaming_source_options

        out["streaming_options"] = (
            aws_sdk_glue.types.kafka_streaming_source_options.deserialize_aws_json_1_1(
                data["StreamingOptions"]
            )
        )
    if "DataPreviewOptions" in data:
        import aws_sdk_glue.types.streaming_data_preview_options

        out["data_preview_options"] = (
            aws_sdk_glue.types.streaming_data_preview_options.deserialize_aws_json_1_1(
                data["DataPreviewOptions"]
            )
        )
    return out
