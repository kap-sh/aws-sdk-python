...
if TYPE_CHECKING:
    ...
    import aws_sdk_glue.types.jwt_token
    import aws_sdk_glue.types.refresh_token
...
class OAuth2Credentials(TypedDict, closed=True):
    ...
    access_token: NotRequired["aws_sdk_glue.types.access_token.AccessToken"]
    """<p>The access token used when the authentication type is OAuth2.</p>"""
    refresh_token: NotRequired["aws_sdk_glue.types.refresh_token.RefreshToken"]
    """<p>The refresh token used when the authentication type is OAuth2.</p>"""
    jwt_token: NotRequired["aws_sdk_glue.types.jwt_token.JwtToken"]
