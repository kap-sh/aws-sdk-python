"""Generated from Smithy shape ``com.amazonaws.guardduty#GetRemainingFreeTrialDays``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.get_remaining_free_trial_days_response.GetRemainingFreeTrialDaysResponse = capo_guardduty.types.get_remaining_free_trial_days_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/freeTrial/daysRemaining"
    url = url.replace("{DetectorId}", quote(str(input_["detector_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.get_remaining_free_trial_days_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_get_remaining_free_trial_days(
