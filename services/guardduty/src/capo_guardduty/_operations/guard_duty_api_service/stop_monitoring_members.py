"""Generated from Smithy shape ``com.amazonaws.guardduty#StopMonitoringMembers``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.stop_monitoring_members_response.StopMonitoringMembersResponse = capo_guardduty.types.stop_monitoring_members_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/member/stop"
    url = url.replace("{DetectorId}", quote(str(input_["detector_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.stop_monitoring_members_request.serialize_json(input_)
    ).encode()
...
async def async_stop_monitoring_members(
