"""Generated from Smithy shape ``com.amazonaws.guardduty#GetIPSet``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.get_ip_set_response.GetIPSetResponse = (
        capo_guardduty.types.get_ip_set_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/ipset/{IpSetId}"
    url = url.replace("{DetectorId}", quote(str(input_["detector_id"]), safe=""))
    url = url.replace("{IpSetId}", quote(str(input_["ip_set_id"]), safe=""))
    params: dict[str, str] = {}
...
def get_ip_set(
    ...
    input_: capo_guardduty.types.get_ip_set_request.GetIPSetRequest,
) -> tuple[capo_guardduty.types.get_ip_set_response.GetIPSetResponse, zapros.Response]:
