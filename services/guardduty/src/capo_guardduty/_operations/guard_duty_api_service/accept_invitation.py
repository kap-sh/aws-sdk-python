"""Generated from Smithy shape ``com.amazonaws.guardduty#AcceptInvitation``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.accept_invitation_response.AcceptInvitationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/master"
    url = url.replace("{DetectorId}", quote(str(input_["detector_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.accept_invitation_request.serialize_json(input_)
    ).encode()
