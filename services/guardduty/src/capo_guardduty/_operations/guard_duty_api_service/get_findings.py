"""Generated from Smithy shape ``com.amazonaws.guardduty#GetFindings``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.get_findings_response.GetFindingsResponse = (
        capo_guardduty.types.get_findings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/findings/get"
    url = url.replace("{DetectorId}", quote(str(input_["detector_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.get_findings_request.serialize_json(input_)
    ).encode()
...
def get_findings(
    ...
    capo_guardduty.types.get_findings_response.GetFindingsResponse, zapros.Response
]:
