"""Generated from Smithy shape ``com.amazonaws.guardduty#DeclineInvitations``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.decline_invitations_response.DeclineInvitationsResponse = capo_guardduty.types.decline_invitations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/invitation/decline"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.decline_invitations_request.serialize_json(input_)
    ).encode()
