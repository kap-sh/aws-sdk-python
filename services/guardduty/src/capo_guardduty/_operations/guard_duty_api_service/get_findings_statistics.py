"""Generated from Smithy shape ``com.amazonaws.guardduty#GetFindingsStatistics``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.get_findings_statistics_response.GetFindingsStatisticsResponse = capo_guardduty.types.get_findings_statistics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detector/{DetectorId}/findings/statistics"
    url = url.replace("{DetectorId}", quote(str(input_["detector_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.get_findings_statistics_request.serialize_json(input_)
    ).encode()
