"""Generated from Smithy shape ``com.amazonaws.guardduty#DeleteInvitations``."""
...
def handle_response(
    ...
    out: capo_guardduty.types.delete_invitations_response.DeleteInvitationsResponse = (
        capo_guardduty.types.delete_invitations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/invitation/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_guardduty.types.delete_invitations_request.serialize_json(input_)
    ).encode()
