...
class UsageTopAccountsResult(TypedDict, closed=True):
    feature: NotRequired["capo_guardduty.types.usage_feature.UsageFeature"]
    ...
    accounts: NotRequired[
        "capo_guardduty.types.usage_top_accounts_by_feature_list.UsageTopAccountsByFeatureList"
    ]
...
def serialize_json(value: UsageTopAccountsResult) -> dict:
    ...
    if "feature" in value:
        ...
        out["feature"] = capo_guardduty.types.usage_feature.serialize_json(
            value["feature"]
        )
    if "accounts" in value:
        ...
        out["accounts"] = (
            capo_guardduty.types.usage_top_accounts_by_feature_list.serialize_json(
                value["accounts"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> UsageTopAccountsResult:
    ...
    if "accounts" in data:
        ...
        out["accounts"] = (
            capo_guardduty.types.usage_top_accounts_by_feature_list.deserialize_json(
                data["accounts"]
            )
        )
    return out
