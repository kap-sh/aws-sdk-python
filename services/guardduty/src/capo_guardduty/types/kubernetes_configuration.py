...
class KubernetesConfiguration(TypedDict, closed=True):
    audit_logs: NotRequired[
        "capo_guardduty.types.kubernetes_audit_logs_configuration.KubernetesAuditLogsConfiguration"
    ]
...
def serialize_json(value: KubernetesConfiguration) -> dict:
    ...
    if "audit_logs" in value:
        ...
        out["auditLogs"] = (
            capo_guardduty.types.kubernetes_audit_logs_configuration.serialize_json(
                value["audit_logs"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> KubernetesConfiguration:
    ...
    if "auditLogs" in data:
        ...
        out["audit_logs"] = (
            capo_guardduty.types.kubernetes_audit_logs_configuration.deserialize_json(
                data["auditLogs"]
            )
        )
    return out
