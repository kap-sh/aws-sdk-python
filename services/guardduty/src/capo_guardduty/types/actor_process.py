...
if TYPE_CHECKING:
    import capo_guardduty.types.process_name
    import capo_guardduty.types.process_path
    import capo_guardduty.types.process_sha256


class ActorProcess(TypedDict, closed=True):
    name: NotRequired["capo_guardduty.types.process_name.ProcessName"]
    """<p>The name of the process as it appears in the system.</p>"""
    path: NotRequired["capo_guardduty.types.process_path.ProcessPath"]
    """<p>The full file path to the process executable on the system.</p>"""
    sha256: NotRequired["capo_guardduty.types.process_sha256.ProcessSha256"]
