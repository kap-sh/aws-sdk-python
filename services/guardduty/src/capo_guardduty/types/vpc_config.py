...
if TYPE_CHECKING:
    ...
    import capo_guardduty.types.subnet_ids


class VpcConfig(TypedDict, closed=True):
    subnet_ids: NotRequired["capo_guardduty.types.subnet_ids.SubnetIds"]
    """<p>The identifiers of the subnets that are associated with your Lambda function.</p>"""
    vpc_id: NotRequired["capo_guardduty.types.string.String"]
    """<p>The identifier of the Amazon Virtual Private Cloud.</p>"""
    security_groups: NotRequired["capo_guardduty.types.security_groups.SecurityGroups"]
...
def serialize_json(value: VpcConfig) -> dict:
    ...
    if "subnet_ids" in value:
        ...
        out["subnetIds"] = capo_guardduty.types.subnet_ids.serialize_json(
            value["subnet_ids"]
        )
    if "vpc_id" in value:
    ...
    if "security_groups" in value:
        ...
        out["securityGroups"] = capo_guardduty.types.security_groups.serialize_json(
...
def deserialize_json(data: dict) -> VpcConfig:
    ...
    if "subnetIds" in data:
        ...
        out["subnet_ids"] = capo_guardduty.types.subnet_ids.deserialize_json(
            data["subnetIds"]
        )
    if "vpcId" in data:
