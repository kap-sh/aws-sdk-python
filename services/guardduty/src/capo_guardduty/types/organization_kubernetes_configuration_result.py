...
if TYPE_CHECKING:
    import capo_guardduty.types.organization_kubernetes_audit_logs_configuration_result


class OrganizationKubernetesConfigurationResult(TypedDict, closed=True):
    audit_logs: NotRequired[
        "capo_guardduty.types.organization_kubernetes_audit_logs_configuration_result.OrganizationKubernetesAuditLogsConfigurationResult"
    ]
...
def serialize_json(value: OrganizationKubernetesConfigurationResult) -> dict:
    ...
    if "audit_logs" in value:
        ...
        out["auditLogs"] = (
            capo_guardduty.types.organization_kubernetes_audit_logs_configuration_result.serialize_json(
                value["audit_logs"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> OrganizationKubernetesConfigurationResult:
    ...
    if "auditLogs" in data:
        ...
        out["audit_logs"] = (
            capo_guardduty.types.organization_kubernetes_audit_logs_configuration_result.deserialize_json(
                data["auditLogs"]
            )
        )
    return out
