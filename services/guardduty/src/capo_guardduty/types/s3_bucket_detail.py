...
class S3BucketDetail(TypedDict, closed=True):
    ...
    owner: NotRequired["capo_guardduty.types.owner.Owner"]
    ...
    default_server_side_encryption: NotRequired[
        "capo_guardduty.types.default_server_side_encryption.DefaultServerSideEncryption"
    ]
    """<p>Describes the server side encryption method used in the S3 bucket.</p>"""
    public_access: NotRequired["capo_guardduty.types.public_access.PublicAccess"]
    """<p>Describes the public access policies that apply to the S3 bucket.</p>"""
    s3_object_details: NotRequired[
        "capo_guardduty.types.s3_object_details.S3ObjectDetails"
    ]
...
def serialize_json(value: S3BucketDetail) -> dict:
    ...
    if "owner" in value:
        ...
        out["owner"] = capo_guardduty.types.owner.serialize_json(value["owner"])
    if "tags" in value:
        import capo_guardduty.types.tags
    ...
    if "default_server_side_encryption" in value:
        ...
        out["defaultServerSideEncryption"] = (
            capo_guardduty.types.default_server_side_encryption.serialize_json(
    ...
    if "public_access" in value:
        ...
        out["publicAccess"] = capo_guardduty.types.public_access.serialize_json(
            value["public_access"]
        )
    if "s3_object_details" in value:
        import capo_guardduty.types.s3_object_details

        out["s3ObjectDetails"] = capo_guardduty.types.s3_object_details.serialize_json(
...
def deserialize_json(data: dict) -> S3BucketDetail:
    ...
    if "defaultServerSideEncryption" in data:
        ...
        out["default_server_side_encryption"] = (
            capo_guardduty.types.default_server_side_encryption.deserialize_json(
    ...
    if "s3ObjectDetails" in data:
        ...
        out["s3_object_details"] = (
            capo_guardduty.types.s3_object_details.deserialize_json(
