...
class ListFiltersResponse(TypedDict, closed=True):
    filter_names: NotRequired["capo_guardduty.types.filter_names.FilterNames"]
...
def serialize_json(value: ListFiltersResponse) -> dict:
    ...
    if "filter_names" in value:
        ...
        out["filterNames"] = capo_guardduty.types.filter_names.serialize_json(
            value["filter_names"]
        )
...
def deserialize_json(data: dict) -> ListFiltersResponse:
    ...
    if "filterNames" in data:
        ...
        out["filter_names"] = capo_guardduty.types.filter_names.deserialize_json(
            data["filterNames"]
        )
