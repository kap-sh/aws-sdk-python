"""Generated from Smithy shape ``com.amazonaws.guardduty#GetThreatEntitySetResponse``."""
...
class GetThreatEntitySetResponse(TypedDict, closed=True):
    ...
    status: NotRequired[
        "capo_guardduty.types.threat_entity_set_status.ThreatEntitySetStatus"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: GetThreatEntitySetResponse) -> dict:
    out: dict = {}
    if "name" in value:
        out["name"] = value["name"]
    if "format" in value:
    ...
    if "status" in value:
        ...
        out["status"] = capo_guardduty.types.threat_entity_set_status.serialize_json(
            value["status"]
        )
    if "tags" in value:
        import capo_guardduty.types.tag_map
