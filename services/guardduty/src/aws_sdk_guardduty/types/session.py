...
if TYPE_CHECKING:
    import aws_sdk_guardduty.types.mfa_status
...
class Session(TypedDict, closed=True):
    ...
    mfa_status: NotRequired["aws_sdk_guardduty.types.mfa_status.MfaStatus"]
    ...
    created_time: NotRequired["aws_sdk_guardduty.types.timestamp.Timestamp"]
    ...
    issuer: NotRequired["aws_sdk_guardduty.types.string.String"]
...
def serialize_json(value: Session) -> dict:
    ...
    if "mfa_status" in value:
        ...
        out["mfaStatus"] = aws_sdk_guardduty.types.mfa_status.serialize_json(
            value["mfa_status"]
        )
    if "created_time" in value:
        import aws_sdk_guardduty.types.timestamp

        out["createdTime"] = aws_sdk_guardduty.types.timestamp.serialize_json(
            value["created_time"]
        )
    if "issuer" in value:
...
def deserialize_json(data: dict) -> Session:
    ...
    if "mfaStatus" in data:
        ...
        out["mfa_status"] = aws_sdk_guardduty.types.mfa_status.deserialize_json(
            data["mfaStatus"]
        )
    if "createdTime" in data:
        import aws_sdk_guardduty.types.timestamp

        out["created_time"] = aws_sdk_guardduty.types.timestamp.deserialize_json(
            data["createdTime"]
        )
    if "issuer" in data:
