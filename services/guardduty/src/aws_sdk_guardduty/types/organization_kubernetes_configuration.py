...
if TYPE_CHECKING:
    import aws_sdk_guardduty.types.organization_kubernetes_audit_logs_configuration


class OrganizationKubernetesConfiguration(TypedDict):
    audit_logs: NotRequired[
        "aws_sdk_guardduty.types.organization_kubernetes_audit_logs_configuration.OrganizationKubernetesAuditLogsConfiguration"
    ]
...
def serialize_json(value: OrganizationKubernetesConfiguration) -> dict:
    ...
    if "audit_logs" in value:
        ...
        out["auditLogs"] = (
            aws_sdk_guardduty.types.organization_kubernetes_audit_logs_configuration.serialize_json(
                value["audit_logs"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> OrganizationKubernetesConfiguration:
    ...
    if "auditLogs" in data:
        ...
        out["audit_logs"] = (
            aws_sdk_guardduty.types.organization_kubernetes_audit_logs_configuration.deserialize_json(
                data["auditLogs"]
            )
        )
    return out
