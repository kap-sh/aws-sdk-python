...
class TriggerDetails(TypedDict, closed=True):
    ...
    description: NotRequired["aws_sdk_guardduty.types.non_empty_string.NonEmptyString"]
    """<p>The description of the scan trigger.</p>"""
    trigger_type: NotRequired["aws_sdk_guardduty.types.trigger_type.TriggerType"]
...
def serialize_json(value: TriggerDetails) -> dict:
    ...
    if "trigger_type" in value:
        ...
        out["triggerType"] = aws_sdk_guardduty.types.trigger_type.serialize_json(
            value["trigger_type"]
        )
    return out
...
def deserialize_json(data: dict) -> TriggerDetails:
    ...
    if "triggerType" in data:
        ...
        out["trigger_type"] = aws_sdk_guardduty.types.trigger_type.deserialize_json(
            data["triggerType"]
        )
    return out
