...
class GetThreatEntitySetResponse(TypedDict):
    ...
    format: NotRequired[
        "aws_sdk_guardduty.types.threat_entity_set_format.ThreatEntitySetFormat"
    ]
    ...
    status: NotRequired[
        "aws_sdk_guardduty.types.threat_entity_set_status.ThreatEntitySetStatus"
    ]
...
def serialize_json(value: GetThreatEntitySetResponse) -> dict:
    ...
    if "format" in value:
        ...
        out["format"] = aws_sdk_guardduty.types.threat_entity_set_format.serialize_json(
            value["format"]
        )
    ...
    if "status" in value:
        ...
        out["status"] = aws_sdk_guardduty.types.threat_entity_set_status.serialize_json(
            value["status"]
        )
    if "tags" in value:
        import aws_sdk_guardduty.types.tag_map
...
def deserialize_json(data: dict) -> GetThreatEntitySetResponse:
    ...
    if "format" in data:
        ...
        out["format"] = (
            aws_sdk_guardduty.types.threat_entity_set_format.deserialize_json(
                data["format"]
            )
        )
    if "location" in data:
    ...
    if "status" in data:
        ...
        out["status"] = (
            aws_sdk_guardduty.types.threat_entity_set_status.deserialize_json(
                data["status"]
            )
        )
    if "tags" in data:
