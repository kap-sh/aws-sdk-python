...
if TYPE_CHECKING:
    import aws_sdk_guardduty.types.account
...
class User(TypedDict):
    ...
    credential_uid: NotRequired["aws_sdk_guardduty.types.string.String"]
    """<p>The credentials of the user ID.</p>"""
    account: NotRequired["aws_sdk_guardduty.types.account.Account"]
...
def serialize_json(value: User) -> dict:
    ...
    if "account" in value:
        ...
        out["account"] = aws_sdk_guardduty.types.account.serialize_json(
            value["account"]
        )
    return out
...
def deserialize_json(data: dict) -> User:
    ...
    if "account" in data:
        ...
        out["account"] = aws_sdk_guardduty.types.account.deserialize_json(
            data["account"]
        )
    return out
