...
if TYPE_CHECKING:
    import aws_sdk_guardduty.types.detection_source
...
class ScanResultThreat(TypedDict):
    name: NotRequired["aws_sdk_guardduty.types.non_empty_string.NonEmptyString"]
    """<p>The name of the detected threat.</p>"""
    source: NotRequired["aws_sdk_guardduty.types.detection_source.DetectionSource"]
    """<p>The source that detected this threat.</p>"""
    count: NotRequired["aws_sdk_guardduty.types.positive_long.PositiveLong"]
    """<p>The number of instances of this threat that were detected.</p>"""
    hash: NotRequired["aws_sdk_guardduty.types.non_empty_string.NonEmptyString"]
...
def serialize_json(value: ScanResultThreat) -> dict:
    ...
    if "source" in value:
        ...
        out["source"] = aws_sdk_guardduty.types.detection_source.serialize_json(
            value["source"]
        )
    if "count" in value:
...
def deserialize_json(data: dict) -> ScanResultThreat:
    ...
    if "source" in data:
        ...
        out["source"] = aws_sdk_guardduty.types.detection_source.deserialize_json(
            data["source"]
        )
    if "count" in data:
