...
if TYPE_CHECKING:
    import capo_sagemaker.types.initial_number_of_users
    import capo_sagemaker.types.spawn_rate
    import capo_sagemaker.types.traffic_duration_in_seconds


class Phase(TypedDict, closed=True):
    initial_number_of_users: NotRequired[
        "capo_sagemaker.types.initial_number_of_users.InitialNumberOfUsers"
    ]
    """<p>Specifies how many concurrent users to start with. The value should be between 1 and 3.</p>"""
    spawn_rate: NotRequired["capo_sagemaker.types.spawn_rate.SpawnRate"]
    """<p>Specified how many new users to spawn in a minute.</p>"""
    duration_in_seconds: NotRequired[
        "capo_sagemaker.types.traffic_duration_in_seconds.TrafficDurationInSeconds"
    ]
