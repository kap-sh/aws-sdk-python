...
if TYPE_CHECKING:
    import capo_sagemaker.types.auto_ml_channel

AutoMLInputDataConfig: TypeAlias = list[
    "capo_sagemaker.types.auto_ml_channel.AutoMLChannel"
]
...
def serialize_aws_json_1_1(value: AutoMLInputDataConfig) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_sagemaker.types.auto_ml_channel.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> AutoMLInputDataConfig:
    ...
    for item in data:
        out.append(capo_sagemaker.types.auto_ml_channel.deserialize_aws_json_1_1(item))
    return out
