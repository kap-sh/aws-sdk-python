...
if TYPE_CHECKING:
    ...
    import capo_sagemaker.types.ttl_duration_value


class TtlDuration(TypedDict, closed=True):
    unit: NotRequired["capo_sagemaker.types.ttl_duration_unit.TtlDurationUnit"]
    """<p> <code>TtlDuration</code> time unit.</p>"""
    value: NotRequired["capo_sagemaker.types.ttl_duration_value.TtlDurationValue"]
...
def serialize_aws_json_1_1(value: TtlDuration) -> dict:
    ...
    if "unit" in value:
        ...
        out["Unit"] = capo_sagemaker.types.ttl_duration_unit.serialize_aws_json_1_1(
            value["unit"]
        )
    if "value" in value:
