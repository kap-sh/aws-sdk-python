...
if TYPE_CHECKING:
    import capo_sagemaker.types.availability_zone_id
    import capo_sagemaker.types.currency_code
    import capo_sagemaker.types.string256
    import capo_sagemaker.types.timestamp
    import capo_sagemaker.types.training_plan_extension_duration_hours
    import capo_sagemaker.types.training_plan_extension_offering_id


class TrainingPlanExtension(TypedDict, closed=True):
    ...
    extended_at: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>The timestamp when the extension was created.</p>"""
    start_date: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>The start date of the extension period.</p>"""
    end_date: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>The end date of the extension period.</p>"""
    status: NotRequired["capo_sagemaker.types.string256.String256"]
    """<p>The current status of the extension (e.g., Pending, Active, Scheduled, Failed, Expired).</p>"""
    payment_status: NotRequired["capo_sagemaker.types.string256.String256"]
    """<p>The payment processing status of the extension.</p>"""
    availability_zone: NotRequired["capo_sagemaker.types.string256.String256"]
    """<p>The Availability Zone of the extension.</p>"""
    availability_zone_id: NotRequired[
        "capo_sagemaker.types.availability_zone_id.AvailabilityZoneId"
    ]
    """<p>The Availability Zone ID of the extension.</p>"""
    duration_hours: NotRequired[
        "capo_sagemaker.types.training_plan_extension_duration_hours.TrainingPlanExtensionDurationHours"
    ]
...
def serialize_aws_json_1_1(value: TrainingPlanExtension) -> dict:
    ...
    if "extended_at" in value:
        ...
        out["ExtendedAt"] = capo_sagemaker.types.timestamp.serialize_aws_json_1_1(
            value["extended_at"]
        )
    if "start_date" in value:
        import capo_sagemaker.types.timestamp

        out["StartDate"] = capo_sagemaker.types.timestamp.serialize_aws_json_1_1(
            value["start_date"]
        )
    if "end_date" in value:
        import capo_sagemaker.types.timestamp

        out["EndDate"] = capo_sagemaker.types.timestamp.serialize_aws_json_1_1(
            value["end_date"]
        )
    if "status" in value:
