...
if TYPE_CHECKING:
    import capo_sagemaker.types.cluster_instance_group_details

ClusterInstanceGroupDetailsList: TypeAlias = list[
    "capo_sagemaker.types.cluster_instance_group_details.ClusterInstanceGroupDetails"
]
...
def serialize_aws_json_1_1(value: ClusterInstanceGroupDetailsList) -> list:
    ...
    for item in value:
        out.append(
            capo_sagemaker.types.cluster_instance_group_details.serialize_aws_json_1_1(
                item
            )
        )
    return out
...
def deserialize_aws_json_1_1(data: list) -> ClusterInstanceGroupDetailsList:
    ...
    for item in data:
        out.append(
            capo_sagemaker.types.cluster_instance_group_details.deserialize_aws_json_1_1(
                item
            )
        )
    return out
