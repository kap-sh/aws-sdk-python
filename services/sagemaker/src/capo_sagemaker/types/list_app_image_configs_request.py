...
class ListAppImageConfigsRequest(TypedDict, closed=True):
    ...
    modified_time_before: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>A filter that returns only AppImageConfigs modified on or before the specified time.</p>"""
    modified_time_after: NotRequired["capo_sagemaker.types.timestamp.Timestamp"]
    """<p>A filter that returns only AppImageConfigs modified on or after the specified time.</p>"""
    sort_by: NotRequired[
        "capo_sagemaker.types.app_image_config_sort_key.AppImageConfigSortKey"
    ]
...
def serialize_aws_json_1_1(value: ListAppImageConfigsRequest) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["SortBy"] = (
            capo_sagemaker.types.app_image_config_sort_key.serialize_aws_json_1_1(
                value["sort_by"]
            )
        )
...
def deserialize_aws_json_1_1(data: dict) -> ListAppImageConfigsRequest:
    ...
    if "SortBy" in data:
        ...
        out["sort_by"] = (
            capo_sagemaker.types.app_image_config_sort_key.deserialize_aws_json_1_1(
                data["SortBy"]
            )
        )
