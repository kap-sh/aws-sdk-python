...
if TYPE_CHECKING:
    import capo_sagemaker.types.bucket_name
    import capo_sagemaker.types.key
    import capo_sagemaker.types.version_id


class PipelineDefinitionS3Location(TypedDict, closed=True):
    bucket: NotRequired["capo_sagemaker.types.bucket_name.BucketName"]
    """<p>Name of the S3 bucket.</p>"""
    object_key: NotRequired["capo_sagemaker.types.key.Key"]
    """<p>The object key (or key name) uniquely identifies the object in an S3 bucket. </p>"""
    version_id: NotRequired["capo_sagemaker.types.version_id.VersionId"]
