...
if TYPE_CHECKING:
    ...
    import capo_sagemaker.types.stage_description


class ModelLifeCycle(TypedDict, closed=True):
    stage: NotRequired["capo_sagemaker.types.entity_name.EntityName"]
    """<p> The current stage in the model life cycle. </p>"""
    stage_status: NotRequired["capo_sagemaker.types.entity_name.EntityName"]
    """<p> The current status of a stage in model life cycle. </p>"""
    stage_description: NotRequired[
        "capo_sagemaker.types.stage_description.StageDescription"
    ]
