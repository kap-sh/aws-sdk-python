...
if TYPE_CHECKING:
    import capo_sagemaker.types.batch_replace_cluster_nodes_error_code
...
class BatchReplaceClusterNodeLogicalIdsError(TypedDict, closed=True):
    ...
    error_code: NotRequired[
        "capo_sagemaker.types.batch_replace_cluster_nodes_error_code.BatchReplaceClusterNodesErrorCode"
    ]
...
def serialize_aws_json_1_1(value: BatchReplaceClusterNodeLogicalIdsError) -> dict:
    ...
    if "error_code" in value:
        ...
        out["ErrorCode"] = (
            capo_sagemaker.types.batch_replace_cluster_nodes_error_code.serialize_aws_json_1_1(
                value["error_code"]
            )
        )
    if "message" in value:
...
def deserialize_aws_json_1_1(data: dict) -> BatchReplaceClusterNodeLogicalIdsError:
    ...
    if "ErrorCode" in data:
        ...
        out["error_code"] = (
            capo_sagemaker.types.batch_replace_cluster_nodes_error_code.deserialize_aws_json_1_1(
                data["ErrorCode"]
            )
        )
    if "Message" in data:
