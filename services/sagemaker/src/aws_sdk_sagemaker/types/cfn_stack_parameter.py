"""Generated from Smithy shape ``com.amazonaws.sagemaker#CfnStackParameter``."""
...
class CfnStackParameter(TypedDict, closed=True):
    key: NotRequired[
...
# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: CfnStackParameter) -> dict:
    out: dict = {}
    if "key" in value:
        out["Key"] = value["key"]
    if "value" in value:
...
def deserialize_aws_json_1_1(data: dict) -> CfnStackParameter:
    out: CfnStackParameter = {}  # type: ignore[typeddict-item]
    if "Key" in data:
        out["key"] = data["Key"]
    if "Value" in data:
