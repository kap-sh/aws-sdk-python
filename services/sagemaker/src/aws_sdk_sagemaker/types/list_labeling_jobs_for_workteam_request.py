...
class ListLabelingJobsForWorkteamRequest(TypedDict, closed=True):
    ...
    job_reference_code_contains: NotRequired[
        "aws_sdk_sagemaker.types.job_reference_code_contains.JobReferenceCodeContains"
    ]
    """<p>A filter the limits jobs to only the ones whose job reference code contains the specified string.</p>"""
    sort_by: NotRequired[
        "aws_sdk_sagemaker.types.list_labeling_jobs_for_workteam_sort_by_options.ListLabelingJobsForWorkteamSortByOptions"
    ]
...
def serialize_aws_json_1_1(value: ListLabelingJobsForWorkteamRequest) -> dict:
    ...
    if "sort_by" in value:
        ...
        out["SortBy"] = (
            aws_sdk_sagemaker.types.list_labeling_jobs_for_workteam_sort_by_options.serialize_aws_json_1_1(
                value["sort_by"]
            )
        )
...
def deserialize_aws_json_1_1(data: dict) -> ListLabelingJobsForWorkteamRequest:
    ...
    if "SortBy" in data:
        ...
        out["sort_by"] = (
            aws_sdk_sagemaker.types.list_labeling_jobs_for_workteam_sort_by_options.deserialize_aws_json_1_1(
                data["SortBy"]
            )
        )
