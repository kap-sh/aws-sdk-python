...
if TYPE_CHECKING:
    import aws_sdk_amplifyuibuilder.types.component_summary_list


class ListComponentsResponse(TypedDict, closed=True):
    entities: (
        "aws_sdk_amplifyuibuilder.types.component_summary_list.ComponentSummaryList"
    )
...
def serialize_json(value: ListComponentsResponse) -> dict:
    ...
    out["entities"] = (
        aws_sdk_amplifyuibuilder.types.component_summary_list.serialize_json(
            value["entities"]
        )
    )
...
def deserialize_json(data: dict) -> ListComponentsResponse:
    ...
    if "entities" in data:
        ...
        out["entities"] = (
            aws_sdk_amplifyuibuilder.types.component_summary_list.deserialize_json(
                data["entities"]
            )
        )
    else:
