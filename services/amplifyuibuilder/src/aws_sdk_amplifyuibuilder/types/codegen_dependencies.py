...
CodegenDependencies: TypeAlias = list[
    "aws_sdk_amplifyuibuilder.types.codegen_dependency.CodegenDependency"
]
...
def serialize_json(value: CodegenDependencies) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_amplifyuibuilder.types.codegen_dependency.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> CodegenDependencies:
    ...
    for item in data:
        out.append(
            aws_sdk_amplifyuibuilder.types.codegen_dependency.deserialize_json(item)
        )
    return out
