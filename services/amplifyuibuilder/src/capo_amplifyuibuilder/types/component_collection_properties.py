...
if TYPE_CHECKING:
    import capo_amplifyuibuilder.types.component_data_configuration

ComponentCollectionProperties: TypeAlias = dict[
    "str",
    "capo_amplifyuibuilder.types.component_data_configuration.ComponentDataConfiguration",
]
...
def serialize_json(input_to_serialize: ComponentCollectionProperties) -> dict:
    ...
    for key, value in input_to_serialize.items():
        ...
        out[key] = (
            capo_amplifyuibuilder.types.component_data_configuration.serialize_json(
                value
            )
        )
    return out
...
def deserialize_json(data: dict) -> ComponentCollectionProperties:
    ...
    for key, value in data.items():
        ...
        out[key] = (
            capo_amplifyuibuilder.types.component_data_configuration.deserialize_json(
                value
            )
        )
    return out
