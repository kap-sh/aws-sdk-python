...
class FormStyle(TypedDict, closed=True):
    horizontal_gap: NotRequired[
        "capo_amplifyuibuilder.types.form_style_config.FormStyleConfig"
    ]
...
def serialize_json(value: FormStyle) -> dict:
    ...
    if "horizontal_gap" in value:
        ...
        out["horizontalGap"] = (
            capo_amplifyuibuilder.types.form_style_config.serialize_json(
...
def deserialize_json(data: dict) -> FormStyle:
    ...
    if "horizontalGap" in data:
        ...
        out["horizontal_gap"] = (
            capo_amplifyuibuilder.types.form_style_config.deserialize_json(
                data["horizontalGap"]
            )
        )
