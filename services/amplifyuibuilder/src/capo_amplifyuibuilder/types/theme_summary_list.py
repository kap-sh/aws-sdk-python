...
ThemeSummaryList: TypeAlias = list[
    "capo_amplifyuibuilder.types.theme_summary.ThemeSummary"
]
...
def serialize_json(value: ThemeSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_amplifyuibuilder.types.theme_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ThemeSummaryList:
    ...
    for item in data:
        out.append(capo_amplifyuibuilder.types.theme_summary.deserialize_json(item))
    return out
