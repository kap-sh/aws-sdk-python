...
class ComponentBindingPropertiesValueProperties(TypedDict, closed=True):
    model: NotRequired["str"]
    """<p>An Amplify DataStore model.</p>"""
    field: NotRequired["str"]
    """<p>The field to bind the data to.</p>"""
    predicates: NotRequired["capo_amplifyuibuilder.types.predicate_list.PredicateList"]
...
def serialize_json(value: ComponentBindingPropertiesValueProperties) -> dict:
    ...
    if "predicates" in value:
        ...
        out["predicates"] = capo_amplifyuibuilder.types.predicate_list.serialize_json(
            value["predicates"]
        )
