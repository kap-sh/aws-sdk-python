"""Generated from Smithy shape ``com.amazonaws.groundstation#CreateDataflowEndpointGroupV2``."""
...
import capo_groundstation.errors.service_quota_exceeded_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            raise capo_groundstation.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_groundstation.types.create_dataflow_endpoint_group_v2_response.CreateDataflowEndpointGroupV2Response = capo_groundstation.types.create_dataflow_endpoint_group_v2_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/dataflowEndpointGroupV2"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_groundstation.types.create_dataflow_endpoint_group_v2_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_dataflow_endpoint_group_v2(
