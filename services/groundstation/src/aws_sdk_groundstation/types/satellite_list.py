"""Generated from Smithy shape ``com.amazonaws.groundstation#SatelliteList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_groundstation.types.satellite_list_item

SatelliteList: TypeAlias = list[
    "aws_sdk_groundstation.types.satellite_list_item.SatelliteListItem"
]


# --- restJson1 ser/de ---
def serialize_json(value: SatelliteList) -> list:
    import aws_sdk_groundstation.types.satellite_list_item

    out: list = []
    for item in value:
        out.append(aws_sdk_groundstation.types.satellite_list_item.serialize_json(item))
    return out


def deserialize_json(data: list) -> SatelliteList:
    import aws_sdk_groundstation.types.satellite_list_item

    out: SatelliteList = []
    for item in data:
        out.append(
            aws_sdk_groundstation.types.satellite_list_item.deserialize_json(item)
        )
    return out
