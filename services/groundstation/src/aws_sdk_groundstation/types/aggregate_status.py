...
class AggregateStatus(TypedDict):
...
def serialize_json(value: AggregateStatus) -> dict:
    out: dict = {}
    import aws_sdk_groundstation.types.agent_status
    out["status"] = aws_sdk_groundstation.types.agent_status.serialize_json(value["status"])
    if "signature_map" in value:
        import aws_sdk_groundstation.types.signature_map
        out["signatureMap"] = aws_sdk_groundstation.types.signature_map.serialize_json(value["signature_map"])
    return out
...
def deserialize_json(data: dict) -> AggregateStatus:
    ...
    if "status" in data:
        import aws_sdk_groundstation.types.agent_status
        out["status"] = aws_sdk_groundstation.types.agent_status.deserialize_json(data["status"])
    else:
        raise DeserializationError("AggregateStatus.status required")
    if "signatureMap" in data:
        import aws_sdk_groundstation.types.signature_map
        out["signature_map"] = aws_sdk_groundstation.types.signature_map.deserialize_json(data["signatureMap"])
    return out