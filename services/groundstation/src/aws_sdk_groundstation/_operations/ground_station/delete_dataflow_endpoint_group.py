"""Generated from Smithy shape ``com.amazonaws.groundstation#DeleteDataflowEndpointGroup``."""
...
def build_request(
    ...
    input_: aws_sdk_groundstation.types.delete_dataflow_endpoint_group_request.DeleteDataflowEndpointGroupRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/dataflowEndpointGroup/{dataflowEndpointGroupId}"
    url = url.replace(
        "{dataflowEndpointGroupId}",
        quote(str(input_["dataflow_endpoint_group_id"]), safe=""),
    )
    params: dict[str, str] = {}
...
async def async_delete_dataflow_endpoint_group(
