"""Generated from Smithy shape ``com.amazonaws.groundstation#CreateEphemeris``."""
...
def build_request(
    ...
    input_: aws_sdk_groundstation.types.create_ephemeris_request.CreateEphemerisRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/ephemeris"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_groundstation.types.create_ephemeris_request

    body: bytes | None = json.dumps(
        aws_sdk_groundstation.types.create_ephemeris_request.serialize_json(input_)
    ).encode()
...
async def async_create_ephemeris(
