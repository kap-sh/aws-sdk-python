...
def build_request(
    ...
    input_: aws_sdk_groundstation.types.update_mission_profile_request.UpdateMissionProfileRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{missionProfileId}", quote(str(input_["mission_profile_id"]), safe="")
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_groundstation.types.update_mission_profile_request.serialize_json(
            input_
        )
    ).encode()
