...
if TYPE_CHECKING:
    import aws_sdk_qapps.types.submission_mutation_kind
    import aws_sdk_qapps.types.uuid


class SubmissionMutation(TypedDict):
    submission_id: "aws_sdk_qapps.types.uuid.UUID"
    """<p>The unique identifier of the submission.</p>"""
    mutation_type: "aws_sdk_qapps.types.submission_mutation_kind.SubmissionMutationKind"
...
def serialize_json(value: SubmissionMutation) -> dict:
    ...
    out["mutationType"] = aws_sdk_qapps.types.submission_mutation_kind.serialize_json(
        value["mutation_type"]
    )
    return out
...
def deserialize_json(data: dict) -> SubmissionMutation:
    ...
    if "mutationType" in data:
        ...
        out["mutation_type"] = (
            aws_sdk_qapps.types.submission_mutation_kind.deserialize_json(
                data["mutationType"]
            )
        )
    else:
