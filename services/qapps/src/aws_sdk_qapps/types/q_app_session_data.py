...
if TYPE_CHECKING:
    ...
    import aws_sdk_qapps.types.user
    import aws_sdk_qapps.types.uuid


class QAppSessionData(TypedDict):
    ...
    user: "aws_sdk_qapps.types.user.User"
    ...
    timestamp: NotRequired["aws_sdk_qapps.types.q_apps_timestamp.QAppsTimestamp"]
...
def serialize_json(value: QAppSessionData) -> dict:
    ...
    out["user"] = aws_sdk_qapps.types.user.serialize_json(value["user"])
    if "submission_id" in value:
        out["submissionId"] = value["submission_id"]
    if "timestamp" in value:
        import aws_sdk_qapps.types.q_apps_timestamp

        out["timestamp"] = aws_sdk_qapps.types.q_apps_timestamp.serialize_json(
            value["timestamp"]
        )
    return out
...
def deserialize_json(data: dict) -> QAppSessionData:
    ...
    if "user" in data:
        ...
        out["user"] = aws_sdk_qapps.types.user.deserialize_json(data["user"])
    else:
    ...
    if "timestamp" in data:
        ...
        out["timestamp"] = aws_sdk_qapps.types.q_apps_timestamp.deserialize_json(
            data["timestamp"]
        )
    return out
