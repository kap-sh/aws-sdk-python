"""Generated from Smithy shape ``com.amazonaws.qapps#GetQApp``."""
...
def handle_response(
    ...
    out: aws_sdk_qapps.types.get_q_app_output.GetQAppOutput = (
        aws_sdk_qapps.types.get_q_app_output.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_qapps.types.get_q_app_input.GetQAppInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/apps.get"
    params: dict[str, str] = {}
    if "app_id" in input:
        params["appId"] = str(input["app_id"])
    if "app_version" in input:
        params["appVersion"] = str(input["app_version"])
...
def get_q_app(
    options: OperationOptions, input: aws_sdk_qapps.types.get_q_app_input.GetQAppInput
) -> tuple[aws_sdk_qapps.types.get_q_app_output.GetQAppOutput, zapros.Response]:
