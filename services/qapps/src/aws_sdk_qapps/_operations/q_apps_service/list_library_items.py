"""Generated from Smithy shape ``com.amazonaws.qapps#ListLibraryItems``."""
...
def handle_response(
    ...
    out: aws_sdk_qapps.types.list_library_items_output.ListLibraryItemsOutput = (
        aws_sdk_qapps.types.list_library_items_output.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_qapps.types.list_library_items_input.ListLibraryItemsInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/catalog.list"
    params: dict[str, str] = {}
    if "limit" in input:
    ...
    if "category_id" in input:
        params["categoryId"] = str(input["category_id"])
...
async def async_list_library_items(
