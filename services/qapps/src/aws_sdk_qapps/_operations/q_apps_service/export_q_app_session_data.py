"""Generated from Smithy shape ``com.amazonaws.qapps#ExportQAppSessionData``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            import aws_sdk_qapps.errors.conflict_exception

            raise aws_sdk_qapps.errors.conflict_exception.ConflictException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_qapps.types.export_q_app_session_data_output.ExportQAppSessionDataOutput = aws_sdk_qapps.types.export_q_app_session_data_output.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_qapps.types.export_q_app_session_data_input.ExportQAppSessionDataInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/runtime.exportQAppSessionData"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "instance_id" in input:
        headers["instance-id"] = str(input["instance_id"])
    import aws_sdk_qapps.types.export_q_app_session_data_input

    body: bytes | None = json.dumps(
        aws_sdk_qapps.types.export_q_app_session_data_input.serialize_json(input)
    ).encode()
...
async def async_export_q_app_session_data(
