"""Generated from Smithy shape ``com.amazonaws.qapps#StopQAppSession``."""
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/runtime.deleteMiniAppRun"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "instance_id" in input_:
        headers["instance-id"] = str(input_["instance_id"])
    body: bytes | None = json.dumps(
        capo_qapps.types.stop_q_app_session_input.serialize_json(input_)
    ).encode()
...
async def async_stop_q_app_session(
