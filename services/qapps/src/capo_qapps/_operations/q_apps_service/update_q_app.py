"""Generated from Smithy shape ``com.amazonaws.qapps#UpdateQApp``."""
...
import capo_qapps.errors.content_too_large_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ContentTooLargeException":
            raise capo_qapps.errors.content_too_large_exception.ContentTooLargeException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_qapps.types.update_q_app_output.UpdateQAppOutput = (
        capo_qapps.types.update_q_app_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/apps.update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "instance_id" in input_:
        headers["instance-id"] = str(input_["instance_id"])
    body: bytes | None = json.dumps(
        capo_qapps.types.update_q_app_input.serialize_json(input_)
    ).encode()
...
def update_q_app(
    ...
    input_: capo_qapps.types.update_q_app_input.UpdateQAppInput,
) -> tuple[capo_qapps.types.update_q_app_output.UpdateQAppOutput, zapros.Response]:
