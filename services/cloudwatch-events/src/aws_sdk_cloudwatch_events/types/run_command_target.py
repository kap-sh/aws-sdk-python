...
if TYPE_CHECKING:
    import aws_sdk_cloudwatch_events.types.run_command_target_key
    import aws_sdk_cloudwatch_events.types.run_command_target_values


class RunCommandTarget(TypedDict):
    key: "aws_sdk_cloudwatch_events.types.run_command_target_key.RunCommandTargetKey"
    """<p>Can be either <code>tag:</code> <i>tag-key</i> or <code>InstanceIds</code>.</p>"""
    values: "aws_sdk_cloudwatch_events.types.run_command_target_values.RunCommandTargetValues"
...
def serialize_aws_json_1_1(value: RunCommandTarget) -> dict:
    ...
    out["Values"] = (
        aws_sdk_cloudwatch_events.types.run_command_target_values.serialize_aws_json_1_1(
            value["values"]
        )
    )
    return out
...
def deserialize_aws_json_1_1(data: dict) -> RunCommandTarget:
    ...
    if "Values" in data:
        ...
        out["values"] = (
            aws_sdk_cloudwatch_events.types.run_command_target_values.deserialize_aws_json_1_1(
                data["Values"]
            )
        )
    else:
