"""Generated from Smithy shape ``com.amazonaws.cloudwatchevents#OperationDisabledException``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

from aws_sdk_cloudwatch_events.errors import ServiceError

if TYPE_CHECKING:
    import aws_sdk_cloudwatch_events.types.error_message


class OperationDisabledException_(TypedDict):
    message: NotRequired["aws_sdk_cloudwatch_events.types.error_message.ErrorMessage"]


# --- awsJson1_1 ser/de ---
def serialize_aws_json_1_1(value: OperationDisabledException_) -> dict:
    out: dict = {}
    if "message" in value:
        out["message"] = value["message"]
    return out


def deserialize_aws_json_1_1(data: dict) -> OperationDisabledException_:
    out: OperationDisabledException_ = {}  # type: ignore[typeddict-item]
    if "message" in data:
        out["message"] = data["message"]
    return out


class OperationDisabledException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.cloudwatchevents#OperationDisabledException``."""

    code: str | None = "OperationDisabledException"

    def __init__(self, data: OperationDisabledException_):
        super().__init__(
            "client",
            is_throttling_error=False,
            is_retryable=False,
            code="OperationDisabledException",
        )
        self.data = data

    @classmethod
    def from_aws_json_1_1(cls, data: dict) -> "OperationDisabledException":
        return cls(deserialize_aws_json_1_1(data))
