...
def serialize_json(value: PermissionGroupList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_finspace_data.types.permission_group.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PermissionGroupList:
    ...
    for item in data:
        out.append(aws_sdk_finspace_data.types.permission_group.deserialize_json(item))
    return out
