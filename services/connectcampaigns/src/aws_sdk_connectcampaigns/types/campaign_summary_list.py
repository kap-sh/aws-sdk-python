...
CampaignSummaryList: TypeAlias = list[
    "aws_sdk_connectcampaigns.types.campaign_summary.CampaignSummary"
]
...
def serialize_json(value: CampaignSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_connectcampaigns.types.campaign_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> CampaignSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_connectcampaigns.types.campaign_summary.deserialize_json(item)
        )
    return out
