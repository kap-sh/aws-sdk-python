...
class ListCampaignsRequest(TypedDict, closed=True):
    max_results: NotRequired["aws_sdk_connectcampaigns.types.max_results.MaxResults"]
...
def serialize_json(value: ListCampaignsRequest) -> dict:
    ...
    if "filters" in value:
        ...
        out["filters"] = aws_sdk_connectcampaigns.types.campaign_filters.serialize_json(
            value["filters"]
        )
    return out
...
def deserialize_json(data: dict) -> ListCampaignsRequest:
    ...
    if "filters" in data:
        ...
        out["filters"] = (
            aws_sdk_connectcampaigns.types.campaign_filters.deserialize_json(
                data["filters"]
            )
        )
    return out
