...
if TYPE_CHECKING:
    import capo_connectcampaigns.types.campaign


class DescribeCampaignResponse(TypedDict, closed=True):
    campaign: NotRequired["capo_connectcampaigns.types.campaign.Campaign"]
...
def serialize_json(value: DescribeCampaignResponse) -> dict:
    ...
    if "campaign" in value:
        ...
        out["campaign"] = capo_connectcampaigns.types.campaign.serialize_json(
            value["campaign"]
        )
    return out
...
def deserialize_json(data: dict) -> DescribeCampaignResponse:
    ...
    if "campaign" in data:
        ...
        out["campaign"] = capo_connectcampaigns.types.campaign.deserialize_json(
            data["campaign"]
        )
    return out
