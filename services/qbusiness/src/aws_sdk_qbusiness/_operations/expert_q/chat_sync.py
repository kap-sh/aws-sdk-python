...
def build_request(
    ...
    input: aws_sdk_qbusiness.types.chat_sync_input.ChatSyncInput,
) -> zapros.Request:
    ...
    if "user_groups" in input:
        params["userGroups"] = str(input["user_groups"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_qbusiness.types.chat_sync_input

    body: bytes | None = json.dumps(
        aws_sdk_qbusiness.types.chat_sync_input.serialize_json(input)
    ).encode()
...
def chat_sync(
    ...
    input: aws_sdk_qbusiness.types.chat_sync_input.ChatSyncInput,
) -> tuple[aws_sdk_qbusiness.types.chat_sync_output.ChatSyncOutput, zapros.Response]:
