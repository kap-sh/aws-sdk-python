"""Generated from Smithy shape ``com.amazonaws.qbusiness#UpdatePlugin``."""
...
if TYPE_CHECKING:
    ...
    import aws_sdk_qbusiness.types.update_plugin_response
...
) -> aws_sdk_qbusiness.types.update_plugin_response.UpdatePluginResponse:
    out: aws_sdk_qbusiness.types.update_plugin_response.UpdatePluginResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_qbusiness.types.update_plugin_request.UpdatePluginRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/applications/{applicationId}/plugins/{pluginId}"
    url = url.replace("{applicationId}", quote(str(input["application_id"]), safe=""))
    url = url.replace("{pluginId}", quote(str(input["plugin_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_qbusiness.types.update_plugin_request

    body: bytes | None = json.dumps(
        aws_sdk_qbusiness.types.update_plugin_request.serialize_json(input)
    ).encode()
...
def update_plugin(
    ...
    aws_sdk_qbusiness.types.update_plugin_response.UpdatePluginResponse, zapros.Response
]:
