...
class BrowserExtensionConfiguration(TypedDict):
    enabled_browser_extensions: "aws_sdk_qbusiness.types.browser_extension_list.BrowserExtensionList"
...
def serialize_json(value: BrowserExtensionConfiguration) -> dict:
    out: dict = {}
    import aws_sdk_qbusiness.types.browser_extension_list
    out["enabledBrowserExtensions"] = aws_sdk_qbusiness.types.browser_extension_list.serialize_json(value["enabled_browser_extensions"])
    return out
...
def deserialize_json(data: dict) -> BrowserExtensionConfiguration:
    ...
    if "enabledBrowserExtensions" in data:
        import aws_sdk_qbusiness.types.browser_extension_list
        out["enabled_browser_extensions"] = aws_sdk_qbusiness.types.browser_extension_list.deserialize_json(data["enabledBrowserExtensions"])
    else:
        raise DeserializationError("BrowserExtensionConfiguration.enabled_browser_extensions required")
    return out