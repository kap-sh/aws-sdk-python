...
def serialize_json(value: DataSourceVpcConfiguration) -> dict:
    out: dict = {}
    import aws_sdk_qbusiness.types.subnet_ids
    out["subnetIds"] = aws_sdk_qbusiness.types.subnet_ids.serialize_json(value["subnet_ids"])
    import aws_sdk_qbusiness.types.security_group_ids
    out["securityGroupIds"] = aws_sdk_qbusiness.types.security_group_ids.serialize_json(value["security_group_ids"])
    return out
...
def deserialize_json(data: dict) -> DataSourceVpcConfiguration:
    ...
    if "subnetIds" in data:
        import aws_sdk_qbusiness.types.subnet_ids
        out["subnet_ids"] = aws_sdk_qbusiness.types.subnet_ids.deserialize_json(data["subnetIds"])
    else:
        raise DeserializationError("DataSourceVpcConfiguration.subnet_ids required")
    if "securityGroupIds" in data:
        import aws_sdk_qbusiness.types.security_group_ids
        out["security_group_ids"] = aws_sdk_qbusiness.types.security_group_ids.deserialize_json(data["securityGroupIds"])
    else:
        raise DeserializationError("DataSourceVpcConfiguration.security_group_ids required")
    return out