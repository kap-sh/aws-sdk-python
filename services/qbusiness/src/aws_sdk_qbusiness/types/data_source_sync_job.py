...
class DataSourceSyncJob(TypedDict):
    ...
    status: NotRequired["aws_sdk_qbusiness.types.data_source_sync_job_status.DataSourceSyncJobStatus"]
    ...
    metrics: NotRequired["aws_sdk_qbusiness.types.data_source_sync_job_metrics.DataSourceSyncJobMetrics"]
...
def serialize_json(value: DataSourceSyncJob) -> dict:
    ...
    if "end_time" in value:
        import aws_sdk_qbusiness.types.timestamp
        out["endTime"] = aws_sdk_qbusiness.types.timestamp.serialize_json(value["end_time"])
    if "status" in value:
        import aws_sdk_qbusiness.types.data_source_sync_job_status
        out["status"] = aws_sdk_qbusiness.types.data_source_sync_job_status.serialize_json(value["status"])
    if "error" in value:
        import aws_sdk_qbusiness.types.error_detail
        out["error"] = aws_sdk_qbusiness.types.error_detail.serialize_json(value["error"])
    if "data_source_error_code" in value:
        out["dataSourceErrorCode"] = value["data_source_error_code"]
    if "metrics" in value:
        import aws_sdk_qbusiness.types.data_source_sync_job_metrics
        out["metrics"] = aws_sdk_qbusiness.types.data_source_sync_job_metrics.serialize_json(value["metrics"])
    return out
...
def deserialize_json(data: dict) -> DataSourceSyncJob:
    ...
    if "endTime" in data:
        import aws_sdk_qbusiness.types.timestamp
        out["end_time"] = aws_sdk_qbusiness.types.timestamp.deserialize_json(data["endTime"])
    if "status" in data:
        import aws_sdk_qbusiness.types.data_source_sync_job_status
        out["status"] = aws_sdk_qbusiness.types.data_source_sync_job_status.deserialize_json(data["status"])
    if "error" in data:
        import aws_sdk_qbusiness.types.error_detail
        out["error"] = aws_sdk_qbusiness.types.error_detail.deserialize_json(data["error"])
    if "dataSourceErrorCode" in data:
        out["data_source_error_code"] = data["dataSourceErrorCode"]
    if "metrics" in data:
        import aws_sdk_qbusiness.types.data_source_sync_job_metrics
        out["metrics"] = aws_sdk_qbusiness.types.data_source_sync_job_metrics.deserialize_json(data["metrics"])
    return out