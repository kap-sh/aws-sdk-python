...
def serialize_json(value: CreateSubscriptionRequest) -> dict:
    out: dict = {}
    import aws_sdk_qbusiness.types.subscription_principal
    out["principal"] = aws_sdk_qbusiness.types.subscription_principal.serialize_json(value["principal"])
    import aws_sdk_qbusiness.types.subscription_type
    out["type"] = aws_sdk_qbusiness.types.subscription_type.serialize_json(value["type"])
...
def deserialize_json(data: dict) -> CreateSubscriptionRequest:
    ...
    if "principal" in data:
        import aws_sdk_qbusiness.types.subscription_principal
        out["principal"] = aws_sdk_qbusiness.types.subscription_principal.deserialize_json(data["principal"])
    else:
        raise DeserializationError("CreateSubscriptionRequest.principal required")
    if "type" in data:
        import aws_sdk_qbusiness.types.subscription_type
        out["type"] = aws_sdk_qbusiness.types.subscription_type.deserialize_json(data["type"])
    else:
