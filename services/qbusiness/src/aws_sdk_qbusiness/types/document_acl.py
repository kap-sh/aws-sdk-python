...
class DocumentAcl(TypedDict, closed=True):
    allowlist: NotRequired[
        "aws_sdk_qbusiness.types.document_acl_membership.DocumentAclMembership"
    ]
...
def serialize_json(value: DocumentAcl) -> dict:
    ...
    if "allowlist" in value:
        ...
        out["allowlist"] = (
            aws_sdk_qbusiness.types.document_acl_membership.serialize_json(
                value["allowlist"]
            )
        )
...
def deserialize_json(data: dict) -> DocumentAcl:
    ...
    if "allowlist" in data:
        ...
        out["allowlist"] = (
            aws_sdk_qbusiness.types.document_acl_membership.deserialize_json(
                data["allowlist"]
            )
        )
    if "denyList" in data:
