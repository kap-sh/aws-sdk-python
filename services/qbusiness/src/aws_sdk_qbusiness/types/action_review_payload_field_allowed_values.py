...
ActionReviewPayloadFieldAllowedValues: TypeAlias = list[
    "aws_sdk_qbusiness.types.action_review_payload_field_allowed_value.ActionReviewPayloadFieldAllowedValue"
]
...
def serialize_json(value: ActionReviewPayloadFieldAllowedValues) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_qbusiness.types.action_review_payload_field_allowed_value.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ActionReviewPayloadFieldAllowedValues:
    ...
    for item in data:
        out.append(
            aws_sdk_qbusiness.types.action_review_payload_field_allowed_value.deserialize_json(
                item
            )
        )
    return out
