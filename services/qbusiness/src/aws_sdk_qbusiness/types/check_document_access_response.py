...
class CheckDocumentAccessResponse(TypedDict):
    user_groups: NotRequired[
        "aws_sdk_qbusiness.types.associated_groups.AssociatedGroups"
    ]
    """<p>An array of groups the user is part of for the specified data source. Each group has a name and type.</p>"""
    user_aliases: NotRequired[
        "aws_sdk_qbusiness.types.associated_users.AssociatedUsers"
    ]
...
def serialize_json(value: CheckDocumentAccessResponse) -> dict:
    ...
    if "user_groups" in value:
        ...
        out["userGroups"] = aws_sdk_qbusiness.types.associated_groups.serialize_json(
            value["user_groups"]
        )
    if "user_aliases" in value:
        import aws_sdk_qbusiness.types.associated_users

        out["userAliases"] = aws_sdk_qbusiness.types.associated_users.serialize_json(
            value["user_aliases"]
        )
    ...
    if "document_acl" in value:
        ...
        out["documentAcl"] = aws_sdk_qbusiness.types.document_acl.serialize_json(
            value["document_acl"]
        )
    return out
...
def deserialize_json(data: dict) -> CheckDocumentAccessResponse:
    ...
    if "userGroups" in data:
        ...
        out["user_groups"] = aws_sdk_qbusiness.types.associated_groups.deserialize_json(
            data["userGroups"]
        )
    if "userAliases" in data:
        import aws_sdk_qbusiness.types.associated_users

        out["user_aliases"] = aws_sdk_qbusiness.types.associated_users.deserialize_json(
            data["userAliases"]
        )
    ...
    if "documentAcl" in data:
        ...
        out["document_acl"] = aws_sdk_qbusiness.types.document_acl.deserialize_json(
            data["documentAcl"]
        )
    return out
