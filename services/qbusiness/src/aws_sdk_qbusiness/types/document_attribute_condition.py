...
def serialize_json(value: DocumentAttributeCondition) -> dict:
    ...
    out["key"] = value["key"]
    import aws_sdk_qbusiness.types.document_enrichment_condition_operator
    out["operator"] = aws_sdk_qbusiness.types.document_enrichment_condition_operator.serialize_json(value["operator"])
    if "value" in value:
        import aws_sdk_qbusiness.types.document_attribute_value
...
def deserialize_json(data: dict) -> DocumentAttributeCondition:
    ...
    if "operator" in data:
        import aws_sdk_qbusiness.types.document_enrichment_condition_operator
        out["operator"] = aws_sdk_qbusiness.types.document_enrichment_condition_operator.deserialize_json(data["operator"])
    else:
