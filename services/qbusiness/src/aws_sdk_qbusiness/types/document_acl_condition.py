...
class DocumentAclCondition(TypedDict):
...
def serialize_json(value: DocumentAclCondition) -> dict:
    ...
    if "users" in value:
        import aws_sdk_qbusiness.types.document_acl_users
        out["users"] = aws_sdk_qbusiness.types.document_acl_users.serialize_json(value["users"])
    if "groups" in value:
        import aws_sdk_qbusiness.types.document_acl_groups
        out["groups"] = aws_sdk_qbusiness.types.document_acl_groups.serialize_json(value["groups"])
    return out
...
def deserialize_json(data: dict) -> DocumentAclCondition:
    ...
    if "users" in data:
        import aws_sdk_qbusiness.types.document_acl_users
        out["users"] = aws_sdk_qbusiness.types.document_acl_users.deserialize_json(data["users"])
    if "groups" in data:
        import aws_sdk_qbusiness.types.document_acl_groups
        out["groups"] = aws_sdk_qbusiness.types.document_acl_groups.deserialize_json(data["groups"])
    return out