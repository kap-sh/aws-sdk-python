...
class _ContentSource_retriever(TypedDict):
...
def serialize_json(value: ContentSource) -> dict:
    if "retriever" in value:
        import aws_sdk_qbusiness.types.retriever_content_source
        return {"retriever": aws_sdk_qbusiness.types.retriever_content_source.serialize_json(value["retriever"])}
    else:
...
def deserialize_json(data: dict) -> ContentSource:
    if "retriever" in data:
        import aws_sdk_qbusiness.types.retriever_content_source
        return {"retriever": aws_sdk_qbusiness.types.retriever_content_source.deserialize_json(data["retriever"])}
    else:
