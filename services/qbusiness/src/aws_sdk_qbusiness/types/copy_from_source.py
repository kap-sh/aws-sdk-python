...
def serialize_json(value: CopyFromSource) -> dict:
    if "conversation" in value:
        import aws_sdk_qbusiness.types.conversation_source
        return {"conversation": aws_sdk_qbusiness.types.conversation_source.serialize_json(value["conversation"])}
    else:
...
def deserialize_json(data: dict) -> CopyFromSource:
    if "conversation" in data:
        import aws_sdk_qbusiness.types.conversation_source
        return {"conversation": aws_sdk_qbusiness.types.conversation_source.deserialize_json(data["conversation"])}
    else:
