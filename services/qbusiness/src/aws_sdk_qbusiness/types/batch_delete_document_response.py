...
class BatchDeleteDocumentResponse(TypedDict):
    failed_documents: NotRequired["aws_sdk_qbusiness.types.failed_documents.FailedDocuments"]
...
def serialize_json(value: BatchDeleteDocumentResponse) -> dict:
    ...
    if "failed_documents" in value:
        import aws_sdk_qbusiness.types.failed_documents
        out["failedDocuments"] = aws_sdk_qbusiness.types.failed_documents.serialize_json(value["failed_documents"])
    return out
...
def deserialize_json(data: dict) -> BatchDeleteDocumentResponse:
    ...
    if "failedDocuments" in data:
        import aws_sdk_qbusiness.types.failed_documents
        out["failed_documents"] = aws_sdk_qbusiness.types.failed_documents.deserialize_json(data["failedDocuments"])
    return out