...
class ContentRetrievalRule(TypedDict):
    eligible_data_sources: NotRequired["aws_sdk_qbusiness.types.eligible_data_sources.EligibleDataSources"]
...
def serialize_json(value: ContentRetrievalRule) -> dict:
    ...
    if "eligible_data_sources" in value:
        import aws_sdk_qbusiness.types.eligible_data_sources
        out["eligibleDataSources"] = aws_sdk_qbusiness.types.eligible_data_sources.serialize_json(value["eligible_data_sources"])
    return out
...
def deserialize_json(data: dict) -> ContentRetrievalRule:
    ...
    if "eligibleDataSources" in data:
        import aws_sdk_qbusiness.types.eligible_data_sources
        out["eligible_data_sources"] = aws_sdk_qbusiness.types.eligible_data_sources.deserialize_json(data["eligibleDataSources"])
    return out