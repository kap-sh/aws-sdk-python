"""Generated from Smithy shape ``com.amazonaws.qbusiness#CreateUser``."""
...
def handle_response(
    ...
    out: capo_qbusiness.types.create_user_response.CreateUserResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/applications/{applicationId}/users"
    url = url.replace("{applicationId}", quote(str(input_["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qbusiness.types.create_user_request.serialize_json(input_)
    ).encode()
...
def create_user(
    ...
    capo_qbusiness.types.create_user_response.CreateUserResponse, zapros.Response
]:
