"""Generated from Smithy shape ``com.amazonaws.qbusiness#UpdateChatControlsConfiguration``."""
...
def handle_response(
    ...
    out: capo_qbusiness.types.update_chat_controls_configuration_response.UpdateChatControlsConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/applications/{applicationId}/chatcontrols"
    url = url.replace("{applicationId}", quote(str(input_["application_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_qbusiness.types.update_chat_controls_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_chat_controls_configuration(
