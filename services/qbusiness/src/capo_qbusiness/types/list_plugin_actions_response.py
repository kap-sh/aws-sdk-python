...
class ListPluginActionsResponse(TypedDict, closed=True):
    ...
    items: NotRequired["capo_qbusiness.types.actions.Actions"]
...
def serialize_json(value: ListPluginActionsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_qbusiness.types.actions.serialize_json(value["items"])
    return out
...
def deserialize_json(data: dict) -> ListPluginActionsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = capo_qbusiness.types.actions.deserialize_json(data["items"])
    return out
