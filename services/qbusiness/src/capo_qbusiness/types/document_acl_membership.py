...
class DocumentAclMembership(TypedDict, closed=True):
    ...
    conditions: NotRequired[
        "capo_qbusiness.types.document_acl_conditions.DocumentAclConditions"
    ]
...
def serialize_json(value: DocumentAclMembership) -> dict:
    ...
    if "conditions" in value:
        ...
        out["conditions"] = capo_qbusiness.types.document_acl_conditions.serialize_json(
            value["conditions"]
        )
    return out
...
def deserialize_json(data: dict) -> DocumentAclMembership:
    ...
    if "conditions" in data:
        ...
        out["conditions"] = (
            capo_qbusiness.types.document_acl_conditions.deserialize_json(
                data["conditions"]
            )
        )
    return out
