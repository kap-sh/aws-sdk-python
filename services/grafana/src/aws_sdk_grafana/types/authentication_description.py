...
class AuthenticationDescription(TypedDict):
    ...
    aws_sso: NotRequired["aws_sdk_grafana.types.aws_sso_authentication.AwsSsoAuthentication"]
...
def serialize_json(value: AuthenticationDescription) -> dict:
    out: dict = {}
    import aws_sdk_grafana.types.authentication_providers
    out["providers"] = aws_sdk_grafana.types.authentication_providers.serialize_json(value["providers"])
    if "saml" in value:
        import aws_sdk_grafana.types.saml_authentication
        out["saml"] = aws_sdk_grafana.types.saml_authentication.serialize_json(value["saml"])
    if "aws_sso" in value:
        import aws_sdk_grafana.types.aws_sso_authentication
        out["awsSso"] = aws_sdk_grafana.types.aws_sso_authentication.serialize_json(value["aws_sso"])
    return out
...
def deserialize_json(data: dict) -> AuthenticationDescription:
    ...
    if "providers" in data:
        import aws_sdk_grafana.types.authentication_providers
        out["providers"] = aws_sdk_grafana.types.authentication_providers.deserialize_json(data["providers"])
    else:
        raise DeserializationError("AuthenticationDescription.providers required")
    if "saml" in data:
        import aws_sdk_grafana.types.saml_authentication
        out["saml"] = aws_sdk_grafana.types.saml_authentication.deserialize_json(data["saml"])
    if "awsSso" in data:
        import aws_sdk_grafana.types.aws_sso_authentication
        out["aws_sso"] = aws_sdk_grafana.types.aws_sso_authentication.deserialize_json(data["awsSso"])
    return out