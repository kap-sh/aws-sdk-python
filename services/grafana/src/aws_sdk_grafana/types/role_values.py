...
def serialize_json(value: RoleValues) -> dict:
    ...
    if "editor" in value:
        import aws_sdk_grafana.types.role_value_list
        out["editor"] = aws_sdk_grafana.types.role_value_list.serialize_json(value["editor"])
    if "admin" in value:
        import aws_sdk_grafana.types.role_value_list
        out["admin"] = aws_sdk_grafana.types.role_value_list.serialize_json(value["admin"])
    return out
...
def deserialize_json(data: dict) -> RoleValues:
    ...
    if "editor" in data:
        import aws_sdk_grafana.types.role_value_list
        out["editor"] = aws_sdk_grafana.types.role_value_list.deserialize_json(data["editor"])
    if "admin" in data:
        import aws_sdk_grafana.types.role_value_list
        out["admin"] = aws_sdk_grafana.types.role_value_list.deserialize_json(data["admin"])
    return out