...
ServiceAccountTokenList: TypeAlias = list[
    "aws_sdk_grafana.types.service_account_token_summary.ServiceAccountTokenSummary"
]
...
def serialize_json(value: ServiceAccountTokenList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_grafana.types.service_account_token_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ServiceAccountTokenList:
    ...
    for item in data:
        out.append(
            aws_sdk_grafana.types.service_account_token_summary.deserialize_json(item)
        )
    return out
