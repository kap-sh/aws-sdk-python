...
def handle_response(
    ...
    out: aws_sdk_grafana.types.associate_license_response.AssociateLicenseResponse = (
        aws_sdk_grafana.types.associate_license_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_grafana.types.associate_license_request.AssociateLicenseRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{licenseType}", quote(str(input_["license_type"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "grafana_token" in input_:
        headers["Grafana-Token"] = str(input_["grafana_token"])
    body: bytes | None = b""
...
async def async_associate_license(
