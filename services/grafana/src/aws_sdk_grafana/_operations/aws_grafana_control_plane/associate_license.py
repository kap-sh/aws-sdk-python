"""Generated from Smithy shape ``com.amazonaws.grafana#AssociateLicense``."""
...
def handle_response(response: zapros.Response, is_async: bool) -> aws_sdk_grafana.types.associate_license_response.AssociateLicenseResponse:
    import aws_sdk_grafana.types.associate_license_response
    out: aws_sdk_grafana.types.associate_license_response.AssociateLicenseResponse = aws_sdk_grafana.types.associate_license_response.deserialize_json(json.loads(response.read()))
    return out
...
def build_request(options: OperationOptions | AsyncOperationOptions, input: aws_sdk_grafana.types.associate_license_request.AssociateLicenseRequest) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/workspaces/{workspaceId}/licenses/{licenseType}"
    url = url.replace("{workspaceId}", quote(str(input["workspace_id"]), safe=""))
    url = url.replace("{licenseType}", quote(str(input["license_type"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "grafana_token" in input:
        headers["Grafana-Token"] = str(input["grafana_token"])
    body: bytes | None = b""
...
def associate_license(options: OperationOptions, input: aws_sdk_grafana.types.associate_license_request.AssociateLicenseRequest) -> tuple[aws_sdk_grafana.types.associate_license_response.AssociateLicenseResponse, zapros.Response]:
...
async def async_associate_license(options: AsyncOperationOptions, input: aws_sdk_grafana.types.associate_license_request.AssociateLicenseRequest) -> tuple[aws_sdk_grafana.types.associate_license_response.AssociateLicenseResponse, zapros.Response]:
