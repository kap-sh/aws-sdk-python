...
def deserialize_aws_json_1_0(data: dict) -> AssociateProtectConfigurationResult:
    ...
    if "ConfigurationSetArn" in data:
        ...
        raise DeserializationError("AssociateProtectConfigurationResult.configuration_set_arn required")
    if "ConfigurationSetName" in data:
        out["configuration_set_name"] = data["ConfigurationSetName"]
    else:
        raise DeserializationError("AssociateProtectConfigurationResult.configuration_set_name required")
    if "ProtectConfigurationArn" in data:
        out["protect_configuration_arn"] = data["ProtectConfigurationArn"]
    else:
        raise DeserializationError("AssociateProtectConfigurationResult.protect_configuration_arn required")
    if "ProtectConfigurationId" in data:
        out["protect_configuration_id"] = data["ProtectConfigurationId"]
    else:
        raise DeserializationError("AssociateProtectConfigurationResult.protect_configuration_id required")
    return out