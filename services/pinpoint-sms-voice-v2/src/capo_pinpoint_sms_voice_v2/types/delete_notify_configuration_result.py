"""Generated from Smithy shape ``com.amazonaws.pinpointsmsvoicev2#DeleteNotifyConfigurationResult``."""
...
class DeleteNotifyConfigurationResult(TypedDict, closed=True):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: DeleteNotifyConfigurationResult) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_0(data: dict) -> DeleteNotifyConfigurationResult:
    out: DeleteNotifyConfigurationResult = {}  # type: ignore[typeddict-item]
    if "NotifyConfigurationArn" in data:
        out["notify_configuration_arn"] = data["NotifyConfigurationArn"]
    else:
        raise DeserializationError(
            "DeleteNotifyConfigurationResult.notify_configuration_arn required"
        )
    if "NotifyConfigurationId" in data:
        out["notify_configuration_id"] = data["NotifyConfigurationId"]
    else:
        raise DeserializationError(
            "DeleteNotifyConfigurationResult.notify_configuration_id required"
        )
    if "DisplayName" in data:
        out["display_name"] = data["DisplayName"]
    else:
        raise DeserializationError(
            "DeleteNotifyConfigurationResult.display_name required"
        )
    if "UseCase" in data:
        out["use_case"] = data["UseCase"]
    else:
        raise DeserializationError("DeleteNotifyConfigurationResult.use_case required")
    ...
    if "EnabledChannels" in data:
        ...
        raise DeserializationError(
            "DeleteNotifyConfigurationResult.enabled_channels required"
        )
    if "Tier" in data:
        out["tier"] = data["Tier"]
    else:
        raise DeserializationError("DeleteNotifyConfigurationResult.tier required")
    if "TierUpgradeStatus" in data:
        out["tier_upgrade_status"] = data["TierUpgradeStatus"]
    else:
        raise DeserializationError(
            "DeleteNotifyConfigurationResult.tier_upgrade_status required"
        )
    if "Status" in data:
        out["status"] = data["Status"]
    else:
        raise DeserializationError("DeleteNotifyConfigurationResult.status required")
    ...
    if "CreatedTimestamp" in data:
        ...
        raise DeserializationError(
            "DeleteNotifyConfigurationResult.created_timestamp required"
        )
    return out
