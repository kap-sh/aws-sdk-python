...
if TYPE_CHECKING:
    import aws_sdk_iot.types.asset_property_offset_in_nanos
    import aws_sdk_iot.types.asset_property_time_in_seconds


class AssetPropertyTimestamp(TypedDict):
    time_in_seconds: (
        "aws_sdk_iot.types.asset_property_time_in_seconds.AssetPropertyTimeInSeconds"
    )
    """<p>A string that contains the time in seconds since epoch. Accepts substitution templates.</p>"""
    offset_in_nanos: NotRequired[
        "aws_sdk_iot.types.asset_property_offset_in_nanos.AssetPropertyOffsetInNanos"
    ]
