...
if TYPE_CHECKING:
    import aws_sdk_iot.types.certificate_provider_operation

CertificateProviderAccountDefaultForOperations: TypeAlias = list[
    "aws_sdk_iot.types.certificate_provider_operation.CertificateProviderOperation"
]
...
def serialize_json(value: CertificateProviderAccountDefaultForOperations) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_iot.types.certificate_provider_operation.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> CertificateProviderAccountDefaultForOperations:
    ...
    for item in data:
        out.append(
            aws_sdk_iot.types.certificate_provider_operation.deserialize_json(item)
        )
    return out
