...
ThingTypeList: TypeAlias = list[
    "aws_sdk_iot.types.thing_type_definition.ThingTypeDefinition"
]
...
def serialize_json(value: ThingTypeList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iot.types.thing_type_definition.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> ThingTypeList:
    ...
    out: ThingTypeList = []
    for item in data:
        out.append(aws_sdk_iot.types.thing_type_definition.deserialize_json(item))
    return out
