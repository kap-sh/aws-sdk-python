"""Generated from Smithy shape ``com.amazonaws.iot#GetTopicRuleResponse``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

if TYPE_CHECKING:
    import aws_sdk_iot.types.rule_arn
    import aws_sdk_iot.types.topic_rule


class GetTopicRuleResponse(TypedDict):
    rule_arn: NotRequired["aws_sdk_iot.types.rule_arn.RuleArn"]
    """<p>The rule ARN.</p>"""
    rule: NotRequired["aws_sdk_iot.types.topic_rule.TopicRule"]
    """<p>The rule.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: GetTopicRuleResponse) -> dict:
    out: dict = {}
    if "rule_arn" in value:
        out["ruleArn"] = value["rule_arn"]
    if "rule" in value:
        import aws_sdk_iot.types.topic_rule

        out["rule"] = aws_sdk_iot.types.topic_rule.serialize_json(value["rule"])
    return out


def deserialize_json(data: dict) -> GetTopicRuleResponse:
    out: GetTopicRuleResponse = {}  # type: ignore[typeddict-item]
    if "ruleArn" in data:
        out["rule_arn"] = data["ruleArn"]
    if "rule" in data:
        import aws_sdk_iot.types.topic_rule

        out["rule"] = aws_sdk_iot.types.topic_rule.deserialize_json(data["rule"])
    return out
