...
CommandSummaryList: TypeAlias = list["aws_sdk_iot.types.command_summary.CommandSummary"]
...
def serialize_json(value: CommandSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_iot.types.command_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> CommandSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_iot.types.command_summary.deserialize_json(item))
    return out
