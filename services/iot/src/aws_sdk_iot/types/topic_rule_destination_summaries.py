...
TopicRuleDestinationSummaries: TypeAlias = list[
    "aws_sdk_iot.types.topic_rule_destination_summary.TopicRuleDestinationSummary"
]
...
def serialize_json(value: TopicRuleDestinationSummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_iot.types.topic_rule_destination_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> TopicRuleDestinationSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_iot.types.topic_rule_destination_summary.deserialize_json(item)
        )
    return out
