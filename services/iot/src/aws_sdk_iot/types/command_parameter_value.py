...
if TYPE_CHECKING:
    ...
    import aws_sdk_iot.types.boolean_parameter_value
    import aws_sdk_iot.types.double_parameter_value
    import aws_sdk_iot.types.integer_parameter_value
    import aws_sdk_iot.types.long_parameter_value
...
def serialize_json(value: CommandParameterValue) -> dict:
    ...
    if "bin" in value:
        ...
        out["BIN"] = aws_sdk_iot.types.binary_parameter_value.serialize_json(
            value["bin"]
        )
    if "ul" in value:
        out["UL"] = value["ul"]
...
def deserialize_json(data: dict) -> CommandParameterValue:
    ...
    if "BIN" in data:
        ...
        out["bin"] = aws_sdk_iot.types.binary_parameter_value.deserialize_json(
            data["BIN"]
        )
    if "UL" in data:
        out["ul"] = data["UL"]
