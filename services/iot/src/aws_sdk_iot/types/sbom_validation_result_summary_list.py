...
SbomValidationResultSummaryList: TypeAlias = list[
    "aws_sdk_iot.types.sbom_validation_result_summary.SbomValidationResultSummary"
]
...
def serialize_json(value: SbomValidationResultSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_iot.types.sbom_validation_result_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> SbomValidationResultSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_iot.types.sbom_validation_result_summary.deserialize_json(item)
        )
    return out
