...
if TYPE_CHECKING:
    ...
    import aws_sdk_iot.types.connection_attribute_name
    import aws_sdk_iot.types.user_property_key_name


class PropagatingAttribute(TypedDict):
    user_property_key: NotRequired[
        "aws_sdk_iot.types.user_property_key_name.UserPropertyKeyName"
    ]
    """<p>The key of the user property key-value pair.</p>"""
    thing_attribute: NotRequired["aws_sdk_iot.types.attribute_name.AttributeName"]
    """<p>The user-defined thing attribute that is propagating for MQTT 5 message enrichment.</p>"""
    connection_attribute: NotRequired[
        "aws_sdk_iot.types.connection_attribute_name.ConnectionAttributeName"
    ]
