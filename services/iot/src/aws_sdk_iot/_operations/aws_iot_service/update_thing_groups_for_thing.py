"""Generated from Smithy shape ``com.amazonaws.iot#UpdateThingGroupsForThing``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.update_thing_groups_for_thing_response.UpdateThingGroupsForThingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iot.types.update_thing_groups_for_thing_request.UpdateThingGroupsForThingRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/thing-groups/updateThingGroupsForThing"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.update_thing_groups_for_thing_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.update_thing_groups_for_thing_request.serialize_json(input_)
    ).encode()
...
async def async_update_thing_groups_for_thing(
