"""Generated from Smithy shape ``com.amazonaws.iot#RegisterCACertificate``."""
...
def build_request(
    ...
    input: aws_sdk_iot.types.register_ca_certificate_request.RegisterCACertificateRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/cacertificate"
    params: dict[str, str] = {}
    params["setAsActive"] = str(input.get("set_as_active", False))
    params["allowAutoRegistration"] = str(input.get("allow_auto_registration", False))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.register_ca_certificate_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.register_ca_certificate_request.serialize_json(input)
    ).encode()
...
async def async_register_ca_certificate(
