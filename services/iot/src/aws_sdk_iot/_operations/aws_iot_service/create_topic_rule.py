"""Generated from Smithy shape ``com.amazonaws.iot#CreateTopicRule``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "SqlParseException":
            import aws_sdk_iot.errors.sql_parse_exception

            raise aws_sdk_iot.errors.sql_parse_exception.SqlParseException.from_json(
                data
            )
...
def build_request(
    ...
    input: aws_sdk_iot.types.create_topic_rule_request.CreateTopicRuleRequest,
) -> zapros.Request:
    ...
    if "tags" in input:
        headers["x-amz-tagging"] = str(input["tags"])
    if "topic_rule_payload" in input:
...
async def async_create_topic_rule(
