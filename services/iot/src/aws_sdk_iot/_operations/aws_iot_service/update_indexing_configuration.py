"""Generated from Smithy shape ``com.amazonaws.iot#UpdateIndexingConfiguration``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.update_indexing_configuration_response.UpdateIndexingConfigurationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iot.types.update_indexing_configuration_request.UpdateIndexingConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_iot.types.update_indexing_configuration_request.serialize_json(input_)
    ).encode()
...
async def async_update_indexing_configuration(
