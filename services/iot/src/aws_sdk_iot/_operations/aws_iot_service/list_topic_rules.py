...
def handle_response(
    ...
    out: aws_sdk_iot.types.list_topic_rules_response.ListTopicRulesResponse = (
        aws_sdk_iot.types.list_topic_rules_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_iot.types.list_topic_rules_request.ListTopicRulesRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "topic" in input:
        params["topic"] = str(input["topic"])
    if "max_results" in input:
        params["maxResults"] = str(input["max_results"])
    if "next_token" in input:
        params["nextToken"] = str(input["next_token"])
    if "rule_disabled" in input:
        params["ruleDisabled"] = str(input["rule_disabled"])
...
def list_topic_rules(
    ...
    aws_sdk_iot.types.list_topic_rules_response.ListTopicRulesResponse, zapros.Response
]:
