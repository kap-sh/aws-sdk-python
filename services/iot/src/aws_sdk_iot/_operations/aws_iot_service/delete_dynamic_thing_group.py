"""Generated from Smithy shape ``com.amazonaws.iot#DeleteDynamicThingGroup``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.delete_dynamic_thing_group_response.DeleteDynamicThingGroupResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_iot.types.delete_dynamic_thing_group_request.DeleteDynamicThingGroupRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/dynamic-thing-groups/{thingGroupName}"
    url = url.replace(
        "{thingGroupName}", quote(str(input["thing_group_name"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_delete_dynamic_thing_group(
