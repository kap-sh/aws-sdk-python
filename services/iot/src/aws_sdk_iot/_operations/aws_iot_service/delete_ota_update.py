...
def handle_response(
    ...
    out: aws_sdk_iot.types.delete_ota_update_response.DeleteOTAUpdateResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input: aws_sdk_iot.types.delete_ota_update_request.DeleteOTAUpdateRequest,
) -> zapros.Request:
    ...
    url = url.replace("{otaUpdateId}", quote(str(input["ota_update_id"]), safe=""))
    params: dict[str, str] = {}
    params["deleteStream"] = str(input.get("delete_stream", False))
    params["forceDeleteAWSJob"] = str(input.get("force_delete_aws_job", False))
