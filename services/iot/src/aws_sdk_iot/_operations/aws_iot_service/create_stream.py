...
def build_request(
    ...
    input: aws_sdk_iot.types.create_stream_request.CreateStreamRequest,
) -> zapros.Request:
    ...
    url = url.replace("{streamId}", quote(str(input["stream_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.create_stream_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.create_stream_request.serialize_json(input)
    ).encode()
