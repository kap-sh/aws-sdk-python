"""Generated from Smithy shape ``com.amazonaws.iot#UpdateAuditSuppression``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.update_audit_suppression_response.UpdateAuditSuppressionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iot.types.update_audit_suppression_request.UpdateAuditSuppressionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/audit/suppressions/update"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_iot.types.update_audit_suppression_request.serialize_json(input_)
    ).encode()
...
async def async_update_audit_suppression(
