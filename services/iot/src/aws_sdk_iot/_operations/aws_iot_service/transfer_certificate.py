...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "TransferConflictException":
            import aws_sdk_iot.errors.transfer_conflict_exception

            raise aws_sdk_iot.errors.transfer_conflict_exception.TransferConflictException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_iot.types.transfer_certificate_response.TransferCertificateResponse = (
        aws_sdk_iot.types.transfer_certificate_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_iot.types.transfer_certificate_request.TransferCertificateRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "target_aws_account" in input:
        params["targetAwsAccount"] = str(input["target_aws_account"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.transfer_certificate_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.transfer_certificate_request.serialize_json(input)
    ).encode()
