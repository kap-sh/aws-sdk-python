...
def build_request(
    ...
    input: aws_sdk_iot.types.create_fleet_metric_request.CreateFleetMetricRequest,
) -> zapros.Request:
    ...
    url = url.replace("{metricName}", quote(str(input["metric_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.create_fleet_metric_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.create_fleet_metric_request.serialize_json(input)
    ).encode()
