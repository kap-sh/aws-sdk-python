"""Generated from Smithy shape ``com.amazonaws.iot#SetV2LoggingOptions``."""
...
def build_request(
    ...
    input_: aws_sdk_iot.types.set_v2_logging_options_request.SetV2LoggingOptionsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2LoggingOptions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.set_v2_logging_options_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.set_v2_logging_options_request.serialize_json(input_)
    ).encode()
...
async def async_set_v2_logging_options(
