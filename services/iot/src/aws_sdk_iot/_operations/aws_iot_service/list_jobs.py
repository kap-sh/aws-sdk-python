"""Generated from Smithy shape ``com.amazonaws.iot#ListJobs``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.list_jobs_response.ListJobsResponse = (
        aws_sdk_iot.types.list_jobs_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_iot.types.list_jobs_request.ListJobsRequest,
) -> zapros.Request:
    ...
    if "target_selection" in input:
        params["targetSelection"] = str(input["target_selection"])
    if "max_results" in input:
        params["maxResults"] = str(input["max_results"])
    if "next_token" in input:
        params["nextToken"] = str(input["next_token"])
    if "thing_group_name" in input:
        params["thingGroupName"] = str(input["thing_group_name"])
    if "thing_group_id" in input:
        params["thingGroupId"] = str(input["thing_group_id"])
    if "namespace_id" in input:
        params["namespaceId"] = str(input["namespace_id"])
...
def list_jobs(
    ...
    input: aws_sdk_iot.types.list_jobs_request.ListJobsRequest,
) -> tuple[aws_sdk_iot.types.list_jobs_response.ListJobsResponse, zapros.Response]:
