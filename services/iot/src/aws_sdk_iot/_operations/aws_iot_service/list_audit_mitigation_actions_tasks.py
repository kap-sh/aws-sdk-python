...
def build_request(
    ...
    input: aws_sdk_iot.types.list_audit_mitigation_actions_tasks_request.ListAuditMitigationActionsTasksRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "audit_task_id" in input:
        params["auditTaskId"] = str(input["audit_task_id"])
    if "finding_id" in input:
        params["findingId"] = str(input["finding_id"])
    if "task_status" in input:
        params["taskStatus"] = str(input["task_status"])
