...
def build_request(
    ...
    input: aws_sdk_iot.types.test_invoke_authorizer_request.TestInvokeAuthorizerRequest,
) -> zapros.Request:
    ...
    url = url.replace("{authorizerName}", quote(str(input["authorizer_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.test_invoke_authorizer_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.test_invoke_authorizer_request.serialize_json(input)
    ).encode()
