"""Generated from Smithy shape ``com.amazonaws.iot#TestInvokeAuthorizer``."""
...
import aws_sdk_iot.errors.invalid_response_exception
...
import aws_sdk_iot.types.policy_documents
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "InvalidResponseException":
            raise aws_sdk_iot.errors.invalid_response_exception.InvalidResponseException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_iot.types.test_invoke_authorizer_response.TestInvokeAuthorizerResponse = aws_sdk_iot.types.test_invoke_authorizer_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iot.types.test_invoke_authorizer_request.TestInvokeAuthorizerRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/authorizer/{authorizerName}/test"
    url = url.replace(
        "{authorizerName}", quote(str(input_["authorizer_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_iot.types.test_invoke_authorizer_request.serialize_json(input_)
    ).encode()
