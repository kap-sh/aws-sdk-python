...
def build_request(
    ...
    input: aws_sdk_iot.types.create_role_alias_request.CreateRoleAliasRequest,
) -> zapros.Request:
    ...
    url = url.replace("{roleAlias}", quote(str(input["role_alias"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.create_role_alias_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.create_role_alias_request.serialize_json(input)
    ).encode()
