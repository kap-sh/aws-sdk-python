"""Generated from Smithy shape ``com.amazonaws.iot#CreateThingType``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.create_thing_type_response.CreateThingTypeResponse = (
        aws_sdk_iot.types.create_thing_type_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_iot.types.create_thing_type_request.CreateThingTypeRequest,
) -> zapros.Request:
    ...
    url = url.replace("{thingTypeName}", quote(str(input["thing_type_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.create_thing_type_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.create_thing_type_request.serialize_json(input)
    ).encode()
...
async def async_create_thing_type(
