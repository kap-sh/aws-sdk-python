...
def build_request(
    ...
    input: aws_sdk_iot.types.create_mitigation_action_request.CreateMitigationActionRequest,
) -> zapros.Request:
    ...
    url = url.replace("{actionName}", quote(str(input["action_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.create_mitigation_action_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.create_mitigation_action_request.serialize_json(input)
    ).encode()
