"""Generated from Smithy shape ``com.amazonaws.iot#CreateAuditSuppression``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.create_audit_suppression_response.CreateAuditSuppressionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iot.types.create_audit_suppression_request.CreateAuditSuppressionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/audit/suppressions/create"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.create_audit_suppression_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.create_audit_suppression_request.serialize_json(input_)
    ).encode()
...
async def async_create_audit_suppression(
