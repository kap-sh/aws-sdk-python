"""Generated from Smithy shape ``com.amazonaws.iot#AddThingToBillingGroup``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.add_thing_to_billing_group_response.AddThingToBillingGroupResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iot.types.add_thing_to_billing_group_request.AddThingToBillingGroupRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/billing-groups/addThingToBillingGroup"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.add_thing_to_billing_group_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.add_thing_to_billing_group_request.serialize_json(input_)
    ).encode()
...
async def async_add_thing_to_billing_group(
