...
def build_request(
    ...
    input: aws_sdk_iot.types.list_thing_groups_request.ListThingGroupsRequest,
) -> zapros.Request:
    ...
    if "parent_group" in input:
        params["parentGroup"] = str(input["parent_group"])
    if "name_prefix_filter" in input:
        params["namePrefixFilter"] = str(input["name_prefix_filter"])
    if "recursive" in input:
        params["recursive"] = str(input["recursive"])
