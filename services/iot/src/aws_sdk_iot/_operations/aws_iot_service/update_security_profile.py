"""Generated from Smithy shape ``com.amazonaws.iot#UpdateSecurityProfile``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "VersionConflictException":
            import aws_sdk_iot.errors.version_conflict_exception

            raise aws_sdk_iot.errors.version_conflict_exception.VersionConflictException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_iot.types.update_security_profile_response.UpdateSecurityProfileResponse = aws_sdk_iot.types.update_security_profile_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_iot.types.update_security_profile_request.UpdateSecurityProfileRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/security-profiles/{securityProfileName}"
    url = url.replace(
        "{securityProfileName}", quote(str(input["security_profile_name"]), safe="")
    )
    params: dict[str, str] = {}
    if "expected_version" in input:
        params["expectedVersion"] = str(input["expected_version"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.update_security_profile_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.update_security_profile_request.serialize_json(input)
    ).encode()
...
async def async_update_security_profile(
