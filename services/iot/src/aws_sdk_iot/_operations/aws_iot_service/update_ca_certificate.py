"""Generated from Smithy shape ``com.amazonaws.iot#UpdateCACertificate``."""
...
def build_request(
    ...
    input: aws_sdk_iot.types.update_ca_certificate_request.UpdateCACertificateRequest,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/cacertificate/{certificateId}"
    url = url.replace("{certificateId}", quote(str(input["certificate_id"]), safe=""))
    params: dict[str, str] = {}
    if "new_status" in input:
        params["newStatus"] = str(input["new_status"])
    if "new_auto_registration_status" in input:
        params["newAutoRegistrationStatus"] = str(input["new_auto_registration_status"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_iot.types.update_ca_certificate_request

    body: bytes | None = json.dumps(
        aws_sdk_iot.types.update_ca_certificate_request.serialize_json(input)
    ).encode()
...
async def async_update_ca_certificate(
