"""Generated from Smithy shape ``com.amazonaws.iot#AttachSecurityProfile``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.attach_security_profile_response.AttachSecurityProfileResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_iot.types.attach_security_profile_request.AttachSecurityProfileRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/security-profiles/{securityProfileName}/targets"
    url = url.replace(
        "{securityProfileName}", quote(str(input_["security_profile_name"]), safe="")
    )
    params: dict[str, str] = {}
    if "security_profile_target_arn" in input_:
        params["securityProfileTargetArn"] = str(input_["security_profile_target_arn"])
...
async def async_attach_security_profile(
