"""Generated from Smithy shape ``com.amazonaws.iot#DescribeJobTemplate``."""
...
def handle_response(
    ...
    out: aws_sdk_iot.types.describe_job_template_response.DescribeJobTemplateResponse = aws_sdk_iot.types.describe_job_template_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_iot.types.describe_job_template_request.DescribeJobTemplateRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/job-templates/{jobTemplateId}"
    url = url.replace("{jobTemplateId}", quote(str(input_["job_template_id"]), safe=""))
    params: dict[str, str] = {}
...
async def async_describe_job_template(
