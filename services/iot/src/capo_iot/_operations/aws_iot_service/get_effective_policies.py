"""Generated from Smithy shape ``com.amazonaws.iot#GetEffectivePolicies``."""
...
import capo_iot.types.get_effective_policies_request
import capo_iot.types.get_effective_policies_response
...
def handle_response(
    ...
    out: capo_iot.types.get_effective_policies_response.GetEffectivePoliciesResponse = (
        capo_iot.types.get_effective_policies_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/effective-policies"
    params: dict[str, str] = {}
    if "thing_name" in input_:
        params["thingName"] = str(input_["thing_name"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.get_effective_policies_request.serialize_json(input_)
    ).encode()
...
async def async_get_effective_policies(
