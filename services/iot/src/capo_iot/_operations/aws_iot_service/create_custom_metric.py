"""Generated from Smithy shape ``com.amazonaws.iot#CreateCustomMetric``."""
...
import capo_iot.types.create_custom_metric_request
import capo_iot.types.create_custom_metric_response
...
def handle_response(
    ...
    out: capo_iot.types.create_custom_metric_response.CreateCustomMetricResponse = (
        capo_iot.types.create_custom_metric_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/custom-metric/{metricName}"
    url = url.replace("{metricName}", quote(str(input_["metric_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.create_custom_metric_request.serialize_json(input_)
    ).encode()
...
async def async_create_custom_metric(
