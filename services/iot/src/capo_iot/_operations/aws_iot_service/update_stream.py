"""Generated from Smithy shape ``com.amazonaws.iot#UpdateStream``."""
...
import capo_iot.types.update_stream_request
import capo_iot.types.update_stream_response
...
def handle_response(
    ...
    out: capo_iot.types.update_stream_response.UpdateStreamResponse = (
        capo_iot.types.update_stream_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/streams/{streamId}"
    url = url.replace("{streamId}", quote(str(input_["stream_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_stream_request.serialize_json(input_)
    ).encode()
...
def update_stream(
    ...
    input_: capo_iot.types.update_stream_request.UpdateStreamRequest,
) -> tuple[capo_iot.types.update_stream_response.UpdateStreamResponse, zapros.Response]:
