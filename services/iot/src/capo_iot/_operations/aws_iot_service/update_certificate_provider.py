"""Generated from Smithy shape ``com.amazonaws.iot#UpdateCertificateProvider``."""
...
import capo_iot.types.update_certificate_provider_request
import capo_iot.types.update_certificate_provider_response
...
def handle_response(
    ...
    out: capo_iot.types.update_certificate_provider_response.UpdateCertificateProviderResponse = capo_iot.types.update_certificate_provider_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/certificate-providers/{certificateProviderName}"
    url = url.replace(
        "{certificateProviderName}",
        quote(str(input_["certificate_provider_name"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_certificate_provider_request.serialize_json(input_)
    ).encode()
...
async def async_update_certificate_provider(
