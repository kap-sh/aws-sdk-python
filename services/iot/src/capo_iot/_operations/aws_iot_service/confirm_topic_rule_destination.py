"""Generated from Smithy shape ``com.amazonaws.iot#ConfirmTopicRuleDestination``."""
...
import capo_iot.types.confirm_topic_rule_destination_request
import capo_iot.types.confirm_topic_rule_destination_response
...
def handle_response(
    ...
    out: capo_iot.types.confirm_topic_rule_destination_response.ConfirmTopicRuleDestinationResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/confirmdestination/{confirmationToken+}"
    url = url.replace(
        "{confirmationToken+}", quote(str(input_["confirmation_token"]), safe="/")
    )
    params: dict[str, str] = {}
...
async def async_confirm_topic_rule_destination(
