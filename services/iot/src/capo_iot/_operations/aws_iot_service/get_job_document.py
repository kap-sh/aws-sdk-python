"""Generated from Smithy shape ``com.amazonaws.iot#GetJobDocument``."""
...
import capo_iot.types.get_job_document_request
import capo_iot.types.get_job_document_response
...
def handle_response(
    ...
    out: capo_iot.types.get_job_document_response.GetJobDocumentResponse = (
        capo_iot.types.get_job_document_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/jobs/{jobId}/job-document"
    url = url.replace("{jobId}", quote(str(input_["job_id"]), safe=""))
    params: dict[str, str] = {}
    params["beforeSubstitution"] = str(input_.get("before_substitution", False))
...
def get_job_document(
    ...
    capo_iot.types.get_job_document_response.GetJobDocumentResponse, zapros.Response
]:
...
async def async_get_job_document(
