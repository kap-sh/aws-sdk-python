"""Generated from Smithy shape ``com.amazonaws.iot#RemoveThingFromBillingGroup``."""
...
import capo_iot.types.remove_thing_from_billing_group_request
import capo_iot.types.remove_thing_from_billing_group_response
...
def handle_response(
    ...
    out: capo_iot.types.remove_thing_from_billing_group_response.RemoveThingFromBillingGroupResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/billing-groups/removeThingFromBillingGroup"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.remove_thing_from_billing_group_request.serialize_json(input_)
    ).encode()
...
async def async_remove_thing_from_billing_group(
