"""Generated from Smithy shape ``com.amazonaws.iot#ListPolicyPrincipals``."""
...
import capo_iot.types.list_policy_principals_request
import capo_iot.types.list_policy_principals_response
import capo_iot.types.principals
...
def handle_response(
    ...
    out: capo_iot.types.list_policy_principals_response.ListPolicyPrincipalsResponse = (
        capo_iot.types.list_policy_principals_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/policy-principals"
    params: dict[str, str] = {}
    if "marker" in input_:
    ...
    if "policy_name" in input_:
        headers["x-amzn-iot-policy"] = str(input_["policy_name"])
    body: bytes | None = b""
...
async def async_list_policy_principals(
