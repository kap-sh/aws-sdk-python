"""Generated from Smithy shape ``com.amazonaws.iot#AddThingToThingGroup``."""
...
import capo_iot.types.add_thing_to_thing_group_request
import capo_iot.types.add_thing_to_thing_group_response
...
def handle_response(
    ...
    out: capo_iot.types.add_thing_to_thing_group_response.AddThingToThingGroupResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/thing-groups/addThingToThingGroup"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.add_thing_to_thing_group_request.serialize_json(input_)
    ).encode()
...
async def async_add_thing_to_thing_group(
