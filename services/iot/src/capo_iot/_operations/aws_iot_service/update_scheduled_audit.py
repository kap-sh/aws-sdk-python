"""Generated from Smithy shape ``com.amazonaws.iot#UpdateScheduledAudit``."""
...
import capo_iot.types.update_scheduled_audit_request
import capo_iot.types.update_scheduled_audit_response
...
def handle_response(
    ...
    out: capo_iot.types.update_scheduled_audit_response.UpdateScheduledAuditResponse = (
        capo_iot.types.update_scheduled_audit_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/audit/scheduledaudits/{scheduledAuditName}"
    url = url.replace(
        "{scheduledAuditName}", quote(str(input_["scheduled_audit_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_scheduled_audit_request.serialize_json(input_)
    ).encode()
...
async def async_update_scheduled_audit(
