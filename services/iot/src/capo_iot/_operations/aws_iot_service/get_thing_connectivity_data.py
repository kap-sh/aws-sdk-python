"""Generated from Smithy shape ``com.amazonaws.iot#GetThingConnectivityData``."""
...
import capo_iot.types.get_thing_connectivity_data_request
import capo_iot.types.get_thing_connectivity_data_response
...
def handle_response(
    ...
    capo_iot.types.get_thing_connectivity_data_response.GetThingConnectivityDataResponse
):
    out: capo_iot.types.get_thing_connectivity_data_response.GetThingConnectivityDataResponse = capo_iot.types.get_thing_connectivity_data_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/things/{thingName}/connectivity-data"
    url = url.replace("{thingName}", quote(str(input_["thing_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.get_thing_connectivity_data_request.serialize_json(input_)
    ).encode()
...
async def async_get_thing_connectivity_data(
