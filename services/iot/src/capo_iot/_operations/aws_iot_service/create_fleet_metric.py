"""Generated from Smithy shape ``com.amazonaws.iot#CreateFleetMetric``."""
...
import capo_iot.errors.index_not_ready_exception
import capo_iot.errors.internal_failure_exception
import capo_iot.errors.invalid_aggregation_exception
import capo_iot.errors.invalid_query_exception
...
import capo_iot.types.create_fleet_metric_request
import capo_iot.types.create_fleet_metric_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "IndexNotReadyException":
            raise capo_iot.errors.index_not_ready_exception.IndexNotReadyException.from_json(
                data
            )
        ...
        case "InvalidAggregationException":
            raise capo_iot.errors.invalid_aggregation_exception.InvalidAggregationException.from_json(
                data
            )
        case "InvalidQueryException":
            raise capo_iot.errors.invalid_query_exception.InvalidQueryException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_iot.types.create_fleet_metric_response.CreateFleetMetricResponse = (
        capo_iot.types.create_fleet_metric_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/fleet-metric/{metricName}"
    url = url.replace("{metricName}", quote(str(input_["metric_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.create_fleet_metric_request.serialize_json(input_)
    ).encode()
...
async def async_create_fleet_metric(
