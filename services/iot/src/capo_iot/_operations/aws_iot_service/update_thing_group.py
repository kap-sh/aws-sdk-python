"""Generated from Smithy shape ``com.amazonaws.iot#UpdateThingGroup``."""
...
import capo_iot.types.update_thing_group_request
import capo_iot.types.update_thing_group_response
...
def handle_response(
    ...
    out: capo_iot.types.update_thing_group_response.UpdateThingGroupResponse = (
        capo_iot.types.update_thing_group_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/thing-groups/{thingGroupName}"
    url = url.replace(
        "{thingGroupName}", quote(str(input_["thing_group_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_thing_group_request.serialize_json(input_)
    ).encode()
...
def update_thing_group(
    ...
    capo_iot.types.update_thing_group_response.UpdateThingGroupResponse, zapros.Response
]:
...
async def async_update_thing_group(
