...
import capo_iot.errors.conflict_exception
import capo_iot.errors.internal_server_exception
import capo_iot.errors.service_quota_exceeded_exception
...
import capo_iot.types.create_command_request
import capo_iot.types.create_command_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "ConflictException":
            raise capo_iot.errors.conflict_exception.ConflictException.from_json(data)
        case "InternalServerException":
            raise capo_iot.errors.internal_server_exception.InternalServerException.from_json(
                data
            )
        case "ServiceQuotaExceededException":
            raise capo_iot.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
) -> capo_iot.types.create_command_response.CreateCommandResponse:
    out: capo_iot.types.create_command_response.CreateCommandResponse = (
        capo_iot.types.create_command_response.deserialize_json(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_iot.types.create_command_request.serialize_json(input_)
    ).encode()
...
def create_command(
    ...
    capo_iot.types.create_command_response.CreateCommandResponse, zapros.Response
]:
