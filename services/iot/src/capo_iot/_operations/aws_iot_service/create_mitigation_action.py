"""Generated from Smithy shape ``com.amazonaws.iot#CreateMitigationAction``."""
...
import capo_iot.types.create_mitigation_action_request
import capo_iot.types.create_mitigation_action_response
...
def handle_response(
    ...
    out: capo_iot.types.create_mitigation_action_response.CreateMitigationActionResponse = capo_iot.types.create_mitigation_action_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/mitigationactions/actions/{actionName}"
    url = url.replace("{actionName}", quote(str(input_["action_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.create_mitigation_action_request.serialize_json(input_)
    ).encode()
...
async def async_create_mitigation_action(
