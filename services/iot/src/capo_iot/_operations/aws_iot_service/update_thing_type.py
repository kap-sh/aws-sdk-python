"""Generated from Smithy shape ``com.amazonaws.iot#UpdateThingType``."""
...
import capo_iot.types.update_thing_type_request
import capo_iot.types.update_thing_type_response
...
) -> capo_iot.types.update_thing_type_response.UpdateThingTypeResponse:
    out: capo_iot.types.update_thing_type_response.UpdateThingTypeResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/thing-types/{thingTypeName}"
    url = url.replace("{thingTypeName}", quote(str(input_["thing_type_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_thing_type_request.serialize_json(input_)
    ).encode()
...
def update_thing_type(
    ...
    capo_iot.types.update_thing_type_response.UpdateThingTypeResponse, zapros.Response
]:
...
async def async_update_thing_type(
