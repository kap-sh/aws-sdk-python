"""Generated from Smithy shape ``com.amazonaws.iot#UpdateBillingGroup``."""
...
import capo_iot.types.update_billing_group_request
import capo_iot.types.update_billing_group_response
...
def handle_response(
    ...
    out: capo_iot.types.update_billing_group_response.UpdateBillingGroupResponse = (
        capo_iot.types.update_billing_group_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/billing-groups/{billingGroupName}"
    url = url.replace(
        "{billingGroupName}", quote(str(input_["billing_group_name"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_billing_group_request.serialize_json(input_)
    ).encode()
...
async def async_update_billing_group(
