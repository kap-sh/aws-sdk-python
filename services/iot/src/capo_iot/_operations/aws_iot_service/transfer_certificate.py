"""Generated from Smithy shape ``com.amazonaws.iot#TransferCertificate``."""
...
import capo_iot.errors.transfer_conflict_exception
import capo_iot.errors.unauthorized_exception
import capo_iot.types.transfer_certificate_request
import capo_iot.types.transfer_certificate_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "TransferConflictException":
            raise capo_iot.errors.transfer_conflict_exception.TransferConflictException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_iot.types.transfer_certificate_response.TransferCertificateResponse = (
        capo_iot.types.transfer_certificate_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/transfer-certificate/{certificateId}"
    url = url.replace("{certificateId}", quote(str(input_["certificate_id"]), safe=""))
    params: dict[str, str] = {}
    if "target_aws_account" in input_:
        params["targetAwsAccount"] = str(input_["target_aws_account"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.transfer_certificate_request.serialize_json(input_)
    ).encode()
...
async def async_transfer_certificate(
