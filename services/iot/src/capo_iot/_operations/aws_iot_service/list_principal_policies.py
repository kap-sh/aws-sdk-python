"""Generated from Smithy shape ``com.amazonaws.iot#ListPrincipalPolicies``."""
...
import capo_iot.types.list_principal_policies_request
import capo_iot.types.list_principal_policies_response
...
def handle_response(
    ...
    out: capo_iot.types.list_principal_policies_response.ListPrincipalPoliciesResponse = capo_iot.types.list_principal_policies_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/principal-policies"
    params: dict[str, str] = {}
    if "marker" in input_:
    ...
    if "principal" in input_:
        headers["x-amzn-iot-principal"] = str(input_["principal"])
    body: bytes | None = b""
...
async def async_list_principal_policies(
