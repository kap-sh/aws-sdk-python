"""Generated from Smithy shape ``com.amazonaws.iot#StartDetectMitigationActionsTask``."""
...
import capo_iot.errors.task_already_exists_exception
...
import capo_iot.types.start_detect_mitigation_actions_task_request
import capo_iot.types.start_detect_mitigation_actions_task_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "TaskAlreadyExistsException":
            raise capo_iot.errors.task_already_exists_exception.TaskAlreadyExistsException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_iot.types.start_detect_mitigation_actions_task_response.StartDetectMitigationActionsTaskResponse = capo_iot.types.start_detect_mitigation_actions_task_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/detect/mitigationactions/tasks/{taskId}"
    url = url.replace("{taskId}", quote(str(input_["task_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.start_detect_mitigation_actions_task_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_detect_mitigation_actions_task(
