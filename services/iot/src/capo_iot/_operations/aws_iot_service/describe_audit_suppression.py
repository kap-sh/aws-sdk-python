"""Generated from Smithy shape ``com.amazonaws.iot#DescribeAuditSuppression``."""
...
import capo_iot.types.describe_audit_suppression_request
import capo_iot.types.describe_audit_suppression_response
...
def handle_response(
    ...
    capo_iot.types.describe_audit_suppression_response.DescribeAuditSuppressionResponse
):
    out: capo_iot.types.describe_audit_suppression_response.DescribeAuditSuppressionResponse = capo_iot.types.describe_audit_suppression_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/audit/suppressions/describe"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.describe_audit_suppression_request.serialize_json(input_)
    ).encode()
...
async def async_describe_audit_suppression(
