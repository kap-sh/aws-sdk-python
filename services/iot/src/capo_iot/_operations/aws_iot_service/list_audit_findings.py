"""Generated from Smithy shape ``com.amazonaws.iot#ListAuditFindings``."""
...
import capo_iot.types.list_audit_findings_request
import capo_iot.types.list_audit_findings_response
...
def handle_response(
    ...
    out: capo_iot.types.list_audit_findings_response.ListAuditFindingsResponse = (
        capo_iot.types.list_audit_findings_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/audit/findings"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.list_audit_findings_request.serialize_json(input_)
    ).encode()
