"""Generated from Smithy shape ``com.amazonaws.iot#CreateProvisioningTemplateVersion``."""
...
import capo_iot.errors.versions_limit_exceeded_exception
import capo_iot.types.create_provisioning_template_version_request
import capo_iot.types.create_provisioning_template_version_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "VersionsLimitExceededException":
            raise capo_iot.errors.versions_limit_exceeded_exception.VersionsLimitExceededException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_iot.types.create_provisioning_template_version_response.CreateProvisioningTemplateVersionResponse = capo_iot.types.create_provisioning_template_version_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/provisioning-templates/{templateName}/versions"
    url = url.replace("{templateName}", quote(str(input_["template_name"]), safe=""))
    params: dict[str, str] = {}
    params["setAsDefault"] = str(input_.get("set_as_default", False))
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.create_provisioning_template_version_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_provisioning_template_version(
