"""Generated from Smithy shape ``com.amazonaws.iot#ListThingGroups``."""
...
import capo_iot.types.list_thing_groups_request
import capo_iot.types.list_thing_groups_response
...
def handle_response(
    ...
    out: capo_iot.types.list_thing_groups_response.ListThingGroupsResponse = (
        capo_iot.types.list_thing_groups_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/thing-groups"
    params: dict[str, str] = {}
    if "next_token" in input_:
        params["nextToken"] = str(input_["next_token"])
    if "max_results" in input_:
        params["maxResults"] = str(input_["max_results"])
    if "parent_group" in input_:
        params["parentGroup"] = str(input_["parent_group"])
    if "name_prefix_filter" in input_:
        params["namePrefixFilter"] = str(input_["name_prefix_filter"])
    if "recursive" in input_:
        params["recursive"] = str(input_["recursive"])
...
def list_thing_groups(
    ...
    capo_iot.types.list_thing_groups_response.ListThingGroupsResponse, zapros.Response
]:
...
async def async_list_thing_groups(
