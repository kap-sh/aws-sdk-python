"""Generated from Smithy shape ``com.amazonaws.iot#DeleteAuditSuppression``."""
...
import capo_iot.types.delete_audit_suppression_request
import capo_iot.types.delete_audit_suppression_response
...
def handle_response(
    ...
    out: capo_iot.types.delete_audit_suppression_response.DeleteAuditSuppressionResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/audit/suppressions/delete"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.delete_audit_suppression_request.serialize_json(input_)
    ).encode()
...
async def async_delete_audit_suppression(
