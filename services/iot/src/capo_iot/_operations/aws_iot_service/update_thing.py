"""Generated from Smithy shape ``com.amazonaws.iot#UpdateThing``."""
...
import capo_iot.types.update_thing_request
import capo_iot.types.update_thing_response
...
def handle_response(
    ...
    out: capo_iot.types.update_thing_response.UpdateThingResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/things/{thingName}"
    url = url.replace("{thingName}", quote(str(input_["thing_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.update_thing_request.serialize_json(input_)
    ).encode()
...
def update_thing(
    ...
    input_: capo_iot.types.update_thing_request.UpdateThingRequest,
) -> tuple[capo_iot.types.update_thing_response.UpdateThingResponse, zapros.Response]:
