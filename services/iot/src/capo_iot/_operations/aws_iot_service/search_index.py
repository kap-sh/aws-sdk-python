"""Generated from Smithy shape ``com.amazonaws.iot#SearchIndex``."""
...
import capo_iot.types.search_index_request
import capo_iot.types.search_index_response
...
def handle_response(
    ...
    out: capo_iot.types.search_index_response.SearchIndexResponse = (
        capo_iot.types.search_index_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/indices/search"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.search_index_request.serialize_json(input_)
    ).encode()
...
def search_index(
    ...
    input_: capo_iot.types.search_index_request.SearchIndexRequest,
) -> tuple[capo_iot.types.search_index_response.SearchIndexResponse, zapros.Response]:
