"""Generated from Smithy shape ``com.amazonaws.iot#RegisterThing``."""
...
import capo_iot.errors.resource_registration_failure_exception
...
import capo_iot.types.register_thing_request
import capo_iot.types.register_thing_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceRegistrationFailureException":
            raise capo_iot.errors.resource_registration_failure_exception.ResourceRegistrationFailureException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_iot.types.register_thing_response.RegisterThingResponse = (
        capo_iot.types.register_thing_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/things"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.register_thing_request.serialize_json(input_)
    ).encode()
...
def register_thing(
    ...
    capo_iot.types.register_thing_response.RegisterThingResponse, zapros.Response
]:
...
async def async_register_thing(
