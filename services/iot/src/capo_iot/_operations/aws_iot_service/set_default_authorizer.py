"""Generated from Smithy shape ``com.amazonaws.iot#SetDefaultAuthorizer``."""
...
import capo_iot.types.set_default_authorizer_request
import capo_iot.types.set_default_authorizer_response
...
def handle_response(
    ...
    out: capo_iot.types.set_default_authorizer_response.SetDefaultAuthorizerResponse = (
        capo_iot.types.set_default_authorizer_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/default-authorizer"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_iot.types.set_default_authorizer_request.serialize_json(input_)
    ).encode()
...
async def async_set_default_authorizer(
