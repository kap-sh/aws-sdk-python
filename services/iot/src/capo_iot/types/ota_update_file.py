...
if TYPE_CHECKING:
    ...
    import capo_iot.types.file_type
    import capo_iot.types.ota_update_file_version


class OTAUpdateFile(TypedDict, closed=True):
    file_name: NotRequired["capo_iot.types.file_name.FileName"]
    """<p>The name of the file.</p>"""
    file_type: NotRequired["capo_iot.types.file_type.FileType"]
    ...
    file_version: NotRequired[
        "capo_iot.types.ota_update_file_version.OTAUpdateFileVersion"
    ]
    """<p>The file version.</p>"""
    file_location: NotRequired["capo_iot.types.file_location.FileLocation"]
    """<p>The location of the updated firmware.</p>"""
    code_signing: NotRequired["capo_iot.types.code_signing.CodeSigning"]
    """<p>The code signing method of the file.</p>"""
    attributes: NotRequired["capo_iot.types.attributes_map.AttributesMap"]
...
def serialize_json(value: OTAUpdateFile) -> dict:
    ...
    if "file_location" in value:
        ...
        out["fileLocation"] = capo_iot.types.file_location.serialize_json(
            value["file_location"]
        )
    if "code_signing" in value:
        import capo_iot.types.code_signing

        out["codeSigning"] = capo_iot.types.code_signing.serialize_json(
            value["code_signing"]
        )
    if "attributes" in value:
        import capo_iot.types.attributes_map

        out["attributes"] = capo_iot.types.attributes_map.serialize_json(
            value["attributes"]
        )
    return out
