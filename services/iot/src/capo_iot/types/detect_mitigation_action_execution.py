...
if TYPE_CHECKING:
    import capo_iot.types.detect_mitigation_action_execution_error_code
    import capo_iot.types.detect_mitigation_action_execution_status
...
class DetectMitigationActionExecution(TypedDict, closed=True):
    ...
    execution_start_date: NotRequired["capo_iot.types.timestamp.Timestamp"]
    """<p> The date a mitigation action was started. </p>"""
    execution_end_date: NotRequired["capo_iot.types.timestamp.Timestamp"]
    """<p> The date a mitigation action ended. </p>"""
    status: NotRequired[
        "capo_iot.types.detect_mitigation_action_execution_status.DetectMitigationActionExecutionStatus"
    ]
    """<p> The status of a mitigation action. </p>"""
    error_code: NotRequired[
        "capo_iot.types.detect_mitigation_action_execution_error_code.DetectMitigationActionExecutionErrorCode"
    ]
...
def serialize_json(value: DetectMitigationActionExecution) -> dict:
    ...
    if "execution_start_date" in value:
        ...
        out["executionStartDate"] = capo_iot.types.timestamp.serialize_json(
            value["execution_start_date"]
        )
    if "execution_end_date" in value:
        import capo_iot.types.timestamp

        out["executionEndDate"] = capo_iot.types.timestamp.serialize_json(
    ...
    if "status" in value:
        ...
        out["status"] = (
            capo_iot.types.detect_mitigation_action_execution_status.serialize_json(
                value["status"]
            )
        )
...
def deserialize_json(data: dict) -> DetectMitigationActionExecution:
    ...
    if "executionStartDate" in data:
        ...
        out["execution_start_date"] = capo_iot.types.timestamp.deserialize_json(
            data["executionStartDate"]
        )
    if "executionEndDate" in data:
        import capo_iot.types.timestamp

        out["execution_end_date"] = capo_iot.types.timestamp.deserialize_json(
    ...
    if "status" in data:
        ...
        out["status"] = (
            capo_iot.types.detect_mitigation_action_execution_status.deserialize_json(
                data["status"]
            )
        )
