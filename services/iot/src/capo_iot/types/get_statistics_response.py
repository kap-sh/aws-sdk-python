...
class GetStatisticsResponse(TypedDict, closed=True):
    statistics: NotRequired["capo_iot.types.statistics.Statistics"]
...
def serialize_json(value: GetStatisticsResponse) -> dict:
    ...
    if "statistics" in value:
        ...
        out["statistics"] = capo_iot.types.statistics.serialize_json(
            value["statistics"]
        )
    return out
...
def deserialize_json(data: dict) -> GetStatisticsResponse:
    ...
    if "statistics" in data:
        ...
        out["statistics"] = capo_iot.types.statistics.deserialize_json(
            data["statistics"]
        )
    return out
