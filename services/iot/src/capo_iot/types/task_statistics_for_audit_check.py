...
if TYPE_CHECKING:
    import capo_iot.types.canceled_findings_count
    import capo_iot.types.failed_findings_count
    import capo_iot.types.skipped_findings_count
    import capo_iot.types.succeeded_findings_count
    import capo_iot.types.total_findings_count


class TaskStatisticsForAuditCheck(TypedDict, closed=True):
    total_findings_count: NotRequired[
        "capo_iot.types.total_findings_count.TotalFindingsCount"
    ]
    """<p>The total number of findings to which a task is being applied.</p>"""
    failed_findings_count: NotRequired[
        "capo_iot.types.failed_findings_count.FailedFindingsCount"
    ]
    """<p>The number of findings for which at least one of the actions failed when applied.</p>"""
    succeeded_findings_count: NotRequired[
        "capo_iot.types.succeeded_findings_count.SucceededFindingsCount"
    ]
    """<p>The number of findings for which all mitigation actions succeeded when applied.</p>"""
    skipped_findings_count: NotRequired[
        "capo_iot.types.skipped_findings_count.SkippedFindingsCount"
    ]
    ...
    canceled_findings_count: NotRequired[
        "capo_iot.types.canceled_findings_count.CanceledFindingsCount"
    ]
