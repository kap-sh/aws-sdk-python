...
class JobExecutionStatusDetails(TypedDict, closed=True):
    details_map: NotRequired["capo_iot.types.details_map.DetailsMap"]
...
def serialize_json(value: JobExecutionStatusDetails) -> dict:
    ...
    if "details_map" in value:
        ...
        out["detailsMap"] = capo_iot.types.details_map.serialize_json(
            value["details_map"]
        )
    return out
...
def deserialize_json(data: dict) -> JobExecutionStatusDetails:
    ...
    if "detailsMap" in data:
        ...
        out["details_map"] = capo_iot.types.details_map.deserialize_json(
            data["detailsMap"]
        )
    return out
