...
if TYPE_CHECKING:
    ...
    import capo_iot.types.ota_updates_summary


class ListOTAUpdatesResponse(TypedDict, closed=True):
    ota_updates: NotRequired["capo_iot.types.ota_updates_summary.OTAUpdatesSummary"]
...
def serialize_json(value: ListOTAUpdatesResponse) -> dict:
    ...
    if "ota_updates" in value:
        ...
        out["otaUpdates"] = capo_iot.types.ota_updates_summary.serialize_json(
            value["ota_updates"]
        )
...
def deserialize_json(data: dict) -> ListOTAUpdatesResponse:
    ...
    if "otaUpdates" in data:
        ...
        out["ota_updates"] = capo_iot.types.ota_updates_summary.deserialize_json(
            data["otaUpdates"]
        )
