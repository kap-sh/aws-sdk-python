...
class ListDetectMitigationActionsTasksResponse(TypedDict, closed=True):
    tasks: NotRequired[
        "capo_iot.types.detect_mitigation_actions_task_summary_list.DetectMitigationActionsTaskSummaryList"
    ]
...
def serialize_json(value: ListDetectMitigationActionsTasksResponse) -> dict:
    ...
    if "tasks" in value:
        ...
        out["tasks"] = (
            capo_iot.types.detect_mitigation_actions_task_summary_list.serialize_json(
                value["tasks"]
            )
        )
...
def deserialize_json(data: dict) -> ListDetectMitigationActionsTasksResponse:
    ...
    if "tasks" in data:
        ...
        out["tasks"] = (
            capo_iot.types.detect_mitigation_actions_task_summary_list.deserialize_json(
                data["tasks"]
            )
        )
