...
if TYPE_CHECKING:
    ...
    import capo_iot.types.data_collection_percentage
...
class BehaviorModelTrainingSummary(TypedDict, closed=True):
    ...
    training_data_collection_start_date: NotRequired[
        "capo_iot.types.timestamp.Timestamp"
    ]
    """<p> The date a training model started collecting data. </p>"""
    model_status: NotRequired["capo_iot.types.model_status.ModelStatus"]
    """<p> The status of the behavior model. </p>"""
    datapoints_collection_percentage: NotRequired[
        "capo_iot.types.data_collection_percentage.DataCollectionPercentage"
    ]
    """<p> The percentage of datapoints collected. </p>"""
    last_model_refresh_date: NotRequired["capo_iot.types.timestamp.Timestamp"]
...
def serialize_json(value: BehaviorModelTrainingSummary) -> dict:
    ...
    if "training_data_collection_start_date" in value:
        ...
        out["trainingDataCollectionStartDate"] = (
            capo_iot.types.timestamp.serialize_json(
    ...
    if "model_status" in value:
        ...
        out["modelStatus"] = capo_iot.types.model_status.serialize_json(
            value["model_status"]
        )
    ...
    if "last_model_refresh_date" in value:
        ...
        out["lastModelRefreshDate"] = capo_iot.types.timestamp.serialize_json(
...
def deserialize_json(data: dict) -> BehaviorModelTrainingSummary:
    ...
    if "trainingDataCollectionStartDate" in data:
        ...
        out["training_data_collection_start_date"] = (
            capo_iot.types.timestamp.deserialize_json(
