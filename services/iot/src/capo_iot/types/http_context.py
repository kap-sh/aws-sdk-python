...
if TYPE_CHECKING:
    ...
    import capo_iot.types.http_query_string


class HttpContext(TypedDict, closed=True):
    headers: NotRequired["capo_iot.types.http_headers.HttpHeaders"]
    """<p>The header keys and values in an HTTP authorization request.</p>"""
    query_string: NotRequired["capo_iot.types.http_query_string.HttpQueryString"]
...
def serialize_json(value: HttpContext) -> dict:
    ...
    if "headers" in value:
        ...
        out["headers"] = capo_iot.types.http_headers.serialize_json(value["headers"])
