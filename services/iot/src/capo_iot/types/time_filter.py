...
class TimeFilter(TypedDict, closed=True):
    after: NotRequired["capo_iot.types.string_date_time.StringDateTime"]
    """<p>Filter to display command executions that started or completed only after a particular date and time.</p>"""
    before: NotRequired["capo_iot.types.string_date_time.StringDateTime"]
