...
class ActiveViolation(TypedDict, closed=True):
    ...
    last_violation_value: NotRequired["capo_iot.types.metric_value.MetricValue"]
    ...
    last_violation_time: NotRequired["capo_iot.types.timestamp.Timestamp"]
    """<p>The time the most recent violation occurred.</p>"""
    violation_start_time: NotRequired["capo_iot.types.timestamp.Timestamp"]
...
def serialize_json(value: ActiveViolation) -> dict:
    ...
    if "last_violation_value" in value:
        ...
        out["lastViolationValue"] = capo_iot.types.metric_value.serialize_json(
    ...
    if "last_violation_time" in value:
        ...
        out["lastViolationTime"] = capo_iot.types.timestamp.serialize_json(
            value["last_violation_time"]
        )
    if "violation_start_time" in value:
        import capo_iot.types.timestamp

        out["violationStartTime"] = capo_iot.types.timestamp.serialize_json(
...
def deserialize_json(data: dict) -> ActiveViolation:
    ...
    if "lastViolationValue" in data:
        ...
        out["last_violation_value"] = capo_iot.types.metric_value.deserialize_json(
    ...
    if "lastViolationTime" in data:
        ...
        out["last_violation_time"] = capo_iot.types.timestamp.deserialize_json(
            data["lastViolationTime"]
        )
    if "violationStartTime" in data:
        import capo_iot.types.timestamp

        out["violation_start_time"] = capo_iot.types.timestamp.deserialize_json(
