"""Generated from Smithy shape ``com.amazonaws.resiliencehub#CreateAppVersionResourceResponse``."""
...
class CreateAppVersionResourceResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: CreateAppVersionResourceResponse) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> CreateAppVersionResourceResponse:
    out: CreateAppVersionResourceResponse = {}  # type: ignore[typeddict-item]
    if "appArn" in data:
        out["app_arn"] = data["appArn"]
    else:
        raise DeserializationError("CreateAppVersionResourceResponse.app_arn required")
    if "appVersion" in data:
        out["app_version"] = data["appVersion"]
    else:
        raise DeserializationError(
            "CreateAppVersionResourceResponse.app_version required"
        )
