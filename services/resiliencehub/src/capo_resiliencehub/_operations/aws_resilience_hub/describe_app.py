"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeApp``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.describe_app_response.DescribeAppResponse = (
        capo_resiliencehub.types.describe_app_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/describe-app"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.describe_app_request.serialize_json(input_)
    ).encode()
...
def describe_app(
    ...
    capo_resiliencehub.types.describe_app_response.DescribeAppResponse, zapros.Response
]:
