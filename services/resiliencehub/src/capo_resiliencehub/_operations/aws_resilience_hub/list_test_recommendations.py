"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListTestRecommendations``."""
...
import capo_resiliencehub.types.test_recommendation_list
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_test_recommendations_response.ListTestRecommendationsResponse = capo_resiliencehub.types.list_test_recommendations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-test-recommendations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_test_recommendations_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_test_recommendations(
