"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListAlarmRecommendations``."""
...
import capo_resiliencehub.types.alarm_recommendation_list
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_alarm_recommendations_response.ListAlarmRecommendationsResponse = capo_resiliencehub.types.list_alarm_recommendations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-alarm-recommendations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_alarm_recommendations_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_alarm_recommendations(
