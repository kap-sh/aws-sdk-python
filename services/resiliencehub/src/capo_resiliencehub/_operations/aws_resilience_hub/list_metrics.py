"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListMetrics``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_metrics_response.ListMetricsResponse = (
        capo_resiliencehub.types.list_metrics_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-metrics"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_metrics_request.serialize_json(input_)
    ).encode()
...
def list_metrics(
    ...
    capo_resiliencehub.types.list_metrics_response.ListMetricsResponse, zapros.Response
]:
