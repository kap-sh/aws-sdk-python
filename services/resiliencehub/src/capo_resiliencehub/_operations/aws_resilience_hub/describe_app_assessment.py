"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeAppAssessment``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.describe_app_assessment_response.DescribeAppAssessmentResponse = capo_resiliencehub.types.describe_app_assessment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/describe-app-assessment"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.describe_app_assessment_request.serialize_json(input_)
    ).encode()
...
async def async_describe_app_assessment(
