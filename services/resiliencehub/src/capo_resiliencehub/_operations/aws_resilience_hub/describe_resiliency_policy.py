"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeResiliencyPolicy``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.describe_resiliency_policy_response.DescribeResiliencyPolicyResponse = capo_resiliencehub.types.describe_resiliency_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/describe-resiliency-policy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.describe_resiliency_policy_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_describe_resiliency_policy(
