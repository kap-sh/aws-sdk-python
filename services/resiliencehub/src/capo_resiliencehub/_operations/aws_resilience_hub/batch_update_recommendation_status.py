"""Generated from Smithy shape ``com.amazonaws.resiliencehub#BatchUpdateRecommendationStatus``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.batch_update_recommendation_status_response.BatchUpdateRecommendationStatusResponse = capo_resiliencehub.types.batch_update_recommendation_status_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/batch-update-recommendation-status"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.batch_update_recommendation_status_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_batch_update_recommendation_status(
