"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListAppVersionAppComponents``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_app_version_app_components_response.ListAppVersionAppComponentsResponse = capo_resiliencehub.types.list_app_version_app_components_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-app-version-app-components"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_app_version_app_components_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_list_app_version_app_components(
