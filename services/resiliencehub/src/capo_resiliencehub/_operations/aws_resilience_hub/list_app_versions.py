"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListAppVersions``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_app_versions_response.ListAppVersionsResponse = (
        capo_resiliencehub.types.list_app_versions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-app-versions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_app_versions_request.serialize_json(input_)
    ).encode()
...
async def async_list_app_versions(
