"""Generated from Smithy shape ``com.amazonaws.resiliencehub#PutDraftAppVersionTemplate``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.put_draft_app_version_template_response.PutDraftAppVersionTemplateResponse = capo_resiliencehub.types.put_draft_app_version_template_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/put-draft-app-version-template"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.put_draft_app_version_template_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_draft_app_version_template(
