"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DeleteResiliencyPolicy``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.delete_resiliency_policy_response.DeleteResiliencyPolicyResponse = capo_resiliencehub.types.delete_resiliency_policy_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-resiliency-policy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.delete_resiliency_policy_request.serialize_json(input_)
    ).encode()
...
async def async_delete_resiliency_policy(
