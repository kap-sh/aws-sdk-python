"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DeleteAppAssessment``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.delete_app_assessment_response.DeleteAppAssessmentResponse = capo_resiliencehub.types.delete_app_assessment_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-app-assessment"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.delete_app_assessment_request.serialize_json(input_)
    ).encode()
...
async def async_delete_app_assessment(
