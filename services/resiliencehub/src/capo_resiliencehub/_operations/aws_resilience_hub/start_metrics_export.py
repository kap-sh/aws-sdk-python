"""Generated from Smithy shape ``com.amazonaws.resiliencehub#StartMetricsExport``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.start_metrics_export_response.StartMetricsExportResponse = capo_resiliencehub.types.start_metrics_export_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/start-metrics-export"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.start_metrics_export_request.serialize_json(input_)
    ).encode()
...
async def async_start_metrics_export(
