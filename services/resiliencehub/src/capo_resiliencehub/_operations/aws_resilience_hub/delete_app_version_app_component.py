"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DeleteAppVersionAppComponent``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.delete_app_version_app_component_response.DeleteAppVersionAppComponentResponse = capo_resiliencehub.types.delete_app_version_app_component_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-app-version-app-component"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.delete_app_version_app_component_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_app_version_app_component(
