"""Generated from Smithy shape ``com.amazonaws.resiliencehub#UpdateAppVersionAppComponent``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.update_app_version_app_component_response.UpdateAppVersionAppComponentResponse = capo_resiliencehub.types.update_app_version_app_component_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/update-app-version-app-component"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.update_app_version_app_component_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_app_version_app_component(
