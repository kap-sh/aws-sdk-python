"""Generated from Smithy shape ``com.amazonaws.resiliencehub#ListSopRecommendations``."""
...
def handle_response(
    ...
    out: capo_resiliencehub.types.list_sop_recommendations_response.ListSopRecommendationsResponse = capo_resiliencehub.types.list_sop_recommendations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/list-sop-recommendations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_resiliencehub.types.list_sop_recommendations_request.serialize_json(input_)
    ).encode()
...
async def async_list_sop_recommendations(
