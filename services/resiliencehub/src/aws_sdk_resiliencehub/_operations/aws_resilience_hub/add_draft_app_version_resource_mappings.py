"""Generated from Smithy shape ``com.amazonaws.resiliencehub#AddDraftAppVersionResourceMappings``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ServiceQuotaExceededException":
            import aws_sdk_resiliencehub.errors.service_quota_exceeded_exception

            raise aws_sdk_resiliencehub.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_resiliencehub.types.add_draft_app_version_resource_mappings_response.AddDraftAppVersionResourceMappingsResponse = aws_sdk_resiliencehub.types.add_draft_app_version_resource_mappings_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehub.types.add_draft_app_version_resource_mappings_request.AddDraftAppVersionResourceMappingsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/add-draft-app-version-resource-mappings"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_resiliencehub.types.add_draft_app_version_resource_mappings_request

    body: bytes | None = json.dumps(
        aws_sdk_resiliencehub.types.add_draft_app_version_resource_mappings_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_add_draft_app_version_resource_mappings(
