"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeResourceGroupingRecommendationTask``."""
...
def handle_response(
    ...
    out: aws_sdk_resiliencehub.types.describe_resource_grouping_recommendation_task_response.DescribeResourceGroupingRecommendationTaskResponse = aws_sdk_resiliencehub.types.describe_resource_grouping_recommendation_task_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehub.types.describe_resource_grouping_recommendation_task_request.DescribeResourceGroupingRecommendationTaskRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/describe-resource-grouping-recommendation-task"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_resiliencehub.types.describe_resource_grouping_recommendation_task_request

    body: bytes | None = json.dumps(
        aws_sdk_resiliencehub.types.describe_resource_grouping_recommendation_task_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_describe_resource_grouping_recommendation_task(
