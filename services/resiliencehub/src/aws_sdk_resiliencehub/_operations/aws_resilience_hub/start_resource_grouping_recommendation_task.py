"""Generated from Smithy shape ``com.amazonaws.resiliencehub#StartResourceGroupingRecommendationTask``."""
...
def handle_response(
    ...
    out: aws_sdk_resiliencehub.types.start_resource_grouping_recommendation_task_response.StartResourceGroupingRecommendationTaskResponse = aws_sdk_resiliencehub.types.start_resource_grouping_recommendation_task_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_resiliencehub.types.start_resource_grouping_recommendation_task_request.StartResourceGroupingRecommendationTaskRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/start-resource-grouping-recommendation-task"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_resiliencehub.types.start_resource_grouping_recommendation_task_request

    body: bytes | None = json.dumps(
        aws_sdk_resiliencehub.types.start_resource_grouping_recommendation_task_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_start_resource_grouping_recommendation_task(
