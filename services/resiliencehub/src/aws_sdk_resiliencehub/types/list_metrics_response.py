...
class ListMetricsResponse(TypedDict):
    rows: "aws_sdk_resiliencehub.types.row_list.RowList"
...
def serialize_json(value: ListMetricsResponse) -> dict:
    ...
    out["rows"] = aws_sdk_resiliencehub.types.row_list.serialize_json(value["rows"])
...
def deserialize_json(data: dict) -> ListMetricsResponse:
    ...
    if "rows" in data:
        ...
        out["rows"] = aws_sdk_resiliencehub.types.row_list.deserialize_json(
            data["rows"]
        )
    else:
