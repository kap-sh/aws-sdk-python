"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeAppVersionTemplateResponse``."""
...
class DescribeAppVersionTemplateResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: DescribeAppVersionTemplateResponse) -> dict:
    out: dict = {}
    out["appArn"] = value["app_arn"]
    out["appVersion"] = value["app_version"]
    out["appTemplateBody"] = value["app_template_body"]
    return out


def deserialize_json(data: dict) -> DescribeAppVersionTemplateResponse:
    out: DescribeAppVersionTemplateResponse = {}  # type: ignore[typeddict-item]
    if "appArn" in data:
        out["app_arn"] = data["appArn"]
    else:
        raise DeserializationError(
            "DescribeAppVersionTemplateResponse.app_arn required"
        )
    if "appVersion" in data:
        out["app_version"] = data["appVersion"]
    else:
        raise DeserializationError(
            "DescribeAppVersionTemplateResponse.app_version required"
        )
    if "appTemplateBody" in data:
        out["app_template_body"] = data["appTemplateBody"]
    else:
        raise DeserializationError(
            "DescribeAppVersionTemplateResponse.app_template_body required"
        )
    return out
