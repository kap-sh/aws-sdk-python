"""Generated from Smithy shape ``com.amazonaws.resiliencehub#DescribeAppVersionAppComponentResponse``."""
...
class DescribeAppVersionAppComponentResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: DescribeAppVersionAppComponentResponse) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> DescribeAppVersionAppComponentResponse:
    out: DescribeAppVersionAppComponentResponse = {}  # type: ignore[typeddict-item]
    if "appArn" in data:
        out["app_arn"] = data["appArn"]
    else:
        raise DeserializationError(
            "DescribeAppVersionAppComponentResponse.app_arn required"
        )
    if "appVersion" in data:
        out["app_version"] = data["appVersion"]
    else:
        raise DeserializationError(
            "DescribeAppVersionAppComponentResponse.app_version required"
        )
