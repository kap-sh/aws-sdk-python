...
class AppComponentCompliance(TypedDict):
...
def serialize_json(value: AppComponentCompliance) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = aws_sdk_resiliencehub.types.compliance_status.serialize_json(
            value["status"]
        )
...
def deserialize_json(data: dict) -> AppComponentCompliance:
    ...
    if "status" in data:
        ...
        out["status"] = aws_sdk_resiliencehub.types.compliance_status.deserialize_json(
            data["status"]
        )
