"""Generated from Smithy shape ``com.amazonaws.panorama#CreateApplicationInstance``."""
...
def handle_response(
    ...
    out: capo_panorama.types.create_application_instance_response.CreateApplicationInstanceResponse = capo_panorama.types.create_application_instance_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/application-instances"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_panorama.types.create_application_instance_request.serialize_json(input_)
    ).encode()
...
async def async_create_application_instance(
