"""Generated from Smithy shape ``com.amazonaws.panorama#CreateNodeFromTemplateJob``."""
...
def handle_response(
    ...
    out: capo_panorama.types.create_node_from_template_job_response.CreateNodeFromTemplateJobResponse = capo_panorama.types.create_node_from_template_job_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/packages/template-job"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_panorama.types.create_node_from_template_job_request.serialize_json(input_)
    ).encode()
...
async def async_create_node_from_template_job(
