"""Generated from Smithy shape ``com.amazonaws.panorama#UpdateDeviceMetadata``."""
...
def handle_response(
    ...
    out: capo_panorama.types.update_device_metadata_response.UpdateDeviceMetadataResponse = capo_panorama.types.update_device_metadata_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/devices/{DeviceId}"
    url = url.replace("{DeviceId}", quote(str(input_["device_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_panorama.types.update_device_metadata_request.serialize_json(input_)
    ).encode()
...
async def async_update_device_metadata(
