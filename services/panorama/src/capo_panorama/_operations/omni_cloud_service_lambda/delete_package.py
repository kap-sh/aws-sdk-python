"""Generated from Smithy shape ``com.amazonaws.panorama#DeletePackage``."""
...
def handle_response(
    ...
    out: capo_panorama.types.delete_package_response.DeletePackageResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/packages/{PackageId}"
    url = url.replace("{PackageId}", quote(str(input_["package_id"]), safe=""))
    params: dict[str, str] = {}
    params["ForceDelete"] = str(input_.get("force_delete", False))
...
def delete_package(
    ...
    capo_panorama.types.delete_package_response.DeletePackageResponse, zapros.Response
]:
