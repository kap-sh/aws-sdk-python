...
if TYPE_CHECKING:
    import aws_sdk_panorama.types.validation_exception_field

ValidationExceptionFieldList: TypeAlias = list[
    "aws_sdk_panorama.types.validation_exception_field.ValidationExceptionField"
]
...
def serialize_json(value: ValidationExceptionFieldList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_panorama.types.validation_exception_field.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> ValidationExceptionFieldList:
    ...
    for item in data:
        out.append(
            aws_sdk_panorama.types.validation_exception_field.deserialize_json(item)
        )
    return out
