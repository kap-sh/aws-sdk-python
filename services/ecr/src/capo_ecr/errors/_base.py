...
class ECRError(Exception):
    """Base class for all ECR service errors."""


class ServiceError(ECRError):
...
class SerializationError(ECRError):
    """Raised when a value cannot be serialized."""


class DeserializationError(ECRError):
...
class WaiterFailedError(ECRError):
...
class WaiterTimeoutError(ECRError):
