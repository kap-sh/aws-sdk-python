"""Generated from Smithy shape ``com.amazonaws.elasticbeanstalk#CreateEnvironment``."""
...
import capo_elastic_beanstalk.errors.too_many_environments_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "TooManyEnvironmentsException":
            raise capo_elastic_beanstalk.errors.too_many_environments_exception.TooManyEnvironmentsException.from_query(
                root
            )
        case _:
...
def handle_response(
    ...
    root = fromstring(response.read())
    result = root.find("CreateEnvironmentResult")
    out: capo_elastic_beanstalk.types.environment_description.EnvironmentDescription = (
...
def build_request(
    ...
    pairs.append(("Action", "CreateEnvironment"))
    pairs.append(("Version", "2010-12-01"))
    capo_elastic_beanstalk.types.create_environment_message.serialize_query(
        input_, pairs, ""
    )
