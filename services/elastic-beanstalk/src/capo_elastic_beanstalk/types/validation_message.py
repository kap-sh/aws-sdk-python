...
if TYPE_CHECKING:
    ...
    import capo_elastic_beanstalk.types.validation_message_string
    import capo_elastic_beanstalk.types.validation_severity


class ValidationMessage(TypedDict, closed=True):
    message: NotRequired[
        "capo_elastic_beanstalk.types.validation_message_string.ValidationMessageString"
    ]
    """<p>A message describing the error or warning.</p>"""
    severity: NotRequired[
        "capo_elastic_beanstalk.types.validation_severity.ValidationSeverity"
    ]
...
def deserialize_query(el: Element) -> ValidationMessage:
    ...
    if child_severity is not None:
        ...
        out["severity"] = (
            capo_elastic_beanstalk.types.validation_severity.deserialize_query(
                child_severity
            )
        )
