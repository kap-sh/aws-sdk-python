...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.launch_template

LaunchTemplateList: TypeAlias = list[
    "capo_elastic_beanstalk.types.launch_template.LaunchTemplate"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.launch_template.serialize_query(
...
def deserialize_query(el: Element) -> LaunchTemplateList:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_beanstalk.types.launch_template.deserialize_query(child)
        )
    return out
