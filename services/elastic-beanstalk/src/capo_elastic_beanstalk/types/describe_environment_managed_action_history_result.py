...
class DescribeEnvironmentManagedActionHistoryResult(TypedDict, closed=True):
    managed_action_history_items: NotRequired[
        "capo_elastic_beanstalk.types.managed_action_history_items.ManagedActionHistoryItems"
    ]
...
def deserialize_query(el: Element) -> DescribeEnvironmentManagedActionHistoryResult:
    ...
    if child_managed_action_history_items is not None:
        ...
        out["managed_action_history_items"] = (
            capo_elastic_beanstalk.types.managed_action_history_items.deserialize_query(
