...
class ManagedActionHistoryItem(TypedDict, closed=True):
    ...
    failure_type: NotRequired["capo_elastic_beanstalk.types.failure_type.FailureType"]
    """<p>If the action failed, the type of failure.</p>"""
    status: NotRequired[
        "capo_elastic_beanstalk.types.action_history_status.ActionHistoryStatus"
    ]
    """<p>The status of the action.</p>"""
    failure_description: NotRequired["capo_elastic_beanstalk.types.string.String"]
    """<p>If the action failed, a description of the failure.</p>"""
    executed_time: NotRequired["capo_elastic_beanstalk.types.timestamp.Timestamp"]
    """<p>The date and time that the action started executing.</p>"""
    finished_time: NotRequired["capo_elastic_beanstalk.types.timestamp.Timestamp"]
...
def deserialize_query(el: Element) -> ManagedActionHistoryItem:
    ...
    if child_failure_type is not None:
        ...
        out["failure_type"] = (
            capo_elastic_beanstalk.types.failure_type.deserialize_query(
                child_failure_type
            )
        )
    ...
    if child_status is not None:
        ...
        out["status"] = (
            capo_elastic_beanstalk.types.action_history_status.deserialize_query(
                child_status
            )
        )
    ...
    if child_executed_time is not None:
        ...
        out["executed_time"] = capo_elastic_beanstalk.types.timestamp.deserialize_query(
            child_executed_time
        )
    ...
    if child_finished_time is not None:
        ...
        out["finished_time"] = capo_elastic_beanstalk.types.timestamp.deserialize_query(
            child_finished_time
        )
    return out
