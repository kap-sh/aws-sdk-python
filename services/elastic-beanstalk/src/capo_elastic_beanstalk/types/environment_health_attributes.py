...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.environment_health_attribute

EnvironmentHealthAttributes: TypeAlias = list[
    "capo_elastic_beanstalk.types.environment_health_attribute.EnvironmentHealthAttribute"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        capo_elastic_beanstalk.types.environment_health_attribute.serialize_query(
...
def deserialize_query(el: Element) -> EnvironmentHealthAttributes:
    ...
    for child in el.findall("member"):
        out.append(
            capo_elastic_beanstalk.types.environment_health_attribute.deserialize_query(
                child
            )
        )
    return out
