...
if TYPE_CHECKING:
    import capo_elastic_beanstalk.types.ec2_instance_id
    import capo_elastic_beanstalk.types.environment_info_type
    import capo_elastic_beanstalk.types.message
...
class EnvironmentInfoDescription(TypedDict, closed=True):
    info_type: NotRequired[
        "capo_elastic_beanstalk.types.environment_info_type.EnvironmentInfoType"
    ]
    """<p>The type of information retrieved.</p>"""
    ec2_instance_id: NotRequired[
        "capo_elastic_beanstalk.types.ec2_instance_id.Ec2InstanceId"
    ]
    """<p>The Amazon EC2 Instance ID for this information.</p>"""
    sample_timestamp: NotRequired[
        "capo_elastic_beanstalk.types.sample_timestamp.SampleTimestamp"
    ]
    """<p>The time stamp when this information was retrieved.</p>"""
    message: NotRequired["capo_elastic_beanstalk.types.message.Message"]
...
def deserialize_query(el: Element) -> EnvironmentInfoDescription:
    ...
    if child_sample_timestamp is not None:
        ...
        out["sample_timestamp"] = (
            capo_elastic_beanstalk.types.sample_timestamp.deserialize_query(
