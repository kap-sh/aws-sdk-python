...
if TYPE_CHECKING:
    ...
    import capo_elastic_beanstalk.types.event_message
...
class EventDescription(TypedDict, closed=True):
    event_date: NotRequired["capo_elastic_beanstalk.types.event_date.EventDate"]
    """<p>The date when the event occurred.</p>"""
    message: NotRequired["capo_elastic_beanstalk.types.event_message.EventMessage"]
...
def deserialize_query(el: Element) -> EventDescription:
    ...
    if child_event_date is not None:
        ...
        out["event_date"] = capo_elastic_beanstalk.types.event_date.deserialize_query(
            child_event_date
        )
    ...
    if child_severity is not None:
        ...
        out["severity"] = capo_elastic_beanstalk.types.event_severity.deserialize_query(
            child_severity
        )
    return out
