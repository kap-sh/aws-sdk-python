...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.instances_health_attribute

InstancesHealthAttributes: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.instances_health_attribute.InstancesHealthAttribute"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.instances_health_attribute.serialize_query(
...
def deserialize_query(el: Element) -> InstancesHealthAttributes:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.instances_health_attribute.deserialize_query(
                child
            )
        )
    return out
