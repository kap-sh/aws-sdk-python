...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.validation_message

ValidationMessagesList: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.validation_message.ValidationMessage"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.validation_message.serialize_query(
...
def deserialize_query(el: Element) -> ValidationMessagesList:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.validation_message.deserialize_query(child)
        )
    return out
