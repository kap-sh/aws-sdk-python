...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.load_balancer_listeners_description
    import aws_sdk_elastic_beanstalk.types.string


class LoadBalancerDescription(TypedDict):
    load_balancer_name: NotRequired["aws_sdk_elastic_beanstalk.types.string.String"]
    """<p>The name of the LoadBalancer.</p>"""
    domain: NotRequired["aws_sdk_elastic_beanstalk.types.string.String"]
    """<p>The domain name of the LoadBalancer.</p>"""
    listeners: NotRequired[
        "aws_sdk_elastic_beanstalk.types.load_balancer_listeners_description.LoadBalancerListenersDescription"
    ]
...
def serialize_query(
    ...
    if "listeners" in value:
        ...
        aws_sdk_elastic_beanstalk.types.load_balancer_listeners_description.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerDescription:
    ...
    if child_listeners is not None:
        ...
        out["listeners"] = (
            aws_sdk_elastic_beanstalk.types.load_balancer_listeners_description.deserialize_query(
                child_listeners
            )
        )
    return out
