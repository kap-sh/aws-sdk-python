...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.listener

LoadBalancerListenersDescription: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.listener.Listener"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.listener.serialize_query(
...
def deserialize_query(el: Element) -> LoadBalancerListenersDescription:
    ...
    for child in el.findall("member"):
        out.append(aws_sdk_elastic_beanstalk.types.listener.deserialize_query(child))
    return out
