...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.option_specification

OptionsSpecifierList: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.option_specification.OptionSpecification"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.option_specification.serialize_query(
...
def deserialize_query(el: Element) -> OptionsSpecifierList:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.option_specification.deserialize_query(
                child
            )
        )
    return out
