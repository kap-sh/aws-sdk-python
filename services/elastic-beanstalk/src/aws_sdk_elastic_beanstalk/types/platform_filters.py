...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.platform_filter

PlatformFilters: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.platform_filter.PlatformFilter"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.platform_filter.serialize_query(
...
def deserialize_query(el: Element) -> PlatformFilters:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.platform_filter.deserialize_query(child)
        )
    return out
