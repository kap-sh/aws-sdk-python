...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.solution_stack_description

AvailableSolutionStackDetailsList: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.solution_stack_description.SolutionStackDescription"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.solution_stack_description.serialize_query(
...
def deserialize_query(el: Element) -> AvailableSolutionStackDetailsList:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.solution_stack_description.deserialize_query(
                child
            )
        )
    return out
