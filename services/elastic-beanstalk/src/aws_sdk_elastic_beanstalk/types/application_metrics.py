...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.latency
    import aws_sdk_elastic_beanstalk.types.nullable_integer
    import aws_sdk_elastic_beanstalk.types.request_count
    import aws_sdk_elastic_beanstalk.types.status_codes
...
def serialize_query(
    ...
    if "status_codes" in value:
        ...
        aws_sdk_elastic_beanstalk.types.status_codes.serialize_query(
            value["status_codes"], pairs, f"{prefix}.StatusCodes"
        )
    if "latency" in value:
        import aws_sdk_elastic_beanstalk.types.latency

        aws_sdk_elastic_beanstalk.types.latency.serialize_query(
...
def deserialize_query(el: Element) -> ApplicationMetrics:
    ...
    if child_status_codes is not None:
        ...
        out["status_codes"] = (
            aws_sdk_elastic_beanstalk.types.status_codes.deserialize_query(
                child_status_codes
            )
        )
    ...
    if child_latency is not None:
        ...
        out["latency"] = aws_sdk_elastic_beanstalk.types.latency.deserialize_query(
            child_latency
        )
    return out
