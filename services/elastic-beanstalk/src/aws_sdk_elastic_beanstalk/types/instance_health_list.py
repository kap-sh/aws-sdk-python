...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.single_instance_health

InstanceHealthList: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.single_instance_health.SingleInstanceHealth"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.single_instance_health.serialize_query(
...
def deserialize_query(el: Element) -> InstanceHealthList:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.single_instance_health.deserialize_query(
                child
            )
        )
    return out
