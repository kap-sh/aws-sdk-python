...
if TYPE_CHECKING:
    import aws_sdk_elastic_beanstalk.types.environment_link

EnvironmentLinks: TypeAlias = list[
    "aws_sdk_elastic_beanstalk.types.environment_link.EnvironmentLink"
]
...
def serialize_query(
    ...
    for n, item in enumerate(value, 1):
        aws_sdk_elastic_beanstalk.types.environment_link.serialize_query(
...
def deserialize_query(el: Element) -> EnvironmentLinks:
    ...
    for child in el.findall("member"):
        out.append(
            aws_sdk_elastic_beanstalk.types.environment_link.deserialize_query(child)
        )
    return out
