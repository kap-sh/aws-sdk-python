"""Generated from Smithy shape ``com.amazonaws.elasticbeanstalk#ResourceTypeNotSupportedException``."""
...
class ResourceTypeNotSupportedException_(TypedDict):
    message: NotRequired[
...
# --- awsQuery ser/de ---
def serialize_query(
    value: ResourceTypeNotSupportedException_, pairs: list[tuple[str, str]], prefix: str
) -> None:
    if "message" in value:
        pairs.append((f"{prefix}.message", str(value["message"])))


def deserialize_query(el: Element) -> ResourceTypeNotSupportedException_:
...
class ResourceTypeNotSupportedException(ServiceError):
    """Modeled error for Smithy shape ``com.amazonaws.elasticbeanstalk#ResourceTypeNotSupportedException``."""
