...
if TYPE_CHECKING:
    ...
    import capo_elastic_transcoder.types.pipelines


class ListPipelinesResponse(TypedDict, closed=True):
    pipelines: NotRequired["capo_elastic_transcoder.types.pipelines.Pipelines"]
...
def serialize_json(value: ListPipelinesResponse) -> dict:
    ...
    if "pipelines" in value:
        ...
        out["Pipelines"] = capo_elastic_transcoder.types.pipelines.serialize_json(
            value["pipelines"]
        )
...
def deserialize_json(data: dict) -> ListPipelinesResponse:
    ...
    if "Pipelines" in data:
        ...
        out["pipelines"] = capo_elastic_transcoder.types.pipelines.deserialize_json(
            data["Pipelines"]
        )
