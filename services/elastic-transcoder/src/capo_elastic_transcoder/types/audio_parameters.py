...
if TYPE_CHECKING:
    import capo_elastic_transcoder.types.audio_bit_rate
    import capo_elastic_transcoder.types.audio_channels
    import capo_elastic_transcoder.types.audio_codec
    import capo_elastic_transcoder.types.audio_codec_options
    import capo_elastic_transcoder.types.audio_packing_mode
    import capo_elastic_transcoder.types.audio_sample_rate


class AudioParameters(TypedDict, closed=True):
    codec: NotRequired["capo_elastic_transcoder.types.audio_codec.AudioCodec"]
    ...
    sample_rate: NotRequired[
        "capo_elastic_transcoder.types.audio_sample_rate.AudioSampleRate"
    ]
    ...
    bit_rate: NotRequired["capo_elastic_transcoder.types.audio_bit_rate.AudioBitRate"]
    """<p>The bit rate of the audio stream in the output file, in kilobits/second. Enter an integer between 64 and 320, inclusive.</p>"""
    channels: NotRequired["capo_elastic_transcoder.types.audio_channels.AudioChannels"]
    ...
    audio_packing_mode: NotRequired[
        "capo_elastic_transcoder.types.audio_packing_mode.AudioPackingMode"
    ]
    ...
    codec_options: NotRequired[
        "capo_elastic_transcoder.types.audio_codec_options.AudioCodecOptions"
    ]
...
def serialize_json(value: AudioParameters) -> dict:
    ...
    if "codec_options" in value:
        ...
        out["CodecOptions"] = (
            capo_elastic_transcoder.types.audio_codec_options.serialize_json(
                value["codec_options"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> AudioParameters:
    ...
    if "CodecOptions" in data:
        ...
        out["codec_options"] = (
            capo_elastic_transcoder.types.audio_codec_options.deserialize_json(
                data["CodecOptions"]
            )
        )
    return out
