...
if TYPE_CHECKING:
    ...
    import aws_sdk_elastic_transcoder.types.hls_content_protection
    import aws_sdk_elastic_transcoder.types.output_keys
    import aws_sdk_elastic_transcoder.types.play_ready_drm
...
class CreateJobPlaylist(TypedDict, closed=True):
    ...
    format: NotRequired[
        "aws_sdk_elastic_transcoder.types.playlist_format.PlaylistFormat"
    ]
    """<p>The format of the output playlist. Valid formats include <code>HLSv3</code>, <code>HLSv4</code>, and <code>Smooth</code>.</p>"""
    output_keys: NotRequired["aws_sdk_elastic_transcoder.types.output_keys.OutputKeys"]
    ...
    hls_content_protection: NotRequired[
        "aws_sdk_elastic_transcoder.types.hls_content_protection.HlsContentProtection"
    ]
...
def serialize_json(value: CreateJobPlaylist) -> dict:
    ...
    if "output_keys" in value:
        ...
        out["OutputKeys"] = aws_sdk_elastic_transcoder.types.output_keys.serialize_json(
            value["output_keys"]
        )
    if "hls_content_protection" in value:
        ...
        out["HlsContentProtection"] = (
            aws_sdk_elastic_transcoder.types.hls_content_protection.serialize_json(
    ...
    if "play_ready_drm" in value:
        ...
        out["PlayReadyDrm"] = (
            aws_sdk_elastic_transcoder.types.play_ready_drm.serialize_json(
...
def deserialize_json(data: dict) -> CreateJobPlaylist:
    out: CreateJobPlaylist = {}  # type: ignore[typeddict-item]
    if "Name" in data:
        out["name"] = data["Name"]
    if "Format" in data:
    ...
    if "OutputKeys" in data:
        ...
        out["output_keys"] = (
            aws_sdk_elastic_transcoder.types.output_keys.deserialize_json(
                data["OutputKeys"]
            )
        )
    if "HlsContentProtection" in data:
        ...
        out["hls_content_protection"] = (
            aws_sdk_elastic_transcoder.types.hls_content_protection.deserialize_json(
    ...
    if "PlayReadyDrm" in data:
        ...
        out["play_ready_drm"] = (
            aws_sdk_elastic_transcoder.types.play_ready_drm.deserialize_json(
                data["PlayReadyDrm"]
            )
        )
    return out
