...
if TYPE_CHECKING:
    import aws_sdk_elastic_transcoder.types.artworks
    import aws_sdk_elastic_transcoder.types.merge_policy


class JobAlbumArt(TypedDict):
    merge_policy: NotRequired[
        "aws_sdk_elastic_transcoder.types.merge_policy.MergePolicy"
    ]
    ...
    artwork: NotRequired["aws_sdk_elastic_transcoder.types.artworks.Artworks"]
...
def serialize_json(value: JobAlbumArt) -> dict:
    ...
    if "artwork" in value:
        ...
        out["Artwork"] = aws_sdk_elastic_transcoder.types.artworks.serialize_json(
            value["artwork"]
        )
    return out
...
def deserialize_json(data: dict) -> JobAlbumArt:
    ...
    if "Artwork" in data:
        ...
        out["artwork"] = aws_sdk_elastic_transcoder.types.artworks.deserialize_json(
            data["Artwork"]
        )
    return out
