...
TemplateSummaryList: TypeAlias = list[
    "aws_sdk_migrationhuborchestrator.types.template_summary.TemplateSummary"
]
...
def serialize_json(value: TemplateSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_migrationhuborchestrator.types.template_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> TemplateSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_migrationhuborchestrator.types.template_summary.deserialize_json(
                item
            )
        )
    return out
