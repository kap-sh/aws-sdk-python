...
if TYPE_CHECKING:
    ...
    import capo_migrationhuborchestrator.types.run_environment
    import capo_migrationhuborchestrator.types.s3_bucket
...
class WorkflowStepAutomationConfiguration(TypedDict, closed=True):
    script_location_s3_bucket: NotRequired[
        "capo_migrationhuborchestrator.types.s3_bucket.S3Bucket"
    ]
    """<p>The Amazon S3 bucket where the script is located.</p>"""
    script_location_s3_key: NotRequired[
        "capo_migrationhuborchestrator.types.platform_script_key.PlatformScriptKey"
    ]
    """<p>The Amazon S3 key for the script location.</p>"""
    command: NotRequired[
        "capo_migrationhuborchestrator.types.platform_command.PlatformCommand"
    ]
    """<p>The command required to run the script.</p>"""
    run_environment: NotRequired[
        "capo_migrationhuborchestrator.types.run_environment.RunEnvironment"
    ]
...
# --- restJson1 ser/de ---
def serialize_json(value: WorkflowStepAutomationConfiguration) -> dict:
    out: dict = {}
    ...
    if "script_location_s3_key" in value:
        ...
        out["scriptLocationS3Key"] = (
            capo_migrationhuborchestrator.types.platform_script_key.serialize_json(
    ...
    if "command" in value:
        ...
        out["command"] = (
            capo_migrationhuborchestrator.types.platform_command.serialize_json(
                value["command"]
            )
        )
...
def deserialize_json(data: dict) -> WorkflowStepAutomationConfiguration:
    ...
    if "scriptLocationS3Key" in data:
        ...
        out["script_location_s3_key"] = (
            capo_migrationhuborchestrator.types.platform_script_key.deserialize_json(
    ...
    if "command" in data:
        ...
        out["command"] = (
            capo_migrationhuborchestrator.types.platform_command.deserialize_json(
                data["command"]
            )
        )
