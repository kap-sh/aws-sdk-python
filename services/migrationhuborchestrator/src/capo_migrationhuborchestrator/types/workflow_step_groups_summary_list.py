...
WorkflowStepGroupsSummaryList: TypeAlias = list[
    "capo_migrationhuborchestrator.types.workflow_step_group_summary.WorkflowStepGroupSummary"
]
...
def serialize_json(value: WorkflowStepGroupsSummaryList) -> list:
    ...
    for item in value:
        out.append(
            capo_migrationhuborchestrator.types.workflow_step_group_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> WorkflowStepGroupsSummaryList:
    ...
    for item in data:
        out.append(
            capo_migrationhuborchestrator.types.workflow_step_group_summary.deserialize_json(
                item
            )
        )
    return out
