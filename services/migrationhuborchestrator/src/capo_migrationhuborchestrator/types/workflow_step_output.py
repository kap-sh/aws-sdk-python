...
if TYPE_CHECKING:
    ...
    import capo_migrationhuborchestrator.types.workflow_step_output_name
    import capo_migrationhuborchestrator.types.workflow_step_output_union


class WorkflowStepOutput(TypedDict, closed=True):
    name: NotRequired[
        "capo_migrationhuborchestrator.types.workflow_step_output_name.WorkflowStepOutputName"
    ]
    ...
    value: NotRequired[
        "capo_migrationhuborchestrator.types.workflow_step_output_union.WorkflowStepOutputUnion"
    ]
...
def serialize_json(value: WorkflowStepOutput) -> dict:
    ...
    if "value" in value:
        ...
        out["value"] = (
            capo_migrationhuborchestrator.types.workflow_step_output_union.serialize_json(
                value["value"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> WorkflowStepOutput:
    ...
    if "value" in data:
        ...
        out["value"] = (
            capo_migrationhuborchestrator.types.workflow_step_output_union.deserialize_json(
                data["value"]
            )
        )
    return out
