"""Generated from Smithy shape ``com.amazonaws.securitylake#ListDataLakeExceptions``."""
...
def handle_response(
    ...
    out: capo_securitylake.types.list_data_lake_exceptions_response.ListDataLakeExceptionsResponse = capo_securitylake.types.list_data_lake_exceptions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datalake/exceptions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securitylake.types.list_data_lake_exceptions_request.serialize_json(input_)
    ).encode()
...
async def async_list_data_lake_exceptions(
