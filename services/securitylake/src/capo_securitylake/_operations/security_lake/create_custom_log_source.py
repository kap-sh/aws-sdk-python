"""Generated from Smithy shape ``com.amazonaws.securitylake#CreateCustomLogSource``."""
...
def handle_response(
    ...
    out: capo_securitylake.types.create_custom_log_source_response.CreateCustomLogSourceResponse = capo_securitylake.types.create_custom_log_source_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v1/datalake/logsources/custom"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_securitylake.types.create_custom_log_source_request.serialize_json(input_)
    ).encode()
...
async def async_create_custom_log_source(
