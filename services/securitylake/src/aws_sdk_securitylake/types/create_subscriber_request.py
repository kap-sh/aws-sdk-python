...
class CreateSubscriberRequest(TypedDict):
    ...
    subscriber_description: NotRequired["aws_sdk_securitylake.types.description_string.DescriptionString"]
    ...
    access_types: NotRequired["aws_sdk_securitylake.types.access_type_list.AccessTypeList"]
...
def serialize_json(value: CreateSubscriberRequest) -> dict:
    out: dict = {}
    import aws_sdk_securitylake.types.aws_identity
    out["subscriberIdentity"] = aws_sdk_securitylake.types.aws_identity.serialize_json(value["subscriber_identity"])
    ...
    import aws_sdk_securitylake.types.log_source_resource_list
    out["sources"] = aws_sdk_securitylake.types.log_source_resource_list.serialize_json(value["sources"])
    if "access_types" in value:
        import aws_sdk_securitylake.types.access_type_list
        out["accessTypes"] = aws_sdk_securitylake.types.access_type_list.serialize_json(value["access_types"])
    if "tags" in value:
        import aws_sdk_securitylake.types.tag_list
...
def deserialize_json(data: dict) -> CreateSubscriberRequest:
    ...
    if "subscriberIdentity" in data:
        import aws_sdk_securitylake.types.aws_identity
        out["subscriber_identity"] = aws_sdk_securitylake.types.aws_identity.deserialize_json(data["subscriberIdentity"])
    else:
        raise DeserializationError("CreateSubscriberRequest.subscriber_identity required")
    ...
    if "sources" in data:
        import aws_sdk_securitylake.types.log_source_resource_list
        out["sources"] = aws_sdk_securitylake.types.log_source_resource_list.deserialize_json(data["sources"])
    else:
        raise DeserializationError("CreateSubscriberRequest.sources required")
    if "accessTypes" in data:
        import aws_sdk_securitylake.types.access_type_list
        out["access_types"] = aws_sdk_securitylake.types.access_type_list.deserialize_json(data["accessTypes"])
    if "tags" in data:
        import aws_sdk_securitylake.types.tag_list
