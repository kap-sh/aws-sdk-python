...
def serialize_json(value: CreateAwsLogSourceResponse) -> dict:
    ...
    if "failed" in value:
        import aws_sdk_securitylake.types.account_list
        out["failed"] = aws_sdk_securitylake.types.account_list.serialize_json(value["failed"])
    return out
...
def deserialize_json(data: dict) -> CreateAwsLogSourceResponse:
    ...
    if "failed" in data:
        import aws_sdk_securitylake.types.account_list
        out["failed"] = aws_sdk_securitylake.types.account_list.deserialize_json(data["failed"])
    return out