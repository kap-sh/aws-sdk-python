...
def serialize_json(value: DataLakeAutoEnableNewAccountConfiguration) -> dict:
    ...
    import aws_sdk_securitylake.types.aws_log_source_resource_list
    out["sources"] = aws_sdk_securitylake.types.aws_log_source_resource_list.serialize_json(value["sources"])
    return out
...
def deserialize_json(data: dict) -> DataLakeAutoEnableNewAccountConfiguration:
    ...
    if "region" in data:
        ...
        raise DeserializationError("DataLakeAutoEnableNewAccountConfiguration.region required")
    if "sources" in data:
        import aws_sdk_securitylake.types.aws_log_source_resource_list
        out["sources"] = aws_sdk_securitylake.types.aws_log_source_resource_list.deserialize_json(data["sources"])
    else:
        raise DeserializationError("DataLakeAutoEnableNewAccountConfiguration.sources required")
    return out