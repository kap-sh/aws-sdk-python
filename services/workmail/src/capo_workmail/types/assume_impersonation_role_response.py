...
if TYPE_CHECKING:
    import capo_workmail.types.expires_in
    import capo_workmail.types.impersonation_token


class AssumeImpersonationRoleResponse(TypedDict, closed=True):
    token: NotRequired["capo_workmail.types.impersonation_token.ImpersonationToken"]
    """<p>The authentication token for the impersonation role.</p>"""
    expires_in: NotRequired["capo_workmail.types.expires_in.ExpiresIn"]
