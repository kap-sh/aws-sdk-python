...
if TYPE_CHECKING:
    import capo_kendra.types.query_suggestions_id
    import capo_kendra.types.suggestion_list


class GetQuerySuggestionsResponse(TypedDict, closed=True):
    query_suggestions_id: NotRequired[
        "capo_kendra.types.query_suggestions_id.QuerySuggestionsId"
    ]
    """<p>The identifier for a list of query suggestions for an index.</p>"""
    suggestions: NotRequired["capo_kendra.types.suggestion_list.SuggestionList"]
...
def serialize_aws_json_1_1(value: GetQuerySuggestionsResponse) -> dict:
    ...
    if "suggestions" in value:
        ...
        out["Suggestions"] = capo_kendra.types.suggestion_list.serialize_aws_json_1_1(
            value["suggestions"]
        )
    return out
