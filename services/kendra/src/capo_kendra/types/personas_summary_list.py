...
if TYPE_CHECKING:
    import capo_kendra.types.personas_summary

PersonasSummaryList: TypeAlias = list[
    "capo_kendra.types.personas_summary.PersonasSummary"
]
...
def serialize_aws_json_1_1(value: PersonasSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_kendra.types.personas_summary.serialize_aws_json_1_1(item))
    return out
...
def deserialize_aws_json_1_1(data: list) -> PersonasSummaryList:
    ...
    for item in data:
        out.append(capo_kendra.types.personas_summary.deserialize_aws_json_1_1(item))
    return out
