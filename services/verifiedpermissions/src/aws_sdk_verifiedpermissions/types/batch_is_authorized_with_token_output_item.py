...
def serialize_aws_json_1_0(value: BatchIsAuthorizedWithTokenOutputItem) -> dict:
    out: dict = {}
    import aws_sdk_verifiedpermissions.types.batch_is_authorized_with_token_input_item
    out["request"] = aws_sdk_verifiedpermissions.types.batch_is_authorized_with_token_input_item.serialize_aws_json_1_0(value["request"])
    import aws_sdk_verifiedpermissions.types.decision
...
def deserialize_aws_json_1_0(data: dict) -> BatchIsAuthorizedWithTokenOutputItem:
    ...
    if "request" in data:
        import aws_sdk_verifiedpermissions.types.batch_is_authorized_with_token_input_item
        out["request"] = aws_sdk_verifiedpermissions.types.batch_is_authorized_with_token_input_item.deserialize_aws_json_1_0(data["request"])
    else:
        raise DeserializationError("BatchIsAuthorizedWithTokenOutputItem.request required")
    if "decision" in data:
        import aws_sdk_verifiedpermissions.types.decision
        out["decision"] = aws_sdk_verifiedpermissions.types.decision.deserialize_aws_json_1_0(data["decision"])
    else:
        raise DeserializationError("BatchIsAuthorizedWithTokenOutputItem.decision required")
    if "determiningPolicies" in data:
        ...
        raise DeserializationError("BatchIsAuthorizedWithTokenOutputItem.determining_policies required")
    if "errors" in data:
        import aws_sdk_verifiedpermissions.types.evaluation_error_list
        out["errors"] = aws_sdk_verifiedpermissions.types.evaluation_error_list.deserialize_aws_json_1_0(data["errors"])
    else:
        raise DeserializationError("BatchIsAuthorizedWithTokenOutputItem.errors required")
    return out