...
if TYPE_CHECKING:
    import capo_networkmanager.types.connect_peer_summary_list
    import capo_networkmanager.types.next_token


class ListConnectPeersResponse(TypedDict, closed=True):
    connect_peers: NotRequired[
        "capo_networkmanager.types.connect_peer_summary_list.ConnectPeerSummaryList"
    ]
...
def serialize_json(value: ListConnectPeersResponse) -> dict:
    ...
    if "connect_peers" in value:
        ...
        out["ConnectPeers"] = (
            capo_networkmanager.types.connect_peer_summary_list.serialize_json(
                value["connect_peers"]
            )
        )
...
def deserialize_json(data: dict) -> ListConnectPeersResponse:
    ...
    if "ConnectPeers" in data:
        ...
        out["connect_peers"] = (
            capo_networkmanager.types.connect_peer_summary_list.deserialize_json(
                data["ConnectPeers"]
            )
        )
