...
class ListPeeringsResponse(TypedDict, closed=True):
    peerings: NotRequired["capo_networkmanager.types.peering_list.PeeringList"]
...
def serialize_json(value: ListPeeringsResponse) -> dict:
    ...
    if "peerings" in value:
        ...
        out["Peerings"] = capo_networkmanager.types.peering_list.serialize_json(
            value["peerings"]
        )
...
def deserialize_json(data: dict) -> ListPeeringsResponse:
    ...
    if "Peerings" in data:
        ...
        out["peerings"] = capo_networkmanager.types.peering_list.deserialize_json(
            data["Peerings"]
        )
