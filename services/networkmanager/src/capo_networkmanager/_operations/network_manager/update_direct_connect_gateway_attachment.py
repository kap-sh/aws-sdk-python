"""Generated from Smithy shape ``com.amazonaws.networkmanager#UpdateDirectConnectGatewayAttachment``."""
...
import capo_networkmanager.types.update_direct_connect_gateway_attachment_request
import capo_networkmanager.types.update_direct_connect_gateway_attachment_response
...
) -> capo_networkmanager.types.update_direct_connect_gateway_attachment_response.UpdateDirectConnectGatewayAttachmentResponse:
    out: capo_networkmanager.types.update_direct_connect_gateway_attachment_response.UpdateDirectConnectGatewayAttachmentResponse = capo_networkmanager.types.update_direct_connect_gateway_attachment_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/") + "/direct-connect-gateway-attachments/{AttachmentId}"
    )
    url = url.replace("{AttachmentId}", quote(str(input_["attachment_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_networkmanager.types.update_direct_connect_gateway_attachment_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_update_direct_connect_gateway_attachment(
