...
PrefixListAssociationList: TypeAlias = list[
    "aws_sdk_networkmanager.types.prefix_list_association.PrefixListAssociation"
]
...
def serialize_json(value: PrefixListAssociationList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_networkmanager.types.prefix_list_association.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> PrefixListAssociationList:
    ...
    for item in data:
        out.append(
            aws_sdk_networkmanager.types.prefix_list_association.deserialize_json(item)
        )
    return out
