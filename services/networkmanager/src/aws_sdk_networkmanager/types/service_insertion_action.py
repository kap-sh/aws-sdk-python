...
if TYPE_CHECKING:
    ...
    import aws_sdk_networkmanager.types.send_via_mode
    import aws_sdk_networkmanager.types.via
...
class ServiceInsertionAction(TypedDict, closed=True):
    ...
    mode: NotRequired["aws_sdk_networkmanager.types.send_via_mode.SendViaMode"]
    ...
    when_sent_to: NotRequired["aws_sdk_networkmanager.types.when_sent_to.WhenSentTo"]
    """<p>The list of destination segments if the service insertion action is <code>send-via</code>.</p>"""
    via: NotRequired["aws_sdk_networkmanager.types.via.Via"]
...
def serialize_json(value: ServiceInsertionAction) -> dict:
    ...
    if "action" in value:
        ...
        out["Action"] = (
            aws_sdk_networkmanager.types.segment_action_service_insertion.serialize_json(
                value["action"]
            )
        )
    if "mode" in value:
        ...
        out["Mode"] = aws_sdk_networkmanager.types.send_via_mode.serialize_json(
            value["mode"]
        )
    if "when_sent_to" in value:
        import aws_sdk_networkmanager.types.when_sent_to

        out["WhenSentTo"] = aws_sdk_networkmanager.types.when_sent_to.serialize_json(
            value["when_sent_to"]
        )
    if "via" in value:
        import aws_sdk_networkmanager.types.via

        out["Via"] = aws_sdk_networkmanager.types.via.serialize_json(value["via"])
    return out
...
def deserialize_json(data: dict) -> ServiceInsertionAction:
    ...
    if "Action" in data:
        ...
        out["action"] = (
            aws_sdk_networkmanager.types.segment_action_service_insertion.deserialize_json(
                data["Action"]
            )
        )
    if "Mode" in data:
        ...
        out["mode"] = aws_sdk_networkmanager.types.send_via_mode.deserialize_json(
            data["Mode"]
        )
    if "WhenSentTo" in data:
        ...
        out["when_sent_to"] = (
            aws_sdk_networkmanager.types.when_sent_to.deserialize_json(
                data["WhenSentTo"]
            )
        )
    if "Via" in data:
        ...
        out["via"] = aws_sdk_networkmanager.types.via.deserialize_json(data["Via"])
    return out
