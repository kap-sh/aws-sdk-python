"""Generated from Smithy shape ``com.amazonaws.networkmanager#NetworkResourceCountList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_networkmanager.types.network_resource_count

NetworkResourceCountList: TypeAlias = list[
    "aws_sdk_networkmanager.types.network_resource_count.NetworkResourceCount"
]


# --- restJson1 ser/de ---
def serialize_json(value: NetworkResourceCountList) -> list:
    import aws_sdk_networkmanager.types.network_resource_count

    out: list = []
    for item in value:
        out.append(
            aws_sdk_networkmanager.types.network_resource_count.serialize_json(item)
        )
    return out


def deserialize_json(data: list) -> NetworkResourceCountList:
    import aws_sdk_networkmanager.types.network_resource_count

    out: NetworkResourceCountList = []
    for item in data:
        out.append(
            aws_sdk_networkmanager.types.network_resource_count.deserialize_json(item)
        )
    return out
