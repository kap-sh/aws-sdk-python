"""Generated from Smithy shape ``com.amazonaws.networkmanager#DeleteConnectPeer``."""
...
def handle_response(
    ...
    out: aws_sdk_networkmanager.types.delete_connect_peer_response.DeleteConnectPeerResponse = aws_sdk_networkmanager.types.delete_connect_peer_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_networkmanager.types.delete_connect_peer_request.DeleteConnectPeerRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/connect-peers/{ConnectPeerId}"
    url = url.replace("{ConnectPeerId}", quote(str(input_["connect_peer_id"]), safe=""))
    params: dict[str, str] = {}
...
async def async_delete_connect_peer(
