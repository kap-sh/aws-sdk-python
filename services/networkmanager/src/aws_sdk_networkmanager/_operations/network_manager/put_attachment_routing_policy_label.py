"""Generated from Smithy shape ``com.amazonaws.networkmanager#PutAttachmentRoutingPolicyLabel``."""
...
def handle_response(
    ...
    out: aws_sdk_networkmanager.types.put_attachment_routing_policy_label_response.PutAttachmentRoutingPolicyLabelResponse = aws_sdk_networkmanager.types.put_attachment_routing_policy_label_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_networkmanager.types.put_attachment_routing_policy_label_request.PutAttachmentRoutingPolicyLabelRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/routing-policy-label"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_networkmanager.types.put_attachment_routing_policy_label_request

    body: bytes | None = json.dumps(
        aws_sdk_networkmanager.types.put_attachment_routing_policy_label_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_attachment_routing_policy_label(
