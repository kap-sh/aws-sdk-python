...
class GetTransformerJobResponse(TypedDict, closed=True):
    status: "capo_b2bi.types.transformer_job_status.TransformerJobStatus"
    """<p>Returns the current state of the transformer job, either <code>running</code>, <code>succeeded</code>, or <code>failed</code>.</p>"""
    output_files: NotRequired["capo_b2bi.types.s3_location_list.S3LocationList"]
...
def serialize_aws_json_1_0(value: GetTransformerJobResponse) -> dict:
    ...
    out["status"] = capo_b2bi.types.transformer_job_status.serialize_aws_json_1_0(
        value["status"]
    )
    if "output_files" in value:
        import capo_b2bi.types.s3_location_list

        out["outputFiles"] = capo_b2bi.types.s3_location_list.serialize_aws_json_1_0(
            value["output_files"]
        )
    if "message" in value:
