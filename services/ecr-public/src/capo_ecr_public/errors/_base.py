...
class ECRPUBLICError(Exception):
    """Base class for all ECRPUBLIC service errors."""


class ServiceError(ECRPUBLICError):
...
class SerializationError(ECRPUBLICError):
    """Raised when a value cannot be serialized."""


class DeserializationError(ECRPUBLICError):
...
class WaiterFailedError(ECRPUBLICError):
...
class WaiterTimeoutError(ECRPUBLICError):
