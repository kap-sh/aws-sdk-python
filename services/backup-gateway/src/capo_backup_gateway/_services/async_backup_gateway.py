...
from capo_backup_gateway._services._aws_config import aaws_config
...
class AsyncBackupGatewayClient:
    ...
    async def list_tags_for_resource(
        ...
        *,
        ...
        async def _handler(
            req: "AsyncOperationRequest[capo_backup_gateway.types.list_tags_for_resource_input.ListTagsForResourceInput]",
        ) -> AsyncOperationResponse[
            "capo_backup_gateway.types.list_tags_for_resource_output.ListTagsForResourceOutput"
        ]:
            ...
            (
                output,
                http_response,
            ) = await capo_backup_gateway._operations.backup_on_premises_v20210101.list_tags_for_resource.async_list_tags_for_resource(
                req.options, req.input
            )
            return AsyncOperationResponse(output=output, response=http_response)

        interceptors_, options_ = self.operation_options(config_overrides)
        input_: capo_backup_gateway.types.list_tags_for_resource_input.ListTagsForResourceInput = {}  # type: ignore[typeddict-item]
    ...
    async def tag_resource(
        ...
        *,
        ...
        async def _handler(
            req: "AsyncOperationRequest[capo_backup_gateway.types.tag_resource_input.TagResourceInput]",
        ) -> AsyncOperationResponse[
            "capo_backup_gateway.types.tag_resource_output.TagResourceOutput"
        ]:
            ...
            (
                output,
                http_response,
            ) = await capo_backup_gateway._operations.backup_on_premises_v20210101.tag_resource.async_tag_resource(
                req.options, req.input
            )
            return AsyncOperationResponse(output=output, response=http_response)

        interceptors_, options_ = self.operation_options(config_overrides)
        input_: capo_backup_gateway.types.tag_resource_input.TagResourceInput = {}  # type: ignore[typeddict-item]
        input_["resource_arn"] = resource_arn
        input_["tags"] = tags
    ...
    async def untag_resource(
        ...
        *,
        ...
        async def _handler(
            req: "AsyncOperationRequest[capo_backup_gateway.types.untag_resource_input.UntagResourceInput]",
        ) -> AsyncOperationResponse[
            "capo_backup_gateway.types.untag_resource_output.UntagResourceOutput"
        ]:
            ...
            (
                output,
                http_response,
            ) = await capo_backup_gateway._operations.backup_on_premises_v20210101.untag_resource.async_untag_resource(
                req.options, req.input
            )
            return AsyncOperationResponse(output=output, response=http_response)

        interceptors_, options_ = self.operation_options(config_overrides)
        input_: capo_backup_gateway.types.untag_resource_input.UntagResourceInput = {}  # type: ignore[typeddict-item]
        input_["resource_arn"] = resource_arn
