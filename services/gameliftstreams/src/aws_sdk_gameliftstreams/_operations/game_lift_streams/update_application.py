"""Generated from Smithy shape ``com.amazonaws.gameliftstreams#UpdateApplication``."""
...
def handle_response(
    ...
    out: aws_sdk_gameliftstreams.types.update_application_output.UpdateApplicationOutput = aws_sdk_gameliftstreams.types.update_application_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_gameliftstreams.types.update_application_input.UpdateApplicationInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/applications/{Identifier}"
    url = url.replace("{Identifier}", quote(str(input_["identifier"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        aws_sdk_gameliftstreams.types.update_application_input.serialize_json(input_)
    ).encode()
