"""Generated from Smithy shape ``com.amazonaws.gameliftstreams#ExportStreamSessionFiles``."""
...
def handle_response(
    ...
    out: aws_sdk_gameliftstreams.types.export_stream_session_files_output.ExportStreamSessionFilesOutput = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_gameliftstreams.types.export_stream_session_files_input.ExportStreamSessionFilesInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        ...
        quote(str(input_["stream_session_identifier"]), safe=""),
    )
    params: dict[str, str] = {}
    ...
    body: bytes | None = json.dumps(
        aws_sdk_gameliftstreams.types.export_stream_session_files_input.serialize_json(
            input_
        )
    ).encode()
...
async def async_export_stream_session_files(
