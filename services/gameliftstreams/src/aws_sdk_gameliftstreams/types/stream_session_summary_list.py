...
StreamSessionSummaryList: TypeAlias = list[
    "aws_sdk_gameliftstreams.types.stream_session_summary.StreamSessionSummary"
]
...
def serialize_json(value: StreamSessionSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_gameliftstreams.types.stream_session_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> StreamSessionSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_gameliftstreams.types.stream_session_summary.deserialize_json(item)
        )
    return out
