...
def serialize_json(value: AddStreamGroupLocationsInput) -> dict:
    out: dict = {}
    import aws_sdk_gameliftstreams.types.location_configurations
    out["LocationConfigurations"] = aws_sdk_gameliftstreams.types.location_configurations.serialize_json(value["location_configurations"])
    return out
...
def deserialize_json(data: dict) -> AddStreamGroupLocationsInput:
    ...
    if "LocationConfigurations" in data:
        import aws_sdk_gameliftstreams.types.location_configurations
        out["location_configurations"] = aws_sdk_gameliftstreams.types.location_configurations.deserialize_json(data["LocationConfigurations"])
    else:
        raise DeserializationError("AddStreamGroupLocationsInput.location_configurations required")
    return out