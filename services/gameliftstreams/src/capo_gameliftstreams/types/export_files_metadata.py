...
if TYPE_CHECKING:
    import capo_gameliftstreams.types.export_files_reason
    import capo_gameliftstreams.types.export_files_status
    import capo_gameliftstreams.types.output_uri


class ExportFilesMetadata(TypedDict, closed=True):
    status: NotRequired[
    ...
    output_uri: NotRequired["capo_gameliftstreams.types.output_uri.OutputUri"]
...
def serialize_json(value: ExportFilesMetadata) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_gameliftstreams.types.export_files_status.serialize_json(
            value["status"]
        )
