"""Generated from Smithy shape ``com.amazonaws.gameliftstreams#AssociateApplications``."""
...
def handle_response(
    ...
    out: capo_gameliftstreams.types.associate_applications_output.AssociateApplicationsOutput = capo_gameliftstreams.types.associate_applications_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/streamgroups/{Identifier}/associations"
    url = url.replace("{Identifier}", quote(str(input_["identifier"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_gameliftstreams.types.associate_applications_input.serialize_json(input_)
    ).encode()
...
async def async_associate_applications(
