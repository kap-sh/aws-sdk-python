"""Generated from Smithy shape ``com.amazonaws.notifications#DisassociateChannel``."""
...
def handle_response(
    ...
    out: capo_notifications.types.disassociate_channel_response.DisassociateChannelResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/channels/disassociate/{arn}"
    url = url.replace("{arn}", quote(str(input_["arn"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_notifications.types.disassociate_channel_request.serialize_json(input_)
    ).encode()
...
async def async_disassociate_channel(
