...
ManagedNotificationEvents: TypeAlias = list[
    "capo_notifications.types.managed_notification_event_overview.ManagedNotificationEventOverview"
]
...
def serialize_json(value: ManagedNotificationEvents) -> list:
    ...
    for item in value:
        out.append(
            capo_notifications.types.managed_notification_event_overview.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> ManagedNotificationEvents:
    ...
    for item in data:
        out.append(
            capo_notifications.types.managed_notification_event_overview.deserialize_json(
                item
            )
        )
    return out
