...
if TYPE_CHECKING:
    import capo_notifications.types.arn
...
class Resource(TypedDict, closed=True):
    ...
    arn: NotRequired["capo_notifications.types.arn.Arn"]
    """<p>The Amazon Resource Name (ARN) of the resource. At least one id or ARN is required.</p>"""
    detail_url: NotRequired["capo_notifications.types.url.Url"]
    ...
    tags: NotRequired["capo_notifications.types.tags.Tags"]
...
def serialize_json(value: Resource) -> dict:
    ...
    if "tags" in value:
        ...
        out["tags"] = capo_notifications.types.tags.serialize_json(value["tags"])
    return out
