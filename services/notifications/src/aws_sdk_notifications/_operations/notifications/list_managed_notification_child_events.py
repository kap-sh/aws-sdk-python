"""Generated from Smithy shape ``com.amazonaws.notifications#ListManagedNotificationChildEvents``."""
...
def handle_response(
    ...
    out: aws_sdk_notifications.types.list_managed_notification_child_events_response.ListManagedNotificationChildEventsResponse = aws_sdk_notifications.types.list_managed_notification_child_events_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_notifications.types.list_managed_notification_child_events_request.ListManagedNotificationChildEventsRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        ...
        quote(str(input["aggregate_managed_notification_event_arn"]), safe=""),
    )
    params: dict[str, str] = {}
    ...
    if "locale" in input:
        params["locale"] = str(input["locale"])
    if "max_results" in input:
        params["maxResults"] = str(input["max_results"])
    if "related_account" in input:
        params["relatedAccount"] = str(input["related_account"])
    if "organizational_unit_id" in input:
        params["organizationalUnitId"] = str(input["organizational_unit_id"])
...
async def async_list_managed_notification_child_events(
