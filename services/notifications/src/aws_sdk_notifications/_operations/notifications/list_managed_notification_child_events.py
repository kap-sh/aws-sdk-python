...
def build_request(
    ...
    input_: aws_sdk_notifications.types.list_managed_notification_child_events_request.ListManagedNotificationChildEventsRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        ...
        quote(str(input_["aggregate_managed_notification_event_arn"]), safe=""),
    )
    params: dict[str, str] = {}
    ...
    if "related_account" in input_:
        params["relatedAccount"] = str(input_["related_account"])
    if "organizational_unit_id" in input_:
        params["organizationalUnitId"] = str(input_["organizational_unit_id"])
