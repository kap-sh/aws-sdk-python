...
def build_request(
    ...
    input: aws_sdk_notifications.types.list_notification_configurations_request.ListNotificationConfigurationsRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "event_rule_source" in input:
        params["eventRuleSource"] = str(input["event_rule_source"])
    if "channel_arn" in input:
        params["channelArn"] = str(input["channel_arn"])
    if "status" in input:
        params["status"] = str(input["status"])
    if "subtype" in input:
        params["subtype"] = str(input["subtype"])
