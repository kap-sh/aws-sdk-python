...
EventRules: TypeAlias = list[
    "aws_sdk_notifications.types.event_rule_structure.EventRuleStructure"
]
...
def serialize_json(value: EventRules) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_notifications.types.event_rule_structure.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> EventRules:
    ...
    for item in data:
        out.append(
            aws_sdk_notifications.types.event_rule_structure.deserialize_json(item)
        )
    return out
