...
class GetManagedNotificationEventResponse(TypedDict):
...
def serialize_json(value: GetManagedNotificationEventResponse) -> dict:
    ...
    import aws_sdk_notifications.types.managed_notification_event
    out["content"] = aws_sdk_notifications.types.managed_notification_event.serialize_json(value["content"])
    return out
...
def deserialize_json(data: dict) -> GetManagedNotificationEventResponse:
    ...
    if "managedNotificationConfigurationArn" in data:
        ...
        raise DeserializationError("GetManagedNotificationEventResponse.managed_notification_configuration_arn required")
    if "creationTime" in data:
        import aws_sdk_notifications.types.creation_time
        out["creation_time"] = aws_sdk_notifications.types.creation_time.deserialize_json(data["creationTime"])
    else:
        raise DeserializationError("GetManagedNotificationEventResponse.creation_time required")
    if "content" in data:
        import aws_sdk_notifications.types.managed_notification_event
        out["content"] = aws_sdk_notifications.types.managed_notification_event.deserialize_json(data["content"])
    else:
        raise DeserializationError("GetManagedNotificationEventResponse.content required")
    return out