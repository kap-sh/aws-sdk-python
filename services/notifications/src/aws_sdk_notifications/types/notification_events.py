...
NotificationEvents: TypeAlias = list[
    "aws_sdk_notifications.types.notification_event_overview.NotificationEventOverview"
]
...
def serialize_json(value: NotificationEvents) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_notifications.types.notification_event_overview.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> NotificationEvents:
    ...
    for item in data:
        out.append(
            aws_sdk_notifications.types.notification_event_overview.deserialize_json(
                item
            )
        )
    return out
