...
class GetNotificationEventResponse(TypedDict):
    ...
    content: "aws_sdk_notifications.types.notification_event_schema.NotificationEventSchema"
...
def serialize_json(value: GetNotificationEventResponse) -> dict:
    ...
    import aws_sdk_notifications.types.notification_event_schema
    out["content"] = aws_sdk_notifications.types.notification_event_schema.serialize_json(value["content"])
    return out
...
def deserialize_json(data: dict) -> GetNotificationEventResponse:
    ...
    if "notificationConfigurationArn" in data:
        ...
        raise DeserializationError("GetNotificationEventResponse.notification_configuration_arn required")
    if "creationTime" in data:
        import aws_sdk_notifications.types.creation_time
        out["creation_time"] = aws_sdk_notifications.types.creation_time.deserialize_json(data["creationTime"])
    else:
        raise DeserializationError("GetNotificationEventResponse.creation_time required")
    if "content" in data:
        import aws_sdk_notifications.types.notification_event_schema
        out["content"] = aws_sdk_notifications.types.notification_event_schema.deserialize_json(data["content"])
    else:
