"""Generated from Smithy shape ``com.amazonaws.synthetics#StartCanaryDryRun``."""
...
def handle_response(
    ...
    out: capo_synthetics.types.start_canary_dry_run_response.StartCanaryDryRunResponse = capo_synthetics.types.start_canary_dry_run_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/canary/{Name}/dry-run/start"
    url = url.replace("{Name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_synthetics.types.start_canary_dry_run_request.serialize_json(input_)
    ).encode()
...
async def async_start_canary_dry_run(
