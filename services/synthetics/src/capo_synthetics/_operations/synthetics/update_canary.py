"""Generated from Smithy shape ``com.amazonaws.synthetics#UpdateCanary``."""

from __future__ import annotations

import json
from typing import Any
from urllib.parse import quote

import zapros
from typing_extensions import Never

import capo_synthetics._auth._signers
import capo_synthetics._auth._sigv4
import capo_synthetics.errors.access_denied_exception
import capo_synthetics.errors.conflict_exception
import capo_synthetics.errors.internal_server_exception
import capo_synthetics.errors.request_entity_too_large_exception
import capo_synthetics.errors.resource_not_found_exception
import capo_synthetics.errors.validation_exception
import capo_synthetics.types.artifact_config_input
import capo_synthetics.types.browser_configs
import capo_synthetics.types.canary_code_input
import capo_synthetics.types.canary_run_config_input
import capo_synthetics.types.canary_schedule_input
import capo_synthetics.types.provisioned_resource_cleanup_setting
import capo_synthetics.types.update_canary_request
import capo_synthetics.types.update_canary_response
import capo_synthetics.types.visual_reference_input
import capo_synthetics.types.visual_references
import capo_synthetics.types.vpc_config_input
from capo_synthetics._protocol.errors import parse_error_metadata_json
from capo_synthetics._rule_engine._endpoint_rule_set import EndpointParams, resolve
from capo_synthetics._services._pipeline import AsyncOperationOptions, OperationOptions
from capo_synthetics.errors import UnknownServiceError


def handle_error(response: zapros.Response) -> Never:
    data = json.loads(response.read())
    code, message = parse_error_metadata_json(response, data)
    match code:
        case "AccessDeniedException":
            raise capo_synthetics.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
        case "ConflictException":
            raise capo_synthetics.errors.conflict_exception.ConflictException.from_json(
                data
            )
        case "InternalServerException":
            raise capo_synthetics.errors.internal_server_exception.InternalServerException.from_json(
                data
            )
        case "RequestEntityTooLargeException":
            raise capo_synthetics.errors.request_entity_too_large_exception.RequestEntityTooLargeException.from_json(
                data
            )
        case "ResourceNotFoundException":
            raise capo_synthetics.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
        case "ValidationException":
            raise capo_synthetics.errors.validation_exception.ValidationException.from_json(
                data
            )
        case _:
            raise UnknownServiceError(code=code, message=message, response=response)


def handle_response(
    response: zapros.Response,
) -> capo_synthetics.types.update_canary_response.UpdateCanaryResponse:
    out: capo_synthetics.types.update_canary_response.UpdateCanaryResponse = {}  # type: ignore[typeddict-item]
    return out


async def async_handle_response(
    response: zapros.Response,
) -> capo_synthetics.types.update_canary_response.UpdateCanaryResponse:
    out: capo_synthetics.types.update_canary_response.UpdateCanaryResponse = {}  # type: ignore[typeddict-item]
    return out


def get_signer(
    options: AsyncOperationOptions | OperationOptions,
    auth_schemes: list[dict[str, Any]] | None = None,
) -> capo_synthetics._auth._signers.Signer | None:
    name_to_schema = {s["name"]: s for s in (auth_schemes or [])}  # noqa: F841
    if options.credentials_provider is not None:
        sigv4_config = (
            name_to_schema.get("sigv4")
            or name_to_schema.get("sigv4a")
            or name_to_schema.get("sigv4-s3express")
            or capo_synthetics._auth._sigv4.build_sigv4_auth_scheme(
                "synthetics", options.region
            )
        )
        if sigv4_config is not None:
            return capo_synthetics._auth._signers.SigV4Signer(
                options.credentials_provider, auth_scheme=sigv4_config
            )
    raise RuntimeError("Auth was not resolved")


def build_request(
    options: OperationOptions | AsyncOperationOptions,
    input_: capo_synthetics.types.update_canary_request.UpdateCanaryRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
            Region=options.region,
            UseDualStack=options.use_dual_stack,
            UseFIPS=options.use_fips,
            Endpoint=options.endpoint,
        )
    )  # noqa: F841
    url = endpoint.url.rstrip("/") + "/canary/{Name}"
    url = url.replace("{Name}", quote(str(input_["name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_synthetics.types.update_canary_request.serialize_json(input_)
    ).encode()
    headers["content-type"] = "application/json"
    signer = get_signer(options, auth_schemes=endpoint.properties.get("authSchemes"))
    normalized_url = zapros.URL(url)
    normalized_url.search_params.update(params)
    return zapros.Request(
        normalized_url, "PATCH", headers=headers, body=body, context={"signer": signer}
    )


def update_canary(
    options: OperationOptions,
    input_: capo_synthetics.types.update_canary_request.UpdateCanaryRequest,
) -> tuple[
    capo_synthetics.types.update_canary_response.UpdateCanaryResponse, zapros.Response
]:
    response = options.client.handler.handle(build_request(options, input_))
    try:
        if response.status >= 400:
            response.read()
            handle_error(response)
        return handle_response(response), response
    except BaseException:
        response.close()
        raise


async def async_update_canary(
    options: AsyncOperationOptions,
    input_: capo_synthetics.types.update_canary_request.UpdateCanaryRequest,
) -> tuple[
    capo_synthetics.types.update_canary_response.UpdateCanaryResponse, zapros.Response
]:
    response = await options.client.handler.ahandle(build_request(options, input_))
    try:
        if response.status >= 400:
            await response.aread()
            handle_error(response)
        return await async_handle_response(response), response
    except BaseException:
        await response.aclose()
        raise
