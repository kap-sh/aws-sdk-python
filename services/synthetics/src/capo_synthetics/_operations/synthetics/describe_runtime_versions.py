"""Generated from Smithy shape ``com.amazonaws.synthetics#DescribeRuntimeVersions``."""
...
def handle_response(
    ...
    out: capo_synthetics.types.describe_runtime_versions_response.DescribeRuntimeVersionsResponse = capo_synthetics.types.describe_runtime_versions_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/runtime-versions"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_synthetics.types.describe_runtime_versions_request.serialize_json(input_)
    ).encode()
...
async def async_describe_runtime_versions(
