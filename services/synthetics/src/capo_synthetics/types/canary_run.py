...
class CanaryRun(TypedDict, closed=True):
    id: NotRequired["capo_synthetics.types.uuid.UUID"]
    """<p>A unique ID that identifies this canary run.</p>"""
    scheduled_run_id: NotRequired["capo_synthetics.types.uuid.UUID"]
    """<p>The ID of the scheduled canary run.</p>"""
    retry_attempt: NotRequired["capo_synthetics.types.retry_attempt.RetryAttempt"]
    ...
    status: NotRequired["capo_synthetics.types.canary_run_status.CanaryRunStatus"]
    """<p>The status of this run.</p>"""
    timeline: NotRequired["capo_synthetics.types.canary_run_timeline.CanaryRunTimeline"]
    ...
    dry_run_config: NotRequired[
        "capo_synthetics.types.canary_dry_run_config_output.CanaryDryRunConfigOutput"
    ]
...
def serialize_json(value: CanaryRun) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_synthetics.types.canary_run_status.serialize_json(
            value["status"]
        )
    if "timeline" in value:
        import capo_synthetics.types.canary_run_timeline

        out["Timeline"] = capo_synthetics.types.canary_run_timeline.serialize_json(
            value["timeline"]
        )
    ...
    if "dry_run_config" in value:
        ...
        out["DryRunConfig"] = (
            capo_synthetics.types.canary_dry_run_config_output.serialize_json(
...
def deserialize_json(data: dict) -> CanaryRun:
    ...
    if "DryRunConfig" in data:
        ...
        out["dry_run_config"] = (
            capo_synthetics.types.canary_dry_run_config_output.deserialize_json(
                data["DryRunConfig"]
            )
        )
