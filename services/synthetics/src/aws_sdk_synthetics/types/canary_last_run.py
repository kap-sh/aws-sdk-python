...
class CanaryLastRun(TypedDict):
    canary_name: NotRequired["aws_sdk_synthetics.types.canary_name.CanaryName"]
    """<p>The name of the canary.</p>"""
    last_run: NotRequired["aws_sdk_synthetics.types.canary_run.CanaryRun"]
...
def serialize_json(value: CanaryLastRun) -> dict:
    ...
    if "last_run" in value:
        ...
        out["LastRun"] = aws_sdk_synthetics.types.canary_run.serialize_json(
            value["last_run"]
        )
    return out
...
def deserialize_json(data: dict) -> CanaryLastRun:
    ...
    if "LastRun" in data:
        ...
        out["last_run"] = aws_sdk_synthetics.types.canary_run.deserialize_json(
            data["LastRun"]
        )
    return out
