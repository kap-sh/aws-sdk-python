"""Generated from Smithy shape ``com.amazonaws.sesv2#CreateEmailIdentity``."""
...
import capo_sesv2.types.create_email_identity_request
import capo_sesv2.types.create_email_identity_response
...
def handle_response(
    ...
    out: capo_sesv2.types.create_email_identity_response.CreateEmailIdentityResponse = (
        capo_sesv2.types.create_email_identity_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/identities"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.create_email_identity_request.serialize_json(input_)
    ).encode()
