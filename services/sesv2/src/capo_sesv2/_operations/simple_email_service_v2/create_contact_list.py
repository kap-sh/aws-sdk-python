"""Generated from Smithy shape ``com.amazonaws.sesv2#CreateContactList``."""
...
import capo_sesv2.types.create_contact_list_request
import capo_sesv2.types.create_contact_list_response
...
) -> capo_sesv2.types.create_contact_list_response.CreateContactListResponse:
    out: capo_sesv2.types.create_contact_list_response.CreateContactListResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/contact-lists"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.create_contact_list_request.serialize_json(input_)
    ).encode()
...
async def async_create_contact_list(
