"""Generated from Smithy shape ``com.amazonaws.sesv2#UpdateReputationEntityPolicy``."""
...
import capo_sesv2.errors.conflict_exception
import capo_sesv2.errors.too_many_requests_exception
import capo_sesv2.types.reputation_entity_type
import capo_sesv2.types.update_reputation_entity_policy_request
import capo_sesv2.types.update_reputation_entity_policy_response
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_sesv2.errors.conflict_exception.ConflictException.from_json(data)
...
) -> capo_sesv2.types.update_reputation_entity_policy_response.UpdateReputationEntityPolicyResponse:
    out: capo_sesv2.types.update_reputation_entity_policy_response.UpdateReputationEntityPolicyResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = url.replace(
        "{ReputationEntityType}", quote(str(input_["reputation_entity_type"]), safe="")
    )
    url = url.replace(
        "{ReputationEntityReference}",
        quote(str(input_["reputation_entity_reference"]), safe=""),
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.update_reputation_entity_policy_request.serialize_json(input_)
    ).encode()
...
async def async_update_reputation_entity_policy(
