"""Generated from Smithy shape ``com.amazonaws.sesv2#PutAccountDedicatedIpWarmupAttributes``."""
...
) -> capo_sesv2.types.put_account_dedicated_ip_warmup_attributes_response.PutAccountDedicatedIpWarmupAttributesResponse:
    out: capo_sesv2.types.put_account_dedicated_ip_warmup_attributes_response.PutAccountDedicatedIpWarmupAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/account/dedicated-ips/warmup"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_account_dedicated_ip_warmup_attributes_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_account_dedicated_ip_warmup_attributes(
