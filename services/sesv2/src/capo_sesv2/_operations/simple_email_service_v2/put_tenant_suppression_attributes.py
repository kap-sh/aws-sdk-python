"""Generated from Smithy shape ``com.amazonaws.sesv2#PutTenantSuppressionAttributes``."""
...
import capo_sesv2.types.put_tenant_suppression_attributes_request
import capo_sesv2.types.put_tenant_suppression_attributes_response
...
) -> capo_sesv2.types.put_tenant_suppression_attributes_response.PutTenantSuppressionAttributesResponse:
    out: capo_sesv2.types.put_tenant_suppression_attributes_response.PutTenantSuppressionAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/tenant/suppression"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_tenant_suppression_attributes_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_tenant_suppression_attributes(
