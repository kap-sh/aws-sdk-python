"""Generated from Smithy shape ``com.amazonaws.sesv2#PutDedicatedIpPoolScalingAttributes``."""
...
import capo_sesv2.types.put_dedicated_ip_pool_scaling_attributes_request
import capo_sesv2.types.put_dedicated_ip_pool_scaling_attributes_response
...
) -> capo_sesv2.types.put_dedicated_ip_pool_scaling_attributes_response.PutDedicatedIpPoolScalingAttributesResponse:
    out: capo_sesv2.types.put_dedicated_ip_pool_scaling_attributes_response.PutDedicatedIpPoolScalingAttributesResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/dedicated-ip-pools/{PoolName}/scaling"
    url = url.replace("{PoolName}", quote(str(input_["pool_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_dedicated_ip_pool_scaling_attributes_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_put_dedicated_ip_pool_scaling_attributes(
