"""Generated from Smithy shape ``com.amazonaws.sesv2#PutDedicatedIpInPool``."""
...
import capo_sesv2.types.put_dedicated_ip_in_pool_request
import capo_sesv2.types.put_dedicated_ip_in_pool_response
...
) -> capo_sesv2.types.put_dedicated_ip_in_pool_response.PutDedicatedIpInPoolResponse:
    out: capo_sesv2.types.put_dedicated_ip_in_pool_response.PutDedicatedIpInPoolResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/dedicated-ips/{Ip}/pool"
    url = url.replace("{Ip}", quote(str(input_["ip"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.put_dedicated_ip_in_pool_request.serialize_json(input_)
    ).encode()
