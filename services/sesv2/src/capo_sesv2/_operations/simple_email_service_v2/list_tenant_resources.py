"""Generated from Smithy shape ``com.amazonaws.sesv2#ListTenantResources``."""
...
) -> capo_sesv2.types.list_tenant_resources_response.ListTenantResourcesResponse:
    out: capo_sesv2.types.list_tenant_resources_response.ListTenantResourcesResponse = (
        capo_sesv2.types.list_tenant_resources_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/tenants/resources/list"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_sesv2.types.list_tenant_resources_request.serialize_json(input_)
    ).encode()
...
async def async_list_tenant_resources(
