...
if TYPE_CHECKING:
    import capo_sesv2.types.blacklisting_description
    import capo_sesv2.types.rbl_name
    import capo_sesv2.types.timestamp


class BlacklistEntry(TypedDict, closed=True):
    rbl_name: NotRequired["capo_sesv2.types.rbl_name.RblName"]
    """<p>The name of the blacklist that the IP address appears on.</p>"""
    listing_time: NotRequired["capo_sesv2.types.timestamp.Timestamp"]
    """<p>The time when the blacklisting event occurred.</p>"""
    description: NotRequired[
        "capo_sesv2.types.blacklisting_description.BlacklistingDescription"
    ]
...
def serialize_json(value: BlacklistEntry) -> dict:
    ...
    if "listing_time" in value:
        ...
        out["ListingTime"] = capo_sesv2.types.timestamp.serialize_json(
            value["listing_time"]
        )
