...
if TYPE_CHECKING:
    import capo_sesv2.types.case_id
    import capo_sesv2.types.review_status


class ReviewDetails(TypedDict, closed=True):
    status: NotRequired["capo_sesv2.types.review_status.ReviewStatus"]
    ...
    case_id: NotRequired["capo_sesv2.types.case_id.CaseId"]
...
def serialize_json(value: ReviewDetails) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_sesv2.types.review_status.serialize_json(value["status"])
    if "case_id" in value:
