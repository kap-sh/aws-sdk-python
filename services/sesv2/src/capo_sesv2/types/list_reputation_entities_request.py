...
if TYPE_CHECKING:
    ...
    import capo_sesv2.types.reputation_entity_filter


class ListReputationEntitiesRequest(TypedDict, closed=True):
    filter: NotRequired[
        "capo_sesv2.types.reputation_entity_filter.ReputationEntityFilter"
    ]
...
def serialize_json(value: ListReputationEntitiesRequest) -> dict:
    ...
    if "filter" in value:
        ...
        out["Filter"] = capo_sesv2.types.reputation_entity_filter.serialize_json(
            value["filter"]
        )
...
def deserialize_json(data: dict) -> ListReputationEntitiesRequest:
    ...
    if "Filter" in data:
        ...
        out["filter"] = capo_sesv2.types.reputation_entity_filter.deserialize_json(
            data["Filter"]
        )
