...
MultiRegionEndpoints: TypeAlias = list[
    "capo_sesv2.types.multi_region_endpoint.MultiRegionEndpoint"
]
...
def serialize_json(value: MultiRegionEndpoints) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_sesv2.types.multi_region_endpoint.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> MultiRegionEndpoints:
    ...
    for item in data:
        out.append(capo_sesv2.types.multi_region_endpoint.deserialize_json(item))
    return out
