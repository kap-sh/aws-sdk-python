...
class MailboxValidation(TypedDict, closed=True):
    ...
    evaluations: NotRequired[
        "capo_sesv2.types.email_address_insights_mailbox_evaluations.EmailAddressInsightsMailboxEvaluations"
    ]
...
def serialize_json(value: MailboxValidation) -> dict:
    ...
    if "is_valid" in value:
        ...
        out["IsValid"] = capo_sesv2.types.email_address_insights_verdict.serialize_json(
            value["is_valid"]
        )
    if "evaluations" in value:
        ...
        out["Evaluations"] = (
            capo_sesv2.types.email_address_insights_mailbox_evaluations.serialize_json(
                value["evaluations"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> MailboxValidation:
    ...
    if "Evaluations" in data:
        ...
        out["evaluations"] = (
            capo_sesv2.types.email_address_insights_mailbox_evaluations.deserialize_json(
                data["Evaluations"]
            )
        )
    return out
