...
class InsightsEvent(TypedDict, closed=True):
    timestamp: NotRequired["capo_sesv2.types.timestamp.Timestamp"]
    """<p>The timestamp of the event.</p>"""
    type: NotRequired["capo_sesv2.types.event_type.EventType"]
    ...
    details: NotRequired["capo_sesv2.types.event_details.EventDetails"]
...
def serialize_json(value: InsightsEvent) -> dict:
    ...
    if "timestamp" in value:
        ...
        out["Timestamp"] = capo_sesv2.types.timestamp.serialize_json(value["timestamp"])
    if "type" in value:
        import capo_sesv2.types.event_type

        out["Type"] = capo_sesv2.types.event_type.serialize_json(value["type"])
    if "details" in value:
        import capo_sesv2.types.event_details

        out["Details"] = capo_sesv2.types.event_details.serialize_json(value["details"])
    return out
