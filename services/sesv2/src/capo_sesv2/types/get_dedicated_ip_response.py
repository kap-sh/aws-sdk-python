...
class GetDedicatedIpResponse(TypedDict, closed=True):
    dedicated_ip: NotRequired["capo_sesv2.types.dedicated_ip.DedicatedIp"]
...
def serialize_json(value: GetDedicatedIpResponse) -> dict:
    ...
    if "dedicated_ip" in value:
        ...
        out["DedicatedIp"] = capo_sesv2.types.dedicated_ip.serialize_json(
            value["dedicated_ip"]
        )
    return out
...
def deserialize_json(data: dict) -> GetDedicatedIpResponse:
    ...
    if "DedicatedIp" in data:
        ...
        out["dedicated_ip"] = capo_sesv2.types.dedicated_ip.deserialize_json(
            data["DedicatedIp"]
        )
    return out
