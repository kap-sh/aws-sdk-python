"""Generated from Smithy shape ``com.amazonaws.sesv2#DeleteContactList``."""
...
def handle_response(
    ...
    out: aws_sdk_sesv2.types.delete_contact_list_response.DeleteContactListResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_sesv2.types.delete_contact_list_request.DeleteContactListRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/v2/email/contact-lists/{ContactListName}"
    url = url.replace(
        "{ContactListName}", quote(str(input_["contact_list_name"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_delete_contact_list(
