"""Generated from Smithy shape ``com.amazonaws.sesv2#CreateContact``."""
...
def handle_response(
    ...
    out: aws_sdk_sesv2.types.create_contact_response.CreateContactResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    input_: aws_sdk_sesv2.types.create_contact_request.CreateContactRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/v2/email/contact-lists/{ContactListName}/contacts"
    )
    url = url.replace(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_sesv2.types.create_contact_request.serialize_json(input_)
    ).encode()
...
def create_contact(
    ...
    aws_sdk_sesv2.types.create_contact_response.CreateContactResponse, zapros.Response
]:
