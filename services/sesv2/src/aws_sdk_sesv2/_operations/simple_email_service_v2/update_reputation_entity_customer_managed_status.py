...
def build_request(
    ...
    input: aws_sdk_sesv2.types.update_reputation_entity_customer_managed_status_request.UpdateReputationEntityCustomerManagedStatusRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{ReputationEntityType}", quote(str(input["reputation_entity_type"]), safe="")
    )
    url = url.replace(
        "{ReputationEntityReference}",
        quote(str(input["reputation_entity_reference"]), safe=""),
    )
    params: dict[str, str] = {}
