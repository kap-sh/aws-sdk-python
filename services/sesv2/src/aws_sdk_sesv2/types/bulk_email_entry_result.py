...
if TYPE_CHECKING:
    import aws_sdk_sesv2.types.bulk_email_status
    import aws_sdk_sesv2.types.error_message
    import aws_sdk_sesv2.types.outbound_message_id


class BulkEmailEntryResult(TypedDict):
    status: NotRequired["aws_sdk_sesv2.types.bulk_email_status.BulkEmailStatus"]
    ...
    error: NotRequired["aws_sdk_sesv2.types.error_message.ErrorMessage"]
...
def serialize_json(value: BulkEmailEntryResult) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = aws_sdk_sesv2.types.bulk_email_status.serialize_json(
            value["status"]
        )
    if "error" in value:
...
def deserialize_json(data: dict) -> BulkEmailEntryResult:
    ...
    if "Status" in data:
        ...
        out["status"] = aws_sdk_sesv2.types.bulk_email_status.deserialize_json(
            data["Status"]
        )
    if "Error" in data:
