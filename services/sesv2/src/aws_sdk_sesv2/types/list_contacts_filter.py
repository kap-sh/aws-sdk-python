...
if TYPE_CHECKING:
    ...
    import aws_sdk_sesv2.types.topic_filter


class ListContactsFilter(TypedDict):
    ...
    topic_filter: NotRequired["aws_sdk_sesv2.types.topic_filter.TopicFilter"]
...
def serialize_json(value: ListContactsFilter) -> dict:
    ...
    if "filtered_status" in value:
        ...
        out["FilteredStatus"] = aws_sdk_sesv2.types.subscription_status.serialize_json(
            value["filtered_status"]
        )
    if "topic_filter" in value:
        import aws_sdk_sesv2.types.topic_filter

        out["TopicFilter"] = aws_sdk_sesv2.types.topic_filter.serialize_json(
            value["topic_filter"]
        )
    return out
...
def deserialize_json(data: dict) -> ListContactsFilter:
    ...
    if "TopicFilter" in data:
        ...
        out["topic_filter"] = aws_sdk_sesv2.types.topic_filter.deserialize_json(
            data["TopicFilter"]
        )
    return out
