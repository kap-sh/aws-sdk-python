...
def serialize_json(value: Destination) -> dict:
    ...
    if "to_addresses" in value:
        ...
        out["ToAddresses"] = aws_sdk_sesv2.types.email_address_list.serialize_json(
            value["to_addresses"]
        )
    if "cc_addresses" in value:
        import aws_sdk_sesv2.types.email_address_list

        out["CcAddresses"] = aws_sdk_sesv2.types.email_address_list.serialize_json(
            value["cc_addresses"]
        )
    if "bcc_addresses" in value:
        import aws_sdk_sesv2.types.email_address_list

        out["BccAddresses"] = aws_sdk_sesv2.types.email_address_list.serialize_json(
            value["bcc_addresses"]
        )
    return out
...
def deserialize_json(data: dict) -> Destination:
    ...
    if "ToAddresses" in data:
        ...
        out["to_addresses"] = aws_sdk_sesv2.types.email_address_list.deserialize_json(
            data["ToAddresses"]
        )
    if "CcAddresses" in data:
        import aws_sdk_sesv2.types.email_address_list

        out["cc_addresses"] = aws_sdk_sesv2.types.email_address_list.deserialize_json(
            data["CcAddresses"]
        )
    if "BccAddresses" in data:
        import aws_sdk_sesv2.types.email_address_list

        out["bcc_addresses"] = aws_sdk_sesv2.types.email_address_list.deserialize_json(
            data["BccAddresses"]
        )
    return out
