...
if TYPE_CHECKING:
    ...
    import aws_sdk_sesv2.types.warmup_status


class DedicatedIp(TypedDict, closed=True):
    ip: "aws_sdk_sesv2.types.ip.Ip"
    """<p>An IPv4 address.</p>"""
    warmup_status: "aws_sdk_sesv2.types.warmup_status.WarmupStatus"
...
def serialize_json(value: DedicatedIp) -> dict:
    ...
    out["WarmupStatus"] = aws_sdk_sesv2.types.warmup_status.serialize_json(
        value["warmup_status"]
    )
...
def deserialize_json(data: dict) -> DedicatedIp:
    ...
    if "WarmupStatus" in data:
        ...
        out["warmup_status"] = aws_sdk_sesv2.types.warmup_status.deserialize_json(
            data["WarmupStatus"]
        )
    else:
