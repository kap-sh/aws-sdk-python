"""Generated from Smithy shape ``com.amazonaws.sesv2#TenantResourceList``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_sesv2.types.tenant_resource

TenantResourceList: TypeAlias = list[
    "aws_sdk_sesv2.types.tenant_resource.TenantResource"
]


# --- restJson1 ser/de ---
def serialize_json(value: TenantResourceList) -> list:
    import aws_sdk_sesv2.types.tenant_resource

    out: list = []
    for item in value:
        out.append(aws_sdk_sesv2.types.tenant_resource.serialize_json(item))
    return out


def deserialize_json(data: list) -> TenantResourceList:
    import aws_sdk_sesv2.types.tenant_resource

    out: TenantResourceList = []
    for item in data:
        out.append(aws_sdk_sesv2.types.tenant_resource.deserialize_json(item))
    return out
