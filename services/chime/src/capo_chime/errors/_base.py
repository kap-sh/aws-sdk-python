...
class ChimeError(Exception):
    """Base class for all Chime service errors."""


class ServiceError(ChimeError):
...
class SerializationError(ChimeError):
    """Raised when a value cannot be serialized."""


class DeserializationError(ChimeError):
...
class WaiterFailedError(ChimeError):
...
class WaiterTimeoutError(ChimeError):
