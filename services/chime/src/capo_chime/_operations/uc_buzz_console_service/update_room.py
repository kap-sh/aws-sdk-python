"""Generated from Smithy shape ``com.amazonaws.chime#UpdateRoom``."""
...
def handle_response(
    ...
    out: capo_chime.types.update_room_response.UpdateRoomResponse = (
        capo_chime.types.update_room_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AccountId}/rooms/{RoomId}"
    ...
    body: bytes | None = json.dumps(
        capo_chime.types.update_room_request.serialize_json(input_)
    ).encode()
...
def update_room(
    ...
    input_: capo_chime.types.update_room_request.UpdateRoomRequest,
) -> tuple[capo_chime.types.update_room_response.UpdateRoomResponse, zapros.Response]:
