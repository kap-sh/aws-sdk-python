"""Generated from Smithy shape ``com.amazonaws.chime#BatchSuspendUser``."""
...
def handle_response(
    ...
    out: capo_chime.types.batch_suspend_user_response.BatchSuspendUserResponse = (
        capo_chime.types.batch_suspend_user_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AccountId}/users?operation=suspend"
    url = url.replace("{AccountId}", quote(str(input_["account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime.types.batch_suspend_user_request.serialize_json(input_)
    ).encode()
...
async def async_batch_suspend_user(
