"""Generated from Smithy shape ``com.amazonaws.chime#CreateBot``."""
...
def handle_response(
    ...
    out: capo_chime.types.create_bot_response.CreateBotResponse = (
        capo_chime.types.create_bot_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AccountId}/bots"
    url = url.replace("{AccountId}", quote(str(input_["account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime.types.create_bot_request.serialize_json(input_)
    ).encode()
...
def create_bot(
    ...
    input_: capo_chime.types.create_bot_request.CreateBotRequest,
) -> tuple[capo_chime.types.create_bot_response.CreateBotResponse, zapros.Response]:
