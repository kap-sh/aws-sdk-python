"""Generated from Smithy shape ``com.amazonaws.chime#UpdateAccountSettings``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_chime.errors.conflict_exception.ConflictException.from_json(data)
...
def handle_response(
    ...
    out: capo_chime.types.update_account_settings_response.UpdateAccountSettingsResponse = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AccountId}/settings"
    url = url.replace("{AccountId}", quote(str(input_["account_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime.types.update_account_settings_request.serialize_json(input_)
    ).encode()
