"""Generated from Smithy shape ``com.amazonaws.chime#CreateMeetingDialOut``."""
...
def handle_response(
    ...
    out: capo_chime.types.create_meeting_dial_out_response.CreateMeetingDialOutResponse = capo_chime.types.create_meeting_dial_out_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/meetings/{MeetingId}/dial-outs"
    url = url.replace("{MeetingId}", quote(str(input_["meeting_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chime.types.create_meeting_dial_out_request.serialize_json(input_)
    ).encode()
...
async def async_create_meeting_dial_out(
