        ...
        if _locals["PartitionResult"] is not None:
            if string_equals(
                ...
                if p.UseFIPS is False:
                    if p.UseDualStack is False:
                        return Endpoint(
                            url=interpolate(
                                "https://chime.us-east-1.amazonaws.com", p, _locals
                            ),
                            properties={
                                "authSchemes": [
                                    {
                                        "name": interpolate("sigv4", p, _locals),
                                        "signingName": interpolate("chime", p, _locals),
            ...
            if p.UseFIPS is True:
                if (
                    ...
                    return Endpoint(
                        url=interpolate(
                            "https://chime-fips.{Region}.{PartitionResult#dnsSuffix}",
                            p,
                            _locals,
                        ),
                        properties={},
            ...
            return Endpoint(
                url=interpolate(
                    "https://chime.{Region}.{PartitionResult#dnsSuffix}", p, _locals
                ),
                properties={},
                headers={},
            )
