...
if TYPE_CHECKING:
    ...
    import aws_sdk_chime.types.ordered_phone_number_status
...
class OrderedPhoneNumber(TypedDict, closed=True):
    ...
    status: NotRequired[
        "aws_sdk_chime.types.ordered_phone_number_status.OrderedPhoneNumberStatus"
    ]
...
def serialize_json(value: OrderedPhoneNumber) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = aws_sdk_chime.types.ordered_phone_number_status.serialize_json(
            value["status"]
        )
    return out
...
def deserialize_json(data: dict) -> OrderedPhoneNumber:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            aws_sdk_chime.types.ordered_phone_number_status.deserialize_json(
                data["Status"]
            )
        )
    return out
