...
if TYPE_CHECKING:
    ...
    import aws_sdk_chime.types.member
...
class RoomMembership(TypedDict, closed=True):
    ...
    invited_by: NotRequired["aws_sdk_chime.types.non_empty_string.NonEmptyString"]
...
def serialize_json(value: RoomMembership) -> dict:
    ...
    if "member" in value:
        ...
        out["Member"] = aws_sdk_chime.types.member.serialize_json(value["member"])
    if "role" in value:
        import aws_sdk_chime.types.room_membership_role
...
def deserialize_json(data: dict) -> RoomMembership:
    ...
    if "Member" in data:
        ...
        out["member"] = aws_sdk_chime.types.member.deserialize_json(data["Member"])
    if "Role" in data:
        import aws_sdk_chime.types.room_membership_role
