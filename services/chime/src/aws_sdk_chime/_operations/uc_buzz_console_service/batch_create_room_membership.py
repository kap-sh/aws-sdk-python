...
def build_request(
    ...
    input: aws_sdk_chime.types.batch_create_room_membership_request.BatchCreateRoomMembershipRequest,
) -> zapros.Request:
    ...
    url = url.replace("{RoomId}", quote(str(input["room_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_chime.types.batch_create_room_membership_request

    body: bytes | None = json.dumps(
        aws_sdk_chime.types.batch_create_room_membership_request.serialize_json(input)
    ).encode()
