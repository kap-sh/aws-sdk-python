"""Generated from Smithy shape ``com.amazonaws.chime#BatchUnsuspendUser``."""
...
def handle_response(
    ...
    out: aws_sdk_chime.types.batch_unsuspend_user_response.BatchUnsuspendUserResponse = aws_sdk_chime.types.batch_unsuspend_user_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_chime.types.batch_unsuspend_user_request.BatchUnsuspendUserRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/accounts/{AccountId}/users?operation=unsuspend"
    ...
    body: bytes | None = json.dumps(
        aws_sdk_chime.types.batch_unsuspend_user_request.serialize_json(input_)
    ).encode()
...
async def async_batch_unsuspend_user(
