...
class ACMError(Exception):
    """Base class for all ACM service errors."""


class ServiceError(ACMError):
...
class SerializationError(ACMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(ACMError):
...
class WaiterFailedError(ACMError):
...
class WaiterTimeoutError(ACMError):
