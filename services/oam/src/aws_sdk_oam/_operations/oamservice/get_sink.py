"""Generated from Smithy shape ``com.amazonaws.oam#GetSink``."""
...
def handle_response(
    ...
    out: aws_sdk_oam.types.get_sink_output.GetSinkOutput = (
        aws_sdk_oam.types.get_sink_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
def build_request(
    ...
    input: aws_sdk_oam.types.get_sink_input.GetSinkInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/GetSink"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_oam.types.get_sink_input

    body: bytes | None = json.dumps(
        aws_sdk_oam.types.get_sink_input.serialize_json(input)
    ).encode()
...
def get_sink(
    options: OperationOptions, input: aws_sdk_oam.types.get_sink_input.GetSinkInput
) -> tuple[aws_sdk_oam.types.get_sink_output.GetSinkOutput, zapros.Response]:
...
async def async_get_sink(
    options: AsyncOperationOptions, input: aws_sdk_oam.types.get_sink_input.GetSinkInput
) -> tuple[aws_sdk_oam.types.get_sink_output.GetSinkOutput, zapros.Response]:
