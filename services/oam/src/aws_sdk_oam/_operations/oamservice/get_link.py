"""Generated from Smithy shape ``com.amazonaws.oam#GetLink``."""
...
def handle_response(
    ...
    out: aws_sdk_oam.types.get_link_output.GetLinkOutput = (
        aws_sdk_oam.types.get_link_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
def build_request(
    ...
    input: aws_sdk_oam.types.get_link_input.GetLinkInput,
) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/GetLink"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_oam.types.get_link_input

    body: bytes | None = json.dumps(
        aws_sdk_oam.types.get_link_input.serialize_json(input)
    ).encode()
...
def get_link(
    options: OperationOptions, input: aws_sdk_oam.types.get_link_input.GetLinkInput
) -> tuple[aws_sdk_oam.types.get_link_output.GetLinkOutput, zapros.Response]:
...
async def async_get_link(
    options: AsyncOperationOptions, input: aws_sdk_oam.types.get_link_input.GetLinkInput
) -> tuple[aws_sdk_oam.types.get_link_output.GetLinkOutput, zapros.Response]:
