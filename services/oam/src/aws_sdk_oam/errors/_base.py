...
class OAMError(Exception):
    """Base class for all OAM service errors."""


class ServiceError(OAMError):
...
class SerializationError(OAMError):
    """Raised when a value cannot be serialized."""


class DeserializationError(OAMError):
...
class WaiterFailedError(OAMError):
...
class WaiterTimeoutError(OAMError):
