...
class ListAttachedLinksOutput(TypedDict):
    items: "aws_sdk_oam.types.list_attached_links_items.ListAttachedLinksItems"
...
def serialize_json(value: ListAttachedLinksOutput) -> dict:
    ...
    out["Items"] = aws_sdk_oam.types.list_attached_links_items.serialize_json(
        value["items"]
    )
...
def deserialize_json(data: dict) -> ListAttachedLinksOutput:
    ...
    if "Items" in data:
        ...
        out["items"] = aws_sdk_oam.types.list_attached_links_items.deserialize_json(
            data["Items"]
        )
    else:
