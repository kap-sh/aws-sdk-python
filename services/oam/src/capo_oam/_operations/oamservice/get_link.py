...
def handle_response(
    ...
    out: capo_oam.types.get_link_output.GetLinkOutput = (
        capo_oam.types.get_link_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_oam.types.get_link_output.GetLinkOutput = (
        capo_oam.types.get_link_output.deserialize_json(
...
def build_request(
    ...
    input_: capo_oam.types.get_link_input.GetLinkInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        capo_oam.types.get_link_input.serialize_json(input_)
    ).encode()
...
def get_link(
    options: OperationOptions, input_: capo_oam.types.get_link_input.GetLinkInput
) -> tuple[capo_oam.types.get_link_output.GetLinkOutput, zapros.Response]:
...
async def async_get_link(
    options: AsyncOperationOptions, input_: capo_oam.types.get_link_input.GetLinkInput
) -> tuple[capo_oam.types.get_link_output.GetLinkOutput, zapros.Response]:
