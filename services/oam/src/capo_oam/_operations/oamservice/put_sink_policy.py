"""Generated from Smithy shape ``com.amazonaws.oam#PutSinkPolicy``."""
...
def handle_response(
    ...
    out: capo_oam.types.put_sink_policy_output.PutSinkPolicyOutput = (
        capo_oam.types.put_sink_policy_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/PutSinkPolicy"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_oam.types.put_sink_policy_input.serialize_json(input_)
    ).encode()
...
def put_sink_policy(
    ...
    input_: capo_oam.types.put_sink_policy_input.PutSinkPolicyInput,
) -> tuple[capo_oam.types.put_sink_policy_output.PutSinkPolicyOutput, zapros.Response]:
...
async def async_put_sink_policy(
