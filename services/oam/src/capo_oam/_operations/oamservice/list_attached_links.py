"""Generated from Smithy shape ``com.amazonaws.oam#ListAttachedLinks``."""
...
def handle_response(
    ...
    out: capo_oam.types.list_attached_links_output.ListAttachedLinksOutput = (
        capo_oam.types.list_attached_links_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/ListAttachedLinks"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_oam.types.list_attached_links_input.serialize_json(input_)
    ).encode()
...
def list_attached_links(
    ...
    capo_oam.types.list_attached_links_output.ListAttachedLinksOutput, zapros.Response
]:
...
async def async_list_attached_links(
