...
def handle_response(
    ...
    out: capo_oam.types.update_link_output.UpdateLinkOutput = (
        capo_oam.types.update_link_output.deserialize_json(json.loads(response.read()))
    )
    return out
...
async def async_handle_response(
    ...
    out: capo_oam.types.update_link_output.UpdateLinkOutput = (
        capo_oam.types.update_link_output.deserialize_json(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_oam.types.update_link_input.serialize_json(input_)
    ).encode()
...
def update_link(
    options: OperationOptions, input_: capo_oam.types.update_link_input.UpdateLinkInput
) -> tuple[capo_oam.types.update_link_output.UpdateLinkOutput, zapros.Response]:
