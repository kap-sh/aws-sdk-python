"""Generated from Smithy shape ``com.amazonaws.kinesisvideo#ListOfProtocols``."""

from typing import TYPE_CHECKING, TypeAlias

if TYPE_CHECKING:
    import aws_sdk_kinesis_video.types.channel_protocol

ListOfProtocols: TypeAlias = list[
    "aws_sdk_kinesis_video.types.channel_protocol.ChannelProtocol"
]


# --- restJson1 ser/de ---
def serialize_json(value: ListOfProtocols) -> list:
    import aws_sdk_kinesis_video.types.channel_protocol

    out: list = []
    for item in value:
        out.append(aws_sdk_kinesis_video.types.channel_protocol.serialize_json(item))
    return out


def deserialize_json(data: list) -> ListOfProtocols:
    import aws_sdk_kinesis_video.types.channel_protocol

    out: ListOfProtocols = []
    for item in data:
        out.append(aws_sdk_kinesis_video.types.channel_protocol.deserialize_json(item))
    return out
