...
class StreamInfo(TypedDict):
    ...
    status: NotRequired["aws_sdk_kinesis_video.types.status.Status"]
...
def serialize_json(value: StreamInfo) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = aws_sdk_kinesis_video.types.status.serialize_json(
            value["status"]
        )
...
def deserialize_json(data: dict) -> StreamInfo:
    ...
    if "Status" in data:
        ...
        out["status"] = aws_sdk_kinesis_video.types.status.deserialize_json(
            data["Status"]
        )
