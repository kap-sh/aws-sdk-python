...
if TYPE_CHECKING:
    import aws_sdk_kinesis_video.types.delete_after_upload
    import aws_sdk_kinesis_video.types.edge_retention_in_hours
    import aws_sdk_kinesis_video.types.local_size_config


class DeletionConfig(TypedDict):
    edge_retention_in_hours: NotRequired[
        "aws_sdk_kinesis_video.types.edge_retention_in_hours.EdgeRetentionInHours"
    ]
    ...
    local_size_config: NotRequired[
        "aws_sdk_kinesis_video.types.local_size_config.LocalSizeConfig"
    ]
    """<p>The value of the local size required in order to delete the edge configuration.</p>"""
    delete_after_upload: NotRequired[
        "aws_sdk_kinesis_video.types.delete_after_upload.DeleteAfterUpload"
    ]
...
def serialize_json(value: DeletionConfig) -> dict:
    ...
    if "local_size_config" in value:
        ...
        out["LocalSizeConfig"] = (
            aws_sdk_kinesis_video.types.local_size_config.serialize_json(
...
def deserialize_json(data: dict) -> DeletionConfig:
    ...
    if "LocalSizeConfig" in data:
        ...
        out["local_size_config"] = (
            aws_sdk_kinesis_video.types.local_size_config.deserialize_json(
