"""Generated from Smithy shape ``com.amazonaws.partnercentralaccount#CancelConnectionResponse``."""
...
class CancelConnectionResponse(TypedDict, closed=True):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: CancelConnectionResponse) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_0(data: dict) -> CancelConnectionResponse:
    out: CancelConnectionResponse = {}  # type: ignore[typeddict-item]
    if "Catalog" in data:
        out["catalog"] = data["Catalog"]
    else:
        raise DeserializationError("CancelConnectionResponse.catalog required")
    if "Id" in data:
        out["id"] = data["Id"]
    else:
        raise DeserializationError("CancelConnectionResponse.id required")
    if "Arn" in data:
        out["arn"] = data["Arn"]
    else:
        raise DeserializationError("CancelConnectionResponse.arn required")
    if "OtherParticipantAccountId" in data:
        out["other_participant_account_id"] = data["OtherParticipantAccountId"]
    else:
        raise DeserializationError(
            "CancelConnectionResponse.other_participant_account_id required"
        )
    if "UpdatedAt" in data:
        ...
        raise DeserializationError("CancelConnectionResponse.updated_at required")
    if "ConnectionTypes" in data:
        ...
        raise DeserializationError("CancelConnectionResponse.connection_types required")
    return out
