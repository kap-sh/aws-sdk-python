"""Generated from Smithy shape ``com.amazonaws.cognitosync#DescribeIdentityPoolUsage``."""
...
def handle_response(
    ...
    out: capo_cognito_sync.types.describe_identity_pool_usage_response.DescribeIdentityPoolUsageResponse = capo_cognito_sync.types.describe_identity_pool_usage_response.deserialize_json(
...
def build_request(
    ...
    input_: capo_cognito_sync.types.describe_identity_pool_usage_request.DescribeIdentityPoolUsageRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/identitypools/{IdentityPoolId}"
    url = url.replace(
...
async def async_describe_identity_pool_usage(
