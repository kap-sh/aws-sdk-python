...
def handle_response(
    ...
    out: aws_sdk_cognito_sync.types.list_records_response.ListRecordsResponse = (
        aws_sdk_cognito_sync.types.list_records_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_cognito_sync.types.list_records_request.ListRecordsRequest,
) -> zapros.Request:
    ...
    url = url.replace("{DatasetName}", quote(str(input["dataset_name"]), safe=""))
    params: dict[str, str] = {}
    if "last_sync_count" in input:
        params["lastSyncCount"] = str(input["last_sync_count"])
    if "next_token" in input:
        params["nextToken"] = str(input["next_token"])
    if "max_results" in input:
        params["maxResults"] = str(input["max_results"])
    if "sync_session_token" in input:
        params["syncSessionToken"] = str(input["sync_session_token"])
