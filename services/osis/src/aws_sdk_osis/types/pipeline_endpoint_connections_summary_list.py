...
PipelineEndpointConnectionsSummaryList: TypeAlias = list[
    "aws_sdk_osis.types.pipeline_endpoint_connection.PipelineEndpointConnection"
]
...
def serialize_json(value: PipelineEndpointConnectionsSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_osis.types.pipeline_endpoint_connection.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> PipelineEndpointConnectionsSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_osis.types.pipeline_endpoint_connection.deserialize_json(item)
        )
    return out
