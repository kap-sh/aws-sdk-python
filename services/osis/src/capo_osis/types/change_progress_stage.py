...
if TYPE_CHECKING:
    import capo_osis.types.change_progress_stage_statuses
    import capo_osis.types.string
    import capo_osis.types.timestamp


class ChangeProgressStage(TypedDict, closed=True):
    name: NotRequired["capo_osis.types.string.String"]
    """<p>The name of the stage.</p>"""
    status: NotRequired[
        "capo_osis.types.change_progress_stage_statuses.ChangeProgressStageStatuses"
    ]
...
def serialize_json(value: ChangeProgressStage) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = capo_osis.types.change_progress_stage_statuses.serialize_json(
            value["status"]
        )
...
def deserialize_json(data: dict) -> ChangeProgressStage:
    ...
    if "Status" in data:
        ...
        out["status"] = capo_osis.types.change_progress_stage_statuses.deserialize_json(
            data["Status"]
        )
