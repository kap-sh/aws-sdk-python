...
class ChangeProgressStatus(TypedDict, closed=True):
    start_time: NotRequired["capo_osis.types.timestamp.Timestamp"]
    """<p>The time at which the configuration change is made on the pipeline.</p>"""
    status: NotRequired[
        "capo_osis.types.change_progress_statuses.ChangeProgressStatuses"
    ]
    """<p>The overall status of the pipeline configuration change.</p>"""
    total_number_of_stages: "capo_osis.types.integer.Integer"
    """<p>The total number of stages required for the pipeline configuration change.</p>"""
    change_progress_stages: NotRequired[
        "capo_osis.types.change_progress_stage_list.ChangeProgressStageList"
    ]
...
def serialize_json(value: ChangeProgressStatus) -> dict:
    ...
    if "start_time" in value:
        ...
        out["StartTime"] = capo_osis.types.timestamp.serialize_json(value["start_time"])
    if "status" in value:
        import capo_osis.types.change_progress_statuses

        out["Status"] = capo_osis.types.change_progress_statuses.serialize_json(
            value["status"]
        )
    ...
    if "change_progress_stages" in value:
        ...
        out["ChangeProgressStages"] = (
            capo_osis.types.change_progress_stage_list.serialize_json(
...
def deserialize_json(data: dict) -> ChangeProgressStatus:
    ...
    if "ChangeProgressStages" in data:
        ...
        out["change_progress_stages"] = (
            capo_osis.types.change_progress_stage_list.deserialize_json(
