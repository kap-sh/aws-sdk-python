...
class GetPipelineBlueprintResponse(TypedDict, closed=True):
    blueprint: NotRequired["capo_osis.types.pipeline_blueprint.PipelineBlueprint"]
    """<p>The requested blueprint in YAML format.</p>"""
    format: NotRequired["capo_osis.types.string.String"]
...
def serialize_json(value: GetPipelineBlueprintResponse) -> dict:
    ...
    if "blueprint" in value:
        ...
        out["Blueprint"] = capo_osis.types.pipeline_blueprint.serialize_json(
            value["blueprint"]
        )
    if "format" in value:
