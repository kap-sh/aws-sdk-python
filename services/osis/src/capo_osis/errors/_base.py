...
class OSISError(Exception):
    """Base class for all OSIS service errors."""


class ServiceError(OSISError):
...
class SerializationError(OSISError):
    """Raised when a value cannot be serialized."""


class DeserializationError(OSISError):
...
class WaiterFailedError(OSISError):
...
class WaiterTimeoutError(OSISError):
