"""Generated from Smithy shape ``com.amazonaws.chatbot#DeleteSlackWorkspaceAuthorization``."""
...
import capo_chatbot.errors.delete_slack_workspace_authorization_fault
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DeleteSlackWorkspaceAuthorizationFault":
            raise capo_chatbot.errors.delete_slack_workspace_authorization_fault.DeleteSlackWorkspaceAuthorizationFault.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_chatbot.types.delete_slack_workspace_authorization_result.DeleteSlackWorkspaceAuthorizationResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/delete-slack-workspace-authorization"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.delete_slack_workspace_authorization_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_delete_slack_workspace_authorization(
