"""Generated from Smithy shape ``com.amazonaws.chatbot#DescribeChimeWebhookConfigurations``."""
...
import capo_chatbot.errors.describe_chime_webhook_configurations_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "DescribeChimeWebhookConfigurationsException":
            raise capo_chatbot.errors.describe_chime_webhook_configurations_exception.DescribeChimeWebhookConfigurationsException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_chatbot.types.describe_chime_webhook_configurations_result.DescribeChimeWebhookConfigurationsResult = capo_chatbot.types.describe_chime_webhook_configurations_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/describe-chime-webhook-configurations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.describe_chime_webhook_configurations_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_describe_chime_webhook_configurations(
