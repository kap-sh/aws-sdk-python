"""Generated from Smithy shape ``com.amazonaws.chatbot#AssociateToConfiguration``."""
...
def handle_response(
    ...
    out: capo_chatbot.types.associate_to_configuration_result.AssociateToConfigurationResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/associate-to-configuration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.associate_to_configuration_request.serialize_json(input_)
    ).encode()
...
async def async_associate_to_configuration(
