"""Generated from Smithy shape ``com.amazonaws.chatbot#DisassociateFromConfiguration``."""
...
def handle_response(
    ...
    out: capo_chatbot.types.disassociate_from_configuration_result.DisassociateFromConfigurationResult = {}  # type: ignore[typeddict-item]
    return out
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/disassociate-from-configuration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.disassociate_from_configuration_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_disassociate_from_configuration(
