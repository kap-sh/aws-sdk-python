"""Generated from Smithy shape ``com.amazonaws.chatbot#CreateCustomAction``."""
...
import capo_chatbot.errors.unauthorized_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "UnauthorizedException":
            raise capo_chatbot.errors.unauthorized_exception.UnauthorizedException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_chatbot.types.create_custom_action_result.CreateCustomActionResult = (
        capo_chatbot.types.create_custom_action_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/create-custom-action"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_chatbot.types.create_custom_action_request.serialize_json(input_)
    ).encode()
...
async def async_create_custom_action(
