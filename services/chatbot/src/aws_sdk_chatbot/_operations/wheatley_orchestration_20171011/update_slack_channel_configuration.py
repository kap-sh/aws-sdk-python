"""Generated from Smithy shape ``com.amazonaws.chatbot#UpdateSlackChannelConfiguration``."""
...
import aws_sdk_chatbot.errors.resource_not_found_exception
import aws_sdk_chatbot.errors.update_slack_channel_configuration_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            raise aws_sdk_chatbot.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
        case "UpdateSlackChannelConfigurationException":
            raise aws_sdk_chatbot.errors.update_slack_channel_configuration_exception.UpdateSlackChannelConfigurationException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: aws_sdk_chatbot.types.update_slack_channel_configuration_result.UpdateSlackChannelConfigurationResult = aws_sdk_chatbot.types.update_slack_channel_configuration_result.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_chatbot._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_chatbot._auth._sigv4.build_sigv4_auth_scheme(
                "chatbot", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_chatbot._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_chatbot.types.update_slack_channel_configuration_request.UpdateSlackChannelConfigurationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/update-slack-channel-configuration"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_chatbot.types.update_slack_channel_configuration_request

    body: bytes | None = json.dumps(
        aws_sdk_chatbot.types.update_slack_channel_configuration_request.serialize_json(
            input_
        )
    ).encode()
