...
class GetModelsResponse(TypedDict, closed=True):
    items: NotRequired["capo_apigatewayv2.types.__list_of_model.__listOfModel"]
...
def serialize_json(value: GetModelsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_model.serialize_json(
            value["items"]
        )
