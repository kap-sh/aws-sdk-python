...
if TYPE_CHECKING:
    import capo_apigatewayv2.types.__list_of_portal_product_summary
    import capo_apigatewayv2.types.__string_min1_max2048


class ListPortalProductsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_apigatewayv2.types.__list_of_portal_product_summary.__listOfPortalProductSummary"
    ]
...
def serialize_json(value: ListPortalProductsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = (
            capo_apigatewayv2.types.__list_of_portal_product_summary.serialize_json(
                value["items"]
            )
        )
...
def deserialize_json(data: dict) -> ListPortalProductsResponse:
    ...
    if "items" in data:
        ...
        out["items"] = (
            capo_apigatewayv2.types.__list_of_portal_product_summary.deserialize_json(
                data["items"]
            )
        )
