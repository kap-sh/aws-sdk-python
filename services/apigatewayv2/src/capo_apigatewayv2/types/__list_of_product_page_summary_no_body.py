...
if TYPE_CHECKING:
    import capo_apigatewayv2.types.product_page_summary_no_body

__listOfProductPageSummaryNoBody: TypeAlias = list[
    "capo_apigatewayv2.types.product_page_summary_no_body.ProductPageSummaryNoBody"
]
...
def serialize_json(value: __listOfProductPageSummaryNoBody) -> list:
    ...
    for item in value:
        out.append(
            capo_apigatewayv2.types.product_page_summary_no_body.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> __listOfProductPageSummaryNoBody:
    ...
    for item in data:
        out.append(
            capo_apigatewayv2.types.product_page_summary_no_body.deserialize_json(item)
        )
    return out
