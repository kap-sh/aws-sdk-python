...
class RoutingRuleMatchHeaders(TypedDict, closed=True):
    any_of: NotRequired[
        "capo_apigatewayv2.types.__list_of_routing_rule_match_header_value.__listOfRoutingRuleMatchHeaderValue"
    ]
...
def serialize_json(value: RoutingRuleMatchHeaders) -> dict:
    ...
    if "any_of" in value:
        ...
        out["anyOf"] = (
            capo_apigatewayv2.types.__list_of_routing_rule_match_header_value.serialize_json(
                value["any_of"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> RoutingRuleMatchHeaders:
    ...
    if "anyOf" in data:
        ...
        out["any_of"] = (
            capo_apigatewayv2.types.__list_of_routing_rule_match_header_value.deserialize_json(
                data["anyOf"]
            )
        )
    return out
