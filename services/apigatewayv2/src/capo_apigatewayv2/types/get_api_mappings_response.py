...
class GetApiMappingsResponse(TypedDict, closed=True):
    items: NotRequired[
        "capo_apigatewayv2.types.__list_of_api_mapping.__listOfApiMapping"
    ]
...
def serialize_json(value: GetApiMappingsResponse) -> dict:
    ...
    if "items" in value:
        ...
        out["items"] = capo_apigatewayv2.types.__list_of_api_mapping.serialize_json(
            value["items"]
        )
