"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#CreateProductRestEndpointPage``."""
...
import capo_apigatewayv2.errors.access_denied_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            raise capo_apigatewayv2.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.create_product_rest_endpoint_page_response.CreateProductRestEndpointPageResponse = capo_apigatewayv2.types.create_product_rest_endpoint_page_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{PortalProductId}", quote(str(input_["portal_product_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.create_product_rest_endpoint_page_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_product_rest_endpoint_page(
