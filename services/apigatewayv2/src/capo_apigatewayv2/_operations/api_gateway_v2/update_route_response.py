"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#UpdateRouteResponse``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.update_route_response_response.UpdateRouteResponseResponse = capo_apigatewayv2.types.update_route_response_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{RouteId}", quote(str(input_["route_id"]), safe=""))
    url = url.replace(
        "{RouteResponseId}", quote(str(input_["route_response_id"]), safe="")
    )
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.update_route_response_request.serialize_json(input_)
    ).encode()
...
async def async_update_route_response(
