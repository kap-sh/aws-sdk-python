"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#ImportApi``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.import_api_response.ImportApiResponse = (
        capo_apigatewayv2.types.import_api_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/apis"
    params: dict[str, str] = {}
    if "basepath" in input_:
        params["basepath"] = str(input_["basepath"])
    if "fail_on_warnings" in input_:
        params["failOnWarnings"] = str(input_["fail_on_warnings"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.import_api_request.serialize_json(input_)
    ).encode()
...
def import_api(
    ...
    capo_apigatewayv2.types.import_api_response.ImportApiResponse, zapros.Response
]:
