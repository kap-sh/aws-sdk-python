"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#UpdateApiMapping``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.update_api_mapping_response.UpdateApiMappingResponse = capo_apigatewayv2.types.update_api_mapping_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace("{ApiMappingId}", quote(str(input_["api_mapping_id"]), safe=""))
    url = url.replace("{DomainName}", quote(str(input_["domain_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.update_api_mapping_request.serialize_json(input_)
    ).encode()
...
async def async_update_api_mapping(
