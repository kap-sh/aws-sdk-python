"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#CreateDomainName``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.create_domain_name_response.CreateDomainNameResponse = capo_apigatewayv2.types.create_domain_name_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/domainnames"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.create_domain_name_request.serialize_json(input_)
    ).encode()
...
async def async_create_domain_name(
