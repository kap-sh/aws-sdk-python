...
def handle_response(
    ...
    out: capo_apigatewayv2.types.create_portal_response.CreatePortalResponse = (
        capo_apigatewayv2.types.create_portal_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/portals"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.create_portal_request.serialize_json(input_)
    ).encode()
...
def create_portal(
    ...
    capo_apigatewayv2.types.create_portal_response.CreatePortalResponse, zapros.Response
]:
