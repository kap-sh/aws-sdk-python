"""Generated from Smithy shape ``com.amazonaws.apigatewayv2#CreateIntegration``."""
...
def handle_response(
    ...
    out: capo_apigatewayv2.types.create_integration_result.CreateIntegrationResult = (
        capo_apigatewayv2.types.create_integration_result.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/v2/apis/{ApiId}/integrations"
    url = url.replace("{ApiId}", quote(str(input_["api_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_apigatewayv2.types.create_integration_request.serialize_json(input_)
    ).encode()
