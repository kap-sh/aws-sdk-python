...
def handle_response(
    ...
    out: aws_sdk_apigatewayv2.types.reimport_api_response.ReimportApiResponse = (
        aws_sdk_apigatewayv2.types.reimport_api_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_apigatewayv2.types.reimport_api_request.ReimportApiRequest,
) -> zapros.Request:
    ...
    if "basepath" in input:
        params["basepath"] = str(input["basepath"])
    if "fail_on_warnings" in input:
        params["failOnWarnings"] = str(input["fail_on_warnings"])
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_apigatewayv2.types.reimport_api_request

    body: bytes | None = json.dumps(
        aws_sdk_apigatewayv2.types.reimport_api_request.serialize_json(input)
    ).encode()
