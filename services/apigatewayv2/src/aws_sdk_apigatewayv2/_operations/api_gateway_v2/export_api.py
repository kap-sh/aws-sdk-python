...
def handle_response(
    ...
    out: aws_sdk_apigatewayv2.types.export_api_response.ExportApiResponse = {
        "body": aws_sdk_apigatewayv2.types.exported_api.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_apigatewayv2.types.export_api_request.ExportApiRequest,
) -> zapros.Request:
    ...
    url = url.replace("{Specification}", quote(str(input["specification"]), safe=""))
    params: dict[str, str] = {}
    if "export_version" in input:
        params["exportVersion"] = str(input["export_version"])
    if "include_extensions" in input:
        params["includeExtensions"] = str(input["include_extensions"])
    if "output_type" in input:
        params["outputType"] = str(input["output_type"])
    if "stage_name" in input:
        params["stageName"] = str(input["stage_name"])
...
def export_api(
    ...
    aws_sdk_apigatewayv2.types.export_api_response.ExportApiResponse, zapros.Response
]:
