...
def build_request(
    ...
    input: aws_sdk_apigatewayv2.types.get_product_rest_endpoint_page_request.GetProductRestEndpointPageRequest,
) -> zapros.Request:
    ...
    url = url.replace(
        "{PortalProductId}", quote(str(input["portal_product_id"]), safe="")
    )
    url = url.replace(
        "{ProductRestEndpointPageId}",
        quote(str(input["product_rest_endpoint_page_id"]), safe=""),
    )
    params: dict[str, str] = {}
    if "include_raw_display_content" in input:
        params["includeRawDisplayContent"] = str(input["include_raw_display_content"])
    if "resource_owner_account_id" in input:
        params["resourceOwnerAccountId"] = str(input["resource_owner_account_id"])
