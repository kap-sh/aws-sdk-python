...
if TYPE_CHECKING:
    import aws_sdk_apigatewayv2.types.product_rest_endpoint_page_summary_no_body

__listOfProductRestEndpointPageSummaryNoBody: TypeAlias = list[
    "aws_sdk_apigatewayv2.types.product_rest_endpoint_page_summary_no_body.ProductRestEndpointPageSummaryNoBody"
]
...
def serialize_json(value: __listOfProductRestEndpointPageSummaryNoBody) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_apigatewayv2.types.product_rest_endpoint_page_summary_no_body.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> __listOfProductRestEndpointPageSummaryNoBody:
    ...
    for item in data:
        out.append(
            aws_sdk_apigatewayv2.types.product_rest_endpoint_page_summary_no_body.deserialize_json(
                item
            )
        )
    return out
