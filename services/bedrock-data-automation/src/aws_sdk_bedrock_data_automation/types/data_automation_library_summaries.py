...
DataAutomationLibrarySummaries: TypeAlias = list[
    "aws_sdk_bedrock_data_automation.types.data_automation_library_summary.DataAutomationLibrarySummary"
]
...
def serialize_json(value: DataAutomationLibrarySummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_data_automation.types.data_automation_library_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> DataAutomationLibrarySummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_data_automation.types.data_automation_library_summary.deserialize_json(
                item
            )
        )
    return out
