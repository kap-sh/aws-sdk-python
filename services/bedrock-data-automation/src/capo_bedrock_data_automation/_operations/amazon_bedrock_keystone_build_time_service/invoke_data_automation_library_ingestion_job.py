...
import capo_bedrock_data_automation.errors.conflict_exception
...
import capo_bedrock_data_automation.errors.service_quota_exceeded_exception
import capo_bedrock_data_automation.errors.throttling_exception
import capo_bedrock_data_automation.errors.validation_exception
...
from capo_bedrock_data_automation._protocol.errors import parse_error_metadata_json
from capo_bedrock_data_automation._rule_engine._endpoint_rule_set import (
    EndpointParams,
    resolve,
)
...
from capo_bedrock_data_automation.errors import UnknownServiceError
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            raise capo_bedrock_data_automation.errors.conflict_exception.ConflictException.from_json(
                data
            )
        ...
        case "ServiceQuotaExceededException":
            raise capo_bedrock_data_automation.errors.service_quota_exceeded_exception.ServiceQuotaExceededException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_bedrock_data_automation.types.invoke_data_automation_library_ingestion_job_response.InvokeDataAutomationLibraryIngestionJobResponse = capo_bedrock_data_automation.types.invoke_data_automation_library_ingestion_job_response.deserialize_json(
...
def get_signer(
...
) -> capo_bedrock_data_automation._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_bedrock_data_automation._auth._sigv4.build_sigv4_auth_scheme(
                "bedrock", options.region
            )
        )
        if sigv4_config is not None:
            return capo_bedrock_data_automation._auth._signers.SigV4Signer(
...
def build_request(
    ...
    body: bytes | None = json.dumps(
        capo_bedrock_data_automation.types.invoke_data_automation_library_ingestion_job_request.serialize_json(
            input_
        )
    ).encode()
