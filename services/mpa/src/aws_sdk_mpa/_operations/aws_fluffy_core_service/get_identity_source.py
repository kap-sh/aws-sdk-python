...
def handle_response(
    ...
    out: aws_sdk_mpa.types.get_identity_source_response.GetIdentitySourceResponse = (
        aws_sdk_mpa.types.get_identity_source_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_mpa.types.get_identity_source_request.GetIdentitySourceRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{IdentitySourceArn}", quote(str(input_["identity_source_arn"]), safe="")
    )
    params: dict[str, str] = {}
