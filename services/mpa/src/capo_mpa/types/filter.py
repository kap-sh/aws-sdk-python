...
class Filter(TypedDict, closed=True):
    field_name: NotRequired["capo_mpa.types.filter_field.FilterField"]
    ...
    operator: NotRequired["capo_mpa.types.operator.Operator"]
    ...
    value: NotRequired["capo_mpa.types.string.String"]
...
def serialize_json(value: Filter) -> dict:
    ...
    if "field_name" in value:
        ...
        out["FieldName"] = capo_mpa.types.filter_field.serialize_json(
            value["field_name"]
        )
    if "operator" in value:
        import capo_mpa.types.operator

        out["Operator"] = capo_mpa.types.operator.serialize_json(value["operator"])
    if "value" in value:
