"""Generated from Smithy shape ``com.amazonaws.mpa#GetPolicyVersion``."""
...
def handle_response(
    ...
    out: capo_mpa.types.get_policy_version_response.GetPolicyVersionResponse = (
        capo_mpa.types.get_policy_version_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/policy-versions/{PolicyVersionArn}"
    url = url.replace(
        "{PolicyVersionArn}", quote(str(input_["policy_version_arn"]), safe="")
    )
    params: dict[str, str] = {}
...
def get_policy_version(
    ...
    capo_mpa.types.get_policy_version_response.GetPolicyVersionResponse, zapros.Response
]:
