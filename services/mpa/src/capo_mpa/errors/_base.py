...
class MPAError(Exception):
    """Base class for all MPA service errors."""


class ServiceError(MPAError):
...
class SerializationError(MPAError):
    """Raised when a value cannot be serialized."""


class DeserializationError(MPAError):
...
class WaiterFailedError(MPAError):
...
class WaiterTimeoutError(MPAError):
