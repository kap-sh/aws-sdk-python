...
GraphSummaryList: TypeAlias = list[
    "aws_sdk_neptune_graph.types.graph_summary.GraphSummary"
]
...
def serialize_json(value: GraphSummaryList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(aws_sdk_neptune_graph.types.graph_summary.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> GraphSummaryList:
    ...
    for item in data:
        out.append(aws_sdk_neptune_graph.types.graph_summary.deserialize_json(item))
    return out
