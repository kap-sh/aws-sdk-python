"""Generated from Smithy shape ``com.amazonaws.neptunegraph#GetGraphSnapshotOutput``."""
...
class GetGraphSnapshotOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetGraphSnapshotOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["name"] = value["name"]
    out["arn"] = value["arn"]
...
def deserialize_json(data: dict) -> GetGraphSnapshotOutput:
    out: GetGraphSnapshotOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("GetGraphSnapshotOutput.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("GetGraphSnapshotOutput.name required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("GetGraphSnapshotOutput.arn required")
