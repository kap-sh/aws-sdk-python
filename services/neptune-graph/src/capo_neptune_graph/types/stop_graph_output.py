"""Generated from Smithy shape ``com.amazonaws.neptunegraph#StopGraphOutput``."""
...
class StopGraphOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: StopGraphOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["name"] = value["name"]
    out["arn"] = value["arn"]
...
def deserialize_json(data: dict) -> StopGraphOutput:
    out: StopGraphOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("StopGraphOutput.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("StopGraphOutput.name required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("StopGraphOutput.arn required")
    if "status" in data:
        import capo_neptune_graph.types.graph_status
