"""Generated from Smithy shape ``com.amazonaws.neptunegraph#RestoreGraphFromSnapshotOutput``."""
...
class RestoreGraphFromSnapshotOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: RestoreGraphFromSnapshotOutput) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    out["name"] = value["name"]
    out["arn"] = value["arn"]
...
def deserialize_json(data: dict) -> RestoreGraphFromSnapshotOutput:
    out: RestoreGraphFromSnapshotOutput = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("RestoreGraphFromSnapshotOutput.id required")
    if "name" in data:
        out["name"] = data["name"]
    else:
        raise DeserializationError("RestoreGraphFromSnapshotOutput.name required")
    if "arn" in data:
        out["arn"] = data["arn"]
    else:
        raise DeserializationError("RestoreGraphFromSnapshotOutput.arn required")
    if "status" in data:
        import capo_neptune_graph.types.graph_status
