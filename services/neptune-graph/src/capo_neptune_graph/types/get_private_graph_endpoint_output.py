"""Generated from Smithy shape ``com.amazonaws.neptunegraph#GetPrivateGraphEndpointOutput``."""
...
class GetPrivateGraphEndpointOutput(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: GetPrivateGraphEndpointOutput) -> dict:
    out: dict = {}
...
def deserialize_json(data: dict) -> GetPrivateGraphEndpointOutput:
    out: GetPrivateGraphEndpointOutput = {}  # type: ignore[typeddict-item]
    if "vpcId" in data:
        out["vpc_id"] = data["vpcId"]
    else:
        raise DeserializationError("GetPrivateGraphEndpointOutput.vpc_id required")
    if "subnetIds" in data:
        ...
        raise DeserializationError("GetPrivateGraphEndpointOutput.subnet_ids required")
    if "status" in data:
        import capo_neptune_graph.types.private_graph_endpoint_status
        ...
        raise DeserializationError("GetPrivateGraphEndpointOutput.status required")
