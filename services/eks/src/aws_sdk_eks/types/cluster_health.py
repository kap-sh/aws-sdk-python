...
if TYPE_CHECKING:
    import aws_sdk_eks.types.cluster_issue_list


class ClusterHealth(TypedDict):
    issues: NotRequired["aws_sdk_eks.types.cluster_issue_list.ClusterIssueList"]
...
def serialize_json(value: ClusterHealth) -> dict:
    ...
    if "issues" in value:
        ...
        out["issues"] = aws_sdk_eks.types.cluster_issue_list.serialize_json(
            value["issues"]
        )
    return out
...
def deserialize_json(data: dict) -> ClusterHealth:
    ...
    if "issues" in data:
        ...
        out["issues"] = aws_sdk_eks.types.cluster_issue_list.deserialize_json(
            data["issues"]
        )
    return out
