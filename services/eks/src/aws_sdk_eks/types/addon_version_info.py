...
if TYPE_CHECKING:
    ...
    import aws_sdk_eks.types.compatibilities
...
class AddonVersionInfo(TypedDict, closed=True):
    ...
    architecture: NotRequired["aws_sdk_eks.types.string_list.StringList"]
    """<p>The architectures that the version supports.</p>"""
    compute_types: NotRequired["aws_sdk_eks.types.string_list.StringList"]
    """<p>Indicates the compute type of the add-on version.</p>"""
    compatibilities: NotRequired["aws_sdk_eks.types.compatibilities.Compatibilities"]
    """<p>An object representing the compatibilities of a version.</p>"""
    requires_configuration: "aws_sdk_eks.types.boolean.Boolean"
    """<p>Whether the add-on requires configuration.</p>"""
    requires_iam_permissions: "aws_sdk_eks.types.boolean.Boolean"
...
def serialize_json(value: AddonVersionInfo) -> dict:
    ...
    if "architecture" in value:
        ...
        out["architecture"] = aws_sdk_eks.types.string_list.serialize_json(
            value["architecture"]
        )
    if "compute_types" in value:
        import aws_sdk_eks.types.string_list

        out["computeTypes"] = aws_sdk_eks.types.string_list.serialize_json(
            value["compute_types"]
        )
    if "compatibilities" in value:
        import aws_sdk_eks.types.compatibilities

        out["compatibilities"] = aws_sdk_eks.types.compatibilities.serialize_json(
...
def deserialize_json(data: dict) -> AddonVersionInfo:
    ...
    if "architecture" in data:
        ...
        out["architecture"] = aws_sdk_eks.types.string_list.deserialize_json(
            data["architecture"]
        )
    if "computeTypes" in data:
        import aws_sdk_eks.types.string_list

        out["compute_types"] = aws_sdk_eks.types.string_list.deserialize_json(
            data["computeTypes"]
        )
    if "compatibilities" in data:
        import aws_sdk_eks.types.compatibilities

        out["compatibilities"] = aws_sdk_eks.types.compatibilities.deserialize_json(
