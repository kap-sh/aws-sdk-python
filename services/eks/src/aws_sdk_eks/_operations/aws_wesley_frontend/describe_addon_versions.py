...
if TYPE_CHECKING:
    import aws_sdk_eks.types.describe_addon_versions_request
    import aws_sdk_eks.types.describe_addon_versions_response
...
) -> aws_sdk_eks.types.describe_addon_versions_response.DescribeAddonVersionsResponse:
    import aws_sdk_eks.types.describe_addon_versions_response

    out: aws_sdk_eks.types.describe_addon_versions_response.DescribeAddonVersionsResponse = aws_sdk_eks.types.describe_addon_versions_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_eks.types.describe_addon_versions_request.DescribeAddonVersionsRequest,
) -> zapros.Request:
    ...
    params: dict[str, str] = {}
    if "kubernetes_version" in input:
        params["kubernetesVersion"] = str(input["kubernetes_version"])
    ...
    if "types" in input:
        params["types"] = str(input["types"])
    if "publishers" in input:
        params["publishers"] = str(input["publishers"])
    if "owners" in input:
        params["owners"] = str(input["owners"])
