"""Generated from Smithy shape ``com.amazonaws.eks#UpdateNodegroupVersion``."""
...
import aws_sdk_eks.types.update_nodegroup_version_request
import aws_sdk_eks.types.update_nodegroup_version_response
...
) -> aws_sdk_eks.types.update_nodegroup_version_response.UpdateNodegroupVersionResponse:
    out: aws_sdk_eks.types.update_nodegroup_version_response.UpdateNodegroupVersionResponse = aws_sdk_eks.types.update_nodegroup_version_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_eks.types.update_nodegroup_version_request.UpdateNodegroupVersionRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace("{nodegroupName}", quote(str(input_["nodegroup_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_eks.types.update_nodegroup_version_request

    body: bytes | None = json.dumps(
        aws_sdk_eks.types.update_nodegroup_version_request.serialize_json(input_)
    ).encode()
...
async def async_update_nodegroup_version(
