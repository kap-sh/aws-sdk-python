...
def build_request(
    ...
    input: aws_sdk_eks.types.update_nodegroup_version_request.UpdateNodegroupVersionRequest,
) -> zapros.Request:
    ...
    url = url.replace("{nodegroupName}", quote(str(input["nodegroup_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_eks.types.update_nodegroup_version_request

    body: bytes | None = json.dumps(
        aws_sdk_eks.types.update_nodegroup_version_request.serialize_json(input)
    ).encode()
