"""Generated from Smithy shape ``com.amazonaws.eks#DeleteAddon``."""
...
if TYPE_CHECKING:
    import aws_sdk_eks.types.delete_addon_request
    import aws_sdk_eks.types.delete_addon_response
...
) -> aws_sdk_eks.types.delete_addon_response.DeleteAddonResponse:
    import aws_sdk_eks.types.delete_addon_response

    out: aws_sdk_eks.types.delete_addon_response.DeleteAddonResponse = (
        aws_sdk_eks.types.delete_addon_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_eks.types.delete_addon_request.DeleteAddonRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/clusters/{clusterName}/addons/{addonName}"
    url = url.replace("{clusterName}", quote(str(input_["cluster_name"]), safe=""))
    url = url.replace("{addonName}", quote(str(input_["addon_name"]), safe=""))
    params: dict[str, str] = {}
    params["preserve"] = str(input_.get("preserve", False))
...
def delete_addon(
    ...
    aws_sdk_eks.types.delete_addon_response.DeleteAddonResponse, zapros.Response
]:
