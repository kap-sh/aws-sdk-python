...
def handle_response(
    ...
    out: aws_sdk_eks.types.update_capability_response.UpdateCapabilityResponse = (
        aws_sdk_eks.types.update_capability_response.deserialize_json(
...
def build_request(
    ...
    input: aws_sdk_eks.types.update_capability_request.UpdateCapabilityRequest,
) -> zapros.Request:
    ...
    url = url.replace("{capabilityName}", quote(str(input["capability_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_eks.types.update_capability_request

    body: bytes | None = json.dumps(
        aws_sdk_eks.types.update_capability_request.serialize_json(input)
    ).encode()
