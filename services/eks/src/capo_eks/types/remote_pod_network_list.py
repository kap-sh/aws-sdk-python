...
if TYPE_CHECKING:
    import capo_eks.types.remote_pod_network

RemotePodNetworkList: TypeAlias = list[
    "capo_eks.types.remote_pod_network.RemotePodNetwork"
]
...
def serialize_json(value: RemotePodNetworkList) -> list:
    ...
    out: list = []
    for item in value:
        out.append(capo_eks.types.remote_pod_network.serialize_json(item))
    return out
...
def deserialize_json(data: list) -> RemotePodNetworkList:
    ...
    for item in data:
        out.append(capo_eks.types.remote_pod_network.deserialize_json(item))
    return out
