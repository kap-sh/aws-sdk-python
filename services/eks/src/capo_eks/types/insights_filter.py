...
class InsightsFilter(TypedDict, closed=True):
    categories: NotRequired["capo_eks.types.category_list.CategoryList"]
    ...
    kubernetes_versions: NotRequired["capo_eks.types.string_list.StringList"]
    """<p>The Kubernetes versions to use to filter the insights.</p>"""
    statuses: NotRequired[
        "capo_eks.types.insight_status_value_list.InsightStatusValueList"
    ]
...
def serialize_json(value: InsightsFilter) -> dict:
    ...
    if "categories" in value:
        ...
        out["categories"] = capo_eks.types.category_list.serialize_json(
            value["categories"]
        )
    if "kubernetes_versions" in value:
        import capo_eks.types.string_list

        out["kubernetesVersions"] = capo_eks.types.string_list.serialize_json(
            value["kubernetes_versions"]
        )
    if "statuses" in value:
        import capo_eks.types.insight_status_value_list

        out["statuses"] = capo_eks.types.insight_status_value_list.serialize_json(
            value["statuses"]
        )
    return out
