...
if TYPE_CHECKING:
    ...
    import capo_eks.types.zero_capacity
...
class WarmPoolConfig(TypedDict, closed=True):
    ...
    min_size: NotRequired["capo_eks.types.zero_capacity.ZeroCapacity"]
    ...
    pool_state: NotRequired["capo_eks.types.warm_pool_state.WarmPoolState"]
    ...
    reuse_on_scale_in: NotRequired["capo_eks.types.boxed_boolean.BoxedBoolean"]
...
def serialize_json(value: WarmPoolConfig) -> dict:
    ...
    if "pool_state" in value:
        ...
        out["poolState"] = capo_eks.types.warm_pool_state.serialize_json(
            value["pool_state"]
        )
