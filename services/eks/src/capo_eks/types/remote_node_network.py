...
class RemoteNodeNetwork(TypedDict, closed=True):
    cidrs: NotRequired["capo_eks.types.string_list.StringList"]
...
def serialize_json(value: RemoteNodeNetwork) -> dict:
    ...
    if "cidrs" in value:
        ...
        out["cidrs"] = capo_eks.types.string_list.serialize_json(value["cidrs"])
    return out
