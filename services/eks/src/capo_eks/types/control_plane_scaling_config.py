...
class ControlPlaneScalingConfig(TypedDict, closed=True):
    tier: NotRequired[
        "capo_eks.types.provisioned_control_plane_tier.ProvisionedControlPlaneTier"
    ]
...
def serialize_json(value: ControlPlaneScalingConfig) -> dict:
    ...
    if "tier" in value:
        ...
        out["tier"] = capo_eks.types.provisioned_control_plane_tier.serialize_json(
            value["tier"]
        )
    return out
