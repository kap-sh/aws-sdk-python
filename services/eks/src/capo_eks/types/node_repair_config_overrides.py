...
class NodeRepairConfigOverrides(TypedDict, closed=True):
    node_monitoring_condition: NotRequired["capo_eks.types.string.String"]
    """<p>Specify an unhealthy condition reported by the node monitoring agent that this override would apply to.</p>"""
    node_unhealthy_reason: NotRequired["capo_eks.types.string.String"]
    ...
    min_repair_wait_time_mins: NotRequired[
        "capo_eks.types.non_zero_integer.NonZeroInteger"
    ]
    ...
    repair_action: NotRequired["capo_eks.types.repair_action.RepairAction"]
...
def serialize_json(value: NodeRepairConfigOverrides) -> dict:
    ...
    if "repair_action" in value:
        ...
        out["repairAction"] = capo_eks.types.repair_action.serialize_json(
            value["repair_action"]
        )
    return out
