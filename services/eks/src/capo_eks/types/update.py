...
class Update(TypedDict, closed=True):
    ...
    status: NotRequired["capo_eks.types.update_status.UpdateStatus"]
    """<p>The current status of the update.</p>"""
    type: NotRequired["capo_eks.types.update_type.UpdateType"]
    """<p>The type of the update.</p>"""
    params: NotRequired["capo_eks.types.update_params.UpdateParams"]
...
def serialize_json(value: Update) -> dict:
    ...
    if "status" in value:
        ...
        out["status"] = capo_eks.types.update_status.serialize_json(value["status"])
    if "type" in value:
        import capo_eks.types.update_type

        out["type"] = capo_eks.types.update_type.serialize_json(value["type"])
    if "params" in value:
        import capo_eks.types.update_params

        out["params"] = capo_eks.types.update_params.serialize_json(value["params"])
