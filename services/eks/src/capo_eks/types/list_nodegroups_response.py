...
class ListNodegroupsResponse(TypedDict, closed=True):
    nodegroups: NotRequired["capo_eks.types.string_list.StringList"]
...
def serialize_json(value: ListNodegroupsResponse) -> dict:
    ...
    if "nodegroups" in value:
        ...
        out["nodegroups"] = capo_eks.types.string_list.serialize_json(
            value["nodegroups"]
        )
...
def deserialize_json(data: dict) -> ListNodegroupsResponse:
    ...
    if "nodegroups" in data:
        ...
        out["nodegroups"] = capo_eks.types.string_list.deserialize_json(
            data["nodegroups"]
        )
