"""Generated from Smithy shape ``com.amazonaws.eks#CreateCapability``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ThrottlingException":
            raise capo_eks.errors.throttling_exception.ThrottlingException.from_json(
                data
            )
        case _:
...
def handle_response(
    ...
    out: capo_eks.types.create_capability_response.CreateCapabilityResponse = (
        capo_eks.types.create_capability_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/clusters/{clusterName}/capabilities"
    url = url.replace("{clusterName}", quote(str(input_["cluster_name"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_eks.types.create_capability_request.serialize_json(input_)
    ).encode()
...
def create_capability(
    ...
    capo_eks.types.create_capability_response.CreateCapabilityResponse, zapros.Response
]:
