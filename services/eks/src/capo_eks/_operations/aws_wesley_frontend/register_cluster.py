"""Generated from Smithy shape ``com.amazonaws.eks#RegisterCluster``."""
...
import capo_eks.errors.access_denied_exception
...
import capo_eks.errors.resource_propagation_delay_exception
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        case "AccessDeniedException":
            raise capo_eks.errors.access_denied_exception.AccessDeniedException.from_json(
                data
            )
        ...
        case "ResourcePropagationDelayException":
            raise capo_eks.errors.resource_propagation_delay_exception.ResourcePropagationDelayException.from_json(
                data
            )
...
def handle_response(
    ...
    out: capo_eks.types.register_cluster_response.RegisterClusterResponse = (
        capo_eks.types.register_cluster_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/cluster-registrations"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_eks.types.register_cluster_request.serialize_json(input_)
    ).encode()
...
def register_cluster(
    ...
    capo_eks.types.register_cluster_response.RegisterClusterResponse, zapros.Response
]:
...
async def async_register_cluster(
