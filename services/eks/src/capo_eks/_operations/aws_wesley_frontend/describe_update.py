"""Generated from Smithy shape ``com.amazonaws.eks#DescribeUpdate``."""
...
def handle_response(
    ...
    out: capo_eks.types.describe_update_response.DescribeUpdateResponse = (
        capo_eks.types.describe_update_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/clusters/{name}/updates/{updateId}"
    url = url.replace("{name}", quote(str(input_["name"]), safe=""))
    url = url.replace("{updateId}", quote(str(input_["update_id"]), safe=""))
    params: dict[str, str] = {}
...
def describe_update(
    ...
    capo_eks.types.describe_update_response.DescribeUpdateResponse, zapros.Response
]:
...
async def async_describe_update(
