"""Generated from Smithy shape ``com.amazonaws.eks#ListPodIdentityAssociations``."""
...
def handle_response(
    ...
    out: capo_eks.types.list_pod_identity_associations_response.ListPodIdentityAssociationsResponse = capo_eks.types.list_pod_identity_associations_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/clusters/{clusterName}/pod-identity-associations"
    ...
    if "service_account" in input_:
        params["serviceAccount"] = str(input_["service_account"])
...
async def async_list_pod_identity_associations(
