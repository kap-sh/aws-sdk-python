...
class EKSError(Exception):
    """Base class for all EKS service errors."""


class ServiceError(EKSError):
...
class SerializationError(EKSError):
    """Raised when a value cannot be serialized."""


class DeserializationError(EKSError):
...
class WaiterFailedError(EKSError):
...
class WaiterTimeoutError(EKSError):
