"""Generated from Smithy shape ``com.amazonaws.dataexchange#UpdateEventActionResponse``."""
...
class UpdateEventActionResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: UpdateEventActionResponse) -> dict:
    out: dict = {}
    if "action" in value:
        import aws_sdk_dataexchange.types.action
...
def deserialize_json(data: dict) -> UpdateEventActionResponse:
    out: UpdateEventActionResponse = {}  # type: ignore[typeddict-item]
    if "Action" in data:
        import aws_sdk_dataexchange.types.action
