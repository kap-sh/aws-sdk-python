"""Generated from Smithy shape ``com.amazonaws.dataexchange#ListDataGrantsResponse``."""

from typing import TYPE_CHECKING, TypedDict

from typing_extensions import NotRequired

if TYPE_CHECKING:
    import aws_sdk_dataexchange.types.list_of_data_grant_summary_entry
    import aws_sdk_dataexchange.types.next_token


class ListDataGrantsResponse(TypedDict):
    data_grant_summaries: NotRequired[
        "aws_sdk_dataexchange.types.list_of_data_grant_summary_entry.ListOfDataGrantSummaryEntry"
    ]
    """<p>An object that contains a list of data grant information.</p>"""
    next_token: NotRequired["aws_sdk_dataexchange.types.next_token.NextToken"]
    """<p>The pagination token used to retrieve the next page of results for this operation.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListDataGrantsResponse) -> dict:
    out: dict = {}
    if "data_grant_summaries" in value:
        import aws_sdk_dataexchange.types.list_of_data_grant_summary_entry

        out["DataGrantSummaries"] = (
            aws_sdk_dataexchange.types.list_of_data_grant_summary_entry.serialize_json(
                value["data_grant_summaries"]
            )
        )
    if "next_token" in value:
        out["NextToken"] = value["next_token"]
    return out


def deserialize_json(data: dict) -> ListDataGrantsResponse:
    out: ListDataGrantsResponse = {}  # type: ignore[typeddict-item]
    if "DataGrantSummaries" in data:
        import aws_sdk_dataexchange.types.list_of_data_grant_summary_entry

        out["data_grant_summaries"] = (
            aws_sdk_dataexchange.types.list_of_data_grant_summary_entry.deserialize_json(
                data["DataGrantSummaries"]
            )
        )
    if "NextToken" in data:
        out["next_token"] = data["NextToken"]
    return out
