"""Generated from Smithy shape ``com.amazonaws.dataexchange#RevokeRevisionResponse``."""
...
class RevokeRevisionResponse(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: RevokeRevisionResponse) -> dict:
    out: dict = {}
    if "arn" in value:
        out["Arn"] = value["arn"]
    if "comment" in value:
...
def deserialize_json(data: dict) -> RevokeRevisionResponse:
    out: RevokeRevisionResponse = {}  # type: ignore[typeddict-item]
    if "Arn" in data:
        out["arn"] = data["Arn"]
    if "Comment" in data:
