...
if TYPE_CHECKING:
    import aws_sdk_dataexchange.types.list_of_lf_tag_values


class LFTag(TypedDict, closed=True):
    tag_key: "str"
    """<p>The key name for the LF-tag.</p>"""
    tag_values: "aws_sdk_dataexchange.types.list_of_lf_tag_values.ListOfLFTagValues"
...
def serialize_json(value: LFTag) -> dict:
    ...
    out["TagValues"] = aws_sdk_dataexchange.types.list_of_lf_tag_values.serialize_json(
        value["tag_values"]
    )
    return out
...
def deserialize_json(data: dict) -> LFTag:
    ...
    if "TagValues" in data:
        ...
        out["tag_values"] = (
            aws_sdk_dataexchange.types.list_of_lf_tag_values.deserialize_json(
                data["TagValues"]
            )
        )
    else:
