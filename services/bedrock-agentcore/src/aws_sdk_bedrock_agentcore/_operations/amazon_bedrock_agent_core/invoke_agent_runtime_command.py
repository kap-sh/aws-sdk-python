...
def handle_response(
    ...
    _union_deser = aws_sdk_bedrock_agentcore.types.invoke_agent_runtime_command_stream_output.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: aws_sdk_bedrock_agentcore.types.invoke_agent_runtime_command_response.InvokeAgentRuntimeCommandResponse = {
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agentcore.types.invoke_agent_runtime_command_request.InvokeAgentRuntimeCommandRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
