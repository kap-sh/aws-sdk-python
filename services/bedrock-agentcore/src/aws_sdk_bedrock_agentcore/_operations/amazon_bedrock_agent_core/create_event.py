"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#CreateEvent``."""
...
def handle_response(response: zapros.Response, is_async: bool) -> aws_sdk_bedrock_agentcore.types.create_event_output.CreateEventOutput:
    import aws_sdk_bedrock_agentcore.types.create_event_output
    out: aws_sdk_bedrock_agentcore.types.create_event_output.CreateEventOutput = aws_sdk_bedrock_agentcore.types.create_event_output.deserialize_json(json.loads(response.read()))
    return out
...
def build_request(options: OperationOptions | AsyncOperationOptions, input: aws_sdk_bedrock_agentcore.types.create_event_input.CreateEventInput) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/memories/{memoryId}/events"
    url = url.replace("{memoryId}", quote(str(input["memory_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_bedrock_agentcore.types.create_event_input
    body: bytes | None = json.dumps(aws_sdk_bedrock_agentcore.types.create_event_input.serialize_json(input)).encode()
...
def create_event(options: OperationOptions, input: aws_sdk_bedrock_agentcore.types.create_event_input.CreateEventInput) -> tuple[aws_sdk_bedrock_agentcore.types.create_event_output.CreateEventOutput, zapros.Response]:
...
async def async_create_event(options: AsyncOperationOptions, input: aws_sdk_bedrock_agentcore.types.create_event_input.CreateEventInput) -> tuple[aws_sdk_bedrock_agentcore.types.create_event_output.CreateEventOutput, zapros.Response]:
