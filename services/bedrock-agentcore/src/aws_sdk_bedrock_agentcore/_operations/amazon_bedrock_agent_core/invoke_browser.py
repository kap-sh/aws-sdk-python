"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#InvokeBrowser``."""
...
def handle_response(response: zapros.Response, is_async: bool) -> aws_sdk_bedrock_agentcore.types.invoke_browser_response.InvokeBrowserResponse:
    import aws_sdk_bedrock_agentcore.types.invoke_browser_response
    out: aws_sdk_bedrock_agentcore.types.invoke_browser_response.InvokeBrowserResponse = aws_sdk_bedrock_agentcore.types.invoke_browser_response.deserialize_json(json.loads(response.read()))
    out["session_id"] = str(response.headers["x-amzn-browser-session-id"])
    return out
...
def build_request(options: OperationOptions | AsyncOperationOptions, input: aws_sdk_bedrock_agentcore.types.invoke_browser_request.InvokeBrowserRequest) -> zapros.Request:
    ...
    url = endpoint.url.rstrip("/") + "/browsers/{browserIdentifier}/sessions/invoke"
    url = url.replace("{browserIdentifier}", quote(str(input["browser_identifier"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "session_id" in input:
        headers["x-amzn-browser-session-id"] = str(input["session_id"])
    import aws_sdk_bedrock_agentcore.types.invoke_browser_request
    body: bytes | None = json.dumps(aws_sdk_bedrock_agentcore.types.invoke_browser_request.serialize_json(input)).encode()
...
def invoke_browser(options: OperationOptions, input: aws_sdk_bedrock_agentcore.types.invoke_browser_request.InvokeBrowserRequest) -> tuple[aws_sdk_bedrock_agentcore.types.invoke_browser_response.InvokeBrowserResponse, zapros.Response]:
...
async def async_invoke_browser(options: AsyncOperationOptions, input: aws_sdk_bedrock_agentcore.types.invoke_browser_request.InvokeBrowserRequest) -> tuple[aws_sdk_bedrock_agentcore.types.invoke_browser_response.InvokeBrowserResponse, zapros.Response]:
