...
) -> aws_sdk_bedrock_agentcore.types.get_memory_record_output.GetMemoryRecordOutput:
    import aws_sdk_bedrock_agentcore.types.get_memory_record_output

    out: aws_sdk_bedrock_agentcore.types.get_memory_record_output.GetMemoryRecordOutput = aws_sdk_bedrock_agentcore.types.get_memory_record_output.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agentcore.types.get_memory_record_input.GetMemoryRecordInput,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = (
        endpoint.url.rstrip("/") + "/memories/{memoryId}/memoryRecord/{memoryRecordId}"
    )
    url = url.replace("{memoryId}", quote(str(input_["memory_id"]), safe=""))
    url = url.replace(
        "{memoryRecordId}", quote(str(input_["memory_record_id"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_get_memory_record(
