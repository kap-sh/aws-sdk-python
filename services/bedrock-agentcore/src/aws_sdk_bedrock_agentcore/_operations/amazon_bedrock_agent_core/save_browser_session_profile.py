...
def handle_response(
    ...
    out: aws_sdk_bedrock_agentcore.types.save_browser_session_profile_response.SaveBrowserSessionProfileResponse = aws_sdk_bedrock_agentcore.types.save_browser_session_profile_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_bedrock_agentcore.types.save_browser_session_profile_request.SaveBrowserSessionProfileRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = url.replace(
        "{profileIdentifier}", quote(str(input_["profile_identifier"]), safe="")
    )
    params: dict[str, str] = {}
...
async def async_save_browser_session_profile(
