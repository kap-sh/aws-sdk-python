...
class _GroundTruthSource_inline(TypedDict, closed=True):
...
def serialize_json(value: GroundTruthSource) -> dict:
    if "inline" in value:
        ...
        return {
            "inline": aws_sdk_bedrock_agentcore.types.inline_ground_truth.serialize_json(
                value["inline"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> GroundTruthSource:
    if "inline" in data:
        ...
        return {
            "inline": aws_sdk_bedrock_agentcore.types.inline_ground_truth.deserialize_json(
                data["inline"]
            )
        }
    else:
