...
class GatewayFilter(TypedDict):
    target_paths: NotRequired[
        "aws_sdk_bedrock_agentcore.types.target_path_list.TargetPathList"
    ]
...
def serialize_json(value: GatewayFilter) -> dict:
    ...
    if "target_paths" in value:
        ...
        out["targetPaths"] = (
            aws_sdk_bedrock_agentcore.types.target_path_list.serialize_json(
                value["target_paths"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> GatewayFilter:
    ...
    if "targetPaths" in data:
        ...
        out["target_paths"] = (
            aws_sdk_bedrock_agentcore.types.target_path_list.deserialize_json(
                data["targetPaths"]
            )
        )
    return out
