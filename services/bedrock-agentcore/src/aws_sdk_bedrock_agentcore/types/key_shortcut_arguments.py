...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agentcore.types.key_list


class KeyShortcutArguments(TypedDict):
    keys: "aws_sdk_bedrock_agentcore.types.key_list.KeyList"
...
def serialize_json(value: KeyShortcutArguments) -> dict:
    ...
    out["keys"] = aws_sdk_bedrock_agentcore.types.key_list.serialize_json(value["keys"])
    return out
...
def deserialize_json(data: dict) -> KeyShortcutArguments:
    ...
    if "keys" in data:
        ...
        out["keys"] = aws_sdk_bedrock_agentcore.types.key_list.deserialize_json(
            data["keys"]
        )
    else:
