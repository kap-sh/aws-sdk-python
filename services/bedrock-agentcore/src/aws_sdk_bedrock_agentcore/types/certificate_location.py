...
def serialize_json(value: CertificateLocation) -> dict:
    if "secretsManager" in value:
        import aws_sdk_bedrock_agentcore.types.secrets_manager_location
        return {"secretsManager": aws_sdk_bedrock_agentcore.types.secrets_manager_location.serialize_json(value["secretsManager"])}
    else:
...
def deserialize_json(data: dict) -> CertificateLocation:
    if "secretsManager" in data:
        import aws_sdk_bedrock_agentcore.types.secrets_manager_location
        return {"secretsManager": aws_sdk_bedrock_agentcore.types.secrets_manager_location.deserialize_json(data["secretsManager"])}
    else:
