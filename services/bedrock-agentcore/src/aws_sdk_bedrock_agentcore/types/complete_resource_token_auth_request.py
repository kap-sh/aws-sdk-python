...
def serialize_json(value: CompleteResourceTokenAuthRequest) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.user_identifier
    out["userIdentifier"] = aws_sdk_bedrock_agentcore.types.user_identifier.serialize_json(value["user_identifier"])
...
def deserialize_json(data: dict) -> CompleteResourceTokenAuthRequest:
    ...
    if "userIdentifier" in data:
        import aws_sdk_bedrock_agentcore.types.user_identifier
        out["user_identifier"] = aws_sdk_bedrock_agentcore.types.user_identifier.deserialize_json(data["userIdentifier"])
    else:
        raise DeserializationError("CompleteResourceTokenAuthRequest.user_identifier required")
    if "sessionUri" in data:
        out["session_uri"] = data["sessionUri"]
    else:
        raise DeserializationError("CompleteResourceTokenAuthRequest.session_uri required")
    return out