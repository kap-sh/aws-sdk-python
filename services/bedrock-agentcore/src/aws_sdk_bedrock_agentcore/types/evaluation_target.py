...
def serialize_json(value: EvaluationTarget) -> dict:
    if "spanIds" in value:
        import aws_sdk_bedrock_agentcore.types.span_ids
        return {"spanIds": aws_sdk_bedrock_agentcore.types.span_ids.serialize_json(value["spanIds"])}
    elif "traceIds" in value:
        import aws_sdk_bedrock_agentcore.types.trace_ids
        return {"traceIds": aws_sdk_bedrock_agentcore.types.trace_ids.serialize_json(value["traceIds"])}
    else:
...
def deserialize_json(data: dict) -> EvaluationTarget:
    if "spanIds" in data:
        import aws_sdk_bedrock_agentcore.types.span_ids
        return {"spanIds": aws_sdk_bedrock_agentcore.types.span_ids.deserialize_json(data["spanIds"])}
    elif "traceIds" in data:
        import aws_sdk_bedrock_agentcore.types.trace_ids
        return {"traceIds": aws_sdk_bedrock_agentcore.types.trace_ids.deserialize_json(data["traceIds"])}
    else:
