...
class EvaluateResponse(TypedDict, closed=True):
    evaluation_results: (
        "aws_sdk_bedrock_agentcore.types.evaluation_results.EvaluationResults"
    )
...
def serialize_json(value: EvaluateResponse) -> dict:
    ...
    out["evaluationResults"] = (
        aws_sdk_bedrock_agentcore.types.evaluation_results.serialize_json(
...
def deserialize_json(data: dict) -> EvaluateResponse:
    ...
    if "evaluationResults" in data:
        ...
        out["evaluation_results"] = (
            aws_sdk_bedrock_agentcore.types.evaluation_results.deserialize_json(
