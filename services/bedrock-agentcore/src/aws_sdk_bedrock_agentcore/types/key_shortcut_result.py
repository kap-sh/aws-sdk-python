"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#KeyShortcutResult``."""
...
class KeyShortcutResult(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: KeyShortcutResult) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.browser_action_status
...
def deserialize_json(data: dict) -> KeyShortcutResult:
    out: KeyShortcutResult = {}  # type: ignore[typeddict-item]
    if "status" in data:
        import aws_sdk_bedrock_agentcore.types.browser_action_status
        ...
        raise DeserializationError("KeyShortcutResult.status required")
    if "error" in data:
