...
class _EvaluationInput_sessionSpans(TypedDict):
...
def serialize_json(value: EvaluationInput) -> dict:
    if "sessionSpans" in value:
        import aws_sdk_bedrock_agentcore.types.spans
        return {"sessionSpans": aws_sdk_bedrock_agentcore.types.spans.serialize_json(value["sessionSpans"])}
    else:
...
def deserialize_json(data: dict) -> EvaluationInput:
    if "sessionSpans" in data:
        import aws_sdk_bedrock_agentcore.types.spans
        return {"sessionSpans": aws_sdk_bedrock_agentcore.types.spans.deserialize_json(data["sessionSpans"])}
    else:
