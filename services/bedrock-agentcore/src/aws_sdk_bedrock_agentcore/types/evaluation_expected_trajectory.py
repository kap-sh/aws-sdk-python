...
class EvaluationExpectedTrajectory(TypedDict, closed=True):
    tool_names: NotRequired[
        "aws_sdk_bedrock_agentcore.types.evaluation_tool_names.EvaluationToolNames"
    ]
...
def serialize_json(value: EvaluationExpectedTrajectory) -> dict:
    ...
    if "tool_names" in value:
        ...
        out["toolNames"] = (
            aws_sdk_bedrock_agentcore.types.evaluation_tool_names.serialize_json(
                value["tool_names"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> EvaluationExpectedTrajectory:
    ...
    if "toolNames" in data:
        ...
        out["tool_names"] = (
            aws_sdk_bedrock_agentcore.types.evaluation_tool_names.deserialize_json(
                data["toolNames"]
            )
        )
    return out
