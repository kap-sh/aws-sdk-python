...
HarnessContentBlockStart: TypeAlias = _HarnessContentBlockStart_toolUse | _HarnessContentBlockStart_toolResult

# --- restJson1 ser/de ---
def serialize_json(value: HarnessContentBlockStart) -> dict:
    if "toolUse" in value:
        import aws_sdk_bedrock_agentcore.types.harness_tool_use_block_start
        return {"toolUse": aws_sdk_bedrock_agentcore.types.harness_tool_use_block_start.serialize_json(value["toolUse"])}
    elif "toolResult" in value:
        import aws_sdk_bedrock_agentcore.types.harness_tool_result_block_start
        return {"toolResult": aws_sdk_bedrock_agentcore.types.harness_tool_result_block_start.serialize_json(value["toolResult"])}
    else:
...
def deserialize_json(data: dict) -> HarnessContentBlockStart:
    if "toolUse" in data:
        import aws_sdk_bedrock_agentcore.types.harness_tool_use_block_start
        return {"toolUse": aws_sdk_bedrock_agentcore.types.harness_tool_use_block_start.deserialize_json(data["toolUse"])}
    elif "toolResult" in data:
        import aws_sdk_bedrock_agentcore.types.harness_tool_result_block_start
        return {"toolResult": aws_sdk_bedrock_agentcore.types.harness_tool_result_block_start.deserialize_json(data["toolResult"])}
    else:
        raise DeserializationError("HarnessContentBlockStart: no recognized variant key")