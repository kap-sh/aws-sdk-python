...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agentcore.types.s3_location


class _ResourceLocation_s3(TypedDict):
    s3: "aws_sdk_bedrock_agentcore.types.s3_location.S3Location"
...
def serialize_json(value: ResourceLocation) -> dict:
    if "s3" in value:
        ...
        return {
            "s3": aws_sdk_bedrock_agentcore.types.s3_location.serialize_json(
                value["s3"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> ResourceLocation:
    if "s3" in data:
        ...
        return {
            "s3": aws_sdk_bedrock_agentcore.types.s3_location.deserialize_json(
                data["s3"]
            )
        }
    else:
