...
BatchEvaluationSummaryList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.batch_evaluation_summary.BatchEvaluationSummary"
]
...
def serialize_json(value: BatchEvaluationSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.batch_evaluation_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> BatchEvaluationSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.batch_evaluation_summary.deserialize_json(
                item
            )
        )
    return out
