...
class EmbeddedCryptoWallet(TypedDict):
    ...
    linked_accounts: "aws_sdk_bedrock_agentcore.types.linked_account_list.LinkedAccountList"
...
def serialize_json(value: EmbeddedCryptoWallet) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.crypto_wallet_network
    out["network"] = aws_sdk_bedrock_agentcore.types.crypto_wallet_network.serialize_json(value["network"])
    import aws_sdk_bedrock_agentcore.types.linked_account_list
    out["linkedAccounts"] = aws_sdk_bedrock_agentcore.types.linked_account_list.serialize_json(value["linked_accounts"])
...
def deserialize_json(data: dict) -> EmbeddedCryptoWallet:
    ...
    if "network" in data:
        import aws_sdk_bedrock_agentcore.types.crypto_wallet_network
        out["network"] = aws_sdk_bedrock_agentcore.types.crypto_wallet_network.deserialize_json(data["network"])
    else:
        raise DeserializationError("EmbeddedCryptoWallet.network required")
    if "linkedAccounts" in data:
        import aws_sdk_bedrock_agentcore.types.linked_account_list
        out["linked_accounts"] = aws_sdk_bedrock_agentcore.types.linked_account_list.deserialize_json(data["linkedAccounts"])
    else:
