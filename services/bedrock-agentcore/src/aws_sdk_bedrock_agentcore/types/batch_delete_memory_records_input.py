...
def serialize_json(value: BatchDeleteMemoryRecordsInput) -> dict:
    ...
    out["records"] = (
        aws_sdk_bedrock_agentcore.types.memory_records_delete_input_list.serialize_json(
            value["records"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> BatchDeleteMemoryRecordsInput:
    ...
    if "records" in data:
        ...
        out["records"] = (
            aws_sdk_bedrock_agentcore.types.memory_records_delete_input_list.deserialize_json(
                data["records"]
            )
        )
    else:
