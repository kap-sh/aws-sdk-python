...
BrowserEnterprisePolicies: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.browser_enterprise_policy.BrowserEnterprisePolicy"
]
...
def serialize_json(value: BrowserEnterprisePolicies) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.browser_enterprise_policy.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> BrowserEnterprisePolicies:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.browser_enterprise_policy.deserialize_json(
                item
            )
        )
    return out
