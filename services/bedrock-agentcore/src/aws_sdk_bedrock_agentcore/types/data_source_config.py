...
class _DataSourceConfig_cloudWatchLogs(TypedDict):
    cloudWatchLogs: "aws_sdk_bedrock_agentcore.types.cloud_watch_logs_source.CloudWatchLogsSource"
...
def serialize_json(value: DataSourceConfig) -> dict:
    if "cloudWatchLogs" in value:
        import aws_sdk_bedrock_agentcore.types.cloud_watch_logs_source
        return {"cloudWatchLogs": aws_sdk_bedrock_agentcore.types.cloud_watch_logs_source.serialize_json(value["cloudWatchLogs"])}
    else:
...
def deserialize_json(data: dict) -> DataSourceConfig:
    if "cloudWatchLogs" in data:
        import aws_sdk_bedrock_agentcore.types.cloud_watch_logs_source
        return {"cloudWatchLogs": aws_sdk_bedrock_agentcore.types.cloud_watch_logs_source.deserialize_json(data["cloudWatchLogs"])}
    else:
