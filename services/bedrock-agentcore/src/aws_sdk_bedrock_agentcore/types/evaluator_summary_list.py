...
EvaluatorSummaryList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.evaluator_summary.EvaluatorSummary"
]
...
def serialize_json(value: EvaluatorSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.evaluator_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> EvaluatorSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.evaluator_summary.deserialize_json(item)
        )
    return out
