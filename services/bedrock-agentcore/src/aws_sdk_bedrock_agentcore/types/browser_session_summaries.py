...
BrowserSessionSummaries: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.browser_session_summary.BrowserSessionSummary"
]
...
def serialize_json(value: BrowserSessionSummaries) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.browser_session_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> BrowserSessionSummaries:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.browser_session_summary.deserialize_json(
                item
            )
        )
    return out
