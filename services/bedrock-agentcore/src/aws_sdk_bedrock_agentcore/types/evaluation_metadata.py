...
class _EvaluationMetadata_sessionMetadata(TypedDict):
    sessionMetadata: "aws_sdk_bedrock_agentcore.types.session_metadata_list.SessionMetadataList"
...
def serialize_json(value: EvaluationMetadata) -> dict:
    if "sessionMetadata" in value:
        import aws_sdk_bedrock_agentcore.types.session_metadata_list
        return {"sessionMetadata": aws_sdk_bedrock_agentcore.types.session_metadata_list.serialize_json(value["sessionMetadata"])}
    else:
...
def deserialize_json(data: dict) -> EvaluationMetadata:
    if "sessionMetadata" in data:
        import aws_sdk_bedrock_agentcore.types.session_metadata_list
        return {"sessionMetadata": aws_sdk_bedrock_agentcore.types.session_metadata_list.deserialize_json(data["sessionMetadata"])}
    else:
