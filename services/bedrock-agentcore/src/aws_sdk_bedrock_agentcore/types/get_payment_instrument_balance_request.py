...
class GetPaymentInstrumentBalanceRequest(TypedDict):
...
def serialize_json(value: GetPaymentInstrumentBalanceRequest) -> dict:
    ...
    import aws_sdk_bedrock_agentcore.types.blockchain_chain_id
    out["chain"] = aws_sdk_bedrock_agentcore.types.blockchain_chain_id.serialize_json(value["chain"])
    import aws_sdk_bedrock_agentcore.types.instrument_balance_token
    out["token"] = aws_sdk_bedrock_agentcore.types.instrument_balance_token.serialize_json(value["token"])
    return out
...
def deserialize_json(data: dict) -> GetPaymentInstrumentBalanceRequest:
    ...
    if "paymentManagerArn" in data:
        ...
        raise DeserializationError("GetPaymentInstrumentBalanceRequest.payment_manager_arn required")
    if "paymentConnectorId" in data:
        out["payment_connector_id"] = data["paymentConnectorId"]
    else:
        raise DeserializationError("GetPaymentInstrumentBalanceRequest.payment_connector_id required")
    if "paymentInstrumentId" in data:
        out["payment_instrument_id"] = data["paymentInstrumentId"]
    else:
        raise DeserializationError("GetPaymentInstrumentBalanceRequest.payment_instrument_id required")
    if "chain" in data:
        import aws_sdk_bedrock_agentcore.types.blockchain_chain_id
        out["chain"] = aws_sdk_bedrock_agentcore.types.blockchain_chain_id.deserialize_json(data["chain"])
    else:
        raise DeserializationError("GetPaymentInstrumentBalanceRequest.chain required")
    if "token" in data:
        import aws_sdk_bedrock_agentcore.types.instrument_balance_token
        out["token"] = aws_sdk_bedrock_agentcore.types.instrument_balance_token.deserialize_json(data["token"])
    else:
