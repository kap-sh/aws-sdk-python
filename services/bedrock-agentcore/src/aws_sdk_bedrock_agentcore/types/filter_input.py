...
class FilterInput(TypedDict):
    ...
    event_metadata: NotRequired["aws_sdk_bedrock_agentcore.types.event_metadata_filter_list.EventMetadataFilterList"]
...
def serialize_json(value: FilterInput) -> dict:
    ...
    if "branch" in value:
        import aws_sdk_bedrock_agentcore.types.branch_filter
        out["branch"] = aws_sdk_bedrock_agentcore.types.branch_filter.serialize_json(value["branch"])
    if "event_metadata" in value:
        import aws_sdk_bedrock_agentcore.types.event_metadata_filter_list
        out["eventMetadata"] = aws_sdk_bedrock_agentcore.types.event_metadata_filter_list.serialize_json(value["event_metadata"])
    return out
...
def deserialize_json(data: dict) -> FilterInput:
    ...
    if "branch" in data:
        import aws_sdk_bedrock_agentcore.types.branch_filter
        out["branch"] = aws_sdk_bedrock_agentcore.types.branch_filter.deserialize_json(data["branch"])
    if "eventMetadata" in data:
        import aws_sdk_bedrock_agentcore.types.event_metadata_filter_list
        out["event_metadata"] = aws_sdk_bedrock_agentcore.types.event_metadata_filter_list.deserialize_json(data["eventMetadata"])
    return out