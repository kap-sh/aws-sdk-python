...
class _Context_spanContext(TypedDict):
...
def serialize_json(value: Context) -> dict:
    if "spanContext" in value:
        import aws_sdk_bedrock_agentcore.types.span_context
        return {"spanContext": aws_sdk_bedrock_agentcore.types.span_context.serialize_json(value["spanContext"])}
    else:
...
def deserialize_json(data: dict) -> Context:
    if "spanContext" in data:
        import aws_sdk_bedrock_agentcore.types.span_context
        return {"spanContext": aws_sdk_bedrock_agentcore.types.span_context.deserialize_json(data["spanContext"])}
    else:
