...
def serialize_json(value: HarnessContentBlockDeltaEvent) -> dict:
    ...
    import aws_sdk_bedrock_agentcore.types.harness_content_block_delta
    out["delta"] = aws_sdk_bedrock_agentcore.types.harness_content_block_delta.serialize_json(value["delta"])
    return out
...
def deserialize_json(data: dict) -> HarnessContentBlockDeltaEvent:
    ...
    if "contentBlockIndex" in data:
        ...
        raise DeserializationError("HarnessContentBlockDeltaEvent.content_block_index required")
    if "delta" in data:
        import aws_sdk_bedrock_agentcore.types.harness_content_block_delta
        out["delta"] = aws_sdk_bedrock_agentcore.types.harness_content_block_delta.deserialize_json(data["delta"])
    else:
