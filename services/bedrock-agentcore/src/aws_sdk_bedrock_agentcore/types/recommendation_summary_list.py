...
RecommendationSummaryList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.recommendation_summary.RecommendationSummary"
]
...
def serialize_json(value: RecommendationSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.recommendation_summary.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> RecommendationSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.recommendation_summary.deserialize_json(
                item
            )
        )
    return out
