...
def serialize_json(value: HarnessGatewayOutboundAuth) -> dict:
    if "awsIam" in value:
        ...
        return {
            "oauth": aws_sdk_bedrock_agentcore.types.o_auth_credential_provider.serialize_json(
                value["oauth"]
            )
        }
    else:
...
def deserialize_json(data: dict) -> HarnessGatewayOutboundAuth:
    if "awsIam" in data:
        ...
        return {
            "oauth": aws_sdk_bedrock_agentcore.types.o_auth_credential_provider.deserialize_json(
                data["oauth"]
            )
        }
    else:
