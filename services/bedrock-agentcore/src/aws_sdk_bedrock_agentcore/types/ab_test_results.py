...
class ABTestResults(TypedDict):
    ...
    evaluator_metrics: "aws_sdk_bedrock_agentcore.types.evaluator_metric_list.EvaluatorMetricList"
...
def serialize_json(value: ABTestResults) -> dict:
    ...
    if "analysis_timestamp" in value:
        ...
        out["analysisTimestamp"] = aws_sdk_bedrock_agentcore.types._prelude.timestamp.serialize_json(value["analysis_timestamp"])
    import aws_sdk_bedrock_agentcore.types.evaluator_metric_list
    out["evaluatorMetrics"] = aws_sdk_bedrock_agentcore.types.evaluator_metric_list.serialize_json(value["evaluator_metrics"])
    return out
...
def deserialize_json(data: dict) -> ABTestResults:
    ...
    if "analysisTimestamp" in data:
        import aws_sdk_bedrock_agentcore.types._prelude.timestamp
        out["analysis_timestamp"] = aws_sdk_bedrock_agentcore.types._prelude.timestamp.deserialize_json(data["analysisTimestamp"])
    if "evaluatorMetrics" in data:
        import aws_sdk_bedrock_agentcore.types.evaluator_metric_list
        out["evaluator_metrics"] = aws_sdk_bedrock_agentcore.types.evaluator_metric_list.deserialize_json(data["evaluatorMetrics"])
    else:
