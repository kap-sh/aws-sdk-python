...
def serialize_json(value: ExtractionJobMessages) -> dict:
    if "messagesList" in value:
        import aws_sdk_bedrock_agentcore.types.messages_list
        return {"messagesList": aws_sdk_bedrock_agentcore.types.messages_list.serialize_json(value["messagesList"])}
    else:
...
def deserialize_json(data: dict) -> ExtractionJobMessages:
    if "messagesList" in data:
        import aws_sdk_bedrock_agentcore.types.messages_list
        return {"messagesList": aws_sdk_bedrock_agentcore.types.messages_list.deserialize_json(data["messagesList"])}
    else:
