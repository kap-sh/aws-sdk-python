...
GroundTruthTurnList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.ground_truth_turn.GroundTruthTurn"
]
...
def serialize_json(value: GroundTruthTurnList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.ground_truth_turn.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> GroundTruthTurnList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.ground_truth_turn.deserialize_json(item)
        )
    return out
