...
class DeletePaymentSessionResponse(TypedDict):
    status: "aws_sdk_bedrock_agentcore.types.payment_session_status.PaymentSessionStatus"
...
def serialize_json(value: DeletePaymentSessionResponse) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.payment_session_status
    out["status"] = aws_sdk_bedrock_agentcore.types.payment_session_status.serialize_json(value["status"])
    return out
...
def deserialize_json(data: dict) -> DeletePaymentSessionResponse:
    ...
    if "status" in data:
        import aws_sdk_bedrock_agentcore.types.payment_session_status
        out["status"] = aws_sdk_bedrock_agentcore.types.payment_session_status.deserialize_json(data["status"])
    else:
