...
def deserialize_json(data: dict) -> GetResourceApiKeyRequest:
    ...
    if "workloadIdentityToken" in data:
        ...
        raise DeserializationError("GetResourceApiKeyRequest.workload_identity_token required")
    if "resourceCredentialProviderName" in data:
        out["resource_credential_provider_name"] = data["resourceCredentialProviderName"]
    else:
        raise DeserializationError("GetResourceApiKeyRequest.resource_credential_provider_name required")
    return out