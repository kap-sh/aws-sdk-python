...
CloudWatchLogsFilterList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.cloud_watch_logs_filter.CloudWatchLogsFilter"
]
...
def serialize_json(value: CloudWatchLogsFilterList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.cloud_watch_logs_filter.serialize_json(item)
        )
    return out
...
def deserialize_json(data: list) -> CloudWatchLogsFilterList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.cloud_watch_logs_filter.deserialize_json(
                item
            )
        )
    return out
