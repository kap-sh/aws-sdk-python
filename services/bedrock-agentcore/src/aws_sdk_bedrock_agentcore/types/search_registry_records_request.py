...
class SearchRegistryRecordsRequest(TypedDict):
    search_query: "str"
...
def serialize_json(value: SearchRegistryRecordsRequest) -> dict:
    ...
    out["registryIds"] = (
        aws_sdk_bedrock_agentcore.types.registry_id_list.serialize_json(
            value["registry_ids"]
        )
    )
...
def deserialize_json(data: dict) -> SearchRegistryRecordsRequest:
    ...
    if "registryIds" in data:
        ...
        out["registry_ids"] = (
            aws_sdk_bedrock_agentcore.types.registry_id_list.deserialize_json(
                data["registryIds"]
            )
        )
    else:
