...
PaymentInstrumentSummaryList: TypeAlias = list[
    "aws_sdk_bedrock_agentcore.types.payment_instrument_summary.PaymentInstrumentSummary"
]
...
def serialize_json(value: PaymentInstrumentSummaryList) -> list:
    ...
    for item in value:
        out.append(
            aws_sdk_bedrock_agentcore.types.payment_instrument_summary.serialize_json(
                item
            )
        )
    return out
...
def deserialize_json(data: list) -> PaymentInstrumentSummaryList:
    ...
    for item in data:
        out.append(
            aws_sdk_bedrock_agentcore.types.payment_instrument_summary.deserialize_json(
                item
            )
        )
    return out
