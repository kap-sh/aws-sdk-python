...
def serialize_json(value: BrowserExtension) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.resource_location
    out["location"] = aws_sdk_bedrock_agentcore.types.resource_location.serialize_json(value["location"])
    return out
...
def deserialize_json(data: dict) -> BrowserExtension:
    ...
    if "location" in data:
        import aws_sdk_bedrock_agentcore.types.resource_location
        out["location"] = aws_sdk_bedrock_agentcore.types.resource_location.deserialize_json(data["location"])
    else:
