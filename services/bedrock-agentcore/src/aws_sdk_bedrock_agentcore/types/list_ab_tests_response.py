...
class ListABTestsResponse(TypedDict):
    ab_tests: "aws_sdk_bedrock_agentcore.types.ab_test_summary_list.ABTestSummaryList"
...
def serialize_json(value: ListABTestsResponse) -> dict:
    ...
    out["abTests"] = (
        aws_sdk_bedrock_agentcore.types.ab_test_summary_list.serialize_json(
            value["ab_tests"]
        )
    )
...
def deserialize_json(data: dict) -> ListABTestsResponse:
    ...
    if "abTests" in data:
        ...
        out["ab_tests"] = (
            aws_sdk_bedrock_agentcore.types.ab_test_summary_list.deserialize_json(
                data["abTests"]
            )
        )
    else:
