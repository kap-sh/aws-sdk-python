...
def serialize_json(value: DeletePaymentInstrumentResponse) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.payment_instrument_status
    out["status"] = aws_sdk_bedrock_agentcore.types.payment_instrument_status.serialize_json(value["status"])
    return out
...
def deserialize_json(data: dict) -> DeletePaymentInstrumentResponse:
    ...
    if "status" in data:
        import aws_sdk_bedrock_agentcore.types.payment_instrument_status
        out["status"] = aws_sdk_bedrock_agentcore.types.payment_instrument_status.deserialize_json(data["status"])
    else:
