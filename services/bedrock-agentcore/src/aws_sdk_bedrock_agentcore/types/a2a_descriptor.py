...
class A2aDescriptor(TypedDict):
    agent_card: (
        "aws_sdk_bedrock_agentcore.types.agent_card_definition.AgentCardDefinition"
    )
...
def serialize_json(value: A2aDescriptor) -> dict:
    ...
    out["agentCard"] = (
        aws_sdk_bedrock_agentcore.types.agent_card_definition.serialize_json(
            value["agent_card"]
        )
    )
    return out
...
def deserialize_json(data: dict) -> A2aDescriptor:
    ...
    if "agentCard" in data:
        ...
        out["agent_card"] = (
            aws_sdk_bedrock_agentcore.types.agent_card_definition.deserialize_json(
                data["agentCard"]
            )
        )
    else:
