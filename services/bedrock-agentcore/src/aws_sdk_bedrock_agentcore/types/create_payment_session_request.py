...
def serialize_json(value: CreatePaymentSessionRequest) -> dict:
    ...
    if "limits" in value:
        import aws_sdk_bedrock_agentcore.types.session_limits
        out["limits"] = aws_sdk_bedrock_agentcore.types.session_limits.serialize_json(value["limits"])
...
def deserialize_json(data: dict) -> CreatePaymentSessionRequest:
    ...
    if "paymentManagerArn" in data:
        ...
        raise DeserializationError("CreatePaymentSessionRequest.payment_manager_arn required")
    if "limits" in data:
        import aws_sdk_bedrock_agentcore.types.session_limits
        out["limits"] = aws_sdk_bedrock_agentcore.types.session_limits.deserialize_json(data["limits"])
    if "expiryTimeInMinutes" in data:
        out["expiry_time_in_minutes"] = data["expiryTimeInMinutes"]
    else:
        raise DeserializationError("CreatePaymentSessionRequest.expiry_time_in_minutes required")
