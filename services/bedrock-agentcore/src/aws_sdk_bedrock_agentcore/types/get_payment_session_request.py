...
def deserialize_json(data: dict) -> GetPaymentSessionRequest:
    ...
    if "paymentManagerArn" in data:
        ...
        raise DeserializationError("GetPaymentSessionRequest.payment_manager_arn required")
    if "paymentSessionId" in data:
        out["payment_session_id"] = data["paymentSessionId"]
    else:
        raise DeserializationError("GetPaymentSessionRequest.payment_session_id required")
    return out