...
if TYPE_CHECKING:
    import aws_sdk_bedrock_agentcore.types.control_stats
    import aws_sdk_bedrock_agentcore.types.variant_result_list

class EvaluatorMetric(TypedDict):
    evaluator_arn: "str"
    """<p>The Amazon Resource Name (ARN) of the evaluator.</p>"""
    control_stats: "aws_sdk_bedrock_agentcore.types.control_stats.ControlStats"
    """<p>The statistics for the control variant.</p>"""
    variant_results: "aws_sdk_bedrock_agentcore.types.variant_result_list.VariantResultList"
...
def serialize_json(value: EvaluatorMetric) -> dict:
    ...
    import aws_sdk_bedrock_agentcore.types.control_stats
    out["controlStats"] = aws_sdk_bedrock_agentcore.types.control_stats.serialize_json(value["control_stats"])
    import aws_sdk_bedrock_agentcore.types.variant_result_list
    out["variantResults"] = aws_sdk_bedrock_agentcore.types.variant_result_list.serialize_json(value["variant_results"])
    return out
...
def deserialize_json(data: dict) -> EvaluatorMetric:
    ...
    if "controlStats" in data:
        import aws_sdk_bedrock_agentcore.types.control_stats
        out["control_stats"] = aws_sdk_bedrock_agentcore.types.control_stats.deserialize_json(data["controlStats"])
    else:
        raise DeserializationError("EvaluatorMetric.control_stats required")
    if "variantResults" in data:
        import aws_sdk_bedrock_agentcore.types.variant_result_list
        out["variant_results"] = aws_sdk_bedrock_agentcore.types.variant_result_list.deserialize_json(data["variantResults"])
    else:
