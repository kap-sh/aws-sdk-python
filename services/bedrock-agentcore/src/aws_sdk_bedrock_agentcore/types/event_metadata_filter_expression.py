...
class EventMetadataFilterExpression(TypedDict):
    ...
    right: NotRequired["aws_sdk_bedrock_agentcore.types.right_expression.RightExpression"]
...
def serialize_json(value: EventMetadataFilterExpression) -> dict:
    out: dict = {}
    import aws_sdk_bedrock_agentcore.types.left_expression
    out["left"] = aws_sdk_bedrock_agentcore.types.left_expression.serialize_json(value["left"])
    import aws_sdk_bedrock_agentcore.types.operator_type
    out["operator"] = aws_sdk_bedrock_agentcore.types.operator_type.serialize_json(value["operator"])
    if "right" in value:
        import aws_sdk_bedrock_agentcore.types.right_expression
        out["right"] = aws_sdk_bedrock_agentcore.types.right_expression.serialize_json(value["right"])
    return out
...
def deserialize_json(data: dict) -> EventMetadataFilterExpression:
    ...
    if "left" in data:
        import aws_sdk_bedrock_agentcore.types.left_expression
        out["left"] = aws_sdk_bedrock_agentcore.types.left_expression.deserialize_json(data["left"])
    else:
        raise DeserializationError("EventMetadataFilterExpression.left required")
    if "operator" in data:
        import aws_sdk_bedrock_agentcore.types.operator_type
        out["operator"] = aws_sdk_bedrock_agentcore.types.operator_type.deserialize_json(data["operator"])
    else:
        raise DeserializationError("EventMetadataFilterExpression.operator required")
    if "right" in data:
        import aws_sdk_bedrock_agentcore.types.right_expression
        out["right"] = aws_sdk_bedrock_agentcore.types.right_expression.deserialize_json(data["right"])
    return out