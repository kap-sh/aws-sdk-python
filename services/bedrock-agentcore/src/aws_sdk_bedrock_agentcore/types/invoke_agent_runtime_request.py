...
class InvokeAgentRuntimeRequest(TypedDict):
...
def serialize_json(value: InvokeAgentRuntimeRequest) -> dict:
    ...
    out["payload"] = aws_sdk_bedrock_agentcore.types.body.serialize_json(
        value["payload"]
    )
    return out
...
def deserialize_json(data: dict) -> InvokeAgentRuntimeRequest:
    ...
    if "payload" in data:
        ...
        out["payload"] = aws_sdk_bedrock_agentcore.types.body.deserialize_json(
            data["payload"]
        )
    else:
