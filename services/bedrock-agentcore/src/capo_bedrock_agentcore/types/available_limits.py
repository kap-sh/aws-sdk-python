...
class AvailableLimits(TypedDict, closed=True):
    available_spend_amount: NotRequired["capo_bedrock_agentcore.types.amount.Amount"]
    """<p>The remaining available amount that can be spent.</p>"""
    updated_at: NotRequired["capo_bedrock_agentcore.types.date_timestamp.DateTimestamp"]
...
def serialize_json(value: AvailableLimits) -> dict:
    ...
    if "available_spend_amount" in value:
        ...
        out["availableSpendAmount"] = (
            capo_bedrock_agentcore.types.amount.serialize_json(
...
def deserialize_json(data: dict) -> AvailableLimits:
    ...
    if data.get("availableSpendAmount") is not None:
        ...
        out["available_spend_amount"] = (
            capo_bedrock_agentcore.types.amount.deserialize_json(
