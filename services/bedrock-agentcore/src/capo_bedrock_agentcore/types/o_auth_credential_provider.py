...
if TYPE_CHECKING:
    import capo_bedrock_agentcore.types.o_auth_credential_provider_arn
    import capo_bedrock_agentcore.types.o_auth_custom_parameters
    import capo_bedrock_agentcore.types.o_auth_default_return_url
    import capo_bedrock_agentcore.types.o_auth_grant_type
    import capo_bedrock_agentcore.types.o_auth_scopes


class OAuthCredentialProvider(TypedDict, closed=True):
    provider_arn: "capo_bedrock_agentcore.types.o_auth_credential_provider_arn.OAuthCredentialProviderArn"
    """<p>The ARN of the OAuth 2.0 credential provider in AgentCore Identity.</p>"""
    scopes: "capo_bedrock_agentcore.types.o_auth_scopes.OAuthScopes"
    """<p>The OAuth 2.0 scopes to request when obtaining an access token.</p>"""
    custom_parameters: NotRequired[
        "capo_bedrock_agentcore.types.o_auth_custom_parameters.OAuthCustomParameters"
    ]
    """<p>Additional custom parameters to include in the OAuth 2.0 token request.</p>"""
    grant_type: "capo_bedrock_agentcore.types.o_auth_grant_type.OAuthGrantType"
    """<p>The OAuth 2.0 grant type to use for authentication.</p>"""
    default_return_url: NotRequired[
        "capo_bedrock_agentcore.types.o_auth_default_return_url.OAuthDefaultReturnUrl"
    ]
...
def serialize_json(value: OAuthCredentialProvider) -> dict:
    ...
    out["scopes"] = capo_bedrock_agentcore.types.o_auth_scopes.serialize_json(
        value["scopes"]
    )
    if "custom_parameters" in value:
        ...
        out["customParameters"] = (
            capo_bedrock_agentcore.types.o_auth_custom_parameters.serialize_json(
                value["custom_parameters"]
            )
        )
    import capo_bedrock_agentcore.types.o_auth_grant_type

    out["grantType"] = capo_bedrock_agentcore.types.o_auth_grant_type.serialize_json(
...
def deserialize_json(data: dict) -> OAuthCredentialProvider:
    ...
    if "scopes" in data:
        ...
        out["scopes"] = capo_bedrock_agentcore.types.o_auth_scopes.deserialize_json(
            data["scopes"]
        )
    else:
    ...
    if "customParameters" in data:
        ...
        out["custom_parameters"] = (
            capo_bedrock_agentcore.types.o_auth_custom_parameters.deserialize_json(
    ...
    if "grantType" in data:
        ...
        out["grant_type"] = (
            capo_bedrock_agentcore.types.o_auth_grant_type.deserialize_json(
                data["grantType"]
            )
        )
    else:
