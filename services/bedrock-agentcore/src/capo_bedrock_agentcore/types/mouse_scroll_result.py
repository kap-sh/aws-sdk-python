"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#MouseScrollResult``."""
...
class MouseScrollResult(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: MouseScrollResult) -> dict:
    out: dict = {}
    import capo_bedrock_agentcore.types.browser_action_status
...
def deserialize_json(data: dict) -> MouseScrollResult:
    out: MouseScrollResult = {}  # type: ignore[typeddict-item]
    if "status" in data:
        import capo_bedrock_agentcore.types.browser_action_status
        ...
        raise DeserializationError("MouseScrollResult.status required")
    if "error" in data:
