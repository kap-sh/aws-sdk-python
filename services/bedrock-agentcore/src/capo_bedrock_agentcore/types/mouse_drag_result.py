"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#MouseDragResult``."""
...
class MouseDragResult(TypedDict, closed=True):
...
# --- restJson1 ser/de ---
def serialize_json(value: MouseDragResult) -> dict:
    out: dict = {}
    import capo_bedrock_agentcore.types.browser_action_status
...
def deserialize_json(data: dict) -> MouseDragResult:
    out: MouseDragResult = {}  # type: ignore[typeddict-item]
    if data.get("status") is not None:
        ...
        raise DeserializationError("MouseDragResult.status required")
