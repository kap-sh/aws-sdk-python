...
class _HarnessSkill_path(TypedDict, closed=True):
    path: "capo_bedrock_agentcore.types.harness_skill_path.HarnessSkillPath"


class _HarnessSkill_s3(TypedDict, closed=True):
    s3: "capo_bedrock_agentcore.types.harness_skill_s3_source.HarnessSkillS3Source"
...
def serialize_json(value: HarnessSkill) -> dict:
    if "path" in value:
        ...
        return {
            "s3": capo_bedrock_agentcore.types.harness_skill_s3_source.serialize_json(
                value["s3"]
            )
        }
    elif "git" in value:
...
def deserialize_json(data: dict) -> HarnessSkill:
    if data.get("path") is not None:
        ...
        return {
            "s3": capo_bedrock_agentcore.types.harness_skill_s3_source.deserialize_json(
                data["s3"]
            )
        }
