...
class _CertificateLocation_secretsManager(TypedDict, closed=True):
    secretsManager: (
        "capo_bedrock_agentcore.types.secrets_manager_location.SecretsManagerLocation"
    )
...
def serialize_json(value: CertificateLocation) -> dict:
    if "secretsManager" in value:
        ...
        return {
            "secretsManager": capo_bedrock_agentcore.types.secrets_manager_location.serialize_json(
...
def deserialize_json(data: dict) -> CertificateLocation:
    if data.get("secretsManager") is not None:
        ...
        return {
            "secretsManager": capo_bedrock_agentcore.types.secrets_manager_location.deserialize_json(
                data["secretsManager"]
            )
        }
    else:
