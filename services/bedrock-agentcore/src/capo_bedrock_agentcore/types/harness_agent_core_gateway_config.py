...
class HarnessAgentCoreGatewayConfig(TypedDict, closed=True):
    ...
    outbound_auth: NotRequired[
        "capo_bedrock_agentcore.types.harness_gateway_outbound_auth.HarnessGatewayOutboundAuth"
    ]
...
def serialize_json(value: HarnessAgentCoreGatewayConfig) -> dict:
    ...
    if "outbound_auth" in value:
        ...
        out["outboundAuth"] = (
            capo_bedrock_agentcore.types.harness_gateway_outbound_auth.serialize_json(
                value["outbound_auth"]
            )
        )
    return out
...
def deserialize_json(data: dict) -> HarnessAgentCoreGatewayConfig:
    ...
    if "outboundAuth" in data:
        ...
        out["outbound_auth"] = (
            capo_bedrock_agentcore.types.harness_gateway_outbound_auth.deserialize_json(
                data["outboundAuth"]
            )
        )
    return out
