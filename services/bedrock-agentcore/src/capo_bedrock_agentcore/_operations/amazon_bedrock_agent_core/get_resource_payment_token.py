"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#GetResourcePaymentToken``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore.types.get_resource_payment_token_response.GetResourcePaymentTokenResponse = capo_bedrock_agentcore.types.get_resource_payment_token_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/identities/payment/token"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore.types.get_resource_payment_token_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_get_resource_payment_token(
