"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#CreatePaymentInstrument``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore.types.create_payment_instrument_response.CreatePaymentInstrumentResponse = capo_bedrock_agentcore.types.create_payment_instrument_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/payments/createPaymentInstrument"
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "user_id" in input_:
        headers["X-Amzn-Bedrock-AgentCore-Payments-User-Id"] = input_["user_id"]
    if "agent_name" in input_:
        headers["X-Amzn-Bedrock-AgentCore-Payments-Agent-Name"] = input_["agent_name"]
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore.types.create_payment_instrument_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
...
async def async_create_payment_instrument(
