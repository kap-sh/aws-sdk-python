...
def handle_response(
    ...
    out: capo_bedrock_agentcore.types.invoke_browser_response.InvokeBrowserResponse = (
        capo_bedrock_agentcore.types.invoke_browser_response.deserialize_json(
            json.loads(response.read())
        )
    )
    out["session_id"] = response.headers["x-amzn-browser-session-id"]
    return out
...
def build_request(
    ...
    if "session_id" in input_:
        headers["x-amzn-browser-session-id"] = input_["session_id"]
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore.types.invoke_browser_request.serialize_json(input_),
        allow_nan=False,
    ).encode()
