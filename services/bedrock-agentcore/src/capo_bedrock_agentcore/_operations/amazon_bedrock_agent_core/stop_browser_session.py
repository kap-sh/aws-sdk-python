...
def handle_response(
    ...
    out: capo_bedrock_agentcore.types.stop_browser_session_response.StopBrowserSessionResponse = capo_bedrock_agentcore.types.stop_browser_session_response.deserialize_json(
...
def build_request(
    ...
    url = url.replace(
        "{browserIdentifier}", quote(input_["browser_identifier"], safe="")
    )
    params: list[tuple[str, str]] = []
    if "session_id" in input_:
        params.append(("sessionId", input_["session_id"]))
    ...
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore.types.stop_browser_session_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
