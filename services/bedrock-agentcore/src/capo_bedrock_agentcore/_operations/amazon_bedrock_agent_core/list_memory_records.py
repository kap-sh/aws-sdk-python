"""Generated from Smithy shape ``com.amazonaws.bedrockagentcore#ListMemoryRecords``."""
...
def handle_response(
    ...
    out: capo_bedrock_agentcore.types.list_memory_records_output.ListMemoryRecordsOutput = capo_bedrock_agentcore.types.list_memory_records_output.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/memories/{memoryId}/memoryRecords"
    url = url.replace("{memoryId}", quote(input_["memory_id"], safe=""))
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore.types.list_memory_records_input.serialize_json(input_),
        allow_nan=False,
    ).encode()
...
async def async_list_memory_records(
