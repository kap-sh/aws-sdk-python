...
import capo_bedrock_agentcore.types.code_interpreter_stream_output
...
def handle_response(
    ...
    _union_deser = capo_bedrock_agentcore.types.code_interpreter_stream_output.deserialize_event_json
    _iter = cast(Any, response.iter_bytes())
    out: capo_bedrock_agentcore.types.invoke_code_interpreter_response.InvokeCodeInterpreterResponse = {
    ...
    if "x-amzn-code-interpreter-session-id" in response.headers:
        out["session_id"] = response.headers["x-amzn-code-interpreter-session-id"]
    return out
...
def build_request(
    ...
    url = url.replace(
        ...
        quote(input_["code_interpreter_identifier"], safe=""),
    )
    params: list[tuple[str, str]] = []
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    if "session_id" in input_:
        headers["x-amzn-code-interpreter-session-id"] = input_["session_id"]
    ...
    body: bytes | None = json.dumps(
        capo_bedrock_agentcore.types.invoke_code_interpreter_request.serialize_json(
            input_
        ),
        allow_nan=False,
    ).encode()
