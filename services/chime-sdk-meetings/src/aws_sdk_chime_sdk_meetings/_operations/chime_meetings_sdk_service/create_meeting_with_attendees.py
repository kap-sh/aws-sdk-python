"""Generated from Smithy shape ``com.amazonaws.chimesdkmeetings#CreateMeetingWithAttendees``."""
...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ConflictException":
            import aws_sdk_chime_sdk_meetings.errors.conflict_exception

            raise aws_sdk_chime_sdk_meetings.errors.conflict_exception.ConflictException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_chime_sdk_meetings.types.create_meeting_with_attendees_response.CreateMeetingWithAttendeesResponse = aws_sdk_chime_sdk_meetings.types.create_meeting_with_attendees_response.deserialize_json(
...
def build_request(
    ...
    input_: aws_sdk_chime_sdk_meetings.types.create_meeting_with_attendees_request.CreateMeetingWithAttendeesRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    url = endpoint.url.rstrip("/") + "/meetings?operation=create-attendees"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    import aws_sdk_chime_sdk_meetings.types.create_meeting_with_attendees_request

    body: bytes | None = json.dumps(
        aws_sdk_chime_sdk_meetings.types.create_meeting_with_attendees_request.serialize_json(
            input_
        )
    ).encode()
...
async def async_create_meeting_with_attendees(
