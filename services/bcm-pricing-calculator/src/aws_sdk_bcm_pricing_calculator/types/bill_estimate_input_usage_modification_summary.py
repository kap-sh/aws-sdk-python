...
def deserialize_aws_json_1_0(data: dict) -> BillEstimateInputUsageModificationSummary:
    ...
    if "serviceCode" in data:
        ...
        raise DeserializationError("BillEstimateInputUsageModificationSummary.service_code required")
    if "usageType" in data:
        out["usage_type"] = data["usageType"]
    else:
        raise DeserializationError("BillEstimateInputUsageModificationSummary.usage_type required")
    if "operation" in data:
        out["operation"] = data["operation"]
    else:
        raise DeserializationError("BillEstimateInputUsageModificationSummary.operation required")
    if "location" in data:
