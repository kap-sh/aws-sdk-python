...
class BatchCreateBillScenarioUsageModificationEntry(TypedDict):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: BatchCreateBillScenarioUsageModificationEntry) -> dict:
    out: dict = {}
...
def deserialize_aws_json_1_0(data: dict) -> BatchCreateBillScenarioUsageModificationEntry:
    out: BatchCreateBillScenarioUsageModificationEntry = {}  # type: ignore[typeddict-item]
    if "serviceCode" in data:
        out["service_code"] = data["serviceCode"]
    else:
        raise DeserializationError("BatchCreateBillScenarioUsageModificationEntry.service_code required")
    if "usageType" in data:
        out["usage_type"] = data["usageType"]
    else:
        raise DeserializationError("BatchCreateBillScenarioUsageModificationEntry.usage_type required")
    if "operation" in data:
        out["operation"] = data["operation"]
    else:
        raise DeserializationError("BatchCreateBillScenarioUsageModificationEntry.operation required")
    if "availabilityZone" in data:
        out["availability_zone"] = data["availabilityZone"]
    if "key" in data:
        out["key"] = data["key"]
    else:
        raise DeserializationError("BatchCreateBillScenarioUsageModificationEntry.key required")
    if "group" in data:
        out["group"] = data["group"]
    if "usageAccountId" in data:
        out["usage_account_id"] = data["usageAccountId"]
    else:
        raise DeserializationError("BatchCreateBillScenarioUsageModificationEntry.usage_account_id required")
    if "amounts" in data:
        import aws_sdk_bcm_pricing_calculator.types.usage_amounts
