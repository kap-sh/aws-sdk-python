"""Generated from Smithy shape ``com.amazonaws.bcmpricingcalculator#WorkloadEstimateSummary``."""
...
class WorkloadEstimateSummary(TypedDict, closed=True):
...
# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: WorkloadEstimateSummary) -> dict:
    out: dict = {}
    out["id"] = value["id"]
    if "name" in value:
        out["name"] = value["name"]
...
def deserialize_aws_json_1_0(data: dict) -> WorkloadEstimateSummary:
    out: WorkloadEstimateSummary = {}  # type: ignore[typeddict-item]
    if "id" in data:
        out["id"] = data["id"]
    else:
        raise DeserializationError("WorkloadEstimateSummary.id required")
    if "name" in data:
        out["name"] = data["name"]
