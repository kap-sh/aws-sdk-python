...
class BatchCreateBillScenarioCommitmentModificationEntry(TypedDict):
    ...
    commitment_action: "aws_sdk_bcm_pricing_calculator.types.bill_scenario_commitment_modification_action.BillScenarioCommitmentModificationAction"
    """<p> The specific commitment action to be taken (e.g., adding a Reserved Instance or Savings Plan). </p>"""

# --- awsJson1_0 ser/de ---
def serialize_aws_json_1_0(value: BatchCreateBillScenarioCommitmentModificationEntry) -> dict:
    out: dict = {}
    out["key"] = value["key"]
    if "group" in value:
...
def deserialize_aws_json_1_0(data: dict) -> BatchCreateBillScenarioCommitmentModificationEntry:
    out: BatchCreateBillScenarioCommitmentModificationEntry = {}  # type: ignore[typeddict-item]
    if "key" in data:
        out["key"] = data["key"]
    else:
        raise DeserializationError("BatchCreateBillScenarioCommitmentModificationEntry.key required")
    if "group" in data:
        out["group"] = data["group"]
    if "usageAccountId" in data:
        out["usage_account_id"] = data["usageAccountId"]
    else:
        raise DeserializationError("BatchCreateBillScenarioCommitmentModificationEntry.usage_account_id required")
    if "commitmentAction" in data:
        ...
        raise DeserializationError("BatchCreateBillScenarioCommitmentModificationEntry.commitment_action required")
    return out