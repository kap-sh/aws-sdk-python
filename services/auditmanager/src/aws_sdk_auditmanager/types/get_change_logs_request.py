"""Generated from Smithy shape ``com.amazonaws.auditmanager#GetChangeLogsRequest``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import aws_sdk_auditmanager.types.control_set_id
    import aws_sdk_auditmanager.types.max_results
    import aws_sdk_auditmanager.types.token
    import aws_sdk_auditmanager.types.uuid


class GetChangeLogsRequest(TypedDict, closed=True):
    assessment_id: "aws_sdk_auditmanager.types.uuid.UUID"
    """<p>The unique identifier for the assessment. </p>"""
    control_set_id: NotRequired[
        "aws_sdk_auditmanager.types.control_set_id.ControlSetId"
    ]
    """<p> The unique identifier for the control set. </p>"""
    control_id: NotRequired["aws_sdk_auditmanager.types.uuid.UUID"]
    """<p> The unique identifier for the control. </p>"""
    next_token: NotRequired["aws_sdk_auditmanager.types.token.Token"]
    """<p> The pagination token that's used to fetch the next set of results. </p>"""
    max_results: NotRequired["aws_sdk_auditmanager.types.max_results.MaxResults"]
    """<p>Represents the maximum number of results on a page or for an API request call. </p>"""


# --- restJson1 ser/de ---
def serialize_json(value: GetChangeLogsRequest) -> dict:
    out: dict = {}
    return out


def deserialize_json(data: dict) -> GetChangeLogsRequest:
    out: GetChangeLogsRequest = {}  # type: ignore[typeddict-item]
    return out
