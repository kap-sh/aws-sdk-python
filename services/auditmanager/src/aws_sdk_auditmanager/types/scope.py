...
def serialize_json(value: Scope) -> dict:
    ...
    if "aws_accounts" in value:
        ...
        out["awsAccounts"] = aws_sdk_auditmanager.types.aws_accounts.serialize_json(
            value["aws_accounts"]
        )
    if "aws_services" in value:
        import aws_sdk_auditmanager.types.aws_services

        out["awsServices"] = aws_sdk_auditmanager.types.aws_services.serialize_json(
            value["aws_services"]
        )
    return out
...
def deserialize_json(data: dict) -> Scope:
    ...
    if "awsAccounts" in data:
        ...
        out["aws_accounts"] = aws_sdk_auditmanager.types.aws_accounts.deserialize_json(
            data["awsAccounts"]
        )
    if "awsServices" in data:
        import aws_sdk_auditmanager.types.aws_services

        out["aws_services"] = aws_sdk_auditmanager.types.aws_services.deserialize_json(
            data["awsServices"]
        )
    return out
