...
if TYPE_CHECKING:
    ...
    import aws_sdk_auditmanager.types.account_name
    import aws_sdk_auditmanager.types.email_address


class AWSAccount(TypedDict, closed=True):
    id: NotRequired["aws_sdk_auditmanager.types.account_id.AccountId"]
    """<p> The identifier for the Amazon Web Services account. </p>"""
    email_address: NotRequired["aws_sdk_auditmanager.types.email_address.EmailAddress"]
    """<p> The email address that's associated with the Amazon Web Services account. </p>"""
    name: NotRequired["aws_sdk_auditmanager.types.account_name.AccountName"]
