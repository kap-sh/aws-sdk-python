...
class GetInsightsByAssessmentResponse(TypedDict, closed=True):
    insights: NotRequired[
        "capo_auditmanager.types.insights_by_assessment.InsightsByAssessment"
    ]
...
def serialize_json(value: GetInsightsByAssessmentResponse) -> dict:
    ...
    if "insights" in value:
        ...
        out["insights"] = capo_auditmanager.types.insights_by_assessment.serialize_json(
            value["insights"]
        )
    return out
...
def deserialize_json(data: dict) -> GetInsightsByAssessmentResponse:
    ...
    if "insights" in data:
        ...
        out["insights"] = (
            capo_auditmanager.types.insights_by_assessment.deserialize_json(
                data["insights"]
            )
        )
    return out
