...
if TYPE_CHECKING:
    import capo_auditmanager.types.cloud_trail_arn
...
class EvidenceFinderEnablement(TypedDict, closed=True):
    event_data_store_arn: NotRequired[
        "capo_auditmanager.types.cloud_trail_arn.CloudTrailArn"
    ]
    ...
    enablement_status: NotRequired[
        "capo_auditmanager.types.evidence_finder_enablement_status.EvidenceFinderEnablementStatus"
    ]
    ...
    backfill_status: NotRequired[
        "capo_auditmanager.types.evidence_finder_backfill_status.EvidenceFinderBackfillStatus"
    ]
    ...
    error: NotRequired["capo_auditmanager.types.error_message.ErrorMessage"]
...
def serialize_json(value: EvidenceFinderEnablement) -> dict:
    ...
    if "enablement_status" in value:
        ...
        out["enablementStatus"] = (
            capo_auditmanager.types.evidence_finder_enablement_status.serialize_json(
    ...
    if "backfill_status" in value:
        ...
        out["backfillStatus"] = (
            capo_auditmanager.types.evidence_finder_backfill_status.serialize_json(
...
def deserialize_json(data: dict) -> EvidenceFinderEnablement:
    ...
    if "enablementStatus" in data:
        ...
        out["enablement_status"] = (
            capo_auditmanager.types.evidence_finder_enablement_status.deserialize_json(
    ...
    if "backfillStatus" in data:
        ...
        out["backfill_status"] = (
            capo_auditmanager.types.evidence_finder_backfill_status.deserialize_json(
                data["backfillStatus"]
            )
        )
    if "error" in data:
