"""Generated from Smithy shape ``com.amazonaws.rtbfabric#UpdateRequesterGateway``."""
...
def handle_response(
    ...
    out: capo_rtbfabric.types.update_requester_gateway_response.UpdateRequesterGatewayResponse = capo_rtbfabric.types.update_requester_gateway_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/requester-gateway/{gatewayId}/update"
    url = url.replace("{gatewayId}", quote(str(input_["gateway_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_rtbfabric.types.update_requester_gateway_request.serialize_json(input_)
    ).encode()
...
async def async_update_requester_gateway(
