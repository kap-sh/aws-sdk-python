"""Generated from Smithy shape ``com.amazonaws.rtbfabric#CreateLinkRoutingRule``."""
...
def handle_response(
    ...
    out: capo_rtbfabric.types.create_link_routing_rule_response.CreateLinkRoutingRuleResponse = capo_rtbfabric.types.create_link_routing_rule_response.deserialize_json(
...
def build_request(
    ...
    url = (
        endpoint.url.rstrip("/")
        + "/responder-gateway/{gatewayId}/link/{linkId}/routing-rule"
    )
    ...
    body: bytes | None = json.dumps(
        capo_rtbfabric.types.create_link_routing_rule_request.serialize_json(input_)
    ).encode()
...
async def async_create_link_routing_rule(
