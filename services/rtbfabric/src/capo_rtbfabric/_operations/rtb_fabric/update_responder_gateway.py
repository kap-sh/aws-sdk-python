"""Generated from Smithy shape ``com.amazonaws.rtbfabric#UpdateResponderGateway``."""
...
def handle_response(
    ...
    out: capo_rtbfabric.types.update_responder_gateway_response.UpdateResponderGatewayResponse = capo_rtbfabric.types.update_responder_gateway_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/responder-gateway/{gatewayId}/update"
    url = url.replace("{gatewayId}", quote(str(input_["gateway_id"]), safe=""))
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_rtbfabric.types.update_responder_gateway_request.serialize_json(input_)
    ).encode()
...
async def async_update_responder_gateway(
