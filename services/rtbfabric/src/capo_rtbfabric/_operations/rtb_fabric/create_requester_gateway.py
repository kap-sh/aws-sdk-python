"""Generated from Smithy shape ``com.amazonaws.rtbfabric#CreateRequesterGateway``."""
...
def handle_response(
    ...
    out: capo_rtbfabric.types.create_requester_gateway_response.CreateRequesterGatewayResponse = capo_rtbfabric.types.create_requester_gateway_response.deserialize_json(
...
def build_request(
    ...
    url = endpoint.url.rstrip("/") + "/requester-gateway"
    params: dict[str, str] = {}
    headers: dict[str, str] = {k: ", ".join(v) for k, v in endpoint.headers.items()}
    body: bytes | None = json.dumps(
        capo_rtbfabric.types.create_requester_gateway_request.serialize_json(input_)
    ).encode()
...
async def async_create_requester_gateway(
