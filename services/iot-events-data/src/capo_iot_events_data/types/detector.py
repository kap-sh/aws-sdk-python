...
if TYPE_CHECKING:
    ...
    import capo_iot_events_data.types.detector_model_version
    import capo_iot_events_data.types.detector_state
...
class Detector(TypedDict, closed=True):
    ...
    detector_model_version: NotRequired[
        "capo_iot_events_data.types.detector_model_version.DetectorModelVersion"
    ]
    """<p>The version of the detector model that created this detector (instance).</p>"""
    state: NotRequired["capo_iot_events_data.types.detector_state.DetectorState"]
...
def serialize_json(value: Detector) -> dict:
    ...
    if "state" in value:
        ...
        out["state"] = capo_iot_events_data.types.detector_state.serialize_json(
            value["state"]
        )
...
def deserialize_json(data: dict) -> Detector:
    ...
    if "state" in data:
        ...
        out["state"] = capo_iot_events_data.types.detector_state.deserialize_json(
            data["state"]
        )
