...
class AlarmState(TypedDict, closed=True):
    ...
    rule_evaluation: NotRequired[
        "capo_iot_events_data.types.rule_evaluation.RuleEvaluation"
    ]
    """<p>Information needed to evaluate data.</p>"""
    customer_action: NotRequired[
        "capo_iot_events_data.types.customer_action.CustomerAction"
    ]
    """<p>Contains information about the action that you can take to respond to the alarm.</p>"""
    system_event: NotRequired["capo_iot_events_data.types.system_event.SystemEvent"]
...
def serialize_json(value: AlarmState) -> dict:
    ...
    if "rule_evaluation" in value:
        ...
        out["ruleEvaluation"] = (
            capo_iot_events_data.types.rule_evaluation.serialize_json(
    ...
    if "customer_action" in value:
        ...
        out["customerAction"] = (
            capo_iot_events_data.types.customer_action.serialize_json(
    ...
    if "system_event" in value:
        ...
        out["systemEvent"] = capo_iot_events_data.types.system_event.serialize_json(
            value["system_event"]
        )
    return out
...
def deserialize_json(data: dict) -> AlarmState:
    ...
    if "ruleEvaluation" in data:
        ...
        out["rule_evaluation"] = (
            capo_iot_events_data.types.rule_evaluation.deserialize_json(
                data["ruleEvaluation"]
            )
        )
    if "customerAction" in data:
        ...
        out["customer_action"] = (
            capo_iot_events_data.types.customer_action.deserialize_json(
                data["customerAction"]
            )
        )
