...
class DetectorSummary(TypedDict, closed=True):
    ...
    state: NotRequired[
        "capo_iot_events_data.types.detector_state_summary.DetectorStateSummary"
    ]
...
def serialize_json(value: DetectorSummary) -> dict:
    ...
    if "state" in value:
        ...
        out["state"] = capo_iot_events_data.types.detector_state_summary.serialize_json(
            value["state"]
        )
...
def deserialize_json(data: dict) -> DetectorSummary:
    ...
    if "state" in data:
        ...
        out["state"] = (
            capo_iot_events_data.types.detector_state_summary.deserialize_json(
                data["state"]
            )
        )
