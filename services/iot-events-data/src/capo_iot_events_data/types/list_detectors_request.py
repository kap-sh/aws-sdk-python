"""Generated from Smithy shape ``com.amazonaws.ioteventsdata#ListDetectorsRequest``."""

from typing import TYPE_CHECKING

from typing_extensions import NotRequired, TypedDict

if TYPE_CHECKING:
    import capo_iot_events_data.types.detector_model_name
    import capo_iot_events_data.types.max_results
    import capo_iot_events_data.types.next_token
    import capo_iot_events_data.types.state_name


class ListDetectorsRequest(TypedDict, closed=True):
    detector_model_name: (
        "capo_iot_events_data.types.detector_model_name.DetectorModelName"
    )
    """<p>The name of the detector model whose detectors (instances) are listed.</p>"""
    state_name: NotRequired["capo_iot_events_data.types.state_name.StateName"]
    """<p>A filter that limits results to those detectors (instances) in the given state.</p>"""
    next_token: NotRequired["capo_iot_events_data.types.next_token.NextToken"]
    """<p>The token that you can use to return the next set of results.</p>"""
    max_results: NotRequired["capo_iot_events_data.types.max_results.MaxResults"]
    """<p>The maximum number of results to be returned per request.</p>"""


# --- restJson1 ser/de ---
def serialize_json(value: ListDetectorsRequest) -> dict:
    out: dict = {}
    return out


def deserialize_json(data: dict) -> ListDetectorsRequest:
    out: ListDetectorsRequest = {}  # type: ignore[typeddict-item]
    return out
