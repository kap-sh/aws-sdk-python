"""Generated from Smithy shape ``com.amazonaws.pcs#GetQueue``."""
...
def handle_response(
    ...
    out: capo_pcs.types.get_queue_response.GetQueueResponse = (
        capo_pcs.types.get_queue_response.deserialize_aws_json_1_0(
...
def get_signer(
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or capo_pcs._auth._sigv4.build_sigv4_auth_scheme("pcs", options.region)
        )
...
def build_request(
    ...
    headers["X-Amz-Target"] = "AWSParallelComputingService.GetQueue"
    body: bytes | None = json.dumps(
        capo_pcs.types.get_queue_request.serialize_aws_json_1_0(input_)
    ).encode()
...
def get_queue(
    options: OperationOptions, input_: capo_pcs.types.get_queue_request.GetQueueRequest
) -> tuple[capo_pcs.types.get_queue_response.GetQueueResponse, zapros.Response]:
