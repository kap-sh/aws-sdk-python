...
class ImportFailureListItem(TypedDict, closed=True):
    location: NotRequired["capo_cloudtrail.types.string.String"]
    """<p> The location of the failure in the S3 bucket. </p>"""
    status: NotRequired[
        "capo_cloudtrail.types.import_failure_status.ImportFailureStatus"
    ]
    """<p> The status of the import. </p>"""
    error_type: NotRequired["capo_cloudtrail.types.string.String"]
    """<p> The type of import error. </p>"""
    error_message: NotRequired["capo_cloudtrail.types.string.String"]
    """<p> Provides the reason the import failed. </p>"""
    last_updated_time: NotRequired["capo_cloudtrail.types.date.Date"]
...
def serialize_aws_json_1_1(value: ImportFailureListItem) -> dict:
    ...
    if "status" in value:
        ...
        out["Status"] = (
            capo_cloudtrail.types.import_failure_status.serialize_aws_json_1_1(
                value["status"]
            )
        )
    ...
    if "last_updated_time" in value:
        ...
        out["LastUpdatedTime"] = capo_cloudtrail.types.date.serialize_aws_json_1_1(
...
def deserialize_aws_json_1_1(data: dict) -> ImportFailureListItem:
    ...
    if "Status" in data:
        ...
        out["status"] = (
            capo_cloudtrail.types.import_failure_status.deserialize_aws_json_1_1(
                data["Status"]
            )
        )
