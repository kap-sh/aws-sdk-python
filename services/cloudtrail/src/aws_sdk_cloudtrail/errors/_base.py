...
class CloudTrailError(Exception):
    """Base class for all CloudTrail service errors."""


class ServiceError(CloudTrailError):
...
class SerializationError(CloudTrailError):
    """Raised when a value cannot be serialized."""


class DeserializationError(CloudTrailError):
...
class WaiterFailedError(CloudTrailError):
...
class WaiterTimeoutError(CloudTrailError):
