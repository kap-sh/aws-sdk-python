...
def handle_error(response: zapros.Response) -> Never:
    ...
    match code:
        ...
        case "ResourceNotFoundException":
            import aws_sdk_account.errors.resource_not_found_exception

            raise aws_sdk_account.errors.resource_not_found_exception.ResourceNotFoundException.from_json(
                data
            )
...
def handle_response(
    ...
    out: aws_sdk_account.types.get_contact_information_response.GetContactInformationResponse = aws_sdk_account.types.get_contact_information_response.deserialize_json(
...
def get_signer(
...
) -> aws_sdk_account._auth._signers.Signer | None:
    ...
    if options.credentials_provider is not None:
        sigv4_config = (
            ...
            or aws_sdk_account._auth._sigv4.build_sigv4_auth_scheme(
                "account", options.region
            )
        )
        if sigv4_config is not None:
            return aws_sdk_account._auth._signers.SigV4Signer(
...
def build_request(
    ...
    input_: aws_sdk_account.types.get_contact_information_request.GetContactInformationRequest,
) -> zapros.Request:
    endpoint = resolve(
        EndpointParams(
    ...
    body: bytes | None = json.dumps(
        aws_sdk_account.types.get_contact_information_request.serialize_json(input_)
    ).encode()
