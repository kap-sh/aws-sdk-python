...
if TYPE_CHECKING:
    import aws_sdk_sfn.types.map_iteration_failure_count
    import aws_sdk_sfn.types.retrier_retry_count
    import aws_sdk_sfn.types.sensitive_data
    import aws_sdk_sfn.types.test_state_state_name


class TestStateConfiguration(TypedDict):
    retrier_retry_count: NotRequired[
        "aws_sdk_sfn.types.retrier_retry_count.RetrierRetryCount"
    ]
    ...
    map_iteration_failure_count: NotRequired[
        "aws_sdk_sfn.types.map_iteration_failure_count.MapIterationFailureCount"
    ]
    """<p>The number of Map state iterations that failed during the Map state invocation.</p>"""
    map_item_reader_data: NotRequired["aws_sdk_sfn.types.sensitive_data.SensitiveData"]
